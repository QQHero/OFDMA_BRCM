...
#endif
#ifdef CONFIG_TENDA_PRIVATE_KM
int get_l4proto(const struct sk_buff *skb,
...
void nf_conntrack_free(struct nf_conn *ct)
{
    struct net *net = nf_ct_net(ct);
#ifdef CONFIG_TENDA_PRIVATE_KM
    km_nf_conntrack_destroy_hook(ct);
#endif
          ...
          const struct nf_conntrack_l4proto *l4proto)
{
    ...
    if (!h) {
#ifdef CONFIG_TENDA_PRIVATE_KM
        km_nf_conntrack_create_prerouting_hook(skb);
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_KM
    km_nf_conntrack_create_postrouting_hook(skb,ct);
#endif
