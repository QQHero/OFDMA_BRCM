             ...
             int offs, int quiet, int must_chk_crc)
{
    ...
    if (!must_chk_crc && type == UBIFS_DATA_NODE && !c->mounting &&
        ...
        return 0;
#if defined(CONFIG_VFS_LOCK_SUPPORT)
    crc = tenda_crc32_ubifs(UBIFS_CRC32_INIT, buf + 8, node_len - 8);
#else
...
#if defined(CONFIG_VFS_LOCK_SUPPORT)
        crc = tenda_crc32_ubifs(UBIFS_CRC32_INIT, buf + 8, UBIFS_PAD_NODE_SZ - 8);
#else
...
void ubifs_prepare_node(struct ubifs_info *c, void *node, int len, int pad)
{
    ...
    ch->padding[0] = ch->padding[1] = 0;
#if defined(CONFIG_VFS_LOCK_SUPPORT)
    crc = tenda_crc32_ubifs(UBIFS_CRC32_INIT, node + 8, len - 8);
#else
