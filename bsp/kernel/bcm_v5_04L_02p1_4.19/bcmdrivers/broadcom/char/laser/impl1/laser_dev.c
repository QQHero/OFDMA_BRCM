...
static u16 Laser_Dev_Ioctl_Get_Voltage(void)
{
    ...
    if (Laser_Dev_Is_XFP())
    {
        ...
        Voltage = -EOPNOTSUPP; /* optional, not implemented */
    }
    else
    {
    ...
    unsigned long arg)
{
    ...
    switch (cmd) 
    {
        ...
        {           
            laserPwr = Laser_Dev_Ioctl_Get_Rx_Optical_Pwr();
            break;
        }
        case LASER_IOCTL_GET_TX_PWR:
        {
            laserPwr = Laser_Dev_Ioctl_Get_Tx_Optical_Pwr();
            break;
        }
        case LASER_IOCTL_GET_TEMPTURE:
        {
            laserPwr = Laser_Dev_Ioctl_Get_Temp();
            break;
        }
        case LASER_IOCTL_GET_VOLTAGE:
        {
            if (Laser_Dev_Ioctl_Get_Voltage)
                laserPwr = Laser_Dev_Ioctl_Get_Voltage();
            break;
        }
        case LASER_IOCTL_GET_BIAS_CURRENT:
        {
            if (Laser_Dev_Ioctl_Get_Bias_Current)
                laserPwr = Laser_Dev_Ioctl_Get_Bias_Current();
            break;
        }
    ...
    if (!Ret) 
    {
        if (copy_to_user((void *)arg, (void*)&laserPwr, sizeof(uint32_t)))
        {
