...
{
    int rxq_index = is_netdev_wan(dev_p) ?
    ...
    mlt_data.data1 = 0;
    mlt_data.valid = 1;
    mlt_data.mac_da_type = SYSPORT_MLT_MAC_ADDR_TYPE_HOST;
...
static int archer_host_netdev_event(struct notifier_block *this, unsigned long event, void *ptr)
{
    ...
    switch (event) {
        ...
        case NETDEV_CHANGE:
            ...
            {
                __logInfo("Link Change UP 0x%px\n", dev_p);
                ret = archer_host_netdev_event_add_host_mac(dev_p);
            }
            else
            {
                __logInfo("Link Change DN 0x%px\n", dev_p);
                ret = archer_host_netdev_event_delete_host_mac(dev_p);
            }
            break;

        case NETDEV_UNREGISTER:
            __logInfo("Unregister 0x%px\n", dev_p);
            ret = archer_host_netdev_event_delete_host_mac(dev_p);
            break;
    }
