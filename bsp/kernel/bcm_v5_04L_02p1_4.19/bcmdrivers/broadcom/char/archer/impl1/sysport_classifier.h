...
#define CC_CLASSIFIER_CONTEXT_ETH_WAN
#endif
...
#define SYSPORT_UCAST_FLOW_HASH_INDEX_MAX (1 << SYSPORT_UCAST_FLOW_HASH_WIDTH)
#define SYSPORT_UCAST_FLOW_BUCKET_SIZE    4
#define SYSPORT_UCAST_FLOW_TABLE_SIZE     (SYSPORT_UCAST_FLOW_HASH_INDEX_MAX * SYSPORT_UCAST_FLOW_BUCKET_SIZE)
...
#define SYSPORT_MCAST_FLOW_HASH_INDEX_MAX (1 << SYSPORT_MCAST_FLOW_HASH_WIDTH)
#define SYSPORT_MCAST_FLOW_BUCKET_SIZE    16 // larger than, or equal to SYSPORT_UCAST_FLOW_BUCKET_SIZE
#define SYSPORT_MCAST_FLOW_TABLE_SIZE     (SYSPORT_MCAST_FLOW_HASH_INDEX_MAX * SYSPORT_MCAST_FLOW_BUCKET_SIZE)

#define SYSPORT_FLOW_TABLE_SIZE           SYSPORT_UCAST_FLOW_TABLE_SIZE
#define SYSPORT_FLOW_MAX                  ((1 << 14) + (1 << 10) + (1 << 7))
...
#define SYSPORT_FLOW_WLAN_PORTS_MAX      3
#endif
#define SYSPORT_FLOW_EGRESS_QUEUES_MAX   16
#define SYSPORT_FLOW_CMDLIST_LENGTH_16_MAX  7
#define SYSPORT_FLOW_WLAN_SSID_MAX       16
...
#define SYSPORT_FLOW_MCAST_BITMAP_SIZE_32  2
#define SYSPORT_FLOW_MCAST_BITMAP_SIZE     (32 * SYSPORT_FLOW_MCAST_BITMAP_SIZE_32)
#define SYSPORT_FLOW_MCAST_CLIENTS_MAX     (SYSPORT_FLOW_MCAST_BITMAP_SIZE - 1)

#define SYSPORT_FLOW_IS_MCAST_MASTER(_flow_p)                           \
    ( (SYSPORT_RSB_FLOW_TYPE_MCAST == (_flow_p)->tuple.header.flow_type) && \
      (BLOG_MCAST_MASTER_CLIENT_ID == (_flow_p)->tuple.mcast.client_id) )
...
typedef union {
    struct {
        ...
        uint32_t table_index  : 16; // MAX(SYSPORT_UCAST_FLOW_HASH_INDEX_MAX, SYSPORT_MCAST_FLOW_HASH_INDEX_MAX)
#else
        uint32_t table_index  : 16; // MAX(SYSPORT_UCAST_FLOW_HASH_INDEX_MAX, SYSPORT_MCAST_FLOW_HASH_INDEX_MAX)
        uint32_t bucket_index : 4;  // MAX(SYSPORT_UCAST_FLOW_BUCKET_SIZE, SYSPORT_MCAST_FLOW_BUCKET_SIZE)
        uint32_t flow_type    : 2;  // sysport_rsb_flow_type_t
...
typedef struct {
    ...
    union {
        struct {
            ...
            uint32_t mcast_fwd_trap : 1;  // MCAST Forward and Trap packet
            ...
            uint32_t egress_phy_dhd : 1;
            uint32_t egress_phy     : 2;  // sysport_rsb_phy_t
            uint32_t egress_port    : 4;  // SYSPORT_FLOW_PORTS_MAX
        };
        uint32_t u32_0;
    };
    ...
    union {
        struct {
#if defined(CC_CLASSIFIER_CONTEXT_ETH_WAN)
    ...
    union {
        struct {
            ...
            union {
                ...
                int padding_len; // LAN Tx Only
#endif
                sysport_classifier_wlan_t wlan; // WLAN Tx Only
            };
        } ucast;

        struct {
            uint32_t bitmap[SYSPORT_FLOW_MCAST_BITMAP_SIZE_32];
        } mcast;
    };
...
static inline int sysport_classifier_bitmap_to_bit(int bitmap)
{
    int bit = ffs(bitmap);
...
static inline int sysport_classifier_mcast_client_id(int bitmap, int index)
{
    return (32 * index) + sysport_classifier_bitmap_to_bit(bitmap);
}
...
int sysport_classifier_flow_bitmap_set(sysport_flow_key_t flow_key, uint32_t *bitmap_p);

int sysport_classifier_flow_fwd_trap_set(sysport_flow_key_t flow_key, int fwd_trap);
