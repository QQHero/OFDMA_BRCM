...
void __sysport_rsb_tuple_dump(sysport_rsb_flow_tuple_t *tuple_p,
                              sysport_rsb_flow_tuple_info_t *tuple_info_p,
                              const char *ingress_phy_name_override)
{
    sysport_rsb_flow_header_t *header_p = &tuple_p->header;
    const char *ingress_phy_name;

    if(ingress_phy_name_override)
    {
        ingress_phy_name = ingress_phy_name_override;
    }
    else
    {
        ingress_phy_name = sysport_rsb_phy_name[header_p->ingress_phy];
    }
    ...
    bcm_print("valid %d, flow_type %s (%d), ingress_phy %s (%d), ingress_port %d\n",
              ...
              ingress_phy_name, header_p->ingress_phy, header_p->ingress_port);
    ...
    switch(header_p->flow_type)
    {
        ...
        case SYSPORT_RSB_FLOW_TYPE_MCAST:
        {
            ...
            if(tuple_info_p)
            {
                if(tuple_info_p->l3.is_ipv6)
                {
                    bcm_print("client_id %d, ip_protocol %d, ip_tos 0x%02x\n"
                              "IPv6 src_addr " BLOG_IPV6_ADDR_FMT " (0x%08x)\n"
                              "IPv6 dst_addr " BLOG_IPV6_ADDR_FMT " (0x%08x)\n"
                              "nbr_of_vlans %d, outer_vlan_id %d, inner_vlan_id %d\n",
                              mcast_p->client_id, mcast_p->ip_protocol, mcast_p->ip_tos,
                              ...
                              mcast_p->outer_vlan_id, mcast_p->inner_vlan_id);
                }
                else
                {
                    bcm_print("client_id %d, ip_protocol %d, ip_tos 0x%02x\n"
...
static char *__rsb_l4_type_name(sysport_rsb_t *rsb_p)
{
    switch(rsb_p->l4_type)
    {
        case L4_TYPE_UNKNOWN:
            return "UNKNOWN";
...
static char *__rsb_l4_type_name(sysport_rsb_t *rsb_p)
{
    if(rsb_p->tcp == 0 && rsb_p->udp == 0)
    {
        return "UNKNOWN";
    }
    else if(rsb_p->tcp == 0 && rsb_p->udp == 1)
    {
        return "UDP";
    }
    else if(rsb_p->tcp == 1 && rsb_p->udp == 0)
    {
        return "TCP";
    }
...
void sysport_rsb_dump(sysport_rsb_t *rsb_p, sysport_rsb_flow_tuple_info_t *tuple_info_p, int mlt_enable)
{
    ...
    bcm_print("nbr_of_vlans %d, pppoe %d, llc_snap %d, l3_type %s (%d)\n"
              "l4_type %s, ip_version_mismatch %d, ipv4_options %d, ttl_expired %d\n"
              ...
              rsb_p->l3_type, __rsb_l4_type_name(rsb_p), rsb_p->ip_version_mismatch,
