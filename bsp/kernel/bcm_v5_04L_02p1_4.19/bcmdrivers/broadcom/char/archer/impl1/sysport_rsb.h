...
#define SYSPORT_RSB_PHY_IS_WAN(_phy)                                    \
    ( SYSPORT_RSB_PHY_ETH_WAN == (_phy) ||                              \
      SYSPORT_RSB_PHY_DSL == (_phy) )
#else
#define SYSPORT_RSB_PHY_IS_WAN(_phy)                                    \
    ( SYSPORT_RSB_PHY_ETH_WAN == (_phy) )
#endif
...
#if defined(CC_ARCHER_DSL)
#define SYSPORT_RSB_PHY_IS_ETH(_phy)                                    \
    ( SYSPORT_RSB_PHY_ETH_LAN == (_phy) ||                              \
      SYSPORT_RSB_PHY_ETH_WAN == (_phy) )
#else
#define SYSPORT_RSB_PHY_IS_ETH(_phy)                                    \
    ( SYSPORT_RSB_PHY_ETH_LAN == (_phy) ||                              \
      SYSPORT_RSB_PHY_ETH_WAN == (_phy) ||                              \
      SYSPORT_RSB_PHY_ETH_LAN_STACKED == (_phy) )
#endif

#if defined(CC_ARCHER_DSL)
#define SYSPORT_RSB_PHY_IS_ETH_LAN(_phy)                                \
    ( SYSPORT_RSB_PHY_ETH_LAN == (_phy) )
#else
#define SYSPORT_RSB_PHY_IS_ETH_LAN(_phy)                                \
    ( SYSPORT_RSB_PHY_ETH_LAN == (_phy) ||                              \
...
typedef enum {
    ...
    SYSPORT_RSB_PHY_DSL,
#else
    SYSPORT_RSB_PHY_ETH_LAN_STACKED,
#endif
    SYSPORT_RSB_PHY_WLAN,
    SYSPORT_RSB_PHY_MAX
} sysport_rsb_phy_t;
...
typedef struct {
    ...
    union {
        struct {
            ...
            uint32_t ip_tos        : 8;  // Not supported in HW yet
        };
        uint32_t misc;
    };
...
typedef enum {
    ...
    L4_TYPE_UDP,
    L4_TYPE_TCP,
    L4_TYPE_TCP_PURE_ACK,
    L4_TYPE_MAX
} sysport_rsb_l4_type_t;
#endif
...
#if defined(CC_SYSPORT_PARSER_V2)
            uint16_t l4_type             : 2; // Defined in sysport_rsb_l4_type_t
#else
            uint16_t udp                 : 1;
            uint16_t tcp                 : 1;
#endif
...
static inline void sysport_rsb_tuple_dump(sysport_rsb_flow_tuple_t *tuple_p,
                                          sysport_rsb_flow_tuple_info_t *tuple_info_p)
{
    __sysport_rsb_tuple_dump(tuple_p, tuple_info_p, NULL);
}
