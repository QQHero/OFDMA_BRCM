...
#define CC_ARCHER_AFFINITY_CHECK
...
const int archer_cpu_affinity_g = CONFIG_BCM_ARCHER_CPU_AFFINITY;

const int archer_packet_length_max_g = BCM_MAX_PKT_LEN;

const int archer_packet_headroom_g = BCM_PKT_HEADROOM;

const int archer_skb_tailroom_g = BCM_SKB_TAILROOM;
...
int archer_activate(Blog_t *blog_orig_p, uint32_t key_in)
{
    ...
    else
    {
        ...
        {
            ret = archer_mcast_activate(blog_p, &flow_key);
            if(ret)
            {
                             ...
                             uint32_t *octets_p, struct blog_t *blog_p)
{
    ...
    else
    {
        if(blog_p->rx.multicast)
        {
            ret = archer_mcast_deactivate(flow_key);
            if(ret < 0)
            {
                goto abort_deactivate;
            }
...
static int archer_update_bitmap(sysport_flow_key_t flow_key, Blog_t *blog_p)
{
    uint32_t client_bitmap_p[SYSPORT_FLOW_MCAST_BITMAP_SIZE_32];
    int ret = blog_copy_mcast_client_bitmap(blog_p->mcast_bitmap_idx, 
                                            client_bitmap_p, sizeof(client_bitmap_p)/4);

    if(!ret)
    {
        ret = sysport_classifier_flow_bitmap_set(flow_key, client_bitmap_p);
    }
...
static int archer_update(BlogUpdate_t update, uint32_t fhw_tuple, Blog_t *blog_p)
{
    ...
    switch(update)
    {
        ...
        case BLOG_UPDATE_BITMAP:
            ret = archer_update_bitmap(flow_key, blog_p);
            break;

        case BLOG_UPDATE_FWD_AND_TRAP:
            ret = sysport_classifier_flow_fwd_trap_set(flow_key, blog_p->fwd_and_trap);
            break;
...
#endif
        hwHooks.max_ent = SYSPORT_FLOW_MAX;
...
#else
        arch_setup_dma_ops(archer_driver_g.dummy_dev, 0, 0, NULL, false);
#endif
        /* according to spec, we should be able to do 40-bits */
        dma_coerce_mask_and_coherent(archer_driver_g.dummy_dev, DMA_BIT_MASK(32));
    }
...
static int archer_driver_host_bind(void *arg_p)
{
    ...
    switch(hooks_p->host_type)
    {
        case ARCHER_HOST_TYPE_ENET:
            ...
            hooks_p->archer_task_p = archer_thread_us_g.rx_thread;
            break;

        case ARCHER_HOST_TYPE_XTMRT:
#if (defined(CC_ARCHER_DSL) && (defined(CONFIG_BCM_XTMRT) || defined(CONFIG_BCM_XTMRT_MODULE)))
...
int archer_driver_chip_id_get(void)
{
    return UtilGetChipId(); // eg: 0x63178
}

int archer_driver_chip_revision_get(void)
{
    return UtilGetChipRev(); // eg: 0xA0
}
...
#if defined(CC_ARCHER_AFFINITY_CHECK)
static inline void archer_affinity_check(void)
{
    int cpu_id = smp_processor_id();

    if(unlikely(cpu_id != CONFIG_BCM_ARCHER_CPU_AFFINITY))
    {
        printk("\n\n******** Archer Thread migration: cpu_id %d != %d ********\n\n",
               cpu_id, CONFIG_BCM_ARCHER_CPU_AFFINITY);
        BUG();
    }
}
#else
#define archer_affinity_check()
#endif
...
static void archer_ioctl_flow_dump(archer_ioctl_cmd_t cmd, unsigned long arg)
{
    ...
    if(!dump.max)
    {
        dump.max = SYSPORT_FLOW_TABLE_SIZE;
    }
    ...
    switch(cmd)
    {
        case ARCHER_IOC_STATUS:
            bcm_print("ARCHER Status:\n"
                      ...
                      archer_driver_g.active, SYSPORT_FLOW_MAX,
#if defined(CONFIG_BLOG_IPV6)
                      "Enabled",
#else
                      "Disabled",
        ...
        case ARCHER_IOC_ENETDROPALG_SET:
        {
            ...
            if(!ret)
            {
                __debug("Drop Profile Set: %s, logical_port %d, queue_id %d\n",
        ...
        case ARCHER_IOC_ENETDROPALG_GET:
        {
            ...
            if(dev)
            {
                ...
                if(!ret)
                {
                    __debug("Drop Profile Get: %s, logical_port %d, queue_id %d\n",
...
int __init archer_construct(void)
{
    ...
    ret = archer_dhd_construct();
    if(ret)
    {
        __logError("Could not archer_dhd_construct");
