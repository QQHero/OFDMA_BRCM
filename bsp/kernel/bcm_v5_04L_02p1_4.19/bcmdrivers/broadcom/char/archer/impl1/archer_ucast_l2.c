...
#if (SYSPORT_PARSER_CONFIG_UCAST_L2_TOS_UNKNOWN != SYSPORT_CLASSIFIER_UCAST_L2_IP_TOS_UNKNOWN)
#error "SYSPORT_PARSER_CONFIG_UCAST_L2_TOS_UNKNOWN != SYSPORT_CLASSIFIER_UCAST_L2_IP_TOS_UNKNOWN"
#endif
#endif

static int archer_ucast_l2_vlan_tag_crc32(int nbr_of_vlans, BlogVlanHdr_t *vlan_header_p,
                                          uint32_t *vlan_tag_crc32_p)
{
    if(nbr_of_vlans > SYSPORT_RSB_VLAN_TAGS_MAX)
    {
        __logError("Too many VLAN Tags");
    ...
    if(nbr_of_vlans)
    {
        BlogVlanHdr_t vlan_buffer[SYSPORT_RSB_VLAN_TAGS_MAX];
        BlogVlanHdr_t vlan_mask;
        int nbr_of_vlans_crc32 = 0;
        int i;

        vlan_mask.tpid = 0xFFFF;
        vlan_mask.tci.pbits = 0x7 * SYSPORT_PARSER_CONFIG_PCP_ENABLE;
        vlan_mask.tci.dei = 0x1 * SYSPORT_PARSER_CONFIG_DEI_ENABLE;
        vlan_mask.tci.vid = 0xFFF;

        vlan_mask.u16[0] = ntohs(vlan_mask.u16[0]);
        vlan_mask.u16[1] = ntohs(vlan_mask.u16[1]);

        for(i=0; i<nbr_of_vlans; ++i)
        {
            if(i < SYSPORT_PARSER_CONFIG_UCAST_L2_VLAN_MAX)
            {
                BlogVlanHdr_t vlan_header = { .u32[0] = vlan_header_p[i].u32[0] };

                vlan_buffer[i].u32[0] = vlan_mask.u32[0] & vlan_header.u32[0];

                __debug("vlan_mask 0x%08X & vlan_header 0x%08X = vlan_buffer 0x%08X\n",
                        vlan_mask.u32[0], vlan_header.u32[0], vlan_buffer[i].u32[0]);

                nbr_of_vlans_crc32++;
            }
            else if(SYSPORT_PARSER_CONFIG_UCAST_L2_VLAN_CALC_MODE)
            {
                vlan_buffer[i].u32[0] = 0;

                nbr_of_vlans_crc32++;
            }
        }

        *vlan_tag_crc32_p = sysport_parser_crc32(vlan_buffer, (nbr_of_vlans_crc32 * sizeof(BlogVlanHdr_t)));
    }
    else
    {
        *vlan_tag_crc32_p = 0;
                                    ...
                                    sysport_classifier_rsb_overwrite_t *rsb_overwrite_p)
{
    ...
    {
        ucast_l2_p->ip_tos = blog_p->rx.tuple.tos & SYSPORT_PARSER_CONFIG_UCAST_L2_TOS_MASK;
    }
...
#if !defined(CC_SYSPORT_PARSER_V2)
        rsb_overwrite_p->parser.ucast_l2.ip_tos = SYSPORT_PARSER_CONFIG_UCAST_L2_TOS_UNKNOWN;
#endif
    }
    else
    {
    ...
    ret = archer_ucast_l2_vlan_tag_crc32(blog_p->vtag_num, (BlogVlanHdr_t *)vlan_p,
