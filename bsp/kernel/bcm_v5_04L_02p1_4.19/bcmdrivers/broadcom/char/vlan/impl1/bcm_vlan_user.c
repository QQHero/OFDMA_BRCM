...
static int bcm_attach_vlan_hook(struct net_device *dev) { return 0; }
static void bcm_detach_vlan_hook(struct net_device *dev) {}
#endif 
...
static int unregisterVlanDevice(const char *vlanDevName)
{
    ...
    BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "Found %s (%px)", vlanDev->name, (void *)vlanDev);
    ...
    BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "%px -> %px -> %px -> %px -> %px",
                  (void *)vlanDev,
...
static int bcmVlan_ethtoolGetKSettings(struct net_device *vlanDev,
                                      struct ethtool_link_ksettings *cmd)
{
    struct net_device *realDev = BCM_VLAN_REAL_DEV(vlanDev);

    if (!realDev->ethtool_ops ||
        !realDev->ethtool_ops->get_link_ksettings)
        return -EOPNOTSUPP;

    return realDev->ethtool_ops->get_link_ksettings(realDev, cmd);
}
...
static const struct ethtool_ops bcmVlan_ethtool_ops = {
    .get_link_ksettings =   bcmVlan_ethtoolGetKSettings,
...
static void vlanSetup(struct net_device *vlanDev)
{
    ...
    netdev_vlan_set(vlanDev);
    vlanDev->tx_queue_len = 0;

    vlanDev->netdev_ops  = &bcmVlan_netdev_ops;

#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,0))
    vlanDev->needs_free_netdev = true;
    vlanDev->priv_destructor = NULL;
#else
...
#if defined(CONFIG_BLOG)
    bcm_netdev_ext_field_set(vlanDev, blog_stats_flags, BLOG_DEV_STAT_FLAG_INCLUDE_ALL);
...
static int allocMacAddress(struct net_device *vlanDev)
{
    ...
    {
        bcm_print("Unable to get MAC address from persistent storage\n");
                                                   ...
                                                   int isSwOnly)
{
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,0))
    bcm_netdev_ext_inherit(realDev, newDev);
#endif
    ...
    netdev_path_set_hw_port_type(newDev, netdev_path_get_hw_port_type(realDev));
...
/*                   realDev->refcnt.counter, vlanDev->refcnt.counter); */
    ...
    ret = bcm_attach_vlan_hook(realDev);
    if (ret)
    {
        BCM_LOG_ERROR(BCM_LOG_ID_VLAN, "Failed to attach vlan hook to %s (%d)",
                      realDev->name, ret);
...
out_detach_vlan_hook:
    bcm_detach_vlan_hook(newDev);

out_unregister_free:
    unregister_netdevice(newDev);
    newDev = NULL; /* unregister_netdevice will trigger free_netdev by itself. Avoid calling free_netdev. */
...
static int dumpLocalStats(char *realDevName)
{
    ...
    if(realDev != NULL)
    {
        ...
        if(realDevCtrl != NULL)
        {
            ...
            bcm_print("*** %s Local Stats ***\n", realDev->name);

            bcm_print("rx_Misses             : %d\n", localStats->rx_Misses);
            bcm_print("tx_Misses             : %d\n", localStats->tx_Misses);
            bcm_print("error_PopUntagged     : %d\n", localStats->error_PopUntagged);
            bcm_print("error_PopNoMem        : %d\n", localStats->error_PopNoMem);
            bcm_print("error_PushTooManyTags : %d\n", localStats->error_PushTooManyTags);
            bcm_print("error_PushNoMem       : %d\n", localStats->error_PushNoMem);
            bcm_print("error_SetEtherType    : %d\n", localStats->error_SetEtherType);
            bcm_print("error_SetTagEtherType : %d\n", localStats->error_SetTagEtherType);
            bcm_print("error_InvalidTagNbr   : %d\n", localStats->error_InvalidTagNbr);
            bcm_print("error_UnknownL3Header : %d\n", localStats->error_UnknownL3Header);
        }
        else
        {
