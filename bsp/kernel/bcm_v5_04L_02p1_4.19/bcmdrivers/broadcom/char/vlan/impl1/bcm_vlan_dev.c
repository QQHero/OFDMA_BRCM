...
int bcmVlan_devInit(struct net_device *vlanDev)
{
    ...
    vlanDev->hard_header_len = (realDev->hard_header_len + (is_netdev_vlan(realDev)? 0 : addl_l2_hdr_len));
...
out:
    ...
    bcm_print("%s MAC address set to %02X:%02X:%02X:%02X:%02X:%02X\n",
           vlanDev->name,
                          ...
                          const void *saddr, unsigned len)
{
    ...
    BCM_ASSERT(vlanDev);
    BCM_ASSERT(daddr);
    BCM_ASSERT(is_netdev_vlan(vlanDev));
#endif
...
//    BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "*** RX *** Receiving skb from %s", realDev->name);
    ...
    if (stats == NULL)
    {
        bcm_print("bcmVlan_devGetStats is NULL, go out.\n");
        kfree_skb(skb);
        ret = -EFAULT;
        goto out;  
    }
...
#ifdef CONFIG_TENDA_PRIVATE_KM
    km_vlan_do_receive_hook(skb, skb->dev);
#endif
