...
static void parseFrameHeader(bcmVlan_cmdInfo_t *cmdInfo)
{
    ...
    else if(vlanHeader->etherType == BCM_VLAN_ETHERTYPE_PPPOE_SESSION)
    {
            ...
            pppoeSessionHeader->pppHeader == BCM_VLAN_PPP_PROTOCOL_IPv6))
        {
            ...
            BCM_VLAN_DP_DEBUG("PPPoE Session Header @ 0x%px", pppoeSessionHeader);
        }
        else
        {
            BCM_VLAN_DP_ERROR("Invalid PPPoE Session Header @ 0x%px", pppoeSessionHeader);
    ...
    BCM_VLAN_DP_DEBUG("ETH:0x%p, Tags=%d: [0]0x%p, [1]0x%p, [2]0x%p, [3]0x%p, IP:0x%px",
...
static inline void adjustSkb(bcmVlan_cmdInfo_t *cmdInfo, struct sk_buff *oldSkb, struct sk_buff *newSkb)
{
    if(newSkb && (newSkb != oldSkb))
    {
        ...
        BCM_VLAN_DP_DEBUG("New skb 0x%px : data 0x%p, mac_header 0x%08X, network_header 0x%08X\n",
                          (void *)newSkb,
                          (void *)newSkb->data,
...
static struct sk_buff *insertOuterTag(bcmVlan_cmdInfo_t *cmdInfo, bcmVlan_cmdArg_t *arg, struct sk_buff *skb, int is_tx)
{
    bcmVlan_ethHeader_t *ethHeader;
...
//    skb->protocol = __constant_htons(ETH_P_8021Q);
    ...
    skb->mac_header -= BCM_VLAN_HEADER_LEN;
    if (!is_tx) /* receive path, update the network header */
        skb->network_header -= BCM_VLAN_HEADER_LEN;
...
static inline struct sk_buff *insertOuterTagOnTransmit(bcmVlan_cmdInfo_t *cmdInfo, bcmVlan_cmdArg_t *arg, struct sk_buff *skb)
{
    return insertOuterTag(cmdInfo, arg, skb, 1);
}
...
static struct sk_buff *insertOuterTagOnReceive(bcmVlan_cmdInfo_t *cmdInfo, bcmVlan_cmdArg_t *arg, struct sk_buff *skb)
{
    ...
    skb = insertOuterTag(cmdInfo, arg, skb, 0);
    if(skb)
    {
...
static int cmdHandler_popVlanTag(bcmVlan_cmdInfo_t *cmdInfo, bcmVlan_cmdArg_t *arg)
{
    ...
    if(!ret)
    {
        ...
        BCM_VLAN_DP_DEBUG("ETH:0x%p, Tags=%d: 0x%p, 0x%p, 0x%p, 0x%p, IP:0x%px",
                          (void *)cmdInfo->ethHeader,
                          cmdInfo->nbrOfTags,
                          (void *)cmdInfo->vlanHeader[0],
                          (void *)cmdInfo->vlanHeader[1],
...
static int cmdHandler_copyPbits(bcmVlan_cmdInfo_t *cmdInfo, bcmVlan_cmdArg_t *arg)
{
    ...
    {
        BCM_VLAN_DP_ERROR("Invalid Source/Target Vlan Tags (s:0x%p, t:0x%px)",
                           ...
                           bcmVlan_tableEntry_t *tableEntry)
{
    ...
    else
    {
        ...
        {
            bcm_print("\tVlanDev MacAddr : Yes (ignore if multicast)\n");
        }
        else if (filter_p->vlanDevMacAddr == BCM_VLAN_MATCH_VLANDEV_MACADDR)
        {
            bcm_print("\tVlanDev MacAddr : Yes\n");
        }
        else
        {
            bcm_print("\tVlanDev MacAddr : No\n");
        }
    }
    if(filter_p->flags)
    {
    ...
    for(i=0; i<BCM_VLAN_MAX_TAGS; ++i)
    {
        ...
        {
            bcm_print("\tVLAN Tag %d      : ", i);
            bcm_print("pbits %s, ", BCM_VLAN_PRINT_VAL(filter_p->vlanTag[i].pbits, 0));
            bcm_print("cfi %s, ", BCM_VLAN_PRINT_VAL(filter_p->vlanTag[i].cfi, 0));
            bcm_print("vid %s, ", BCM_VLAN_PRINT_VAL(filter_p->vlanTag[i].vid, 0));
            bcm_print("(tci 0x%04X/0x%04X), ", filter_p->vlanTag[i].tciMask,
    ...
    for(i=0; i<BCM_VLAN_MAX_RULE_COMMANDS; ++i)
    {
        ...
        bcm_print("\n\t");
        bcm_print("%02d:[%s", i, cmdHandler[tableEntry->tagRule.cmd[i].opCode].name);
        for(j=0; j<BCM_VLAN_MAX_CMD_ARGS; ++j)
        {
            bcm_print(", 0x%08X", (int)tableEntry->tagRule.cmd[i].arg[j]);
        }
        bcm_print("] ");
    }
    bcm_print("\n\n");
...
int bcmVlan_blogVlanFlows(void *arg_p,
...
int bcmVlan_dumpTpidTable(char *realDevName)
{
    ...
    bcm_print("%s: TPID Table : ", realDev->name);

    for(i=0; i<BCM_VLAN_MAX_TPID_VALUES; ++i)
    {
        bcm_print("0x%04X ", realDevCtrl->tpidTable[i]);
    }
...
/*
 * Entry points: free_vlan_device_unlocked in bcm_vlan_local.c
 *
 * Note that this function is being called from inside vlan critical section.
 * Therefore it assumes that BCM_VLAN_GLOBAL_LOCK is being held by the caller.
 */
...
int bcmVlan_setRealDevMode(char *realDevName, bcmVlan_realDevMode_t mode)
{
    ...
    bcm_print("BCMVLAN : %s mode was set to %s\n", realDevCtrl->realDev->name,
...
int bcmVlan_dumpDscpToPbitsTable(char *realDevName, UINT8 dscp)
{
    ...
    bcm_print("%s: DSCP to PBITS Table\n", realDev->name);
...
{
    if(dev != realDev && is_netdev_vlan(dev))
    {
                             ...
                             struct sk_buff *skb, int broadcast)
{
    ...
    while(vlanDevCtrl)
    {
        if(vlanDevCtrl->flags.multicast)
        {
                ...
                if(skb2 != NULL)
                {
                   skbuff_bcm_ext_vlan_get(skb2, bcm_flags).restore_rx_vlan = 1;
#else
                {
#endif
                    netif_rx(skb2);
                        ...
                        int rxMulticast)
{
    ...
    if(tableEntry->txVlanDev == NULL || tableEntry->txVlanDev == vlanDev)
    {
        ...
        if(tagRuleFilter->rxRealDevName[0] != '\0' &&
           ((strcmp(tagRuleFilter->rxRealDevName,"lo") && tableEntry->rxRealDev != skbuff_bcm_ext_vlan_get(cmdInfo->skb, rxdev)) ||
            (!strcmp(tagRuleFilter->rxRealDevName,"lo") && skbuff_bcm_ext_vlan_get(cmdInfo->skb, rxdev))))
        {
            /* the rx device of this packet does not match the rx real device
             * specified for this tx tag rule.
             */
            BCM_VLAN_MATCH_DEBUG("Pkt Rx device does not match Rx real device. skb->rxdev 0x%px tagRuleFilter->rxRealDevName %s\n", 
            skbuff_bcm_ext_vlan_get(cmdInfo->skb, rxdev), tagRuleFilter->rxRealDevName);
            return 0; /* no match */
        }
                                ...
                                cmdInfo->dscpToPbits[BCM_VLAN_GET_IP_DSCP(cmdInfo->ipHeader)]))
                            {
                                   ...
                                   tagRuleFilter->ipProto == BCM_VLAN_GET_IPV6_PROTO((bcmVlan_ipv6Header_t *)cmdInfo->ipHeader)))
                                {
                                    ...
                                    BCM_VLAN_MATCH_DEBUG("MATCH: IPPROTO <%d>, VER %d",
                                      tagRuleFilter->ipProto, cmdInfo->ipHeader ? BCM_VLAN_GET_IP_VERSION(cmdInfo->ipHeader) : -1);
                                            ...
                                            unsigned int nbrOfTags, UINT16 protocol)
{
    ...
    skbuff_bcm_ext_vlan_get(skb, vlan_count) = nbrOfTags;
    ...
    skbuff_bcm_ext_vlan_get(skb, vlan_tpid) = ntohs(ethHeader->etherType);
    for(i=0; i<nbrOfTags; ++i)
    {
        skbuff_bcm_ext_vlan_get(skb, vlan_header)[i] = ntohl(*((UINT32*)vlanHeader));
        BCM_VLAN_DP_DEBUG("%d: skb->VlanHeader[%d]: 0x%x\n\r", __LINE__,i,skbuff_bcm_ext_vlan_get(skb, vlan_header)[i]);
        vlanHeader++;
    }
...
static struct sk_buff *restoreSkbVlanOnTransmit(bcmVlan_cmdInfo_t *cmdInfo, struct sk_buff *skb)
{
    ...
    int vlanHeaderLen;
    unsigned int nbrOfTags = skbuff_bcm_ext_vlan_get(skb, vlan_count);
    int i;
    ...
    /* set ethertype */
    ethHeader->etherType = htons(skbuff_bcm_ext_vlan_get(skb, vlan_tpid));
    ...
    for(i=0; i<nbrOfTags; ++i)
    {
        *skbVlanHeader = htonl(skbuff_bcm_ext_vlan_get(skb, vlan_header)[i]);
...
static int modifyRxMulticastRG(bcmVlan_cmdInfo_t *cmdInfo,
                               bcmVlan_tableEntry_t *tableEntry)
{
    struct net_device *rxdev = skbuff_bcm_ext_vlan_get(cmdInfo->skb, rxdev);
...
//                }
         ...
         */
        skbuff_bcm_ext_vlan_get(cmdInfo->skb, rxdev) = rxdev;
    }
        ...
        if(tagRuleMatch(&cmdInfoSave, tableEntry, vlanDev, 1))
        {
            ...
            else
            {
                ...
                *rxVlanDevInStackp = 1;
                ...
                skbuff_bcm_ext_vlan_get(cmdInfo->skb, rxdev) = skbuff_bcm_ext_vlan_get(cmdInfoSave.skb,
                                                                                       rxdev);
...
//        skb->protocol = skb->vlan_proto;
    ...
    while (!ret && skb && vlan_count)
    {
         ...
         */
        skb_push(skb, offset);
        skb = vlan_insert_tag_set_proto(skb, vlan_proto[vlan_count], vlan_tci[vlan_count]);
        if (!skb)
        {
            break;
        }
                         ...
                         int *rxVlanDevInStackp)
{
    ...
    if(tableDir == BCM_VLAN_TABLE_DIR_RX)
    {
        ...
        skbuff_bcm_ext_vlan_get(*skbp, rxdev) = (*skbp)->dev;
        while(!netdev_path_is_root(skbuff_bcm_ext_vlan_get(*skbp, rxdev)))
            skbuff_bcm_ext_vlan_get(*skbp, rxdev) = netdev_path_next_dev(skbuff_bcm_ext_vlan_get(*skbp, rxdev));
    }
    else
    {
...
//        BCM_VLAN_DP_DEBUG("%s has no VLAN Interfaces", realDev->name);
...
#if defined(BCM_VLAN_ENABLE_SKB_VLAN)
    if(realDevCtrl->mode == BCM_VLAN_MODE_ONT && skbuff_bcm_ext_vlan_get(cmdInfo->skb, bcm_flags).restore_rx_vlan)
    {
        if(tableDir == BCM_VLAN_TABLE_DIR_TX)
        {
    ...
    if(tableDir == BCM_VLAN_TABLE_DIR_RX)
    {
       ...
       skbuff_bcm_ext_vlan_get(cmdInfo->skb, rxdev) = skbuff_bcm_ext_vlan_get(*skbp,
                                                                              rxdev);
                ...
                *rxVlanDevInStackp = isDevRxVlanDevInStack(cmdInfo->skb->dev, realDev);
                ...
                if(realDevCtrl->mode == BCM_VLAN_MODE_ONT)
                {
                    if(!(*rxVlanDevInStackp))
                    {
                        ...
                        skbuff_bcm_ext_vlan_get(cmdInfo->skb, bcm_flags).restore_rx_vlan = 1;
                    }
                }
#endif
            }
...
#define BCM_VLAN_FLOW_DUMP_BLOG_RULE(_blogRule_p)                       \
    do {bcm_print("\n===============================================================\n"); \
        blog_rule_dump(_blogRule_p); bcm_print("\n");} while(0)
#else
#define BCM_VLAN_FLOW_DEBUG(_fmt, _arg...)
#define BCM_VLAN_FLOW_DUMP_ENTRY(_realDevName, _realDevMode, _nbrOfTags, _tableDir, _tableEntry)
#define BCM_VLAN_FLOW_DUMP_BLOG_RULE(_blogRule_p)
                          ...
                          int isMulticast)
{
    ...
    if(!BCM_VLAN_IS_DONT_CARE(rxTagRuleFilter->skbMarkPort))
    {
        ...
        else if(blog_p->rx.info.phyHdrType == BLOG_EPONPHY)
        {
            if(isMulticast)
            {
                /* For EPON Multicast, do not create multiple blog rules based on skbMarkPort/LLID
                   as the accelerator does not care about the Rx LLID and forwards mcast pkt from
                   any LLID as long as other filter rules match. So, create one blog rule for
                   default unicast LLID 0 */
                if(rxTagRuleFilter->skbMarkPort != 0)
                {
                    ret = -EINVAL;
                                ...
                                bcmVlan_tableEntry_t *origTableEntry)
{
    ...
    {
        BCM_LOG_ERROR(BCM_LOG_ID_VLAN, "ruleTable %p, origTableEntry %px",
