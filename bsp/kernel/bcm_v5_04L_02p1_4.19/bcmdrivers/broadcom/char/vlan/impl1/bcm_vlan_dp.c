...
UBOOL8 bcmVlan_lookupDp(struct net_device *dev, uint8 *data, unsigned int len, bool *isDpConfigured);
...
#ifdef VLAN_DP_DEBUG
    bcm_print("isPktDropEligible(code%x, pbit%x, dei%x), val%d\n",
...
#ifdef VLAN_DP_DEBUG
    bcm_print("dp_code_set(): dir=%d, code=%d\n", dir, code);
#endif /* VLAN_DP_DEBUG */
...
UBOOL8 bcmVlan_lookupDp(struct net_device *dev, uint8 *data, unsigned int len, bool *isDpConfigured)
{
    ...
    if ( !isDpConfigured )
    {
        BCM_LOG_ERROR(BCM_LOG_ID_VLAN, "isDpEnabled NULL");
    }
