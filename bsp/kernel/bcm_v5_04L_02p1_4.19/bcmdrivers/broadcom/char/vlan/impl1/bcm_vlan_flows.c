...
#else
#define VLANCTL_ACTIVATE(_blog_p)   vlanctl_activate(_blog_p, VLANCTL_BIND_CLIENT_UNKNOWN)
#define VLANCTL_DEACTIVATE(_ruleid) vlanctl_deactivate(_ruleid, VLANCTL_BIND_CLIENT_UNKNOWN)
#endif
#endif
...
static inline void free_flowDev(bcmVlan_flowDev_t *free_p)
{
   ...
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "txDev=%px node=%px",
...
{
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "rxDev=%px node=%px", 
...
{
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "id=0x%x node=%px",
                 free_p->id, free_p);
...
bcmVlan_blogRule_id_t * activate_blog_rules(Blog_t *blog_p)
{
   ...
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "return ruleId_p=%px", ruleId_p);
                                           ...
                                           struct net_device *rxVlanDev_p)
{
   ...
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "rxVlanDev=%px flowDev_p=%px",
   ...
   while (path_p != NULL)
   {
      ...
      {
         BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "path_p=%px", path_p);
                            ...
                            struct net_device *txDev_p)
{
   ...
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "rxDev=%px txDev=%px",
                 rxDev_p, txDev_p);
   ...
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "rxVlanDev=%px txVlanDev=%px",
...
//   blog_p->key.protocol         = BLOG_IPPROTO_UDP;
   ...
   if (path_p == NULL)
   {
      ...
      BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "alloc new path_p=%px", path_p);
      if (path_p == NULL)  
      {
   ...
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "flowDev_p=%px flowPath_p=%px blogRuleId_p=%px",
...
int bcmVlan_flowDev_delete(struct net_device *dev_p)
{
   ...
   BCM_LOG_DEBUG(BCM_LOG_ID_VLAN, "dev_p %px", dev_p);
