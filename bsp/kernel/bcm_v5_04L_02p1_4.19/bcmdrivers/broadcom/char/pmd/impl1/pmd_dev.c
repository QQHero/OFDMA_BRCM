...
static long pmd_dev_file_ioctl(struct file *file, unsigned int cmd, unsigned long arg)
{
    ...
    switch (cmd) 
    {
        ...
        case PMD_IOCTL_SET_WAN_TYPE:
        {
            ...
            else if((net_port.port >= NET_PORT_LAN_0) && (net_port.port <= NET_PORT_LAN_7))
            {
                BCM_LOG_NOTICE(BCM_LOG_ID_PMD, "Skip reconfigure PMD wan type port=%d subtype=%d speed=%d\n", net_port.port,
                    net_port.sub_type, net_port.speed);
                break;   /* skip reconfigure PMD if LAN port is configured to Wan */
            }
            else
            {
...
static int pmd_probe(struct platform_device *pdev)
{
...
#ifdef CONFIG_BCM_PON
    trxbus_module_present(i2c->nr, NULL, 1);
#endif
    ...
    if (of_property_read_bool(np, "apd-flyback"))
        pdata->cap |= BP_PMD_APD_TYPE_FLYBACK;

    if (of_property_read_bool(np, "apd-boost"))
        pdata->cap |= BP_PMD_APD_TYPE_BOOST;

    if (of_property_read_bool(np, "apd-reg-enabled"))
        pdata->cap |= BP_PMD_APD_REG_ENABLED;
