...
{
    unsigned char i2c_buf[MAX_READ_SIZE + (MAX_READ_SIZE >> 2) - 1 + PMD_I2C_HEADER];
    ...
    if (read_op)
    {
        ...
        for (i = 0; i < num_of_transaction ; i++)
        {
            ...
            rc = pmd_i2c_read(client, i2c_buf, length_with_redundancy);
    ...
    else
    {
        ...
        rc = pmd_i2c_write(client, i2c_buf, length_with_redundancy);
        if(rc != length_with_redundancy)
            error_flag = 1;
    }
    if (error_flag)
...
int pmd_op_file(pmd_dev_client client, unsigned char * buf, int len, bool read_op)
{
    ...
    if (read_op)
    {
...
#endif
        {
            ...
            if ((rc = (int)kernel_read(fp, (void *) buf, len, &fp->f_pos)) <= 0)
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
            if((rc = (int) kernel_write(fp, (void *) buf, len, &fp->f_pos)) == len)
#elif (LINUX_VERSION_CODE >= KERNEL_VERSION(4,0,0))
