...
def write_read_all_text(order, text=">", timeout=0.5):
    '''
    This function is emptying the memory
    sending the "order" and reading data before "text" or till "timeout" is reached
    '''
    tn.read_very_eager()  # clean buffer
    tn.write(order + '\r\n')
    text_back = tn.read_until(text, timeout)
...
def read_calibration_results():
    ...
    rogue_thr = registers_value[11][4:6]
    rogue_thr_gap = registers_value[11][6:8]
...
class CalibrationResults(object):
    def __init__(self, data):
        ...
        self.dcyctl = data[25]
        self.rogue_thr = data[26]
        self.rogue_thr_gap = data[27]
