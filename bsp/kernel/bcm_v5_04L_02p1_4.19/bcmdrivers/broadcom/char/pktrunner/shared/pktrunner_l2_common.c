...
uint32_t pktrunner_l2flow_key_construct(Blog_t *blog_p, rdpa_l2_flow_key_t* l2_key)
{
    ...
    l2_key->eth_type = htons(blog_p->eth_type);
    ...
    else if (pktrunner_is_rx_enet_wan_port(blog_p))
    {
    ...
    {
        __logDebug("source.phy ETH/WLAN\n");
    ...
    l2_key->is_llc_snap = blog_p->tx.info.bmap.LLC_SNAP ? 1 : 0;
    /* End of key build */
