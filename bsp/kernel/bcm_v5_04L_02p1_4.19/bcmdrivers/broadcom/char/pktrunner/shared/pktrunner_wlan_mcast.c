...
static int __ssid_add(uint16_t *ssid_vector_p, uint8_t ssid)
{
    ...
    {
        __logInfo("SSID %d already joined the group (0x%04X)", ssid, *ssid_vector_p);
...
static int __ssid_delete(uint16_t *ssid_vector_p, uint8_t ssid)
{
    ...
    if(!(*ssid_vector_p & ssid_mask))
    {
        __logInfo("SSID %d did not join the group (0x%04X)", ssid, *ssid_vector_p);
                                   ...
                                   rdpa_wlan_mcast_ssid_mac_address_t *ssid_mac_address_p)
{
    ...
    {
        __logDebug("WLAN MCAST %s: WFD", add ? "ADD" : "DEL");
    ...
    else
    {
        ...
        __logDebug("WLAN MCAST %s: RNR", add ? "ADD" : "DEL");
...
*******************************************************************************
*/
void pktrunner_wlan_mcast_destruct(void)
{
    if(wlan_mcast_class)
    {
