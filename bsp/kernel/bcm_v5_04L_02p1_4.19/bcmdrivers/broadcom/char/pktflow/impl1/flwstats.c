...
static inline bool matchPHY(uint8_t *phy1, uint8_t dir, 
                            Blog_t    *blog_p)
{
    uint8_t i;
    uint8_t *devName=NULL;
    struct net_device *dev_p = NULL;
    struct net_device *virt_dev = NULL;
    uint8_t name_len = 0;
    ...
    {
        //check for virtual devices match in blog
        for (i=0;i<MAX_VIRT_DEV;i++)
        {
            virt_dev = (struct net_device *)blog_p->virt_dev_info[i].dev_p;
            if (virt_dev != NULL)
            {
                if (DEV_DIR(virt_dev) == dir)
                {
                    virt_dev = DEVP_DETACH_DIR(virt_dev);
                    if (strncasecmp(phy1, virt_dev->name,name_len) == 0)
                    {
                        return true;
                    }
                }
...
static inline bool is_valid_mcastflw_forquery(Blog_t *blog_p, uint8_t dir)
{
   /* 
    * This check is only for multicast flows and for unicast just return true.
    * - If any RX side param in the query then reject client flows.
    * - If any TX side param in the query then reject master flow.
    */
   if (blog_p->rx.multicast)
   {
       if ((dir == DIR_RX) && (blog_p->mcast_client_id != BLOG_MCAST_MASTER_CLIENT_ID))
           return false;
       else if((dir == DIR_TX) && (blog_p->mcast_client_id == BLOG_MCAST_MASTER_CLIENT_ID))
           return false;
   }
   return true;
}
                                  ...
                                  Blog_t *blog_p)
{
    ...
    if (   ((queryTuple->mask & FLWSTATS_QUERYMASK_SRCIPVER) &&
            ...
            || (!is_valid_mcastflw_forquery(blog_p, DIR_RX))) )
        || ((queryTuple->mask & FLWSTATS_QUERYMASK_DSTIPVER) &&
            ((TX_IPV4(blog_p) && queryTuple->dstipver != FLWSTATS_IPVER_V4)
            || (TX_IPV6(blog_p) && queryTuple->dstipver != FLWSTATS_IPVER_V6) 
            || (!is_valid_mcastflw_forquery(blog_p, DIR_TX))) )
        ...
        || ((queryTuple->mask & FLWSTATS_QUERYMASK_SRCPHY) && 
            (!(matchPHY(queryTuple->srcphy, DIR_RX,
                       blog_p)) ||
             !is_valid_mcastflw_forquery(blog_p, DIR_RX)))
        || ((queryTuple->mask & FLWSTATS_QUERYMASK_DSTPHY) && 
            (!(matchPHY(queryTuple->dstphy, DIR_TX,
                       blog_p)) ||
             !is_valid_mcastflw_forquery(blog_p, DIR_TX)))
