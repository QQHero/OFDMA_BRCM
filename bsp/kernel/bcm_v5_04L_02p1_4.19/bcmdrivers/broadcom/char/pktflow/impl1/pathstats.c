 ...
 *------------------------------------------------------------------------------
 */
static inline int path_pathstat_print(struct seq_file *m)
{
    bcm_seq_printf(m, "\nMax # of Path = %d\n", devpathdb.max_sw_path_ent);
    bcm_seq_printf(m, "Max # of HWACC cntr = %d\n", devpathdb.max_hwacc_counter);
    bcm_seq_printf(m, "Active # of Path = %d\n", devpathdb.active_sw_path_num);        
    bcm_seq_printf(m, "Active # of vt device = %d\n", devpathdb.active_vtdev_num);
    bcm_seq_printf(m, "Auto refresh path HW slice_idx_step = %u\n", devpathdb.slice_idx_step);
    bcm_seq_printf(m, "Auto refresh path HW path_num_per_slice = %u\n", devpathdb.path_num_per_slice);
    bcm_seq_printf(m, "Auto refresh cur_refresh_idx = %u\n\n", devpathdb.cur_refresh_idx);
 ...
 *------------------------------------------------------------------------------
 */
int path_vtdev_stats_print(struct seq_file *m, int * index_p, int * linesbudget_p)
{
    int j;
    Dll_t  * list_p;
    VtDev_t * vtdev_p;
    ...
    if (*index_p == 0)
    {
        ...
        bcm_seq_printf(m, "Active # vt device = %d\n", devpathdb.active_vtdev_num);
 ...
 *----------------------------------------------------------------------------
 */
static inline int pathstat_print_by_idx(struct seq_file *s, int index)
{
    ...
    bcm_seq_printf( s, "\t sw_pkt_count=%llu, sw_pkt_bytes=%llu \n",
    ...
    if ( _path_fhw_live() )
    {
        ...
        bcm_seq_printf( s, "\t hw_counter_valid=%d, hw_pkt_count=%llu, hw_pkt_bytes=%llu \n",
...
{
    return (*pos >= devpathdb.max_sw_path_ent) ? NULL : pos;
}
...
static int pathstatDrv_seq_show(struct seq_file *s, void *v)
{
    ...
    ret = pathstat_print_by_idx(s, (int) *pos);
...
/*
 *------------------------------------------------------------------------------
 * Function Name: pathstatDrvPathStatsShow
 * Description  : Handler to print the path stats
 *------------------------------------------------------------------------------
 */
static int pathstatDrvPathStatsShow(struct seq_file *m, void *v)
{
    blog_lock();

    // MOD_INC_USE_COUNT;

    path_pathstat_print( m );
...
/*
 *------------------------------------------------------------------------------
 * Function Name: pathstatDrvVtdevStatsShow
 * Description  : Handler to print the vt device stats
 *------------------------------------------------------------------------------
 */
static int pathstatDrvVtdevStatsShow(struct seq_file *m, void *v)
{
    int index=0;
    int linesbudget = 10;
    ...
    path_vtdev_stats_print( m, &index, &linesbudget );
...
int path_drv_construct(int procfs)
{
    ...
    if ( procfs == 1 ) {
        ...
        entry = proc_create_single(FCACHE_STATS_PROCFS_DIR_PATH "/path_usage",
            S_IRUGO, NULL, pathstatDrvPathStatsShow);
        if (!entry)
        {
        ...
        entry = proc_create_single(FCACHE_STATS_PROCFS_DIR_PATH "/vtdev",
            S_IRUGO, NULL, pathstatDrvVtdevStatsShow);
        if (!entry)
        {
...
static int path_create_path(Blog_t * blog_p, void **path_dev_p, int8_t *delta)
{
    ...
    dbgl_print(DBG_PDATA, " Rx_edge: %s_rx flag=0x%x  -> ", 
               tmp_dev_p->name, tmp_dev_p->priv_flags);
    
    if ( path_exclude_dev(tmp_dev_p) )
        return FCACHE_ERROR;
        
    if ( bcm_netdev_ext_field_get(tmp_dev_p, blog_stats_flags)
    ...
    {                
        blog_virt_dev_info_t *vdev_info_p = &blog_p->virt_dev_info[i];
        /* virt_dev[] ends with NULL */
        if ( vdev_info_p->dev_p == NULL )
        {
            /* Append edge tx_dev if blog_stats_flag on */
            tmp_dev_p = (struct net_device *)blog_p->tx_dev_p;
            dbgl_print(DBG_PDATA, " Tx_edge: %s_tx flag=0x%x -> ", 
                       tmp_dev_p->name, tmp_dev_p->priv_flags);
        ...
        tmp_dev_p = (struct net_device *)DEVP_DETACH_DIR(vdev_info_p->dev_p);
        dbgl_print(DBG_PDATA, " virt_%d: %s -> ", i, tmp_dev_p->name);
        if ( path_exclude_dev(tmp_dev_p) )
            return FCACHE_ERROR;        
        
        path_dev_p[dev_idx + i] = vdev_info_p->dev_p;
        delta[dev_idx + i] = vdev_info_p->delta;
    }
    return FCACHE_SUCCESS;
}
...
int path_add_flow(Flow_t * flow_p)
{
    ...
    for (dev_idx = 0; dev_idx < MAX_PATH_DEV; dev_idx++)
    {
        ...
        if ( unlikely( vt_dev_p == NULL ) )
        {
            ...
            dbgl_print(DBG_CTLFL, "%s, vt_dev_p=%px\n", "Create new vt dev", vt_dev_p);
        }
        else
...
int path_query_dev_stat(void * net_p, BlogStats_t * sw_stats_p, BlogStats_t * hw_stats_p)
{
    ...
    dbgl_print(DBG_CTLFL, "Query dev_%px: \n", net_p);
...
int path_clear_dev_stat(void * net_p)
{
    ...
    dbgl_print(DBG_CTLFL, "Clear dev_%px stats\n", net_p);
...
int path_exclude_dev(void * net_p)
{
    ...
    dbgl_print(DBG_CTLFL, " %s flag=0x%x ", 
...
static void path_push_stats_clear(Path_t * path_p)
{
    ...
    for (dev_idx=0; dev_idx<MAX_PATH_DEV; dev_idx++)
    {
        ...
        dbgl_print(DBG_CTLFL, " Path_%d evict or clear: dev<%px><%s> Dir<%u><%s> \n",
...
int path_evict_flow(Flow_t * flow_p)
{
    ...
    dbgl_print(DBG_CTLFL, " blog_p=%px\n", blog_p);
    if ( path_build_hash_key(blog_p, &enroute_pathkey) == FCACHE_ERROR )
        return FCACHE_ERROR;
