    ...
    UINT32 *pulRxVpiVciCamShadow, XtmBondConfig bondConfig)
{
    ...
    XtmOsMemSet(m_szNetDeviceName, 0x00, sizeof(m_szNetDeviceName));
    XtmOsMemSet(&m_Addr, 0x00, sizeof(m_Addr));
    XtmOsMemSet(&m_Cfg, 0x00, sizeof(m_Cfg));
    XtmOsMemSet(m_ucVcids, INVALID_VCID, sizeof(m_ucVcids));
    XtmOsMemSet(m_TxQIds, 0x00, sizeof(m_TxQIds));
    XtmOsMemSet(m_VcidInfo, 0x00, sizeof(m_VcidInfo));
...
{
    //XtmOsPrintf(XTM_LOG_DBG,"%s Enter",__FUNCTION__);
    XtmOsMemCpy(pAddr, &m_Addr, sizeof(m_Addr));
...
{
    XtmOsMemCpy(&m_Addr, pAddr, sizeof(m_Addr));
    if ((m_Addr.ulTrafficType & TRAFFIC_TYPE_ATM_MASK) == TRAFFIC_TYPE_ATM ) {
        XtmOsPrintf (XTM_LOG_INF, "ulTrafficType:%x ulPortMask:%x usVpi:%u usVci:%u",m_Addr.ulTrafficType,
                                                                                    m_Addr.u.Conn.ulPortMask,
                                                                                    m_Addr.u.Vcc.usVpi,
                                                                                    m_Addr.u.Vcc.usVci);
    }
    else {
        XtmOsPrintf (XTM_LOG_INF, "ulTrafficType:%x ulPortMask:%x ulPtmPriority%u",m_Addr.ulTrafficType,
                                                                                    m_Addr.u.Conn.ulPortMask,
                                                                                    m_Addr.u.Conn.ulPtmPriority);
    }
    return( XTMSTS_SUCCESS );
} /* SetAddr */
...
{
    //XtmOsPrintf(XTM_LOG_DBG,"%s Enter",__FUNCTION__);
    XtmOsMemCpy(pCfg, &m_Cfg, sizeof(m_Cfg));
    XtmOsPrintf (XTM_LOG_DBG, "G TxQ Index = %d\n", pCfg->TransmitQParms[0].ulTxQueueIdx);
    ...
    if( m_ulConnected && m_Cfg.ulAdminStatus == ADMSTS_UP )
    {
        ...
        XtmOsPrintf(XTM_LOG_DBG,"%s:calling XTMRT_CMD_GET_DEVICE_STATE",__FUNCTION__);
...
/***************************************************************************
 * Function Name: GetTxQMBR
 * Description  : Calculates the MBR required based on DSL mode based on the US
 *                link rates,
 * Returns      : XTMSTS_SUCCESS if successful or error status.
 ***************************************************************************/
UINT32 XTM_CONNECTION::GetTxQMBR (UINT32 usRateInBits )
{
   UINT32 ulQMBR ;
   UINT32 usRateInKBits = XTMCFG_DIV_ROUND(usRateInBits, 1000) ;

   XtmOsPrintf (XTM_LOG_INF,"\nbcmxtmcfg: Get Tx Q MBR Rate = %d\n", usRateInBits) ;

   switch (m_ulXTMLinkMode) {

      case XTM_LINK_MODE_ADSL      :
      case XTM_LINK_MODE_VDSL      :
         ulQMBR =  XTM_TXQ_DEFAULT_MBR*2;
         break;
      case XTM_LINK_MODE_VDSL_RTX  :
         ulQMBR =  (XTMCFG_DIV_ROUND(usRateInKBits,100000) * XTM_TXQ_DEFAULT_MBR)/2 ;
         ulQMBR = (ulQMBR < XTM_TXQ_DEFAULT_MBR) ? XTM_TXQ_DEFAULT_MBR : ulQMBR ;
         break;
         break;
      case XTM_LINK_MODE_GFAST     :
         ulQMBR =  (XTMCFG_DIV_ROUND(usRateInKBits,100000) * XTM_TXQ_DEFAULT_MBR) ;
         ulQMBR = (ulQMBR < XTM_TXQ_DEFAULT_MBR) ? XTM_TXQ_DEFAULT_MBR : ulQMBR ;
         break;
      default                      :
         ulQMBR =  XTM_TXQ_DEFAULT_MBR;
         break;
   }
...
BCMXTM_STATUS XTM_CONNECTION::SetCfg( PXTM_CONN_CFG pCfg, UINT32 ulXTMLinkMode, UINT32 ulXTMNitroMode )
{
    ...
    XTM_TRANSMIT_QUEUE_PARMS SavedTransmitQParms [MAX_TRANSMIT_QUEUES];
    ...
    if( pCfg )
    {
        ...
        for( UINT32 i = 0; i < pCfg->ulTransmitQParmsSize; i++ )
        {
            ...
            {
                XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: ulPid =%x, ulPortMask=%x ", ulPid, ulPortMask) ;
                bxStatus = XTMSTS_PARAMETER_ERROR;
                break;
            }
#if defined(CONFIG_BCM963148) || defined(CONFIG_BCM963138) || defined(CONFIG_BCM963158) || defined(CONFIG_BCM963178) || defined(CONFIG_BCM963146)
            pCfg->TransmitQParms[i].usSize = GetTxQSize () ;
            XtmOsPrintf(XTM_LOG_INF,"Q Size = %d ", pCfg->TransmitQParms[i].usSize) ;
#endif
            pCfg->TransmitQParms[i].ulMBR = GetTxQMBR (m_ulXTMUSRateAggr) ;
            XtmOsPrintf (XTM_LOG_INF,"Q%d, MBR = %d \n", i, pCfg->TransmitQParms[i].ulMBR) ;
        }
        ...
        if (bxStatus == XTMSTS_SUCCESS)
        {
               ...
               ( m_Addr.ulTrafficType == TRAFFIC_TYPE_PTM_BONDED))
            {
                ...
                for( UINT32 i = 0; i < pCfg->ulTransmitQParmsSize; i++ )
                {
                    ...
                    {
                        XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Param Error. QPrio =%d, AddrPrio=%x Q=%d ", ulPtmPri, 
                                    m_Addr.u.Flow.ulPtmPriority, i) ;
                        bxStatus = XTMSTS_PARAMETER_ERROR;
                        break;
                    }
                }
        ...
        if( bxStatus == XTMSTS_SUCCESS )
        {
            ...
            XtmOsMemCpy (&SavedTransmitQParms, &m_Cfg.TransmitQParms[0], 
                         (pCfg->ulTransmitQParmsSize*sizeof (XTM_TRANSMIT_QUEUE_PARMS))) ;
            XtmOsMemCpy(&m_Cfg, pCfg, sizeof(m_Cfg));
            for( UINT32 i = 0; i < pCfg->ulTransmitQParmsSize; i++ ) {
               m_Cfg.TransmitQParms[i].ulTxQueueIdx = SavedTransmitQParms[i].ulTxQueueIdx ;
            }
            m_ulRemovePending = 0;
...
{
    if (m_ulConnSem == NULL) {
        XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: STdt- ConnSem NULL");        
                                           ...
                                           UINT32 ulXTMLinkMode, UINT32 ulXTMNitroMode)
{
   ...
   if (m_ulConnSem == NULL) {
      XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: SLI- ConnSem NULL");        
      return XTMSTS_STATE_ERROR;
   }

   XtmOsPrintf(XTM_LOG_DBG,"%s Enter",__FUNCTION__);

   XtmOsMemSet(&Lsc, 0x00, sizeof(Lsc));
   ...
   m_ulXTMUSRateAggr = Lsc.ulLinkUsRate + Lsc.ulOtherLinkUsRate ;
   ...
   if( pLinkInfo->ulLinkState == LINK_UP )
   {
      if (!m_ulConnected)
      {
         ...
         if( bxStatus == XTMSTS_SUCCESS )
         {
                  ...
                  i++, pTxQParms++ )
            {
               ...
               XtmOsPrintf (XTM_LOG_DBG, "2TxQ Index = %d\n", pTxQParms->ulTxQueueIdx);
         ...
         if( bxStatus == XTMSTS_SUCCESS )
         {
            ...
            XtmOsMemCpy(Lsc.TransmitQueueIds, m_TxQIds, m_ulTxQIdsSize *
            ...
            XtmOsReleaseLock(m_ulConnSem);
            {
               ...
               if( g_pfnAdslGetObjValue != NULL )
               {
                  ...
                  if (ret != 0)
                     XtmOsPrintf(XTM_LOG_ERR,"g_pfnAdslGetObjValue port - %d return value - %d ", ulPort, ret) ;

                  if( PhyMibInfo.IkanosCO4Detected )
                  {
                     /* use ikanos ptm crc/enet fcs */
                     XtmOsPrintf(XTM_LOG_INF,"CO4 DSLAM based operations for PTM CRC & ENET FCS. ") ;
                  ...
                  {
                     XtmOsPrintf(XTM_LOG_INF,"NON-CO4 DSLAM based operations for PTM CRC & ENET FCS. ") ;
                     XP_REGS->ulTxSarCfg &= ~TXSARP_USE_ALT_PTM_CRC;
                     ulRxPafCfg |= RP_TC_CRC_ONEC_OUTPUT;
                     ulRxPafCfg &= ~(RP_TC_CRC_BITREV_OUTPUT | RP_TC_CRC_BITREV_INPUT);
                     ulRxPafCfg |= RP_ENET_CRC_DROP_PKT ;
                  ...
                  XtmOsPrintf(XTM_LOG_INF,"Default DSLAM based operations for PTM CRC & ENET FCS. ") ;
                  XP_REGS->ulTxSarCfg &= ~TXSARP_USE_ALT_PTM_CRC;
                  ulRxPafCfg |= RP_TC_CRC_ONEC_OUTPUT;
                  ulRxPafCfg &= ~(RP_TC_CRC_BITREV_OUTPUT | RP_TC_CRC_BITREV_INPUT);
                  ulRxPafCfg |= RP_ENET_CRC_DROP_PKT ;
      ...
      else
      {
         ...
         Lsc.ulTransmitQueueIdsSize = m_ulTxQIdsSize;
         XtmOsMemCpy(Lsc.TransmitQueueIds, m_TxQIds, m_ulTxQIdsSize * sizeof(XTMRT_TRANSMIT_QUEUE_ID));
   ...
   if( pendingConnected || pendingDisconnected || pendingConnectionUpdate)
   {
      if ((m_hDev != INVALID_HANDLE) && ((*m_pfnXtmrtReq) (m_hDev, XTMRT_CMD_LINK_STATUS_CHANGED, &Lsc) == 0 ) )
      {
         ...
         {
            XtmOsPrintf(XTM_LOG_NOT,"XTM W1B %u", XtmOsGetTimeStampMs()) ;
            for(queueIdx=0;queueIdx<m_ulTxQIdsSize;queueIdx++)
               WaitForTxComplete(m_TxQIds[queueIdx].ulQueueIndex) ;
            XtmOsPrintf(XTM_LOG_NOT,"XTM W1E %u", XtmOsGetTimeStampMs()) ;
            m_ulConnected = 0;
            UnreserveVcidsAndTxShapers();
   ...
   if (connected)
      XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Connection UP, LinkActiveStatus=0x%x, US=%d, DS=%d ",
            ulLinkMask,
      ...
      XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Connection DOWN, LinkActiveStatus=0x%x ",
            ulLinkMask) ;

   XtmOsPrintf (XTM_LOG_DBG, "3 TxQ Index = %d\n", m_Cfg.TransmitQParms[0].ulTxQueueIdx);
   return( bxStatus );
...
BCMXTM_STATUS XTM_CONNECTION::CreateNetworkDevice( char *pszNetDeviceName, UINT32 ulTxPafEnabled )
{
    ...
    if( !m_ulCreated )
    {
        ...
        XtmOsMemCpy(&Cni.ConnAddr, &m_Addr, sizeof(m_Addr));
//FIXME        strcpy(Cni.szNetworkDeviceName, pszNetDeviceName);
        XtmOsMemCpy(Cni.szNetworkDeviceName, pszNetDeviceName, NETWORK_DEVICE_NAME_SIZE);
        XtmOsMemCpy(m_Cfg.netDeviceName, pszNetDeviceName, NETWORK_DEVICE_NAME_SIZE);
        ...
        if( m_ulHwPtmBonding == 1 )
            Cni.ulFlags |= CNI_HW_PTM_BONDING ;
        if( m_ulHwPad == 1 )
            Cni.ulFlags |= CNI_HW_PAD ;

        if( (*m_pfnXtmrtReq) (NULL, XTMRT_CMD_CREATE_DEVICE, &Cni) == 0 )
        {
//FIXME            strcpy(m_szNetDeviceName, pszNetDeviceName);
            XtmOsMemCpy(m_szNetDeviceName, pszNetDeviceName, NETWORK_DEVICE_NAME_SIZE);
            m_hDev = Cni.hDev;
            m_ulCreated = 1;
            bxStatus = XTMSTS_SUCCESS;
...
BCMXTM_STATUS XTM_CONNECTION::DeleteNetworkDevice( void )
{
    ...
    if( m_ulConnected )
    {
        ...
        //XtmOsPrintf(XTM_LOG_ERR,"xtm: Conn DeleteNetworkDevice Connected ") ;
        XtmOsMemSet(&LinkInfo, 0x00, sizeof(LinkInfo));
    ...
    if( m_ulCreated )
    {
        //XtmOsPrintf(XTM_LOG_DBG,"xtm: Created. Delete_Device ") ;
        ...
        XtmOsMemSet(m_Cfg.netDeviceName, 0, NETWORK_DEVICE_NAME_SIZE);
        bxStatus = XTMSTS_SUCCESS;
    }
    else
        bxStatus = XTMSTS_STATE_ERROR;
...
BCMXTM_STATUS XTM_CONNECTION::ReserveVcidEntries( void )
{
   ...
   UINT32 ulPortMask = m_Addr.u.Conn.ulPortMask ;
    ...
    for( ulPort = PHY_PORTID_0;
         ...
         ulPort++ )
   {
      XtmOsPrintf(XTM_LOG_INF,"ulPort=%d, PortMask=%x ", ulPort, ulPortMask) ;
...
BCMXTM_STATUS XTM_CONNECTION::ReserveAtmVcidEntry( UINT32 ulPort )
{
    ...
    for( ulVcid = 0; ulVcid < MAX_VCIDS; ulVcid++ )
    {
        if( (m_pulRxVpiVciCamShadow[ulVcid] & RXCAM_VALID) == 0 )
        {
            ...
            //XtmOsPrintf ("\n XTM1: VcID=%d, RAM=%x \n", ulVcid, XP_REGS->ulRxVpiVciCam[(ulVcid*2)+1]) ;
...
BCMXTM_STATUS XTM_CONNECTION::ReserveOneMpGroup( VcidInfo *pInfo )
{
         ...
         i++, ulShift += TXMP_GROUP_SIZE )
    {
        if( (ulMpAalCfg & (TXMP_GROUP_EN << ulShift)) == 0 )
        {
            ...
            XP_REGS->ulSsteMpScr[i][TMUEXT_SCRINCR_INDEX] &=
...
BCMXTM_STATUS XTM_CONNECTION::ReserveOneTxQueue( PXTM_TRANSMIT_QUEUE_PARMS pTxQParms )
{
    BCMXTM_STATUS bxStatus = XTMSTS_SUCCESS;
    PXTMRT_TRANSMIT_QUEUE_ID pTxQId;
    UINT32 i, ulTxQueueIdx,maxTxQueuesAllowed;
    ...
    VcidInfo *pInfo;
    //XtmOsPrintf("%s:Enter\n",__FUNCTION__);
    maxTxQueuesAllowed = ulIsPtm?MAX_PTM_TRANSMIT_QUEUES:MAX_ATM_TRANSMIT_QUEUES;
    UINT32 ulPadConfig ;
    ...
    if( ulVcid >= MAX_VCIDS )
    {
        XtmOsPrintf(XTM_LOG_NOT,"ReserveOneTxQueue: tx queue has NO vcid %d. PortId=%x Prio=%d ", ulVcid,
                    pTxQParms->ulPortId, pTxQParms->ulPtmPriority);
        return( XTMSTS_STATE_ERROR );
    }

    /* Find an unused Tx queue. */
    for(i = 0, ulTxQueueIdx = INVALID_INDEX; i < maxTxQueuesAllowed; i++)
    {
    ...
    if( ulTxQueueIdx < maxTxQueuesAllowed )
    {
        XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Reserve TxQueueIdx=%d for vcid %d", ulTxQueueIdx, ulVcid);
    }
    else
    {
        XtmOsPrintf(XTM_LOG_ERR,"ReserveOneTxQueue: Failed to allocate tx queue.");
        ...
        ( ulIsPtm && pInfo->ulMpQueueCount == 0) )
    {
        ...
        if( bxStatus != XTMSTS_SUCCESS )
        {
            ...
            XtmOsPrintf(XTM_LOG_ERR,"ReserveOneTxQueue: Reserve MP grp error 0x%x", bxStatus);
            return( bxStatus );
        }
    }
    ...
    else if( pInfo->ulQueueIdx != INVALID_INDEX )
    {
        ...
        {
            XtmOsPrintf(XTM_LOG_ERR,"ReserveOneTxQueue: Could not find pInfo->ulQueueIdx %d",
                        pInfo->ulQueueIdx);
            pInfo->ulQueueIdx = INVALID_INDEX;
        }
    }
    ...
    if( (m_Addr.ulTrafficType&TRAFFIC_TYPE_ATM_MASK) == TRAFFIC_TYPE_ATM )
    {
        ...
        case AAL_5:
            ...
            ulPadConfig = 0 ; /* Will be determined later */
            break;
    ...
    if (m_ulHwPad) {
       ...
       switch (HT_TYPE(m_Cfg.ulHeaderType)) {

          case HT_TYPE_LLC_SNAP_ETHERNET     :
          case HT_TYPE_VC_MUX_ETHERNET       :
          case HT_TYPE_PTM                   :
             ulPadConfig = 1 ;
             break ;
          case HT_TYPE_LLC_SNAP_ROUTE_IP     :
          case HT_TYPE_LLC_ENCAPS_PPP        :
          case HT_TYPE_VC_MUX_IPOA           :
          case HT_TYPE_VC_MUX_PPPOA          :
             ulPadConfig = 0 ;
             break ;
          default                            :
             ulPadConfig = 0 ;
       ...
       if (ulPadConfig == 1) {
          ...
          i &= ~TXCH_DIS_64BYTE_ZERO_PAD_MASK ;
          i &= ~TXCH_MIN_PKT_SIZE_ZERO_PAD_MASK ;
          i |= (TXCH_MIN_PKT_SIZE << TXCH_MIN_PKT_SIZE_ZERO_PAD_SHIFT) ;
       }
       else {
          i |= TXCH_DISABLE_ZERO_PAD ;
       }
    } /* if (m_ulHwPad) */
    ...
    XtmOsPrintf(XTM_LOG_INF,"Q Size2 = %d ", pTxQId->usQueueSize) ;
    pTxQId->ulMBR           = pTxQParms->ulMBR ;
    XtmOsPrintf (XTM_LOG_INF,"Q MBR = %d \n", pTxQId->ulMBR) ;
...
BCMXTM_STATUS XTM_CONNECTION::UnreserveOneTxQueue( PXTMRT_TRANSMIT_QUEUE_ID pTxQId )
{
    ...
    if( ulVcid >= MAX_VCIDS )
    {
        XtmOsPrintf(XTM_LOG_ERR,"UnreserveOneTxQueue: tx queue has NO vcid %d. PortId=%x Prio=%d ", ulVcid,
    ...
    XP_REGS->ulSsteQueueScr[pTxQId->ulQueueIndex][TMUEXT_SCRINCR_INDEX] &=
        ~(SSTE_SCR_PLVLWT_MASK << SSTE_SCR_PLVLWT_SHIFT);
    ...
    else
    {
            ...
            ( ulIsPtm && pInfo->ulMpQueueCount == 0) )
        {
            ...
            XP_REGS->ulSsteMpScr[ulIdx][TMUEXT_SCRINCR_INDEX]  &=
                ~(SSTE_SCR_PLVLWT_MASK<<SSTE_SCR_PLVLWT_SHIFT);
                                                  ...
                                                  UINT32 ulShaperIdx )
{
    ...
    XtmOsPrintf(XTM_LOG_INF,"===>pcr=%u scr=%u mbs=%u mcr=%u SitUt=%u SitLoUt=%u",
    ...
    if( bxStatus == XTMSTS_SUCCESS )
    {
        ...
        if( ulPcr )
        {
            ulPcrSitLmt = XTMCFG_DIV_ROUND(ulSitsPerSec,ulPcr) ;
            if( ulPcrSitLmt == 0 )
            {
                XtmOsPrintf(XTM_LOG_ERR,"ConfigureAtmShaper: PCR_SITLMT=0x0 is invalid. No shaping.");
                return XTMSTS_PARAMETER_ERROR;
            }
            else if( ulPcrSitLmt > 0x7FFF )
            {
               XtmOsPrintf(XTM_LOG_INF,"ConfigureAtmShaper: PCR_SITLMT=0x%x", ulPcrSitLmt);
            
                /* Try using SIT_LO_CNT */
                ulPcrSitLmt = XTMCFG_DIV_ROUND(ulSitsPerSecLo,ulPcr);
                if( ulPcrSitLmt == 0 || ulPcrSitLmt > 0x7FFF )
                {
                    XtmOsPrintf(XTM_LOG_ERR,"ConfigureAtmShaper: PCR_SITLMT=0x%x is invalid. No shaping.",
                                ulPcrSitLmt);
                    return XTMSTS_PARAMETER_ERROR;
                }
                pcrSitLo = TRUE;
        ...
        if( ulScr )
        {
            ...
            if( ulScrSitLmt == 0 )
            {
                XtmOsPrintf(XTM_LOG_ERR,"ConfigureAtmShaper: SCR_SITLMT=0x0 is invalid. No shaping.");
                return XTMSTS_PARAMETER_ERROR;
            }
            else if( ulScrSitLmt > 0x7FFF )
            {
               XtmOsPrintf(XTM_LOG_INF,"ConfigureAtmShaper: SCR_SITLMT=0x%x", ulScrSitLmt);
            
                /* Try using SIT_LO_CNT */
                ulScrSitLmt = XTMCFG_DIV_ROUND(ulSitsPerSecLo,ulScr) ;
                if( ulScrSitLmt == 0 || ulScrSitLmt > 0x7FFF )
                {
                    XtmOsPrintf(XTM_LOG_ERR,"ConfigureAtmShaper: SCR_SITLMT=0x%x is invalid. No shaping.",
                                ulScrSitLmt);
                    return XTMSTS_PARAMETER_ERROR;
                }
                scrSitLo = TRUE;
        ...
        if( ulMcr )
        {
            ulMcrSitLmt = XTMCFG_DIV_ROUND(ulSitsPerSec,ulMcr) ;
            if( ulMcrSitLmt == 0 )
            {
                XtmOsPrintf(XTM_LOG_ERR,"ConfigureAtmShaper: MCR_SITLMT=0x0 is invalid. No shaping.");
                return XTMSTS_PARAMETER_ERROR;
            }
            else if( ulMcrSitLmt > 0x7FFF )
            {
               XtmOsPrintf(XTM_LOG_INF,"ConfigureAtmShaper: MCR_SITLMT=0x%x", ulMcrSitLmt);
            
                /* Try using SIT_LO_CNT */
                ulMcrSitLmt = XTMCFG_DIV_ROUND(ulSitsPerSecLo,ulMcr) ;
                if( ulMcrSitLmt == 0 || ulMcrSitLmt > 0x7FFF )
                {
                    XtmOsPrintf(XTM_LOG_ERR,"ConfigureAtmShaper: MCR_SITLMT=0x%x is invalid. No shaping.",
                                ulMcrSitLmt);
                    return XTMSTS_PARAMETER_ERROR;
                }
                mcrSitLo = TRUE;
        ...
        if( ulShaperType == QUEUE_SHAPER )
        {
            ...
            pulScr = &XP_REGS->ulSsteQueueScr[ulShaperIdx][TMUEXT_SCRINCR_INDEX];
            pulPcr = &XP_REGS->ulSsteQueuePcr[ulShaperIdx][TMUEXT_PCRINCR_INDEX];
        }
        else if( ulShaperType == MPAAL_SHAPER )
        {
            pulGts = &XP_REGS->ulSsMpGtsCfg[ulShaperIdx];
            pulScr = &XP_REGS->ulSsteMpScr[ulShaperIdx][TMUEXT_SCRINCR_INDEX];
            pulPcr = &XP_REGS->ulSsteMpPcr[ulShaperIdx][TMUEXT_PCRINCR_INDEX];
        }
        ...
        if( pulGts )
        {
            *pulGts = 0;
            ...
            pulScr[TMUEXT_SCRINCR_INDEX] &= (SSTE_SCR_PLVLWT_MASK << SSTE_SCR_PLVLWT_SHIFT);
            pulScr[TMUEXT_ACCLMT_INDEX] = pulScr[TMUEXT_CURACC_INDEX] = pulScr[TMUEXT_SITLMT_CURSIT_INDEX] = 0 ;
            pulPcr[TMUEXT_PCRINCR_INDEX] = pulPcr[TMUEXT_ACCLMT_INDEX] = pulPcr[TMUEXT_CURACC_INDEX]
                                         = pulPcr[TMUEXT_SITLMT_CURSIT_INDEX] = 0;

            switch( ulCategory )
            {
            case SC_CBR:
                pulScr[TMUEXT_SCRINCR_INDEX] |= (1 << SSTE_SCR_INCR_SHIFT);
                pulScr[TMUEXT_ACCLMT_INDEX]  = (CDVT << SSTE_SCR_ACCLMT_SHIFT);
                pulScr[TMUEXT_SITLMT_CURSIT_INDEX]  = (ulPcrSitLmt << SSTE_SCR_SITLMT_SHIFT);
            ...
            case SC_UBR:
                ...
                if( ulMcrSitLmt )
                {
                    pulPcr[TMUEXT_PCRINCR_INDEX] |= (1 << SSTE_PCR_INCR_SHIFT) |
                                                     (MCR_PRIORITY << SSTE_PCR_PLVLWT_SHIFT);
                    pulPcr[TMUEXT_ACCLMT_INDEX]  = (CDVT << SSTE_PCR_ACCLMT_SHIFT);
                    pulPcr[TMUEXT_SITLMT_CURSIT_INDEX]  = (ulMcrSitLmt << SSTE_PCR_SITLMT_SHIFT);
            ...
            case SC_NRT_VBR:
                pulScr[TMUEXT_SCRINCR_INDEX] |= (1 << SSTE_SCR_INCR_SHIFT);
                pulScr[TMUEXT_ACCLMT_INDEX]  = (CDVT << SSTE_SCR_ACCLMT_SHIFT);
                pulScr[TMUEXT_SITLMT_CURSIT_INDEX]  = (ulScrSitLmt << SSTE_SCR_SITLMT_SHIFT);
                pulPcr[TMUEXT_PCRINCR_INDEX]  = (1 << SSTE_PCR_INCR_SHIFT);
                pulPcr[TMUEXT_ACCLMT_INDEX]  = (ulMbs << SSTE_PCR_ACCLMT_SHIFT);
                pulPcr[TMUEXT_SITLMT_CURSIT_INDEX]  = (ulPcrSitLmt << SSTE_PCR_SITLMT_SHIFT);
...
/***************************************************************************
 * Function Name: CalculateMbs
 * Description  : Reserves and configures available shapers.
 *                New Mbs = (minrate)/8/(10^9/750000). Explanation that this is suitable
 *     		  for asymmetric Gfast is in SWBCACPE-23733. Refer to JIRA.
 * Returns      : XTMSTS_SUCCESS if successful or error status.
 ***************************************************************************/
UINT16 XTM_CONNECTION::CalculateMbs( UINT32 ulBitRate,
                                     UINT16 usMbs )
{
   UINT16 calcMbs ;
   XtmOsPrintf(XTM_LOG_DBG,"%s Enter",__FUNCTION__);
   ...
   if (m_ulXTMLinkMode == XTM_LINK_MODE_GFAST) {
      calcMbs = XTMCFG_DIV_ROUND(ulBitRate,8) ;
      calcMbs = XTMCFG_DIV_ROUND(calcMbs,GFAST_SCHED_DIV_FACTOR) ;
   }
   else {
      calcMbs = usMbs ;
   }
                                                       ...
                                                       UINT16 usMbs )
{
    ...
    volatile UINT32 *pulScr = &XP_REGS->ulSsteQueueScr[ulShaperIdx][TMUEXT_SCRINCR_INDEX];
    volatile UINT32 *pulPcr = &XP_REGS->ulSsteQueuePcr[ulShaperIdx][TMUEXT_PCRINCR_INDEX];
    UINT32 ulGts = 0;
    UINT32 ulShapeRateKbps, ulMinRateKbps ;
    UINT32 ulScrSitLmt, ulPcrSitLmt ;
#if defined(CONFIG_BCM963158) || defined(CONFIG_BCM963146)
    UINT64 ScrSitLmt, PcrSitLmt ;
#else
    UINT32 ScrSitLmt, PcrSitLmt ; /* 64-bit operations of div/mod unsupported */
#endif
    UINT16 usMinRateMbs ;
    ...
    ulMinRateKbps   = XTMCFG_DIV_ROUND(ulMinBitRate,1000) ;

    usMinRateMbs = CalculateMbs (ulMinBitRate, usMbs) ;

    XtmOsPrintf(XTM_LOG_INF,"===>ConfigurePtmQueueShaper: ShaperIdx=%u MinRate=%u ShapeRate=%u Mbs=%u MinRateMbs=%u, SitUt=%u SitLoUT=%u", ulShaperIdx, ulMinBitRate, ulShapeRate, usMbs, usMinRateMbs, ms_ulSitUt, ms_ulSitLoUt);
...
//        UINT32 ulScrSitLmt = ulSitsPerSec / ulScr;
        ScrSitLmt = ulSitsPerSec * PTM_TOKEN_BUCKET_INCREMENT ;
        ScrSitLmt = XTMCFG_DIV_ROUND(ScrSitLmt,1000) ;   /* In Kbps */
        ulScrSitLmt = XTMCFG_DIV_FACTOR_AND_ROUND(ScrSitLmt,8,ulShapeRateKbps) ;

     	XtmOsPrintf(XTM_LOG_INF,"ScrSitLmt[%u] SitsPerSec[%u]\n",ulScrSitLmt, ulSitsPerSec);
        if (ulScrSitLmt == 0)
        {
           XtmOsPrintf(XTM_LOG_ERR,"ConfigurePtmQueueShaper: SCR_SITLMT=0x0 is invalid. No shaping.");
           return XTMSTS_PARAMETER_ERROR;
        }
        else if (ulScrSitLmt > 0x7FFF)
        {
           XtmOsPrintf(XTM_LOG_INF,"ConfigurePtmQueueShaper: SCR_SITLMT=0x%x", ulScrSitLmt);
           
           /* Try using SIT_LO_CNT */
//           ulScrSitLmt  = ulSitsPerSecLo / ulScr;
           ScrSitLmt = ulSitsPerSecLo*PTM_TOKEN_BUCKET_INCREMENT ;
           ScrSitLmt = XTMCFG_DIV_ROUND(ScrSitLmt, 1000) ; /* In Kbps */
           ulScrSitLmt = XTMCFG_DIV_FACTOR_AND_ROUND(ScrSitLmt,8,ulShapeRateKbps) ;
           if (ulScrSitLmt == 0 || ulScrSitLmt > 0x7FFF)
           {
              XtmOsPrintf(XTM_LOG_ERR,"ConfigurePtmQueueShaper: SCR_SITLMT=0x%x is invalid. No shaping.",
                          ulScrSitLmt);
              return XTMSTS_PARAMETER_ERROR;
           }
        ...
        /* Preserve SCR priority which is used for arbitration. */
        pulScr[TMUEXT_SCRINCR_INDEX] &= (SSTE_SCR_PLVLWT_MASK << SSTE_SCR_PLVLWT_SHIFT);
        pulScr[TMUEXT_SCRINCR_INDEX] |= (PTM_TOKEN_BUCKET_INCREMENT << SSTE_SCR_INCR_SHIFT);
        pulScr[TMUEXT_ACCLMT_INDEX]  = (usMbs << SSTE_SCR_ACCLMT_SHIFT);
        pulScr[TMUEXT_SITLMT_CURSIT_INDEX]  = (ulScrSitLmt << SSTE_SCR_SITLMT_SHIFT);

	XtmOsPrintf(XTM_LOG_INF,"pulScr[%d-%08x] pulScr[%d-%08x] ",TMUEXT_SCRINCR_INDEX, pulScr[TMUEXT_SCRINCR_INDEX], TMUEXT_ACCLMT_INDEX, pulScr[TMUEXT_ACCLMT_INDEX]);
	XtmOsPrintf(XTM_LOG_INF,"pulScr[%d-%08x] pulScr[%d-%08x] ",TMUEXT_CURACC_INDEX, pulScr[TMUEXT_CURACC_INDEX],TMUEXT_SITLMT_CURSIT_INDEX, pulScr[TMUEXT_SITLMT_CURSIT_INDEX]);
    }

    if (ulMinBitRate && usMinRateMbs)
    {
        /* Calculate PCR.
...
//        UINT32 ulPcrSitLmt = ulSitsPerSec / ulPcr;
        PcrSitLmt = ulSitsPerSec * PTM_TOKEN_BUCKET_INCREMENT ;
	PcrSitLmt = XTMCFG_DIV_ROUND(PcrSitLmt,1000) ; /* In Kbps */
        ulPcrSitLmt = XTMCFG_DIV_FACTOR_AND_ROUND(PcrSitLmt,8,ulMinRateKbps) ;

  	XtmOsPrintf(XTM_LOG_INF,"PcrSitLmt[%u] SitsPerSec[%u]\n",ulPcrSitLmt, ulSitsPerSec);
        if (ulPcrSitLmt == 0)
        {
           XtmOsPrintf(XTM_LOG_ERR,"ConfigurePtmQueueShaper: PCR_SITLMT=0x0 is invalid. No shaping.");
           return XTMSTS_PARAMETER_ERROR;
        }
        else if (ulPcrSitLmt > 0x7FFF)
        {
           XtmOsPrintf(XTM_LOG_INF,"ConfigurePtmQueueShaper: PCR_SITLMT=0x%x", ulPcrSitLmt);
           
           /* Try using SIT_LO_CNT */
//           ulPcrSitLmt  = ulSitsPerSecLo / ulPcr;
           PcrSitLmt = ulSitsPerSecLo * PTM_TOKEN_BUCKET_INCREMENT ;
	   PcrSitLmt = XTMCFG_DIV_ROUND(PcrSitLmt,1000) ; /* In Kbps */
           ulPcrSitLmt = XTMCFG_DIV_FACTOR_AND_ROUND(PcrSitLmt,8,ulMinRateKbps) ;

           if (ulPcrSitLmt == 0 || ulPcrSitLmt > 0x7FFF)
           {
              XtmOsPrintf(XTM_LOG_ERR,"ConfigurePtmQueueShaper: PCR_SITLMT=0x%x is invalid. No shaping.",
                          ulPcrSitLmt);
              return XTMSTS_PARAMETER_ERROR;
           }
        ...
        pulPcr[TMUEXT_PCRINCR_INDEX] = (PTM_TOKEN_BUCKET_INCREMENT << SSTE_PCR_INCR_SHIFT) |
                                        (MCR_PRIORITY << SSTE_PCR_PLVLWT_SHIFT);
        pulPcr[TMUEXT_ACCLMT_INDEX] = (usMinRateMbs << SSTE_PCR_ACCLMT_SHIFT);
        pulPcr[TMUEXT_SITLMT_CURSIT_INDEX] = (ulPcrSitLmt << SSTE_PCR_SITLMT_SHIFT);
	XtmOsPrintf(XTM_LOG_INF,"pulPcr[%d-%08x] pulPcr[%d-%08x] ",TMUEXT_PCRINCR_INDEX, pulPcr[TMUEXT_PCRINCR_INDEX], TMUEXT_ACCLMT_INDEX, pulPcr[TMUEXT_ACCLMT_INDEX]);
	XtmOsPrintf(XTM_LOG_INF,"pulPcr[%d-%08x] pulPcr[%d-%08x] ",TMUEXT_CURACC_INDEX, pulPcr[TMUEXT_CURACC_INDEX],TMUEXT_SITLMT_CURSIT_INDEX, pulPcr[TMUEXT_SITLMT_CURSIT_INDEX]);
    }

    *pulGts = ulGts;

    XtmOsPrintf(XTM_LOG_INF,"pulGts[%08x] ",*pulGts);
                                           ...
                                           UINT32 ulSubPriority )
{
    ...
    if( ulQueueIdx != INVALID_INDEX )
    {
        ...
        XP_REGS->ulSsteQueueScr[ulQueueIdx][TMUEXT_SCRINCR_INDEX] &=
            ~(SSTE_SCR_PLVLWT_MASK << SSTE_SCR_PLVLWT_SHIFT);
        XP_REGS->ulSsteQueueScr[ulQueueIdx][TMUEXT_SCRINCR_INDEX] |=
...
BCMXTM_STATUS XTM_CONNECTION::UnreserveVcidsAndTxShapers( void )
{
    ...
    for( i = 0; i < m_ulTxQIdsSize; i++ )
    {
        ...
        pulScr = &XP_REGS->ulSsteQueueScr[j][TMUEXT_SCRINCR_INDEX];
        pulPcr = &XP_REGS->ulSsteQueuePcr[j][TMUEXT_PCRINCR_INDEX];
        pulScr[TMUEXT_SCRINCR_INDEX] = pulScr[TMUEXT_ACCLMT_INDEX] = pulScr[TMUEXT_SITLMT_CURSIT_INDEX] = 0;
        pulScr[TMUEXT_CURACC_INDEX] = 0 ;
        pulPcr[TMUEXT_PCRINCR_INDEX] = pulPcr[TMUEXT_ACCLMT_INDEX] = pulPcr[TMUEXT_SITLMT_CURSIT_INDEX] = 0;
        pulPcr[TMUEXT_CURACC_INDEX] = 0 ;
    }
    ...
    for( i = 0, pInfo = m_VcidInfo; i < MAX_VCIDS; i++, pInfo++ )
    {
        ...
        if( pInfo->ulMpIdx != INVALID_INDEX )
        {
            ...
            pulScr = &XP_REGS->ulSsteMpScr[pInfo->ulMpIdx][TMUEXT_SCRINCR_INDEX];
            pulPcr = &XP_REGS->ulSsteMpPcr[pInfo->ulMpIdx][TMUEXT_PCRINCR_INDEX];
            pulScr[TMUEXT_SCRINCR_INDEX] = pulScr[TMUEXT_ACCLMT_INDEX] = pulScr[TMUEXT_SITLMT_CURSIT_INDEX] = 0;
            pulScr[TMUEXT_CURACC_INDEX] = 0;
            pulPcr[TMUEXT_PCRINCR_INDEX] = pulPcr[TMUEXT_ACCLMT_INDEX] = pulPcr[TMUEXT_SITLMT_CURSIT_INDEX] = 0;
            pulPcr[TMUEXT_CURACC_INDEX] = 0;
    ...
    for( i = 0; i < MAX_INTERFACES; i++ )
        for( j = 0; j < MAX_PTM_PRIORITIES; j++ )
        {
            if( (ucVcid = m_ucVcids[i][j]) != INVALID_VCID )
            {
                ...
                //XtmOsPrintf ("\n XTM4: VcID=%d, RAM=%x \n", ucVcid, XP_REGS->ulRxVpiVciCam[(ucVcid*2)+1]) ;
                ...
                if (((ulPafVcId>>(i * 4)) & RXPAF_VCID_MASK) == ucVcid)
                {
                    ...
                    XP_REGS->ulRxPafVcid |= (RXPAF_DEFAULT_VCID << (i*RXPAF_VCID_SHIFT)) ;
                }
                ...
                if (((ulPafVcId>>(i * 4)) & RXPAF_VCID_MASK) == ucVcid)
                {
                    ...
                    XP_REGS->ulRxPafVcid |= (RXPAF_DEFAULT_VCID << 
...
void XTM_CONNECTION:: WaitForTxComplete (UINT32 ulQueueIdx)
{
...
#if defined(CONFIG_BCM963146)
   {
      ...
      unsigned int waitCntMain ;

      XtmOsPrintf(XTM_LOG_NOT,"Flushing XRDP SAR Tx BBH & SAR sub-system \n");
      XP_REGS->ulSoftReset |= TX_FLUSH_REQ;
      for (waitCntMain=100; waitCntMain > 0; waitCntMain--) {
         for (waitCnt=50; waitCnt>0; waitCnt--)
            asm ("nop") ;
         softReset = XP_REGS->ulSoftReset ;
         if (softReset & TX_FLUSH_DONE)
            break;
      }
      if (softReset & TX_FLUSH_DONE)
      	 XtmOsPrintf(XTM_LOG_NOT,"Flushing XRDP SAR Tx BBH & SAR sub-system done. \n");
      else
      	 XtmOsPrintf(XTM_LOG_NOT,"*****  Flushing XRDP SAR Tx BBH & SAR sub-system INCOMPLETE. ABNORMAL*****  \n");
      XP_REGS->ulSoftReset &= ~(TX_FLUSH_REQ | TX_FLUSH_DONE);
   }
#else
   {
      ...
      XtmOsPrintf(XTM_LOG_NOT,"Flushing SAR\n");
      while (1) {
      ...
      XtmOsPrintf(XTM_LOG_NOT,"Flushing SAR done.\n");
   }
#endif
}
 ...
 * Returns      : XTMSTS_SUCCESS
 ***************************************************************************/
BCMXTM_STATUS XTM_CONNECTION::CheckTransmitQueues( void )
{
    BCMXTM_STATUS bxStatus = XTMSTS_SUCCESS;
    ...
    if (m_ulConnSem == NULL) {
        XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: CTQ- ConnSem NULL");        
    }
    ...
    if( m_hDev != INVALID_HANDLE )
    {
       ...
       for( i = 0, pTxQId = m_TxQIds; i < m_ulTxQIdsSize; i++, pTxQId++ )
       {
                ...
                j++, pTxQParms++ )
          {
             if( pTxQId->ucQosQId      == pTxQParms->ucQosQId &&
                   ...
                   pTxQId->ulWeightValue == pTxQParms->ulWeightValue )
             {
                XtmOsMemCpy(&TxQIdsSave[ulTxQIdsSizeSave++], pTxQId,
                      sizeof(XTMRT_TRANSMIT_QUEUE_ID));
                ulFound = 1;
                break;
             }
          ...
          if (!ulFound) {
             ...
             if (((*m_pfnXtmrtReq) (m_hDev, XTMRT_CMD_UNSET_TX_QUEUE, pTxQId)) == 0) {
                ...
                XtmOsPrintf(XTM_LOG_NOT,"XTM W2B %u", XtmOsGetTimeStampMs()) ;
                WaitForTxComplete(pTxQId->ulQueueIndex) ;
                XtmOsPrintf(XTM_LOG_NOT,"XTM W2E %u", XtmOsGetTimeStampMs()) ;
       ...
       XtmOsMemCpy(m_TxQIds, TxQIdsSave, m_ulTxQIdsSize * sizeof(XTMRT_TRANSMIT_QUEUE_ID));
             ...
             j++, pTxQParms++ )
       {
          ...
          if (!ulFound) {
             ...
             XtmOsPrintf (XTM_LOG_DBG, "TxQ Index = %d\n", pTxQParms->ulTxQueueIdx);
             ...
             if( bxStatus == XTMSTS_SUCCESS )
             {
                ...
                XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: CTQ tx queue reservation failed.");
          }
       } /* for j */
       ...
       XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: CTQ Inv Dev Hdl");
...
#endif
    XtmOsMemSet (m_pConnTable, 0x00, sizeof(m_pConnTable)) ;
...
void XTM_CONNECTION_TABLE::Initialize( XTM_CONNECTION_TABLE *pConnTable )
{
   ...
   XtmOsMemSet(pConnTable, 0x00, sizeof(XTM_CONNECTION_TABLE));
...
void XTM_CONNECTION_TABLE::setThreshold( UINT32 mode, UINT32 ulThreshold)
{
   if(m_ulMuSem == NULL)
   {
      XtmOsPrintf(XTM_LOG_ERR,"m_ulMuSem is NULL");
      return;
   }
...
BCMXTM_STATUS XTM_CONNECTION_TABLE::Add( XTM_CONNECTION *pConn )
{
    ...
    {
        XtmOsPrintf(XTM_LOG_ERR,"xtmcfg: ulPm = %x ", ulPm) ;
        bxStatus = XTMSTS_PARAMETER_ERROR;
    }
    else
    {
    ...
    //XtmOsPrintf(XTM_LOG_DBG,"xtmcfg: bxStatus = %x ", bxStatus) ;
    return( bxStatus );
} /* Add */
...
XTM_CONNECTION *XTM_CONNECTION_TABLE::Enum( UINT32 *pulIdx )
{
    XTM_CONNECTION *pConn = NULL;
    UINT32 i;
    //XtmOsPrintf(XTM_LOG_DBG,"m_ulMuSem[%pk]",m_ulMuSem);
    if(m_ulMuSem == NULL)
    {
       XtmOsPrintf(XTM_LOG_ERR,"m_ulMuSem is NULL");
       return NULL;
