...
void XTM_ASM_HANDLER::resetBondingGroup ()
{
   ...
   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: resetBondingGroup ") ;
   ...
   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Bonding State is %s ", BONDING_STATE(BND_IDLE)) ;
   m_BondMgmt.vt = XtmOsGetTimeStamp () ;
   m_BondMgmt.refLink = ~0 ; //* Currently no ref link */
   XtmOsMemSet (&m_BondMgmt.asmCell, 0, sizeof (Asm)) ;
...
void XTM_ASM_HANDLER::mappingReset(UINT32 ulLogicalPort, int timeout)
{
   ...
   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: tx %d disabled ", ulLogicalPort) ;
...
void XTM_ASM_HANDLER::PollLines ()
{
   ...
   if (localId != MAX_BONDED_LINES) {
        ...
        {
            XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: XTM_ASM_HANDLER::PollLines() failed due to uninialized m_pInterfaces[] array.") ;
            return;
        }
...
void  XTM_ASM_HANDLER::LinkStateNotify (UINT32 ulLogicalPort)
{
   ...
   XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: LinkStateNotify %d  us %d  ds %d ",
   ...
   */
   ...
   if (trainingState == LINK_DOWN)
   {
      ...
      objp->usecTxAsm = ~0;
      objp->asmReady = 0;
      XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: tx link %d disabled ", ulLogicalPort) ;
      //m_pInterfaces[ulLogicalPort].SetLinkDataStatus (DATA_STATUS_DISABLED) ;
   }
   else
   {
      ...
      XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: tx link %d enabled ", ulLogicalPort) ;
      ...
      if (m_BondMgmt.lines[i].asmReady) {
         ...
         if (pLi->ulLinkState == LINK_UP) {
            ...
            if (
               ...
               /* Link up, but ASMs cant be sent for some reason */
               XtmOsPrintf (XTM_LOG_DBG,"bcmxtmcfg: No Tx ASM over 0x%x. Rate=%d, LtoR=0x%x, resetState=%d \n", i,
                             pLi->ulLinkUsRate, m_BondMgmt.mapping[i].localIdToRemoteId, m_BondMgmt.resetState) ;
            }
         ...
         //XtmOsPrintf (XTM_LOG_DBG,"bcmxtmcfg: NOT asmReady %d", i) ;
      }
...
void XTM_ASM_HANDLER::MgmtTimerCb (void *ulParm)
{
   ...
   //XtmOsPrintf(XTM_LOG_DBG,"XTM: pAsmHandle = %lx ", pAsmHandle) ;
   if (bondMgmtValid) {
...
BCMXTM_STATUS XTM_ASM_HANDLER::SendAsmCell ( UINT32 ulPhysPort, UINT8 *pAsmData)
{
   ...
   XtmOsPrintf(XTM_LOG_DBG,"SendAsmCell %d ", ulPhysPort) ;
   ...
   switch (ulPhysPort) {
      ...
      case PHY_PORTID_3    :
         ...
         break ;
      default              :
         XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Tx'ed ASM Unknown Port %d ", ulPhysPort) ;
   ...
   if( bxStatus == XTMSTS_SUCCESS ) {
      ...
      XtmOsMemCpy (Cell.ucData, pAsmData, sizeof (Asm)) ;
...
{
#if 0
   XtmOsPrintf(XTM_LOG_DBG,"------------------%s-------------------",pMsg) ;
   XtmOsPrintf(XTM_LOG_DBG,"ASM Message ") ;
   XtmOsPrintf(XTM_LOG_DBG,"msgType        = 0x%x ", pAsm->msgType) ;
   XtmOsPrintf(XTM_LOG_DBG,"asmId          = %d ", pAsm->asmId) ;
   XtmOsPrintf(XTM_LOG_DBG,"txLinkId       = %d ", pAsm->txLinkId) ;
   XtmOsPrintf(XTM_LOG_DBG,"txLinkNumber   = %d ", pAsm->txLinkNumber) ;
   XtmOsPrintf(XTM_LOG_DBG,"rxLinkStatus   = 0x%x 0x%x ", pAsm->rxLinkStatus[0], pAsm->rxLinkStatus[1]);
   XtmOsPrintf(XTM_LOG_DBG,"txLinkStatus   = 0x%x 0x%x ", pAsm->txLinkStatus[0], pAsm->txLinkStatus[1]);
   XtmOsPrintf(XTM_LOG_DBG,"groupId        = %d ", pAsm->groupId) ;
   XtmOsPrintf(XTM_LOG_DBG,"rxAsmStatus    = 0x%x 0x%x ", pAsm->rxAsmStatus [0], pAsm->rxAsmStatus [1]) ;
   XtmOsPrintf(XTM_LOG_DBG,"groupLostCell  = %d ", pAsm->groupLostCell) ;
   XtmOsPrintf(XTM_LOG_DBG,"timestamp      = 0x%x ", pAsm->timestamp) ;
   XtmOsPrintf(XTM_LOG_DBG,"delay          = %d ", pAsm->delay) ;
   XtmOsPrintf(XTM_LOG_DBG,"appliedDelay   = %d ", pAsm->appliedDelay) ;
   XtmOsPrintf(XTM_LOG_DBG,"msgLen         = %d ", pAsm->msgLen) ;
   XtmOsPrintf(XTM_LOG_DBG,"crc            = %x ", pAsm->crc) ;
   XtmOsPrintf(XTM_LOG_DBG,"---------------------------------------") ;
#else
   XtmOsPrintf(XTM_LOG_NOT,"%s",pMsg) ;
   XtmOsPrintf(XTM_LOG_NOT,"mt = 0x%x ", pAsm->msgType) ;
   XtmOsPrintf(XTM_LOG_NOT,"id = %d ", pAsm->txLinkId) ;
   XtmOsPrintf(XTM_LOG_NOT,"rx = 0x%x 0x%x ", pAsm->rxLinkStatus[0], pAsm->rxLinkStatus[1]);
   XtmOsPrintf(XTM_LOG_NOT,"tx = 0x%x 0x%x ", pAsm->txLinkStatus[0], pAsm->txLinkStatus[1]);
   XtmOsPrintf(XTM_LOG_NOT,"groupId        = %d ", pAsm->groupId) ;
#endif
}
...
int XTM_ASM_HANDLER::mappingKnown ()
{
  ...
  if (m_BondMgmt.numberOfLines != 0) {
     ...
     XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: atm bond mgmt number of lines is 0 ") ;
...
UINT32 XTM_ASM_HANDLER::UpdateResetFsm (int wrongCheck, int newMsgType, UINT32 ulLogicalPort)
{
  ...
  XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: update Reset FSM newMsgType = %x ", newMsgType) ;
  ...
  if (wrongCheck) {
    ...
    if (resetState == BND_RUNNING) {
      if (newMsgType == 0xFF) {
        ...
        XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: ASM New MsgType=%d", newMsgType) ;
        resetBondingGroup ();
        resetState = BND_IDLE;
      }
    }
      ...
      if ((newMsgType == 0xFF) || timeoutPassed) {
        ...
        /* go idle */
        XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: ASM New MsgType=%d, TimeOutPassed=%d ", newMsgType,
                     timeoutPassed) ;
      ...
      if (mappingKnown() && (newMsgType != 0xFF)) {
        XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: State is %s ASM New MsgType=%d ", BONDING_STATE(resetState), newMsgType);

        resetState = BND_RUNNING ;
        XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: Bonding State is %s ", BONDING_STATE(resetState)) ;
      }
    }
  } /* if good asm */
  ...
  if (resetState == BND_RUNNING) {
     ...
     if (newMsgType != m_BondMgmt.asmCell.msgType)
        XtmOsPrintf(XTM_LOG_INF,"Port:%lu, CO SID type = %d, CPE SID type = %lu ", ulLogicalPort, newMsgType, m_BondMgmt.asmCell.msgType) ;
    ...
    if ((UINT32) newMsgType != m_BondMgmt.sidType) {
       if (newMsgType == ATMBOND_ASM_MESSAGE_TYPE_8BITSID) {
          XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: SID MODE SET to 8 BIT MODE ") ;
          i &= ~TXSARA_SID12_EN ;
          j &= ~RXSARA_SID12_EN ;
       }
       else {
          XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: SID MODE SET to 12 BIT MODE ") ;
          i |= TXSARA_SID12_EN ;
          j |= RXSARA_SID12_EN ;
       }
  ...
  if (m_BondMgmt.resetState != resetState)
     XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Bonding State is %s ", BONDING_STATE(resetState)) ;
...
void XTM_ASM_HANDLER::UpdateLineState (UINT32 ulLogicalPort)
{
  ...
  if (!objp->inUseTx) {
    ...
    XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: tx %d enabled ", ulLogicalPort) ;
    //m_pInterfaces [ulLogicalPort].SetLinkDataStatus (DATA_STATUS_ENABLED) ;
  }
}
...
int XTM_ASM_HANDLER::ProcessReceivedAsm (Asm *pAsm, UINT32 ulLogicalPort)
{
   ...
   XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: Rx ASM port : %d ", ulLogicalPort) ;
   ...
   if ((pAsm->msgType != 0xFF) && ((pAsm->msgType & ~1) != 0)) {
      ...
      XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: ASM msgType %d received ", pAsm->msgType) ;
   ...
   *((UINT32 *) &m_BondMgmt.asmCell.rxAsmStatus [0]) &= ~(0x1<<(31-ulLogicalPort)) ;
   ...
   if ((m_BondMgmt.knownMapping & (1<<ulLogicalPort)) == 0)
   {
      ...
      if (m_BondMgmt.remoteIdToLocal[txLinkId] != -1) {
         ...
         XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Asm Check Failed ") ;
         checkFailed = 1;
      }
      else
      {
         if(m_BondMgmt.numberOfLines != 0)
         {
            ...
            if(m_BondMgmt.bondingId != pAsm->groupId)
            {
               ...
               XtmOsPrintf(XTM_LOG_ERR,"Bonding Group Id mismatch m_BondMgmt.bondingId:[%u] Port[%u] bondingId:[%u]",m_BondMgmt.bondingId,
                                                                                                           ulLogicalPort,
                                                                                                           pAsm->groupId);
               return 0;
            }
...

   {
      ...
      if ((m_BondMgmt.mapping [ulLogicalPort].localIdToRemoteId != txLinkId)
            || (m_BondMgmt.bondingId != pAsm->groupId)) {
         XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: checkFailed 2 ") ;
         checkFailed = 1;
      }
      ...
      if (newResetFsmState != BND_RUNNING) {
         XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: newResetFsmState is %d ", newResetFsmState) ;
   ...
   {
      XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: outofSequence ASM id %d last asm id %d",
...
BOOL XTM_ASM_HANDLER::CheckASMActivity ( void)
{
   ...
   for (i=0;i<MAX_BONDED_LINES;i++) {
      XtmOsPrintf(XTM_LOG_INF,"ASM: Line %d RxAsm %u ", i, m_BondMgmt.mapping[i].receivedAsm) ;
                                 ...
                                 XTM_AUTOSENSE *pXtmAutoSense)
{
   ...
   XTMRT_CELL_HDLR CellHdlr;
    ...
    if(m_bInitialized) {
      XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Out of sequence call to XTM_ASM_HANDLER::Initialize().  Recovering.") ;
   ...
   if( !m_ulRegistered )
   {
      ...
      XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ATM Bonding configured in system. Fallback mode = %s ",
                   (m_BondingFallback == XTM_BONDING_FALLBACK_ENABLED) ? "Enabled" : "Disabled");

      /* Initialize bondMgmt structure field definitions */
      XtmOsMemSet (&m_BondMgmt, 0, sizeof (XtmBondMgmt));
      ...
      for (k=0;k<MAX_BONDED_LINES;k++) {
          ...
          */
         XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: tx %d disabled ", k) ;
               ...
               &CellHdlr) == 0 )
      {
         ...
         XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Error registering ASM handler");
         bxStatus = XTMSTS_ERROR ;
      }

      pAsmTimerParm = (void *) this ;
      //XtmOsPrintf(XTM_LOG_DBG,"XTM: pAsmHandle = %lx ", pAsmTimerParm) ;
      XtmOsStartTimer((void *) XTM_ASM_HANDLER::MgmtTimerCb, 0,
            XTM_BOND_MGMT_TIMEOUT_MS) ;
      bondMgmtValid = 1 ;
      m_BondMgmt.printAsm = 0 ;
      XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ATM Bonding Mgmt Log Area = %px ", &m_BondMgmt.printAsm) ;
   }
   else
...
BCMXTM_STATUS XTM_ASM_HANDLER::Uninitialize( void )
{
   ...
   pAsmTimerParm = NULL ;
    ...
    if(!m_bInitialized || m_bUninitialized) {
        XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: Out of sequence call to XTM_ASM_HANDLER::Uninitialize().  Recovering.") ;
        return( XTMSTS_SUCCESS ); /* Quit since there's nothing to do */
    }
   ...
   if( m_ulRegistered )
   {
      ...
      for (k=0;k<MAX_BONDED_LINES;k++) {
         ...
         XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: tx %d disabled ", k) ;
         //m_pInterfaces[k].SetLinkDataStatus (DATA_STATUS_DISABLED) ;
      }
    ...
    void *pParam, void *pContext )
{
   ...
   switch (pCell->ucCircuitType) {
      ...
      case CTYPE_ASM_P3    :
         ...
         break ;
      default              :
         XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Received ASM Unknown %d ", pCell->ucCircuitType) ;
         status = -1 ;
         break ;
   }
