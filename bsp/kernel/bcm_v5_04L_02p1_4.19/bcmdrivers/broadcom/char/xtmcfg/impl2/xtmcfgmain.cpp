...
BCMXTM_STATUS BcmXtm_Initialize( PXTM_INITIALIZATION_PARMS pInitParms )
{
    ...
    if( g_pXtmProcessor == NULL )
    {
        ...
        if( g_pXtmProcessor != NULL )
        {
            ...
            XtmOsMemSet(g_pXtmProcessor, 0x00, sizeof(XTM_PROCESSOR));
        ...
        if( g_pXtmProcessor != NULL )
        {
            ...
            if( bxStatus == XTMSTS_SUCCESS )
            {
                ...
                else
                {
                    ...
                    XtmOsPrintf(XTM_LOG_NOT,"DPU Profile Need to create PTM Interface here");
                    XtmOsMemSet(DeviceName,0,sizeof(DeviceName));
                    XtmOsMemSet(&Addr, 0x0, sizeof(Addr));
                    XtmOsMemSet(&ConnCfg, 0x0, sizeof(ConnCfg));
...
BCMXTM_STATUS BcmXtm_GetInterfaceCfg( UINT32 ulPortId, PXTM_INTERFACE_CFG
    pInterfaceCfg )
{
    XtmOsPrintf(XTM_LOG_DBG,"%s:Enter g_pXtmProcessor [%px] ",__FUNCTION__,g_pXtmProcessor);
#if (1) 
    return( (g_pXtmProcessor)
    ...
    PATM_INTERFACE_LINK_INFO pInterfaceLinkInfo )
{
    ...
    XtmOsPrintf(XTM_LOG_INF,"%s:Enter",__FUNCTION__);
...
/***************************************************************************
 * Function Name: BcmXtm_ChipInitialization (63138/63381/63148/63158/63178/63146)
    ...
    UINT32 *pulBusSpeed )
{
   ...
   XtmOsPrintf(XTM_LOG_DBG,"%s:Enter",__FUNCTION__);
#if defined(NO_XDSL_MODULE)
   bxStatus = (BCMXTM_STATUS) pmc_dsl_power_up();
   if(bxStatus != 0) {
      XtmOsPrintf(XTM_LOG_ERR, "*** %s: pmc_dsl_power_up failed(%d)!!! ***\n", __FUNCTION__, bxStatus);
      bxStatus = XTMSTS_STATE_ERROR ;
   }
   else
      XtmOsPrintf(XTM_LOG_DBG, "*** %s: pmc_dsl_power_up passed (%d)!!! ***\n", __FUNCTION__, bxStatus);
#endif
   return( bxStatus );
