...
int XTM_PROCESSOR::Cxtor( void )
{
   ...
   XtmOsMemSet(&m_InitParms, 0x00, sizeof(m_InitParms));
   ...
   XtmOsMemSet (m_ulRxVpiVciCamShadow, 0x00, sizeof(m_ulRxVpiVciCamShadow));
...

{
   ...
   {
      UINT32 ulPort ;
      //XtmOsPrintf("%s Enter",__FUNCTION__,);
      m_pTrafficDescrEntries = NULL;
      m_ulNumTrafficDescrEntries = 0;
      XtmOsMemSet(&m_InitParms, 0x00, sizeof(m_InitParms));
      XtmOsPrintf(XTM_LOG_INF,"SAR Base:%px",XP_REGS);
   ...
   if( pInitParms ) {
      ...
      XtmOsMemCpy(&m_InitParms, pInitParms, sizeof(m_InitParms));
   }
   ...
   XtmOsPrintf(XTM_LOG_DBG,"accessing rxsarcfg \n") ;
   j = XP_REGS->ulRxSarCfg;
   XtmOsPrintf(XTM_LOG_DBG,"rxsarcfg = %x \n", j) ;
...
#if defined(CONFIG_BCM963138) || defined(CONFIG_BCM963148)
   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ChipId Rev-%x ", m_ulChipIdRev) ;
...
/* No selection necessary for 63158. BBH is the only option */
   ...
   for (l=0; l<XP_MAX_RXPAF_WR_CHANNELS; l++)
      XP_REGS->ulRxPafVcid |= (RXPAF_DEFAULT_VCID<<(l*RXPAF_VCID_SHIFT)) ;
   ...
   switch( (m_InitParms.ulPortConfig & PC_INTERNAL_EXTERNAL_MASK) )
   {
      ...
      default:
         XtmOsMemSet(ucInternalPort, 0x01, sizeof(ucInternalPort));
         break;
   }
   ...
   if( pInitParms)
   {
      ...
      XtmOsMemCpy(GlobInit.ulReceiveQueueSizes, m_InitParms.ulReceiveQueueSizes,
      ...
      if (bxStatus != XTMSTS_SUCCESS) {
          ...
          XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: ATM Modes Initialization Error. Can't initialize OAM Handler!  FATAL!!!! ") ;
          return(bxStatus);
      }
   ...
   if (bxStatus != XTMSTS_SUCCESS) {
      ...
      XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: ATM Bonding Initialization Error. Can't initialize Autosense!  FATAL!!!! ") ;
   ...
   if (bxStatus != XTMSTS_SUCCESS) {
      /* No - flag error and then try to uninitialize and deallocate */
      XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: ATM Bonding Initialization Error. Can't initialize ASM handler!  FATAL!!!! ") ;
      m_AsmHandler.Uninitialize() ;
      m_XtmAutoSense.Uninitialize() ;
   ...
   //XtmOsPrintf(XTM_LOG_DBG,"XTM: pBondTmrParm = %lx ", pBondTmrParm) ;
             ...
             PXTM_INTERFACE_LINK_INFO pOtherLinkInfo)
{
   ...
   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ReprogramRxPaf traffictype %d ", ulTrafficType) ;
   ...
   if ((pLinkInfo->ulLinkState == LINK_UP) && (pOtherLinkInfo->ulLinkState == LINK_UP)) {
      ...
      ulBaseTimerUs = XTMCFG_DIV_ROUND(XP_REGS->ulRxBondBaseTimer,RXBOND_CLK_DIVIDER) ;
 ...
 * Description  : Reconfigures the SAR based on the traffic type.
 *                Bonding/Normal modes can change dynamically, atleast the SW
 *                provisions for the same. 
 * Returns      : XTMSTS_SUCCESS if successful or error status.
 ***************************************************************************/
void XTM_PROCESSOR::ReconfigureSAR ( UINT32 ulPort, UINT32 ulTrafficType, UINT32 ulSRAEvent )
{
   UINT32   i,j,k,l,m ;
   BCMXTM_STATUS bxStatus = XTMSTS_SUCCESS ;

   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ReconfSAR P-%d, T-%d ", ulPort, ulTrafficType) ;
   ...
   switch (ulTrafficType) {
      ...
      case   TRAFFIC_TYPE_PTM_BONDED   :
         ...
         if (m_ulTrafficType != ulTrafficType) {
            ...
            l = (TXFRAG_MIN_FRAG_SIZE << TXFRAG_MIN_FRAG_SIZE_SHIFT) |
                (TXFRAG_MAX_FRAG_SIZE << TXFRAG_MAX_FRAG_SIZE_SHIFT) ;

            XP_REGS->ulTxFragCfg = l;
#endif
...
#ifdef XTM_BOND_24HDR
            i = XP_REGS->ulSsNewCfgD0_or_Reserved6 ;
            i |= SHPR_NEWCFGD0_EN24HDR ;
            XP_REGS->ulSsNewCfgD0_or_Reserved6 = i ;
#endif
          ...
          */
          ...
          XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: TxPAF Status = %s ", (m_ulTxPafEnabled==1) ? "Enabled" : "Disabled") ;
#else
          m_ulTxPafEnabled = 0 ;
#endif

#if defined (CONFIG_BCM963146)
          XP_REGS->ulSsForceVcArbRR |= SST_USE_FIFO_EMPTY_ONLY ;
          XP_REGS->ulTxIfCfg        &= ~TXIF_FIFO_SPACE_CFG_MASK ;
          XP_REGS->ulTxIfCfg        |= (TXIF_INIT_REQ | TXIF_FIFO_SPACE_8_CHANNEL) ;
#endif

          i = XP_REGS->ulSsNewCfgD0_or_Reserved6 ;
          i &= ~SHPR_NEWCFGD0_DISABLE_OLD ;
          XP_REGS->ulSsNewCfgD0_or_Reserved6 = i ;

          if (!g_pfnAdslSetObjValue) {
             /* Enable SAR loopback */
             XP_REGS->ulDiagCtrl  |= DIAG_SAR_LBPK_EN ;
          }

          XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: PTM Bonding enabled ");
      ...
      case   TRAFFIC_TYPE_ATM_BONDED   :
         ...
         if (m_ulTrafficType != ulTrafficType) {
                 ...
                 XP_REGS->ulSsForceVcArbRR |= SST_USE_FIFO_EMPTY_ONLY ;
                 XP_REGS->ulTxIfCfg        &= ~TXIF_FIFO_SPACE_CFG_MASK ;
                 XP_REGS->ulTxIfCfg        |= (TXIF_INIT_REQ | TXIF_FIFO_SPACE_16_CHANNEL) ;
#endif

                 /* Bonding Bits Controls */
                 i = XP_REGS->ulTxSarCfg ;

                 /* Disable Bonding on all ports */
                 i |= TXSARA_BOND_PORT_DIS_MASK ;
                 /* Enable bonding on ports 0 & 1 - used for now */
                 i &= ~(0x3 << TXSARA_BOND_PORT_DIS_SHIFT) ;
                 ...
                 XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: ATM Bonding enabled ");
         }
         break;
      ...
      default                        :
         ...
         if((ulTrafficType & TRAFFIC_TYPE_ATM_MASK) == 0) {
          ...
          /* Check TxPAF status */
          m_ulTxPafEnabled = TXPAF_REGS->ulControl & TXPAF_PROCESSOR_CONTROL_ENABLE_MASK ;
          XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: TxPAF Status = %s ", (m_ulTxPafEnabled==1) ? "Enabled" : "Disabled") ;
#else
          m_ulTxPafEnabled = 0 ;
#endif

#if defined (CONFIG_BCM963146)
          if (ulTrafficType == TRAFFIC_TYPE_NOT_CONNECTED)
            XP_REGS->ulSsForceVcArbRR &= ~SST_USE_FIFO_EMPTY_ONLY ;
          else {
             XP_REGS->ulSsForceVcArbRR |= SST_USE_FIFO_EMPTY_ONLY ;
         ...
         XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: Normal(XTM/PTM) Mode enabled ");
         break ;
   } /* switch (ulTrafficType) */
...
BCMXTM_STATUS XTM_PROCESSOR::ReInitialize (UINT32 ulTrafficType)
{
   ...
   if(m_pfnXtmrtReq != NULL) {
      ...
      if (ret != 0)
      {
         XtmOsPrintf(XTM_LOG_ERR,"xtmrt initialization failed with %d",ret);
         bxStatus = XTMSTS_STATE_ERROR ;
      }
   } else {
      XtmOsPrintf(XTM_LOG_ERR,"m_pfnXtmrtReq is NULL");
      bxStatus = XTMSTS_STATE_ERROR;
   }
...
BCMXTM_STATUS XTM_PROCESSOR::Configure (PXTM_CONFIGURATION_PARMS pConfigInfo)
{
   BCMXTM_STATUS bxStatus = XTMSTS_SUCCESS ;
    ...
    if(pConfigInfo->sParamsSelected.trafficParam==XTM_CONFIGURATION_PARM_SET)
    {
        ...
        XtmOsPrintf(XTM_LOG_NOT,"  Setting traffic timeout parameter to %lu mS.  Was %lu mS.", 
    ...
    if(pConfigInfo->sParamsSelected.trafficParam==XTM_CONFIGURATION_PARM_DUMP)
    {
        ...
        XtmOsPrintf(XTM_LOG_NOT,"  Existing traffic timeout parameter is %lu mS.", 
    ...
    if(pConfigInfo->sParamsSelected.singleLineParam==XTM_CONFIGURATION_PARM_SET)
    {
        ...
        /* Tell the user what we're doing */
        XtmOsPrintf(XTM_LOG_NOT,"  Setting single line timeout parameter to %lu mS.  Was %lu mS.", 
    ...
    {
        /* Tell the user the value of the parameter */
        XtmOsPrintf(XTM_LOG_NOT,"  Existing single line timeout parameter is %lu mS.", 
...
BCMXTM_STATUS XTM_PROCESSOR::validateThreshold (UINT32 ulThreshold)
{
   ...
   {
      /* Tell the user what we're doing */
      XtmOsPrintf(XTM_LOG_INF,"  Invalid XTM Mode Q threshold %d ", ulThreshold) ;
...
BCMXTM_STATUS XTM_PROCESSOR::ManageThreshold (PXTM_THRESHOLD_PARMS pThreshold)
{
   ...
   if (pThreshold->sParams.adslParam == XTM_THRESHOLD_PARM_SET) {
      ...
      if (bxStatus == XTMSTS_SUCCESS) {
         ...
         XtmOsPrintf(XTM_LOG_NOT,"  Setting ADSL threshold to %d ", ulThreshold) ;
   ...
   {
      /* Tell the user the value of the parameter */
      XtmOsPrintf(XTM_LOG_NOT,"Existing ADSL threshold parameter is %lu.",
   ...
   if (pThreshold->sParams.vdslParam == XTM_THRESHOLD_PARM_SET) {
      ...
      if (bxStatus == XTMSTS_SUCCESS) {
         ...
         XtmOsPrintf(XTM_LOG_NOT,"  Setting VDSL threshold to %d ", ulThreshold) ;
   ...
   {
      /* Tell the user the value of the parameter */
      XtmOsPrintf(XTM_LOG_NOT,"Existing VDSL threshold parameter is %lu.",
   ...
   if (pThreshold->sParams.vdslRtxParam == XTM_THRESHOLD_PARM_SET) {
      ...
      if (bxStatus == XTMSTS_SUCCESS) {
         ...
         XtmOsPrintf(XTM_LOG_NOT,"  Setting VDSL RTX threshold to %d ", ulThreshold) ;
   ...
   {
      /* Tell the user the value of the parameter */
      XtmOsPrintf(XTM_LOG_NOT,"Existing VDSL RTX threshold parameter is %lu.",
   ...
   if (pThreshold->sParams.gfastParam == XTM_THRESHOLD_PARM_SET) {
      ...
      if (bxStatus == XTMSTS_SUCCESS) {
         ...
         XtmOsPrintf(XTM_LOG_NOT,"  Setting GFAST threshold to %d ", ulThreshold) ;
   ...
   {
      /* Tell the user the value of the parameter */
      XtmOsPrintf(XTM_LOG_NOT,"Existing GFAST threshold parameter is %lu.",
    ...
    pTrafficDescrTable, UINT32 *pulTrafficDescrTableSize)
{
    ...
    if( *pulTrafficDescrTableSize >= m_ulNumTrafficDescrEntries )
    {
        ...
        {
            XtmOsMemCpy(pTrafficDescrTable, m_pTrafficDescrEntries,
    ...
    pTrafficDescrTable, UINT32  ulTrafficDescrTableSize)
{
    ...
    if( ulTrafficDescrTableSize )
    {
        ...
        if( m_pTrafficDescrEntries )
        {
            ...
            XtmOsMemCpy( m_pTrafficDescrEntries, pTrafficDescrTable, ulSize );
...
void XTM_PROCESSOR::UpdateConnAddrForBonding ( PXTM_ADDR pConnAddr, UINT32 operation )
{
   ...
   XtmOsPrintf(XTM_LOG_INF,"ptmBond:%d atmBond:%d \n", m_InitParms.bondConfig.sConfig.ptmBond, 
                                       m_InitParms.bondConfig.sConfig.atmBond) ;
   if (( m_InitParms.bondConfig.sConfig.ptmBond == BC_PTM_BONDING_ENABLE) ||
       ( m_InitParms.bondConfig.sConfig.atmBond == BC_ATM_BONDING_ENABLE)) {
      for (ulPortId = PORT_PHY0_PATH0 ; ulPortId <= PORT_PHY1_PATH1 ; ulPortId <<= 0x1) {
         XtmOsPrintf(XTM_LOG_INF,"XTM: portMask = %lx ", *pulPortMask) ;
         if (*pulPortMask & ulPortId) {
            ulPort = PORTID_TO_PORT (ulPortId) ;
            if ( ulPort >= MAX_INTERFACES )
               continue;
            ulBondingPort = m_Interfaces[ulPort].GetBondingPortNum () ;
            XtmOsPrintf(XTM_LOG_INF,"XTM: ulBondingPort = %lx ", ulBondingPort) ;
            if (ulBondingPort != MAX_INTERFACES) {
      ...
      *pulPortMask = PORT_TO_PORTID(m_ulTrafficMonitorPort) ;
   }
   XtmOsPrintf(XTM_LOG_INF,"XTM: portMask = %lx ulTrafficType = %lx", *pulPortMask, *pulTrafficType) ;
}
      ...
      if (pLi->ulLinkState == LINK_UP) {
         ...
         if (m_Interfaces[ulInterfaceId].GetLinkErrorStatus (&ulUsStatus, &ulDsStatus) != XTMSTS_SUCCESS) {
            ...
            if (m_Interfaces[ulInterfaceId].GetPortDataStatus () == DATA_STATUS_ENABLED) {
               ...
               XtmOsMemSet (&Tpdsc, 0, sizeof (Tpdsc)) ;
...
void XTM_PROCESSOR::XtmMonitorTimerCb (void *ulParm)
{
   ...
   //XtmOsPrintf(XTM_LOG_DBG,"XTM: pBondTmrParm = %lx ", pXtmProcessor) ;
   ...
   if (pXtmProcessor->m_ulPrevRelatedUpEvtValid == 0) {
      for (ulPort=0; ulPort<MAX_BONDED_LINES; ulPort++) {
         if (pXtmProcessor->m_ulPendingEvtValid[ulPort]) {
            XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg:Pend Evt Svc Port-%d, PortId-%d, LI-%d",
   ...
   if ((pXtmProcessor->m_ulTrafficMonitorPort != MAX_INTERFACES) && ((pXtmProcessor->m_ulTrafficType == TRAFFIC_TYPE_NOT_CONNECTED) || (pXtmProcessor->m_ulTrafficType & TRAFFIC_TYPE_ATM_MASK) )) {
      ...
      if(pXtmProcessor->m_XtmAutoSense.PhyChangePending() && pXtmProcessor->m_XtmAutoSense.PhyStatus() != PHYSTATUS_UNKNOWN) {
         ...
         if(pXtmProcessor->m_XtmAutoSense.PhyStatus() == PHYSTATUS_ATM_SINGLELINE) {
            XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Changing over to Single Line PHY") ;
            ...
            // We want Bonding PHY.
            XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Changing over to Bonded PHY") ;
         ...
         XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Traffic incompatibility between CO & CPE. Reconfiguring the system. ") ;

         // Set the traffic type
         if (pXtmProcessor->ReInitialize  (ulTrafficType) != XTMSTS_SUCCESS)
            XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Fatal - ReInitialize XTM part due to Traffic type change has failed (traffic type=%lu) ", ulTrafficType) ;

         pXtmProcessor->UpdateConnAddrs (ulConnAddrAction) ;

         XtmOsPrintf(XTM_LOG_DBG,"ReconfigureSAR and Update Interface for m_ulTrafficMonitorPort[%u]",pXtmProcessor->m_ulTrafficMonitorPort);
         pXtmProcessor->ReconfigureSAR  (pXtmProcessor->m_ulTrafficMonitorPort, ulTrafficType, 0) ; //Not a SRA event here.
         ...
         if ((pXtmProcessor->m_ulTrafficMonitorPort == PHY_PORTID_1) && (ulTrafficType == TRAFFIC_TYPE_ATM)) {
            ...
            idx = 0;
            XtmOsMemCpy(&Li,pLi,sizeof(XTM_INTERFACE_LINK_INFO));
            while( (pConn = pXtmProcessor->m_ConnTable.Enum( &idx )) != NULL ) {
               ...
               **/
               XtmOsPrintf(XTM_LOG_DBG,"Updating Connection Link Information for m_ulTrafficMonitorPort[%u]",pXtmProcessor->m_ulTrafficMonitorPort);
               if (pConn->SetLinkInfo( (ulPortMask & PORT_TO_PORTID(pXtmProcessor->m_ulTrafficMonitorPort)),
                  ...
                  XtmOsPrintf(XTM_LOG_ERR,"Failed to reprogram the connection for this Non-Bonded mode on Port1");
               }
            }
         }
         ...
         if (g_pfnAdslSetObjValue != NULL) {
            ret = g_pfnAdslSetObjValue (pXtmProcessor->m_ulTrafficMonitorPort, dslSysCtlCommand, sizeof (dslSysCtlCommand),
                  NULL, 0) ;
            if (ret != 0) {
               XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: dslSysCtl command for type of the PHY Failed. ") ;
            }
         }
      ...
      if (pXtmProcessor->m_ulRxPafLockupMonitorEvtValid == 0) {
         ...
         if (((rxBondErrStatus & RB_ERR_STATUS_PAF_BAD_FRAG) != 0) &&
            ...
            if ((XP_REGS->ulRxBondExpectedSid0 == pXtmProcessor->m_ulRxBondPrevExpSid01) &&
               ...
               XtmOsPrintf(XTM_LOG_DBG,"Flushing RxPaf Channels.... ") ;
            ...
            {
               XtmOsPrintf(XTM_LOG_INF,"rxBondErrStatus:%08x",rxBondErrStatus);
    ...
    PXTM_CONN_CFG pConnCfg )
{
    ...
    if( pConn )
    {
        ...
        XtmOsPrintf(XTM_LOG_NOT, "XTM connection NOT found \n") ;
    ...
    PXTM_CONN_CFG pConnCfg )
{
    ...
    XtmOsPrintf(XTM_LOG_DBG,"ConnCfgEventValid[1]");

    logicalPort = PORTMAP_TO_LOGICALPORT(pConnAddr->u.Conn.ulPortMask);
    XtmOsPrintf(XTM_LOG_INF,"Logical portmask[%u] Logical PortId[%u]",pConnAddr->u.Conn.ulPortMask,logicalPort);
    ...
    if ((pConnCfg) && (m_LogPhyPortmap[logicalPort].PhysicalPortId == PORT_PHY_INVALID)) {
       ...
       /* Only possible case for the invalid traffic monitor port is when,
       ** 1) Conn cfg occurs prior to DSL link activity
       ** 2) DSL is non-existing and we are exercising XTM/SAR only.
       */
       m_ulTrafficMonitorPort = (m_ulTrafficMonitorPort == MAX_INTERFACES) ? PHY_PORTID_0 : m_ulTrafficMonitorPort ;
       XtmOsPrintf(XTM_LOG_INF,"SetConnCfg:m_ulTrafficMonitorPort[%u]",m_ulTrafficMonitorPort);
...
#if defined(XTM_PORT_SHAPING) && defined(CONFIG_BCM963158)
    if (((m_ulTrafficType == TRAFFIC_TYPE_PTM) || (m_ulTrafficType == TRAFFIC_TYPE_PTM_RAW) 
             || (m_ulTrafficType == TRAFFIC_TYPE_PTM_BONDED)))
    ...
    if( pConn == NULL )
    {
        ...
        if( pConn != NULL )
        {
            XtmOsPrintf(XTM_LOG_DBG,"Created new connection");
            ...
            {
                XtmOsPrintf(XTM_LOG_ERR,"Failed to add this connection to the table");
                // delete pConn;
                pConn->Dxtor();
                XtmOsFree((char *)pConn);
    ...
    UpdateConnAddrForBonding ( pConnAddr, XTM_REMOVE ) ;
    XtmOsPrintf(XTM_LOG_DBG,"m_ulConnCfgEventValid[0]");
    return( bxStatus );
} /* SetConnCfg */
    ...
    UINT32 *pulNumConns )
{
    ...
    while( (pConn = m_ConnTable.Enum( &i )) != NULL )
    {
        if( ulOutNum < ulInNum )
        {
            ...
            XtmOsMemCpy(&pConnAddrs[ulOutNum++], &Addr, sizeof(Addr));
        }
        else
        {
            bxStatus = XTMSTS_PARAMETER_ERROR;
...
void XTM_PROCESSOR::UpdateConnAddrs( UINT32 ulConnAddrAction )
{
    ...
    XtmOsPrintf (XTM_LOG_INF, "In UpdateConnAddrs Action:%u \n",ulConnAddrAction) ;

    while( (pConn = m_ConnTable.Enum( &i )) != NULL )
    {
       XtmOsPrintf (XTM_LOG_INF, "connection not null \n") ;
       pConn->GetAddr( &Addr );
       XtmOsPrintf (XTM_LOG_INF, "ulTrafficType:%x ulPortMask:%x usVpi:%u usVci:%u",Addr.ulTrafficType,
                                                                                    Addr.u.Conn.ulPortMask,
                                                                                    Addr.u.Vcc.usVpi,
                                                                                    Addr.u.Vcc.usVci) ;
    ...
    PXTM_INTERFACE_STATS pIntfStats, UINT32 ulReset )
{
    ...
    PXTM_INTERFACE_LINK_INFO pBondLinkInfo = NULL;
    XTM_INTERFACE_STATS bondIntfStats = {};

    XtmOsMemSet (pIntfStats, 0, sizeof (XTM_INTERFACE_STATS)) ;
    if( ulPort < MAX_INTERFACES ) {
        pLinkInfo = m_Interfaces[ulPort].GetLinkInfo();
        if ((pLinkInfo != NULL) && (pLinkInfo->ulLinkState == LINK_UP))
        {
            bxStatus = m_Interfaces[ulPort].GetStats( pIntfStats, ulReset );
            //For Bonding all TX traffic goes through the PORT0 even when
            //line0 state is LINK_DOWN. So we still need to read its counters
            //to get TX stats.
            if (((m_ulTrafficType == TRAFFIC_TYPE_PTM_BONDED) || (m_ulTrafficType == TRAFFIC_TYPE_ATM_BONDED))
                  && (ulPort == PHY_PORTID_1))
            {
                ulBondingPort = m_Interfaces[ulPort].GetBondingPortNum();
                pBondLinkInfo = m_Interfaces[ulBondingPort].GetLinkInfo();
                if ((pBondLinkInfo != NULL) && (pBondLinkInfo->ulLinkState == LINK_DOWN))
                {
                    bxStatus = m_Interfaces[ulBondingPort].GetStats( &bondIntfStats, ulReset );
                    pIntfStats->ulIfOutPackets += bondIntfStats.ulIfOutPackets;
                    pIntfStats->ulIfOutOctets  += bondIntfStats.ulIfOutOctets;
                }
            }
        }
    }
...
BCMXTM_STATUS XTM_PROCESSOR::GetErrorStatistics( PXTM_ERROR_STATS pErrStats )
{
    ...
    if(pErrStats != NULL)
    {
        ...
        XtmOsMemSet (pErrStats, 0, sizeof (XTM_ERROR_STATS)) ;
...
{
   XtmOsPrintf (XTM_LOG_DBG, "Updata Link Mode Ginp %d, DSL %d \n", ulDSLGinpMode, ulDSLMode) ;

   if (ulDSLMode < kVdslModVdsl2) {
      m_ulXTMLinkMode = XTM_LINK_MODE_ADSL ;
      XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: XTM Link Mode is ADSL. ") ;
   }
   else if (ulDSLMode == kXdslModGfast) {
      m_ulXTMLinkMode = XTM_LINK_MODE_GFAST ;
      XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: XTM Link Mode is G.Fast ") ;
   }
   else {
      /* It should be VDSL based link */
      if (ulDSLGinpMode) {
         m_ulXTMLinkMode =  XTM_LINK_MODE_VDSL_RTX ;
         XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: XTM Link Mode is VDSL G.inpRtx ") ;
      }
      else {
         m_ulXTMLinkMode = XTM_LINK_MODE_VDSL ;
         XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: XTM Link Mode is VDSL ") ;
      }
   }
} /* UpdateLinkMode */
...
void XTM_PROCESSOR::UpdateLinkStatus ( UINT32 ulPhysPort,  PXTM_INTERFACE_LINK_INFO pLinkInfo )
{
   ...
   if (pLinkInfo->ulLinkState == LINK_UP) {
      ...
      if (g_pfnAdslGetObjValue != NULL) {
         ...
         XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ULS Port %d ", ulPhysPort) ;
         ret  = g_pfnAdslGetObjValue (ulPhysPort, NULL, 0, (char *) &PhyMibInfo, &size) ;
         if (ret != 0)
            XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: SILI:Error g_pfnAdslGetObjValue port - %d return value - %d ", ulPhysPort, ret) ;
         else {
         ...
         XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ULS Port %d Setting link mode to vdsl", ulPhysPort) ;
         UpdateLinkMode (0, kVdslModVdsl2) ;   /* Set to VDSL default */
      }
   } /* Link Status */
}
    ...
    PXTM_INTERFACE_LINK_INFO pLinkInfo )
{
   ...
   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: AutoSensePreProcess Port=%d, Traffic type=%d State=%d ", (UINT32) ulPhysPort, (UINT32) ulTrafficType, (UINT32) pLinkInfo->ulLinkState) ;
   switch(ulTrafficType) {
      ...
      case TRAFFIC_TYPE_ATM_TEQ: {
         ...
         if (pLinkInfo->ulLinkState == LINK_DOWN) {
            ...
            if (g_pfnAdslGetObjValue != NULL) {
               ...
               ret  = g_pfnAdslGetObjValue (ulPhysPort, NULL, 0, (char *) &PhyMibInfo, &size) ;
               if (ret != 0)
                  XtmOsPrintf(XTM_LOG_ERR,"Xtm SILI:Error g_pfnAdslGetObjValue port - %d return value - %d ", ulPhysPort, ret) ;
               else
            ...
            if (m_ulXTMNitroMode == XTM_MODE_NITRO_ENABLE) {
               XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ATM Nitro Mode enabled. ") ;
               m_XtmAutoSense.ForceSingleLinePhy() ;
            }
            else {
               XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Port %d State = %d ", (UINT32) ulPhysPort, (UINT32) pLinkInfo->ulLinkState) ;
 ...
 * Description  : Called when a DSL connection has come up or gone down.
 * Returns      : XTMSTS_SUCCESS if successful or error status.
 ***************************************************************************/
...
void XTM_PROCESSOR::UpdateSitSlr(UINT32 ulPort)
{
   ...
   XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: Port-%d, PortShaping-%d, QShaping-%d, TxPAF-%d ", ulPort,
   ...
   if( (m_ulPortShapingConfig == PORT_Q_SHAPING_ON) ||
      ...
      XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: UnConfigure Max UT port shaping controls. %d", ulPort) ;
      XP_REGS->ulTxLineRateTimer = m_ulSlrCnt;
      XP_REGS->ulSstSitValue     = m_ulSitCnt;
      if (m_ulPortShapingConfig != PORT_Q_SHAPING_ON)
          m_Interfaces[ulPort].UnConfigureMaxUTPortShaping() ;
   }
   else
   {
      XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: Re-Configure Max UT port shaping controls. %d", ulPort) ;
      m_Interfaces[ulPort].ConfigureMaxUTPortShaping(m_ulTrafficType) ;
   }
}
#endif /* 158 */
...
void XTM_PROCESSOR::ConfigureTxPortShapingRatios(UINT32 ulPhysPort, UINT32 ulTrafficType)
{
   ...
   XtmOsPrintf(XTM_LOG_INF,"ulTrafficType:%u ",ulTrafficType);
   ...
   if((ulTrafficType == TRAFFIC_TYPE_PTM_BONDED)) {
      ...
      XtmOsPrintf(XTM_LOG_INF,"ulPhysPort:%u ulBondingPhysPort:%u ",ulPhysPort,ulBondingPhysPort);
      XtmOsPrintf(XTM_LOG_INF,"ulPhysPort LS:%u ulBondingPhysPort LS:%u ",pLinkInfo->ulLinkState
      ...
      {
         XtmOsPrintf(XTM_LOG_INF,"Both links are down disabling TX port shaping");
         m_Interfaces[PHY_PORTID_0].DisableTxPortShaping();
      }
   }
   else {
      // other cases. ATM cases to be looked at.
      if(pLinkInfo->ulLinkState == LINK_UP)
      {
...
{
   UINT32 ulSRAEvent = FALSE, timerStopped = 1 ;
   BCMXTM_STATUS bxStatus = XTMSTS_SUCCESS;
   ...
   XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: P=%d,S=%d,T=%d",  ulPhysPort, pLinkInfo->ulLinkState, ulTrafficType) ;
   ...
   if (pLinkInfo->ulLinkState == LINK_UP) {
      if ((timerStopped) && (m_ulPrevRelatedUpEvtValid == 0)) {
         ...
         XtmOsMemCpy (&m_PendingEvtLinkInfo[ulPhysPort], pLinkInfo, sizeof (XTM_INTERFACE_LINK_INFO)) ;
         XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: PendEvt1 LI, P=%d, S=%d ", ulPhysPort, pLinkInfo->ulLinkState) ;
         goto _End ;
      }
   }
   else {
      if ((!timerStopped) || (m_ulPendingEvtValid[ulPhysPort] == 1)) {
         ...
         XtmOsMemCpy (&m_PendingEvtLinkInfo[ulPhysPort], pLinkInfo, sizeof (XTM_INTERFACE_LINK_INFO)) ;
         XtmOsPrintf(XTM_LOG_INF,"bcmxtmcfg: PendEvt2 LI, P=%d, S=%d ", ulPhysPort, pLinkInfo->ulLinkState) ;
         goto _End ;
      }
      else {
         m_ulPrevRelatedUpEvtValid = 0;
   ...
   if (pLinkInfo->ulLinkState == LINK_DOWN) {
        ...
        if ((m_InitParms.bondConfig.sConfig.autoSenseAtm == BC_ATM_AUTO_SENSE_ENABLE)
           ...
           **/
           if (ulPhysPort == PHY_PORTID_1) {
               XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Link down on port %d ", ulPhysPort) ;
               ...
               XtmOsPrintf(XTM_LOG_INF,"ulTrafficMonitorPort:%u",m_ulTrafficMonitorPort);
               goto _End ;
           }
            }
   }

    XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg:ulPhysPort[%x] m_ulTrafficMonitorPort[%x]",ulPhysPort,m_ulTrafficMonitorPort);
    /* Is this ATM traffic? */
    if ((ulTrafficType == TRAFFIC_TYPE_ATM) || (ulTrafficType == TRAFFIC_TYPE_ATM_BONDED)) {
        ...
        if (m_InitParms.bondConfig.sConfig.autoSenseAtm == BC_ATM_AUTO_SENSE_ENABLE) {
           ...
           XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: AutoSenseATM is not enabled. Link Traffic type from PHY=%lu ",
                      ulTrafficType) ;
           // Correct the port number to the physical port
           m_ulTrafficMonitorPort = ulPhysPort;
        }
   ...
   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: XTM LinkInfo. P-%d, S-%s(%d), Service =%s(%d) m_ulTrafficMonitorPort[%u]",
                ulPhysPort,
   ...
   if ((pCurrLinkInfo) && (pCurrLinkInfo->ulLinkState == pLinkInfo->ulLinkState)) /* duplicate */ {
      if (pLinkInfo->ulLinkState == LINK_UP) {
         ulSRAEvent = TRUE ;
         XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: XTM Link Information, port = %d, Line UP SRA Event from PHY ", ulPhysPort) ;
      }
      else {
         XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: XTM Link Information, port = %d, Duplicate Event from PHY ", ulPhysPort) ;
         goto _End ;
      }
   }
   ...
   if ((pLinkInfo->ulLinkState == LINK_UP) &&
      ...
      if (CheckTrafficCompatibility (ulTrafficType) != XTMSTS_SUCCESS) {
         ...
         if (ReInitialize  (ulTrafficType) != XTMSTS_SUCCESS)
            XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Fatal - ReInitialize XTM part due to Traffic type change has failed ") ;
      }
      ...
      XtmOsPrintf(XTM_LOG_DBG,"SILI:ReconfigureSAR ulPhysPort[%u]",ulPhysPort);
      ReconfigureSAR  (ulPhysPort, ulTrafficType, ulSRAEvent) ;
   }
   ...
   switch( pLinkInfo->ulLinkState )
   {
...

         {
            ...
            if ((ulTrafficType & TRAFFIC_TYPE_ATM_MASK) == TRAFFIC_TYPE_ATM)
            {
               ...
               ulLinkRateUnitsPerSec = (pOtherLinkInfo) ? 
                  XTMCFG_DIV_ROUND((pLinkInfo->ulLinkUsRate+pOtherLinkInfo->ulLinkUsRate),ulBitsPerCell) :
                  XTMCFG_DIV_ROUND(pLinkInfo->ulLinkUsRate,ulBitsPerCell);

               /* SLR_CNT is the number of SAR clocks per cell */
                    ulSlrCnt = (ulLinkRateUnitsPerSec != 0) ? XTMCFG_DIV_ROUND(SAR_CLOCK,ulLinkRateUnitsPerSec) : 0;

               /* Scale SLR_CNT for a faster rate */
               ulSlrCnt = XTMCFG_DIV_ROUND((ulSlrCnt * 8),10);
            }
            else
            {
               ...
               ulLinkRateUnitsPerSec = (pOtherLinkInfo) ? 
                  XTMCFG_DIV_ROUND((pLinkInfo->ulLinkUsRate+pOtherLinkInfo->ulLinkUsRate),ulBitsPerPacket) :
                  XTMCFG_DIV_ROUND(pLinkInfo->ulLinkUsRate,ulBitsPerPacket);
               ...
               /* Scale SLR_CNT for a faster rate */
               ulSlrCnt = XTMCFG_DIV_ROUND(ulSlrCnt,4);
#if defined(CONFIG_BCM963158) || defined(CONFIG_BCM963178) || defined(CONFIG_BCM963146)
             ...
             * Set it to 2 us.
             */
            ulSitCnt = XTMCFG_DIV_ROUND((SAR_CLOCK*2),1000000L) ;
...
#if defined(XTM_PORT_SHAPING)
            XtmOsPrintf(XTM_LOG_DBG,"Interface SetSitUt ulPhysPort[%u]",ulPhysPort);
            ...
            {
               {
                  ...
                  XtmOsPrintf(XTM_LOG_DBG,"ulLinkState[%u]",pLinkInfo->ulLinkState);
                  //FIXME: We have modified the following register writes to
                  //fix an issue noticed with ARM64 bit platform and 4.19
                  //builds. Issue is that with new compiler 8.2 gcc we saw the
                  //below SIT/SITLO register is converted to a 64 bit register.
                  XP_REGS->ulSstSitValue     = (UINT32)ulSitCnt;
                  XP_REGS->ulTxLineRateTimer = ulSlrCnt;
                  XP_REGS->ulSstSitSlowValue = ulSitLoCnt;
                  m_ulSlrCnt = ulSlrCnt;
                  m_ulSitCnt = ulSitCnt;
                  m_ulSitLoCnt = ulSitLoCnt;
                  XtmOsPrintf(XTM_LOG_INF,"m_ulXtmLinkMode[%u]",m_ulXTMLinkMode);
#if defined(CONFIG_BCM963158)
#if (CONFIG_BRCM_CHIP_REV != 0x63158A0)
                  if (m_ulXTMLinkMode == XTM_LINK_MODE_GFAST) {
            ...
            if( ulPhysPort < MAX_INTERFACES )
            {
               ...
               XtmOsPrintf(XTM_LOG_DBG,"LINK_UP ulPhysPort[%u]",ulPhysPort);
               ...
               XtmOsPrintf(XTM_LOG_DBG,"ulLinkMask[%08x]",ulLinkMask);
               ...
               while( (pConn = m_ConnTable.Enum( &i )) != NULL )
               {
                  ...
                  if (ulDoConnSetLinkInfo)
                  {
                     ...
                     // No need to update connection configuration if the event
                     // is an SRA event. Connection configuration pertains to
                     // existing configuration which is unaffected with an SRA
                     // event.
                     if (ulSRAEvent == FALSE) {
                        ...
                        if((bxStatus == XTMSTS_SUCCESS) && 
                           ...
                           (ulBondingPhysPort != MAX_INTERFACES)) {
                           ...
                           if((bxStatus = pConn->SetCfg(&ConnCfg, m_ulXTMLinkMode, m_ulXTMNitroMode))!= XTMSTS_SUCCESS)
                              XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Failed to reconfigure the connection configuration for bonded case and PORT 0 LINK up status=%d ", bxStatus);
                        }
                     } //if(!SRA)

                     XtmOsPrintf(XTM_LOG_DBG,"Calling SetLinkInfo 1");
                     ...
                     if( bxStatus == XTMSTS_IN_USE)
                     {
                         ...
                         m_ulPendingEvtPortId[ulPhysPort] = ulPortId ;
                         XtmOsMemCpy (&m_PendingEvtLinkInfo[ulPhysPort], pLinkInfo, sizeof (XTM_INTERFACE_LINK_INFO)) ;
                         XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: PendEvt3 LI, P=%d, S=%d ", ulPhysPort, pLinkInfo->ulLinkState) ;
                         goto _End ;
                     }
                     else if( bxStatus != XTMSTS_SUCCESS )
                     {
                        XtmOsPrintf(XTM_LOG_ERR,"Connection SetLinkInfo failed status=%d", bxStatus);
                        break;
                     }
                  }
               } // while (conn is valid)
...
#if defined(XTM_PORT_SHAPING)
               XtmOsPrintf(XTM_LOG_DBG,"Calling Port Shaping ");
               ...
               if( pLinkInfo->ulLinkState == LINK_DOWN ) {
                  ...
                  if (!GetAvailLinkStatus ()) { /* No links are available/UP */
                     ...
                     XP_REGS->ulTxLineRateTimer = 0;
                     XtmOsPrintf(XTM_LOG_DBG,"SILI: LINK_DOWN ReconfigureSAR ulPhysPort[%u]",ulPhysPort);
                     ReconfigureSAR  (ulPhysPort, TRAFFIC_TYPE_NOT_CONNECTED, ulSRAEvent);
                     m_ulTrafficMonitorPort = MAX_INTERFACES ;
                     ...
                     if (m_ulTrafficMonitorPort == ulPhysPort)
                        m_ulTrafficMonitorPort = ulBondingPhysPort ;
                  }
               } /* (if Link is DOWN) */
      ...
      case LINK_START_TEQ_DATA:
         ...
         XP_REGS->ulRxPafVcid &= ~(RXPAF_DEFAULT_VCID<<(ulPhysPort*RXPAF_VCID_SHIFT)) ;
...
_End :
   ...
   XtmOsPrintf(XTM_LOG_DBG,"SILI done port=%d ", ulPhysPort) ;
    ...
    PXTM_OAM_CELL_INFO pOamCellInfo )
{
    ...
    XtmOsMemCpy (&internalConnAddr, pConnAddr, sizeof (XTM_ADDR)) ;
                                                  ...
                                                  char *pszNetworkDeviceName )
{
    ...
    XtmOsPrintf(XTM_LOG_INF,"portmask[%u] ulLogicalPort[%u]",pConnAddr->u.Conn.ulPortMask,ulLogicalPort);
    ...
    XtmOsPrintf(XTM_LOG_INF,"m_LogPhyPortmap[%u] updated portmask[%u]",(m_LogPhyPortmap[ulLogicalPort].PhysicalPortId),
    ...
    if( pConn )
    {
        ...
        XtmOsMemSet (&Li, 0, sizeof (XTM_INTERFACE_LINK_INFO)) ;
        ...
        if (ulLinkMask != 0)
        {
           ...
           XtmOsPrintf(XTM_LOG_DBG,"LINK_UP Event");
    ...
    /* setup links with default rates, when,
     * 1. Links are external. (or)
     * 2. no DSL is built in.
     */
    {
...
#if !defined(CONFIG_BCM_55153_DPU)
       if (
           (!g_pfnAdslSetObjValue)    /* DSL is inactive  - Either compiled out (or) Not initialized */
                   ||
          /* All external ports */
           (
            (bxStatus == XTMSTS_SUCCESS && ulLinkState == LINK_DOWN) 
                                           &&
          ...
          )
          {
             ...
             for( ulPort = PHY_PORTID_0; ulPort < MAX_INTERFACES; ulPort++ )
             {
                if( ulPortMask & PORT_TO_PORTID(ulPort) )
                {
                   XtmOsPrintf(XTM_LOG_DBG,"CreateNetworkDevice: Force link up, port=%lu, traffictype=%lu",
...
BCMXTM_STATUS XTM_PROCESSOR::DeleteNetworkDevice( PXTM_ADDR pConnAddr )
{
    ...
    XtmOsPrintf(XTM_LOG_DBG,"xtm: DeleteNetworkDevice....") ;
    XtmOsPrintf(XTM_LOG_INF,"portmask[%u]",pConnAddr->u.Conn.ulPortMask);
    //Here incoming portmask is based on logical port map. Convert this to physical port map now.
    ...
    XtmOsPrintf(XTM_LOG_INF,"m_LogPhyPortmap[%u] updated portmask[%u]",(m_LogPhyPortmap[PORTID_TO_PORT(pConnAddr->u.Conn.ulPortMask)].PhysicalPortId),
    ...
    if( pConn )
    {
        //XtmOsPrintf(XTM_LOG_DBG,"xtm: conn->DeleteNetworkDevice....") ;
    ...
    UpdateConnAddrForBonding ( pConnAddr, XTM_REMOVE ) ;
    //XtmOsPrintf(XTM_LOG_DBG,"xtm: DeleteNetworkDevice Done....") ;
    return( bxStatus );
} /* DeleteNetworkDevice */
...
BCMXTM_STATUS XTM_PROCESSOR::GetBondingInfo ( PXTM_BOND_INFO pBondInfo )
{
     ...
     */
    XtmOsMemSet (pBondInfo, 0, sizeof (XTM_BOND_INFO)) ;
             ...
             PXTM_INTERFACE_LINK_INFO pOtherLinkInfo)
{
   ...
   XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: ChkSILI Port = %x, BondingPort = %x ", ulPort, ulBondingPort) ;
    ...
    PXTM_INTERFACE_LINK_INFO pLinkInfo )
{
   ...
   if( pLinkInfo->ulLinkState == LINK_DOWN ) {
      ...
      if (!GetAvailLinkStatus ()) { /* No links are available/UP */
         ...
         pmc_sar_soft_reset();
#elif defined(CONFIG_BCM963158) || defined(CONFIG_BCM963146) || defined(CONFIG_BCM963178)

#if defined(CONFIG_BCM963146)
   XtmOsPrintf(XTM_LOG_NOT,"Flushing SAR Rx \n");
   XP_REGS->ulSoftReset |= RX_FLUSH_REQ;
   while (1) {
      for (waitCnt=100; waitCnt>0; waitCnt--)
         asm ("nop") ;
      softReset = XP_REGS->ulSoftReset ;
      if (softReset & RX_FLUSH_DONE)
         break;
   }
   XP_REGS->ulSoftReset &= ~(RX_FLUSH_REQ | RX_FLUSH_DONE);
#endif
