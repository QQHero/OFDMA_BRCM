...
#define XTMAUTOSENSE_ONELINETIMEOUTMS      40000     // Timeout to determine if only one line is configured.
                                                     // SWBCACPE-41300. Under some circumstances,
                                                     // 63146 takes more time to signel useful
                                                     // traffic type to aid with faster auto sensing.
#else
...
BCMXTM_STATUS XTM_AUTOSENSE::Initialize( PXTM_INITIALIZATION_PARMS pInitParms ) {
   ...
   XtmOsPrintf(XTM_LOG_DBG,"[XTM]m_eCurrentState = %d", m_eCurrentState) ;
...
void XTM_AUTOSENSE::TimerUpdate() {
    ...
    //timerStrike++ ;
    //if (!(timerStrike % 1000))
       //XtmOsPrintf(XTM_LOG_DBG,"TimerUpdate %d ", timerStrike) ;
...
int XTM_AUTOSENSE::LineActive(UINT32 ulPhysPort) {
    XtmOsPrintf(XTM_LOG_DBG," ulPhysPort=%lu, lineActivity[] = [%d,%d]",ulPhysPort, m_pbLineActivityDetected[0], m_pbLineActivityDetected[1]);
    ...
    if(ulPhysPort < MAX_BONDED_LINES) {
        ...
        XtmOsPrintf(XTM_LOG_ERR,"Invalid port ID passed - ulPhysPort=%lu", ulPhysPort);
        return(-1);
    }
}
...
int XTM_AUTOSENSE::LineIdle(UINT32 ulPhysPort) {
    XtmOsPrintf(XTM_LOG_DBG,"ulPhysPort=%lu", ulPhysPort);
...
int XTM_AUTOSENSE::Transition(XtmAutoSenseTransition eTransition) {
   ...
   case TRANSITION_ONE_LINE_NONIDLE:
       ...
       if(m_eCurrentState == BOTH_LINES_IDLE) {
           ...
           XtmOsPrintf(XTM_LOG_INF,"m_ulOneLineStartedTimestampMs = %d", m_ulOneLineStartedTimestampMs) ;
   ...
   case TRANSITION_TIMERSTRIKE:
       ...
       if((m_eCurrentState == ONE_LINE_NONIDLE) 
          ...
          )
         {
           ...
           XtmOsPrintf(XTM_LOG_INF,"[XTM]m_eCurrentState = %u, m_ulOneLineTimeoutMs = %u, m_ulOneLineStartedTimestampMs = %u, ulCurrTimestampMs = %u ",
       ...
       if((m_eCurrentState == BOTH_LINES_NONIDLE) 
         ...
         {
           // Set state to reflect Single Line PHY operation
           XtmOsPrintf(XTM_LOG_INF,"[XTM]m_eCurrentState = %u, m_ulBondingTrafficTimeoutMs = %u, m_ulOneLineStartedTimestampMs = %u, ulCurrTimestampMs = %u ", 
   ...
   if(eOriginalState != m_eCurrentState) {
       ...
       XtmOsPrintf(XTM_LOG_DBG,"Transition %d causes state change from %d to %d.",eTransition, eOriginalState, m_eCurrentState);

       // Yes.  Perform actions for certain states.
       switch(m_eCurrentState) {
       case SINGLE_LINE_DATA_OPERATION:
           // Announce PHY
           XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Autosense state machine commands Single Line PHY");
       ...
       case BONDING_DATA_OPERATION:
           // Announce PHY
           XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg: Autosense state machine commands Bonding PHY");
