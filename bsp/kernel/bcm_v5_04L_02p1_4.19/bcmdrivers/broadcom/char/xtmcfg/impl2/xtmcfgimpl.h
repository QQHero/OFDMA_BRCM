...
void XtmOsPrintf( Xtm_LogLevel loglevel, const char *pFmt, ... );
...
//#define PTM_TOKEN_BUCKET_INCREMENT  2286  /* Experimental - bytes. Not active now. Keep it here for data reference of tried value prior */
...
#define XTM_ADDR_MASK_CMP(A,B)                                    \
    (((*((UINT32 *) (A)+0) == *((UINT32 *) (B)+0))       &&       \
      ((*((UINT32 *) (A)+1) & *((UINT32 *) (B)+1)) != 0) &&       \
      (*((UINT32 *) (A)+2) == *((UINT32 *) (B)+2))) ? 1 : 0)
...
private:
    ...
    UINT32 m_ulHwPtmBonding;
    UINT32 m_ulHwPad; /* Archer/ SAR pads */
    UINT32 m_ulCreated;
    ...
    BCMXTM_STATUS CheckTransmitQueues( void );
    UINT32 GetTxQSize ( void );
    UINT32 GetTxQMBR ( UINT32 usRateInBits );
...
public:
    ...
    BCMXTM_STATUS UnConfigureMaxUTPortShaping ( );
#endif
 ...
 */
#define XTM_MAJ_VER           25
#define XTM_MIN_VER           0
#define XTM_BUILD_VER         1
...
public:
    ...
    void ReconfigureSAR ( UINT32 ulPort, UINT32 trafficType, UINT32 ulSRAEvent );
...
#define XTMCFG_DIV_ROUND(x,y)    (((x)/(y)) + (((x)%(y)!=0) ? ((((((x)%(y))*10)/(y))<5) ? 0 : 1) : 0))
#define XTMCFG_DIV_FACTOR_AND_ROUND(x,y,z)    (((((x)*(y)))+(XTMCFG_DIV_ROUND(z,2)))/z)  /* NEW */
//#define XTMCFG_DIV_FACTOR_AND_ROUND(x,y,z)    XTMCFG_DIV_ROUND(x,z)*y - OLD
