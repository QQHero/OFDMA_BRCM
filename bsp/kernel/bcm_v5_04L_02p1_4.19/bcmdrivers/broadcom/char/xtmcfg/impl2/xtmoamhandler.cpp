...
XTM_OAM_HANDLER::XTM_OAM_HANDLER( void )
{
   XtmOsPrintf(XTM_LOG_DBG,"%s Enter",__FUNCTION__);
    m_pfnXtmrtReq = NULL;
    m_ulRegistered = 0;
    XtmOsMemSet(m_PendingOamReqs, 0x00, sizeof(m_PendingOamReqs));
    XtmOsMemSet(m_ulRspTimes, 0x00, sizeof(m_ulRspTimes));

    /* In 63138 and 63148, implement strip byte workaround */
#if defined (CONFIG_BCM963138) || defined (CONFIG_BCM963148)
    m_bStripByteWorkaroundActive = false;
    XtmOsMemSet(m_pulSavedRxVcamRxRamReg, 0x00, sizeof(m_pulSavedRxVcamRxRamReg));
...
BCMXTM_STATUS XTM_OAM_HANDLER::Initialize( FN_XTMRT_REQ pfnXtmrtReq, XTM_PROCESSOR *pXtmProcessor )
{
    ...
    if( m_ulRegistered == 0)
    {
            ...
            &CellHdlr) == 0 )
        {
            ...
            XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: Error registering OAM handler");
    }
    else
...
{
   XtmOsPrintf(XTM_LOG_NOT,"%s Enter",__FUNCTION__);
    if( m_ulRegistered )
    {
        XTMRT_CELL_HDLR CellHdlr;
    ...
    PXTM_OAM_CELL_INFO pOamCellInfo, XTM_CONNECTION_TABLE *pConnTable )
{
   ...
   XtmOsPrintf(XTM_LOG_DBG,"%s Enter",__FUNCTION__);
    ...
    if( bxStatus == XTMSTS_SUCCESS )
    {
        ...
        XtmOsMemCpy(&Cell.ConnAddr, pConnAddr, sizeof(XTM_ADDR));
        ...
        XtmOsMemSet( &pData[OAM_LB_CORRELATION_TAG_BYTE_OFS], 0xbc, sizeof(int) );
        if( ucCircuitType == CTYPE_OAM_F5_END_TO_END ||
            ucCircuitType == CTYPE_OAM_F4_END_TO_END )
        {
            /* 16 bytes location ID and 16 bytes source ID */
            XtmOsMemSet( &pData[OAM_LB_LOCATION_ID_BYTE_OFS], 0xff,
        ...
        else
        {
             ...
             * locationId_3-0
             */
            XtmOsMemSet(&pData[OAM_LB_LOCATION_ID_BYTE_OFS], 0xff, sizeof(int));
            XtmOsMemSet(&pData[OAM_LB_LOCATION_ID_BYTE_OFS+4], 0xff, sizeof(int));
            XtmOsMemSet(&pData[OAM_LB_LOCATION_ID_BYTE_OFS+8], 0xff, sizeof(int));
            XtmOsMemSet(&pData[OAM_LB_LOCATION_ID_BYTE_OFS+12],0xff, sizeof(int));

            XtmOsMemSet(&pData[OAM_LB_SRC_ID_BYTE_OFS], 0xff, sizeof(int));
            XtmOsMemSet(&pData[OAM_LB_SRC_ID_BYTE_OFS+4], 0xff, sizeof(int));
            XtmOsMemSet(&pData[OAM_LB_SRC_ID_BYTE_OFS+8], 0xff, sizeof(int));
            XtmOsMemSet(&pData[OAM_LB_SRC_ID_BYTE_OFS+12], 0xff, sizeof(int));
        }

        XtmOsMemSet( &pData[OAM_LB_UNUSED_BYTE_OFS], OAM_LB_UNUSED_BYTE_DEFAULT,
            OAM_LB_UNUSED_BYTE_LEN );
        XtmOsMemSet( &pData[OAM_LB_CRC_BYTE_OFS], 0, OAM_LB_CRC_BYTE_LEN );
        ...
        if( pReqAddr )
        {
            ...
            {
                XtmOsMemCpy(pReqAddr, pInternalConnAddr, sizeof(XTM_ADDR));
                ...
                *pulRspTime = XtmOsTickGet();
                ...
                if( (*m_pfnXtmrtReq) (hDev, XTMRT_CMD_SEND_CELL, &Cell) == 0 )
                {
                    ...
                    {
                        XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: OAM loopback response not "
                ...
                else
                {
                    ...
                    XtmOsPrintf(XTM_LOG_ERR,"bcmxtmcfg: OAM Send Cell Failed ") ;
                }
            }
    ...
    void *pParam, void *pContext )
{
    ...
    pCellConnAddr->ulTrafficType = pCell->ucCircuitType;
    ...
    case OAM_FAULT_MGMT_LB:
        if( pData[OAM_LB_INDICATION_BYTE_OFS] == OAM_FAULT_MGMT_LB_RESPONSE )
        {
            ...
            for( i = 0; i < ulSize; i++ )
            {
                XtmOsPrintf(XTM_LOG_DBG,"traffic type = A : B = %d : %d\n", pThis->m_PendingOamReqs->ulTrafficType,
                            pCellConnAddr->ulTrafficType) ;
                XtmOsPrintf(XTM_LOG_DBG,"port mask = A : B = %d : %d\n", pThis->m_PendingOamReqs->u.Vcc.ulPortMask,
                            pCellConnAddr->u.Vcc.ulPortMask) ;
                XtmOsPrintf(XTM_LOG_DBG,"vpi = A : B = %d : %d\n", pThis->m_PendingOamReqs->u.Vcc.usVpi,
                            pCellConnAddr->u.Vcc.usVpi) ;
                XtmOsPrintf(XTM_LOG_DBG,"vci = A : B = %d : %d\n", pThis->m_PendingOamReqs->u.Vcc.usVci,
                            pCellConnAddr->u.Vcc.usVci) ;

                if(XTM_ADDR_MASK_CMP(&pThis->m_PendingOamReqs[i], pCellConnAddr))
                {
                    /* Found. */
                    XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg : OAM LB Response Received vpi %d vci %d ",
                                  pCellConnAddr->u.Vcc.usVpi, pCellConnAddr->u.Vcc.usVci) ;
        ...
        else
        {
                ...
                pCell->ucCircuitType == CTYPE_OAM_F4_END_TO_END )
            {
                ...
                XP_REGS->ulRxOamCfg &=
                    ~(RXOAM_F4_SEG_VPI_MASK | RXOAM_F4_E2E_VPI_MASK);
                XP_REGS->ulTxOamCfg |=
                    (pCellConnAddr->u.Vcc.usVpi << 8) | pCellConnAddr->u.Vcc.usVpi;
                XP_REGS->ulRxOamCfg |=
                    (pCellConnAddr->u.Vcc.usVpi << 8) | pCellConnAddr->u.Vcc.usVpi;
            }

            XtmOsPrintf(XTM_LOG_NOT,"bcmxtmcfg : OAM LB Response Sent vpi %d vci %d ",
                                  pCellConnAddr->u.Vcc.usVpi, pCellConnAddr->u.Vcc.usVci) ;
    ...
    if(bValidOamRxd == true)
    {
        /* Since we RX'd a valid OAM, disable byte strip workaround
           if it is in use */
        //XtmOsPrintf ("XTM_OAM2: DisableStripByteWorkaround\n");
        DisableStripByteWorkaround();
    }
#endif
...
int XTM_OAM_HANDLER::EnableStripByteWorkaround( void )
{
    ...
    //XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: Enabling 63138 strip byte workaround for OAM traffic."
    ...
    if(BondInfo.ulTrafficType == TRAFFIC_TYPE_ATM_BONDED)
    {
        ...
        for(i=0; i<XP_MAX_CONNS; i++)
        {
           ...
           if ( (XP_REGS->ulRxVpiVciCam[(i*2)] & RXCAM_VALID) != 0 )
              ulTemp = XP_REGS->ulRxVpiVciCam[(i*2) + 1] ; /* Valid RAM */
           else
              ulTemp = 0 ;  /* Invalid RAM */
            ...
            * for this register?
            * Applicable only for Valid RAM contents which are Non-Zero.
            */
           if((ulTemp & (RXCAM_STRIP_BYTE_MASK | RXCAM_STRIP_EN)) != 0)
           {

               /* Clear bit fields */
               XtmOsPrintf(XTM_LOG_INF,"    DEBUG: Setting SAR_RX_VCAM_RX_RAM%d reg: %8.8x ", i, (unsigned int)ulTemp);
               ulTemp = ulTemp & ~(RXCAM_STRIP_BYTE_MASK | RXCAM_STRIP_EN); 
               XtmOsPrintf(XTM_LOG_INF,"to %8.8x", (unsigned int)ulTemp);

               /* Write stripped value back to register */
               XP_REGS->ulRxVpiVciCam[(i*2) + 1] = ulTemp;
               //XtmOsPrintf ("\n XTM5: VcID=%d, RAM=%x \n", i, XP_REGS->ulRxVpiVciCam[(i*2) + 1]) ;
           }
               //else XtmOsPrintf(XTM_LOG_DBG,"DEBUG(%s): Leaving SAR_RX_VCAM_RX_RAM[%d] as is:  %8.8x", __func__, i, (unsigned int)ulTemp);
        } /* for (i) */
...
int XTM_OAM_HANDLER::DisableStripByteWorkaround( void )
{
    ...
    //XtmOsPrintf(XTM_LOG_DBG,"bcmxtmcfg: Disabling 63138 strip byte workaround for OAM traffic"
    ...
    if(m_bStripByteWorkaroundActive)
    {
        ...
        for(i=0; i<XP_MAX_CONNS; i++)
        {
            /* Restore register if previous contents are valid. */
            if (m_pulSavedRxVcamRxRamReg[i] != 0)
...
int XTM_OAM_HANDLER::TimerUpdate( void )
{
   ...
   if(m_bStripByteWorkaroundActive == false)
   {
       ...
       if((lCurrOamRxCount - lCurrOamTxCount) > (lSavedOamRxCount - lSavedOamTxCount))
       {
          ...
          //XtmOsPrintf ("\n XTM_OAM: lCurrOamRxCount=%ld, lCurrOamTxCount=%ld, lSavedOamRxCount=%ld, lSavedOamTxCount=%ld \n",
               // lCurrOamRxCount, lCurrOamTxCount, lSavedOamRxCount, lSavedOamTxCount) ;
...

   {
          ...
          ))
       {
           ...
           // XtmOsPrintf ("XTM_OAM3: DisableStripByteWorkaround\n");
           DisableStripByteWorkaround();
       }
   }
