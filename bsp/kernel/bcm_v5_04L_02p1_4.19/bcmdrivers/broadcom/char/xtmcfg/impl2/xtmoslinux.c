...
/* Globals. */
typedef struct _XtmOsTimer {
   struct timer_list Timer;
   void (*TimerHdlr)(unsigned long arg);
   unsigned long TimerHdlrArg;
   int TimerShutDown;
}XtmOsTimer;
...
static struct InternalData
{
    ...
    XtmOsTimer IntDataTimer[MAX_INT_DATA_TIMERS];
#else
...
void *XtmOsMemSet( void *buf, int c, unsigned int size)
{
    return memset(buf, c, (size_t)size);
}

void *XtmOsMemCpy( void *dest, const void *src, unsigned int size)
{
...
#else
void XtmOsRequestSemTo( struct timer_list *timer)
#endif
{
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,0))
   sema_timer_list *sema_to_timer;
   unsigned long ulSem;
   sema_to_timer = from_timer(sema_to_timer,timer,to_timer);
   ulSem = (sema_to_timer->semid);
#endif
...
#if (LINUX_VERSION_CODE < KERNEL_VERSION(4,19,0))
            init_timer (&Timer.to_timer);
            Timer.to_timer.function = XtmOsRequestSemTo;
            Timer.to_timer.data = (unsigned long)ulSem;
#else
            Timer.semid = ulSem;
            timer_setup(&Timer.to_timer, XtmOsRequestSemTo, 0);
#endif
            Timer.to_timer.expires = jiffies + ulTimeout;

            add_timer (&Timer.to_timer);
            nRet = down_interruptible( (struct semaphore *) ulSem );
            nTimerDeleted = del_timer( &Timer.to_timer );
...
#else
void XtmOsDeferredTo( struct timer_list *timer )
{
    DEFERRED_INFO *pDeferredInfo = from_timer(pDeferredInfo,timer,TimerList);
    if( pDeferredInfo->ulInUse == 1 )
    {
        /* Schedule the bottom half. */
        tasklet_schedule(&pDeferredInfo->Task);
...
#else
        timer_setup(&g_pData->DeferredInfo.TimerList, XtmOsDeferredTo, 0);
#endif
...
#if (LINUX_VERSION_CODE >= KERNEL_VERSION(4,19,0))
void XtmOsTimerCb(struct timer_list *timer)
{
   XtmOsTimer *pXtmOsTimer;
   pXtmOsTimer = (XtmOsTimer *)from_timer(pXtmOsTimer,timer,Timer);
   if(pXtmOsTimer != NULL)
   {
      pXtmOsTimer->TimerHdlr(pXtmOsTimer->TimerHdlrArg);
   }
}
/***************************************************************************
...
UINT32 XtmOsStartTimer( void *pFnEntry, unsigned long ulFnParm, UINT32 ulTimeout )
{
    ...
    **/
    for( i = 0, pTimer = &g_pData->IntDataTimer[0]; i < MAX_INT_DATA_TIMERS;
        i++, pTimer++ )
    {
       if (pTimer->TimerHdlr == pFnEntry) {

          if (pTimer->TimerShutDown)
             goto _unlock_exit ;
       }
    }
    ...
    /* Loop through timers to find the timer or a free one for (Re)start.  */
       if ((pTimer->TimerHdlr == NULL) || (pTimer->TimerHdlr == pFnEntry)) {
        ...
        if( !timer_pending( &pTimer->Timer ) )
        {
            ...
            timer_setup(&pTimer->Timer, XtmOsTimerCb, 0);
            pTimer->TimerHdlr = pFnEntry;
            pTimer->TimerHdlrArg = ulFnParm;
            pTimer->Timer.expires = jiffies + ulTimeout;
            add_timer( &pTimer->Timer ) ;
            pTimer->TimerShutDown = 0 ;
            ulRet = 0;
            goto _unlock_exit;
    }
       } /* if */
    ...
    if(i>=MAX_INT_DATA_TIMERS) {
        ...
        for( j = 0, pTimer = &g_pData->IntDataTimer[0]; j < MAX_INT_DATA_TIMERS;
            ...
            printk("    timer %d, pending=%c, expires=%lu, function=%pS\n", 
                   j, (timer_pending( &pTimer->Timer ) ? 'Y' : 'N'), pTimer->Timer.expires, pTimer->TimerHdlr);
        }
    }
        ...
        i++, pTimer++ )
    {
        ...
        if( timer_pending( &pTimer->Timer ) )
        {
            ...
            if(pTimer->TimerHdlr == pFnEntry) {
                int try = 3, delay ;
                /* Delete the timer */
                pTimer->TimerShutDown = 1 ;
                spin_unlock_bh(&g_pData->timerlock);
                while (((iRet = try_to_del_timer_sync(&pTimer->Timer)) <= 0)
                       &&
                       (try!=0)) {
                   try-- ;
 ...
 * Returns      : None.
 ***************************************************************************/
void XtmOsPrintf(Xtm_LogLevel loglevel, char *pFmt, ... )
{
    va_list args;
    char buf[256];
    ...
    {
       bcmLogLevel_t bcmlevel = bcmLog_getLogLevel(BCM_LOG_ID_XTM);
       char *mod_name  = bcmLog_getModName(BCM_LOG_ID_XTM);
       if(((unsigned int)bcmlevel >= (unsigned int)loglevel))
       {
          switch(loglevel)
          {
             case XTM_LOG_ERR:
                bcm_printk(CLRerr "[ERROR " "%s" "] %s"CLRnl,mod_name,buf);
                break;
             case XTM_LOG_NOT:
                bcm_printk(CLRb "[NTC " "%s" "] %s"CLRnl,mod_name,buf);
                break;
             case XTM_LOG_INF:
                bcm_printk(CLRg "[INF " "%s" "] %s"CLRnl,mod_name,buf);
                break;
             case XTM_LOG_DBG:
                bcm_printk(CLRm "[DBG " "%s" "] %s"CLRnl,mod_name,buf);
                break;
             default:
                break;
          }
