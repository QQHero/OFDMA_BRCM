...
bdmf_error_t ag_drv_rescal_cfg_set(const rescal_cfg *cfg)
{
    uint32_t reg_cfg=0;
...
bdmf_error_t ag_drv_rescal_cfg_get(rescal_cfg *cfg)
{
    uint32_t reg_cfg;
...
static int bcm_rescal_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_rescal_cfg:
    {
        rescal_cfg cfg = { .cfg_wan_rescal_rstb=parm[1].value.unumber, .cfg_wan_rescal_diag_on=parm[2].value.unumber, .cfg_wan_rescal_pwrdn=parm[3].value.unumber, .cfg_wan_rescal_ctrl=parm[4].value.unumber};
        err = ag_drv_rescal_cfg_set(&cfg);
        break;
    }
    default:
...
int bcm_rescal_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        rescal_cfg cfg;
        err = ag_drv_rescal_cfg_get(&cfg);
        bdmf_session_print(session, "cfg_wan_rescal_rstb = %u (0x%x)\n", cfg.cfg_wan_rescal_rstb, cfg.cfg_wan_rescal_rstb);
        bdmf_session_print(session, "cfg_wan_rescal_diag_on = %u (0x%x)\n", cfg.cfg_wan_rescal_diag_on, cfg.cfg_wan_rescal_diag_on);
        bdmf_session_print(session, "cfg_wan_rescal_pwrdn = %u (0x%x)\n", cfg.cfg_wan_rescal_pwrdn, cfg.cfg_wan_rescal_pwrdn);
        bdmf_session_print(session, "cfg_wan_rescal_ctrl = %u (0x%x)\n", cfg.cfg_wan_rescal_ctrl, cfg.cfg_wan_rescal_ctrl);
        break;
    }
    case cli_rescal_status_0:
    {
...
static int bcm_rescal_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        rescal_cfg cfg = {.cfg_wan_rescal_rstb=gtmv(m, 1), .cfg_wan_rescal_diag_on=gtmv(m, 1), .cfg_wan_rescal_pwrdn=gtmv(m, 1), .cfg_wan_rescal_ctrl=gtmv(m, 13)};
        if(!err) bdmf_session_print(session, "ag_drv_rescal_cfg_set( %u %u %u %u)\n", cfg.cfg_wan_rescal_rstb, cfg.cfg_wan_rescal_diag_on, cfg.cfg_wan_rescal_pwrdn, cfg.cfg_wan_rescal_ctrl);
        if(!err) ag_drv_rescal_cfg_set(&cfg);
        if(!err) ag_drv_rescal_cfg_get( &cfg);
        if(!err) bdmf_session_print(session, "ag_drv_rescal_cfg_get( %u %u %u %u)\n", cfg.cfg_wan_rescal_rstb, cfg.cfg_wan_rescal_diag_on, cfg.cfg_wan_rescal_pwrdn, cfg.cfg_wan_rescal_ctrl);
        if(err || cfg.cfg_wan_rescal_rstb!=gtmv(m, 1) || cfg.cfg_wan_rescal_diag_on!=gtmv(m, 1) || cfg.cfg_wan_rescal_pwrdn!=gtmv(m, 1) || cfg.cfg_wan_rescal_ctrl!=gtmv(m, 13))
...
bdmfmon_handle_t ag_drv_rescal_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="cfg", .val=cli_rescal_cfg, .parms=set_cfg },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="cfg", .val=cli_rescal_cfg, .parms=set_default },
            { .name="status_0", .val=cli_rescal_status_0, .parms=set_default },
            { .name="status_1", .val=cli_rescal_status_1, .parms=set_default },
            BDMFMON_ENUM_LAST
        };
