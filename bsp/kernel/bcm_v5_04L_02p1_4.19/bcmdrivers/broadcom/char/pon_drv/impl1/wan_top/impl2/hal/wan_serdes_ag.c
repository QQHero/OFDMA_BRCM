...
typedef enum
{
    ...
    bdmf_address_pram_ctl_2,
    bdmf_address_pram_ctl_3,
}
bdmf_address;
...
static int bcm_wan_serdes_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_wan_serdes_pram_ctl:
    {
        wan_serdes_pram_ctl pram_ctl = { .cfg_pram_go=parm[1].value.unumber, .cfg_pram_we=parm[2].value.unumber, .cfg_pram_cs=parm[3].value.unumber, .cfg_pram_ability=parm[4].value.unumber, .cfg_pram_datain=parm[5].value.unumber, .cfg_pram_addr=parm[6].value.unumber};
        err = ag_drv_wan_serdes_pram_ctl_set(&pram_ctl);
        break;
    }
    case cli_wan_serdes_pram_ctl_2:
        err = ag_drv_wan_serdes_pram_ctl_2_set(parm[1].value.unumber);
        break;
    case cli_wan_serdes_pram_ctl_3:
        err = ag_drv_wan_serdes_pram_ctl_3_set(parm[1].value.unumber);
        break;
    default:
...
int bcm_wan_serdes_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_wan_serdes_pram_ctl:
    {
        ...
        bdmf_session_print(session, "cfg_pram_go = %u (0x%x)\n", pram_ctl.cfg_pram_go, pram_ctl.cfg_pram_go);
    ...
    case cli_wan_serdes_pram_ctl_2:
    {
        uint32_t cfg_pram_datain_0;
        err = ag_drv_wan_serdes_pram_ctl_2_get(&cfg_pram_datain_0);
        bdmf_session_print(session, "cfg_pram_datain_0 = %u (0x%x)\n", cfg_pram_datain_0, cfg_pram_datain_0);
        break;
    }
    case cli_wan_serdes_pram_ctl_3:
    {
        uint32_t cfg_pram_datain_1;
        err = ag_drv_wan_serdes_pram_ctl_3_get(&cfg_pram_datain_1);
        bdmf_session_print(session, "cfg_pram_datain_1 = %u (0x%x)\n", cfg_pram_datain_1, cfg_pram_datain_1);
        break;
    }
    default:
        err = BDMF_ERR_NOT_SUPPORTED;
...
static int bcm_wan_serdes_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        wan_serdes_pram_ctl pram_ctl = {.cfg_pram_go=gtmv(m, 1), .cfg_pram_we=gtmv(m, 1), .cfg_pram_cs=gtmv(m, 1), .cfg_pram_ability=gtmv(m, 1), .cfg_pram_datain=gtmv(m, 8), .cfg_pram_addr=gtmv(m, 16)};
        if(!err) bdmf_session_print(session, "ag_drv_wan_serdes_pram_ctl_set( %u %u %u %u %u %u)\n", pram_ctl.cfg_pram_go, pram_ctl.cfg_pram_we, pram_ctl.cfg_pram_cs, pram_ctl.cfg_pram_ability, pram_ctl.cfg_pram_datain, pram_ctl.cfg_pram_addr);
        if(!err) ag_drv_wan_serdes_pram_ctl_set(&pram_ctl);
        if(!err) ag_drv_wan_serdes_pram_ctl_get( &pram_ctl);
        if(!err) bdmf_session_print(session, "ag_drv_wan_serdes_pram_ctl_get( %u %u %u %u %u %u)\n", pram_ctl.cfg_pram_go, pram_ctl.cfg_pram_we, pram_ctl.cfg_pram_cs, pram_ctl.cfg_pram_ability, pram_ctl.cfg_pram_datain, pram_ctl.cfg_pram_addr);
        if(err || pram_ctl.cfg_pram_go!=gtmv(m, 1) || pram_ctl.cfg_pram_we!=gtmv(m, 1) || pram_ctl.cfg_pram_cs!=gtmv(m, 1) || pram_ctl.cfg_pram_ability!=gtmv(m, 1) || pram_ctl.cfg_pram_datain!=gtmv(m, 8) || pram_ctl.cfg_pram_addr!=gtmv(m, 16))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfg_pram_datain_0=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_wan_serdes_pram_ctl_2_set( %u)\n", cfg_pram_datain_0);
        if(!err) ag_drv_wan_serdes_pram_ctl_2_set(cfg_pram_datain_0);
        if(!err) ag_drv_wan_serdes_pram_ctl_2_get( &cfg_pram_datain_0);
        if(!err) bdmf_session_print(session, "ag_drv_wan_serdes_pram_ctl_2_get( %u)\n", cfg_pram_datain_0);
        if(err || cfg_pram_datain_0!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfg_pram_datain_1=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_wan_serdes_pram_ctl_3_set( %u)\n", cfg_pram_datain_1);
        if(!err) ag_drv_wan_serdes_pram_ctl_3_set(cfg_pram_datain_1);
        if(!err) ag_drv_wan_serdes_pram_ctl_3_get( &cfg_pram_datain_1);
        if(!err) bdmf_session_print(session, "ag_drv_wan_serdes_pram_ctl_3_get( %u)\n", cfg_pram_datain_1);
        if(err || cfg_pram_datain_1!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    return err;
}
...
static int bcm_wan_serdes_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_pram_ctl_2 : reg = &RU_REG(WAN_SERDES, PRAM_CTL_2); blk = &RU_BLK(WAN_SERDES); break;
    case bdmf_address_pram_ctl_3 : reg = &RU_REG(WAN_SERDES, PRAM_CTL_3); blk = &RU_BLK(WAN_SERDES); break;
    default :
...
bdmfmon_handle_t ag_drv_wan_serdes_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_pram_ctl[]={
            BDMFMON_MAKE_PARM("cfg_pram_go", "cfg_pram_go", BDMFMON_PARM_NUMBER, 0),
        ...
        static bdmfmon_cmd_parm_t set_pram_ctl_2[]={
            BDMFMON_MAKE_PARM("cfg_pram_datain_0", "cfg_pram_datain_0", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_pram_ctl_3[]={
            BDMFMON_MAKE_PARM("cfg_pram_datain_1", "cfg_pram_datain_1", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="pll_ctl", .val=cli_wan_serdes_pll_ctl, .parms=set_pll_ctl },
            { .name="pram_ctl", .val=cli_wan_serdes_pram_ctl, .parms=set_pram_ctl },
            { .name="pram_ctl_2", .val=cli_wan_serdes_pram_ctl_2, .parms=set_pram_ctl_2 },
            { .name="pram_ctl_3", .val=cli_wan_serdes_pram_ctl_3, .parms=set_pram_ctl_3 },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="pll_ctl", .val=cli_wan_serdes_pll_ctl, .parms=set_default },
            { .name="temp_ctl", .val=cli_wan_serdes_temp_ctl, .parms=set_default },
            { .name="pram_ctl", .val=cli_wan_serdes_pram_ctl, .parms=set_default },
            { .name="pram_ctl_2", .val=cli_wan_serdes_pram_ctl_2, .parms=set_default },
            { .name="pram_ctl_3", .val=cli_wan_serdes_pram_ctl_3, .parms=set_default },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="PRAM_CTL_2" , .val=bdmf_address_pram_ctl_2 },
            { .name="PRAM_CTL_3" , .val=bdmf_address_pram_ctl_3 },
            BDMFMON_ENUM_LAST
        };
