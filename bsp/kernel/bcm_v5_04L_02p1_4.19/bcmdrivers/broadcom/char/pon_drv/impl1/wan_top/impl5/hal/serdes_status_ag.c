...
int bcm_serdes_status_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    switch(parm[0].value.unumber)
    {
    case cli_serdes_status_serdes_status_status:
    {
        serdes_status_serdes_status_status serdes_status_status;
        err = ag_drv_serdes_status_serdes_status_status_get(&serdes_status_status);
        bdmf_session_print(session, "pmd_pll0_lock = %u (0x%x)\n", serdes_status_status.pmd_pll0_lock, serdes_status_status.pmd_pll0_lock);
        bdmf_session_print(session, "pmd_rx_lock_0_invert = %u (0x%x)\n", serdes_status_status.pmd_rx_lock_0_invert, serdes_status_status.pmd_rx_lock_0_invert);
        bdmf_session_print(session, "pmd_rx_clk_vld_0 = %u (0x%x)\n", serdes_status_status.pmd_rx_clk_vld_0, serdes_status_status.pmd_rx_clk_vld_0);
        bdmf_session_print(session, "pmd_tx_clk_vld = %u (0x%x)\n", serdes_status_status.pmd_tx_clk_vld, serdes_status_status.pmd_tx_clk_vld);
        bdmf_session_print(session, "pmd_rx_lock_0 = %u (0x%x)\n", serdes_status_status.pmd_rx_lock_0, serdes_status_status.pmd_rx_lock_0);
        bdmf_session_print(session, "pmd_energy_detect_0 = %u (0x%x)\n", serdes_status_status.pmd_energy_detect_0, serdes_status_status.pmd_energy_detect_0);
        bdmf_session_print(session, "pmd_signal_detect_0 = %u (0x%x)\n", serdes_status_status.pmd_signal_detect_0, serdes_status_status.pmd_signal_detect_0);
        bdmf_session_print(session, "pmi_lp_acknowledge = %u (0x%x)\n", serdes_status_status.pmi_lp_acknowledge, serdes_status_status.pmi_lp_acknowledge);
        bdmf_session_print(session, "pmi_lp_error = %u (0x%x)\n", serdes_status_status.pmi_lp_error, serdes_status_status.pmi_lp_error);
        bdmf_session_print(session, "o_laser_burst_en = %u (0x%x)\n", serdes_status_status.o_laser_burst_en, serdes_status_status.o_laser_burst_en);
        bdmf_session_print(session, "pmd_pll1_lock = %u (0x%x)\n", serdes_status_status.pmd_pll1_lock, serdes_status_status.pmd_pll1_lock);
        break;
    }
    default:
        err = BDMF_ERR_NOT_SUPPORTED;
...
static int bcm_serdes_status_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        serdes_status_serdes_status_status serdes_status_status = {.pmd_pll0_lock=gtmv(m, 1), .pmd_rx_lock_0_invert=gtmv(m, 1), .pmd_rx_clk_vld_0=gtmv(m, 1), .pmd_tx_clk_vld=gtmv(m, 1), .pmd_rx_lock_0=gtmv(m, 1), .pmd_energy_detect_0=gtmv(m, 1), .pmd_signal_detect_0=gtmv(m, 1), .pmi_lp_acknowledge=gtmv(m, 1), .pmi_lp_error=gtmv(m, 1), .o_laser_burst_en=gtmv(m, 1), .pmd_pll1_lock=gtmv(m, 1)};
        if(!err) ag_drv_serdes_status_serdes_status_status_get( &serdes_status_status);
        if(!err) bdmf_session_print(session, "ag_drv_serdes_status_serdes_status_status_get( %u %u %u %u %u %u %u %u %u %u %u)\n", serdes_status_status.pmd_pll0_lock, serdes_status_status.pmd_rx_lock_0_invert, serdes_status_status.pmd_rx_clk_vld_0, serdes_status_status.pmd_tx_clk_vld, serdes_status_status.pmd_rx_lock_0, serdes_status_status.pmd_energy_detect_0, serdes_status_status.pmd_signal_detect_0, serdes_status_status.pmi_lp_acknowledge, serdes_status_status.pmi_lp_error, serdes_status_status.o_laser_burst_en, serdes_status_status.pmd_pll1_lock);
    }
    return err;
}
...
bdmfmon_handle_t ag_drv_serdes_status_cli_init(bdmfmon_handle_t driver_dir)
{
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="serdes_status_status", .val=cli_serdes_status_serdes_status_status, .parms=set_default },
            BDMFMON_ENUM_LAST
        };
