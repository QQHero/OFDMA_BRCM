...
bdmf_error_t ag_drv_pmi_lp_0_set(bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_en, bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_write)
{
    ...
    if((cr_xgwan_top_wan_misc_pmi_lp_write >= _1BITS_MAX_VAL_) ||
...
#ifdef VALIDATE_PARMS
    if(!cr_xgwan_top_wan_misc_pmi_lp_write || !cr_xgwan_top_wan_misc_pmi_lp_en)
    {
...
#ifdef VALIDATE_PARMS
    if(!cr_xgwan_top_wan_misc_pmi_lp_maskdata || !cr_xgwan_top_wan_misc_pmi_lp_wrdata)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
...
bdmf_error_t ag_drv_pmi_lp_3_get(bdmf_boolean *cr_xgwan_top_wan_misc_pmi_lp_err, bdmf_boolean *cr_xgwan_top_wan_misc_pmi_lp_ack, uint16_t *cr_xgwan_top_wan_misc_pmi_lp_rddata)
{
    uint32_t reg_lp_3;

#ifdef VALIDATE_PARMS
    if(!cr_xgwan_top_wan_misc_pmi_lp_rddata || !cr_xgwan_top_wan_misc_pmi_lp_ack || !cr_xgwan_top_wan_misc_pmi_lp_err)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    *cr_xgwan_top_wan_misc_pmi_lp_rddata = RU_FIELD_GET(0, PMI, LP_3, CR_XGWAN_TOP_WAN_MISC_PMI_LP_RDDATA, reg_lp_3);
    *cr_xgwan_top_wan_misc_pmi_lp_ack = RU_FIELD_GET(0, PMI, LP_3, CR_XGWAN_TOP_WAN_MISC_PMI_LP_ACK, reg_lp_3);
    *cr_xgwan_top_wan_misc_pmi_lp_err = RU_FIELD_GET(0, PMI, LP_3, CR_XGWAN_TOP_WAN_MISC_PMI_LP_ERR, reg_lp_3);
...
int bcm_pmi_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_pmi_lp_0:
    {
        ...
        err = ag_drv_pmi_lp_0_get(&cr_xgwan_top_wan_misc_pmi_lp_write, &cr_xgwan_top_wan_misc_pmi_lp_en);
    ...
    case cli_pmi_lp_2:
    {
        ...
        err = ag_drv_pmi_lp_2_get(&cr_xgwan_top_wan_misc_pmi_lp_maskdata, &cr_xgwan_top_wan_misc_pmi_lp_wrdata);
    ...
    case cli_pmi_lp_3:
    {
        uint16_t cr_xgwan_top_wan_misc_pmi_lp_rddata;
        bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_ack;
        bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_err;
        err = ag_drv_pmi_lp_3_get(&cr_xgwan_top_wan_misc_pmi_lp_rddata, &cr_xgwan_top_wan_misc_pmi_lp_ack, &cr_xgwan_top_wan_misc_pmi_lp_err);
        bdmf_session_print(session, "cr_xgwan_top_wan_misc_pmi_lp_rddata = %u (0x%x)\n", cr_xgwan_top_wan_misc_pmi_lp_rddata, cr_xgwan_top_wan_misc_pmi_lp_rddata);
        bdmf_session_print(session, "cr_xgwan_top_wan_misc_pmi_lp_ack = %u (0x%x)\n", cr_xgwan_top_wan_misc_pmi_lp_ack, cr_xgwan_top_wan_misc_pmi_lp_ack);
        bdmf_session_print(session, "cr_xgwan_top_wan_misc_pmi_lp_err = %u (0x%x)\n", cr_xgwan_top_wan_misc_pmi_lp_err, cr_xgwan_top_wan_misc_pmi_lp_err);
        break;
    }
    default:
        err = BDMF_ERR_NOT_SUPPORTED;
...
static int bcm_pmi_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_pmi_lp_0_set( %u %u)\n", cr_xgwan_top_wan_misc_pmi_lp_write, cr_xgwan_top_wan_misc_pmi_lp_en);
        if(!err) ag_drv_pmi_lp_0_set(cr_xgwan_top_wan_misc_pmi_lp_write, cr_xgwan_top_wan_misc_pmi_lp_en);
        if(!err) ag_drv_pmi_lp_0_get( &cr_xgwan_top_wan_misc_pmi_lp_write, &cr_xgwan_top_wan_misc_pmi_lp_en);
        if(!err) bdmf_session_print(session, "ag_drv_pmi_lp_0_get( %u %u)\n", cr_xgwan_top_wan_misc_pmi_lp_write, cr_xgwan_top_wan_misc_pmi_lp_en);
        if(err || cr_xgwan_top_wan_misc_pmi_lp_write!=gtmv(m, 1) || cr_xgwan_top_wan_misc_pmi_lp_en!=gtmv(m, 1))
    ...
    }
    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_pmi_lp_2_set( %u %u)\n", cr_xgwan_top_wan_misc_pmi_lp_maskdata, cr_xgwan_top_wan_misc_pmi_lp_wrdata);
        if(!err) ag_drv_pmi_lp_2_set(cr_xgwan_top_wan_misc_pmi_lp_maskdata, cr_xgwan_top_wan_misc_pmi_lp_wrdata);
        if(!err) ag_drv_pmi_lp_2_get( &cr_xgwan_top_wan_misc_pmi_lp_maskdata, &cr_xgwan_top_wan_misc_pmi_lp_wrdata);
        if(!err) bdmf_session_print(session, "ag_drv_pmi_lp_2_get( %u %u)\n", cr_xgwan_top_wan_misc_pmi_lp_maskdata, cr_xgwan_top_wan_misc_pmi_lp_wrdata);
        if(err || cr_xgwan_top_wan_misc_pmi_lp_maskdata!=gtmv(m, 16) || cr_xgwan_top_wan_misc_pmi_lp_wrdata!=gtmv(m, 16))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint16_t cr_xgwan_top_wan_misc_pmi_lp_rddata=gtmv(m, 16);
        bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_ack=gtmv(m, 1);
        bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_err=gtmv(m, 1);
        if(!err) ag_drv_pmi_lp_3_get( &cr_xgwan_top_wan_misc_pmi_lp_rddata, &cr_xgwan_top_wan_misc_pmi_lp_ack, &cr_xgwan_top_wan_misc_pmi_lp_err);
        if(!err) bdmf_session_print(session, "ag_drv_pmi_lp_3_get( %u %u %u)\n", cr_xgwan_top_wan_misc_pmi_lp_rddata, cr_xgwan_top_wan_misc_pmi_lp_ack, cr_xgwan_top_wan_misc_pmi_lp_err);
    }
    return err;
}
