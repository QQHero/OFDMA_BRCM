...
#define EAGLE_TX_AMP_PSP_KEY "TxAmp"
static char *tx_fir_psp_keys[] = { "TxFir0", "TxFir1", "TxFir2", "TxFir3", "TxFir4", };
...
void writePONSerdesReg(uint16_t lane, uint16_t address, uint16_t value, uint16_t  mask)
{
    ...
    RU_FIELD_WRITE(0, PMI, LP_1, CR_XGWAN_TOP_WAN_MISC_PMI_LP_ADDR, serdes_addr);

    RU_FIELD_WRITE(0, PMI, LP_2, CR_XGWAN_TOP_WAN_MISC_PMI_LP_WRDATA   , value);
    RU_FIELD_WRITE(0, PMI, LP_2, CR_XGWAN_TOP_WAN_MISC_PMI_LP_MASKDATA , mask_inv);

    RU_FIELD_WRITE(0, PMI, LP_0, CR_XGWAN_TOP_WAN_MISC_PMI_LP_WRITE, 1);  /* Select Write access */
    RU_FIELD_WRITE(0, PMI, LP_0, CR_XGWAN_TOP_WAN_MISC_PMI_LP_EN   , 1);  /* Initiate SerDes reg access */

    udelay(5);
    RU_FIELD_WRITE(0, PMI, LP_0, CR_XGWAN_TOP_WAN_MISC_PMI_LP_EN   , 0);
    RU_FIELD_WRITE(0, PMI, LP_0, CR_XGWAN_TOP_WAN_MISC_PMI_LP_WRITE, 0);
...
uint16_t readPONSerdesReg(uint16_t lane, uint16_t address)
{
    ...
    RU_FIELD_WRITE(0, PMI, LP_0, CR_XGWAN_TOP_WAN_MISC_PMI_LP_WRITE, 0); /* Select Read access */
    RU_FIELD_WRITE(0, PMI, LP_0, CR_XGWAN_TOP_WAN_MISC_PMI_LP_EN   , 1); /* Initiate SerDes reg access */
    udelay(5);
...
void set_pll_refclk(uint8_t pll_refclk)
{
    ...
    if (pll_refclk == LCREF)  /* -->  WAN_SERDES_PLL_CTL = 0x105 */
    {
        ...
        RU_FIELD_WRITE(0, WAN_SERDES, PLL_CTL, CFG_PLL1_LCREF_SEL, 0); /* 0 = select pll1_lcref. 1 = pll0_lcref. */
        RU_FIELD_WRITE(0, WAN_SERDES, PLL_CTL, CFG_PLL1_REFIN_EN,  1); /* Reference select. 0 = select pad_pll1_refclkp/n. 1 - select pll1_lcrefp/n. */
        RU_FIELD_WRITE(0, WAN_SERDES, PLL_CTL, CFG_PLL0_LCREF_SEL, 1); /* 0 = pll0_lcref.  1 = pll1_lcref. */
        RU_FIELD_WRITE(0, WAN_SERDES, PLL_CTL, CFG_PLL0_REFIN_EN,  1); /* Reference select: 0 = select pad_pll0_refclkp/n. 1 = select pll0_lcrefp/n. */
    }
    else /* pll_refclk = PADREF  -->  WAN_SERDES_PLL_CTL =0x400 */
    {
        __logDebug("\n PLL RefClock = PADREF (155.52MHz or 156.25MHz)");
        RU_FIELD_WRITE(0, WAN_SERDES, PLL_CTL, CFG_PLL1_LCREF_SEL, 1);
        RU_FIELD_WRITE(0, WAN_SERDES, PLL_CTL, CFG_PLL1_REFIN_EN,  0);
        RU_FIELD_WRITE(0, WAN_SERDES, PLL_CTL, CFG_PLL0_LCREF_SEL, 0);
        RU_FIELD_WRITE(0, WAN_SERDES, PLL_CTL, CFG_PLL0_REFIN_EN,  0);
    }
...
void pll_ppm_adj_FracN_10G(uint8_t mode, int16_t ppm_target, bool pll_adj_en)
{
    ...
    else if ((mode == EPON_10_10) || (mode == AE_5_5) || (mode == AE_2_2_R))  /* 10G- IEEE modes */
    {
        nom_pll_fracn_sel  = 0x1;
...
void pmd_rx_restart(void)
{
    // RX Restart adaptation (reg{0D019}  DSC_B_dsc_sm_ctrl_9[bit0] ) -- after Reconnect (O4-->5)
    //  [bit0] = rx_restart_pmd           = 1 will reset the DSC SM into RESTART state. This is a self-clear register bit.
    //  [bit1] = rx_restart_pmd_hold      = 0
    uint16_t wr_serdes = (0x0001);
    uint16_t wr_mask   = (0x0003);       // set bit[0]=rx_restart_pmd=1    & [bit1] =rx_restart_pmd_hold = 0
    writePONSerdesReg(DEVID_1, DSC_B_dsc_sm_ctrl_9 , wr_serdes, wr_mask);
}

void rx_sigdet_force(void)
{
    // RX Force signal-detect (reg{0xD0C1} SIGDET_SIGDET_CTRL_1  )  - to always ON
    // bit[8] = signal_detect_frc_val
    // bit[7] = signal_detect_frc

    uint16_t signal_detect_frc_val = 1, signal_detect_frc = 1;
    uint16_t wr_serdes = (signal_detect_frc_val << 8) | (signal_detect_frc << 7);
    uint16_t wr_mask   =                (0x0001 << 8) |            (0x0001 << 7);
    writePONSerdesReg(0x0800, SIGDET_SIGDET_CTRL_1 , wr_serdes, wr_mask);
}
...
static void get_tx_amp_fir_from_scratchpad(uint16_t *tx_amp, int16_t tx_fir[])
{
    char buf[64] = {};
    int i;

    if (kerSysScratchPadGet(EAGLE_TX_AMP_PSP_KEY, buf, sizeof(buf)) > 0)
    {
        sscanf(buf, " %hx", tx_amp);
    }
    for (i = 0; i <= 4; i++)
    {
        if (kerSysScratchPadGet(tx_fir_psp_keys[i], buf, sizeof(buf)) > 0)
        {
            sscanf(buf, " %hd", &tx_fir[i]);
        }
    }
    if (SERDES_DEBUG)
    {
        __logInfo("%s: tx_amp=%04X, tx_fir0=%d, tx_fir1=%d, tx_fir2=%d, tx_fir3=%d, tx_fir4=%d\n", __FUNCTION__,
            *tx_amp, tx_fir[0], tx_fir[1], tx_fir[2], tx_fir[3], tx_fir[4]);
    }
}

static const uint16_t tx_fir_read_addr_a[] = { TX_FED_txfir_control1, TX_FED_txfir_control2, TX_FED_txfir_control1, TX_FED_txfir_control2, TX_FED_txfir_control4, };
static const uint16_t tx_fir_mask_a[] = { 0x001f, 0x007f, 0x07e0, 0x0f80, 0x000f, };

static void set_g_fir(int i)
{
    g_fir_type = i;
    g_fir_read_addr = tx_fir_read_addr_a[i];
    g_fir_mask = tx_fir_mask_a[i];
}

uint16_t get_serdes_tx_amp(void)
{
    uint16_t val;
...
void check_and_set_tx_amp(uint32_t val)
{
    if (val > 0xf)
    {
...
int16_t get_serdes_tx_fir(int i)
{
    set_g_fir(i);
    return get_serdes_tx_fir_g();
}

int16_t get_serdes_tx_fir_g(void)
{
    int16_t val;
...
ssize_t check_and_set_tx_fir(int i, int16_t val)
{
    set_g_fir(i);
    return check_and_set_tx_fir_g(val, 2, 0);
}

ssize_t check_and_set_tx_fir_g(int16_t val, int ret, size_t len)
{
    int16_t tmp_val;
...
void set_tx_amp_fir(uint16_t dts_tx_amp, int16_t dts_tx_fir_a[5])
{
    uint16_t tx_amp = get_serdes_tx_amp();
    int16_t tx_fir_a[5] = { TX_FIR0_DEFAULT, TX_FIR1_DEFAULT, TX_FIR2_DEFAULT, TX_FIR3_DEFAULT, TX_FIR4_DEFAULT };
    int i;

    if (dts_tx_amp != UNDEF_TX_AMP_FIR)
        tx_amp = dts_tx_amp;
    for (i = 0; i <= 4; i++)
        if (dts_tx_fir_a[i] != UNDEF_TX_AMP_FIR)
            tx_fir_a[i] = dts_tx_fir_a[i];

    get_tx_amp_fir_from_scratchpad(&tx_amp, tx_fir_a);

    check_and_set_tx_amp(tx_amp);
    for (i = 0; i <= 4; i++)
        check_and_set_tx_fir(i, tx_fir_a[i]);
}
