...
static int bcm_top_scratch_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
    case cli_top_scratch_top_scratch_atch:
        err = ag_drv_top_scratch_top_scratch_atch_set(parm[1].value.unumber);
        break;
    default:
...
int bcm_top_scratch_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    bdmf_error_t err = BDMF_ERR_OK;
    ...
    {
        uint32_t data;
        err = ag_drv_top_scratch_top_scratch_atch_get(&data);
...
static int bcm_top_scratch_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
...

    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_top_scratch_top_scratch_atch_set( %u)\n", data);
        if(!err) ag_drv_top_scratch_top_scratch_atch_set(data);
        if(!err) ag_drv_top_scratch_top_scratch_atch_get( &data);
        if(!err) bdmf_session_print(session, "ag_drv_top_scratch_top_scratch_atch_get( %u)\n", data);
...
bdmfmon_handle_t ag_drv_top_scratch_cli_init(bdmfmon_handle_t driver_dir)
{
    ...
    {
        static bdmfmon_cmd_parm_t set_top_scratch_atch[]={
            BDMFMON_MAKE_PARM("data", "data", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="top_scratch_atch", .val=cli_top_scratch_top_scratch_atch, .parms=set_top_scratch_atch },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="top_scratch_atch", .val=cli_top_scratch_top_scratch_atch, .parms=set_default },
            BDMFMON_ENUM_LAST
        };
