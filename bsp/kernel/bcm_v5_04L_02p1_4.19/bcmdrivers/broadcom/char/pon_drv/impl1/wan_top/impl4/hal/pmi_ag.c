...
bdmf_error_t ag_drv_pmi_lp_0_set(bdmf_boolean cr_xgwan_top_wan_pcs_pmi_lp_en, bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_en, bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_write)
{
    uint32_t reg_lp_0=0;

#ifdef VALIDATE_PARMS
    if((cr_xgwan_top_wan_pcs_pmi_lp_en >= _1BITS_MAX_VAL_) ||
       (cr_xgwan_top_wan_misc_pmi_lp_en >= _1BITS_MAX_VAL_) ||
    ...
    reg_lp_0 = RU_FIELD_SET(0, PMI, LP_0, CR_XGWAN_TOP_WAN_PCS_PMI_LP_EN, reg_lp_0, cr_xgwan_top_wan_pcs_pmi_lp_en);
...
bdmf_error_t ag_drv_pmi_lp_0_get(bdmf_boolean *cr_xgwan_top_wan_pcs_pmi_lp_en, bdmf_boolean *cr_xgwan_top_wan_misc_pmi_lp_en, bdmf_boolean *cr_xgwan_top_wan_misc_pmi_lp_write)
{
    uint32_t reg_lp_0;

#ifdef VALIDATE_PARMS
    if(!cr_xgwan_top_wan_pcs_pmi_lp_en || !cr_xgwan_top_wan_misc_pmi_lp_en || !cr_xgwan_top_wan_misc_pmi_lp_write)
    {
    ...
    *cr_xgwan_top_wan_pcs_pmi_lp_en = RU_FIELD_GET(0, PMI, LP_0, CR_XGWAN_TOP_WAN_PCS_PMI_LP_EN, reg_lp_0);
...
bdmf_error_t ag_drv_pmi_lp_4_get(bdmf_boolean *pcs_pmi_lp_err, bdmf_boolean *pcs_pmi_lp_ack, uint16_t *pcs_pmi_lp_rddata)
{
    uint32_t reg_lp_4;

#ifdef VALIDATE_PARMS
    if(!pcs_pmi_lp_err || !pcs_pmi_lp_ack || !pcs_pmi_lp_rddata)
    {
        bdmf_trace("ERROR driver %s:%u| err=%s (%d)\n", __FILE__, __LINE__, bdmf_strerror(BDMF_ERR_PARM), BDMF_ERR_PARM);
        return BDMF_ERR_PARM;
    }
    ...
    RU_REG_READ(0, PMI, LP_4, reg_lp_4);

    *pcs_pmi_lp_err = RU_FIELD_GET(0, PMI, LP_4, PCS_PMI_LP_ERR, reg_lp_4);
    *pcs_pmi_lp_ack = RU_FIELD_GET(0, PMI, LP_4, PCS_PMI_LP_ACK, reg_lp_4);
    *pcs_pmi_lp_rddata = RU_FIELD_GET(0, PMI, LP_4, PCS_PMI_LP_RDDATA, reg_lp_4);
...
static int bcm_pmi_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
    case cli_pmi_lp_0:
        err = ag_drv_pmi_lp_0_set(parm[1].value.unumber, parm[2].value.unumber, parm[3].value.unumber);
        break;
    case cli_pmi_lp_1:
...
int bcm_pmi_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_pmi_lp_0:
    {
        bdmf_boolean cr_xgwan_top_wan_pcs_pmi_lp_en;
        bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_en;
        bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_write;
        err = ag_drv_pmi_lp_0_get(&cr_xgwan_top_wan_pcs_pmi_lp_en, &cr_xgwan_top_wan_misc_pmi_lp_en, &cr_xgwan_top_wan_misc_pmi_lp_write);
        bdmf_session_print(session, "cr_xgwan_top_wan_pcs_pmi_lp_en = %u (0x%x)\n", cr_xgwan_top_wan_pcs_pmi_lp_en, cr_xgwan_top_wan_pcs_pmi_lp_en);
    ...
    case cli_pmi_lp_4:
    {
        bdmf_boolean pcs_pmi_lp_err;
        bdmf_boolean pcs_pmi_lp_ack;
        uint16_t pcs_pmi_lp_rddata;
        err = ag_drv_pmi_lp_4_get(&pcs_pmi_lp_err, &pcs_pmi_lp_ack, &pcs_pmi_lp_rddata);
        bdmf_session_print(session, "pcs_pmi_lp_err = %u (0x%x)\n", pcs_pmi_lp_err, pcs_pmi_lp_err);
        bdmf_session_print(session, "pcs_pmi_lp_ack = %u (0x%x)\n", pcs_pmi_lp_ack, pcs_pmi_lp_ack);
        bdmf_session_print(session, "pcs_pmi_lp_rddata = %u (0x%x)\n", pcs_pmi_lp_rddata, pcs_pmi_lp_rddata);
        break;
    }
    default:
        err = BDMF_ERR_NOT_SUPPORTED;
...
static int bcm_pmi_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        bdmf_boolean cr_xgwan_top_wan_pcs_pmi_lp_en=gtmv(m, 1);
        bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_en=gtmv(m, 1);
        bdmf_boolean cr_xgwan_top_wan_misc_pmi_lp_write=gtmv(m, 1);
        if(!err) bdmf_session_print(session, "ag_drv_pmi_lp_0_set( %u %u %u)\n", cr_xgwan_top_wan_pcs_pmi_lp_en, cr_xgwan_top_wan_misc_pmi_lp_en, cr_xgwan_top_wan_misc_pmi_lp_write);
        if(!err) ag_drv_pmi_lp_0_set(cr_xgwan_top_wan_pcs_pmi_lp_en, cr_xgwan_top_wan_misc_pmi_lp_en, cr_xgwan_top_wan_misc_pmi_lp_write);
        if(!err) ag_drv_pmi_lp_0_get( &cr_xgwan_top_wan_pcs_pmi_lp_en, &cr_xgwan_top_wan_misc_pmi_lp_en, &cr_xgwan_top_wan_misc_pmi_lp_write);
        if(!err) bdmf_session_print(session, "ag_drv_pmi_lp_0_get( %u %u %u)\n", cr_xgwan_top_wan_pcs_pmi_lp_en, cr_xgwan_top_wan_misc_pmi_lp_en, cr_xgwan_top_wan_misc_pmi_lp_write);
        if(err || cr_xgwan_top_wan_pcs_pmi_lp_en!=gtmv(m, 1) || cr_xgwan_top_wan_misc_pmi_lp_en!=gtmv(m, 1) || cr_xgwan_top_wan_misc_pmi_lp_write!=gtmv(m, 1))
    ...
    {
        bdmf_boolean pcs_pmi_lp_err=gtmv(m, 1);
        bdmf_boolean pcs_pmi_lp_ack=gtmv(m, 1);
        uint16_t pcs_pmi_lp_rddata=gtmv(m, 16);
        if(!err) ag_drv_pmi_lp_4_get( &pcs_pmi_lp_err, &pcs_pmi_lp_ack, &pcs_pmi_lp_rddata);
        if(!err) bdmf_session_print(session, "ag_drv_pmi_lp_4_get( %u %u %u)\n", pcs_pmi_lp_err, pcs_pmi_lp_ack, pcs_pmi_lp_rddata);
    }
    return err;
}
...
static int bcm_pmi_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_lp_4 : reg = &RU_REG(PMI, LP_4); blk = &RU_BLK(PMI); break;
    default :
...
bdmfmon_handle_t ag_drv_pmi_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        static bdmfmon_cmd_parm_t set_lp_0[]={
            BDMFMON_MAKE_PARM("cr_xgwan_top_wan_pcs_pmi_lp_en", "cr_xgwan_top_wan_pcs_pmi_lp_en", BDMFMON_PARM_NUMBER, 0),
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="lp_0", .val=cli_pmi_lp_0, .parms=set_lp_0 },
            { .name="lp_1", .val=cli_pmi_lp_1, .parms=set_lp_1 },
            { .name="lp_2", .val=cli_pmi_lp_2, .parms=set_lp_2 },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            { .name="lp_0", .val=cli_pmi_lp_0, .parms=set_default },
            { .name="lp_1", .val=cli_pmi_lp_1, .parms=set_default },
            { .name="lp_2", .val=cli_pmi_lp_2, .parms=set_default },
            { .name="lp_3", .val=cli_pmi_lp_3, .parms=set_default },
            { .name="lp_4", .val=cli_pmi_lp_4, .parms=set_default },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="LP_4" , .val=bdmf_address_lp_4 },
            BDMFMON_ENUM_LAST
        };
