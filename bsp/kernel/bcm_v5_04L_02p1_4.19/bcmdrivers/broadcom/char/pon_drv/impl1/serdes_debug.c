...
int g_fir_type= 0;
uint16_t g_fir_mask= 0;
uint16_t g_fir_read_addr = 0;
...
static ssize_t proc_set_serdes_reinit(struct file *file, const char *buff, size_t len, loff_t *offset)
{
    ...
    {
        int off = 0;
        off += HELP_PRINTK(off, "    - wan_type: %d:GPON, %d:EPON_1G, %d:EPON_2G, %d:AE, %d:AE_2_5G, %d:AE_2_5G_R, %d:AE_5G, %d:AE_10G, %d:EPON_10G_SYM, %d:EPON_10G_ASYM,\n"
                                "                %d:XGPON_10G_2_5G, %d:NGPON_10G_10G/XGS\n",
                                SERDES_WAN_TYPE_GPON, SERDES_WAN_TYPE_EPON_1G, SERDES_WAN_TYPE_EPON_2G,
                                SERDES_WAN_TYPE_AE, SERDES_WAN_TYPE_AE_2_5G, SERDES_WAN_TYPE_AE_2_5G_R, SERDES_WAN_TYPE_AE_5G, SERDES_WAN_TYPE_AE_10G, SERDES_WAN_TYPE_EPON_10G_SYM,
...
static ssize_t proc_set_serdes_prbs(struct file *file, const char *buff, size_t len, loff_t *offset)
{
    ...
    void help(void)
    {
        ...
        off += HELP_PRINTK(off, "    - enable: <0|1>\n"
                                ...
                                "    - wan_type: %d:GPON, %d:EPON_1G, %d:EPON_2G, %d:AE, %d:AE_2_5G, %d:AE_2_5G_R, %d:AE_5G, %d:AE_10G, %d:EPON_10G_SYM, %d:EPON_10G_ASYM,\n"
                                "                %d:XGPON_10G_2_5G, %d:NGPON_10G_10G/XGS\n",
                                SERDES_WAN_TYPE_GPON, SERDES_WAN_TYPE_EPON_1G, SERDES_WAN_TYPE_EPON_2G,
                                SERDES_WAN_TYPE_AE, SERDES_WAN_TYPE_AE_2_5G, SERDES_WAN_TYPE_AE_2_5G_R, SERDES_WAN_TYPE_AE_5G, SERDES_WAN_TYPE_AE_10G, SERDES_WAN_TYPE_EPON_10G_SYM,
                                SERDES_WAN_TYPE_EPON_10G_ASYM, SERDES_WAN_TYPE_XGPON_10G_2_5G, SERDES_WAN_TYPE_NGPON_10G_10G);
...
static ssize_t proc_get_serdes_tx_amp(struct file *file, char *buff, size_t len, loff_t *offset)
{
    ...
    if (0 == *offset)
    {
        *offset = sprintf(buff, "%04X", get_serdes_tx_amp());
        ret = *offset;
    }
    return ret;
}
...
static ssize_t proc_set_serdes_tx_amp(struct file *file, const char *buff, size_t len, loff_t *offset)
{
    ...
    ret = sscanf(input, "%hx", &val);
    if (1 == ret)
    {
        check_and_set_tx_amp(val);
    }
    else
    {
        __logError("Error format!");
...
static ssize_t proc_get_serdes_tx_fir(struct file *file, char *buff, size_t len, loff_t *offset)
{
    ...
    {
        __logDebug("addr=0x%04X\n", g_fir_read_addr);
        *offset = sprintf(buff, "%d", get_serdes_tx_fir_g());
        ret = *offset;
    }
    return ret;
}
...
static ssize_t proc_set_serdes_tx_fir(struct file *file, const char *buff, size_t len, loff_t *offset)
{
    ...
    ret = sscanf(input, "%d %hd", &g_fir_type, &val);

    if ((ret == 1) || (ret == 2))
    {
        if ((g_fir_type < 0) || (g_fir_type > 4))
        {
            __logError("Error format!");
            help();
            return -EFAULT;
            ...
            return check_and_set_tx_fir_g(val, ret, len);
    }
    return len;
}
...
static ssize_t proc_set_serdes_rmt_lpbk(struct file *file, const char *buff, size_t len, loff_t *offset)
{
    ...
    if (ret >= 1)
    {
        if ((wan_serdes_type_get() == SERDES_WAN_TYPE_AE) || (wan_serdes_type_get() == SERDES_WAN_TYPE_AE_2_5G) ||
            (wan_serdes_type_get() == SERDES_WAN_TYPE_AE_10G) || (wan_serdes_type_get() == SERDES_WAN_TYPE_AE_5G) ||
            (wan_serdes_type_get() == SERDES_WAN_TYPE_AE_2_5G_R))
