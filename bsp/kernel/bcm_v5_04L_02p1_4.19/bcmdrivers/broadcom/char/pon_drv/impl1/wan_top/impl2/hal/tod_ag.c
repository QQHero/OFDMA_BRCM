...
static int bcm_tod_cli_set(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        tod_config_0 config_0 = { .cfg_ts48_pre_sync_fifo_disable=parm[1].value.unumber, .cfg_ts48_pre_sync_fifo_load_rate=parm[2].value.unumber, .cfg_tod_pps_clear=parm[3].value.unumber, .cfg_ts48_read=parm[4].value.unumber, .cfg_ts48_offset=parm[5].value.unumber, .cfg_ts48_enable=parm[6].value.unumber, .cfg_ts48_mac_select=parm[7].value.unumber};
        err = ag_drv_tod_config_0_set(&config_0);
        break;
    }
    case cli_tod_config_1:
    {
        tod_config_1 config_1 = { .cfg_tod_load_ns=parm[1].value.unumber, .cfg_tod_epon_read=parm[2].value.unumber, .cfg_tod_epon_read_sel=parm[3].value.unumber, .cfg_tod_load=parm[4].value.unumber, .cfg_tod_seconds=parm[5].value.unumber};
        err = ag_drv_tod_config_1_set(&config_1);
        break;
    }
    ...
    case cli_tod_config_3:
        ...
        break;
    case cli_tod_ns:
        err = ag_drv_tod_ns_set(parm[1].value.unumber);
        break;
    case cli_tod_mpcp:
        err = ag_drv_tod_mpcp_set(parm[1].value.unumber);
        break;
    default:
...
int bcm_tod_cli_get(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case cli_tod_config_1:
    {
        tod_config_1 config_1;
        err = ag_drv_tod_config_1_get(&config_1);
        bdmf_session_print(session, "cfg_tod_load_ns = %u (0x%x)\n", config_1.cfg_tod_load_ns, config_1.cfg_tod_load_ns);
        bdmf_session_print(session, "cfg_tod_epon_read = %u (0x%x)\n", config_1.cfg_tod_epon_read, config_1.cfg_tod_epon_read);
        bdmf_session_print(session, "cfg_tod_epon_read_sel = %u (0x%x)\n", config_1.cfg_tod_epon_read_sel, config_1.cfg_tod_epon_read_sel);
        bdmf_session_print(session, "cfg_tod_load = %u (0x%x)\n", config_1.cfg_tod_load, config_1.cfg_tod_load);
        bdmf_session_print(session, "cfg_tod_seconds = %u (0x%x)\n", config_1.cfg_tod_seconds, config_1.cfg_tod_seconds);
        break;
    }
    case cli_tod_msb:
    {
    ...
    {
        uint32_t cfg_tod_ns;
        err = ag_drv_tod_ns_get(&cfg_tod_ns);
    ...
    case cli_tod_mpcp:
    {
        ...
        err = ag_drv_tod_mpcp_get(&cfg_tod_mpcp);
...
static int bcm_tod_cli_test(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    {
        tod_config_0 config_0 = {.cfg_ts48_pre_sync_fifo_disable=gtmv(m, 1), .cfg_ts48_pre_sync_fifo_load_rate=gtmv(m, 5), .cfg_tod_pps_clear=gtmv(m, 1), .cfg_ts48_read=gtmv(m, 1), .cfg_ts48_offset=gtmv(m, 10), .cfg_ts48_enable=gtmv(m, 1), .cfg_ts48_mac_select=gtmv(m, 3)};
        if(!err) bdmf_session_print(session, "ag_drv_tod_config_0_set( %u %u %u %u %u %u %u)\n", config_0.cfg_ts48_pre_sync_fifo_disable, config_0.cfg_ts48_pre_sync_fifo_load_rate, config_0.cfg_tod_pps_clear, config_0.cfg_ts48_read, config_0.cfg_ts48_offset, config_0.cfg_ts48_enable, config_0.cfg_ts48_mac_select);
        ...
        if(err || config_0.cfg_ts48_pre_sync_fifo_disable!=gtmv(m, 1) || config_0.cfg_ts48_pre_sync_fifo_load_rate!=gtmv(m, 5) || config_0.cfg_tod_pps_clear!=gtmv(m, 1) || config_0.cfg_ts48_read!=gtmv(m, 1) || config_0.cfg_ts48_offset!=gtmv(m, 10) || config_0.cfg_ts48_enable!=gtmv(m, 1) || config_0.cfg_ts48_mac_select!=gtmv(m, 3))
            return err ? err : BDMF_ERR_IO;
    }
    {
        tod_config_1 config_1 = {.cfg_tod_load_ns=gtmv(m, 1), .cfg_tod_epon_read=gtmv(m, 1), .cfg_tod_epon_read_sel=gtmv(m, 2), .cfg_tod_load=gtmv(m, 1), .cfg_tod_seconds=gtmv(m, 19)};
        if(!err) bdmf_session_print(session, "ag_drv_tod_config_1_set( %u %u %u %u %u)\n", config_1.cfg_tod_load_ns, config_1.cfg_tod_epon_read, config_1.cfg_tod_epon_read_sel, config_1.cfg_tod_load, config_1.cfg_tod_seconds);
        if(!err) ag_drv_tod_config_1_set(&config_1);
        if(!err) ag_drv_tod_config_1_get( &config_1);
        if(!err) bdmf_session_print(session, "ag_drv_tod_config_1_get( %u %u %u %u %u)\n", config_1.cfg_tod_load_ns, config_1.cfg_tod_epon_read, config_1.cfg_tod_epon_read_sel, config_1.cfg_tod_load, config_1.cfg_tod_seconds);
        if(err || config_1.cfg_tod_load_ns!=gtmv(m, 1) || config_1.cfg_tod_epon_read!=gtmv(m, 1) || config_1.cfg_tod_epon_read_sel!=gtmv(m, 2) || config_1.cfg_tod_load!=gtmv(m, 1) || config_1.cfg_tod_seconds!=gtmv(m, 19))
            return err ? err : BDMF_ERR_IO;
    }
    {
    ...
    }
    {
        ...
        if(!err) bdmf_session_print(session, "ag_drv_tod_ns_set( %u)\n", cfg_tod_ns);
        if(!err) ag_drv_tod_ns_set(cfg_tod_ns);
        if(!err) ag_drv_tod_ns_get( &cfg_tod_ns);
        if(!err) bdmf_session_print(session, "ag_drv_tod_ns_get( %u)\n", cfg_tod_ns);
        if(err || cfg_tod_ns!=gtmv(m, 32))
            return err ? err : BDMF_ERR_IO;
    }
    {
        uint32_t cfg_tod_mpcp=gtmv(m, 32);
        if(!err) bdmf_session_print(session, "ag_drv_tod_mpcp_set( %u)\n", cfg_tod_mpcp);
        if(!err) ag_drv_tod_mpcp_set(cfg_tod_mpcp);
        if(!err) ag_drv_tod_mpcp_get( &cfg_tod_mpcp);
        if(!err) bdmf_session_print(session, "ag_drv_tod_mpcp_get( %u)\n", cfg_tod_mpcp);
...
static int bcm_tod_cli_address(bdmf_session_handle session, const bdmfmon_cmd_parm_t parm[], uint16_t n_parms)
{
    ...
    case bdmf_address_ns : reg = &RU_REG(TOD, NS); blk = &RU_BLK(TOD); break;
    case bdmf_address_mpcp : reg = &RU_REG(TOD, MPCP); blk = &RU_BLK(TOD); break;
    default :
...
bdmfmon_handle_t ag_drv_tod_cli_init(bdmfmon_handle_t driver_dir)
{
...

    {
        ...
        static bdmfmon_cmd_parm_t set_config_1[]={
            ...
            BDMFMON_MAKE_PARM("cfg_tod_epon_read", "cfg_tod_epon_read", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_MAKE_PARM("cfg_tod_epon_read_sel", "cfg_tod_epon_read_sel", BDMFMON_PARM_NUMBER, 0),
        ...
        static bdmfmon_cmd_parm_t set_ns[]={
            BDMFMON_MAKE_PARM("cfg_tod_ns", "cfg_tod_ns", BDMFMON_PARM_NUMBER, 0),
            BDMFMON_PARM_LIST_TERMINATOR
        };
        static bdmfmon_cmd_parm_t set_mpcp[]={
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="ns", .val=cli_tod_ns, .parms=set_ns },
            { .name="mpcp", .val=cli_tod_mpcp, .parms=set_mpcp },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        ...
        static bdmfmon_enum_val_t selector_table[] = {
            ...
            { .name="status_2", .val=cli_tod_status_2, .parms=set_default },
            { .name="ns", .val=cli_tod_ns, .parms=set_default },
            { .name="mpcp", .val=cli_tod_mpcp, .parms=set_default },
            BDMFMON_ENUM_LAST
        };
    ...
    }
    {
        static bdmfmon_enum_val_t enum_table_address[] = {
            ...
            { .name="NS" , .val=bdmf_address_ns },
            { .name="MPCP" , .val=bdmf_address_mpcp },
            BDMFMON_ENUM_LAST
        };
