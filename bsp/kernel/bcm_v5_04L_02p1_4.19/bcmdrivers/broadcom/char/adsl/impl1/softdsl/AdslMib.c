...
static uchar                   gRmcBitAlloc[MAX_GFAST_TONE_NUM];
static short                   gRts[MAX_GFAST_TONE_NUM];
...
Public Boolean XdslMibIsGfastMod(void *gDslVars)
{
    ...
    || (kXdslModMgfast == AdslMibGetModulationType(gDslVars))
#endif
    );
}
...
Private void XdslMibAdjustGfastMibDataPtr(void *gDslVars, int toLineNum)
{
    ...
    if(0 == toLineNum) {
        ...
        globalVar.rmcBitAlloc = &gRmcBitAlloc[0];
        globalVar.rts = &gRts[0];
    }
    else {
        ...
        globalVar.rmcBitAlloc = &gRmcBitAlloc[0] + gLineId(gDslVars) * (MAX_GFAST_TONE_NUM/2);
        globalVar.rts = &gRts[0] + gLineId(gDslVars) * (MAX_GFAST_TONE_NUM/2);
    }
}
#endif
...
Public void AdslMibResetConectionStatCounters(void *gDslVars)
{
    ...
    globalVar.adslMib.gfastDta.dtaEventCnt = 0;
    globalVar.adslMib.gfastDta.lastMdsIdx = 0;
    BlockByteClear (sizeof(globalVar.adslMib.gfastDta.lastMds), (void *) &globalVar.adslMib.gfastDta.lastMds[0]);
#endif
    ...
    case kDslTrainingStatus:
      ...
      switch (status->param.dslTrainingInfo.code) {
        ...
        case kDsl993p2Profile:
            ...
            if(XdslMibIsGfastMod(gDslVars)) {
                ...
                if(PROFILEGFAST212C == val)
                   globalVar.adslMib.vdslInfo[0].vdsl2Profile = kGfastProfile212c;
                else
                   globalVar.adslMib.vdslInfo[0].vdsl2Profile = val;
                globalVar.adslMib.xdslInfo.vdsl2Profile = (ushort)(globalVar.adslMib.vdslInfo[0].vdsl2Profile & 0xFFFF);
            }
            else
#endif
            {
            globalVar.adslMib.vdslInfo[0].vdsl2Profile = (val & 0x0FFF);
            globalVar.adslMib.xdslInfo.vdsl2Profile = (ushort)globalVar.adslMib.vdslInfo[0].vdsl2Profile;
            }
#if defined(CONFIG_VDSLBRCMPRIV1_SUPPORT)
#ifdef CONFIG_VDSLBRCMPRIV2_SUPPORT
            ...
            n = kVdslProfile35b;
#endif
            if(n == globalVar.adslMib.vdslInfo[0].vdsl2Profile) {
...
#ifdef CONFIG_VDSLBRCMPRIV2_SUPPORT
            else if(kVdslProfile35b == globalVar.adslMib.vdslInfo[0].vdsl2Profile)
...
#if defined(CONFIG_BCM_DSL_GFAST)
            if(XdslMibIsGfastMod(gDslVars)) {
                if((kGfastProfile212a == globalVar.adslMib.vdslInfo[0].vdsl2Profile) ||
                   (kGfastProfile212c == globalVar.adslMib.vdslInfo[0].vdsl2Profile) ||
                   (kMgfastProfileP424a == globalVar.adslMib.vdslInfo[0].vdsl2Profile) ||
                   (kMgfastProfileP424d == globalVar.adslMib.vdslInfo[0].vdsl2Profile) ||
                   (kMgfastProfileQ424c == globalVar.adslMib.vdslInfo[0].vdsl2Profile) ||
                   (kMgfastProfileQ424d == globalVar.adslMib.vdslInfo[0].vdsl2Profile)) {
...
#if defined(CONFIG_BCM_DSL_GFAST)
                if((kGfastProfile212a == globalVar.adslMib.xdslInfo.vdsl2Profile) ||
        ...
        case kDslG992p2TxShowtimeActive:
            ...
            globalVar.adslMib.gfastDta.dtaEventCnt = 0;
            globalVar.adslMib.gfastDta.lastMdsIdx = -1;
            BlockByteClear (sizeof(globalVar.adslMib.gfastDta.lastMds), (void *) &globalVar.adslMib.gfastDta.lastMds[0]);
#endif
#endif
            ...
            switch (val) {
               ...
               case kMgfastAnnexA:
                   ...
                   if(kGfastAnnexA == val)
                      ...
                      AdslMibSetModulationType(gDslVars, kXdslModMgfast);
    ...
    case kDslReceivedEocCommand:
      switch(status->param.dslClearEocMsg.msgId) {
        ...
        case    kDslGfastDtaInfo:
        {
          ...
          globalVar.adslMib.xdslInfo.dirInfo[DS_DIRECTION].lpInfo[0].M = pDta->currentMds;
          globalVar.adslMib.xdslInfo.dirInfo[US_DIRECTION].lpInfo[0].M = globalVar.adslMib.xdslInfo.dirInfo[US_DIRECTION].lpInfo[0].Mf - 1 - pDta->currentMds;
          if(-1 != globalVar.adslMib.gfastDta.lastMdsIdx) {
            globalVar.adslMib.gfastDta.lastMds[globalVar.adslMib.gfastDta.lastMdsIdx++] = globalVar.adslMib.gfastDta.curMds;
            if(globalVar.adslMib.gfastDta.lastMdsIdx >= MAX_LAST_Mds)
              globalVar.adslMib.gfastDta.lastMdsIdx = 0;
            globalVar.adslMib.gfastDta.dtaEventCnt++;
          }
          else
            globalVar.adslMib.gfastDta.lastMdsIdx = 0;
          break;
      ...
      case kDsl993p2BandPlanDsDump:
      {
        ...
        if(globalVar.waitBandPlan)
        {
          ...
          if(XdslMibIsGfastMod(gDslVars)) {
            ...
            BlockByteClear(sizeof(globalVar.rmcBitAlloc[0])*globalVar.nTonesGfast, (void*)globalVar.rmcBitAlloc);
            BlockByteClear(sizeof(globalVar.rts[0])*globalVar.nTonesGfast, (void*)globalVar.rts);
            globalVar.nTonesRmc = 0;
          }
#endif
      ...
      case kDslLineFeaturesFE:
      {
        ...
        break;
      case kDslLineLogicalFrameCfg:
      {
        LogicalFrameCfg *pDst, *pSrc = (LogicalFrameCfg *)status->param.dslClearEocMsg.dataPtr;
        n = status->param.dslClearEocMsg.msgType & kDslClearEocMsgLengthMask;
        n = (n < sizeof(LogicalFrameCfg)) ? n: sizeof(LogicalFrameCfg);
        if((0 == pSrc->direction) || (1 == pSrc->direction)) {
          pDst = (LogicalFrameCfg *)&globalVar.logicalFrameCfg[pSrc->direction];
          BlockByteMove(n, (uchar *)pSrc, (uchar *)pDst);
          pDst->sfCountApply = ADSL_ENDIAN_CONV_SHORT(pDst->sfCountApply);
        }
      }
        break;
#endif
      case kDsl993p2NeRmcBi:
      case kDsl993p2FeRmcBi:
      {
        uchar *pBuf, *pRmcBitAlloc;
        int nTones = status->param.dslClearEocMsg.msgType & 0xFFFF;
        
        nTones = MIN(nTones, globalVar.nTonesGfast);
        pBuf =(uchar *)(status->param.dslClearEocMsg.dataPtr);
        
        if(kDsl993p2FeRmcBi == status->param.dslClearEocMsg.msgId)
          pRmcBitAlloc = globalVar.rmcBitAlloc + globalVar.nTonesGfast/2;
        else {
          pRmcBitAlloc = globalVar.rmcBitAlloc;
          globalVar.nTonesRmc = nTones;
        }
        BlockByteMove(nTones, pBuf, pRmcBitAlloc);
        
        break;
      }
      case kDsl993p2NeRTSPhy:
      case kDsl993p2FeRTSPhy:
      {
        short *rtsBuff, *pRts;
        int nTones = (status->param.dslClearEocMsg.msgType & 0xFFFF) >> 1;
        
        nTones = MIN(nTones, globalVar.nTonesGfast);
        rtsBuff=(short *)(status->param.dslClearEocMsg.dataPtr);
        
        if(kDsl993p2FeRTSPhy == status->param.dslClearEocMsg.msgId)
          pRts = globalVar.rts + globalVar.nTonesGfast/2;
        else
          pRts = globalVar.rts;
        
        BlockShortMoveReverse(nTones, rtsBuff, pRts);
