...
#ifdef SUPPORT_HMI
					|| (kPwrReject == status->param.dslConnectInfo.value)
#endif
				)) {
...
void AdslCoreSetL3(uchar lineId)
{
	...
	else if (AdslCoreOvhMsgSupported(pDslVars))
...
//#define DELAY_DG_INTR
#ifdef  DELAY_DG_INTR

#define  DG_TOTAL_TIMEOUT   3000
#define  DG_READY_DELAY     1000

LOCAL int xdslCoreDelayEnableDGIntr = 0;
LOCAL OS_TICKS	dgStartTick, dgTimeout;
...
#endif
#ifdef DELAY_DG_INTR
	xdslCoreDelayEnableDGIntr = 1;
	bcmOsGetTime(&dgStartTick);
	dgTimeout = DG_TOTAL_TIMEOUT;
#else
		...
		if(BCM_IMAGETYPE_SINGLELINE == newVal) {
			...
			if((unsigned char)-1 != lineId) {
				...
				if ( (adslCoreCfgProfile[0].xdslCfg2Mask & adslCoreCfgProfile[0].xdslCfg2Value & kPhyCfg2PreferBondingOverPhyR) 
					 ...
					 && (0 == (pMibInfo->xdslStat[0].fireStat.status & (kFireDsEnabled | kFireUsEnabled)))
					 )
					lineReset = 1;
...
#ifdef DELAY_DG_INTR
		if (1 == xdslCoreDelayEnableDGIntr) {
		  OS_TICKS	dgCurTick;
		  bcmOsGetTime(&dgCurTick);
		  if (((dgCurTick - dgStartTick) * BCMOS_MSEC_PER_TICK) > dgTimeout) {
		    xdslCoreDelayEnableDGIntr = 0;
		    kerSysEnableDyingGaspInterrupt();
		  }
		}
...
#ifdef DELAY_DG_INTR
			case kDslReportDGReadyForConfig:
				if (1 == xdslCoreDelayEnableDGIntr) {
				  bcmOsGetTime(&dgStartTick);
				  dgTimeout = DG_READY_DELAY;
				}
				break;
#endif
...
{
    void *pDslVars = XdslCoreGetDslVars(lineId);
    pAdslRates->fastUpRate = AdslMibGetGetChannelRate (pDslVars, US_DIRECTION, kAdslFastChannel);
    pAdslRates->fastDnRate = AdslMibGetGetChannelRate (pDslVars, DS_DIRECTION, kAdslFastChannel);
    pAdslRates->intlUpRate = AdslMibGetGetChannelRate (pDslVars, US_DIRECTION, kAdslIntlChannel);
    pAdslRates->intlDnRate = AdslMibGetGetChannelRate (pDslVars, DS_DIRECTION, kAdslIntlChannel);
