...
//#define DISABLE_35b_BONDING
#endif

#ifdef DISABLE_35b_BONDING
int  BcmAdslCoreIs35bBlocked(unsigned char lineId);
void BcmAdslCoreIs35bBlock(unsigned char lineId);
void BcmAdslCoreIs35bUnblock(unsigned char lineId);
#endif
...
{
#if defined(BOARD_H_API_VER) && (BOARD_H_API_VER > 18)
	bcmbca_wd_start(timeUs);
#else
		...
		case kDslTrainingStatus:
			...
			switch (status->param.dslTrainingInfo.code) {
				...
				case kDsl993p2Profile:
					mibLen = sizeof(adslMibInfo);
					pMibInfo = (void *) AdslCoreGetObjectValue (lineId, NULL, 0, NULL, &mibLen);
					if ((PROFILE30e == status->param.dslTrainingInfo.value) && (kDslBondingPTM == pMibInfo->xdslStat[0].bondingStat.status)) {
						DiagStrPrintf(0,DIAG_DSL_CLIENT, "DISABLE_35b_BONDING: Retrain w/o 35b in bonding\n");
						BcmAdslCoreIs35bBlock(lineId);
...
#ifdef DISABLE_35b_BONDING
					if ((kDslBondingPTM != val) && BcmAdslCoreIs35bBlocked(lineId)) {
						DiagStrPrintf(0,DIAG_DSL_CLIENT, "DISABLE_35b_BONDING: Retrain with 35b no bonding\n");
						BcmAdslCoreIs35bUnblock(lineId);
						BcmAdslCoreConnectionStart(lineId);
					}
					break;
#endif
...
LOCAL void BcmAdslCoreSetConnectionParam(unsigned char lineId, dslCommandStruct *pDslCmd, dslCommandStruct *pCfgCmd, adslCfgProfile *pAdslCfg)
{
	...
	if (ADSL_PHY_SUPPORT(kAdslPhyGfast)) {
		...
		if ((modCfg & kDslCfgModMgfastOnly) && ADSL_PHY_SUPPORT(kAdslPhyMgfast))
			pDslCmd->param.dslModeSpec.capabilities.modulations |= kMgfastAnnexA;
#endif
...
#ifdef CONFIG_MGFAST_SUPPORT
				if (ADSL_PHY_SUPPORT(kAdslPhyMgfast)) {
					pG993p2Cap->profileSel |= (pAdslCfg->vdslParam1 << 16) & kMgfastProfileMask;
				}
#endif
			}
#endif
	...
	if (NULL != pAdslCfg)
	{
		...
		unsigned short	bpGpioAFELDRelay;
#endif
#if defined(CONFIG_BCM_DSL_GFASTCOMBO) && defined(CONFIG_BCM963138)
		/* Old 63138 PHY versions need special setting to enable Gfast */
		{
		adslPhyInfo		*pInfo = AdslCoreGetPhyInfo();

		if ((pInfo->mjVerNum != 0) && (pInfo->mjVerNum <= 43)) {
			pAdslCfg->xdslCfg3Mask |= 0x8000;
			pAdslCfg->xdslCfg3Value|= 0x8000; 		
		}
		}
#endif
	...
	if (NULL != pAdslCfg) {
		if( NULL != pDslCmd->param.dslModeSpec.capabilities.carrierInfoG992p5AnnexA ) {
			...
			pG992p5Cap->maxDownPTM_TPS_TC[2] = (pAdslCfg->maxDsDataRateKbps <= 0x3FFFC) ? pAdslCfg->maxDsDataRateKbps >> 2 : 0;  /* 4000 BPS */
			pG992p5Cap->maxUpPTM_TPS_TC[2]   = (pAdslCfg->maxUsDataRateKbps <= 0x3FFFC) ? pAdslCfg->maxUsDataRateKbps >> 2 : 0;
			pG992p5Cap->maxDownPTM_TPS_TC[3] = (pAdslCfg->maxAggrDataRateKbps <= 0x7FFF8) ? pAdslCfg->maxAggrDataRateKbps >> 3 : 0; /* 8000 BPS */
		}
	}
#endif
}
...
int BcmAdslCoreIs35bBlocked(unsigned char lineId)
{
	...
	return (pG993p2Cap->profileSel & kVdslProfile35b) != (pAdslCoreCfgProfile[lineId]->vdslParam & kVdslProfile35b);
}
...
void BcmAdslCoreIs35bBlock(unsigned char lineId)
{
	...
	pG993p2Cap->profileSel &= ~kVdslProfile35b;
}
...
void BcmAdslCoreIs35bUnblock(unsigned char lineId)
{
	...
	if (pAdslCoreCfgProfile[lineId]->vdslParam & kVdslProfile35b)
		pG993p2Cap->profileSel |= kVdslProfile35b;
}
#endif /* DISABLE_35b_BONDING */
