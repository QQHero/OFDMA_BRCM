 ...
 * :>
 */
/* Copyright (C) 2019 - 2020 - NComm of New Hampshire, Inc. */

/*
 * TUNABLE parameters.  You probably don't need/want to touch these.
...
#else
#ifdef EXTRA_TARGET_STUFF_2
	EXTRA_TARGET_STUFF_2;
#endif
#endif
...
/*--------------------------------------------------------------------------*/
	...
	#define NCOMM_TXPKT(mytype, mywhere, nameSpace, myskb, mydev) \
		NF_HOOK_COND(mytype, mywhere, \
			nameSpace, NULL,\
			myskb, NULL, mydev, my_dev_xmit, 0)
			//brcm: Avoid crash in bcm_br_blog_hook()
...
/*--------------------------------------------------------------------------*/
	...
	#define _PKT_ALL			NFPROTO_NETDEV
	#define _PKT_STATION			NFPROTO_NETDEV
	...
	#define _PKT_BRIDGE			NFPROTO_BRIDGE
	#define _PKT_ALL			(-1)
	#define _PKT_STATION			NFPROTO_BRIDGE
...
/*--------------------------------------------------------------------------*/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 13, 0)
	#define UPDATE_HOOKNUM(hooknum, pf) \
		((pf == NFPROTO_NETDEV) ? NF_NETDEV_INGRESS : hooknum)
#else
	#define UPDATE_HOOKNUM(in_hooknum, in_pf) (in_hooknum)
#endif
...
/*--------------------------------------------------------------------------*/
/*
 * Define the module entry and exit routines 
 */
...
/*--------------------------------------------------------------------------*/
/*
 * Define interface for retrieving network stats
 */
...
uint64_t retval = 0;
	...
	dev = devPtr;
        ...
        if (dev->ethtool_ops && dev->ethtool_ops->get_link_ksettings) {
                ...
                if (!(*dev->ethtool_ops->get_link_ksettings)(dev, &cmd)) {

                        retval = ((uint64_t)cmd.base.speed * 1000000) / 8;
                }
        }
#else
...
/*--------------------------------------------------------------------------*/
/*
 * This is a linux version independent transmit function.
 */
...
unsigned char *p;
	...
	if ((skb = alloc_skb(macLen + datLen + ((vlanTag != 0) ? 4 : 0), 
			GFP_ATOMIC)) == NULL) {
	...
	if(vlanTag != 0) {
		skb_reserve(skb, macLen + 4);

		p = skb_push(skb, macLen + 4);
		memcpy(p, macPtr, macLen - 2);
		p += macLen - 2;
		...
		*p++ = (protocol >> 0) & 0xff;
		...
		p = skb_push(skb, macLen);
		memcpy(p, macPtr, macLen);
	}
...
int packet_type;		/* type of packet */
	...
	/* brcm: we turn off VLAN_TAG_PRESENT in exLinuxETH_bcm1agHandleFrameCheck().
	   So now we check skb->vlan_tci instead of only checking VLAN_TAG_PRESENT bit. */
	//if(skb_vlan_tag_present(skb)) {
	if(skb->vlan_tci) {
		vlanTagPresent = 1;
		vlanTag = 
			/* brcm: fix vlan priority is missing when receiving vlan packet. */
...
unsigned int hooknum;
	...
	hooknum = UPDATE_HOOKNUM(hooknum, pf);
...
change_hook:
	...
	if ((i = NF_REGISTER_NET_HOOK(nameSpace, hookPtr)) < 0) {
...
int exLinuxETH_getSocket(void *nameSpace, void *return_pointer)
{
int rc;
struct socket **mySocket = return_pointer;

	*mySocket = NULL;

        rc = sock_create_kern(nameSpace,
                                AF_INET,
                                SOCK_STREAM,
                                IPPROTO_TCP,
				mySocket);

	if((rc != 0) || (*mySocket == NULL)) {
		return(0);
	}

	return(1);
}
EXPORT_SYMBOL(exLinuxETH_getSocket);

void exLinuxETH_socketRelease(void *socket)
{
	sock_release(socket);
}
EXPORT_SYMBOL(exLinuxETH_socketRelease);

int exLinuxETH_getFile(void *socket, void **file)
{
void *p;
	...
	*file = NULL;
	...
	p = sock_alloc_file(socket, O_RDWR, NULL);
...
EXPORT_SYMBOL(exLinuxETH_getFile);

int exLinuxETH_addVlan(void *file, void *userSpace, int vid, void *device) 
{
struct vlan_ioctl_args vlan_request;
int rc;
struct net_device *dev = device;
	...
	if((file == NULL) || (userSpace == NULL) || (device == NULL)) {
		return(0);
	}

	memset(&vlan_request, 0, sizeof(vlan_request));

	vlan_request.cmd = SET_VLAN_NAME_TYPE_CMD;
	vlan_request.u.name_type = VLAN_NAME_TYPE_RAW_PLUS_VID_NO_PAD;

	rc = copy_to_user(userSpace, 
		(void *)(&vlan_request), 
		sizeof(vlan_request));
	...
	rc = vfs_ioctl(file,
			SIOCSIFVLAN,	    	/* Execute vlan command */
			(unsigned long)(userSpace));  /* argment */
	...
	vlan_request.u.VID = vid;

	strncpy(&vlan_request.device1[0], dev->name,
		sizeof(vlan_request.device1)/sizeof(vlan_request.device1[0]));
	...
	rc = vfs_ioctl(file,
			SIOCSIFVLAN,	    /* create vlan command */
			(unsigned long)(userSpace));  /* argment */
...
EXPORT_SYMBOL(exLinuxETH_addVlan);

/*
 * undo what exLinuxETH_addVlan did
 */

int exLinuxETH_delVlan(void *file, void *userSpace, int vid, void *device) 
{
struct vlan_ioctl_args vlan_request;
int rc;
struct net_device *dev = device;
	...
	rc = snprintf(&vlan_request.device1[0],
		sizeof(vlan_request.device1)/sizeof(vlan_request.device1[0]),
		"%s.%d", dev->name, vid);

	if((rc < 0) ||
	 (rc >= sizeof(vlan_request.device1)/sizeof(vlan_request.device1[0]))) {

		/*
		 * Size of name is too big, so just delete socket
		 */
		return(0);
	}
...
EXPORT_SYMBOL(exLinuxETH_delVlan);
...
static int protocollist[] = {
	PROTOCOL_LIST
};

int exLinuxETH_GetProcolList(int *pl_size, int *pl_list)
{
int protocol_list_size = sizeof(protocollist)/sizeof(protocollist[0]);
	...
	*pl_size = protocol_list_size;

	memcpy(pl_list, &protocollist[0], sizeof(protocollist));

	return(1);
}
EXPORT_SYMBOL(exLinuxETH_GetProcolList);
...
int exLinuxETH_bcm1agHandleFrameCheck(struct sk_buff *skb)
{
	...
	if(is1agFrame)
	{
		/* TMS will handle VLAN tags. Turn off VLAN_TAG_PRESENT bit to avoid
		   Linux kernel reset vlan_tci field. */
