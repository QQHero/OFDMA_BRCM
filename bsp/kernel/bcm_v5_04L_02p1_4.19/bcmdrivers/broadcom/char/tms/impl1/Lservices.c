...
/*
 * The following routine handles the new timer architecture.
 */

typedef void (*FUNCTION_CALL)(unsigned long);

static void *timerFunction = NULL;
static unsigned long userData = 0;

static void _timerWrapper(struct timer_list *Timer)
{
FUNCTION_CALL userFunction;

	if(timerFunction == NULL) {
		return;
	}

	userFunction = (FUNCTION_CALL)(timerFunction);
...
#else
        /* Nothing needed for earlier versions linux */
#endif
...
unsigned long del;
	...
	*t = timer;
	...
	timerFunction = fooptr;
	userData = (unsigned long)data;
	timer_setup_on_stack(timer, _timerWrapper, 0);
#else
...
#if 1 /* brcm: move task name operation to external module to keep binary compatibility. */
void LservicesUpdateTaskName(char *name)
{

	/* Linux-ize the name
	 */
	current->comm[0] = (char)'k';

	memcpy(&current->comm[1], name, 4);

	current->comm[5] = 0;

}
EXPORT_SYMBOL(LservicesUpdateTaskName);
#endif
