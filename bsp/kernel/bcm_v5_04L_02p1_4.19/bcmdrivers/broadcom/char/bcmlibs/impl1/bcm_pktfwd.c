    ...
    void * driver, const char * driver_name, uint32_t unit)
{
    ...
    PKTLIST_TRACE(PKTFWD_VRP_FMT " Constructed %px for %s",
        ...
        void * driver, uint32_t domain)
{
    ...
    PKTQUEUE_TRACE(PKTFWD_VRP_FMT " Constructed %px for %d",
...
{
    printk("\t ext %px flow_key " D3LUT_KEY_FMT
...
d3lut_bflt_init(uint32_t d3lut_bflt_elem_total)
{
    ...
    D3LUT_TRACE("mem %px elem_total %u", d3lut_bflt, d3lut_bflt->elem_total);
...
d3lut_pool_init(d3lut_pool_t * d3lut_pool, uint32_t d3lut_elem_tot)
{
    ...
    D3LUT_TRACE("elem mem %px", d3lut_elem_base);
    ...
    for (pool = 0U; pool < D3LUT_POOL_TOT; ++pool)
    {
        ...
        D3LUT_TRACE("pool %u head %px tail %px free %u base %px %s",
    ...
    D3LUT_TRACE("Constructed Elem base %px mem_bytes %u pools %u elems max %u",
...
d3lut_pool_fini(d3lut_elem_t * d3lut_elem_base)
{
    ...
    D3LUT_TRACE("Destructing Elem base %px ...", d3lut_elem_base);
...
__d3lut_elem_get(d3lut_t * d3lut, uint32_t pool, d3lut_policy_t d3lut_policy)
{
    ...
    if (d3lut_pool->elem_free > 0)
    {
        ...
        D3LUT_PTRACE("elem %px " D3LUT_KEY_FMT "free %u",
...
d3lut_get(d3lut_t * d3lut, uint32_t pool, d3lut_policy_t d3lut_policy)
{
    ...
    d3lut_elem = d3lut_get_nolock(d3lut, pool, d3lut_policy);
...
d3lut_elem_t *  /* Allocate element from free pool */
d3lut_get_nolock(d3lut_t * d3lut, uint32_t pool, d3lut_policy_t d3lut_policy)
{
    d3lut_elem_t * d3lut_elem;
...
__d3lut_elem_put(d3lut_t * d3lut, d3lut_elem_t * d3lut_elem)
{
    ...
    D3LUT_PTRACE("elem %px " D3LUT_KEY_FMT,
...
d3lut_put(d3lut_t * d3lut, d3lut_elem_t * d3lut_elem)
{
    ...
    d3lut_put_nolock(d3lut, d3lut_elem);
...
void /* Free an element back to free pool: with lock */
d3lut_put_nolock(d3lut_t * d3lut, d3lut_elem_t * d3lut_elem)
{
...
_d3lut_dict_init(void)
{
    ...
    D3LUT_TRACE("Constructed Dict mem %px last %u bkts %u bins %u", d3lut_dict,
...
_d3lut_dict_fini(d3lut_dict_t * d3lut_dict)
{
    ...
    if (d3lut_dict != D3LUT_DICT_NULL)
    {
        D3LUT_TRACE("Destructing Dict mem %px ...", d3lut_dict);
...
d3lut_init_success:
    ...
    D3LUT_TRACE(PKTFWD_VRP_FMT " Constructed LUT %px",
...
d3lut_fini(d3lut_t * d3lut)
{
    ...
    D3LUT_TRACE("Desctructing LUT %px ...", d3lut);
...
{
    printk("\t Elem %px pool %u " D3LUT_ELEM_FMT "\n",
...
d3lut_dump(d3lut_t * d3lut)
{
    ...
    if (dump_verbose == true)
    {
        ...
        int i;
        d3lut_elem_t * d3lut_elem;
        for (i = 0; i < D3LUT_ELEM_TOT; i++) 
        {
            d3lut_elem = D3LUT_TABLE_ELEM(d3lut->elem_base, i);
            if (d3lut_elem->ext.inuse)
                d3fwd_elem_dump(d3lut_elem);
        }
#endif
    }   /* dump_verbose */
...
d3lut_k2e(d3lut_t * d3lut, d3lut_key_t key)
{
    ...
    /* compare the retreived key with the supplied key to ensure the provided
     * entry is not stale
     */
...
EXPORT_SYMBOL(d3lut_put_nolock);
EXPORT_SYMBOL(d3lut_get);
EXPORT_SYMBOL(d3lut_get_nolock);
