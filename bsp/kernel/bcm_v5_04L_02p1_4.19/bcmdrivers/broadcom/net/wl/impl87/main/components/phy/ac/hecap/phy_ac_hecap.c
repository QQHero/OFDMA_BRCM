...
#define HE_PHY_SOUND_DIM_BELOW80 (ACMAJORREV_51_131(pi->pubpi->phy_rev) ? 0x1 : \
				  ACMAJORREV_129(pi->pubpi->phy_rev) ? 0x2 : 0x3)
/* Number of sounding dimensions for > 80MHz */
#define HE_PHY_SOUND_DIM_ABOVE80 (ACMAJORREV_131(pi->pubpi->phy_rev) ? 0x1 : \
				  ACMAJORREV_51_129(pi->pubpi->phy_rev) ? 0x0 : 0x3)
/* Maximum Nc for beamforming sounding feedback */
#define HE_PHY_MAX_NC (ACMAJORREV_51_131(pi->pubpi->phy_rev) ? 0x1 : \
		       ACMAJORREV_129(pi->pubpi->phy_rev) ? 0x2 : 0x3)

#define HE_DEFAULT_NPP_DURATION		2	/* set to 2 for nominal packet padding 16usec */
...
/**
 * 43684b0/c0 and 6715a0:
 * 80MHz, 160Mhz: tx/rx max mcs 0..11x4
 * 80p80: not supported
 */
static const wlc_he_rateset_t he_rateset_rev47_130 = {
	/* .bw80_tx_mcs_nss = */
...
/**
 * 6756:
 * 80MHz, 160Mhz: tx/rx max mcs 0..11x2
 * 80p80: not supported
 */
static wlc_he_rateset_t he_rateset_rev131 = {
	/* .bw80_tx_mcs_nss = */
	(HE_CAP_MAX_MCS_0_11 << 0)  | (HE_CAP_MAX_MCS_0_11 << 2)  |
	(HE_CAP_MAX_MCS_NONE << 4)  | (HE_CAP_MAX_MCS_NONE << 6)  |
	(HE_CAP_MAX_MCS_NONE << 8)  | (HE_CAP_MAX_MCS_NONE << 10) |
...
phy_ac_hecap_get_phycap_info(phy_type_hecap_ctx_t *ctx, he_phy_cap_t *phycap)
{
	...
	uint32 sflags;
	uint32 phyaccap = wlc_phy_cap_get(pi);
	uint8 bwcap = 0;
	...
	/* b22: Full BW UL-MUMIMO */
	if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
		setbit((uint8 *)phycap, HE_PHY_FULL_BW_UL_MU_IDX);
	}

	/* b23: Partial BW UL-MUMIMO, not supported */
	...
	/* b77: Rx Full BW SU Using HE MU PPDU Non-Compressed SIGB */

	/* b78-b79: Nominal Packet Padding to two for 16usec */
	setbits((uint8 *)phycap, sizeof(*phycap), HE_PHY_NOMINAL_PACKET_PADDING_IDX,
		HE_PHY_NOMINAL_PACKET_PADDING_FSZ, HE_DEFAULT_NPP_DURATION);
}
