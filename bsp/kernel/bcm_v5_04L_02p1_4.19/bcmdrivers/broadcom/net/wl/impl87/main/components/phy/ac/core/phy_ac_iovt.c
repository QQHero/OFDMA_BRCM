 ...
 *
 * $Id: phy_ac_iovt.c 810513 2022-04-08 06:25:38Z $
 */
...
BCMATTACHFN(phy_ac_register_iovt)(phy_info_t *pi, phy_type_info_t *ti, wlc_iocv_info_t *ii)
{
	...
	/* Register RxIQCal module ACPHY iovar table/handlers */
	if ((err = phy_ac_rxiqcal_register_iovt(pi, ii)) != BCME_OK) {
		PHY_ERROR(("%s: phy_ac_rxiqcal_register_iovt failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register TXSS/TXSHAPER module ACPHY iovar table/handlers */
	if ((err = phy_ac_txss_register_iovt(pi, ii)) != BCME_OK) {
		PHY_ERROR(("%s: phy_ac_txss_register_iovt failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register noise module ACPHY iovar table/handlers */
	if ((err = phy_ac_noise_register_iovt(pi, ii)) != BCME_OK) {
		PHY_ERROR(("%s: phy_ac_noise_register_iovt failed\n", __FUNCTION__));
		goto fail;
	}
