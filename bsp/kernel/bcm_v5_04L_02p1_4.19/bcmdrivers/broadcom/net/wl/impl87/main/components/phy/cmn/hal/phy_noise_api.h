 ...
 *
 * $Id: phy_noise_api.h 810501 2022-04-08 03:34:50Z $
 */
...
void wlc_phy_interf_rssi_update(phy_info_t *pi, chanspec_t chanNum, int8 leastRSSI);
#endif
/* HWACI Interrupt Handler */
void wlc_phy_hwaci_mitigate_intr(phy_info_t *pih);

/* Interference and Mitigation (ACI, Noise) Module */
int phy_noise_interf_chan_stats_upd(phy_info_t *pi, chanspec_t chanspec, uint32 crsglitch,
	uint32 bphy_crsglitch, uint32 badplcp, uint32 bphy_badplcp, uint8 txop, uint32 mbsstime);
int phy_noise_sched_set(phy_info_t *pih, phy_bgnoise_schd_mode_t reason, bool upd);
bool phy_noise_sched_get(phy_info_t *pih);
int phy_noise_pmstate_set(phy_info_t *pih, bool pmstate);
int8 phy_noise_avg_per_antenna(phy_info_t *pih, int coreidx);
int8 phy_noise_avg(phy_info_t *wpi);
int8 phy_noise_lte_avg(phy_info_t *wpi);
int phy_noise_sample_request_external(phy_info_t *ppi);
int phy_noise_sample_request_crsmincal(phy_info_t *ppi);
int phy_noise_sample_intr(phy_info_t *ppi);
int phy_noise_hwknoise_isenabled(phy_info_t *pi);
