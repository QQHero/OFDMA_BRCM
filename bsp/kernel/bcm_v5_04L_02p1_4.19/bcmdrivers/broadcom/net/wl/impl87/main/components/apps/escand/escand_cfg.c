                      ...
                      unsigned int vector_size)
{
	...
	*/
	...
	if (list_length >= ESCAND_MAX_VECTOR_LEN) {
		vector_length = ESCAND_MAX_VECTOR_LEN;
	} else {
		vector_length = list_length;
	}
	strncpy(chan_buf, channel_list, vector_length);
	strncat(chan_buf, ",", vector_length);
...
escand_retrieve_config(escand_chaninfo_t *c_info, char *prefix)
{
	...
	if ((str = nvram_get(strcat_r(prefix, "escand_ci_scan_type", tmp))) == NULL)
		c_info->escand_ci_scan_type = ESCAND_DFLT_CI_SCAN_TYPE;
	else
		c_info->escand_ci_scan_type = atoi(str);
	...
	if ((str = nvram_get(strcat_r(prefix, "escand_ci_busy_min_scan_timer", tmp))) == NULL)
		c_info->escand_ci_busy_min_scan_timer = ESCAND_BUSY_MIN_CI_SCAN_TIMER;
	else
		c_info->escand_ci_busy_min_scan_timer = atoi(str);

	if ((str = nvram_get(strcat_r(prefix, "escand_scan_promisc", tmp))) == NULL)
		c_info->escand_scan_promisc = 1;
	else
		c_info->escand_scan_promisc = atoi(str);

	if ((str = nvram_get(strcat_r(prefix, "escand_traffic_thresh", tmp))) == NULL)
		c_info->escand_traffic_thresh = -1;
	else
		c_info->escand_traffic_thresh = atoi(str);

	if ((str = nvram_get(strcat_r(prefix, "escand_traffic_type", tmp))) == NULL)
		c_info->escand_traffic_type = 0;
	else
		c_info->escand_traffic_type = atoi(str);
	...
	escand_dump_config_extra(c_info);
}
