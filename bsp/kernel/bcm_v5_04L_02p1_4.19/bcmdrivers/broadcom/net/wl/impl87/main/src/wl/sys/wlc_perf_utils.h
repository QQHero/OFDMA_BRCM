 ...
 *
 * $Id: wlc_perf_utils.h 805611 2021-12-02 06:38:21Z $
 */
...
#define MAX_NSS		4
struct wlc_txrx_summary_counters {
	uint64 phyrate;		/* running total of phyrate */
	uint64 bytes;		/* running total of bytes */
	uint32 bw;		/* running total of bw of successful packets */
	uint32 nss[MAX_NSS];	/* running total of nss of sucessful packets */
...
typedef struct {
	uint32 time_delta;
	wlc_txrx_summary_counters_t tx;
	wlc_txrx_summary_counters_t rx;
} wlc_txrx_summary_scb_counters_t;

typedef wlc_txrx_summary_scb_counters_t wlc_txrx_summary_scb_cubby_t;
extern wlc_txrx_summary_scb_counters_t *wlc_txrx_summary_scb_counters(struct wlc_info *wlc,
	struct scb *scb);
...
#ifdef GAME_SPEEDUP
void wlc_txq_special(wlc_info_t *wlc, special_scb_t *special_scb, uint8 txop_percent);
#endif
