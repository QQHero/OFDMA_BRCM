...
({ \
	...
	PCIEDEV_MAX_FLOW_STATS_COUNT] = (MSGBUF_WR(flowring) & 0xffff) | \
		((MSGBUF_RD(flowring) & 0xffff) << 16); \
...
#define FLOW_RING_SUP_PEND_SET(msgbuf) \
	((msgbuf)->status |= FLOW_RING_SUP_PENDING)
#define FLOW_RING_SUP_PEND_ISSET(msgbuf) \
	((msgbuf)->status & FLOW_RING_SUP_PENDING)
#define FLOW_RING_SUP_PEND_RESET(msgbuf) \
	((msgbuf)->status &= ~FLOW_RING_SUP_PENDING)

#define FLOW_RING_FLUSH_PEND_SET(msgbuf) \
	((msgbuf)->status |= FLOW_RING_FLUSH_PENDING)
#define FLOW_RING_FLUSH_PEND_ISSET(msgbuf) \
	((msgbuf)->status & FLOW_RING_FLUSH_PENDING)
#define FLOW_RING_FLUSH_PEND_RESET(msgbuf) \
	((msgbuf)->status &= ~FLOW_RING_FLUSH_PENDING)
...
extern uint16  sqs_v2r_request(void* arg, uint16 ringid, uint16 v2r_request, bool sqs_force);
...
extern void pciedev_sqs_stride_resume(struct dngl_bus *pciedev);
...
#ifdef PCIE_HBQD
/* When PCIE_HBQD is enabled, value: 16bit:qd and 16bit:idx */
#define MSGBUF_RD(msgbuf)               (MSGBUF_RD_PTR(msgbuf)->idx)
#define MSGBUF_WR(msgbuf)               (MSGBUF_WR_PTR(msgbuf)->idx)
#define MSGBUF_QD(msgbuf)               (MSGBUF_WR_PTR(msgbuf)->qd)
#define MSGBUF_QDWR(msgbuf)             (MSGBUF_WR_PTR(msgbuf)->value)
#define MSGBUF_QDRD(msgbuf)             (MSGBUF_RD_PTR(msgbuf)->value)

/* SQS: Value of WR index since the last time vpkts were computed */
#define MSGBUF_WR_PEEKED(msgbuf)        ((msgbuf)->wr_peeked.value)
#define MSGBUF_WR_IDX_PEEKED(msgbuf)    ((msgbuf)->wr_peeked.idx)
#define MSGBUF_WR_QD_PEEKED(msgbuf)     ((msgbuf)->wr_peeked.qd)
#else /* !PCIE_HBQD */
...
#define MSGBUF_WR_IDX_PEEKED(msgbuf)    MSGBUF_WR_PEEKED(msgbuf)
#endif /* !PCIE_HBQD */

#define MSGBUF_TID(msgbuf)			((msgbuf)->flow_info.tid_ac)
...
#ifdef PCIE_HBQD
#define MSGBUF_VPKTS_INCREMENT(msgbuf) \
	({ \
		pcie_rw_index_t new_wr = {.value = MSGBUF_QDWR(msgbuf)}; \
		pcie_rw_index_t old_wr = {.value = MSGBUF_WR_PEEKED(msgbuf)}; \
		int vpkts  = NTXPACTIVE(old_wr.idx, new_wr.idx, MSGBUF_MAX(msgbuf)); \
		vpkts += (new_wr.qd - old_wr.qd); \
		ASSERT(vpkts >= 0); \
		MSGBUF_WR_PEEKED(msgbuf) = new_wr.value; \
		(vpkts); \
	})
#define MSGBUF_SQS_AVAIL_SPACE(msgbuf) \
	({ \
		pcie_rw_index_t new_wr = {.value = MSGBUF_QDWR(msgbuf)}; \
		int avail = \
			NTXPACTIVE((msgbuf)->fetch_ptr, new_wr.idx, MSGBUF_MAX(msgbuf)); \
		avail += (new_wr.qd - MSGBUF_WR_QD_PEEKED(msgbuf)); \
		ASSERT(avail >= 0); \
		(avail); \
	})
#else /* !PCIE_HBQD */
#define MSGBUF_VPKTS_INCREMENT(msgbuf) \
	({ \
		...
		int old_wr = MSGBUF_WR_IDX_PEEKED(msgbuf); \
		int vpkts  = NTXPACTIVE(old_wr, new_wr, MSGBUF_MAX(msgbuf)); \
		MSGBUF_WR_IDX_PEEKED(msgbuf) = new_wr; \
		(vpkts); \
	})
#define MSGBUF_SQS_AVAIL_SPACE(msgbuf) \
...
#define MSGBUF_VPKTS_INCREMENT(msgbuf) \
	({ \
		...
		MSGBUF_WR_IDX_PEEKED(msgbuf) = \
			((old_wr + vpkts) == MSGBUF_MAX(msgbuf)) ? 0 : new_wr; \
		(vpkts); \
	})
#define MSGBUF_SQS_AVAIL_SPACE(msgbuf)	MSGBUF_READ_AVAIL_SPACE(msgbuf)
#endif /* !HWA_TXPOST_BUILD */

#define	MSGBUF_VPKTS_ENQ(msgbuf) \
	({\
		uint16 v_pkts; \
		v_pkts = MSGBUF_VPKTS_INCREMENT(msgbuf); \
		ASSERT(SCB_FLOWID_VALID(msgbuf->scb_flowid)); \
		if (v_pkts) { \
			wlc_sqs_vpkts_enqueue((msgbuf)->scb_flowid, MSGBUF_TID(msgbuf), v_pkts); \
...
#define MSGBUF_VPKTS(msgbuf) \
	({ \
		uint16 v_pkts; \
		if (SCB_FLOWID_VALID(msgbuf->scb_flowid)) { \
			v_pkts = wlc_sqs_vpkts((msgbuf)->scb_flowid, MSGBUF_TID(msgbuf)); \
		} else { \
			v_pkts = MSGBUF_SQS_AVAIL_SPACE(msgbuf); \
		} \
		(v_pkts); \
	})
...
typedef struct pciedev_dbg_bus_cntrs {
	...
	uint32 pciedev_v2r_fetch_packets;
...
#define PCIEDEV_FLOW_FETCH_FLAG_EOPS_RQST	0x04
#define PCIEDEV_FLOW_FETCH_FLAG_SQS_FORCE	0x08

#define PCIEDEV_FLOW_FETCH_FLAG_SQS_FORCE_SET(fetch_rqst) \
	((fetch_rqst)->flags |= PCIEDEV_FLOW_FETCH_FLAG_SQS_FORCE)
#define PCIEDEV_FLOW_FETCH_FLAG_SQS_FORCE_ISSET(fetch_rqst) \
	((fetch_rqst)->flags & PCIEDEV_FLOW_FETCH_FLAG_SQS_FORCE)

#define PCIEDEV_FLOW_FETCH_FLAG_EOPS_RQST_SET(fetch_rqst) \
	((fetch_rqst)->flags |= PCIEDEV_FLOW_FETCH_FLAG_EOPS_RQST)
#define PCIEDEV_FLOW_FETCH_FLAG_EOPS_RQST_ISSET(fetch_rqst) \
	((fetch_rqst)->flags &PCIEDEV_FLOW_FETCH_FLAG_EOPS_RQST)
...
/*
 * When Compact Work Item "-acwi" target is enabled, there is no marker field
 * in D2H completion messages for TxCpln or RxCpln, and the explicit D2H Sync
 * mechanism cannot be supported.
 */
...
#if defined(PCIE_D2H_DOORBELL_RINGER)
	/** doorbell ringing using sbtopcie translation host memory write */
...
#endif /* PROP_TXSTATUS */
	flow_fetch_rqst_t * last_flowfetchp;
};
...
extern void pciedev_sync_dmaindex_d2h_rd(struct dngl_bus * pciedev, msgbuf_ring_t* ring);
#endif /* SBTOPCIE_INDICES */
#endif /* PCIE_DMA_INDEX */
...
void pciedev_scb_flow_delink(struct dngl_bus *pciedev, uint16 cfp_flowid);
...
extern int sqs_fetch_packets_force(struct dngl_bus *pciedev, msgbuf_ring_t *msgbuf);
extern void pciedev_rewind_flow_fetch_ptr(msgbuf_ring_t *flow_ring, uint16 index);
extern void pciedev_flow_ring_ps_update(struct dngl_bus * pciedev, uint16 ringid, bool ps_on);
