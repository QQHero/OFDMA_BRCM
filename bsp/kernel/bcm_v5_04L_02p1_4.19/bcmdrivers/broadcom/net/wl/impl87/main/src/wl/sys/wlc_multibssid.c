 ...
 *
 * $Id: wlc_multibssid.c 805952 2021-12-09 08:47:14Z $
 */
#include <wlc_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
static uint8 *wlc_multibssid_locate_profile(wlc_info_t *wlc,
  dot11_mbssid_ie_t *mbssid_ie, uint8 current_cnt, uint16 *prof_buf_len);
static uint8 *wlc_multibssid_match_bcnie_in_nontransprofile(wlc_info_t *wlc,
  wlc_iem_tag_t bcn_ie_tag, uint8 *prof_buf, uint16 prof_buf_len, uint32 *prof_ies_bitmap);
static int wlc_multibssid_update_bcn(wlc_info_t *wlc,
  struct dot11_management_header *hdr, uint8 *body, uint body_len,
  uint8 *prof_buf, uint16 prof_buf_len, uint8 new_bssid_lsb);

static bool wlc_is_multibssid_profile_split(uint8 *prof_buf, uint16 prof_buf_len,
  uint8 **next_profile_add, uint8 *next_profile_len);

uint8 wlc_multibssid_tag_non_inherited(dot11_mbssid_non_inheritance_t *non_inheritance_ie,
...
/* Return TRUE if Non-Transmitted BSS pofile is split between
 * two Multiple BSSID IEs
 */
static bool
wlc_is_multibssid_profile_split(uint8 *prof_buf, uint16 prof_buf_len,
		uint8 **next_profile_add, uint8 *next_profile_len)
{
	uint8 *next_mbssid_ie;
	wlc_iem_tag_t next_ie_tag;
	dot11_mbssid_cap_t *next_mbssid_cap_ie = NULL;

	next_mbssid_ie = prof_buf + prof_buf_len;
	next_ie_tag = wlc_iem_get_id(next_mbssid_ie);

	if (next_ie_tag == (wlc_iem_tag_t)DOT11_MNG_MULTIPLE_BSSID_ID) {
		*next_profile_add = next_mbssid_ie + MULTIBSSID_IDX_IE_LEN3 + TLV_HDR_LEN;
		*next_profile_len = *(next_mbssid_ie + MULTIBSSID_IDX_IE_LEN3 + 1u);

		next_mbssid_cap_ie = (dot11_mbssid_cap_t *)bcm_parse_tlvs(*next_profile_add,
				*next_profile_len, DOT11_MNG_NONTRANS_BSSID_CAP_ID);
	...
	struct dot11_management_header *hdr, uint8 current_cnt, dot11_mbssid_ie_t *mbssid_ie)
{
	...
	uint8 *next_profile_add, next_profile_len;
	uint8 *new_prof_buf = NULL;
	uint16 new_prof_buf_len = 0, prof_buf_len;
	...
	if (prof_buf) {
		if (wlc_is_multibssid_profile_split(prof_buf, prof_buf_len,
			&next_profile_add, &next_profile_len)) {

			new_prof_buf_len = prof_buf_len + next_profile_len;
			new_prof_buf = (uint8 *)MALLOCZ(wlc->osh, new_prof_buf_len);
			if (!new_prof_buf) {
				WL_SCAN_ERROR((WLC_MALLOC_ERR, WLCWLUNIT(wlc),
					"wlc_multibssid_handling_eiscan",
					new_prof_buf_len, MALLOCED(wlc->osh)));
				return BCME_NOMEM;
			}

			(void)memcpy(new_prof_buf, prof_buf, prof_buf_len);
			(void)memcpy(new_prof_buf + prof_buf_len, next_profile_add,
				next_profile_len);
			prof_buf = new_prof_buf;
			prof_buf_len = new_prof_buf_len;
		}
	}
	...
	if (new_prof_buf != NULL) {
		MFREE(wlc->osh, new_prof_buf, new_prof_buf_len);
		new_prof_buf = NULL;
	}
	return BCME_OK;
}
...
wlc_multibssid_locate_profile(wlc_info_t *wlc, dot11_mbssid_ie_t *mbssid_ie,
  uint8 current_cnt, uint16 *prof_buf_len)
{
	uint8 *prof_buf, i;
	int allprofiles_len;
...
{
	uint8 *prof_buf, lsb_bits, mbssid_index, mbssid_n;
	...
	if (prof_buf) {
		if (wlc_is_multibssid_profile_split(prof_buf, prof_buf_len,
			...
			if (!new_prof_buf) {
				WL_SCAN_ERROR((WLC_MALLOC_ERR, WLCWLUNIT(wlc),
					"wlc_multibssid_recv_nontransmitted_ssid_bssid malloc fail",
					new_prof_buf_len, MALLOCED(wlc->osh)));
				return NULL;
			}
   ...
   dot11_mbssid_ie_t *mbssid_ie, uint8 current_cnt, uint8 lastbyte)
{
	...
	if (current_cnt) {
		...
		if (!mbssid_index_ie) {
			bcm_tlv_t *next_mbssid_index_ie = NULL;

			if (wlc_is_multibssid_profile_split(prof_buf, prof_buf_len,
				&next_profile_add, &next_profile_len)) {

				next_mbssid_index_ie = bcm_parse_tlvs(next_profile_add,
						next_profile_len, DOT11_MNG_MULTIPLE_BSSID_IDX_ID);
				if (!next_mbssid_index_ie) {
					WL_SCAN_ERROR(("Wrong! No MBSSID index IE!\n"));
					return BCME_ERROR;
				}
				mbssid_index_ie = next_mbssid_index_ie;
			}
		}
		if (!mbssid_index_ie) {
			return BCME_ERROR;
	...
	uint8 *prof_buf, uint16 prof_buf_len, uint8 new_bssid_lsb)
{
	...
	dot11_mbssid_non_inheritance_t *non_inheritance_ie = NULL;
	uint8 *next_profile_add, next_profile_len;
	uint8 *new_prof_buf = NULL;
	uint16 new_prof_buf_len = 0;
	int err = BCME_OK;
	...
	if (prof_buf) {
		...
		non_inheritance_ie = (dot11_mbssid_non_inheritance_t *)bcm_parse_tlvs_ext(prof_buf,
			prof_buf_len, DOT11_MNG_ID_EXT_ID, EXT_MNG_MBSSID_NON_INHERITANCE_ID);
	}
	/* update ies */
	...
	for (; tags_len >= (int)(TLV_HDR_LEN + tags[TLV_LEN_OFF]);
		...
		if (non_inheritance_ie) {
			if (wlc_multibssid_tag_non_inherited(non_inheritance_ie, ie_tag)) {
				continue;
			}
		}
		if (prof_buf) {
	...
	err = ies_recreated_len + DOT11_BCN_PRB_LEN;
...
uint8
wlc_multibssid_tag_non_inherited(dot11_mbssid_non_inheritance_t *non_inheritance_ie,
		wlc_iem_tag_t bcn_ie_tag)
{
	uint8 ie_len = 0;
	uint8 ext_ie_len = 0;
	uint8 *ie_list = NULL;
	uint8 *ext_ie_list = NULL;
	uint i = 0;
	bool skip_ie = FALSE;

	ie_len = non_inheritance_ie->var_list[0];
	ie_list = &(non_inheritance_ie->var_list[1]);
	for (i = 0; i < ie_len; i ++)
	{
		if (ie_list[i] == bcn_ie_tag) {
			skip_ie = TRUE;
			goto done;
		}
	}
	ext_ie_len = ie_list[ie_len];
	ext_ie_list = ie_list + ie_len + 1; // 1 byte is length field of Element ID Extension List

	for (i = 0; i < ext_ie_len; i++)
	{
		if ((ext_ie_list[i] + DOT11_MNG_ID_EXT_ID) == bcn_ie_tag) {
			skip_ie = TRUE;
			goto done;
		}
	}
  ...
  uint body_len, uint8 bssid_lastbyte, uint8 prof_index)
{
	...
	bcm_tlv_t *mbssid_index_ie, *next_mbssid_index_ie = NULL;
			...
			if (mbssid_index_ie == NULL) {
				...
				if (wlc_is_multibssid_profile_split(prof_buf, prof_buf_len,
					...
					next_mbssid_index_ie = bcm_parse_tlvs(next_profile_add,
							...
							DOT11_MNG_MULTIPLE_BSSID_IDX_ID);
					if (next_mbssid_index_ie &&
					(prof_index == next_mbssid_index_ie->data[0])) {
						return wlc_multibssid_update_bcn(wlc, hdr,
								body, body_len,
								prof_buf, prof_buf_len,
								bssid_lastbyte);
