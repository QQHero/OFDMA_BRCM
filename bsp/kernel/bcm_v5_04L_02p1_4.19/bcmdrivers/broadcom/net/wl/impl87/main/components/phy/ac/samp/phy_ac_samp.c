 ...
 *
 * $Id: phy_ac_samp.c 810162 2022-03-31 23:36:57Z $
 */
...
static int phy_ac_samp_capture_enable(phy_type_samp_ctx_t *ctx, uint32 *save_clkgatests,
	uint32 start_ptr, uint32 end_ptr);
static int phy_ac_samp_capture_disable(phy_type_samp_ctx_t *ctx);
#if defined(SAMPLE_COLLECT) || defined(WL_PROXDETECT) || defined(DBG_SAMPCAP)
void phy_ac_disab_mac_clkgate_sc(phy_info_t *pi, uint32 *savereg);
void phy_ac_enab_mac_clkgate_sc(phy_info_t *pi, uint32 restorereg);
#endif /* SAMPLE_COLLECT || WL_PROXDETECT || defined(DBG_SAMPCAP) */
...
#ifdef WL_PROXD_PHYTS
	fns.sampcap_enable = phy_ac_samp_capture_enable;
	fns.sampcap_disable = phy_ac_samp_capture_disable;
#endif /* WL_PROXD_PHYTS */
...
phy_ac_set_startptr(phy_ac_samp_info_t *sampi, uint32 start_idx)
{
	...
	W_REG(pi->sh->osh, D11_SCP_STRTPTR(pi), (uint16)(start_idx & 0xFFFF));
...
phy_ac_set_stopptr(phy_ac_samp_info_t *sampi, uint32 stop_idx)
{
	...
	W_REG(pi->sh->osh, D11_SCP_STOPPTR(pi), (uint16)(stop_idx & 0xFFFF));
	ptr_high = R_REG(pi->sh->osh, D11_SMP_PTR_H(pi)) & ~0xF0;
	ptr_high |= (stop_idx >> 12) & 0xF0;
	W_REG(pi->sh->osh, D11_SMP_PTR_H(pi), (uint16)ptr_high);
}
	...
	switch (mo)
	{
		...
		case SC_MODE_5s_iq_comp:
			...
			if (mo == SC_MODE_5s_iq_comp)
				md_datapath_os = (ACMAJORREV_4(pi->pubpi->phy_rev) ||
		...
		const uint32 sc_buff_start)
{
	...
	if (collect->mode == SC_MODE_12_gpio_trans) {
		...
		if (D11REV_IS(pi->sh->corerev, 50) || D11REV_GE(pi->sh->corerev, 54)) {
			W_REG(pi->sh->osh, D11_SMP_CTRL(pi), (uint16)(play_ctl | (1 << 0)));
		} else {
			W_REG(pi->sh->osh, D11_PSM_PHY_CTL(pi), (uint16)(phy_ctl | (1 << 4)));
		}
		OSL_DELAY(1000000);
	} else {
		...
		do {
			...
			if (phy_ac_get_currptr(sampi) == prev_currptr &&
				...
				if (D11REV_IS(pi->sh->corerev, 50) ||
					...
					W_REG(pi->sh->osh,
						D11_SMP_CTRL(pi),
						(uint16)(play_ctl & 0xFFFA));
				} else {
					W_REG(pi->sh->osh,
						D11_PSM_PHY_CTL(pi),
						(uint16)(phy_ctl & 0xFFCF));
				}
...
phy_ac_sample_collect(phy_type_samp_ctx_t *ctx, wl_samplecollect_args_t *collect, void *buf)
{
	...
	if (ACMAJORREV_2(pi->pubpi->phy_rev)) {
		...
		if ((sicoreunit == DUALMAC_MAIN) || ACMAJORREV_128(pi->pubpi->phy_rev)) {
			szbytes = 256*1024;
			sc_buff_offs = 192 * 1024 >> 2;
		} else {
			szbytes = 112*1024;
	...
	switch (collect->mode)
	{
		...
		case  SC_MODE_4s_rx_farrow_1core:
			...
			/* Special mode to mux in pktproc (CRDOT11ACPHY-590) */
			if (ACMAJORREV_4(pi->pubpi->phy_rev) ||
				ACMAJORREV_GE47(pi->pubpi->phy_rev)) {
	...
	if (ACMAJORREV_128(pi->pubpi->phy_rev)) {
		...
		W_REG(pi->sh->osh, D11_PSMPowerReqStatus(pi),
			(uint16)(readreg & ~(1<<AUTO_MEM_STBY_RET_SHIFT)));
		...
		W_REG(pi->sh->osh, D11_CLK_GATE_REQ0(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_REQ1(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_REQ2(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_REQ0(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_REQ1(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_REQ2(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_STRETCH0(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_STRETCH1(pi), (uint16)0x2000);
		W_REG(pi->sh->osh, D11_CLK_GATE_MISC(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_DIV_CTRL(pi), (uint16)0x22);
		W_REG(pi->sh->osh, D11_CLK_GATE_PHY_CLK_CTRL(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_STS(pi), (uint16)0x9853);
		W_REG(pi->sh->osh, D11_CLK_GATE_EXT_REQ0(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_EXT_REQ1(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_EXT_REQ2(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_PHY_CLK_CTRL(pi), (uint16)0x0);
	}

	if (ACMAJORREV_128(pi->pubpi->phy_rev)) {
		W_REG(pi->sh->osh, D11_CLK_GATE_STS(pi), (uint16)0x0013);
	}
	PHY_TRACE(("wl%d: %s Start capture, trigger = %d\n", pi->sh->unit, __FUNCTION__,
		collect->trigger));

	if (ACMAJORREV_GE47(pi->pubpi->phy_rev)) {
		collect->timeout = 65500;
	}
	timer = collect->timeout;
	...
	if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
	    ACMAJORREV_33(pi->pubpi->phy_rev)) {
		W_REG(pi->sh->osh, D11_SMP_CTRL(pi), (uint16)(1 << 11));
	} else if (ACMAJORREV_GE47(pi->pubpi->phy_rev) &&
		!ACMAJORREV_128(pi->pubpi->phy_rev)) {
		if (mo == 0x1ff) {
			W_REG(pi->sh->osh, D11_SMP_CTRL(pi), (uint16)(1 << 0));
		} else {
			W_REG(pi->sh->osh, D11_SMP_CTRL(pi), (uint16)((1 << 0) | (1 << 2) |
	...
	if (collect->trigger == TRIGGER_NOW) {
		...
		W_REG(pi->sh->osh, D11_PSM_BRED_3(pi), (uint16)(bred | 0x10));
		dur_1_8th_us = collect->post_dur << 3;
		W_REG(pi->sh->osh, D11_TSF_GPT_2_CTR_L(pi), (uint16)(dur_1_8th_us & 0x0000FFFF));
		W_REG(pi->sh->osh, D11_TSF_GPT_2_CTR_H(pi), (uint16)(dur_1_8th_us >> 16));
		...
		if (D11REV_IS(pi->sh->corerev, 50) || D11REV_GE(pi->sh->corerev, 54)) {
			...
			W_REG(pi->sh->osh, D11_PSM_PHY_CTL(pi), (uint16)(phy_ctl & 0xFFCF));
		}
	...
	if (ACMAJORREV_128(pi->pubpi->phy_rev)) {
		...
		W_REG(pi->sh->osh, D11_CLK_GATE_REQ0(pi), (uint16)0xffff);
		W_REG(pi->sh->osh, D11_CLK_GATE_REQ1(pi), (uint16)0xffff);
		W_REG(pi->sh->osh, D11_CLK_GATE_REQ2(pi), (uint16)0x3ff);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_REQ0(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_REQ1(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_REQ2(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_STRETCH0(pi), (uint16)0x1010);
		W_REG(pi->sh->osh, D11_CLK_GATE_STRETCH1(pi), (uint16)0x2000);
		W_REG(pi->sh->osh, D11_CLK_GATE_MISC(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_DIV_CTRL(pi), (uint16)0x27);
		W_REG(pi->sh->osh, D11_CLK_GATE_PHY_CLK_CTRL(pi), (uint16)0x7fff);
		W_REG(pi->sh->osh, D11_CLK_GATE_EXT_REQ0(pi), (uint16)0xffff);
		W_REG(pi->sh->osh, D11_CLK_GATE_EXT_REQ1(pi), (uint16)0xff);
		W_REG(pi->sh->osh, D11_CLK_GATE_EXT_REQ2(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_PHY_CLK_CTRL(pi), (uint16)0x0);
	...
	W_REG(pi->sh->osh, D11_PSM_PHY_CTL(pi), (uint16)0x2);
	...
	ptr[3] = htol32(0xFFFF0000 | (pi->pubpi->phy_rev << 8) |
		((ACMAJORREV_GE47(pi->pubpi->phy_rev) && !ACMAJORREV_128(pi->pubpi->phy_rev)) ?
		PHY_TYPE_AX : pi->pubpi->phy_type));
...
#if defined(SAMPLE_COLLECT) || defined(WL_PROXDETECT)
void
phy_ac_disab_mac_clkgate_sc(phy_info_t *pi, uint32 *savereg)
{
	if (D11REV_GE(pi->sh->corerev, 40) && D11REV_LE(pi->sh->corerev, 127)) {
		*savereg = R_REG(pi->sh->osh, D11_CLK_GATE_STS(pi));

		if (!D11REV_IS(pi->sh->corerev, 86)) {
			AND_REG(pi->sh->osh, D11_PSMPowerReqStatus(pi),
				(uint16)(~(1 << D11_AUTO_MEM_STBY_RET_SHIFT_REV(pi->sh->corerev))));
		}

		AND_REG(pi->sh->osh, D11_POWERCTL(pi), ~(1 << PWRCTL_ENAB_MEM_CLK_GATE_SHIFT));
		OR_REG(pi->sh->osh, D11_POWERCTL(pi), (1 << PWRCTL_AUTO_MEM_STBYRET));
		AND_REG(pi->sh->osh, D11_BMCCTL(pi), (uint16)(~(1 << 8)));
...
void
phy_ac_enab_mac_clkgate_sc(phy_info_t *pi, uint32 restore)
{
	if (D11REV_GE(pi->sh->corerev, 40) && D11REV_LE(pi->sh->corerev, 127)) {
		/* Restore clkgatests register */
		if (!D11REV_IS(pi->sh->corerev, 86)) {
			OR_REG(pi->sh->osh, D11_PSMPowerReqStatus(pi),
				(uint16)(1 << D11_AUTO_MEM_STBY_RET_SHIFT_REV(pi->sh->corerev)));
		}

		OR_REG(pi->sh->osh, D11_POWERCTL(pi),
			(1 << PWRCTL_ENAB_MEM_CLK_GATE_SHIFT) |
			(1 << PWRCTL_AUTO_MEM_STBYRET));
		OR_REG(pi->sh->osh, D11_BMCCTL(pi), (uint16)(1 << 8));
		...
		W_REG(pi->sh->osh, D11_CLK_GATE_EXT_REQ2(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_UCODE_PHY_CLK_CTRL(pi), (uint16)0x0);
		W_REG(pi->sh->osh, D11_CLK_GATE_STS(pi), (uint16)restore);
	}
}
#endif /* SAMPLE_COLLECT || WL_PROXDETECT */

static int
phy_ac_samp_capture_enable(phy_type_samp_ctx_t *ctx, uint32 *save_clkgatests,
	uint32 start_ptr, uint32 end_ptr)
{
	phy_ac_samp_info_t *sampi = (phy_ac_samp_info_t *)ctx;
	phy_info_t *pi = sampi->pi;
	...
	MOD_PHYREG(pi, AdcDataCollect, adcDataCollectEn, 1u);
	/* Disable mac clock gating */
	phy_ac_disab_mac_clkgate_sc(pi, save_clkgatests);
	if (D11REV_GE(pi->sh->corerev, 40) && D11REV_LE(pi->sh->corerev, 127)) {
		W_REG(pi->sh->osh, D11_CLK_GATE_STS(pi), (uint16)0x0013u);
	}
	/* Set Tx-FIFO collect start pointer to sc_buff_start */
	phy_ac_set_startptr(sampi, start_ptr);
	phy_ac_set_stopptr(sampi, end_ptr);
	/* Clear ucode trigger-based sample collect procedure */
	wlapi_bmac_write_shm(pi->sh->physhim, M_SMPL_COL_BMP(pi), 0x0u);
	/* Set Start bit (start capture) */
	play_ctl = R_REG(pi->sh->osh, D11_SMP_CTRL(pi));
	...
	W_REG(pi->sh->osh, D11_SMP_CTRL(pi),
		(uint16)(play_ctl | (1u << 0u)));

	return ret;
}

static int
phy_ac_samp_capture_disable(phy_type_samp_ctx_t *ctx)
{
	phy_ac_samp_info_t *sampi = (phy_ac_samp_info_t *)ctx;
	phy_info_t *pi = sampi->pi;
	int ret = BCME_OK;
	...
	play_ctl = R_REG(pi->sh->osh, D11_SMP_CTRL(pi));
	play_ctl &= ~((1u << 0u) | (1u << 2u));
	W_REG(pi->sh->osh, D11_SMP_CTRL(pi), play_ctl);
	MOD_PHYREG(pi, AdcDataCollect, adcDataCollectEn, 0u);
