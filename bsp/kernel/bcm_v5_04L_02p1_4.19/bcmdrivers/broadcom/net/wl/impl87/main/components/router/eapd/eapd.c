 ...
 *
 * $Id: eapd.c 805327 2021-11-23 02:05:37Z $
 */
			...
			if (bytes > ETHER_HDR_LEN) {
				...
				if (cb) {
					...
					cevent_generic_hub(nwksp, ifname, cb,
							(uint8 *)eth, bytes, FALSE, CEVENT_ST_WPS);
#endif /* BCM_CEVENT */
				}
			}
		}
...
eapd_brcm_dispatch(eapd_wksp_t *nwksp, eapd_brcm_socket_t *from, uint8 *pData, int Len)
{
	 ...
	 * event. Hence send CEVENT_ST_LAST in below function.
	 */
	cevent_app_handle_event(nwksp, pData, Len, from->ifname, TRUE, CEVENT_ST_LAST);
#endif /* BCM_CEVENT */
...
/* #endif */
	...
	case EAPOL_START:
		...
		if (eapd_ceventd_enable) {
			dest_app = CEVENT_ST_EAPD;  /* EAPOL Start packet is consumed by EAPD */
...
#ifdef BCM_CEVENT
	cevent_generic_hub(nwksp, from->ifname, cb, (uint8 *)eapol, len, TRUE, CEVENT_ST_WAI);
#endif /* BCM_CEVENT */
	return;
}
#endif /* BCMWAPI_WAI */
