...
#ifndef WLSQS
#error "Bad configuration, PCIE FULL DONGLE build Need WLSQS"
#endif /* !WLSQS */
...
static int  sqs_vpkts_dump(struct dngl_bus *pciedev, uint16 scb_flowid);

static int /* wl bus:sqs_vpkts debug dump the vpkts for flowrings */
sqs_vpkts_dump(struct dngl_bus *pciedev, uint16 scb_flowid)
{
	...
	while (!dll_end(prio, &pciedev->active_prioring_list)) {
		...
		while (!dll_end(cur, &prioring->active_flowring_list)) {
			...
			/* Check for matching scb_flowid */
			if ((scb_flowid != (uint16)SCB_ALL_FLOWS) &&
				(msgbuf->scb_flowid != scb_flowid)) {
				cur = dll_next_p(cur);
				continue;
			}
			...
			printf("   tid<%u> vpkts<%u> v2r<%u>\n",
				tid_ac,
				wlc_sqs_vpkts(msgbuf->scb_flowid, tid_ac),
				wlc_sqs_v2r_pkts(msgbuf->scb_flowid, tid_ac));
...
volatile void *regs, uint bustype)
{
	...
	/* Initialize the Silicon Backplane to PCIE Translation service
	 * 128-MB PCIE space accessible via two 64-MB windows or four 32-MB windows
	 */
	if (sbtopcie_init(pciedev->osh, pciedev->corerev, pciedev->coreid,
	                  pciedev->sih->chip, pciedev->regs) <= 0) {
		PCI_TRACE(("%s: sbtopcie_init failed\n", __FUNCTION__));
		goto fail;
	}

#ifdef SW_PAGING
	/* Setup sb2pcie for initial hostmem access before HME is ready */
	if (sw_paging_bind_pcie() == BCME_ERROR) {
		PCI_TRACE(("%s: sw_paging_bind_pcie failed\n", __FUNCTION__));
		goto fail;
	}
#endif /* SW_PAGING */
...
BCMATTACHFN(pciedev_tunables_init)(struct dngl_bus *pciedev)
{
	...
	STATIC_ASSERT(PCIEDEV_MAX_PACKETFETCH_COUNT <= HWA_TXPOST_SCHED_COUNT_MAX);
#endif
...
#ifdef PCIE_HBQD
#ifndef HWA_TXPOST_BUILD
#error "PCIE_HBQD not supported"
#endif
	pciedev->pcie_ipc->dcap2 |= PCIE_IPC_DCAP2_PCIE_HBQD;
#else /* !PCIE_HBQD */
 ...
 * the PCIE complex via the SBTOPCIE. Host side PCIE RC will route the
...
{
	*((uint32*)db_addr) = db_data; /* write to host addr via sbtopcie */
}

void
pciedev_d2h_txcpl_doorbell_toggle(struct dngl_bus *pciedev)
{
	d2h_doorbell_ringer_t *ringer;

	ringer = &pciedev->d2h_doorbell_ringer[BCMPCIE_D2H_MSGRING_TX_COMPLETE_IDX];

	PCI_ERROR(("%s():" HADDR64_FMT " val 0x%08x HWA TX CPLE\n",
		__FUNCTION__, HADDR64_VAL(ringer->db_info.haddr), ringer->db_info.value));
...
BCMATTACHFN(pciedev_setup_pcie_ipc)(struct dngl_bus *pciedev)
{
	...
	/* Size of a WR or RD index is defined by "hbqd" */
...
pciedev_link_pcie_ipc(struct dngl_bus *pciedev)
{
...
#ifdef SW_PAGING
	/* SW paging releases sbtopcie private window and switches to use BME dma */
	if ((ret = sw_paging_link_pcie()) != BCME_OK) {
		PCI_ERROR(("Link SWP failure\n"));
		goto link_failure;
	}
#endif /* SW_PAGING */
...
#ifdef PCIE_HBQD
	/* Host acknowledges updating Queue Depth in DMA index */
	if (pcie_ipc->hcap2 & PCIE_IPC_HCAP2_PCIE_HBQD) {
		PCI_PRINT(("Host updates Queue Depth in WR DMA index\n"));
	}
#endif /* PCIE_HBQD */
	...
	if ((ret = pciedev_flowrings_link_pcie_ipc(pciedev)) != BCME_OK) {
...
BCMATTACHFN(pciedev_idma_bind_pcie_ipc)(struct dngl_bus *pciedev)
{
	...
	size = BCMPCIE_IDMA_BYTES_PER_SET;
	PCI_PRINT(("%s: using %d iDMA flows per set\n", __FUNCTION__,
		size/sizeof(pcie_rw_index_t)));
...
pciedev_idma_link_pcie_ipc(struct dngl_bus *pciedev)
{
	...
	while (dmaindex->inited && index < MAX_IDMA_DESC) {
		PCI_ERROR(("link idma index%d "
			"size %d" HADDR64_FMT " daddr64 lo 0x%08x\n",
...
BCMATTACHFN(pciedev_setup_flowrings)(struct dngl_bus *pciedev)
{
	...
	for (i = 0; i < pciedev->max_flowrings; i++) {
		...
		flowring->scb_flowid		= SCB_FLOWID_INVALID;
	}
	...
	msgbuf_ring_t *ring)
{
	 ...
	 */
	if (pciedev->dmaindex_d2h_rd.inited) {
		pciedev_sync_dmaindex_d2h_rd(pciedev, pciedev->dtoh_ctrlcpl);
	}
#endif /* PCIE_DMA_INDEX */
...
#define D11_REG_OFF	0x120	/* maccontrol */
	...
	/* BUZZZ control for CR4, CA7 */
		...
		*outlen = ret_len;
		return BCME_OK;
#if defined(PCIE_D2H_DOORBELL_RINGER)
	} else if (!strcmp(cmd, "txcpl_toggle") && !set) {
		pciedev_d2h_txcpl_doorbell_toggle(pciedev);
#endif
	} else {
		return BCME_UNSUPPORTED;
	}
...
pciedev_process_flow_ring_create_rqst(struct dngl_bus * pciedev, void *p)
{
	...
	/**
	 * Link a flow ring with SCB flowid.
	 * Store CFP TCB state ptr in BUS layer for easy access.
	 */
	{
		int response;

		ASSERT(flow_ring->scb_flowid == SCB_FLOWID_INVALID);

		response = dngl_flowring_update(pciedev->dngl, flow_ring_create_req->msg.if_id,
				flowid, flow_ring_create_req->tid,
				flow_ring_create_req->da, FLOW_RING_CREATE,
				&flow_ring->tcb_state, &flow_ring->scb_flowid,
				&flow_ring->flow_info.iftype_ap);

		if (response != BCME_OK) {
			PCI_ERROR(("SCB link failed for flowid %d response code  %d \n",
				flowid, response));
			status = BCMPCIE_NOTFOUND;
			goto send_resp;
		}

		PCI_TRACE(("%s: Ringid %d tid %d scb flowid %d \n",
			__FUNCTION__, flowid, flow_ring_create_req->tid,
			flow_ring->scb_flowid));
	}
	...
	MSGBUF_TID(flow_ring) = flow_ring_create_req->tid;
...
send_resp:
	if (flow_ring && (status != BCMPCIE_SUCCESS)) {
		/* De-Link the flowring and SCB flow */
		dngl_flowring_update(pciedev->dngl, flow_ring_create_req->msg.if_id,
			flowid, flow_ring_create_req->tid,
			flow_ring_create_req->da,
			FLOW_RING_DELETE, NULL, &(flow_ring->scb_flowid), NULL);
...
#ifdef MEM_ALLOC_STATS
	hnd_get_heapuse(&mu);
	mu.total_flowring_alloc -= mu.arena_free - freemem_before;
	hnd_update_mem_alloc_stats(&mu);
#endif /* MEM_ALLOC_STATS */
...
pciedev_flush_flow_ring(struct dngl_bus * pciedev, uint16 flowid)
{
	...
	/* Forcefully fetch packets outside of scheduler */
	sqs_fetch_packets_force(pciedev, flow_ring);
	...
	if (flow_ring->flow_info.pktinflight) {
		...
		dngl_flowring_update(pciedev->dngl, flow_ring->flow_info.ifindex,
			flow_ring->ringid, MSGBUF_TID(flow_ring),
			(uint8 *)flow_ring->flow_info.da,
			FLOW_RING_FLUSH, NULL, &(flow_ring->scb_flowid), NULL);
		...
		if (flow_ring->flow_info.pktinflight) {
			dngl_flowring_update(pciedev->dngl, flow_ring->flow_info.ifindex,
				flow_ring->ringid, MSGBUF_TID(flow_ring),
				(uint8 *)flow_ring->flow_info.da, FLOW_RING_FLUSH_TXFIFO,
				NULL, &(flow_ring->scb_flowid), NULL);
		}
	}
...
#else /* ! PROP_TXSTATUS */
		flow_ring->status |= FLOW_RING_FAST_DELETE_ACTIVE;
		flow_ring->delete_idx = MSGBUF_RD(flow_ring);
#endif /* ! PROP_TXSTATUS */
	}
...
pciedev_free_msgbuf_flowring(struct dngl_bus *pciedev, msgbuf_ring_t *flow_ring)
{
	...
	if (flow_ring != NULL) {
		...
		MSGBUF_WR_PEEKED(flow_ring) = 0;

#if defined(PCIE_DMA_INDEX) && defined(SBTOPCIE_INDICES)
		/* Sync the read index with host mem */
		pciedev_sync_dmaindex_h2d_rd(pciedev, flow_ring);
...
#endif /* PCIEDEV_SUPPR_RETRY_TIMEOUT */
		flow_ring->scb_flowid = SCB_FLOWID_INVALID;
	}
}
 ...
 *
 * NOTE: A SBTOPCIE Translation window is reserved for soft doorbell.
 * When DoR is enabled along with HWA Completion Engines, the HWA4b TxCple and
 * HWA2b RxCple would directly wakes Runner (i.e. CPU subsystem does not ring
 * the doorbell). HWA2b,4b uses a DMA to write 4B, and allows for a full 64b
 * addressing to the physical addresses of Runner registers - i.e. the DMA is
 * not performed to a SBTOPCIE translation window, as done by the CPU subsystem.
 * Consequently, SBTOPCIE_USR_RNR need not reserve as mode private and use the
 * shared mode (i.e. switch before use) in pciedev_d2h_doorbell_ringer()
 */
void
	...
	const ring_config_req_t *ring_config_req)
{
	...
	d2h_doorbell_ringer_t *ringer = &pciedev->d2h_doorbell_ringer[d2h_ring_idx];

	uint64 db_haddr_u64;
	HADDR64_TO_U64(db_info->haddr, db_haddr_u64); // convert haddr64 to uint64

	PCI_INFORM(("D2H ring_id %d haddr" HADDR64_FMT " value 0x%08x\n", ring_id,
		HADDR64_VAL(db_info->haddr), db_info->value));

	if (HADDR64_IS_ZERO(db_info->haddr)) { /* Soft Doorbell not requested */
		use_default_ringer = TRUE;
	} else if (pciedev->d2h_doorbell_ringer_inited != TRUE) {
		if (sbtopcie_setup(SBTOPCIE_USR_RNR, db_haddr_u64, sizeof(uint32),
				SBTOPCIE_MODE_PRIVATE) == SBTOPCIE_ERROR) {
			PCI_ERROR(("%s" HADDR64_FMT " overflows window\n",
				__FUNCTION__, HADDR64_VAL(db_info->haddr)));
			use_default_ringer = TRUE;
		...
		/* Ascertain that d2h doorbell is in configured window */
		if (!sbtopcie_valid(SBTOPCIE_USR_RNR, db_haddr_u64, sizeof(uint32))) {
			PCI_ERROR(("%s" HADDR64_FMT " not in window\n",
				__FUNCTION__, HADDR64_VAL(db_info->haddr)));
			sbtopcie_dump();
			use_default_ringer = TRUE;
			resp_status = BCMPCIE_BADOPTION;
	...
	if (use_default_ringer) { /* Default PCIE doorbell interrupt */
		...
		if (hwa_dev) {
			...
			if (ring_id == BCMPCIE_D2H_MSGRING_TX_COMPLETE) {
				...
				PCI_ERROR(("%s():" HADDR64_FMT " val 0x%08x HWA TX CPLE\n",
					__FUNCTION__, HADDR64_VAL(db_haddr64), db_info->value));
				...
				hwa_sw_doorbell_request(hwa_dev, HWA_RXCPL_DOORBELL,
					...
					db_haddr64, db_info->value);
				PCI_ERROR(("%s():" HADDR64_FMT " val 0x%08x HWA RX[%d] CPLE\n",
					__FUNCTION__, HADDR64_VAL(db_haddr64), db_info->value,
					(ring_id - BCMPCIE_D2H_MSGRING_RX_COMPLETE)));
			}
		}
...
pciedev_dump_flow_ring(msgbuf_ring_t *ring)
{
	...
	PCI_ERROR((
		...
		ring->inited, eabuf, MSGBUF_TID(ring), ring->flow_info.ifindex,
		ring->status, ring->scb_flowid));
	...
	for (i = 0; i < pciedev->max_flowrings; i++) {
		...
		if (flow_ring->inited) {
			...
			flow_rate = dngl_flowring_update(pciedev->dngl,
				...
				flow_ring->ringid, MSGBUF_TID(flow_ring),
				(uint8 *)flow_ring->flow_info.da,
				FLOW_RING_GET_PKT_MAX, NULL, &(flow_ring->scb_flowid), NULL);
...
/**
 * Delink a scb flowid from bus layer
 * Loop through all valid flowrings to check for any linked flows
 */
void
pciedev_scb_flow_delink(struct dngl_bus * pciedev, uint16 scb_flowid)
{
	...
	while (!dll_end(prio, &pciedev->active_prioring_list)) {
		...
		while (!dll_end(cur, &prioring->active_flowring_list)) {
			...
			/* Check for matching scb_flowid */
			if (flow_ring->scb_flowid != scb_flowid) {
				cur = dll_next_p(cur);
				continue;
			}
			...
			PCI_TRACE(("%s: Ringid %d  scb flowid %d tid %d \n",
				__FUNCTION__, flow_ring->ringid, scb_flowid,
				MSGBUF_TID(flow_ring)));
...
buzzz_bus(uint8 *buzzz_log)
{
	...
	struct buzzz_log_ring {
		...
		uint16 wr; uint8 scb_flowid; uint8 max512;
	} * buzzz_log_ring;
	...
	for (ring = 0; ring < pciedev->max_h2d_rings; ring++)
	{
		if (pciedev->h2d_submitring_ptr[ring])
		{
			...
			if (msgbuf->inited) {
				...
				buzzz_log_ring->scb_flowid = msgbuf->scb_flowid;
