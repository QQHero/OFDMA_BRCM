...
#define WBD_FBT_DEF_USE_SAME_MDID	0
...
wbd_create_vif(int unit, int subunit)
{
	...
	/* Remove interface fm lanX_ifnames, if not already */
	for (i = 1; i < WLIFU_MAX_NO_BRIDGE; ++i) {
		char lanX_ifnames[NVRAM_MAX_PARAM_LEN] = {0};

		snprintf(lanX_ifnames, sizeof(lanX_ifnames), "lan%d_ifnames", i);
		str = wbd_nvram_safe_get(lanX_ifnames);
		if (!str || (strlen(str) <= 0)) {
			break;
		}
		strncpy_n(interface_list, str, interface_list_size);
		...
		if (ret == 0) {
			nvram_set(lanX_ifnames, interface_list);
		}
	}
...
{
	uint16 mdid = 0, use_same_mdid = 0;
	static uint16 prev_mdid = 0;

	mdid = wbd_nvram_safe_get_uint(prefix, NVRAM_FBT_MDID, 0);
	use_same_mdid = wbd_nvram_safe_get_uint(NULL, WBD_NVRAM_USE_SAME_MDID,
		WBD_FBT_DEF_USE_SAME_MDID);

	if (mdid) {
		return mdid;
	} else if (use_same_mdid && prev_mdid) {
...
wbd_enable_fbt(char *prefix)
{
	...
	if (find_in_list(nvval, "psk2") && !find_in_list(nvval, "psk")) {
...
wbd_is_fbt_possible(char *prefix)
{
	...
	if (find_in_list(nvval, "psk") ||
		((find_in_list(nvval, "psk2") == NULL) && (find_in_list(nvval, "sae") == NULL))) {
		WBD_RC_PRINT("%s%s[%s]. Not psk2 or sae or mixed mode with psk. "
...
/* Unset self fbt nvrams for this BSS */
void
wbd_unset_self_fbt_nvrams(char *prefix)
{
	char data[WBD_MAX_BUF_256] = {0};

	nvram_unset(strcat_r(prefix, NVRAM_FBT_AP, data));
	nvram_unset(strcat_r(prefix, NVRAM_FBT_R0KH_ID, data));
	nvram_unset(strcat_r(prefix, NVRAM_FBT_R0KH_KEY, data));
	nvram_unset(strcat_r(prefix, NVRAM_FBT_R1KH_ID, data));
	nvram_unset(strcat_r(prefix, NVRAM_FBT_MDID, data));
	nvram_unset(strcat_r(prefix, NVRAM_FBT_OVERDS, data));
	nvram_unset(strcat_r(prefix, NVRAM_FBT_REASSOC_TIME, data));
}

/* Unset peer fbt nvrams for this BSS & ap_name */
void
wbd_unset_peer_fbt_nvrams(char *ap_name)
{
	char data[WBD_MAX_BUF_256] = {0};

	nvram_unset(strcat_r(ap_name, NVRAM_FBT_R0KH_ID, data));
	nvram_unset(strcat_r(ap_name, NVRAM_FBT_R0KH_ID_LEN, data));
	nvram_unset(strcat_r(ap_name, NVRAM_FBT_R0KH_KEY, data));
	nvram_unset(strcat_r(ap_name, NVRAM_FBT_R1KH_ID, data));
	nvram_unset(strcat_r(ap_name, NVRAM_FBT_R1KH_KEY, data));
	nvram_unset(strcat_r(ap_name, NVRAM_FBT_ADDR, data));
	nvram_unset(strcat_r(ap_name, NVRAM_FBT_BR_ADDR, data));
}

/* Unset all the non active peer FBT NVRAMs. Compare old fbt_aps and new fbt_aps
 * and remove all the NVRAMs which are not there in the new list
 */
uint32
wbd_unset_nonactive_peer_fbt_nvrams(char *prefix, char *old_fbt_aps, char *new_fbt_aps)
{
	uint32 rc_flags = 0, max_len;
	char *next, data[WBD_MAX_BUF_256] = {0};
	...
	foreach(ap_name_base, old_fbt_aps, next) {
		...
		wbd_unset_peer_fbt_nvrams(ap_name);
		rc_flags = 1;
	}

	/* Unset FBT_APS, If FBT is Disabled */
	if ((strlen(old_fbt_aps) > 0) && (strlen(new_fbt_aps) <= 0)) {
		/* [8] Unset NVRAM FBT_APS for this BSS */
		nvram_unset(strcat_r(prefix, NVRAM_FBT_APS, data));
	}
end:
	return rc_flags;
}

/* DeInitialize FBT NVRAMs for this BSS on Downstream AP */
uint32
wbd_dap_deinit_fbt_nvram_config(char *prefix)
{
	uint32 rc_flags = 0;
	WBD_RC_PRINT("Prefix[%s] Disabling FBT...\n", prefix);
	wbd_disable_fbt(prefix);
	wbd_unset_self_fbt_nvrams(prefix);
	wbd_unset_nonactive_peer_fbt_nvrams(prefix,
		wbd_nvram_prefix_safe_get(prefix, NVRAM_FBT_APS), "");
...
/* Unset fbt NVRAMs for this BSS */
void
wbd_unset_fbt(char *prefix)
{
	char data[WBD_MAX_BUF_256] = {0};

	nvram_unset(strcat_r(prefix, NVRAM_FBT_ENABLED, data));
	nvram_unset(strcat_r(prefix, WBD_NVRAM_FBT, data));
}
...
int
wbd_strong_sta_identification(struct ether_addr *sta_mac, wbd_strong_sta_metrics_t *sta_stats,
	wbd_strong_sta_policy_t *thresholds)
{
	int isstrong = FALSE;

	/* Skipped idle, or active STA */
	if (thresholds->flags & WBD_STRONG_STA_POLICY_FLAG_ACTIVE_STA) {
	...
	/* check for RSSI Threshold */
	if (thresholds->flags & WBD_STRONG_STA_POLICY_FLAG_RSSI) {
		sta_stats->hysterisis = abs(sta_stats->rssi - sta_stats->last_strong_rssi);

		WBD_RC_PRINT("sta->rssi[%d] sta->last_strong_rssi[%d] hysterisis[%d]\n\n",
			sta_stats->rssi, sta_stats->last_strong_rssi, sta_stats->hysterisis);

		if ((sta_stats->rssi > thresholds->t_rssi) ||
			((sta_stats->last_strong_rssi > thresholds->t_rssi) &&
			(sta_stats->hysterisis > thresholds->t_hysterisis))) {
