 ...
 *
 * $Id: wlc_bmac.c 804400 2021-10-29 00:20:53Z $
 */
...
wlc_bmac_processpmq(wlc_hw_info_t *wlc_hw, bool bounded)
{
	...
	bmac_pmq_t *pmq = wlc_hw->bmac_pmq;
	int8 ps_on, ps_pretend, ps_disassoc_deauth, read_count = 0;
...
#else
		wlc_apps_process_ps_switch(wlc_hw->wlc, &eaddr,
			ps_on | ps_pretend | ps_disassoc_deauth);
#endif /* WLC_LOW_ONLY */
...
BCMINITFN(wlc_coreinit)(wlc_hw_info_t *wlc_hw)
{
	...
	*/
	...
	/* Set RTS Durthrsh to 0 instead of default 100us, for interop reason */
	if (D11REV_IS(wlc->pub->corerev, 65)) {
		wlc_bmac_write_shm(wlc_hw, M_RTS_DURTHRSH, 0);
	}
