/*
 * RPC Transport layer(for RTE bus driver)
 ...
 *
 * $Id: bcm_rpc_tp_rte.c 708017 2017-06-29 14:11:45Z $
 */
...
bcm_rpc_tp_watchdog(rpc_tp_info_t *rpcb)
{
	...
	/* (2) XXX use the watchdog(every second) heartbeat to control the lazy agg
	 * when traffic is low(e.g. association), avoid excessive delaying critical pkt
	 *   (like key exchange ?) due to aggregation
	 * This can be removed if USB DMA/fifo back pressure flowcontrol is working well
	 */
...
bcm_rpc_tp_rx_from_dnglbus(rpc_tp_info_t *rpc_th, struct lbuf *lb)
{
	...
	while (p != NULL) {
		...
		if (pktlen >= BCM_RPC_TP_ENCAP_LEN) {
			...
			padlen = (tp_len >> BCM_RPC_TP_PADEND_SHIFT) & 0xFF;
			tp_len = (tp_len & 0xFFFF) + padlen;
		...
		if (p == NULL) {
			RPC_TP_ERR(("DEAGG: p is NULL, iter %d tp_len %d pktlen %d\n",
				iter, tp_len, pktlen));
			break;
		}
		...
		while ((tp_len >= pktlen) && p) {
			...
			if (p != NULL) {
				...
				if (tp_len != 0) {
					...
					RPC_TP_ERR(("DEAGG, rpc_p %p totlen %d "
						"pktl %d tp_len %d\ntplen:",
						rpc_p, totlen, pktlen, tp_len));
					for (i = 1; i <= MIN(iter, ARRAYSIZE(dbg_data) - 1); i++)
						RPC_TP_ERR((" %d", dbg_data[i]));
					RPC_TP_ERR(("\n"));
					p = rpc_p;
					while (p != NULL) {
		...
		if (tp_len == 0) {
			...
			new_p = hnd_pkt_clone(osh, p, 0, tp_len);
			if (new_p == NULL) {

				/* release all the packets  -
				* rpc_p is first undelivered chained pkt
				*/
				while (rpc_p != NULL) {
					void *ptemp;
					RPC_TP_DEAGG(("this seg len %d rpc_p = 0x%p\n",
						PKTLEN(osh, rpc_p), rpc_p));
					ptemp = PKTNEXT(osh, rpc_p);
					PKTSETNEXT(osh, rpc_p, NULL);
					PKTFREE(osh, rpc_p, FALSE);
					rpc_p = ptemp;
				}
				p = NULL;
		...
		if (padlen) {
			if (PKTNEXT(osh, rpc_p)) {
				PKTSETLEN(osh, PKTNEXT(osh, rpc_p), PKTLEN(osh,
					PKTNEXT(osh, rpc_p)) - padlen);
			} else {
				PKTSETLEN(osh, rpc_p, PKTLEN(osh, rpc_p) - padlen);
			}
		}
		/* !! send up */
...
bcm_rpc_tp_tx_encap(rpc_tp_info_t * rpcb, rpc_buf_t *b)
{
	...
	uint32 rpc_len;
#ifdef BCM_FD_AGGR
	uint32 padhead_len = 0;
	uint32 padend_len = 0;

	rpc_len = PKTLEN(rpcb->osh, b);
	if ((uint32)PKTDATA(rpcb->osh, b) & 0x3)
		padhead_len = sizeof(uint32) - ((uint32)PKTDATA(rpcb->osh, b) & 0x3);
	PKTPUSH(rpcb->osh, b, padhead_len);

	if ((rpc_len + padhead_len) & 0x3) {
		padend_len = sizeof(uint32) - ((rpc_len + padhead_len) & 0x3);
		PKTSETLEN(rpc_th->osh, b, PKTLEN(rpcb->osh, b) + padend_len);
	}

	tp_lenp = (uint32*)PKTPUSH(rpcb->osh, b, BCM_RPC_TP_ENCAP_LEN);
	*tp_lenp = (rpc_len & 0xFFFF) | ((padhead_len & 0xFF) << BCM_RPC_TP_PADHEAD_SHIFT) |
		((padend_len & 0xff) << BCM_RPC_TP_PADEND_SHIFT);
#else
	rpc_len = PKTLEN(rpcb->osh, b);
	tp_lenp = (uint32*)PKTPUSH(rpcb->osh, b, BCM_RPC_TP_ENCAP_LEN);
	*tp_lenp = rpc_len;
...
bcm_rpc_tp_send_callreturn(rpc_tp_info_t * rpc_th, rpc_buf_t *b)
{
	int err, pktlen;
	struct lbuf *lb;
	hnd_dev_t *chained = rpc_th->ctx->chained;
	...
	if (rpc_th->has_2nd_bulk_in_ep) {
		err = chained->ops->xmit2(rpc_th->ctx, chained, lb, USBDEV_BULK_IN_EP2);
	} else {
		err = chained->ops->xmit_ctl(rpc_th->ctx, chained, lb);
	}
...
bcm_rpc_tp_buf_send_enq(rpc_tp_info_t * rpc_th, rpc_buf_t *b)
{
	...
	if (rpc_th->tx_q_flowctl_segcnt > rpc_th->tx_q_flowctl_hiwm) {
		...
		RPC_TP_DBG(("bcm_rpc_tp_buf_send_enq, wm hit high!\n"));
...
bcm_rpc_tp_buf_pad(rpc_tp_info_t * rpcb, rpc_buf_t *bb, uint padbytes)
{
	...
	*tp_lenp = htol32(tp_len);
	...
	b = (rpc_buf_t*)PKTGET(rpcb->osh, padbytes, FALSE);

	if (b != NULL) {
		bzero(PKTDATA(rpcb->osh, b), padbytes);
		while (PKTNEXT(rpcb->osh, bb) != NULL)
			bb = (rpc_buf_t *)PKTNEXT(rpcb->osh, bb);
		PKTSETNEXT(osh, bb, b);
	}
#endif /* BCM_FD_AGGR */
}
...
bcm_rpc_tp_buf_send_internal(rpc_tp_info_t * rpcb, rpc_buf_t *b, uint32 tx_ep_index)
{
	...
	uint pktlen;
#ifndef BCM_FD_AGGR
	hnd_dev_t *chained = rpcb->ctx->chained;
	ASSERT(chained);
#endif

	/* XXX PR60308 WAR 4322USB: USB core on dongle sometime transfers data at incorrect boundary
	 * For problem value BCM_RPC_TP_DNGL_TOTLEN_BAD, pad some bytes by
	 *    changing tp_len(crack out with endianness conversion) and PKTLEN
	 * Pad also required if pkt size is a multiple of MPS
	 */
		ASSERT(b != NULL);
	...
	if ((err = chained->ops->xmit(rpcb->ctx, chained, lb)) != 0) {
#else
	if ((err = dngl_sendpkt_aggr(rpcb->ctx, lb)) != 0) {
#endif
		RPC_TP_ERR(("%s: xmit failed; free pkt %p\n", __FUNCTION__, lb));
		rpcb->txerr_cnt++;
		lb_free(lb);
...
void
BCMATTACHFN(bcm_rpc_tp_txflowctlcb_deinit)(rpc_tp_info_t *rpc_th)
{
...
{
	return (uint32) rpcb->tp_dngl_aggregation;
}
