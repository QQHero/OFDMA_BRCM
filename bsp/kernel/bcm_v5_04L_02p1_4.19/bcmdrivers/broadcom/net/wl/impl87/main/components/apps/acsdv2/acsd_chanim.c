 ...
 *
 * $Id: acsd_chanim.c 804807 2021-11-10 08:19:56Z $
 */
...
chanim_intf_detected(chanim_info_t *ch_info, uint8 version)
{
	...
	if (version == WL_CHANIM_STATS_V2) {
		if (noise_detect && (latestv2->ccastats[CCASTATS_INBSS] < 2) &&
			(latestv2->ccastats[CCASTATS_TXDUR] < 2)) {
			detected = TRUE;
		}
	} else if (version <= WL_CHANIM_STATS_VERSION) {

		if (noise_detect && (latest->ccastats[CCASTATS_INBSS] < 2) &&
			latest->ccastats[CCASTATS_TXDUR] < 2) {
			detected = TRUE;
		}
	}
	return detected;
...
{
	if (!ch_info) {
		ACSD_ERROR("Not updating acs_record when ch_info is NULL\n");
		return;
	}
	acs_record_t* cur_record = &ch_info->record[chanim_mark(ch_info).record_idx];
	chanim_stats_t *cur_stats = &ch_info->stats[chanim_mark(ch_info).stats_idx];
	time_t now = time(NULL);

	bzero(cur_record, sizeof(acs_record_t));

	cur_record->trigger = trigger;
	cur_record->timestamp = now;
	cur_record->selected_chspc = selected;
	cur_record->valid = TRUE;
	cur_record->ticks = ticks;
	...
	if (chanim_mark(ch_info).record_idx == ACS_MAX_RECORD)
		chanim_mark(ch_info).record_idx = 0;
#ifdef BCM_CEVENT
	CE_SEND_CEVENT_A2C_EXT(ifname, NULL, 0, trigger, CEVENT_ST_ACSD,
			CEVENT_A2C_MT_ACS_CH_SW, 0, &selected, sizeof(chanspec_t));
#endif /* BCM_CEVENT */
}
...
chanim_chk_lockout(chanim_info_t *ch_info)
{
	...
	for (i = 0, j = 0; i < CHANIM_STATS_RECORD; i++) {
		...
		start_idx = MODSUB(cur_idx, i+1, ACS_MAX_RECORD);
		start_record = &ch_info->record[start_idx];

		if (start_record->trigger == ACS_INIT)
			return FALSE;

		if (start_record->trigger == ACS_CHANIM)
			j++;
...
chanim_upd_state(acs_chaninfo_t * c_info, uint8 version, uint ticks)
{
	...
	case CHANIM_STATE_ACTON: {
		...
		if (acs_allow_scan(c_info, ACS_SCAN_TYPE_CS, ticks) ||
			c_info->acs_chanim_intf_force_cs_scan) {
			...
			if (acs_select_chspec(c_info)) {
				...
				*/
				c_info->switch_reason = ACS_CHANIM;
...
acsd_update_chanim(acs_chaninfo_t * c_info, wl_chanim_stats_t * chanim_stats, uint ticks)
{
	...
	if (chanim_stats->version == WL_CHANIM_STATS_V2) {
		...
		if (BAND_5G(c_info->rs_info.band_type)) {
			...
			if (stats->chan_idle < c_info->acs_scan_chanim_stats) {
				c_info->scan_chspec_list.ci_pref_scan_request = TRUE;
			}
		}
		else {
		...
		c_info->txrx_score = stats->ccastats[CCASTATS_TXDUR] +
			stats->ccastats[CCASTATS_INBSS];
		c_info->channel_free = stats->ccastats[CCASTATS_TXOP];
		if (c_info->acs_nonwifi_enable) {
			c_info->glitch_cnt = stats->glitchcnt;
		}
	}
...
#if (WL_CHANIM_STATS_VERSION != WL_CHANIM_STATS_VERSION_4)
#error "chanim_stats support only up to version 4"
#endif
	if (list->buflen == 0) {
		list->version = 0;
		list->count = 0;
	...
	for (i = 0; i < list->count; i++) {
		if (list->version == WL_CHANIM_STATS_V2) {
			...
			uint8 *cc = stats->ccastats;
			if (!stats->chanspec ||
					input_band != CHSPEC_BAND(stats->chanspec)) {
				continue;
			}
			c_info->ch_avail[j] = cc[CCASTATS_TXOP] + cc[CCASTATS_INBSS] +
				cc[CCASTATS_TXDUR];
