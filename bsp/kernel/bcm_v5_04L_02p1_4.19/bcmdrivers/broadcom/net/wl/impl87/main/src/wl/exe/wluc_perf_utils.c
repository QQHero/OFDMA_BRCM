...
static cmd_func_t wl_scb_txrx_summary;
...
static cmd_t wl_perf_utils_cmds[] = {
	...
	"    -nooverall  Do not display total of all stations\n"
	"    -noreset    Do not reset counters after reading" },
	{ "txrx_summary", wl_scb_txrx_summary, WLC_GET_VAR, -1,
	"Display per station live data about rx datapath\n"
	"usage: rx_report [options]\n"
	"  options are:\n"
	"    -sta xx:xx:xx:xx:xx:xx  only display specific mac addr.\n"
...
wl_iov_pktqlog_params(void *wl, cmd_t *cmd, char **argv)
{
	...
	do {
		...
		while (params->num_addrs < WL_IOV_MAC_PARAM_LEN && *macaddrs) {
			...
			if (ptr[0] == '-') {
				++ptr;
				if ((!strcmp(ptr, "help")) || (!strcmp(ptr, "h"))) {
					return BCME_USAGE_ERROR;
				} else if (dpstats && (!strcmp(ptr, "nr") ||
				                       !strcmp(ptr, "noreset"))) {
					resetstats = FALSE;
				} else if (!strcmp(ptr, "%") || !strcmp(ptr, "percent")) {
					percent = TRUE;
				} else {
					fprintf(stdout, "bad option '%s', try <command> -h\n",
					        *macaddrs);
...
wl_scb_rx_report(void *wl, cmd_t *cmd, char **argv)
{
	...
	char st_tones[10];
	uint8 tid_count;
	iov_rx_report_counters_t zeroc;
	...
	data->length_struct_record = dtoh16(data->length_struct_record);
	data->length_struct_counters = dtoh16(data->length_struct_counters);

	if (data->length_struct_record != sizeof(iov_rx_report_record_t)) {
		printf("warning: length_struct_record is %u bytes, expected %u\n",
			data->length_struct_record, (uint32)sizeof(iov_rx_report_record_t));
		return BCME_VERSION;
	}

	if (data->length_struct_counters != sizeof(iov_rx_report_counters_t)) {
		printf("warning: length_struct_counters is %u bytes, expected %u\n",
			data->length_struct_counters, (uint32)sizeof(iov_rx_report_counters_t));
	}
	...
	case DISPLAY_RAW:
		printf("%8.8s %8.8s %8.8s %8.8s %8.8s %8.8s %8.8s"
			"%8.8s %8.8s %8.8s %8.8s %8.8s %8.8s %8.8s %8.8s %8.8s\n",
			"tid", "rxampdu", "rxmpdu", "rxbyte", "rxphyrate",
			"rxholes", "dup", "oow", "rtries", "rxbw",
			"rxmcs", "rxnss", "mmu", "ofdma", "tones", "dtime");
		break;
	case DISPLAY_COOKED:
		printf("%4s%c%7s%c%8s%c"
			"%10s%c%10s%c%5s%c%5s%c"
			"%5s%c%5s%c%5s%c"
			"%5s%c%5s%c%6s%c%6s%c%6s%c%6s\n",
			"tid", sep, "ampdu", sep, "mpdu", sep,
			"Data Mbps", sep, "PHY Mbps", sep, "bw", sep, "mcs", sep,
			"Nss", sep, "oow", sep, "holes", sep,
			"dup", sep, "rtries", sep, "mmu", sep, "ofdma", sep, "tones", sep, "air");
		break;
	}

	/* Convert returned counters to host byte order, and sum up all stations */
	memset(&allc, 0, sizeof(allc));
	rec = data->structure_record;
	nrec = (uint8 *)rec;

	for (argn = 0; argn < data->structure_count; ++argn) {
		rec = (iov_rx_report_record_t *)nrec;
		tid_count = bcm_bitcount((uint8*) &rec->tid_report_mask, 1);
		nrec += (sizeof(iov_rx_report_record_t) +
			(int32)((tid_count - 1) * data->length_struct_counters));

		for (idx = 0; idx < tid_count; idx++) {
			ctr = &rec->station_counters[idx];
...
#define DEVICE_TO_HOST32(xyzzy) ctr->xyzzy = dtoh32(ctr->xyzzy); allc.xyzzy += ctr->xyzzy;
			...
			DEVICE_TO_HOST32(rxmpdu_mmu);
#undef DEVICE_TO_HOST32
#undef DEVICE_TO_HOST64
		}
	}

	memset(&zeroc, 0, sizeof(zeroc));
	rec = data->structure_record;
	nrec = (uint8 *)rec;
	for (argn = 0; argn < data->structure_count; ++argn) {
		...
		for (tid = 0; tid < NUMPRIO; tid++) {
			...
			if (!(rec->tid_monitor_mask & (1<<tid))) continue;

			/* Select reported tid or fake zero'ed data */
			if (!(rec->tid_report_mask & (1<<tid))) {
				ctr = &zeroc;
			} else {
				ctr = &rec->station_counters[idx++];
			}
			...
			case DISPLAY_RAW:
				printf("%7u %8u %8u %8llu "
					...
					ctr->rxmcs, ctr->rxnss, ctr->rxmpdu_mmu,
			...
			case DISPLAY_COOKED:
				...
				float ratio_air = 0, ratio_ofdma = 0, ratio_mmu = 0;
				...
				if (ctr->rxmpdu) {
					...
					ratio_mmu = ctr->rxmpdu_mmu * 100.0 / ctr->rxmpdu;
				...
				printf("%3d%c%7d%c%8d%c"
					...
					ratio_mmu, sep, ratio_ofdma, sep,
					st_tones, sep, ratio_air);
				}
				break;
			}
		}
...
/* 11ax subcarrier spacing  78.125 KHz */
#define SUBCARRIER_SPACING_11AX		0.078125
static void
wl_print_rx_stats(iov_txrx_summary_scb_counters_t *ctr)
{
	char sep = ' ';
	float bw = 0, bw_cnt_su = 0, bw_cnt_mu = 0, bw_avg = 0;
	float bw_ratio = 0, phyrate = 0, tones = 0, tried = 0;
	float mbps_capacity = 0, mbps_percieved = 0, psr = 0;

	if (ctr->rx.mpdu) {
		bw = ctr->rx.bw/ctr->rx.mpdu;
	}

	if (ctr->rx.mpdu_ofdma) {
		tones = ctr->rx.tones * 1.0 / ctr->rx.mpdu_ofdma;
	}
	...
	if (bw > 0) {
		...
		bw_cnt_su = bw * (ctr->rx.mpdu - ctr->rx.mpdu_ofdma);
		bw_cnt_mu = tones * SUBCARRIER_SPACING_11AX *
			1.05 * ctr->rx.mpdu_ofdma;
		bw_avg = (bw_cnt_su + bw_cnt_mu) / ctr->rx.mpdu;
		bw_ratio = bw_avg / bw;
	}
	tried = ctr->rx.mpdu + ctr->rx.retried;
	if (ctr->rx.mpdu) {
		phyrate = (ctr->rx.phyrate/1000.0)/ctr->rx.mpdu;
	}
	if (bw_ratio) {
		mbps_capacity = phyrate/bw_ratio;
	}
	mbps_percieved = phyrate;
	if (ctr->rx.mpdu) {
		psr = ctr->rx.mpdu/tried;
	}

	printf("\n%18s", "Rx:");
	printf("%10.0f%c%13.1f%c%14.1f%c"
		"%4.2f\n",
		tried, sep, mbps_capacity, sep, mbps_percieved, sep,
		psr);
}

static inline unsigned long
wl_sta_arr_sub(float *arr, size_t arr_len, float budget)
{
	unsigned long sub;
	size_t i;
	for (i = 0; i < arr_len; i++) {
...
static void
wl_print_tx_stats(iov_txrx_summary_scb_counters_t *ctr)
{
	char sep = ' ';
	float mbps;
	float sum_nss = 0;
	float tones = 0;
	float bw;
	float bw_cnt_su, bw_cnt_mu;
	float bw_avg, bw_ratio = 0;
	float acked = 0, retry = 0;
	float mumimo = 0, muofdma = 0;
	float tried = 0, mbps_capacity = 0, mbps_perceived = 0, psr = 0;
	float sum_nss_su = 0, max_nss = 0, avg_nss = 0, nss_ratio = 0, avg_nss_su = 0;
	float nss[4];

	mbps = ctr->tx.phyrate/1000.0;
	bw = ctr->tx.bw;
	acked = ctr->tx.mpdu;
	retry = ctr->tx.retried;
	mumimo = ctr->tx.mpdu_mumimo;
	muofdma = ctr->tx.mpdu_ofdma;
	tones = ctr->tx.tones;
	nss[0] = ctr->tx.nss[0];
	nss[1] = ctr->tx.nss[1];
	nss[2] = ctr->tx.nss[2];
	...
	sum_nss = nss[0] + nss[1] + nss[2] + nss[3];
	...
	if (sum_nss_su > 0) {
		wl_sta_arr_sub(nss, ARRAY_SIZE(nss), mumimo);
	...
	if (tried > 0) {
		...
		if (max_nss > 0 && nss_ratio > 0 && avg_nss_su > 0) {
			mbps_capacity /= nss_ratio;
			mbps_capacity *= avg_nss_su;
			mbps_capacity /= max_nss;
		}

		mbps_perceived = mbps;
		if (bw > 0) {
			bw_cnt_su = bw * ((sum_nss ?: acked) - muofdma);
			bw_cnt_mu = tones * SUBCARRIER_SPACING_11AX *
				1.05 * muofdma;
			bw_avg = (bw_cnt_su + bw_cnt_mu) / (sum_nss ?: acked);
			bw_ratio = bw_avg / bw;
			mbps_perceived *= bw_ratio;
		}
	...
	printf("%10.0f%c%13.1f%c%14.1f%c"
		"%4.2f",
		tried, sep, mbps_capacity, sep, mbps_perceived, sep,
		psr);
}
static int
wl_scb_txrx_summary(void *wl, cmd_t *cmd, char **argv)
{
	int err;
	int32 flag_bits = 0;
	bool mac_only = FALSE;
	struct ether_addr ea;
	int argn;
	enum { DISPLAY_COOKED, DISPLAY_RAW, DISPLAY_COOKED_PLUME } display_mode = DISPLAY_COOKED;
	iov_txrx_summary_struct_t *data = (iov_txrx_summary_struct_t *)buf;
	char sep = ' ';
	...
	for (argn = 1; argv[argn]; ++argn) {
		...
		if (!strcmp(argv[argn], "-p")) {	/* Display Plume counters */
			display_mode = DISPLAY_COOKED_PLUME;
		} else
		if (!strcmp(argv[argn], "-help") || !strcmp(argv[argn], "-h")) {
			/* Display usage, do not complain about unknown option. */
			return BCME_USAGE_ERROR;
	...
	if (data->structure_version != SCB_TXRX_SUMMARY_STRUCT_VERSION) {
		fprintf(stderr, "------wlu / wl driver mismatch, expect V%d format, got %d.\n",
			SCB_TXRX_SUMMARY_STRUCT_VERSION, data->structure_version);
		return BCME_IOCTL_ERROR;
	}

	data->sizeof_iov_txrx_summary_record_t = dtoh16(data->sizeof_iov_txrx_summary_record_t);
	data->sizeof_iov_txrx_summary_scb_counters_t =
		dtoh16(data->sizeof_iov_txrx_summary_scb_counters_t);

	if (data->sizeof_iov_txrx_summary_record_t != sizeof(iov_txrx_summary_record_t)) {
		printf("warning: sizeof_iov_txrx_summary_record_t is %u bytes, expected %u\n",
			data->sizeof_iov_txrx_summary_record_t,
			(uint32)sizeof(iov_txrx_summary_record_t));
		return BCME_VERSION;
	}

	if (data->sizeof_iov_txrx_summary_scb_counters_t !=
		sizeof(iov_txrx_summary_scb_counters_t)) {
		printf("warning: sizeof_iov_txrx_summary_scb_counters_t is %u bytes, expected %u\n",
			data->sizeof_iov_txrx_summary_scb_counters_t,
			(uint32)sizeof(iov_txrx_summary_scb_counters_t));
	}
	...
	case DISPLAY_RAW:
		printf("%14s %12s %12s %12s %12s "
			"%12s %12s %12s %12s "
			"%12s %28s\n",
			"MainRate(Kbps)", "Data(Bytes)", "mpdu_attmpd", "mpdu_succ", "retried",
			"su", "mumimo", "muofdma", "tones(RU)",
			"bw(MHz)", "nss:    1/    2/    3/    4/");
		break;
	case DISPLAY_COOKED:
		printf("%14s %12s %12s %12s %12s "
			"%12s %12s %12s %12s "
			"%12s %28s\n",
			"MainRate(Mbps)", "Data(Mbps)", "mpdu_attmpd", "mpdu_succ", "retried",
			"su", "mumimo", "muofdma", "tones(RU)",
			"bw(MHz)", "nss:    1/    2/    3/    4/");
		break;
	case DISPLAY_COOKED_PLUME:
		printf("%10s%c%13s%c%14s%c%4s\n",
			"Tried", sep, "MbpsCapacity", sep, "MbpsPerceived", sep, "Psr");
		break;
	}

	for (argn = 0; argn < data->structure_count; ++argn) {
		iov_txrx_summary_record_t *rec;
		iov_txrx_summary_scb_counters_t *ctr;
		rec = &data->structure_record[argn];
		ctr = &rec->station_counters;
		...
		case DISPLAY_RAW:
			printf("\n%18s", "Tx:");
			printf("%12llu %12llu %12u %12u %12u "
				"%12u %12u %12u "
				"%12u %12u %10u %5u "
				"%5u %5u",
				ctr->tx.phyrate, ctr->tx.bytes, ctr->tx.attempted, ctr->tx.mpdu,
				ctr->tx.retried, ctr->tx.mpdu_su, ctr->tx.mpdu_mumimo,
				ctr->tx.mpdu_ofdma, ctr->tx.tones, ctr->tx.bw, ctr->tx.nss[0],
				ctr->tx.nss[1],	ctr->tx.nss[2], ctr->tx.nss[3]);
			printf("\n%18s", "Rx:");
			printf("%12llu %12llu %12c %12u %12u "
				"%12u %12u %12u "
				"%12u %12u %10u %5u "
				"%5u %5u\n",
				ctr->rx.phyrate, ctr->rx.bytes, '-', ctr->rx.mpdu, ctr->rx.retried,
				ctr->rx.mpdu_su, ctr->rx.mpdu_mumimo, ctr->rx.mpdu_ofdma,
				ctr->rx.tones, ctr->rx.bw, ctr->rx.nss[0], ctr->rx.nss[1],
				ctr->rx.nss[2], ctr->rx.nss[3]);
			break;
		case DISPLAY_COOKED:
			{
			float txphyrate = 0, rxphyrate = 0;
			float txbw = 0, rxbw = 0;
			float txmpdu = 0, rxmpdu = 0;
			float txmpdu_ofdma = 0, rxmpdu_ofdma = 0;
			float txtones = 0, rxtones = 0;
			float tx_throughput = 0, rx_throughput = 0;

			txmpdu = ctr->tx.mpdu;
			rxmpdu = ctr->rx.mpdu;
			/* Convert to Mbps */
			txphyrate = ctr->tx.phyrate/1000.0;
			rxphyrate = ctr->rx.phyrate/1000.0;

			txmpdu_ofdma = ctr->tx.mpdu_ofdma;
			rxmpdu_ofdma = ctr->rx.mpdu_ofdma;
			txtones = ctr->tx.tones;
			rxtones = ctr->rx.tones;

			if (txmpdu) {
				txphyrate /= txmpdu;
				txbw = (ctr->tx.bw) / txmpdu;
			}
			if (rxmpdu) {
				rxphyrate /= rxmpdu;
				rxbw = (ctr->rx.bw) / rxmpdu;
			}

			if (txmpdu_ofdma) {
				txtones /= txmpdu_ofdma;
			}
			if (rxmpdu_ofdma) {
				rxtones /= txmpdu_ofdma;
			}
			if (rec->time_delta) {
				tx_throughput = (ctr->tx.bytes*8.0) / rec->time_delta;
				rx_throughput = (ctr->rx.bytes*8.0) / rec->time_delta;
			}
			printf("\n%18s", "Tx:");
			printf("%14.1f %12.1f %12u %12u %12u "
				"%12u %12u %12u %12.1f "
				"%12.1f %10u %5u %5u %5u",
				txphyrate, tx_throughput, ctr->tx.attempted, ctr->tx.mpdu,
				ctr->tx.retried, ctr->tx.mpdu_su, ctr->tx.mpdu_mumimo,
				ctr->tx.mpdu_ofdma, txtones, txbw, ctr->tx.nss[0], ctr->tx.nss[1],
				ctr->tx.nss[2], ctr->tx.nss[3]);
			printf("\n%18s", "Rx:");
			printf("%14.1f %12.1f %12c %12u %12u "
				"%12u %12u %12u %12.1f "
				"%12.1f %10u %5u %5u %5u\n",
				rxphyrate, rx_throughput, '-', ctr->rx.mpdu, ctr->rx.retried,
				ctr->rx.mpdu_su, ctr->rx.mpdu_mumimo, ctr->rx.mpdu_ofdma,
				rxtones, rxbw, ctr->rx.nss[0], ctr->rx.nss[1],
				ctr->rx.nss[2], ctr->rx.nss[3]);
			}
			break;
		case DISPLAY_COOKED_PLUME:
			{
				wl_print_tx_stats(ctr);
				wl_print_rx_stats(ctr);
