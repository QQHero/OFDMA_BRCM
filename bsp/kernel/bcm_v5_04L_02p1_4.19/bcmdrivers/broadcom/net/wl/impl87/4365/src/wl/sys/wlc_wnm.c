 ...
 *
 * $Id: wlc_wnm.c 808492 2022-02-18 13:26:40Z $
 */
	...
	uint8 *body, int body_len)
{
	...
	case DOT11_WNM_ACTION_BSSTRANS_REQ:
		...
		wlc_bss_mac_event(wnm->wlc, bsscfg, WLC_E_BSSTRANS_REQ, &scb->ea, 0, 0, 0,
			body, body_len);
	...
	case DOT11_WNM_ACTION_BSSTRANS_QUERY: {
		...
		wlc_bss_mac_event(wnm->wlc, bsscfg, WLC_E_BSSTRANS_QUERY, &scb->ea, 0, 0, 0,
			body, body_len);
		...
		if (!(wnm_cfg->flags & WNM_BTQ_RESP_DISABLED)) {
			...
			wlc_bss_mac_event(wlc, bsscfg, WLC_E_WNM_BSSTRANS_QUERY, &scb->ea, 0, 0, 0,
				body, body_len);
