 ...
 *
 * $Id: dngl_msgtrace.c 715608 2017-08-11 22:25:17Z $
 */
...
struct msgtrace {
	...
	bool	filter_cb;		/* Filter the trace generated by the execution callback of
					 *  sending
					 */
	msgtrace_sendup_trace_fn_t func_send; /* Function pointer to send trace event */
	void	*ctx;			/* hdl1 : handler used to send trace event */
...
msgtrace_timeout(hnd_timer_t *t)
{
	...
	if ((len = msgtrace_get(&trace)) > 0) {
		...
		msgtrace.func_send(msgtrace.ctx,
...
static int
msgtrace_put(void *ctx, char *buf, int count)
{
	uint16 freebytes;
...
void
BCMATTACHFN(msgtrace_init)(osl_t *osh)
{
	msgtrace.retry_count = 0;

	printf_set_sendup_output_fn(msgtrace_put, NULL);

	msgtrace.timer  = hnd_timer_create(NULL, NULL, msgtrace_timeout, NULL, NULL);
	...
	hnd_get_sdpcm_shared()->msgtrace_addr = (uint32)msgtrace_get_addr();
#endif
#ifdef BCMPCIEDEV
	if (PCIEDEV_ENAB()) {
		hnd_get_pcie_ipc()->msgtrace_daddr32 = (uint32)msgtrace_get_addr();
	}
#endif
}

void
BCMATTACHFN(msgtrace_set_sendup_trace_fn)(msgtrace_sendup_trace_fn_t func_send, void *ctx)
{
