 ...
 *
 *	$Id: acs_channels.c 791744 2020-10-09 11:01:06Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
acs_candidate_score_txpwr(ch_candidate_t *candi, acs_chaninfo_t* c_info)
{
	...
	if (BAND_2G(c_info->rs_info.band_type)) {
		ACSD_INFO(" txpower calculation is not needed in 2G band\n");
		return;
	}
...
acs_coex_check(acs_chaninfo_t* c_info, chanspec_t input_chspec)
{
	...
	if (conflict) {
		/* overlapping BSS check is implemented for 2G/40MHz only */
		chspec_out = CH20MHZ_CHSPEC(ctrl_ch, WL_CHANSPEC_BAND_2G);
...
recheck:
	...
	if (acs_has_valid_candidate(c_info, bw)) {
		...
		/* FCS does downgrade bandwidth if there is no valid channel can be selected
		 * on current bw.
		 */
