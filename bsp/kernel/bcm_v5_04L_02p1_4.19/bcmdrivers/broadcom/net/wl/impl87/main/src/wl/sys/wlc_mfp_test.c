 ...
 *
 * $Id: wlc_mfp_test.c 804554 2021-11-02 17:00:59Z $
 */
...
/* iovar callback */
int
mfp_test_doiovar(wlc_info_t *wlc, uint32 actionid,
	...
	struct scb *scb, uint16 fc, uint16 reason_code, int flag)
{
	...
	if (BSSCFG_AP(bsscfg) && ETHER_ISMULTI(da)) {
		...
		p = wlc_mfp_frame_get_mgmt(wlc->mfp, fc, 0, da, sa, bssid, 2, &pbody, TRUE);
	}
	else {
...
mfp_disassoc(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, int flag, int flag2)
{
	...
	if (BSSCFG_AP(bsscfg)) {
		if (flag2) {
			...
			mfp_send_disassoc_deauth(wlc, (const struct ether_addr*)&ether_bcast,
				&bsscfg->BSSID, &bsscfg->cur_etheraddr,
				WLC_BCMCSCB_GET(wlc, bsscfg), FC_DISASSOC,
				DOT11_RC_DISASSOC_LEAVING, flag);
		}
		else {
			struct scb_iter scbiter;
...
mfp_deauth(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, int flag, int flag2)
{
	...
	if (BSSCFG_AP(bsscfg)) {
		if (flag2) {
			...
			mfp_send_disassoc_deauth(wlc, (const struct ether_addr*)&ether_bcast,
				&bsscfg->BSSID, &bsscfg->cur_etheraddr,
				WLC_BCMCSCB_GET(wlc, bsscfg), FC_DEAUTH, DOT11_RC_INACTIVITY, flag);
		}
		else {
			struct scb_iter scbiter;
	...
	if (flag == 2) {
		wlc_senddeauth(wlc, bsscfg, scb, &bsscfg->BSSID, &bsscfg->BSSID,
			&bsscfg->cur_etheraddr, DOT11_RC_INACTIVITY, FALSE);
	} else {
		mfp_send_disassoc_deauth(wlc, &bsscfg->BSSID, &bsscfg->BSSID,
			&bsscfg->cur_etheraddr, scb, FC_DEAUTH, DOT11_RC_INACTIVITY, flag);
	}
}
...
mfp_auth(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, int flag)
{
	...
	wlc_sendauth(bsscfg, &scb->ea, &bsscfg->target_bss->BSSID, scb,
		DOT11_OPEN_SYSTEM, 1, DOT11_SC_SUCCESS, NULL, DOT11_CAP_SHORT, NULL, NULL);
}
    ...
    const uint8 *body, int body_len, const uint8 *key, uint8 * mic, size_t mic_len)
{
	...
	data_len += 2;
	memcpy((char *)&micdata[data_len], (const uint8 *)&hdr->da, ETHER_ADDR_LEN);
	data_len += ETHER_ADDR_LEN;
	memcpy((char *)&micdata[data_len], (const uint8 *)&hdr->sa, ETHER_ADDR_LEN);
	data_len += ETHER_ADDR_LEN;
	memcpy((char *)&micdata[data_len], (const uint8 *)&hdr->bssid, ETHER_ADDR_LEN);
	data_len += ETHER_ADDR_LEN;
	...
	aes_cmac_calc(NULL, 0, micdata, data_len, key, BIP_KEY_SIZE, mic, mic_len);
}
