 ...
 *
 * $Id: wlc_tpc.c 803784 2021-10-07 12:42:20Z $
 */
...
static const bcm_iovar_t wlc_tpc_iovars[] = {
	...
	{"chanspec_reg_txpwr", IOV_CHANSPEC_REG_TXPWR, (0), 0, IOVT_BUFFER,
	WL_CHANSPEC_REG_TXPWR_LEN},
...
static int
wlc_tpc_get_reg_txpwr(wlc_info_t *wlc,
	wl_chanspec_reg_txpwr_t *arg, wl_chanspec_reg_txpwr_t *reg_params)
{
	char abbrev[WLC_CNTRY_BUF_SZ] = {0};
	wl_uint32_list_t *list = NULL;
	uint16 alloc_len, cnt;
	...
	chanspec = reg_params->reg_txpwr[0].chanspec;
	WL_INFORM(("input: chanspec: %x\n", chanspec));
	if (chanspec == 0) {
		FOREACH_WLC_BAND(wlc, bandunit) {
			if (bandunit == BAND_2G_INDEX)
	...
	arg->count = list->count;
	arg->len = WL_CHANSPEC_REG_TXPWR_LEN;
	arg->ver = WL_CHANSPEC_REG_TXPWR_VER;

	for (cnt = 0; cnt < list->count; cnt++) {
		chanspec = (chanspec_t)(list->element[cnt]);

		arg->reg_txpwr[cnt].chanspec = chanspec;
		if (CHSPEC_BANDUNIT(chanspec) == BAND_6G_INDEX) {
			...
			wlc_get_6g_tpe_reg_max_power(wlc_cmi, &locales, &limits,
				...
				CLM_DEVICE_CATEGORY_LP, chanspec);
			/* 6dbm is added to client power level to convert it to AP power level */
			arg->reg_txpwr[cnt].reg_txpwr = limits.limit[0][0] + 6;

		} else {
			arg->reg_txpwr[cnt].reg_txpwr =
				wlc_get_reg_max_power_for_channel(wlc->cmi, chanspec, TRUE);
		}
		WL_INFORM(("chspec:%x max: %d \n", chanspec, arg->reg_txpwr[cnt].reg_txpwr));
	}
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_CHANSPEC_REG_TXPWR):
	{
		wl_chanspec_reg_txpwr_t reg_txpwr_params;
		wl_chanspec_reg_txpwr_t *reg_txpwr_arg;
		uint16 max_len;

		/* to avoid unalign, copy input arguments */
		memcpy(&reg_txpwr_params, params, sizeof(wl_chanspec_reg_txpwr_t));

		/* check to make sure enough space for report */
		max_len = sizeof(wl_chanspec_reg_txpwr_t);
		max_len += (sizeof(chanspec_reg_txpwr_t) * (WL_NUMCHANSPECS - 1));
		if (len < max_len) {
			err = BCME_BUFTOOSHORT;
			break;
		}

		reg_txpwr_arg = (wl_chanspec_reg_txpwr_t *)MALLOCZ(wlc->osh, max_len);
		if (reg_txpwr_arg == NULL) {
			err = BCME_NOMEM;
			break;
		}

		err = wlc_tpc_get_reg_txpwr(wlc, reg_txpwr_arg, &reg_txpwr_params);
		memcpy(arg, reg_txpwr_arg, max_len);
		MFREE(wlc->osh, reg_txpwr_arg, max_len);
		break;
	}
#endif /* WL_CHANSPEC_TXPWR_MAX */
	default:
	...
	uint8 *body, int body_len, int8 rssi, ratespec_t rspec)
{
	...
	if (body_len < 3) {
		WL_ERROR(("wl%d: %s: got TPC Request from "MACF", but frame body len"
			" was %d, expected 3\n",
			wlc->pub->unit, __FUNCTION__, ETHERP_TO_MACF(ea), body_len));
		return;
	}

	action_hdr = (struct dot11_action_measure *)body;

	WL_INFORM(("wl%d: wlc_recv_tpc_request: got TPC Request (token %d) from "MACF"\n",
		wlc->pub->unit, action_hdr->token, ETHERP_TO_MACF(ea)));
...
wlc_tpc_get_current(wlc_tpc_info_t *tpc, void *pwr, uint len, wlc_bsscfg_t *bsscfg)
{
	...
	if (ppr_type == PPRTYPE_REGLIMITS) {
		...
		if (old_power) {
			...
			if (!hwtable_txpwr) {
				uint8 phy_offset = 0; /* This offset is from pi->tx_power_offset */
				uint8 stf_offset = 0; /* This offset is from wlc->stf->txpwr_ctl */
				txpwr204080_t txpwrs;
				...
				stf_offset = txpwrs.pbw[((rspec & WL_RSPEC_BW_MASK) >>
					WL_RSPEC_BW_SHIFT) - BW_20MHZ][RSPEC_ISTXBF(rspec)]*2;
				if ((err = wlc_stf_get_power_offset_per_ratespec(wlc,
					pwr_to_wl->last_tx_ratespec, &phy_offset, txbf_chains)) !=
					BCME_OK) {
					goto free_power;
				}
				for (idx = 0; (uint) idx < WL_STA_ANT_MAX;
					...
					if (((txcore & wlc->stf->hw_txchain) >> idx) & 0x1) {
						int8 stf_current_target =
							pwr_to_wl->tx_power_max[idx] - stf_offset;
						pwr_to_wl->target_offsets[idx] = phy_offset;
						/* If open loop Tx power ctrl is on, we should
						 * ignore the est. Power
						 */
						if (tssisen_min[idx] != (int8)WL_RATE_DISABLED &&
							stf_current_target < tssisen_min[idx]) {
							pwr_to_wl->est_Pout[idx] =
								stf_current_target;
