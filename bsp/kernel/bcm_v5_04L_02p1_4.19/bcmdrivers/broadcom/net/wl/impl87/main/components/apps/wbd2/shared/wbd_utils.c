 ...
 *
 * $Id: wbd_utils.c 803080 2021-09-15 05:26:47Z $
 */
...
int wbd_escape_space(char *buf, unsigned int buf_len)
{
	...
	while (*p) {
		if (*p == ' ') {
			...
			memmove(p + 4, p + 1, strlen(p)); /* include 0-termination */
			memcpy(p, "\\x20", 4);
			p += 3;
			room -= 3;
		} else if (*p == '\\') {
			...
			memmove(p + 2, p + 1, strlen(p)); /* include 0-termination */
			memcpy(p, "\\\\", 2);
			p++;
			room--;
		}
...
void wbd_unescape_space(char *buf)
{
	...
	while (*p) {
		if (!memcmp(p, "\\x20", 4)) {
			*p = ' ';
			memmove(p + 1, p + 4, strlen(p) - 3); /* include 0-termination */
		} else if (!memcmp(p, "\\\\", 2)) {
			*p = '\\';
			memmove(p + 1, p + 2, strlen(p) - 1); /* include 0-termination */
		}
		p++;
	}
}
