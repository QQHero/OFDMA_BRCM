 ...
 *
 * $Id: wlc_11h.c 808957 2022-03-03 13:01:01Z $
 */
...
static const bcm_iovar_t wlc_11h_iovars[] = {
	{"measure", IOV_MEASURE, (0), 0, IOVT_BUFFER, sizeof(uint32)+sizeof(struct ether_addr)},
	{NULL, 0, 0, 0, 0, 0}
};
...
BCMATTACHFN(wlc_11h_detach)(wlc_11h_info_t *m11h)
{
	...
	(void)wlc_module_remove_ioctl_fn(wlc->pub, m11h);
...
static int
wlc_11h_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
...
static int
wlc_11h_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
	wlc_11h_info_t *m11h = (wlc_11h_info_t *)ctx;
	wlc_info_t *wlc = m11h->wlc;
	int val = 0, *pval;
...
wlc_11h_tbtt(wlc_11h_info_t *m11h, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_AP(cfg) &&
		...
		/* this function decrements the quiet count */
...
static uint
_wlc_11h_write_sup_chan_ie(wlc_11h_info_t *m11h, wlc_bsscfg_t *cfg, chanspec_t target_chspec,
                           uint8 *cp, int buflen)
{
	wlc_info_t *wlc = m11h->wlc;
	...
	const struct subband_desc *subbands;
	const struct subband_desc subbands_2g[] = {
		{1, 14}			/* 1-14 */
	};
	/* Using Channel set [149..177] since the 20MHz Channel sets starting with
	 * channel number 149 are currently defined in such a manner that if one of the channels
	 * within this set is not supported i.e. invalid,
	 * subsequent higher channel numbers of this Channel set are also not supported.
	 */
	const struct subband_desc subbands_5g[] = {
		{36, 4}, {52, 4},	/* 36-48, 52-64 */
		{100, 12}, {149, 8}	/* 100-144, 149-177 */
	};
#if BAND6G
	/* Channel set from Global Operating Classes (6g2 20MHz not included) */
	const struct subband_desc subbands_6g[] = {
		{1, 59}                 /* 1-233 */
	};
#endif /* BAND6G */
	...
	/* pick up the frequency band from the target chanspec */
	band = CHSPEC_BAND(target_chspec);

	if (CHSPEC_IS2G(target_chspec)) {
		subbands = subbands_2g;
		end_subband = ARRAYSIZE(subbands_2g);
		max_channel = CH_MAX_2G_CHANNEL;
		ch_sep = CH_5MHZ_APART;
	} else if (CHSPEC_IS5G(target_chspec)) {
		subbands = subbands_5g;
		end_subband = ARRAYSIZE(subbands_5g);
		max_channel = MAXCHANNEL;
		ch_sep = CH_20MHZ_APART;
	}
#if BAND6G
	else if (CHSPEC_IS6G(target_chspec)) {
		subbands = subbands_6g;
		end_subband = ARRAYSIZE(subbands_6g);
		max_channel = CH_MAX_6G_CHANNEL;
		ch_sep = CH_20MHZ_APART;
	}
#endif /* BAND6G */
	else {
		/* should not have any other band */
		ASSERT(0);
		return 0;
	}

	for (subband_idx = 0; subband_idx < end_subband; subband_idx++) {
		run_count = 0;
		seen_valid = 0;
		...
		for (cur_chan = subbands[subband_idx].start, j = 0;
			...
			if (chspec != INVCHANSPEC &&
			    wlc_valid_chanspec(wlc->cmi, chspec)) {
				if (!seen_valid) {
					first_chan = cur_chan;
					seen_valid = 1;
				}
...
wlc_11h_calc_sup_chan_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	wlc_bss_info_t *target;
	uint8 buf[257];

	ASSERT(data->ft == FC_ASSOC_REQ || data->ft == FC_REASSOC_REQ);

	target = data->cbparm->ft->assocreq.target;

	if (BSS_WL11H_ENAB(wlc, cfg)) {
		return _wlc_11h_write_sup_chan_ie(m11h, cfg, target->chanspec,
		                                        buf, sizeof(buf));
	}
...
wlc_11h_write_sup_chan_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (BSS_WL11H_ENAB(wlc, cfg)) {
		_wlc_11h_write_sup_chan_ie(m11h, cfg, target->chanspec, data->buf, data->buf_len);
	}
...
wlc_11h_write_pwr_cap_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if ((srommax = ppr_create(wlc->osh, PPR_CHSPEC_BW(bi->chanspec))) != NULL) {
	 ...
	 */
	min_pwr = MAX(min_pwr, wlc_phy_tssivisible_thresh(WLC_PI(wlc)));
...
wlc_11h_tdls_calc_sup_chan_ie_len(void *ctx, wlc_iem_calc_data_t *calc)
{
	...
	return _wlc_11h_write_sup_chan_ie(m11h, calc->cfg,
	                                         calc->cfg->current_bss->chanspec,
	                                         buf, sizeof(buf));
}
...
wlc_11h_tdls_write_sup_chan_ie(void *ctx, wlc_iem_build_data_t *build)
{
	...
	_wlc_11h_write_sup_chan_ie(m11h, build->cfg, build->cfg->current_bss->chanspec,
	...
	wlc_bsscfg_t *cfg, void (*func_ptr)(wlc_info_t *, uint, void *))
{
	...
	report_ie->rep.basic.channel = CHSPEC_CHANNEL(phy_utils_get_chanspec(WLC_PI(wlc)));
	rand_tsf = wlc->clk ? R_REG(wlc->osh, D11_TSF_RANDOM(wlc)) : 0;
	memcpy(report_ie->rep.basic.start_time, (uint32 *)&rand_tsf, 4);
	...
	p = wlc_frame_get_action(wlc, da, &cfg->cur_etheraddr, &cfg->BSSID, body_len, &pbody,
	...
	if (!ETHER_ISBCAST(da) && !ETHER_ISMULTI(da)) {
		/* Use scb(if valid) queue. It tracks per scb's fifo
		 * packet counters with wlc_dotxstatus for each
		 * unicast packet.
		 */
		scb = wlc_scbfindband(wlc, cfg, da,
			CHSPEC_BANDUNIT(cfg->current_bss->chanspec));
	}

	wlc_pcb_fn_register(wlc->pcb, func_ptr, (void*)(uintptr)WLC_BSSCFG_IDX(cfg), p);
	...
	uint8 measure_type)
{
	...
	req->channel = wf_chspec_ctlchan(phy_utils_get_chanspec(WLC_PI(wlc)));
...
wlc_11h_set_spect(wlc_11h_info_t *m11h, uint spect)
{
	...
	wlc->pub->cmn->_11h = spect != SPECT_MNGMT_OFF;
