 ...
 *
 * $Id: hnddma.c 810463 2022-04-07 12:15:53Z $
 */
	...
	uint rxoffset, uint *msg_level)
{
	...
	if (flags & (BCM_DMA_HWA_HME_RX | BCM_DMA_HWA_HME_TX)) {
...
_dma_ddtable_init(dma_info_t *di, uint direction, dmaaddr_t pa)
{
	...
	if (((direction == DMA_TX) && DMA_HWA_HME_TX(di)) ||
		...
		ASSERT(!(PHYSADDR64HI(di->dmacommon->hmeaddr) & PCI32ADDR_HIGH));
		...
		if (direction == DMA_TX) {
			...
			W_REG(di->osh, &di->d64txregs->addrhigh,
				(PHYSADDR64HI(di->dmacommon->hmeaddr) +
				di->txddoffsethigh));
		} else {
 ...
 */
void
dma_rx_waitcomplete_enable(hnddma_t *dmah)
{
	dma_info_t * di = DI_INFO(dmah);

	/* Enable WaitForComplete (readback) */
	di->rx_waitcomplete_enabled = TRUE;
...
dma_dumptx(hnddma_t *dmah, struct bcmstrbuf *b, bool dumpring)
{
	...
	if (DMA_HWA_HME_TX(di)) {
		txdpahi = PHYSADDR64HI(di->dmacommon->hmeaddr);
	}
	...
	if (b) {
		...
		bcm_bprintf(b, "DMA64: txd64 %p txdpa 0x%x txdpahi 0x%lx txp %p txin %d txout %d "
			...
			txdpahi, di->txp, di->txin, di->txout, di->hnddma.txavail,
			di->hnddma.txnodesc);
#endif
...
dma_dumprx(hnddma_t *dmah, struct bcmstrbuf *b, bool dumpring)
{
	...
	if (DMA_HWA_HME_RX(di)) {
		rxdpahi = PHYSADDR64HI(di->dmacommon->hmeaddr);
	}
	...
	bcm_bprintf(b, "DMA64: rxd64 %p rxdpa 0x%x rxdpahi 0x%lx head %p tail "
			...
			di->rxin, di->rxout);
#else
	bcm_bprintf(b, "DMA64: rxd64 %p rxdpa 0x%x rxdpahi 0x%lx rxp %p rxin %d rxout %d\n",
...
dma_common_hme_dump(dma_common_t *dmac, struct bcmstrbuf *b)
{
	...
	if (b) {
		bcm_bprintf(b, "HME: addr 0x%x_%x len 0x%x curr 0x%x remain 0x%x\n",
			PHYSADDR64HI(dmac->hmeaddr), PHYSADDR64LO(dmac->hmeaddr),
			dmac->hmelen, dmac->hmecurr,
			(dmac->hmelen - (dmac->hmecurr - PHYSADDR64LO(dmac->hmeaddr))));
	} else {
		DMA_ERROR(("HME: addr 0x%x_%x len 0x%x curr 0x%x remain 0x%x\n",
			PHYSADDR64HI(dmac->hmeaddr), PHYSADDR64LO(dmac->hmeaddr),
			dmac->hmelen, dmac->hmecurr,
			(dmac->hmelen - (dmac->hmecurr - PHYSADDR64LO(dmac->hmeaddr)))));
	}
