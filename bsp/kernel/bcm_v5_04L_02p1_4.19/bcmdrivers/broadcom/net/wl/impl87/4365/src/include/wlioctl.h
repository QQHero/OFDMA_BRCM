 ...
 *
 * $Id: wlioctl.h 807511 2022-01-24 05:53:22Z $
 */
...
typedef struct {
	...
	uint16                  PAD4;         /* authentication type */
	...
	/* Start Dummy PAD Info to match sta_info version 8 */
	uint8			twt_info;
	uint16                  he_omi;
	...
	/* End Dummy PAD Info to match sta_info version 8 */

	uint8			rrm_capabilities[DOT11_RRM_CAP_LEN];	/* rrm capabilities sta */
	uint8			PAD7;
...
typedef struct {
	...
	uint16 thresh0_sc; /* SC Radar detection, thresh 0 */
	uint16 thresh1_sc; /* SC Radar detection, thresh 1 */
} wl_radar_args_t;
...
typedef struct {
	uint32 version; /* version */
	uint16 thresh0_sc_20_lo;	/* SC Radar detection, thresh 0 for BW 20MHz */
	uint16 thresh1_sc_20_lo;	/* SC Radar detection, thresh 1 for BW 20MHz */
	uint16 thresh0_sc_40_lo;	/* SC Radar detection, thresh 0 for BW 40MHz */
	uint16 thresh1_sc_40_lo;	/* SC Radar detection, thresh 1 for BW 40MHz */
	uint16 thresh0_sc_80_lo;	/* SC Radar detection, thresh 0 for BW 80MHz */
	uint16 thresh1_sc_80_lo;	/* SC Radar detection, thresh 1 for BW 80MHz */
	uint16 thresh0_sc_20_hi;	/* SC Radar detection, thresh 0 for BW 20MHz */
	uint16 thresh1_sc_20_hi;	/* SC Radar detection, thresh 1 for BW 20MHz */
	uint16 thresh0_sc_40_hi;	/* SC Radar detection, thresh 0 for BW 40MHz */
	uint16 thresh1_sc_40_hi;	/* SC Radar detection, thresh 1 for BW 40MHz */
	uint16 thresh0_sc_80_hi;	/* SC Radar detection, thresh 0 for BW 80MHz */
	uint16 thresh1_sc_80_hi;	/* SC Radar detection, thresh 1 for BW 80MHz */
	uint16 fc_varth_sb;
...
typedef struct wl_maccapture_params {
	...
	maccapture_cmd_t cmd;	/* Start / Stop / Reset */
} wl_maccapture_params_t;
#endif /* defined(BCMMACDBG) || defined(BCMDBG) */
...
typedef BWL_PRE_PACKED_STRUCT struct pm_awake_data_v2 {
	...
	uint32 frts_time;	/* Cumulative ms spent in frts since driver load */
	uint32 frts_end_cnt;	/* No of times frts ended since driver load */
	uint32 prev_frts_dur;	/* Frts time at start of pm-period */
	uint32 cal_dur;		/* Cumulative ms spent in calibration */
	uint32 prev_cal_dur;	/* cal time at start of pm-period */
...
/* XXX: These definitions should ideally go to wlioctl_defs.h.
 * But the current wlioctl_defs.h is frozen and we need to move to a
 * new location to apply these changes.
 * We are placing it in this file for specific branches to workaround
 * large-scale changes to build system and reducing the risk.
 */
...
typedef struct wl_taf_define {
	...
	char	text[];         /* used to pass and return ascii text */
} wl_taf_define_t;
...
#define WL_MU_POLICY_MAXCLIENTS_MAX	8
