...
{
   return ((mediaType >= I5_MEDIA_TYPE_WIFI_B) && (mediaType <= I5_MEDIA_TYPE_WIFI_AX));
}
  ...
  ieee1905_backhaul_link_metric *metric, unsigned int rxBytesCumulative, unsigned char updateFlag)
{
  ...
  i5TraceModuleInfo(i5TraceEthStat, "capacity=%d linkAvailability=%d latestRxBytes=%d "
    ...
    "receivedPackets=%d rxPacketErrors=%d rssi=%d\n",
    ...
    metric->phyRate, metric->receivedPackets, metric->rxPacketErrors, metric->rssi);
  ...
  if (updateFlag & I5_DM_LINK_METRIC_UPDATE_TX) {
    ...
    if (neighbor->metric.macThroughPutCapacity &&
      ((neighbor->metric.macThroughPutCapacity / 4) > metric->macThroughPutCapacity)) {
       i5TraceDirPrint("Sudden drop in throughput capacity (cur:%d, new:%d) received for NbIf "
  ...
  if (updateFlag & I5_DM_LINK_METRIC_UPDATE_RX) {
    ...
    neighbor->metric.rssi = metric->rssi;
  }
...
i5_dm_interface_type *i5DmInterfaceNew(i5_dm_device_type *parent)
{
  ...
  if (NULL != newIf) {
    ...
    newIf->chan_sel_resp_code = I5_CHAN_SEL_RESP_CODE_NONE;
  ...
  if ((pinterface = i5DmInterfaceFind(pdevice, interface_id)) == NULL) {
    ...
    if (i5DmIsInterfaceWireless(media_type)) {
      ...
      if ((pinterface->chanspec == 0) && !i5DmDeviceIsSelf(device_id) && media_specific_info) {
        i5WlCfgGetChanspecFromMediaInfo((unsigned char*)media_specific_info, pinterface->band, &pinterface->chanspec);
      }
#endif /* MULTIAP */
    }
#endif
...
#if defined(MULTIAPR2)
      /* Maximum number of VLAN ID's supported */
      new->p2ApCap.max_vids = WLIFU_MAX_NO_BRIDGE;
...
#if defined(MULTIAPR2)
    /* Initialize STORED Channel Scan Result list for Each Device */
...
void i5DmDeviceFree(i5_dm_device_type *device)
{
  ...
  if (!isSelfDevice) {
    i5MessageFlushMessageForDstAlMAC(device->DeviceId);
  }
...
int i5DmInit(unsigned int supServiceFlag, int isRegistrar, unsigned char device_mode)
{
  ...
  /* Remove new channels from RC channel map data for certification
   * because testbeds are not supporting it
   */
  if (I5_IS_MAP_CERT(i5_config.flags) ||
    strtol(nvram_safe_get("map_skip_unii4_channels"), NULL, 0)) {
  ...
  unsigned char ssid_len, unsigned char mapFlags)
{
  ...
  if (NULL != newBSS) {
    ...
    /* Include bit for the Estimated Service Parameters Information field for AC=BE. */
    newBSS->APMetric.include_bit_esp |= IEEE1905_INCL_BIT_ESP_BE;
    ...
    newBSS->flags |= I5_BSS_FLAG_QUERY_STATE; /* Set the state to done */
...
void i5DmChannelScanResultItemInit(ieee1905_chscan_result_item *chscan_rpt_item)
{
  ...
  memset(chscan_rpt_item->ifname, 0, sizeof(chscan_rpt_item->ifname));
  chscan_rpt_item->chanspec_20 = 0;
  memset(&chscan_rpt_item->pry_info, 0, sizeof(chscan_rpt_item->pry_info));
...
void i5DmConfigListFree(ieee1905_policy_config *config_list)
{
  ...
  i5DmGlistCleanup(&config_list->no_bh_bss_list);
  i5DmGlistCleanup(&config_list->no_err_code_list);
}
...
void i5DmRemoveNewChannelsFromRCChannelMap()
{
  ...
  for (i = 0; i < reg_class_count; i++) {
    if (rc_chan_map[i].regclass < 125) {
      continue;
    }
    if (rc_chan_map[i].regclass > 130) {
      break;
    }

    /* Skip last two channels for operating class 125,126 and 127 AND
     * Skip last one channel for operating class 128, 129 and 130
     * So the new channels to skip are:
     * 125 : 173, 177
     * 126 : 165, 173
     * 127 : 169, 177
     * 128 : 171
...
ieee1905_client_bssinfo_type *i5DmFindMDIDInBSSTable(unsigned short mdid)
{
  ...
  foreach_iglist_item(bss, ieee1905_client_bssinfo_type,
    i5_config.client_bssinfo_list) {

    /* MDID cannot be 0. Backhaul will always have 0 MDID and without this condition it will always
     * return bss table of backhaul
     */
    if (bss->fbt_info.mdid == 0) {
      continue;
    }

    if (bss->fbt_info.mdid == mdid) {
      return bss;
    }
  }
  ...
  for (device = i5_dm_network_topology.device_list.ll.next; device; device = device->ll.next) {
    ...
    for (neighbor = device->neighbor1905_list.ll.next; neighbor; neighbor = neighbor->ll.next) {
      ...
      snprintf(tmpBuf, sizeof(tmpBuf), "      MacThroughputCapacity=%d, linkAvailability=%d, "
        "TxErr=%d, TxPkts=%d, phyRate=%d, RxPkts=%d, RxErr=%d, rssi=%d\n",
  ...
  unsigned char curr_channel, unsigned char scan_status_code, ieee1905_chscan_report_msg *chscan_rpt)
{
  ...
  /* Initialize Result Interpretation Fields used by Controller */
  memset(result_item->ifname, 0, sizeof(result_item->ifname));
  result_item->chanspec_20 = 0;
  memset(&result_item->pry_info, 0, sizeof(result_item->pry_info));
...
/* Create Empty Channel Scan Results in this Device to send Result for Requested Channel Scan - Stored */
  ...
  ieee1905_chscan_report_msg *emt_chscan_rpt, unsigned char in_status_code, bool append_nbrs)
{
  ...
  /* Go Through details for Empty Result Item and Fill It */
  foreach_iglist_item(emt_p, ieee1905_chscan_result_item, emt_chscan_rpt->chscan_result_list) {

    /* Find this Empty Item with each Stored Result Item in Self Device */
    fild_p = i5DmFindChannelInScanResult(fild_chscan_rpt, emt_p->opclass, emt_p->channel);
    if (!fild_p) {
      continue;
    }
    ...
    /* Result Interpretation Fields used by Controller */
    memcpy(emt_p->ifname, fild_p->ifname, I5_MAX_IFNAME);
    emt_p->chanspec_20 = fild_p->chanspec_20;
    memcpy(&emt_p->pry_info, &fild_p->pry_info, sizeof(emt_p->pry_info));

    i5TraceInfo("Channel Scan Result: [%s] radio_mac[" I5_MAC_DELIM_FMT "] opclass[%d] "
      "channel[%d] chanspec_20[0x%X] scan_status_code[%d] utilization[%d] noise[%d] agr_scan_dur[%d] "
      "Start TS[%lu] End TS[%lu] nCtrl[%u] nExt20[%u] nExt40[%u] nExt80[%u] Copied\n",
      emt_p->ifname, I5_MAC_PRM(emt_p->radio_mac), emt_p->opclass,
      emt_p->channel, emt_p->chanspec_20, emt_p->scan_status_code, emt_p->utilization, emt_p->noise,
      emt_p->aggregate_scan_duration, (unsigned long)emt_p->ts_scan_start,
      (unsigned long)emt_p->ts_scan_end, emt_p->pry_info.n_ctrl, emt_p->pry_info.n_ext20,
  ...
  unsigned char *ssid)
{
  ...
  /* Traverse each SSID in SSID list for a VLAN ID */
  foreach_iglist_item(ssid_type, ieee1905_ssid_list_type, ts_policy->ssid_list) {

    if (strcmp((char*)ssid_type->ssid.SSID, (char*)ssid) == 0) {
      i5Trace("SSID[%s] found\n", ssid);
      return ssid_type;
    }
  }
  ...
  map_p2_err_codes_t err_code, unsigned char *bssid)
{
  ...
  err_resp = (ieee1905_err_code_t*)malloc(sizeof(*err_resp));
  if (!err_resp) {
    i5TraceDirPrint("malloc error\n");
    goto end;
  }
  ...
  err_resp->err_code = err_code;
  if (NULL != bssid) {
    eacopy(bssid, &err_resp->bssid);
  }
  ieee1905_glist_append(&policy->no_err_code_list, (dll_t*)err_resp);
  i5Trace("Added err_code[%d]\n", err_code);
