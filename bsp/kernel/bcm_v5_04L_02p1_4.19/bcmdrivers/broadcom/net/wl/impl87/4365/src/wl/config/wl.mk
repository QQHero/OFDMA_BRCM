...
# $Id: wl.mk 804400 2021-10-29 00:20:53Z $
...
ifdef BCMPHYCOREMASK
	WLFLAGS += -DBCMPHYCOREMASK=$(BCMPHYCOREMASK)
endif
...
ifeq ($(WL),1)
	...
	WLFILES_SRC_LO += src/wl/sys/wlc_vasip.c

	ifneq ($(MINIAP),1)
		WLFILES_SRC_LO += src/wl/sys/d11ucode_ge40.c
		WLFILES_SRC_LO += src/wl/sys/d11ucode_mu.c
		WLFILES_SRC_LO += src/wl/sys/d11ucode_le15.c
		ifeq ($(BTCX_ENABLED),1)
			WLFILES_SRC_LO += src/wl/sys/d11ucode_btcx.c
			WLFILES_SRC_LO += src/wl/sys/d11ucode_btcx_mu.c
		endif
		ifneq ($(WL_AIRIQ),)
			WLFILES_SRC_LO += src/wl/sys/d11vasip_airiq.c
		else
			WLFILES_SRC_LO += src/wl/sys/d11vasip_code.c
			WLFILES_SRC_LO += src/wl/sys/d11vasip3x3_code.c
		endif
	endif

	ifeq ($(WLCX_ATLAS),1)
		WLFILES_SRC_LO += src/wl/sys/d11ucode_wlcx.c
	endif
	...
	WLFILES_SRC_HI += src/wl/sys/d11reglist.c
	WLFILES_SRC_HI += src/wl/sys/wlc_macdbg.c
	WLFILES_SRC_HI += src/wl/sys/wlc_txtime.c
	WLFILES_SRC_HI += src/wl/sys/wlc_mux.c
	WLFILES_SRC_HI += src/wl/sys/wlc_mux_utils.c
	...
	ifeq ($(WL_PROT_OBSS),1)
		...
		WLFILES_SRC_HI += src/wl/sys/wlc_obss_util.c
	...
	ifeq ($(WL_OBSS_DYNBW),1)
		...
		WLFILES_SRC_HI += src/wl/sys/wlc_obss_util.c
		WLFILES_SRC_HI += src/wl/sys/wlc_obss_dynbw.c
	endif
	ifneq ($(WL_AIRIQ),)
		WL_MODESW = 1
		WLFLAGS += -DWL_AIR_IQ
		WLFILES_SRC_HI += src/wl/airiq/src/wlc_airiq.c
		WLFILES_SRC_HI += src/wl/airiq/src/wlc_airiq_3p1.c
		WLFILES_SRC_HI += src/wl/airiq/src/wlc_airiq_capture.c
		WLFILES_SRC_HI += src/wl/airiq/src/wlc_airiq_iov.c
		WLFILES_SRC_HI += src/wl/airiq/src/wlc_airiq_phy.c
		WLFILES_SRC_HI += src/wl/airiq/src/wlc_airiq_scan.c
	endif
	ifeq ($(WL_MODESW),1)
		WLFLAGS += -DWL_MODESW
		WLFILES_SRC_HI += src/wl/sys/wlc_modesw.c
		WLFLAGS += -DPHYCAL_CACHING
		# enable when required for local builds
		# ifeq ($(DEBUG),1)
	...
	ifeq ($(WL_BCNTRIM),1)
		WLFLAGS += -DWL_BCNTRIM
		WLFILES_SRC_HI += src/wl/sys/wlc_bcntrim.c
	endif
...
ifeq ($(WLNDIS),1)
	...
	ifeq ($(WL_PROXDETECT),1)
		...
		ifeq ($(WL_NAN),1)
			WLFILES_SRC_HI += src/wl/proxd/src/wlc_pdnan.c
		endif
	endif
	ifeq ($(WL_NAN),1)
		WLFLAGS += -DWL_NAN
...
ifeq ($(WL_SPLIT),0)
	ifeq ($(WL_MU_TX),1)
		WLFLAGS += -DWL_MU_TX
		WLFILES_SRC_HI += src/wl/sys/wlc_mutx.c
	endif
endif
	ifeq ($(WL_MU_RX),1)
		WLFLAGS += -DWL_MU_RX
	...
	ifeq ($(DFS_WAVE_MODE),1)
		WLFLAGS += -DWL_DFS_WAVE_MODE
	endif
...
ifeq ($(WLNDIS),1)
	ifeq ($(WLXP), 1)
		...
		WLFILES_SRC += src/wl/ndis/src/wl_ndtimer.c
		WLFILES_SRC += src/wl/ndis/src/wl_ndindicate.c
		WLFILES_SRC += src/wl/ndis/src/wl_ndcommon.c
		WLFILES_SRC += src/wl/ndis/src/wl_ndglobal.c
		WLFILES_SRC += src/wl/ndis/src/wl_ndoid.c
		WLFILES_SRC += src/wl/ndis/src/wl_ndnwifi.c
	endif
...
ifeq ($(WL_OIDS),1)
	...
	ifeq ($(WL),1)
		...
		ifneq ($(WLVISTA)$(WLWIN7),)
			...
			ifneq ($(BCMDONGLE),1)
			   ifneq ($(BCMCCX),0)
			        # Allow for turning off ccx in vista drivers
				WLFLAGS += -DCCX_SDK
			   endif
			else
				# DONGLE vista needs WL_MONITOR to pass RTM
				WLFLAGS += -DEXT_STA -DWL_MONITOR
			endif
		else
...
ifeq ($(EXT_STA_DONGLE),1)
	...
	WLFLAGS += -DIBSS_PEER_DISCOVERY_EVENT
	WLFLAGS += -DIBSS_PEER_MGMT
	# xxx EXT_STA dongle ROM builds need to set WLEIND_DISABLED to disable event indication
endif
#endif
...
# Proxy STA
ifeq ($(PSTA),1)
	WLFILES_SRC_HI += src/shared/bcm_psta.c
...
ifeq ($(WL_STA_MONITOR_COMP),1)
	WLFLAGS += -DWL_STA_MONITOR_COMP
endif
#endif
...
#ifdef BCMCCX
ifeq ($(BCMCCX),1)
	WLFLAGS += -DBCMCCX -DWLCAC
	# external supplicant: need some crypto files for ccx
	ifneq ($(BCMSUP_PSK),1)
		WLFILES_SRC_HI += src/bcmcrypto/hmac.c
		WLFILES_SRC_HI += src/bcmcrypto/prf.c
		WLFILES_SRC_HI += src/bcmcrypto/sha1.c
	...
	WLFILES_SRC_HI += src/wl/sys/wlc_ccx.c
	WLFILES_SRC_HI += src/wl/sys/wlc_cac.c
	WLFILES_SRC_HI += src/bcmcrypto/aes.c
	WLFILES_SRC_HI += src/bcmcrypto/bcmccx.c
	WLFILES_SRC_HI += src/bcmcrypto/ccx.c
	WLFILES_SRC_HI += src/bcmcrypto/des.c
	WLFILES_SRC_HI += src/bcmcrypto/md4.c
	# NetBSD 2.0 has MD5 and AES built in
	ifneq ($(OSLBSD),1)
		WLFILES_SRC_HI += src/bcmcrypto/md5.c
		WLFILES_SRC_HI += src/bcmcrypto/rijndael-alg-fst.c
...
ifeq ($(WLFIPS),1)
	WLFLAGS += -DWLFIPS
	WLFILES_SRC_HI += src/wl/sys/wl_ndfips.c
	WLFILES_SRC_HI += src/bcmcrypto/aes.c
endif
#endif
...
ifeq ($(WOWL),1)
	...
	WLFILES_SRC_HI += src/wl/sys/d11ucode_mu.c
	WLFILES_SRC_HI += src/wl/sys/d11ucode_ge24.c
	WLFILES_SRC_HI += src/wl/sys/wlc_wowl.c
	WLFILES_SRC_HI += src/wl/sys/wowlaestbls.c
	ifeq ($(BTCX_ENABLED),1)
		WLFILES_SRC_HI += src/wl/sys/d11ucode_btcx_p2p.c
		WLFILES_SRC_HI += src/wl/sys/d11ucode_btcx.c
		WLFILES_SRC_HI += src/wl/sys/d11ucode_btcx_mu.c
	endif
endif
#endif
...
#ifdef FLOW_PRIO_MAP_AC
ifeq ($(FLOW_PRIO_MAP_AC),1)
	WLFLAGS += -DFLOW_PRIO_MAP_AC
endif
#endif
...
ifeq ($(WLP2P_UCODE),1)
	...
	ifeq ($(BTCX_ENABLED),1)
		WLFILES_SRC_LO += src/wl/sys/d11ucode_btcx_p2p.c
	endif
endif
...
ifeq ($(WLRSDB),1)
	WLFLAGS += -DWLRSDB -DSHARED_OSL_CMN -DRSDB_DFS_SCAN
	WL_OBJ_REGISTRY := 1
...
#ifdef BCMSDIO
ifeq ($(BCMSDIO),1)
	WLFLAGS += -DBCMSDIO -DBCMCCISSR3
	BCMPCI = 0
	# Non-PCI WL SDIO Driver
	# BRCM SDIO stack for Standard SDIO host
	ifeq ($(BCMSDIOH_STD),1)
		WLFLAGS += -DBCMSDIOH_STD
		WLFILES_SRC += src/bcmsdio/sys/bcmsdh.c
		WLFILES_SRC += src/bcmsdio/sys/bcmsdstd.c
		ifeq ($(WLIOPOS), 1) # Our Arasan stack in IOP OS.
			WLFILES_SRC += src/bcmsdio/sys/bcmsdstd_iopos.c
			WLFILES_SRC += src/bcmsdio/sys/bcmsdh_iopos.c
		endif
		ifeq ($(OSLLX),1)
			WLFILES_SRC += src/bcmsdio/sys/bcmsdstd_linux.c
			WLFILES_SRC += src/bcmsdio/sys/bcmsdh_linux.c
		else
			ifeq ($(OSLNDIS),1)
				WLFILES_SRC += src/bcmsdio/sys/bcmsdstd_ndis.c
				WLFILES_SRC += src/wl/ndis/src/bcmsdh_ndis.c
			endif
		endif
	endif
	...
	ifeq ($(BCMSDIOH_BCM),1)
		...
		WLFILES_SRC += src/bcmsdio/sys/bcmsdh.c
		WLFILES_SRC += src/bcmsdio/sys/bcmsdbrcm.c
		WLFILES_SRC += src/bcmsdio/sys/bcmsdh_linux.c
		WLFILES_SRC += src/bcmsdio/sys/bcmsdbrcm_linux.c
	endif
	...
	ifeq ($(BCMSDIOH_SPI),1)
		...
		WLFILES_SRC += src/bcmsdio/sys/bcmsdh.c
		WLFILES_SRC += src/bcmsdio/sys/bcmsdh_linux.c
		WLFILES_SRC += src/bcmsdio/sys/bcmsdspi.c
		WLFILES_SRC += src/bcmsdio/sys/bcmsdspi_linux.c
		WLFILES_SRC += src/bcmsdio/sys/bcmpcispi.c
	endif
	# BCRM SDIO stub functions
	ifeq ($(BCMSDIOH_STUB),1)
		WLFILES_SRC += src/bcmsdio/sys/bcmsdh_stubs.c
	endif
endif
#endif
...
ifeq ($(WL_MBO),1)
	WLFLAGS += -DWL_MBO_OCE
	WLFLAGS += -DWL_MBO
	WLFILES_SRC_HI += src/wl/mbo_oce/src/wlc_mbo.c
endif
