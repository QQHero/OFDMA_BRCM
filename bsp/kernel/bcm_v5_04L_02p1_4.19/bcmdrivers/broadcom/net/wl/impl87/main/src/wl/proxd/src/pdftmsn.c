 ...
 *
 * $Id: pdftmsn.c 810354 2022-04-05 20:44:02Z $
 */
...
pdftm_session_resolve_ratespec(pdftm_t *ftm, chanspec_t cspec, ratespec_t *rspec)
{
	...
	if (HE_ENAB_BAND(FTM_PUB(ftm), bandtype) ||
	    VHT_ENAB_BAND(FTM_PUB(ftm), bandtype)) {
		if (bandtype == WLC_BAND_6G &&
		    HE_ENAB_BAND(FTM_PUB(ftm), bandtype)) {
...
pdftm_validate_session(pdftm_session_t *sn)
{
	...
	/* enable vht ack - if negotiated rate is vht */
	sncfg->flags &= ~WL_PROXD_SESSION_FLAG_VHTACK;
	if (FTM_SESSION_IS_INITIATOR(sn) && FTM_SESSION_IS_AUTO_VHTACK(sn)) {
		sncfg->flags |= WL_PROXD_SESSION_FLAG_VHTACK;
	}
...
pdftm_free_sessions(pdftm_t *ftm, wlc_bsscfg_t *bsscfg, bool detach)
{
	...
	if ((bsscfg != NULL) && FTM_BSSCFG_SECURE(wlc_ftm_get_handle(ftm->wlc), bsscfg)) {
	...
	for (i = 0; i < ftm->max_sessions; ++i) {
		...
		} else if (sn->state == WL_PROXD_SESSION_STATE_ENDED) {
			(void)pdftm_change_session_state(sn, BCME_OK,
	...
	wl_proxd_session_state_t new_state)
{
	...
	TOF_PRINTF(("***pdftm_change_session_state: time:%u, old:%d, new:%d\n",
		(uint32)OSL_SYSUPTIME_US(), old_state, new_state));
...
pdftm_resolve_ftm_sep(pdftm_t *ftm, wl_proxd_session_flags_t flags, chanspec_t cspec)
{
	...
	if ((!FTM_AVB_TS(ftm->config->flags)) &&
		(!(flags & WL_PROXD_SESSION_FLAG_ONE_WAY))) {
		if (CHSPEC_IS160(cspec)) {
			sep = FTM_SEP_SAMPCAP_160M;
		} else if (CHSPEC_IS80(cspec)) {
			sep = FTM_SEP_SAMPCAP_80M;
		} else if (CHSPEC_IS40(cspec)) {
			sep = FTM_SEP_SAMPCAP_40M;
		} else {
			sep = FTM_SEP_SAMPCAP_20M;
		}
	} else
...
int pdftm_start_session_tsf_scan(pdftm_session_t *sn)
{
	...
	err = wlc_scan_request(ftm->wlc, DOT11_BSSTYPE_ANY,
		&sncfg->burst_config->peer_mac, 1, &ssid, 0, NULL,
