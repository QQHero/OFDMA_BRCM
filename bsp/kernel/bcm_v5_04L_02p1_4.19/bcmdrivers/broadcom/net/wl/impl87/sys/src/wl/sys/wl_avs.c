 ...
 *
 * $Id: wl_avs.c 803998 2021-10-15 03:31:07Z $
 */
...
enum {
	...
	IOV_AVS_DAC        = 105,
	IOV_AVS_VNOMINAL   = 106,
	IOV_AVS_VLOCALFIT  = 107,
	IOV_AVS_OSCCNT     = 108,
	IOV_AVS_TRACE      = 109,
	IOV_AVS_FLAGS      = 110
};
...
static const bcm_iovar_t avs_iovars[] = {
	...
	{"avs_vnominal",   IOV_AVS_VNOMINAL,	0, 0, IOVT_UINT32, sizeof(uint32) },
	{"avs_vlocalfit",  IOV_AVS_VLOCALFIT,	0, 0, IOVT_UINT32, sizeof(uint32) },
	{"avs_osccnt",	   IOV_AVS_OSCCNT,	0, 0, IOVT_UINT32, sizeof(uint32) },
	{"avs_trace",	   IOV_AVS_TRACE,	0, 0, IOVT_UINT32, sizeof(uint32) },
	{"avs_flags",	   IOV_AVS_FLAGS,	0, 0, IOVT_UINT32, sizeof(uint32) },
	{NULL, 0, 0, 0, 0, 0}
};
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_AVS_RESET):
		...
		*pval32 = 1;
		break;
	case IOV_GVAL(IOV_AVS_FLAGS):
		err = avs_get_value(avs->avs, AVS_INDEX_FLAGS, pval32);
		break;
	case IOV_SVAL(IOV_AVS_FLAGS):
		err = avs_set_flags(avs->avs, *pval32);
		break;
	...
	case IOV_SVAL(IOV_AVS_DAC):
		...
		break;
	case IOV_SVAL(IOV_AVS_VNOMINAL):
		err = avs_set_vnominal(avs->avs, *pval32);
		break;
	case IOV_SVAL(IOV_AVS_VLOCALFIT):
		err = avs_set_vlocalfit(avs->avs, *pval32);
		break;
	case IOV_GVAL(IOV_AVS_OSCCNT):
		err = avs_dump_osccnt(avs->avs);
		break;
	case IOV_SVAL(IOV_AVS_TRACE):
		err = avs_set_trace(avs->avs, *pval32);
		break;
#endif /* BCMDBG || WLTEST */
	default:
