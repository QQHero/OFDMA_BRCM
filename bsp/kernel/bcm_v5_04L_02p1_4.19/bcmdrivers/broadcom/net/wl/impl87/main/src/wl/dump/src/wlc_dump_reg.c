 ...
 *
 * $Id: wlc_dump_reg.c 783998 2020-02-14 14:32:04Z $
 */
#include <typedefs.h>
#include <bcmdefs.h>
#include <osl.h>
	...
	wlc_dump_reg_dump_fn_t dump_fn, wlc_dump_reg_clr_fn_t clr_fn, void *ctx)
{
	...
	if (ret == BCME_NOTFOUND) {
		if (reg->cur < reg->max) {
			strncpy(reg->ent[reg->cur].name, name, sizeof(reg->ent[reg->cur].name) - 1);
			reg->ent[reg->cur].name[sizeof(reg->ent[reg->cur].name) - 1] = '\0';
