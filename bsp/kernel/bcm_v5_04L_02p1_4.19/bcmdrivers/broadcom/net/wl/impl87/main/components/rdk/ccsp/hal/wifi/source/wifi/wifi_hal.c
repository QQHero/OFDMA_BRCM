...
void hal_wifi_log(const char *fmt, ...)
{
	...
	log_fd = fopen(HAL_WIFI_LOG_FILE, "a");
	if (log_fd) {
		ltime = time(NULL);
		localtime_r(&ltime, &time_info);
		asctime_r(&time_info, utcstring);
...
extern int wlcsm_nvram_commit(void);
...
static int hal_csi_rec_read(unsigned int RadioIndex, char *mac, char *buf);

static FILE *wifihealth_fp;
...
static void restart_acsd(void)
{
	...
	system("killall -q -9 acsd2 2>/dev/null");
	system("acsd2");
	wlcsm_nvram_set("acsd2_started", "1");
}

static void print_time(char *t, int len)
{
	time_t timer;
	struct tm time_info = {0};
	...
	strftime(t, len, "%Y-%m-%d %H:%M:%S", &time_info);
}
 ...
 */
int wifi_factoryReset_post(int index, int commit, int restart)
{
	char cmd[64] = {0};
	...
	if (restart) {
		snprintf(cmd, sizeof(cmd), "wifi_setup.sh restart %s",
			(index == -1) ? "" : wldm_get_nvifname(index));
...
INT wifi_factoryResetRadio(int radioIndex) //RDKB
{
	...
	snprintf(cmd, sizeof(cmd), "wifi_setup.sh reset_radio_nvram %s", wldm_get_radio_osifname(radioIndex));
	if (system(cmd)) {
		HAL_WIFI_ERR(("%s: failed\n", __FUNCTION__));
		return RETURN_ERR;
	}
...
#else
	system("wifi_setup.sh restart");
	HAL_WIFI_LOG(("%s: Start ACSD! radioIndex=[%d]\n", __FUNCTION__, index));
	restart_acsd();
#endif
	if (!wifi_apiThread) {
	...
	UINT *output_array_size)
{
	...
	*neighbor_ap_array = NULL;
	...
	for (i = 0; i < num; ++i) {
		strncpy(wifi_neighbor_ptr[i].ap_SSID, wldm_neighbor_ptr[i].ap_SSID,
			sizeof(wifi_neighbor_ptr[i].ap_SSID));
		strncpy(wifi_neighbor_ptr[i].ap_BSSID, wldm_neighbor_ptr[i].ap_BSSID,
			sizeof(wifi_neighbor_ptr[i].ap_BSSID));
		strncpy(wifi_neighbor_ptr[i].ap_Mode, wldm_neighbor_ptr[i].ap_Mode,
			sizeof(wifi_neighbor_ptr[i].ap_Mode));
		...
		strncpy(wifi_neighbor_ptr[i].ap_SecurityModeEnabled,
			wldm_neighbor_ptr[i].ap_SecurityModeEnabled,
			sizeof(wifi_neighbor_ptr[i].ap_SecurityModeEnabled));
		strncpy(wifi_neighbor_ptr[i].ap_EncryptionMode,
			wldm_neighbor_ptr[i].ap_EncryptionMode,
			sizeof(wifi_neighbor_ptr[i].ap_EncryptionMode));
		strncpy(wifi_neighbor_ptr[i].ap_OperatingFrequencyBand,
			wldm_neighbor_ptr[i].ap_OperatingFrequencyBand,
			sizeof(wifi_neighbor_ptr[i].ap_OperatingFrequencyBand));
		strncpy(wifi_neighbor_ptr[i].ap_SupportedStandards,
			wldm_neighbor_ptr[i].ap_SupportedStandards,
			sizeof(wifi_neighbor_ptr[i].ap_SupportedStandards));
		strncpy(wifi_neighbor_ptr[i].ap_OperatingStandards,
			wldm_neighbor_ptr[i].ap_OperatingStandards,
			sizeof(wifi_neighbor_ptr[i].ap_OperatingStandards));
		strncpy(wifi_neighbor_ptr[i].ap_OperatingChannelBandwidth,
			wldm_neighbor_ptr[i].ap_OperatingChannelBandwidth,
			sizeof(wifi_neighbor_ptr[i].ap_OperatingChannelBandwidth));
		...
		strncpy(wifi_neighbor_ptr[i].ap_BasicDataTransferRates,
			wldm_neighbor_ptr[i].ap_BasicDataTransferRates,
			sizeof(wifi_neighbor_ptr[i].ap_BasicDataTransferRates));
		strncpy(wifi_neighbor_ptr[i].ap_SupportedDataTransferRates,
			wldm_neighbor_ptr[i].ap_SupportedDataTransferRates,
			sizeof(wifi_neighbor_ptr[i].ap_SupportedDataTransferRates));
...
static INT start_neighbor_scan(INT radioIndex, wifi_neighborScanMode_t scan_mode, INT dwell_time,
	...
	UINT chan_num, UINT *chan_list)
{
	...
	return start_neighbor_scan(radioIndex, scan_mode, dwell_time, chan_num, chan_list);
}
...
{
	int len = sizeof(ap_AirTimePercent);
...
{
	int len = sizeof(*output_ap_AirTimePercent);
...
{
	char assoclist[OUTPUT_STRING_LENGTH_1024], *pmac, *psave;
	int assoclist_len = sizeof(assoclist);
	char stas[WL_MAX_ASSOC_STA][MAC_STR_LEN] = {{0}};
	int stapercs[WL_MAX_ASSOC_STA], num_stas = 0, i;
	wldm_atm_staperc_t wldm_staperc;
	int wldm_staperc_len = sizeof(wldm_staperc), output_array_len;
	...
	for (i = 0; i < num_stas; i++) {
		snprintf(wldm_staperc.macstr, sizeof(wldm_staperc.macstr), "%s", stas[i]);
	...
	for (i = 0, output_array_len = 0; i < num_stas; i++) {
		output_array_len += snprintf(output_sta_MAC_ATM_array + output_array_len,
...
{
	wldm_atm_staperc_t wldm_staperc;
	int wldm_staperc_len = sizeof(wldm_staperc);
...
INT wifi_getRadioNumberOfEntries(ULONG *output) //Tr181
{
	...
	int len;
	if (wldm_RadioNumberOfEntries(CMD_GET, 0, (int *)output, &len, NULL, NULL) < 0) {
		HAL_WIFI_ERR(("%s: wldm_RadioNumberOfEntries failed\n", __FUNCTION__));
		return RETURN_ERR;
	}
...
{
	UINT total, len = sizeof(total);
	if (NULL == output) {
		HAL_WIFI_ERR(("%s: output parameter error!!!\n", __FUNCTION__));
		return RETURN_ERR;
	}

	if (wldm_SSIDNumberOfEntries(CMD_GET, -1, (unsignedInt *) &total, &len, NULL, NULL) < 0) {
...
INT wifi_getRadioIfName(INT radioIndex, CHAR *output_string) //Tr181
{
	...
	*output_string = '\0';
	...
	strcpy(output_string, wldm_get_radio_osifname(radioIndex));
...
INT wifi_getRadioMaxBitRate(INT radioIndex, CHAR *output_string)	//RDKB
{
	...
	sprintf(output_string, "%d Mbps", (MaxBitRate/2)); /* MaxBitRate in Kbps/2 = BR in Mbps */
	...
	unsigned int pureMode, chVar_info_t *chInfo)
{
	...
	if (bwCapStr != NULL) {
		strcpy(chInfo->channelBWStr, bwCapStr);
	}
	if (extChStr != NULL) {
		strcpy(chInfo->extensionChStr, extChStr);
	}
	...
	defined(_CBR2_PRODUCT_REQ_))
		...
		case CHMODE_11AX:
			chInfo->HeFeatValue = HE_FEATURES_DEFAULT;

			/* vhtmode auto for 2G in AX */
	...
	if (opStdp != NULL) {
		strcpy(chInfo->operatingStandards, opStdp);
	} else {
...
INT wl_setRadioMode(INT radioIndex, CHAR *channelMode, UINT pureMode)
{
	...
	ret = wldm_xbrcm_ap(CMD_SET_NVRAM, HAL_RADIO_IDX_TO_HAL_AP(radioIndex), &(chVars.omcVal), &len, "mode_reqd", NULL);
	if (ret != 0) {
	...
	len = sizeof(chVars.HeFeatValue);
	ret = wldm_AXfeatures(CMD_SET, radioIndex, &(chVars.HeFeatValue), &len, NULL, NULL);
	if (ret != RETURN_OK) {
		HAL_WIFI_DBG(("%s, radioIndex=%d Failed setHEfeatures ret=%d HeFeatValue=0x%x\n",
			__FUNCTION__, radioIndex, ret, chVars.HeFeatValue));
		return RETURN_ERR;
	}
...
INT wifi_getRadioDfsEnable(INT radioIndex, BOOL *output_bool)   //Tr181
{
	...
	*output_bool = FALSE;
	...
	returnStatus = wldm_Radio_DfsEnable(CMD_GET, radioIndex, output_bool, &len, NULL, NULL);
	if (RETURN_OK != returnStatus) {
		HAL_WIFI_DBG(("%s: wldm_Radio_DfsEnable CMD_GET failed for radioIndex=%d Status=%d\n",
...
INT wifi_setRadioDfsEnable(INT radioIndex, BOOL enable) //Tr181
{
	...
	int returnStatus;
	int len = sizeof(enable);
	returnStatus = wldm_Radio_DfsEnable(CMD_SET, radioIndex, &enable, &len, NULL, NULL);
	if (RETURN_OK != returnStatus) {
...
INT wifi_getRadioTransmitPower(INT radioIndex, ULONG *output_ulong) //RDKB: absolute tx pwr in dBm
{
	...
	int returnStatus, len = sizeof(*output_ulong);
	*output_ulong = 0;
...
INT wifi_getRadioSupportedDataTransmitRates(INT radioIndex, CHAR *output)
{
	...
	snprintf(output, OUTPUT_STRING_LENGTH_64, "%s", supportedDataTransmitRates);
...
INT wifi_getRadioOperationalDataTransmitRates(INT radioIndex, CHAR *output)
{
	...
	snprintf(output, OUTPUT_STRING_LENGTH_64, "%s", operationalDataTransmitRates);
	return RETURN_OK;
}
...
INT wifi_getRadioBasicDataTransmitRates(INT radioIndex, CHAR *output)
{
	...
	snprintf(output, OUTPUT_STRING_LENGTH_32, "%s", basicDataTransmitRates);
...
INT wifi_getSSIDName(INT apIndex, CHAR *output)
{
	...
	if (wldm_SSID_SSID(CMD_GET_NVRAM, apIndex, output, &len, NULL, NULL) == 0) {
		HAL_WIFI_DBG(("%s, apIndex [%d] ssid=%s\n", __FUNCTION__, apIndex, output));
		return RETURN_OK;
	}
...
{
	int rc = RETURN_OK;
	CHAR line[OUTPUT_STRING_LENGTH_512] = {'\0'}, *devName;
	...
	/* first two lines are lolumn description, ignore them */
...
#ifdef WIFI_HAL_VERSION_3_PHASE2
INT wifi_getNeighboringWiFiDiagnosticResult2(INT radioIndex, BOOL scan, wifi_neighbor_ap2_t **neighbor_ap_array, UINT *output_array_size)
{
	HAL_WIFI_DBG(("%s: radioIndex %d\n", __FUNCTION__, radioIndex));

	RADIO_INDEX_ASSERT(radioIndex);
	if (scan) {
		if (start_neighbor_scan(radioIndex, WIFI_RADIO_SCAN_MODE_FULL,
...
{
	HAL_WIFI_DBG((" %s apIndex %d\n", __FUNCTION__, apIndex));
	unsigned char DeviceMacAddress[18] = {'\0'};
	INT iResult, len;
...
INT wifi_setRadioSTBCEnable(INT radioIndex, BOOL STBC_Enable)
{
	...
	int len = sizeof(STBC_Enable);

	if (wldm_xbrcm_Radio_STBCEnable(CMD_SET, radioIndex, &STBC_Enable, &len, NULL, NULL) < 0) {
	...
	switch(mode) {
		...
		case BASIC_AUTHENTICATION_MODE_EAP:
		{
			if((beaconType[apIndex]) == BEACON_TYPE_WPA) {
					sprintf(encMode, "WPA-Enterprise");
			}
			else if((beaconType[apIndex]) == BEACON_TYPE_11I) {
				sprintf(encMode, "WPA2-Enterprise");
			}  else {
				sprintf(encMode, "WPA-WPA2-Enterprise");
			}
			break;
		}
		case BASIC_AUTHENTICATION_MODE_SHARED:
		{
			HAL_WIFI_DBG(("%s: doesn't support this mode: %d!!! treat as MODE_EAP\n", __FUNCTION__, mode));
			break;
		}
		case BASIC_AUTHENTICATION_MODE_PSK:
		{
			if((beaconType[apIndex]) == BEACON_TYPE_WPA) {
				sprintf(encMode, "WPA-Personal");
			} else if((beaconType[apIndex]) == BEACON_TYPE_11I) {
				sprintf(encMode, "WPA2-Personal");
			} else {
				sprintf(encMode, "WPA-WPA2-Personal");
			}
			break;
		}
		default :
		{
			HAL_WIFI_DBG(("%s: Fail, mode parameter error = %d\n", __FUNCTION__, mode));
			return RETURN_ERR;
		}
	}
...
{
	char *mac_str_array = NULL, *mac_str = NULL, *rest;
...
INT wifi_kickApAclAssociatedDevices(INT apIndex, BOOL enable)
{
	...
	iResult = wldm_xbrcm_ap(CMD_GET, apIndex, assoc_macs, &len, "assoclist", NULL);
	if (iResult != RETURN_OK) {
...
INT wifi_getApBridgeInfo(INT index, CHAR *name, CHAR *IP, CHAR *subnet)
{
	...
	strncpy(name, bridgeName[index], strlen(bridgeName[index])+1);
	strncpy(IP, bridgeIP[index], strlen(bridgeIP[index])+1);
	strncpy(subnet, bridgeSubnet[index], strlen(bridgeSubnet[index])+1);
...
{
	int len = sizeof(enable), radioIndex;
...
INT wifi_getApStatus(INT apIndex, CHAR *output_string)
{
	...
	/* Check if bss is enabled in nvram first */
	wifi_getApEnable(apIndex, &enable);
	if (FALSE == enable) {
		HAL_WIFI_ERR(("%s: apIndex = %d bss is not enabled in NVRAM!!!\n", __FUNCTION__, apIndex));
		return RETURN_ERR;
	}

	/* Check bss running status */
	int bss_status = hal_bss_status(apIndex);
	if (bss_status < 0) {
		HAL_WIFI_ERR(("%s: %d Error!!!\n", __FUNCTION__, __LINE__));
		return RETURN_ERR;
	}
...
INT wifi_setApIsolationEnable(INT apIndex, BOOL enable)
{
	...
	HAL_WIFI_LOG(("%s: apIndex = %d, radioIndex = %d, enable = %d\n", __FUNCTION__,
		apIndex, radioIndex, enable));
...
/* XXX skips internal broadcom ref. board specific ccodes, typically used for testing
 * does not include "#a", "E0", "Q1", "Q2"
 */
...
INT wifi_getApSecurityPreSharedKey(INT apIndex, CHAR *output_string)
{
	...
	snprintf(output_string, OUTPUT_STRING_LENGTH_64, "%s", preSharedKey);
	return RETURN_OK;
}
...
INT wifi_getApWpsConfigMethodsSupported(INT apIndex, CHAR *output)
{
	...
	HAL_WIFI_DBG(("%s: Enter, apIndex %d\n", __FUNCTION__, apIndex));
	output[0] = '\0';
	returnStatus = wldm_AccessPoint_WPS_ConfigMethodsSupported(CMD_GET, apIndex, output, &len, NULL, NULL);
	if (returnStatus != RETURN_OK) {
		HAL_WIFI_DBG(("%s: wldm_AccessPoint_WPS_ConfigMethodsSupported CND_GET Failed, Status = %d\n",
			__FUNCTION__, returnStatus));
		return RETURN_ERR;
	}
	HAL_WIFI_DBG(("%s: apIndex = %d, output = %s\n", __FUNCTION__, apIndex, output));
...
INT wifi_setApWpsConfigMethodsEnabled(INT apIndex, CHAR *methodString)
{
	...
	ret = wldm_AccessPoint_WPS_ConfigMethodsEnabled(CMD_SET, apIndex, methodString, &len, NULL, NULL);
	if (ret < 0) {
...
static void *wifi_wpsPairingCompletionCheck_priv(void *arg)
{
	...
	char buffer[BUF_LEN_26];
	FILE *wifihealth_fp = fopen("/rdklogs/logs/wifihealth.txt", "a+");

	do
	{
		pValue = wlcsm_nvram_get("wps_proc_status");
		if (!pValue) {
...
#endif
			if (wifihealth_fp) {
				fprintf(wifihealth_fp, "%s RDKB_WPS_%d FAILED\n", buffer, input_arg+1);
			}
			goto exit;
		}
		...
		if (status == HAL_WPS_OK) {
			...
			pValue = wlcsm_nvram_get("wps_ifname");
			...
			print_time(buffer, sizeof(buffer));
			if (wifihealth_fp) {
				fprintf(wifihealth_fp, "%s RDKB_WPS_%d SUCCESS\n", buffer, apIndex + 1);
			}
#if defined(_XB7_PRODUCT_REQ_) || defined(_XB8_PRODUCT_REQ_)
	...
	if (timeCount == 0) {
		print_time(buffer, sizeof(buffer));
		if (wifihealth_fp) {
			fprintf(wifihealth_fp, "%s RDKB_WPS_%d TIMEOUT\n", buffer, input_arg+1);
		}
	}
exit:
#if defined(_XB7_PRODUCT_REQ_) || defined(_XB8_PRODUCT_REQ_)
...
INT wifi_setApWpsEnrolleePin(INT apIndex, CHAR *pin)
{
	...
	print_time(buffer, sizeof(buffer));
	if (wifihealth_fp) {
		fprintf(wifihealth_fp, "%s RDKB_WPS_PIN_%d START\n", buffer, apIndex+1);
		fflush(wifihealth_fp);
		fclose(wifihealth_fp);
	}
...
print_err:
	...
	wifihealth_fp=fopen("/rdklogs/logs/wifihealth.txt", "a+");
	if (wifihealth_fp) {
		fprintf(wifihealth_fp, "%s RDKB_WPS_PIN_%d FAILED\n", buffer, apIndex+1);
		fflush(wifihealth_fp);
		fclose(wifihealth_fp);
	}
	return RETURN_ERR;
...
INT wifi_setApWpsButtonPush(INT apIndex)
{
	...
	print_time(buffer, sizeof(buffer));
	if (wifihealth_fp) {
		fprintf(wifihealth_fp, "%s RDKB_WPS_CLI PBC_%d START\n", buffer, apIndex+1);
		fflush(wifihealth_fp);
		fclose(wifihealth_fp);
	}
...
print_err:
	...
	wifihealth_fp=fopen("/rdklogs/logs/wifihealth.txt", "a+");
	if(wifihealth_fp) {
		fprintf(wifihealth_fp, "%s RDKB_WPS_PBC_%d FAILED\n", buffer, apIndex+1);
		fflush(wifihealth_fp);
		fclose(wifihealth_fp);
	}
	return RETURN_ERR;
...
INT wifi_cancelApWPS(INT apIndex)
{
	...
	print_time(buffer, sizeof(buffer));
	if (wifihealth_fp) {
		fprintf(wifihealth_fp, "%s RDKB_WPS_CANCEL_%d START\n", buffer, apIndex+1);
		fflush(wifihealth_fp);
		fclose(wifihealth_fp);
	}
...
print_err:
	...
	wifihealth_fp=fopen("/rdklogs/logs/wifihealth.txt", "a+");
	if (wifihealth_fp) {
		fprintf(wifihealth_fp, "%s RDKB_WPS_CANCEL_%d FAILED\n", buffer, apIndex + 1);
		fflush(wifihealth_fp);
		fclose(wifihealth_fp);
	}
	return RETURN_ERR;
...
INT wifi_getApAssociatedDeviceDiagnosticResult(INT apIndex, wifi_associated_dev_t **associated_dev_array, UINT *output_array_size)
{
	...
	*associated_dev_array = parray;
	for (i = 0; i < len; ++i) {
		...
		strncpy(parray[i].cli_OperatingStandard, p_wldm_array[i].cli_OperatingStandard,
			sizeof(parray[i].cli_OperatingStandard));
		strncpy(parray[i].cli_OperatingChannelBandwidth,
			p_wldm_array[i].cli_OperatingChannelBandwidth,
			sizeof(parray[i].cli_OperatingChannelBandwidth));
...
static INT get_assoc_dev_max_tx_rx_rate(INT apIndex, wifi_associated_dev3_t *dev)
{
	char guard[OUTPUT_STRING_LENGTH_64], *interface,
		cmd[OUTPUT_STRING_LENGTH_256] = {0};
	int gi = 1, i, tbl_size, max_mcs = 0;
	unsigned int nss = 4, temp_nss = 0;
	FILE *fp;
	...
	if (strcmp(dev->cli_OperatingStandard, "a") == 0) {
		...
		interface = wldm_get_nvifname(apIndex);
		...
		if (strcmp(dev->cli_OperatingStandard, "n") == 0) {
			snprintf(cmd, sizeof(cmd), "wl -i %s sta_info %02x:%02x:%02x:%02x:%02x:%02x | \
				awk '/MCS SET/ {print $(NF-1)}'",
				interface, dev->cli_MACAddress[0], dev->cli_MACAddress[1],
				dev->cli_MACAddress[2], dev->cli_MACAddress[3],
				dev->cli_MACAddress[4], dev->cli_MACAddress[5]);

			fp = popen(cmd,"r");
			if (fp != NULL) {
				if (fscanf(fp, "%d", &max_mcs) != 1) {
					HAL_WIFI_ERR(("MCS not read correctly\n"));
					max_mcs = 31;
				}
				pclose(fp);
				if (max_mcs == 31)
				...
				HAL_WIFI_DBG(("%s:unable to do popen in n mode\n",__FUNCTION__));
			}

		} else if ((strcmp(dev->cli_OperatingStandard, "ac") == 0) ||
			(strcmp(dev->cli_OperatingStandard, "ax") == 0)) {
			snprintf(cmd, sizeof(cmd), "wl -i %s sta_info %x:%x:%x:%x:%x:%x | \
				awk -F\"Nss\" '/Nss/{print $2}' | awk -F' ' '{print $1}'",
				interface, dev->cli_MACAddress[0], dev->cli_MACAddress[1],
				dev->cli_MACAddress[2], dev->cli_MACAddress[3],
				dev->cli_MACAddress[4], dev->cli_MACAddress[5]);
			...
			if (fp != NULL) {
				while (1) {
					/* Calculate max nss for rx and tx */
					if (fscanf(fp, "%d", &nss) == 1) {
						temp_nss = (nss > temp_nss) ? nss : temp_nss;
					} else {
						break;
					}
				}
				...
				HAL_WIFI_DBG(("%s:Unable to do popen in ac mode\n",__FUNCTION__));
			}
			nss = (temp_nss > 0) ? temp_nss : 8;
		} else {
			HAL_WIFI_ERR(("%s: Invalid Operating Standard %s\n", __FUNCTION__,
				dev->cli_OperatingStandard));
			return RETURN_ERR;
		}
		HAL_WIFI_DBG(("%s: Operating Standard: %s Operating bandwidth: %s guard: %s = %d NSS:%d MAC %02x:%02x:%02x:%02x:%02x:%02x \n",
			__FUNCTION__, dev->cli_OperatingStandard,
			dev->cli_OperatingChannelBandwidth, guard, gi, nss,
			dev->cli_MACAddress[0], dev->cli_MACAddress[1],
			dev->cli_MACAddress[2], dev->cli_MACAddress[3],
			dev->cli_MACAddress[4], dev->cli_MACAddress[5]));
	...
	wifi_associated_dev3_t **associated_dev_array, UINT *output_array_size)
{
	...
	/* To be clear. By HAL defintion ( struct wifi_associated_dev3_t in wifi_hal.h)
	 * (0) if output_array_size=0,  alloc memory for associated_dev_array,  fill w/ STAs info
	 * (1) if output_array_siz <>0, STAs MAC in associated_dev_array; fetch csi data and STAs info
	 */
	unsigned char flag = 0; /* flag to distinguish the 2 cases. default is case 0 */

	AP_INDEX_ASSERT(apIndex);
	radioIndex = HAL_AP_IDX_TO_HAL_RADIO(apIndex);

#if WIFI_HAL_VERSION_GE_2_19
	flag = (*output_array_size == 0) ? 0 : 1;
#endif /* (WIFI_HAL_VERSION_GE_2_19) */
	...
	if (assoc_num == 0) {
		HAL_WIFI_DBG(("%s apIndex=%d, numberOfAssociatedDevices=0\n", __FUNCTION__,
			apIndex));
		if (flag == 0) {
			/* case 0: return OK to keep consistent w/ original implementation */
			return RETURN_OK;
		} else {
			/* case 1: return fails, no valid csi data to return */
			return RETURN_ERR;
		}
	}
...
#if WIFI_HAL_VERSION_GE_2_19
	HAL_WIFI_DBG(("%s array_size:%d case:%d assoc_cnt:%d\n", __FUNCTION__,
		*output_array_size, flag, assoc_num));

	/* collect CSI data when output_array_size != 0 */
	if (flag == 1) {
		HAL_WIFI_DBG(("fetch CSI data: array_size:%d\n", *output_array_size));
		parray = *associated_dev_array;
		for (i = 0; i < *output_array_size; i++, parray++) {
			HAL_WIFI_DBG(("%s: wl%d read CSI MAC"MACF"\n", __FUNCTION__,
				apIndex, MAC_TO_MACF(parray->cli_MACAddress)));

			/* malloc buffer for csi data. This buffer is free by upper layer apps  */
			parray->cli_CsiData = (wifi_csi_data_t *)malloc(sizeof(wifi_csi_data_t));
			if (parray->cli_CsiData == NULL) {
				HAL_WIFI_ERR(("%s, malloc cli_CsiData fails[i:%d]\n", __FUNCTION__, i));
				break;
			}

			ret = hal_csi_rec_read(radioIndex, parray->cli_MACAddress,
				(char *)(parray->cli_CsiData));

			if (ret != RETURN_OK) {
				HAL_WIFI_ERR(("hal_csi_rec_read fails. radioIndex:%d\n", radioIndex));
				free(parray->cli_CsiData);
				parray->cli_CsiData = NULL;
			}
		}
		parray = *associated_dev_array;
	} else
	...
	HAL_WIFI_DBG(("%s flag=%d size wifi_associated_dev3_t=%d wldm_wifi_associated_dev3_t=%d\n",
		__FUNCTION__, flag, sizeof(wifi_associated_dev3_t),
		sizeof(wldm_wifi_associated_dev3_t)));

	for (i = 0; i < len; ++i) {
#if WIFI_HAL_VERSION_GE_2_19
		if (flag) {
			/* find if this STA is included in the associated_dev_array */
			for (idx = 0; idx < *output_array_size; idx++) {
				if (!memcmp(parray[idx].cli_MACAddress,
					p_wldm_array[i].cli_MACAddress, WIFI_MAC_ADDRESS_LENGTH)) {
					loc = idx;
					HAL_WIFI_LOG(("found loc:%d\n", loc));
					break;
				}
			}
			if (idx >= *output_array_size) {
				HAL_WIFI_DBG(("Not Found. skipped\n"));
				continue;
			}
		} else
#endif /* WIFI_HAL_VERSION_GE_2_19 */
		{
			loc = i;
			memcpy(parray[loc].cli_MACAddress, p_wldm_array[i].cli_MACAddress, ETHER_ADDR_LEN);
		}

		parray[loc].cli_AuthenticationState = p_wldm_array[i].cli_AuthenticationState;
		parray[loc].cli_LastDataDownlinkRate = p_wldm_array[i].cli_LastDataDownlinkRate;
		parray[loc].cli_LastDataUplinkRate = p_wldm_array[i].cli_LastDataUplinkRate;
		parray[loc].cli_SignalStrength = p_wldm_array[i].cli_SignalStrength;
		parray[loc].cli_Retransmissions = p_wldm_array[i].cli_Retransmissions;
		parray[loc].cli_Active = p_wldm_array[i].cli_Active;
		strncpy(parray[loc].cli_OperatingStandard, p_wldm_array[i].cli_OperatingStandard,
			sizeof(parray[loc].cli_OperatingStandard));
		strncpy(parray[loc].cli_OperatingChannelBandwidth,
			p_wldm_array[i].cli_OperatingChannelBandwidth,
			sizeof(parray[loc].cli_OperatingChannelBandwidth));
		parray[loc].cli_SNR = p_wldm_array[i].cli_SNR;
		parray[loc].cli_BytesSent = p_wldm_array[i].cli_BytesSent;
		parray[loc].cli_BytesReceived = p_wldm_array[i].cli_BytesReceived;
		parray[loc].cli_RSSI = p_wldm_array[i].cli_RSSI;
		parray[loc].cli_DataFramesSentAck = p_wldm_array[i].cli_DataFramesSentAck;
		parray[loc].cli_DataFramesSentNoAck = p_wldm_array[i].cli_DataFramesSentNoAck;
		parray[loc].cli_Associations = p_wldm_array[i].cli_Associations;
		parray[loc].cli_PacketsSent = p_wldm_array[i].cli_PacketsSent;
		parray[loc].cli_PacketsReceived = p_wldm_array[i].cli_PacketsReceived;
		parray[loc].cli_ErrorsSent = p_wldm_array[i].cli_ErrorsSent;
		parray[loc].cli_RetransCount = p_wldm_array[i].cli_RetransCount;
		parray[loc].cli_FailedRetransCount = p_wldm_array[i].cli_FailedRetransCount;
		parray[loc].cli_RetryCount = p_wldm_array[i].cli_RetryCount;
		get_assoc_dev_max_tx_rx_rate(apIndex, &parray[loc]);
#ifdef WIFI_HAL_VERSION_3
		/* TBD cli_DownlinkMuStats cli_UplinkMuStats */
		/* cli_TwtParams */
		parray[loc].cli_TwtParams.numTwtSession = p_wldm_array[i].cli_TwtParams.numTwtSession;
		memcpy((char *)(parray[loc].cli_TwtParams.twtParams),
			(char *)(p_wldm_array[i].cli_TwtParams.twtParams),
			(parray[loc].cli_TwtParams.numTwtSession) * sizeof(wifi_twt_params_t));
...
INT wifi_setLED_priv(INT radioIndex, wifi_led_blink_state_t led_state) //RDKB
{
	...
	if (wifi_led_gpio == -1) {
		sprintf(file_name, "/nvram/provisioned_wifi_wl%d_vars.txt", radioIndex);
		wifi_led_gpio = -2;

		if (!(fp = fopen(file_name, "r"))) {
			sprintf(file_name, "/data/provisioned_wifi_wl%d_vars.txt", radioIndex);
			fp = fopen(file_name, "r");
		}
		if (fp) {
			while (fgets(line, sizeof(line), fp)!=NULL) {
				if (sscanf(line,"ledbh%d%s",&wifi_led_gpio,tmpStr)) {
					if (strstr(tmpStr,"=0x")) {
						break;
					} else {
						wifi_led_gpio = -2;
	...
	sprintf(cmd, "wl -i wl%d ledbh %d %d", ((radioIndex == 0) ? 0 : 1), wifi_led_gpio, ((wifi_driver_led_state == 0) ? 21 : 1));
...
{
	int len = sizeof(*output_percentage);
...
{
	char *mac_str_array = NULL, *mac_str, *rest;
	int len, i;
...
INT wifi_getApAssociatedDevice(INT ap_index, CHAR *output_buf, INT output_buf_size)
{
	...
	if (wldm_xbrcm_ap(CMD_GET, ap_index, output_buf, &output_buf_size, "assoclist", NULL)
		== 0) {
...
{
	int len = sizeof(wldm_wifi_associated_dev_stats_t) + sizeof(ULLONG), i;
	...
	HAL_WIFI_ERR(("%s, apIndex = %d, DeviceMacAddress = %s\n", __FUNCTION__, apIndex, mac_address));
	...
	if (wldm_xbrcm_sta(CMD_GET, apIndex, p, &len, "assoc_dev_stats", NULL) == 0) {
		...
		free(p);
		HAL_WIFI_ERR(("%s, apIndex = %d, handle = %llu\n", __FUNCTION__, apIndex, *handle));
		return RETURN_OK;
	}
...
{
	HAL_WIFI_LOG(("%s: AP Index %d, beacodRate %s\n", __FUNCTION__, apIndex, input_string ));
...
INT wifi_getApBeaconRate(INT apIndex, CHAR *output_string)
{
	...
	strcpy(output_string, beaconrate);
	return RETURN_OK;
}
...
INT wifi_getBandSteeringLog(INT record_index, ULONG *pSteeringTime, CHAR *pClientMAC, INT *pSourceSSIDIndex, INT *pDestSSIDIndex, INT *pSteeringReason) //if no steering or redord_index is out of boundary, return -1. pSteeringTime returns the UTC time in seconds. pClientMAC is pre allocated as 64bytes. pSteeringReason returns the predefined steering trigger reason
{
	...
	*/
	str = wlcsm_nvram_get(WLAN_CM_RDK_BSD_LOG_FILE_NVRAM);
...
{
	int len = sizeof(*output_timout_sec);
...
INT wifi_getRadioDcsChannelMetrics(INT radioIndex,
	...
	INT array_size)
{
	int num_channels, acnt, prnCh, ch_rssiCnt, i, k;
	wifi_channelMetrics_t *wifi_cmp, *aPtr;
	wifi_apRssi_t *wifi_cm_apRssip;
	unsigned int num_res, len, macInt[6], chList[MAX_NUMCHANNELS] = {0};
	...
	for (i = 0; i < num_res; ++i) {
		...
		if (prnCh != nbp->ap_Channel) {
			...
			for (k = 0; k < num_channels; k++) {
				csp = &(ch_stats[k]);
				if (csp->ch_number == nbp->ap_Channel) {
					prnCh = csp->ch_number;
					wifi_cmp = &(input_output_channelMetrics_array[k]);
					break;
				}
...
INT wifi_pushRadioChannel2(INT radioIndex, UINT channel, UINT channel_width_MHz, UINT csa_beacon_count)
{
	...
	len = sizeof(csaInfo);
	ret = wldm_11h_dfs(CMD_SET_IOCTL, radioIndex, &csaInfo, &len, "csa_bcast", NULL);
	if (ret != 0) {
...
INT wifi_getRadioChannelStats(INT radioIndex, wifi_channelStats_t *input_output_channelStats_array, INT array_size)
{
	...
	wldm_channel_stats_t *chan_stats;

	HAL_WIFI_LOG(("%s, radioIndex = %d\n", __FUNCTION__, radioIndex));
	if (NULL == input_output_channelStats_array) {
		HAL_WIFI_ERR(("%s, input_output_channelStats_array parameter error!!!\n",
			__FUNCTION__));
		return RETURN_ERR;
	}
	chan_stats = (wldm_channel_stats_t *)malloc(array_size * sizeof(wldm_channel_stats_t));
	if (chan_stats == NULL) {
		HAL_WIFI_ERR(("%s: chan_stats malloc failed\n", __FUNCTION__));
		return RETURN_ERR;
	}
	memset(chan_stats, 0, array_size * sizeof(wldm_channel_stats_t));
	for (i = 0; i < array_size; ++i) {
		chan_stats[i].ch_number = input_output_channelStats_array[i].ch_number;
		chan_stats[i].ch_in_pool = input_output_channelStats_array[i].ch_in_pool;
	}
	if (wldm_xbrcm_lq(CMD_GET, radioIndex, (void *)chan_stats, &len,
		"channel_stats", NULL) != 0) {
		free(chan_stats);
		HAL_WIFI_ERR(("%s: channel_stats CMD_GET returned error\n", __FUNCTION__));
		return RETURN_ERR;
	}
	for (i = 0; i < array_size; ++i) {
		input_output_channelStats_array[i].ch_number = chan_stats[i].ch_number;
		input_output_channelStats_array[i].ch_in_pool = chan_stats[i].ch_in_pool;
		input_output_channelStats_array[i].ch_noise = chan_stats[i].ch_noise;
		input_output_channelStats_array[i].ch_radar_noise = chan_stats[i].ch_radar_noise;
		input_output_channelStats_array[i].ch_max_80211_rssi =
			chan_stats[i].ch_max_80211_rssi;
		input_output_channelStats_array[i].ch_non_80211_noise =
			chan_stats[i].ch_non_80211_noise;
		input_output_channelStats_array[i].ch_utilization = chan_stats[i].ch_utilization;
		input_output_channelStats_array[i].ch_utilization_total =
			chan_stats[i].ch_utilization_total;
		input_output_channelStats_array[i].ch_utilization_busy =
			chan_stats[i].ch_utilization_busy;
		input_output_channelStats_array[i].ch_utilization_busy_tx =
			chan_stats[i].ch_utilization_busy_tx;
		input_output_channelStats_array[i].ch_utilization_busy_rx =
			chan_stats[i].ch_utilization_busy_rx;
		input_output_channelStats_array[i].ch_utilization_busy_self =
			chan_stats[i].ch_utilization_busy_self;
		input_output_channelStats_array[i].ch_utilization_busy_ext =
			chan_stats[i].ch_utilization_busy_ext;
	}
	free(chan_stats);
	return RETURN_OK;
}
...
{
	int len = sizeof(wifi_channelStats2_t);
...
INT wifi_setApCsaDeauth(INT apIndex, INT mode)
{
	...
	if (wlcsm_nvram_set(nvName, deauth_modeStr)) {
...
{
	UCHAR buf[MAX_EVENT_BUFFER_LEN], *buf_ptr;
	wifi_hal_message_t *ptr;
...
#define TEMP_STA_ASSOC_COUNT	"/tmp/temp_sta_assoc_count"

/* This is a local function to store the number of times a STA was associated
 * /tmp/sta_assoc_count will be filled up with the index, sub index,
 * mac and the number of times the STA was associated.
 * This is needed for filling up wifi_associated_dev2_t and wifi_associated_dev3_t */
static int
update_sta_assoc_count(int idx, int bssidx, char *mac)
{
	FILE *fp, *fp_temp;
	int fidx, fbssidx;
	unsigned long long fcount = 0;
	char fmac[MAC_STR_LEN], line[OUTPUT_STRING_LENGTH_128];
	int found_match = 0;
	fp = fopen(STA_ASSOC_COUNT, "r");

	fp_temp = fopen(TEMP_STA_ASSOC_COUNT, "w");

	if (fp_temp == NULL) {
		HAL_WIFI_ERR(("Unable to open %s\n", TEMP_STA_ASSOC_COUNT));
		if (fp) {
			fclose(fp);
		}
		return RETURN_ERR;
	...
	if (fp) {
		while (fgets(line, sizeof(line), fp) != NULL) {
			if (sscanf(line, "%d %d %s %llu\n", &fidx, &fbssidx, fmac, &fcount) == 4) {
				if ((idx == fidx) && (bssidx == fbssidx) && !(strncasecmp(mac, fmac, sizeof(fmac)))) {
					found_match = 1;
					fprintf(fp_temp, "%d %d %s %llu\n", fidx, fbssidx, fmac, ++fcount);
				} else {
					fprintf(fp_temp, "%d %d %s %llu\n", fidx, fbssidx, fmac, fcount);
				}
			}
	...
	if (!found_match) {
		fprintf(fp_temp, "%d %d %s 1\n", idx, bssidx, mac);
	}
	fclose(fp_temp);
	if (remove(STA_ASSOC_COUNT) != 0) {
		HAL_WIFI_ERR(("%s: Fail to remove %s\n", __FUNCTION__, STA_ASSOC_COUNT));
		return RETURN_ERR;
	}
	if (rename(TEMP_STA_ASSOC_COUNT, STA_ASSOC_COUNT) != 0) {
		HAL_WIFI_ERR(("%s: Fail to rename %s to %s\n",
			__FUNCTION__, TEMP_STA_ASSOC_COUNT, STA_ASSOC_COUNT));
		return RETURN_ERR;
	}
	return RETURN_OK;
}
		...
		while (bytes >= len_disassoc) {
			...
			if ((event_type == WLC_E_ASSOC_IND) || (event_type == WLC_E_REASSOC_IND)) {
				if (bytes >= len_assoc) {
					...
					if (update_sta_assoc_count(idx, sub_idx, macStr)) {
						HAL_WIFI_ERR(("%s: update_sta_assoc_count failed\n", __FUNCTION__));
					}
					if ((wifi_numRegisteredAssociatedCallbacks != 0) &&
				...
				HAL_WIFI_ERR(("%s:%d: Invalid STA event[%d]!\n", __FUNCTION__, __LINE__, event_type));
				break;
			}
		} /* while */
	}
	...
	if ((bytes = recv(sock_fd, ptr, MAX_EVENT_BUFFER_LEN, MSG_NOSIGNAL)) > 0) {
		...
		if (dpkt->version == WIFI_HAL_EVT_VERSION) {
			...
			switch (event_id) {
				...
				default:
					HAL_WIFI_ERR(("%s:%d: Unsupported Event: %d \n", __FUNCTION__, __LINE__, event_id));
					break;
			}
		}
	}
		...
		if (dpkt->version == WIFI_HAL_EVT_VERSION) {
			...
			switch (event_id) {
				case WIFI_HAL_CB_RRM_BCNREP:
					measRep = (wifi_MeasurementReportElement_t *)(dpkt->data);
					// HAL_DBG_PRINT_BUF("WIFI_HAL_CB_RRM_BCNREP", dpkt, 144);
					HAL_WIFI_DBG(("%s: Received Report apIndex=%d rep_Id=0x%x"
						" rep_Type=0x%x \n", __FUNCTION__, dpkt->apIndex,
						measRep->rep_Id, measRep->rep_Type));
					if ((measRep->rep_Id == DOT11_MNG_MEASURE_REPORT_ID) &&
						(measRep->rep_Type == DOT11_MEASURE_TYPE_BEACON)) {

						if (measRep->rep_Len <
							sizeof(wifi_BeaconMeasurement_packed_t)) {
							HAL_WIFI_ERR(("%s: apIdx=%d Received Beacon"
						...
						out_array_size =  sizeof(bcnRep);
						out_DialogToken = measRep->rep_Token;
						HAL_WIFI_DBG(("%s: Received Report apIndex=%d "
							"out_array_size=%d out_DialogToken=%d\n",
							__FUNCTION__, dpkt->apIndex,
							out_array_size, out_DialogToken));
						if (callback_fnc.beaconReport_cb != NULL) {
							callback_fnc.beaconReport_cb(dpkt->apIndex,
								(wifi_BeaconReport_t *)(&bcnRep),
								&out_array_size, &out_DialogToken);
						}
					}
					else {
...
wifi_hal_cb_bsstrans_evt_handler (int sock_fd) {
	...
	if ((bytes = recv(sock_fd, ptr, MAX_EVENT_BUFFER_LEN, MSG_NOSIGNAL)) > 0) {
		...
		if (dpkt->version == WIFI_HAL_EVT_VERSION) {
			...
			ptr += 2; /* new structure skip cat/action */
			...
			switch (subtype) {
				case WIFI_HAL_CB_BSSTRANS_RESP:
					...
					if (callback_fnc.btm_resp_cb) {
						btm_resp = (wifi_BTMResponse_t *)ptr; /* tmp, todo */
#ifdef WIFI_HAL_VERSION_3
						callback_fnc.btm_resp_cb(dpkt->apIndex, mac_address, btm_resp);
#else
...
#ifdef WIFI_HAL_VERSION_3
						callback_fnc.btm_query_req_cb(dpkt->apIndex, mac_address, btm_query, 0, btm_req); /* tmp, todo */
#else
...
//INT wifi_apAuthEvent_callback_register(INT radioIndex)
	...
	wifi_NeighborReport_t *neighborReports)
{
	...
	for (i = 0; i < numNeighborReports; i++) {
		in_rep = (wifi_NeighborReport_t *) ((UCHAR *)neighborReports + (i * sizeof(wifi_NeighborReport_t)));
		rep_len = DOT11_NEIGHBOR_REP_IE_FIXED_LEN; /* can be different when optional elements come in */
		...
		/* to next report */
		nbr_rep = (dot11_neighbor_rep_ie_t *)((UCHAR *)(nbr_rep) + (rep_len + TLV_HDR_LEN));
	}

	HAL_WIFI_DBG(("%s: Do setNeighborReports Index=[%d]\n", __FUNCTION__, apIndex));
...
INT wifi_mgmt_frame_callbacks_register(wifi_receivedMgmtFrame_callback dppRecvRxCallback)
{
	...
	wlcsm_nvram_set(NVNM_HS2_ANQP_HAL, "1");
	...
	UINT len)
{
	...
	ret = wl_sendActionFrame(apIndex, sta, NULL, channel, frame, len);
...
static int wifi_hs_oui2str(wifi_roamingConsortiumElement_t *infoElement, char *oui_str)
{
	char byte_str[3];
	int i, j, len;
	UCHAR count, *phex;
	...
	for (i = 0; i < count; i++) {
		...
		for (j = 0; j < len; j++) {
			sprintf(byte_str, "%02x", phex[j]);
			strcat(oui_str, byte_str);
		}
		strcat(oui_str, ":1");
	}
	return RETURN_OK;
...
static int wifi_hs_str2oui(wifi_roamingConsortiumElement_t *infoElement, char *oui_str)
{
	...
	while (ptr) {
		...
		if ((len < 15) && (total < 3)) {
			get_hex_data(ptr, infoElement->wifiRoamingConsortiumOui[total], len);
...
static int wifi_setApRoamingConsortiumElement(INT apIndex, wifi_roamingConsortiumElement_t *infoElement)
{
	...
	if (wifi_hs_oui2str(infoElement, ptr) != RETURN_OK) {
...
INT wifi_getApAssociatedClientDiagnosticResult(INT apIndex, mac_address_t mac_addr, wifi_associated_dev3_t *dev_conn)
{
	...
	if (wifi_getApAssociatedDeviceDiagnosticResult3(apIndex, &associated_dev_array, &array_size)) {
		HAL_WIFI_ERR(("%s: wifi_getApAssociatedDeviceDiagnosticResult3 returning error\n", __FUNCTION__));
		return WIFI_HAL_ERROR;
	}
...
static int wifi_api_socket_init(void)
{
	...
	if (wifi_api_socket < 0) {
		...
		if ((err = bind(wifi_api_socket, (struct sockaddr *)&sockaddr, sizeof(sockaddr))) < 0) {
			HAL_WIFI_DBG(("%s@%d Unable to bind to loopback socket %d\n", __FUNCTION__, __LINE__, wifi_api_socket));
			err = -1;
		}

		if (err < 0  && wifi_api_socket >= 0) {
			HAL_WIFI_DBG(("%s@%d: failure. Close socket\n", __FUNCTION__, __LINE__ ));
...
void wifi_api_thread_main_loop( void )
{
	...
	int sock_len = sizeof(struct sockaddr_in), fdmax = -1, bytes, len = 4096;
	...
	while (1) {
		...
		if (FD_ISSET(wifi_api_socket, &fdset)) {
			bytes = recvfrom(wifi_api_socket, pkt, len, 0, (struct sockaddr *)&from, (socklen_t *)&sock_len);
			...
			if (!strcmp(pApiInfo->api_name, "wifi_setApEnable")) {
				...
				wifi_setApEnable(pApiInfo->radioIndex, enable);
	...
	!defined(_XF3_PRODUCT_REQ_))
				...
				HAL_WIFI_DBG(("%s: wifi_setApSecurityRadiusSettings %d %d %d %d %d %d %d %d %d\n",
					...
					settings.IdentityRequestRetryInterval,
					settings.QuietPeriodAfterFailedAuthentication));
				wifi_setApSecurityRadiusSettings(pApiInfo->radioIndex, &settings);
			} else if (!strcmp(pApiInfo->api_name, "wifi_applyRadioSettings")) {
				wifi_applyRadioSettings(pApiInfo->radioIndex);
			} else if (!strcmp(pApiInfo->api_name, "wifi_applySSIDSettings")) {
				wifi_applySSIDSettings(pApiInfo->radioIndex);
			} else if (!strcmp(pApiInfo->api_name, "wifi_removeApSecVaribles")) {
				wifi_removeApSecVaribles(pApiInfo->radioIndex);
			} else if (!strcmp(pApiInfo->api_name, "wifi_disableApEncryption")) {
				wifi_disableApEncryption(pApiInfo->radioIndex);
	...
	(!defined(_CBR_PRODUCT_REQ_) && !defined(_XF3_PRODUCT_REQ_)) */
				...
				if (!strcmp(essid, "")) {
					wifi_setApScanFilter(pApiInfo->radioIndex, mode, NULL);
				} else {
					wifi_setApScanFilter(pApiInfo->radioIndex, mode, essid);
				}
				...
				wifi_pushApInterworkingElement(pApiInfo->radioIndex, &infoElement);
				...
				wifi_setNeighborReportActivation(pApiInfo->radioIndex, activate);
	...
	(!defined(_CBR_PRODUCT_REQ_) && !defined(_XF3_PRODUCT_REQ_)) */
				...
				wifi_enableGreylistAccessControl(enable ? TRUE : FALSE);
			} else if (!strcmp(pApiInfo->api_name, "wifi_setApDASRadiusServer")) {
				char ipAddress[64], secret[64] = {'\0'};
				int port;

				sscanf(pApiInfo->api_data, "%s %d %s\n", ipAddress, &port, secret);
				wifi_setApDASRadiusServer(pApiInfo->radioIndex,
				...
				wifi_setApWpsConfiguration(pApiInfo->radioIndex, &wpsConfig);
				...
				wifi_setRadioOperatingParameters(pApiInfo->radioIndex, &operationParam);
				...
				wifi_setApSecurity(pApiInfo->radioIndex, &security);
			} else if (!strcmp(pApiInfo->api_name, "wifi_setBSSColor")) {
				wifi_setBSSColor(pApiInfo->radioIndex, atoi(pApiInfo->api_data));
			} else if (!strcmp(pApiInfo->api_name, "wifi_setMuEdca")) {
				wifi_edca_t edca;
				wifi_access_category_t ac;
				sscanf(pApiInfo->api_data, "%d %d %d %d %d\n", &ac, &edca.aifsn,
					&edca.cw_min, &edca.cw_max, &edca.timer);
				wifi_setMuEdca(pApiInfo->radioIndex, ac, edca);
			} else if (!strcmp(pApiInfo->api_name, "wifi_createVAP")) {
				wifi_vap_info_map_t map;
				int i, ret, apIndex, configType = atoi(pApiInfo->api_data);
				int radioIndex = pApiInfo->radioIndex;
				BOOL enabled = 1, showSsid = 1, isolation = 0;

				map.num_vaps = MAX_NUM_VAP_PER_RADIO;

				apIndex = HAL_RADIO_IDX_TO_HAL_AP(radioIndex);
				...
				wifi_createVAP(radioIndex, &map);
			} else if (!strcmp(pApiInfo->api_name, "wifi_setEAP_Param")) {
				char param[128];
				UINT value;

				sscanf(pApiInfo->api_data, "%u %s", &value, param);
				wifi_setEAP_Param(pApiInfo->radioIndex, value, param);
...
UINT wifi_getFTMobilityDomainID(INT apIndex, UCHAR mobilityDomain[2])
{
	...
	int len = 2;
	ret = wldm_11r_ft(WLDM_FT_GET_MobilityDomainID, apIndex, 0, &len, mobilityDomain);
	return ret;
}
...
#endif
	int len = 2;
	ret = wldm_11r_ft(WLDM_FT_SET_MobilityDomainID, apIndex, 0, &len, mobilityDomain);
	return ret;
}
...
UINT wifi_getFTR0KeyHolderID(INT apIndex, UCHAR *keyHolderID)
{
	UINT ret;
#endif
	ret = wldm_11r_ft(WLDM_FT_GET_R0KeyHolderID, apIndex, 0, NULL, keyHolderID);
	return ret;
}
...
UINT wifi_setFTR0KeyHolderID(INT apIndex, UCHAR *keyHolderID)
{
	UINT ret;
#endif
	ret = wldm_11r_ft(WLDM_FT_SET_R0KeyHolderID, apIndex, 0, NULL, keyHolderID);
	return ret;
}
...
UINT wifi_getFTR1KeyHolderID(INT apIndex, UCHAR *keyHolderID)
{
	UINT ret;
#endif
	ret = wldm_11r_ft(WLDM_FT_GET_R1KeyHolderID, apIndex, 0, NULL, keyHolderID);
	return ret;
}
...
UINT wifi_setFTR1KeyHolderID(INT apIndex, UCHAR *keyHolderID)
{
	UINT ret;
#endif
	ret = wldm_11r_ft(WLDM_FT_SET_R1KeyHolderID, apIndex, 0, NULL, keyHolderID);
	return ret;
}
...
INT wifi_getAvailableBSSColor(INT radio_index, INT maxNumberColors, UCHAR* colorList, INT *numColorReturned)
{
	...
	ret = wldm_AXavailableBssColors(CMD_GET, radio_index, colorList, &len, NULL, NULL);
...
INT wifi_setMuEdca(INT radio_index, wifi_access_category_t ac, wifi_edca_t edca)
{
	...
	wldm_edca.aifsn = edca.aifsn;
	wldm_edca.ecw_min = edca.cw_min;
	wldm_edca.ecw_max = edca.cw_max;
	wldm_edca.timer = edca.timer;

	ret = wldm_xbrcm_Radio_AXmuEdca(CMD_SET, radio_index, &wldm_edca, &len, NULL, NULL);
	if (ret < 0) {
		HAL_WIFI_ERR(("%s: Error wldm_xbrxm_Radio_AXmuEdca\n", __FUNCTION__));
		return WIFI_HAL_ERROR;
	}
...
static int wifi_setApInterworking(int apIndex, wifi_interworking_t *interworking)
{
	...
	ret = wifi_setApInterworkingElement(apIndex, &(interworking->interworking));
...
static int wifi_getApInterworking(int apIndex, wifi_interworking_t *interworking)
{
	...
	ret = wifi_getApInterworkingElement(apIndex, &(interworking->interworking));
	if (ret != WIFI_HAL_SUCCESS) {
...
INT wifi_getRadioVapInfoMap(wifi_radio_index_t index, wifi_vap_info_map_t *map)
{
	...
	for (i = 0; i < MAX_NUM_VAP_PER_RADIO; i++) {
		...
		ret = wldm_xbrcm_ap(CMD_GET_NVRAM, apIndex,
			(int *) &(map->vap_array[i].u.bss_info.mgmtPowerControl), &len,
		...
		ret = wldm_AccessPoint_MaxAssociatedDevices(CMD_GET_NVRAM, apIndex,
			(int *) &(map->vap_array[i].u.bss_info.bssMaxSta), &len, NULL, NULL);
		if (ret < 0) {
		...
		if (ret < WIFI_HAL_SUCCESS) {
			HAL_WIFI_ERR(("%s: wifi_getApWpsConfiguration failed for apIndex=[%s]\n",
				__FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		}
...
INT wifi_createVAP(wifi_radio_index_t index, wifi_vap_info_map_t *map)
{
	...
	for (i = 0; i < map->num_vaps; i++) {
		...
		ret = wldm_AccessPoint_Enable(CMD_SET, apIndex,
		...
		len = sizeof(map->vap_array[i].u.bss_info.ssid);
		ret = wldm_SSID_SSID(CMD_SET, apIndex,
		...
		ret = wldm_AccessPoint_SSIDAdvertisementEnabled(CMD_SET, apIndex,
			&(map->vap_array[i].u.bss_info.showSsid), NULL, NULL, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: wldm_AccessPoint_SSIDAdvertisementEnabled CMD_SET"
				" Failed, apIndex=%d, ret = %d\n", __FUNCTION__, apIndex, ret));
			result = WIFI_HAL_ERROR;
		}

		ret = wldm_AccessPoint_IsolationEnable(CMD_SET, apIndex,
		...
		ret = wldm_xbrcm_ap(CMD_SET_NVRAM, apIndex,
			&mgmtPowerControl, &len,
			"bcnprs_txpwr_offset", NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: wldm_xbrcm_ap CMD_SET_NVRAM for bcnprs_txpwr_offset"
				" for apIndex = %d failed\n", __FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		}

		if (map->vap_array[i].u.bss_info.bssMaxSta > 0) {
			ret = wldm_AccessPoint_MaxAssociatedDevices(CMD_SET, apIndex,
				(int *) &(map->vap_array[i].u.bss_info.bssMaxSta), &len, NULL, NULL);
		}
		if (ret < 0) {
		...
		len = DOT11_RRM_CAP_LEN;
		if (wldm_xbrcm_AccessPoint_RMCapabilities(CMD_GET_NVRAM, apIndex, rrm_val,
			&len, NULL, NULL) < 0) {
			HAL_WIFI_ERR(("%s: Error GET rmcap apIndex[%d]\n",
				__FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		...
		len = DOT11_RRM_CAP_LEN;
		ret = wldm_xbrcm_AccessPoint_RMCapabilities(CMD_SET, apIndex, rrm_val, &len, NULL, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: Error setting NeighborReportActivation apIndex=[%d]\n",
				__FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		...
		if (ret != WIFI_HAL_SUCCESS) {
			HAL_WIFI_ERR(("%s: wifi_updateApSecurity failed, apIndex=[%d]\n",
				__FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		}

		ret = wifi_setApInterworking(apIndex, &(map->vap_array[i].u.bss_info.interworking));
		if (ret != WIFI_HAL_SUCCESS) {
			HAL_WIFI_ERR(("%s: wifi_setApInterworking failed, apIndex=[%d]\n",
				__FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		}
		...
		ret = wldm_AccessPoint_MACAddressControMode(CMD_SET, apIndex,
		...
		if (ret != WIFI_HAL_SUCCESS) {
			HAL_WIFI_ERR(("%s: wifi_setApWpsConfiguration failed for apIndex=[%s]\n",
				__FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		}

		ret = wldm_AccessPoint_WMMEnable(CMD_SET, apIndex,
			&(map->vap_array[i].u.bss_info.wmm_enabled), NULL, NULL, NULL);
		if (ret < 0) {
			HAL_WIFI_ERR(("%s: wldm_AccessPoint_WMMEnable CMD_SET failed for apIndex=[%d]\n",
				__FUNCTION__, apIndex));
			result = WIFI_HAL_ERROR;
		}
	}
...
INT wifi_getRadioChannels(INT radioIndex, wifi_channelMap_t *output_map, INT output_map_size)
{
	...
	ret = wldm_xplume_opensync(CMD_GET, radioIndex, (void *)wldm_channelMap_ptr, &numChannels,
...
#if WIFI_HAL_VERSION_GE_2_19
/* CSI fucntion */
/* Read CSI data and convert header format */
static int hal_csi_rec_read(unsigned int RadioIndex, char *mac, char *buf)
{
	...
	raw_buf = (char *)malloc(CSI_MAX_PAYLOAD);
	if (!raw_buf) {
		HAL_WIFI_ERR(("%s: Malloc [size%d] Failure\n", __FUNCTION__, CSI_MAX_PAYLOAD));
		return RETURN_ERR;
	}

	memset(raw_buf, 0, CSI_MAX_PAYLOAD);
	if (wl_csi_rec_read(RadioIndex, mac, raw_buf)) {
		HAL_WIFI_ERR(("%s: wl_csi_rec_read fails\n", __FUNCTION__));
		free(raw_buf);
		return RETURN_ERR;
	}

	/* convert bcm raw data to hal format */
	hdr = (bcm_csimon_hdr_t *)raw_buf;
	cfg = (bcm_csi_rpt_config_t *)(hdr + 1);
	HAL_WIFI_CSI_DBG(("%s Header:\n", __FUNCTION__));
	HAL_WIFI_CSI_DBG(("frameid:%x\n", hdr->data.format_id));
	HAL_WIFI_CSI_DBG(("Client-ea:"MACF"\n", MAC_TO_MACF(hdr->data.client_ea)));
	HAL_WIFI_CSI_DBG(("bss-ea:"MACF"\n", MAC_TO_MACF(hdr->data.bss_ea)));
	HAL_WIFI_CSI_DBG(("chaspec:%04X chan:%d\n", hdr->data.chanspec,
		wf_chspec_ctlchan(hdr->data.chanspec)));
	HAL_WIFI_CSI_DBG(("tx:%X\n", hdr->data.txstreams));
	HAL_WIFI_CSI_DBG(("rx:%X\n", hdr->data.rxstreams));
	HAL_WIFI_CSI_DBG(("report_ts:%x\n", hdr->data.report_ts));
	HAL_WIFI_CSI_DBG(("assoc_ts:%x\n", hdr->data.assoc_ts));
	HAL_WIFI_CSI_DBG(("rssi:%02x:%02x:%02x:%02x\n", hdr->data.rssi[0], hdr->data.rssi[1],
		hdr->data.rssi[2], hdr->data.rssi[3]));
	HAL_WIFI_CSI_DBG(("time_stamp:0x%llx\n", (ULLONG)hdr->data.report_ts));

	hal_csi_data = (wifi_csi_data_t *)buf;
	hal_frame_info = &(hal_csi_data->frame_info);

	HAL_WIFI_CSI_DBG(("%s Read sta_csi_rec[%d]:hal_csi_matrix[%p] hal_frame_info[%p] "
		"hdr[%p] cfg[%p] data[%p]\n",
		__FUNCTION__, RadioIndex, &(hal_csi_data->csi_matrix), (void *)hal_frame_info,
		(void *)hdr, (void *)(hdr + 1), (void *)(cfg +1)));
	...
	/* brcm csi only support 1 stream */
	hal_frame_info->Nc = 1;
	...
	hal_frame_info->time_stamp = (ULLONG)hdr->data.report_ts;
	for (n = 0; n < 4; n++) {
		hal_frame_info->nr_rssi[n] = hdr->data.rssi[n];
	}

	HAL_WIFI_CSI_DBG(("frame_info: phy_bw:%x cap_bw:%x Nr:%x Nc:%x num_sc:%x\n",
		hal_frame_info->phy_bw, hal_frame_info->cap_bw, hal_frame_info->Nr,
		hal_frame_info->Nc, hal_frame_info->num_sc));

	if (hal_frame_info->num_sc > MAX_SUB_CARRIERS) {
		HAL_WIFI_ERR(("%s:num_sc[%d] too big\n", __FUNCTION__, hal_frame_info->num_sc));
		free(raw_buf);
		return RETURN_ERR;
	}

	/* Brcm csi data format (32 bits):
	 Nr:1:  V0[0] V1[0] ... Vn[0]
...
INT wifi_enableCSIEngine(INT apIndex, mac_address_t sta, BOOL enable)
{
	...
	HAL_WIFI_LOG(("Start: %s wl:%d Mac:"MACF " enab:%d\n", __FUNCTION__,
		radioIndex, MAC_TO_MACF(sta), enable));

	if (ETHER_ISNULLADDR(sta)) {
		if (enable) {
			HAL_WIFI_ERR(("%s: failure\n", __FUNCTION__));
			return RETURN_ERR;
		}
	...
	if (!enable) {
		if (stop_csithread) {
			...
			HAL_WIFI_CSI_DBG(("stop csi thread. ret:%d\n", ret));
		}
	}
...
INT wifi_enableGreylistAccessControl(BOOL enable)
{
	...
	for (i = 0; i < ARRAY_SIZE(hotspot_index); i++) {
		...
		if (wldm_AccessPoint_Security_RadiusGreylist(CMD_SET,
			apIndex, &enable, NULL, NULL, NULL)) {
			HAL_WIFI_ERR(("%s: %d: wldm_AccessPoint_Security_RadiusGreylist CMD_SET failed\n",
				__FUNCTION__, apIndex));
			return RETURN_ERR;
		}
	}
...
wl_getRadioCapabilities(int radioIndex, wifi_radio_capabilities_t *rcap)
{
	...
	int i, j, ccnt, len, ret, numbands, bandis2g, bcnt, acnt;
	...
	len = sizeof(bands);
	ret = wldm_xbrcm_radio(CMD_GET, radioIndex, (void *)(&(bands)), &len, "bands", NULL);
	if (ret < 0) {
	...
	ret = wldm_xbrcm_radio(CMD_GET, radioIndex, (void *)(&(rcap->band[0])), &len, "band", NULL);
	if (ret < 0) {
	...
	ret = wldm_xbrcm_radio(CMD_GET, radioIndex, (void *)list, &len, "possibleChannels", NULL);
	if (ret < 0) {
	...
	strncpy(rcap->ifaceName, wldm_get_radio_osifname(radioIndex), MAXIFACENAMESIZE);
	...
	wl_map_str2enum_fromTable(encModeStr, (unsigned int *)(&(rcap->cipherSupported)),
		", ", cipher_cap_table);

	len = sizeof(cbuf);
	ret = wldm_xbrcm_radio(CMD_GET, radioIndex, (void *)(&(cbuf)), &len, "countryList", NULL);
	if (ret < 0) {
...
INT wifi_getAPCapabilities(INT ap_index, wifi_ap_capabilities_t *apCapabilities)
{
	...
	/* Allow comma or space delimiters in string */
	wl_map_str2enum_fromTable(secmStr,
		(unsigned int *)(&(apCapabilities->securityModesSupported)),
		", ", security_mode_table);
	...
	/* Allow comma or space delimiters in string */
	wl_map_str2enum_fromTable(obmStr,
		(unsigned int *)(&(apCapabilities->methodsSupported)),
...
INT wifi_setRadioOperatingParameters(wifi_radio_index_t index, wifi_radio_operationParam_t *operationParam)
{
	...
	if (!operationParam->autoChannelEnabled) {
		...
		if (wldm_Radio_Channel(CMD_SET, index, &operationParam->channel, &len, 0, 0, NULL, NULL) < 0) {
		...
		if (operationParam->csa_beacon_count) {
			...
			len = sizeof(csaInfo);
			if (wldm_11h_dfs(CMD_SET_IOCTL, index, &csaInfo, &len, "csa_bcast", NULL) < 0) {
				HAL_WIFI_ERR(("%s: Error csa radioIndex=%d\n", __FUNCTION__, index));
				return WIFI_HAL_ERROR;
			}
		}
...
INT wifi_getRadioOperatingParameters(wifi_radio_index_t radioIndex,
		wifi_radio_operationParam_t *operationParam)
{
	BOOL autoChannelEnabled, enable, DCSEnabled, ieee80211hEanble = FALSE;
	wifi_channelBandwidth_t channelWidth;	/* The channel bandwidth. */
	wifi_ieee80211Variant_t variant;	/* The radio operating mode */
	UINT csa_beacon_count, pureMode, ch, channel, dtimPeriod, beaconInterval;
	UINT operatingClass, numSecondaryChannels, basicDataTransmitRates;
	UINT operationalDataTransmitRates, fragmentationThreshold;
	UINT transmitPower, rtsThreshold;
	UINT channelSecondary[WLDM_MAX_SECONDARY_CHANNELS];
	INT len, i, ret = WIFI_HAL_SUCCESS;
	csa_t csaInfo;
	CHAR operatingStandards[OUTPUT_STRING_LENGTH_64] = {0};
	...
	if (ret < 0) {
		HAL_WIFI_ERR(("%s: wldm_Radio_IEEE80211hEnabled CMD_GET failed\n",
				__FUNCTION__, radioIndex));
		ret = WIFI_HAL_INTERNAL_ERROR;
	}
	...
	len = sizeof(param);
	ret = wldm_xbrcm_radio(CMD_GET, radioIndex, &param, &len, "chanspec", NULL);
	if (ret < 0) {
	...
	if (wldm_Radio_DTIMPeriod(CMD_GET, radioIndex, &dtimPeriod, &len, NULL, NULL) != 0) {
	...
	if (wldm_Radio_BeaconPeriod(CMD_GET, radioIndex, &beaconInterval, &len, NULL, NULL) != 0) {
...
wifi_updateApSecurity(INT apIndex, wifi_vap_security_t *security)
{
	...
	if (wldm_AccessPoint_Security_ModeEnabled(CMD_SET, apIndex,
		security_mode_table[i].str_val, NULL, NULL, NULL)) {
		HAL_WIFI_ERR(("%s: %d: wldm_AccessPoint_Security_ModeEnabled failed\n",
			__FUNCTION__, apIndex));
		return WIFI_HAL_ERROR;
	}
	...
	if (wldm_AccessPoint_Wpa_Encryptionmode(CMD_SET, apIndex,
		encryption_table[i].str_val, NULL, NULL, NULL)) {
		HAL_WIFI_ERR(("%s: %d: wldm_AccessPoint_Wpa_Encryptionmode failed\n",
			__FUNCTION__, apIndex));
		return WIFI_HAL_ERROR;
	}
	...
	if (security->mode == wifi_security_mode_wpa_personal
		...
		if (security->u.key.type == wifi_security_key_type_psk) {
			...
			if (wldm_AccessPoint_Security_PreSharedKey(CMD_SET,
				apIndex, security->u.key.key, NULL, NULL, NULL)) {
			...
			if (wldm_AccessPoint_Security_KeyPassphrase(CMD_SET, apIndex,
				security->u.key.key, NULL, NULL, NULL)) {
...
#else /* WIFI_HAL_VERSION_3_PHASE2 */
		snprintf(ip_txt, sizeof(ip_txt), security->u.radius.ip);
#endif /* WIFI_HAL_VERSION_3_PHASE2 */
...
#else /* WIFI_HAL_VERSION_3_PHASE2 */
		snprintf(ip_txt, sizeof(ip_txt), security->u.radius.s_ip);
#endif /* WIFI_HAL_VERSION_3_PHASE2 */
...
wl_read_twt_broadcast_ids(int apIndex)
{
	...
	for (i = 0; i <= WL_TWT_ID_BCAST_MAX; i++) {
		fscanf(fp_twt, "%d ", &(twt_bcast_sched_idx[apIndex][i]));
	}
	fflush(fp_twt);
	fclose(fp_twt);
	return WIFI_HAL_SUCCESS;
...
wl_read_twt_sessions_table(int apIndex)
{
	...
	for (i = 0; i < MAX_NUM_TWT_SESSION; i++) {
		// i peer twtId agtype sessId
		sessp = TWT_SESS_INFO_PTR(apIndex, i);
		fscanf(fp_twt, "%d %02x %02x %02x %02x %02x %02x %d %d %d\n",
			&k, &sessp->peer.octet[0], &sessp->peer.octet[1], &sessp->peer.octet[2],
			&sessp->peer.octet[3], &sessp->peer.octet[4], &sessp->peer.octet[5],
			&sessp->twtId, &sessp->agtype, &sessp->sessId);
	}
	fflush(fp_twt);
...
{
	int ret, tlen, i, num, cnt, sessId = 0, totlen, tall_len, dcnt;
	...
	*numSessionReturned = 0;
	totlen = tlen; /* has twt_list for all peers */
	totlp = tallp->tlist;
	tlist = totlp;
...
INT wifi_setApRadiusTransportInterface(UINT RadiusInterface)
{
	...
	switch (RadiusInterface) {
		 ...
		 */
		wlcsm_nvram_set("radius_client_ifname", "privbr:cmnat");
		break;
		case 1:
		/*
		 ...
		 */
		wlcsm_nvram_unset("radius_client_ifname");
		break;
		default:
...
/* dummy functioin added for RDKM 21.3 */
