 ...
 *
 * $Id: phy_ac_tbl.c 809672 2022-03-22 04:44:16Z $
 */
...
static phy_table_info_t acphy_tables_rev130[] = {
	{  2,  0,    58},
	{  5,  1,   278},
	{  6,  0,    72},
	{  7,  1,  1475},
...
/* 6756 TCL acphydefs_rev131.tcl r864322 */
static phy_table_info_t acphy_tables_rev131[] = {
	{  2,  0,   58},
	{  3,  1,  512},
	{  4,  0,  512},
	{  5,  1,  150},
...
phy_ac_tbl_dumptbl(phy_type_tbl_ctx_t *ctx, struct bcmstrbuf *b)
{
	...
	if (ACMAJORREV_32(pi->pubpi->phy_rev)) {
		...
		ti = acphy_tables_rev130;
	} else if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
		ti = acphy_tables_rev131;
...
{
	return ACREV_IS(pi->pubpi->phy_rev, 1) || ACREV_GE(pi->pubpi->phy_rev, 6);
}
...
wlc_phy_ac_ht_proprietary_rates(phy_info_t *pi)
{
	...
	return ACMAJORREV_4(pi->pubpi->phy_rev) || ACMAJORREV_5(pi->pubpi->phy_rev);
}

/** Return True if PHY is capable of STBC */
static bool
...
static bool wlc_phy_ac_160_capable(phy_info_t *pi)
{
	return (ACMAJORREV_131(pi->pubpi->phy_rev) ||
...
WLBANDINITFN(wlc_phy_init_acphy)(phy_ac_tbl_info_t *tbli)
{
	...
	if (ISSIM_ENAB(pi->sh->sih) && ACMAJORREV_GE130(pi->pubpi->phy_rev)) {
		int tables[] = {
			ACPHY_TBL_ID_GAIN0, ACPHY_TBL_ID_GAIN1, ACPHY_TBL_ID_GAIN2,
			ACPHY_TBL_ID_GAIN3, ACPHY_TBL_ID_GAINLIMIT0, ACPHY_TBL_ID_GAINLIMIT1,
			ACPHY_TBL_ID_GAINLIMIT2, ACPHY_TBL_ID_GAINLIMIT3, ACPHY_TBL_ID_GAINBITS0,
			ACPHY_TBL_ID_GAINBITS1, ACPHY_TBL_ID_GAINBITS2, ACPHY_TBL_ID_GAINBITS3
		};
		int tbl_id, tbl_n_els;
		int i, j;
		uint8 zero = 0;
		...
		for (i = 0; i < ARRAYSIZE(tables); i++) {
			tbl_id = tables[i];
			switch (tbl_id) {
			case ACPHY_TBL_ID_GAINLIMIT0:
			case ACPHY_TBL_ID_GAINLIMIT1:
			case ACPHY_TBL_ID_GAINLIMIT2:
			case ACPHY_TBL_ID_GAINLIMIT3:
				tbl_n_els = 105;
				break;
			default:
				tbl_n_els = 119;
			...
			for (j = 0; j < tbl_n_els; j++) {
				wlc_phy_table_write_acphy(pi, tbl_id, 1, j, 8 * sizeof(uint8),
				&zero);
			}
		}
	}
	...
	const void *data)
{
	...
	uint8 mac_suspend = 1;
	uint8 stall_val = 0;
#if defined(WL_PROXDETECT) && defined(WL_PROXD_SEQ)
	phy_ac_tof_info_t *tofi;
	uint16 tof_rfseq_bundle_offset;
#endif /* WL_PROXDETECT && WL_PROXD_SEQ */
...
void
phy_ac_tbl_rfseq_write(phy_info_t *pi, uint8 len, uint16 offset, const uint16 *data)
{
	 wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_RFSEQ, len, offset, 16, data);
}

void
phy_ac_tbl_rfseq_read(phy_info_t *pi, uint8 len, uint16 offset, uint16 *data)
{
	 wlc_phy_table_read_acphy(pi, ACPHY_TBL_ID_RFSEQ, len, offset, 16, data);
}
...
static const uint16 *
wlc_phy_get_txgain_tbl_20708(phy_info_t *pi)
{
	const uint16 *tx_pwrctrl_tbl = NULL;
	...
	case 0:
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			tx_pwrctrl_tbl = acphy16nm_txgain_epa_2g_p5_20708a0_rev0;
		} else {
			tx_pwrctrl_tbl = acphy16nm_txgain_epa_5g_p5_20708a0_rev0;
		}
		break;
	case 1:
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			tx_pwrctrl_tbl = acphy16nm_txgain_epa_2g_p5_20708a1_rev1;
		} else {
			tx_pwrctrl_tbl = acphy16nm_txgain_epa_5g_p5_20708a1_rev1;
		}
		break;
	case 2:
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			tx_pwrctrl_tbl = acphy16nm_txgain_epa_2g_p5_20708b0_rev2;
		} else {
			tx_pwrctrl_tbl = acphy16nm_txgain_epa_5g_p5_20708b0_rev2;
		}
		break;
	default:
		PHY_ERROR(("wl%d: %s: Unsupported radio revision %d\n",
...
static const uint16 *
wlc_phy_get_txgain_tbl_20710(phy_info_t *pi)
{
	const uint16 *tx_pwrctrl_tbl = NULL;
	...
	case 1:
		tx_pwrctrl_tbl = CHSPEC_IS2G(pi->radio_chanspec) ?
			PHY_IPA(pi) ? acphy28nm_txgain_ipa_2g_p5_20710a0_rev0:
			acphy28nm_txgain_epa_2g_p5_20710a0_rev0:
			acphy28nm_txgain_epa_5g_p5_20710a0_rev0;
		break;
	default:
		PHY_ERROR(("wl%d: %s: Unsupported radio revision %d\n",
			pi->sh->unit, __FUNCTION__, RADIOREV(pi->pubpi->radiorev)));
...
wlc_phy_ac_gains_load(phy_ac_tbl_info_t *tbli)
{
	...
	if (RADIOID_IS(pi->pubpi->radioid, BCM2069_ID)) {
		...
		tx_pwrctrl_tbl = wlc_phy_get_txgain_tbl_20708(pi);
		...
		tx_pwrctrl_tbl = wlc_phy_get_txgain_tbl_20710(pi);
	} else {
