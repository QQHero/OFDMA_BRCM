 ...
 *
 * $Id: wlu_client_shared.c 810606 2022-04-11 15:35:29Z $
 */
#include <sys/types.h>
...
static int
rwl_information_socket(void *wl, int cmd, void* input_buf, uint32 *input_len,
	uint32 *tx_len, uint flags)
{
	int error, Sockfd;
...
static int
rwl_information_dongle(void *wl, int cmd, void* input_buf, uint32 *input_len,
...
static int
rwl_shell_information_fe(void *wl, int cmd, uchar* input_buf, uint32 *input_len)
{
...
rwl_queryinformation_fe(void *wl, int cmd, void* input_buf,
	uint32 *input_len, int debug, int rem_ioctl_select) {
	int error = 0;
	uint tx_len;
	...
	switch (remote_type) {
		...
		case REMOTE_SERIAL:
			...
			if (*input_len > (1024 *10))
				...
				DPRINT_DBG(OUTPUT, "Wait time: %u seconds\n",
...
rwl_setinformation_fe(void *wl, int cmd, void* buf,
	uint32 *len, int debug, int rem_ioctl_select) {
	int error = 0;
	uint tx_len;
#if defined(RWL_SERIAL) || RWL_WIFI
	rem_ioctl_t *rem_ptr = NULL;
...
rwl_var_getbuf(void *wl, const char *iovar, void *param, int param_len, void **bufptr)
{
	...
	if ((len > 0) && (len < WLC_IOCTL_MAXLEN)) {
		strncpy(g_rwl_aligned_buf, iovar, len);
	}
	else {
		return BCME_BADLEN;
	}

	if ((param_len > 0) && ((len + param_len) < WLC_IOCTL_MAXLEN)) {
	...
	*/
	...
	error = wl_get(wl, WLC_GET_VAR, g_rwl_aligned_buf, WLC_IOCTL_MAXLEN);
