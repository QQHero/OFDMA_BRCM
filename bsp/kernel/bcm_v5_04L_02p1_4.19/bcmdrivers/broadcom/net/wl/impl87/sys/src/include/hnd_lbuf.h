 ...
 *
 * $Id: hnd_lbuf.h 793616 2020-12-03 10:14:21Z $
 */
...
#if defined(PROP_TXSTATUS)
#define LBUF_FLIST_FRAG_MAX	(LB_FRAG_MAX + 1)
#else
#define LBUF_FLIST_FRAG_MAX	(LB_FRAG_MAX)
#endif /* ! PROP_TXSTATUS */

struct lbuf_flist {
	struct lbuf_finfo finfo[LBUF_FLIST_FRAG_MAX];	/**< Element finfo[0] is context */
	uint16 flen[LBUF_FLIST_FRAG_MAX];	/**< Element flen[0] is total fragment length */
	union {
		uint16 ring_idx;	/**< PROP_TXSTATUS: TxLfrag: flowring index */
		struct {
			uint8 txstatus; /**< HNDPQP: lbuf_finfo_t::txstatus */
			uint8 RSVD_FIELD;
		};
	};
};
