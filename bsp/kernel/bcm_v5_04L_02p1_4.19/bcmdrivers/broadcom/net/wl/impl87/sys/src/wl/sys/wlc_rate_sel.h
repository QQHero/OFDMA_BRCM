 ...
 *
 *
 * $Id: wlc_rate_sel.h 802793 2021-09-06 12:26:36Z $
 */
...
typedef struct ratesel_init {
	...
	bool		bw_auto_dl;	/* if use dl bw_auto policy */
	bool		bw_auto_ul;	/* if use ul bw_auto policy */
	bool		rate_init;	/* cache of rate_init */
	uint8		ulepoch;	/* cache of ul epoch */
	uint8		ulrateid;	/* cache of ul rateid */
} ratesel_init_t;
...
extern void wlc_ratesel_init_params_sync(rcb_t *state, ratesel_init_t *init);
#if defined(WL_MU_TX)
...
extern ratespec_t wlc_ratesel_get_ulrt_txr(rcb_t *state, uint8 rtidx);
#if defined(BCMDBG)
extern void wlc_ratesel_ulrt_dump_clr(rcb_t *state);
#endif
#endif /* WL_ULRT_DRVR */
