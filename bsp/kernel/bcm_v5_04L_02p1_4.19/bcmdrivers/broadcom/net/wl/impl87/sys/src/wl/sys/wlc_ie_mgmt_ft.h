 ...
 *
 * $Id: wlc_ie_mgmt_ft.h 796557 2021-03-08 04:57:09Z $
 */
...
union wlc_iem_ft_cbparm {
	...
	struct {
		...
		uint8 status_data;	/* data associated with status if there was a rejection */
	} assocresp;
	...
	struct {
		...
		uint8 short_ssid_len;	/* bytes referred by pointer short_ssid i.e
					 * [sizeof(uint32) * number of short ssid]
					 */
		uint32 *short_ssid;
	} bcn;
	/* for CS wrapper IE */
...
union wlc_iem_ft_pparm {
	...
	struct {
		...
		uint8 *vht_cap_ie;	/** Output: VHT Capability IE, network endianness */
		uint8 *vht_op_ie;	/** Output: VHT Operation IE, network endianness */
		...
		uint8 *he_cap_ie;	/* Output: HE Capability IE */
		uint8 *he_6g_caps_ie;	/* Output: HE Extended Capability IE */
		uint8 *he_op_ie;	/* Output: HE Operation IE */
	} assocreq;
