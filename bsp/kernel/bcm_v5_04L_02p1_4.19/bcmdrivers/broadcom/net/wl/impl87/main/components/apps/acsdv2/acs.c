 ...
 *
 * $Id: acs.c 807558 2022-01-25 04:23:07Z $
 */
...
/* bgn  itf  {  BSS  BUSY  INTF I-ADJ   FCS TXPWR NOISE TOTAL   CNS   ADJ TXOP  DFS/AGE,
RBSS, RTXOP, CHTW}, pick
 */
/* --- ----   ----- ----- ----- ----- ----- ----- ----- ----- ----- ----- ----  ---  func */
{ -65,  40, {   10,    1,    1,    0,    0,    0,    1,    1,    1,    0,  10,	 5,
5,	5,	1 },	NULL}, /* 2G */
...
acs_pick_chanspec_default2g(acs_chaninfo_t* c_info, int bw)
{
	...
	if (c_info->channel_weights.count) {
		score_type = CH_SCORE_CHWT;
	}
	return acs_pick_chanspec_common(c_info, bw, score_type);
}
...
/* user policy is created to allow customers to change weights as per
 * requirement through nvram(wl<x>_acs_pol="")
 */
static chanspec_t
acs_pick_chanspec_userpolicy(acs_chaninfo_t* c_info, int bw)
{
...
acs_default_policy(acs_policy_t *a_pol, acs_policy_index index)
{
	...
	if (index == ACS_POLICY_DEFAULT2G) {
		...
		a_pol->chan_selector = acs_pick_chanspec_userpolicy;
	}
}
...
void acs_set_initial_chanspec(acs_chaninfo_t *c_info)
{
	...
	if (c_info->rs_info.pref_chspec == 0) {
		...
		if (c_info->rs_info.band_type == WLC_BAND_5G) {
			...
			dcs_data.reason = 0;
			dcs_data.chspec = chanspec;
			if ((acs_get_chanspec(c_info, &cur_chspec)) != ACSD_OK) {
				return;
			}
			if (c_info->acs_restart) {
				int dtim_count = 0;
				if ((cur_chspec == chanspec) || !(acs_is_dfs_chanspec(c_info,
						cur_chspec)) || c_info->country_is_edcrs_eu) {
					return;
				}
				ret = wl_ioctl(c_info->name, WLC_GET_DTIMPRD, &dtim_count,
						sizeof(dtim_count));
				if (ret < 0) {
					return;
				}
				if (c_info->acs_use_csa) {
					...
					ret = dcs_handle_request(c_info->name, &dcs_data,
							DOT11_CSA_MODE_ADVISORY, dtim_count,
							c_info->acs_dcs_csa);
				}
				if (ret == ACSD_OK) {
					dtim_count = dtim_count*100 + 50;
					sleep_ms(dtim_count);
				}
				return;
			} else {
				if (!c_info->acs_use_csa ||
						(ret = dcs_handle_request(c_info->name, &dcs_data,
						DOT11_CSA_MODE_ADVISORY, 2,
						c_info->acs_dcs_csa)) != ACSD_OK) {
					acs_set_chanspec(c_info, chanspec);
					if ((acs_update_driver(c_info)) != ACSD_OK) {
			...
			/* If already on dfs channel/when csa succeeds then allowing acsd to
			 * sleep for 150ms to reflect selected chanspec in firmware
			 * (because of csa count down i.e 100ms)
			 */
			if (acs_is_dfs_chanspec(c_info, cur_chspec) || ret == ACSD_OK) {
				sleep_ms(250);
			}
		}
	}
...
void
acs_get_reg_txpwr_for_allchans(acs_chaninfo_t* c_info)
{
	wl_chanspec_reg_txpwr_t params = {0};
	char *txpwrbuf;
	int i, ret = 0, buflen = ACS_TXPWR_BUF_LEN;
	...
	c_info->reg_txpwr_params = (wl_chanspec_reg_txpwr_t *) txpwrbuf;

	params.ver = htod16(WL_CHANSPEC_REG_TXPWR_VER);
	params.len = htod16(WL_CHANSPEC_REG_TXPWR_LEN);
	params.count = htod32(1);

	ret = acs_get_reg_txpwr_info(c_info, &params, sizeof(wl_chanspec_reg_txpwr_t),
			txpwrbuf, buflen);
	if (ret != 0) {
		ACSD_DEBUG("%s: Failed to get reg txpwr values using iovar and falling back"
				" to old method\n", c_info->name);
		ACS_FREE(txpwrbuf);
		c_info->fallback_to_txpwr_max = TRUE;
		c_info->reg_txpwr_params = NULL;
		return;
	}

	c_info->reg_txpwr_params->ver = dtoh16(c_info->reg_txpwr_params->ver);
	if (c_info->reg_txpwr_params->ver != WL_CHANSPEC_REG_TXPWR_VER) {
		ACSD_DEBUG("%s: Error: version [%d] mismatch Driver version:%d\n",
				c_info->name, WL_CHANSPEC_TXPWR_MAX_VER,
				c_info->reg_txpwr_params->ver);
		ACS_FREE(txpwrbuf);
		c_info->fallback_to_txpwr_max = TRUE;
		c_info->reg_txpwr_params = NULL;
		return;
	...
	c_info->reg_txpwr_params->count = dtoh32(c_info->reg_txpwr_params->count);
	if (c_info->reg_txpwr_params->count > (buflen - WL_CHANSPEC_REG_TXPWR_FIXED_LEN) /
		sizeof(chanspec_reg_txpwr_t)) {
		ACSD_DEBUG("%s: c_info->txpwr_params count %d is greater than default max"
			"count %d\n", c_info->name, c_info->reg_txpwr_params->count, buflen / 4);
		ACS_FREE(txpwrbuf);
		c_info->fallback_to_txpwr_max = TRUE;
		c_info->reg_txpwr_params = NULL;
		return;
	}
	for (i = 0; i < c_info->reg_txpwr_params->count; i++) {
		c_info->reg_txpwr_params->reg_txpwr[i].chanspec =
			dtoh16(c_info->reg_txpwr_params->reg_txpwr[i].chanspec);
		c_info->reg_txpwr_params->reg_txpwr[i].reg_txpwr =
			MIN(dtoh16(c_info->reg_txpwr_params->reg_txpwr[i].reg_txpwr),
			ACS_MAX_TXPWR);
	}
}
static int
acs_start(char *name, acs_chaninfo_t *c_info)
{
	...
	c_info->acs_is_in_ap_mode = acs_is_mode_check(c_info->name);
	...
	/* Update txpwr values for all ACSD modes in AP interface */
	if (c_info->acs_is_in_ap_mode) {
...
scan_retry:
		...
		if (ret < 0) {
			scan_retry_count ++;
			if (scan_retry_count == 1) {
				struct timeval tv1;
				time_t scan_timeout;

				acs_get_time_since_boot(&tv1);
			        scan_timeout = tv1.tv_sec + WL_CS_SCAN_TIMEOUT_6G;
				ACSD_ERROR("%s: cs scan failed because of %d try again after"
						"%ds\n", c_info->name, ret, WL_CS_SCAN_TIMEOUT_6G);
				while (tv1.tv_sec < scan_timeout) {
					acs_get_time_since_boot(&tv1);
				}
				goto scan_retry;
			} else {
...
{
	ifname_idx_map_t *cur_map;
	char prefix[NVRAM_MAX_PARAM_LEN], *ifnames[16] = {0};
	char *token, *delim = " ";
	char *vifname;
...
acs_init_run(acs_info_t ** acs_info_p)
{
	...
	foreach(name, nvram_safe_get("acs_ifnames"), next) {
		...
		if ((AUTOCHANNEL(c_info) || COEXCHECK(c_info)) &&
			 ...
			 */
				...
				if (ret)
					ACSD_ERROR("%s: update driver failed\n", name);
			} else if (c_info->acs_use_csa && c_info->acs_boot_only) {
				/* Wait for CSA so acs_update_status() is reading latest chanspec */
				sleep_ms(ACS_CSA_COUNT * 100 + 100);
			}

			if (ret == 0 && ACS_11H_AND_BGDFS(c_info)) {
				acs_dfsr_chanspec_update(ACS_DFSR_CTX(c_info),
					c_info->selected_chspec, __FUNCTION__, c_info->name);
			...
			chanim_upd_acs_record(c_info->name, c_info->chanim_info,
				c_info->selected_chspec, ACS_INIT, d_info->ticks);
		}
	...
	if (acs_zdfs_2g) {
		...
		for (i = 0; i < ACS_MAX_IF_NUM; i++) {
			...
			is_6g = BAND_6G(tc_info->rs_info.band_type);
			if (is_5g) {
...
acs_update_status(acs_chaninfo_t * c_info)
{
	...
	start_idx = MODSUB(chanim_mark(ch_info).record_idx, 1, ACS_MAX_RECORD);
	 ...
	 */
	if ((c_info->switch_reason_type == ACS_DFSREENTRY) && c_info->cur_is_dfs &&
			CHSPEC_CHANNEL(start_record->selected_chspc) !=
			CHSPEC_CHANNEL(c_info->cur_chspec)) {
		chanim_upd_acs_record(c_info->name, c_info->chanim_info, c_info->cur_chspec,
				ACS_DFSREENTRY, d_info->ticks);
...
acs_update_bw_status(acs_chaninfo_t * c_info)
{
	...
	if (BAND_5G(c_info->rs_info.band_type) && MONITORCHECK(c_info) &&
		(c_info->rs_info.pref_chspec != 0)) {
		c_info->bw_upgradable = CHSPEC_BW_GT(c_info->rs_info.pref_chspec,
			CHSPEC_BW(c_info->cur_chspec));
	} else {
		c_info->bw_upgradable = !ACS_CHSPEC_MAXED_BWCAP(c_info->cur_chspec,
...
acs_check_ifname_is_virtual(char **ifname)
{
	...
	for (i = 0; i < ACS_MAX_IF_NUM; i++) {
		cur_map = &acs_info->acs_ifmap[i];
		if (cur_map == NULL || !cur_map->name[0] ||
