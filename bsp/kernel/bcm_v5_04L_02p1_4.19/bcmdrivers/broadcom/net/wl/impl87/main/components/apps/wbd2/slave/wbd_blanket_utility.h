 ...
 *
 * $Id: wbd_blanket_utility.h 807282 2022-01-17 09:58:21Z $
 */
 ...
 * @param mac		MAC address of the STA
 * @param status	Failed connection association status code of tyep DOT11_SC_XXX
 * @param reason	Reason code of type DOT11_RC_XXX
 */
int
 ...
 */
extern void wbd_blanket_util_enable_disable_all_fronthaul_bss(int enable);

/* Disconnect all the STA's connected to the BSS */
extern void wbd_blanket_util_disconnect_all_stas_from_bss(i5_dm_bss_type *i5_bss);
...
void wbd_slave_update_map_flags_of_all_ifr(i5_dm_device_type *i5_device);
...
/* Update BSSID's of all the backhaul STAs */
int wbd_blanket_util_update_bssids_of_all_bstas(i5_dm_device_type *i5_self_dev);

/* Disable roaming and keep the AP up on all the disconnected BSTAs */
void wbd_blanket_util_vap_up_on_disconnected_bstas(i5_dm_device_type *i5_self_dev);

/* Set keep_ap_up on all BSTAs */
void wbd_blanket_util_set_bsta_keep_ap_up(i5_dm_device_type *i5_self_dev, int keep_ap_up);
