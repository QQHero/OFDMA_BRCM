 ...
 *
 * $Id: wlc_macdbg.c 811004 2022-04-21 15:19:55Z $
 */
...
#else
static int wlc_macdbg_smpl_capture_init(wlc_macdbg_info_t *macdbg);
#endif /* BCMDBG */
...
#define WL_MACDBG_DFLT_TRANSMASK        0x3FFF
...
#else
    wlc_macdbg_smpl_capture_init(macdbg);
#endif
...
BCMATTACHFN(wlc_macdbg_init_dumplist)(wlc_macdbg_info_t *macdbg)
{
    ...
    svmp_list_t svmpmems_ge128[] = {
        ...
        {0x38600, 0xc0},    // m2v/v2m buf for on-the-fly grouping and sounding_update
        ...
        {0x383f0, 0x110},    // txv_header and idx
    ...
    struct bcmstrbuf *b, uint8 **p)
{
    ...
    case D11REG_TYPE_IHR16:
        /* addr is always even */
        if ((addr + sizeof(val16)) <= PIHR2K_BASE) {
...
wlc_dump_trig_tpc_params(wlc_info_t *wlc, struct bcmstrbuf *b)
{
    ...
    uint offset1 = M_RSSICOR_BLK(wlc);
    uint TRGRSSI[4];
    uint32 cmninfo[2];
    uint usrinfo[3];
    int i;
    ...
    if (D11REV_IS(corerev, 132)) {
        TRGRSSI[0] = ((int8) (wlc_bmac_read_shm(wlc->hw, offset) & (0xff)) +
            (int8) (wlc_bmac_read_shm(wlc->hw, offset1) & (0xff)));
        TRGRSSI[1] = ((int8) ((wlc_bmac_read_shm(wlc->hw, offset) & (0xff00)) >> 8) +
            (int8) ((wlc_bmac_read_shm(wlc->hw, offset1) & (0xff00)) >> 8));
        TRGRSSI[2] = ((int8) (wlc_bmac_read_shm(wlc->hw, offset+2) & (0xff)) +
            (int8) (wlc_bmac_read_shm(wlc->hw, offset1+2) & (0xff)));
        TRGRSSI[3] = ((int8) ((wlc_bmac_read_shm(wlc->hw, offset+2) & (0xff00)) >> 8) +
            (int8) ((wlc_bmac_read_shm(wlc->hw, offset1+2) & (0xff00)) >> 8));
    } else {
        TRGRSSI[0] = (int8) (wlc_bmac_read_shm(wlc->hw, offset) & (0xff));
        TRGRSSI[1] = (int8) ((wlc_bmac_read_shm(wlc->hw, offset) & (0xff00)) >> 8);
        TRGRSSI[2] = (int8) (wlc_bmac_read_shm(wlc->hw, offset+2) & (0xff));
    ...
    bcm_bprintf(b, "PATHLOSS: %d dB APTX: %d TRGRSSI: %d %d %d %d TRSSI: %d "
        ...
        TRGRSSI[0], TRGRSSI[1], TRGRSSI[2], TRGRSSI[3],
...
#ifndef DONGLEBUILD
    if (wlc->psm_watchdog_debug == TRUE) {
...
wlc_macdbg_smpl_capture_set(wlc_info_t *wlc, wl_maccapture_params_t *params)
{
    ...
    !WL_MACDBG_IS_EXPERT_MODE) {
        if (D11REV_GE(wlc->pub->corerev, 128)) {
            ...
            params->gpio_sel == WL_MACDBG_GPIO_PC1) {
                params->x_mask = WL_MACDBG_DFLT_TRANSMASK;
    ...
    // if utrace GPIO capture is enabled, disable utrace SHM capture
    // otherwise in all other cases enable utrace SHM capture too
    if (D11REV_GE(wlc->pub->corerev, 129) && params->gpio_sel == WL_MACDBG_GPIO_UTRACE) {
        wlc_write_shm(wlc, M_UTRACE_STS(wlc), FALSE);
    } else {
        wlc_write_shm(wlc, M_UTRACE_STS(wlc), TRUE);
    }
...
#else
/* wlc_macdbg_smpl_capture_init initializes MAC sample capture.
 * If UP, also starts the sample capture.
 * Otherwise, sample capture will be started upon wl up
 */
static int
wlc_macdbg_smpl_capture_init(wlc_macdbg_info_t *macdbg)
{
    wlc_info_t *wlc = macdbg->wlc;
    uint32 d11_regvalue = 0;
    int err = BCME_OK;
    ...
    volatile uint16 *scx_mask_reg_h, *scx_mask_reg_l;
    volatile uint16 *scm_mask_reg_h, *scm_mask_reg_l, *scm_val_reg_h, *scm_val_reg_l;
    volatile uint16 *strtptr_reg, *stopptr_reg;
    uint16 smpl_ctrl;
    ...
    sct_mask_reg_l = (D11_PSM_SCT_MASK_L(wlc));
    sct_mask_reg_h = (D11_PSM_SCT_MASK_H(wlc));
    sct_val_reg_h = (D11_PSM_SCT_VAL_H(wlc));
    sct_val_reg_l = (D11_PSM_SCT_VAL_L(wlc));
    smpl_ctrl_reg = D11_PSM_SMP_CTRL(wlc);
    strtptr_reg = D11_PSM_SCP_STRTPTR(wlc);
    stopptr_reg = D11_PSM_SCP_STOPPTR(wlc);
    smpl_ctrl2_reg = D11_PSM_SMP_CTRL2(wlc);
    scs_mask_reg_l = D11_PSM_SCS_MASK_L(wlc);
    scs_mask_reg_h = D11_PSM_SCS_MASK_H(wlc);
    scx_mask_reg_l = D11_PSM_SCX_MASK_L(wlc);
    scx_mask_reg_h = D11_PSM_SCX_MASK_H(wlc);
    scm_mask_reg_l = D11_PSM_SCM_MASK_L(wlc);
    scm_mask_reg_h = D11_PSM_SCM_MASK_H(wlc);
    scm_val_reg_l = D11_PSM_SCM_VAL_L(wlc);
    scm_val_reg_h = D11_PSM_SCM_VAL_H(wlc);
    ...
    /* GPIO Output Selection */
    d11_regvalue = (WL_MACDBG_GPIO_PC << MCTL1_GPIOSEL_SHIFT)
            & MCTL1_GPIOSEL_MASK; /* GPIO_SEL is 6bits */
    d11_regvalue = (R_REG(wlc->osh, D11_MacControl1(wlc)) & ~MCTL1_GPIOSEL_MASK) | d11_regvalue;
    W_REG(wlc->osh, D11_MacControl1(wlc), d11_regvalue);

    /* Sample Collect Start & Stop Ptr */
    MACDBG_SET_MACPTR_CFG(strtptr_reg, WL_MACCAPT_PSM_STRT_PTR);
    MACDBG_SET_MACPTR_CFG(stopptr_reg, WL_MACCAPT_PSM_STOP_PTR);

    // enable utrace SHM capture
    wlc_write_shm(wlc, M_UTRACE_STS(wlc), TRUE);
    ...
    /* Sample Capture Options */
    MACDBG_SET_MACCPTR_PREG(sct_mask_reg, 0);
    MACDBG_SET_MACCPTR_PREG(sct_val_reg, 0);
    MACDBG_SET_MACCPTR_PREG(scs_mask_reg, WL_MACDBG_DFLT_STOREMASK);
    MACDBG_SET_MACCPTR_PREG(scx_mask_reg, WL_MACDBG_DFLT_TRANSMASK);
    MACDBG_SET_MACCPTR_PREG(scm_mask_reg, 0);
    MACDBG_SET_MACCPTR_PREG(scm_val_reg, 0);
    MACDBG_SET_MACPTR_CFG(smpl_ctrl2_reg, (0xf));

    if ((D11REV_IS(wlc->pub->corerev, 132) ||
    (D11REV_IS(wlc->pub->corerev, 130) &&
    (wlc->pub->corerev_minor == 2)))) {
        /* WAR for CRBCAD11MAC-5947: Need to set stop trigger value
         * to be non-zero even if it is not using stop trigger mode.
         * It applies to GPIOMON case only
         * XXX temporary extend to 130.2
         */
        volatile uint16 *sctstp_val_reg_h, *sctstp_val_reg_l;
        sctstp_val_reg_h = D11_PSM_SCTSTP_VAL_H(wlc);
    ...
    /* Start MAC Sample Capture */
    /* Sample Capture Source and Start bits */
    smpl_ctrl = SC_STORE_EN | SC_TRANS_EN;
    smpl_ctrl |= (SC_SRC_MAC << SC_SRC_SHIFT) | SC_STRT;
    W_REG(wlc->osh, smpl_ctrl_reg, (uint16)(smpl_ctrl
        | (WL_MACCAP_SEL_PSMR0 << WL_MACCAP_SEL_PSMR0_SHIFT)));

    return err;
} /* wlc_macdbg_smpl_capture_init */
#endif
...
#ifdef WLC_MACDBG_FRAMEID_TRACE
    grp_hist_per_blk_t  *grp_hist_per_blk;
#endif
...
wlc_dump_ffqgap(wlc_info_t *wlc, struct bcmstrbuf * b)
{
    ...
    for (i = 0; i < FFQ_GAP_HIST_MAX / 8; i++) {
        for (j = 0; j < 8; j++) {
            ...
            bcm_bprintf(b, "(%02d) AC%d %s %04d | ", i + j * 8,
