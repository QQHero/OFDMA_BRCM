 ...
 *
 * $Id: wlc_prot_obss.c 797454 2021-04-01 22:22:03Z $
 */
...
wlc_prot_obss_init(void *cntxt)
{
	...
	if (WLC_PROT_OBSS_ENAB(wlc->pub)) {
		...
		wlc_bmac_write_shm(wlc->hw, M_SECRSSI0_MIN(wlc),
			priv->config->obss_sec_rssi_lim0);
		wlc_bmac_write_shm(wlc->hw, M_SECRSSI1_MIN(wlc),
...
wlc_prot_obss_down(void *cntxt)
{
	...
	/* Delete any potential timer so we don't attempt to read shmem when down */
	wl_del_timer(wlc->wl, priv->stats_timer);
#else
	BCM_REFERENCE(cntxt);
#endif
	return BCME_OK;
...
BCMATTACHFN(wlc_prot_obss_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_module_register(wlc->pub, wlc_prot_obss_iovars,
			...
			wlc_prot_obss_watchdog, wlc_prot_obss_init, wlc_prot_obss_down)) {
