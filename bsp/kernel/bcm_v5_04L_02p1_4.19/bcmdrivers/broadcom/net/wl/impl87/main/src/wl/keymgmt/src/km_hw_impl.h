 ...
 * $Id: km_hw_impl.h 805999 2021-12-10 08:35:58Z $
 */
...
typedef uint16	skl_idx_t;		/* index into SECKINDXALGO block */
typedef km_amt_attr_t	amt_attr_t;
typedef uint16	shm_addr_t;
typedef uint16	hwktab_addr_t;

#define KM_HW_SKL_IDX_INVALID 0xfff
#define KM_HW_SKL_IDX_TXONLY 0xffe	/* reserved index */
...
#define KM_HW_D11_MAX_KEY_SIZE(_hw) D11_MAX_KEY_SIZE(KM_HW_COREREV(_hw))

#define KM_HW_D11_MIN_KEY_SIZE 16

/*  max value for AMT/RCMTA.  */
#define KM_HW_IDX_MAX(_corerev) AMT_SIZE(_corerev)

/* bitmap of amt/rcmta index */
typedef uint8 amt_idx_bitmap_t;
 ...
 */
#define KM_HW_AMT_IDX_VALID(_hw, _idx) (((_idx) >= 0) && ((_idx) < (_hw)->max_amt_idx))
...
#define KM_HW_KEY_ADDR(_hw, _idx) (((_hw)->shm_info.key_base) + \
	(KM_HW_D11_MAX_KEY_SIZE(_hw)) * (_idx))
...
#define KM_HW_TKIP_GRP_GE80(_hw, _idx) ((KM_HW_COREREV(_hw) >= 80) && \
	(_idx < WLC_KEYMGMT_NUM_GROUP_KEYS))

/* tkip tsc ttak - default keys have no ttak except for wowl */
#define KM_HW_TKIP_MAX_TSC_TTAK(_rev) (_rev) < 80 ? 50 : 7
#define KM_HW_TKIP_TSC_TTAK_GRP_SLOT	7
#define KM_HW_TKIP_TSC_TTAK_IDX(_hw, _idx) ((KM_HW_LEGACY_WOWL(_hw) ? (_idx) : \
	((KM_HW_TKIP_GRP_GE80(_hw, _idx)) ? KM_HW_TKIP_TSC_TTAK_GRP_SLOT : \
	(_idx - WLC_KEYMGMT_NUM_GROUP_KEYS))))
...
#define KM_TKIP_TSC_TTAK_SLOT_SIZE(_rev) (_rev) < 80 ? (KM_HW_TKIP_TSC_TTAK_SIZE) : \
	(KM_HW_TKIP_TSC_TTAK_SIZE << 1)
#define KM_HW_TKIP_TSC_TTAK_ADDR(_hw, _idx) ((_hw)->shm_info.tkip_tsc_ttak_base + \
	(KM_HW_TKIP_TSC_TTAK_IDX(_hw, _idx)) * (KM_TKIP_TSC_TTAK_SLOT_SIZE(KM_HW_COREREV(_hw))))
...
#define KM_HW_SMS4_MIC_KEY_SUPPORTED(_hw, _idx) (\
	(_hw)->shm_info.wapi_mic_key_base != 0 && \
	(_idx) < hw->shm_info.max_wapi_mic_keys)
...
#define KM_HW_BSSCFG_NEED_STA_GROUP_KEYS(_hw, _bsscfg) ((_bsscfg)->BSS && \
	KM_HW_WLC(_hw)->primary_bsscfg != (_bsscfg) && \
	BSSCFG_STA(_bsscfg) && !BSS_TDLS_ENAB(KM_HW_WLC(_hw), _bsscfg) && \
	!BSSCFG_PSTA(_bsscfg) && !((_bsscfg)->flags & WLC_BSSCFG_NOBCMC))

#define KM_HW_IS_DEFAULT_BSSCFG(_hw, _bsscfg) (KM_HW_WLC(_hw)->primary_bsscfg == (_bsscfg))
