 ...
 *
 * $Id: rte_cons.c 794653 2021-01-14 08:56:19Z $
 */
...
static char const echo_cmd[] = "echo";
...
#ifdef BCMSTANDALONE_TEST
static cons_soft_t cons0_test;
static char cons0_testbuf[LOG_BUF_LEN];
#endif

/* Forward decl */
#ifdef RTE_UART
static bool hnd_cons_uart_worklet(void *cbdata);

#ifndef RTE_CONSOLE_UART_ID
#define RTE_CONSOLE_UART_ID	0
#endif

#ifdef RTE_CONSOLE_UART_SECI
#define RTE_CONSOLE_UART_TYPE RTE_UART_TYPE_SECI
#else
#define RTE_CONSOLE_UART_TYPE RTE_UART_TYPE_CC
#endif
...
static bool
hnd_cons_sr_save(void *arg)
{
	cons_soft_t *cons = arg;
	hnd_cons_flush();
	serial_down(cons->uart);
...
void
hnd_cons_uart_muxenab(si_t *sih, uint32 pinmap)
{
	if (active_cons) {
		serial_muxenab(active_cons->uart, pinmap);
	}
}
#endif /* RTE_UART */
...
BCMATTACHFN(hnd_cons_init)(si_t *sih, osl_t *osh)
{
	...
	if (sih != NULL && UART_ENAB()) {
		...
		active_cons->uart = serial_bind_dev(sih,
			RTE_CONSOLE_UART_ID, RTE_CONSOLE_UART_TYPE,
			NULL, hnd_cons_uart_worklet, active_cons);

		/* dump whatever is already in the logbuf to the console */
		if (active_cons->uart != NULL) {
			serial_muxenab(active_cons->uart, UART_DEFAULT_PIN_MAP);
			serial_puts(active_cons);
#if defined(SAVERESTORE)
			if (SR_ENAB()) {
				BCM_REFERENCE(hnd_cons_sr_save);
				sr_register_save(sih, hnd_cons_sr_save, (void *)active_cons);
			}
#endif /* SAVERESTORE */
		}
	}
...
#if defined(RTE_CONS) && !defined(BCM_BOOTLOADER)
	if (!hnd_cons_add_cmd("?", hnd_cons_show_cmds, active_cons))
		return NULL;
#endif
#ifdef RTE_CONS
#if defined(RWL_DONGLE) || defined(UART_REFLECTOR)
	if (!hnd_cons_add_cmd("rwl", remote_cmd_function, active_cons))
		return NULL;
#endif
#endif /* RTE_CONS */
...
static void
hnd_cons_uart_proc(void *cbdata)
{
 ...
 * Also called from rte_arm.c in order to poll for CC UART input whenever
 * the host sends the dongle a Debug UART packet for ram dump
*/
void
hnd_cons_check(void)
{
	hnd_cons_uart_proc(active_cons);
}

void
hnd_uart_enable_cons_uart_output(bool enable)
{
	if (active_cons->uart_console_output == enable) {
		return;
	}

	active_cons->uart_console_output = enable;
}
...
putc(int c)
{
	...
	if (cd->state.log.buf != NULL) {
		...
		if (UART_ENAB() && MODINC(next_idx, cd->state.log.buf_size) ==
			cd->state.log.out_idx) {
...
static ccmd_t*
hnd_cons_find_cmd(cons_soft_t *cd, const char *name)
{
	ccmd_t* ccmd = cd->ccmd;

	while (ccmd != NULL) {
		if (strcmp(ccmd->name, name) == 0)
			return ccmd;
		ccmd = ccmd->next;
	}
	return NULL;
...
process_ccmd(char *line, uint len)
{
	...
	ccmd = hnd_cons_find_cmd(cd, argv[0]);
	if (ccmd != NULL) {
...
hnd_cons_add_cmd(const char *name, cons_fun_t fun, void *arg)
{
	...
	/* No duplicates */
	ccmd = hnd_cons_find_cmd(cd, name);
	if (ccmd != NULL) {
		ASSERT(ccmd->fun == fun);
		ASSERT(ccmd->arg == arg);
		return TRUE;
	...
	new = MALLOCZ(cd->osh, sizeof(*new));

	if (new == NULL) {
#ifdef BCMDBG
		printf("Mem-alloc Failure: %s, Name: %s\n", __FUNCTION__, name);
#endif  /* BCMDBG */
		return FALSE;
	}
...
#endif /* RTE_CONS && !defined(BCM_BOOTLOADER */
...
BCMATTACHFN(hnd_cons_log_init)(osl_t *osh)
{
	...
	if (cons0->state.log.buf == NULL) {
		MFREE(osh, cons0, sizeof(cons_soft_t));
		cons0 = NULL;
		return BCME_ERROR;
	}
#else
	cons0 = &cons0_test;
	cons0->state.log.buf = cons0_testbuf;
#endif /* BCMSTANDALONE_TEST */
...
static bool
hnd_cons_uart_worklet(void *cbdata)
{
	cons_soft_t *cd = (cons_soft_t *)cbdata;

	if (cd != 0 && cd->uart != NULL) {
...
void
hnd_uart_tx_crc32(uint8 *buf, int len, uint32 crc32)
{
	cons_soft_t *cd = active_cons;
	int i = 0;
	...
	if (cd->uart) {
		for (i = 0; i < len; i++) {
			ch = buf[i];
			serial_putc(cd->uart, ch);
			crc32 = hndcrc32(&ch, 1, crc32);
		}

		p = (uint8 *)&crc32;
		for (i = 0; i < sizeof(crc32); i++) {
			serial_putc(cd->uart, *p++);
		}
	}
}

void *
hnd_register_uart_rx_cb(void *ctx, uart_rx_cb_t cb)
{
	cons_soft_t *cd = active_cons;
...
void
process_command_from_uart(uint c)
{
cons_soft_t *cd = active_cons;

	cd->state.cbuf[cd->state.cbuf_idx] = c;
	cd->state.cbuf_idx ++;
}
