 ...
 *
 * $Id: phy_radio.c 790132 2020-08-17 14:55:45Z $
 */
...
void
WLBANDINITFN(phy_radio_por_inform)(phy_info_t *pi)
{
	pi->phy_init_por = TRUE;
}

void
phy_radio_runbist_config(phy_info_t *pi, bool start_end)
{
	if (start_end == OFF) {
		phy_radio_por_inform(pi);
	}
}
...
void
phy_radio_get_valid_chanvec(phy_info_t *pi, int band, chanvec_t *valid_chans)
{
	phy_type_radio_fns_t *fns = pi->radioi->fns;
	int valid_count = 0;
	chanspec_band_t chspec_band;

	if (fns->get_valid_chanvec != NULL) {
		switch (band) {
		case WLC_BAND_2G:
		...
		case WLC_BAND_6G:
			...
			break;
		default:
			PHY_ERROR(("wl%d: %s: invalid band argument %d\n", pi->sh->unit,
				__FUNCTION__, band));
			ASSERT(FALSE);
			return;
		}

		bzero(valid_chans, sizeof(*valid_chans));
		valid_count = fns->get_valid_chanvec(fns->ctx, chspec_band, valid_chans);
	}

	if (!valid_count)
		phy_utils_chanspec_band_validch(pi, band, valid_chans);
}

static bool
phy_radio_freq_in_band(uint16 freq, chanspec_band_t band)
{
	switch (band) {
	case WL_CHANSPEC_BAND_2G:
		return (freq >= 2412 && freq <= 2484);
	case WL_CHANSPEC_BAND_5G:
		return (freq >= 4920 && freq <= 5895);
	case WL_CHANSPEC_BAND_6G:
		return (freq >= 5935 && freq <= 7215);
	default:
		ASSERT(FALSE);
	}
	return FALSE;
...
int
phy_radio_add_valid_20MHz_chan(phy_radio_info_t *ri, uint16 perband_radiorev, chanspec_band_t band,
	uint16 freq, uint8 chan, chanvec_t *valid_chans)
{
	if ((RADIOREV(ri->pi->pubpi->radiorev) >= perband_radiorev ||
	     phy_radio_freq_in_band(freq, band)) &&
	    wf_valid_20MHz_chan(chan, band)) {
		setbit(valid_chans->vec, chan);
		return 1;
	}
	return 0;
}
