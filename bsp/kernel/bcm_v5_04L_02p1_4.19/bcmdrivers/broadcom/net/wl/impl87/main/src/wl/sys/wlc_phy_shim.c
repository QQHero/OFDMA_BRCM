 ...
 *
 * $Id: wlc_phy_shim.c 809675 2022-03-22 05:14:17Z $
 */
...
#if defined(WL_PSMR1)
void
wlapi_bmac_write_shm1(wlc_phy_shim_info_t *physhim, uint offset, uint16 v)
{
	if (PSMR1_ENAB(physhim->wlc_hw)) {
		wlc_bmac_write_shm1(physhim->wlc_hw, offset, v);
	}
}
#endif
...
uint32
wlapi_get_svmp_addr_hwsch_cmd_log(wlc_phy_shim_info_t *physhim)
{
	wlc_hw_info_t *wlc_hw = (wlc_hw_info_t *)physhim->wlc_hw;
	return VASIP_SHARED_OFFSET(wlc_hw, hwsch_cmd_log);
}

uint32
wlapi_get_svmp_addr_hwsch_v2m_buf(wlc_phy_shim_info_t *physhim)
{
	wlc_hw_info_t *wlc_hw = (wlc_hw_info_t *)physhim->wlc_hw;
	return VASIP_SHARED_OFFSET(wlc_hw, hwsch_v2m_buf);
}
...
void
wlapi_set_sc_sz(wlc_phy_shim_info_t *physhim, uint32 sc_sz)
{
	wlc_hw_info_t *wlc_hw = physhim->wlc_hw;
	/* used during bcm_init() that's called from wl up */
	wlc_hw->sc_buf_info->sz = sc_sz;
}

void
wlapi_get_sc_info(wlc_phy_shim_info_t *physhim, uint32 *sc_start, uint32 *sc_sz)
{
	wlc_hw_info_t *wlc_hw = physhim->wlc_hw;

	*sc_start = wlc_hw->sc_buf_info->start;
	*sc_sz	  = wlc_hw->sc_buf_info->sz;
}
...
void
wlapi_set_cts_txcoremask(wlc_phy_shim_info_t *physhim, uint16 *mask)
{
	wlc_info_t *wlc = physhim->wlc_hw->wlc;

	// only set dot11ag and cck txcoremask
	wlc_write_shm(wlc, (wlc->stf->shmem_base + (CCK_IDX*2)), mask[0]);
	wlc_write_shm(wlc, (wlc->stf->shmem_base + (OFDM_IDX*2)), mask[1]);
}

void
wlapi_get_cts_txcoremask(wlc_phy_shim_info_t *physhim, uint16 *mask)
{
	wlc_info_t *wlc = physhim->wlc_hw->wlc;

	// only get dot11ag and cck txcoremask
	mask[0] = wlc_read_shm(wlc, (wlc->stf->shmem_base + (CCK_IDX*2)));
	mask[1] = wlc_read_shm(wlc, (wlc->stf->shmem_base + (OFDM_IDX*2)));
}
