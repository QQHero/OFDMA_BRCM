 ...
 *
 * $Id: wlconf.c 810813 2022-04-14 17:57:20Z $
 */
...
#define WL_MSCHED_IOVAR_SETINT(ifname, iovar, subcmd, val)			\
	if ((ret = wl_msched_iovar_setint(ifname, iovar, subcmd, val)))	\
		...
		"iovar \"%s %s\" to 0x%x failed, err = %d\n", \
		        __FUNCTION__, __LINE__, ifname, iovar, subcmd, val, ret);
#define WL_MSCHED_IOVAR_SETMAXN(ifname, iovar, subcmd, bw, val)			\
		if ((ret = wl_msched_iovar_setmaxn(ifname, iovar, subcmd, bw, val))) \
			fprintf(stderr, "%s:%d:(%s): setting maxn "	\
			"iovar \"%s %s\" bw %d to 0x%x failed, err = %d\n", \
					__FUNCTION__, __LINE__, ifname, iovar, subcmd, bw, val, ret);
...
#define WL_MSCHED_IOVAR_SETMAXN(ifname, iovar, subcmd, bw, val) \
		(ret = wl_msched_iovar_setmaxn(ifname, iovar, subcmd, bw, val))
#ifdef __CONFIG_DHDAP__
...
/*  NBR_AP TBTT OFFSETfield(1) + BSSID(6)+SHORTSSID (4)+BSS(1) + TPE_PSD(1) 13 BYTES */
#define NBR_AP_TBTT_BSSID_SHORT_SSID_BSS_TPE_LEN	13U
...
wlconf_configure_upr_and_fd(char *name, char *prefix, uint8 bandtype, struct bsscfg_list *bclist)
{
	...
	if ((str = nvram_get(strcat_r(prefix, "nbr_discovery_cap", tmp))) != NULL) {
	...
	if ((str = nvram_get(strcat_r(prefix, "upr_fd_enable", tmp))) != NULL) {
		upr_fd_val = atoi(str);
	}

	if ((str = nvram_get("rnr_tbtt_len")) != NULL) {
		rnr_tbtt_len = atoi(str);
		if ((rnr_tbtt_len != NBR_AP_TBTT_BSSID_SHORT_SSID_LEN) &&
			(rnr_tbtt_len != NBR_AP_TBTT_BSSID_SHORT_SSID_BSS_TPE_LEN)) {
			/* only 11 or 13 byte TBTT LEN supported */
			rnr_tbtt_len = NBR_AP_TBTT_BSSID_SHORT_SSID_BSS_TPE_LEN;
		}
	} else {
		rnr_tbtt_len = NBR_AP_TBTT_BSSID_SHORT_SSID_BSS_TPE_LEN;
	...
	if ((str = nvram_get(strcat_r(prefix, "upr_fd_method", tmp))) != NULL) {
	...
	wl_iovar_setint(name, "rnr_tbtt_len", rnr_tbtt_len);
...
/* Setup phytype/nband */
static void
wlconf_set_current_phytype_nband_into_nvram(char *name, char *prefix)
{
	/* During first system boot up from default setting, if no one update
	   correct wlX_phytype and wlX_nband before.
	   The function will prepare the nvram for later use */
	int val, ret, phytype, bandtype;
	char buf[WLC_IOCTL_SMLEN*2];
	char tmp[50] ,tmp2[50];
	char *str = NULL;
	char *var = NULL;
	...
	if (ret) {
		...
		WL_IOCTL(name, WLC_GET_PHYLIST, tmp2, sizeof(tmp2));
		nvram_set(strcat_r(prefix, "phytypes", tmp), tmp2);

		val = WLCONF_STR2PHYTYPE(tmp2[0]);
		val = WLCONF_PHYTYPE2BAND(val);
		WL_SETINT(name, WLC_SET_BAND, val);
	}
	...
	/* Check errors again (will cover band-locked cards) */
	if (ret) {
		int list[3];
	...
	/* update NVRAM with correct "nband" */
	snprintf(tmp2, sizeof(tmp2), "%d", bandtype);
	nvram_set(strcat_r(prefix, "nband", tmp), tmp2);
...
extern void fbt_restore_defaults(char *in_prefix, bool uap_deinit_fbt);
...
wlconf_set_fbt(struct bsscfg_list *bclist)
{
	...
	for (i = 0; i < bclist->count; i++) {
		...
		WLCONF_DBG("i[%d] name[%s] prefix[%s] map_mode[%d] wbd_fbt[%d] fbt[%d] "
			"%sakm[%s]\n", i, bsscfg->ifname, bsscfg->prefix, map_mode,
			wbd_fbt, fbt, strnvval);
		...
		if (fbt <= 0) {
			...
			if (map_mode <= 0) {
				...
				fbt_restore_defaults(bsscfg->prefix, uap_deinit_fbt);
				uap_deinit_fbt = FALSE;
				WLCONF_DBG("MAP Disabled. Unset FBT NVRAMs.\n");
			} else if (wbd_fbt == 0) {
				WLCONF_DBG("MAP Enabled. WBD_FBT Disabled. Keep FBT NVRAMs as.\n");
			} else {
				/* FBT is enabled from WBD */
				fbt = 1;
				WLCONF_DBG("MAP Enabled. WBD_FBT Enabled.\n");
			}
		}

		/* If akm don't contains psk2ft or saeft : Skip Setting FBT IOVARs */
		if ((find_in_list(strnvval, "psk2ft") == NULL) &&
			(find_in_list(strnvval, "saeft") == NULL)) {
			WLCONF_DBG("akm[%s] don't have psk2ft OR saeft. "
				"Skip Setting FBT IOVAR.\n", strnvval);
			continue;
		}
		...
		if (ret != 0) {
			WLCONF_DBG("FBT Set Failed. Skip Setting Other FBT IOVARs.\n");
			continue;
		}

		/* If FBT is disabled continue */
		if (fbt <= 0) {
			WLCONF_DBG("FBT OR MAP Disabled. Skip Setting Other FBT IOVARs.\n");
			continue;
		}
	...
	if (uap_deinit_fbt == FALSE) {
		/* indicates FBT NVRAMs were restored to defaults */
		nvram_commit();
	}
	return 1;
}
...
wlconf_overwrite_txbf_cap_nvram(char *name, char *prefix)
{
	...
	if (revinfo.corerev >= 129 && revinfo.corerev <= 132) {
...
#ifdef CMWIFI_RDKB
/* This function used to apply nvram "oper_stands" used by RDKB release */
static int
wlconf_oper_stands(char *prefix, int bandtype, bool cap_11ax)
{
	char *oper_stands;
	char tmp[100], buf[80];
	...
	switch (bandtype) {
	case WLC_BAND_2G:
		oper_stands = (cap_11ax ? "g,n,ax" : "g,n");
		break;
	case WLC_BAND_5G:
		oper_stands = (cap_11ax ? "a,n,ac,ax" : "a,n,ac");
		break;
	case WLC_BAND_6G:
		oper_stands = "ax";
		break;
	default:
		WLCONF_DBG("Invalid bandtype %d\n", bandtype);
		return -1;
	}
	snprintf(buf, sizeof(buf), "%s", oper_stands);
	nvram_set(strcat_r(prefix, "oper_stands", tmp), buf);
	return 0;
}
#endif /* CMWIFI_RDKB */
...
wlconf(char *name)
{
...
#endif
	{
		...
		wlconf_set_current_phytype_nband_into_nvram(name, prefix);
		...
		/* For debug_monitor application */
		if (nvram_get("watchdog") == NULL)
			nvram_set("watchdog", "4000");

#if defined(BCA_CPEROUTER)
		/* For rxchain_pwrsave */
		if (cap_11ax && rxchain_pwrsave) {
			(void) strcat_r(prefix, "rxchain_pwrsave_stas_assoc_check", tmp);
			nvram_set(tmp, "0");
		}
#endif
	...
	if (str && str[0]) {
		val = atoi(str);
		memset(&mbssid_cfg, 0, sizeof(mbssid_cfg));
		mbssid_cfg.version = WL_MBSSID_VER;
		mbssid_cfg.length = sizeof(wl_mbssid_t);
		mbssid_cfg.config = val;
		WL_IOVAR_SET(name, "mbssid", &mbssid_cfg, sizeof(mbssid_cfg));
	}
...
#define MAX_PCIE_SERDES_COMMAND	9
	...
	for (i = 0; i < bclist->count; i++) {
		...
		if (map_mode <= 0) {
			map_val = map_profile = 0;
		}

		if (strtol(nvram_safe_get("map_r1_cert_compatible"), NULL, 0) == 1) {
			/* For profile 1 certification, profile 2 agents should not send profile 2
			 * IEs in assoc packets. This is required till it is fixed in testbed
			 */
			WL_BSSIOVAR_SETINT(name, "map_profile", bsscfg->idx, 1);
		} else {
		 ...
		 */
		if ((map_mode > 0) && (map_profile >= 2) && (map_val & 0x02) &&
		...
		/* Enable SAE H2E for 6G BSSes/STA, if security has SAE */
		str = nvram_safe_get(strcat_r(bsscfg->prefix, "akm", tmp));
		if ((mbssid_band == WLC_BAND_6G) && (find_in_list(str, "sae"))) {
			nvram_set(strcat_r(bsscfg->prefix, "sae_pwe", tmp), "1");
		}
	}
	...
	for (i = 0; i < bclist->count; i++) {
		...
		if (sta || psta || psr || apsta || wet) {
			...
			/* set the scan parameters of station */
			strcat_r(bsscfg->prefix, "scan_unassoc_time", tmp);
			val = atoi(nvram_safe_get(tmp));
			if (val > 0)
				WL_BSSIOVAR_SETINT(name, "scan_unassoc_time", bsscfg->idx, val);
			strcat_r(bsscfg->prefix, "scan_nprobes", tmp);
			val = atoi(nvram_safe_get(tmp));
			if (val > 0)
				WL_BSSIOVAR_SETINT(name, "scan_nprobes", bsscfg->idx, val);
		}
	...
	*(next = buf) = '\0';
	...
	if ((country && country[0] != '\0') && (country_rev && country_rev[0] != '\0')) {
...
#else /* CMWIFI_RDKB */
		/* Update country code iovar if acsd2 is started AND nvram is not equal to iovar
		 * country code. Note that settting country code iovar resets channel to a default
		 * value, i.e, 1 or 36. This check for CMWIFI_RDKB prevents wlconf wlX up
		 * overwriting the channel selected by acsd2 autochannel command.
		 */
		char* tmp_str = nvram_get("acsd2_started");
		wl_iovar_get(name, "country", &country_spec, sizeof(country_spec));
		if (((tmp_str == NULL) || !strcmp(tmp_str, "0")) &&
			(strcmp(country, country_spec.ccode) ||
			atoi(country_rev) != country_spec.rev)) {
			strncpy(country_spec.country_abbrev, country, WLC_CNTRY_BUF_SZ-1);
			country_spec.country_abbrev[WLC_CNTRY_BUF_SZ-1] = '\0';
			strncpy(country_spec.ccode, country, WLC_CNTRY_BUF_SZ-1);
			country_spec.ccode[WLC_CNTRY_BUF_SZ-1] = '\0';
		...
		/* Add also country abbreviation override support */
		(void) strcat_r(prefix, "country_abbrev_override", tmp);
		country = nvram_get(tmp);
		if (country && country[0] != '\0') {
			WL_IOVAR_SET(name, "country_abbrev_override",
				country, strnlen(country, WLC_CNTRY_BUF_SZ-1) + 1);
		} else {
			WL_IOVAR_SET(name, "country_abbrev_override", "", 1);
		}
	} else {
	...
	/* set bw_switch_160 only for 43684b1 */
	if ((rev.corerev == 129) && (rev.coreminorrev < 2)) {
	...
	*/
	...
	/* Overwrite rateset per supported rate bitmap control if enabled */
	rateset_overwrite_by_supportedRatesBitmap(name, prefix);
	...
	/* Set maxn for UL OFDMA */
	str = nvram_safe_get(strcat_r(prefix, "umsched_maxn", tmp));
	if (str) {
		char *ptr;
		int bw;
		while (*str) {
			...
			if (ptr) {
				str = ++ptr;
				val = atoi(str);
				WL_MSCHED_IOVAR_SETMAXN(name, "umsched", "maxn", bw, val);
				ptr = strstr(str, ",");
				if (ptr) {
					str = ++ptr;
				} else break;
	...
	/* Set maxn for DL OFDMA */
	str = nvram_safe_get(strcat_r(prefix, "msched_maxn", tmp));
	if (str) {
		char *ptr;
		int bw;
		while (*str) {
			...
			if (ptr) {
				str = ++ptr;
				val = atoi(str);
				WL_MSCHED_IOVAR_SETMAXN(name, "msched", "maxn", bw, val);
				ptr = strstr(str, ",");
				if (ptr) {
					str = ++ptr;
				} else break;
	...
	str = nvram_safe_get(strcat_r(prefix, "edcrs_hi_event_mode", tmp));
	if (str[0] && ((str[0] == '-' && (str[1] > '0' && str[1] <= '9')) ||
			(str[0] >= '0' && str[0] <= '9'))) {
		WL_IOVAR_SETINT(name, "edcrs_hi_event_mode", atoi(str));
	}

	str = nvram_safe_get(strcat_r(prefix, "edcrs_txbcn_inact_thresh", tmp));
	if (str[0] && str[0] >= '1' && str[0] <= '9') {
		WL_IOVAR_SETINT(name, "edcrs_txbcn_inact_thresh", atoi(str));
	}
...
#endif /* CMWIFI_RDKB && BCA_CPEROUTER_RDK */
			ea = maclist->ea;
...
#ifdef CMWIFI_RDKB
	/* Set default oper_stands when it is missing */
	if (nvram_get(strcat_r(prefix, "oper_stands", tmp)) == NULL)
		wlconf_oper_stands(prefix, bandtype, cap_11ax);
#endif /* CMWIFI_RDKB */
...
wlconf_start(char *name)
{
		...
		if (ap || apsta || sta || wet) {
			for (ii = 0; ii < MAX_BSS_UP_RETRIES; ii++) {
				...
				if (wl_ap_build) {
					...
					perc = atoi(nvram_safe_get(strcat_r(bsscfg->prefix,
						"atm_bssperc", tmp)));
					if (perc) {
						WL_BSSIOVAR_SETINT(name, "atm_bssperc",
							bsscfg->idx, perc);
					}
				}
				else {
	...
	if (ap)
	{
		str = nvram_safe_get(strcat_r(prefix, "bcnprs_txpwr_offset", tmp));
		if (str[0] != '\0') {
			val = atoi(str);
	...
	if (ap || apsta) {
		...
		for (i = 0; i < bclist->count; i++) {
			...
			ftm_flagsp = nvram_safe_get(strcat_r(bsscfg->prefix, "ftm_flags",
					ftm_flags));
			ftm_modep = nvram_safe_get(strcat_r(bsscfg->prefix, "ftm_mode", ftm_mode));
			lcip = nvram_safe_get(strcat_r(bsscfg->prefix, "ftm_lci", lci_str));
			civicp = nvram_safe_get(strcat_r(bsscfg->prefix, "ftm_civic", civic_str));

			/* set ftm enable/disable */
			if (ftm_modep && (strlen(ftm_modep) > 0)) {
				val = atoi(ftm_modep);
			}

			/* set ftm config flags */
			if (ftm_flagsp && (strlen(ftm_flagsp) > 0)) {
				flags = (uint32)strtoul(ftm_flagsp, NULL, 16);
			}

			if ((wlconf_ftm_enable(name, bsscfg, val) == BCME_OK) &&
			    (val != 0) && (flags != 0)) {
				/* flags=0 is invalid; flags can't set unless ftm is enabled */
				wlconf_ftm_flags(name, bsscfg, flags);
			}

			if ((lcip != NULL) && (strlen(lcip) > 0)) {
				wlconf_rrm_self_lci_civic(name, bsscfg,
					WL_RRM_CONFIG_SET_LCI, lcip);
			}
			if ((civicp != NULL) && (strlen(civicp) > 0)) {
				wlconf_rrm_self_lci_civic(name, bsscfg,
