 ...
 *
 * $Id: phy_btcx.c 807418 2022-01-19 19:08:05Z $
 */
#include <phy_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
void
wlc_phy_set_femctrl_bt_wlan_ovrd(phy_info_t *pi, int8 state)
{
	phy_type_btcx_fns_t *fns = pi->btcxi->priv->fns;
...
int8
wlc_phy_get_femctrl_bt_wlan_ovrd(phy_info_t *pi)
{
	phy_type_btcx_fns_t *fns = pi->btcxi->priv->fns;
	int8 state = AUTO;
...
int
phy_btcx_set_mode(phy_info_t *pi, int btc_mode)
{
	phy_type_btcx_fns_t *fns = pi->btcxi->priv->fns;
	PHY_TRACE(("%s\n", __FUNCTION__));
...
#ifdef WL_UCM
int
phy_btcx_ucm_update_siso_resp_offset(phy_info_t *pi, int8 *siso_resp_pwr, uint8 len)
{
...
void
phy_btcx_ucm_update_only_ack_pwr_offset(phy_info_t *pi)
{
	if (UCM_ENAB(pi->btcxi)) {
		phy_type_btcx_fns_t *fns = pi->btcxi->priv->fns;
		PHY_TRACE(("%s\n", __FUNCTION__));
...
int
phy_btcx_ucm_set_desense_rxgain(phy_info_t *pi, uint band, uint8 num_cores,
		uint8 *desense_array)
{
	phy_btcx_info_t *btcxi = pi->btcxi;
	if (UCM_ENAB(btcxi)) {
