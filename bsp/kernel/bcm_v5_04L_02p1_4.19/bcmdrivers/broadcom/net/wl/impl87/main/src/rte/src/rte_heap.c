 ...
 *
 * $Id: rte_heap.c 799458 2021-05-31 13:05:39Z $
 */
...
static osl_ext_lock_t	heap_lock;	/* Heap protection */
...
arena_merge_segment()
{
	...
	/* Sort the array */
	for (i = 1; i < arena_segment_count; i++)
	{
		j = i;
		while (j > 0 && arena_segments[j - 1].base > arena_segments[j].base)
		{
			/* Swap A[j] and A[j - 1] */
			tmp = arena_segments[j];
			arena_segments[j] = arena_segments[j - 1];
			arena_segments[j - 1] = tmp;
			j--;
		}
	}
	...
	for (i = 1; i < arena_segment_count; i++)
	{
		arena_segment_t prev = arena_segments[i - 1];
		arena_segment_t curr = arena_segments[i];
		if ((uint32)prev.base + (uint32)prev.size == (uint32)curr.base)
		{
			/* Combine contiguous block */
			arena_segments[i - 1].size += arena_segments[i].size;
			/* Shuffle the rest down */
			for (j = i + 1; j < arena_segment_count; j++)
			{
				arena_segments[j - 1] = arena_segments[j];
			}
			arena_segment_count--;
			i--;
		}
...
BCMATTACHFN(hnd_arena_init)(uintptr base, uintptr lim)
{
	...
	/* create lock for critical section locking */
	if (osl_ext_lock_create(&heap_lock, "heap") != OSL_EXT_SUCCESS) {
		return FALSE;
	}
...
hnd_arena_add(uint32 base, uint size)
{
	...
	if (osl_ext_lock_acquire(&heap_lock) != OSL_EXT_SUCCESS) {
		return 0;
	}
	...
	osl_ext_lock_release(&heap_lock);
...
get_viable_persist_segment(uint32 size)
{
	...
	if (this != NULL) {
		...
		if (curr != NULL)
		{
			...
			prev = find_preceeding_list_block(&free_mem, curr);
			SET_MEM_T_NEXT(prev, new);
			/* create new persist header and link into monotonical list ordering */
			prev = find_preceeding_list_block(&persist_mem, curr);
...
#endif
{
	...
	if ((1 << alignbits) > MIN_ALIGN) {
		...
		return hnd_malloc_align(size, alignbits);
	}
	...
	if ((persist = get_viable_persist_segment(size)) != NULL) {
		...
		prev = find_preceeding_list_block(&free_mem, free_follow);
...
hnd_free(void *where)
{
	return hnd_free_callsite(where, CALL_SITE);
}

int
hnd_free_callsite(void *where, void *callsite)
{
	uint32 w = (uint32)where;
	...
	BUZZZ_LVL4(HND_FREE, 1, (uint32)callsite);
	...
	for (i = 0; i < arena_segment_count; i++) {
		block_start = (uint32)arena_segments[i].base;
		block_size = arena_segments[i].size;
		if ((w >= block_start) && (w < (block_start + block_size))) {
			break;
		}
	}
	if (i == arena_segment_count) {
	...
	while ((this = GET_MEM_T_NEXT(prev)) != NULL) {
			...
			((uint32)this + GET_MEM_T_SIZE(this) + sizeof(mem_t))))
		{
			...
			printf("pointer %p in persist block: %p sz: 0x%08x ra: %p\n", where, this,
				GET_MEM_T_SIZE(this), (void *)callsite);
			if (ATTACH_PART_RECLAIMED())
				HND_DIE();
			return 0;
		}
...
hnd_memavail(void)
{
	...
	osl_ext_lock_acquire(&heap_lock);
...
hnd_get_heapuse(memuse_info_t *mu)
{
	...
	/* Pagetable alignment can cause padding in bss section.
	 * To be able to reclaim this padding, the pagetables have been moved
	 * behind the pre-attach reclaim area.
	 * Although the pagetables are no longer part of the bss section, they need to
	 * be taken into account to accurately determine the max memory in use.
	 */
	mu->bss_len += hnd_get_pagetable_size();
...
hnd_memcheck(const char *file, int line)
{
	...
	for (i = 0; i < arena_segment_count; i++) {
		bool done = FALSE;
		uint size_expected = arena_segments[i].size;
		uint max_segment_address = (uint32)arena_segments[i].base + arena_segments[i].size;
		uint size_calculated = 0;
		int last_block_type = -1;
		this = arena_segments[i].base;
		while (!done) {
			if ((uint32)this > max_segment_address) {
				...
				printf("this %p, max 0x%06x, seg_base %p, seg_size %d\n",
					this, max_segment_address, arena_segments[i].base,
					arena_segments[i].size);
				HND_DIE();
			}
			if ((uint32)this + GET_MEM_T_SIZE(this) > max_segment_address) {
				...
				printf("this %p, size %d max 0x%06x, seg_base %p, seg_size %d\n",
					...
					arena_segments[i].base, arena_segments[i].size);
				HND_DIE();
			}
			...
			if (last_block_type == MEM_T_FREE_BLOCK &&
				...
				printf("this %p, seg_base %p, seg_size %d\n",
					this, arena_segments[i].base, arena_segments[i].size);
				HND_DIE();
			}
		...
		if (size_calculated != size_expected) {
			...
			printf("seg_base %p, seg_size %d, calulated_size %d\n",
				arena_segments[i].base, arena_segments[i].size, size_calculated);
			HND_DIE();
		}
	}
