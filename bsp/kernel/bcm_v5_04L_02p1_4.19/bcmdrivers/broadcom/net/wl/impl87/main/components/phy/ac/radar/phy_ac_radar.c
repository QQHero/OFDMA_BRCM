 ...
 *
 * $Id: phy_ac_radar.c 809527 2022-03-17 05:17:00Z $
 */
...
static void phy_ac_radar_init_group1(phy_info_t *pi, uint16 bw_idx);
static void phy_ac_radar_init_group2(phy_info_t *pi, uint16 bw_idx);
...
static const wl_radar_thr_t BCMATTACHDATA(wlc_phy_radar_thresh_axphy_6715A0) = {
	WL_RADAR_THR_VERSION,
	0x6a8, 0x20, 0x6a8, 0x20, 0x6ac, 0x20, 0x6a8, 0x20, 0x6a8, 0x20, 0x6ac, 0x20,
	0x6ac, 0x20, 0x6b0, 0x20,
};

static const wl_radar_thr_t BCMATTACHDATA(wlc_phy_radar_thresh_axphy_6715B0) = {
	WL_RADAR_THR_VERSION,
	0x6a8, 0x20, 0x6a8, 0x20, 0x6ac, 0x20, 0x6ac, 0x20, 0x6a8, 0x20, 0x6b0, 0x20,
	0x6a8, 0x20, 0x6a8, 0x20,
};

static const wl_radar_thr_t BCMATTACHDATA(wlc_phy_radar_thresh_axphy_6756) = {
	WL_RADAR_THR_VERSION,
	0x6b0, 0x20, 0x6b0, 0x20, 0x6b0, 0x20, 0x6b0, 0x20, 0x6b0, 0x20, 0x6b0, 0x20,
	0x6b0, 0x20, 0x6b0, 0x20,
};
...
static const wl_radar_thr2_t BCMATTACHDATA(wlc_phy_radar_thresh2_axphy_6715A0) = {
	WL_RADAR_THR_VERSION,
	0x6b8, 0x20, 0x6b4, 0x20, 0x6b8, 0x20, 0x6b8, 0x20, 0x6b8, 0x20, 0x6bc, 0x20,
	0x6b8, 0x28, 0x6b4, 0x28, 0xa, 0xa, 0xa, 0x258, 0x258, 0x258, 0x258, 0x5124, 0xc,
	0xe, 0x1941, 0x19, 0x0, 0x14, 0x64, 0xf0, 0xfffb, 0x5
};

static const wl_radar_thr2_t BCMATTACHDATA(wlc_phy_radar_thresh2_axphy_6715B0) = {
	WL_RADAR_THR_VERSION,
	0x6b8, 0x20, 0x6b8, 0x20, 0x6b8, 0x20, 0x6c0, 0x20, 0x6c0, 0x20, 0x6bc, 0x20,
	0x6b4, 0x20, 0x6b4, 0x28, 0xa, 0xa, 0xa, 0x258, 0x258, 0x258, 0x258, 0x5124, 0xc,
	0xe, 0x1941, 0x19, 0x0, 0x14, 0x64, 0xf0, 0xfffb, 0x5
};
...
BCMATTACHFN(phy_radar_init_st)(phy_info_t *pi, phy_radar_st_t *st)
{
	...
	if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
		st->rparams.radar_thrs2 = wlc_phy_radar_thresh2_axphy_43684;
	} else if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
		st->rparams.radar_thrs2 = (RADIOREV(pi->pubpi->radiorev) < 2) ?
			wlc_phy_radar_thresh2_axphy_6715A0 : wlc_phy_radar_thresh2_axphy_6715B0;
	} else {
	...
	if (ACMAJORREV_0(pi->pubpi->phy_rev) || ACMAJORREV_4(pi->pubpi->phy_rev)) {
		...
		st->rparams.radar_thrs = (RADIOREV(pi->pubpi->radiorev) < 2) ?
			wlc_phy_radar_thresh_axphy_6715A0 : wlc_phy_radar_thresh_axphy_6715B0;
	} else if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
		st->rparams.radar_thrs = wlc_phy_radar_thresh_axphy_6756;
	...
	st->rparams.thrs2_iov_update = FALSE;
	...
	if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
		st->rparams.radar_args.t2_min = 28416;	/* 0x6f00 */
	} else {
	...
	if (TONEDETECTION)
		if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
			st->rparams.radar_args.max_span_lp = 45332;  /* 0xb114; 1, 3, 1, 20 */
		} else if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
			st->rparams.radar_args.max_span_lp = 45337;  /* 0xb114; 1, 3, 1, 25 */
		} else if (ACMAJORREV_GE32(pi->pubpi->phy_rev)) {
			st->rparams.radar_args.max_span_lp = 45324;  /* 0xb10c; 1, 3, 1, 12 */
		} else {	/* 4360 acphy */
			st->rparams.radar_args.max_span_lp = 45588;  /* 0xb214; 1, 3, 2, 20 */
		}
	else
...
WLBANDINITFN(phy_ac_radar_init)(phy_type_radar_ctx_t *ctx, bool on)
{
	...
	if (ACMAJORREV_47_130(pi->pubpi->phy_rev) || ACMAJORREV_131(pi->pubpi->phy_rev))
		ASSERT(bw_idx < 5);
	else
		ASSERT(bw_idx < 4);
	...
	if (((st->rparams.radar_args.feature_mask & RADAR_FEATURE_FCC_DETECT) != 0) &&
		((st->rparams.radar_args.feature_mask & RADAR_FEATURE_NEWJP_DETECT) == 0)) {
		st->rparams.radar_args.min_pw = 6;
	} else {
		st->rparams.radar_args.min_pw = 2;
	}
	...
	if (ACMAJORREV_0(pi->pubpi->phy_rev) || ACMAJORREV_4(pi->pubpi->phy_rev) ||
		...
		/* Group 1 includes rev0, rev4, rev32/33, rev51, rev128 and rev131 */
		phy_ac_radar_init_group1(pi, bw_idx);
	} else if (ACMAJORREV_47(pi->pubpi->phy_rev) || ACMAJORREV_129(pi->pubpi->phy_rev) ||
		ACMAJORREV_130(pi->pubpi->phy_rev) || ACMAJORREV_131(pi->pubpi->phy_rev) ||
		ACMAJORREV_GE47(pi->pubpi->phy_rev)) {
		/* Group 2 includes rev47, rev129, rev130 and ge47 */
		phy_ac_radar_init_group2(pi, bw_idx);
...
/* Group 1 includes rev0, rev4, rev32/33, rev51, rev128 and rev131 */
static void
phy_ac_radar_init_group1(phy_info_t *pi, uint16 bw_idx)
{
	uint16 k;
	uint16 radar_phyreg_rev0_4[10][4] =
	...
	uint16 radar_phyreg_rev32_33[19][4] =
	...
	uint16 radar_phyreg_rev51[11][4] =
		/* {regname, bw20_val, bw40_val, bw80_val} */
	...
	uint16 radar_phyreg_rev128[11][4] =
		/* {regname, bw20_val, bw40_val, bw80_val} */
	...
	if (ACMAJORREV_0(pi->pubpi->phy_rev) || ACMAJORREV_4(pi->pubpi->phy_rev)) {
		/* 4360 or 53573/47189 */
		for (k = 0; k < ARRAYSIZE(radar_phyreg_rev0_4); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_rev0_4[k][0],
				radar_phyreg_rev0_4[k][bw_idx]);
		}
	} else if (ACMAJORREV_32(pi->pubpi->phy_rev) || ACMAJORREV_33(pi->pubpi->phy_rev)) {
		for (k = 0; k < ARRAYSIZE(radar_phyreg_rev32_33); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_rev32_33[k][0],
				radar_phyreg_rev32_33[k][bw_idx]);
		}
	} else if (ACMAJORREV_51(pi->pubpi->phy_rev)) {
		for (k = 0; k < ARRAYSIZE(radar_phyreg_rev51); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_rev51[k][0],
				radar_phyreg_rev51[k][bw_idx]);
		}
	} else if (ACMAJORREV_128(pi->pubpi->phy_rev)) {
		for (k = 0; k < ARRAYSIZE(radar_phyreg_rev128); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_rev128[k][0],
				radar_phyreg_rev128[k][bw_idx]);
		}
	}
}
...
/* Group 2 includes rev47, rev129, rev130 and ge47 */
static void
phy_ac_radar_init_group2(phy_info_t *pi, uint16 bw_idx)
{
	uint16 k;
	uint16 radar_phyreg_rev47[20][5] =
	...
	uint16 radar_phyreg_rev129[11][4] =
		/* {regname, bw20_val, bw40_val, bw80_val} */
	...
	uint16 radar_phyreg_rev130[20][5] =
		/* {regname, bw20_val, bw40_val, bw80_val, bw160_val} */
		{{ACPHY_RadarBlankCtrl(pi->pubpi->phy_rev), 0xa419, 0xa432, 0xa464, 0xa4c8},
		{ACPHY_RadarMisc(pi->pubpi->phy_rev), 0x1, 0x1, 0x1, 0x1},
		{ACPHY_RadarMaLength(pi->pubpi->phy_rev), 0x08, 0x10, 0x1f, 0x0},
		{ACPHY_Radar_t2_min(pi->pubpi->phy_rev), 0x0, 0x0, 0x0, 0x0},
		{ACPHY_Radar_adc_to_dbm(pi->pubpi->phy_rev), 0xb4a3, 0xb4a3, 0xb4a3, 0x34a3},
		...
		{ACPHY_RadarT3Timeout(pi->pubpi->phy_rev), 0x258, 0x4b0, 0x960, 0x12c0},
		{ACPHY_RadarResetBlankingDelay(pi->pubpi->phy_rev), 0x19, 0x32, 0x64, 0xc8},
		{ACPHY_RadarDetectConfig2(pi->pubpi->phy_rev), 0x141, 0x341, 0x521, 0x741},
		{ACPHY_RadarBlankCtrl_SC(pi->pubpi->phy_rev), 0x8019, 0x8032, 0x8064, 0x80c8},
		{ACPHY_RadarMaLength_SC(pi->pubpi->phy_rev), 0x08, 0x10, 0x1f, 0x0},
		{ACPHY_Radar_adc_to_dbm_SC(pi->pubpi->phy_rev), 0x34a0, 0x34a0, 0x34a0, 0x34a0},
		{ACPHY_RadarBlankCtrl2_SC(pi->pubpi->phy_rev), 0xa000, 0xa000, 0xa000, 0xa000},
	...
	uint16 radar_phyreg_rev131[11][5] =
		/* {regname, bw20_val, bw40_val, bw80_val, bw160_val} */
		{{ACPHY_RadarBlankCtrl(pi->pubpi->phy_rev), 0xa419, 0xac32, 0xac64, 0xacc8},
		{ACPHY_RadarMisc(pi->pubpi->phy_rev), 0x1, 0x1, 0x1, 0x1},
		{ACPHY_RadarMaLength(pi->pubpi->phy_rev), 0x08, 0x10, 0x1f, 0x0},
		{ACPHY_Radar_t2_min(pi->pubpi->phy_rev), 0x0, 0x0, 0x0, 0x0},
		{ACPHY_Radar_adc_to_dbm(pi->pubpi->phy_rev), 0x34a3, 0x34a3, 0x34a3, 0x34a3},
		{ACPHY_RadarBlankCtrl2(pi->pubpi->phy_rev), 0x5f88, 0x5f88, 0x5f88, 0x5f88},
		{ACPHY_RadarDetectConfig1(pi->pubpi->phy_rev), 0x3237, 0x3237, 0x3237, 0x3237},
		{ACPHY_RadarT3BelowMin(pi->pubpi->phy_rev), 0x14, 0x14, 0x14, 0x14},
		{ACPHY_RadarT3Timeout(pi->pubpi->phy_rev), 0x258, 0x4b0, 0x960, 0x12c0},
		{ACPHY_RadarResetBlankingDelay(pi->pubpi->phy_rev), 0x19, 0x32, 0x64, 0xc8},
	...
	if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
		for (k = 0; k < ARRAYSIZE(radar_phyreg_rev47); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_rev47[k][0],
				radar_phyreg_rev47[k][bw_idx]);
		}
	} else if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
		for (k = 0; k < ARRAYSIZE(radar_phyreg_rev129); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_rev129[k][0],
				radar_phyreg_rev129[k][bw_idx]);
		}
	} else if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
		for (k = 0; k < ARRAYSIZE(radar_phyreg_rev130); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_rev130[k][0],
				radar_phyreg_rev130[k][bw_idx]);
		}
	} else if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
		for (k = 0; k < ARRAYSIZE(radar_phyreg_rev131); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_rev131[k][0],
				radar_phyreg_rev131[k][bw_idx]);
		}
	} else if (ACMAJORREV_GE47(pi->pubpi->phy_rev)) { /* for majorrev > 47 chips */
		for (k = 0; k < ARRAYSIZE(radar_phyreg_ge47); k++) {
			phy_utils_write_phyreg(pi, radar_phyreg_ge47[k][0],
			radar_phyreg_ge47[k][bw_idx]);
		}
	}
}
...
phy_ac_radar_war(phy_info_t *pi)
{
	uint16 radar_phyreg_war[][2] =
		...
		{ACPHY_RadarBlankCtrl(pi->pubpi->phy_rev), 0xa432},
		{ACPHY_Radar_adc_to_dbm(pi->pubpi->phy_rev), 0xb4a3},
		{ACPHY_RadarBlankCtrl2(pi->pubpi->phy_rev), 0x5e88},
		{ACPHY_RadarBlankCtrl2(pi->pubpi->phy_rev), 0x5c88},
		{ACPHY_RadarSubBandConfig2(pi->pubpi->phy_rev), 0x18a2}};
	...
	if (ACMAJORREV_32(pi->pubpi->phy_rev) || ACMAJORREV_33(pi->pubpi->phy_rev)) {
		...
		if (CHSPEC_IS80(pi->radio_chanspec) && (CHSPEC_CHANNEL(pi->radio_chanspec) <=
			...
			phy_utils_write_phyreg(pi, radar_phyreg_war[3][0], radar_phyreg_war[3][1]);
	} else if (ACMAJORREV_51(pi->pubpi->phy_rev)) {
		if (CHSPEC_IS20(pi->radio_chanspec) &&
			(CHSPEC_CHANNEL(pi->radio_chanspec) > WL_THRESHOLD_LO_BAND))
		...
		if (RADIOREV(pi->pubpi->radiorev) >= 2) {
			if (CHSPEC_IS20(pi->radio_chanspec))
				phy_utils_write_phyreg(pi, radar_phyreg_war[4][0],
					radar_phyreg_war[4][1]);
			if (CHSPEC_IS80(pi->radio_chanspec) &&
				(CHSPEC_CHANNEL(pi->radio_chanspec) > WL_THRESHOLD_LO_BAND))
				phy_utils_write_phyreg(pi, radar_phyreg_war[5][0],
					radar_phyreg_war[5][1]);
		} else {
			if (CHSPEC_IS20(pi->radio_chanspec) &&
				(CHSPEC_CHANNEL(pi->radio_chanspec) > WL_THRESHOLD_LO_BAND))
				phy_utils_write_phyreg(pi, radar_phyreg_war[4][0],
