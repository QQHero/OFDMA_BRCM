      ...
      nl_msgHdr = NLMSG_NEXT(nl_msgHdr, recvLen))
  {
    ...
    if ( i5_socket_type_netlink == psock->type )
    {
          ...
          (RTM_DELLINK == nl_msgHdr->nlmsg_type) )
      {
        ...
        if ( pif ) {
          /* Remove STP state change timer so that we can recreate if it is only in blocking mode */
          if (pif->u.sll.pSTPStateChangeTmr) {
            i5TimerFree(pif->u.sll.pSTPStateChangeTmr);
            pif->u.sll.pSTPStateChangeTmr = NULL;
          }

          if (i5BrUtilGetPortStpState(pif->u.sll.ifname) == 0) {
            pif->flags |= I5_SOCKET_FLAG_BLOCKING;
            if (pif->u.sll.pLldpProtoSock) {
              pif->u.sll.pLldpProtoSock->flags |= I5_SOCKET_FLAG_BLOCKING;
            }
            /* Create STP state change timer. This is to prevent frequent state toggling. Just
             * giving some more time to settle down
             */
            pif->u.sll.pSTPStateChangeTmr = i5TimerNew(
              I5_INTERFACE_STP_BLOCK_TIMEOUT_MSEC, i5InterfaceStpBlockingStateTimeout, pif);
          } else {
            pif->flags &= ~I5_SOCKET_FLAG_BLOCKING;
            if (pif->u.sll.pLldpProtoSock) {
              pif->u.sll.pLldpProtoSock->flags &= ~I5_SOCKET_FLAG_BLOCKING;
