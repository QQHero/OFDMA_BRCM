 ...
 *
 * $Id: phy_calmgr.c 787715 2020-06-09 22:23:11Z $
 */
#include <phy_cfg.h>
#include <typedefs.h>
#include <bcmutils.h>
 ...
 */
int
BCMINITFN(phy_calmgr_init)(phy_info_t *pi)
{
...
int
phy_calmgr_enable_initcal(phy_info_t *pi, bool initcal)
{
	phy_type_calmgr_fns_t *fns = pi->calmgri->fns;
...
int
phy_calmgr_set_glacial_timer(phy_info_t *pi, uint period)
{
...
uint32
phy_calmgr_get_cal_dur(phy_info_t *pi)
{
	return pi->cal_dur;
}
...
phy_calmgr_mphase_restart(phy_calmgr_info_t * calmgri)
{
	...
	pi->cal_info->cal_core = 0; /* Do per-core PAPD cal in multiphase cal */
	pi->cal_info->txcal_cmdidx = 0; /* needed in nphy only */
	return BCME_OK;
}
