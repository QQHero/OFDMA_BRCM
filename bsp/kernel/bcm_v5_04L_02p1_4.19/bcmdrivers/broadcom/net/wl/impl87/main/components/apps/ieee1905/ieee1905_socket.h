...
#define I5_SOCKET_FLAG_VLAN_DELETED 0x04  /* VLAN interface removed for the socket corresponding to
                                           * an deleted interface
                                           */
...
#define I5_SOCKET_IS_VLAN_DELETED(flags)  ((flags) & I5_SOCKET_FLAG_VLAN_DELETED)

/* Loopback IP address */
#define I5_SOCKET_LOOPBACK_IP	"127.0.0.1"

#define I5_VLAN_IFNAME_OFFSET 100
...
typedef struct {
  ...
  unsigned short prim_vlan_id;  /* Primary VLAN ID created on this interface */
  ieee1905_glist_t vlan_id_list;  /* List of i5_vlan_id_node type objects. Secondary VLANs created
                                   * on this interface
                                   */
} i5_vlan_ifr_node;
...
typedef struct {
  ...
  unsigned int          self_flags; /* Flags of type I5_SELF_FLAG_XXX */
  ...
  unsigned int          msgRtryTimeoutMsec; /* Tiemout to retry a message */
  unsigned int          msgRtryCnt;  /* Number of times a message to be retried */
#ifdef MULTIAP
  ...
  unsigned short vid_offset; /* VLAN ID to VLAN ifname suffix offset */
