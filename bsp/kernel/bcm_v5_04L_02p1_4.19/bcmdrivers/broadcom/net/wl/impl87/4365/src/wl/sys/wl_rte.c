 ...
 *
 * $Id: wl_rte.c 807811 2022-02-01 05:56:18Z $
 */
...
#endif /* P2PO || ANQPO || WL_MBO && !WL_MBO_DISABLED */
...
if (wl->arpi)
	...
	if (wl->gas) {
#if defined(WL11K_AP)&& defined(WL_MBO) && !defined(WL_MBO_DISABLED)
		wlc_rrm_update_gasi(wl->wlc, NULL);
#endif /* WL11K_AP && WL_MBO && !WL_MBO_DISABLED */
...
#ifdef PKTC_TX_DONGLE
		if (PKTC_ENAB(wl->pub)) {
			chainable = cfg_chainable &&
					wl_txframe_chainable(wl, bsscfg, p, head, wlif);
