...
int escand_get_perband_chanspecs(escand_chaninfo_t *c_info, chanspec_t input,
	char *buf, int length)
{
	return wl_iovar_getbuf(c_info->name, "chanspecs", &input, sizeof(chanspec_t),
		buf, length);
}

int escand_get_per_chan_info(escand_chaninfo_t *c_info, chanspec_t sub_chspec,
	char *buf, int length)
{
	return wl_iovar_getbuf(c_info->name, "per_chan_info", &sub_chspec,
		sizeof(chanspec_t), buf, length);
}
...
int escand_set_escan_params(escand_chaninfo_t *c_info, wl_escan_params_t *params,
	int params_size)
{
...
int escand_get_bwcap_info(escand_chaninfo_t *c_info, escand_param_info_t *param,
	int param_len, char *buf,
	int buf_len)
{
...
int escand_get_bssid(char *ifname, char *bssid)
{
	return wl_ioctl(ifname, WLC_GET_BSSID, bssid, ETHER_ADDR_LEN);
}

int escand_get_pktq_data(char *ifname, char *buf, wl_iov_mac_full_params_t *full_params)
{
	return wl_iovar_getbuf(ifname, "pktq_stats", full_params, sizeof(*full_params), buf,
		WLC_IOCTL_MAXLEN);
}
