 ...
 *
 * $Id: phy_ac_dccal.c 806317 2021-12-17 19:06:46Z $
 */
...
typedef struct _phy_ac_dccal_papd_special_phyregs {
	uint16 dccal_common;
	uint16 idacc_update_hw_reset_len;
	uint16 radio_pu_seq;
	uint16 DcFiltAddress;
	uint16 RxControl;
	uint16 RfCtrlCoreITRCtrl[PHY_CORE_MAX];
} phy_ac_dccal_papd_special_phyregs_t;
...
struct phy_ac_dccal_info {
	...
	phy_ac_dccal_papd_special_phyregs_t *dccal_papd_special_phyregs;
	...
	phy_dccal_info_t *dccali)
{
	...
	if ((info->dccal_papd_special_phyregs =
			phy_malloc(pi, sizeof(phy_ac_dccal_papd_special_phyregs_t))) == NULL) {
...
BCMATTACHFN(phy_ac_dccal_unregister_impl)(phy_ac_dccal_info_t *info)
{
	...
	if (info->dccal_papd_special_phyregs != NULL) {
		phy_mfree(pi, info->dccal_papd_special_phyregs,
			sizeof(phy_ac_dccal_papd_special_phyregs_t));
	}
	phy_mfree(pi, info, sizeof(phy_ac_dccal_info_t));
}
...
phy_ac_dccal_init(phy_info_t *pi)
{
	...
	if (ACMAJORREV_47(pi->pubpi->phy_rev) || ACMAJORREV_129(pi->pubpi->phy_rev)) {
		...
		// phy_ax_dccal_digcorr_init is called in phy_ac_dccal_3steps
	} else {
...
phy_ac_load_gmap_tbl(phy_info_t *pi)
{
	if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
		...
		// Add iLNAbyp entry on gmap index = 6, iLNAbyp gm = 6 & rout = 0
		gm =  wlc_phy_get_lna_gain_rout(pi, 6, GET_LNA_GAINCODE);
		rout = wlc_phy_get_lna_gain_rout(pi, 6, GET_LNA_ROUT);
		gmap_tbl[6] = (0x1400 | ((gm & 0x7) << 6) | ((rout & 0xf) << 2) | 3);
...
phy_ac_dccal_papd_special(phy_info_t *pi, int8 tia_init, uint8 core)
{
	...
	/* Save PHY registers (except dccal_control) before doing special DCC for PAPD */
	if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
		phy_ac_dccal_papd_special_save(pi);
	}
	...
	ASSERT(ACMAJORREV_129(pi->pubpi->phy_rev) || ACMAJORREV_130(pi->pubpi->phy_rev));
	...
	/* Restore PHY registers (except dccal_control) after special DCC for PAPD is done */
	if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
		phy_ac_dccal_papd_special_restore(pi);
		/* Re-init to restore dccal_control reg settings */
		phy_ax_dccal_digcorr_init(pi);
	}
...
phy_ac_dccal_restore(phy_info_t *pi)
{
	...
	FOREACH_CORE(pi, core) {
		...
		if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
			MOD_RADIO_REG_20708(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_pu, 0);
			MOD_RADIO_REG_20708(pi, TIA_DCDAC_REG2, core, tia_dcdac_pu, 0);
			MOD_RADIO_REG_20708(pi, TIA_DCDAC_REG2, core, tia_dcdac_scale, 6);
			MOD_RADIO_REG_20708(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_i, 0);
			MOD_RADIO_REG_20708(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_q, 0);
			MOD_RADIO_REG_20708(pi, TIA_DCDAC_REG1, core, tia_dcdac_i, 0);
			MOD_RADIO_REG_20708(pi, TIA_DCDAC_REG2, core, tia_dcdac_q, 0);
		} else if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
			MOD_RADIO_REG_20710(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_pu, 0);
			MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG2, core, tia_dcdac_pu, 0);
			MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG2, core, tia_dcdac_scale, 6);
			MOD_RADIO_REG_20710(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_i, 0);
			MOD_RADIO_REG_20710(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_q, 0);
			MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG1, core, tia_dcdac_i, 0);
			MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG2, core, tia_dcdac_q, 0);
		} else if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
			MOD_RADIO_REG_20707(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_pu, 0);
...
void
phy_ac_dccal_idacres_save(phy_info_t *pi)
{
	uint8 core, idx;
	uint16 tbl_id;
	uint16 idacres_size = 24;
	uint16 read_val;

	FOREACH_CORE(pi, core) {
		tbl_id = AXPHY_TBL_ID_IDAC_RES_TABLE0 + 32 * core;
		for (idx = 0; idx < idacres_size; idx++) {
			wlc_phy_table_read_acphy(pi, tbl_id, 1, idx, 16, &read_val);
			pi->u.pi_acphy->dccali->idacres_table[core][idx] = read_val;
		}
	}
}

void
phy_ac_dccal_idacres_restore(phy_info_t *pi)
{
	uint8 core, idx;
	uint16 tbl_id;
	uint16 idacres_size = 24;
	...
	FOREACH_CORE(pi, core) {
		tbl_id = AXPHY_TBL_ID_IDAC_RES_TABLE0 + 32 * core;
		for (idx = 0; idx < idacres_size; idx++) {
			write_val = pi->u.pi_acphy->dccali->idacres_table[core][idx];
			wlc_phy_table_write_acphy(pi, tbl_id, 1, idx, 16, &write_val);
		}
	}
}
...
phy_ax_dccal_digcorr_init(phy_info_t *pi)
{
	...
	if (CHSPEC_IS6G(pi->radio_chanspec) && ACMAJORREV_131(pi->pubpi->phy_rev)) {
		// Tight DC requirements so enable additional DC reduction for payload decode
		// Limit to 6G as there we have the least gain and for 2/5G we are past PRA
	...
	FOREACH_CORE(pi, core) {
		...
		if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
			// Chose gain index for INIT_GAIN as that is used for low-power decode
			tia_val = 15;
		} else {
			tia_val = 6;
		}
		...
		MOD_PHYREGCE(pi, dccal_control_8,  core, idacc_mag_select, 2);
		MOD_PHYREGCE(pi, dccal_control_1,  core, idacc_tia_init_00, tia_val);
		MOD_PHYREGCE(pi, dccal_control_1,  core, idacc_tia_init_01, tia_val);
		MOD_PHYREGCE(pi, dccal_control_2,  core, idacc_tia_init_02, tia_val);
		MOD_PHYREGCE(pi, dccal_control_2,  core, idacc_tia_init_03, tia_val);
		MOD_PHYREGCE(pi, dccal_control_2,  core, idacc_tia_init_04, tia_val);
		MOD_PHYREGCE(pi, dccal_control_2,  core, idacc_tia_init_05, tia_val);
		MOD_PHYREGCE(pi, dccal_control_3,  core, idacc_tia_init_06, tia_val);
		MOD_PHYREGCE(pi, dccal_control_3,  core, idacc_tia_init_07, tia_val);
		MOD_PHYREGCE(pi, dccal_control_3,  core, idacc_tia_init_08, tia_val);
		MOD_PHYREGCE(pi, dccal_control_3,  core, idacc_tia_init_09, tia_val);
		MOD_PHYREGCE(pi, dccal_control_4,  core, idacc_tia_init_10, tia_val);
		MOD_PHYREGCE(pi, dccal_control_4,  core, idacc_tia_init_11, tia_val);
		MOD_PHYREGCE(pi, dccal_control_4,  core, idacc_tia_init_12, tia_val);
		MOD_PHYREGCE(pi, dccal_control_4,  core, idacc_tia_init_13, tia_val);
...
phy_ax_dccal_digcorr_init_tiainit(phy_info_t *pi, int8 tia_init, uint8 core)
{
	...
	PHY_PAPD(("PAPD special dccal: core =%d, tia_init=%d\n", core, tia_init));
...
void phy_ax_dccal_digcorr_bwspecific_percore(phy_info_t *pi, uint8 core)
{
	...
	if (CHSPEC_IS80(pi->radio_chanspec)) {
		if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
		      ...
		      MOD_PHYREGCE(pi, dccal_control_53,  core, dcoe_abort_threshold, 600);
		}
	} else {
		if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
		      MOD_PHYREGCE(pi, dccal_control_1,  core, dcoe_abort_threshold, 50);
		} else if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
		      MOD_PHYREGCE(pi, dccal_control_53,  core, dcoe_abort_threshold, 200);
		}
	}
...
void phy_ax_dccal_digcorr_idacc_override(phy_info_t  *pi, uint8 core)
{
	...
	while ((retry <= num_rst_retry) && (cal_done < num_actv_cores)) {
		...
		if (READ_PHYREGFLDCE(pi, dccal_control_21, core,
			...
			READ_PHYREGFLDCE(pi, dccal_control_22, core,
				idac_cal_done_1) == 0xff) {
			cal_done++;
			PHY_PAPD(("dccal: idacc complete for core %d, retried = %d times,"
				" cal_done = %d\n",	core, retry, cal_done));
		} else {
			if (retry == num_rst_retry) {
				PHY_PAPD(("dccal: idacc timeout core%d at the final trial\n",
					core));
				// enable the DC fixed corr back since IDAC cal failed
				MOD_PHYREG(pi, DcFiltAddress, bypass_HT_VHT_HE_STF, 0);
			} else {
				PHY_PAPD(("dccal: idacc temporary timeout core%d."
					" Retrying...\n", core));
			}
		}
		retry++;
	...
	PHY_PAPD(("PAPD special dccal: core = %d, idac0_i = %d,"
	...
	if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
		...
		MOD_RADIO_REG_20708(pi, TIA_DCDAC_REG2, core, tia_dcdac_pu, 1);
		MOD_RADIO_REG_20708(pi, TIA_DCDAC_REG2, core, tia_dcdac_scale, 6);
		MOD_RADIO_REG_20708(pi, TIA_DCDAC_REG1, core, tia_dcdac_i, idac0_i);
		MOD_RADIO_REG_20708(pi, TIA_DCDAC_REG2, core, tia_dcdac_q, idac0_q);
		MOD_RADIO_REG_20708(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_pu, 1);
		MOD_RADIO_REG_20708(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_i, 1);
		MOD_RADIO_REG_20708(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_q, 1);
	}
}

void phy_ac_dccal_papd_special_save(phy_info_t *pi)
{
	uint8 core;

	phy_ac_dccal_info_t *dccali = (phy_ac_dccal_info_t *)pi->u.pi_acphy->dccali;
	phy_ac_dccal_papd_special_phyregs_t *porig_phy = dccali->dccal_papd_special_phyregs;

	FOREACH_CORE(pi, core) {
		porig_phy->RfCtrlCoreITRCtrl[core] = READ_PHYREGCE(pi, RfCtrlCoreITRCtrl, core);
	}
	porig_phy->dccal_common = READ_PHYREG(pi, dccal_common);
	porig_phy->idacc_update_hw_reset_len = READ_PHYREG(pi, idacc_update_hw_reset_len);
	porig_phy->radio_pu_seq = READ_PHYREG(pi, radio_pu_seq);
	porig_phy->DcFiltAddress = READ_PHYREG(pi, DcFiltAddress);
	porig_phy->RxControl = READ_PHYREG(pi, RxControl);
}

void phy_ac_dccal_papd_special_restore(phy_info_t *pi)
{
	uint8 core;
	...
	FOREACH_CORE(pi, core) {
		WRITE_PHYREGCE(pi, RfCtrlCoreITRCtrl, core, porig_phy->RfCtrlCoreITRCtrl[core]);
	}
	WRITE_PHYREG(pi, dccal_common, porig_phy->dccal_common);
	WRITE_PHYREG(pi, idacc_update_hw_reset_len, porig_phy->idacc_update_hw_reset_len);
	WRITE_PHYREG(pi, radio_pu_seq, porig_phy->radio_pu_seq);
	WRITE_PHYREG(pi, DcFiltAddress, porig_phy->DcFiltAddress);
	WRITE_PHYREG(pi, RxControl, porig_phy->RxControl);
}

void phy_ac_dccal_idac4papd(phy_info_t *pi, uint8 core, uint8 swap)
{
	const int16 step = 256;
	const uint16 samples = 100;
	phy_hpf_dc_est_t hpf_iqdc[PHY_CORE_MAX];
	const int16 i_start_low = -step;
	const int16 q_start_low = -step;
	const int16 i_start_hi = step;
	const int16 q_start_hi = step;
	int32 i_val_low, i_val_hi;
	int32 q_val_low, q_val_hi;
	int32 i_rc, i_new;
	int32 q_rc, q_new;

	if (ACMAJORREV_131(pi->pubpi->phy_rev) && CHSPEC_IS2G(pi->radio_chanspec)) {
		/* This is a WAR for DCestimateI/Q being stuck when read after MAC sample play
		 * has been running. Issue seen only on 2GHz.
		 */
		wlc_phy_resetcca_acphy(pi);
		OSL_DELAY(250);
	}

	wlc_phy_get_rx_hpf_dc_est_acphy(pi, &hpf_iqdc[0], samples);
	PHY_INFORM(("%s: A%d: before cal: dci=%dmV dcq=%dmV\n", __func__, core,
			(int16)(((int32)(hpf_iqdc[core].i_accum))*600/samples/2048/16),
			(int16)(((int32)(hpf_iqdc[core].q_accum))*600/samples/2048/16)));

	/* Set dcoe and idac_res correction to zero. */
	MOD_PHYREGCE(pi, dccal_control_29, core, dcc_forced_mode, 1);
	MOD_PHYREGCE(pi, dccal_control_29, core, dcc_forced_dcoe_val_i, 0);
	MOD_PHYREGCE(pi, dccal_control_30, core, dcc_forced_dcoe_val_q, 0);
	MOD_PHYREGCE(pi, dccal_control_50, core, idac_res_ovr_en, 1);
	MOD_PHYREGCE(pi, dccal_control_51, core, idac_res_ovr_val_i, 0);
	MOD_PHYREGCE(pi, dccal_control_51, core, idac_res_ovr_val_q, 0);

	MOD_RADIO_REG_20710(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_i, 1);
	MOD_RADIO_REG_20710(pi, TIA_CFG1_OVR, core, ovr_tia_dcdac_q, 1);

	/* Low point */
	MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG1, core, tia_dcdac_i, i_start_low);
	MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG2, core, tia_dcdac_q, q_start_low);

	wlc_phy_get_rx_hpf_dc_est_acphy(pi, &hpf_iqdc[0], samples);
	if (swap) {
		i_val_low = hpf_iqdc[core].q_accum;
		q_val_low = hpf_iqdc[core].i_accum;
	} else {
		i_val_low = hpf_iqdc[core].i_accum;
		q_val_low = hpf_iqdc[core].q_accum;
	}

	/* High point */
	MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG1, core, tia_dcdac_i, i_start_hi);
	MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG2, core, tia_dcdac_q, q_start_hi);

	wlc_phy_get_rx_hpf_dc_est_acphy(pi, &hpf_iqdc[0], samples);
	if (swap) {
		i_val_hi = hpf_iqdc[core].q_accum;
		q_val_hi = hpf_iqdc[core].i_accum;
	} else {
		i_val_hi = hpf_iqdc[core].i_accum;
		q_val_hi = hpf_iqdc[core].q_accum;
	}

	/* Scale everything up since we don't have floating point math */
	i_rc = ((i_val_hi - i_val_low) * 128) / (2 * step);
	q_rc = ((q_val_hi - q_val_low) * 128) / (2 * step);

	/* Extra 50 is added for the rounding */
	i_new = ((i_start_low * i_rc) - (i_val_low * 128) + 50) / (i_rc ? i_rc : 1);
	q_new = ((q_start_low * q_rc) - (q_val_low * 128) + 50) / (q_rc ? q_rc : 1);

	MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG1, core, tia_dcdac_i, i_new);
	MOD_RADIO_REG_20710(pi, TIA_DCDAC_REG2, core, tia_dcdac_q, q_new);

	wlc_phy_get_rx_hpf_dc_est_acphy(pi, &hpf_iqdc[0], samples);
	PHY_INFORM(("%s: A%d: after cal: dci=%dmV dcq=%dmV\n", __func__, core,
			(int16)(((int32)(hpf_iqdc[core].i_accum))*600/samples/2048/16),
			(int16)(((int32)(hpf_iqdc[core].q_accum))*600/samples/2048/16)));
}
