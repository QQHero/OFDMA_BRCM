 ...
 *
 * $Id: wluc_ampdu.c 807958 2022-02-05 00:58:55Z $
 */
...
cmd_func_t wl_ampdu_send_addba;
static cmd_func_t wl_ampdu_send_delba;
static cmd_func_t wl_ampdu_rxba_addr;
...
static cmd_t wl_ampdu_cmds[] = {
	...
	"take txq snapshot"},
	{ "ampdu_rxba_addr", wl_ampdu_rxba_addr, WLC_GET_VAR, WLC_SET_VAR,
...
wl_ampdu_retry_limit_tid(void *wl, cmd_t *cmd, char **argv)
{
	...
	memset(&retry_limit, 0, sizeof(retry_limit));
	tid = atoi(param);
	if (tid > MAXPRIO)
		return BCME_USAGE_ERROR;
	retry_limit.tid = tid;
...
static int
wl_ampdu_rxba_addr(void *wl, cmd_t *cmd, char **argv)
{
	const char *cmdname = "ampdu_rxba_addr";
	struct ether_addr ea, *r_ea;
	void *ptr = NULL;
	int err;
	...
	if (*argv == NULL) {
		...
		if ((err = wlu_var_getbuf(wl, cmdname, &ea, sizeof(ea), &ptr)) < 0) {
			return err;
		}
		r_ea = (struct ether_addr *)ptr;
		printf("AMPDU rx addba filter %s\n", ETHER_ISNULLADDR(r_ea) ? "OFF" : "ON");
		if (!ETHER_ISNULLADDR(r_ea)) {
			printf("\taddba only accepted for MAC %s\n", wl_ether_etoa(r_ea));
		}
		return BCME_OK;
	}
	...
	return wlu_var_setbuf(wl, cmdname, &ea, sizeof(ea));
}
