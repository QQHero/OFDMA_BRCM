 ...
 *
 * $Id: wlc_tbtt.c 684669 2017-02-14 05:44:39Z $
 */
...
fail:
	MODULE_DETACH(ti, wlc_tbtt_detach);
	return NULL;
}
	...
	wlc_tbtt_ent_fn_t pre_fn, wlc_tbtt_ent_fn_t fn, void *arg)
{
	...
	if (pre_fn == NULL && fn == NULL) {
		WL_ERROR(("wl%d: %s: both tbtt handler functions are NULL\n",
	...
	if (pre_fn && ((ret = bcm_notif_add_interest(te->pre_notif_hdl,
		(bcm_notif_client_callback)pre_fn, arg)) != BCME_OK)) {
		WL_ERROR(("wl%d: %s: fail to add pretbtt callback (%d)\n",
			cfg->wlc->pub->unit, __FUNCTION__, ret));
		ASSERT(0);
		return ret;
	}

	if (fn && ((ret = bcm_notif_add_interest(te->notif_hdl,
		(bcm_notif_client_callback)fn, arg)) != BCME_OK)) {
		WL_ERROR(("wl%d: %s: fail to add tbtt callback (%d)\n",
			cfg->wlc->pub->unit, __FUNCTION__, ret));
		ASSERT(0);
		return ret;
	}
	...
	wlc_tbtt_ent_fn_t pre_fn, wlc_tbtt_ent_fn_t fn, void *arg)
{
	...
	if (pre_fn && ((ret = bcm_notif_remove_interest(te->pre_notif_hdl,
		(bcm_notif_client_callback)pre_fn, arg)) != BCME_OK)) {
		WL_ERROR(("wl%d: %s: fail to remove pretbtt callback (%d)\n",
			cfg->wlc->pub->unit, __FUNCTION__, ret));
		ASSERT(0);
		return ret;
	}

	if (fn && ((ret = bcm_notif_remove_interest(te->notif_hdl,
		(bcm_notif_client_callback)fn, arg)) != BCME_OK)) {
		WL_ERROR(("wl%d: %s: fail to remove pretbtt callback (%d)\n",
			cfg->wlc->pub->unit, __FUNCTION__, ret));
		ASSERT(0);
...
wlc_tbtt_pre_tbtt_cb(void *ctx, wlc_mcnx_intr_data_t *notif_data)
{
	...
	if (tt_next_tbtt > 0 &&
		...
		if (bcm_notif_exist_interest(te->notif_hdl) == BCME_OK) {
