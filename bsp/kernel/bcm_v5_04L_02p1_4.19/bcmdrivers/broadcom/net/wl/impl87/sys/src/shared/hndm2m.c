...
typedef struct m2m_dd_eng {
	...
	m2m_dd_d64_t    * txd_table;            // (aligned) table of Tx DDs
	m2m_dd_d64_t    * rxd_table;            // (aligned) table of Rx DDs
	void            * txd_table_orig;       // unaligned buffer containing table of Tx DDs
	void            * rxd_table_orig;       // unaligned buffer containing table of Rx DDs
	dmaaddr_t         txd_table_pa_orig;    // unaligned physical addr of the table of Tx DDs
	dmaaddr_t         rxd_table_pa_orig;    // unaligned physical addr of the table of Rx DDs
	uint32            txd_alloced;          // allocated buffersize for Tx DDs
	uint32            rxd_alloced;          // allocated buffersize for Rx DDs
#if !defined(DONGLEBUILD)
...
__m2m_dd_ring_mem_alloc(osl_t *osh, uint32 ring_depth, const char * ring_name, dmaaddr_t *pap,
	uint32 *alloced, uint32 *align_bits)
{
	void    * va;
	uintptr   base;
	...
	va = DMA_ALLOC_CONSISTENT(osh, mem_size, *align_bits, alloced, pap, NULL);
	if (va == NULL) goto done;

	base = ROUNDUP(PHYSADDRLO(*pap), (1 << *align_bits));
	...
	if (((base + mem_size - 1) & boundary) != (base & boundary)) {
		...
		uint32 boundary_align_bits = 0;
		while (mem_size2bits >>= 1) boundary_align_bits++; /* align to mem_size */
		*align_bits = boundary_align_bits;

		/* Free previous and re-attempt with alignment of mem_size */
		DMA_FREE_CONSISTENT(osh, va, *alloced, *pap, NULL);
		va = DMA_ALLOC_CONSISTENT(osh, mem_size, *align_bits, alloced, pap, NULL);
	}
...
m2m_dump(osl_t *osh, bool verbose)
{
	...
	for (eng_idx = 0U; eng_idx < M2M_DD_ENG; eng_idx++) {
		// Presently all DD based m2m channels index are 0, 1.
...
BCMATTACHFN(m2m_fini)(si_t * sih, osl_t * osh)
{
	...
	for (eng_idx = 0U; eng_idx < M2M_DD_ENG; eng_idx++) {
		...
		if (eng->txd_table != NULL)
			DMA_FREE_CONSISTENT(osh, eng->txd_table_orig,
			    eng->txd_alloced, eng->txd_table_pa_orig, NULL);
		if (eng->rxd_table != NULL)
			DMA_FREE_CONSISTENT(osh, eng->rxd_table_orig,
			    eng->rxd_alloced, eng->rxd_table_pa_orig, NULL);
...
BCMATTACHFN(m2m_init)(si_t * sih, osl_t * osh)
{
	...
	dmaaddr_t rxd_table_pa, txd_table_pa;
...
#error "Both HNDM2M && BCMHWA features are enabled"
	...
	for (eng_idx = 0U; eng_idx < M2M_DD_ENG; eng_idx++)
	{
		...
		eng->txd_table_orig = __m2m_dd_ring_mem_alloc(osh, M2M_DD_DEPTH, "TXD",
				&eng->txd_table_pa_orig, &eng->txd_alloced, &align_bits);

		if (!eng->txd_table_orig) {
			M2M_ERROR("M2M: FAILURE eng %u alloc TxD ring depth %u\n",
				eng_idx, M2M_DD_DEPTH);
		...
		PHYSADDRLOSET(txd_table_pa, ROUNDUP(PHYSADDRLO(eng->txd_table_pa_orig),
			(1 << align_bits)));
		PHYSADDRHISET(txd_table_pa, PHYSADDRHI(eng->txd_table_pa_orig));

		/* Make sure that alignment didn't overflow */
		ASSERT(PHYSADDRLO(txd_table_pa) >= PHYSADDRLO(eng->txd_table_pa_orig));

		/* find the alignment offset that was used */
		offset = (uint)(PHYSADDRLO(txd_table_pa) -
			PHYSADDRLO(eng->txd_table_pa_orig));

		/* make sure we don't cross our boundaries */
		ASSERT((M2M_DD_DEPTH * sizeof(m2m_dd_d64_t)) + offset <= eng->txd_alloced);

		/* adjust the local buffer by the same offset */
		eng->txd_table = (void*)((uintptr)eng->txd_table_orig + offset);

		ASSERT(ISALIGNED(PHYSADDRLO(txd_table_pa), (1 << align_bits)));

		align_bits = 6; /* 64-Byte alignment */
		eng->rxd_table_orig = __m2m_dd_ring_mem_alloc(osh, M2M_DD_DEPTH, "RXD",
				&eng->rxd_table_pa_orig, &eng->rxd_alloced, &align_bits);

		if (!eng->rxd_table_orig) {
			M2M_ERROR("M2M: FAILURE eng %u alloc RxD ring depth %u\n",
				eng_idx, M2M_DD_DEPTH);
			m2m_fini(sih, osh); /* resets m2m_sys to NULL */
			ret = BCME_NOMEM;
			goto done;
		...
		/* adjust the pa by rounding up to the alignment */
		PHYSADDRLOSET(rxd_table_pa, ROUNDUP(PHYSADDRLO(eng->rxd_table_pa_orig),
			(1 << align_bits)));
		PHYSADDRHISET(rxd_table_pa, PHYSADDRHI(eng->rxd_table_pa_orig));

		/* Make sure that alignment didn't overflow */
		ASSERT(PHYSADDRLO(rxd_table_pa) >= PHYSADDRLO(eng->rxd_table_pa_orig));

		/* find the alignment offset that was used */
		offset = (uint)(PHYSADDRLO(rxd_table_pa) -
			PHYSADDRLO(eng->rxd_table_pa_orig));

		/* make sure we don't cross our boundaries */
		ASSERT((M2M_DD_DEPTH * sizeof(m2m_dd_d64_t)) + offset <= eng->rxd_alloced);

		/* adjust the local buffer by the same offset */
		eng->rxd_table = (void*)((uintptr)eng->rxd_table_orig + offset);

		ASSERT(ISALIGNED(PHYSADDRLO(rxd_table_pa), (1 << align_bits)));
#if defined(DONGLEBUILD)
		...
		PHYSADDRTOULONG(txd_table_pa, eng->txd_table_pa);
		PHYSADDRTOULONG(rxd_table_pa, eng->rxd_table_pa);
		...
		if (!eng->xid_table) {
			M2M_ERROR("M2M: FAILURE eng %u alloc xid table rings <%p,%p>,%p depth %u\n",
...
#if defined(BCMDBG)
		if (m2m_dd_bl > pcie_dmarxblen(osh, sih)) {
			M2M_PRINT("=== CAUTION: Rx DMA BurstLen in M2M is downsized "
			          "from %d to %d per PCIE MPS ===\n",
			          m2m_dd_bl, pcie_dmarxblen(osh, sih));
		}
#endif /* BCMDBG */
		rxblen = MIN(m2m_dd_bl, pcie_dmarxblen(osh, sih));
		v32  = R_REG(osh, &m2m_eng_regs->rx.control);
		M2M_DEBUG(M2M_PRINT("M2M: eng %u RcvCtrl 0x%08x\n", eng_idx, v32));
		v32  = BCM_CBF(v32, D64_RC_BL);
		v32 |= M2M_DD_RC_CONFIG(rxblen) | D64_RC_RE;
...
#if defined(BCMDBG)
		if (m2m_dd_bl > pcie_dmatxblen(osh, sih)) {
			M2M_PRINT("=== CAUTION: Tx DMA BurstLen in M2M is downsized "
			          "from %d to %d per PCIE MRRS ===\n",
			          m2m_dd_bl, pcie_dmatxblen(osh, sih));
		}
#endif /* BCMDBG */
		txblen = MIN(m2m_dd_bl, pcie_dmatxblen(osh, sih));
		v32  = R_REG(osh, &m2m_eng_regs->tx.control);
		M2M_DEBUG(M2M_PRINT("M2M: eng %u XmtCtrl 0x%08x\n", eng_idx, v32));
		v32  = BCM_CBF(v32, D64_XC_BL);
		v32 |= M2M_DD_XC_CONFIG(txblen) | D64_XC_XE;
#if !defined(OSL_CACHE_COHERENT)
