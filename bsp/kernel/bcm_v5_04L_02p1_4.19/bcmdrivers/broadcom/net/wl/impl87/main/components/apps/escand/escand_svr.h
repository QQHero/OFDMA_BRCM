...
#define ESCAND_MAX_INTERFACES		4
#define ESCAND_MAX_IF_NUM ESCAND_MAX_INTERFACES
#define ESCAND_MAX_INTF_ATTEMPTS	10
#define ESCAND_EXCLUDE_IFACE_LIST		16

#define ESCAND_TICKS_PER_SEC	2

/* default polling interval in usecs */
#define ESCAND_DFLT_POLL_INTERVAL (1000000/ESCAND_TICKS_PER_SEC)
#define ESCAND_STARTUP_DELAY	(60 * ESCAND_TICKS_PER_SEC) /* Don't scan immediately */
...
#define BAND_AUTO(band) (band == WLC_BAND_AUTO)
#define ESCAND_SM_BUF_LEN  1024

#define ESCAND_CS_MIN_2G_CHAN	CH_MIN_2G_CHANNEL	/* min channel # in 2G band */
#define ESCAND_CS_MAX_2G_CHAN	CH_MAX_2G_CHANNEL	/* max channel # in 2G band */
#define ESCAND_CS_MIN_5G_CHAN	CH_MIN_5G_CHANNEL	/* min channel # in 5G band */
#define ESCAND_CS_MAX_5G_CHAN	CH_MAX_5G_CHANNEL	/* max channel # in 5G band */
#define ESCAND_CS_MIN_6G_CHAN	CH_MIN_6G_CHANNEL	/* min channel # in 6G band */
#define ESCAND_CS_MAX_6G_CHAN	CH_MAX_6G_CHANNEL	/* max channel # in 6G band */
...
#define ESCAND_CI_SCAN_WINDOW	(5 * ESCAND_TICKS_PER_SEC)  /* sec: how often for ci scan */
#define ESCAND_CS_SCAN_TIMER_MIN	60 /* sec */
#define ESCAND_DFLT_CS_SCAN_TIMER	900 /* sec */
#define ESCAND_DFLT_CI_SCAN_TYPE  0 /* active (passive = 1) */
#define ESCAND_DFLT_CI_SCAN_TIMER  1 /* sec */
#define ESCAND_BUSY_MIN_CI_SCAN_TIMER  10 /* sec */
...
#define ESCAND_SCAN_POLL	(ESCAND_DFLT_CI_SCAN_TIMER * ESCAND_TICKS_PER_SEC)
#define ESCAND_STATUS_POLL	(5 * ESCAND_TICKS_PER_SEC)
#define ESCAND_TRAFFIC_POLL	(1 * ESCAND_TICKS_PER_SEC)
#define ESCAND_ASSOCLIST_POLL	(5 * ESCAND_TICKS_PER_SEC)

/* on 160MHz with dyn160 enabled, use this center chanspec */
#define ESCAND_DYN160_CENTER_CH		50
/* with dyn160 enabled, use this center chanspec on 80MHz */
#define ESCAND_DYN160_CENTER_CH_80	58
...
#define ESCAND_CI_SCAN_TIMEOUT			1 		/* seconds */
...
#define ESCAND_PKTQ_STATS_VER	6
...
#define ESCAND_MAX_INFO_BUF			WLC_IOCTL_MAXLEN
...
/* display ticks every 30 seconds */
#define ESCAND_TICK_DISPLAY_INTERVAL		(30 * ESCAND_TICKS_PER_SEC)
...
#define ESCAND_CHINFO_IS_UNCLEAR(CHINFO) (((CHINFO) & WL_CHAN_PASSIVE) != 0) /* uncleared DFS ch */
...
typedef struct escand_chaninfo {
	...
	uint escand_ci_scan_type;	/* ci scan type (active/passive) */
	uint escand_ci_scan_timer; /* ci scan timer */
	uint escand_ci_busy_min_scan_timer; /* ci scan timer when associated stations present */
	...
	int escand_nscantx;
	int escand_traffic_type;
	int escand_traffic_thresh;
	int escand_traffic_measured;
	int escand_traffic_thresh_measured;
...
typedef struct escand_wksp_s {
	...
	uint poll_interval; /* polling interval in usecs */
...
extern int escand_escan_prep_cs(escand_chaninfo_t *c_info,
	wl_scan_params_t *params, int *params_size);
extern int escand_escan_prep_ci(escand_chaninfo_t *c_info,
	wl_scan_params_t *params, int *params_size);
...
extern int escand_cs_scan_timer_check(escand_chaninfo_t * c_info);
extern int escand_ci_scan_check(escand_chaninfo_t * c_info);
extern void escand_dump_scan_entry(escand_chaninfo_t *c_info);
extern void escand_dump_map(void);
extern void escand_dump_config_extra(escand_chaninfo_t *c_info);
...
extern int escand_get_bwcap_info(escand_chaninfo_t *c_info, escand_param_info_t *param,
	int param_len, char *buf, int buf_len);
extern int escand_get_cap_info(escand_chaninfo_t *c_info, uint32 *param, int param_len,
	char *cap_buf, int cap_len);
...
extern int escand_get_bssid(char *ifname, char *bssid);
extern int escand_get_pktq_data(char *ifname, char *buf, wl_iov_mac_full_params_t *full_params);
