...
pciedev_ioctl(hnd_dev_t *rtedev, uint32 cmd, void *buf, int len, int *used, int *needed, int set)
{
	...
	case BUS_FLOW_FLUSH_PEND:
		...
		break;
	case BUS_SCB_FLOW_DELINK:
	{
		uint16 flowid = *((uint16*)buf);
		pciedev_scb_flow_delink(drv->pciedev, flowid);
		break;
	}
	case BUS_FLOW_PS_UPDATE:
	{
		uint16 ringid  = (uint16)(((uint32*)buf)[0]);
		bool ps_on = (((uint32*)buf)[1] != 0);

		pciedev_flow_ring_ps_update(drv->pciedev, ringid, ps_on);
		break;
	}
	...
	case BUS_SCHEDULE_SQS_STRIDE:
	{
		pciedev_sqs_stride_resume(drv->pciedev);
		break;
	}
	default:
		ret = BCME_ERROR;
