 ...
 *
 * $Id: wl_eventq.c 767941 2018-10-01 13:01:09Z $
 *
 */
...
BCMATTACHFN(wl_eventq_detach)(wl_eventq_info_t *wlevtq)
{
	...
	while (wlevtq->client) {
		...
		MFREE(WLCOSH(wlevtq), client, sizeof(client_t) +
			sizeof(client->event) * (client->count - 1));
	}
...
wl_eventq_process_event(void *arg)
{
	...
	if (wlevtq->head) {
		...
		wl_event_msg_t *wl_event_msg = &e->event;
		uint8 *data = e->data;
		uint32 length = e->event.datalen;

		client_t *client = wlevtq->client;
		while (client) {
			/* save next client as callback may invoke
			 * wl_eventq_unregister_event_cb() which will free
			 * the current client
			 */
				...
				for (i = 0; i < client->count; i++) {
					if (client->event[i] == event_type) {
						(client->cb)(client->arg,
							event_type, wl_event_msg, data, length);
						break;
					}
				}
			}
