 ...
 *
 * $Id: wlc_bs_data.c 778454 2019-09-02 12:20:49Z $
 */
...
static const char module_name[] = "bs_data";
...
static wlc_bs_data_scb_cubby_t **
...
static const bcm_iovar_t bs_data_iovars[] = {
	{"bs_data", IOV_SCB_BS_DATA, (IOVF_GET_UP), 0, IOVT_BUFFER, 0 },
	{NULL, 0, 0, 0, 0, 0}
};

static int
wlc_bs_data_iovar_handler(void *handle, uint32 actionid,
	...
	case IOV_GVAL(IOV_SCB_BS_DATA):
		{
			...
			FOREACH_BSS_SCB(wlc->scbstate, &scbiter, bsscfg, scb) {
				if (SCB_ASSOCIATED(scb)) {
					...
					cd->acked = cs->acked;
					cd->ru_acked = cs->ru_acked;
					cd->mu_acked = cs->mu_acked;
