	...
	for (idx = 0; idx < info->max_ifnum; idx++) {
		...
		for (bssidx = 0; bssidx < WL_MAXBSSCFG; bssidx++) {
			...
			while (sta) {
				...
				to_bss_band = bssinfo->steer_bssinfo->intf_info->band;
				if (!(bsd_get_sta_band(info, &sta->addr) & to_bss_band)) {
					BSD_STEER("sta[%p]:"MACF" is not %s band. Skipped.\n",
						sta, ETHERP_TO_MACF(&sta->addr),
						to_bss_band == BSD_BAND_6G ? "6G" : "5G");
					goto next;
				}
...
static int bsd_get_preferred_if(bsd_info_t *info, int ifidx)
{
	...
	while (if_bss_list) {
		...
		if ((cnt == 1) && BSD_BSS_SKIP_FIRST_TO_BSS(bssinfo)) {
			BSD_MULTI_RF("Skip first target bss \n");
			goto next;
		}
...
static bool bsd_if_qualify_sta(bsd_info_t *info, int to_ifidx, bsd_sta_info_t *sta)
{
	...
	int vht;
	int band_detect = 0;
	bsd_maclist_t *prbsta = NULL;
	uint8 to_bss_band;
	uint8 *band_detect_cnt = NULL;
	...
	to_bss_band = to_bssinfo->intf_info->band;
	if (bssinfo->intf_info->band != to_bss_band) {
		/* different band, e.g. 5G <-> 2G, 5G <-> 6G, 2G <-> 6G */
		if (!(bsd_get_sta_band(info, &sta->addr) & to_bss_band)) {
			BSD_STEER("STA "MACF" not %s band capable\n", ETHER_TO_MACF(sta->addr),
				to_bss_band == BSD_BAND_6G ? "6G" : "5G");
			band_detect = 1;
			if (info->sta_skip_single) {
				BSD_STEER("singband STA skip\n");
				return FALSE;
			}
		}
	}
	...
	if (band_detect) {
		...
		if (prbsta) {
			if (to_bss_band == BSD_BAND_6G) {
				band_detect_cnt = &prbsta->band_detect_cnt_6g;
			} else {
				band_detect_cnt = &prbsta->band_detect_cnt_5g;
			}
		}
		if ((band_detect_cnt && (*band_detect_cnt > BSD_BAND_DETECT_MAX_TRY)) ||
			!(sta->wnm_cap & WL_WNM_BSSTRANS)) {
			BSD_STEER("Skip STA "MACF" reason: mutli-band[%s] detect fail wnm[%d]\n",
				ETHER_TO_MACF(sta->addr), to_bss_band == BSD_BAND_6G ? "6G" : "5G",
				sta->wnm_cap);
			/* If there are multiple target bss check for next interface */
			if (bssinfo->to_if_bss_list->count > 1) {
				BSD_MULTI_RF("Skip first target bss for next attempt\n");
				bssinfo->flags |= BSD_FLAG_BSS_SKIP_FIRST_TO_BSS;
			}
			return FALSE;
		} else {
			BSD_STEER("%s: STA "MACF" band detect count: %d\n",
				__FUNCTION__, ETHERP_TO_MACF(&sta->addr),
				band_detect_cnt ? *band_detect_cnt : 0);
		}
	}
		...
		while (sta) {
			...
			if (steering_cfg->rssi != 0) {
				...
				if (((ge_check == 0) && (sta->rssi < steering_cfg->rssi)) ||
					...
					BSD_STEER("set RSSI ifidx:%d bssidx:%d intf:%s rssi_hits:%d info_times:%d\n",
						ifidx, bssidx, bssinfo->ifnames, sta->hits_rssi, info->sta_hit_rssi_times);

					sta->hits_rssi++;
					if ((sta->hits_rssi % info->sta_hit_rssi_times) == 0) {
						BSD_STEER("Mask the Steer RSSI flag, preparing.....\n");
...
void bsd_steer_scheme_policy(bsd_info_t *info)
{
	...
	bsd_bssinfo_t *bssinfo;
	bsd_maclist_t *prbsta = NULL;
	uint8 curr_bss_band, to_bss_band;
	uint8 *band_detect_cnt = NULL;
	...
	for (idx = 0; idx < info->max_ifnum; idx++) {
		...
		/* Handler if the STA is not detected as 5G or 6G band capable so far */
		curr_bss_band = sta->bssinfo->intf_info->band;
		to_bss_band = sta->to_bssinfo->intf_info->band;
		if ((((curr_bss_band != to_bss_band)) &&
			!(bsd_get_sta_band(info, &sta->addr) & to_bss_band)) ||
			...
			if (band_detect_cnt && (*band_detect_cnt <= BSD_BAND_DETECT_MAX_TRY)) {
				(*band_detect_cnt)++;
				BSD_STEER("%s: %s band STA detect count: %d\n",
				__FUNCTION__, to_bss_band == BSD_BAND_6G ? "6G" : "5G",
				*band_detect_cnt);
			} else {
				BSD_STEER("Skip STA "MACF" reason: %s band invalid\n",
					ETHERP_TO_MACF(&sta->addr),
					to_bss_band == BSD_BAND_6G ? "6G" : "5G");
				continue;
			}
		...
		if (BSD_BSS_SKIP_FIRST_TO_BSS(sta->bssinfo)) {
			sta->bssinfo->flags &= ~BSD_FLAG_BSS_SKIP_FIRST_TO_BSS;
		}
	}
...
#ifdef CMWIFI_RDKB
		if (rate > sta_select_cfg->idle_rate)
			intf_info->steering_flags |= BSD_STEERING_POLICY_FLAG_ACTIVE;
		else
			intf_info->steering_flags &= ~BSD_STEERING_POLICY_FLAG_ACTIVE;
#endif
...
int bsd_get_steerable_bss(bsd_info_t *info, bsd_intf_info_t *intf_info)
{
	...
	for (bssidx = 0; bssidx < WL_MAXBSSCFG; bssidx++) {
		...
		if (bssinfo->valid && (bssinfo->steerflag != BSD_BSSCFG_NOTSTEER) &&
			BSD_BSS_BSD_ENABLED(bssinfo) && !(BSD_BSS_IS_CHAN_PASSIVE(bssinfo))) {
