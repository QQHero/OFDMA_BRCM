 ...
 *
 * $Id: phy_ac_ana.c 799392 2021-05-28 10:21:16Z $
 */
...
#ifdef BCMROMOFFLOAD
#undef BCMATTACHDATA
#define BCMATTACHDATA(_data_) _data_
#endif
...
static int BCMATTACHFN(phy_ac_ana_srom_papd_swctrl_read)(phy_info_t *pi);
static uint8 BCMATTACHFN(phy_ac_ana_parse_papd_srom_swctrl)(uint16 val, uint8 core, uint8 width);
	...
	phy_ana_info_t *ani)
{
	...
	/* function to read papd_femctrl params from nvram */
	if (!phy_ac_ana_srom_papd_swctrl_read(pi))
		goto fail;
...
static const swctrlmap4_mode_t femctrl_mapping[2][SWCTRLMAP4_FEMCTRL_TBL_MAP_LEN] = {
	{
		MISC, MISC, RX, RXBYP,
		MISC, MISC, RX, RXBYP,
		MISC, TX, MISC, TX,
...
wlc_phy_write_femctrl_table_swctrlmap4(phy_info_t *pi)
{
	...
	if (swctrl->misc_usage >= ARRAYSIZE(femctrl_mapping)) {
...
/* Overwrite the FEMCLTR RX entries by the ones used for T/R bypass or restore */
static void
wlc_phy_femctrl_table_swctrlmap4_tr2t(phy_info_t *pi, bool force_tr2t)
{
	uint8 core, i;
	uint8 num_tbl_entries_band = SWCTRLMAP4_FEMCTRL_TBL_MAP_LEN;
	acphy_swctrlmap4_t *swctrl = pi->u.pi_acphy->sromi->swctrlmap4;

	/* Only supported for SWCTRLMAP4 */
	ASSERT(ACPHY_SWCTRLMAP4_EN(pi));

	FOREACH_CORE(pi, core) {
		for (i = 0; i < num_tbl_entries_band; i++) {
			if (femctrl_mapping[swctrl->misc_usage][i] == RX) {
				/* Change RX femctrl entry to RxBypass or restore to normal Rx */
				uint16 swctrl_2g, swctrl_5g;
				if (force_tr2t) {
					swctrl_2g = swctrl->rxbyp2g[core];
					swctrl_5g = swctrl->rxbyp5g[core];
				} else {
					swctrl_2g = swctrl->rx2g[core];
					swctrl_5g = swctrl->rx5g[core];
				}
				wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_FEMCTRLLUT,
					1, i + 256*core, 16, &swctrl_2g);
				wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_FEMCTRLLUT,
					1, num_tbl_entries_band + i + 256*core, 16, &swctrl_5g);
			}
		}
	}
}

/* Reprogram the FEMCLTR table setting thew RX to LOW_GAIN_RX or restore */
static void
wlc_phy_femctrl_legacy_tr2t(phy_info_t *pi, bool force_tr2t)
{
	phy_info_acphy_t *pi_ac = pi->u.pi_acphy;
	uint8 band;

	/* FEMCTRL tables read from NVRAM */
	uint32 *femctrlmap[2] = {
		&(pi_ac->sromi->nvram_femctrl.swctrlmap_2g[0]),
		&(pi_ac->sromi->nvram_femctrl.swctrlmap_5g[0]) };
	uint32 *femctrlmapext[2] = {
		&(pi_ac->sromi->nvram_femctrl.swctrlmapext_2g[0]),
		&(pi_ac->sromi->nvram_femctrl.swctrlmapext_5g[0]) };

	/* Per band backup for the swctrl value we are going to override */
	uint32 swctrlmap_backup[2], swctrlmapext_backup[2];

	/* Only for FEMCTRL using wlc_phy_write_regtbl_fc_from_nvram */
	ASSERT(!ACPHY_SWCTRLMAP4_EN(pi) && !ACPHY_FEMCTRL_ACTIVE(pi));

	if (force_tr2t) {
		/* Copy NVRAM LOW_GAIN_RX value to RX value and make backup */
		for (band = 0; band <= 1; band++) {
			swctrlmap_backup[band] = femctrlmap[band][1];
			femctrlmap[band][1] = femctrlmap[band][2];

			swctrlmapext_backup[band] = femctrlmapext[band][1];
			femctrlmapext[band][1] = femctrlmapext[band][2];
		}
	}

	/* Program SWCTRL according to the hacked or normal FEMCTRL tables */
	wlc_phy_write_regtbl_fc_from_nvram(pi);

	if (force_tr2t) {
		/* Restore */
		for (band = 0; band <= 1; band++) {
			femctrlmap[band][1] = swctrlmap_backup[band];
			femctrlmapext[band][1] = swctrlmapext_backup[band];
		}
	}
}
...
/* Overwrite the FEMCLTR RX entries by the ones used for T/R bypass or restore */
int
wlc_phy_femctrl_table_tr2t(phy_info_t *pi, bool force_tr2t)
{
	int ret = BCME_OK;

	/* Different function for legacy and SWCTRLMAP4 implementation */
	if (ACPHY_SWCTRLMAP4_EN(pi)) {
		wlc_phy_femctrl_table_swctrlmap4_tr2t(pi, force_tr2t);
	} else if (!ACPHY_FEMCTRL_ACTIVE(pi)) {
		wlc_phy_femctrl_legacy_tr2t(pi, force_tr2t);
	} else {
...
BCMATTACHFN(phy_ac_ana_srom_swctrlmap4_read)(phy_info_t *pi)
{
	...
	uint8 num_modes = ARRAYSIZE(mode_3to0);
...
static int
BCMATTACHFN(phy_ac_ana_srom_papd_swctrl_read)(phy_info_t *pi)
{
	uint8 core, band, swctrl[3];
	uint16 tmp, swctrl_fem[3], swctrl_femlp[3];
	acphy_papd_swctrl_t *papd_swctrl;
	acphy_swctrlmap4_t *swctrlmap4;
	uint8 width;

	if (pi->u.pi_acphy->sromi->papd_swctrl == NULL) {
	  if ((pi->u.pi_acphy->sromi->papd_swctrl =
	       phy_malloc(pi, sizeof(*(pi->u.pi_acphy->sromi->papd_swctrl)))) == NULL) {
	    PHY_ERROR(("%s: swctrlmap4 malloc failed\n", __FUNCTION__));
	    return BCME_NOMEM;
	  }
	}
	papd_swctrl = pi->u.pi_acphy->sromi->papd_swctrl;

	tmp = (uint16) PHY_GETINTVAR_DEFAULT(pi, rstr_papd_swctrl, 0xffff);
	if (tmp == 0xffff) {
	  papd_swctrl->valid = 0;
	  return TRUE;
	} else {
	...
	swctrlmap4 = pi->u.pi_acphy->sromi->swctrlmap4;
	if (swctrlmap4->bitwidth8) {
		width = 8;
	} else {
		width = 4;
	}

	papd_swctrl->band[0].mode = (uint8) (tmp & 0x3);
	papd_swctrl->band[1].mode = (uint8) ((tmp >> 2) & 0x3);
	papd_swctrl->band[2].mode = (uint8) ((tmp >> 4) & 0x3);
	swctrl[0] = (uint8) ((tmp >> 8) & 0xff);

	tmp = (uint16) PHY_GETINTVAR_DEFAULT(pi, rstr_papd_swctrl_5g6g, 0);
	swctrl[1] = (uint8) (tmp & 0xff);
	swctrl[2] = (uint8) ((tmp >> 8) & 0xff);

	swctrl_fem[0] = (uint16) PHY_GETINTVAR_DEFAULT(pi, rstr_papd_swctrlmap_fem2g, 0);
	swctrl_fem[1] = (uint16) PHY_GETINTVAR_DEFAULT(pi, rstr_papd_swctrlmap_fem5g, 0);
	swctrl_fem[2] = (uint16) PHY_GETINTVAR_DEFAULT(pi, rstr_papd_swctrlmap_fem6g, 0);

	swctrl_femlp[0] = (uint16) PHY_GETINTVAR_DEFAULT(pi, rstr_papd_swctrlmap_femlp2g, 0);
	swctrl_femlp[1] = (uint16) PHY_GETINTVAR_DEFAULT(pi, rstr_papd_swctrlmap_femlp5g, 0);
	swctrl_femlp[2] = (uint16) PHY_GETINTVAR_DEFAULT(pi, rstr_papd_swctrlmap_femlp6g, 0);

	for (band = 0; band < PAPD_SWCTRL_NUM_BANDS; band++) {
	  FOREACH_CORE(pi, core) {
		papd_swctrl->band[band].lp_core[core] =
		  phy_ac_ana_parse_papd_srom_swctrl(swctrl[band], core, 2);
		papd_swctrl->band[band].femctrl[core] =
		  phy_ac_ana_parse_papd_srom_swctrl(swctrl_fem[band], core, width);
		papd_swctrl->band[band].femctrl_lp[core] =
		  phy_ac_ana_parse_papd_srom_swctrl(swctrl_femlp[band], core, width);
	  }
	}
...
static uint8
BCMATTACHFN(phy_ac_ana_parse_papd_srom_swctrl)(uint16 val, uint8 core, uint8 width)
{
	uint16 sh, mask;
	uint8 ret;
