 ...
 *
 * $Id: phy_samp.c 809672 2022-03-22 04:44:16Z $
 */
...
int
phy_samp_capture_enable(phy_info_t *pi, uint32 *save_clkgatests,
	uint32 start_ptr, uint32 end_ptr)
{
	phy_type_samp_fns_t *fns = pi->sampi->fns;

	if (fns->sampcap_enable != NULL) {
		return fns->sampcap_enable(fns->ctx, save_clkgatests,
			start_ptr, end_ptr);
	} else {
		return BCME_UNSUPPORTED;
	}
...
int
phy_samp_capture_disable(phy_info_t *pi)
{
	phy_type_samp_fns_t *fns = pi->sampi->fns;

	if (fns->sampcap_disable != NULL) {
		return fns->sampcap_disable(fns->ctx);
	} else {
		return BCME_UNSUPPORTED;
	}
}
