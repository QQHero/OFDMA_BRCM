              ...
              int only_first_ssid)
{
    ...
    for (i = 0; i < wpa_s->last_scan_res_used; i++) {
        ...
        selected = wpa_scan_res_match(wpa_s, i, bss, group,
                            only_first_ssid, 1);
        ...
        if (!best_bss || (bss->level > best_bss->level)) {
            best_bss = bss;             //wpa源码是选择第一个符合条件的进行关联，现改成信号最好的进行关联(如果扫描列表上报有问题，可能会导致不稳定的情况)
            *selected_ssid =  selected;
            wpa_dbg(wpa_s, MSG_DEBUG, MACSTR
                            "select bss freq=%d level=%d snr=%d est_throughput=%u noise=%d",
                            MAC2STR(bss->bssid), bss->freq, bss->level,
                            bss->snr, bss->est_throughput, bss->noise);
        }
    }
