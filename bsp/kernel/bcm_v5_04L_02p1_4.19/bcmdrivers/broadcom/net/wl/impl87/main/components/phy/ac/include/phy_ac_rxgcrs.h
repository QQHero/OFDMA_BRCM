 ...
 *
 * $Id: phy_ac_rxgcrs.h 810501 2022-04-08 03:34:50Z $
 */
 ...
 */
#define ACPHY_INIT_GAIN_TINY (ACPHY_LO_NF_MODE_ELNA_TINY(pi) ? 64 : 61)
#define ACPHY_HI_GAIN_TINY   (ACPHY_LO_NF_MODE_ELNA_TINY(pi) ? 43 : 37)
/* End of GBD used MACRO
 */

#define ACPHY_INIT_GAIN_28NM (\
	(ACMAJORREV_47(pi->pubpi->phy_rev) && (RADIOREV(pi->pubpi->radiorev) > 0)) ? 64 : \
	(ACMAJORREV_51_131(pi->pubpi->phy_rev) && BF_ELNA_2G(pi->u.pi_acphy)) ? 68 : \
	(ACMAJORREV_51_131(pi->pubpi->phy_rev) && CHSPEC_IS2G(pi->radio_chanspec)) ? 64 : \
	(ACMAJORREV_51_131(pi->pubpi->phy_rev) && CHSPEC_ISPHY5G6G(pi->radio_chanspec)) ? 68 : \
	(ACMAJORREV_129(pi->pubpi->phy_rev)) ? 64 : \
	(ACMAJORREV_128(pi->pubpi->phy_rev)) ? 66 : \
	(ACMAJORREV_130(pi->pubpi->phy_rev)) ? 64 :  \
	67)
...
typedef struct acphy_rxgainctrl_params {
	int8  gaintbl[ACPHY_MAX_RX_GAIN_STAGES][16];
	uint8 gainbitstbl[ACPHY_MAX_RX_GAIN_STAGES][16];
} acphy_rxgainctrl_t;
...
extern void wlc_phy_mclip_agc_rxgainctrl(phy_info_t *pi);
...
int phy_ac_rxgcrs_iovar_set_lesiscale_offset(phy_ac_rxgcrs_info_t *rxgcrsi, int8 band,
	int32 set_val);
int phy_ac_rxgcrs_iovar_get_force_rxgain(phy_ac_rxgcrs_info_t *rxgcrsi,	int32 *ret_val);
int phy_ac_rxgcrs_iovar_set_force_rxgain(phy_ac_rxgcrs_info_t *rxgcrsi,	int32 set_val);
int phy_ac_rxgcrs_iovar_get_subband_edadj(phy_ac_rxgcrs_info_t *rxgcrsi, int32 *ret_val);
int phy_ac_rxgcrs_iovar_set_subband_edadj(phy_ac_rxgcrs_info_t *rxgcrsi, int32 set_val);
...
extern int8 phy_ac_rxgcrs_get_init_gain_dB(phy_info_t *pi, uint8 core);
...
void phy_ac_rxgcrs_cal(phy_ac_rxgcrs_info_t *rxgcrsi);
...
void phy_ac_rxgcrs_upd_knoise_shmem(phy_info_t *pi, bool lowgain_eq_initgain);

#ifdef WL_PROXD_PHYTS
bool phy_ac_tof_phyts_is_loopback_setup(phy_info_t *pi);
uint32 phy_ac_rxgcrs_phyts_encode_rxgain_bundle(phy_info_t *pi, int8 req_gaindB,
	int8 *gain_applied);
