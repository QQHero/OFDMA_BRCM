 ...
 *
 * $Id: wlc_channel.c 803106 2021-09-16 06:31:45Z $
 */
...
skip_clm_init:
	result = wlc_channel_init_ccode(wlc_cmi, country_abbrev, wlc_cm->srom_regrev,
		sizeof(country_abbrev));
...
wlc_handle_clm_dload(wlc_cm_info_t *wlc_cmi, segment_t *segments, uint32 segment_count)
{
	...
	wlc_channel_init_ccode(wlc_cmi, country_abbrev, wlc_cm->srom_regrev,
		sizeof(country_abbrev));
	...
	int bandtype, chanvec_t *channels)
{
	...
	if (!strcmp(wlc->cmi->cm->country_abbrev, country_abbrev) ||
		!strcmp(wlc->cmi->cm->ccode, country_abbrev)) {
		 ...
		 */
		strncpy(ccode, wlc->cmi->cm->ccode, WLC_CNTRY_BUF_SZ-1);
		ccode[WLC_CNTRY_BUF_SZ-1] = '\0';
		regrev = wlc->cmi->cm->regrev;
	} else {
...
wlc_set_countrycode(wlc_cm_info_t *wlc_cmi, const char* ccode, uint regrev)
{
	...
	return wlc_set_countrycode_rev(wlc_cmi, ccode, regrev);
}
...
wlc_country_lookup_ext(wlc_info_t *wlc, const char *ccode, clm_country_t *country)
{
	...
	if (!strcmp(wlc->cmi->cm->country_abbrev, country_str_lookup) ||
		!strcmp(wlc->cmi->cm->ccode, country_str_lookup)) {
		/* If the desired ccode matches the current advertised ccode
		 * or the current ccode, set regrev for the current regrev
		 * and set ccode for the current ccode
		 */
		strncpy(country_str_lookup, wlc->cmi->cm->ccode, WLC_CNTRY_BUF_SZ-1);
                        ...
                        const char *abbrev)
{
	...
	if (strcmp(abbrev, "")) {
		if (!strcmp(wlc->cmi->cm->country_abbrev, abbrev) ||
			!strcmp(wlc->cmi->cm->ccode, abbrev)) {
			/* If the desired ccode matches the current advertised ccode
			 * or the current ccode, set regrev for the current regrev
			 * and set ccode for the current ccode
			 */
...
void
wlc_channel_send_chan_event(wlc_info_t *wlc, wlc_bsscfg_t *cfg,
