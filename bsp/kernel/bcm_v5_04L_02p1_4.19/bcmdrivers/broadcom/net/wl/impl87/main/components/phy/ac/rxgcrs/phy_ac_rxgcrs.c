 ...
 *
 * $Id: phy_ac_rxgcrs.c 810422 2022-04-07 01:47:44Z $
 */
...
/* See https://confluence.broadcom.net/x/XhnoD#ACPHYRxGaincontrol-Multiclip(mclip)documentation */
typedef enum {
    MCLIP_INIT     = 0,
    MCLIP_ADC      = 1,
    MCLIP_NB2_LOW  = 2,
...
static const int8 tiaStage_Gains_Main[] = {3, 10, 16, 22, 30, 36, 42, 44};
static const int8 tiaStage_Gains_Aux[] = {3, 12, 18, 24, 30, 36, 42, 44};
...
uint8 ind_2d_tbl)
{
    ...
    if (RADIOREV(pi->pubpi->radiorev) < 2) {
        ...
        // 6756A1, 6756A0 is following 63178/rev 51
        ASSERT(!ACMINORREV_0(pi));
        ...
        if (BF_ELNA_2G(pi_ac)) {
            ...
            lna12_gain_tbl_2g = (void *)lna12_gain_tbl_2g_20710rX_ilna[ind_2d_tbl];
            lna1_rout_map_2g = (void *)lna1_rout_map_2g_20710rx;
            lna1_gain_map_2g = (void *)lna1_gain_map_2g_20710rX;
        }
        switch (tbl_id) {
            case LNA12_GAIN_TBL_2G:
                return lna12_gain_tbl_2g;
            ...
            case LNA12_GAIN_TBL_5G:
                return (void *)lna12_gain_tbl_5g_20710rX[ind_2d_tbl];
            ...
            case LNA12_GAIN_TBL_6G_HI:
            case LNA12_GAIN_TBL_6G_HI2:
                return lna12_gain_tbl_6gs05_20710rX[ind_2d_tbl];

            case LNA12_GAIN_TBL_6G_HI3:
                return lna12_gain_tbl_6gs6_20710rX[ind_2d_tbl];
            ...
            case BIQ01_GAIN_TBL:
                return (void *)biq01_gain_tbl_20710rX[ind_2d_tbl];

            case BIQ01_GAIN_BITS_TBL:
                return (void *)biq01_gainbits_tbl_20710rX[ind_2d_tbl];
            ...
            case LNA1_ROUTMAP_TBL_5G:
                return (void *)lna1_rout_map_5g_20710rX;

            case LNA1_ROUTMAP_TBL_6G_LOW:
                return lna1_rout_map_6gs0_20710rX;

            case LNA1_ROUTMAP_TBL_6G_LOW2:
                return lna1_rout_map_6gs1_20710rX;

            case LNA1_ROUTMAP_TBL_6G_LOW3:
                return lna1_rout_map_6gs2_20710rX;

            case LNA1_ROUTMAP_TBL_6G_MID:
                return lna1_rout_map_6gs3_20710rX;

            case LNA1_ROUTMAP_TBL_6G_HI:
                return lna1_rout_map_6gs4_20710rX;

            case LNA1_ROUTMAP_TBL_6G_HI2:
                return lna1_rout_map_6gs5_20710rX;

            case LNA1_ROUTMAP_TBL_6G_HI3:
                return lna1_rout_map_6gs6_20710rX;

            case LNA1_GAINMAP_TBL_2G:
                return (void *)lna1_gain_map_2g;

            case LNA1_GAINMAP_TBL_5G:
                return (void *)lna1_gain_map_5g_20710rX;

            case LNA1_GAINMAP_TBL_6G_LOW:
                return lna1_gain_map_6gs0_20710rX;

            case LNA1_GAINMAP_TBL_6G_LOW2:
                return lna1_gain_map_6gs1_20710rX;

            case LNA1_GAINMAP_TBL_6G_LOW3:
                return lna1_gain_map_6gs2_20710rX;

            case LNA1_GAINMAP_TBL_6G_MID:
                return lna1_gain_map_6gs3_20710rX;

            case LNA1_GAINMAP_TBL_6G_HI:
                return lna1_gain_map_6gs4_20710rX;

            case LNA1_GAINMAP_TBL_6G_HI2:
                return lna1_gain_map_6gs5_20710rX;

            case LNA1_GAINMAP_TBL_6G_HI3:
                return lna1_gain_map_6gs6_20710rX;
...
wlc_phy_desense_apply_default_acphy(phy_info_t *pi)
{
    ...
    */
    if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
        ...
        if (phy_ac_noise_get_sec_desense(pi_ac->rxgcrsi->aci->noisei)) {
        ...
        if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
            if ((CHSPEC_IS6G(pi->radio_chanspec)) &&
                ...
                wlc_phy_set_crs_min_pwr_logain_acphy(pi, 0x26, FALSE);
                FOREACH_CORE(pi, core) {
                    MOD_PHYREGC(pi, mClpAgcMDClipCntTh2, core,
                        mClpagc_hpf_bw_idx_th2, 8);
                }
            } else {
                wlc_phy_set_crs_min_pwr_logain_acphy(pi, 0x32, FALSE);
                FOREACH_CORE(pi, core) {
                    MOD_PHYREGC(pi, mClpAgcMDClipCntTh2, core,
                        mClpagc_hpf_bw_idx_th2, 9);
                }
            }
        }
    } else {
    ...
    int8 crsmin_md, int8 crsmin_lo, bool sec_only)
{
    ...
    if (phy_ac_noise_get_sec_desense(pi->u.pi_acphy->rxgcrsi->aci->noisei)) {
...
wlc_phy_desense_apply_acphy(phy_info_t *pi, bool apply_desense)
{
    ...
    */
    ...
    if (!TINY_RADIO(pi) && !IS_28NM_RADIO(pi) && !IS_16NM_RADIO(pi)) {
        ...
        // Apply secondary desense. Secondary desense cap with
...
wlc_phy_upd_lna1_lna2_gaintbls_acphy(phy_info_t *pi, uint8 lna12)
{
    ...
    // Recover 6G high-band Rx Gain
    if (ACMAJORREV_131(pi->pubpi->phy_rev) && !ACMINORREV_0(pi)) {
        uint16 rxdb_spare = 0;
        if (CHSPEC_IS6G(pi->radio_chanspec)) {
            uint16 freq = CHAN6G_FREQ(CHSPEC_CHANNEL(pi->radio_chanspec));
            if (freq >= 6585) {
                rxdb_spare = 2;
            }
            else if (freq >= 6425) {
        ...
        FOREACH_CORE(pi, core) {
            MOD_RADIO_REG_20710(pi, RX5G_REG6, core, rxdb_spare, rxdb_spare);
        }
    }
...
chanspec_setup_rxgcrs(phy_info_t *pi)
{
    ...
    if (CCT_BAND_CHG(pi_ac) || CCT_SUBBAND_CHG(pi_ac) ||
        (!ACMAJORREV_47_130(pi->pubpi->phy_rev) && CHSPEC_IS6G(pi->radio_chanspec))) {
    ...
    wlc_phy_rxgainctrl_set_gaintbls_acphy(pi,
        CCT_INIT(pi_ac), CCT_BAND_CHG(pi_ac) || CCT_SUBBAND_CHG(pi_ac), CCT_BW_CHG(pi_ac));
    ...
    for (desense_state = desense_state_init; desense_state >= 0; desense_state--) {
        ...
        if (TINY_RADIO(pi)) {
            ...
            wlc_phy_agc_28nm(pi, CCT_INIT(pi_ac),
                CCT_BAND_CHG(pi_ac) || CCT_SUBBAND_CHG(pi_ac),
                CCT_BW_CHG(pi_ac)); //TODO:Need to Check
        } else {
...
static void wlc_phy_mclip_agc_rssi_setup_rev130(phy_info_t *pi)
{
    ...
    uint8 ana_thld_20708rX_5g_bw160_b0_lna1byp_off[9]  = {3, 3, 1, 5, 5, 5, 0, 0, 0};
    ...
    FOREACH_CORE(pi, core) {
        ...
        if (RADIOMAJORREV(pi) < 2) {
            ...
            if (CHSPEC_IS2G(pi->radio_chanspec)) {
                ...
                if (CHSPEC_IS160(pi->radio_chanspec)) {
                    if (CHSPEC_CHANNEL(pi->radio_chanspec) < 129) {
                        phy_ac_set_20708_RSSI_detectors(pi, core,
                            ana_thld_20708rX_5g_bw160_b0);
                    } else {
                        phy_ac_set_20708_RSSI_detectors(pi, core,
                            ana_thld_20708rX_5g_bw160_b0_lna1byp_off);
                    }
                } else {
                    if (CHSPEC_IS5G(pi->radio_chanspec)) {
                        phy_ac_set_20708_RSSI_detectors(pi, core,
...
static void wlc_phy_mclip_agc_rssi_setup_rev131(phy_info_t *pi)
{
    ...
    FOREACH_CORE(pi, core) {
        ...
        if (CHSPEC_IS2G(pi->radio_chanspec)) {
            ...
            mclip_rssi[9] = mclip_rssi[8] + 7;    // W1hi = W1md + 15
        }
    }
}
...
void wlc_phy_mclip_agc_rxgainctrl(phy_info_t *pi)
{
    ...
    uint8 bw_indx = CHSPEC_IS20(pi->radio_chanspec) ? 0 :
            ...
            ACMAJORREV_47_130(pi->pubpi->phy_rev) ? 3 :
            ACMAJORREV_131(pi->pubpi->phy_rev) ? 3 : 2;
    ...
    uint32 clp2_tbl[8];
    uint8 tbl;
    uint8 ChNum = CHSPEC_CHANNEL(pi->radio_chanspec);
        ...
        if (elna_present) {
            ...
            if (pi_ac->rxgcrsi->curr_desense->elna_bypass) {
                ...
                if (ACMAJORREV_130(pi->pubpi->phy_rev) && (chspec_is160)) {
                    elnaByp_gain_idx =
                        (RADIOMAJORREV(pi) < 2) ? MCLIP_W1_MID:
                        (ChNum < 129) ? MCLIP_W1_LOW : MCLIP_NB1_HI;
                }
                if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
                    // BCAWLAN-225793, BCAWLAN-228870, BCAWLAN-228904
                    elnaByp_gain_idx = MCLIP_NB1_HI;
                }
            }
        }
    ...
    */
    for (i = 0; i < loop_count; i++) {
        if (ACMAJORREV_GE47(pi->pubpi->phy_rev) &&
            !ACMAJORREV_128(pi->pubpi->phy_rev) &&
            CHSPEC_IS160(pi->radio_chanspec)) {
            // IFS_STAT_CRS, bits 15:8 are ED and 7:0 are phycrs
            reg_val = R_REG(pi->sh->osh, D11_IFS_STAT_CRS(pi));
            reg_val = reg_val >> 8;
        } else {
...
phy_ac_rxgcrs_lesi(phy_ac_rxgcrs_info_t *rxgcrsi, bool on, uint8 delta_halfdB)
{
    ...
    uint8 low_regime_rev131_5g = 83;
    uint8 low_regime_rev131_6g = 80;
        ...
        if (ACMAJORREV_47_129_130_131(pi->pubpi->phy_rev)) {
            ...
            if (CHSPEC_IS2G(pi->radio_chanspec)) {
                ...
                if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
                  ...
                  upper = (CHSPEC_IS6G(pi->radio_chanspec) ? low_regime_rev131_6g :
                           low_regime_rev131_5g);
                  lower = upper - 3;
                } else {
                    upper = low_regime_5g;
                    lower = upper - 6;
            ...
            if (!ACMAJORREV_130(pi->pubpi->phy_rev)) {
                ...
                if (RADIOREV(pi->pubpi->radiorev) < 2) {
                    ...
                    MOD_PHYREG(pi, lesiFstrModeSwitchLoPower,
                        fstrSwitchPwrLo1c, chspec_is20 ?
                        (pdet_range_id != 17) ? 0x134 : 0xd8 :
                        chspec_is40 ? 0x2e0 : chspec_is80 ? 0x2e0 : 0x27b);
                    MOD_PHYREG(pi, lesiFstrModeSwitchHiPower,
                        fstrSwitchPwrHi1c, chspec_is20 ?
                        (pdet_range_id != 17) ? 0x2a1 : 0x1d7 :
...
#define PHYTS_N_TIA_GAINS 4
#define PHYTS_N_LPF_GAINS 8

uint32
phy_ac_rxgcrs_phyts_encode_rxgain_bundle(phy_info_t *pi, int8 req_gaindB, int8 *gain_applied)
{
    uint8 gm_idx = 2u;
    uint8 req_gain = req_gaindB;
    int8 max_tia_gain, max_lpf_gain, i;
    uint8 tia_idx = 0, lpf_idx = 0, vpath_sw = 0;
    int8 tia_gain = 0, lpf_gain = 0;
    int8 tot_gain_applied = 0;
    uint32 rxgain_bundle = 0;

    /* 3dB gain is provided by matching network */
    req_gain = req_gain - 3;
    tot_gain_applied += 3;

    /* Find TIA gain index */
    max_tia_gain = req_gain;   /* LPF min gain is 0dB */
    for (i = PHYTS_N_TIA_GAINS - 1; i >= 0; i--) {
        tia_gain = IS_SLICE_MAIN(pi) ? tiaStage_Gains_Main[i] :
            tiaStage_Gains_Aux[i];
        if (tia_gain <= max_tia_gain) {
            tia_idx = i;
            break;
        }
    }
    tot_gain_applied += tia_gain;
    req_gain = req_gain - tia_gain;

    /* Find LPF gain index */
    max_lpf_gain = req_gain;
    for (i = PHYTS_N_LPF_GAINS - 1; i >= 0; i--) {
        lpf_gain = (i * FLEX_LPF_GAIN_STEP);
        if (lpf_gain <= max_lpf_gain) {
            lpf_idx = i;
            break;
        }
    }
    tot_gain_applied += lpf_gain;
     ...
     */
    vpath_sw = (tia_idx <= 3u) ? 0u : 1u;
    rxgain_bundle = ((1u << 15u) | (vpath_sw << 14u) | (lpf_idx << 11u) | (tia_idx << 8u) |
        (gm_idx << 5u));
    *gain_applied = tot_gain_applied;
    return rxgain_bundle;
}
