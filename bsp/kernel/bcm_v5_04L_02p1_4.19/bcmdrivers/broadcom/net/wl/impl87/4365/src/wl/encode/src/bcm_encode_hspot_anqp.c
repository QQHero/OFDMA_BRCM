	...
	uint16 providerLength, uint8 *provider)
{
	...
	bcm_encode_le16(pkt, nonTransProfileLength);
	if (nonTransProfileLength > 0) {
		bcm_encode_bytes(pkt, nonTransProfileLength, nonTransProfile);
	}
	bcm_encode_byte(pkt, legacyOsuSsidLength);
	if (legacyOsuSsidLength > 0) {
		bcm_encode_bytes(pkt, legacyOsuSsidLength, legacyOsuSsid);
	}
	if (providerLength > 0) {
