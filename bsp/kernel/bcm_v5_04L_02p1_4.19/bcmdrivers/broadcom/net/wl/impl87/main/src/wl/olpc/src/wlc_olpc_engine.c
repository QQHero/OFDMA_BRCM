...
#define WL_OLPC_INITCAL_ENAB_DEF 1
...
/* wlc_olpc_check's defines for tx power index failure range.
 * All chips: PHY's index is 0 (max power) to 128 (min power).
 */
#define PHY_TXPWR_INDEX_MAX 0
#define PHY_TXPWR_INDEX_MIN 128
/* When checking the power index, allow a little empirically observed margin */
#define OLPC_CHECK_TXPWRIDX_HI_FAIL  (PHY_TXPWR_INDEX_MIN - 3)
#define OLPC_CHECK_TXPWRIDX_LOW_FAIL (PHY_TXPWR_INDEX_MAX + 2)

/* wlc_olpc_check's defines for TSSI visibility threshold in qdBm.
 * All chips & boards: the visi thresh should never be
 *   above 30 dB (1 W) - no room for CLPC
 *   below 1 dB (1.26 mW) - detectors don't go that low, but it's
 *    more a check for '0' which means the threshold is unset in SROM.
 */
#define OLPC_CHECK_TSSITHRESH_HI_FAIL  (30 * 4) /* 30 dBm in qdBm */
#define OLPC_CHECK_TSSITHRESH_LOW_FAIL (1 * 4)  /* 1 dBm in qdBm */
...
static const bcm_iovar_t olpc_iovars[] = {
	...
	{"olpc_initcal", IOV_OLPC_INITCAL, (IOVF_SET_DOWN), 0, IOVT_BOOL, 0},
	{"olpc_autocal", IOV_OLPC_AUTOCAL, 0, 0, IOVT_BOOL, 0},
	{"olpc_autocal_period", IOV_OLPC_AUTOCAL_PER, 0, 0, IOVT_UINT32, 0},
	{"olpc_check", IOV_OLPC_CHECK, IOVF_SET_UP | IOVF_GET_UP, 0, IOVT_BOOL, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
static int wlc_olpc_get_min_beacon_pwr(wlc_olpc_eng_info_t *olpc_info,
	ppr_t *txpwr, chanspec_t channel);
...
static int
wlc_olpc_check_iovar(wlc_olpc_eng_info_t *olpc, bool verbose);
...
static void wlc_olpc_bsscfg_updn(void *ctx, bsscfg_up_down_event_data_t *evt)
{
	...
	if (evt->up) {
		if (!wlc_olpc_find_other_up_on_chan(olpc->wlc, evt->bsscfg) &&
			...
			if (olpc->initcal_enabled) {
				WL_OLPC(olpc, ("%s: BSS (%p) up, ignored (others already up)\n",
					__FUNCTION__, OSL_OBFUSCATE_BUF(evt->bsscfg)));
			} else {
				WL_OLPC(olpc, ("%s: BSS (%p) up, initcal is turned off\n",
					__FUNCTION__, OSL_OBFUSCATE_BUF(evt->bsscfg)));
			}
		}
	} else {
...
wlc_olpc_eng_terminate_cal(wlc_info_t *wlc)
{
	...
	if (olpc_info->cur_chan && (olpc_info->last_txchain_chk != wlc->stf->txchain) &&
		wlc_olpc_chan_has_active_cal(wlc->olpc_info, olpc_info->cur_chan)) {
		WL_OLPC_DBG(olpc_info, ("%s: Calibration terminated due to txchain change\n",
			__FUNCTION__));
		wlc_olpc_chan_terminate_active_cal(wlc, olpc_info->cur_chan);
	}
	return BCME_OK;
...
static int
wlc_olpc_get_min_beacon_pwr(wlc_olpc_eng_info_t *olpc,
	ppr_t *txpwr, chanspec_t channel)
{
	int8 sdmmin;
	int min_txpwr_limit = 128; // max txpwr is limited to 128qdbm
	...
	wl_tx_bw_t txbw;
	wl_tx_mode_t txmode;
	if (PPR_CHSPEC_BW(channel) == WL_TX_BW_20) {
		txbw = WL_TX_BW_20;
	} else if (PPR_CHSPEC_BW(channel) == WL_TX_BW_40) {
		txbw = WL_TX_BW_20IN40;
	} else if (PPR_CHSPEC_BW(channel) == WL_TX_BW_80) {
		txbw = WL_TX_BW_20IN80;
	} else {
		txbw = WL_TX_BW_20IN160;
	}
	if ((olpc->wlc->stf->psd_limit_indicator == 1 &&
		olpc->wlc->lpi_mode == AUTO) || olpc->wlc->lpi_mode == ON) {
		txbw = PPR_CHSPEC_BW(channel);
	}

	txmode = WL_TX_MODE_CDD;
	if (txchains == WL_TX_CHAINS_1) {
		txmode = WL_TX_MODE_NONE;
	}
	err = ppr_get_ofdm_min(txpwr, txbw, txmode, txchains, &sdmmin);
	if (err == BCME_OK) {
		if (txchains == WL_TX_CHAINS_1) {
			WL_OLPC_ENTRY(olpc, ("wl%d:%s: nss1 min=%d sdmmin=%d\n",
			olpc->wlc->pub->unit, __FUNCTION__,
			...
			WL_OLPC_ENTRY(olpc, ("wl%d:%s: nss1_cdd%d min=%d sdmmin=%d\n",
				olpc->wlc->pub->unit, __FUNCTION__, txchains-1,
				min_txpwr_limit, sdmmin));
		}
		if (sdmmin != WL_RATE_DISABLED) {
			min_txpwr_limit = MIN(min_txpwr_limit, sdmmin);
...
wlc_olpc_get_min_tgt_pwr(wlc_olpc_eng_info_t *olpc, chanspec_t channel)
{
	...
	if (BAND_6G(wlc->band->bandtype)) {
		cur_min = wlc_olpc_get_min_beacon_pwr(olpc, txpwr, channel);
	} else
	{
	...
	cur_min = WL_OLPC_PPR_TO_TSSI_PWR(cur_min);
...
wlc_olpc_eng_pkt_complete(wlc_info_t *wlc, void *pkt, uint txs)
{
	...
	if (olpc_chan->cores_cal == wlc->stf->txchain) {
...
#endif /* PHYCAL_CACHING || WLMCHAN */
		wlc_phy_txpower_get(pi, &qdbm, &override);
		if (BAND_6G(wlc->band->bandtype) && olpc->wlc->clk) {
			bcnpwr = wlc_olpc_get_min_tgt_pwr(olpc, olpc->wlc->chanspec);
		}
		...
		if ((bcnpwr < tssi_visi_thresh) || (override && (qdbm < tssi_visi_thresh))) {
...
wlc_olpc_send_dummy_pkts(wlc_info_t *wlc, wlc_olpc_eng_chan_t* chan, uint8 npkts)
{
	...
	int prec;
	wlc_olpc_eng_info_t *olpc_info = wlc->olpc_info;
	int8 core_idx = olpc_info->core_idx;
	bool override;
	phy_info_t *pi = WLC_PI(wlc);
	int8 qdbm;
	wlc_phy_txpower_get(pi, &qdbm, &override);
	...
	if (D11REV_GE(wlc->pub->corerev, 128) && override) {
	...
	for (pktnum = 0; pktnum < npkts; pktnum++) {
		...
		if (!cpktq_prec_enq(wlc, &wlc->active_queue->cpktq, pkt, prec, TRUE)) {
	...
	void *params, uint p_len, void *arg, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	if (BAND_6G(wlc->band->bandtype) && olpc->wlc->clk) {
		qdbm = wlc_olpc_get_min_tgt_pwr(olpc, olpc->wlc->chanspec);
	}
	switch (actionid) {
#if WL_OLPC_IOVARS_ENAB
		...
		case IOV_GVAL(IOV_OLPC_FORCE_CAL):
			...
			if (chan) {
				...
				wlc->olpc_info->core_idx = 0;
				err = wlc_olpc_eng_recal_ex(wlc, olpc->npkts);
		...
		case IOV_SVAL(IOV_OLPC_INITCAL):
			if (olpc->initcal_enabled != bool_val) {
				olpc->initcal_enabled = bool_val;
			}
			break;
		case IOV_GVAL(IOV_OLPC_INITCAL):
			*ret_int_ptr = olpc->initcal_enabled;
			break;
		...
		case IOV_SVAL(IOV_OLPC_CHECK):
		case IOV_GVAL(IOV_OLPC_CHECK):
			*ret_int_ptr = -1;
			/* Scanning or calibration invalidiate the check */
			if (is_scanning || wlc_olpc_eng_has_active_cal(olpc)) {
				err = BCME_BUSY;
			} else {
				*ret_int_ptr = 0;
				err = wlc_olpc_check_iovar(olpc, int_val ? TRUE:FALSE);
			}
			break;
...
wlc_dump_olpc(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "\nOLPC is %s - count = %llu, last = %u, now = %u",
...
BCMATTACHFN(wlc_olpc_eng_attach)(wlc_info_t *wlc)
{
	...
	olpc_info->initcal_enabled = WL_OLPC_INITCAL_ENAB_DEF;
...
wlc_olpc_autocal_timer_reload(wlc_olpc_eng_info_t *olpc)
{
	...
	if (BAND_6G(olpc->wlc->band->bandtype) && olpc->wlc->clk) {
		qdbm = wlc_olpc_get_min_tgt_pwr(olpc, olpc->wlc->chanspec);
	}
...
wlc_olpc_check_iovar(wlc_olpc_eng_info_t *olpc, bool verbose)
{
	wlc_info_t *wlc = olpc->wlc;
	int i, qdbm;
	uint32 phytxpwrindices, txpwridx[WL_OLPC_MAX_NUM_CORES] = { 0 };
	uint chainmap;

	/* 1. OLPC disabled is a fail */
	if (!OLPC_ENAB(wlc)) {
		if (verbose) {
			WL_PRINT(("%s Error: olpc not enabled\n", __FUNCTION__));
		}
		return BCME_DISABLED;
	}

	/* 2. Chip down is a fail */
	if (!wlc->pub->up) {
		if (verbose) {
			WL_PRINT(("%s Error: chip is not up\n", __FUNCTION__));
		}
		return BCME_NOTUP;
	}

	/* 3. TSSI Visibility Threshold (CPLC/OLPC cut-off) < 1 dBm is a fail */
	qdbm = wlc_phy_tssivisible_thresh(WLC_PI(wlc));
	WL_OLPC_DBG(olpc, ("%s tssivisi_thresh %d qdbm\n", __FUNCTION__, qdbm));
	if ((qdbm < OLPC_CHECK_TSSITHRESH_LOW_FAIL) ||
		(qdbm > OLPC_CHECK_TSSITHRESH_HI_FAIL)) {
		if (verbose) {
			WL_PRINT(("%s Error: bad tssivisi_thresh %d qdBm\n",
				__FUNCTION__, qdbm));
		}
		return BCME_RANGE;
	}

	/* 4. PHY's per-chain tx power indices, at the rails, is a fail */
	phytxpwrindices = wlc_phy_txpwr_idx_get_acphy(wlc->pi);
	txpwridx[0] = phytxpwrindices & 0xff;
	txpwridx[1] = (phytxpwrindices & 0xff00) >> 8;
	txpwridx[2] = (phytxpwrindices & 0xff0000) >> 16;
	txpwridx[3] = (phytxpwrindices & 0xff000000) >> 24;
	/* Don't check power on disabled chains */
	chainmap = wlc->stf->txchain;
	WL_OLPC_DBG(olpc, ("%s phy_txpwrindex %d %d %d %d chainmap 0x%x\n",
		__FUNCTION__, txpwridx[0], txpwridx[1], txpwridx[2],
		txpwridx[3], chainmap));
	for (i = 0; i < WL_OLPC_MAX_NUM_CORES; i++) {
		WL_OLPC_DBG(olpc, ("%s index check: txchain %d (chainmap 0x%x & 0x%x)\n",
			__FUNCTION__, i, chainmap, (1<<i)));
		/* If chain is active */
		if ((1<<i) & chainmap) {
			WL_OLPC_DBG(olpc, ("%s txchain %d active, phytxpwr index=%d\n",
				__FUNCTION__, i, txpwridx[i]));
			/* It's an error if the PHY's tx power is near the rails */
			if ((txpwridx[i] > OLPC_CHECK_TXPWRIDX_HI_FAIL) ||
				(txpwridx[i] < OLPC_CHECK_TXPWRIDX_LOW_FAIL)) {
				if (verbose) {
					WL_PRINT(("%s Error: Chain #%d bad txpwrindex %d\n",
						__FUNCTION__, i, txpwridx[i]));
				}
				return BCME_TXFAIL;
			}
