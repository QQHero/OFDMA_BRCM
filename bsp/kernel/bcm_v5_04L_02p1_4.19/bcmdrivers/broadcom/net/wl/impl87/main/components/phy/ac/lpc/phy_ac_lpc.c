 ...
 *
 * $Id: phy_ac_lpc.c 689894 2017-03-13 23:32:18Z $
 */
...
/* local functions */
static uint8 phy_ac_lpc_getminidx(void);
static uint8 phy_ac_lpc_getoffset(uint8 index);
#ifdef WL_LPC_DEBUG
static uint8 * phy_ac_lpc_get_pwrlevelptr(void);
#endif
...
BCMATTACHFN(phy_ac_lpc_register_impl)(phy_info_t *pi, phy_ac_info_t *aci, phy_lpc_info_t *cmn_info)
{
	...
	fns.ctx = ac_info;
	fns.setmode = NULL;
	fns.gettxcpwrval = NULL;
	fns.settxcpwrval = NULL;
	fns.getpwros = phy_ac_lpc_getoffset;
	fns.getminidx = phy_ac_lpc_getminidx;
#ifdef WL_LPC_DEBUG
	fns.getpwrlevelptr = phy_ac_lpc_get_pwrlevelptr;
#endif
...
static uint8
phy_ac_lpc_getminidx(void)
{
	return LPC_MIN_IDX;
}

static uint8
BCMRAMFN(phy_ac_lpc_getoffset)(uint8 index)
{
...
#ifdef WL_LPC_DEBUG
static uint8 *
BCMRAMFN(phy_ac_lpc_get_pwrlevelptr)(void)
{
