 ...
 *
 * $Id: wbd_tlv.h 801316 2021-07-20 10:11:09Z $
 */
...
#define WBD_MIN_LEN_TLV_STRONG_STA_POLICY		1
#define WBD_MAX_LEN_TLV_STRONG_STA_POLICY_ITEM		22

/* Modify wbd_tlv_list_type_name when modifying this.
 * Values 1 to 127 are reserved for Broadcom Vendor TLVs.
 * Clients must use values more than 127 to define their Vendor TLVs.
 */
typedef enum wbd_tlv_types {
	...
	WBD_TLV_VNDR_STRONG_STA_POLICY_TYPE		= 20,
	WBD_TLV_MAX_BRCM_TYPE				= 127
} wbd_tlv_types_t;
...
extern int wbd_tlv_decode_vndr_metric_policy(void* data,
	unsigned char* tlv_data, unsigned int tlv_data_len, unsigned int *bytes_read);

/* Encode Vendor Specific TLV for Message : weak/strong client response */
extern int wbd_tlv_encode_weak_strong_client_response(void* data, unsigned char* tlv_data,
	unsigned int* tlv_data_len);

/* Decode Vendor Specific TLV for Message : weak/strong client response on recieve */
extern int wbd_tlv_decode_weak_strong_client_response(void* data, unsigned char* tlv_data,
	unsigned int tlv_data_len);
...
extern void wbd_tlv_encode_vndr_strong_sta_policy(void* data,
	unsigned char* tlv_data, unsigned int* tlv_data_len);

/* Decode Vendor Specific TLV : Strong Sta Reporting Policy Vendor Data on receive */
extern int wbd_tlv_decode_vndr_strong_sta_policy(void* data,
	unsigned char* tlv_data, unsigned int tlv_data_len, unsigned int *bytes_read);
#endif /* _WBD_TLV_H_ */
