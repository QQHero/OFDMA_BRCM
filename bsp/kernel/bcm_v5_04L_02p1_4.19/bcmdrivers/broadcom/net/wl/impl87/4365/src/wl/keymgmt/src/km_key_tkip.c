 ...
 * $Id: km_key_tkip.c 795538 2021-02-09 07:19:20Z $
 */
...
#else /* causes approx 1KB ROM abandon on 4365c0 */
		 KEY_ERR(("wl%d: %s: TKIP MIC failure for frame"
			" from %08x%04x to %08x%04x"
			", key idx %d, expected %08x.%08x got %08x.%08x\n",
			KEY_WLUNIT(key), __FUNCTION__,
			ntoh32_ua(&hdr->ether_shost[0]), ntoh16_ua(&hdr->ether_shost[4]),
			ntoh32_ua(&hdr->ether_dhost[0]), ntoh16_ua(&hdr->ether_dhost[4]),
			key->info.key_idx, micl, micr, rx_micl, rx_micr));
