 ...
 *
 * $Id: wlc_frag.c 803989 2021-10-14 19:10:00Z $
 */
	...
	struct wlc_frminfo *f)
{
	...
	/* if p is from shared pool, use heap instead to not occupy limited shared pool */
	if (((uint)((PKTTAILROOM(osh, p) + PKTHEADROOM(osh, p) + PKTLEN(osh, p))) < rxbufsz) ||
#ifdef BCMPKTPOOL
		(lb_getpool((struct lbuf *)p) == SHARED_POOL) ||
#endif /* BCMPKTPOOL */
		FALSE) {
	...
	if (scb_frag->fragbuf[prio] != p) {
		/* Copy wl pkttag area */
		wlc_pkttag_info_move(wlc, p, scb_frag->fragbuf[prio]);
