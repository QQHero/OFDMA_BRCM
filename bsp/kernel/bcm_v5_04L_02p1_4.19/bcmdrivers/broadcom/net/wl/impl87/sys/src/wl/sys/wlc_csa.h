 ...
 *
 * $Id: wlc_csa.h 806838 2022-01-05 16:38:14Z $
*/
...
/* APIs */
#ifdef WLCSA
typedef struct wlc_csa_cb_data {
	wlc_bsscfg_t *cfg;
	int status;
	int signal;
	chanspec_t new_chspec;
	chanspec_t old_chspec;
...
enum CSA_NOTIF_SIGNALS {
	CSA_CHANNEL_CHANGE_START_INFRA = 1,
	CSA_CHANNEL_CHANGE_START_PBM = 2,
	CSA_CHANNEL_CHANGE_END = 3
};

typedef void (*wlc_csa_cb_fn_t)(void *ctx, wlc_csa_cb_data_t *notif_data);
...
extern int wlc_csa_send_rrm_report_and_csa_to_upstream_ap(wlc_csa_info_t *csam,
	wlc_bsscfg_t *cfg, wl_chan_switch_t *csa, uint8 radarch_len, uint8 *radarch);
#endif /* SLAVE_RADAR */
extern void  wlc_csa_prepare_csa_info(wlc_info_t *wlc, wlc_bsscfg_t *cfg, chanspec_t chspec,
...
extern void wlc_csa_each_up_ap(wlc_info_t *wlc, wl_chan_switch_t *csa, bool docs);
extern void wlc_csa_reset_each_ap(wlc_info_t *wlc);
...
extern int wlc_csa_cb_register(wlc_csa_info_t *csam, wlc_csa_cb_fn_t cb, void *arg);
extern int wlc_csa_cb_unregister(wlc_csa_info_t *csam, wlc_csa_cb_fn_t cb, void *arg);
extern chanspec_t wlc_csa_max_bw_chspec(wlc_info_t *wlc, chanspec_t chanspec);
...
#define wlc_csa_each_up_ap(wlc, csa, docs)  do {} while (0)
#define wlc_csa_reset_each_ap(wlc)  do {} while (0)
...
#define wlc_csa_cb_register(csam, cb, arg)  0
#define wlc_csa_cb_unregister(csam, cb, arg)  0
