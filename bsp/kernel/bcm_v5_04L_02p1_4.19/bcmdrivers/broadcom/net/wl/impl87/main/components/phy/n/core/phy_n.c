 ...
 *
 * $Id: phy_n.c 669568 2016-11-10 05:45:58Z $
 */
...
BCMATTACHFN(phy_n_register_impl)(phy_info_t *pi, phy_type_info_t *ti, int bandtype)
{
	...
	if (pi->cachei != NULL &&
	    (ni->cachei = phy_n_cache_register_impl(pi, ni, pi->cachei)) == NULL) {
		PHY_ERROR(("%s: phy_n_cache_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with MISC module */
	if (pi->misci != NULL &&
		(ni->misci = phy_n_misc_register_impl(pi, ni, pi->misci)) == NULL) {
		PHY_ERROR(("%s: phy_n_misc_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with Channel Manager module */
	if (pi->chanmgri != NULL &&
		(ni->chanmgri = phy_n_chanmgr_register_impl(pi, ni, pi->chanmgri)) == NULL) {
		PHY_ERROR(("%s: phy_n_chanmgr_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

	/* Register with BTCX module */
	if (pi->btcxi != NULL &&
		(ni->btcxi = phy_n_btcx_register_impl(pi, ni, pi->btcxi)) == NULL) {
		PHY_ERROR(("%s: phy_n_btcx_register_impl failed\n", __FUNCTION__));
		goto fail;
	}

		/* Register with Link Power Control module */
	if (pi->lpci != NULL &&
		(ni->lpci = phy_n_lpc_register_impl(pi, ni, pi->lpci)) == NULL) {
...
BCMATTACHFN(phy_n_unregister_impl)(phy_info_t *pi, phy_type_info_t *ti)
{
	...
	if (ni->lpci != NULL)
		phy_n_lpc_unregister_impl(ni->lpci);

	/* Unregister from BTCX control module */
	if (ni->btcxi != NULL)
		phy_n_btcx_unregister_impl(ni->btcxi);

	/* Unregister from Channel Manager module */
	if (ni->chanmgri != NULL)
		phy_n_chanmgr_unregister_impl(ni->chanmgri);

	/* Unregister from MISC module */
	if (ni->misci != NULL)
		phy_n_misc_unregister_impl(ni->misci);

	/* Unregister from CACHE module */
	if (ni->cachei != NULL)
		phy_n_cache_unregister_impl(ni->cachei);
