 ...
 *
 * $Id: wlu_common.h 806789 2022-01-04 03:10:10Z $
 */

#ifndef _wlu_common_h_
...
#ifdef _CFE_			/* router boot loader */
#define CALLOC(n_bytes) ({		\
	void *p;					\
	p = KMALLOC((n_bytes), 0);	\
	if (p != NULL) {			\
		memset(p, 0, n_bytes);	\
...
/* structure to hold scan parse params */
typedef struct wl_scan_parse_param {
	...
	wlc_ssid_t ssid[WL_SCAN_PARAMS_SSID_MAX];
	uint32	n_short_ssid;
	uint32 short_ssid[WL_SCAN_PARAMS_SSID_MAX];
} wl_scan_parse_param_t;
...
extern cmd_func_t wlu_pktdrop_stats;
...
extern cmd_func_t wl_wlc_ver, wl_scan_ver;
...
extern int wl_mk_ie_setbuf(void *wl, const char *command, uint32 pktflag_ok, char **argv,
	vndr_ie_setbuf_t *ie_setbuf);
extern cmd_func_t wl_list_ie;
extern int wl_clr_ie(void *wl, cmd_t *cmd, char* buf, char **argv);
extern int wl_vndr_ie_ver(void *wl);
...
extern int wl_scan_parse_params(void *wl, cmd_t *cmd, char **argv, int *params_size,
		wl_scan_parse_param_t *scan_params, uint16 version);
extern int wl_scan_init_scan_parse_params(wl_scan_parse_param_t *params);
...
extern void wl_bands_print(uint8 *bands);

enum bandstring_e {
	BND_IS_NUMERICAL,
	BND_IS_STRING,
	BND_IS_STRING_INC_BAND_ALL, /**< 'all' as a band string is allowed */
	BND_IS_STRING_INC_BAND_AUTO /**< 'auto' as a band string is allowed */
};

extern bool str2bandtype(char *str, uint32 *band, enum bandstring_e);

#endif /* _wlu_common_h_ */
