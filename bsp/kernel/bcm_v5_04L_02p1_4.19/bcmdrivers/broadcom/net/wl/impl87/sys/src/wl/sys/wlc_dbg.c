 ...
 *
 * $Id: wlc_dbg.c 794016 2020-12-17 05:49:28Z $
 */
...
#include <bcmwifi_rspec.h>
	...
	defined(WLMSG_ASSOC) || (defined(CFP_DEBUG) && (CFP_DEBUG_LEVEL != CFP_DEBUG_NONE))
...
wlc_dump_vht_plcp(uint8 *plcp)
{
	...
	(defined(CFP_DEBUG) && (CFP_DEBUG_LEVEL != CFP_DEBUG_NONE))
static void
...
#if defined(BCMDBG) || defined(WLTEST) || defined(BCMDBG_DUMP_MUTX) || \
	defined(BCMDBG_AMPDU) || defined(DL_RU_STATS_DUMP) || defined(UL_RU_STATS_DUMP)
void
...
#define MAX_COLUM 12
	...
	uint col, value;
	int sum, last, titlelen = 0, unacked, pcent, i, pad, max_colums = 1;
	...
	if (max_idx <= nbr_colums) {
		/* table with one line only, don't display zero at the end */
		nbr_colums = max_colums + 1;
		last = max_colums;
	}
	...
	switch (table_type) {
		...
		case TABLE_NSS:
			...
			for (col = 0; col < nbr_colums; col++) {
				...
				bcm_bprintf(b, "%*s%d  ", pad, "NSS", col+1);
			}
			bcm_bprintf(b, "\n");
			break;
		...
		case TABLE_LEN:
			bcm_bprintf(b, "%*s  ", titlelen, " ");
			for (col = 0; col < nbr_colums; col++) {
				pad = table_width[col]-1;
				if (col > 9) pad--;

				bcm_bprintf(b, "%*s%d-%dk   ", pad, "", col, col+1);
			}
			bcm_bprintf(b, "\n");
			break;
...
#endif /* BCMDBG || BCMDBG_DUMP || WLTEST || BCMDBG_DUMP_MUTX ||
		* BCMDBG_AMPDU || DL_RU_STATS_DUMP || UL_RU_STATS_DUMP || WL_EAP_TPDUMP
		*/
