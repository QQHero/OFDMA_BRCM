 ...
 *
 * $Id: wlc_assoc.h 802080 2021-08-16 04:00:23Z $
 */
...
#define AS_WAIT_AUTH_CONFIRM	39	/* Wait for SAE confirm from upper layer or peer device */
...
/* Additional Auth Status Success cases for WPA3, in addition to DOT11_SC_SUCCESS */
#define WPA3_VALID_AUTH_STATUS(auth_status)  ((auth_status == DOT11_SC_ANTICLOG_TOCKEN_REQUIRED) ||\
					     (auth_status == DOT11_SC_SAE_HASH_TO_ELEMENT) ||\
					     (auth_status == DOT11_SC_INVALID_FINITE_CYCLIC_GRP))
...
struct wlc_assoc {
	...
	dh_params_ie_t *dh_params;		/* Diffie-Hellman Parameter */
};
...
extern void wlc_assoc_dh_params_free(wlc_info_t *wlc, wlc_bsscfg_t *cfg);
