...
#
# $Id: epivers.sh 795502 2021-02-08 11:00:41Z $

if [ -f epivers.h ]; then

	# if git is installed, try to extract a git log.
	# If found, create epicustomver.h with the last git hash if different
	if command -v git &> /dev/null;	then
		GIT_ID=$(git log --oneline -1 2> /dev/null | cut -d ' ' -f 1)
	fi
	if [ -n "$GIT_ID" ]; then
		if [ ! -f epicustomver.h ] || [ `grep -c $GIT_ID epicustomver.h` -eq 0 ]; then
			echo "git hash is $GIT_ID"
			echo -e "#undef  EPI_CUSTOM_VER\n#define EPI_CUSTOM_VER \" [$GIT_ID]\"" > epicustomver.h
			exit 0
		fi
	fi
	...
	SVNURL='$HeadURL: svn://bcawlan-svn.lvn.broadcom.net/svn/bcawlan/proj/tags/KUDU/KUDU_REL_17_10_188_75/src/include/epivers.sh $'
	...
	extra_ver_info=${EXTRAVER}
	if [ -n "${extra_ver_info}" ]; then
		extra_ver_info=" ${extra_ver_info}"
	fi
	...
	sed \
		...
		-e "s;@EPI_EXTRAVER@;${extra_ver_info};" \
