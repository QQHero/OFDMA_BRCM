 ...
 *
 * $Id: wlc_prot_n.c 795628 2021-02-11 05:12:45Z $
 */
...
static const bcm_iovar_t prot_n_iovars[] = {
	{"mimo_preamble", IOV_PREAMBLE_OVERRIDE, (IOVF_SET_DOWN|IOVF_OPEN_ALLOW), 0, IOVT_INT8, 0},
...
BCMATTACHFN(wlc_prot_n_attach)(wlc_info_t *wlc)
{
	...
	/* error handling */
fail:
	MODULE_DETACH(prot, wlc_prot_n_detach);
	return NULL;
}
...
static int
wlc_prot_n_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
...
wlc_prot_n_cond_upd(wlc_prot_n_info_t *prot, struct scb *scb)
{
	...
	if ((scb->flags & SCB_NONERP) == 0 && !SCB_HT_CAP(scb))
...
wlc_prot_n_build_add_ie(wlc_prot_n_info_t *prot, wlc_bsscfg_t *cfg, ht_add_ie_t *add_ie)
{
	...
	/* CCFS2 has to be filled when 160/80p80 Mhz is operational at reduced
	 * NSS than max operational NSS. Table 9-274 IEEE P802.11-REVmd/D3.4
	 */
