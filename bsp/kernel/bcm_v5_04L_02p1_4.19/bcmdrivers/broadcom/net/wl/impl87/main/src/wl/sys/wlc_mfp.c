 ...
 *
 * $Id: wlc_mfp.c 810463 2022-04-07 12:15:53Z $
 *
...
/* timeout in milliseconds to wait for SA query response */
#define WLC_MFP_SA_QUERY_TIMEOUT_MS	200

/* total time in seconds to wait for SA query to complete */
#define SA_QUERY_TOTAL_TIME_SECONDS 2

/* association come back time interval - in TUs - must be greater
 * than SA query max timeout
 * Add margin over SA Query timeout for the response packet and convert from ms to TU.
 ...
 */
#define WLC_MFP_COMEBACK_TIE_TU		(((WLC_MFP_SA_QUERY_TIMEOUT_MS + 100) * 1000) >> 10)
...
static void wlc_mfp_watchdog(void *arg);
...
static void wlc_mfp_sa_query_cb(wlc_info_t *wlc, void *pkt, uint txstatus);
static void wlc_mfp_sa_query_fail(wlc_info_t *wlc, scb_t *scb);
...
static void mfp_sa_query_timeout(void *arg);
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_MFP_BIP): {
		...
		if (memcmp((const uint8 *)BIP_OUI_TYPE, (uint8 *)arg, WPA_SUITE_LEN) == 0) {
			...
			bss_mfp->bip_type = 0; //reset bip_type if none from host
		}
		else {
...
static void
wlc_mfp_watchdog(void *arg)
{
	wlc_mfp_info_t *mfp = (wlc_mfp_info_t *)arg;
	wlc_info_t *wlc = mfp->wlc;
	struct scb_iter scbiter;
	struct scb *scb;
	mfp_scb_t* scb_mfp;
	...
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		saq = &scb_mfp->saq;
		if (saq->started &&
			((wlc->pub->now - saq->started) > SA_QUERY_TOTAL_TIME_SECONDS)) {
			WL_ASSOC(("wl%d.%d: "MACF" %s timeouts %d\n", WLCWLUNIT(wlc),
				WLC_BSSCFG_IDX(SCB_BSSCFG(scb)), ETHER_TO_MACF(scb->ea),
				__FUNCTION__, saq->timeouts));
			wl_del_timer(wlc->wl, saq->timer);
			saq->timeouts = 0;
			saq->started = 0;
			wlc_mfp_sa_query_fail(wlc, scb);
		}
	}
}
...
mfp_scb_dump(void *ctx, struct scb *scb, struct bcmstrbuf *b)
{
	...
	if (saq->started)
		bcm_bprintf(b, "\t\tid: 0x%04x timeouts: %d\n", saq->id, saq->timeouts);
}
#else
...
BCMATTACHFN(wlc_mfp_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_module_register(wlc->pub, mfp_iovars, "mfp", mfp, mfp_doiovar,
		wlc_mfp_watchdog, NULL /* up */, wlc_mfp_down /* down */) != BCME_OK)
		goto err;
	...
	mfp->h_scb = wlc_scb_cubby_reserve(wlc, sizeof(mfp_scb_t),
		NULL, mfp_scb_deinit, mfp_scb_dump, (void*)mfp);
	if (mfp->h_scb < 0)
		goto err;
	...
	/* Register packet class callback, SA Query Action Frames */
	if (wlc_pcb_fn_set(wlc->pcb, 0, WLF2_PCB1_MFP_SA, wlc_mfp_sa_query_cb) != BCME_OK) {
		goto err;
	}
...
/* sa query failed, release the SCB */
static void
wlc_mfp_sa_query_fail(wlc_info_t *wlc, scb_t *scb)
{
	wlc_bsscfg_t *cfg;

	cfg = SCB_BSSCFG(scb);

	WL_ASSOC(("wl%d.%d: %s: Enter "MACF"\n", WLCWLUNIT(wlc), WLC_BSSCFG_IDX(cfg), __FUNCTION__,
		ETHER_TO_MACF(scb->ea)));

	wlc_deauth_complete(wlc, cfg, WLC_E_STATUS_SUCCESS, &scb->ea, DOT11_RC_NOT_AUTH, 0);
#ifdef AP
	if (AP_ENAB(wlc->pub) && BSSCFG_AP(cfg)) {
		wlc_bss_mac_event(wlc, cfg, WLC_E_DISASSOC_IND, &scb->ea,
				WLC_E_STATUS_SUCCESS, DOT11_RC_DISASSOC_LEAVING, 0, 0, 0);

		/* Avoid sending a deauth frame from wlc_ap_scb_unusable_stas_timeout */
		scb->mark |= SCB_UNUSABLE_STA_TO;
	}
#endif

	/* Mark the scb for deletion first and scb state change notify callback functions
	 * will be handled later to protect against recursion in a.o. wlc_apps_ps_flush_tid.
	 */
	SCB_MARK_DEL(scb);
}

/* sa query txstatus packet callback handler */
static void
wlc_mfp_sa_query_cb(wlc_info_t *wlc, void *pkt, uint txstatus)
{
	...
	if ((scb == NULL) || SCB_MARKED_DEL(scb) || ((mfp_scb = MFP_SCB(wlc->mfp, scb)) == NULL)) {
		return;
	}
	q = &mfp_scb->saq;
	...
	if (no_ack) {
		if (q->timeouts >= (WLC_MFP_SA_QUERY_MAX_TIMEOUTS)) {
			...
			/* Transaction ID cannot be zero */
			uint16 txid = (++q->id) ? q->id : ++q->id;
			q->timeouts++;
			mfp_send_sa_query(wlc->mfp, scb, SA_QUERY_REQUEST, txid);
		}
	} else {
		/* Start a timeout, waiting for response (if not received yet) */
		if ((!q->match_found) && (q->timer)) {
			wl_add_timer(wlc->wl, q->timer, WLC_MFP_SA_QUERY_TIMEOUT_MS, FALSE);
		}
	}
}
...
mfp_send_sa_query(wlc_mfp_info_t *mfp, struct scb *scb, uint8 action, uint16 id)
{
	...
	p = wlc_frame_get_action(wlc, &scb->ea, &cfg->cur_etheraddr, &cfg->BSSID,
	...
	if (p) {
		...
		af->action = action;
		af->id = id;
		wlc_lifetime_set(wlc, p, WLC_MFP_SA_QUERY_TIMEOUT_MS * 1000);
		if (action == SA_QUERY_REQUEST) {
			WLF2_PCB1_REG(p, WLF2_PCB1_MFP_SA);
		}
	...
	const struct dot11_action_sa_query *af)
{
	...
	/* We might have retried SA query WLC_MFP_SA_QUERY_MAX_TIMEOUTS
	 * times. In busy environment we might get response for the old
	 * queries. So have some tolerance here
	 */
	if (ABS(af->id - q->id) <= WLC_MFP_SA_QUERY_MAX_TIMEOUTS) { /* got out response */
...
mfp_sa_query_timeout(void *arg)
{
	...
	if (q->timeouts >= (WLC_MFP_SA_QUERY_MAX_TIMEOUTS)) {
		...
		/* Transaction ID cannot be zero */
		uint16 txid = (++q->id) ? q->id : ++q->id;
		mfp_send_sa_query(mfp, scb, SA_QUERY_REQUEST, txid);
	}
}
...
static const uint8 mfp_robust_categories[] = {
	0x6f, 0x67, 0x8d, 0xbf, 0x00, 0x00, 0x00, 0x00,
	...
	d11rxhdr_t *rxhdr, struct dot11_management_header *hdr, void *p)
{
	...
	wlc_key_info_t key_info;
	bool ismulti = FALSE;
#if defined(BCMDBG) || defined(WLMSG_WSEC) || defined(WLMSG_ASSOC)
	...
	case FC_DISASSOC:
		...
		if (!ret) { /* disallowed deauth/disassoc get sa query */
			if (!SCB_IS_UNUSABLE(scb)) {
				WL_WSEC(("wl%d.%d: %s: starting SA Query %s\n", WLCWLUNIT(wlc),
					WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
					bcm_ether_ntoa(&hdr->sa, ea_str)));
				WL_ASSOC(("wl%d.%d: %s: starting SA Query %s fk=0x%02x\n",
					WLCWLUNIT(wlc),	WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
					bcm_ether_ntoa(&hdr->sa, ea_str), fk));
