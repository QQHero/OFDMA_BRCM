...
/*
 * toa_load_settings() - load TOA settings from storage (NVRAM or whatever).
 *
 * We need to be able to load all configured values. This could be done either by parsing the
 * entire nvram, or relying on a sequentially numbered name.
 * NVRAM format: toastXXX="macaddr attr=value attr=value ..."
 * Where XXX is a number starting at 001. Loading stops at the first hole in the sequence,
 * which means when deleting a record we must renumber all of them and save them.
 *
 */
