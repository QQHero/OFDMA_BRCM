 ...
 *
 * $Id: bcmsrom.c 709082 2017-07-06 01:03:07Z $
 */
...
#elif defined(BCMDBG_ERR) || defined(BCMDBG) || defined(WLTEST)
...
static uint8* srom_offset(si_t *sih, void *curmap)
{
	...
	/* XXX: DSLCPE builds use variable SI_ENUM_BASE, so need access to sii here. Furthermore,
	 * below check doesn't seem necessary for them so compile it out completely for now.
	 */
	...
	*count = 0;
	...
	case JTAG_BUS:
		...
		if (BCMPCIEDEV_ENAB())
		{
			...
			if (CHIPID(sih->chip) == BCM4349_CHIP_ID)
...
#if defined(BCMSDIODEV) || defined(BCMUSBDEV)
			...
			case CISTPL_BRCM_HNBU:
				...
				case HNBU_RXGAIN_ERR_5GA2:
					varbuf_append(&b, vstr_rxgainerr5ga, 2,
					              ...
					              (cis[i + 4] & 0x1f));
					break;
				case HNBU_RXGAIN_ERR_5GA3:
					varbuf_append(&b, vstr_rxgainerr5ga, 3,
					              (cis[i + 1] & 0x1f),
					              (cis[i + 2] & 0x1f),
					              (cis[i + 3] & 0x1f),
					              (cis[i + 4] & 0x1f));
...
#if defined(STB) && defined(BCMEXTNVM)
	if (err) {
		printf("wl:srom/otp not programmed, using external nvram file\n");

		/* read from vars file */
		if (!init_sromvars_map(sih, sih->chip,
			(void *)sih->wl_sromvars_map, sizeof(sih->wl_sromvars_map))) {
			unsigned int len;
