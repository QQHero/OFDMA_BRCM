 ...
 *
 * $Id: wbd_master_control.c 808103 2022-02-09 10:29:22Z $
 */
...
static wbd_tbss_wght_t predefined_tbss_wght[] = {
	/* w_rssi w_hops w_sta_cnt w_phyrate w_nss w_tx_rate w_band flags */
	/* 0 : All weightage */
	{7, 7, 4, 6, 4, 7, 4, 0x7F},
	/* 1 : High RSSI */
	{1, 0, 0, 0, 0, 0, 0, 0x01},
	...
	wbd_get_score_param_t *tbss_param, wbd_tbss_wght_t *wght_cfg)
{
	...
	if (wght_cfg->flags & WBD_WGHT_FLAG_BAND) {
		if (i5_ifr->band == WBD_BAND_LAN_6G) {
			n_band = 100;
		} else {
			n_band = 50;
		}
...
scoring:
	...
	WBD_INFO("For STA "MACF" on BSS "MACDBG" - weight_flags=0x%x, rssi=%d, N_RSSI=%d, "
		...
		"N_BAND=%d, SCORE=%d\n", ETHERP_TO_MACF(mac), MAC2STRDBG(i5_bss->BSSID),
		wght_cfg->flags, rssi, n_rssi, n_hops, total_sta_cnt, assoc_cnt, n_sta_cnt, phyrate,
		n_phyrate, bss_nss, master_info->tbss_info.max_nss,
		n_nss, i5_bss->avg_tx_rate, master_info->max_avg_tx_rate,
		n_tx_rate, n_band, node_score);
...
wbd_master_identify_target_bss_timer_cb(bcm_usched_handle *hdl, void *arg)
{
	...
	/* If STA is no more weak/strong or backhaul optimization is not running for this STA then
	 * remove the timer.
	 */
	if (assoc_sta && (assoc_sta->status != WBD_STA_STATUS_WEAK) &&
		...
		if (I5_CLIENT_IS_BSTA(sta)) {
			if (!WBD_BKT_BH_OPT_ENAB(master_info->parent->flags)) {
				...
				WBD_INFO("STA["MACDBG"] is NOT weak AND STRONG AND Backhaul "
					"Optimization is disabled, Exit. Flags[0x%08x]\n",
					MAC2STRDBG(args->sta_mac), master_info->parent->flags);
				goto end;
			}
			if (!WBD_ASSOC_ITEM_IS_BH_OPT(assoc_sta->flags)) {
				WBD_DEBUG("STA["MACDBG"] is NOT weak AND strong AND Backhaul "
					"optimization is not running for the STA, Exit."
					" flag[%x] status[%d]\n",
					MAC2STRDBG(args->sta_mac), assoc_sta->flags,
					assoc_sta->status);
				goto end;
			}
		} else {
			WBD_DEBUG("STA["MACDBG"] is NOT weak/strong, Exit. flag[%x] status[%d]\n",
				MAC2STRDBG(args->sta_mac), assoc_sta->flags, assoc_sta->status);
			goto end;
		}
	}
	...
	if (tbss == NULL || tbss == sbss) {
		...
		if (assoc_sta && (assoc_sta->status == WBD_STA_STATUS_WEAK)) {
			WBD_DS_UP_ASSOCSTA_STATUS(master_info, assoc_sta,
				WBD_STA_STATUS_WEAK_STEERING);
		} else if (assoc_sta && (assoc_sta->status == WBD_STA_STATUS_STRONG)) {
			WBD_DS_UP_ASSOCSTA_STATUS(master_info, assoc_sta,
				WBD_STA_STATUS_STRONG_STEERING);
		}
		...
		wbd_ds_add_sta_to_bounce_table(master_info->parent, (struct ether_addr*)&sta->mac,
			(struct ether_addr*)&args->bssid, bounce_sta_flag, assoc_sta->status);
...
/* Create the timer to identify the target BSS for weak/strong STA */
int
