 ...
 *
 * $Id: wluc_phy.c 809740 2022-03-22 23:43:00Z $
 */
...
static cmd_func_t wl_phy_hwrssi_trt_2g, wl_phy_hwrssi_trt_5g6g;
...
static void base64_start();
static uint base64_output(uint8 *buf, uint len);
static void base64_output_finish();

static char base64_buf[3];
static uint base64_buf_index = 0;
static uint base64_line_length = 0;
...
static cmd_t wl_phy_cmds[] = {
	...
	{ "phy_hwrssi_trt_2g", wl_phy_hwrssi_trt_2g, WLC_GET_VAR, WLC_SET_VAR,
	"usage: phy_hwrssi_trt_2g [val0 val1 ....]\n"
	"Set/get 2g hwrssioffset trt values"
	},
	{ "phy_hwrssi_trt_5g6g", wl_phy_hwrssi_trt_5g6g, WLC_GET_VAR, WLC_SET_VAR,
		...
		if (phytype == WLC_PHY_TYPE_AC) {
			...
			if (mode == INTERFERE_NONE) {
				...
				if (mode & ACPHY_HWOBSS_MITIGATION)
					printf("\tbit-mask %d: OBSS Detection + Mitigation\n",
	...
	uint32 *buff, FILE *fp)
{
	...
	if (fp == NULL) {
		err = base64_output((uint8 *)header, nbytes);
	} else {
...
static uint
base64_output(uint8 *buf, uint len)
{
	uint i;
	char out[5];
	...
	for (i = 0; i < len; i++) {
		...
		base64_buf[base64_buf_index++] = buf[i];
		if (base64_buf_index < 3) {
			continue;
		}

		// 3 bytes in the buffer: encode as 4 base64 characters.
		out[0] = basis_64[(base64_buf[0] >> 2) & 0x3F];
		out[1] = basis_64[((base64_buf[0] & 0x3) << 4) | ((base64_buf[1] & 0xF0) >> 4)];
		out[2] = basis_64[((base64_buf[1] & 0xF) << 2) | ((base64_buf[2] & 0xC0) >> 6)];
		out[3] = basis_64[base64_buf[2] & 0x3F];
		out[4] = '\0';
		base64_buf_index = 0;

		if (base64_line_length >= 76) {
			fputs("\n", stdout);
			base64_line_length = 0;
		}
		base64_line_length += 4;
		fputs(out, stdout);
	}
...
static void
base64_output_finish()
{
	char out[6];

	if (base64_buf_index == 0) {
		return;
	}

	// Write remaining bytes.
	out[0] = basis_64[(base64_buf[0] >> 2) & 0x3F];
	if (base64_buf_index == 1) {
		out[1] = basis_64[((base64_buf[0] & 0x3) << 4)];
		out[2] = '=';
	} else {
		out[1] = basis_64[((base64_buf[0] & 0x3) << 4) | ((base64_buf[1] & 0xF0) >> 4)];
		out[2] = basis_64[((base64_buf[1] & 0xF) << 2)];
	}
	out[3] = '=';
	out[4] = '\n';
	out[5] = '\0';
...
wl_sample_collect(void *wl, cmd_t *cmd, char **argv)
{
	...
	// For base64 output, use NULL fp and default to stdout.
	if (use_base64) {
		base64_start();
	} else {
	...
	if ((phytype == WLC_PHY_TYPE_HT) || (phytype == WLC_PHY_TYPE_AC)) {
		...
		if (use_base64) {
			fprintf(stderr, "base64 output not supported for this PHY.\n");
			ret = -1;
			goto exit;
		}
...
static int
wl_phy_hwrssi_trt_2g(void *wl, cmd_t *cmd, char **argv)
{
	int i;
	int err = -1;
	int8 deltaValues[18] = { 0 };
...
static int
wl_phy_hwrssi_trt_5g6g(void *wl, cmd_t *cmd, char **argv)
{
	int i;
	int err = -1;
	int8 deltaValues[28] = { 0 };
...
wl_get_trig_info(void *wl, cmd_t *cmd, char **argv)
{
	...
	BCM_REFERENCE(argv);
	/* Get Functionality */
	if (wlu_var_getbuf(wl, cmd->name, NULL, 0, &temp) < 0)
		return BCME_NOMEM;
	...
	*resolution = 0;
	...
	while ((opt_err = miniopt(&to, argv)) != -1) {
		...
		if (to.opt == 'r') {
			...
			*resolution = to.val & 0x1;
			params->rxiq = ((*resolution << 16) | (params->rxiq & 0xfffeffff));
		}
		if (to.opt == 'q') {
			if (!to.good_int) {
				fprintf(stderr,
					"%s: could not parse \"%s\" as an int"
					" for adc_referred (0, 1)\n", fn_name, to.valstr);
				err = BCME_BADARG;
				goto exit;
			}
			if ((to.val < 0) || (to.val > 1)) {
				fprintf(stderr, "%s: invalid adc_referred select %d"
					" (0,1)\n", fn_name, to.val);
				err = BCME_BADARG;
				goto exit;
			}
			adcref = to.val & 0x1;
			params->rxiq = ((adcref << 17) | (params->rxiq & 0xfffdffff));
		}
		if (to.opt == 's') {
			if (!to.good_int) {
				fprintf(stderr,
...
wl_rxiq(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (BCM4365_CHIP(dtoh32(revinfo.chipnum)) ||
		...
		BCM6715_CHIP(dtoh32(revinfo.chipnum)) ||
...
wl_rxiq_sweep(void *wl, cmd_t *cmd, char **argv)
{
	...
	while ((err = miniopt(&to, argv)) != -1) {
		if (err == 1) {
			err = BCME_USAGE_ERROR;
			fprintf(stderr, SWEEP_ERROR "Option parse error\n");
			goto Exit;
		}
...
wl_phy_ulofdma_stats(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (*argv == NULL) {
		...
		if (phyrev >= 130) {
			printf("User snr delta:  [");
			for (user = 0; user < MAX_NUM_ULOFDMA_USERS; user++) {
				printf("%8d", per_user_stats.user_snr_delta[user]);
			}
			printf("] dB\n");

			printf("User preempted:  [");
			for (user = 0; user < MAX_NUM_ULOFDMA_USERS; user++) {
				printf("%8d", per_user_stats.user_preempted[user]);
			}
			printf("]\n");
		}
	} else {
