 ...
 *
 * $Id: wlc_ie_mgmt.c 799714 2021-06-07 04:26:53Z $
 */
...
/*
 * list of IE's not allowed in non transmit profile of MBSSID IE.
 */
static const wlc_iem_tag_t ie_tags_nallow_ntxprof[] = {
	/* id */
...
static const wlc_iem_tag_t BCMINITDATA(ie_tags_bcn)[] = {
	...
	/* 71 */ DOT11_MNG_MAX_CHANNEL_SWITCH_TIME_ID,	/* Draft P802.11REVmd_D3.0 */
	/* 76 */ DOT11_MNG_MBSSID_CONFIG_ID,    /* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 77 */ DOT11_MNG_HE_CAP_ID,		/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 78 */ DOT11_MNG_HE_OP_ID,		/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 79 */ DOT11_MNG_TWT_ID,		/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 80 */ DOT11_MNG_RAPS_ID,		/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 81 */ DOT11_MNG_COLOR_CHANGE_ID,	/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 82 */ DOT11_MNG_SRPS_ID,		/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 83 */ DOT11_MNG_MU_EDCA_ID,		/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 84 */ DOT11_MNG_ESS_REPORT_ID,	/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 85 */ DOT11_MNG_NDP_FR_ID,		/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 86 */ DOT11_MNG_HE_BSS_LOAD_ID,	/* IEEE P802.11ax/D7.0 Table 9-34 */
	/* 87 */ DOT11_MNG_HE_6G_BAND_CAPS_ID,	/* IEEE P802.11ax/D7.0 Table 9-34 */
...
static const wlc_iem_tag_t BCMINITDATA(ie_tags_prs)[] = {
	...
	/* 93 */ DOT11_MNG_MBSSID_CONFIG_ID,
	/* 94 */ DOT11_MNG_HE_CAP_ID,		/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 95 */ DOT11_MNG_HE_OP_ID,		/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 96 */ DOT11_MNG_TWT_ID,		/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 97 */ DOT11_MNG_RAPS_ID,		/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 98 */ DOT11_MNG_COLOR_CHANGE_ID,	/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 99 */ DOT11_MNG_SRPS_ID,		/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 100 */ DOT11_MNG_MU_EDCA_ID,		/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 101 */ DOT11_MNG_ESS_REPORT_ID,	/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 102 */ DOT11_MNG_NDP_FR_ID,		/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 103 */ DOT11_MNG_HE_BSS_LOAD_ID,	/* IEEE P802.11ax/D7.0 Table 9-27 */
	/* 104 */ DOT11_MNG_HE_6G_BAND_CAPS_ID,	/* IEEE P802.11ax/D7.0 Table 9-27 */
#ifdef BCMWAPI_WAI
	/* XX */ DOT11_MNG_WAPI_ID,
#endif
	/* 201 */ DOT11_MNG_RNR_ID,
	/* 75? */ DOT11_MNG_RSNXE_ID,
	/* XX */  DOT11_MNG_SHORT_SSID_ID,
...
/* Function to figure out which IE's needed to be in non tx profile
 * and also IE's to be added in non inheritance IE
 */
int
wlc_iem_fill_mbssid_tag_list(wlc_info_t *wlc, wlc_bsscfg_t *trans_cfg,
		wlc_bsscfg_t *nontrans_cfg, wlc_iem_calc_data_t *data)
{
	wlc_mbssid_tag_list_t *tag_list = nontrans_cfg->mbssid_tag_list;
	wlc_iem_tag_t tag;
	bool tag_allowed = TRUE;
	int i, j, len_tx, len_nontx;
	wlc_iem_tag_t *build_tag = BUILD_TAG_TBL(wlc->iemi);
	wlc_iem_cbe_t *build_cb = BUILD_CB_TBL(wlc->iemi);
	wlc_iem_ft_t *build_ft = BUILD_FT_TBL(wlc->iemi);
	uint16 cur, next;
	uint8 build_tx[TLV_BODY_LEN_MAX];
	uint8 build_nontx[TLV_BODY_LEN_MAX];
	bool non_tx_prof = FALSE, inherit_tag = FALSE;
	wlc_iem_calc_data_t calc_data_tx;
	wlc_iem_calc_data_t calc_data_nontx;
	wlc_iem_build_data_t build_data_tx;
	wlc_iem_build_data_t build_data_nontx;

	bcopy(data, &calc_data_tx, sizeof(wlc_iem_calc_data_t));
	bcopy(data, &calc_data_nontx, sizeof(wlc_iem_calc_data_t));

	calc_data_tx.cfg = trans_cfg;
	calc_data_nontx.cfg = nontrans_cfg;

	build_data_tx.cbparm = calc_data_tx.cbparm;
	build_data_tx.ft = calc_data_tx.ft;
	build_data_tx.buf = build_tx;
	build_data_tx.buf_len = TLV_BODY_LEN_MAX;
	build_data_tx.cfg = trans_cfg;

	build_data_nontx.cbparm = calc_data_tx.cbparm;
	build_data_nontx.ft = calc_data_tx.ft;
	build_data_nontx.buf = build_nontx;
	build_data_nontx.cfg = nontrans_cfg;
	build_data_nontx.buf_len = TLV_BODY_LEN_MAX;

	TBL_POS_GET(build_ft, calc_data_tx.ft, cur, next);
	...
	for (i = 0; i < (next - cur); i ++) {
		...
		tag = build_tag[cur + i];
		tag_allowed = TRUE;
		if (tag == DOT11_MNG_SSID_ID || tag == DOT11_MNG_MULTIPLE_BSSID_ID) {
			tag_allowed = FALSE;
		} else {
			for (j = 0; j < ARRAYSIZE(ie_tags_nallow_ntxprof); j ++)
			{
				if (tag == ie_tags_nallow_ntxprof[j]) {
					tag_allowed = FALSE;
					break;
				}
			}
		...
		len_tx = build_cb_t.calc(build_cb_t.ctx, &calc_data_tx);
		len_nontx = build_cb_t.calc(build_cb_t.ctx, &calc_data_nontx);
		if (len_tx || len_nontx) {
			if (len_tx == len_nontx) {
				memset(build_tx, 0, TLV_BODY_LEN_MAX);
				memset(build_nontx, 0, TLV_BODY_LEN_MAX);
				build_cb_t.build(build_cb_t.ctx, &build_data_tx);
				build_cb_t.build(build_cb_t.ctx, &build_data_nontx);
				if (bcmp(build_data_tx.buf, build_data_nontx.buf, len_tx)) {
					non_tx_prof = TRUE;
				}
			} else {
				if (!len_nontx) {
		...
		if (non_tx_prof) {
			if (tag_list->tags_len == MAX_TAG_MBSSID_IE) {
				return BCME_ERROR;
			}
			tag_list->tag[tag_list->tags_len] = tag;
			tag_list->tags_len++;
		}
		if (inherit_tag) {
			if (tag < DOT11_MNG_ID_EXT_ID) {
				if (tag_list->non_inherit_len
						== MAX_TAG_MBSSID_IE) {
					return BCME_ERROR;
				}
				tag_list->non_inherit[tag_list->non_inherit_len] = tag;
				tag_list->non_inherit_len++;
			}
			if (tag > DOT11_MNG_ID_EXT_ID) {
				if (tag_list->non_inherit_ext_len == MAX_TAG_MBSSID_IE) {
					return BCME_ERROR;
				}
				tag_list->non_inherit_ext[tag_list->non_inherit_ext_len] =
					tag - DOT11_MNG_ID_EXT_ID;
				tag_list->non_inherit_ext_len++;
			}
		}
	}
	return 0;
