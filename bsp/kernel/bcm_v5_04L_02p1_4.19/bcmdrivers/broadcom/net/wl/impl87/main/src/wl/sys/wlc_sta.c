 ...
 *
 * $Id: wlc_sta.c 807674 2022-01-27 13:34:11Z $
 */
...
wlc_sta_assoc_state_clbk(void *arg, bss_assoc_state_data_t *notif_data)
{
	...
	if (notif_data->state == AS_IBSS_CREATE || notif_data->state == AS_JOIN_ADOPT) {
		...
		FOREACH_UP_AP(cfg->wlc, idx, apcfg) {
			wlc_ap_timeslot_unregister(apcfg);
		}
...
#ifdef WL11AC
			if (!SCAN_IN_PROGRESS(wlc->scan) &&
					CHSPEC_IS80(cfg->ap_orig_chanspec) && VHT_ENAB(wlc->pub) &&
					(wf_bw_chspec_to_half_mhz(cfg->ap_orig_chanspec) >
					 wf_bw_chspec_to_half_mhz(cb_info->chanspec))) {
				WL_REGULATORY(("wl%d: %s Sending VHT Operation Mode: "
					"Value 0x%x(hex) to bss : "MACF"\n",
					wlc->pub->unit, __FUNCTION__, cfg->oper_mode,
