 ...
 *
 *
 * $Id: wlc_ratelinkmem.c 808087 2022-02-09 04:01:33Z $
 */
...
#define D11_MAX_RATELINK_ENTRIES_REV128		(256)
#define D11_MAX_RATELINK_ENTRIES_REV132		(512)
#define D11_MAX_RATELINK_ENTRIES(corerev)	(D11REV_GE((corerev), 132) ? \
	D11_MAX_RATELINK_ENTRIES_REV132 : D11_MAX_RATELINK_ENTRIES_REV128)
...
struct wlc_ratelinkmem_info {
	...
	scb_t		**scb_index;	/* lookup table index -> SCB */
...
#ifdef BCM_SECURE_DMA
	struct sec_cma_info sec_cma_info_rlmi_tx;
#endif /* BCM_SECURE_DMA */
};
...
static void wlc_ratelinkmem_chansw_upd_cb(void *arg, wlc_chansw_notif_data_t *data);
...
wlc_ratelinkmem_scb_deinit(void *ctx, struct scb *scb)
{
	...
	for (i = 0; i < D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev); i++) {
...
wlc_ratelinkmem_scb_dump(void *ctx, struct scb *scb, struct bcmstrbuf *b)
{
	...
	if (scbh->rate_state != RATE_ENTRY_STATE_UNINITED) {
		...
		if (k != D11_RATE_LINK_MEM_IDX_INVALID) {
			ASSERT(k < D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev));
			bcm_bprintf(b, "In HW: ");
		...
		switch (*++p) {
			...
			case 'i':
				while (*++p != '\0') {
					...
					if (val >= D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev)) {
						err = BCME_BADARG;
						goto exit;
					}
					setbit(bmp, val);
			...
			case 'I':
				...
				if ((end >= strt) &&
					(end < D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev))) {
					for (i = strt; i <= end; i++) {
						setbit(bmp, i);
					}
				} else {
...
wlc_ratelinkmem_module_dump(void *ctx, struct bcmstrbuf *b)
{
	wlc_ratelinkmem_info_t *rlmi = ctx;
	int i;
	uint8 entry_bitmap[D11_MAX_RATELINK_ENTRIES_REV132/NBBY + 1] = { 0 };
	...
	int amt_index)
{
	...
	ASSERT(amt_index < D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev));
...
static void
wlc_ratelinkmem_chansw_upd_cb(void *arg, wlc_chansw_notif_data_t *data)
{
	...
	WL_TRACE(("wl%d: %s: old chanspec 0x%x new chanspec 0x%x\n", wlc->pub->unit,
	...
	if ((CHSPEC_BW(new_chspec) != CHSPEC_BW(old_chspec)) ||
		...
		/* upon BAND or BW switch (for an 'up' BSS) may need to update rates and link mem */
		FOR_ALL_UP_BSS(wlc, idx, icfg) {
			/* force a rate selection init. */
			wlc_scb_ratesel_init_bss(wlc, icfg);
			if (RATELINKMEM_ENAB(wlc->pub) == TRUE) {
				wlc_ratelinkmem_update_link_entry_all(wlc, icfg, FALSE, TRUE);
			}
		}
	}
}
...
BCMATTACHFN(wlc_ratelinkmem_attach)(wlc_info_t *wlc)
{
	...
	rlmi->scb_index = MALLOCZ(wlc->osh,
		D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev) * sizeof(scb_t*));
	if (rlmi->scb_index == NULL) {
		WL_ERROR(("wl%d: %s: no memory for scb_index array\n",
			wlc->pub->unit, __FUNCTION__));
		goto fail;
	}
	...
	ASSERT(D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev) >= AMT_SIZE(wlc->pub->corerev));
	...
	if ((wlc_chansw_notif_register(wlc, wlc_ratelinkmem_chansw_upd_cb, wlc)) != BCME_OK) {
...
BCMATTACHFN(wlc_ratelinkmem_detach)(wlc_ratelinkmem_info_t *rlmi)
{
	...
	(void)wlc_chansw_notif_unregister(rlmi->wlc, wlc_ratelinkmem_chansw_upd_cb, rlmi->wlc);
	if (!PHYSADDRISZERO(rlmi->rate_dma_pa)) {
		...
		SECURE_DMA_UNMAP(rlmi->wlc->osh, rlmi->rate_dma_pa, rlmi->rate_dma_sz,
			DMA_TX, NULL, NULL, &rlmi->sec_cma_info_rlmi_tx, 0);
#endif /* BCM_SECURE_DMA */
	}
	...
	if (rlmi->scb_index != NULL) {
		MFREE(rlmi->wlc->osh, rlmi->scb_index,
			D11_MAX_RATELINK_ENTRIES(rlmi->wlc->pub->corerev) * sizeof(scb_t*));
	}
...
wlc_ratelinkmem_retrieve_cur_rate_store(wlc_info_t *wlc, uint16 index, bool newrate)
{
	...
	if (index >= D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev)) {
...
wlc_ratelinkmem_upd_rmem(wlc_ratelinkmem_info_t *rlmi, uint8 *buf, uint sz, uint16 rmem_idx)
{
	...
	ASSERT(rmem_idx < D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev));
	...
	rlmi->rate_dma_pa = SECURE_DMA_MAP(wlc->osh, buf, sz, DMA_TX, NULL, NULL,
		&rlmi->sec_cma_info_rlmi_tx, 0, SECDMA_TXBUF_POST);
#endif /* BCM_SECURE_DMA */
	rlmi->rate_dma_sz = sz;
#ifdef BCMDMA64OSL
	...
	void *rs, uint16 flags)
{
	...
	if (CHSPEC_IS6G(wlc->chanspec) && wlc->stf->psd_limit_indicator) {
		flags |= D11_REV128_RCTS_FULL_BW;
	}
	...
	if (!PHYSADDRISZERO(rlmi->rate_dma_pa)) {
		...
		NULL, NULL, &rlmi->sec_cma_info_rlmi_tx, 0);
#endif /* BCM_SECURE_DMA */
	...
	ASSERT(scbh->rate_link_index < D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev));
	...
	bool clr_txbf_stats)
{
	...
	pa = SECURE_DMA_MAP(wlc->osh, &scbh->link_entry, size, DMA_TX,
			NULL, NULL, &rlmi->sec_cma_info_rlmi_tx, 0, SECDMA_TXBUF_POST);
#endif /* BCM_SECURE_DMA */
	...
	pa = SECURE_DMA_MAP(wlc->osh, &rlmi->null_link_entry, WLC_RLM_LINK_ENTRY_UCODE_BLOCKSIZE,
			DMA_TX,	NULL, NULL, &rlmi->sec_cma_info_rlmi_tx, 0, SECDMA_TXBUF_POST);
#endif /* BCM_SECURE_DMA */
#ifdef BCMDMA64OSL
		PHYSADDRTOULONG(pa, paddr);
		bme_eng_idx = bme_copy64(wlc->osh, rlmi->bme_key,
...
wlc_ratelinkmem_scb_amt_release(wlc_info_t *wlc, scb_t *scb, int index)
{
	...
	if ((index < 0) || (index >= D11_MAX_RATELINK_ENTRIES(wlc->pub->corerev))) {
