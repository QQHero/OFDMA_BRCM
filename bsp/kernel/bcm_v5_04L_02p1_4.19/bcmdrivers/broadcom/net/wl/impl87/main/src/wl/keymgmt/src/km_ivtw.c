 ...
 * $Id: km_ivtw.c 804620 2021-11-04 18:56:37Z $
 */
...
/* On reset there can still be RX packets to be processed. Reset on first replay detection with
 * a max number of packets defined by:
 */
#define IVTW_INITIAL_RESET_COUNT	256
...
km_ivtw_reset(km_ivtw_t *ivtw,  wlc_key_index_t key_idx)
{
	...
	ivtw_key_info->reset_count = IVTW_INITIAL_RESET_COUNT;
	...
	uint8 *key_seq, const uint8 *rx_seq, size_t seq_len)
{
	...
	if (ivtw_key_info->reset_count) {
		ivtw_key_info->reset_count--;
		if ((replay) && (rx > 0)) {
			IVTW_LOG(("wl%d: %s: replay CLEARED, after reset (rekeying), skipped %d\n",
				IVTW_UNIT(ivtw), __FUNCTION__, IVTW_INITIAL_RESET_COUNT -
				ivtw_key_info->reset_count));
			replay = FALSE;
			memset(ivtw_key_info, 0,  SIZEOF_IVTW_KEY_INFO(ivtw));
			for (i = 0; i < IVTW_NUM_RX_SEQ(ivtw); ++i) {
				seq_info = &ivtw_key_info->seq_info[i];
				seq_info->wstart = 1;
			}
		}
	}
...
km_ivtw_dump(km_ivtw_t *ivtw, struct bcmstrbuf *b)
{
	...
	for (i = 0; i < ivtw->max_keys; ++i) {
		...
		for (j = 0; j < IVTW_NUM_RX_SEQ(ivtw); ++j) {
			...
			bcm_bprintf(b, "\t\tivtw seq info: seq index %d, lb %llu.%llu\n",
