 ...
 *
 * $Id: wbd_shared.h 808794 2022-02-28 05:52:07Z $
 */
...
#define CLI_CMD_I5_STA_FMT \
"        STA[%d]: "MACDBG" [%s]\n\
                RSSI(dBm) : %d\n\
                DownLink Rate(Mbps) : %d\n\
                UpLink Rate(Mbps) : %d\n\
                Last DownLink Rate(Mbps) : %d\n\
                Last UpLink Rate(Mbps) : %d\n\
                Tx Bytes%s : %llu\n\
                Rx Bytes%s : %llu\n\
                Tx Packets : %u\n\
                Rx Packets : %u\n\
...
#define CLI_CMD_LOGS_STRONG "%s Strong STA: "MACDBG" on "MACDBG" RSSI=%d/%d\n"
...
#define WBD_CLI_FLAGS_FORCE	0x08	/* To add Force option to a command */
#define WBD_CLI_FLAGS_QUERY	0x10	/* To send Query to Daemon */
...
typedef enum wbd_cmd_type {
	...
	WBD_CMD_STRONG_CLIENT_BSD_REQ,
	WBD_CMD_STRONG_CANCEL_BSD_REQ
} wbd_cmd_type_t;
...
} wbd_cmd_weak_strong_client_resp_t;
...
/* Strong STA Metric report policy configuration for a band */
typedef struct wbd_vndr_strong_sta_rpt_policy {
	unsigned int t_idle_rate;	/* data rate threshold to measure STA is idle */
	unsigned int t_rssi;		/* RSSI threshold to measure STA is strong */
	unsigned int t_hysterisis;	/* RSSI Hysterisis Margin */
	unsigned int flags;		/* WBD_STRONG_STA_POLICY_FLAG_XXX,
					 * Flags indicates which params to consider
					*/
} wbd_vndr_strong_sta_rpt_policy_t;

/* For command VNDR_METRIC_POLICY, IFR List entry */
typedef struct wbd_strong_sta_policy_ifr_entry {
	dll_t node;	/* self referencial (next,prev) pointers of type dll_t */

	unsigned char ifr_mac[MAC_ADDR_LEN];	/* MAC of the Interface */
	wbd_vndr_strong_sta_rpt_policy_t vndr_policy; /* Vendor Strong Sta Report Policy Config */

} wbd_strong_sta_policy_ifr_entry_t;

/* For command VNDR_STRONG_STA_POLICY */
struct wbd_cmd_vndr_strong_sta_policy_config {
	int num_entries;	/* Number of wbd_strong_sta_policy_ifr_entry_t type objects */
	wbd_glist_t entry_list; /* List of wbd_strong_sta_policy_ifr_entry_t type objects */
};
...
extern void* wbd_realloc(void *buf, unsigned int old_len, unsigned int new_len, int *error);

/* Allocate & Initialize the info structure */
extern wbd_info_t* wbd_info_init(int *error, wbd_info_t *g_info, bool reload);
...
/* Gets WBD Band Enumeration from ifname's Chanspec */
extern int wbd_identify_wbd_band_type(chanspec_t ifr_chanspec, int *out_band);
...
int wbd_snprintf_i5_client(i5_dm_clients_type *client, char *format, char **outdata, int *outlen,
...
/* Add a Strong Sta Policy item in a Metric Policy List */
extern int wbd_add_item_in_strong_sta_policylist(wbd_strong_sta_policy_ifr_entry_t *in_policy_ifr,
...
extern wbd_strong_sta_policy_ifr_entry_t *
...
extern int wbd_convert_chspeclist_to_string(char *s_chlist, unsigned int s_chlist_size,
