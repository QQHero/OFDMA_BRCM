 ...
 *
 * $Id: wlc_rate.c 793840 2020-12-11 13:10:41Z $
 */
    ...
    uint8 txstreams)
{
    ...
    /* only set the mcs rate bit if the equivalent hw is supported. */
    for (i = 0; i < txstreams; i++) {
        if (rs->mcs[i] != hw_rs->mcs[i]) {
            rs->mcs[i] = 0xff;/*mcs 0-7*/
        }
    }
#else
