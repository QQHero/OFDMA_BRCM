 ...
 *
 * $Id: wbd_ds.c 807523 2022-01-24 09:32:25Z $
 */
...
void
wbd_ds_glist_prepend(wbd_glist_t *list, dll_t *new_obj)
{
...
wbd_ds_blanket_master_init(wbd_info_t *info)
{
	...
	/* channel selection request vendor information List of Blanket Master */
...
wbd_ds_blanket_master_cleanup(wbd_info_t *info)
{
	...
	/* Remove channel selection vendor information */
...
wbd_ds_master_info_init(uint8 bkt_id, char *bkt_name, int *error)
{
	...
	/* Updated Master's Weak and strong client count, STA_REPOSTS cmd will update the same */
	master_info->weak_client_count = 0;
	master_info->strong_client_count = 0;
...
wbd_ds_master_info_cleanup(wbd_master_info_t *master_info, bool free_master_mem)
{
	...
	/* Updated Weak and strong client count maintained by Master */
	master_info->weak_client_count = 0;
	master_info->strong_client_count = 0;
	...
	wbd_trigger_t *out_trigger)
{
	...
	WBD_DS("BKTID[%d] Slave Removed ["MACF"] From blanket. Now Total Clients[%d] "
		"Weak Clients[%d] Strong Clients[%d] \n", master_info->bkt_id,
		ETHERP_TO_MACF(slave_bssid), master_info->blanket_client_count,
		master_info->weak_client_count, master_info->strong_client_count);
...
wbd_ds_add_monitorlist_fm_peer_bss_assoclist(i5_dm_bss_type *i5_dst_bss, i5_dm_bss_type *i5_src_bss)
{
	...
	foreach_i5glist_item(i5_sta, i5_dm_clients_type, i5_src_bss->client_list) {
		...
		/* Only weak or strong STAs are monitored. So, if not weak/strong skip this STA */
...
wbd_ds_add_monitorlist_fm_peer_devices_assoclist(i5_dm_bss_type *i5_bss)
{
	...
	i5_topology = ieee1905_get_datamodel();
	/* Traverse BSS List to add the weak/strong STA to the new BSS monitor list if any */
...
int
wbd_ds_add_vndr_chan_set_info_to_list(wbd_glist_t *list, wbd_cmd_vndr_set_chan_t *vndr_set_chan)
{
	int ret = WBDE_OK;
	wbd_cmd_vndr_set_chan_lst_t *new_node = NULL;
	WBD_ENTER();

	/* Create a vendor chan set item */
	new_node = (wbd_cmd_vndr_set_chan_lst_t*)wbd_malloc(sizeof(*new_node), &ret);
	WBD_ASSERT();

	/* Fill vendor chan set item data */
	memcpy(&new_node->vndr_set_chan, vndr_set_chan, sizeof(*vndr_set_chan));

	wbd_ds_glist_append(list, (dll_t *) new_node);

	WBD_DS("Added Channel[%d] opclass[%d] for Radio["MACF"] to list\n",
		vndr_set_chan->cntrl_chan, vndr_set_chan->rclass,
		ETHER_TO_MACF(vndr_set_chan->mac));
end:
	WBD_EXIT();
	return ret;
}
	...
	struct ether_addr *src_addr, uint8 flags, wbd_sta_status_t command)
{
	...
	if (command == WBD_STA_STATUS_WEAK) {
		...
		entry->sta_status = WBD_STA_STATUS_STRONG_STEERING;
	}
...
wbd_ds_interface_init(i5_dm_interface_type *i5_ifr)
{
	...
	ieee1905_glist_init(&ifr_vndr_data->pref0_exclude_chlist);
...
wbd_ds_interface_deinit(i5_dm_interface_type *i5_ifr)
{
	...
	if (i5_ifr->vndr_data) {
		...
		i5DmGlistCleanup(&(ifr_vndr_data->pref0_exclude_chlist));
