 ...
 *
 * $Id: wlc_macfltr.c 803784 2021-10-07 12:42:20Z $
 */
...
static const wlc_ioctl_cmd_t wlc_macfltr_ioctls[] = {
	...
	{WLC_GET_FLTR_COUNT, 0, 0},
	{WLC_SET_FLTR_COUNT, 0, 0},
...
typedef struct {
	...
	int max_maclist;        /* max entry of maclist ACKSUPR_MAC_FILTER */
#if defined(WLC_MACFLTR_STATS)
	uint16 total_supr_count;
#ifdef WLAUTHRESP_MAC_FILTER
	uint16 *auth_supr_count;
#endif /* WLAUTHRESP_MAC_FILTER */
#ifdef WLPROBRESP_MAC_FILTER
	uint16 *prob_supr_count;
...
BCMATTACHFN(wlc_macfltr_attach)(wlc_info_t *wlc)
{
	...
	/* error handling */
fail:
	MODULE_DETACH(mfi, wlc_macfltr_detach);
	return NULL;
}
...
#endif /* ACKSUPR_MAC_FILTER */
	(void)wlc_module_remove_ioctl_fn(wlc->pub, mfi);
...
wlc_macfltr_acksupr_set(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int *max_maclist, bool config)
{
	...
	if (cfg->acksupr_mac_filter && config) {
		...
		/* reset max maxlist to default value when acksupr is disabled */
		*max_maclist = bfi->max_maclist;
		bfi->max_maclist = wlc->pub->tunables->max_maclist;
...
/* ioctl entry */
static int
wlc_macfltr_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
...
#if defined(WLC_MACFLTR_STATS)
	case WLC_GET_FLTR_COUNT:
		err = wlc_macfltr_dump_get(mfi, cfg, (fltr_list_t *)arg, (uint)len);
		break;
	case WLC_SET_FLTR_COUNT:
		err = wlc_macfltr_dump_reset(mfi, cfg, (fltr_list_t *)arg, (uint)len);
		break;
#endif /* WLC_MACFLTR_STATS */
	default:
		err = BCME_UNSUPPORTED;
...
#if defined(WLC_MACFLTR_STATS)
int
wlc_macfltr_dump_reset(wlc_macfltr_info_t *mfi, wlc_bsscfg_t *cfg, fltr_list_t  *fltrlist, uint len)
{
	int i;
	bss_macfltr_info_t *bfi;
	ASSERT(cfg != NULL);
	bfi = BSS_MACFLTR_INFO(mfi, cfg);
	ASSERT(bfi != NULL);
	bfi->total_supr_count = 0;
	for (i = 0; i < bfi->nmac; i++) {
#ifdef WLAUTHRESP_MAC_FILTER
	       bfi->auth_supr_count[i] = 0;
#endif /* WLAUTHRESP_MAC_FILTER */
#ifdef WLPROBRESP_MAC_FILTER
	       bfi->prob_supr_count[i] = 0;
#endif /* WLPROBRESP_MAC_FILTER */
	}
	return BCME_OK;
}
int
wlc_macfltr_dump_get(wlc_macfltr_info_t *mfi, wlc_bsscfg_t *cfg, fltr_list_t  *fltrlist, uint len)
{
	int i;
	bss_macfltr_info_t *bfi;
	ASSERT(cfg != NULL);
	...
	if (len < (bfi->nmac * sizeof(struct fltr_unit)) + sizeof(uint))
		return BCME_BUFTOOSHORT;

	fltrlist->count = bfi->nmac;
	fltrlist->total_supr_count = bfi->total_supr_count;
	for (i = 0; i < bfi->nmac; i++) {
	       bcopy(&bfi->maclist[i], &fltrlist->unit[i].ea, ETHER_ADDR_LEN);
#ifdef WLAUTHRESP_MAC_FILTER
	       fltrlist->unit[i].auth_fltr_count = bfi->auth_supr_count[i];
#endif /* WLAUTHRESP_MAC_FILTER */
#ifdef WLPROBRESP_MAC_FILTER
	       fltrlist->unit[i].prob_fltr_count = bfi->prob_supr_count[i];
#endif /* WLPROBRESP_MAC_FILTER */
	}
	return BCME_OK;
}
...
#ifdef WLAUTHRESP_MAC_FILTER
void
wlc_macfltr_authcount_incr(wlc_macfltr_info_t *mfi, wlc_bsscfg_t *cfg,
	const struct ether_addr *addr)
{
	...
	for (i = 0; i < bfi->nmac; i++) {
		if (bcmp(addr, &bfi->maclist[i], ETHER_ADDR_LEN) == 0)
			bfi->auth_supr_count[i]++;
		bfi->total_supr_count++;
	}
}
#endif /* WLAUTHRESP_MAC_FILTER */
...
#ifdef WLPROBRESP_MAC_FILTER
void
wlc_macfltr_probcount_incr(wlc_macfltr_info_t *mfi, wlc_bsscfg_t *cfg,
	const struct ether_addr *addr)
{
	bss_macfltr_info_t *bfi;
	uint i;
...
wlc_macfltr_bss_deinit(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	if (bfi->maclist != NULL) {
		...
		if (bfi->nmac) {
#ifdef WLAUTHRESP_MAC_FILTER
			MFREE(wlc->osh, bfi->auth_supr_count, bfi->nmac * sizeof(uint16));
#endif /* WLAUTHRESP_MAC_FILTER */
#ifdef WLPROBRESP_MAC_FILTER
			MFREE(wlc->osh, bfi->prob_supr_count, bfi->nmac * sizeof(uint16));
#endif /* WLPROBRESP_MAC_FILTER */
		}
		bfi->total_supr_count = 0;
#endif /* WLC_MACFLTR_STATS */
...
wlc_macfltr_list_set(wlc_macfltr_info_t *mfi, wlc_bsscfg_t *cfg, struct maclist *maclist, uint len)
{
...
#if !defined(ACKSUPR_MAC_FILTER)
	if (maclist->count > wlc->pub->tunables->max_maclist) {
		return BCME_RANGE;
	}
#else
	...
	bfi->maclist = MALLOCZ(wlc->osh, OFFSETOF(struct maclist, ea) +
	...
	if (maclist->count) {
#ifdef WLAUTHRESP_MAC_FILTER
		bfi->auth_supr_count = MALLOCZ(wlc->osh,  maclist->count * sizeof(uint16));
		if (bfi->auth_supr_count == NULL) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			return BCME_NOMEM;
		}
#endif
#ifdef WLPROBRESP_MAC_FILTER
		bfi->prob_supr_count = MALLOCZ(wlc->osh,  maclist->count * sizeof(uint16));
		if (bfi->prob_supr_count == NULL) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			return BCME_NOMEM;
		}
...
wlc_macfltr_list_get(wlc_macfltr_info_t *mfi, wlc_bsscfg_t *cfg, struct maclist *maclist, uint len)
{
	...
	if (len < OFFSETOF(struct maclist, ea) + (bfi->nmac * ETHER_ADDR_LEN)) {
		return BCME_BUFTOOSHORT;
	}
