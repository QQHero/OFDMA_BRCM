...
typedef enum i5_tlv_types {
  ...
  i5TlvBackhaulSTARadioCapabilitiesType         = 0xcb,
  i5TlvBackhaulBSSConfigPolicyType              = 0xd0,
#endif /* MULTIAPR2 */
#endif /* MULTIAP */
...
static i5_tlv_types_name_t i5_tlv_list_type_name[] = {
  ...
  {i5TlvBackhaulSTARadioCapabilitiesType, "Backhaul STA Radio Capabilities"},
  {i5TlvBackhaulBSSConfigPolicyType, "Backhaul BSS Configuration Policy"},
#endif /* MULTIAPR2 */
#endif /* MULTIAP */
  ...
  if ((pdmdev = i5DmGetSelfDevice()) != NULL) {
    *pbuf = pdmdev->InterfaceNumberOfEntries;
    ...
    while (pdmif != NULL) {
      ...
      pbuf += MAC_ADDR_LEN;
      MediaType = pdmif->MediaType;
      /* Wi-Fi 6 media type is added in profile-2 spec, so, send media type as AC for all profile-1
       * devices
       */
      if ((profile < ieee1905_map_profile2) && (MediaType == I5_MEDIA_TYPE_WIFI_AX)) {
        /* We cannot add AC media typpe blindly as the AC media type is only for 5G. So get either
         * AC or N24 media type based on chanspec
         */
        MediaType = i5WlCfgACMediaTypeFromChanspec(pdmif->chanspec);
      }

      /* Workaround to pass certification test where the Marvell controller gets killed if we
       * don't send AX media type. So, for MAP R1 certification if the device is
       * acting as agent send AX media type itself
       */
      if (I5_IS_R1_CERT_COMPATIBLE(i5_config.flags) && I5_IS_MULTIAP_AGENT(i5_config.flags)) {
	      if (i5DmIsInterfaceWireless(pdmif->MediaType)) {
			MediaType = I5_MEDIA_TYPE_WIFI_AX;
	      } else {
			MediaType = pdmif->MediaType;
	      }
      }
      *((unsigned short *)pbuf) = htons(MediaType);
                             ...
                             i5_tlv_linkMetricRx_t const * rxStats, int numLinks)
{
  ...
  for (;linkCounter < numLinks; linkCounter++) {
    ...
    buf[index] = (unsigned char)rxStats[linkCounter].rssi;
    index += 1;
                              ...
                              i5_tlv_linkMetricRx_t * rxStats, int maxLinks, int *numLinksReturned)
{
  ...
  *numLinksReturned = (length-i5TlvLinkMetricRxOverhead_Length) / i5TlvLinkMetricRxPerLink_Length;
  ...
  for ( ; (linkCounter < *numLinksReturned) && (linkCounter < maxLinks) ; linkCounter++) {
    ...
    rxStats[linkCounter].rssi = (signed char)*pValue;
    pValue += 1;
  }
  ...
  while (1) {
    ...
    for ( ; linkIndex < numLinks ; linkIndex ++) {
      i5TraceInfo("Rep's If: " I5_MAC_DELIM_FMT " Other I/f: " I5_MAC_DELIM_FMT " %d/%d/%d \n",
        ...
        rxStats[linkIndex].receivedPackets, rxStats[linkIndex].rssi);
...
/* TLV to report the vendor specific data TLV */
...
{
  unsigned char *pbuf, *data = vendorSpec_msg;
  unsigned int rc = 0, rem = vendorSpec_len;
  i5_tlv_t *ptlv;
  i5_dm_device_type *pdevice;

  /* Check for NULL before using pdevice.
   * Here it is not checked since vendor specific TLV can go
   * in multicast message when device is not present in datamodel
  */
  pdevice = i5DmDeviceFind(i5MessageDstMacAddressGet(pmsg));

  /* Max Fragment Size (Max payload - OUI length(3)) */
  unsigned int max_fragment_sz = I5_MAX_TLV_PAYLOAD_LEN - i5TlvVendorSpecificOui_Length;
  ...
  while (rem > 0) {
    ...
    unsigned short index = 0, this_fragment_sz = 0;

    memset(pbuf, 0, I5_PACKET_BUF_LEN);

    if (rem > max_fragment_sz) {
      this_fragment_sz = max_fragment_sz;
    } else {
      this_fragment_sz = rem;
    }

    ptlv = (i5_tlv_t *)pbuf;
    ptlv->type = i5TlvVendorSpecificType;
    ptlv->length = htons(this_fragment_sz + i5TlvVendorSpecificOui_Length);
    index += sizeof(i5_tlv_t);

    /* copy the OUI */
    memcpy(&pbuf[index], oui, i5TlvVendorSpecificOui_Length);
    ...
    /* calculate remaining */
    rem -= this_fragment_sz;
    data += this_fragment_sz;

    if (!pdevice || !I5_IS_SPLIT_VNDR_MSG(pdevice->flags)) {
      break;
    }
  }
  ...
  unsigned char **vendorSpec_data, unsigned int * vendorSpec_len, const char *oui)
{
  ...
  /* Extract all the vendor specific TLV till we get the required OUI */
  while (i5MessageTlvExtract(pmsg, i5TlvVendorSpecificType, &length, vendorSpec_data,
    i5MessageTlvExtractWithoutReset) == 0) {
    i5TraceInfo("Extracted %d bytes at %p\n", length, *vendorSpec_data);
    if (memcmp(*vendorSpec_data, oui, I5_VNDR_OUI_LEN) == 0) {
...
int i5TlvChannelSelectionResponseTypeExtract(i5_message_type *pmsg)
{
  ...
  while ((rc = i5MessageTlvExtract(pmsg, i5TlvChannelSelectionResponseType,
    ...
    pdmif->chan_sel_resp_code = (t_I5_CHAN_SEL_RESP_CODE) *pvalue;
    i5Trace("Neighbour Interface " I5_MAC_DELIM_FMT " Channel Selection Response: %s(0x%02x)\n", I5_MAC_PRM(mac),
      pdmif->chan_sel_resp_code ? "Decline": "Accept", pdmif->chan_sel_resp_code);

    if (i5_config.cbs.recv_chan_selection_resp) {
      i5_config.cbs.recv_chan_selection_resp(pdevice->DeviceId, pdmif->InterfaceId,
        pdmif->chan_sel_resp_code);
    }
  }
  if (!found) {
    rc = -1;
...
int i5TlvAPMetricsTypeExtract(i5_message_type *pmsg, unsigned char isCombined)
{
  ...
  while ((rc = i5MessageTlvExtract(pmsg, i5TlvAPMetricsType, &length, &pvalue,
    ...
    pdmbss->APMetric.ts_recvd = now;
    pdmbss->APMetric.flags |= I5_FLAG_BSS_AP_METRICS_RCVD;
  ...
  ieee1905_sta_traffic_stats *traffic_stats, i5_dm_device_type *pDestDevice)
{
  ...
  if (I5_P2APCAP_COUNTER_UNITS_GET(byte_cntr_unit) == I5_P2APCAP_COUNTER_UNITS_KBYTES) {
    bytes_sent = (unsigned int)(traffic_stats->bytes_sent / 1024);
    bytes_recv = (unsigned int) (traffic_stats->bytes_recv / 1024);
  } else if (I5_P2APCAP_COUNTER_UNITS_GET(byte_cntr_unit) == I5_P2APCAP_COUNTER_UNITS_MBYTES) {
    bytes_sent = (unsigned int) (traffic_stats->bytes_sent / (1024 * 1024));
    bytes_recv = (unsigned int) (traffic_stats->bytes_recv/ (1024 * 1024));
  } else {
...
int i5TlvMultiAPProfileInsert(i5_message_type *pmsg)
{
  ...
  if (I5_IS_R1_CERT_COMPATIBLE(i5_config.flags)) {
    i5Trace("Do not insert MultiAP Profile TLV to pass certification\n");
    return 0;
  }
...
int i5TlvProfile2APCapabilityInsert(i5_message_type *pmsg, i5_dm_p2_ap_capability_type *p2ApCap)
{
  ...
  if (I5_IS_R1_CERT_COMPATIBLE(i5_config.flags)) {
    i5Trace("Do not insert Profile2 AP Capability TLV to pass certification\n");
    return 0;
  }
...
int i5TlvR2ErrorCodeTypeInsert(i5_message_type *pmsg, map_p2_err_codes_t err_code,
  unsigned char *bssid)
{
  unsigned char *pbuf, *pmem;
  i5_tlv_t *ptlv;
  ...
  if (!I5_IS_MULTIAP_AGENT(i5_config.flags)) {
    /* send traffic separation Error Code Response only from agent */
    return 0;
  }
  ...
  /* Fill error reason code */
  *pbuf = err_code;
  pbuf++;

  /* Fill BSSIDs in the end if its available based on err_code */
  if (((err_code == map_p2_err_ts_fh_prof1_not_supported) ||
    (err_code == map_p2_err_ts_prof1_prof2_not_supported)) &&
    (NULL != bssid)) {
    eacopy(bssid, pbuf);
  }
  pbuf += MAC_ADDR_LEN;
  ...
  ptlv = (i5_tlv_t *)pmem;
  ptlv->type = i5TlvR2ErrorCodeType;
  ptlv->length = htons(pbuf - pmem - sizeof(i5_tlv_t));
  rc = i5MessageInsertTlv(pmsg, pmem, pbuf - pmem);
...
{
  unsigned char *pvalue;
  unsigned int length, ibuf_len;
  map_p2_err_codes_t err_code;
  unsigned char bssid[IEEE1905_MAC_ADDR_LEN] = {0x00};

  i5Trace("\n");

  if (!I5_IS_MULTIAP_CONTROLLER(i5_config.flags)) {
    /* receive traffic separation Error Code Response only from controller */
    return 0;
  }

  i5MessageReset(pmsg);

  while(i5MessageTlvExtract(pmsg, i5TlvR2ErrorCodeType, &length, &pvalue,
    i5MessageTlvExtractWithoutReset) == 0) {

    ibuf_len = length;

    if (length < i5TLvR2ErrorCode_Min_Length) {
      goto end;
    }
    ...
    i5TraceInfo("Profile 2 Error Response reason code [0x%01x] \n", err_code);

    if (((err_code == map_p2_err_ts_fh_prof1_not_supported) ||
      (err_code == map_p2_err_ts_prof1_prof2_not_supported)) &&
      ibuf_len >= MAC_ADDR_LEN) {

      eacopy(pvalue, &bssid);

      i5TraceInfo("Profile 2 Error Response BSSID [" I5_MAC_DELIM_FMT "] \n",
        I5_MAC_PRM(&bssid));
    }
    pvalue += MAC_ADDR_LEN;
    ibuf_len -= MAC_ADDR_LEN;
...
int i5TlvAPRadioAdvancedCapabilitiesTypeInsert(i5_message_type *pmsg, i5_dm_interface_type *pdmif)
{
  ...
  if (I5_IS_R1_CERT_COMPATIBLE(i5_config.flags)) {
    i5Trace("Do not insert AP Radio Advanced Capabilities TLV to pass certification\n");
    return 0;
  }
  ...
  i5_dm_device_type *pDestDevice)
{
  ...
  if (I5_P2APCAP_COUNTER_UNITS_GET(byte_cntr_unit) == I5_P2APCAP_COUNTER_UNITS_KBYTES) {
    unicastBytesSent = (unsigned int)(pbss->APMetric.unicastBytesSent / 1024);
    unicastBytesReceived = (unsigned int)(pbss->APMetric.unicastBytesReceived / 1024);
    multicastBytesSent = (unsigned int)(pbss->APMetric.multicastBytesSent / 1024);
    multicastBytesReceived = (unsigned int)(pbss->APMetric.multicastBytesReceived / 1024);
    broadcastBytesSent = (unsigned int)(pbss->APMetric.broadcastBytesSent / 1024);
    broadcastBytesReceived = (unsigned int)(pbss->APMetric.broadcastBytesReceived / 1024);
  } else if (I5_P2APCAP_COUNTER_UNITS_GET(byte_cntr_unit) == I5_P2APCAP_COUNTER_UNITS_MBYTES) {
    unicastBytesSent = (unsigned int)(pbss->APMetric.unicastBytesSent / (1024 * 1024));
    unicastBytesReceived = (unsigned int)(pbss->APMetric.unicastBytesReceived / (1024 * 1024));
    multicastBytesSent = (unsigned int)(pbss->APMetric.multicastBytesSent / (1024 * 1024));
    multicastBytesReceived = (unsigned int)(pbss->APMetric.multicastBytesReceived / (1024 * 1024));
    broadcastBytesSent = (unsigned int)(pbss->APMetric.broadcastBytesSent / (1024 * 1024));
    broadcastBytesReceived = (unsigned int)(pbss->APMetric.broadcastBytesReceived / (1024 * 1024));
  } else {
...
int i5TlvCacStatusTypeExtract(i5_message_type *pmsg)
{
  ...
  if (i5MessageTlvExtract(pmsg, i5TlvCACStatusReportType, &length, &pvalue,
    ...
    /* Extract radar channel list */
    n_chan = *pvalue++;
    if (n_chan == 0) {
      i5Trace(" No inactive(radar detected) channels\n");
      if (pdevice->inactive_info) {
        free(pdevice->inactive_info);
	pdevice->inactive_info = NULL;
      }
    } else {
      min_length += n_chan * sizeof (ieee1905_opclass_chan_info_t);
      ...
      buf_size = IEEE1905_MAX_DFS_CHANNELS * sizeof (ieee1905_opclass_chan_info_t) +
        sizeof(ieee1905_opclass_chan_info_list_t);
      if (pdevice->inactive_info == NULL) {
        pdevice->inactive_info = (ieee1905_opclass_chan_info_list_t *)malloc(buf_size);
      }
      if (pdevice->inactive_info == NULL) {
        i5TraceDirPrint("Out of memory error\n");
        goto end;
      }
      memset(pdevice->inactive_info, 0, buf_size);

      pdevice->inactive_info->n_count = n_chan;
      i5TraceInfo("Device ["I5_MAC_DELIM_FMT"] has %d channels radar detected\n",
        I5_MAC_PRM(pdevice->DeviceId), n_chan);

      for(i = 0; i < n_chan; i++) {
        ieee1905_opclass_chan_info_t *info = &pdevice->inactive_info->info[i];
        ieee1905_opclass_chan_info_t *tmp = (ieee1905_opclass_chan_info_t *)pvalue;
...
int i5TlvBackhaulBSSConfigurationTypeInsert(i5_message_type *pmsg, unsigned char *bssid, unsigned char bsta_flag)
{
  unsigned char *pbuf, *pmem;
  i5_tlv_t *ptlv;
  int rc = 0;
  ...
  if (!I5_IS_MULTIAP_CONTROLLER(i5_config.flags)) {
    /* send backhaul BSS policy only from controller */
    return 0;
  }
  ...
  /* Fill Backhaul BSS Configuration Policy */
  eacopy(bssid, pbuf);
  pbuf += MAC_ADDR_LEN;

  i5TraceInfo("Backhaul BSSID: [" I5_MAC_DELIM_FMT "] map_flag [0x%x] \n",
    I5_MAC_PRM(bssid), bsta_flag);

  if (I5_IS_BSS_PROF1_DISALLOWED(bsta_flag)) {
    *pbuf = I5_TLV_BH_BSS_CONFIG_P1_BSTA_DISALLOWED;
  }
  if (I5_IS_BSS_PROF2_DISALLOWED(bsta_flag)) {
    *pbuf = I5_TLV_BH_BSS_CONFIG_P2_BSTA_DISALLOWED;
  }
  pbuf++;

  ptlv = (i5_tlv_t *)pmem;
  ptlv->type = i5TlvBackhaulBSSConfigPolicyType;
  ptlv->length = htons(pbuf - pmem - sizeof(i5_tlv_t));
  rc = i5MessageInsertTlv(pmsg, pmem, pbuf - pmem);
...
/* TLV to extract BackhaulBSS Config policy */
int i5TlvBackhaulBSSConfigurationTypeExtract(i5_message_type *pmsg)
{
  unsigned char *pvalue;
  unsigned int length;
  ieee1905_bh_bss_list *bh_bss;

  if (!I5_IS_MULTIAP_AGENT(i5_config.flags)) {
    /* receive backhaul BSS policy only from agent */
    return 0;
  }

  /* Clean up before filling the latest data. */
  i5DmGlistCleanup(&i5_config.policyConfig.no_bh_bss_list);
  ieee1905_glist_init(&i5_config.policyConfig.no_bh_bss_list);

  i5MessageReset(pmsg);
  while(i5MessageTlvExtract(pmsg, i5TlvBackhaulBSSConfigPolicyType, &length, &pvalue,
    i5MessageTlvExtractWithoutReset) == 0) {

    if (length < i5TlvBackhaulBSSConfigPolicyType_Min_Length) {
      goto end;
    }

    bh_bss = (ieee1905_bh_bss_list*)malloc(sizeof(*bh_bss));
    if (!bh_bss) {
      i5TraceDirPrint("malloc error\n");
      goto end;
    }
    /* get backhaul bssid */
    eacopy(pvalue, bh_bss->bssid);
    pvalue += MAC_ADDR_LEN;

    /* get backhaul sta flag */
    bh_bss->bsta_flag = *pvalue;
    pvalue++;

    i5TraceInfo(" Backhaul BSSID [" I5_MAC_DELIM_FMT "] bsta_flag [%x] \n",
      I5_MAC_PRM(bh_bss->bssid), bh_bss->bsta_flag);

    ieee1905_glist_append(&i5_config.policyConfig.no_bh_bss_list, (dll_t*)bh_bss);
  }
  ...
  unsigned char stamac_flag, unsigned char *sta_mac)
{
  ...
  if (!I5_IS_MULTIAP_AGENT(i5_config.flags)) {
    /* send backhaul STA radio capabilities only from agent */
    return 0;
  }
  ...
  *pbuf = stamac_flag;
  ...
  i5TraceInfo("Backhaul MAC [" I5_MAC_DELIM_FMT "] mac_flag [0x%x]\n",
    I5_MAC_PRM(mac), stamac_flag);

  /* get backhaul STA mac address for the associated STA */
  if ((NULL != sta_mac) && (stamac_flag & I5_TLV_BSTA_RADIO_CAPS_MAC_INCLUDED)) {

    i5TraceInfo("STA MAC [" I5_MAC_DELIM_FMT "] \n",
      I5_MAC_PRM(sta_mac));

    eacopy(sta_mac, pbuf);
  }

  if (stamac_flag & I5_TLV_BSTA_RADIO_CAPS_MAC_INCLUDED) {
    pbuf += MAC_ADDR_LEN;
  }

  ptlv = (i5_tlv_t *)pmem;
  ptlv->type = i5TlvBackhaulSTARadioCapabilitiesType;
  ptlv->length = htons(pbuf - pmem - sizeof(i5_tlv_t));
  rc = i5MessageInsertTlv(pmsg, pmem, pbuf - pmem);
  free(pmem);
  return rc;
...
/* TLV to extract BackhaulSTA Radio Capabilities */
int i5TlvBackhaulSTARadioCapabilitiesTypeExtract(i5_message_type *pmsg)
{
  unsigned char *pvalue;
  unsigned int length, ibuf_len;
  unsigned char mac[IEEE1905_MAC_ADDR_LEN] = {0x00}; /* Backahaul mac */
  unsigned char stamac_flag = 0;  /* stamac flag included/not included */
  unsigned char bsta_mac[IEEE1905_MAC_ADDR_LEN] = {0x00}; /* Backhaul sta mac address */
  i5_dm_device_type *pdevice;
  i5_dm_interface_type *pdmif, *pdmruidif;

  if (!I5_IS_MULTIAP_CONTROLLER(i5_config.flags)) {
    /* receive backhaul STA radio capabilities only from controller */
    return 0;
  }
  ...
  while(i5MessageTlvExtract(pmsg, i5TlvBackhaulSTARadioCapabilitiesType, &length, &pvalue,
    i5MessageTlvExtractWithoutReset) == 0) {

    if (length < i5TlvBackhaulSTACapabilityType_Min_Length) {
      goto end;
    }
    ibuf_len = length;

    /* get backhaul mac of STA Radio capabilities */
    eacopy(pvalue, &mac);
    pvalue += MAC_ADDR_LEN;
    ibuf_len -= MAC_ADDR_LEN;

    /* get backhaul sta mac_flag of STA Radio capabilities */
    stamac_flag = *pvalue;
    pvalue++;
    ibuf_len--;

    i5TraceInfo(" Backhaul MAC [" I5_MAC_DELIM_FMT "] stamac_flag [%x] \n",
      I5_MAC_PRM(&mac), stamac_flag);

    /* Find interface matching RUID. M1 message comes for each interface with MAC address as
     * RUID
     */
    pdmruidif = i5DmInterfaceFind(pdevice, mac);
    if (pdmruidif) {
      pdevice->bsta_band |= pdmruidif->band;
      i5Trace("In Device["I5_MAC_DELIM_FMT"] For RUID [" I5_MAC_DELIM_FMT "] BSTA band[0x%x] "
        "Band From RUID[0x%x]\n", I5_MAC_PRM(pdevice->DeviceId), I5_MAC_PRM(mac),
        pdevice->bsta_band, pdmruidif->band);
    } else {
      i5TraceError("In Device["I5_MAC_DELIM_FMT"] Interface with RUID [" I5_MAC_DELIM_FMT "] "
        "not found\n", I5_MAC_PRM(pdevice->DeviceId), I5_MAC_PRM(mac));
      goto end;
    }

    if ((stamac_flag & I5_TLV_BSTA_RADIO_CAPS_MAC_INCLUDED) && (ibuf_len >= MAC_ADDR_LEN)) {

      eacopy(pvalue, &bsta_mac);
      pvalue += MAC_ADDR_LEN;
      ibuf_len -= MAC_ADDR_LEN;

      i5TraceInfo(" BSTA_MAC [" I5_MAC_DELIM_FMT "] \n", I5_MAC_PRM(bsta_mac));

      /* STA MAC address and RUID can be same. If so, update the MAP flag to STA */
      if (eacmp(mac, bsta_mac) == 0) {
        pdmruidif->mapFlags |= IEEE1905_MAP_FLAG_STA;
      } else {
        /* Else find the interface matching STA MAC address and update the MAP flag and band */
        pdmif = i5DmInterfaceFind(pdevice, bsta_mac);
        if (pdmif) {
          /* We can assign the band of RUID to pdmif. M1 message comes with RUID and band is
           * calculated when we recieve M1
           */
          pdmif->band |= pdmruidif->band;
          pdmif->mapFlags |= IEEE1905_MAP_FLAG_STA;
          i5Trace("In Device["I5_MAC_DELIM_FMT"] For RUID [" I5_MAC_DELIM_FMT "] and "
            "BSTA_MAC [" I5_MAC_DELIM_FMT "] BSTA band[0x%x] IFR band[0x%x] MapFlag[0x%x]\n",
            I5_MAC_PRM(pdevice->DeviceId), I5_MAC_PRM(mac), I5_MAC_PRM(bsta_mac),
            pdevice->bsta_band, pdmif->band, pdmif->mapFlags);
        } else {
          i5TraceError("In Device["I5_MAC_DELIM_FMT"] RUID [" I5_MAC_DELIM_FMT "] Interface with "
            "BSTA_MAC [" I5_MAC_DELIM_FMT "] not found\n",
            I5_MAC_PRM(pdevice->DeviceId), I5_MAC_PRM(mac), I5_MAC_PRM(bsta_mac));
          goto end;
        }
      }
