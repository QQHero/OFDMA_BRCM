 ...
 *
 * $Id: rte.c 804053 2021-10-18 16:20:23Z $
 */
...
#ifdef BCMDBG_ASSERT
	hnd_shared_assert_init(NULL, file, line);
	ASSERT_WITH_TRAP(0);
#endif /* BCMDBG_ASSERT */
}
...
extern uint32 _stackguard;

uint32 gFWID;

static void
BCMATTACHFN(hnd_mem_setup)(uintptr limit)
{
...
BCMATTACHFN(hnd_init)(void)
{
	...
	hnd_mem_setup((uintptr)_stackguard);
...
#ifdef HNDUIC
	/* Initialize the UIC service */
	if (uic_init(hnd_sih, hnd_osh) == BCME_OK) {
		hnd_cons_add_cmd("uic", uic_cmd, hnd_osh);
	}
#endif /* HNDUIC */
	...
	if (BCMPCIEDEV_ENAB()) {
		...
		if (!hnd_cons_add_cmd("pqp", pqp_cmd, hnd_osh)) {
			return NULL;
		}
		pqp_bind_hme(); // bind to HME
#endif /* HNDPQP */
#endif /* BCMHME */
#ifdef HNDMBX
		mbx_init(hnd_sih, hnd_osh); // Acquires BME_USR_H2D
		if (!hnd_cons_add_cmd("mbx", mbx_cmd, hnd_osh)) {
			return NULL;
		}
#endif /* HNDMBX */
	}
...
hnd_get_fatal_logbuf(uint32 size_requested, uint32 *allocated_size)
{
	...
	if ((hnd_fatal_logbuf_size == 0) && (hnd_fatal_logbuf_addr == NULL)) {
		...
		ca7_disable_rodata_protection(hnd_sih);
#endif /* __ARM_ARCH_7A__ && RTE_CACHED && !BCM_BOOTLOADER */
