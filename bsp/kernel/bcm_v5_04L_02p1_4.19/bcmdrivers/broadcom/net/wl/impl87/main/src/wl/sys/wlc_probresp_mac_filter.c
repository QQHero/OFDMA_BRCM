 ...
 *
 * $Id: wlc_probresp_mac_filter.c 796050 2021-02-22 15:47:14Z $
 */
...
static const bcm_iovar_t prb_iovars[] = {
	{"probresp_mac_filter", IOV_PROB_RESP_MAC_FILTER, (0), 0, IOVT_BOOL, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
struct wlc_probresp_mac_filter_info {
	...
	prob_resp_ea_t probresp_intransit_pool[MAX_PROBRESP]; /**< pool of prob_resp_ea objects */
	prob_resp_ea_t *probresp_intransit_freelist; /**< list of available prob_resp_ea objects */
};
...
BCMATTACHFN(wlc_probresp_mac_filter_attach)(wlc_info_t *wlc)
{
...
#if defined(BCMDBG)
	wlc_dump_register(wlc->pub, "prbresp_mac_filt", wlc_probresp_mac_filter_dump,
	...
	if (wlc_probresp_register(wlc->mprobresp, mprobresp_mac_filter,
		...
		WL_ERROR(("could not register probe response mac filter!\n"));
		goto fail;
	}
#ifdef WLPROBRESP_INTRANSIT_FILTER
	if (wlc_probresp_register(wlc->mprobresp, mprobresp_mac_filter,
	...
	for (i = 0; i < MAX_PROBRESP; i++) {
		stap = &mprobresp_mac_filter->probresp_intransit_pool[i];
		stap->next = mprobresp_mac_filter->probresp_intransit_freelist;
		mprobresp_mac_filter->probresp_intransit_freelist = stap;
	}
#endif /* WLPROBRESP_INTRANSIT_FILTER */
	...
	uint8 *body, int body_len, bool *psendProbeResp)
{
	...
	if (addr_match == WLC_MACFLTR_ADDR_DENY ||
		...
		wlc_macfltr_probcount_incr(wlc->macfltr, cfg, &hdr->sa);
	...
	wlc_bsscfg_t *cfg, struct ether_addr *da)
{
	...
	if ((stap = mprobresp_mac_filter->probresp_intransit_freelist) == NULL) {
		/* Maximum nr of probe response frames are in transit */
		ASSERT(mprobresp_mac_filter->intransit_inuse == MAX_PROBRESP);
		return FALSE;
	}

	mprobresp_mac_filter->probresp_intransit_freelist = stap->next;
	eacopy(da, &stap->mac);
	...
	wlc_bsscfg_t *cfg, struct ether_addr *da)
{
	...
	while (stap != NULL) {
		...
		if (!PROB_RESP_EA_CMP(&stap->mac, da)) {
			...
			stap->next = mprobresp_mac_filter->probresp_intransit_freelist;
			mprobresp_mac_filter->probresp_intransit_freelist = stap;
			ASSERT(mprobresp_mac_filter->intransit_inuse > 0);
			mprobresp_mac_filter->intransit_inuse--;
			break;
		}
		prevp = stap;
