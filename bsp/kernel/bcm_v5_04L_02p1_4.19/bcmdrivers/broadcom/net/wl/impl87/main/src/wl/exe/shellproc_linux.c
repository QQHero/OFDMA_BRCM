 ...
 *
 * $Id: shellproc_linux.c 794024 2020-12-17 11:04:02Z $
 */
...
remote_shell_async_exec(char *buf_ptr)
{
	...
	char temp_async_file_name[MAX_ASYNC_FILE_LENGTH+sizeof(SHELL_RESP_PATH)];
	pid_t pid;
...
{
	char file_name[MAX_ASYNC_FILE_LENGTH+sizeof(SHELL_RESP_PATH)];
...
remote_shell_sync_exec(char *cmd_buf_ptr, void *wl)
{
	...
	if (strcmp("reboot", cmd_buf_ptr) == 0) { /* reboot command */
		...
		strncpy(buf, REBOOT_MSG, sizeof(buf));
