 ...
 *
 * $Id: phy_ac_fcbs.c 789478 2020-07-29 13:38:18Z $
 */
...
/* local functions */
#ifdef ENABLE_FCBS
static bool phy_ac_fcbs_fcbsinit(phy_type_fcbs_ctx_t *ctx, int chanidx, chanspec_t chanspec);
static bool phy_ac_fcbs_prefcbsinit(phy_type_fcbs_ctx_t *ctx, int chanidx);
static bool phy_ac_fcbs_postfcbsinit(phy_type_fcbs_ctx_t *ctx, int chanidx);
static bool phy_ac_fcbs_fcbs(phy_type_fcbs_ctx_t *ctx, int chanidx);
	...
	phy_fcbs_info_t *cmn_info)
{
	...
	if (CHIPID(pi->sh->chip) == BCM4360_CHIP_ID) {
		fns.fcbsinit = phy_ac_fcbs_fcbsinit;
		fns.prefcbsinit = phy_ac_fcbs_prefcbsinit;
		fns.postfcbsinit = phy_ac_fcbs_postfcbsinit;
		fns.fcbs = phy_ac_fcbs_fcbs;
		fns.prefcbs = phy_ac_fcbs_prefcbs;
		fns.postfcbs = phy_ac_fcbs_postfcbs;
		pi->HW_FCBS = TRUE;
		pi->FCBS = TRUE;
 ...
 */
#define M_FCBS_ACPHY_PHYTBL16(x)		(M_FCBS_BLK(x) + 2)
#define M_FCBS_ACPHY_PHYTBL32(x)		(M_FCBS_BLK(x) + 4)
#define M_FCBS_ACPHY_PHYREG(x)		(M_FCBS_BLK(x) + 6)
...
static bool
phy_ac_fcbs_prefcbsinit(phy_type_fcbs_ctx_t *ctx, int chanidx)
{
	phy_ac_fcbs_info_t *info = (phy_ac_fcbs_info_t *)ctx;
	return wlc_phy_prefcbsinit_acphy(info->pi, chanidx);
}
...
static bool
phy_ac_fcbs_postfcbsinit(phy_type_fcbs_ctx_t *ctx, int chanidx)
{
	return TRUE;
...
static bool
phy_ac_fcbs_fcbsinit(phy_type_fcbs_ctx_t *ctx, int chanidx, chanspec_t chanspec)
{
	phy_ac_fcbs_info_t *info = (phy_ac_fcbs_info_t *)ctx;
	...
	if (pi->phy_fcbs->FCBS_ucode) {
		...
		*/
		pi->phy_fcbs->shmem_radioreg = M_FCBS_TEMPLATE_LENS(pi);
		pi->phy_fcbs->shmem_phytbl16 = M_FCBS_ACPHY_PHYTBL16(pi);
		pi->phy_fcbs->shmem_phytbl32 = M_FCBS_ACPHY_PHYTBL32(pi);
		pi->phy_fcbs->shmem_phyreg = M_FCBS_ACPHY_PHYREG(pi);
		pi->phy_fcbs->shmem_bphyctrl = M_FCBS_BPHY_CTRL(pi);
		pi->phy_fcbs->shmem_cache_ptr = M_FCBS_TEMPLATE_PTR(pi);
	}
	return TRUE;
}

static bool
phy_ac_fcbs_prefcbs(phy_type_fcbs_ctx_t *ctx, int chanidx)
{
	phy_ac_fcbs_info_t *info = (phy_ac_fcbs_info_t *)ctx;
	phy_info_t *pi = info->pi;
...
static bool
phy_ac_fcbs_postfcbs(phy_type_fcbs_ctx_t *ctx, int chanidx)
{
	return TRUE;
	/* Nothing to do right now */
}
...
static bool
phy_ac_fcbs_fcbs(phy_type_fcbs_ctx_t *ctx, int chanidx)
{
	uint16 chanspec;
	phy_ac_fcbs_info_t *info = (phy_ac_fcbs_info_t *)ctx;
	phy_info_t *pi = info->pi;
	...
	if ((READ_PHYREG(pi, ChannelSwitch) &
			(0x1 << ACPHY_ChannelSwitch_SwitchTrigger_SHIFT(pi->pubpi->phy_rev)))) {
		PHY_FATAL_ERROR_MESG((" %s: SPINWAIT ERROR : FCBS ch switch failed \n",
				__FUNCTION__));
		PHY_FATAL_ERROR(pi, PHY_RC_FCBS_CHSW_FAILED);
	}
...
static uint16
phy_ac_fcbs_channelindicator_obtain(phy_type_fcbs_ctx_t *ctx)
{
	phy_ac_fcbs_info_t *info = (phy_ac_fcbs_info_t *)ctx;
	phy_info_t *pi = info->pi;
...
phy_ac_fcbs_iov_arm_get(phy_type_fcbs_ctx_t *ctx, chanspec_t chanspec, int chanidx)
{
	...
	wlc_phy_fcbs_arm(pi, chanspec, chanidx);
	return BCME_OK;
}
