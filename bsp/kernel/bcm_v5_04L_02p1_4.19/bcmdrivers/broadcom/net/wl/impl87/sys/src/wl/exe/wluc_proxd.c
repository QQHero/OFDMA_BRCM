 ...
 *
 * $Id: wluc_proxd.c 809675 2022-03-22 05:14:17Z $
 */
...
#define WL_PROXD_TUNE_VERSION_MAJOR_4	4
...
int proxd_tune_ver_get(int ver_major)
{
	...
	if (ver_major <= 3) { /* Maccabee (9.44) major ver is 3 */
		...
		/* use v4 */
		proxd_tune_expected_ver = WL_PROXD_TUNE_VERSION_MAJOR_4;
#ifdef WL_FTM_DEBUG
		printf("ver_major is %d\n", ver_major);
#endif /* WL_FTM_DEBUG */
	}
...
static int proxd_tune_set_params_from_opt_v4(const char* cmd_name, miniopt_t *mopt,
	wl_proxd_params_tof_tune_v4_t *proxd_tune)
{
	if (mopt->opt == 'k') {
		if (!mopt->good_int) {
			char *p = strstr(mopt->valstr, ",");
		...
		const char* s[4] = {"s", "l", "o", NULL};
		const char **p;
		int v[TOF_BW_NUM_V2] = {0, 0, 0, 0};
		int m[TOF_BW_NUM_V2] = {0, 0, 0, 0};
		int i;
		if (*mopt->valstr == 's')
			p = s;
		else
		...
		if (m[0]) {
			/* Got bw */
			if (v[0] == TOF_BW_160MHZ)
				i = TOF_BW_80MHZ_INDEX_V2;
			else if (v[0] == TOF_BW_80MHZ)
				i = TOF_BW_80MHZ_INDEX_V2;
			else if (v[0] == TOF_BW_40MHZ)
				i = TOF_BW_40MHZ_INDEX_V2;
			else if (v[0] == TOF_BW_20MHZ)
				i = TOF_BW_20MHZ_INDEX_V2;
			else {
				fprintf(stderr,
					"%s: could not parse \"%s\" as window params\n",
					cmd_name, mopt->valstr);
			...
			if (p == n) {
				...
				/* Seq */
				if (m[1] && i == TOF_BW_20MHZ_INDEX_V2) {
					/* Got length */
					proxd_tune->seq_5g20.w_len = (int16)v[1];
					proxd_tune->seq_2g20.w_len = (int16)v[1];
				}
				if (m[2]&& i == TOF_BW_20MHZ_INDEX_V2) {
					/* Got offset */
					proxd_tune->seq_5g20.w_offset = (int16)v[2];
					proxd_tune->seq_2g20.w_offset = (int16)v[2];
				}
	...
	miniopt_t *mopt, void *proxd_tune, int ver_major)
{
	...
	wl_proxd_params_tof_tune_v4_t * proxd_tune_v4 = NULL;
	...
	case WL_PROXD_TUNE_VERSION_MAJOR_3:
		...
		break;
	case WL_PROXD_TUNE_VERSION_MAJOR_4:
		proxd_tune_v4 = (wl_proxd_params_tof_tune_v4_t *)proxd_tune;
		ret = proxd_tune_set_params_from_opt_v4(cmd_name, mopt, proxd_tune_v4);
		break;
	default:
		ret = BCME_UNSUPPORTED;
		break;
...
static int proxd_tune_ver_get_size(int expected_ver)
{
	...
	case WL_PROXD_TUNE_VERSION_MAJOR_3:
		...
		break;
	case WL_PROXD_TUNE_VERSION_MAJOR_4:
		ret = sizeof(wl_proxd_params_tof_tune_v4_t);
		break;
	default:
		ret = BCME_UNSUPPORTED;
		break;
...
{
	wl_proxd_params_iovar_v2_t proxd_tune, *reply;
	uint16 method;
	void *ptr = NULL;
	int ret, opt_err;
	miniopt_t to;
	...
	if (!*++argv) {
		...
		reply = (wl_proxd_params_iovar_v2_t *)ptr;
...
ftm_unpack_and_display_rtt_result_v2(const uint8 *p_data, uint16 tlvid, uint16 len)
{
	...
	p_sample = &p_data_info->rtt[0];
	if (flags & WL_PROXD_LTFSEQ_STARTED) {
		printf(">\tavg_rtt sample: rssi=%d snr=%d bitflips=%d rtt=%d%s "
		...
		printf(">\tavg_rtt sample: rssi=%d rtt=%d%s "
			"std_deviation = %d.%d ratespec=0x%08x chanspec=0x%08x\n",
			(wl_proxd_rssi_t) ltoh16_ua(&p_sample->rssi),
			ltoh32_ua(&p_sample->rtt.intvl),
			ftm_tmu_value_to_logstr(ltoh16_ua(&p_sample->rtt.tmu)),
	...
	if (flags & WL_PROXD_REQUEST_SENT) {
		if (flags & WL_PROXD_REQUEST_ACKED) {
			if (flags & WL_PROXD_RESULT_FLAG_HEACK)
				printf("(REQSENT/ACKED/HE ACK)");
			else if (flags & WL_PROXD_RESULT_FLAG_VHTACK)
	...
	if (num_rtt > 0)
	{
		...
		for (i = 0; i < num_rtt; i++)
		{
			...
			if (flags & WL_PROXD_LTFSEQ_STARTED) {
				...
				printf(">\t sample[%d]: id=%d rssi=%d snr=%f bitflips=%d"
				       " tof_phy_error %x tof_phy_tgt_error %x target_snr=%f"
				       " target_bitflips=%d dist=%d rtt=%d%s status %s Type %s"
				       " coreid=%d rspec=0x%08x\n",
				       i, p_sample->id, rssi, ((double)snr),
				       bitflips, tof_phy_error, tof_phy_tgt_error,
				       ((double)tof_target_snr),
				       tof_target_bitflips, dist,
				       ...
				       ftm_tmu_value_to_logstr(ltoh16_ua(&p_sample->rtt.tmu)),
				       ftm_status_value_to_logstr(ltoh32_ua(&p_sample->status)),
				       ftm_frame_types[i % TOF_DEFAULT_FTMCNT_SEQ],
				       p_sample->coreid, ltoh32_ua(&p_sample->ratespec));
			} else {
				rssi = (wl_proxd_rssi_t) ltoh16_ua(&p_sample->rssi);
				printf(">\t id=%d rssi=%d rtt=%d%s"
				       " coreid=%d rspec=0x%08x tpts=%d ipts=%d %s\n",
				       p_sample->id, rssi,
				       ltoh32_ua(&p_sample->rtt.intvl),
				       ftm_tmu_value_to_logstr(ltoh16_ua(&p_sample->rtt.tmu)),
				       p_sample->coreid, ltoh32_ua(&p_sample->ratespec),
				       ((p_sample->flags & WL_PROXD_RTT_SAMPLE_TGT_PHYTS)? 1:0),
				       ((p_sample->flags & WL_PROXD_RTT_SAMPLE_INIT_PHYTS)? 1:0),
				       ((p_sample->flags & WL_PROXD_RTT_SAMPLE_DISCARD) ?
						"discard" : ""));
			}
			p_sample++;
		}
...
static const ftm_config_param_info_t ftm_config_param_info[] = {
	...
	FTM_SUBCMD_FLAG_ALL },
	{ "dev-addr", WL_PROXD_TLV_ID_DEV_ADDR, "device address(method only)",
	FTM_SUBCMD_FLAG_METHOD },
...
static const ftm_config_options_info_t ftm_method_options_info[] = {
	...
	{ "avb-ts", WL_PROXD_FLAG_AVB_TS, "Force AVB usage" },
	...
	wl_proxd_method_t method, wl_proxd_session_id_t session_id, char **argv)
{
	...
	wl_proxd_params_tof_tune_v4_t tof_tune_v4;
	...
	memset(&tof_tune_v4, 0, sizeof(tof_tune_v4));
	...
	if (!*++argv) {
		...
		if (proxd_tune_expected_ver ==
			...
			WL_PROXD_TUNE_VERSION_MAJOR_4) {
			selected_proxd_tune = &tof_tune_v4;
		}
		if (p_iovresp != NULL) {
			int tlvs_len = ltoh16(p_iovresp->len) - WL_PROXD_IOV_HDR_SIZE;
...
static void proxd_tune_display_v4(wl_proxd_params_tof_tune_v4_t *tof_tune_v4)
{
	uint i = 0, internal_version = 0;

	if (!tof_tune_v4)
		return;

#ifdef WL_FTM_DEBUG
	printf("%s: Displaying wl_proxd_params_tof_tune_v4_t\n", __FUNCTION__);
#endif /* WL_FTM_DEBUG */

	printf("Ki=%d \n", dtoh32(tof_tune_v4->Ki));
	printf("Kt=%d \n", dtoh32(tof_tune_v4->Kt));
	printf("vhtack=%d \n", dtoh16(tof_tune_v4->vhtack));
	printf("seq_en=%d\n", dtoh16(tof_tune_v4->seq_en));
	printf("core=%d\n", tof_tune_v4->core);
	printf("sw_adj=%d\n", dtoh16(tof_tune_v4->sw_adj));
	printf("hw_adj=%d\n", dtoh16(tof_tune_v4->hw_adj));
	printf("minDT = %d\n", tof_tune_v4->minDT);
	printf("maxDT = %d\n", tof_tune_v4->maxDT);
	printf("threshold_log2=%d %d %d %d seqtx %d seqrx %d 2g %d seqtx5g20 %d"
		" seqrx5g20 %d seqtx2g20 %d seqrx2g20 %d\n",
		dtoh16(tof_tune_v4->N_log2[TOF_BW_20MHZ_INDEX_V2]),
		dtoh16(tof_tune_v4->N_log2[TOF_BW_40MHZ_INDEX_V2]),
		dtoh16(tof_tune_v4->N_log2[TOF_BW_80MHZ_INDEX_V2]),
		dtoh16(tof_tune_v4->N_log2[TOF_BW_160MHZ_INDEX_V2]),
		dtoh16(tof_tune_v4->N_log2[TOF_BW_SEQTX_INDEX_V2]),
		dtoh16(tof_tune_v4->N_log2[TOF_BW_SEQRX_INDEX_V2]),
		dtoh16(tof_tune_v4->N_log2_2g),
		dtoh16(tof_tune_v4->seq_5g20.N_tx_log2),
		dtoh16(tof_tune_v4->seq_5g20.N_rx_log2),
		dtoh16(tof_tune_v4->seq_2g20.N_tx_log2),
		dtoh16(tof_tune_v4->seq_2g20.N_rx_log2));
	printf("threshold_scale=%d %d %d %d seqtx %d seqrx %d 2g %d seqtx5g20 %d"
		" seqrx5g20 %d seqtx2g20 %d seqrx2g20 %d\n",
		dtoh16(tof_tune_v4->N_scale[TOF_BW_20MHZ_INDEX_V2]),
		dtoh16(tof_tune_v4->N_scale[TOF_BW_40MHZ_INDEX_V2]),
		dtoh16(tof_tune_v4->N_scale[TOF_BW_80MHZ_INDEX_V2]),
		dtoh16(tof_tune_v4->N_scale[TOF_BW_160MHZ_INDEX_V2]),
		dtoh16(tof_tune_v4->N_scale[TOF_BW_SEQTX_INDEX_V2]),
		dtoh16(tof_tune_v4->N_scale[TOF_BW_SEQRX_INDEX_V2]),
		dtoh16(tof_tune_v4->N_scale_2g),
		dtoh16(tof_tune_v4->seq_5g20.N_tx_scale),
		dtoh16(tof_tune_v4->seq_5g20.N_rx_scale),
		dtoh16(tof_tune_v4->seq_2g20.N_tx_scale),
		dtoh16(tof_tune_v4->seq_2g20.N_rx_scale));
	printf("total_frmcnt=%d \n", tof_tune_v4->totalfrmcnt);
	printf("reserve_media=%d \n", tof_tune_v4->rsv_media);
	printf("flags=0x%x \n", dtoh16(tof_tune_v4->flags));
	for (i = 0; i < TOF_BW_NUM_V2; i++) {
		printf("window length %dMHz = %d\n",
			(20 << i), tof_tune_v4->w_len[i]);
		printf("window offset %dMHz = %d\n",
			(20 << i), tof_tune_v4->w_offset[i]);
	}
	printf("seq window length 5G-20MHz = %2d 2G-20MHz = %2d\n",
		tof_tune_v4->seq_5g20.w_len,
		tof_tune_v4->seq_2g20.w_len);
	printf("seq window offset 5G-20MHz = %2d 2G-20MHz = %2d\n",
		tof_tune_v4->seq_5g20.w_offset,
		tof_tune_v4->seq_2g20.w_offset);
	printf("frame count=%d %d %d %d seq %d\n",
		tof_tune_v4->ftm_cnt[TOF_BW_20MHZ_INDEX_V2],
		tof_tune_v4->ftm_cnt[TOF_BW_40MHZ_INDEX_V2],
		tof_tune_v4->ftm_cnt[TOF_BW_80MHZ_INDEX_V2],
		tof_tune_v4->ftm_cnt[TOF_BW_160MHZ_INDEX_V2],
		tof_tune_v4->ftm_cnt[TOF_BW_SEQTX_INDEX_V2]);

	printf("bitflip threshold=%u\n", dtoh16(tof_tune_v4->bitflip_thresh));
	printf("SNR threshold=%u\n", dtoh16(tof_tune_v4->snr_thresh));
	printf("2G receive sensitivity threshold=%d\n", tof_tune_v4->recv_2g_thresh);
	printf("ACS GDV threshold=%u\n", dtoh32(tof_tune_v4->acs_gdv_thresh));
	printf("ACS RSSI threshold=%d\n", tof_tune_v4->acs_rssi_thresh);
	printf("Smoothing window enable=%u\n", tof_tune_v4->smooth_win_en);

	internal_version = dtoh32(tof_tune_v4->version);
#ifdef WL_FTM_DEBUG
	printf("internal_version %d\n", internal_version);
#endif /* WL_FTM_DEBUG */
	switch (internal_version) {
	case WL_PROXD_TUNE_VERSION_1: /* target */
		printf("emulator delay=%u\n", dtoh32(tof_tune_v4->emu_delay));
		break;
	case WL_PROXD_TUNE_VERSION_2: /* initiator */
	case WL_PROXD_TUNE_VERSION_3: /* initiator */
		printf("ACS GDMM threshold=%u\n", dtoh32(tof_tune_v4->acs_gdmm_thresh));
		printf("ACS RSSI delta threshold=%d\n", tof_tune_v4->acs_delta_rssi_thresh);
		printf("emulator delay=%u\n", dtoh32(tof_tune_v4->emu_delay));
		printf("Core Mask = 0x%x\n", tof_tune_v4->core_mask);
		break;
	default:
		break;
	}
...
proxd_tune_display(void *tof_tune, uint16 len)
{
	...
	wl_proxd_params_tof_tune_v4_t * tof_tune_v4 = NULL;
	...
	printf("sizeof(wl_proxd_params_tof_tune_v4_t) is %d \n",
		(int) sizeof(wl_proxd_params_tof_tune_v4_t));
#endif /* WL_FTM_DEBUG */
	...
	} else if (len == sizeof(wl_proxd_params_tof_tune_v3_t)) {
		tof_tune_v3 = (wl_proxd_params_tof_tune_v3_t *)tof_tune;
		proxd_tune_display_v3(tof_tune_v3);
	} else {
		tof_tune_v4 = (wl_proxd_params_tof_tune_v4_t *)tof_tune;
		proxd_tune_display_v4(tof_tune_v4);
	}
done:
	return err;
}
