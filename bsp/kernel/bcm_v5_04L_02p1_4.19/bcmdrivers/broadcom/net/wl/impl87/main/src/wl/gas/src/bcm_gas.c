	...
	wl_event_msg_t *wlEvent, uint8 *data, uint32 length)
{
	...
	case WLC_E_ACTION_FRAME_RX:
	{
		...
		if (bcm_decode_init(&dec, actFrameLen, actFrame) &&
			...
			fsm(gas, event, &gasDecode, actFrameLen, actFrame);
		}
...
static void do_notification(bcm_gas_t *gas, bcm_gas_event_t *eventp)
{
	...
	/* Free the event body buffer allocated in query_request_notification */
	if (eventp->body) {
		wl_gas_free(drv, eventp->body);
	}
...
static void query_request_notification(bcm_gas_t *gas, uint16 reqLen, uint8 *req, int dataLen,
	uint8 *data)
{
	gasEventCallback_t *p = get_gasEventCallback(gas);
	if (p->fn) {
		...
		eventp->body_len = dataLen;
		eventp->body = wl_gas_malloc(gas->drv, eventp->body_len);
		if (eventp->body) {
			memcpy(eventp->body, data, eventp->body_len);
		}
	...
	case EVENT_RX_REQUEST:
	{
		...
		if (!is_valid_advertisement_protocol(&gasDecode->request.apie)) {
			...
			query_request_notification(gas,
				gasDecode->request.reqLen, gasDecode->request.req, dataLen, data);
	...
	bcm_decode_gas_t *gasDecode, int dataLen, uint8 *data)
{
	...
	if (gas->state < STATE_LAST && event < EVENT_LAST) {
		...
		/* delete gas if it was marked for deletion */
		if (gas->is_no_response && gas->isIncoming) {
			/*
			 * registerd callback for gas event has no response for
			 * this incoming query request, switch to next state is
			 * not required. release gas object, return to idle
			 * state.
			 */
			idle_reset(gas);
			bcm_gas_destroy(gas);
...
/* update gas instance as incoming, move the entry to correct list */
void
bcm_gas_update_gas_incoming_info(bcm_gas_t* gas, bool is_incoming)
{
	int retCode;

	ASSERT(is_entry_valid(gas));

	if (gas->isIncoming == is_incoming) {
		return;
	}

	/* add entry to new list, if that is success then remove from old list */

	/* change incoming type */
	gas->isIncoming = is_incoming;
	if (add_entry(gas)) {
		/* Success, now remove it from old list, temp switch is_incoming to do this */
		gas->isIncoming = !is_incoming;
		retCode = del_entry(gas);
		ASSERT(retCode);
		BCM_REFERENCE(retCode);
		gas->isIncoming = is_incoming;
	} else {
		gas->isIncoming = !is_incoming;
	}
