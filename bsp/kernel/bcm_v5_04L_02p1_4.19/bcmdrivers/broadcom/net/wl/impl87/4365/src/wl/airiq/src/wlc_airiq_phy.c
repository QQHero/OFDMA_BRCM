 ...
 *
 * $Id: wlc_airiq_phy.c 766110 2018-07-24 22:40:18Z $
 *
 */
...
{
	wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_STATUS_ADDR, 1, 0);
}
...
wlc_airiq_phy_init(airiq_info_t *airiqh)
{
	...
	case 65:
		...
		} else if (IS_WAVE2_2G(airiqh->wlc->deviceid)) {
	...
	uint8 restore, rxgain_t rxgain[], uint8 lnarout, uint8 elna)
{
	...
	if (restore == 1) {
		...
		if (phy_get_phymode(pi) == PHYMODE_3x3_1x1) {
			if (CHSPEC_IS5G(pi->radio_chanspec_sc)) {
		...
		if (phy_get_phymode(pi) == PHYMODE_3x3_1x1) {
			is5g = CHSPEC_IS5G(pi->radio_chanspec_sc);
		} else {
	...
	uint16 fft_count)
{
	...
	if (airiqh->phy_mode == PHYMODE_3x3_1x1) {
		wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_CHANSPEC_ADDR, 1,
			pi->radio_chanspec_sc);
	} else {
		wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_CHANSPEC_ADDR, 1,
			pi->radio_chanspec);
	}
	wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_INTERVAL_ADDR, 1, fft_interval);
	wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_CORE_ADDR, 1, airiqh->core);
	wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_SEQ_NUM_ADDR, 1, 0);

	MOD_PHYREG(pi, RxFeCtrl1, swap_iq3, 0);

	wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_GAIN_ADDR, 1, gaindb);
	wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_CHANSPEC_3X3_ADDR, 1, pi->radio_chanspec);
	wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_CTRL_ADDR, 1, fft_count);
	/* Reinitialize VASIP FFT handshake */
	wlc_airiq_phy_clear_svmp_status(airiqh);
	/* Check if AIRIQ is offloaded */
	wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_ENABLE_ADDR, 1,
...
wlc_airiq_phy_disable_fft_capture(airiq_info_t *airiqh)
{
	...
	wlc_svmp_mem_read_axi(airiqh->wlc->hw, &svmp_enable, SVMP_SMPL_ENABLE_ADDR, 1);
	if (svmp_enable) {
		pi = (phy_info_t*)WLC_PI(wlc);
		wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_CTRL_ADDR, 1, 0);
		wlc_svmp_mem_set_axi(airiqh->wlc->hw, SVMP_SMPL_ENABLE_ADDR, 1, 0);
