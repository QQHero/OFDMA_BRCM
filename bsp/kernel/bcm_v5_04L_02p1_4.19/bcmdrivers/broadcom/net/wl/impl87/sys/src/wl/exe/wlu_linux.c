 ...
 *
 * $Id: wlu_linux.c 810606 2022-04-11 15:35:29Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <unistd.h>
...
#define INTERACTIVE_NUM_ARGS		150
#define INTERACTIVE_MAX_INPUT_LENGTH	8192
#define NO_ERROR						0
...
wl_find(struct ifreq *ifr)
{
	...
	while (fgets(buf, sizeof(buf), fp)) {
		 ...
		 * wlanX is Android
		 */
		if (!strncmp(name, "wl", 2) && strstr(name, ".")) {
			continue;
		}
...
static int
ioctl_queryinformation_fe(void *wl, int cmd, void* input_buf, uint32 *input_len)
{
	int error = NO_ERROR;
...
static int
ioctl_setinformation_fe(void *wl, int cmd, void* buf, uint32 *input_len)
{
	int error = 0;
...
int wl_ir_init_rwl(void **irh)
{
	...
	case REMOTE_WIFI: {
		struct ifreq *ifr;
		ifr = (struct ifreq *)CALLOC(sizeof(*ifr));
		if (ifr != NULL) {
			wl_find(ifr);
		}
		*irh = ifr;
...
process_args(struct ifreq* ifr, char **argv)
{
	...
	if (help) {
		...
		fprintf(stderr, "type -h <optional command> for help\n");
