 ...
 *
 * $Id: sms4.c 680841 2017-01-23 22:28:09Z $
 */
...
/* FIXME: should use md32_common.h or other platform-optimized versions of
 * ROTATE, but need to verify that they're actually faster.  The ARM compiler
 * is known to generate a single rotate instruction for this expression!
 */
#define ROTATE(x, n)     (((x) << (n)) | ((x) >> (32 - (n))))

static void
sms4_print_bytes(const char *name, const uchar *data, int len)
{
}
 ...
 */
void
BCMRAMFN(sms4_key_exp)(uint32 *MK, uint32 *RK)
{
	int i;
	uint32 K[36];
	...
	uint8 *ptxt)
{
	...
	sms4_print_bytes("MIC Key", (const uchar *)ick, SMS4_WPI_CBC_MAC_LEN);
	sms4_print_bytes("PN ", (const uchar *)pn, SMS4_WPI_PN_LEN);
	sms4_print_bytes("MIC data: PART1", (const uchar *)aad, aad_len);
	sms4_print_bytes("MIC data: PART 2", (const uchar *)ptxt, data_len);
	...
	for (k = 0; k < SMS4_BLOCK_WORDS; k++)
		if ((uintptr)ick & 3)
			...
			X[k] = ntoh32(*(const uint32 *)(ick + (SMS4_WORD_SZ * k)));
	sms4_key_exp(X, RK);
	...
	for (k = 0; k < SMS4_BLOCK_WORDS; k++)
		if ((uintptr)pn & 3)
			...
			X[k] = ntoh32(*(const uint32 *)(pn + (SMS4_WORD_SZ * k)));
	sms4_enc(Y, X, RK);
	...
	for (j = 0; j < aad_len/SMS4_BLOCK_SZ; j++) {
		for (k = 0; k < SMS4_BLOCK_WORDS; k++)
			if ((uintptr)aad & 3)
				...
				X[k] = Y[k] ^ ntoh32(*(const uint32 *)(aad + (SMS4_WORD_SZ * k)));
		aad += SMS4_BLOCK_SZ;
		sms4_enc(Y, X, RK);
	}
	...
	if (rem_len) {
		memcpy(tmp, aad, rem_len);
		memset(tmp + rem_len, 0, SMS4_BLOCK_SZ - rem_len);
		for (k = 0; k < SMS4_BLOCK_WORDS; k++)
			if ((uintptr)tmp & 3)
...
/* FIXME: need to have only one cbc_mac function - consolidate
 * sms4_wpi_cbc_mac() and sms4_cbc_mac(), maybe by passing in RK instead of ick
 * here, and having sms4_wpi_cbc_mac call this function
 */
static int
	...
	uint8 *ptxt)
{
	...
	sms4_print_bytes("ENC Key", (const uchar *)ek, SMS4_WPI_CBC_MAC_LEN);
	sms4_print_bytes("PN ", (const uint8 *)pn, SMS4_WPI_PN_LEN);
	sms4_print_bytes("data", (const uchar *)ptxt, data_len);
	...
	for (k = 0; k < SMS4_BLOCK_WORDS; k++)
		if ((uintptr)ek & 3)
			...
			X[k] = ntoh32(*(const uint32 *)(ek + (SMS4_WORD_SZ * k)));
	sms4_key_exp(X, RK);
                          ...
                          uint8 *p)
{
	...
	uint16 fc, seq;
	uint k, header_len, aad_len, qos_len = 0, hdr_add_len = 0;
	bool wds = FALSE;
	uint8 tmp[SMS4_BLOCK_SZ];

	memset(aad, 0, SMS4_WPI_MAX_AAD_LEN);
	...
	*paad++ = (uint8)((fc >> 8) & 0xff);
	...
	memcpy(paad, (uchar *)&h->a1, 2*ETHER_ADDR_LEN);
	...
	*paad++ = (uint8)((seq >> 8) & 0xff);
	...
	memcpy(paad, (uchar *)&h->a3, ETHER_ADDR_LEN);
	paad += ETHER_ADDR_LEN;

	if (wds) {
		memcpy(paad, (uchar *)&h->a4, ETHER_ADDR_LEN);
	}
                          ...
                          uint8 *p)
{
	...
	*paad++ = (data_len - header_len - SMS4_WPI_CBC_MAC_LEN) & 0xff;
	...
	/* store MAC */
	memcpy(MAC, p + data_len - SMS4_WPI_CBC_MAC_LEN, SMS4_WPI_CBC_MAC_LEN);
	...
	/* compare MAC */
	if (memcmp(p + data_len - SMS4_WPI_CBC_MAC_LEN, MAC, SMS4_WPI_CBC_MAC_LEN))
		return (SMS4_WPI_CBC_MAC_ERROR);
...
sms4_test_cbc_mac()
{
	...
	for (k = 0; k < NUM_SMS4_CBC_MAC_VECTORS; k++) {
		...
		if (memcmp(mac, sms4_cbc_mac_vec[k].ref, SMS4_WPI_CBC_MAC_LEN) != 0) {
...
sms4_test_ofb_crypt()
{
	...
	for (k = 0; k < NUM_SMS4_OFB_VECTORS; k++) {
		...
		if (memcmp(sms4_ofb_vec[k].input, sms4_ofb_vec[k].ref,
			sms4_ofb_vec[k].il) != 0) {
...
sms4_test_wpi_pkt_encrypt()
{
	...
	for (k = 0; k < NUM_SMS4_WPI_PKT_VECTORS; k++) {
		...
		if (memcmp(sms4_wpi_pkt_vec[k].input, sms4_wpi_pkt_vec[k].ref,
...
sms4_test_wpi_pkt_micfail()
{
	...
	for (k = 0; k < NUM_SMS4_WPI_PKT_VECTORS; k++) {
		...
		memcpy(pkt, sms4_wpi_pkt_vec[k].ref, sms4_wpi_pkt_vec[k].il);
