 ...
 *
 * $Id: wbd_blanket_utility.c 807282 2022-01-17 09:58:21Z $
 */
...
int
blanket_sta_assoc_failed_connection(struct ether_addr *mac, uint16 status, uint16 reason)
{
	return ieee1905_sta_assoc_failed_connection((unsigned char *)mac, status, reason);
}
#endif /* MULTIAPR2 */
...
/* Enable(BSS Up) or Disable(BSS Down) a BSS. Incase of Disable, disassociate all the STAs
 * associated to it
 */
static void
wbd_blanket_util_enable_disable_bss(i5_dm_bss_type *i5_bss, int enable)
{
	int ret, bssidx, isup, unit, subunit;
	WBD_ENTER();

	/* Get BSS index. If subunit is greater than zero then it's a virtual interface and that
	 * will be the BSS index else BSS index will be 0
	 */
	if (get_ifname_unit(i5_bss->ifname, &unit, &subunit) < 0) {
		WBD_WARNING("ifname %s Failed to get BSS index\n", i5_bss->ifname);
		goto end;
	}
	if (subunit > 0) {
		bssidx = subunit;
	...
	isup = blanket_is_bss_enabled(i5_bss->ifname, bssidx);

	if (enable) {
		if (isup) {
			WBD_INFO("ifname %s bssidx %d is already enabled\n",
				i5_bss->ifname, bssidx);
			return;
		}
		ret = blanket_bss_enable(i5_bss->ifname, bssidx);
		WBD_INFO("ifname %s bssidx %d enabled ret %d\n", i5_bss->ifname, bssidx, ret);
	} else {
		if (!isup) {
			WBD_INFO("ifname %s bssidx %d is already disabled\n",
				i5_bss->ifname, bssidx);
			return;
		}
		wbd_blanket_util_disconnect_all_stas_from_bss(i5_bss);
		ret = blanket_bss_disable(i5_bss->ifname, bssidx);
		WBD_INFO("ifname %s bssidx %d Disabled ret %d\n", i5_bss->ifname, bssidx, ret);
	}
...
/* Enable(BSS Up) or Disable(BSS Down) All fronthaul BSS's. Incase of Disable, disassociate
 * all the STAs associated to it
 */
void
wbd_blanket_util_enable_disable_all_fronthaul_bss(int enable)
{
	i5_dm_device_type *i5_device = i5DmGetSelfDevice();
	...
	/* Do not continue if the slave is configured to keep the BSS up */
	if (wbd_get_ginfo()->wbd_slave->flags & WBD_BKT_SLV_FLAGS_KEEP_FH_BSS_UP) {
		goto end;
	}
	...
	foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type, i5_device->interface_list) {
		...
		foreach_i5glist_item(i5_iter_bss, i5_dm_bss_type, i5_iter_ifr->bss_list) {
			...
			/* Only for Fronthaul BSS */
			if (!I5_IS_BSS_FRONTHAUL(i5_iter_bss->mapFlags)) {
				continue;
			}

			wbd_blanket_util_enable_disable_bss(i5_iter_bss, enable);
		}
	}
...
/* Disconnect all the STA's connected to the BSS */
void
wbd_blanket_util_disconnect_all_stas_from_bss(i5_dm_bss_type *i5_bss)
{
	int ret;
	i5_dm_clients_type *i5_iter_sta;
	WBD_ENTER();

	/* Loop for all the Clients in BSS */
	foreach_i5glist_item(i5_iter_sta, i5_dm_clients_type, i5_bss->client_list) {
		/* Deauth the STA from current BSS */
		ret = blanket_deauth_sta(i5_bss->ifname, (struct ether_addr*)i5_iter_sta->mac,
			DOT11_RC_DEAUTH_LEAVING);
		WBD_INFO("Ifname[%s] BSS["MACF"] Disassociate STA["MACF"]. ret %d\n",
			i5_bss->ifname, ETHERP_TO_MACF(i5_bss->BSSID),
			ETHERP_TO_MACF(i5_iter_sta->mac), ret);
	}
	WBD_EXIT();
}
...
/* Disable STA roam and VAP up. Mostly when some other connection is available, we need to disable
 * the BSTA roaming in other BSTAs and make the virtual BSS up by setting keep AP to 1
 */
void
wbd_blanket_util_disable_bh_sta_roam_and_vap_up(i5_dm_interface_type *i5_ifr)
{
	int ret = WBDE_OK;
	...
	WBD_INFO("Disable backhaul STA("MACF") roaming and up virtual APs even if the backhaul "
		"STA is not associated\n", ETHERP_TO_MACF(i5_ifr->InterfaceId));

	blanket_set_keep_ap_up(i5_ifr->ifname, 1); /* VAP UP */
	if (WBD_IS_HAPD_ENABLED(wbd_get_ginfo()->flags)) {
		/* If hostapd is enabled, roaming in firmware is disabled from wlconf. Actual
		 * raoming will be handled by wpa_supplicant. So here raoming in wpa_supplicant
		 * should be disabled
		 */
		snprintf(tmp, sizeof(tmp), "wpa_cli -p %s/%s_wpa_supplicant -i %s ap_scan 0",
			WBD_HAPD_SUPP_CTRL_DIR, i5_ifr->wlParentName, i5_ifr->ifname);
		system(tmp);
		WBD_INFO("%s\n", tmp);

		/* ROAM OFF IOVAR - disable firmware roaming which we set it to 2 for doing
		 * low RSSI roaming(in function wbd_slave_set_bh_sta_roam_state)
		 */
		blanket_set_roam_off(i5_ifr->ifname, 1);
		WBD_INFO("wl -i %s roam_off 1\n", i5_ifr->ifname);
	...
	i5_ifr->roam_enabled = FALSE;
	blanket_set_allow_bgdfs_on_apsta(i5_ifr->ifname, 1);
...
/* Enable STA roam and VAP follow. Mostly when the backhaul STA is disconnected we need to enable
 * roaming and set keep ap up to 0
 */
void
wbd_blanket_util_enable_bh_sta_roam_and_vap_follow(i5_dm_interface_type *i5_ifr)
{
	int ret = WBDE_OK;
	wbd_ifr_item_t *ifr_vndr_data;
	char tmp[256];
	...
	WBD_INFO("Enable backhaul STA("MACF") roaming and up virtual APs up only if the backhaul "
		"STA is associated\n", ETHERP_TO_MACF(i5_ifr->InterfaceId));

	blanket_set_keep_ap_up(i5_ifr->ifname, 0); /* VAP Follow */
	if (WBD_IS_HAPD_ENABLED(wbd_get_ginfo()->flags)) {
		snprintf(tmp, sizeof(tmp), "wpa_cli -p %s/%s_wpa_supplicant -i %s ap_scan 1",
			WBD_HAPD_SUPP_CTRL_DIR, i5_ifr->wlParentName, i5_ifr->ifname);
		system(tmp);
		WBD_INFO("%s\n", tmp);
		ifr_vndr_data->flags &= ~WBD_FLAGS_IFR_AP_SCAN_DISABLED;
		...
		if (!wbd_blanket_is_sta_associated(i5_ifr->ifname)) {
			snprintf(tmp, sizeof(tmp), "wpa_cli -p %s/%s_wpa_supplicant -i %s scan",
				WBD_HAPD_SUPP_CTRL_DIR, i5_ifr->wlParentName, i5_ifr->ifname);
			system(tmp);
			WBD_INFO("%s\n", tmp);
		}

		/* ROAM OFF IOVAR - enable low RSSI based roaming(which we set initially in
		 * function wbd_slave_set_bh_sta_roam_state
		 */
		blanket_set_roam_off(i5_ifr->ifname, 2);
		WBD_INFO("wl -i %s roam_off 2\n", i5_ifr->ifname);
	} else {
		/* ROM OFF IOVAR - enable roaming */
		blanket_set_roam_off(i5_ifr->ifname, 0);
	}
	i5_ifr->roam_enabled = TRUE;
	blanket_set_allow_bgdfs_on_apsta(i5_ifr->ifname, 0);
	...
	foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type,
		...
		switch (cmd) {
			case IEEE1905_BH_STA_ROAM_DISB_VAP_UP:
				...
				wbd_blanket_util_disable_bh_sta_roam_and_vap_up(i5_iter_ifr);
				break;
			case IEEE1905_BH_STA_ROAM_ENAB_VAP_FOLLOW:
				...
				wbd_blanket_util_enable_bh_sta_roam_and_vap_follow(i5_iter_ifr);
				break;
			default:
...
wbd_slave_disconnect_all_bstas(i5_dm_interface_type *i5_ifr)
{
	...
	foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type, i5_self_device->interface_list) {
		...
		if ((blanket_get_bssid(i5_iter_ifr->ifname, &bssid) != WBDE_OK) ||
			(ETHER_ISNULLADDR(&bssid))) {
			WBD_INFO("ifname[%s] is not associated\n", i5_iter_ifr->ifname);
			continue;
		}

		WBD_INFO("ifname : %s Disconnect backhaul STA ["MACF"] from BSSID ["MACF"]\n",
			i5_iter_ifr->ifname, ETHERP_TO_MACF(i5_iter_ifr->InterfaceId),
			ETHER_TO_MACF(bssid));
		blanket_deauth_sta(i5_iter_ifr->ifname, &bssid, DOT11_RC_DEAUTH_LEAVING);
	}
end:
	WBD_EXIT();
}

/* Update BSSID's of all the backhaul STAs. Return 1 if any of the BSTA associated */
int
wbd_blanket_util_update_bssids_of_all_bstas(i5_dm_device_type *i5_self_dev)
{
	int ret = 0;
	i5_dm_interface_type *i5_iter_ifr = NULL;
	WBD_ENTER();
	...
	foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type, i5_self_dev->interface_list) {
		...
		/* Get the BSSID */
		if (blanket_get_bssid(i5_iter_ifr->ifname, (struct ether_addr*)&i5_iter_ifr->bssid)
			== WBDE_OK) {
			if (!ETHER_ISNULLADDR(i5_iter_ifr->bssid)) {
				WBD_INFO("%s is associated to ["MACF"]\n", i5_iter_ifr->ifname,
					ETHERP_TO_MACF(i5_iter_ifr->bssid));
				ret = 1;
			}
		}
	}
...
/* Disable roaming and keep the AP up on all the disconnected BSTAs */
void
wbd_blanket_util_vap_up_on_disconnected_bstas(i5_dm_device_type *i5_self_dev)
{
	i5_dm_interface_type *i5_iter_ifr = NULL;
	WBD_ENTER();
	...
	foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type, i5_self_dev->interface_list) {
		...
		/* If the BSSID not NULL, that means it is connected. So skip */
		if (!ETHER_ISNULLADDR(i5_iter_ifr->bssid)) {
			continue;
		}
...
/* Set keep_ap_up on all BSTAs */
void
wbd_blanket_util_set_bsta_keep_ap_up(i5_dm_device_type *i5_self_dev, int keep_ap_up)
{
	i5_dm_interface_type *i5_iter_ifr = NULL;
	WBD_ENTER();
	...
	foreach_i5glist_item(i5_iter_ifr, i5_dm_interface_type, i5_self_dev->interface_list) {
		...
		blanket_set_keep_ap_up(i5_iter_ifr->ifname, keep_ap_up);
	}
