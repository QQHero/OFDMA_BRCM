 ...
 *
 * $Id: d11.h 799308 2021-05-26 12:59:24Z $
 */
...
#define GET_MACINTSTATUS_X(osh, regs)		R_REG((osh), &((regs)->altmacintstatus_x))
#define SET_MACINTSTATUS_X(osh, regs, val)	W_REG((osh), &((regs)->altmacintstatus_x), (val))
#define GET_MACINTMASK_X(osh, regs)		R_REG((osh), &((regs)->altmacintmask_x))
#define SET_MACINTMASK_X(osh, regs, val)	W_REG((osh), &((regs)->altmacintmask_x), (val))
#else
...
typedef volatile struct _d11regs {
	...
	uint16	PAD[2];			/* 0x518 - 0x51A */
	uint16	txe_status2;		/* 0x51C */
	uint16	PAD[1];			/* 0x51E */
	...
	union {
		...
		struct {
			...
			uint16  BytCntInTxFrmLo;	/* 0xa5a */
			uint16  BytCntInTxFrmHi;	/* 0xa5c */
			uint16	PAD[1];
...
#define AMT_SIZE(_corerev)   ((D11REV_GE(_corerev, 64)) ? AMT_SIZE_128 : AMT_SIZE_64)
...
/* XXX In order for the ucode to realize that the corresponding entry in AMT
 * is "monitored" STA entry the driver needs to write the AMT index into
 * this shm location (0x12 * 2).
 * NOTE: this shmem is currently defined in the driver for TX power control,
 * but it is not used by 11AC ucode.
 */
...
#define MHF4_RSDB_DFS_SCAN	0x8000		/* RSDB DFS SCAN: blank radar detection */
...
#define MXHF0_MUTXUFWAR		0x0200		/* MU aggregation optimization */
...
#define	RXHDR_LEN   36	/* sizeof d11rxhdr_t */
...
#define	WRXHDR_LEN		47	/* sizeof wlc_d11rxhdr_t */
...
#define RXS_SECKINDX_MASK(_corerev)   ((D11REV_GE(_corerev, 64)) ? \
	(0x1FE0) : (0x07E0))
...
#define	M_BOM_REV_MAJOR		(M_PSM_SOFT_REGS + 0x0)		/* bom/tag low */
#define	M_BOM_REV_MINOR		(M_PSM_SOFT_REGS + 0x1 * 2)	/* bom/tag high */
#define	M_REV_L			(M_PSM_SOFT_REGS + 0x2 * 2)	/* svn rev low */
#define	M_REV_H			(M_PSM_SOFT_REGS + 0x3 * 2)	/* svn rev high */
#define	M_UCODE_FEATURES	(M_PSM_SOFT_REGS + 0x5 * 2)	/* ucode feature flags */
#define M_SHM_BYT_CNT		(M_PSM_SOFT_REGS + 0xa * 2)
#define M_PAPDOFF_MCS		(M_PSM_SOFT_REGS + 0x12 * 2)	/* WAR disabling PAPD in mcs */
#define	M_UCODE_DBGST		(M_PSM_SOFT_REGS + 0x20 * 2)	/* ucode debug status code */
#define	M_UCODE_MACSTAT		(M_PSM_SOFT_REGS + 0x70 * 2)	/* macstat counters */
#define	M_UCODE_MACSTAT1_PTR	(M_PSM_SOFT_REGS + 0x1c * 2)	/* macstat1 counters, crev >= 40 */
#define M_BCMC_TIMEOUT		(M_PSM_SOFT_REGS + 0x13 * 2)	/* max wait time for BCMC in PM
								 * corerev >= 40 only
								 */
#define M_AGING_THRSH		(M_PSM_SOFT_REGS + 0x3e * 2)	/* max wait time for medium free
								 * before tx
								 */
#define	M_MBURST_SIZE		(M_PSM_SOFT_REGS + 0x40 * 2)	/* max frames in a frameburst */
#define	M_MBURST_TXOP		(M_PSM_SOFT_REGS + 0x41 * 2)	/* max frameburst TXOP in usec */
#define M_SYNTHPU_DLY		(M_PSM_SOFT_REGS + 0x4a * 2)	/* pre-wakeup time for synthpu,
								 * default: 500
								 */
#define	M_PRETBTT		(M_PSM_SOFT_REGS + 0x4b * 2)	/* wait-time before tbtt */
...
/* Bit definition 0~7 are only for RCMTA (corerev < 40) */
...
#define M_WLCX_BLK_PRE40		(0x38a)		/* dual radio coex */
/* bitmap for the feature */
#define M_WLCX_CONFIG_PRE40		((M_WLCX_BLK_PRE40 + 0xD) * 2)
/* duration(in us) for protection */
#define M_WLCX_GUARD_INTV_PRE40		((M_WLCX_BLK_PRE40 + 0xE) * 2)
/* GPIO to be used for 5357/43235 */
#define M_WLCX_GPIO_CONFIG_PRE40	((M_WLCX_BLK_PRE40 + 0xF) * 2)
...
#define M_USEQ_PWRUP_PTR        (M_PSM_SOFT_REGS_EXT + 0)   /* vco calcode cache addr for pm mode */
...
#define M_PHYPREEMPT_VAL        (M_PSM_SOFT_REGS_EXT + 0x8) /* Preemption phyreg value */
...
#define M_TXBCN_DUR             (M_PSM_SOFT_REGS_EXT + 0x38)
...
/* define PSMX interface for corerev >= 64 */
/* PSM SHM_X variable offsets */
#define	MX_PSM_SOFT_REGS	0x0
#define	MX_BOM_REV_MAJOR	(MX_PSM_SOFT_REGS +    0)
#define	MX_BOM_REV_MINOR	(MX_PSM_SOFT_REGS +  1*2)
#define	MX_UCODE_DATE		(MX_PSM_SOFT_REGS +  2*2)	/* 4:4:8 year:month:day format */
#define	MX_UCODE_TIME		(MX_PSM_SOFT_REGS +  3*2)	/* 5:6:5 hour:min:sec format */
#define	MX_UCODE_DBGST		(MX_PSM_SOFT_REGS +  6*2)	/* ucode debug status code */
#define	MX_WATCHDOG_8TU		(MX_PSM_SOFT_REGS +  7*2)
#define	MX_MACHW_VER		(MX_PSM_SOFT_REGS +  8*2)
#define	MX_PHYVER		(MX_PSM_SOFT_REGS +  9*2)
#define	MX_PHYTYPE		(MX_PSM_SOFT_REGS + 10*2)
#define	MX_HOST_FLAGS0		(MX_PSM_SOFT_REGS + 11*2)
#define	MX_HOST_FLAGS1		(MX_PSM_SOFT_REGS + 12*2)
#define	MX_HOST_FLAGS2		(MX_PSM_SOFT_REGS + 13*2)
#define	MX_UCX2R_FLAGS		(MX_PSM_SOFT_REGS + 14*2)
...
#define	MX_UTRACEX_EPTR		(MX_PSM_SOFT_REGS + 37*2)

/* ucode  knoise gain override shmem */
#define M_RXGAIN_HI (0x120+0x59)*2
#define M_RXGAIN_LO (0x120+0x5A)*2
