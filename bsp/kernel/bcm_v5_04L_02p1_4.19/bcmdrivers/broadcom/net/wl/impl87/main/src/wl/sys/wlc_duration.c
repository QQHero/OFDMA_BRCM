 ...
 *
 * $Id: wlc_duration.c 787875 2020-06-15 09:20:16Z $
 */
...
wlc_dump_duration_stats(duration_info_t *info, int index, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "%25.25s %8d  %5d.%03d %5d.%03d  %3u.%01d%%    %5d.%03d   %5d.%03d\n",
...
wlc_dump_duration(wlc_info_t *wlc, struct bcmstrbuf *b)
{
	if (wlc->dur_info) {
		...
		if (wlc->clk) {
			cur_time = R_REG(wlc->osh, D11_TSFTimerLow(wlc));
...
void
wlc_duration_enter(wlc_info_t *wlc, duration_enum idx)
{
	if ((wlc->clk) && (idx < DUR_LAST)) {
		duration_stats_t *stats = &wlc->dur_info->duration_table[idx];

		stats->event_count++;
		stats->event_start = R_REG(wlc->osh, D11_TSFTimerLow(wlc));
...
void
wlc_duration_exit(wlc_info_t *wlc, duration_enum idx)
{
	if ((wlc->clk) && (idx < DUR_LAST)) {
		duration_stats_t *stats = &wlc->dur_info->duration_table[idx];

		stats->event_end = R_REG(wlc->osh, D11_TSFTimerLow(wlc));
