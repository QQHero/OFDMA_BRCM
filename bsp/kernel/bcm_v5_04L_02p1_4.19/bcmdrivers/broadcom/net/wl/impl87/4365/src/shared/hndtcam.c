 ...
 *
 * $Id: hndtcam.c 708017 2017-06-29 14:11:45Z $
 */
...
BCMATTACHFN(hnd_tcam_load_default)(uint32 rambase)
{
	...
	if (((uint32)PATBL_TABLE < rambase) ||
		...
		(rambase + hnd_tcam_atcmsize((void *)armp)))
		OSL_SYS_HALT();
...
hnd_tcam_patchdisable(void)
{
	hnd_tcam_disablepatch((void *)sysmem_regs);
}
...
BCMATTACHFN(hnd_tcam_load_default)(uint32 rambase)
{
	...
	for (i = 0; i < n; i++, hdr++) {
		...
		tidx = PATBL_IDX(hdr->entry);
		pa = (uint32) PATBL_ESTART(hdr->addr);
		hnd_tcam_write((void *)srp, tidx, TCAM_WRDAT(pa, valid));
	}
	hnd_tcam_enablepatch((void *)srp);
	hnd_tcam_verify();
}
