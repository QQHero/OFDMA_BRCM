 ...
 * $Id: km_iovars.c 803784 2021-10-07 12:42:20Z $
 */
...
static const size_t km_iov_wsec_tlv_num_entries = ARRAYSIZE(km_iov_wsec_tlv_entries);
...
static const bcm_iovar_t km_iovars[] = {
	...
	{"brcmapivtwo", IOV_BRCMAPIVTW_OVERRIDE, 0, 0, IOVT_INT8, 0},
	{"wsec_info", IOV_WSEC_INFO, (IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, sizeof(wl_wsec_info_t)},
	{"wsec", IOV_WSEC, (IOVF_OPEN_ALLOW), 0, IOVT_UINT32, 0},
#ifdef MFP
	/* Use software encryption for unicast management frames */
	{"use_sw_uc_mfp", IOV_USE_SW_UC_MFP, (0), 0, IOVT_BOOL, 0},
#endif /* MFP */
	{NULL, 0, 0, 0, 0, 0}
};
	...
	void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_WSEC_KEY):
	{
		...
		KM_LOG(("wl%d: %s: key id %02x addr "MACF"\n",  KM_UNIT(km), __FUNCTION__,
			key_id,  ETHERP_TO_MACF(ea)));
		...
		if (WLC_MFP_ENAB(wlc->pub) && KM_VALID_MGMT_KEY_ID(key_id)) {
			...
			if (wl_key.len) {
				err = wlc_key_set_data(key, wl_key.algo,
		...
		if ((wl_key.algo == CRYPTO_ALGO_SMS4) && WLC_KEY_IS_PAIRWISE(&key_info)) {
			...
			/* nothing special if updating the primary key, otherwise need to rotate */
			scb_km = KM_SCB(km, scb);
			if (scb_km->key_idx != key_info.key_idx) {
				cur_key = wlc_keymgmt_get_key(km, scb_km->key_idx, NULL);
				err = km_key_wapi_rotate_key(cur_key, key, &key_info);
				if (err != BCME_OK)
					break;
				key = cur_key;
			}
		...
		if (scb != NULL) {
			int cnt = 0;
			if ((cnt = SCB_PKTS_TOT_INFLT_CQCNT_VAL(scb)) > 0) {
				wlc_scb_cq_flush(wlc, scb);
				WLCNTADD(wlc->pub->_cnt->txfail, cnt);
				WLCNTSCBADD(scb->scb_stats.tx_failures, cnt);
			}
			KM_ASSOC(("wl%d %s: "MACF" KM: Program new key, dropped %d\n", KM_UNIT(km),
				__FUNCTION__, ETHER_TO_MACF(scb->ea), cnt));
			scb->dropblock_dur = 0; /* Reset drop and block flag */
		}
...
#ifdef MFP
	case IOV_GVAL(IOV_USE_SW_UC_MFP):
	{
		...
		*ret_int_ptr = SW_MFP_ENAB(wlc->pub);
		break;
	}
	case IOV_SVAL(IOV_USE_SW_UC_MFP):
	{
...
wlc_keymgmt_wsec(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint32 val)
{
	...
	if (!WAPI_HW_WPI_CAP(wlc) && (val & SMS4_ENABLED))
...
km_iov_check_psta_replay(keymgmt_t *km, wlc_bsscfg_t *bsscfg)
{
	...
	scb_t *scb = NULL;
	struct scb_iter scbiter;
	pri_bsscfg = bsscfg->wlc->primary_bsscfg;
	if (pri_bsscfg == NULL) {
