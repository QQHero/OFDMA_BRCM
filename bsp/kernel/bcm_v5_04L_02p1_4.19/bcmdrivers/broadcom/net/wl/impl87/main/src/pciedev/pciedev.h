...
extern void pciedev_sync_dmaindex_h2d_rd(struct dngl_bus * pciedev, msgbuf_ring_t* ring);
...
typedef struct hwa_cfp_tx_info {
	...
	uint16 scb_flowid;	/* Packet list SCB Flow ID */
...
void pciedev_hwa_d2h_req_q_send(struct dngl_bus *pciedev);
#endif
...
#ifdef PCIE_D2H_DOORBELL_RINGER
void pciedev_d2h_txcpl_doorbell_toggle(struct dngl_bus *pciedev);
#define PCIEDEV_D2H_TXCPL_DOORBELL_TOGGLE(pciedev)	pciedev_d2h_txcpl_doorbell_toggle(pciedev)
#else
#define PCIEDEV_D2H_TXCPL_DOORBELL_TOGGLE(pciedev)	do { /* no-op */ } while (0)
#endif
