 ...
 *
 * $Id: wlc_ie_misc_hndlrs.c 810655 2022-04-12 06:08:44Z $
 */
...
wlc_bcn_calc_ssid_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	uint8 short_ssid_len = 0;
	uint32 cfg_short_ssid = 0;
	int i = 0;
	...
	if ((data->ft == FC_PROBE_RESP) && (cfg->closednet || cfg->nobcnssid)) {
		if (!data->cbparm->ft->bcn.ssid && !data->cbparm->ft->bcn.short_ssid) {
			/* prepare 6G bcast prb response template, bcn.ssid wont be set */
			return TLV_HDR_LEN;
		}

		ssid = data->cbparm->ft->bcn.ssid;
		short_ssid = data->cbparm->ft->bcn.short_ssid;
		short_ssid_len = data->cbparm->ft->bcn.short_ssid_len;
		cfg_short_ssid = ~hndcrc32(cfg->SSID, cfg->SSID_len, CRC32_INIT_VALUE);

		if (ssid) {
			if ((ssid->SSID_len == cfg->SSID_len) &&
				(bcmp(ssid->SSID, cfg->SSID, ssid->SSID_len) == 0)) {
				 /* cases:
				  * 1: response for prb rqst with ssid matching tx/non tx hidden
				  *    ssid.
				  * 2: prepare mbssid profile for non tx bss's ssid matching
				  *    rqst's ssid.
				  */
				return TLV_HDR_LEN + cfg->SSID_len;
			} else {
				if (!short_ssid || !cfg->wlc->check_short_ssid) {
					/* cases:
					 * 1: hidden ssid does not match with ssid in prb rqst.
					 * 2: prepare mbssid profile for non tx bss's ssid not
					 *    matching rqst's ssid.
					 */
					return TLV_HDR_LEN;
				}
			}
		}
		for (i = 0; i < short_ssid_len / sizeof(uint32); i++) {
			if (short_ssid[i] == cfg_short_ssid) {
				return TLV_HDR_LEN + cfg->SSID_len;
			}
		}
		return TLV_HDR_LEN;
...
wlc_bcn_write_ssid_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if ((data->ft == FC_BEACON) && (cfg->closednet || cfg->nobcnssid)) {
		if (BSSCFG_IS_EMPTY_SSID_SUPPORTED_ON_CLOSED_NW(cfg)) {
			bcm_write_tlv(DOT11_MNG_SSID_ID, NULL, 0, data->buf);
		} else {
			uint8 null_ssid[DOT11_MAX_SSID_LEN];

			bzero(null_ssid, cfg->SSID_len);
			bcm_write_tlv(DOT11_MNG_SSID_ID, null_ssid, cfg->SSID_len, data->buf);
		}
	} else if ((data->ft == FC_PROBE_RESP) && (cfg->closednet || cfg->nobcnssid)) {
		...
		if (ssid) {
			if ((ssid->SSID_len == cfg->SSID_len) &&
				(bcmp(ssid->SSID, cfg->SSID, ssid->SSID_len) == 0)) {
				bcm_write_tlv(DOT11_MNG_SSID_ID, cfg->SSID, cfg->SSID_len,
					data->buf);
				return BCME_OK;
			} else {
				if (!short_ssid || !cfg->wlc->check_short_ssid) {
...
/* SSID IE */
static uint
wlc_prs_calc_short_ssid_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	wlc_bsscfg_t *cfg = data->cfg;
	wlc_info_t *wlc = (wlc_info_t *)ctx;
	wlc_iem_ft_cbparm_t *ftcbparm = data->cbparm->ft;

	/* add only in UPR - when in closed/hidden network */
	if (!BAND_6G(wlc->band->bandtype) ||
			(ftcbparm->bcn.ssid || ftcbparm->bcn.short_ssid_len) ||
			!(cfg->closednet || cfg->nobcnssid)) {
		return 0;
	}

	return (SHORT_SSID_LIST_IE_FIXED_LEN + sizeof(cfg->short_ssid));
}

static int
wlc_prs_write_short_ssid_ie(void *ctx, wlc_iem_build_data_t *data)
{
	wlc_bsscfg_t *cfg = data->cfg;
	wlc_info_t *wlc = (wlc_info_t *)ctx;
	wlc_iem_ft_cbparm_t *ftcbparm = data->cbparm->ft;
	...
	short_ssid_ie->len = ((SHORT_SSID_LIST_IE_FIXED_LEN -
			...
			sizeof(cfg->short_ssid));
	short_ssid_ie->short_ssid[0] = cfg->short_ssid;
	return BCME_OK;
...
#endif
	if ((err = wlc_iem_add_build_fn_mft(iemi, FT2BMP(FC_PROBE_RESP), DOT11_MNG_SHORT_SSID_ID,
	      wlc_prs_calc_short_ssid_ie_len, wlc_prs_write_short_ssid_ie, wlc)) != BCME_OK) {
		WL_ERROR(("wl%d: %s wlc_iem_add_build_fn failed, err %d, short ssid ie\n",
		          wlc->pub->unit, __FUNCTION__, err));
		goto fail;
	}
	return BCME_OK;
