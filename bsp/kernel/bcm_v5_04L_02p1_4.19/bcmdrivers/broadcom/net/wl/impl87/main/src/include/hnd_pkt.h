 ...
 *
 * $Id: hnd_pkt.h 793616 2020-12-03 10:14:21Z $
 */
...
/* Transmit / suppress related */
#if defined(PROP_TXSTATUS)

#define PKTFRAGTXSTATUS(osh, lb)	((LBFP(lb)->flist.finfo[LB_FRAG_MAX].txstatus))
#define PKTFRAGSETTXSTATUS(osh, lb, txs) \
	({BCM_REFERENCE(osh); ((LBFP(lb)->flist.finfo[LB_FRAG_MAX].txstatus) = (txs));})

#define PKTFRAGPKTFLAGS(osh, lb) \
	(((LBFP(lb)->flist.finfo[LB_FRAG_MAX].pktflags & 0xF0) >> 4))
#define PKTFRAGRESETPKTFLAGS(osh, lb) \
	(((LBFP(lb)->flist.finfo[LB_FRAG_MAX].pktflags) &= 0x0F))
#define PKTFRAGSETPKTFLAGS(osh, lb, flags) \
	({BCM_REFERENCE(osh); ((LBFP(lb)->flist.finfo[LB_FRAG_MAX].pktflags) |= (flags << 4));})
...
#define PKTFRAGTXSTATUS(osh, lb)	((LBFP(lb)->flist.txstatus))
#define PKTFRAGSETTXSTATUS(osh, lb, txs) \
	({BCM_REFERENCE(osh); ((LBFP(lb)->flist.txstatus));})
