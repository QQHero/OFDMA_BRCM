 ...
 *
 * $Id: phy_ac_prephy.c 789524 2020-07-30 09:12:33Z $
 */
...
{
	int ret = BCME_OK;
	int phy_rev = pi->pubpi->phy_rev; /* for short hand */
	int phy_minor_rev;

	BCM_REFERENCE(phy_rev);
	ASSERT(pacaps);
	if (ACREV_LT(phy_rev, HECAP_FIRST_ACREV) || (ACMAJORREV_128(phy_rev))) {
		ret = BCME_UNSUPPORTED;
		goto done;
	}

	ASSERT(USE_HW_MINORVERSION(phy_rev));
#ifdef BCMPHYACMINORREV
	phy_minor_rev = BCMPHYACMINORREV;
#else /* BCMPHYACMINORREV */
	phy_minor_rev = READ_PHYREGFLD_PREPHY(pi, pi->regs, MinorVersion, minorversion);
	...
	*pacaps |= READ_PHYREGFLD_PREPHY(pi, pi->regs, PhyInternalCapability1, Support2GHz) ?
	...
	if ((ACMAJORREV_47(phy_rev) && phy_minor_rev >= 3) || /* >=43684c0 */
	    ACMAJORREV_51_129_130_131(phy_rev)) { /* 47622, 6710, 6715, 6756 */
		*pacaps |= PHY_PREATTACH_CAP_SUP_6G;
	}
