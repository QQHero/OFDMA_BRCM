 ...
 *
 * $Id: wlc_pdsvc.c 809675 2022-03-22 05:14:17Z $
 */
...
static const bcm_iovar_t  wlc_proxd_iovars[] = {
	...
	0, IOVT_BUFFER, sizeof(wl_proxd_params_iovar_ext_t)},
	...
	void *params, uint p_len, void *arg, uint a_len, uint val_size, struct wlc_if *wlcif)
{
	...
	{
		wl_proxd_params_iovar_ext_t *proxd_iov;

		proxd_iov = (wl_proxd_params_iovar_ext_t*)params;
		if (p_len < sizeof(wl_proxd_params_iovar_ext_t)) {
			return BCME_BUFTOOSHORT;
		}
		proxd_iov->version = WL_PROXD_IOVAR_VERSION_2;
		proxd_iov->method = method;

		if (pdsvc_info->ftm && pdsvc_info->tunep) {
			err = pdburst_get_tune(wlc, pdsvc_info->tunep,
				((uint8*) arg + OFFSETOF(wl_proxd_params_iovar_ext_t, u.tof_tune)),
				p_len - OFFSETOF(wl_proxd_params_iovar_ext_t, u.tof_tune));
		} else {
			err = BCME_NOTREADY;
		}
		break;
	...
	case IOV_SVAL(IOV_PROXD_TUNE):
	{
		...
		if (pdsvc_info->ftm && pdsvc_info->tunep) {
			err = pdburst_set_tune(pdsvc_info->tunep,
				((uint8*) params + OFFSETOF(wl_proxd_params_iovar_ext_t,
					u.tof_tune)),
				p_len - OFFSETOF(wl_proxd_params_iovar_ext_t, u.tof_tune));
		}
		else
...
wlc_proxd_AVB_clock_factor(wlc_pdsvc_info_t* pdsvc, uint8 shift, uint32 *ki, uint32 *kt)
{
	...
	ASSERT(pdsvc != NULL);
	if (EMBEDDED_2x2AX_CORE(pdsvc->wlc->pub->sih->chip)) {
		...
		/** AVB Clock = 242.2504 */
		factor = 135265;
	} else if (BCM6710_CHIP(pdsvc->wlc->pub->sih->chip)) {
		/* factor = 1000*2^15/242.2504 */
		factor = 135265;
	} else if (BCM6715_CHIP(pdsvc->wlc->pub->sih->chip)) {
		/* factor = 1000*2^15/363.375 */
		factor = 90177;
	} else {
		WL_ERROR(("%s: FTM clock factor missing for chip %x\n",
			__FUNCTION__, CHIPID(pdsvc->wlc->pub->sih->chip)));
		ASSERT(0);
	}
...
uint16 wlc_proxd_core_get(wlc_info_t *wlc)
{
	wlc_pdsvc_info_t* pdsvc;
	uint8 core_shift;
	uint16 phytxant;
	...
	pdsvc = wlc->pdsvc_info;
	core_shift = (pdsvc->tunep->core == 255) ? 0 : pdsvc->tunep->core;
	phytxant = (1 << core_shift);
...
bool wlc_is_proxd_supported(wlc_info_t *wlc)
{
	...
	if (wlc_hw->machwcap1 & MCAP1_AVB_MASK) {
		ret = TRUE;
	}
    ...
    const struct ether_addr *addr, wl_proxd_event_t *event, uint16 len)
{
	...
	if (!wlc->pub->up || !wlc->hw->clk || wlc->state == WLC_STATE_GOING_DOWN) {
		return;
	}
...
static wl_proxd_params_tof_tune_t *proxd_init_tune(wlc_info_t *wlc)
{
	...
	if (tunep) {
		tunep->version = WL_PROXD_TUNE_VERSION_4;
		tunep->N_log2[TOF_BW_20MHZ_INDEX_V2] = TOF_DEFAULT_THRESHOLD_LOG2_20M;
		tunep->N_log2[TOF_BW_40MHZ_INDEX_V2] = TOF_DEFAULT_THRESHOLD_LOG2_40M;
		tunep->N_log2[TOF_BW_80MHZ_INDEX_V2] = TOF_DEFAULT_THRESHOLD_LOG2_80M;
		tunep->N_log2[TOF_BW_160MHZ_INDEX_V2] = TOF_DEFAULT_THRESHOLD_LOG2_160M;
		tunep->N_log2[TOF_BW_SEQTX_INDEX_V2] = TOF_DEFAULT_TX_THRESHOLD_LOG2;
		tunep->N_log2[TOF_BW_SEQRX_INDEX_V2] = TOF_DEFAULT_RX_THRESHOLD_LOG2;
		tunep->N_scale[TOF_BW_20MHZ_INDEX_V2] = TOF_DEFAULT_THRESHOLD_SCALE_20M;
		tunep->N_scale[TOF_BW_40MHZ_INDEX_V2] = TOF_DEFAULT_THRESHOLD_SCALE_40M;
		tunep->N_scale[TOF_BW_80MHZ_INDEX_V2] = TOF_DEFAULT_THRESHOLD_SCALE_80M;
		tunep->N_scale[TOF_BW_160MHZ_INDEX_V2] = TOF_DEFAULT_THRESHOLD_SCALE_160M;
		tunep->N_scale[TOF_BW_SEQTX_INDEX_V2] = TOF_DEFAULT_TX_THRESHOLD_SCALE;
		tunep->N_scale[TOF_BW_SEQRX_INDEX_V2] = TOF_DEFAULT_RX_THRESHOLD_SCALE;
		tunep->ftm_cnt[TOF_BW_20MHZ_INDEX_V2] = TOF_DEFAULT_FTMCNT_20M;
		tunep->ftm_cnt[TOF_BW_40MHZ_INDEX_V2] = TOF_DEFAULT_FTMCNT_40M;
		tunep->ftm_cnt[TOF_BW_80MHZ_INDEX_V2] = TOF_DEFAULT_FTMCNT_80M;
		tunep->ftm_cnt[TOF_BW_160MHZ_INDEX_V2] = TOF_DEFAULT_FTMCNT_160M;
		tunep->ftm_cnt[TOF_BW_SEQTX_INDEX_V2] = TOF_DEFAULT_FTMCNT_SEQ;
		tunep->ftm_cnt[TOF_BW_SEQRX_INDEX_V2] = TOF_DEFAULT_FTMCNT_SEQ;
		...
		/* 160MHz */
		tunep->w_len[TOF_BW_160MHZ_INDEX_V2] = 64;
		tunep->w_offset[TOF_BW_160MHZ_INDEX_V2] = 12;
		/* 80MHz */
		tunep->w_len[TOF_BW_80MHZ_INDEX_V2] = 32;
		tunep->w_offset[TOF_BW_80MHZ_INDEX_V2] = 10;
		/* 40MHz */
		tunep->w_len[TOF_BW_40MHZ_INDEX_V2] = 16;
		tunep->w_offset[TOF_BW_40MHZ_INDEX_V2] = 8;
		/* 20MHz */
		tunep->w_len[TOF_BW_20MHZ_INDEX_V2] = 8;
		tunep->w_offset[TOF_BW_20MHZ_INDEX_V2] = 4;
	...
	chanspec_t cspec, wl_proxd_params_tof_tune_t **tunepp)
{
	...
	if (flags & WL_PROXD_SESSION_FLAG_SEQ_EN) {
		if (flags & WL_PROXD_SESSION_FLAG_INITIATOR)
			idx = TOF_BW_SEQRX_INDEX_V2;
		else
			idx = TOF_BW_SEQTX_INDEX_V2;

	} else if (CHSPEC_IS160(cspec))
		idx = TOF_BW_160MHZ_INDEX_V2;
	else if (CHSPEC_IS80(cspec))
		idx = TOF_BW_80MHZ_INDEX_V2;
	else if (CHSPEC_IS40(cspec))
		idx = TOF_BW_40MHZ_INDEX_V2;
	else
		idx = TOF_BW_20MHZ_INDEX_V2;
...
proxd_update_tunep_values(wl_proxd_params_tof_tune_t *tunep, chanspec_t cspec, bool vhtack)
{
	...
	if (!(tunep->setflags & WL_PROXD_SETFLAG_N))
	{
		...
		if (CHSPEC_IS160(cspec)) {
			if (vhtack) {
				tunep->N_log2[TOF_BW_160MHZ_INDEX_V2] =
					TOF_DEFAULT_THRESHOLD_LOG2_160M;
			} else {
				tunep->N_log2[TOF_BW_160MHZ_INDEX_V2] =
					TOF_LEGACY_THRESHOLD_LOG2_160M;
			}
		} else if (CHSPEC_IS80(cspec)) {
			if (vhtack) {
				tunep->N_log2[TOF_BW_80MHZ_INDEX_V2] =
					TOF_DEFAULT_THRESHOLD_LOG2_80M;
			} else {
				tunep->N_log2[TOF_BW_80MHZ_INDEX_V2] =
					TOF_LEGACY_THRESHOLD_LOG2_80M;
			}
		} else if (CHSPEC_IS40(cspec)) {
			if (vhtack) {
				tunep->N_log2[TOF_BW_40MHZ_INDEX_V2] =
					TOF_DEFAULT_THRESHOLD_LOG2_40M;
			} else {
				tunep->N_log2[TOF_BW_40MHZ_INDEX_V2] =
					TOF_LEGACY_THRESHOLD_LOG2_40M;
			}
		}
	}
	if (!(tunep->setflags & WL_PROXD_SETFLAG_S))
	{
		...
		if (CHSPEC_IS160(cspec)) {
			if (vhtack) {
				tunep->N_scale[TOF_BW_160MHZ_INDEX_V2] =
					TOF_DEFAULT_THRESHOLD_SCALE_160M;
			} else {
				tunep->N_scale[TOF_BW_160MHZ_INDEX_V2] =
					TOF_LEGACY_THRESHOLD_SCALE_160M;
			}
		} else if (CHSPEC_IS80(cspec)) {
			if (vhtack) {
				tunep->N_scale[TOF_BW_80MHZ_INDEX_V2] =
					TOF_DEFAULT_THRESHOLD_SCALE_80M;
			} else {
				tunep->N_scale[TOF_BW_80MHZ_INDEX_V2] =
					TOF_LEGACY_THRESHOLD_SCALE_80M;
			}
		} else if (CHSPEC_IS40(cspec)) {
			if (vhtack) {
				tunep->N_scale[TOF_BW_40MHZ_INDEX_V2] =
					TOF_DEFAULT_THRESHOLD_SCALE_40M;
			} else {
				tunep->N_scale[TOF_BW_40MHZ_INDEX_V2] =
					TOF_LEGACY_THRESHOLD_SCALE_40M;
			}
		}
	}
}
