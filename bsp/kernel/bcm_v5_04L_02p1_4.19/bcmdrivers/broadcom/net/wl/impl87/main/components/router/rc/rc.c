 ...
 *
 * $Id: rc.c 798255 2021-04-28 07:06:03Z $
 */
...
wbd_restore_defaults(void)
{
	...
	char *list_names = NULL, iter_name[15], *next = NULL;
	...
	char* wbd_nvrams[] = {
		...
		"wbd_bh_opt_try_on_weak",
		"wbd_mchan",
		"wbd_smart_conv_rcpi",
		"wbd_valid_rssi_range",
		"wbd_crossband_rssi_est",
		"wbd_enab_single_chan_bh",
		"wbd_chan_util_thld_2g",
		"wbd_chan_util_thld_5g",
		"wbd_chan_util_thld_6g",
		"wbd_master_ip",
		"wbd_fbt",
		"wbd_master_flags",
		"wbd_slave_flags",
		...
		"wbd_tm_onboot_chscan_req",
		"wbd_tm_process_ap_configure",
		"wbd_tm_do_onboot_scan",
		"wbd_tm_probe",
		...
		"map_ts_iptables",
		"map_ts_add_fh_rule",
		"map_agent_active_bh_type",
		"map_default_5GL_chanspec",
		"map_default_5GH_chanspec",
		"map_2g_bsta_bs_rssi_thld",
		"map_bhsta_ifnames",
		"map_bringdown_controller",
		/* NVRAMs for Channel Scan Feature Configuration */
		"map_chscan_indep_scan",
		"map_chscan_scantype_flags",
		"map_chscan_impact",
		"map_chscan_min_scan_int",
		/* Misc NVRAMS */
		"bcn_req_sub_ele",
		"wps_custom_ifnames",
		"wps_map_timeout"
	...
	/* Erase map_ts_vlans and its NVRAMs */
	list_names = nvram_safe_get("map_ts_vlans");
	foreach(iter_name, list_names, next) {
		snprintf(tmp, sizeof(tmp), "map_ts_%s_ssid", iter_name);
		nvram_unset(tmp);
	}

	/* Erase map_bss_names and its NVRAMs */
	list_names = nvram_safe_get("map_bss_names");
	foreach(iter_name, list_names, next) {
		for (i = 0; i < ARRAYSIZE(map_bss_nvrams); i++) {
			snprintf(tmp, sizeof(tmp), "%s_%s", iter_name, map_bss_nvrams[i]);
			nvram_unset(tmp);
		}
	}
...
#endif /* __CONFIG_WBD__ */
#ifdef WLHOSTFBT
		fbt_restore_defaults(NULL, TRUE);
#endif /* WLHOSTFBT */
#ifdef __CONFIG_WPS__
