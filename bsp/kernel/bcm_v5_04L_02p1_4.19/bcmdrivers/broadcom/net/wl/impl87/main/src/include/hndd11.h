 ...
 *
 * $Id: hndd11.h 788427 2020-07-01 03:38:24Z $
 */
...
BWL_PRE_PACKED_STRUCT struct wlc_d11rxhdr {
	...
	uint8	radio_unit;	/**< WLAN radio unit */
	uint8	pad;
 ...
 *
 *  For MAC rev >= 129, PhyRxStatus is sent to a seperate PhyRx Status buffer asynchronous to
 *  ucode status.
 *  WLAN subsystem will process one Rx packet at a time and if Rx packet has a valid PhyRx Status
 *  then WLAN will set the hndd11_phyrxs_cur_status[] to corresponding PhyRx Status buffer.
 * ------------------------------------------------------------------------------------------------
 */

#if defined(DONGLEBUILD)
#define HNDD11_RADIO_UNIT_MAX		(1)
#else /* ! DONGLEBUILD */
#define HNDD11_RADIO_UNIT_MAX		(4)
#endif /* ! DONGLEBUILD */

#define HNDD11_RADIO_UNIT_INVALID	(-1)

extern d11phyrxsts_t *hndd11_phyrxs_cur_status[HNDD11_RADIO_UNIT_MAX];

/** Accessor MACROS to set/reset current PhyRx Status */
#define HNDD11_PHYRXSTS_GET_CUR_STATUS(_unit)			\
	hndd11_phyrxs_cur_status[(_unit)]

#define HNDD11_PHYRXSTS_SET_CUR_STATUS(_unit, d11phyrxsts)	\
	hndd11_phyrxs_cur_status[(_unit)] = (d11phyrxsts)
...
({												\
	...
	ASSERT(_wrxh->radio_unit < HNDD11_RADIO_UNIT_MAX);					\
						\
	ASSERT(D11RXHDR_GE129_SEQNUM(rxh) ==							\
		D11PHYRXSTS_SEQNUM(hndd11_phyrxs_cur_status[_wrxh->radio_unit]));		\
	(&((hndd11_phyrxs_cur_status[_wrxh->radio_unit])->member));				\
})
