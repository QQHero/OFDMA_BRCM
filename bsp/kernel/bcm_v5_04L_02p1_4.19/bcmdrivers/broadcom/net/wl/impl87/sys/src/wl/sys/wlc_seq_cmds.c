 ...
 *
 * $Id: wlc_seq_cmds.c 802692 2021-09-03 10:19:13Z $
 *
 */
...
)
{
	...
	switch (actionid) {
		...
		case IOV_SVAL(IOV_SEQ_CMDS_LIST):
		{
			...
			* commands internally maintain a 32-bit aligned pattern,
			* that pattern (i.e. the very first command) always starts on a
			* 32-bit boundary aligned by the top dispatcher.
			*/
...
run_concat_cmds(wlc_seq_cmds_info_t *info, const uint8 *buf, int buflen)
{
	...
	/* IOCTL struct should be 32-bit aligned by the top level dispatcher. */
	...
	while (end_buf > buf) {
		...
		/* Invoke user registered wl_ioctl callback. */
		rc = info->ioctl_callback(info->wlc, seq_ioc->cmd, ioctl_buf,
			seq_ioc->len, info->cmd_list.head->wlcif);
		if (BCME_OK != rc)
			break;

		/* Skip to next IOCTL in contiguous buffer. No need to
		 * skip padding since IOCTL struct should be 32-bit aligned by
