 ...
 *
 * $Id: rte_pmu.c 797638 2021-04-12 09:21:53Z $
 */
...
BCMATTACHFN(hnd_pmu_init)(si_t *sih)
{
	...
	if (hnd_sih->pmurev >= 130) {
		/* XXX From PMU PG, alp = 0x80000000 | (2^26/ALP_CLK_IN_MHz)
		 * for PMU slowclkperiod (0x6DC) register
		 */
		alp = si_pmu_alp_clock(sih, osh) / 1000000; /* in MHz unit */
		alp = ((1 << 26) / alp) & 0x00FFFFFF;
		alp = 0x80000000 | alp; /* enable bit[31]: OneMHzToggleEn */

		/* Enable Precision Usec Timer */
		OR_REG(hnd_osh, &hnd_pmur->PrecisionTmrCtrlStatus, PMU_PREC_USEC_TIMER_ENABLE);
	} else if (hnd_sih->pmurev >= 34) {
...
hnd_pmu_get_tick(void)
{
	...
	ticks1 = R_REG(hnd_osh, PMUREGADDR(hnd_sih, hnd_pmur, hnd_ccr, pmutimer));
	ticks2 = R_REG(hnd_osh, PMUREGADDR(hnd_sih, hnd_pmur, hnd_ccr, pmutimer));
	loop_count = 5;
	while (loop_count) {
		ticks3 = R_REG(hnd_osh, PMUREGADDR(hnd_sih, hnd_pmur, hnd_ccr, pmutimer));
