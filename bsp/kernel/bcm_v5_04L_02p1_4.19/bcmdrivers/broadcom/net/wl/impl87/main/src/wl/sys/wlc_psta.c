 ...
 *
 * $Id: wlc_psta.c 791680 2020-10-07 09:04:42Z $
 */
...
static const bcm_iovar_t psta_iovars[] = {
	{"psta", IOV_PSTA, (IOVF_SET_DOWN), 0, IOVT_INT8, 0 },
	{"psta_if", IOV_IS_PSTA_IF, (IOVF_GET_UP), 0, IOVT_BOOL, 0 },
	{"psta_inact", IOV_PSTA_INACT, (0), 0, IOVT_UINT32, 0 },
	{"psta_allow_join", IOV_PSTA_ALLOW_JOIN, (0), 0, IOVT_UINT32, 0 },
	{"psta_mrpt", IOV_PSTA_MRPT, (IOVF_SET_DOWN), 0, IOVT_BOOL, 0 },
	{NULL, 0, 0, 0, 0, 0 }
};
...
static int32 wlc_psta_init(wlc_psta_info_t *psta, wlc_bsscfg_t *pcfg);
static void wlc_psta_watchdog(void *arg);
static int wlc_psta_doiovar(void *hdl, uint32 actionid,
...
static struct scb *wlc_psta_client_is_ds_sta(wlc_psta_info_t *psta, const struct ether_addr *mac);
...
static int wlc_psta_wlc_up(void *ctx);
static int wlc_psta_wlc_down(void *ctx);
...
static void wlc_psta_scb_state_upd(void *ctx, scb_state_upd_data_t *data);
static void wlc_psta_bss_state_upd(void *ctx, bsscfg_state_upd_data_t *evt);
static void wlc_psta_assoc_state_upd(void *ctx, bss_assoc_state_data_t *notif);
static void wlc_psta_disassoc_notif(void *ctx, bss_disassoc_notif_data_t *notif);
...
BCMATTACHFN(wlc_psta_attach)(wlc_info_t *wlc)
{
	...
	if (!psta) {
		WL_ERROR((WLC_MALLOC_ERR, wlc->pub->unit, __FUNCTION__,
			(int)sizeof(wlc_psta_info_t), MALLOCED(wlc->pub->osh)));
		return NULL;
	}
	...
	if (wlc_scb_state_upd_register(wlc, wlc_psta_scb_state_upd, (void*)psta) != BCME_OK) {
		WL_ERROR(("wl%d: %s wlc_scb_state_upd_register failed\n",
			wlc->pub->unit, __FUNCTION__));
		goto fail;
	}

	if (wlc_bsscfg_state_upd_register(wlc, wlc_psta_bss_state_upd, psta) != BCME_OK) {
	...
	if (wlc_bss_assoc_state_register(wlc, wlc_psta_assoc_state_upd, psta) != BCME_OK) {
	...
	if (wlc_bss_disassoc_notif_register(wlc, wlc_psta_disassoc_notif, psta) != BCME_OK) {
...
wlc_psta_wlc_up(void *context)
{
	...
	if (PSTA_ENAB(psta->pub)) {
		...
		wlc_psta_init(psta, wlc->primary_bsscfg);
		wlc_mhf(wlc, MHF4, MHF4_PROXY_STA, MHF4_PROXY_STA, WLC_BAND_AUTO);
	} else {
		/* Disable PSTA */
		wlc_mhf(wlc, MHF4, MHF4_PROXY_STA, 0, WLC_BAND_AUTO);
	}
...
wlc_psta_mode_enable(wlc_psta_info_t *psta, bool enable)
{
	...
	if (enable == TRUE) {
		/* Initialize and enable PSTA */
		wlc_psta_init(psta, psta->wlc->primary_bsscfg);
		/* Enable PSTA in ucode */
		wlc_mhf(psta->wlc, MHF4, MHF4_PROXY_STA, MHF4_PROXY_STA, WLC_BAND_AUTO);
	} else {
		wlc_psta_deinit(psta, psta->wlc->primary_bsscfg);
...
BCMATTACHFN(wlc_psta_detach)(wlc_psta_info_t *psta)
{
	...
	(void)wlc_bss_disassoc_notif_unregister(wlc, wlc_psta_disassoc_notif, psta);
	(void)wlc_bss_assoc_state_unregister(wlc, wlc_psta_assoc_state_upd, psta);
	(void)wlc_bsscfg_state_upd_unregister(wlc, wlc_psta_bss_state_upd, psta);

	/* Clear scb state update */
	wlc_scb_state_upd_unregister(wlc, wlc_psta_scb_state_upd, (void*) psta);
...
static void
wlc_psta_deauth_client(wlc_psta_info_t *psta, const struct ether_addr *addr)
{
	wlc_bsscfg_t *cfg;
	struct scb *scb;
	...
	(void)wlc_senddeauth(psta->wlc, cfg, scb, &cfg->BSSID, &cfg->BSSID,
	                     &cfg->cur_etheraddr, DOT11_RC_UNSPECIFIED, TRUE);
...
wlc_psta_mode_save(wlc_psta_info_t *psta, wlc_bsscfg_t *cfg, struct ether_addr *addr)
{
	...
	if (PSTA_ENAB(wlc->pub) && (BSSCFG_IS_PRIMARY(cfg))) {
		scb = addr ? wlc_scbfind(wlc, cfg, addr) : NULL;
		if ((scb != NULL) && SCB_DWDS_CAP(scb) && DWDS_ENAB(cfg)) {
...
{
	wlc_bsscfg_t *primary_bss = psta->wlc->primary_bsscfg;
...
wlc_psta_watchdog(void *arg)
{
	...
	FOREACH_PSTA(psta->wlc, idx, cfg) {
	 ...
	 /* for Ralink AP deauth idle session issue, primary send keep alive null data */
	cfg = psta->wlc->primary_bsscfg;
	if ((psta->wlc->pub->now & 31) == 0) {
		wlc_sendnulldata(psta->wlc, cfg, &cfg->current_bss->BSSID, 0, 0,
...
static int
wlc_psta_doiovar(void *hdl, uint32 actionid,
...
wlc_psta_disable(wlc_psta_info_t *psta, wlc_bsscfg_t *cfg)
{
	...
	/* Enable Roaming */
	cfg->roam->state = WLC_ROAM_STATE_ENABLED;
                ...
                wlc_bsscfg_t *pcfg)
{
	...
	WL_PSTA(("wl%d: Checking PSTA %s: assoc count is %d/%d, PSTA cfg %p\n",
		...
		PSTA_BSS_COUNT(wlc), PSTA_MAX_ASSOC(wlc), OSL_OBFUSCATE_BUF(cfg)));

	if (cfg == NULL) {
		wlc_bsscfg_type_t type = {BSSCFG_TYPE_PSTA, BSSCFG_SUBTYPE_NONE};
		...
		/* Allocate bsscfg */
		cfg = wlc_bsscfg_alloc(wlc, idx, &type, 0, 0, ea);
		if (cfg == NULL)
			return BCME_NOMEM;
	...
	/* Disable Roaming */
	cfg->roam->state = WLC_ROAM_STATE_DISABLED;
	...
	assoc_params.chanspec_list[0] = CH20MHZ_CHSPEC(wf_chspec_primary20_chan(chanspec),
...
wlc_psta_pkt_alloc_copy(wlc_pub_t *pub, void *p)
{
	...
	if ((n = PKTGET(pub->osh, wlc->txhroff + totlen, TRUE)) == NULL) {
		WL_ERROR(("wl%d: %s: PKTGET of length %d failed\n",
		          pub->unit, __FUNCTION__, (wlc->txhroff + PKTLEN(pub->osh, p))));
		return NULL;
	}
	PKTPULL(pub->osh, n, wlc->txhroff);
                    ...
                    wlc_bsscfg_t **cfg, uint8 **shost, bool tx)
{
	...
	if (BCMPCIEDEV_ENAB() &&
		 ...
		 * the complete payload in the dongle. So, we should skip all payload
		 * processing here.
	...
	if (tx) {
		...
		if (bcmc && (!skip_proto_proc)) {
			...
			if ((n = PKTGET(psta->pub->osh, wlc->txhroff + pull, TRUE)) == NULL) {
				WL_ERROR(("wl%d: %s: PKTGET headroom %d failed\n",
				          psta->pub->unit, __FUNCTION__, wlc->txhroff));
				WLCNTINCR(psta->pub->_cnt->txnobuf);
				return BCME_NOMEM;
			}
			PKTPULL(psta->pub->osh, n, wlc->txhroff);
...
static struct scb *
wlc_psta_client_is_ds_sta(wlc_psta_info_t *psta, const struct ether_addr *mac)
{
	uint32 idx;
	wlc_bsscfg_t *cfg;
	struct scb *scb = NULL;
...
wlc_psta_disassoc_all(wlc_psta_info_t *psta)
{
	...
	FOREACH_PSTA(psta->wlc, idx, cfg) {
		...
		if (1) { /* Set to zero to disable roam re-trigger */
			...
			if (!spsta) {
				spsta = MALLOCZ(psta->wlc->pub->osh, sizeof(*spsta));
				if (spsta) {
					...
					WL_ERROR((WLC_MALLOC_ERR, WLCWLUNIT(psta->wlc),
						__FUNCTION__, (int)sizeof(*spsta),
						MALLOCED(psta->wlc->pub->osh)));
				}
			}
		}
cleanup:
...
wlc_psta_dump(wlc_psta_info_t *psta, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "PSTA BSS %s is %sconnected, primary is %sassociated, "
		...
		bcm_ether_ntoa(&psta->wlc->primary_bsscfg->cur_etheraddr, eabuf),
		wlc_bss_connected(psta->wlc->primary_bsscfg) ? "":"NOT ",
...
static void
wlc_psta_scb_state_upd(void *ctx, scb_state_upd_data_t *data)
{
	wlc_psta_info_t *psta = (wlc_psta_info_t *)ctx;
...
static void
wlc_psta_bss_state_upd(void *ctx, bsscfg_state_upd_data_t *evt)
{
	wlc_psta_info_t *psta = (wlc_psta_info_t *)ctx;
	wlc_info_t *wlc = psta->wlc;
...
/* assoc state change notification */
static void
wlc_psta_assoc_state_upd(void *ctx, bss_assoc_state_data_t *notif)
{
	wlc_psta_info_t *psta = (wlc_psta_info_t *)ctx;
	wlc_info_t *wlc = psta->wlc;
	...
	/* Put connection in PM 1 mode after association completes */
	if (notif->state == AS_JOIN_ADOPT) {
		int force_pm = PM_MAX;
		if (wlc_ioctl(wlc, WLC_SET_PM, (void *)&force_pm, sizeof(int),
		              notif->cfg->wlcif)) {
			WL_PSTA(("wl%d.%d: wlancoex sta entering to PM error\n",
			         WLCWLUNIT(wlc), WLC_BSSCFG_IDX(notif->cfg)));
		}
	}
	/* Clean up the proxy STAs before starting roam scan */
	else if (notif->type == AS_ROAM && notif->state == AS_SCAN &&
	         BSSCFG_IS_PRIMARY(notif->cfg)) {
		wlc_psta_disassoc_all(wlc->psta);
	}
	/* Re-associate PSTA clients after primary has roamed */
	else if (notif->type == AS_ROAM && notif->state == AS_IDLE &&
	         BSSCFG_IS_PRIMARY(notif->cfg)) {
		wlc_psta_reassoc_all(wlc->psta, notif->cfg);
	}
}

/* disassoc state change notification */
static void
wlc_psta_disassoc_notif(void *ctx, bss_disassoc_notif_data_t *notif)
{
	wlc_psta_info_t *psta = (wlc_psta_info_t *)ctx;
	wlc_info_t *wlc = psta->wlc;
	 ...
	 */
	if (notif->type == DAN_TYPE_LOCAL_DEAUTH) {
		wlc_psta_deauth_client(wlc->psta, notif->addr);
	}
	else if (notif->type == DAN_TYPE_LOCAL_DISASSOC &&
	         BSSCFG_IS_PRIMARY(notif->cfg)) {
		int32 idx;
		wlc_bsscfg_t *cfg;
