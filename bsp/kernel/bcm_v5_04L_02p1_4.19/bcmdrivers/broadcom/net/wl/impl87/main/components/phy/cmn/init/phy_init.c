 ...
 *
 * $Id: phy_init.c 690566 2017-03-16 23:31:01Z $
 */
...
BCMATTACHFN(phy_init_attach)(phy_info_t *pi)
{
	...
	phy_dbg_add_dump_fn(pi, "phyinit", phy_init_dump, info);
#endif
...
phy_init_invoke_init_fns(phy_init_info_t *ii)
{
	...
	/* make a note of the state of this particular PHY */
	phy_set_sliceupstate(pi->cmni, pi->sh->unit, TRUE);
...
phy_init_invoke_down_fns(phy_init_info_t *ii)
{
	...
	/* make a note of the state of this particular PHY */
	phy_set_sliceupstate(pi->cmni, pi->sh->unit, FALSE);
}
#endif /* BCMNODOWN */
