 ...
 *
 * $Id: phy_chanmgr.c 794561 2021-01-12 23:00:57Z $
 */
...
bool
wlc_phy_is_smth_en(phy_info_t *pi, bool ishe)
{
	phy_chanmgr_info_t *chanmgri = pi->chanmgri;
	phy_type_chanmgr_fns_t *fns = chanmgri->fns;

	if ((fns->is_smth_en != NULL) && ISACPHY(pi)) {
		return (fns->is_smth_en)(fns->ctx, ishe);
	} else {
		return FALSE;
	}
}
...
phy_chanmgr_set_shm(phy_info_t *pi, chanspec_t chanspec)
{
	...
	wlapi_bmac_write_shm1(pi->sh->physhim, M_CURCHANNEL(pi), curchannel);
#if defined(WL_PSMX)
