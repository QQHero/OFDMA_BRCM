 ...
 *
 * $Id: bcmwpa.c 798827 2021-05-13 04:50:28Z $
 */
...
#endif	/* BCMCCX */
#if defined(BCMSUP_PSK) || defined(BCMSUPPL) || defined(BCM_OL_DEV) || defined(WLTDLS)
...
#include <bcmwpa.h>
	...
	defined(BCM_OL_DEV) || defined(WL_OKC) || defined(WLTDLS)
	...
	uint8 *pmk, uint pmk_len, uint8 *pmkr0, uint8 *pmkr0name)
{
	...
	ssid_len = MIN(ssid_len, DOT11_MAX_SSID_LEN);
	bcopy(prefix, data, strlen(prefix));
	data_len += strlen(prefix);
	...
	/* set r0kh_len to prevent buffer overflow. The -4 at the end comes from the
	 * internal implementation of hmac_sha256_n.  We put it here to minimize memory
	 * increase - albeit the result is not clean implementation
	 */

	r0kh_len = MIN(r0kh_len, (sizeof(data) - ETHER_ADDR_LEN - data_len - 4));
...
#ifdef	BCMCCX
	if (!bcmp((char *)suite->oui, CISCO_AIRONET_OUI, DOT11_OUI_LEN)) {
		if (!wep_ok)
			ret = FALSE;
		else
			switch (suite->type + CISCO_BASE) {
			case WPA_CIPHER_CKIP_MMH:
				*cipher = CRYPTO_ALGO_CKIP_MMH;
				break;
			case WPA_CIPHER_CKIP:
				*cipher = CRYPTO_ALGO_CKIP;
				break;
			case WPA_CIPHER_WEP_MMH:
				*cipher = CRYPTO_ALGO_WEP_MMH;
				break;
			default:
				ret = FALSE;
				break;
...
#ifdef BCMCCX
	if (!bcmp(akm, CISCO_AIRONET_OUI, DOT11_OUI_LEN)) {
		switch (akm[DOT11_OUI_LEN]) {
		case RSN_AKM_NONE:
			if (!sta_iswpa)
				*auth = WPA2_AUTH_CCKM;
...
{
#ifdef BCMCCX
	if (!bcmp(cipher, CISCO_AIRONET_OUI, DOT11_OUI_LEN)) {
		switch (cipher[DOT11_OUI_LEN] + CISCO_BASE) {
		case WPA_CIPHER_CKIP:
		case WPA_CIPHER_CKIP_MMH:
		case WPA_CIPHER_WEP_MMH:
			*wsec = WEP_ENABLED;
...
#ifdef BCMCCX
	case WPA_CIPHER_CKIP:
		wsec = CKIP_KP_ENABLED;
		break;
	case WPA_CIPHER_CKIP_MMH:
		wsec = CKIP_MIC_ENABLED;
		break;
	case WPA_CIPHER_WEP_MMH:
		wsec = WEP_ENABLED;
		break;
