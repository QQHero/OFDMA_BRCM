 ...
 *
 * $Id: phy_cache.c 810784 2022-04-13 19:59:25Z $
 */
#include <phy_cfg.h>
#include <typedefs.h>
#include <bcmutils.h>
...
struct phy_cache_info {
	...
	uint16	backup_buffer_size;
	void*	backup_buffer;	/* backup scratch buffer */
...
WLBANDINITFN(phy_cache_init)(phy_init_ctx_t *ctx)
{
	...
	cachei->backup_buffer = NULL;
	cachei->backup_buffer_size = 0;
...
BCMUNINITFN(phy_cache_down)(phy_init_ctx_t *ctx)
{
	...
	if (cachei->backup_buffer) {
		phy_mfree(cachei->pi, cachei->backup_buffer, cachei->backup_buffer_size);
		cachei->backup_buffer = NULL;
	}
	...
	if (calbuf->cal_cache_buf && !moreslicesup) {
...
{
	void * ret = NULL;
	if (size <= cachei->reuse_buffer_size) {
		if (cachei->reuse_buffer_lock == 0) {
			cachei->reuse_buffer_lock = 1;
			ret = cachei->reuse_buffer;
		} else if (cachei->backup_buffer == NULL) {
			PHY_ERROR(("%s: alloc backup buffer %d\n", __FUNCTION__, size));
			ret = phy_malloc_fatal(cachei->pi, size);
			cachei->backup_buffer = ret;
			cachei->backup_buffer_size = size;
		} else {
			PHY_ERROR(("%s: failed to get reuse_buffer %d\n", __FUNCTION__, size));
			ASSERT(cachei->reuse_buffer_lock == 0);
		}
	} else {
...
void
phy_cache_release_reuse_buffer(phy_cache_info_t *cachei, void *ptr)
{
	if (ptr == NULL) {
		return;
	}
	if (ptr == cachei->backup_buffer) {
		PHY_ERROR(("%s: free backup buffer %d\n", __FUNCTION__,
			cachei->backup_buffer_size));
		phy_mfree(cachei->pi, ptr, cachei->backup_buffer_size);
		cachei->backup_buffer = NULL;
		return;
	}
...
int
BCMATTACHFN(phy_cache_register_cb)(phy_info_t*pi, phy_cache_validate_fn_t fn, void *context)
{
...
#if defined(PHYCAL_CACHING)
int
wlc_phy_cal_cache_init(phy_info_t *pi)
{
	return 0;
}

void
wlc_phy_cal_cache_deinit(phy_info_t *pi)
{
...
void
wlc_phy_cal_cache_set(phy_info_t *pi, bool state)
{
...
bool
wlc_phy_cal_cache_get(phy_info_t *pi)
{
...
#if defined(PHYCAL_CACHING)
int
wlc_phy_invalidate_chanctx(phy_info_t *pi, chanspec_t chanspec)
{
	int ctl;
	phy_cache_info_t *ci = pi->cachei;
...
*/
int
wlc_phy_reuse_chanctx(phy_info_t *pi, chanspec_t chanspec)
{
...
void
wlc_phy_update_chctx_glacial_time(phy_info_t *pi, chanspec_t chanspec)
{
	ch_calcache_t *ctx;

	PHY_CAL(("wlc_phy_update_chctx_glacial_time: wlc_phy_get_chanctx\n"));

	if ((ctx = wlc_phy_get_chanctx(pi, chanspec)))
...
void
wlc_phy_get_all_cached_ctx(phy_info_t *pi, chanspec_t *chanlist)
{
	phy_cache_info_t *ci = pi->cachei;
	int ctl;
	int i = 0;
...
uint32
wlc_phy_get_current_cachedchans(phy_info_t *pi)
{
	if (pi) {
