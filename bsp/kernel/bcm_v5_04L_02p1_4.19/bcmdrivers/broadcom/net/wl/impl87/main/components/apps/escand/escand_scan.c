...
escand_ci_scan_update_idx(escand_scan_chspec_t *chspec_q, uint8 increment)
{
	...
	do {
		...
		if ((chspec_q->ci_scan_running == ESCAND_CI_SCAN_RUNNING_PREF)) {
			if ((chan_flags & ESCAND_CI_SCAN_CHAN_PREF) && !(chan_flags &
				ESCAND_CI_SCAN_CHAN_EXCL))
				break;
...
escand_build_scanlist(escand_chaninfo_t *c_info)
{
	...
	if (BAND_6G(rsi->band_type)) {
		...
		ESCAND_ERR(ret, "failed to determine valid band");
		return -1;
	}
...
int escand_ci_scan_check(escand_chaninfo_t *c_info)
{
	...
	if (c_info->sta_count != 0) {
		timeout = c_info->escand_ci_busy_min_scan_timer;
	} else {
		timeout = c_info->escand_ci_scan_timeout;
	}
	...
	*/
	...
	if ((now - c_info->timestamp_escand_scan) >= timeout) {
		if (started[index] == 1) {
			ESCAND_DEBUG("%s: index %d, now %u, timestamp_escand_scan %u, "
				"scan already started\n", c_info->name, index, (uint) now,
				(uint) c_info->timestamp_escand_scan);
			if ((now - c_info->timestamp_escand_scan) > (timeout * 2)) {
				ESCAND_INFO("escand_ci_scan_check: too much time expired, "
					"restarting\n");
			} else {
				return 1;
			}
		...
		escand_ci_scan_update_idx(&c_info->scan_chspec_list, 0);
		new_index = escand_idx_from_map(c_info->name);
		ESCAND_INFO("escand_ci_scan_timeout start CI scan: %s, now %u, timestamp %u, "
			"index %d, new_index %d, scan_count %d, scan_running %d\n",
			c_info->name, (uint)now, c_info->timestamp_escand_scan, index, new_index,
...
int escand_check_traffic_thresh_level(escand_chaninfo_t *c_info)
{
	int time_base = ESCAND_TRAFFIC_POLL / ESCAND_TICKS_PER_SEC;

	if (c_info->escand_traffic_thresh == 0) {
		if (c_info->escand_traffic_measured > 0) {
			return c_info->escand_traffic_measured;
		}
	} else {
		if (c_info->escand_traffic_measured > 0) {
			c_info->escand_traffic_thresh_measured =
				c_info->escand_traffic_measured / time_base;
			if (c_info->escand_traffic_thresh_measured >
				c_info->escand_traffic_thresh) {
				return c_info->escand_traffic_thresh_measured;
			}
		}
	}

	/* Traffic not greater than threshold, so scan */
	return 0;
}
...
escand_do_ci_update(uint ticks, escand_chaninfo_t * c_info)
{
	...
	if (ticks % (c_info->escand_ci_scan_timer * ESCAND_TICKS_PER_SEC))
		return ret;

	if (c_info->sta_count > 0)
	{
		if (ticks % (c_info->escand_ci_busy_min_scan_timer * ESCAND_TICKS_PER_SEC))
			return ret;
	}

	if (c_info->escand_traffic_thresh != -1)
	{
		ret = escand_check_traffic_thresh_level(c_info);

		if (ret != 0) {
			ESCAND_INFO("escand_do_ci_update: thresh %d, measured %d, skip scan\n",
				c_info->escand_traffic_thresh,
				c_info->escand_traffic_thresh_measured);
			return ret;
		}
	...
	ESCAND_INFO("escand_do_ci_update: sta_count %d\n", c_info->sta_count);
	...
	if (!skip) {
		...
		if (errno != EBUSY)
			...
			ESCAND_INFO("ci scan rejected -- channel busy\n");
			return ret;
		}
...
escand_escan_prep_ci(escand_chaninfo_t *c_info, wl_scan_params_t *params, int *params_size)
{
	...
	memcpy(&params->bssid, &ether_bcast, ETHER_ADDR_LEN);
	params->bss_type = DOT11_BSSTYPE_ANY;
	params->scan_type = is_dfs ? WL_SCANFLAGS_PASSIVE :
		(c_info->escand_ci_scan_type ? WL_SCANFLAGS_PASSIVE : 0);
	params->nprobes = -1;
	params->active_time = ESCAND_CI_SCAN_DWELL;
	params->passive_time = ESCAND_CI_SCAN_DWELL;
	params->home_time = -1;
	...
	if (previous == NULL)
		*rootp = new;
		...
		if (curptr == NULL)
			...
			memcpy(&curptr->binfo_local, &new->binfo_local,
				sizeof(escand_bss_info_sm_t));
			ESCAND_FREE(new);
		}
	}
	return 0;
...
escand_update_escanresult_queue(escand_chaninfo_t *c_info)
{
	...
	for (escan_bss_head = c_info->escand_escan->escan_bss_head;
		...
		new_entry = (escand_bss_info_entry_t*)
			escand_malloc(sizeof(escand_bss_info_entry_t));
...
/*
 * escand_cs scan_timer_check() - check for CS scan timer
 *
...
int
escand_cs_scan_timer_check(escand_chaninfo_t * c_info)
{
	uint cs_scan_timer;
	int ret = 0;
