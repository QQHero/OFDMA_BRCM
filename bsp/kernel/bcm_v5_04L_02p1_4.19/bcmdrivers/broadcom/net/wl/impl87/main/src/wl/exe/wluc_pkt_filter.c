 ...
 *
 * $Id: wluc_pkt_filter.c 793727 2020-12-08 10:07:44Z $
 */
...
wl_pkt_filter_add(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((ftype == WL_PKT_FILTER_TYPE_PATTERN_LIST_MATCH) ||
		...
		wl_pkt_filter_pattern_listel_t *pf_el =
			(wl_pkt_filter_pattern_listel_t *)pkt_filterp->u.patlist.patterns;
...
wl_pkt_filter_list(void *wl, cmd_t *cmd, char **argv)
{
	...
	*/
	...
	filterp = (wl_pkt_filter_t *)list->filter;
	for (i = 0; i < list->num; i++)
	{
		...
		if (type == WL_PKT_FILTER_TYPE_PATTERN_MATCH_TIMEOUT) {
			...
			wl_pkt_filter_pattern_listel_t *listel =
				(wl_pkt_filter_pattern_listel_t *)filterp->u.patlist.patterns;
