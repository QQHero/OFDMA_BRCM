 ...
 *
 * $Id: bsd_wbd.c 801292 2021-07-20 06:33:36Z $
 */
...
bsd_wbd_add_bssinfo(bsd_wbd_info_t *wbd_info, bsd_bssinfo_t *bssinfo)
{
	...
	newnode->strong_sta_cfg =
		(wbd_strong_sta_policy_t*)malloc(sizeof(*(newnode->strong_sta_cfg)));
	if (newnode->strong_sta_cfg == NULL) {
		BSD_WBD("Failed to allocate memory for strong_sta_cfg\n");
		free(newnode->weak_sta_cfg);
		free(newnode);
		return NULL;
	}
	memset(newnode->strong_sta_cfg, 0, sizeof(*(newnode->strong_sta_cfg)));
...
int
bsd_wbd_identify_wbd_band_type(chanspec_t ifr_chanspec, int *out_band)
{
	...
	*out_band = WBD_BAND_LAN_INVALID;
	...
	/* If Current Channel is in Band 6G */
	if (CHSPEC_IS6G(ifr_chanspec)) {

		/* WBD band type is LAN_6G */
		*out_band = WBD_BAND_LAN_6G;
		goto end;

		/* If Current Channel is in Band 2G */
	} else if (CHSPEC_IS2G(ifr_chanspec)) {
	...
	BSD_WBD("Chanspec[0x%x] Channel[%d] WBD_BAND[%d]\n",
			ifr_chanspec, channel,
...
bsd_wbd_identify_band_type(bsd_bssinfo_t *bssinfo, int *out_band)
{
	...
	*out_band = WBD_BAND_LAN_INVALID;
	...
	/* Get WBD Band Enumeration from ifname's Chanspec & Bridge Type */
	ret = bsd_wbd_identify_wbd_band_type(bssinfo->chanspec, out_band);
	if (ret != BSD_OK) {
		ret = BSD_FAIL;
		goto end;
	}
...
bsd_wbd_init_bss(bsd_info_t *info, bsd_wbd_bss_list_t *wbd_bssinfo)
{
	...
	/* Strong Sta Configurations defined in NVRAM */
	str = bsd_wbd_nvram_safe_get(strcat_r(prefix, BSD_WBD_NVRAM_STRONG_STA_CFG, tmp), &ret);

	if (ret == BSD_OK) {
		num = sscanf(str, "%d %d %d %x",
			&strong_sta_policy.t_idle_rate, &strong_sta_policy.t_rssi,
			&strong_sta_policy.t_hysterisis, &strong_sta_policy.flags);
		if (num == 4) {
			memcpy(wbd_bssinfo->strong_sta_cfg, &strong_sta_policy,
				sizeof(*wbd_bssinfo->strong_sta_cfg));
		} else {
			BSD_ERROR("BSS[%s] %s[%s] format error\n", wbd_bssinfo->bssinfo->ifnames,
				BSD_WBD_NVRAM_STRONG_STA_CFG, str);
		}
	}
	...
	BSD_WBD("Band[%d] Algo[%d] weak_sta_policy[%d]: "
		...
		"strong_sta_idle_rate=%d strong_sta_rssi=%d "
		"strong_sta_rssi_hysterisis=%d strong_sta_flags=0x%x\n",
		...
		wbd_bssinfo->strong_sta_cfg->t_idle_rate,
		wbd_bssinfo->strong_sta_cfg->t_rssi,
		wbd_bssinfo->strong_sta_cfg->t_hysterisis,
		wbd_bssinfo->strong_sta_cfg->flags);
...
bsd_wbd_init(bsd_info_t *info)
{
	...
	char ifnames[NVRAM_MAX_VALUE_LEN], name[IFNAMSIZ], *next = NULL;
	...
	wbd_info->highest_band = WBD_BAND_LAN_INVALID;
				...
				if (bssinfo->valid &&
					...
					if (cur) {
						...
						if (ret != BSD_OK)
							goto done;
						/* Update highest band */
						if (wbd_info->highest_band < cur->wbd_band_type) {
							wbd_info->highest_band =
								cur->wbd_band_type;
						}
					}
					break;
...
bsd_wbd_reinit(bsd_info_t *info)
{
	...
	while (cur) {
		...
		/* Update highest band */
		if (wbd_info->highest_band < cur->wbd_band_type) {
			wbd_info->highest_band = cur->wbd_band_type;
		}
...
bsd_cleanup_wbd(bsd_wbd_info_t *info)
{
	...
	if (info != NULL) {
		...
		while (cur != NULL) {
			...
			if (cur->strong_sta_cfg)
				free(cur->strong_sta_cfg);
			free(cur);
			cur = next;
		}
...
/* Creates the request and sends the weak/strong client request to WBD */
static int
bsd_wbd_send_weak_strong_client_req(bsd_info_t *info, bsd_bssinfo_t *bssinfo, bsd_sta_info_t *sta,
	int weak_flag, int wbd_band_type, int *wbd_ret, int command)
{
	char data[BSD_WBD_REQ_BUFSIZE], read_buf[BSD_WBD_REQ_BUFSIZE], cmd[BSD_WBD_CMD_BUFSIZE];
	int ret = BSD_OK;
	uint8 band;
	BSD_ENTER();

	if (command == BSD_WBD_STA_WEAK) {
		BSDSTRNCPY(cmd, "WEAK_CLIENT_BSD", sizeof(cmd));
	} else if (command == BSD_WBD_STA_STRONG) {
		BSDSTRNCPY(cmd, "STRONG_CLIENT_BSD", sizeof(cmd));
	}

	if (wbd_band_type == WBD_BAND_LAN_6G) {
		sta->band = WBD_BAND_LAN_ALL;
	} else {
		if (wbd_band_type == WBD_BAND_LAN_2G) {
			sta->band = WBD_BAND_LAN_2G;
		} else {
			sta->band = WBD_BAND_LAN_2G5G;
		}

		/* Check from probe table if the sta supports 6G or 5G */
		band = bsd_get_sta_band(info, &sta->addr);
		if (band & BSD_BAND_6G) {
			sta->band = WBD_BAND_LAN_ALL;
		} else if ((wbd_band_type == WBD_BAND_LAN_2G) && (band & BSD_BAND_5G)) {
			sta->band = WBD_BAND_LAN_2G5G;
		}
	}

	snprintf(data, sizeof(data), "{ \"Cmd\": \"%s\", \"SrcMAC\": \""MACF"\", "
		...
		"\"TxTotFailures\": %d, \"Band\": %d } }",
		cmd, ETHER_TO_MACF(bssinfo->bssid), ETHER_TO_MACF(bssinfo->bssid),  wbd_band_type,
...
/* Creates the request and sends the weak/strong cancel request to WBD */
static int
bsd_wbd_send_weak_strong_cancel_req(bsd_info_t *info, bsd_bssinfo_t *bssinfo, bsd_sta_info_t *sta,
	int wbd_band_type, int *wbd_ret, int command)
{
	char data[BSD_WBD_REQ_BUFSIZE], read_buf[BSD_WBD_REQ_BUFSIZE], cmd[BSD_WBD_CMD_BUFSIZE];
	int ret = BSD_OK;
	BSD_ENTER();
	...
	if (command == BSD_WBD_STA_WEAK) {
		BSDSTRNCPY(cmd, "WEAK_CANCEL_BSD", sizeof(cmd));
	} else if (command == BSD_WBD_STA_STRONG) {
		BSDSTRNCPY(cmd, "STRONG_CANCEL_BSD", sizeof(cmd));
	}
...
/* Check whether the STA is strong or not */
static int
bsd_wbd_is_strong_sta(bsd_info_t *info, bsd_wbd_bss_list_t *wbd_bssinfo,
	bsd_sta_info_t *sta)
{
	bsd_bssinfo_t *bssinfo;
	int isstrong = FALSE;
	wbd_strong_sta_metrics_t sta_stats;
	BSD_ENTER();

	bssinfo = wbd_bssinfo->bssinfo;
	BSD_WBD("STA="MACF" in ifname=%s ssid=%s BSSID="MACF"\n",
		ETHER_TO_MACF(sta->addr), bssinfo->ifnames,
		bssinfo->ssid, ETHER_TO_MACF(bssinfo->bssid));
	BSD_WBD("RSSI[%d] ReportedRSSI[%d] steer_flag[%d] band[%d] flags[0x%X] \n",
		sta->rssi, sta->reported_rssi, sta->steerflag, sta->band, sta->flags);
	...
	/* Fill STA Stats, which needs to be compared for Strong Client Indentification */
	memset(&sta_stats, 0, sizeof(sta_stats));
	sta_stats.idle_rate = sta->datarate;
	sta_stats.rssi = sta->rssi;
	sta_stats.last_strong_rssi = sta->reported_rssi;

	/* Common algo to identify if STA is Strong or not */
	isstrong = wbd_strong_sta_identification(&sta->addr, &sta_stats,
		wbd_bssinfo->strong_sta_cfg);

	if (isstrong) {
		BSD_WBD("Found Strong STA="MACF" in ifname=%s ssid=%s BSSID="MACF"\n",
			ETHER_TO_MACF(sta->addr), bssinfo->ifnames,
			bssinfo->ssid, ETHER_TO_MACF(bssinfo->bssid));
	}
end:
		...
		while (sta) {
			...
			if (bsd_wbd_is_weak_sta(info, cur, sta, &weak_flag) == TRUE) {
				...
				if (!(sta->wbd_sta_status & BSD_WBD_STA_WEAK)) {
					ret = bsd_wbd_send_weak_strong_client_req(info, bssinfo,
						sta, weak_flag, wbd_band_type, &wbd_ret,
						BSD_WBD_STA_WEAK);
					if (wbd_ret == BSDE_WBD_OK) {
			...
			} else if (!(wbd_info->highest_band & wbd_band_type) &&
				bsd_wbd_is_strong_sta(info, cur, sta) == TRUE) {
				/* If its already strong, don't inform to WBD */
				if (!(sta->wbd_sta_status & BSD_WBD_STA_STRONG)) {
					ret = bsd_wbd_send_weak_strong_client_req(info, bssinfo,
						sta, wbd_band_type, weak_flag, &wbd_ret,
						BSD_WBD_STA_STRONG);
					if (wbd_ret == BSDE_WBD_OK) {
						sta->reported_rssi = sta->rssi;
						sta->wbd_sta_status |= BSD_WBD_STA_STRONG;
						sta->wbd_sta_status |= BSD_WBD_STA_STRONG_PENDING;
						BSD_WBD("ifname[%s] BSSID["MACF"] STA["MACF"] "
							"sta->reported_rssi[%d]\n\n",
							bssinfo->ifnames,
							ETHER_TO_MACF(bssinfo->bssid),
				...
				/* Not weak or strong. So check if its already weak or strong */
				if ((sta->wbd_sta_status & BSD_WBD_STA_WEAK)) {
					/* Inform WBD */
					ret = bsd_wbd_send_weak_strong_cancel_req(info, bssinfo,
						sta, wbd_band_type, &wbd_ret, BSD_WBD_STA_WEAK);
					if (wbd_ret == BSDE_WBD_OK) {
						sta->reported_rssi = sta->rssi;
				...
				} else if ((sta->wbd_sta_status & BSD_WBD_STA_STRONG)) {
					/* Inform WBD */
					ret = bsd_wbd_send_weak_strong_cancel_req(info, bssinfo,
						sta, wbd_band_type, &wbd_ret, BSD_WBD_STA_STRONG);
					if (wbd_ret == BSDE_WBD_OK) {
						sta->reported_rssi = sta->rssi;
						BSD_WBD("sta->reported_rssi[%d]\n\n",
							sta->reported_rssi);
