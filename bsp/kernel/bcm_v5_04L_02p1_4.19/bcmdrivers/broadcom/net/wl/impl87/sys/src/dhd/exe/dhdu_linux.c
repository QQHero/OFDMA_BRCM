 ...
 *
 * $Id: dhdu_linux.c 806645 2021-12-24 13:35:25Z $
 */
...
wl_check(void *wl)
{
	...
	/* Check for dhd adapter. If found return success
	 * The dhd_ioctl() has no way to differentiate between
	 * a WLC_GET_MAGIC/VERSION and a DHD_GET_MAGIC/VERSION.
	 * So it would definitely fail when queried for WLC_GET_MAGIC
	 * or VERSION from this function as the conditions under which
	 * DHD_DRIVER_MAGIC is passed to the driver are satisfied from
	 * this call and so this workaround
	 */
	if (!dhd_check (wl))
		return 0;
