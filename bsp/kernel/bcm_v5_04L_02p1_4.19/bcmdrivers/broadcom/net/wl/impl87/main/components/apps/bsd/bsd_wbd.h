 ...
 *
 * $Id: bsd_wbd.h 801292 2021-07-20 06:33:36Z $
 */
...
#define BSD_WBD_NVRAM_STRONG_STA_CFG	"wbd_strong_sta_cfg"
...
#define BSD_WBD_STA_STRONG_PENDING	0x0020	/* STA is strong and reported to slave */
#define BSD_WBD_STA_STRONG		0x0040	/* STA is strong and accepted by master */
...
/* Threshold parameters for finding strong clients */
typedef struct bsd_wbd_strong_sta_policy {
	int idle_rate;		/* data rate threshold to measure STA is idle */
	int rssi;		/* rssi threshold */
	int hysterisis;		/* rssi hysterisis */
	uint32 flags;		/* extension flags (Rules) */
} bsd_wbd_strong_sta_policy_t;
...
typedef struct bsd_wbd_bss_list {
	...
	wbd_strong_sta_policy_t *strong_sta_cfg;	/* Strong client configuration */
...
typedef struct bsd_wbd_info {
	...
	uint8 highest_band;		/* Highest band in device */
} bsd_wbd_info_t;
