 ...
 *
 * $Id: bcm_usched.c 806480 2021-12-22 05:36:58Z $
 */
...
typedef struct usched_timers {
	...
	bcm_usched_comparefn *comparefn;	/* User provided optional compare function */
} usched_timers_t;
...
static usched_timers_t*
bcm_usched_create_timer_node(unsigned long long interval, short int repeat_flag,
	bcm_usched_timerscbfn *cbfn, void *arg, bcm_usched_comparefn *comparefn);
...
/* Get the boot time */
static
void bcm_usched_get_boot_time(struct timeval *tv)
{
	struct timespec ts;
	...
	if (clock_gettime(CLOCK_BOOTTIME, &ts) == 0) {
		...
		USCHED_ERROR("Error from clock_gettime : %s\n", strerror(errno));
		tv->tv_sec  = 0;
		tv->tv_usec = 0;
	}
}
...
bcm_usched_swap_timers_data(usched_timers_t *a, usched_timers_t *b)
{
	...
	USCHED_SWAP(a->comparefn, b->comparefn, bcm_usched_comparefn*);
}
...
bcm_usched_process_timers(usched_handle_t *hdl)
{
	...
	bcm_usched_get_boot_time(&curr_time);
	...
	bcm_usched_timerscbfn *cbfn, void *arg)
{
	...
	entry.repeat_flag = repeat_flag;
	entry.cbfn = cbfn;
	entry.arg = arg;
	entry.comparefn = NULL;

	return bcm_usched_add_timer_ex(hdl, &entry);
}
...
bcm_usched_strerror(BCM_USCHED_STATUS errorcode)
{
	switch (errorcode) {
		...
		case BCM_USCHEDE_INV_TIMER_ENTRY:
			return USCHED_ERROR_PREFIX"Invalid timer entry";
...
/* create the new timer */
BCM_USCHED_STATUS
bcm_usched_add_timer_ex(bcm_usched_handle *hdl, bcm_usched_timer_entry_t *entry)
{
	usched_timers_t *new;
	...
	if (!entry) {
		USCHED_ERROR("Invalid timer entry\n");
		return BCM_USCHEDE_INV_TIMER_ENTRY;
	}

	if (bcm_usched_is_timer_exists((usched_handle_t*)hdl, entry->cbfn, entry->arg)
		!= BCM_USCHEDE_OK) {
		return BCM_USCHEDE_TIMER_EXISTS;
	}

	new = bcm_usched_create_timer_node(entry->interval, entry->repeat_flag, entry->cbfn,
		entry->arg, entry->comparefn);
	if (!new) {
		USCHED_ERROR("Failed to create timer node\n");
		return BCM_USCHEDE_MEMORY;
	}
...
/* Finds the timer matching to the arg */
void*
bcm_usched_find_timer(bcm_usched_handle *handle, bcm_usched_timerscbfn *cbfn,
	bcm_usched_comparefn *comparefn, void *arg)
{
	...
	if (dll_empty(&hdl->timers_list_head)) {
		USCHED_WARNING("Empty list of timers\n");
		return NULL;
	}

	/* Check if callback function and compare function do match and compare function
	 * returns 0 than return the corresponding timer data pointer.
	 */
	for (item_p = dll_head_p(&hdl->timers_list_head); !dll_end(&hdl->timers_list_head, item_p);
		item_p = dll_next_p(item_p)) {
		usched_timers_t *tmp = (usched_timers_t*)item_p;

		if (tmp->cbfn != cbfn || tmp->comparefn == NULL ||
			tmp->comparefn != comparefn || tmp->remove_flag) {
			continue;
		}
		match = tmp->comparefn(tmp->arg, arg);
		if (match) {
			USCHED_INFO("Timer matching %p found at %p\n", arg, tmp->arg);
			return tmp->arg;
		}
	}

	USCHED_WARNING("No timer matching arg [%p] found \n", arg);
