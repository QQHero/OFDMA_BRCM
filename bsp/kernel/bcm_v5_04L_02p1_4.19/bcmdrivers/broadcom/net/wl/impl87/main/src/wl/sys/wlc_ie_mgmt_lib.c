 ...
 *
 * $Id: wlc_ie_mgmt_lib.c 785259 2020-03-18 11:40:09Z $
 */
	...
	void *tmp_cbe, uint cbe_sz, const wlc_iem_tag_t *tag, uint16 cnt)
{
	...
	IEM_TRACE(("%s: tag %p cb %p sz %u tplt %p cnt %u\n",
	           __FUNCTION__, OSL_OBFUSCATE_BUF(cbe_tag),
			OSL_OBFUSCATE_BUF(cbe_tbl), cbe_cnt,
	...
	for (ti = 0, first = 0; ti < cnt && first < cbe_cnt; ti ++) {
		...
		while (l <= r) {
			...
			/* move all callbacks for the tag...
			 * TODO: optimize the move - can move the entries
			 * all together but right now we have to move one
			 * entry at a time because the temporary buffer
			 * we are given 'tmp_cbe' is only one entry big.
			 */
			/* find the first entry */
			while (m > 0 && m >= l && t == cbe_tag[m - 1]) {
				m --;
			}
			/* move until the last entry */
			while (m <= r && t == cbe_tag[m]) {
	...
	void *cbe, uint cbe_sz, wlc_iem_tag_t tag)
{
	...
	IEM_TRACE(("%s: tag %p cb %p sz %u ent %u\n",
	           __FUNCTION__, OSL_OBFUSCATE_BUF(cbe_tag),
			OSL_OBFUSCATE_BUF(cbe_tbl), cbe_cnt, tag));
 ...
 * the 'upp' parameter.
 *
 * @param[in] ft         802.11 Frame Type
 * @param[in] is_tag     FALSE when a Vendor specific IE needs to be parsed.
 * @param[in] parse_tag  Only valid when 'is_tag' is TRUE. Points at an array of parse tags.
 *                       Typically, this array contains all the TLV 'type' values for the frame type
 *                       given by parameter 'ft'.
 * @param[in] parse_cnt  Contains the number of array elements in array 'parse_tag'.
 *
 * If the function is called for a Vendor Specific IE ('is_tag' is FALSE) the 'vsie' callback is
 * invoked to query the Vendor Specific IE's ID first and above process is applied then.
 *
...
wlc_ieml_parse_frame(wlc_bsscfg_t *cfg, wlc_iem_ft_t ft,
	...
	uint8 *buf, uint buf_len)
{
	uint8 cbvec[CEIL(WLC_IEM_ID_MAX, NBBY)]; /**< bit[0] represents TLV type 0, etc. */
	uint i;
	uint8 *ie;
	uint ie_len;
	...
	while ((ie = buf) != NULL && buf_len >= TLV_HDR_LEN &&
		...
		/* Check tag for invalid extended ID */
		if (ie_tag == WLC_IEM_ID_INVALID)
			goto n_ie;

		if (is_tag &&
		    isclr(cbvec, ie_tag)) {
			/* tag was already called back during the course of this function, or the
			 * caller did not provide a callback for this particular tag in array
			 * parse_tag[]
			 */
			IEM_TRACE(("%s: skip %u\n", __FUNCTION__, ie_tag));
			goto n_ie;
