...
#if defined(BCMQT)// || TRUE
#define HWA_QT_TEST
#define PKTPGR_ALLOC_LOOP_TEST
#endif

#ifdef HWA_QT_TEST
extern uint32 hwa_kflag;
// XXX, CRBCAHWA-663 case2: PGI TXS ncons = 2 but 1 packet in HW
#define HWA_KFLAG_663_CASE_2		0x00000001
// XXX, CRBCAHWA-663 case1: PGI TXS ncons = 1 but 0 packet in HW
#define HWA_KFLAG_663_CASE_1		0x00000002
// XXX, CRBCAHWA-663 case1 aux flag
#define HWA_KFLAG_663_CASE_1_AUX	0x00000004
// XXX, CRBCAHWA-664
#define HWA_KFLAG_664				0x00000008
#endif
...
typedef enum hwa_callback
{
	...
	HWA_PKTPGR_PAGEOUT_CALLBACK      =      /* ref: hwa_pp_pageout_cmd_t */
	    ...
	    HWA_PKTPGR_PAGEOUT_LOCAL     =      /* hwa_pktpgr_pageout_pktlist() */
	        (HWA_PKTPGR_PAGEOUT_CALLBACK + HWA_PP_PAGEOUT_PKTLOCAL),
	    HWA_PKTPGR_PAGEOUT_HDBM_PKTLIST =   /* hwa_pktpgr_pageout_pktlist() */
	        (HWA_PKTPGR_PAGEOUT_CALLBACK + HWA_PP_PAGEOUT_HDBM_PKTLIST_WR),
	...
	HWA_PKTPGR_PAGEMGR_CALLBACK      =      /* ref: hwa_pp_pagemgr_cmd_t */
	    ...
	    HWA_PKTPGR_PAGEMGR_PUSH_MPDU =      /* hwa_pktpgr_pagemgr_push() */
	        (HWA_PKTPGR_PAGEMGR_CALLBACK + HWA_PP_PAGEMGR_PUSH_MPDU),
	    HWA_PKTPGR_PAGEMGR_PULL_MPDU =      /* hwa_pktpgr_pagemgr_pull() */
	        (HWA_PKTPGR_PAGEMGR_CALLBACK + HWA_PP_PAGEMGR_PULL_MPDU),
...
#if defined(HWA_PKTPGR_BUILD)
#define HWA_RXFILL_FIFO_INTRAGGR_COUNT  4
#define HWA_RXFILL_FIFO_INTRAGGR_TMOUT  24    // usecs
#else
...
#if defined(HWA_PKTPGR_BUILD)
	// HWA_PKTPGR_D11B_AUDIT_ENABLED or 6715Ax SW WAR
...
#endif
	/* rx error counters */
	uint32      rxgiants[HWA_RX_CORES_MAX]; // rx giant frames
} hwa_rxfill_t;
...
#if defined(HWA_PKTPGR_BUILD)
#define HWA_TXFIFO_AGGR_AQM_DESC_THRESHOLD  7
#define HWA_TXFIFO_AGGR_AQM_DESC_THRESHOLD2 3
#define HWA_TXFIFO_AQM_DESC_FREE_SPACE \
	((HWA_TXFIFO_AGGR_AQM_DESC_THRESHOLD2 << 3) + HWA_TXFIFO_AGGR_AQM_DESC_THRESHOLD + 1)
#else
#define HWA_TXFIFO_AGGR_AQM_DESC_THRESHOLD  3
#define HWA_TXFIFO_AQM_DESC_FREE_SPACE      (HWA_TXFIFO_AGGR_AQM_DESC_THRESHOLD + 1)
#endif
...
typedef struct hwa_txfifo
{
	...
	HWA_PKTPGR_EXPR(uint32  shadow_inited[HWA_TX_FIFOS_MAX / NBU32]); // bitmap
} hwa_txfifo_t;
...
#define HWA_TXSTAT_RING_ELEM_SIZE       TX_STATUS_MACTXS_BYTES
...
#define HWA_TXSTAT_QUEUE_DEPTH          \
	(TX_STATUS_MACTXS_RING_DEPTH * (TX_STATUS_MACTXS_BYTES / HWA_TXSTAT_PKG_SIZE))
...
// TxCpl monitor
void    hwa_txcpl_monitor(hwa_dev_t *dev);
...
#ifdef HWA_QT_TEST
#define PSPL_TX_TEST	// For PUSH/PULL related functionality test on veloce
#endif

#define HWA_PKTPGR_PAGEOUT_PKTCOUNT_MAX    255 // For A0, size of uint8

#define HWA_PKTPGR_PGI_RXPKT_REQ_MAX      4
#define HWA_PKTPGR_PGI_RXPKT_COUNT_MAX    256

// XXX, Reserve for txfifo shadow reclaim and MGMR ring req.
#define HWA_PKTPGR_DDBM_SW_RESV     256
...
#define HWA_PKTPGR_PGO_REQ_WAITTIME 60      // PGO Req delay cycles between requests
...
// Note: HW suggest to set BM threshold >= 8
#define HWA_PKTPGR_PAGEIN_REQ_DDBMTH  8
#define HWA_PKTPGR_PAGEIN_REQ_HDBMTH  8
...
// TXDMA
#define HWA_PKTPGR_MACAQM_EMPTY_CNT  HWA_TXFIFO_AQM_DESC_FREE_SPACE
...
// PAGERBM: exclude PP#DBMFULLINT, PP#DBMTH2INT, PPDDBMTH1INT and alloc/free err
#define HWA_COMMON_PAGERBM_INT_MASK \
	(HWA_COMMON_PAGEINTSTATUS_PPHDBMTH1INT_MASK)

// Errors
#define HWA_COMMON_PAGEERR_INT_MASK \
	(HWA_COMMON_PAGEINTSTATUS_PPINREQERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPINRSPERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPOUTREQERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPOUTRSPERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPALLOCREQERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPALLOCRSPERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPFREERPHREQERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPHDBMFREEERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPHDBMALLOCERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPDDBMFREEERRINT_MASK | \
	 HWA_COMMON_PAGEINTSTATUS_PPDDBMALLOCERRERRINT_MASK)
...
// PGI Tx Status emergency flag
#define PGI_EMER_TXS_RECLAIM        0x1    // Force using emergency resv DDBM in reclaim.
#define PGI_EMER_TXS_STARVE         0x2    // TxS in starvation, low shared DDBM use resv DDBM
#define PGI_EMER_RX_STARVE          0x4    // Rx in starvation, low shared DDBM use resv DDBM
#define PGIEMERINUSE(pp)            ((pp)->pgi_emer_flag != 0)

#define PGI_EMER_TYPE_TXS           0      // TxS
#define PGI_EMER_TYPE_RX            1      // Rx
#define PGI_EMER_TYPE_MAX           2      // TxS and Rx.

#define PGIEMERSETTXSRECLAIM(pp)    ((pp)->pgi_emer_flag |= PGI_EMER_TXS_RECLAIM)
#define PGIEMERISTXSRECLAIM(pp)     (((pp)->pgi_emer_flag & PGI_EMER_TXS_RECLAIM) != 0)
#define PGIEMERRESETTXSRECLAIM(pp)  ((pp)->pgi_emer_flag &= ~PGI_EMER_TXS_RECLAIM)

#define PGIEMERSETTXSSTARVE(pp)     ((pp)->pgi_emer_flag |= PGI_EMER_TXS_STARVE)
#define PGIEMERISTXSSTARVE(pp)      (((pp)->pgi_emer_flag & PGI_EMER_TXS_STARVE) != 0)
#define PGIEMERRESETTXSSTARVE(pp)   ((pp)->pgi_emer_flag &= ~PGI_EMER_TXS_STARVE)

#define PGIEMERSETRXSSTARVE(pp)     ((pp)->pgi_emer_flag |= PGI_EMER_RX_STARVE)
#define PGIEMERISRXSSTARVE(pp)      (((pp)->pgi_emer_flag & PGI_EMER_RX_STARVE) != 0)
#define PGIEMERRESETRXSSTARVE(pp)   ((pp)->pgi_emer_flag &= ~PGI_EMER_RX_STARVE)
...
typedef struct hwa_pktpgr
{
	...
	uint32          rsp_ring_int_mask[hwa_pktpgr_rsp_ring_max];
	...
	int16           pgo_pktlist_req;        // Counter of pageout local request
	int16           pgo_local_req;          // Counter of pageout local request
	int16           pgi_txs_req_tot;        // Counter of pagein txs request
	int16           pgi_txs_req_tot_hwm;    // High water mark of pagein txs request
	int16           pgi_txs_req[HWA_TX_FIFOS_MAX]; // Counter of pagein txs request
	uint8           pgi_itxs[HWA_TX_FIFOS_MAX / NBBY]; // Bitmap of pagein intermediate txs
	uint16          pgi_txs_unprocessed;    // Counter of pagein txs request unprocessed
	bool            pgi_txs_mismatch;       // WAR for TXs framid mismatch
	uint16          pgi_emer_flag;          // Emergency flag
	uint8           pgi_emer_id[PGI_EMER_TYPE_MAX]; // trans_id of using resv DDBM
	uint32          pgi_txs_cont_fail;      // # of continuous fail of pagein txs
	int16           pgi_rxpkt_req;          // Counter of pagein rx request
	int16           pgi_rxpkt_in_transit;   // Counter of pagein rx packet
	uint16          pgi_rxpkt_req_max;      // Maximum # of pagein rx request
	uint16          pgi_rxpkt_count_max;    // Maximum # of pkt_count in pagein rx request
	hwa_pp_rxlbufpool_t rxlbufpool;         // Local rxlbuf storage for SW
	hwa_pp_txlbufpool_t txlbufpool;         // Local txlbuf storage for SW
#ifdef HNDPQP
	hwa_pp_pqplbufpool_t pqplbufpool;       // Local txlbuf storage for PQP
	int16           pqplbuf_alloc_req;      // Counter of tagged txlbuf request
	bool            pqplbuf_use_rsv_ddbm;   // Use reserved DDBM to fill txlbuf storage for PQP
	bool            pqp_pgi_wake;           // resume PQP PGI after response ring is processed
#endif
	struct spktq    tag_pull_q;             // Single priority tagged pull packets queue
	int16           tag_pull_req;           // Counter of tagged pull request

	int16           tag_push_req;           // Counter of tagged push request
	hwa_pp_ring_t * req_ring_reg[hwa_pktpgr_req_ring_max];

	uint16          rsp_ring_in_process;    // Bitmap of inprocess response ring
	uint16          partial_relcaim_cnt;    // Maximum mpdu count of partial shadow reclaim

	// PGI fail counters
	uint32          pgi_txpost_fail;        // # of times reject txpost service
	uint32          pgi_txstatus_fail;      // # of times reject txstatus service
	uint32          pgi_txstatus_emer;      // # of times use resv DDBM to service txstatus
	uint32          pgi_rxpkt_fail;         // # of times reject rxpkt service
	uint32          pgi_rx_emer;            // # of times use resv DDBM to service rx

	// DDBM accounting.
	int16           ddbm_avail_sw;          // Available BM buffer, SW accounting
	int16           ddbm_avail_sw_lwm;      // Low water mark of SW hold ddbm count
	int16           ddbm_sw_tx;             // SW hold ddbm count for tx
	int16           ddbm_sw_tx_hwm;         // High water mark of SW hold ddbm count for tx
	int16           ddbm_sw_rx;             // SW hold ddbm count for rx
	int16           ddbm_sw_rx_hwm;         // High water mark of SW hold ddbm count for rx

	uint32          txs_ddbm_resv;          // Extra resv ddbm for Txs

#if defined(HWA_PKTPGR_DBM_AUDIT_ENABLED)
	// DBM audit
	struct bcm_mwbmap * hdbm_map;
	const char    * hdbm_func[HWA_HOST_PKTS_MAX + 1];
	uint16          hdbm_line[HWA_HOST_PKTS_MAX + 1];
	uint32          hdbm_err_alloc;
	uint32          hdbm_err_free;
	struct bcm_mwbmap * ddbm_map;
	const char    * ddbm_func[HWA_DNGL_PKTS_MAX + 1];
	uint16          ddbm_line[HWA_DNGL_PKTS_MAX + 1];
	uint32          ddbm_err_alloc;
	uint32          ddbm_err_free;
#endif
	...
	bool            pspl_enable;            // Enable/Disable pspl test
	bool            pspl_mpdu_enable;       // Enable/Disable pspl MPDU test
	bool            pspl_pgo_mpdu_enable;   // Enable/Disable pgo MPDU test

	bool            pspl_mpdu_append_enable; // Enable/Disable pspl MPDU test w/ prev_pkt_mapid
	uint8           pspl_mpdu_append_count;
	uint16          pspl_mpdu_pkt_mapid_h;
	uint16          pspl_mpdu_pkt_mapid_t;
	uint16          pspl_mpdu_count;
	uint16          pspl_pkt_count;
#endif

#ifdef HWA_QT_TEST
	hwa_pp_rxlbufpool_t rxpktpool;          // Local rxlbuf w/o RPH storage for SW
	bool            rx_alt_en;              // rxpkt allocation loop test enable
	uint16          rx_alt_count;           // How many rxpkt count per request in alt
	uint16          rx_alt_times;           // How many allocation request in alt
	uint16          rx_alt_times_n;
#endif
...
hwa_ring_t * BCMATTACHFN(hwa_pktpgr_ring_init)(hwa_dev_t *dev, hwa_ring_t *hwa_ring,
...
void    BCMATTACHFN(hwa_fastdma_attach)(hwa_dev_t *dev);
...
void    hwa_pktpgr_response(hwa_dev_t *dev, hwa_pktpgr_ring_t pp_ring,
...
// PageIn Txs
int	hwa_txstat_pagein_req(hwa_dev_t *dev, void *txs_pkg16, uint32 elem_ix);
...
// PageMgr Tx allocate
void    hwa_txpost_pagemgr_alloc_tx_rsp(hwa_dev_t *dev,
...
// PageMgr Push MPDU
void    hwa_pktpgr_push_mpdu_req(hwa_dev_t *dev, uint32 fifo_idx, uint8 tagged,
            void *pktlist_head, void *pktlist_tail, uint16 pkt_count, uint16 mpdu_count,
            uint16 prev_pkt_mapid);

#ifdef PSPL_TX_TEST
void    hwa_txfifo_pktchain_xmit_request_pspl_test_pull(hwa_dev_t *dev, uint8 pktpgr_trans_id,
...
void    hwa_rxpath_rxlbufpool_fill(hwa_dev_t *dev);
#ifdef HWA_QT_TEST
void    hwa_rxpktpool_add(hwa_dev_t *dev, uint16 count, uint16 times);
int     hwa_rxpktpool_del(hwa_dev_t *dev, uint16 count);
#endif
...
bool    hwa_pktpgr_rsp_ring_avail_for_reqcnt(hwa_dev_t *dev, hwa_pktpgr_ring_t pp_ring,
	uint32 counts);

// Enuqeqe Tx pacekts to SW shadow
void    hwa_txfifo_shadow_enq(hwa_dev_t *dev, int fifo_idx, int mpdu_count, int pkt_count,
            int txd_count, hwa_pp_lbuf_t *pktlist_head, hwa_pp_lbuf_t *pktlist_tail);

// Increase TX/AQM available descriptor count.
void    hwa_txfifo_descs_inc(hwa_txfifo_t *txfifo, uint32 fifo_idx, uint16 txdescs,
            uint16 aqmdescs);

// Decrease TX/AQM available descriptor count.
void    hwa_txfifo_descs_dec(hwa_txfifo_t *txfifo, uint32 fifo_idx, uint16 txdescs,
            uint16 aqmdescs);

// Sanity check and fixup head and end for local packet
hwa_pp_lbuf_t *hwa_txstat_pagein_rsp_fixup_local(hwa_dev_t *dev, hwa_pp_lbuf_t *txlbuf,
...
int     _hwa_pktpgr_dbm_free(hwa_dev_t *dev, hwa_dbm_instance_t dbm_instance, uint16 buf_idx,
            bool tx, const char *func, uint32 line);
#define hwa_pktpgr_dbm_free(dev, dbm_instance, buf_idx, tx) \
	_hwa_pktpgr_dbm_free(dev, dbm_instance, buf_idx, tx, __FUNCTION__, __LINE__)
...
void    hwa_pktpgr_pageout_pktlist_rsp_wait_to_finish(hwa_dev_t *dev);
...
void    hwa_pktpgr_txcpl_sync(hwa_dev_t *dev);

// Refresh TxCple for available space to add workitem for the D2H TxCompletion common ring.
void    hwa_txcple_refresh(hwa_dev_t *dev);

// Request pciedev to send pending jobs in d2h_req queue.
void    hwa_txcple_pciedev_d2h_req_q_send(hwa_dev_t *dev);

#ifdef HWA_PKTPGR_DBM_AUDIT_ENABLED
void    hwa_pktpgr_dbm_audit_dump(hwa_pktpgr_t *pktpgr, uint32 type);
#endif

void    hwa_hdbm_dump(hwa_dev_t *dev, uint32 pktmap_id, uint32 offset, uint32 size);
...
typedef struct hwa_fastdma
{
	uint8 channels_max;                     // channel count
	uint8 burst_length_max;                 // burst length
	struct {
		uint8 tx;                           // tx burst length
		uint8 rx;                           // rx burst length
	} burst_length[HWA_FASTDMA_CHANNELS_MAX];
	struct {
		uint8 tx;
		uint8 rx;
	} arbitration;                          // 1 = RR, 0 = fixed Chnl# prio
	bool  enabled;                          // all channels, tx and rx
} hwa_fastdma_t;
...
struct hwa_dev
{
	...
	HWA_PKTPGR_EXPR(hwa_fastdma_t fastdma);   // HWA fast DMA engine SW state
	...
	bool                    txcpl_monitor;    // Debug: TxCpl ring stuck monitor.
	...
	uint8                   fastdma_en;       // 0: use legacy HWA dma, 1: use fast dma
	...
	// D11B recycle from pagein.  B0: HWA recycle dirty part too.
	bool                    d11b_recycle_pagein;
#endif /* HWA_PKTPGR_BUILD */
...
#if defined(HWA_PKTPGR_BUILD)
#define HWA_FSM_IDLE_POLLLOOP		1000 * 1000
#define HWA_TXS_CONSUME_POLLLOOP	10
#else
