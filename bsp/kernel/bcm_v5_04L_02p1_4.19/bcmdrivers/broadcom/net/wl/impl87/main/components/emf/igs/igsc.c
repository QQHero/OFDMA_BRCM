 ...
 *
 * $Id: igsc.c 808500 2022-02-18 17:49:48Z $
 */
	...
	switch (igmph[IGMPV2_TYPE_OFFSET])
	{
		case IGMPV2_HOST_NEW_MEMBERSHIP_REPORT:
			if (!IP_ISMULTI(mgrp_ip)) {
				...
				igsc_update_join_member(igsc_info, ifp, dest_ip,
					mgrp_ip, mh_ip);
#ifdef BCA_CPEROUTER
				ret = EMF_SENDUP;
#endif
		...
		case IGMPV2_LEAVE_GROUP_MESSAGE:
			if (!IP_ISMULTI(mgrp_ip)) {
				...
				igsc_update_leave_member(igsc_info, ifp, mgrp_ip, mh_ip);
#ifdef BCA_CPEROUTER
				ret = EMF_SENDUP;
#endif
			}
...
igsc_init(int8 *inst_id, void *igs_info, osl_t *osh, igsc_wrapper_t *wrapper)
{
	...
	strncpy(igsc_info->inst_id, inst_id, IFNAMSIZ-1);
#endif
