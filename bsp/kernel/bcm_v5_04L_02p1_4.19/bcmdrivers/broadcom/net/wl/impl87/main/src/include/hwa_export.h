 ...
 * HWA2.1   129          43684Bx
 * HWA2.1   130          43684Cx
 * HWA2.2   131           6715A0 (Deprecated/Deleted)
 * HWA2.2   132           6715A1 (Deprecated/Deleted)
 * HWA2.2   133           6715B0
 *
...
#define HWA_DEFAULT	0x00000026	/* Supported  HWA revs: 129(43684b0), 130(43684c0),
					 * 133(6715b0)
					 */
...
#define _HWACONF_HAS(config, val)	(((config) & (1U << (val))) != 0)
#define _HWACONF_IS(config, val)	((config) == (1U << (val)))
#define _HWACONF_GE(config, val)	(((config) & (0 - (1U << (val)))) != 0)
#define _HWACONF_GT(config, val)	(((config) & (0 - 2 * (1U << (val)))) != 0)
#define _HWACONF_LT(config, val)	(((config) & ((1U << (val)) - 1)) != 0)
#define _HWACONF_LE(config, val)	(((config) & (2 * (1U << (val)) - 1)) != 0)
...
#define HWACONF_HAS(val) (\
	(HWACONF_CHK(val, HWACONF_BASE) && _HWACONF_HAS(HWACONF, HWACONF_REV(val, HWACONF_BASE))))
#define HWACONF_IS(val) (\
	(HWACONF_CHK(val, HWACONF_BASE) && _HWACONF_IS(HWACONF, HWACONF_REV(val, HWACONF_BASE))))
#define HWACONF_GE(val) (\
	(HWACONF_CHK(val, HWACONF_BASE) && _HWACONF_GE(HWACONF, HWACONF_REV(val, HWACONF_BASE))))
#define HWACONF_GT(val) (\
	(HWACONF_CHK(val, HWACONF_BASE) && _HWACONF_GT(HWACONF, HWACONF_REV(val, HWACONF_BASE))))
#define HWACONF_LT(val) (\
	(HWACONF_CHK(val, HWACONF_BASE) && _HWACONF_LT(HWACONF, HWACONF_REV(val, HWACONF_BASE))))
#define HWACONF_LE(val) (\
	(HWACONF_CHK(val, HWACONF_BASE) && _HWACONF_LE(HWACONF, HWACONF_REV(val, HWACONF_BASE))))
...
#define HWA_TXPOST_SCHED_COUNT_MAX      255 // size of uint8
...
// SW PacketQueuePager page-in allocation of DBM packets managed by PacketPager
// in private pqplbufpool. Allocated DBM packet has a HBM paired packet
void    hwa_pktpgr_pqplbufpool_fill(struct hwa_dev *dev);
bool    hwa_pktpgr_pqplbufpool_enq(struct hwa_dev *dev, hwa_pp_lbuf_t *pkt);
int     hwa_pqp_pkt_cnt(struct hwa_dev *dev); // pkts available in pqplbufpool
void  * hwa_pqp_pkt_get(struct hwa_dev *dev); // allocate a pkt from pqplbufpool
void    hwa_pqp_pkt_put(struct hwa_dev *dev, void *pkt); // deallocate a pkt
void    hwa_pktpgr_pagemgr_alloc_pqp_pkt_wait_to_finish(struct hwa_dev *dev);
void    hwa_pktpgr_pqplbufpool_rsv_ddbm_set(struct hwa_dev *dev, bool rsv_ddbm);
...
#define HWA_TXFIFO_TXDESC_MAX_PER_MPDU       14 // MAX: 12-in-1 need 13 txdescs
...
// Push pkts to HDBM due to low DDBM resource
void    hwa_pktpgr_txfifo_shadow_push_to_host(struct hwa_dev *dev, int fifo_idx,
	struct spktq *pkt_list, hwa_pp_hdbm_pktq_t *hpktq);

// Pull pkts from HDBM for later page out process
bool    hwa_pktpgr_txfifo_shadow_pull_from_host(struct hwa_dev *dev, int fifo_idx,
	struct spktq *pkt_list, hwa_pp_hdbm_pktq_t *hpktq, bool push_to_host);

// Pagein packets in HW shadow to SW shadow.
bool    hwa_pktpgr_txfifo_shadow_reclaim(struct hwa_dev *dev, uint32 fifo_idx, bool fifo_sync);

// Check TX/AQM status.
bool    hwa_txfifo_descs_isfull(struct hwa_dev *dev, uint fifo, uint16 txdescs, uint16 aqmdescs);
...
void    _hwa_pktpgr_dbm_audit(struct hwa_dev *dev, void *ddbm, bool dir_tx, uint32 type,
            const bool alloc, const char *func, uint32 line);
void    _hwa_dbm_idx_audit(struct hwa_dev *dev, uint32 dbm_idx, bool dir_tx, uint32 type,
            const bool alloc, const char *func, uint32 line);

#define hwa_pktpgr_dbm_audit(dev, ddbm, dir_tx, type, alloc) \
	_hwa_pktpgr_dbm_audit(dev, ddbm, dir_tx, type, alloc, __FUNCTION__, __LINE__)
#define hwa_dbm_idx_audit(dev, dbm_idx, dir_tx, type, alloc) \
	_hwa_dbm_idx_audit(dev, dbm_idx, dir_tx, type, alloc, __FUNCTION__, __LINE__)
#endif
...
extern void hwa_worklet_invoke(struct hwa_dev *dev, uint32 intmask, uint32 pageintmask);
