...
void i5CtlDataModelPrint(char *pBuf)
{
  ...
  for( devNum = 0; devNum < pDmNet->DevicesNumberOfEntries; devNum++) {
    ...
    if (I5_IS_MULTIAP_AGENT(device->flags) && device->dev_info.is_present) {
      printf("DeviceName: %s   Manufacturer: %s   ModelName: %s   ModelNumber: %s   SerialNumber: %s\n",
        device->dev_info.device_name, device->dev_info.manufacturer, device->dev_info.model_name,
        device->dev_info.model_num, device->dev_info.serial_num);
    }
    ...
    for( entryNum = 0; entryNum < device->Ieee1905NeighborNumberOfEntries; entryNum++) {
      ...
      printf("\tMacThroughputCapacity=%d, linkAvailability=%d, txpacketErrors=%d, "
        "transmittedPackets=%d, phyRate=%d, receivedPackets=%d, rxpacketErrors=%d, rssi=%d\n",
