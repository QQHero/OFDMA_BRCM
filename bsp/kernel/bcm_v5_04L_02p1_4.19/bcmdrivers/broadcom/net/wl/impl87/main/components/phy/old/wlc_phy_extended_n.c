 ...
 *
 * $Id: wlc_phy_extended_n.c 789667 2020-08-04 15:08:55Z $
 */
...
phy_n_sample_collect(phy_info_t *pi, wl_samplecollect_args_t *collect, uint32 *buf)
{
	...
	uint16 curval_psm;
	uint16 saved_tsf_clk_frac_l, saved_tsf_clk_frac_h;
	...
	W_REG(pi->sh->osh, D11_PSMCoreCtlStat(pi), (uint16)((1 << 6) | curval_psm));
	...
	W_REG(pi->sh->osh, D11_TSF_CLK_FRAC_L(pi), (uint16)0xCCCC);
	W_REG(pi->sh->osh, D11_TSF_CLK_FRAC_H(pi), (uint16)0XC);
	...
	pi_nphy->pstart = 0;
	W_REG(pi->sh->osh, D11_SampleCollectStartPtr(pi), (uint16)pi_nphy->pstart);
	...
	if (collect->trigger == TRIGGER_NOW) {
		...
		W_REG(pi->sh->osh, D11_SampleCollectStopPtr(pi), (uint16)pi_nphy->pstop);

		/* set Stop bit and Start bit (start capture) */
		phy_ctl = R_REG(pi->sh->osh, D11_PSM_PHY_CTL(pi));
		val = (1 << 4) | (1 << 5);
		W_REG(pi->sh->osh, D11_PSM_PHY_CTL(pi), (uint16)(phy_ctl | val));
...
wlc_phy_noise_home_channel_nphy(phy_info_t *pi, chanspec_t chanspec)
{
	PHY_ACI(("In %s: Resetting OFDM and BPHY desense, chanspec = %X,"
		"home_channel = %X \n", __FUNCTION__,
		CHSPEC_CHANNEL(chanspec), CHSPEC_CHANNEL(pi->interf->home_chanspec)));
