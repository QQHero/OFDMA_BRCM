...
#ifdef GAME_SPEEDUP
static cmd_func_t wl_special_sta, wl_special_fixrate,wl_special_traffic;
#endif
...
static cmd_func_t wl_dfs_reset;
...
cmd_t wl_cmds[] = {
    ...
    "Get dfs status"},
    { "dfs_reset", wl_dfs_reset, -1, WLC_SET_VAR,
    "Clear radar status, clear cac time"},
...
#ifdef GAME_SPEEDUP
    { "special_sta", wl_special_sta, WLC_GET_VAR, WLC_SET_VAR,
    "sta_mac, idlerate_thresh[0], idlerate_thresh[1], rssi_thresh[0], rssi_thresh[1],\n" 
    "utils_thresh[0], utils_thresh[1], timer_windows, default_event_cnt\n"
    },
   { "special_fixrate", wl_special_fixrate, WLC_GET_VAR, WLC_SET_VAR,
   "rspec\n"
   },
    { "special_traffic", wl_special_traffic, WLC_GET_VAR, WLC_SET_VAR,
    "special traffic priority\n"
    },
#endif
    { NULL, NULL, 0, 0, NULL }
};
...
static int
wl_dfs_reset(void *wl, cmd_t *cmd, char **argv)
{
    int ret;

    UNUSED_PARAMETER(argv);
    ret = wlu_var_setbuf(wl, cmd->name, NULL, 0);
...
#ifdef GAME_SPEEDUP
static int 
wl_special_sta(void *wl, cmd_t *cmd, char **argv)
{
    int err=0;
    special_sta_t *params, *reply;

    params = CALLOC(sizeof(special_sta_t));
    if (params == NULL) {
        printf("alloc erron for special_sta\n");
        return BCME_NOMEM;
    }
    memset(params, 0, sizeof(special_sta_t));

    if (!argv[1]) {
        printf("no mac input\n");
        return -1;
    } else {
        if (!wl_ether_atoe(argv[1], &params->ea)) {
    ...
    if (!argv[2]) {
        //get
        if ((err = wlu_iovar_getbuf(wl, cmd->name, params, sizeof(struct ether_addr), buf, WLC_IOCTL_MAXLEN)) < 0) {
            printf("get failed %x:%x:%x:%x:%x:%x\n", params->ea.octet[0],params->ea.octet[1],params->ea.octet[2],params->ea.octet[3],params->ea.octet[4],params->ea.octet[5]);
            return err;
        }
        reply = (special_sta_t *) buf;
        printf("idlerate_thresh=%u, rssi_thres=%d, utils_thresh=%u, timer=%u\n",dtoh32(reply->idlerate_thresh[0]), reply->rssi_thresh[0], dtoh32(reply->utils_thresh[0]), reply->timer_windows);
    } else {
        //set
        if (!argv[9]) {
            ...
            params->action = atoi(argv[2]);
            params->idlerate_thresh[0] = atoi(argv[3]);
            params->idlerate_thresh[1] = atoi(argv[4]);
            params->rssi_thresh[0] = atoi(argv[5]);
            params->rssi_thresh[1] = atoi(argv[6]);
            params->utils_thresh[0] = htod32(atoi(argv[7]));
            params->utils_thresh[1] = htod32(atoi(argv[8]));
            params->timer_windows = atoi(argv[9]);
            if ((err = wlu_iovar_setbuf(wl, cmd->name, params, sizeof(special_sta_t), buf, WLC_IOCTL_MAXLEN)) < 0) {
                printf("set failed\n");
            }
        }
...
static int
wl_special_fixrate(void *wl, cmd_t *cmd, char **argv)
{
    int err = 0;
    uint32 rspec;
    ...
    if (!argv[1]) {
        err = wlu_iovar_get(wl, cmd->name, &rspec, sizeof(uint32));
        rspec = dtoh32(rspec);
        printf("special fix rate: 0x%x\n",rspec);
    } else {
        rspec = htod32(atoi(argv[1]));
        err = wlu_iovar_set(wl, cmd->name, &rspec, sizeof(uint32));
    }
    return err;
}
static int
wl_special_traffic(void *wl, cmd_t *cmd, char **argv)
{
    int err = 0;
    uint32  prio;

    if (!argv[1]) {
        err = wlu_iovar_get(wl, cmd->name, &prio, sizeof(uint32));
        prio = dtoh32(prio);
        printf("special traffic prio: %d\n", prio);
    } else {
        prio = htod32(atoi(argv[1]));
        err = wlu_iovar_set(wl, cmd->name, &prio, sizeof(uint32));
    }
    return err;
}
#endif
