 ...
 *
 * $Id: bcmwpa.c 788597 2020-07-06 06:57:28Z $
 */
...
wpa_find_kde(const uint8 *parse, uint len, uint8 type)
{
	...
	while ((ie = bcm_find_tlv(parse, (int)len, DOT11_MNG_PROPR_ID))) {
...
#ifdef BCMWAPI_WAI
	if (!memcmp(akm, WAPI_OUI, DOT11_OUI_LEN)) {
		switch (akm[DOT11_OUI_LEN]) {
		case RSN_AKM_NONE:
			*auth = WAPI_AUTH_NONE;
			break;
...
bcmwpa_includes_rsn_auth(uint32 auth)
{
	if (auth & (WPA2_AUTH_UNSPECIFIED | WPA2_AUTH_PSK | BRCM_AUTH_PSK | WPA2_AUTH_1X_SHA256 |
		...
		WPA3_AUTH_DPP | WPA3_AUTH_SAE_FBT | WPA_AUTH_OWE | WPA3_AUTH_SUITEB))
		return TRUE;
	else
		return FALSE;
}
