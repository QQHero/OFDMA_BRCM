 ...
 *
 * $Id: wbd_slave_com_hdlr.h 807523 2022-01-24 09:32:25Z $
 */
...
/* Bit fields for wbd_slave_flags NVRAM */
#define WBD_NVRAM_SLAVE_FLAGS_KEEP_FH_BSS_UP	0x0001	/* Keep Fronthaul BSS up upon connection
							 * lost with the controller
							 */

#define WBD_NVRAM_SLAVE_FLAGS_DEF		0x0000 /* Default value for wbd_slave_flags NVRAM */
...
extern int wbd_slave_send_weak_strong_client_cmd(i5_dm_bss_type *i5_bss,
	i5_dm_clients_type *i5_assoc_sta, wbd_sta_status_t command);

/* Send WEAK_CANCEL/STRONG_CANCEL command and Update STA Status = Normal */
extern int wbd_slave_send_weak_strong_cancel_cmd(i5_dm_bss_type *i5_bss,
	i5_dm_clients_type *i5_assoc_sta, wbd_sta_status_t command);
...
extern int wbd_slave_set_chanspec_through_acsd(chanspec_t chspec,
	wl_chan_change_reason_t reason, i5_dm_interface_type *interface);

/* Wait for to Set chanspec through ACSD deamon */
extern void wbd_slave_wait_set_chanspec(i5_dm_interface_type *interface, chanspec_t chspec);
...
/* Issue 'autochannel' command to ACSD daemon for the given interface */
extern int wbd_slave_start_acsd_autochannel(i5_dm_interface_type *pdmif);
...
void wbd_slave_process_channel_scan_req_cb(unsigned char *src_al_mac,
...
void wbd_slave_update_chanspec(i5_dm_interface_type *sifr, chanspec_t chanspec);
...
extern int wbd_slave_send_fbt_config_request_cmd();

/* Enable FBT on this agent */
extern int wbd_slave_set_fbt_on_agent_using_m2(wbd_info_t *info);

/* Remove and deauth STA entry from assoclist */
extern void wbd_slave_remove_and_deauth_sta(char *ifname,
	unsigned char *bssid, unsigned char *sta_mac);

/* FBT Config Response Vendor Message Watchdog timer Callback Function */
extern void wbd_slave_fbt_resp_watchdog_timer_cb();

/* Create FBT Config Response Vendor Message Watchdog Timer */
extern void wbd_slave_create_fbt_resp_watchdog_timer();
#if defined(MULTIAPR2)
/* Process the traffic separation policy. Return 0 If there is any change in the policy So that
 * the caller can restart
 */
extern int wbd_slave_process_traffic_separation_policy(ieee1905_policy_config *policy,
	unsigned char policy_type);

/* Prepare cac completion TLV payload with CAC status on all radios received CAC rqst
 * from controller
 */
extern void wbd_slave_prepare_cac_complete_tlv_payload(uint8 **pbuf, uint32 *payload_len);

/* Process CAC message type from controller */
extern void wbd_slave_process_cac_msg(wbd_info_t *info, uint8 *al_mac, void *msg, uint32 msg_type);

/* prepare cac capability for agent */
extern void wbd_slave_prepare_cac_capability(uint8 **pbuf, uint16 *payload_len);

/* Create OnBoot Channel Scan timer to Perform ONBOOT Channel Scan on this Agent  */
int wbd_slave_create_onboot_channel_scan_timer(wbd_info_t *info);

/* Prepare CAC status payload for all Radio received CAC request from controller  */
extern void wbd_slave_prepare_cac_status_payload(uint8 **pbuf, uint16 *payload_len);

/* Process the backhaul BSS config policy. Return 0 If there is any change in the policy So that
 * the caller can restart
 */
extern int wbd_slave_process_backhaulbss_config_policy(ieee1905_policy_config *policy);
#endif /* MULTIAPR2 */
/* This is to fill the slave item for Profile 1/2 disallowed case BHSTA not associated */
extern void wbd_slave_linkup_fill_interface_info(wbd_slave_item_t *slave);
