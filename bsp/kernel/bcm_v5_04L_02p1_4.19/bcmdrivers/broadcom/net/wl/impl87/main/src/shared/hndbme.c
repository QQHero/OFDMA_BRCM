...
const char * bme_usr_str[BME_USR_MAX] =
	{"SYS", "RLM", "H2D", "D2H", "FD0", "FD1", "SWP", "UN7" };
...
#if defined(BME_MCOPY_BUILD)
		bme->bme_mcopy_align = 2^(burst_len+4);
...
int /* Get the number of availble engines for BME service */
...
int
bme_copy64(osl_t *osh, uint bme_key_id, const uint64 src64, uint64 dst64, uint len)
{
	bme_reg_t bme_reg = { 0 };
	bme_ctx_t *bme_ctx;
	...
	bme_compute_regs(&bme_reg, bme_ctx->src_type, bme_ctx->dst_type,
	            BME_U64HI_U32(src64), BME_U64HI_U32(dst64));
...
bme_mcopy64(osl_t *osh, uint bme_key_id, const uint64 src64, uint64 dst64, uint len)
{
	...
	if (len > BME_MCOPY_THRESH)
	{
		...
		mcopy_len = CEIL(len, engines);
		mcopy_len = ROUNDUP(mcopy_len, bme->bme_mcopy_align);

		bme_sync_usr(osh, bme_ctx->bme_usr);
		/* Single pass over eligible engines */
		for (offset = 0U; offset < len; offset += mcopy_len)
		{
			if ((offset + mcopy_len) > len)
				burst = len - offset; // last burst
			else
