 ...
 *
 * $Id: wlc_hw.c 809675 2022-03-22 05:14:17Z $
 */
...
BCMATTACHFN(wlc_hw_attach)(wlc_info_t *wlc, osl_t *osh, uint unit, uint *err)
{
	...
	if ((wlc_hw->sc_buf_info = MALLOCZ(wlc_hw->osh, sizeof(*wlc_hw->sc_buf_info))) == NULL) {
		WL_ERROR(("wl%d: %s: sc_buf_info allocation failed\n", unit,
			__FUNCTION__));
		goto fail;
	}
...
#ifdef WL_SAMPLE_COLLECT_BMC_ALLOC
	if (wlc_hw->sc_buf_info != NULL) {
		MFREE(osh, wlc_hw->sc_buf_info, sizeof(*wlc_hw->sc_buf_info));
	}
#endif /* WL_SAMPLE_COLLECT_BMC_ALLOC */
...
wlc_hw_update_nfifo(wlc_hw_info_t *wlc_hw)
{
	...
	/* ATE only uses legacy TX FIFOs, so the number of TX FIFOs can be limited
	 * as 6 legacy TX FIFOs for releasing more the free memory
	 */
	wlc_hw->pub->nfifo_total = NFIFO_LEGACY;
#else
	...
	if (D11REV_GE(wlc_hw->corerev, 128) && (HE_DLOMU_ENAB(wlc_hw->wlc->pub) ||
...
uint
wlc_hw_map_txfifo(wlc_hw_info_t *wlc_hw, uint logical_fifo)
{
	uint hnddma_fifo;
	uint su_offset = TX_FIFO_SU_OFFSET(wlc_hw->corerev);

	ASSERT(su_offset != 0);
	ASSERT(logical_fifo < WLC_HW_NFIFO_TOTAL(wlc_hw->wlc));

	hnddma_fifo = (logical_fifo < NFIFO_LEGACY ?
		logical_fifo + su_offset :
		logical_fifo - NFIFO_LEGACY);

	WL_TRACE(("%s: %u->%u\n", __FUNCTION__, logical_fifo, hnddma_fifo));

	return hnddma_fifo;
}

uint
wlc_hw_unmap_txfifo(wlc_hw_info_t *wlc_hw, uint hnddma_fifo)
{
	uint logical_fifo;
	uint su_offset = TX_FIFO_SU_OFFSET(wlc_hw->corerev);

	ASSERT(su_offset != 0);
	ASSERT(hnddma_fifo < WLC_HW_NFIFO_TOTAL(wlc_hw->wlc));

	logical_fifo = (hnddma_fifo >= su_offset ?
		hnddma_fifo - su_offset :
		hnddma_fifo + NFIFO_LEGACY);

	WL_TRACE(("%s: %u->%u\n", __FUNCTION__, hnddma_fifo, logical_fifo));
