 ...
 *
 * $Id: wlc_vht.c 807087 2022-01-12 11:16:36Z $
 */
		...
		chanspec_t ht_chanspec, uint8 ht_ccfs2)
{
	...
	chanspec_t ret_chspec = 0;		/**< chspec that is returned by this function */
	...
	switch (vht_op_ie->chan_width) {
		case VHT_OP_CHAN_WIDTH_80:
			if (ccfs1 == 0) { /* implies 80MHz */
				ret_chspec = wf_channel2chspec(ht_ctl, WL_CHANSPEC_BW_80);
			} else if (ccfs1 == 50 || ccfs1 == 114) {
				ret_chspec = wf_channel2chspec(ht_ctl, WL_CHANSPEC_BW_160);
			} else {
