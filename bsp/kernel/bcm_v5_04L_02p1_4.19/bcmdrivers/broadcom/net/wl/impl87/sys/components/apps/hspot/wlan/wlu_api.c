	...
	uint16 len, uint8 *data)
{
	...
	if ((af_params = (wl_af_params_t *) calloc(1, WL_WIFI_AF_PARAMS_SIZE)) == NULL) {
		return -1;
	}
	...
	if (wl_bssiovar_getint(wl, "chanspec", bsscfg_idx, &chspec) == BCME_OK) {
		chanspec = wl_chspec32_from_driver(chspec);
		af_params->bandtype = htod16(CHSPEC_BANDTYPE(chanspec));
	}
...
wl_vndr_ie_ver(void *wl)
{
	...
	if ((err = wlu_iovar_get(wl, "vndr_ie_ver", buf, sizeof(buf)))) {
		fprintf(stderr, "error on getting vndr_ie_ver , err %d\n", err);
		return ver;
	}
...
static int
wl_mk_ie_setbuf(void* wl, const char *command, uint32 pktflag, int ielen, uchar *data,
	struct ether_addr *sta_mac, vndr_ie_setbuf_t **buf, int *buf_len)
{
	...
	ie_setbuf = (vndr_ie_setbuf_t *) calloc(1, buflen);
	...
	if (ver == 2) {
		...
		ieinfo_v2->pktflag = pktflag;
		ieinfo_v2->vndr_ie_data.id = (uchar)DOT11_MNG_VS_ID;
		ieinfo_v2->vndr_ie_data.len = (uchar) ielen;
		if (sta_mac) {
			memcpy(&ieinfo_v2->sta_addr, sta_mac, sizeof(struct ether_addr));
		}

	} else {
		/* Now, add the IE to the buffer */
		ieinfo_v1->pktflag = pktflag;
		ieinfo_v1->vndr_ie_data.id = (uchar)DOT11_MNG_VS_ID;
		ieinfo_v1->vndr_ie_data.len = (uchar) ielen;
	}

	/* copy OUI */
	memcpy(p_vied_oui, data, VNDR_IE_MIN_LEN);

	/* copy IE data */
	if (datalen > 0) {
		memcpy(p_vied_data, &data[VNDR_IE_MIN_LEN], datalen);
	}
...
wl_vndr_ie(void *wl, int bsscfg_idx, const char *command, uint32 pktflag, int len, uchar *data)
{
	...
	err = wl_mk_ie_setbuf(wl, command, pktflag, len, data, &sta_mac, &ie_setbuf, &buflen);
	if (err) {
		return err;
	}
