 ...
 *
 * $Id: wluc_scan.c 783700 2020-02-06 10:16:19Z $
 */
...
wl_extdscan(void *wl, cmd_t *cmd, char **argv)
{
	...
	params = (wl_extdscan_params_t *)CALLOC(params_size);
	if (params == NULL) {
	...
	while ((p = *argv) != NULL) {
		...
		if (opt == 'b' || !strcmp(key, "band")) {
			if (!str2bandtype(valstr, &params->band, BND_IS_STRING_INC_BAND_ALL)) {
				fprintf(stderr,
				"scan_type value should be \"6G\", \"5G\" "
