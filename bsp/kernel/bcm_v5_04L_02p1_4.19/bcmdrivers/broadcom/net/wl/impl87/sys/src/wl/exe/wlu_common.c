 ...
 *
 * $Id: wlu_common.c 809989 2022-03-29 05:40:48Z $
 */
...
/* Defining MAX clients in WL utility to the max of all configurations */
#define WLU_MAXSCB	512
...
static int wlu_var_getbuf_param_len(void *wl, const char *iovar, void *param, int param_len,
	void **bufptr);
static void wl_dump_ie_buf(void *wl, vndr_ie_buf_t *ie_getbuf);
...
wl_sta_info_print(void *wl, void *buf)
{
	...
	printf("\t flags 0x%x:%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s%s\n",
	       sta->flags,
	       ...
	       (sta->flags & WL_SPP_AMSDU_CAP) ? " SPP_AMSDU" : "",
	       (sta->flags & WL_STA_DTPC_CAP) ? " DTPC" : "");
	...
	if (sta_v8 && sta_v8->len >= STRUCT_SIZE_THROUGH(sta_v8, bands)) {
		wl_bands_print(sta_v8->bands);
	}
    
#ifdef GAME_SPEEDUP
    printf("rts_tx_cnt=%d, rts_failed_cnt=%d, ps_cnt=%d\n",sta_v8->rts_tx_cnt, sta_v8->rts_failed_cnt,sta_v8->ps_cnt);
#endif
...
wl_sta_info_all(void *wl, cmd_t *cmd)
{
	...
	if (err >= 0) {
		...
		if (dtoh16(sta_info_all->version) != WL_STA_INFO_ALL_VER_1) {
			...
			if ((len > WLC_IOCTL_MAXLEN) || (sta_info_all->length > WLC_IOCTL_MAXLEN) ||
				(sta_info_all->length != len)) {
				err = BCME_BADLEN;
			...
			if (sta_info_all->count > WLU_MAXSCB) {
				err = BCME_RANGE;
				printf("err = %d sta_info_all->count %d is not in range [MAX %d]\n",
					err, sta_info_all->count, WLU_MAXSCB);
				return err;
			}
		...
		printf("Cannot get sta_info_all\n");
	}
...
wl_list_ie(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (err == 0) {
		wl_dump_ie_buf(wl, (vndr_ie_buf_t *)ptr);
	} else {
...
static void
wl_dump_ie_buf(void *wl, vndr_ie_buf_t *ie_getbuf)
{
	uchar *iebuf;
	uchar *data;
	int tot_ie, pktflag, iecount, count, datalen, col;
	vndr_ie_info_v1_t *ie_info_v1;
	vndr_ie_info_v2_t *ie_info_v2 = NULL;
	vndr_ie_t *ie;
	uint32 *p_pktflag;
	unsigned int ver;

	ver = wl_vndr_ie_ver(wl);
	printf("wl_dump_ie_buf. ver %d \n", ver);
	...
	for (iecount = 0; iecount < tot_ie; iecount++) {
		if (ver == 2) {
			ie_info_v2 = (vndr_ie_info_v2_t *) iebuf;
			p_pktflag = &ie_info_v2->pktflag;
			ie = &ie_info_v2->vndr_ie_data;
			iebuf += OFFSETOF(vndr_ie_info_v2_t, vndr_ie_data);
		} else {
			ie_info_v1 = (vndr_ie_info_v1_t *) iebuf;
			p_pktflag = &ie_info_v1->pktflag;
			ie = &ie_info_v1->vndr_ie_data;
			iebuf += OFFSETOF(vndr_ie_info_v1_t, vndr_ie_data);
		}
		...
		if (ver >= 2 && !ETHER_ISNULLADDR(&ie_info_v2->sta_addr))
			printf("Sta Addr = %s\n", wl_ether_etoa(&ie_info_v2->sta_addr));
...
int
wl_clr_ie(void *wl, cmd_t *cmd, char * buf, char **argv)
{
	...
	memset(&ie_setbuf, 0, sizeof(vndr_ie_setbuf_t));
	buflen = sizeof(vndr_ie_setbuf_t);
	strncpy(ie_setbuf.cmd, "clr", VNDR_IE_CMD_LEN);
	ie_setbuf.cmd[VNDR_IE_CMD_LEN - 1] = '\0';

	if (bsscfg_idx == -1) {
		err = wlu_var_setbuf(wl, buf, &ie_setbuf, buflen);
	}
	else {
		err = wlu_bssiovar_setbuf(wl, buf, bsscfg_idx,
...
wl_is_he(void *wl)
{
	...
	strncpy(buf, "cap", WLC_IOCTL_MEDLEN);
...
int
wl_sh_ssid(void *wl, cmd_t *cmd, char **argv)
{
	wlc_ssid_t ssid = { 0, {0} };
	int error = BCME_OK;
	...
	if (*argv) {
		if (argv[1] == '\0') {
			fprintf(stderr, "sh_ssid needs an argument to convert, ");
			return BCME_BADARG;
		}
		if (strlen(argv[1]) > DOT11_MAX_SSID_LEN) {
			fprintf(stderr, "SSID arg \"%s\" must be 32 chars or less \n", argv[1]);
			return BCME_BADARG;
		}
		ssid.SSID_len = strlen(argv[1]);
		memcpy(ssid.SSID, argv[1], ssid.SSID_len);
		ssid.SSID_len = htod32(ssid.SSID_len);
		if (cmd->get == WLC_GET_SH_SSID) {
			error = wlu_get(wl, WLC_GET_SH_SSID, &ssid, sizeof(wlc_ssid_t));
		}
	}
	return error;
}
...
int
wl_vndr_ie_ver(void *wl)
{
	int err, ver = 1;

	char buf[256];

	if ((err = wlu_iovar_get(wl, "vndr_ie_ver", buf, sizeof(buf)))) {
		printf("error on getting vndr_ie_ver , err %d\n", err);
		return ver;
	}

	memcpy(&ver, (void *)buf, sizeof(int));
	ver = dtoh32(ver);

	printf(buf, "vndr_id_ver %d\n", ver);
	return ver;
}

/* Allocated memory for *buf* should be freed by caller */
int
wl_mk_ie_setbuf(void* wl, const char *command, uint32 pktflag_ok, char **argv,
	vndr_ie_setbuf_t *ie_setbuf)
{
	vndr_ie_info_v1_t *ieinfo_v1 = NULL;
	vndr_ie_info_v2_t *ieinfo_v2 = NULL;
	/* Vendir IE Sta Specific */
	int is_viesspfc = 0, viess_argv_idx = 5;
	struct ether_addr sta_mac;
	int ver;
	uchar *p_vied_oui = NULL, *p_vied_data = NULL;
	uint32 *p_pktflag = NULL;
	uint32 pktflag;
	int ielen, datalen, iecount;
	int err = 0;
	...
	/* Vendor IE STA Specific dont support IE len is 0, i.e. arg[4] == NULL */
	is_viesspfc = (argv[5] != NULL)? 1:0;
	...
	if ((ver < 2) && is_viesspfc) {
		fprintf(stderr, "Driver not support Vendor IE STA Specific settings.\n");
		return BCME_USAGE_ERROR;
	}

	if (is_viesspfc && !wl_ether_atoe(argv[viess_argv_idx], &sta_mac)) {
	...
	if (ver == 2) {
		ieinfo_v2 = (vndr_ie_info_v2_t *)&ie_setbuf->vndr_ie_buffer.vndr_ie_list[0];
		p_pktflag = &ieinfo_v2->pktflag;
		p_vied_oui = &ieinfo_v2->vndr_ie_data.oui[0];
		p_vied_data = &ieinfo_v2->vndr_ie_data.data[0];
	} else {
		ieinfo_v1 = (vndr_ie_info_v1_t *)&ie_setbuf->vndr_ie_buffer.vndr_ie_list[0];
	...
	if (ver == 2) {
		...
		ieinfo_v2->vndr_ie_data.id = (uchar)DOT11_MNG_VS_ID;
		ieinfo_v2->vndr_ie_data.len = (uchar) ielen;
		memcpy(&ieinfo_v2->sta_addr,
			&sta_mac, sizeof(struct ether_addr));

	} else {
		/* Now, add the IE to the buffer */
		ieinfo_v1->vndr_ie_data.id = (uchar)DOT11_MNG_VS_ID;
		ieinfo_v1->vndr_ie_data.len = (uchar) ielen;
	}
...
int
wl_parse_short_ssid_list(char* list_str, uint32* short_ssid, int max)
{
	char *str, *ptr;
	char* endptr = NULL;
	int idx = 0;
	...
	for (str = list_str; str && (idx < max); str = ptr, idx++) {
		...
		if (strlen(str) > 10) {
			fprintf(stderr, "short ssid <%s> exceeds %d\n", str, 10);
			return -1;
		}

		short_ssid[idx] = (uint32)strtoul(str, &endptr, 16);
	}
	...
	wl_scan_parse_param_t *params, uint16 version)
{
	...
	while ((p = *argv) != NULL) {
		...
		if (version >= 2) {
			...
			if (opt == 'F' || !strcmp(key, "FULLSCAN")) {
				if (val != 0) {
					params->scan_type_2 |= (WLC_SCANFLAGS_FULL >> 17);
				}
			}
		}
		...
		if (opt == 'x' || !strcmp(key, "sh_ssid")) {
			n_short_ssid = wl_parse_short_ssid_list(valstr, params->short_ssid,
					WL_SCAN_PARAMS_SSID_MAX);
			if (n_short_ssid < 0) {
				err = BCME_BADARG;
				goto exit;
	...
	if (n_short_ssid > WL_SCAN_PARAMS_SSID_MAX) {
		fprintf(stderr, "short ssid count %d exceeds max of %d\n",
		        n_short_ssid, WL_SCAN_PARAMS_SSID_MAX);
		err = BCME_BADARG;
		goto exit;
	} else if (n_short_ssid > 0) {
		params->n_short_ssid = n_short_ssid;
	} else {
		params->n_short_ssid = 0;
	}
	...
	wl_scan_parse_param_t *scan_parse_params, uint16 version)
{
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		...
		params_v2->scan_type_2 = htod32(scan_parse_params->scan_type_2);
	}
	...
	n_short_ssid = scan_parse_params->n_short_ssid;
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		params_v2->n_short_ssid = htod32(n_short_ssid);
	}
	...
	/* Copy short_ssid array if applicable */
	if (n_short_ssid > 0) {
		if (version == WL_SCAN_VERSION_MAJOR_V2) {
			i = OFFSETOF(wl_scan_params_v2_t, channel_list) + nchan * sizeof(uint16);
		} else {
		...
		i = ROUNDUP(i, sizeof(uint32));
		if (((i + nssid * sizeof(wlc_ssid_t)) +
			(n_short_ssid * DOT11_SHORT_SSID_LEN)) > (uint)*params_size) {
			fprintf(stderr, "additional short_ssids exceed params_size\n");
			err = BCME_BADARG;
			goto exit;
		}

		p = (char*)params + i + (nssid * sizeof(wlc_ssid_t));
		memcpy(p, scan_parse_params->short_ssid, n_short_ssid * DOT11_SHORT_SSID_LEN);
		p += n_short_ssid * DOT11_SHORT_SSID_LEN;
	} else {
		if (version == 2) {
			p = (char*)params_v2->channel_list + (nchan * sizeof(uint16)) +
	...
	if (version == WL_SCAN_VERSION_MAJOR_V2) {
		*params_size = p - (char*)params_v2;
	} else {
		*params_size = p - (char*)params_v1;
	}
exit:
	return err;
}
...
wl_atoipv6(const char *a, struct ipv6_addr *n)
{
	...
	for (;;) {
		hton16_ua_store((uint16)strtoul((char *)t, &c, 16), &n->addr[i]);
		i += 2;
...
void
wl_bands_print(uint8* bands)
{
	uint8 i;

	printf("Frequency Bands Supported: ");
	if (bands[0] > 3) {
		bands[0] = 3;
	}

	for (i = 1; i <= bands[0]; i++) {
		if (bands[i] == WLC_BAND_6G) {
			printf("6G ");
		} else if (bands[i] == WLC_BAND_5G) {
			printf("5G ");
		} else if (bands[i] == WLC_BAND_2G) {
			printf("2.4G ");
		}
	}
	printf("\n");
