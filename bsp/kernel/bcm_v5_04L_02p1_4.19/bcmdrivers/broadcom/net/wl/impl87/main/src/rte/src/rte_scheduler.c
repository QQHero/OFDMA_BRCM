...
scheduler_execute_next_worklet(hnd_scheduler_t *scheduler)
{
...
#ifdef SCHEDULER_NO_CONTEXT_SWITCH
	...
	/* Lock out interrupts while preparing */
	OSL_DISABLE

	ASSERT(scheduler->executing == NULL);
...
#ifdef SCHEDULER_NO_CONTEXT_SWITCH
	/* Don't allow context switches during worklet execution */
	OSL_PREEMPT_DISABLE
#endif /* SCHEDULER_NO_CONTEXT_SWITCH */

	/* Allow interrupts during worklet execution */
	OSL_RESTORE
	...
	/* Lock out interrupts while updating state */
	OSL_DISABLE
...
#ifdef BCMDBG
	worklet->self_reschedule_count += reschedule;
#endif /* BCMDBG */
...
hnd_worklet_schedule(hnd_worklet_t *worklet)
{
	...
	if (!hnd_worklet_is_pending(worklet)) {
		...
		/* If a new scheduler period has started since this worklet was last
		 * executed, reset its quota.
		 */
		if ((int)(scheduler->period_start - worklet->start) > 0) {
