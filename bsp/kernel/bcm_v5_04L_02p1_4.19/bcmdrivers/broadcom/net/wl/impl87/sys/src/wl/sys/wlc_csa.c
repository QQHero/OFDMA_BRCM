 ...
 *
 * $Id: wlc_csa.c 808957 2022-03-03 13:01:01Z $
 */
...
static void wlc_csa_nulldata_complete(wlc_info_t *wlc, void *pkt, uint txs);
...
BCMATTACHFN(wlc_csa_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_pcb_fn_set(wlc->pcb, 1, WLF2_PCB2_CSA, wlc_csa_nulldata_complete) != BCME_OK) {
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_CHANSPEC_SWITCH): {
		...
		if (csa->chspec == wlc->home_chanspec) {
			WL_ERROR(("wl%d: %s: Reject CSA to current home channel 0x%x\n",
		...
		csa->chspec = wlc_csa_max_bw_chspec(wlc, csa->chspec);
		...
		if (BSSCFG_STA(wlc->primary_bsscfg) && (wlc->primary_bsscfg->associated)) {
			if (BSSCFG_STA(bsscfg)) {
				 ...
				 */
				wlc_send_unicast_action_switch_channel(csam, bsscfg, &bsscfg->BSSID,
		...
		/* return err(BCME_NOTUP) to applications when bss is in down state */
		if (!is_any_bss_up) {
			return BCME_NOTUP;
		}
		break;
...
static void
wlc_csa_nulldata_complete(wlc_info_t *wlc, void *pkt, uint txs)
{
	struct scb *scb;
	wlc_csa_info_t *csam = wlc->csa;
	...
	csa_scb = CSA_SCB_CUBBY(csam, scb);
	ASSERT(csa_scb != NULL);
	ASSERT(csa_scb->train_cnt);
	csa_scb->train_cnt--;
}

static int
wlc_csa_sendnulldata_cb(wlc_info_t *wlc, wlc_bsscfg_t *cfg, void *pkt, void *data)
{
	BCM_REFERENCE(wlc);
	BCM_REFERENCE(data);
	BCM_REFERENCE(cfg);
	...
	WLF2_PCB2_REG(pkt, WLF2_PCB2_CSA);

	return BCME_OK;
}

static int
wlc_csa_train_txpower_sendpkt(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg, struct scb *scb)
{
	wlc_csa_scb_cubby_t *csa_scb = CSA_SCB_CUBBY(csam, scb);
	ASSERT(csa_scb != NULL);

	if (csa_scb->train_cnt >= WL_CS_TRAIN_TXPOWER_PKTSNUM) {
		return BCME_NORESOURCE;
	}

	if (wlc_sendnulldata(cfg->wlc, cfg, &scb->ea, 0, 0, PRIO_8021D_VO,
		wlc_csa_sendnulldata_cb, NULL)) {
		csa_scb->train_cnt++;
		return BCME_OK;
	}
...
wlc_csa_train_txpower(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_STA(cfg)) {
		if (!BSSCFG_PSTA(cfg) && cfg->BSS) {
			while (wlc_sendnulldata(wlc, cfg, &cfg->BSSID, 0, 0, PRIO_8021D_VO,
				NULL, NULL) && --count);
		}
		...
		do {
			...
			FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
				if (SCB_ASSOCIATED(scb) || SCB_LEGACY_WDS(scb)) {
					ret = wlc_csa_train_txpower_sendpkt(csam, cfg, scb);
					if (ret == BCME_ERROR) {
						can_again = FALSE;
					} else if (ret == BCME_OK) {
						can_again = (--count != 0);
		...
		} while (can_again && (start_count != count));
	}
}
#else
...
wlc_csa_timeout(void *arg)
{
	...
	if (wlc_dfs_cac_in_progress(wlc->dfs)) {
		WL_DFS(("wl%d %s CAC is in progress. Aborting CAC and process CSA\n",
...
#ifdef STA
	if (cfg->roam && BSSCFG_STA(cfg)) {
		cfg->roam->recvd_curr_bss_bcn = FALSE;
	}
...
wlc_csa_process_channel_switch(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg)
{
	...
	if (BSSCFG_STA(cfg) && SCAN_IN_PROGRESS(wlc->scan)) {
		WL_REGULATORY(("wl%d:%s: Avoid CSA when Scan is in progress\n",
			wlc->pub->unit, __FUNCTION__));
		return;
	}
	...
	cfg->ap_orig_chanspec = csa->csa.chspec;
	/* add 160 and 80+80 */
	...
	*/
	...
	if (!wlc_valid_chanspec_db(wlc->cmi, csa->csa.chspec)) {
		...
		csa->csa.chspec = wlc_next_chanspec(
					wlc->cmi,
					CH20MHZ_CHSPEC(CH_MAX_2G_CHANNEL + 1, WL_CHANSPEC_BAND_2G),
					CHAN_TYPE_CHATTY, 0);
		...
		uint8 *radarch_len, uint8 **radarch)
{
	...
	for (i = 0; i < *radarch_len; i++) {
		...
		/* Only expecting 5GHz channels on which radar is detected,
		 * so no need for 6GHz support.
		 */
	...
	struct dot11_management_header *hdr, uint8 *body, int body_len)
{
	...
	chspec = wlc_csa_max_bw_chspec(wlc, chspec);
	...
	if (BSSCFG_AP(cfg)) {
		...
		/* prepare csa info */
		wlc_csa_prepare_csa_info(wlc, wlc->primary_bsscfg, chspec,
			csa_ie->mode, CSA_BROADCAST_ACTION_FRAME, &csa);
	...
	wl_chan_switch_t *csa, uint8 radarch_len, uint8 *radarch)
{
	...
	wlc_dfs_send_action_frame(wlc, cfg);
	...
	chanspec_t chanspec, uint8 mode, uint8 count, uint8 reg_class, uint8 frame_type)
{
	...
	if (BSSCFG_IS_CSA_IN_PROGRESS(cfg)) {
		WL_REGULATORY(("wl%d.%d: %s Previous CSA is in progress\n", WLCWLUNIT(wlc),
	...
	if (csa->csa.frame_type == CSA_UNICAST_ACTION_FRAME) {
		...
		FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
			...
			if ((SCB_ASSOCIATED(scb) || SCB_LEGACY_WDS(scb)) && !SCB_ISMULTI(scb) &&
				(psta_prim == NULL)) {
				bcmerror = wlc_send_unicast_action_switch_channel(csam, cfg,
					&scb->ea, &csa->csa, DOT11_SM_ACTION_CHANNEL_SWITCH, 0,
					NULL);
		...
		wlc_send_unicast_action_switch_channel(csam, cfg, &ether_bcast,
...
wlc_csa_count_down(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg)
{
	...
	/* Is the last beacon (with count 1) about to be sent out? If so, switch after that
	 * beacon being transmitted.
	 */
	if (csa->csa.count == 0) {
...
wlc_csa_do_csa(wlc_csa_info_t *csam, wlc_bsscfg_t *cfg, wl_chan_switch_t *cs, bool docs)
{
	...
	if (cs->chspec == wlc->home_chanspec) {
		WL_REGULATORY(("wl%d: %s: Reject CSA to current home channel 0x%x\n",
			wlc->pub->unit, __FUNCTION__, cs->chspec));
		return;
	}
...
/*
 * wlc_csa_each_up_ap - announce CSA on each AP interface that is up
 ...
 */
void
wlc_csa_each_up_ap(wlc_info_t *wlc, wl_chan_switch_t *csa, bool docs)
{
	wlc_bsscfg_t *apcfg;
	int idx;
	...
	if (WLC_IS_CSA_TIMER_IN_PROGRESS(wlc) ||
		...
		WL_CSA(("wl%d: already CSA timer running, not required to send CSA per ap cfg \n",
			wlc->pub->unit));
		return;
	}

	if (csa->chspec == wlc->home_chanspec) {
		WL_CSA(("wl%d: %s: Reject CSA to current home channel 0x%x\n",
			wlc->pub->unit, __FUNCTION__, csa->chspec));
		return;
	}

	WL_CSA(("wl%d: Announcing CSA to chanspec 0x%x on all up APs\n",
...
/*
 * wlc_csa_reset_each_ap - reset CSA on each AP interface
 * wlc - handle to the wl interface
 */
void
wlc_csa_reset_each_ap(wlc_info_t *wlc)
{
	wlc_bsscfg_t *apcfg;
	int idx;
	WL_CSA(("wl%d: Reset CSA on all APs\n", wlc->pub->unit));
...
/* Get the max allowable chanspec. This considers BW and other limitations */
chanspec_t
wlc_csa_max_bw_chspec(wlc_info_t *wlc, chanspec_t org_chspec)
{
	chanspec_t chanspec = org_chspec;

	/* Update chanspec as per max allowable BW */
	if (CHSPEC_IS160(chanspec) && !VALID_160CHANSPEC(wlc, chanspec)) {
		chanspec = wf_chspec_primary80_chspec(chanspec);
	}
	if (CHSPEC_IS80(chanspec) && !VALID_80CHANSPEC(wlc, chanspec)) {
		chanspec = wf_chspec_primary40_chspec(chanspec);
	}
	if (CHSPEC_IS40(chanspec) && !WL_BW_CAP_40MHZ(wlc->band->bw_cap)) {
		chanspec = wf_chspec_primary20_chspec(chanspec);
	}
	if (org_chspec != chanspec) {
		WL_REGULATORY(("wl%d: %s: chanspec modified from 0x%04X to 0x%04X "
				"due to BW limits bw_cap 0x%X\n", wlc->pub->unit,
				__FUNCTION__, org_chspec, chanspec, wlc->band->bw_cap));
	}
	return chanspec;
}
