 ...
 *
 * $Id: wluc_he.c 807215 2022-01-14 08:10:13Z $
 */
...
static sub_cmd_func_t wl_he_cmd_sr;
static sub_cmd_func_t wl_he_cmd_color_event;
...
static const sub_cmd_t he_cmd_list[] = {
	...
	{"sr", WL_HE_CMD_SR, IOVT_BUFFER, wl_he_cmd_sr},
	{"color_collision", WL_HE_CMD_COLOR_COLLISION, IOVT_UINT8, wl_he_cmd_uint},
	{"trigger_color_event", WL_HE_CMD_TRIGGER_COLOR_EVENT, IOVT_BUFFER, wl_he_cmd_color_event},
	{NULL, 0, 0, NULL}
};
...
static int
wl_he_cmd_sr(void *wl, const cmd_t *cmd, const sub_cmd_t *sub, char **argv)
{
	int res = BCME_OK;
	uint8 mybuf[64];
	uint8 *rem = mybuf;
	...
	if (*argv == NULL) { /* get */
		...
		if (sr->enabled) {
			...
			if (sr->src & SRC_PSR_DIS) {
				printf("\t\t- PSR Disallowed (is SET)\n");
			}
			if (sr->src & SRC_NON_SRG_OBSS_PD_SR_DIS) {
				printf("\t\t- Non-SRG OBSS PD SR Disallowed (is SET)\n");
			}
			if (sr->src & SRC_NON_SRG_OFFSET_PRESENT) {
				printf("\t\t- Non-SRG Offset Present (is SET)\n");
				printf("\t\t\tdata 0x%02x\n", sr->non_srg_offset);
			}
			if (sr->src & SRC_SRG_INFORMATION_PRESENT) {
				printf("\t\t- SRG Information Present (is SET)\n");
				printf("\t\t\tdata 0x%02x:0x%02x:0x%02x:0x%02x"
					" 0x%02x:0x%02x:0x%02x:0x%02x\n",
					sr->srg[0], sr->srg[1], sr->srg[2], sr->srg[3],
					sr->srg[4], sr->srg[5], sr->srg[6], sr->srg[7]);
				printf("\t\t\t     0x%02x:0x%02x:0x%02x:0x%02x"
					" 0x%02x:0x%02x:0x%02x:0x%02x\n",
					sr->srg[8], sr->srg[9], sr->srg[10], sr->srg[11],
					sr->srg[12], sr->srg[13], sr->srg[14], sr->srg[15]);
				printf("\t\t\t     0x%02x:0x%02x\n",
					sr->srg[16], sr->srg[17]);
			}
			if (sr->src & SRC_HESIGA_SPATIAL_REUSE_VALUE15_ALLOWED) {
				printf("\t\t- HESIGA_Spatial_reuse_value15_allowed (is SET)\n");
			}
		...
		memset(&set_sr, 0, sizeof(set_sr));
		set_sr.version = WL_HE_SR_VER_1;
		set_sr.length = sizeof(set_sr) - 4;
		while (*argv != NULL) {
			if ((*argv)[0] == '-') {
				command = (*argv)[1];
				argv++;
				...
				if (command == 'p') {
					value = strtol(*argv, NULL, 0);
					if ((value < 0) || (value > 1)) {
						fprintf(stderr, "invalid value PSR disallowed:%d\n",
							value);
						res = BCME_BADARG;
						goto fail;
					}
					set_sr.src |= value ? SRC_PSR_DIS : 0;
					set_sr.enabled = 1;
				}
				if (command == 'o') {
					value = strtol(*argv, NULL, 0);
					if ((value < 0) || (value > 1)) {
						fprintf(stderr, "invalid value Non-SRG OBSS PD"
							" SR Disallowed:%d\n", value);
						res = BCME_BADARG;
						goto fail;
					}
					set_sr.src |= value ? SRC_NON_SRG_OBSS_PD_SR_DIS : 0;
					set_sr.enabled = 1;
				}
				if (command == 'n') {
					value = strtol(*argv, NULL, 0);
					if ((value < 0) || (value > 1)) {
						fprintf(stderr, "invalid value Non-SRG Offset"
							" Present:%d\n", value);
						res = BCME_BADARG;
						goto fail;
					}
					...
					set_sr.src |= value ? SRC_NON_SRG_OFFSET_PRESENT : 0;
					set_sr.enabled = 1;
					value = strtol(*argv, NULL, 0);
					set_sr.non_srg_offset = (uint8)value;
				}
				if (command == 's') {
					value = strtol(*argv, NULL, 0);
					if ((value < 0) || (value > 1)) {
						fprintf(stderr, "invalid value SRG Information"
							"Present:%d\n", value);
						res = BCME_BADARG;
						goto fail;
					}
					set_sr.src |= value ? SRC_SRG_INFORMATION_PRESENT : 0;
					set_sr.enabled = 1;
					for (i = 0; i < HE_SR_SRG_INFO_LEN; i++) {
						argv++;
						if (*argv == NULL) {
							res = BCME_USAGE_ERROR;
							goto fail;
						...
						value = strtol(*argv, NULL, 0);
						set_sr.srg[i] = (uint8)value;
					}
				}
				if (command == 'h') {
					value = strtol(*argv, NULL, 0);
					if ((value < 0) || (value > 1)) {
						fprintf(stderr, "invalid value  HESIGA_Spatial_"
							"reuse_value15_allowed:%d\n", value);
						res = BCME_BADARG;
						goto fail;
					}
					set_sr.src |= value ?
						SRC_HESIGA_SPATIAL_REUSE_VALUE15_ALLOWED : 0;
					set_sr.enabled = 1;
				}
			} else {
				res = BCME_USAGE_ERROR;
...
complete:
		rem = mybuf;
		rem_len = sizeof(mybuf);
		res = bcm_pack_xtlv_entry(&rem, &rem_len, sub->id, sizeof(set_sr),
			(uint8 *)&set_sr, BCM_XTLV_OPTION_ALIGN32);
		if (res != BCME_OK) {
			goto fail;
		}
		res = wlu_var_setbuf(wl, cmd->name, mybuf, sizeof(mybuf) - rem_len);
...
static int
wl_he_cmd_color_event(void *wl, const cmd_t *cmd, const sub_cmd_t *sub, char **argv)
{
	int res;
	/* always use get to trigger event and print data. Ignore set */
	...
	memset(&getbuf, 0, sizeof(getbuf));
	getbuf.id = sub->id;
	getbuf.len = sizeof(*color);

	res = wlu_var_getbuf_sm(wl, cmd->name, &getbuf, sizeof(getbuf), (void **)&color);

	/* check the response buff  */
	if ((res != BCME_OK) || (color == NULL)) {
		goto fail;
	}
	...
	if (color->collision & WL_COLOR_COLLISION_DETECT_LOCAL) {
		printf("by local AP");
		if (color->collision & WL_COLOR_COLLISION_DETECT_REMOTE) {
			printf(" and ");
		}
	}
	if (color->collision & WL_COLOR_COLLISION_DETECT_REMOTE) {
		printf("by remote STA");
	}
	if (!color->collision) {
		printf("No");
	}
