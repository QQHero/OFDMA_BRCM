...
wlc_defrag_first_frag_body_get(wlc_info_t* wlc, struct scb *scb, uint8 prio)
{
	...
	if (scb && scb->fragbuf[prio] &&
		scb->fragbufbodyoffset[prio] != 0xFFu) {
		body = (uint8*)PKTDATA(wlc->osh, scb->fragbuf[prio]);
		body += scb->fragbufbodyoffset[prio];
	}
	...
	if (!f->ismulti) {
		...
		if ((f->seq & FRAGNUM_MASK) == 0) {
			...
			if (more_frag) {
				...
				/* Save offset from first fragment */
				if (f->fc & FC_WEP) {
					scb->fragbufbodyoffset[prio] =
						f->pbody - PKTDATA(scb->bsscfg->wlc->osh, f->p);
				} else {
					scb->fragbufbodyoffset[prio] = 0xFFu;
				}
	...
	wlc_d11rxhdr_t *wrxh, struct scb **pscb, int len)
{
	...
	case FC_TYPE_MNG:
		...
		case FC_ACTION:
			...
			if (wlc_is_publicaction(wlc, h, len, scb, bsscfg)) {
				goto done;
			}
			/* Fall Through */
		case FC_PS_POLL:
...
static bool
wlc_is_publicaction(wlc_info_t * wlc, struct dot11_header *hdr, int len, struct scb *scb,
	wlc_bsscfg_t *bsscfg)
{
	uint8 cat;
	uint8 act;
			...
			*/
			...
			if (SCB_MFP(scb)) {
				if (cat == DOT11_ACTION_CAT_PUBLIC) {
					...
					if (wlc_is_protected_dual_publicaction(act)) {
