# Makefile for hndrte based 6715b0 full ram Image
#
...
TARGETS		:= \
	...
	pcie-p2p-mchan-idauth-idsup-pno-aoe-noe-ndoe-pktfilter-pf2-keepalive-toe-ccx-mfp-anqpo-p2po-wl11k-wl11u-wnm-relmcast-txbf-fbt-tdls-sr-pktctx-amsdutx \
	pcie-fdap-mbss-mfp-wl11k-wl11u-wnm-txbf-pktctx-amsdutx-ampduretry-proptxstatus-chkd2hdma-11nprop-obss-dbwsw-ringer-bgdfs-cevent-hostpmac-murx
...
WLCONFFILE	:= wlconfig_rte_6715b0_dev
WLTUNEFILE	:= wltunable_rte_6715b0.h
...
# 6715b0 doesn't support CPU clock change on-the-fly. Need to change CPU clock from ALP.
EXTRA_DFLAGS	+= -DNO_ONTHEFLY_FREQ_CHANGE

H2D_DMAQ_LEN	:= 256
D2H_DMAQ_LEN	:= 256

ifneq ($(filter 1, $(call opt,hwaall) $(call opt,hwapp)),)
PCIE_H2D_NTXD	:=32
PCIE_H2D_NRXD	:=32
PCIE_D2H_NTXD	:=64
PCIE_D2H_NRXD	:=64
...
ifeq ($(filter 1, $(call opt,hwaall) $(call opt,hwapp)),)
	WL_NRXD         := 1024
	WL_NRXBUFPOST   := 512

	EXTRA_DFLAGS	+= -DWATCHDOG_DISABLE_FOR_MFGTEST
else
	WL_NRXD         := 512
	WL_NRXBUFPOST   := 256
endif
...
ifeq ($(call opt,hwa),1)
	# HWA Core Rev 133
	EXTRA_DFLAGS    += -DHWACONF=0x00000020
...
ifeq ($(call opt,hwapp),1)
	...
	# Enlarge WL_NTXD_LFRAG to 2048 in PP mode
	WL_NTXD_LFRAG   := 2048
else
...
# Enable Packet Queue Pager service. Support paging of 256 queue contexts
# PQP support limited to HWA/PP compatible Packet format hwa_pp_lbuf.h
ifeq ($(call opt,hwapp),1)
ifeq ($(call opt,pqp),1)
	HNDPQP := 256
	PROP_TXSTATUS   := 0
	EXTRA_DFLAGS    += -DWL_USE_SUPR_PSQ
	EXTRA_DFLAGS	+= -DPQP_HEAP_BUFFER_LEN=8
endif # Packet Queue Pager
endif # Packet Pager lbuf format
...
ifeq ($(call opt,mfgtest),1)
	# Set console log buf size to 32KB for internal/mfgtest build
	LOG_BUF_LEN	:= 32768
else
	# Set console log buf size to 16KB for external build
	LOG_BUF_LEN	:= 16384
endif
endif
...
# Support fast delete flow-ring
EXTRA_DFLAGS    += -DPCIEDEV_FAST_DELETE_RING
...
# Do not enable the Event pool for 6715
...
EXTRA_DFLAGS	+= -DCYCLES_PER_USEC=1600
endif
...
ifeq ($(call opt,mbx),1)
# Set Tx/PhyRx Status Ring depth
EXTRA_DFLAGS	+= -DTX_STATUS_MACTXS_RING_DEPTH=512
EXTRA_DFLAGS	+= -DPHYRX_STATUS_RING_DEPTH=512
endif
