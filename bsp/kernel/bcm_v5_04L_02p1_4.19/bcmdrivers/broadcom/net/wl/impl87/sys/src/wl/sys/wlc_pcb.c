 ...
 *
 * $Id: wlc_pcb.c 804003 2021-10-15 10:06:09Z $
 */
...
BCMATTACHFN(wlc_pcb_detach)(wlc_pcb_info_t *pcbi)
{
	...
	for (i = 1; i <= pcbi->maxpktcb; i++) {
		pkt_cb_t *cb = &pcbi->pkt_callback[i];

		if (cb->fn != NULL) {
			WL_ERROR(("wl%d: %s: pending callback idx=%d fn=" FPTR_FMT "\n",
				wlc->pub->unit, __FUNCTION__, i, OSL_OBFUSCATE_BUF(cb->fn)));
			ASSERT(0);
		}
	}
#endif
