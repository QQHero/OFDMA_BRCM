 ...
 *
 * $Id: wlc_act_frame.c 802654 2021-09-02 09:40:49Z $
 */
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_AF): {
		...
		if (af->channel != 0 && af->bandtype == 0) {
			...
			af->bandtype = WL_CHANNEL_2G5G_BANDTYPE(af->channel);
		}
...
wlc_actionframetx_complete(wlc_info_t *wlc, void *pkt, uint txstatus)
{
	...
	if (actframe_cubby->action_frame || wlc_act_frame_isonchan(wlc, cfg)) {
		...
		if (status == WLC_E_STATUS_NO_ACK) {
			...
			/* Don't abort for broadcast action frame as
			 * there will not be an ack for such frames.
			 */
	...
	wl_af_params_t *af, bool *do_event)
{
	...
	err = wlc_scan_request(wlc, DOT11_BSSTYPE_ANY, &af->BSSID,
			1, &ssid, 0, NULL, -1, 1, af->dwell_time, af->dwell_time,
