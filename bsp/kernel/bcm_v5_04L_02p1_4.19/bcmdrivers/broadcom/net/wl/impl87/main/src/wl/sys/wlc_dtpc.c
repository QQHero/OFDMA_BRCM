 ...
 *
 * $Id: wlc_dtpc.c 807051 2022-01-12 00:17:38Z $
 *
 *
 * @file wlc_dtpc.c
 * @brief
...
dtpc_is_bfm_pkt(wlc_dtpc_info_t *dtpci, tx_status_t *txs)
{
	...
	if ((TX_STATUS_MACTXS_S5(txs) & TX_STATUS40_IMPBF_MASK) ||
		(TX_STATUS_MACTXS_S5(txs) & TX_STATUS40_BFTX)) {
		ret = TRUE;
	}
	return ret;
}
	...
	void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	switch (actionid) {
		...
		case IOV_GVAL(IOV_DTPC_HEADROOM):
		{
			...
			if (bsscfg == NULL || bsscfg->current_bss == NULL) {
				WL_ERROR(("bsscfg pointer null access error\n"));
				return BCME_ERROR;
			}
