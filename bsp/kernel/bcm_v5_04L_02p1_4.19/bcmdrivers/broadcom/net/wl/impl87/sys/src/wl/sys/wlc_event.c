 ...
 *
 * $Id: wlc_event.c 798923 2021-05-17 03:49:08Z $
 */
...
BCMATTACHFN(wlc_eventq_detach)(wlc_eventq_t *eq)
{
	...
	if (eq->superevd) {
		MFREE(wlc->osh, eq->superevd, wlc->pub->tunables->evpool_maxdata);
	}
	if (eq->superev) {
		MFREE(wlc->osh, eq->superev, sizeof(wlc_event_t));
	}
	pktpool_deinit(wlc->osh, &eq->superpool);

	for (n = 0; n < wlc->pub->tunables->evpool_size; n++) {
		if (eq->evpred && eq->evpred[n]) {
			MFREE(wlc->osh, eq->evpred[n], wlc->pub->tunables->evpool_maxdata);
		}
		if (eq->evpre && eq->evpre[n]) {
			MFREE(wlc->osh, eq->evpre[n], sizeof(wlc_event_t));
		}
	}
	pktpool_deinit(wlc->osh, &eq->evpool);

	if (eq->evpred) {
		MFREE(wlc->osh, eq->evpred, wlc->pub->tunables->evpool_size * sizeof(*eq->evpred));
	}
	if (eq->evpre) {
		MFREE(wlc->osh, eq->evpre, wlc->pub->tunables->evpool_size * sizeof(*eq->evpre));
	}
#endif /*  BCMPKTPOOL */
...
wlc_event_alloc(wlc_eventq_t *eq, uint eventid)
{
	...
	if (e == NULL) {
		...
		if (SUPERCRITICAL(eventid) && (eq->supermsk & SUPEREV)) {
			eq->supermsk &= ~SUPEREV;
			e = eq->superev;
		} else if (eq->evpre_msk && wlc_eventq_test_evpool_mask(eq, eventid)) {
			uint num, msk;
	...
	ASSERT(e->event.event_type < WLC_E_LAST);
	return e;
}
...
wlc_eventq_enq(wlc_eventq_t *eq, wlc_event_t *e)
{
	...
	/* process event immediately if interface in down process. */
	if (wlc->state == WLC_STATE_DOWN) {
		_wlc_eventq_process(eq);
	} else if (!eq->tpending) {
		eq->tpending = TRUE;
