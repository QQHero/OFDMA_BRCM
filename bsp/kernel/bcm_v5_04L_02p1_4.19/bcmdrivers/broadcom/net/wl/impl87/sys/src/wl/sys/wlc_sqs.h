 ...
 *
 * $Id: wlc_sqs.h 803989 2021-10-14 19:10:00Z $
 */
...
typedef uint16 (*pkt_pull_cb_t)(void *arg, uint16 ringid, uint16 request_cnt, bool sqs_force);
...
typedef struct scb_sqs {
	...
	uint16			v_pkts_tot;		/**< V pkts across tids */
	uint16			v2r_pkts_tot;		/**< V pkts across tids */
} scb_sqs_t;
...
#define SCB_SQS_V_PKTS_TOT(hdl)	((hdl)->v_pkts_tot)
#define SCB_SQS_V2R_PKTS_TOT(hdl)	((hdl)->v2r_pkts_tot)
...
extern uint16 wlc_sqs_vpkts_tot(struct scb* scb);
extern uint16 wlc_sqs_vpkts_multi_ac(struct scb* scb, uint8 tid_bmp);
extern uint16 wlc_sqs_v2r_pkts(uint16 cfp_flowid, uint8 prio);
extern uint16 wlc_sqs_v2r_pkts_tot(struct scb* scb);
extern uint16 wlc_sqs_v2r_pkts_multi_ac(struct scb* scb, uint8 tid_bmp);
...
uint16 wlc_sqs_taf_get_scb_tid_pkts(void* sqsh, void *scbh, void *tidh, uint32 ts);
bool wlc_sqs_taf_release(void* sqsh, void* scbh, void* tidh, bool force,
	taf_scheduler_public_t* taf);
#endif /* WLTAF */
extern uint16 wlc_sqs_psmode_pull_packets(wlc_info_t* wlc, struct scb* scb,
	uint8 ac_bitmap, uint16 request_cnt);
#endif /* _WLC_SQS_H_ */
