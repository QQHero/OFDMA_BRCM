 ...
 *
 * $Id: acsd_cli.c 809799 2022-03-23 21:05:18Z $
 */
...
typedef struct acs_cli_info {
	...
	char cmd_buf[ACSD_BUFSIZE_DEFAULT]; /* Server command and response buffer */
} acs_cli_info_t;
...
static int do_dump_scan_results(acs_cli_info_t *, int);
...
static const dispatch_table_t dump_table[] = {
	...
	{ "scanresults", do_dump_scan_results,	"ACSD Scan results" },
	{ NULL, NULL }
};
...
acsd_variables[] = {	/* Sorted alphabetically for nicer help display */
	...
	"acs_chanim_poll", /* chanim polling interval */
	"acs_chanim_intf_force_cs_scan", /* Force CS scan when interference detected */
	"acs_channel_weights", /* scoring weights, per channel, total score * channel_weight */
	"acs_excl_chans", /* set/get exclude chans list */
	"acs_bgdfs", /* set/get BGDFS state, enable/disable */
	"acs_bgdfs_preclear_etsi", /* set/get ETSI preclearing channels, enable/disable */
	"acs_allow_uncleared_dfs_ch", /* set/get allow uncleared DFS channel, enable/disable */
	NULL
};
...
{
	printf("%10s %8s %3s %3s ", "Channel", "(Chspec)", "Use", "DFS");
...
acs_dump_cscore_body(ch_candidate_t * candi)
{
	...
	printf("%-10s (0x%04x)  %3s %s", wf_chspec_ntoa(candi->chspec, buf), candi->chspec,
		(candi->is_dfs) ? "dfs" : " - ",
...
static void
display_scan_results(acs_bss_info_sm_t *bsm)
{
...
static int
do_dump_scan_results(acs_cli_info_t *ctx, int rcount)
{
	acs_bss_info_entry_t *curptr = (acs_bss_info_entry_t *)ctx->cmd_buf;
	int i = 1, len = 0, scan_info_len, bss_count = 0;
	scan_info_len = sizeof(acs_bss_info_sm_t);

	while (curptr) {
		if ((len + scan_info_len) > rcount) {
			break;
		}
		display_scan_results(&curptr->binfo_local);
		printf("timestamp: %u\n", (uint32)curptr->binfo_local.timestamp);
		curptr = (acs_bss_info_entry_t *)(ctx->cmd_buf + (i++ * scan_info_len));
		len = len + scan_info_len;
		bss_count ++;
	}
	printf("%s ifname:%s bss count is:%d\n", __func__, ctx->wl_ifname, bss_count);
	return BCME_OK;
}
...
{
	static const char *trig_str[ACS_MAX] = {
		"INIT", "IOCTL", "CHANIM", "TIMER", "SIGN-CHANGE",
		"TXOP", "NONACS", "DFS-REENTRY", "TXFAIL", "ZDFS", "EDCRS_HI"
	};
	return (trigger < ACS_MAX) ? trig_str[trigger] : "(invalid)";
}

static int
do_dump_acs_record(acs_cli_info_t *ctx, int rcount)
{
	acs_record_t *result = (acs_record_t *) ctx->cmd_buf;
	int i;
	int count;
	char buf[CHANSPEC_STR_LEN];

	count = rcount / sizeof(acs_record_t);
	...
	printf("Ticks(s)  Timestamp(s)  ACS-Trigger  ChanSpec    BG-Noise  CCA-Count  Idle\n");
	for (i = 0; i < count; i++) {
		time_t ltime = ntohl(result->timestamp);
		uint32 ticks = ntohl(result->ticks);
		printf("%8u  %12u  %11s  %-10s  %8d  %9d  %d\n",
			ticks, (uint32)ltime,
...
create_server_command(acs_cli_info_t *ctx)
{
	...
	len = acs_snprintf(ctx->cmd_buf, ACSD_BUFSIZE_DEFAULT, "%s&ifname=%s",
		ctx->cmdarg[CMDARG_COMMAND], ctx->wl_ifname);

	if (ctx->cmdarg[CMDARG_PARAMETER][0]) {
		len += acs_snprintf(ctx->cmd_buf+len, ACSD_BUFSIZE_DEFAULT - len,
...
do_command_response(acs_cli_info_t *ctx)
{
	...
	len = sread(ctx->socket, ctx->cmd_buf, ACSD_BUFSIZE_DEFAULT-1);
	if (len < 0) {
