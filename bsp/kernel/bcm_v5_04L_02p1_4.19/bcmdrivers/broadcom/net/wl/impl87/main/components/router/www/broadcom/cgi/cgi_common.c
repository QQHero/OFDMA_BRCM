 ...
 * $Id: cgi_common.c 802420 2021-08-26 07:02:42Z $
 */
...
static int ej_dpp_process(int eid, webs_t wp, int argc, char_t **argv);
...
ej_dpp_display(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	websWrite(wp, "<tr>"
	          "<th width=\"310\""
	          "onMouseOver=\"return overlib('DPP Status', LEFT);\""
	          "onMouseOut=\"return nd();\">"
	          "DPP Status:&nbsp;&nbsp;"
	          "</th>"
	          "<td>&nbsp;&nbsp;</td>\n"
	...
	if (!dpp_sta)
		ej_dpp_process(eid, wp, argc, argv);
...
static int
ej_dpp_refresh(int eid, webs_t wp, int argc, char_t **argv)
{
	int interval = 5;
	char *value = nvram_get("dpp_refresh_interval");
...
#define CHANIMSTR(a, b, c, d) ((a) ? ((b) ? c : d) : "")
	...
	if ((phytype != WLC_PHY_TYPE_N) && (phytype != WLC_PHY_TYPE_SSN) &&
		...
		/* obsolete IOV GET CHANNEL is used which does not support 6GHz channels */
		chanspec = CH20MHZ_CHSPEC(channel, WL_CHANNEL_2G5G_BAND(channel));
	} else {
...
static int
ej_dpp_process(int eid, webs_t wp, int argc, char_t **argv)
{
	long status = 0;
	char prefix[] = "wlXXXXXXXXXX_";
	char tmp[64];
	...
	snprintf(tmp, sizeof(tmp), "%sdpp_status", prefix);
	value = nvram_safe_get(tmp);
	...
	switch (status) {
		case DPP_UI_INIT:
			websWrite(wp, "DPP INIT INPROGRESS");
			break;
		case DPP_UI_STATUS_AUTH_FAILURE:
			websWrite(wp, "DPP AUTH FAILURE");
			break;
		case DPP_UI_STATUS_CONFIGURE_FAILURE:
			websWrite(wp, "DPP CONFIG FAILURE");
			break;
		case DPP_UI_STATUS_CONFIG_SUCCESS:
			websWrite(wp, "DPP CONFIG SUCCESS");
			break;
		case DPP_UI_STATUS_NOT_COMPATIBLE:
			websWrite(wp, "DPP NOT COMPATIBLE");
			break;
		case DPP_UI_ASSOCIATED:
			websWrite(wp, "DPP AP-STA ASSOCIATED");
			break;
		case DPP_UI_DISASSOCIATED:
			websWrite(wp, "DPP AP_STA DISCONNECTED");
			break;
	}
	return 0;
}
...
ej_wl_dongle_fw_upload(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if (strstr(ver, "FWID")) { /* FD mode */
		...
		websWrite(wp,"	onMouseOut=\"return nd();\">");
		websWrite(wp,"	(Debug only- Not intended for customer UI)&nbsp;&nbsp;<br>Upload dongle fw file:&nbsp;&nbsp;");
		websWrite(wp,"    </th>");
		websWrite(wp,"    <td>&nbsp;&nbsp;</td>");
		websWrite(wp,"    <td><input id=\"fwfile\" type=\"file\" name=\"fwfile\" onChange=\"wl_fwfile_recalc();\"></td>");
		websWrite(wp,"  </tr>");
		websWrite(wp,"</table>");
		websWrite(wp,"<p>");
		websWrite(wp,"<table border=\"0\" cellpadding=\"0\" cellspacing=\"0\">");
		...
		websWrite(wp,"    <td>&nbsp;&nbsp;</td>");
		websWrite(wp,"    <td><input id=\"btn_fwfile\" type=\"submit\" value=\"Upload dongle fw file\"></td>");
		websWrite(wp,"  </tr>");
		websWrite(wp,"</table>");
		websWrite(wp,"<p>");
	}
...
ej_wl_driver_upload(int eid, webs_t wp, int argc, char_t **argv)
{
	...
	if (!strstr(ver, "FWID")) { /* NIC mode */
		...
		websWrite(wp,"	onMouseOut=\"return nd();\">");
		websWrite(wp,"	(Debug only- Not intended for customer UI)&nbsp;&nbsp;<br>Upload wl driver file:&nbsp;&nbsp;");
		websWrite(wp,"    </th>");
		websWrite(wp,"    <td>&nbsp;&nbsp;</td>");
...
valid_firmware_path(webs_t wp, char *value, struct variable *v, char *varname )
{
...
#ifdef BCA_SUPPORT_UNFWLCFG
	old_value = nvram_safe_kget(varname ? varname : v->name);
#else
	old_value = nvram_safe_get(varname ? varname : v->name);
#endif
	...
	/* Apply the setting */
#ifdef BCA_SUPPORT_UNFWLCFG
	nvram_kset(varname ? varname : v->name, value);
	nvram_kcommit();
	eval("/etc/init.d/bcm-wlan-drivers.sh", "restart");
#elif BCA_CPEROUTER
	{
		char cmd[100];
		sprintf(cmd, "%s=%s", varname ? varname : v->name, value);
		eval("nvram","kset", cmd);
		eval("nvram","kcommit");
	}
	eval("/etc/init.d/bcm-wlan-drivers.sh", "restart");
#else
	nvram_set(varname ? varname : v->name, value) ;
	nvram_commit();
#endif
...
static void
valid_wl_path(webs_t wp, char *value, struct variable *v, char *varname )
{
	ret_code = EINVAL;
	char *old_value = NULL;
...
wl_unit(webs_t wp, char *value, struct variable *v, char *varname)
{
	...
	for (; v >= variables && !strncmp(v->name, "wl_", 3); v--){
		...
		/* skip wl_path due to it is specific for updating nic driver (one for all interfaces) */
		if (!strncmp(v->name, "wl_path", 7))
			continue;
...
#ifdef __CONFIG_DHDAP__
	{ "firmware_path0", "DHDAP firmware_path0", NULL, valid_firmware_path, NULL, FALSE, NVRAM_MI },
	{ "firmware_path1", "DHDAP firmware_path1", NULL, valid_firmware_path, NULL, FALSE, NVRAM_MI },
	{ "firmware_path2", "DHDAP firmware_path2", NULL, valid_firmware_path, NULL, FALSE, NVRAM_MI },
#endif /*__CONFIG_DHDAP__*/

	{ "wl_path", "NICAP wl_path", NULL, valid_wl_path, NULL, FALSE, 0 },
	  ...
	  char_t *url, char_t *path, char_t *query)
{
	...
	if ((value = websGetVar(wp, "dpp_peer_qr", NULL))) {
		FILE *fp;
		char result[256]={0x0}, ssid_hex[(2*MAX_SSID_LEN) + 1] = {0};
		int peer_id;
		char *peer_role = NULL;
		if (!strcmp(action_mode, "Add DPP Enrollee")) {
			...
			snprintf(tmp, sizeof(tmp), "%sssid", prefix);
			ssid = nvram_safe_get(tmp);
			wl_ascii_str_to_hex_str(ssid, MIN((MAX_SSID_LEN + 1), strlen(ssid) + 1),
				...
				if ((peer_role = websGetVar(wp, "dpp_peer_role", NULL))) {
					snprintf(tmp, sizeof(tmp), "hostapd_cli -i %s dpp_auth_init"
						" peer=%d ssid=\"%s\" conf=%s-dpp configurator=1",
							osifname, peer_id, ssid_hex, peer_role);
				}
			} else {
