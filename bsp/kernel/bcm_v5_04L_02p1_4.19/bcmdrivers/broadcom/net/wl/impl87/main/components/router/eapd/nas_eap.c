 ...
 *
 * $Id: nas_eap.c 783049 2020-01-13 11:48:23Z $
 */
		...
		if (isset(nas->bitvec, type) && !strcmp(cb->ifname, from)) {
			...
			if (type == WLC_E_ASSOC_IND || type == WLC_E_REASSOC_IND ||
				...
				if (type == WLC_E_DISASSOC_IND || type == WLC_E_DEAUTH_IND ||
					...
					/* XXX, Ralink STA initial a WPS from sending a association
					 * w/o deauth/disassoc previous association. Here if the
					 * association contains WPS IE means initial a WPS
					 */
					/* PR87728, ignore parsing WPA IE if it has WPS IE also
					 * can be handled here
					 */
					sta->mode = EAPD_STA_MODE_UNKNOW;
