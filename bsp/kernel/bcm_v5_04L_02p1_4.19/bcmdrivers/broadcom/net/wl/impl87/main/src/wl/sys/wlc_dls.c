 ...
 *
 * $Id: wlc_dls.c 676803 2016-12-24 20:04:01Z $
 */
...
static const bcm_iovar_t dls_iovars[] = {
	{"dls", IOV_DLS, (0), 0, IOVT_BOOL, 0},
	{"dls_reject", IOV_DLS_REJECT, (0), 0, IOVT_BOOL, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
/* local functions */
/* module */
static int wlc_dls_doiovar(void *ctx, uint32 actionid,
...
static int
wlc_dls_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	wlc_bsscfg_t *bsscfg, uint8 *body, int body_len)
{
	...
	if (dls->reject_dls && BSSCFG_AP(bsscfg)) {
		...
		p = wlc_frame_get_action(wlc, da, &bsscfg->cur_etheraddr, &bsscfg->BSSID,
		                       len, &pbody, DOT11_ACTION_CAT_DLS);
		if (p == NULL) {
