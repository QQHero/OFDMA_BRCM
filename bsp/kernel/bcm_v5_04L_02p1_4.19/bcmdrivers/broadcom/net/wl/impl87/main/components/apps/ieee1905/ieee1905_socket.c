...
void i5SocketDumpSockets(void)
{
    ...
    while (p != NULL) {
       printf("  [%2d:%p]: %2d %p() %04x %02x", i, p, p->sd, p->process, p->type, p->flags);
...
int i5SocketClose(i5_socket_type *psock)
{
  ...
  /* If the VLAN interface is already deleted for the socket which got created on top of it, do not
   * delete again
   */
  if ((!I5_SOCKET_IS_VLAN_DELETED(psock->flags)) && (i5_config.vlan_ifr_list.count > 0) &&
    (psock->type == i5_socket_type_ll)) {
    if (i5GlueFindVlanIfr(psock->u.sll.ifname, real_ifname)) {
  ...
  if ( psock->type == i5_socket_type_ll) {
    ...
    if ( psock->u.sll.pSTPStateChangeTmr != NULL ) {
    ...
    if ( psock->u.sll.pInterfaceCtx != NULL ) {
      ...
      psock->u.sll.pInterfaceCtx = NULL;
    }
  }
