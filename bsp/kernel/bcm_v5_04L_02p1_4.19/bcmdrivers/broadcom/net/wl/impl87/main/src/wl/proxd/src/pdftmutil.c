 ...
 *
 * $Id: pdftmutil.c 782552 2019-12-23 12:20:25Z $
 */
...
pdftm_need_proxd(pdftm_t *ftm, uint flag)
{
	...
	num_bits = ftm->enabled_bss_len * NBBY;
	for (i = 0; i < num_bits; i += FTM_BSSCFG_NUM_OPTIONS) {
		test_bit = i + flag;
		if (isset(ftm->enabled_bss, test_bit)) {
			need = TRUE;
			break;
		}
	}
...
pdftm_get_pmu_tsf(pdftm_t *ftm)
{
	...
	/* rollover support */
	if (pmu_time < ftm->last_pmu)
		pmu_diff = ~ftm->last_pmu + pmu_time;
	else
		pmu_diff = pmu_time - ftm->last_pmu;

	cur_tsf = ftm->last_tsf;
	if (!wlc->scc_per)
		cur_tsf += FTM_PMU_TIME_TO_MICRO(pmu_diff); /* nominal */
	else
		cur_tsf += wlc_ref_tick_to_us(wlc, pmu_diff);

	ftm->last_pmu = pmu_time;
	ftm->last_tsf = cur_tsf;
