...
#define BSD_BLOCK_STA_2G_TIMEOUT	15 /* Unblock STA on 2G after 30 seconds */

/* BSD watchdog interval */
#define BSD_WATCHDOG_INTERVAL	10
...
/* Flags used in bsd_bssinfo structure */
#define BSD_FLAG_BSS_BSD_ENABLED	0x01
#define BSD_FLAG_BSS_WBD_ENABLED	0x02
#define BSD_FLAG_BSS_CHAN_PASSIVE	0x04
#define BSD_FLAG_BSS_SKIP_FIRST_TO_BSS	0x08
...
#define BSD_BSS_IS_CHAN_PASSIVE(bssinfo)	((bssinfo)->flags & BSD_FLAG_BSS_CHAN_PASSIVE)
#define BSD_BSS_SKIP_FIRST_TO_BSS(bssinfo)	((bssinfo)->flags & BSD_FLAG_BSS_SKIP_FIRST_TO_BSS)
...
#define BSD_BAND_6G	WLC_BAND_6G
...
typedef struct bsd_maclist {
	...
	struct bsd_bssinfo *to_bssinfo;
	uint8 steer_state;
	uint8 band_detect_cnt_5g;	/* send BSS transition request if 5G band is not
					 *  detected yet
					 */
	uint8 band_detect_cnt_6g;	/* send BSS transition request if 6G band is not
					 * detected yet
					 */
#ifdef CMWIFI_RDKB
	uint32 steering_flags;	/* keep the reason info for steering result event log */
#endif
...
struct bsd_bssinfo {
	...
	uint8 flags;		/* Flags of type BSD_FLAG_BSS_XXX */
};
...
#define BSD_AKM_KEY_CHECK_NVRAM		"bsd_akm_key_check"
...
#define BSD_STEERING_RESULT_NOT_HAPPENED		0x00040000
...
struct bsd_info {
	...
	uint steer_timeout;
	uint sta_timeout;
	uint sta_hit_rssi_times;
	uint8 sta_skip_single;
	uint8 prefer_5g;
	uint8 scheme;
	...
	int block_sta_timeout; /* timer for blocking STA in other than 2G, unblock STA if timeout */
	int block_sta_2g_timeout; /* timer for blocking STA in 2G, unblock STA if timeout */
};
...
#ifndef WIFISYSTEM_NUMSSIDSPERRADIO
#define WIFISYSTEM_NUMSSIDSPERRADIO     8
#endif

/* new reason for rdk log defined in XF3-3318 */
#define RDKB_BS_REASON_POST_UTIL 0
#define RDKB_BS_REASON_POST_RSSI 1
#define RDKB_BS_REASON_ACTIVE_UTIL 20
#define RDKB_BS_REASON_ACTIVE_RSSI 21
#define RDKB_BS_REASON_ACTIVE_UPGRADE 22
#define RDKB_BS_REASON_ACTIVE_PHYRATE 23   /* BCOM-4717 */
#define RDKB_BS_REASON_OTHER_AP_REQUEST 30
#define RDKB_BS_REASON_CLI_STEEREXEC 100
#define RDKB_BS_REASON_UNKNOWN 999
#define RDKB_BS_REASON_STRING_LENGTH 64
#endif /* CMWIFI_RDKB */
...
extern uint8 bsd_get_sta_band(bsd_info_t *info, struct ether_addr *addr);
...
extern void bsd_bss_trans_response_event(bsd_info_t *info, char *ifname, uint8 remote,
	char *pkt, int bytes);
extern bsd_sta_info_t *bsd_sta_by_addr(bsd_info_t *info, bsd_bssinfo_t *bssinfo,
	struct ether_addr *addr, bool enable);
#endif /*  _bsd_h_ */
