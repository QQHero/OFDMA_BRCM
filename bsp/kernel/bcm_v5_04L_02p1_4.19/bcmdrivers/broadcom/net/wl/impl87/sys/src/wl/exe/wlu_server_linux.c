 ...
 *
 * $Id: wlu_server_linux.c 804923 2021-11-12 11:52:02Z $
 */
...
/** all 'real' if names are smaller than this bogus name w.r.t. strncmp() */
#define BOGUS_IF_NAME "}"
...
wl_find_adapter(struct ifreq *ifr)
{
	...
	char prev_if_name[IFNAMSIZ] = BOGUS_IF_NAME;
	...
	while (fgets(buf, sizeof(buf), fp)) {
		...
		if (wl_get_dev_type(name, dev_type, DEV_TYPE_LEN) >= 0 &&
			!strncmp(dev_type, "wl", 2) && wl_check((void *) ifr) == 0) {
			if (strncmp(ifr->ifr_name, prev_if_name, IFNAMSIZ - 1) < 0) {
				/* prefer e.g. 'eth2' above 'eth3' as the default interface */
				strncpy(prev_if_name, ifr->ifr_name, IFNAMSIZ - 1);
			}
		}
	...
	if (strncmp(prev_if_name, BOGUS_IF_NAME, IFNAMSIZ - 1) != 0) {
		strncpy(ifr->ifr_name, prev_if_name, IFNAMSIZ - 1);
		ifr->ifr_name[IFNAMSIZ-1] = 0;
	} else {
		ifr->ifr_name[0] = '\0';
	}
...
daemonize()
{
	...
	/* Fork: the PID of the child process is returned in the parent, */
...
main(int argc, char **argv)
{
	...
	if ((argc >= 2) && (strcmp("-b", argv[1])) == 0) {
		...
		argv++;
		daemonize();
		DPRINT_INFO(OUTPUT, "%s: running in the background as daemon\n", rwl_server_name);
		defined_debug = DEBUG_ERR;	/* Supress further DPRINT_INFO */
	}
