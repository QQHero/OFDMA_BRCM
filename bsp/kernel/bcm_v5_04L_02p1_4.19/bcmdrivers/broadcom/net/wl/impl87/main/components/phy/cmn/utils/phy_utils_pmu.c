 ...
 *
 * $Id: phy_utils_pmu.c 796163 2021-02-24 15:26:36Z $
 */
...
phy_utils_pmu_bbpll_freq_switch(phy_info_t *pi, uint *pll_val, uint restore)
{
	...
	if (BCM6710_CHIP(sih->chip) || ACMAJORREV_GE131(pi->pubpi->phy_rev)) {
		if (restore) {
			si_pmu_pllcontrol(sih, PMU_CHIPCTL4, ~0, *pll_val);
		} else {
			*pll_val = si_pmu_pllcontrol(sih, PMU_CHIPCTL4, 0, 0);
			if (BCM6710_CHIP(sih->chip)) {
				div_ratio = BBPLL_DIV_16_6710;
			} else {
				/* Divide all BBPLL clock channels */
				div_ratio = (*pll_val) << BBPLL_DIV_SHIFT;
			}
