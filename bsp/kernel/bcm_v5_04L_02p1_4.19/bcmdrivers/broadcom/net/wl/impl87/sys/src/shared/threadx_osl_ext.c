 ...
 * $Id: threadx_osl_ext.c 790770 2020-09-02 19:34:28Z $
 */
 ...
 * Disable interrupts.
 *
 * Consider using @see OSL_INTERRUPT_SAVE_AREA and @see OSL_DISABLE instead.
 *
 * @return		Previous interrupt posture, for restoring interrupt state.
 */

INLINE osl_ext_interrupt_state_t
osl_ext_interrupt_disable(void)
{
 ...
 * Restore interrupts.
 *
 * Consider using @see OSL_RESTORE instead.
 *
 * @param state		Previous interrupt posture as returned by @see osl_ext_interrupt_disable.
 */

INLINE void
osl_ext_interrupt_restore(osl_ext_interrupt_state_t state)
{
 ...
 * Get current context.
 *
 * Consider using @see ASSERT_IN_ISR and @see ASSERT_NOT_ISR instead.
 *
 ...
 *
 * Consider using @see OSL_PREEMPT_SAVE_AREA and @see OSL_PREEMPT_DISABLE instead.
 *
 * @note Can't be used in interupt context.
 */

INLINE osl_ext_status_t
osl_ext_preempt_disable(uint *previous_threshold)
{
	UINT status;

	ASSERT_NOT_ISR();

	status = tx_thread_preemption_change(tx_thread_identify(), 0, previous_threshold);
	ASSERT(status == TX_SUCCESS);

	return status == TX_SUCCESS ? OSL_EXT_SUCCESS : OSL_EXT_ERROR;
}

/**
 * Restore thread preemption.
 *
 * Consider using @see OSL_PREEMPT_RESTORE instead.
 *
 * @note Can't be used in interupt context.
 */

INLINE osl_ext_status_t
osl_ext_preempt_restore(uint threshold)
{
	UINT status;
	UINT previous_threshold;

	ASSERT_NOT_ISR();

	status = tx_thread_preemption_change(tx_thread_identify(), threshold, &previous_threshold);
	ASSERT(status == TX_SUCCESS);
...
/**
 * Start reentrancy check.
 *
 * An ASSERT will be generated if code between @see osl_ext_reentrancy_check_start and @see
 * osl_ext_reentrancy_check_end is re-entered by the same thread, another thread, or an ISR.
 *
 * Consider using @see OSL_REENTRANCY_CHECK_START instead.
 */

void
osl_ext_reentrancy_check_start(uint* count, const char* file, uint line)
{
	BCM_REFERENCE(count);

	if (*count != 0) {
		printf("reentrancy check failed in %s(%u), entered by [%s], count %u\n",
			file, line, CURRENT_THREAD_NAMEPTR, *count);
	}
...
/**
 * End reentrancy check.
 *
 * Consider using @see OSL_REENTRANCY_CHECK_END instead.
 */

INLINE void
osl_ext_reentrancy_check_end(uint* count)
{
	BCM_REFERENCE(count);
 ...
 * Create a lock.
 *
 * On multiprocessor systems, critical sections are protected using ThreadX mutexes.
 *
 * @note Can't be used in interupt context.
 *
 * @param lock		Pointer to lock variable.
 * @param name		Optional lock identifier.
 * return		OSL_EXT_SUCCESS on success.
 */

INLINE osl_ext_status_t
osl_ext_lock_create(osl_ext_lock_t *lock, char *name)
{
	UINT status;

	ASSERT_NOT_ISR();
	ASSERT(lock && lock->tx_mutex_id != TX_MUTEX_ID);

	status = tx_mutex_create(lock, name, TX_INHERIT);
	ASSERT(status == TX_SUCCESS);
...
INLINE osl_ext_status_t
osl_ext_lock_delete(osl_ext_lock_t *lock)
{
	UINT status;

	ASSERT_NOT_ISR();

	status = tx_mutex_delete(lock);
	ASSERT(status == TX_SUCCESS);
...
INLINE osl_ext_status_t
osl_ext_lock_acquire(osl_ext_lock_t *lock)
{
	UINT status;

	ASSERT_NOT_ISR();

	status = tx_mutex_get(lock, TX_WAIT_FOREVER);
	ASSERT(status == TX_SUCCESS || status == TX_DELETED || status == TX_WAIT_ABORTED);

	return (status == TX_SUCCESS || status == TX_DELETED || status == TX_WAIT_ABORTED) ?
		OSL_EXT_SUCCESS : OSL_EXT_ERROR;
}
...
INLINE osl_ext_status_t
osl_ext_lock_release(osl_ext_lock_t *lock)
{
	UINT status;

	ASSERT_NOT_ISR();

	status = tx_mutex_put(lock);
	ASSERT(status == TX_SUCCESS);
 ...
 * Create a lock.
 *
 * On uniprocessor/multithreaded systems, critical sections are protected by controlling
 * ThreadX preemption.
 *
 * @note Can't be used in interupt context.
 *
...
osl_ext_lock_create(osl_ext_lock_t *lock, char *name)
{
	...
	/* No explicit lock creation actions are needed. */
	return OSL_EXT_SUCCESS;
}
...
osl_ext_lock_acquire(osl_ext_lock_t *lock)
{
	return osl_ext_preempt_disable((uint*)lock);
}
...
osl_ext_lock_release(osl_ext_lock_t *lock)
{
	return osl_ext_preempt_restore((uint)*lock);
}
