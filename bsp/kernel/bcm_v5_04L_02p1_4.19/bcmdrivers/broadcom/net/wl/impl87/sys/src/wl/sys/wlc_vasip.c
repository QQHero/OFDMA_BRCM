 ...
 *
 * $Id: wlc_vasip.c 809997 2022-03-29 07:03:37Z $
 */
...
#define SVMP_MEM_OFFSET_MAX_BCM6756     0x30020
...
BCMINITFN(wlc_vasip_init)(wlc_hw_info_t *wlc_hw, uint32 vasipver, bool nopi)
{
	...
	if (D11REV_IS(wlc_hw->corerev, 129)) {
		...
		if (D11MINORREV_GE(wlc_hw->corerev_minor, 2)) {
			vasip_fw = &vasip_11ax_2x2_160_fw;
			WL_INFORM(("wl%d: vasip 2x2 11ax 160MHz ver %d.%d\n",
			    wlc_hw->unit, d11vasipcode_major, d11vasipcode_minor));
		} else {
	...
	/* err == 1: no VASIP FW for this chip                                 */
	/* err == 2: VASIP FW for this chip doesn't support current Ntx config */
	if (err == 1) {
...
wlc_dump_vasip_error_mu_general(struct bcmstrbuf *b, int err_code)
{
	...
	if (err_code & 0x800) {
		bcm_bprintf(b, "  ERROR: BFD cmd-7 execution is failed or abnormal.\n");
	}
}
...
wlc_svmp_mem_offset_max(wlc_hw_info_t *wlc_hw)
{
	...
	if D11REV_IS(wlc_hw->corerev, 65) {
		...
		if (D11MINORREV_GE(wlc_hw->corerev_minor, 2)) {
			svmp_mem_offset_max = SVMP_MEM_OFFSET_MAX_BCM6756;
		} else {
