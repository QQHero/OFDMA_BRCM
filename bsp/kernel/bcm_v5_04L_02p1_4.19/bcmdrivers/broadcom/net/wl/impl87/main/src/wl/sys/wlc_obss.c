 ...
 *
 * $Id: wlc_obss.c 801357 2021-07-20 23:22:46Z $
 */
...
static uint8 wlc_obss_get_num_chan(wlc_obss_info_t *obss);
...
static bool wlc_ht_obss_scanparams_upd(wlc_bsscfg_t *cfg, obss_params_t *obss_param);
...
static int
wlc_obss_doiovar(void *context, uint32 actionid,
...
static void wlc_obss_scb_state_upd(void *ctx, scb_state_upd_data_t *data);
...
static int wlc_obss_parse_ext_cap_ie(void *ctx, wlc_iem_parse_data_t *data);

static void wlc_ht_upd_coex_state(wlc_bsscfg_t *cfg, uint8 detected);
static void wlc_ht_coex_filter_scanresult(wlc_bsscfg_t *cfg);
#ifdef AP
static void wlc_ht_coex_exclusion_range(wlc_info_t *wlc, uint8 *ch_min,
...
static const bcm_iovar_t obss_iovars[] = {
	...
	(IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, sizeof(wl_obss_scan_arg_t)
	},
	...
	(IOVF_SET_UP|IOVF_OPEN_ALLOW), 0, IOVT_BUFFER, 0
	},
...
BCMATTACHFN(wlc_obss_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_scb_state_upd_register(wlc, wlc_obss_scb_state_upd, (void*)obss) != BCME_OK) {
	...
	if (wlc_iem_add_parse_fn_mft(wlc->iemi, ext_cap_parse_fstbmp, DOT11_MNG_EXT_CAP_ID,
			wlc_obss_parse_ext_cap_ie, obss) != BCME_OK) {
	...
	uint8 *body, int body_len, wlc_d11rxhdr_t *wrxh)
{
	...
	obss_coex_tlv = bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_HT_BSS_COEXINFO_ID);
	...
	if ((coex_bits & WL_COEX_40MHZ_INTOLERANT) &&
		...
		FOREACH_AS_STA(wlc, idx, cfg) {
				...
				continue;
			if (!_wlc_ovlp_chanspec(wlc, cfg, chspec, CH_20MHZ_APART))
				continue;
	...
	if (coex_bits & WL_COEX_WIDTH20) {
		rx_bandunit = CHSPEC_BANDUNIT(D11RXHDR_ACCESS_VAL(&wrxh->rxhdr, wlc->pub->corerev,
			RxChan));
	...
	obss_chanlist_tlv = bcm_find_tlv(tlvs, tlvs_len, DOT11_MNG_HT_BSS_CHANNEL_REPORT_ID);
...
wlc_ht_coex_enab(wlc_bsscfg_t *cfg, int8 setting)
{
	...
	wlc_bsscfg_set_ext_cap(cfg, DOT11_EXT_CAP_OBSS_COEX_MGMT,
		COEX_ACTIVE(wlc->obss, cfg));
	if (cfg->up && wlc->clk && (BSSCFG_AP(cfg) || BSSCFG_IBSS(cfg))) {
		/* update AP or IBSS beacons */
		wlc_bss_update_beacon(wlc, cfg, TRUE);
...
wlc_ht_send_action_obss_coex(wlc_bsscfg_t *cfg, uint8 coex_bits, uint8 *coex_map)
{
	...
	if ((p = wlc_frame_get_action(wlc, &scb->ea, &cfg->cur_etheraddr,
...
wlc_write_obss_intol_chanlist_ie(wlc_info_t *wlc, uint8* cp, uint8 len, uint8 *coex_map)
{
	...
	if ((wlc_japan(wlc) == TRUE) && coex_map[14] && len) {
		...
		chanlist_ie->regclass = wlc_get_regclass(wlc->cmi,
		                                         CH20MHZ_CHSPEC(14, WL_CHANSPEC_BAND_2G));
	...
	chanlist_ie->regclass = wlc_get_regclass(wlc->cmi, CH20MHZ_CHSPEC(1, WL_CHANSPEC_BAND_2G));
...
wlc_ht_chanlist_2g_init(wlc_obss_info_t *obss)
{
	...
	for (i = 1; i <= CH_MAX_2G_CHANNEL; i++) {
		if (wlc_valid_channel20_in_band(obss->wlc->cmi, BAND_2G_INDEX, i)) {
...
wlc_obss_scan_update_countdown(wlc_obss_info_t *obss, wlc_bsscfg_t *cfg,
	uint8* chanvec, int n_channels)
{
	wlc_obss_bss_info_t *obss_bss_info = BSS_OBSS_INFO(obss, cfg);

	if (n_channels < wlc_obss_get_num_chan(obss)) {
		WL_COEX(("wl%d: %s: scanned channel [%d] < OBSS Scan Params [%d]\n",
			obss->wlc->pub->unit, __FUNCTION__, n_channels,
			wlc_obss_get_num_chan(obss)));
		return;
	}
...
wlc_ht_coex_switch_bw(wlc_bsscfg_t *cfg, bool downgrade, uint8 reason_code)
{
	...
	if (downgrade) {
		newchanspec = wf_ctlchspec20_from_chspec(chanspec);
	...
	csa.count = CHSPEC_PRIM20_EQUAL(chanspec, newchanspec) ? 0 : 10;
...
wlc_ht_ap_coex_scan_parse_cap_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	rxchspec = (data->pparm->ft->bcn.chan | WL_CHANSPEC_BW_20);
	rxchspec |= BANDTYPE_CHSPEC(data->pparm->ft->bcn.bandtype);
...
wlc_ht_ap_coex_bcn_parse_cap_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	FOREACH_UP_AP(wlc, idx, cfg) {
		...
		if (!COEX_ACTIVE(wlc->obss, cfg))
			continue;
		if (!CHSPEC_CHNLOVLP(chspec, rxchspec, WLC_2G_25MHZ_OFFSET))
			continue;
...
wlc_ht_ap_coex_ted_chk(wlc_bsscfg_t *cfg, bcm_tlv_t *bss_chanlist_tlv, uint8 coex_bits)
{
	...
	uint8 ch_min = 0, ch_max = 0, ch_ctl = 0;
	...
	if (!chanlist_ie) {
		WL_COEX(("wl%d: %s: 20/40 BSS Intolerant Channel Report IE NOT found\n",
			wlc->pub->unit, __FUNCTION__));
		return FALSE;
	}
	...
	if (bss_chanlist_tlv->len < DOT11_OBSS_CHANLIST_FIXED_LEN) {
		WL_COEX(("wl%d: %s: Invalid 20/40 BSS Intolerant Channel Report IE len %d\n",
...
static void
wlc_obss_scb_state_upd(void *ctx, scb_state_upd_data_t *data)
{
	wlc_obss_info_t *obss = (wlc_obss_info_t *)ctx;
...
#ifdef STA
			/* OBSS scan only when in 2g band */
...
static int
wlc_obss_parse_ext_cap_ie(void *ctx, wlc_iem_parse_data_t *data)
{
		...
		if (data->ie != NULL) {
			...
			if (extcap_ie_tlv->len >= DOT11_EXTCAP_LEN_COEX) {
				if (isset(cap->extcap, DOT11_EXT_CAP_OBSS_COEX_MGMT))
					scb->flags |= SCB_COEX_MGMT;
				if (isset(cap->extcap, DOT11_EXT_CAP_EXT_CHAN_SWITCHING)) {
					scb->flags3 |= SCB3_ECSA_SUPPORT;
				}
			}
		}
	}
