 ...
 *
 * $Id: wlc_hw_priv.h 809675 2022-03-22 05:14:17Z $
 */
...
typedef struct wlc_hw_btc_info {
	...
	uint16		bt_period;
	bool		btc_init_flag;
	bool		presuspend_critstate;	/**< 1: Set critical state prior to MAC Suspend */
...
typedef struct wlc_hw_sc_buf_info {
	uint32 start; /* SAMPLE_COLLECT: buffer start location in 32bit words */
	uint32 sz; /* SAMPLE_COLLECT: buffer length in 32bit words */
} wlc_hw_sc_buf_info_t;
...
struct wlc_hw_info {
	...
	wlc_hw_sc_buf_info_t *sc_buf_info; /* sample collect buffer size/position */
