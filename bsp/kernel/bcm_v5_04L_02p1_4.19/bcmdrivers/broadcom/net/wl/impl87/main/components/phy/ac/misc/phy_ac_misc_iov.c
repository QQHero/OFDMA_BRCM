...
enum {
	...
	IOV_PHY_HWRSSI_TRT_2G = 11,
	IOV_PHY_HWRSSI_TRT_5G6G = 12
};
...
static const bcm_iovar_t phy_ac_misc_iovars[] = {
	...
	{"phy_hwrssi_trt_2g", IOV_PHY_HWRSSI_TRT_2G,
	(IOVF_GET_UP | IOVF_SET_UP | IOVF_MFG), 0, IOVT_BUFFER, 4*sizeof(int8)},
	{"phy_hwrssi_trt_5g6g", IOV_PHY_HWRSSI_TRT_5G6G,
	(IOVF_GET_UP | IOVF_SET_UP | IOVF_MFG), 0, IOVT_BUFFER, 5*sizeof(int8)},
	{NULL, 0, 0, 0, 0, 0}
};
	...
	void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_PHY_HWRSSI_TRT_2G):
		actvcore = 0;
		FOREACH_CORE(pi, core) {
			 // check if core is active then only assign to active ones
			if ((rx_coremask >> core) & 0x1) {
				pi->hwrssioffset_trt_2g[core] = setDeltaValues[actvcore];
				actvcore++;
			} else {  // assign 0dB to inactive core
				pi->hwrssioffset_trt_2g[core] = 0;
			}
		}
		break;

	case IOV_GVAL(IOV_PHY_HWRSSI_TRT_2G):
		actvcore = 0;
		FOREACH_CORE(pi, core) {
			 // check if core is active then only assign to active ones
			if ((rx_coremask >> core) & 0x1) {
				getDeltaValues[actvcore] = pi->hwrssioffset_trt_2g[core];
				actvcore++;
			}
		}
		break;
	...
	case IOV_SVAL(IOV_PHY_HWRSSI_TRT_5G6G):
		{
			uint8 sb = setDeltaValues[0]; // args: subband core0 core1 core2 core3
			FOREACH_CORE(pi, core) {
				pi->hwrssioffset_trt_5g_6g[core][sb] = setDeltaValues[core+1];
			}
		}
		break;

	case IOV_GVAL(IOV_PHY_HWRSSI_TRT_5G6G):
		{
			uint8 sb;
			uint16 channel;
			...
			getDeltaValues[0] = sb;
			FOREACH_CORE(pi, core) {
				getDeltaValues[core+1] = pi->hwrssioffset_trt_5g_6g[core][sb];
			}
		}
		break;
