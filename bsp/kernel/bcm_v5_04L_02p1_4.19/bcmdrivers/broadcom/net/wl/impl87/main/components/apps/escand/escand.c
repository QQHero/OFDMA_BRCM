...
#define ESCAND_CS_MIN_CHAN(band)	((band == WLC_BAND_5G) ? ESCAND_CS_MIN_5G_CHAN : \
			(band == WLC_BAND_6G) ? ESCAND_CS_MIN_6G_CHAN : \
...
#define ESCAND_CS_MAX_CHAN(band)	((band == WLC_BAND_5G) ? ESCAND_CS_MAX_5G_CHAN : \
			(band == WLC_BAND_6G) ? ESCAND_CS_MAX_6G_CHAN : \
...
escand_dump_config_extra(escand_chaninfo_t *c_info)
{
	...
	ESCAND_INFO("\t escand_ci_scan_type: %s\n\n",
		(c_info->escand_ci_scan_type ? "passive" : "active"));
	ESCAND_INFO("\t escand_ci_busy_min_scan_timer: %d\n\n",
		c_info->escand_ci_busy_min_scan_timer);
	ESCAND_INFO("\t escand_scan_promisc: %d\n\n", c_info->escand_scan_promisc);
}

static void
escand_dump_scan_tx_frame(void)
{
	int i;

	ESCAND_PRINT("escand_dump_scan_tx_frame:\n");
	for (i = 0; i < 40; i++)
	{
		ESCAND_PRINT_QUIET("%02x:", escandScanTxFrame[i]);
	}
	ESCAND_PRINT_QUIET("\n");
	return;
}
...
escand_get_rs_info(escand_chaninfo_t * c_info, char* prefix)
{
	...
	ret = escand_get_chanspec(c_info, &radio_chspec);
	ESCAND_ERR(ret, "failed to get chanspec");

	radio_chspec = dtoh32(radio_chspec);

	if (str && (strcmp(str, "0") && strcmp(str, "2g0") &&
		strcmp(str, "5g0") && strcmp(str, "6g0"))) {

		rsi->pref_chspec = radio_chspec;
	}
	...
	rsi->band_type = CHSPEC_BANDTYPE(radio_chspec);
	ESCAND_INFO("band_type: %d, from chanspec\n",  rsi->band_type);

	memset(&param, 0, sizeof(param));
	param.band = htod32(rsi->band_type);
...
escand_check_bss_is_enabled(char *name, escand_chaninfo_t **c_info_ptr, char *prefix)
{
	...
	if (atoi(bss_check) != 1) {
		...
		ESCAND_ERROR("interface is disabled %s (%s, %s)\n", name, prefix, buf);
	...
	*c_info_ptr = escand_info->chan_info[index] =
	...
	strncpy((*c_info_ptr)->name, name, (sizeof((*c_info_ptr)->name) - 1));
	(*c_info_ptr)->name[sizeof((*c_info_ptr)->name) - 1] = '\0';
	ESCAND_ERROR("bss enabled for name :%s\n", (*c_info_ptr)->name);
	return BCME_OK;
}
...
escand_channel_info(escand_chaninfo_t *c_info, chanspec_t chspec)
{
	...
	FOREACH_20_SB(chspec, sub_channel) {
		sub_chspec = CH20MHZ_CHSPEC(sub_channel, CHSPEC_BAND(chspec));
...
escand_init_run(escand_info_t ** escand_info_p)
{
	...
	foreach(name, nvram_safe_get("escand_ifnames"), next) {
		...
		if (!strncmp(name, "none", 4)) {
			ESCAND_PRINT("disabled (interfaces=none)\n");
			break;
		}

		if (osifname_to_nvifname(name, prefix, sizeof(prefix)) != BCME_OK) {
			ESCAND_ERROR("interface ""%s"" not found\n", name);
			continue;
		}
		...
		ESCAND_PRINT("%s: interface ""%s"" starting escand\n", name, __FUNCTION__);
		ret = escand_start(name, c_info);
		ESCAND_PRINT(
			"escand_start for interface %s, wait %d seconds to start scanning\n",
			name, ESCAND_STARTUP_DELAY/ESCAND_TICKS_PER_SEC);
...
escand_update_status(escand_chaninfo_t * c_info)
{
	...
	ESCAND_INFO("%s: chanspec changed, chanspec:0x%x is160_bwcap %d "
		"is160_upgradable %d, is160_downgradable %d\n",
...
static int
escand_get_pktq_stats(escand_chaninfo_t *c_info, escand_traffic_info_t *t_info, char *buf)
{
	char buf2[ESCAND_MAX_INFO_BUF];
	wl_iov_mac_full_params_t*  full_params = (wl_iov_mac_full_params_t*)buf2;
	wl_iov_mac_params_t*       params = &full_params->params;
	wl_iov_mac_extra_params_t* extra_params = &full_params->extra_params;
	int i, ret;
	wl_iov_pktq_log_t *pktq_log = (wl_iov_pktq_log_t *) buf;
	uint8 num_prec;
	uint32 version, wme_ac;
	uint32 traffic_count = 0;

	memset(full_params, 0, sizeof(*full_params));

	params->addr_type[0] = 'C';
	extra_params->addr_info[0] = htod32(0xFFFF);
	/* set a "version" indication (ie extra_params present) */
	params->num_addrs = htod32(1 | (ESCAND_PKTQ_STATS_VER << 8));

	if ((ret = escand_get_pktq_data(c_info->name, buf, full_params)) < 0) {
		ESCAND_ERROR("%s: error %d getting %s (%u)\n", c_info->name, ret,
			"pktq_stats", (uint32)sizeof(wl_iov_pktq_log_t));
		return ret;
	}

	num_prec = pktq_log->pktq_log.v06.num_prec[0];
	wme_ac = pktq_log->pktq_log.v06.wme_ac[0];
	version = pktq_log->version;

	ESCAND_DEBUG("version %d; num_prec[0] = %d; wme_ac[0] = 0x%08x, escand_traffic_type 0x%x\n",
		version, num_prec, wme_ac, c_info->escand_traffic_type);
	for (i = 0; i < num_prec; i++) {
		uint8 _ac = (wme_ac >> (i * 2)) & 0x3;
		char *ac = "";
		...
		ESCAND_DEBUG_QUIET("%d: %s %d, ", i, ac,
			pktq_log->pktq_log.v06.counters[0].pktq[i].requested);

		/* Check for traffic corresponding to the configured traffic type(s) */
		if (c_info->escand_traffic_type & (1 << _ac)) {
			/* Count frames from this prec */
			traffic_count += pktq_log->pktq_log.v06.counters[0].pktq[i].requested;
		}
	}
	ESCAND_DEBUG_QUIET("\ntraffic_count %d\n", traffic_count);
	c_info->escand_traffic_measured = traffic_count;
...
escand_get_traffic_info(escand_chaninfo_t *c_info, escand_traffic_info_t *t_info)
{
	...
	if (escand_get_counters(c_info->name, cntbuf) < 0) {
		ESCAND_ERROR("Failed to fetch interface counters for '%s'\n", c_info->name);
		ret = BCME_ERROR;
		goto exit;
	}
	...
	if (wl_cntbuf_to_xtlv_format(NULL, cntbuf, ESCAND_WL_CNTBUF_SIZE, 0)
		!= BCME_OK) {
		ESCAND_ERROR("wl_cntbuf_to_xtlv_format failed for '%s'\n", c_info->name);
		ret = BCME_ERROR;
		goto exit;
	}

	if ((wlc_cnt = GET_WLCCNT_FROM_CNTBUF(cntinfo)) == NULL) {
		ESCAND_ERROR("GET_WLCCNT_FROM_CNTBUF NULL for '%s'\n", c_info->name);
		ret = BCME_ERROR;
		goto exit;
	}
	...
	/* Get pktq stat data for precendence traffic stats */
	escand_get_pktq_stats(c_info, t_info, buf);
...
{
	char stabuf[ESCAND_MAX_INFO_BUF];
	sta_info_v6_t *sta;
	int i, ret = BCME_OK;
