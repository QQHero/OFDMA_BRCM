 ...
 *
 * $Id: wlc_offld.c 803989 2021-10-14 19:10:00Z $
 */
 ...
 *
 *	CAUTION: wlc_offld assumes that HNDM2M will not set any M2M core level interrupts
...
BCMATTACHFN(wlc_offload_isr_register)(wlc_info_t *wlc, uint orig_unit, uint bus,
		...
		void *cbdata, osl_ext_task_t* thread)
{
	 ...
	 */
	char * irqname = wlc_offload_irqname(wlc, NULL);
...
wlc_offload_m2m_intstatus(wlc_offload_t *wlc_offload)
{
	...
	if (m2m_intstatus) {
		...
		if (BCM_GBF(m2m_intstatus, M2M_CORE_INTCONTROL_PHYRXSWRINDUPD_INTMASK)) {
			wlc_intr_process_rxfifo_interrupts(wlc_offload->wlc, TRUE);
		}
#endif /* WLC_OFFLOADS_RXSTS */
	}
...
wlc_offload_intrsoff(wlc_offload_t *wlc_offload)
{
	...
	v32 = R_REG(wlc_offload->osh, &m2m_core_regs->intcontrol);
	/* CAUTION: wlc_offld assumes that HNDM2M library will not set any core level interrupts */
	ASSERT((v32 == 0) || (v32 == wlc_offload->m2m_intcontrol));
#endif /* HNDM2M && BCMDBG */
...
void
wlc_offload_intrs_deassert(wlc_offload_t *wlc_offload)
{
	volatile m2m_core_regs_t *m2m_core_regs = wlc_offload->m2m_core_regs;
