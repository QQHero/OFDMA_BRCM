 ...
 *
 * $Id: wl_iw.c 789856 2020-08-10 12:22:24Z $
 */
...
)
{
	...
	if ((error = wldev_iovar_getint(dev, "chanspec", (s32 *)&chspec)))
		return error;

	/* Return radio channel in channel form */
	fwrq->m = dtoh32(wf_chspec_primary20_chan(chspec));
	fwrq->e = dtoh32(0);
	return 0;
}
	...
	struct iw_request_info *info, wl_bss_info_t *bi)
{
	...
	if (bi->ie_length) {
		...
		/* OSEN IE */
		if ((ie = bcm_find_tlv(ptr, ptr_len, DOT11_MNG_VS_ID)) &&
		...
		if ((ie = bcm_find_tlv(ptr, ptr_len, DOT11_MNG_RSN_ID))) {
			iwe.cmd = IWEVGENIE;
			iwe.u.data.length = ie->len + 2;
			event = IWE_STREAM_ADD_POINT(info, event, end, &iwe, (char *)ie);
		}
		...
		if ((ie = bcm_find_tlv(ptr, ptr_len, DOT11_MNG_MDIE_ID))) {
			iwe.cmd = IWEVGENIE;
			iwe.u.data.length = ie->len + 2;
			event = IWE_STREAM_ADD_POINT(info, event, end, &iwe, (char *)ie);
		}
		...
		while ((ie = bcm_find_tlv(ptr, ptr_len, DOT11_MNG_WPA_ID))) {
			/* look for WPS IE */
		...
		while ((ie = bcm_find_tlv(ptr, ptr_len, DOT11_MNG_WAPI_ID))) {
