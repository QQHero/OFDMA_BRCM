 ...
 *
 * $Id: wlu_subcounters.c 807758 2022-01-31 09:38:36Z $
 */
#include <bcmendian.h>
...
#define CAT_BCN   (1<<10) /**< Counters related to beacons */
...
#define PRFMT64(name) #name " %llu "  /* converts in the 'default' counters printf format string */
...
#define PRVAL64(name)						\
	if (prcnt_filter(p_cnt_filters, #name, cnt->name != 0 ? TRUE:FALSE)) { \
		printf(PRFMT64(name), dtoh64(cnt->name));	\
		at_start_of_line = FALSE;			\
	}
...
/** prints one variable with dtoh64 conversion */
#define PRVAL_RENAME64(name, prname)				\
	if (prcnt_filter(p_cnt_filters, #name, cnt->name)) {	\
		printf(PRFMT64(prname), dtoh64(cnt->name));	\
		at_start_of_line = FALSE;			\
	}
...
/** prints one variable with dtoh64 conversion */
#define PRVALSF_RENAME64(varname, prname)				\
	if (dtoh64(cnt->varname) != INVALID_CNT_VAL)		\
		PRVAL_RENAME64(varname, prname)
...
int print_wl_if_v2_struct(wl_if_stats_v2_t *cnt, struct cnt_filters_s cnt_filters,
		struct cnt_filters_s *p_cnt_filters);
...
static const struct cnt_properties_s cnt_props[] = {
	{"active_ant", 0},
	{"ampdu_wds", CAT_AMPDU},
	{"apdecerr",    CAT_RX | CAT_ERR },
	{"auto_en", 0},
	{"avg_snr_per_ant0", 0},
	{"avg_snr_per_ant1", 0},
	{"avg_snr_per_ant2", 0},
	{"bcntxcancl",       CAT_TX | CAT_ERR | CAT_MCAST | CAT_UCODE | CAT_ASSOC | CAT_BCN},
	...
	{"maxchainsz",       0},
	{"mboxout",          0},
	{"misc_cnt1",	CAT_UCODE | CAT_CTRL},
	{"missbcn_dbg",      CAT_ERR | CAT_MCAST | CAT_UCODE | CAT_CTRL | CAT_ASSOC | CAT_BCN},
	{"musnd",            CAT_CTRL},
	...
	{"rxbcast",      CAT_RX},
	{"rxbeaconmbss", CAT_RX | CAT_UCODE | CAT_CTRL | CAT_ASSOC | CAT_BCN},
	{"rxbeaconobss", CAT_RX | CAT_UCODE | CAT_CTRL | CAT_ASSOC | CAT_BCN},
	{"rxbyte",       CAT_RX},
	...
	{"rxdropped",       CAT_RX | CAT_ERR | CAT_UCODE},
	...
	{"rxprobereq",   CAT_RX | CAT_CTRL | CAT_ASSOC},
	{"rxprobersp",   CAT_RX | CAT_CTRL | CAT_ASSOC},
	...
	{"txbcast", CAT_TX | CAT_MCAST},
	{"txbcnfrm", CAT_TX | CAT_MCAST | CAT_UCODE | CAT_CTRL | CAT_ASSOC | CAT_BCN},
	{"txbcnloss", CAT_TX | CAT_MCAST | CAT_CTRL | CAT_ERR | CAT_ASSOC | CAT_BCN},
	{"txbyte", CAT_TX},
	...
	{"txdropped", CAT_TX | CAT_UCODE},
	{"txerror", CAT_TX | CAT_ERR},
	{"txexptime", CAT_TX | CAT_ERR},
...
static int wl_counters_cbfn(void *ctx, const uint8 *data, uint16 type, uint16 len)
{
	...
	switch (type) {
		...
		case WL_CNT_XTLV_WLC_AP: {
			wl_ap_cnt_t *cnt = (wl_ap_cnt_t *)data;
			if (len != sizeof(wl_ap_cnt_t)) {
				printf("type %d: cnt struct length mismatch! %d != %d\n",
					type, len, (int)sizeof(wl_ap_cnt_t));
			}
			PRVAL(txbcnloss); PRVAL(stadecerr); PRVAL(apdecerr);
			PRVAL(misc_cnt1); PRNL1();
			break;
		}
		default:
...
wl_counters(void *wl, cmd_t *cmd, char **argv)
{
	...
	while (*++argv != NULL) {
		...
		else if (!strcmp(*argv, "--bcn"))
			cbfn_info.cnt_filters.filter |= CAT_BCN;
		else {
	...
	cntdata = (uint8 *)CALLOC(cntinfo->datalen);
	if (cntdata == NULL) {
		err = BCME_NOMEM;
		printf("malloc fail!\n");
		goto exit;
...
static int wl_if_counters_cbfn(void *ctx, const uint8 *data, uint16 type, uint16 len)
{
	int err = BCME_OK;
	wl_cnt_cbfn_info_t *cbfn_info = ctx;
	struct cnt_filters_s cnt_filters = cbfn_info->cnt_filters;
	struct cnt_filters_s *p_cnt_filters = &cbfn_info->cnt_filters;
	...
	switch (type) {
		...
		case WL_IFSTATS_XTLV_GENERIC: {
			wl_if_stats_v2_t *cnt = (wl_if_stats_v2_t *)data;

			if (dtoh16(cnt->version) == WL_IF_STATS_VER_1 &&
				len > sizeof(wl_if_stats_v1_t)) {
					printf("type %d: cntbuf length too long! %d > %d\n"
						"May need to use up-to-date wl utility.\n",
						type, len, (int)sizeof(wl_if_stats_v1_t));
			}
			else if (dtoh16(cnt->version) == WL_IF_STATS_VER_2 &&
				len > sizeof(wl_if_stats_v2_t)) {
					printf("type %d: cntbuf length too long! %d > %d\n"
						"May need to use up-to-date wl utility.\n",
						type, len, (int)sizeof(wl_if_stats_v2_t));
			}

			/* summary stat counter line */
			PRINIT();
			PRVAL64(txframe); PRVAL64(txbyte); PRVAL64(txerror);
			PRVAL64(rxframe); PRVAL64(rxbyte); PRVAL64(rxerror); PRNL1();

			PRVAL64(txnobuf); PRVAL64(txfail); PRVAL64(txretrans); PRNL1();
			PRVALSF_RENAME64(txfrag, d11_txfrag);
			PRVALSF_RENAME64(txmulti, d11_txmulti);
			PRVALSF_RENAME64(txretry, d11_txretry);
			PRVALSF_RENAME64(txretrie, d11_txretrie);
			PRNL1();
			PRVALSF_RENAME64(txfrmsnt, d11_txfrmsnt);
			PRNL1();

			PRVAL64(rxnobuf); PRVAL64(rxfragerr); PRNL1();
			PRVAL64(rxrunt); PRNL1();
			PRVALSF_RENAME64(rxmulti, d11_rxmulti);
			PRNL1();

			if (dtoh16(cnt->version) == WL_IF_STATS_VER_2) {
				print_wl_if_v2_struct(cnt, cnt_filters, p_cnt_filters);
			}
...
/* old if_counters before if_stats came in */
int
wl_if_counters_before_ifstats(void *wl, cmd_t *cmd, char **argv)
{
	...
	statsbuf = (char *)ptr;
	cnt = (wl_if_stats_v2_t *)CALLOC(sizeof(wl_if_stats_v2_t));
	if (cnt == NULL) {
		printf("\tCan not allocate %d bytes for counters struct\n",
		       (int)sizeof(wl_if_stats_v2_t));
		return BCME_NOMEM;
	} else
		memcpy(cnt, statsbuf, sizeof(wl_if_stats_v2_t));
	/*
	 * summary stat counter line.
	 * Two structure versions of wl_if_stats:
	 * Version 2 is identical to version 1 + more counters for BSS_STATS + EAP_STATS rates
	 * Because they overlap for the counters both have we can savely cast
	 * statsbuf to wl_if_stats_v2_t and for version 1 only print the counters that version 1 has
	 */
	PRINIT();
	PRVAL64(txframe); PRVAL64(txbyte); PRVAL64(txerror);
	PRVAL64(rxframe); PRVAL64(rxbyte); PRVAL64(rxerror); PRNL1();

	PRVAL64(txnobuf); PRVAL64(txfail); PRNL1();
	PRVALSF_RENAME64(txfrag, d11_txfrag);
	PRVALSF_RENAME64(txmulti, d11_txmulti);
	PRVALSF_RENAME64(txretry, d11_txretry);
	PRVALSF_RENAME64(txretrie, d11_txretrie);
	PRNL1();
	PRVALSF_RENAME64(txfrmsnt, d11_txfrmsnt); PRVALSF_RENAME64(rxmulti, d11_rxmulti);
	PRNL1();

	PRVAL64(rxnobuf); PRVAL64(rxfragerr);
	PRVAL64(rxrunt); PRVAL64(rxerror);
	PRNL1();
	if (dtoh16(cnt->version) == WL_IF_STATS_VER_1) {
		if (dtoh16(cnt->length) > OFFSETOF(wl_if_stats_v1_t, txexptime)) {
			PRVAL64(txexptime);
			PRVAL64(txrts); PRVAL64(txnocts); PRNL1();
		}
	}
...
int
print_wl_if_v2_struct(wl_if_stats_v2_t *cnt, struct cnt_filters_s cnt_filters,
		struct cnt_filters_s *p_cnt_filters)
{
	if (dtoh16(cnt->length) > OFFSETOF(wl_if_stats_v2_t, txexptime)) {
		PRVAL64(txexptime); PRVAL64(txrts); PRVAL64(txnocts);
		PRNL1();
	}

	PRVAL64(txucastpkts); PRVAL64(txucastbytes); PRVAL64(txmcastpkts); PRVAL64(txmcastbytes);
	PRNL1();
	PRVAL64(txbcastpkts); PRVAL64(txbcastbytes); PRVAL64(txaggrpktcnt);
	PRNL1();

	PRVAL64(txnoack); PRVAL64(txretransfail); PRVAL64(txretrans); PRVAL64(txdiscard);
	PRNL1();

	PRVAL64(rxucastpkts); PRVAL64(rxucastbytes); PRVAL64(rxmcastpkts); PRVAL64(rxmcastbytes);
	PRNL1();
	PRVAL64(rxbcastpkts); PRVAL64(rxbcastbytes);
	PRNL1();

	PRVAL64(rx_decrypt_failures); PRVAL64(rx_mic_errors);
	PRVAL64(rx_replay_failures); PRVAL64(rx_dup);
	PRNL1();
	PRVAL64(rxdiscard); PRVAL64(rxbadprotopkts); PRVAL64(rxunknownprotopkts);
	PRNL1();

	UNUSED_PARAMETER(cnt_filters);
...
wl_delta_stats(void *wl, cmd_t *cmd, char **argv)
{
	...
	cnt = (wl_delta_stats_t*)CALLOC(sizeof(wl_delta_stats_t));
	if (cnt == NULL) {
...
{
	char *statsbuf;
	struct wlc_swdiv_stats_v1 *cnt_v1 = NULL;
	struct wlc_swdiv_stats_v2 *cnt = NULL;
	int err = BCME_OK;
	void *ptr;
	struct cnt_filters_s cnt_filters;
	struct cnt_filters_s *p_cnt_filters = &cnt_filters;
