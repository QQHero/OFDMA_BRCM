 ...
 *
 * $Id: bcm_steering.c 806942 2022-01-10 09:16:47Z $
 */
...
typedef struct wl_wlif_hdl_data {
	...
	static_maclist_t *nobcnssid_timer_data;	/* Nobcnssid timer data */
...
/* Remove nobcnssid timer */
static void wl_wlif_remove_nobcnssid_timer(wl_wlif_hdl_data_t *hdl_data);
/* Create Timer */
static int wl_wlif_create_timer(void *uschd_hdl, bcm_timer_module_id timer_hdl,
	void* cbfn, void *data, int timeout, int repeat, void *comparefn,
	bcm_timer_id *out_timer_id);
...
static void wl_wlif_unblock_mac_usched_cb(bcm_usched_handle* hdl, void* arg);
static int wl_wlif_compare_macblock_timer_args(void *arg1, void *arg2);
	...
	wl_wlif_bss_trans_data_t *ioctl_data)
{
	...
	ret = wl_wlif_create_timer(hdl_data->uschd_hdl, hdl_data->timer_hdl,
		(hdl_data->uschd_hdl ? wl_wlif_steer_resp_wait_tiemout_usched_cb :
		wl_wlif_steer_resp_wait_tiemout_cb), new, hdl_data->resp_timeout,
		FALSE, NULL, &timer_id);
	if (!ret) {
	...
	wl_wlif_bss_trans_resp_t *btm_resp, int bsstrans_ret)
{
	...
	*/
	...
	if ((ioctl_hndlr_data->resp_hndlr != NULL) &&
		ioctl_hndlr_data->resp_hndlr(ioctl_hndlr_data->resp_hndlr_data, bsstrans_ret, brute_force)) {
		goto end;
	}
	...
	if (!brute_force) {
		...
		if (bsstrans_ret != WLIFU_BSS_TRANS_RESP_ACCEPT) {
			...
			wl_wlif_remove_nobcnssid_timer(hdl_data);
		}
		goto end;
	}
#ifdef BCM_CEVENT
	if (!strcmp(hdl_data->appname, "wbd")) {
		ce_subtype = CEVENT_ST_WBD;
	} else if (!strcmp(hdl_data->appname, "bsd")) {
		ce_subtype = CEVENT_ST_BSD;
	} else {
		ce_subtype = CEVENT_ST_LAST;
	}
	/* cevent sent with STA MAC address for BTM Brute Force from app */
	CE_SEND_CEVENT_A2C_EXT(hdl_data->ifname, &ioctl_hndlr_data->bssid, 0,
		ioctl_hndlr_data->reason, ce_subtype, CEVENT_A2C_MT_BTM_BRUTE_FORCE,
...
wl_wlif_restore_old_maclist_timer(wl_wlif_hdl_data_t *hdl)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		if (strcmp(entry.ifname, hdl->ifname) == 0) {
	...
	wl_wlif_bss_trans_data_t *ioctl_data)
{
	...
	if (ioctl_data->flags & WLIFU_BSS_TRANS_FLAGS_DISASSOC_IMNT) {
		...
		ret = wl_wlif_create_timer(hdl_data->uschd_hdl, hdl_data->timer_hdl,
			(hdl_data->uschd_hdl ? wl_wlif_disassoc_mac_usched_cb :
			wl_wlif_disassoc_mac_cb), disassoc_data, timeout, FALSE, NULL, NULL);
		if (!ret) {
...
/* Create a Timer */
static int
wl_wlif_create_timer(void *uschd_hdl, bcm_timer_module_id timer_hdl, void* cbfn, void *data,
	int timeout, int repeat, void* comparefn, bcm_timer_id *out_timer_id)
{
	int ret = -1;
	bcm_timer_id timer_id;
	...
	/* If uschd library is used, Create uschd timer */
		...
		entry.interval = SEC_TO_MICROSEC((unsigned long long)timeout);
		entry.repeat_flag = repeat;
		entry.cbfn = cbfn;
		entry.arg = data;
		entry.comparefn = comparefn;
		/* Create Timer */
		ret = bcm_usched_add_timer_ex(uschd_hdl, &entry);
	...
	int timeout, short flag)
{
	...
	while (fgets(line, sizeof(line), fp) != NULL) {
		...
		if ((strncmp(entry.macaddr, eabuf, strlen(entry.macaddr)) == 0) &&
			(strcmp(entry.ifname, hdl->ifname) == 0)) {
			add = FALSE;
			break;
		}
	}
...
wl_wlif_block_mac(wl_wlif_hdl *hdl, struct ether_addr addr, int timeout)
{
	...
	static short flag;
	wl_wlif_hdl_data_t *hdl_data;
	static_maclist_t *timer_arg = NULL;
	...
	WLIF_BSSTRANS("%s %s: Blocking "MACF" for %d seconds\n", hdl_data->appname,
		hdl_data->ifname, ETHER_TO_MACF(addr), timeout);

	/* Get the updated mac list to be set. If there is no change, find the timer entry
	 * corresponding to the sta in the usched timers list. If entry is present then remove
	 * the existing unblock timer so that sta gets blocked for the new timeout value of the
	 * current request.
	 */
	if (!wl_update_block_mac_list(s_maclist, maclist, macmode, &addr, TRUE)) {
		wl_wlif_timercmp_data_t timercmp_data;

		memset(&timercmp_data, 0, sizeof(timercmp_data));
		timercmp_data.ifname = hdl_data->ifname;
		timercmp_data.addr = &addr;
		timer_arg = bcm_usched_find_timer(hdl_data->uschd_hdl,
			wl_wlif_unblock_mac_usched_cb, wl_wlif_compare_macblock_timer_args,
			&timercmp_data);
		if (timer_arg != NULL) {
			WLIF_BSSTRANS("%s: Entry for "MACF" already present in block mac list. "
				"Updating unblock time to %d\n", hdl_data->ifname,
				ETHER_TO_MACF(addr), timeout);
			bcm_usched_remove_timer(hdl_data->uschd_hdl,
				wl_wlif_unblock_mac_usched_cb, timer_arg);
			free(timer_arg);
		} else {
			WLIF_BSSTRANS("%s: MAC list unchanged while blocking "MACF". Unable to "
				"locate a matching entry in timer list. Returning.\n",
...
wl_set_channel_util(wl_wlif_hdl *hdl, uint8 chan_util)
{
	...
	*param = chan_util;	/* Initialize the channel utilization value */
	...
	ret = hdl_data->ioctl_hndlr(hdl_data->ifname, WLC_SET_VAR, (void *)maclist_buf,
			sizeof(maclist_buf), hdl_data->data);
	if (ret < 0) {
		WLIF_BSSTRANS("%s: Error setting bssload_fake_chan_util ret %d\n",
			hdl_data->ifname, ret);
		return ret;
	}
	return  ret;
...
wl_wlif_do_bss_trans(wl_wlif_hdl *hdl, wl_wlif_bss_trans_data_t *ioctl_hndlr_data)
{
	...
	CE_SEND_CEVENT_A2C_EXT(hdl_data->ifname, &ioctl_hndlr_data->bssid, 0,
		ioctl_hndlr_data->reason, ce_subtype, CEVENT_A2C_MT_BTM_REQ,
		CEVENT_FRAME_DIR_TX, NULL, 0);
#endif /* BCM_CEVENT */
...
wl_wlif_unblock_mac(wl_wlif_hdl *hdl, struct ether_addr addr, int flag)
{
	...
	WLIF_BSSTRANS("%s %s: Unblocking STA "MACF" from maclist\n", hdl_data->appname,
		hdl_data->ifname, ETHER_TO_MACF(addr));
	...
	struct ether_addr addr, int timeout, int flag)
{
	...
	ret = wl_wlif_create_timer(hdl_data->uschd_hdl, hdl_data->timer_hdl,
		(hdl_data->uschd_hdl ? wl_wlif_unblock_mac_usched_cb : wl_wlif_unblock_mac_cb),
		tmr_data, timeout, FALSE, wl_wlif_compare_macblock_timer_args, &timer_id);
	if (!ret) {
	...
	struct ether_addr addr, int timeout)
{
	...
	ret = wl_wlif_create_timer(hdl_data->uschd_hdl, hdl_data->timer_hdl,
		(hdl_data->uschd_hdl ? wl_wlif_defiant_sta_wd_usched_cb :
		wl_wlif_defiant_sta_wd_cb), defiant_tmr_data, tm_defiant_wd, FALSE, NULL, NULL);
	if (!ret) {
...
wl_wlif_unset_nobcnssid(wl_wlif_hdl *hdl)
{
	...
	if (IS_FAKE_CHAN_UTIL_ENABLED(hdl_data->steer_flags) &&
		((ret = wl_wlif_clear_fake_chan_util_for_sta(hdl, NULL)) != 0))
	{
...
wl_wlif_create_nobcnssid_timer(wl_wlif_hdl_data_t *hdl_data, int timeout)
{
	...
	ret = wl_wlif_create_timer(hdl_data->uschd_hdl, hdl_data->timer_hdl,
		(hdl_data->uschd_hdl ? wl_wlif_unset_nobcnssid_usched_cb :
		wl_wlif_unset_nobcnssid_cb), tm_data, timeout, FALSE, NULL, &timer_id);
	if (!ret) {
	...
	tm_data->timer_id = timer_id;
	hdl_data->nobcnssid_timer_data = tm_data;
...
/* Remove nobcnssid timer */
static void
wl_wlif_remove_nobcnssid_timer(wl_wlif_hdl_data_t *hdl_data)
{
	static_maclist_t *data;

	if (!hdl_data || !hdl_data->nobcnssid_timer_data) {
		return;
	}

	data = (static_maclist_t*)hdl_data->nobcnssid_timer_data;
	WLIF_BSSTRANS("%s: Removing nobcnssid timer \n", hdl_data->ifname);

	wl_wlif_unset_nobcnssid(hdl_data);
	...
	if (hdl_data->uschd_hdl == NULL) {
		...
		bcm_usched_remove_timer(hdl_data->uschd_hdl,
			wl_wlif_unset_nobcnssid_usched_cb, data);
	}

	/* Free the argument */
	free(data);
	hdl_data->nobcnssid_timer_data = NULL;
}
...
wl_wlif_set_nobcnssid(wl_wlif_hdl *hdl)
{
	...
	if (IS_FAKE_CHAN_UTIL_ENABLED(hdl_data->steer_flags) &&
		((ret = wl_wlif_set_fake_chan_util_for_sta(hdl, NULL)) != 0))
	{
		...
		wl_wlif_bss_trans_data_t *ioctl_hndlr_data)
{
	...
	while (ptr) {
		if ((!oui_cmp(ptr->oui, &ioctl_hndlr_data->addr)) ||
			...
			/* Create an Appropriate Timer, For Defiant STA Watchdog */
			ret = wl_wlif_create_timer(hdl_data->uschd_hdl, hdl_data->timer_hdl,
				(hdl_data->uschd_hdl ? wl_wlif_post_bss_transreq_usched_cb :
				wl_wlif_post_bss_transreq_cb), tmr_data, timeout,
				FALSE, NULL, NULL);
			if (!ret) {
				free(tmr_data);
				return FALSE;
...
/* Compare function for the macblock request timer on successful match returns 1 otherwise 0 */
static int
wl_wlif_compare_macblock_timer_args(void *arg1, void *arg2)
{
	static_maclist_t *maclist_ptr;
	wl_wlif_timercmp_data_t *timercmp_data;
	int ret;

	maclist_ptr = (static_maclist_t*)arg1;
	timercmp_data = (wl_wlif_timercmp_data_t *)arg2;

	ret = (eacmp(&maclist_ptr->addr, timercmp_data->addr) == 0) &&
		(strcmp(maclist_ptr->hdl_data->ifname, timercmp_data->ifname) == 0);
