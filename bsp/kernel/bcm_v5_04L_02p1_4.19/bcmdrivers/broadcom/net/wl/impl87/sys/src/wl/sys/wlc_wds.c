 ...
 *
 * $Id: wlc_wds.c 804115 2021-10-20 13:36:09Z $
 */
...
wlc_wds_calc_multiap_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (cfg->multiap_ie[TLV_LEN_OFF] > 0) {
		if (scb && !SCB_MAP_P2_CAP(scb)) {
			/* If the MAP BH STA is not supporting profile 2,
			 * consider only profile 1 Multi-AP IE length
			 */
			len = TLV_HDR_LEN + MAP_IE_FIXED_LEN + MAP_EXT_ATTR_LEN + TLV_HDR_LEN;
		} else {
...
wlc_wds_write_multiap_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (cfg->multiap_ie[TLV_LEN_OFF] > 0) {
		uint len = cfg->multiap_ie[TLV_LEN_OFF];
		/* If the MAP BH STA is not supporting profile 2,
		 * write only profile 1 Multi-AP IE
		 */
		if (scb && !SCB_MAP_P2_CAP(scb)) {
			cfg->multiap_ie[TLV_LEN_OFF] =
				MAP_IE_FIXED_LEN + MAP_EXT_ATTR_LEN + TLV_HDR_LEN;
		}
		bcm_copy_tlv(cfg->multiap_ie, data->buf);
		/* Restore original Multi-AP IE length */
		cfg->multiap_ie[TLV_LEN_OFF] = len;
	}
...
wlc_dwds_dump_sa_list(void *ctx, wlc_bsscfg_t *cfg, struct bcmstrbuf *b)
{
	...
	bcm_bprintf(b, "----- DWDS STA loop back list bsscfg_idx[%d]---------\n",
		WLC_BSSCFG_IDX(cfg));
	for (hash = 0; hash < DWDS_SA_HASH_SZ; hash++) {
		sta = cfg->sa_list[hash];
		while (sta != NULL) {
...
void
wlc_wds_scb_switch_queue(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_txq_info_t *new_q)
{
	struct scb *scb;
	struct scb_iter scbiter;
