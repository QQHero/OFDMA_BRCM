 ...
 * $Id: km_scb.c 803784 2021-10-07 12:42:20Z $
 */
...
km_scb_cleanup(keymgmt_t *km, scb_t *scb)
{
	 ...
	 */
	other_scb = NULL;
	if (!KM_ADDR_IS_BCMC(&scb->ea) && !IS_SINGLEBAND(km->wlc)) {
		enum wlc_bandunit bandunit;
		FOREACH_WLC_BAND(km->wlc, bandunit) {
			if (bandunit == scb->bandunit) {
				continue; /* search only other bands */
			}
			other_scb = wlc_scbfindband(km->wlc, SCB_BSSCFG(scb), &scb->ea, bandunit);
			if (other_scb != NULL) {
				break;
			}
		}
...
#if defined(IBSS_PEER_GROUP_KEY)
	/* XXX IBSS indicies are allocated along with pairwise key. They are
	 * offset by WSEC_IBSS_MAX_PEERS by convention. We may look into
	 * relaxing this. these must also use the last set of indicies
	 * from key_idx_arr
	 */
	if (KM_IBSS_PGK_ENABLED(km) && BSSCFG_IBSS(bsscfg)) {
		key_idx = key_idx_arr[key_idx_arr_pos++];
	...
	wlc_bsscfg_t *bsscfg, wlc_key_info_t *key_info)
{
	...
	if (scb) {
		...
		if (kc && !BSSCFG_PSTA(scb->bsscfg) && !ether_cmp(&scb->ea, &kc->key_info->addr)) {
			key = kc->key;
			if (key_info) {
...
km_scb_amt_alloc(keymgmt_t *km, scb_t *scb)
{
	...
	if ((BSSCFG_AP(bsscfg) && WLC_ACKSUPR(km->wlc))) {
		...
		if (amt_idx != KM_HW_AMT_IDX_INVALID) {
			...
			KM_LOG(("wl%d.%d: %s: allocate amt_idx %d for "MACF"\n",
				km->wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
				__FUNCTION__, amt_idx, CONST_ETHERP_TO_MACF(ea)));
			goto done;
		}
	}
...
upd_amt:
	...
	KM_LOG(("wl%d.%d: %s: reserved amt index %d for addr "MACF"\n",
		KM_UNIT(km), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
		amt_idx,  CONST_ETHERP_TO_MACF(ea)));
	return amt_idx;
}
