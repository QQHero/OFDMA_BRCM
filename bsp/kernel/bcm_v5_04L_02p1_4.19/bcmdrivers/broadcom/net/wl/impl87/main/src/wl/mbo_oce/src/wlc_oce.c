...
static const chanspec_t oce_pref_2G_chanspecs[] = {
	CH20MHZ_CHSPEC(1, WL_CHANSPEC_BAND_2G),
	CH20MHZ_CHSPEC(6, WL_CHANSPEC_BAND_2G),
	CH20MHZ_CHSPEC(11, WL_CHANSPEC_BAND_2G),
	};
...
wlc_oce_wlc_up(void *ctx)
{
	...
	if (oce->wlc->upr_fd_info && oce->wlc->upr_fd_info->cfg) {
		wlc_oce_set_short_ssid(oce->wlc, oce->wlc->upr_fd_info->cfg);
	}

	if ((oce->wlc->lpi_mode == AUTO && oce->wlc->stf->psd_limit_indicator) ||
		(oce->wlc->lpi_mode == ON)) {
		oce->oce_cmn_info->non_ht_ppdu = TRUE;
	}
#endif /* WL_OCE_AP */
...
wlc_oce_wlc_down(void *ctx)
{
	...
	oce->oce_cmn_info->non_ht_ppdu = FALSE;
#endif /* WL_OCE_AP */
...
wlc_oce_get_pref_channels(chanspec_t *chanspec_list)
{
	...
	STATIC_ASSERT(ARRAYSIZE(oce_pref_2G_chanspecs) <= SCAN_MAX_CHANSPECS);

	for (i = 0; i < ARRAYSIZE(oce_pref_2G_chanspecs); i++) {
		chanspec_list[i] = oce_pref_2G_chanspecs[i];
	}

	return ARRAYSIZE(oce_pref_2G_chanspecs);
}
...
bool wlc_oce_is_pref_channel(chanspec_t chanspec)
{
	...
	for (i = 0; i < ARRAYSIZE(oce_pref_2G_chanspecs); i++) {
		if (chanspec == oce_pref_2G_chanspecs[i])
			return TRUE;
	}
	...
	while (len >= NEIGHBOR_AP_INFO_FIELD_HDR_LEN) {
		...
		for (i = 0; i <= tbtt_count; i++) {
			...
			if ((tbtt_info_length >= 1) && chan_6g_valid) {
				memcpy((uint8*)&tbtt_info, cp, MIN(tbtt_info_length,
					sizeof(tbtt_info)));
#if BAND6G
...
wlc_oce_get_fd_frame_len(wlc_oce_info_t *oce, wlc_bsscfg_t *bsscfg)
{
	...
	if (oce->oce_cmn_info->non_ht_ppdu) {
		frame_len += FD_INFO_OPCLASS_SUBFIELD_SIZE + FD_INFO_PRIM_CHAN_SUBFIELD_SIZE;
	}
	...
	uint8 *buf, uint16 buf_len)
{
	...
	/* add fils discovery information field IE:
	 *  -----------------------------------------------------------------
	 * |frame   | time | BCN | ssid/ | len | fd  | reg | prim.| optional |
	 * |control | stamp| dur |s_ssid |     | cap |     | ch   | fields   |
	 *  -----------------------------------------------------------------
	 *
	 */
	...
	ptr = fd_info->disc_info;
	pbuf = buf;

	memcpy(ptr, &oce->oce_cmn_info->short_ssid, SHORT_SSID_LEN);
	ptr += SHORT_SSID_LEN;

	*ptr = sizeof(oce->oce_cmn_info->fd_cap) + (oce->oce_cmn_info->non_ht_ppdu ? 2 : 0);
	ptr++;

	memcpy(ptr, &oce->oce_cmn_info->fd_cap, sizeof(oce->oce_cmn_info->fd_cap));
	ptr += sizeof(oce->oce_cmn_info->fd_cap);

	if (oce->oce_cmn_info->non_ht_ppdu) {
		FD_INFO_SET_PRIMARY_CH_PRESENT(fd_info->framecontrol);
		*ptr++ = wlc_get_regclass(oce->wlc->cmi, bsscfg->current_bss->chanspec);
		*ptr++ = wf_chspec_primary20_chan(bsscfg->current_bss->chanspec);
	}
	...
	if (BAND_6G(wlc->band->bandtype)) {
		(wlc_write_transmit_power_envelope_ie_6g(wlc,
		     bsscfg->current_bss->chanspec, ptr, (buf_len - (ptr - pbuf))));
	}
...
wlc_oce_update_ap_chrep(wlc_oce_info_t* oce, wlc_bsscfg_t *bsscfg)
{
	...
	for (i = 0; i < bss_count; i++) {
		wlc_bss_info_t *bi = oce->wlc->scan_results->ptrs[i];
		uint16 primary_chan = wf_chspec_primary20_chan(bi->chanspec);
	...
	size_t ilen, uint8 *obuf, size_t *olen)
{
	...
	if (OCE_ENAB(oce->wlc->pub)) {
		...
		if (*data) {
			...
			/* update rnr tbtt len to 11 instead of 13, current test plan: SEP 2020
			 * expects tbtt len to be 11.
			 * Expect latest OCE cert will update tbtt_len to 13, in that case this
			 * change is not required
			 */
			oce->wlc->rnr_tbtt_len = NBR_AP_TBTT_BSSID_SHORT_SSID_LEN;
		} else {
			obc->flags &= ~OCE_BSSCFG_ENABLE;
		}
		/* update retry number of retry probe response as per OCE SPEC */
		wlc_ap_update_retry_prb_rsp(oce->wlc);
	} else {
	...
	size_t ilen, uint8 *obuf, size_t *olen)
{
	...
	ret = wlc_ap_set_assoc_rej_rssi_th(oce->wlc, dig->bsscfg, *data);
	return ret;
}
	...
	size_t ilen, uint8 *obuf, size_t *olen)
{
	...
	if ((ret = wlc_ap_get_assoc_rej_rssi_th(oce->wlc, dig->bsscfg, &rssi_th)) != BCME_OK) {
		WL_ERROR(("wl%d.%d Error in getting assoc reject rssi threshold\n",
			oce->wlc->pub->unit, WLC_BSSCFG_IDX(dig->bsscfg)));
		return ret;
	}
	xtlv_size = bcm_xtlv_size_for_data(sizeof(rssi_th),
		BCM_XTLV_OPTION_ALIGN32);
	if (xtlv_size > *olen) {
		WL_OCE_ERR(("wl%d: %s: short buffer length %d expected %u\n",
			oce->wlc->pub->unit, __FUNCTION__, (int)*olen, xtlv_size));
