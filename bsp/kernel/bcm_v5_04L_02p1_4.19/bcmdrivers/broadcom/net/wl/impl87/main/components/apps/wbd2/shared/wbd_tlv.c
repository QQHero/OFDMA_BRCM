 ...
 *
 * $Id: wbd_tlv.c 802267 2021-08-20 09:59:45Z $
 */
...
static wbd_tlv_types_name_t wbd_tlv_list_type_name[] = {
	...
	{WBD_TLV_TYPE_UNUSED_9, "Unused TLV"},
	{WBD_TLV_TYPE_UNUSED_10, "Unused TLV"},
	...
	{WBD_TLV_VNDR_STRONG_STA_POLICY_TYPE, "Brcm Strong STA Reporting Policy Vndr TLV"},
};
	...
	*pbuf = (unsigned char)cmd->entry_list.count;
	...
	foreach_glist_item(fbt_item_p, cmd->entry_list) {
		...
		*pbuf = (unsigned char)fbt_data->len_r0kh_key;
		...
		if (I5_IS_SPLIT_VNDR_MSG(controller_device->flags)) {
			/* 2 bytes for mdid */
			memcpy(pbuf, (unsigned char *) &(fbt_data->fbt_info.mdid), 2);
			pbuf += 2;
		}
	}
...
{
	int ret = WBDE_TLV_ERROR, iter = 0, num_entries = 0;
	wbd_cmd_fbt_config_t *cmd = (wbd_cmd_fbt_config_t *)data;
	...
	/* 1 byte for the num of FBT enabled BSSs included in this TLV */
	num_entries = (unsigned char)pValue[pos];
	pos++;
	...
	for (iter = 0; iter < num_entries; iter++) {
		...
		if (I5_IS_SPLIT_VNDR_MSG(src_device->flags)) {
			/* 2 bytes for mdid */
			memcpy((unsigned char *) &(new_fbt_bss.fbt_info.mdid), &pValue[pos], 2);
			pos += 2;
		}

		/* Add a FBT BSS item in a Slave's FBT Request List */
		wbd_add_item_in_fbt_cmdlist(&new_fbt_bss, cmd, NULL, FALSE);
	}
	ret = WBDE_OK;
	...
	bool split_vndr_msg)
{
	...
	/* 1 byte for the num of FBT enabled BSSs included in this TLV */
	*pbuf = (unsigned char)cmd->entry_list.count;
	/* For older devices which dont support split vendor message limit the no. of
	 * config response entries to 35
	*/
	if (!split_vndr_msg && *pbuf > WBD_MAX_FBT_BSS_IN_FRAME) {
		*pbuf = WBD_MAX_FBT_BSS_IN_FRAME;
	}
	count = *pbuf;
	pbuf++;

	/* Travese FBT Config Response List items */
	foreach_glist_item_ex(fbt_item_p, cmd->entry_list, count > 0) {
...
int wbd_tlv_decode_vndr_metric_policy(void* data,
	unsigned char* tlv_data, unsigned int tlv_data_len, unsigned int *read)
{
...
int
wbd_tlv_encode_weak_strong_client_response(void* data,
	unsigned char* tlv_data, unsigned int* tlv_data_len)
{
	int ret = WBDE_OK;
	wbd_cmd_weak_strong_client_resp_t *cmdclientresp = (wbd_cmd_weak_strong_client_resp_t*)data;
	unsigned char *pbuf, *pmem;
	i5_tlv_t *ptlv;
	WBD_ENTER();
	...
	memcpy(pbuf, &cmdclientresp->sta_mac, MAC_ADDR_LEN);
	pbuf += MAC_ADDR_LEN;

	/* 6 bytes for BSSID of the BSS for which the STA is associated */
	memcpy(pbuf, &cmdclientresp->BSSID, MAC_ADDR_LEN);
	pbuf += MAC_ADDR_LEN;

	/* 1 bytes for reason code */
	*pbuf = (unsigned char)cmdclientresp->error_code;
	pbuf++;

	/* 4 bytes for dwell time */
	*((unsigned int *)pbuf) = htonl(cmdclientresp->dwell_time);
	pbuf += 4;
...
/* Decode Vendor Specific TLV for Message : weak/strong client response on recieve */
int
wbd_tlv_decode_weak_strong_client_response(void* data,
	unsigned char* tlv_data, unsigned int tlv_data_len)
{
	int ret = WBDE_OK;
	wbd_cmd_weak_strong_client_resp_t *cmdclientresp = (wbd_cmd_weak_strong_client_resp_t*)data;
	...
	if (tlv_data_len < WBD_MAX_LEN_TLV_WEAK_CLIENT_RESP) {
		WBD_WARNING("tlv_data_len[%d] < Weak/Strong Client Resp Len[%d]\n",
	...
	memcpy(&cmdclientresp->sta_mac, &pValue[pos], MAC_ADDR_LEN);
	pos += MAC_ADDR_LEN;

	/* 6 bytes for BSSID of the BSS for which the STA is associated */
	memcpy(&cmdclientresp->BSSID, &pValue[pos], MAC_ADDR_LEN);
	pos += MAC_ADDR_LEN;

	/* 1 byte for the reason code */
	cmdclientresp->error_code = (unsigned char)pValue[pos];
	pos++;

	/* 4 byte for the dwell time */
	cmdclientresp->dwell_time = ntohl(*((unsigned int *)&pValue[pos]));
	pos += 4;
	...
	unsigned int* tlv_data_len)
{
	...
	*pbuf = (unsigned char)cmd->n_bsss;
	...
	/* 6 bytes for MAC address of interface */
	memcpy(pbuf, &cmd->mac, sizeof(cmd->mac));
	pbuf += sizeof(cmd->mac);

	/* 1 bytes for Number of Number of SSIDs */
	*pbuf = (unsigned char)cmd->n_ssid;
	pbuf++;

	/* Fill all the BSSs */
	if (cmd->n_ssid > 0 && cmd->ssid_nvrams) {
		for (idx = 0; idx < cmd->n_ssid; idx++) {
			/* 1 bytes for SSID length */
			*pbuf = (unsigned char)cmd->ssid_nvrams[idx].ssid.SSID_len;
			pbuf++;

			/* SSID */
			memcpy(pbuf, &cmd->ssid_nvrams[idx].ssid.SSID,
				cmd->ssid_nvrams[idx].ssid.SSID_len);
			pbuf += cmd->ssid_nvrams[idx].ssid.SSID_len;

			/* 1 bytes for Number of NVRAMs */
			*pbuf = (unsigned char)cmd->ssid_nvrams[idx].n_nvrams;
			pbuf++;

			/* Fill all the BSS NVRAMs */
			if (cmd->ssid_nvrams[idx].n_nvrams <= 0 ||
				!cmd->ssid_nvrams[idx].nvrams) {
				continue;
			}

			for (k = 0; k < cmd->ssid_nvrams[idx].n_nvrams; k++) {
				/* 1 bytes for NVRAM name length */
				*pbuf = (unsigned char)strlen(cmd->ssid_nvrams[idx].nvrams[k].name);
				pbuf++;

				/* Name of the NVRAM */
				if (strlen(cmd->ssid_nvrams[idx].nvrams[k].name) > 0) {
					memcpy(pbuf, cmd->ssid_nvrams[idx].nvrams[k].name,
						strlen(cmd->ssid_nvrams[idx].nvrams[k].name));
					pbuf += strlen(cmd->ssid_nvrams[idx].nvrams[k].name);
				}

				/* 1 bytes for NVRAM value length */
				*pbuf = (unsigned char)strlen(
					cmd->ssid_nvrams[idx].nvrams[k].value);
				pbuf++;

				/* Value of the NVRAM */
				if (strlen(cmd->ssid_nvrams[idx].nvrams[k].value) > 0) {
					memcpy(pbuf, cmd->ssid_nvrams[idx].nvrams[k].value,
						strlen(cmd->ssid_nvrams[idx].nvrams[k].value));
					pbuf += strlen(cmd->ssid_nvrams[idx].nvrams[k].value);
				}
			}
		}
	}
	...
	unsigned int tlv_data_len)
{
	...
	memcpy(&cmd->mac, &pValue[pos], sizeof(cmd->mac));
	pos += sizeof(cmd->mac);

	/* 1 bytes for Number of SSIDs */
	if (tlv_data_len < pos + 1) {
		goto end;
	}
	cmd->n_ssid = (unsigned char)pValue[pos];
	pos++;
	...
	if (cmd->n_ssid > 0) {
		...
		for (idx = 0; idx < cmd->n_ssid; idx++) {
			/* 1 bytes for Length of SSID */
			if (tlv_data_len < pos + 1) {
				goto end;
			}
			cmd->ssid_nvrams[idx].ssid.SSID_len = (unsigned char)pValue[pos];
			pos++;

			/* SSID */
			if (tlv_data_len < pos + cmd->ssid_nvrams[idx].ssid.SSID_len) {
				goto end;
			}
			memcpy(&cmd->ssid_nvrams[idx].ssid.SSID, &pValue[pos],
				cmd->ssid_nvrams[idx].ssid.SSID_len);
			pos += cmd->ssid_nvrams[idx].ssid.SSID_len;

			/* 1 bytes for Number of SSID NVRAMs */
			if (tlv_data_len < pos + 1) {
				goto end;
			}
			cmd->ssid_nvrams[idx].n_nvrams = (unsigned char)pValue[pos];
			pos++;
			...
			cmd->ssid_nvrams[idx].nvrams = (wbd_cmd_vndr_nvram_t*)
				wbd_malloc((sizeof(wbd_cmd_vndr_nvram_t) *
				cmd->ssid_nvrams[idx].n_nvrams), &ret);
			WBD_ASSERT();
			...
			for (k = 0; k < cmd->ssid_nvrams[idx].n_nvrams; k++) {
				...
				if (name_len > 0) {
					...
					memcpy(cmd->ssid_nvrams[idx].nvrams[k].name, &pValue[pos],
						name_len);
					pos += name_len;
				}
				...
				if (value_len > 0) {
					...
					memcpy(cmd->ssid_nvrams[idx].nvrams[k].value, &pValue[pos],
						value_len);
					pos += value_len;
				}
				WBD_DEBUG("pos[%d] tlv_data_len[%d] SSID[%s] name_len[%d] "
					"name[%s] value_len[%d] value[%s]\n", pos, tlv_data_len,
					cmd->ssid_nvrams[idx].ssid.SSID, name_len,
					cmd->ssid_nvrams[idx].nvrams[k].name, value_len,
					cmd->ssid_nvrams[idx].nvrams[k].value);
			}
		}
	}
...
/* Encode Vendor Specific TLV : Strong Sta Reporting Policy Vendor Data to send */
void wbd_tlv_encode_vndr_strong_sta_policy(void* data,
	unsigned char* tlv_data, unsigned int* tlv_data_len)
{
	int ret = WBDE_OK;
	wbd_cmd_vndr_strong_sta_policy_config_t *cmd =
		(wbd_cmd_vndr_strong_sta_policy_config_t *)data;
	unsigned char *pbuf, *pmem;
	i5_tlv_t *ptlv;
	dll_t *policy_item_p = NULL;
	wbd_strong_sta_policy_ifr_entry_t *policy_data = NULL;
	WBD_ENTER();
	...
	*pbuf = (unsigned char)cmd->entry_list.count;
	...
	foreach_glist_item(policy_item_p, cmd->entry_list) {
		...
		/* 4 bytes for RSSI to measure STA is strong */
		pbuf += i5_cpy_host32_to_netbuf(pbuf, policy_data->vndr_policy.t_rssi);

		/* 4 bytes for RSSI hysterisis */
		pbuf += i5_cpy_host32_to_netbuf(pbuf, policy_data->vndr_policy.t_hysterisis);
	...
	/* Fill the TLV Hdr now */
	ptlv = (i5_tlv_t *)pmem;
	ptlv->type = WBD_TLV_VNDR_STRONG_STA_POLICY_TYPE;
	ptlv->length = htons(pbuf-pmem-sizeof(i5_tlv_t));
...
/* Decode Vendor Specific TLV : Metric Reporting Policy Vendor Data on receive */
int wbd_tlv_decode_vndr_strong_sta_policy(void* data,
	unsigned char* tlv_data, unsigned int tlv_data_len, unsigned int *read)
{
	int ret = WBDE_TLV_ERROR, iter = 0;
	wbd_cmd_vndr_strong_sta_policy_config_t *cmd =
	...
	*read = 0;
	...
	if (tlv_data_len < WBD_MIN_LEN_TLV_STRONG_STA_POLICY) {
		WBD_WARNING("tlv_data_len[%d] < Vendor Metric Policy Min Len[%d]\n",
			tlv_data_len, WBD_MIN_LEN_TLV_STRONG_STA_POLICY);
		goto end;
	}
	...
	if (tlv_data_len < (pos + (cmd->num_entries * (WBD_MAX_LEN_TLV_STRONG_STA_POLICY_ITEM)))) {
		WBD_INFO("tlv_data_len[%d] < Vendor Metric Policy Len[%d]\n",
			tlv_data_len,
			(pos + (cmd->num_entries * (WBD_MAX_LEN_TLV_STRONG_STA_POLICY_ITEM))));
		goto end;
	}

	for (iter = 0; iter < cmd->num_entries; iter++) {

		wbd_strong_sta_policy_ifr_entry_t new_policy_ifr;
		memset(&new_policy_ifr, 0, sizeof(new_policy_ifr));
		memset(&(new_policy_ifr.vndr_policy), 0, sizeof(new_policy_ifr.vndr_policy));
		...
		/* 4 bytes for rssi of STA */
		pos += i5_cpy_netbuf_to_host32(&(new_policy_ifr.vndr_policy.t_rssi),
			&pValue[pos]);

		/* 4 bytes for hysterisis */
		pos += i5_cpy_netbuf_to_host32(&(new_policy_ifr.vndr_policy.t_hysterisis),
			&pValue[pos]);
		...
		/* Add a Metric Policy item in a Metric Policy List */
		wbd_add_item_in_strong_sta_policylist(&new_policy_ifr, cmd, NULL);
	}
	*read = pos + sizeof(i5_tlv_t);
	ret = WBDE_OK;
