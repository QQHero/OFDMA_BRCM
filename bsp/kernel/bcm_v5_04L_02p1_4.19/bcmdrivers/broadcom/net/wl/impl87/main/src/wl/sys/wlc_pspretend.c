 ...
 *
 * $Id: wlc_pspretend.c 806974 2022-01-10 16:24:33Z $
 */
...
#define WLC_PSPRETEND_PROBE_TIMER_INTERVAL		10		/* milliseconds */
...
/* others */
static void wlc_pspretend_doprobe(wlc_pps_info_t *pps, struct scb *scb, uint32 elapsed_time);
 ...
 */
void
wlc_pspretend_dotxstatus(wlc_pps_info_t *pps, scb_t *scb, bool pps_recvd_ack)
{
	wlc_info_t *wlc = pps->wlc;
	...
	*/
	if (D11REV_GE(wlc->pub->corerev, 40)) {
		...
		if (!SCB_PS_PRETEND_THRESHOLD(scb) &&
		    ...
		    (SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb) == 0)) {
	...
	if (SCB_TOT_PKTS_INFLT_FIFOCNT_VAL(scb) == 0) {
		...
		if (!SCB_PS_PRETEND_PROBING(scb)) {
			wlc_ap_reset_psp_attempt(wlc->ap, scb, 0);
			scb->ps_pretend |= PS_PRETEND_PROBING;
			/* send a probe immediately to validate the PS pretend PM mode */
			wlc_pspretend_doprobe(pps, scb, 0);
		}

		/* The probing continues work on the back of a periodic timer. If that timer isn't
		 * running, then start it with half the normal interval, so on average we get the
		 * same till second probe.
		 */
		if (pps->ps_pretend_probe_timer && !pps->is_ps_pretend_probe_timer_running) {
			wl_add_timer(wlc->wl, pps->ps_pretend_probe_timer,
				WLC_PSPRETEND_PROBE_TIMER_INTERVAL / 2, FALSE);
...
bool
wlc_pspretend_on(wlc_pps_info_t *pps, scb_t *scb, uint8 flags)
{
	wlc_info_t *wlc = pps->wlc;
	bool ps_retry = FALSE;
...
wlc_pspretend_doprobe(wlc_pps_info_t *pps, struct scb *scb, uint32 elapsed_time)
{
	...
	if (wlc_ap_pspretend_probe(wlc->ap, cfg, scb)) {
...
wlc_pspretend_probe(void *arg)
{
	...
	scb_t *scb;
	scb_pps_info_t *pps_scb;
	bool ps_pretend_probe_continue;
	uint32 tsf_low;
	uint32 pretend_elapsed;
	...
	ps_pretend_probe_continue = FALSE;
	tsf_low = R_REG(wlc->osh, D11_TSFTimerLow(wlc));
	FOREACHSCB(wlc->scbstate, &scbiter, scb) {
		...
		if (SCB_PS_PRETEND_PROBING(scb)) {
			...
			pretend_elapsed = (tsf_low - pps_scb->ps_pretend_start) / 1000;
			wlc_pspretend_doprobe(pps, scb, pretend_elapsed);
		}
	}

	/* restart timer as long as at least one scb is in pretend state */
	if (ps_pretend_probe_continue) {
		/* 10 milliseconds */
		wl_add_timer(wlc->wl, pps->ps_pretend_probe_timer,
			WLC_PSPRETEND_PROBE_TIMER_INTERVAL, FALSE);
		pps->is_ps_pretend_probe_timer_running = TRUE;
	}
}
...
void
wlc_pspretend_probe_sched(wlc_pps_info_t *pps, scb_t *scb)
{
	scb_pps_info_t *pps_scb = SCB_PPSINFO(pps, scb);
	wlc_info_t *wlc = pps->wlc;
	uint32 elapsed;
	...
	wlc_pspretend_doprobe(pps, scb, elapsed);
}

/* Returns TRUE if packet should be freed, FALSE in case it is requeued */
bool
wlc_pspretend_pkt_retry(wlc_pps_info_t *pps, scb_t *scb, void *pkt, tx_status_t *txs,
	d11txhdr_t *txh)
{
	scb_pps_info_t *pps_scb = SCB_PPSINFO(pps, scb);
	wlc_bsscfg_t *cfg;
	uint16 macctlhigh_val;
	uint16 *macctlhigh_ptr;
	...
	cfg = SCB_BSSCFG(scb);
	/* check to see if the succession of pps transitions exceed the limit or
	 * the scb is in psp prevent state
	 */
	if ((pps_scb->ps_pretend_succ_count >= cfg->ps_pretend_retry_limit) ||
		(scb->ps_pretend & PS_PRETEND_PREVENT)) {
		macctlhigh_ptr = D11_TXH_GET_MACHIGH_PTR(pps->wlc, txh);
		macctlhigh_val = ltoh16(*macctlhigh_ptr);

		/* The retry limit has been hit, so clear the PPS bit so
		 * they no longer trigger another ps pretend.
		 */
		macctlhigh_val &= ~D11AC_TXC_PPS;
		*macctlhigh_ptr = htol16(macctlhigh_val);
	}
...
void
wlc_pspretend_rate_reset(wlc_pps_info_t *pps, scb_t *scb, uint8 ac)
{
	wlc_info_t *wlc = pps->wlc;
	wlc_bsscfg_t *cfg;
	scb_pps_info_t *pps_scb = SCB_PPSINFO(pps, scb);
...
bool
wlc_pspretend_pkt_relist(wlc_pps_info_t *pps, scb_t *scb, int retry_limit, int txretry)
{
	int max_ps_retry = retry_limit + SCB_BSSCFG(scb)->ps_pretend_retry_limit;
 ...
 */
bool
wlc_pspretend_limit_transit(wlc_pps_info_t *pps, scb_t *scb, int in_transit, bool agg_enab)
{
	wlc_info_t *wlc = pps->wlc;
...
void
wlc_pspretend_scb_ps_off(wlc_pps_info_t *pps, scb_t *scb)
{
	scb_pps_info_t *pps_scb = SCB_PPSINFO(pps, scb);
...
uint
wlc_pspretend_scb_time_get(wlc_pps_info_t *pps, scb_t *scb)
{
	scb_pps_info_t *pps_scb = SCB_PPSINFO(pps, scb);
...
bool
wlc_pspretend_pps_retry(wlc_info_t *wlc, scb_t *scb, void *p, tx_status_t *txs)
{
