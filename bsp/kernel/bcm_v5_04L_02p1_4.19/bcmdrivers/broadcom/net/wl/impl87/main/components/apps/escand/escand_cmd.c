...
escand_proc_cmd(escand_wksp_t* d_info, char* buf, uint rcount, uint* r_size)
{
	...
	*c++ = '\0';
	...
	if (!strcmp(buf, "info")) {
		...
		*r_size += sprintf(buf+ *r_size, "escand poll_interval: %d usecs\n",
	...
	if (!strcmp(buf, "disable")) {
		...
		for (i = 0; i < ESCAND_MAX_IF_NUM; i++) {
			...
			if (!c_info) {
				ESCAND_PRINT("No c_info for intf %d\n", i);
				continue;
			}

			c_info->mode = ESCAND_MODE_DISABLE;
			ESCAND_PRINT("Disabling ESCAND on intf %d, %s\n", i, c_info->name);
		}
	...
	if (!strcmp(buf, "enable")) {
		...
		for (i = 0; i < ESCAND_MAX_IF_NUM; i++) {
			...
			c_info->mode = ESCAND_MODE_MONITOR;
			ESCAND_PRINT("Enabling ESCAND on intf %d, %s\n", i, c_info->name);
		}
	...
	if (!strcmp(buf, "dump")) {
		...
		if (!strcmp(param, "help")) {
			*r_size = sprintf(buf,
				"dump: stats bss scanresults intfmap");
	...
	if (!strcmp(buf, "get")) {
		...
		if (!strcmp(param, "escand_scan_promisc")) {
			*r_size = sprintf(buf, "%d", c_info->escand_scan_promisc);
			goto done;
		}
		if (!strcmp(param, "escand_ci_scan_type")) {
			*r_size = sprintf(buf, "%d (%s)", c_info->escand_ci_scan_type,
		...
		if (!strcmp(param, "escand_ci_busy_min_scan_timer")) {
			*r_size = sprintf(buf, "%d sec", c_info->escand_ci_busy_min_scan_timer);
			goto done;
		}
		if (!strcmp(param, "escand_traffic_type")) {
			*r_size = sprintf(buf, "0x%x", c_info->escand_traffic_type);
			goto done;
		}
		if (!strcmp(param, "escand_traffic_thresh")) {
			*r_size = sprintf(buf, "%d", c_info->escand_traffic_thresh);
			goto done;
		}
	...
	if (!strcmp(buf, "readchans")) {
		...
		if ((str = nvram_get(strcat_r(prefix, "escand_pref_chans", tmp))) == NULL)	{
			...
			chan_count = escand_set_chan_table(str, c_info->pref_chans.clist,
				ESCAND_MAX_LIST_LEN);
		...
		if ((str = nvram_get(strcat_r(prefix, "escand_excl_chans", tmp))) == NULL)	{
			...
			chan_count = escand_set_chan_table(str, c_info->excl_chans.clist,
				ESCAND_MAX_LIST_LEN);
		...
		ESCAND_PRINT("Scanlist:n");
		ret = escand_build_scanlist(c_info);
		goto done;
	}

	if (!strcmp(buf, "sendframe")) {
		char param[128];
		char val[16];
		int setval = 0;
		int index;
		...
		ESCAND_DEBUG("cmd: %s, data: %s, param: %s val: %d (0x%x), ifname: %s\n",
			buf, data, param, setval, setval, ifname);

		d_info->stats.valid_cmds++;

		if (!strcmp(param, "number")) {
			c_info->escand_nscantx = setval;
			*r_size = sprintf(buf, "%d", c_info->escand_nscantx);
			ESCAND_ERROR("cmd: %s, data: %s, param: %s val: %d (0x%x)\n",
				buf, data, param, setval, setval);
			goto done;
		}

		*r_size = sprintf(buf, "SENDFRAME: Unknown variable \"%s\".", param);
		err = -1;
		goto done;
	}
	...
	if (!strcmp(buf, "set")) {
		...
		if (!strcmp(param, "escand_scan_promisc")) {
			c_info->escand_scan_promisc = setval;
			*r_size = sprintf(buf, "%d", c_info->escand_scan_promisc);
			goto done;
		}
		if (!strcmp(param, "escand_ci_scan_type")) {
			c_info->escand_ci_scan_type = setval;
			*r_size = sprintf(buf, "%d (%s)", c_info->escand_ci_scan_type,
				(c_info->escand_ci_scan_type ? "passive" : "active"));
			goto done;
		}
		...
		if (!strcmp(param, "escand_ci_busy_min_scan_timer")) {
			c_info->escand_ci_busy_min_scan_timer = setval;
			*r_size = sprintf(buf, "%d sec", c_info->escand_ci_busy_min_scan_timer);
			goto done;
		}
	...
	if (!strcmp(buf, "traffic")) {
		...
		if (!strcmp(param, "thresh")) {
			c_info->escand_traffic_thresh = setval;
			*r_size = sprintf(buf, "%d", c_info->escand_traffic_thresh);
			ESCAND_ERROR("cmd: %s, data: %s, param: %s val: %d (0x%x)\n",
				buf, data, param, setval, setval);
			goto done;
		...
		if (!strcmp(param, "type")) {
			c_info->escand_traffic_type = setval;
			*r_size = sprintf(buf, "%d", c_info->escand_traffic_type);
			ESCAND_ERROR("cmd: %s, data: %s, param: %s val: %d (0x%x)\n",
				buf, data, param, setval, setval);
			goto done;
		}
