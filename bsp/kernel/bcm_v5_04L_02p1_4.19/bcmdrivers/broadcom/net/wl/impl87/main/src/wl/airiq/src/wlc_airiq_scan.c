...
airiq_update_scan_config(airiq_info_t *airiqh, airiq_config_t *sc)
{
	...
	cnt = MIN(MAX_SCAN_CHANSPECS, sc->chanspec_cnt); /* ensure bounds */
...
static void wlc_airiq_scantimer_3p1(void *arg)
{
	...
	if (airiqh->scan.scan_start) {
		...
		wlc_svmp_mem_read_axi(airiqh->wlc->hw, &fft_smpl_ctrl, airiqh->svmp_smpl_ctrl_addr, 1);
