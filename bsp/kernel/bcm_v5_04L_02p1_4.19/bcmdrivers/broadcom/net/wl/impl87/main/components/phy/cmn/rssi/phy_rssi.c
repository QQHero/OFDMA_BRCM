 ...
 *
 * $Id: phy_rssi.c 803650 2021-10-04 21:30:48Z $
 */
	...
	if (pi->u.pi_acphy->sniffer_aligner.enabled &&
		...
		if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
			if (CHSPEC_IS2G(pi->radio_chanspec)) {
				pll_wild_base_offset = (int32)(total * 5208) / 250000;
			} else {
				pll_wild_base_offset = (int32)(total * 4688) / 500000;
			}
		} else  {
			if (CHSPEC_IS2G(pi->radio_chanspec)) {
...
phy_rssi_compute_rssi(phy_info_t *pi, wlc_d11rxhdr_t *wrxh)
{
	...
	if (D11REV_GE(pi->sh->corerev, 128)) {
		...
		if (!RXS_GE128_VALID_PHYRXSTS(rxh, pi->sh->corerev)) {
			goto end;
		}
	} else {
...
#if defined(WLTEST)
void
wlc_phy_pkteng_rxstats_update(phy_info_t *pi, uint8 statidx)
{
