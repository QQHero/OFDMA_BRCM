 ...
 *
 * $Id: wlc_quiet.c 795628 2021-02-11 05:12:45Z $
 */
...
#ifdef AP
	{"quiet", IOV_QUIET, (IOVF_SET_UP|IOVF_BSSCFG_AP_ONLY), 0, IOVT_BUFFER,
	sizeof(dot11_quiet_t)},
#endif
	{NULL, 0, 0, 0, 0, 0}
};
...
/* local functions */
/* module */
static int wlc_quiet_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif);
static int wlc_quiet_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif);
...
BCMATTACHFN(wlc_quiet_attach)(wlc_info_t *wlc)
{
	...
	/* error handling */
fail:
	MODULE_DETACH(qm, wlc_quiet_detach);
	return NULL;
}
...
BCMATTACHFN(wlc_quiet_detach)(wlc_quiet_info_t *qm)
{
	...
	(void)wlc_module_remove_ioctl_fn(wlc->pub, qm);
...
static int
wlc_quiet_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	int err = BCME_OK;
	int32 int_val = 0;
...
static int
wlc_quiet_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
	wlc_quiet_info_t *qm = (wlc_quiet_info_t *)ctx;
	wlc_info_t *wlc = qm->wlc;
	int err = BCME_OK;
...
wlc_quiet_read_tsf(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint32 *tsfl, uint32 *tsfh)
{
	...
	wlc_force_ht(wlc, TRUE, &prev_wake);
...
#endif /* WLMCNX */
	wlc_force_ht(wlc, prev_wake, NULL);
	return mcnx_alter;
}
...
wlc_start_quiet1(wlc_quiet_info_t *qm, wlc_bsscfg_t *cfg)
{
	...
	*/
	...
	if (wlc_mac_request_entry(wlc, cfg, WLC_ACTION_QUIET, 0) != BCME_OK) {
...
wlc_start_quiet2(wlc_quiet_info_t *qm, wlc_bsscfg_t *cfg)
{
	...
	if (((tsf_lo == (uint32)-1) && (tsf_hi == (uint32)-1)) || (tsf_lo > quiet->end_tsf) ||
		((uint32)(quiet->end_tsf - tsf_lo) >
			(uint32)((quiet->duration + MAX_FIFO_DRAIN_TIME) * (DOT11_TU_TO_US)))) {
		/* If tsf_lo wrapped but end_tsf yet to wrap can make quiet duration very long,
		 * or we miscalculated all together due tsf change or tsf offset change,
		 * or wlc_read_tsf failed,
		 * end the quiet period immediately in these cases.
		 */
