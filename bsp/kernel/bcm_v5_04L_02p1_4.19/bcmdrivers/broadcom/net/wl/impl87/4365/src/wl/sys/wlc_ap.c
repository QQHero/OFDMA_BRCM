 ...
 *
 * $Id: wlc_ap.c 810943 2022-04-20 06:01:31Z $
 */
...
enum {
	...
	IOV_ASSOC_REJECT_RSSI_TH = 60,	/* reject association based on confiured rssi thd */
	IOV_KEEP_AP_UP = 61,	/**< Set/Unset AP can up if bsscfg->disable_ap_up is TRUE */
...
static const bcm_iovar_t wlc_ap_iovars[] = {
	...
	0, IOVT_BOOL, 0},
	{"assoc_rssi_th", IOV_ASSOC_REJECT_RSSI_TH, 0, IOVT_INT8, 0},
	{"keep_ap_up", IOV_KEEP_AP_UP, 0, IOVT_UINT8, 0},
	{NULL, 0, 0, 0, 0 }
};
...
send_result:
		...
		if (scb != NULL) {
			...
			if (mac_mode_deny_client && !scb) {
				...
				wlc_bss_mac_event(wlc, bsscfg, WLC_E_ASSOC_FAIL, &scb->ea,
...
wlc_ap_process_assocreq(wlc_ap_info_t *ap, wlc_bsscfg_t *bsscfg,
	struct dot11_management_header *hdr, uint8 *body, uint body_len,
	struct scb *scb, wlc_d11rxhdr_t *wrxh, bool short_preamble)
{
	wlc_ap_info_pvt_t *appvt = (wlc_ap_info_pvt_t *) ap;
	wlc_info_t *wlc = appvt->wlc;
	...
	if (ap_cfg->assoc_rej_rssi_thd && (wrxh->rssi < ap_cfg->assoc_rej_rssi_thd)) {
		...
		WL_ASSOC(("wl%d:%d  AP: **Reject Association due to insufficient RSSI (%d) "
			"by delta(%d)\n", wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), wrxh->rssi,
			(ap_cfg->assoc_rej_rssi_thd - wrxh->rssi)));
	...
	assoc_decision_t * dc, struct scb *scb, wlc_assoc_req_t * param)
{
	...
	/* STA has chosen SAE or DPP AKM? Is this result of hostapd reply? */
...
wlc_restart_ap(wlc_ap_info_t *ap)
{
	...
	if (APSTA_ENAB(wlc->pub) && (MAP_ENAB(wlc->cfg) || DWDS_ENAB(wlc->cfg) ||
#if defined(WET) || defined(WET_DONGLE)
			(WET_ENAB(wlc)) ||
#endif /* WET || WET_DONGLE */
			(wlc->cfg->_psta)) && wlc->cfg->disable_ap_up) {
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_ASSOC_REJECT_RSSI_TH):
	{
		...
		ASSERT(ap_cfg != NULL);
		if (BSSCFG_AP(bsscfg)) {
			*ret_int_ptr = (int8)ap_cfg->assoc_rej_rssi_thd;
		} else {
			err = BCME_NOTAP;
		}
		break;
	...
	case IOV_SVAL(IOV_ASSOC_REJECT_RSSI_TH):
	{
		if (BSSCFG_AP(bsscfg)) {
			if ((int8)int_val > 0) {
				WL_ERROR(("wl%d:%d Invalid value for IOVAR:assoc_rssi_th\n",
					wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
				err = BCME_RANGE;
				break;
			}
			...
			ap_cfg->assoc_rej_rssi_thd = (int8)int_val;
		} else {
			err = BCME_NOTAP;
		}
		break;
...
wlc_ap_scb_free_notify(void *context, struct scb *scb)
{
	...
	if (bsscfg && BSSCFG_AP(bsscfg) && wlc->eventq) {
		if (SCB_ASSOCIATED(scb)) {
