 ...
 *
 * $Id: qmath.c 799428 2021-05-29 05:05:42Z $
 */
...
qm_log10(int32 N, int16 qN, int16 *log10N, int16 *qLog10N)
{
	...
	*/
	...
	if (s16tableIndex != 31) {
		temp = qm_mulu16(u16offset,
			(uint16)(log_table[s16tableIndex+1]-log_table[s16tableIndex])); /* q.15 */
		s16errorApproximation = (temp > 0x7fff) ? 0x7fff : temp; // To avoid overflow
	} else {
		/* Assume log_table[32] = 65536 when doing interpolation. */
		uint16 temp2 = 33512; // log_table[32]-log_table[31]
		temp = qm_mulu16(u16offset, temp2); /* q.15 */
		s16errorApproximation = (temp > 0x7fff) ? 0x7fff : temp; // To avoid overflow
	}
