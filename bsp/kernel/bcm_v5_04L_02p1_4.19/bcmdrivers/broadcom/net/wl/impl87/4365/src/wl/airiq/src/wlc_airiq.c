 ...
 *
 * $Id: wlc_airiq.c 768460 2018-10-15 23:51:08Z $
 *
 */
...
BCMATTACHFN(wlc_airiq_attach) (wlc_info_t * wlc)
{
	...
	airiqh->next_chan_index = 0;
	airiqh->num_chan_scanned = 0;
	airiqh->throttle_maxscan1 = 9;
	airiqh->throttle_maxscan2 = 6;
	airiqh->throttle_thr1 = 80;
	airiqh->throttle_thr2 = 90;
...
#if defined(WLRSDB)
		if (airiqh->wlc->pub->corerev == 56) {
			is_rsdb_mode = WLC_RSDB_DUAL_MAC_MODE(WLC_RSDB_CURR_MODE(airiqh->wlc));
			/* non-rsdb mode */
			if (! is_rsdb_mode) {
				/* need to inform that radio 1 has completed
				 *  scan since it maps to second radio
				 */
				if (radio == 2) {
					radio = 1;
				}
