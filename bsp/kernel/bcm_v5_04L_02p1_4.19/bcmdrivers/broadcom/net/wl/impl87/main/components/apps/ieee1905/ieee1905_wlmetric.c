...
{
    maclist_t *maclist=(maclist_t *)macs;
    if(maclist->count>0)
    {
...
int i5wlmMetricUpdateLinkMetrics(char *wlname, int numMacs, unsigned char const *macAddresses)
{
   ...
   pMsg[strlen(cmd)] = '\0';
   metricConfig = (maclist_t *)&pMsg[strlen(cmd)+1];
...
void i5WlmUpdateMAPMetrics(void *arg)
{
  ...
  if ( NULL != pDevice ) {
    ...
    while ( pInterface != NULL ) {
      ...
      if ( i5DmIsInterfaceWireless(pInterface->MediaType) ) {
        ...
        while (pNeighbor != NULL) {
          pbss = i5DmFindBSSFromInterface(pInterface, pNeighbor->LocalInterfaceId);
          if (pbss || (0 == memcmp(pNeighbor->LocalInterfaceId, pInterface->InterfaceId, MAC_ADDR_LEN))) {
            memset(&metric, 0, sizeof(metric));
            if (i5_config.cbs.backhaul_link_metric) {
              i5_config.cbs.backhaul_link_metric(pbss ? pbss->ifname : pInterface->ifname,
              pNeighbor->NeighborInterfaceId, &metric);
            }
  ...
  if (isCreateTimer && i5_config.selfLinkMetricTimeout) {
