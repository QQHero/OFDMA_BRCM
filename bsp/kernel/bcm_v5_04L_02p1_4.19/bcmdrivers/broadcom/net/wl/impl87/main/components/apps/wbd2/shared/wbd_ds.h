 ...
 *
 * $Id: wbd_ds.h 805708 2021-12-06 10:42:51Z $
 */
...
struct wbd_cmd_vndr_metric_policy_config;
typedef struct wbd_cmd_vndr_metric_policy_config wbd_cmd_vndr_metric_policy_config_t;

struct wbd_cmd_vndr_strong_sta_policy_config;
typedef struct wbd_cmd_vndr_strong_sta_policy_config wbd_cmd_vndr_strong_sta_policy_config_t;
/* ----------------------------------- Forward Declarations ------------------------------------ */
...
#define WBD_WGHT_FLAG_BAND		0x0040
...
/* Metric Reporting Policy NVRAMs default values */
#define WBD_STA_METRICS_REPORTING_FLAGS		0 /* Associated STA Traffic Stats Inclusion = 0
						     * Associated STA Link Metrics Inclusion = 0
						   */
#define WBD_STA_METRICS_REPORTING_APMETRIC_INT	0 /* Default interval mentioned by Controller
						    * in Metrics Reporting Policy for Agent
						    * to Periodically Send AP Metrics Reports
						   */
...
/* Metric Reporting Policy */
#define WBD_STRONG_STA_METRICS_REPORTING_RSSI_THLD		-50
#define WBD_STRONG_STA_METRICS_REPORTING_RSSI_HYSTERISIS_MARGIN	3
#define WBD_STRONG_STA_METRICS_REPORTING_IDLE_RATE_THLD		1000
...
typedef enum {
	...
	WBD_STA_STATUS_STRONG,		/* Strong STA, try to steer */
	WBD_STA_STATUS_STRONG_STEERING,	/* Strong STA and steer command is sent */
} wbd_sta_status_t;
...
/* List to map chanspec for particular band in the controller if the matching self interface
 * is not present. This is required in case of any band is not present at controller but
 * present in the repeaters, all the repeaters should have same chanspec
 */
typedef struct wbd_band_chspec_map {
	dll_t node;		/* self referencial (next,prev) pointers of type dll_t */
	uint8 band_flag;	/* Band flag of type BAND_XXX */
	chanspec_t chanspec;	/* Chanspec to be set for a particular band */
} wbd_band_chspec_map_t;
...
typedef struct wbd_fbt_bss_entry {
	...
	ieee1905_fbt_info_type fbt_info;	/* FBT info sent by Controller,
						 *  used in FBT_CONFIG_RESP
						*/
...
/* Length of each FBT bss entry
 * BSSID		6 bytes
 * MDID			2 bytes
 * FT CApability	1 byte
 * Reassoc Interval	4 bytes
 * Bridge Address	6 bytes
 * R0KH_ID_Length	1 byte
 * R0KH_ID		5 bytes
 * R0KH_Key_Length	1 byte
 * R0KH_key		16 bytes
 * ============================
 * Total		42 bytes
*/
#define WBD_FBT_BSS_ENTRY_LEN	42

#define WBD_MAX_FBT_BSS_IN_FRAME	(I5_MAX_TLV_PAYLOAD_LEN / WBD_FBT_BSS_ENTRY_LEN)
...
/* To store channel selection request information for all the radio's to send it in channel
 * selection request as vendor specific tlv
 */
typedef struct wbd_cmd_vndr_set_chan_lst {
	dll_t node;				/* self referencial (next,prev) pointers of
						 * type dll_t
						 */
	wbd_cmd_vndr_set_chan_t vndr_set_chan;	/* Channel set info */
} wbd_cmd_vndr_set_chan_lst_t;
...
typedef struct wbd_assoc_sta_item {
	...
	int32 last_strong_rssi;		/* Strong RSSI reported earlier, only used in Controller */
...
struct wbd_slave_item {
	...
	wbd_strong_sta_policy_t strong_sta_policy;	/* Strong sta policy */
};
...
typedef struct wbd_ifr_item {
	...
	uint8 non_map_bss_count;		/* Number of non MAP BSS */
	...
	int chscan_sync_id; /* CH Scan Sync ID :
		* This data would be required when ESCAN Event comes to Slave, Slave should know,
		* if ESCAN Result is of Channel Scan, which is recently initiated by Agent or not
		*/
	...
	ieee1905_glist_t dynamic_exclude_chlist; /* Dynamically Non-Operable Chanspecs
		...
		* List of i5_chspec_item_t type objects
		*/
	ieee1905_glist_t pref0_exclude_chlist; /* Preference = 0 Non-Operable Chanspecs
		* informed to this Agent's this Radio by Controller in Channel Selection Request.
		* Agent should avoid selecting these Chspecs by setting exclude_chlist in ACSD
		* List of i5_chspec_item_t type objects
		*/
} wbd_ifr_item_t;

#define WBD_FLAGS_BSS_CAC_STARTED	0x1 /* Flag to indicate the cac is being started from wbd */
...
struct wbd_blanket_slave {
	...
	wbd_cmd_vndr_strong_sta_policy_config_t *vndr_strong_sta_policy;	/* Strong Sta Vendor
										 * Data
										 */
...
struct wbd_master_info {
	...
	int strong_client_count;	/* Number of Strong Clients in Blanket */
	...
	ieee1905_fbt_info_type fbt_info;	/* FBT info sent by Controller,
						 * used in FBT_CONFIG_RESP
						*/
	uint8 metric_policy_flags;		/* Metric report policy flags */
	int metric_policy_ap_rpt_intvl;		/* Metric report policy : AP Metrics Rpt Intrval */
	...
	wbd_strong_sta_policy_t strong_metric_policy_2g;	/* Metric report policy
							 * configuration for
							 * strong sta for 2G band
							 */
	wbd_strong_sta_policy_t strong_metric_policy_5g;	/* Metric report policy
							 * configuration for
							 * strong sta for 5G band
							 */
...
/* AP Metrics Query Related Information */
typedef struct wbd_ap_metrics_query_args {
	i5_dm_bss_type *query_bss;
	time_t ts_query_sent;
} wbd_ap_metrics_query_args_t;
...
struct wbd_blanket_master {
	...
	wbd_glist_t band_chspec_map;	/* List of type wbd_band_chspec_map_t */
	wbd_logs_info_t master_logs;	/* stores logs information. */
	wbd_glist_t vndr_set_chan_list;	/* List of type wbd_cmd_vndr_set_chan_lst_t */
	wbd_ap_metrics_query_args_t apm_query_data; /* AP Metrics Query Args for CLIENTLIST CLI */
	wbd_info_t *parent;		/* keeps track of its parent */
};
...
/* Update STA's Status to NORMAL/IGNORE/WEAK/STRONG/STEERED/BOUNCING,
 * update Weak/Strong Client Count
*/
	...
	do { \
		if (((value) == WBD_STA_STATUS_WEAK) && \
			...
			((info)->weak_client_count)++; \
				...
				((sta)->status == WBD_STA_STATUS_WEAK)) \
			((info)->weak_client_count)--; \
		else if (((value) == WBD_STA_STATUS_STRONG) && \
				((sta)->status != WBD_STA_STATUS_STRONG)) \
			((info)->strong_client_count)++; \
		else if (((value) != WBD_STA_STATUS_STRONG) && \
				((sta)->status == WBD_STA_STATUS_STRONG)) \
			((info)->strong_client_count)--; \
		(sta)->status = (value); \
...
#define WBD_SAFE_GET_I5_CTLR_DEVICE(device, find_ret) \
		do { \
			...
			if (((*(find_ret)) == WBDE_AGENT_NT_JOINED) || (!device->psock)) { \
				WBD_WARNING("%s\n", wbderrorstr(WBDE_AGENT_NT_JOINED)); \
				ret = WBDE_AGENT_NT_JOINED; \
				goto end; \
			} \
...
extern void wbd_ds_glist_prepend(wbd_glist_t *list, dll_t *new_obj);
...
/* Get the band and chanspec mapping entry from the table for the matching band */
extern wbd_band_chspec_map_t *wbd_ds_find_band_in_band_chspec_map(wbd_glist_t *band_chspec_map_list,
	uint8 band_flag);
...
/* Add channel information of a radio to list */
extern int wbd_ds_add_vndr_chan_set_info_to_list(wbd_glist_t *list,
...
extern int wbd_ds_add_sta_to_bounce_table(wbd_blanket_master_t *wbd_master, struct ether_addr *addr,
	struct ether_addr *src_addr, uint8 flags, wbd_sta_status_t command);
...
extern int wbd_ds_get_logs_from_master(wbd_blanket_master_t *blanket_master_info,
	char *logs, int len);
/* Add band to band chanspec mapping table */
extern wbd_band_chspec_map_t* wbd_ds_add_band_in_band_chspec_map(wbd_glist_t *band_chspec_map_list,
	uint8 band_flag);
/* ----------------------------- Extern Function Declaration -------------------------------- */
