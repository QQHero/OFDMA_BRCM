 ...
 *
 * $Id: wlc_interfere.c 796777 2021-03-12 04:36:35Z $
 */

/**
 * XXX Attempts to identify (non-WiFi) interferers in the 2.4G band.
 */
...
static int itfr_get_sample(itfr_info_t *itfr, itfr_sample_t *itfr_sample, uint sample_time);
...
static const bcm_iovar_t wlc_itfr_iovars[] = {
	...
	(0), 0, IOVT_BUFFER, sizeof(interference_source_rep_t),
	},
...
static int
wlc_itfr_doiovar(void *ctx, uint32 actionid,
        void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
...
itfr_scan_request(itfr_info_t *itfr)
{
	...
	return wlc_scan_request(wlc, DOT11_BSSTYPE_ANY, &ether_bcast, 1,
		&req_ssid, 0, NULL, DOT11_SCANTYPE_ACTIVE, -1, ITFR_SCAN_SAMPLE_TM,
		ITFR_SCAN_SAMPLE_TM, ITFR_SCAN_SAMPLE_TM, itfr->chanlist,
		itfr->chan_num, 0, FALSE, itfr_scan_complete, wlc,
...
itfr_get_valid_2g_channels(itfr_info_t *itfr)
{
	chanspec_t chanspec;
	wlc_info_t *wlc = itfr->wlc;
	uint chan_num = wlc_cca_get_channels_num(wlc->cca_info);
	uint8 i, j;

	/* init scan channels */
	for (i = 0, j = 0; i < chan_num; i++) {
		chanspec = wlc_cca_get_chanspec(wlc->cca_info, i);
...
itfr_get_sample(itfr_info_t *itfr, itfr_sample_t *itfr_sample, uint sample_time)
{
	...
	for (i = 0; i < itfr->chan_num; i++) {
		...
		ret = cca_query_stats(wlc, CH20MHZ_CHSPEC(ch, WL_CHANSPEC_BAND_2G),
				sample_time, result, result_len);
...
itfr_stats_watchdog(void *ctx)
{
	...
	if (itfr->op_mode == ITFR_MODE_DISABLE ||
	    ...
	    (!wlc_valid_chanspec_db(wlc->home_chanspec) ||
	     !CHSPEC_IS2G(wlc->home_chanspec)) {
		if (itfr->state != ITFR_STATE_CLEAN || itfr->verify_pending)
			itfr_reset_state(itfr, FALSE);
		return;
	}

	home_ch = wf_chspec_primary20_chan(wlc->home_chanspec);
