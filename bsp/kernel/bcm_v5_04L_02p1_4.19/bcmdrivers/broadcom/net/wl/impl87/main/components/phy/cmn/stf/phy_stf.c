 ...
 *
 * $Id: phy_stf.c 783949 2020-02-13 09:58:24Z $
 */
...
/* ********************************************* */
int
phy_stf_chain_set(phy_info_t *pi, uint8 txchain, uint8 rxchain)
{
...
int
phy_stf_chain_init(phy_info_t *pi, uint8 txchain, uint8 rxchain)
{
	int err = BCME_OK;
	phy_stf_info_t *stfi = NULL;
	phy_type_stf_fns_t *fns = NULL;
...
int
phy_stf_chain_get(phy_info_t *pi, uint8 *txchain, uint8 *rxchain)
{
	phy_stf_data_t	*data = NULL;
...
uint16
phy_stf_duty_cycle_chain_active_get(phy_info_t *pi)
{
