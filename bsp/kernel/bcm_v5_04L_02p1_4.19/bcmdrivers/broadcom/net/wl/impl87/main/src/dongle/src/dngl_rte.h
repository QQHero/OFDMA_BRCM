 ...
 *
 * $Id: dngl_rte.h 745122 2018-02-07 06:12:40Z $
 */
...
typedef struct dngl {
	struct dngl_bus *bus;		/* generic bus handle */
	osl_t *osh;			/* OS abstraction layer handler */
	si_t *sih;			/* sb/si backplane handler */
	dngl_proto_info_t *proto;	/* BDC/MSGBUF proto handler */
	dngl_stats_t stats;
	int medium;
	hnd_dev_t *rtedev;
	...
	/* Interface index to os interface index map */
	int *iface2osif;
	int tunable_max_host_if;
