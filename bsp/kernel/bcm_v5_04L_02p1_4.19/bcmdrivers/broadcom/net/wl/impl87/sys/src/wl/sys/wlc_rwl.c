 ...
 *
 * $Id: wlc_rwl.c 786648 2020-05-04 14:21:00Z $*
 *
 */
	...
	case IOV_GVAL(IOV_RWLDONGLE_DATA):
		...
		if (g_rwl_dongle_data.packet_status) {
			if (g_rwl_dongle_data.packet_buf != NULL) {
				...
				/* XXX Action item on Brett to check if NULL should be the
				 * first param or not
				 */
                       ...
                       const struct ether_addr * dest_addr_ptr)
{
	...
	wlc_send_action_frame(wlc, wlc->primary_bsscfg, NULL, (void *)action_frame);
