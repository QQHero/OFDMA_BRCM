 ...
 *
 * $Id: wlc_prot_g.c 795628 2021-02-11 05:12:45Z $
 */
...
BCMATTACHFN(wlc_prot_g_attach)(wlc_info_t *wlc)
{
	...
	/* error handling */
fail:
	MODULE_DETACH(prot, wlc_prot_g_detach);
	return NULL;
}
...
BCMATTACHFN(wlc_prot_g_detach)(wlc_prot_g_info_t *prot)
{
	...
	(void)wlc_module_remove_ioctl_fn(wlc->pub, prot);
...
static int
wlc_prot_g_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
...
wlc_prot_g_watchdog(void *ctx)
{
	...
	FOREACH_AS_BSS(wlc, idx, cfg) {
		...
		if (BSSCFG_AP(cfg) || BSSCFG_IBSS(cfg)) {
			if (wlc->bandstate[BSSCFG_BANDUNIT(cfg)] == wlc->band &&
				wlc->band->gmode)
...
wlc_prot_g_mode_upd(wlc_prot_g_info_t *prot, wlc_bsscfg_t *cfg)
{
	...
	if (pgd->longpre_assoc || pgt->longpre_detect_timeout > 0 ||
		cfg->PLCPHdr_override != WLC_PLCP_SHORT)
	...
	} else if (BSSCFG_IBSS(cfg)) {	/* IBSS shortslot setting */
...
static int
wlc_prot_g_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
	wlc_prot_g_info_t *prot = (wlc_prot_g_info_t *)ctx;
	wlc_prot_g_info_priv_t *priv = WLC_PROT_G_INFO_PRIV(prot);
	wlc_info_t *wlc = priv->wlc;
