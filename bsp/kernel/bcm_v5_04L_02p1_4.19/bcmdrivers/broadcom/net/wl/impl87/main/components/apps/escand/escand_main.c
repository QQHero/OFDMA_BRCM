...
escand_main_loop(struct timeval *tv)
{
	...
	if (d_info->event_fd !=  ESCAND_DFLT_FD && FD_ISSET(d_info->event_fd, &fdset)) {
		...
		if ((evt_type != WLC_E_ESCAN_RESULT) &&
			c_info->escand_escan->escand_escan_inprogress) {
		...
		switch (evt_type) {
			...
			case WLC_E_ESCAN_RESULT:
			{
				...
				if (escan_event_status == WLC_E_STATUS_PARTIAL) {
					...
					if (c_info->escand_escan->scan_type ==
						ESCAND_SCAN_TYPE_CS) {
...
static void
escand_watchdog(uint ticks, int *up_attempts)
{
	int i, ret, isup = 0;
	escand_chaninfo_t* c_info;
	...
	for (i = 0; i < ESCAND_MAX_IF_NUM; i++) {
		...
		if (ret < 0) {
			up_attempts[i]++;
			ESCAND_ERROR("Couldn't get up status on %s, up_attempts[%d] = %d\n",
				c_info->name, i, up_attempts[i]);
			if (up_attempts[i] >= ESCAND_MAX_INTF_ATTEMPTS) {
				ESCAND_ERROR("Interface %s, up attempts at %d, "
					"disabling intf %s for escand\n",
					c_info->name, up_attempts[i], c_info->name);
		...
		if (isup == 0) {
			...
			if (c_info->wasdown == 1) {
				ESCAND_PRINT("Interface %s was down, reload escand "
					"intf configuration\n", c_info->name);
				c_info->wasdown = 0;
		...
		if (!escand_is_mode_check(c_info->name)) {
			ESCAND_INFO("do not perform scan when ESCAND is not in AP mode\n");
			continue;
		}

		if (ticks % ESCAND_TICK_DISPLAY_INTERVAL == 0) {
			ESCAND_DEBUG("escand_watchdog: tick:%u, secs %d\n", ticks,
				(ticks / ESCAND_TICKS_PER_SEC));
		}
		...
		if (ticks % ESCAND_TRAFFIC_POLL == 0)  {
			if (c_info->escand_traffic_thresh != -1) {
				if ((ret = escand_activity_update(c_info)) != BCME_OK) {
					ESCAND_INFO("%s: activity update failed", c_info->name);
				}
			}
		}

		/*
		Not currently using CS scan timer
		escand_cs_scan_timer_check(c_info);
		*/

		if (ticks % ESCAND_SCAN_POLL == 0)  {
...
main(int argc, char *argv[])
{
	...
	char *val;
	int daemonize = 1;
	int up_attempts[ESCAND_MAX_IF_NUM];

	memset((int *)up_attempts, 0, (ESCAND_MAX_IF_NUM * sizeof(int)));
...
#endif
	tv.tv_usec = d_info->poll_interval;
	tv.tv_sec = 0;
	...
	while (escand_running) {
		...
		if (ESCAND_WPS_RUNNING) {
			sleep_ms(d_info->poll_interval/1000);
			continue;
		}
		...
		if (tv.tv_sec == 0 && tv.tv_usec == 0) {
			...
			tv.tv_usec = d_info->poll_interval;
			tv.tv_sec = 0;
			ESCAND_DEBUG("main: ticks: %d, secs %d\n", d_info->ticks,
				(d_info->ticks / ESCAND_TICKS_PER_SEC));
			if (d_info->ticks > ESCAND_STARTUP_DELAY) {
				escand_watchdog(d_info->ticks, up_attempts);
			}
		}
		escand_main_loop(&tv);
	}
