 ...
 *
 * $Id: pdftmproto.c 810932 2022-04-19 20:14:14Z $
 */

#include "pdftmpvt.h"
#include <wlc_iocv.h>

#define FTM1_VENDOR_OUI BRCM_PROP_OUI
/* initialize vendor specific IE in the FTM1  */
static void
ftm_vendor_ie_init(vndr_ie_t *ie)
{
	ie->id = DOT11_MNG_VS_ID;
	ie->len = sizeof(vndr_ie_t) - VNDR_IE_HDR_LEN;
	bcopy(FTM1_VENDOR_OUI, ie->oui, DOT11_OUI_LEN);
	ie->data[0] = 0;
}
...
ftm_proto_get_chaninfo(pdftm_t *ftm, pdftm_session_t *sn)
{
	...
	if (CHSPEC_IS20(chspec)) {
		...
		if (RSPEC_ISHE(rspec)) {
			ci = FTM_PARAMS_CHAN_INFO_HE_80;
		} else {
			ci = FTM_PARAMS_CHAN_INFO_VHT_80;
		}
	} else if (CHSPEC_IS8080(chspec)) {
		if (RSPEC_ISHE(rspec)) {
			ci = FTM_PARAMS_CHAN_INFO_HE_80_80;
		} else {
			ci = FTM_PARAMS_CHAN_INFO_VHT_80_80;
		}
	} else if (CHSPEC_IS160(chspec)) {
		if (RSPEC_ISHE(rspec)) {
			ci = FTM_PARAMS_CHAN_INFO_HE_160;
		} else {
	...
	chanspec_t rx_cspec, uint8 chaninfo, chanspec_t *ftm_cspec)
{
	...
	uint16 bw;
	scb_t *scb = NULL;
	chanspec_band_t band = (rx_cspec & WL_CHANSPEC_BAND_MASK);
	...
	case FTM_PARAMS_CHAN_INFO_HE_160:
		bw = WL_CHANSPEC_BW_160;
		break;
	case FTM_PARAMS_CHAN_INFO_VHT_160_2_RFLOS: /* fall through */
	case FTM_PARAMS_CHAN_INFO_HE_160_2_RFLOS: /* fall through */
	...
	cspec = wf_channel2chspec(ctl_chan, bw, band);
	...
	ratespec_t rx_rspec, ratespec_t *rspec)
{
	...
	case FTM_PARAMS_CHAN_INFO_HE_80_80:
	case FTM_PARAMS_CHAN_INFO_HE_160:
	case FTM_PARAMS_CHAN_INFO_HE_160_2_RFLOS:
	...
	bandtype = CHSPEC_BANDTYPE(cspec);
#endif /* WL11AC */
	switch (chaninfo) {
	...
	case FTM_PARAMS_CHAN_INFO_VHT_160_2_RFLOS:
		...
		ratespec |= VHT_RSPEC(FTM_MCS, FTM_NSS);
		break;
	case FTM_PARAMS_CHAN_INFO_HE_80_80:
	case FTM_PARAMS_CHAN_INFO_HE_160:
	case FTM_PARAMS_CHAN_INFO_HE_160_2_RFLOS:
	...
	case FTM_PARAMS_CHAN_INFO_HE_40:
	case FTM_PARAMS_CHAN_INFO_HE_20:
		ratespec |= HE_RSPEC(FTM_MCS, FTM_NSS);
		ratespec |= HE_GI_TO_RSPEC(WL_RSPEC_HE_2x_LTF_GI_1_6us);
		break;
	default:
		ASSERT(FALSE);
		err = BCME_UNSUPPORTED;
...
do_sep:
	...
	if (FTM_PARAMS_BURST_PERIOD_MS(params) != 0) {
	...
	const uint8 *body, uint body_len, const wlc_d11rxhdr_t *wrxh, ratespec_t rspec)
{
	...
	FTM_LOGPROTO(ftm, (("wl%d.%d: %s: handling request from " MACF " rssi %d\n",
		FTM_UNIT(ftm), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
		ETHERP_TO_MACF(&hdr->sa), wrxh->rssi)));
	...
	const uint8 *body, uint body_len, const wlc_d11rxhdr_t *wrxh, ratespec_t rspec)
{
	...
	FTM_LOGPROTO(ftm, (("wl%d.%d: %s: handling measurement from " MACF " rspec:%x\n",
		FTM_UNIT(ftm), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
		ETHERP_TO_MACF(&hdr->sa), rspec)));
...
param_ok:
	...
	if (err != BCME_OK && sn != NULL &&
		...
		if ((sn->state != WL_PROXD_SESSION_STATE_BURST) ||
			(err == WL_PROXD_E_REMOTE_CANCEL)) {
			(void)pdftm_stop_session(sn, err);
		} else if (sn->state == WL_PROXD_SESSION_STATE_BURST) {
			/* tell the peer we can't complete the request */
			sn->status = WL_PROXD_E_INCAPABLE;
			(void)pdftm_stop_session(sn, err);
		} else {
	...
	FTM_LOG_STATUS(ftm, err, (("wl%d: %s: err %d sn->state %d RSSI %d \n",
		FTM_UNIT(ftm), __FUNCTION__, err, sn ? sn->state : 0, wrxh->rssi)));
	return err;
}
...
#define FTM_TOD_NOT_CONT 1
	...
	if (!(sn->flags & FTM_SESSION_FTM1_SENT)) {
		ftm_vendor_ie_init((vndr_ie_t *)(body));
		body += sizeof(vndr_ie_t);
		body_len -= sizeof(vndr_ie_t);
	}
