 ...
 *
 * $Id: wluc_rrm.c 807402 2022-01-19 13:01:36Z $
 */
...
static cmd_t wl_rrm_cmds[] = {
	...
	"set the report radio measurement capability bitvector\n"
...
wl_rrm(void *wl, cmd_t *cmd, char **argv)
{
	...
	err = wlu_var_getbuf_sm(wl, cmd->name, NULL, 0, &ptr);
	if (err < 0)
		return err;
...
wl_rrm_lm_req(void *wl, cmd_t *cmd, char **argv)
{
	...
	if (argv[1]) {
		...
		printf("wl_rrm_lm_req NULL MAC address passed\n");
		return BCME_USAGE_ERROR;
	}
...
wl_rrm_nbr_ssid(void *wl, cmd_t *cmd, char **argv)
{
	...
	int err = 0;
	nbr_rpt_elem_t *nbr_elt;
	char smbuf[WLC_IOCTL_SMLEN] = "ssid";
	int buflen = sizeof("ssid");
...
wl_rrm_nbr_chanspec(void *wl, cmd_t *cmd, char **argv)
{
	...
	int err = 0;
	nbr_rpt_elem_t *nbr_elt;
	char smbuf[WLC_IOCTL_SMLEN] = "chanspec";
	int buflen = sizeof("chanspec");
...
wl_rrm_nbr_lci(void *wl, cmd_t *cmd, char **argv)
{
	...
	int err = 0;
	int len = 0;
	char smbuf[WLC_IOCTL_SMLEN*2] = "lci";
	char *bufptr = &smbuf[0];
	int buflen = sizeof("lci");
...
wl_rrm_nbr_civic(void *wl, cmd_t *cmd, char **argv)
{
	...
	int err = 0;
	int len = 0;
	char smbuf[WLC_IOCTL_SMLEN*2] = "civic";
	char *bufptr = &smbuf[0];
	int buflen = sizeof("civic");
