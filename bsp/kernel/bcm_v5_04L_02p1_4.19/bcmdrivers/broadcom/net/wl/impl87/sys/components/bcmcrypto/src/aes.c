 ...
 *
 * $Id: aes.c 809200 2022-03-09 07:50:58Z $
 */
            ...
            const size_t mac_len)
{
	...
	if (aad_len > AES_CCMP_AAD_MAX_LEN) return (-1);
	...
	/* B_0 = Flags || Nonce || l(m) */
	B_0[0] = (uint8)AES_CCM_AUTH_FLAGS(mac_len);
	if (aad_len)
	...
	pres("aes_ccm_mac: MAC:", mac_len, mac);

	return (0);
}

/* AES-CCM mode encryption
 *	- computes mac_len MAC and then encrypts ptxt and MAC
 *	- nonce must be AES_CCM_NONCE_LEN bytes
                ...
                const size_t mac_len)
{
	...
	if (aes_ccm_mac(rk, key_len, nonce, aad_len, aad, data_len, ptxt, X, mac_len))
		return (-1);
	pres("aes_ccm_encrypt: MAC:", mac_len, X);
	if (aes_ctr_crypt(rk, key_len, A, mac_len, X, X))
		return (-1);
	pres("aes_ccm_encrypt: encrypted MAC:", mac_len, X);
	memcpy(mac, X, mac_len);
 ...
 *	- decrypts ctxt, then computes mac_len MAC and checks it against
 *	  then decrypted MAC
                ...
                const size_t mac_len)
{
	...
	/* decrypt data */
	if (aes_ctr_crypt(rk, key_len, A, data_len-mac_len, ctxt, ptxt))
		return (-1);
	pres("aes_ccm_decrypt: decrypted data:", data_len-mac_len, ptxt);

	/* decrypt MAC */
	A[AES_BLOCK_SZ - 2] = 0;
	A[AES_BLOCK_SZ - 1] = 0;
	if (aes_ctr_crypt(rk, key_len, A, mac_len,
	                  ctxt+data_len-mac_len, ptxt + data_len - mac_len))
		return (-1);
	pres("aes_ccm_decrypt: decrypted MAC:", mac_len,
	     ptxt + data_len - mac_len);

	/* calculate MAC */
	if (aes_ccm_mac(rk, key_len, nonce, aad_len, aad,
	                data_len - mac_len, ptxt, X, mac_len))
		return (-1);
	pres("aes_ccm_decrypt: MAC:", mac_len, X);
	if (memcmp(X, ptxt + data_len - mac_len, mac_len) != 0)
		return (-1);
                 ...
                 const size_t mac_len)
{
	...
	status = aes_ccm_encrypt(rk, key_len, nonce, la, aad,
	                         data_len - lh, p + lh, p + lh, p + data_len, mac_len);
                    ...
                    uint8 nonce_1st_byte, uint8 *nonce, uint8 *aad, uint *la, uint *lh)
{
	...
	if (qos) {
		...
		if (htc) {
			dbg(("aes_ccmp_cal_params: HTC present\n"));
			addlen += DOT11_HTC_LEN;
		}
	}
...
typedef struct cmac_pfx_ctx cmac_pfx_ctx_t;

static void
cmac_pfx_ctx_init(cmac_pfx_ctx_t *ctx, bcm_const_xlvp_t *pfxs, int npfxs,
	int data_len, const uint8 *data, int blk_size)
{
	int i;
	...
	ctx->total_len = data_len;
	for (i = 0; pfxs != NULL && i < npfxs; ++i) {
		ctx->total_len += pfxs[i].len;
	}
	...
	if (!ctx->total_len)
		...
		ctx->nblks = CEIL(ctx->total_len, blk_size);
...
static void
cmac_pfx_get_next_block(cmac_pfx_ctx_t *ctx, uint8 *blk, bool *padded)
{
	int rem;
	int blk_off = 0;

	*padded = FALSE;
	while (ctx->cur_pfx < &ctx->pfxs[ctx->npfxs]) { /* all prefixes processed? */
		if (!ctx->cur_pfx->len) { /* ignore 0 len prefixes */
			goto next_pfx;
		}

		rem = ctx->cur_pfx->len - ctx->cur_off; /* how much data is in the pfx */
		if (rem >= (ctx->blk_size - blk_off)) { /* prefix data can fill the block */
			memcpy(&blk[blk_off], &ctx->cur_pfx->data[ctx->cur_off],
				ctx->blk_size - blk_off);
			rem -= ctx->blk_size - blk_off;
			if (rem >= 0) { /* more data in prefix */
				ctx->cur_off += ctx->blk_size - blk_off;
				goto done; /* have complete block */
			} /* else no more data, advance to next */
		} else {
			memcpy(&blk[blk_off], &ctx->cur_pfx->data[ctx->cur_off], rem);
			blk_off += rem;
		}

		next_pfx:
			ctx->cur_pfx++; /* no more data, advance to next */
			ctx->cur_off = 0;
	}

	rem = ctx->data_len - ctx->cur_off;
	if (rem >= (ctx->blk_size - blk_off)) {
		memcpy(&blk[blk_off], &ctx->data[ctx->cur_off], ctx->blk_size - blk_off);
		ctx->cur_off += ctx->blk_size - blk_off; /* more data */
		goto done; /* have complete block */
	}

	/* partial final block */
	*padded = TRUE;
	memcpy(&blk[blk_off], &ctx->data[ctx->cur_off], rem);
	blk_off += rem;

	/* pad w/ b'10* */
	blk[blk_off++] = 0x80;
	if (blk_off < ctx->blk_size) {
		memset(&blk[blk_off], 0, ctx->blk_size - blk_off);
	}

done:;
	/* prhex(*padded ? "Padded block" : "Unpadded block", blk, AES_BLOCK_SZ); */
}
...
aes_cmac(const int nrounds, const uint32* rk,
	...
	bcm_const_xlvp_t *pfxs, int npfxs,
	const size_t in_dlen,
	const uint8 *in_data,
	uint8 *mac,
	const size_t mac_len)
{
	int i;
	uint8 C[AES_BLOCK_SZ];
	uint8 blk[AES_BLOCK_SZ];
	cmac_pfx_ctx_t pfx_ctx;
	bool padded;
	...
	*/
	...
	cmac_pfx_ctx_init(&pfx_ctx, pfxs, npfxs, (int)in_dlen, in_data, AES_BLOCK_SZ);

	/* Let C0 = 0b */
	memset(C, 0, AES_BLOCK_SZ);
	/* For i = 1 to n, let Ci = CIPHK(Ci-1 ^ Mi) */
	for (i = 1; i < pfx_ctx.nblks; i++) {
		cmac_pfx_get_next_block(&pfx_ctx, blk, &padded);
		xor_128bit_block(C, blk, blk);
		aes_block_encrypt(nrounds, rk, blk, C);
	}

	cmac_pfx_get_next_block(&pfx_ctx, blk, &padded);
	if (!padded) { /* Mn* is a complete block */
		xor_128bit_block(K1, blk, blk);
	} else {
		xor_128bit_block(K2, blk, blk);
	}
	xor_128bit_block(C, blk, blk);
	aes_block_encrypt(nrounds, rk, blk, C);
...
void
aes_cmac_calc(bcm_const_xlvp_t *pfxs, int npfxs,
	const uint8 *data, const size_t data_len, const uint8 *mic_key,
	const size_t key_len, uint8 *mic, const size_t mic_len)
{
	...
	aes_cmac(nrounds, rk, K1, K2, pfxs, npfxs, data_len, data, mic, mic_len);
}
...
#define TEST_AES_CCM_AUTH_LEN 8
		if (memcmp(output, aes_ccm_vec[k].mac, TEST_AES_CCM_AUTH_LEN) != 0) {
			dbg(("%s: aes_ccm_mac failed\n", *argv));
			fail++;
		}
		...
		pres("AES CCM Encrypt: ", aes_ccm_vec[k].il + TEST_AES_CCM_AUTH_LEN, output);

		if (retv) {
			dbg(("%s: aes_ccm_encrypt failed\n", *argv));
			fail++;
		}
		if (memcmp(output, aes_ccm_vec[k].ref, aes_ccm_vec[k].il +
				TEST_AES_CCM_AUTH_LEN) != 0) {
			dbg(("%s: aes_ccm_encrypt failed\n", *argv));
			fail++;
		}
		...
		retv = aes_ccm_decrypt(rk, aes_ccm_vec[k].kl,
		                       aes_ccm_vec[k].nonce, aes_ccm_vec[k].al,
		                       aes_ccm_vec[k].aad, aes_ccm_vec[k].il +
		                       TEST_AES_CCM_AUTH_LEN, aes_ccm_vec[k].ref, input2);
		pres("AES CCM Decrypt: ", aes_ccm_vec[k].il + TEST_AES_CCM_AUTH_LEN, input2);
	...
	for (k = 0; k < NUM_CCMP_VECTORS; k++) {
		...
		if (memcmp(output, aes_ccmp_vec[k].ref,
			aes_ccmp_vec[k].il+TEST_AES_CCM_AUTH_LEN) != 0) {
		...
		rijndaelKeySetupEnc(rk, aes_ccmp_vec[k].key,
		                    AES_KEY_BITLEN(aes_ccmp_vec[k].kl));
		memcpy(output, aes_ccmp_vec[k].ref, aes_ccmp_vec[k].il+TEST_AES_CCM_AUTH_LEN);
		nonce_1st_byte = get_nonce_1st_byte((struct dot11_header *)output);
		retv = aes_ccmp_decrypt(rk, aes_ccmp_vec[k].kl,
			aes_ccmp_vec[k].il+TEST_AES_CCM_AUTH_LEN, output,
			aes_ccmp_vec[k].flags[2],
			nonce_1st_byte);
