 ...
 *
 * $Id: wbd_json_utility.c 802917 2021-09-09 10:13:20Z $
 */
...
wbd_json_create_cli_cmd(void *data)
{
	...
	json_object_object_add(object_main, JSON_TAG_ALMAC, json_object_new_string(cmdcli->almac));
		...
		foreach_i5glist_item(ifr, i5_dm_interface_type, device->interface_list) {
			...
			foreach_i5glist_item(bss, i5_dm_bss_type, ifr->bss_list) {
				...
				foreach_i5glist_item(client, i5_dm_clients_type,
					...
					if (assoc_sta) {
						ptr = (assoc_sta->status == WBD_STA_STATUS_STRONG) ?
							"STRONG":
...
wbd_json_parse_cli_cmd(void *data)
{
	...
	/* Get the ALMAC address from the JSON object */
	wbd_json_get_stringval_fm_tag(object_main, JSON_TAG_ALMAC, clidata->almac,
		sizeof(clidata->almac));
...
/* Parse WEAK_CLIENT_BSD/STRONG_CLIENT_BSD command and fill the structure */
void*
wbd_json_parse_weak_strong_client_bsd_cmd(void *data)
{
	int ret = WBDE_OK;
...
/* Parse WEAK_CANCEL_BSD/STRONG_CANCEL_BSD command and fill the structure */
void*
wbd_json_parse_weak_strong_cancel_bsd_cmd(void *data)
{
	int ret = WBDE_OK;
	json_object *object_main, *object_data;
