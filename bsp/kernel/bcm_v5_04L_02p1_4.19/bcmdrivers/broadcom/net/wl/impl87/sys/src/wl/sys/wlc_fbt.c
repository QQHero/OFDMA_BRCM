 ...
 *
 * $Id: wlc_fbt.c 810261 2022-04-04 13:41:23Z $
 */
...
wlc_fbt_ft_calc_mic(bss_fbt_priv_t *fbt_bss_priv, wlc_bsscfg_t *cfg, struct scb *scb,
	...
	int ricdata_len, int ric_ie_count, rsnxe_ie_info_t *rsnxe_info, uint8 trans_seq_nbr)
{
	...
	if (BSSCFG_AP(cfg)) {
		...
		if (rsnxe_info && rsnxe_info-> rsnxe && rsnxe_info-> rsnxe_len) {
			/* set RSNXEUsed BIT */
   ...
   uint8 *ricdata, int ricdata_len, int ric_ie_count)
{
	...
	if (fbt_bss_priv->ftie && fbt_bss_priv->ftie_len) {
		...
		if (ftptr == NULL) {
			...
			/* Own RSNXE */
			rsnxe_ie_info_t *rsnxe_info = RSNXE_INFO_GET(cfg);
	...
	uint8 trans_seq_nbr)
{
	...
	if (ftie != NULL) {
		...
		if (wlc_fbt_ft_calc_mic(fbt_bss_priv, cfg, scb, myftie, mdie,
			rsnie, ricie, (int)(ricend - ricie), ric_ie_count,
			rsnxe_info, trans_seq_nbr)) {
	...
	*/
	...
	if (scb->auth_alg == DOT11_FAST_BSS) {
		...
		if ((ftcbparm->assocresp.md_ie != NULL) &&
			...
			/* Own RSNXE */
			rsnxe_ie_info_t * rsnxe_info = RSNXE_INFO_GET(bsscfg);
			if (!wlc_fbt_ft_calc_mic(fbt_bss_priv, bsscfg, scb, (dot11_ft_ie_t *)fbtie,
				...
				ricdata, ricdata_len, ric_ie_count,
				rsnxe_info, FT_MIC_REASSOC_RESPONSE_TSN)) {
					return BCME_ERROR;
				}
		}
	}
