...
void bsd_retrieve_config(bsd_info_t *info)
{
	...
	info->sta_hit_rssi_times = 1;
	if ((str = nvram_get("sta_hit_rssi_times")))
		info->sta_hit_rssi_times = strtol(str, &endptr, 0);

	info->sta_skip_single = 0;
	if ((str = nvram_get("sta_skip_single"))) {
		info->sta_skip_single = (uint8)strtol(str, &endptr, 0);
	}
	...
	info->block_sta_2g_timeout = BSD_BLOCK_STA_2G_TIMEOUT;
	if ((str = nvram_get("bsd_block_sta_2g_timeout"))) {
		info->block_sta_2g_timeout = (int)strtol(str, &endptr, 0);
	}
...
void bsd_dump_config_info(bsd_info_t *info)
{
	...
	fprintf(out, "block_sta_2g_timeout: %d\n", info->block_sta_2g_timeout);
...

		{
			...
			if (intf_info->chan_util.chanim_version == WL_CHANIM_STATS_V2) {
				...
				for (num = 0; num < BSD_CHANIM_STATS_MAX; num++) {
					...
					stats = (chanim_stats_t *)&(rec[num].stats);
			...
			if (strlen(policy_str)) {
				...
				foreach(var, policy_str, next) {
					...
					if ((to_idx < info->max_ifnum) &&
						...
						/* Increment the count of target bss */
						bssinfo->to_if_bss_list->count++;
					}
				}
			}
		}
...
int bsd_info_init(bsd_info_t *info)
{
	...
	} else if (info->enable_flag & BSD_FLAG_WBD_ENABLED) {
		info->enable_flag &= ~BSD_FLAG_ENABLED;
		BSD_WBD("BSD enabled but ssid's dont match, since WBD is enabled run BSD without"
			" BSD functionality\n");
		BSD_EXIT();
		return BSD_OK;
	}
...
uint8 bsd_get_sta_band(bsd_info_t *info, struct ether_addr *addr)
{
	bsd_maclist_t *mac;
...
void bsd_auth_sta(bsd_info_t *info, char *ifname, uint8 remote, struct ether_addr *addr)
{
	...
	if (sta) {
		...
		if (bssinfo->intf_info->band == BSD_BAND_6G) {
			sta->band |= BSD_BAND_5G;
			sta->band |= BSD_BAND_2G;
		} else if (bssinfo->intf_info->band == BSD_BAND_5G) {
			sta->band |= BSD_BAND_2G;
		}
	}
...
*/
			...
			if (bssinfo->chanspec != (chanspec_t)(*((uint32 *)ioctl_buf))) {
			...
			/* Check for passive channels only in 5G band */
			if (CHSPEC_IS5G(bssinfo->chanspec)) {
				if (bsd_is_chan_passive(bssinfo)) {
					bssinfo->flags |= BSD_FLAG_BSS_CHAN_PASSIVE;
				} else {
					bssinfo->flags &= ~BSD_FLAG_BSS_CHAN_PASSIVE;
				}
			}
			...
			for (cnt = 0; cnt < count; cnt++) {
				...
				/* Update STA bands from sta_info */
				if (prbsta && sta_info->ver >= WL_STA_VER_8 &&
					sta_info->len >= STRUCT_SIZE_THROUGH(sta_info, bands)) {
					for (i = 1; i <= BSD_MAX_INTF; i++) {
						prbsta->band |= sta_info->bands[i];
					}
					BSD_PROB("STA["MACF"] bands[0x%x]\n",
						ETHER_TO_MACF(prbsta->addr), prbsta->band);
				}
...
void bsd_update_cca_stats(bsd_info_t *info)
{
	...
	for (ifidx = 0; ifidx < info->max_ifnum; ifidx++) {
		...
		if (list->version > WL_CHANIM_STATS_VERSION) {
			BSD_ERROR("Err: chanim_stats version %d is not supported\n",
				list->version);
			BSD_CCAEXIT();
			return;
		}
		if (list->version == WL_CHANIM_STATS_V2) {
			...
			chanim_stats_t *stats = (chanim_stats_t *)list->stats;
			...
			for (idx = 0; idx < CCASTATS_MAX; idx++)
				...
				BSD_CCA_PLAIN("%d\t%d\t%d\t%d\t%d\n", stats->glitchcnt,
					stats->badplcp, stats->bgnoise, stats->timestamp,
					stats->chan_idle);
...
void bsd_reset_chan_busy(bsd_info_t *info, int ifidx)
{
	...
	if (intf_info->chan_util.chanim_version == WL_CHANIM_STATS_V2) {
		...
		for (num = 0; num < cnt; num++) {
			rec = &(intf_info->chan_util.rec[idx]);
			stats = (chanim_stats_t *)&rec->stats;
			rec->valid = 0;
			idx = MODINC(idx, BSD_CHANIM_STATS_MAX);
			BSD_CCA("invalid: rec[%d] idle[%d]\n", idx,
				stats->ccastats[CCASTATS_TXOP]);
		}
	}
	BSD_CCAEXIT();
	return;
...
int bsd_check_steer_fail(bsd_info_t *info, bsd_maclist_t *sta, bsd_bssinfo_t *bssinfo)
{
	...
	if ((sta->steer_state == BSD_STA_STEERING) && (sta->to_bssinfo)) {
		...
		if ((sta_assoc = bsd_add_assoclist(sta->to_bssinfo, &(sta->addr), FALSE))) {
			...
			/* Check if there are multiple interfaces in target,
			 * Skip first interface as target
			*/
			if ((bssinfo->to_if_bss_list->count > 1) &&
				(sta->to_bssinfo == bssinfo->to_if_bss_list->bssinfo)) {
...
bool bsd_is_chan_passive(bsd_bssinfo_t *bssinfo)
{
	...
	FOREACH_20_SB(bssinfo->chanspec, sub_channel) {
		...
		chanspec_arg = CH20MHZ_CHSPEC(sub_channel, WL_CHANNEL_2G5G_BAND(sub_channel));
...
bsd_get_phy_type(bsd_bssinfo_t *bssinfo, wl_bss_info_t *in_bi, uint8 *out_phy_type)
{
	...
	if (!in_bi) {
		...
		ret = bsd_get_bss_info(bssinfo, (wl_bss_info_t **)&bi);
		if (ret != BSD_OK) {
			goto end;
		}
	} else {
...
bsd_get_bssid_info_field(bsd_bssinfo_t *bssinfo, wl_bss_info_t *in_bi, uint32 *out_bssid_info)
{
	...
	*out_bssid_info |= (bi->he_cap) ? DOT11_NGBR_BI_HE : 0;
