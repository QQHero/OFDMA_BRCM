 ...
 *
 * $Id: dngl_rte.c 792253 2020-10-21 18:30:57Z $
 */
 ...
 */
int dngl_flowring_update(struct dngl *dngl, uint8 ifindex, uint16 ringid, uint8 tid,
	uint8* da, uint8 op, uint8** tcb_state, uint16* scb_flowid, uint8* mode)
{
	hnd_dev_t *slave;
	...
	if (!slave) {
		...
		/* call to the WL subsystem to link SCB with bus layer */
		return slave->ops->flowring_link_update(slave, ringid, tid,
			da, op, tcb_state, scb_flowid, mode);
	}
