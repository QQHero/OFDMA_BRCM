 ...
 *
 * $Id: wbd_cli.c 802919 2021-09-09 10:26:16Z $
 */
...
#define WBD_TIMEOUT_POLL_AP_METRICS	6   /* Timeout to stop Polling for AP Metrics Resp */
#define WBD_USLEEP_POLL_AP_METREICS	500 /* Millisec sleep bet each Poll iter for AP Metrics */

#define VALIDATE_ARG_ONLY_ONE(cmdarg) \
	(((strlen((cmdarg)->mac) > 0) && \
		(strlen((cmdarg)->bssid) <= 0) && (strlen((cmdarg)->almac) <= 0)) || \
	((strlen((cmdarg)->bssid) > 0) && \
		(strlen((cmdarg)->mac) <= 0) && (strlen((cmdarg)->almac) <= 0)) || \
	((strlen((cmdarg)->almac) > 0) && \
		(strlen((cmdarg)->mac) <= 0) && (strlen((cmdarg)->bssid) <= 0)))
...
wbd_usage()
{
	printf("usage: wb_cli <options> [ <value>]\n"
			...
			"	-r, --almac	Provide AL Mac Address\n"
			...
			"	exit		Exits the master applicantion\n"
			"	reload		Reload the application to take new configuration\n"
			"\n"
			"Slave Commands are:\n"
			"	version		Displays the version of WBD\n"
...
/* WBD CLI TCP Client */
static int
wbd_cli_send(char *clidata, int portno, uint flags)
{
	...
	if (flags & WBD_CLI_FLAGS_QUERY) {
		sscanf(read_buf, "%d", &ret);
	} else {
...
wbd_cli_parse_opt(int argc, char *argv[], cmdargs_t *cmdarg)
{
	...
	while (1) {
		int option_index = 0;
		c = getopt_long(argc, argv, "s:m:a:r:p:b:c:g:t:l:d:feh",
		...
		switch (c) {
			...
			case 'r' :
				strncpy(cmdarg->almac, optarg, sizeof(cmdarg->almac)-1);
				break;
			...
			case 'f' :
				cmdarg->flags |= WBD_CLI_FLAGS_FORCE;
				break;
...
wbd_cli_validate_cmdarg(cmdargs_t *cmdarg)
{
	...
	if (cmdarg->cmd == WBD_CMD_CLI_EXIT) {
		if (cmdarg->use_master == 0) {
			ret = WBDE_CLI_INV_SLAVE_CMD;
		}
	}

	/* Validate Arguments for reload command */
	if (cmdarg->cmd == WBD_CMD_CLI_RELOAD) {
		if (cmdarg->use_master == 0) {
			ret = WBDE_CLI_INV_SLAVE_CMD;
		}
	}

	/* Validate Arguments for CLIENTLIST command on Master */
	if (cmdarg->cmd == WBD_CMD_CLI_CLIENTLIST) {
		if ((cmdarg->use_master) &&
			!VALIDATE_ARG_ONLY_ONE((cmdarg))) {
			ret = WBDE_CLI_INV_ONLY_ONE;
		}
	}
...
wbd_cli_process(cmdargs_t *cmdarg)
{
	...
	memcpy(clidata.mac, cmdarg->mac, sizeof(clidata.mac)-1);
	memcpy(clidata.bssid, cmdarg->bssid, sizeof(clidata.bssid)-1);
	memcpy(clidata.almac, cmdarg->almac, sizeof(clidata.almac)-1);
	...
	if (cmdarg->use_master)
		ret = wbd_cli_send(data, EAPD_WKSP_WBD_TCP_MASTERCLI_PORT, cmdarg->flags);
	else
		ret = wbd_cli_send(data, EAPD_WKSP_WBD_TCP_SLAVECLI_PORT, cmdarg->flags);

end:
	return ret;
}

/* Special Processing for CLIENTLIST command with -f option */
static int
wbd_cli_process_clientlist(cmdargs_t *cmdarg)
{
	int ret = WBDE_OK;
	time_t ts_apm_query_sent = time(NULL);

	/* [1] FIRST call of API wbd_cli_process : clientlist with FORCE option :
	 * SEND : wb_cli asks Controller Daemon to Send AP Metric Query to Agent
	 * This is done by : wb_cli sending clientlist with FORCE option - Exposed to user
	 * Read Buffer will have Dummy data to be printed as CLI Output
	 */
	wbd_cli_process(cmdarg);
	cmdarg->flags &= ~WBD_CLI_FLAGS_FORCE;

	/* [2] SECOND call of API wbd_cli_process : clientlist with QUERY option :
	 * QUERY : wb_cli asks Controller Daemon to Compare
	 * APMetrics query_ts with Received stats_ts, and Get the Result.
	 * This is done by : wb_cli sending clientlist with QUERY option - NOT Exposed to user
	 * Read Buffer will have Compare TS Result, sscanf it : No data to be printed as CLI Output
	 */
	cmdarg->flags |= WBD_CLI_FLAGS_QUERY;
	/* POLL : Until updated stats are not received in AP Metrics Response - This is done by
	 * wb_cli in a Loop do QUERY, until stats_timestamp >= query_timestamp OR Poll Timeout
	 */
	while (wbd_cli_process(cmdarg)) {
		time_t now = time(NULL);

		/* If AP Metrics Response not revcd till timeout, Exit Polling */
		if ((now - ts_apm_query_sent) >= WBD_TIMEOUT_POLL_AP_METRICS) {
			WBD_CLI_PRINT("Error : AP Metrics Response not"
				" received in %d sec\n", WBD_TIMEOUT_POLL_AP_METRICS);
			break;
		}
		/* Millisec sleep bet each Polling iter for AP Metrics Resp */
		usleep(WBD_USLEEP_POLL_AP_METREICS);
	}
	cmdarg->flags &= ~WBD_CLI_FLAGS_QUERY;

	/* [3] THIRD call of API wbd_cli_process : clientlist without any options :
	 * PRINT : wb_cli asks Controller Daemon to get print buffer with Updated STA stats
	 * This is done by : wb_cli sending clientlist without any options - Exposed to user also
	 * Read Buffer will have Updated STA stats to be printed as CLI Output
	 */
...
main(int argc, char **argv)
{
	...
	/* Set Process Name */
	snprintf(g_wbd_process_name, sizeof(g_wbd_process_name), "wb_cli");
	...
	/* Special Processing for CLIENTLIST command with -f option */
	if ((cmdarg.cmd == WBD_CMD_CLI_CLIENTLIST) &&
		(cmdarg.flags & WBD_CLI_FLAGS_FORCE) && cmdarg.use_master) {
		ret = wbd_cli_process_clientlist(&cmdarg);
		WBD_ASSERT();
		goto end;
	}
