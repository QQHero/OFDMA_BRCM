 ...
 *
 * $Id: circularbuf.c 708017 2017-06-29 14:11:45Z $
 */
...
circularbuf_read_complete(circularbuf_t *handle, uint16 bytes_read)
{
	...
	/* XXX: If circular buffer wraps around (e_ptr marks the end of useful info),
	 * r_ptr should also wrap around. But e_ptr should set the mark for wrap-around for
	 * r_ptr (and not depth)
	 * Previous implementation: if ((handle->r_ptr + bytes_read) >= handle->depth)
	 */
