 ...
 *
 * $Id: phy_tof.c 809672 2022-03-22 04:44:16Z $
 */
		...
		for (core = 0; core < core_max; core++) {
			for (sts = 0; ((sts < sts_max) && (ret_val == 0)); sts++) {
				...
				ret_val += (fns->chan_freq_response)(fns->ctx, len, nbits,
						...
						Hraw_tmp, core, sts, single_core, tdcs_en, he_frm);
			}
		}
	} else {
...
#define CORDIC32_PI_FRAC_BITS 19u
...
phy_tof_gdcomp(cint32* H, int32 theta, int nfft, int delay_imp)
{
	...
	theta = (int32)(((int64) theta * PI_DEG) >> (CORDIC32_PI_FRAC_BITS));
...
int
phy_tof_phyts_setup_api(phy_info_t *ppi, bool setup, uint8 role)
{
	...
	PHY_TRACE(("phy_tof_phyts_setup_api\n"));

	if (fns->phyts_setup != NULL) {
		return (fns->phyts_setup)(fns->ctx, setup, role);
	}
...
int
phy_tof_phyts_read_sc_api(phy_info_t *ppi, cint16* sc_mem)
{
	phy_info_t *pi = (phy_info_t *)ppi;
	phy_type_tof_fns_t *fns;
	...
	PHY_TRACE(("phy_tof_phyts_read_sc_api\n"));

	if (fns->phyts_read_sc != NULL) {
		return (fns->phyts_read_sc)(fns->ctx, sc_mem);
	}
...
int
phy_tof_phyts_enable_sc_api(phy_info_t *ppi)
{
	phy_info_t *pi = (phy_info_t *)ppi;
	phy_type_tof_fns_t *fns = pi->tofi->fns;

	PHY_TRACE(("phy_tof_phyts_enable_sc_api\n"));

	if (fns->phyts_enable_sc != NULL) {
		return (fns->phyts_enable_sc)(fns->ctx);
	}
...
int
phy_tof_phyts_pkt_detect_api(phy_info_t *ppi, cint16 *sc_mem)
{
	phy_info_t *pi = (phy_info_t *)ppi;
	phy_type_tof_fns_t *fns = pi->tofi->fns;

	PHY_TRACE(("phy_tof_phyts_pkt_detect_api\n"));

	if (fns->phyts_pkt_detect != NULL) {
		return (fns->phyts_pkt_detect)(fns->ctx, sc_mem);
	}
...
int
phy_tof_phyts_mf_api(phy_info_t *ppi, cint16* sc_mem, uint32 *ts, bool mode)
{
	phy_info_t *pi = (phy_info_t *)ppi;
	phy_type_tof_fns_t *fns;
	...
	if (fns->phyts_mf != NULL) {
		return (fns->phyts_mf)(fns->ctx, sc_mem, ts, mode);
	}
...
int
phy_tof_phyts_loopback(phy_info_t *pi, bool setup)
{
	phy_type_tof_fns_t *fns = pi->tofi->fns;

	PHY_TRACE(("phy_tof_phyts_loopback\n"));

	if (fns->phyts_loopback != NULL) {
		return (fns->phyts_loopback)(fns->ctx, setup);
	}
...
int
phy_tof_phyts_set_txpwr(phy_info_t *pi, int8 txpwr_dbm)
{
	phy_type_tof_fns_t *fns = pi->tofi->fns;

	PHY_TRACE(("phy_tof_phyts_set_txpwr\n"));

	if (fns->phyts_set_txpwr != NULL) {
		return (fns->phyts_set_txpwr)(fns->ctx, txpwr_dbm);
	}
...
int
phy_tof_phyts_set_txpwr_api(phy_info_t *ppi, int8 txpwr_dbm)
{
	phy_info_t *pi = (phy_info_t *)ppi;
	ASSERT(pi != NULL);
	return phy_tof_phyts_set_txpwr(pi, txpwr_dbm);
}

int
phy_tof_phyts_get_sc_read_buf_sz_api(phy_info_t *ppi, uint32 *sc_read_buf_sz)
{
	phy_info_t *pi = (phy_info_t *)ppi;
	phy_type_tof_fns_t *fns = pi->tofi->fns;
	PHY_TRACE(("phy_tof_phyts_get_sc_read_buf_sz_api\n"));

	if (fns->phyts_get_sc_read_buf_sz != NULL) {
		return (fns->phyts_get_sc_read_buf_sz)(fns->ctx, sc_read_buf_sz);
	}
