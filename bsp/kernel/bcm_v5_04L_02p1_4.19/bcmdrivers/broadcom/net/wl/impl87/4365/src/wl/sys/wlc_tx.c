 ...
 *
 * $Id: wlc_tx.c 809943 2022-03-28 08:17:33Z $
 *
 */
	...
	if (!PIO_ENAB(wlc->pub)) {
		...
		while ((p = pktq_swdeq(swq, fifo_idx))) {
			...
			if (bsscfg == NULL) {
				 ...
				 */
				WL_ERROR(("wl%d: %s: null bsscfg from packet WLF2_PCB1=0x%x\n",
					wlc->pub->unit, __FUNCTION__, WLF2_PCB1(p)));

#ifdef WLPROBRESP_INTRANSIT_FILTER
				if (WLF2_PCB1(p) & WLF2_PCB1_PROBRESP) {
					/* SW probe responses are excluded */
				} else
#endif /* WLPROBRESP_INTRANSIT_FILTER */
				{
					/* We do not expet bsscfg to be null */
					ASSERT(bsscfg != NULL);
				}
...
wlc_hdr_proc(wlc_info_t *wlc, void *sdu, struct scb *scb)
{
	...
	*/
	...
	if ((wlc->vlan_mode == OFF) && (ether_type == ETHER_TYPE_802_1X)) {
		...
		{
			/* The first packet is Ethernet Header. Next packet is actual eapol frame
			 * with llc snap header
			*/
			void *pkt_next = PKTNEXT(osh, sdu);
			pkt = pkt_next;
		...
		if (MAP_ENAB(bsscfg) && SCB_MAP_P2_CAP(scb) && (bsscfg->map_prim_vlan_id > 0) &&
			...
			struct dot11_llc_snap_header *lsh =
				(struct dot11_llc_snap_header*)(PKTDATA(osh, pkt));
			uint8 *src;

			ASSERT((uint)PKTHEADROOM(osh, pkt) > (headroom + VLAN_TAG_LEN));

			src = (uint8*)PKTDATA(osh, pkt);
			...
			if (rfc894chk(wlc, lsh)) {
				...
				vh = (struct vlan_header*)
					(PKTPUSH(osh, pkt, VLAN_TAG_LEN));
				bcopy(src, vh, DOT11_LLC_SNAP_HDR_LEN);
				/* Insert Vlan header after snap header */
				vh = (struct vlan_header*)
					(src + DOT11_LLC_HDR_LEN_MIN + DOT11_OUI_LEN -
					 VLAN_TAG_LEN);
				vh->vlan_type = hton16(ETHER_TYPE_8021Q);
				...
				vh = (struct ethervlan_header *)PKTPUSH(osh, pkt, VLAN_TAG_LEN);
				bcopy(src, vh, VLAN_TAG_OFFSET);
