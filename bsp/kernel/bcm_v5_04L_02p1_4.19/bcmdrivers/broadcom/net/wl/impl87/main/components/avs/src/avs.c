...
#if defined(BCMDBG)
#define AVS_TRACE(args)				do { if (avs->trace) WL_PRINT(args); } while(0)
#define AVS_TRACE_IF(cond, args)		do { if (cond) AVS_TRACE(args); } while(0)
#else
#define AVS_TRACE(args)
#define AVS_TRACE_IF(cond, args)
#endif /* BCMDBG */
...
static const char BCMATTACHDATA(rstr_avsflags)[]   = "avsflags";
...
#define NUMBER_OF_CENTRALS_43684		24		/* Should adjust CEN_ROSC_MASK_0 accordingly */
#define NUMBER_OF_CENTRALS_6710			24
#define NUMBER_OF_CENTRALS_6715			34
#define NUMBER_OF_CENTRALS(sih)			(BCM6715_CHIP(sih->chip) ? NUMBER_OF_CENTRALS_6715 : \
						(BCM6710_CHIP(sih->chip) ? NUMBER_OF_CENTRALS_6710 : \
						NUMBER_OF_CENTRALS_43684))
#define NUMBER_OF_CENTRALS_MAX			MAX(MAX(NUMBER_OF_CENTRALS_43684, NUMBER_OF_CENTRALS_6710), NUMBER_OF_CENTRALS_6715)
#define NUMBER_OF_REMOTES_43684			9
#define NUMBER_OF_REMOTES_6710			5
#define NUMBER_OF_REMOTES_6715			8
#define NUMBER_OF_REMOTES(sih)			(BCM6715_CHIP(sih->chip) ? NUMBER_OF_REMOTES_6715 : \
						(BCM6710_CHIP(sih->chip) ? NUMBER_OF_REMOTES_6710 : \
						NUMBER_OF_REMOTES_43684))
#define NUMBER_OF_REMOTES_MAX			MAX(MAX(NUMBER_OF_REMOTES_43684, NUMBER_OF_REMOTES_6710), NUMBER_OF_CENTRALS_6715)
#define AVS_DETECT_THRESHOLD			10		/* Required voltage delta (mV) for detecting AVS feature */
#define ROSC_DETECT_THRESHOLD			100		/* Required counts per measuring interval for detecting oscillator */
#define LVM_ENABLE_THRESHOLD_43684		920		/* Threshold (mV) for enabling Low Voltage Memory assist, this value includes */
								/* a margin of 20mV for dynamic IR drop at the local memories and voltage */
								/* measurement error. LVM threshold can be set as high as 0.95V for both */
								/* LL en GL memories with no adverse impact on either cell type. */
#define LVM_ENABLE_THRESHOLD_6715		765
#define LVM_ENABLE_THRESHOLD(sih)		(BCM6715_CHIP(sih->chip) ? LVM_ENABLE_THRESHOLD_6715 : \
						LVM_ENABLE_THRESHOLD_43684)
#define LVM_ENABLE_HYSTERESIS			10		/* Hysteresis (mV) to apply to LVM control */
#define ADC_FULL_SCALE_VOLTAGE_28NM		880		/* (mV) */
#define ADC_FULL_SCALE_VOLTAGE_16NM		944		/* (mV) */
#define ADC_FULL_SCALE_VOLTAGE(sih)		(BCM6715_CHIP(sih->chip) ? ADC_FULL_SCALE_VOLTAGE_16NM : \
						ADC_FULL_SCALE_VOLTAGE_28NM)
...
#define DEFAULT_DAC_VOLTAGE_STEP_RATIO_2	0.600		/* Voltage step corresponding to 1 DAC code (mV) when Rdac/Rtop=2 [BOARD] */
#define DEFAULT_DAC_VOLTAGE_STEP_RATIO_4	0.300		/* Voltage step corresponding to 1 DAC code (mV) when Rdac/Rtop=4 [BOARD] */
#define DEFAULT_REGULATOR_SETTLE_TIME_MS	10		/* Voltage regulator settle time (ms) [BOARD] */
#define DAC_CAP_CHARGE_TIME_US			200		/* Time to charge the external DAC output capacitor (us) [BOARD] */
								/* We need to charge the capacitor before connecting the DAC output to the */
								/* external regulator feedback circuit to avoid pulling the feedback voltage */
								/* down and causing a voltage spike on regulator output. DAC charging takes */
								/* ~50us for 0.47uF on PVTMON_DAC */
#define NUMBER_OF_SENSORS_28NM			2		/* number of sensors of each remote oscillator */
#define NUMBER_OF_SENSORS_16NM			6		/* number of sensors of each remote oscillator */
#define NUMBER_OF_SENSORS_MAX			MAX(NUMBER_OF_SENSORS_28NM, NUMBER_OF_SENSORS_16NM)

/*
 * Hard limits (for configuration validation)
 */

#define MAX_VMARGIN				200		/* Maximum margin in either corner (mV) */
#define MIN_VLIMIT_28NM				800		/* Minimum operating voltage (mV) */
#define MIN_VLIMIT_16NM				650		/* Minimum operating voltage (mV) */
#define MAX_VLIMIT_28NM				1100		/* Maximum operating voltage (mV) */
#define MAX_VLIMIT_16NM				900		/* Maximum operating voltage (mV) */
#define MIN_THRESHOLD_MARGIN			1		/* Minimum treshold margin (mV) */
#define MAX_THRESHOLD_MARGIN			20		/* Maximum treshold margin (mV) */
#define MIN_INITIAL_DAC_CODE			500		/* Minimum initial DAC code */
#define MAX_INITIAL_DAC_CODE			800		/* Maximum initial DAC code */
#define MIN_DAC_VOLTAGE_STEP_MV_28NM		0.500		/* Minimum voltage step per dac code (mV) */
#define MIN_DAC_VOLTAGE_STEP_MV_16NM		0.200		/* Minimum voltage step per dac code (mV) */
#define MIN_DAC_VOLTAGE_STEP_MV(sih)		(BCM6715_CHIP(sih->chip) ? MIN_DAC_VOLTAGE_STEP_MV_16NM : \
						MIN_DAC_VOLTAGE_STEP_MV_28NM)
...
#define DEFAULT_VMARGIN_L_43684			55		/* Margin given in FF corner (mV) [BOARD] */
#define DEFAULT_VMARGIN_H_43684			60		/* Margin given in SS corner (mV) [BOARD] */
#define DEFAULT_VLIMIT_L_43684			860		/* Minimum operating voltage (mV) [BOARD] */
#define DEFAULT_VLIMIT_H_43684			1035		/* Maximum operating voltage (mV) [BOARD] */

#define DEFAULT_VMARGIN_L_6710			55		/* Margin given in FF corner (mV) [BOARD] */
#define DEFAULT_VMARGIN_H_6710			60		/* Margin given in SS corner (mV) [BOARD] */
#define DEFAULT_VLIMIT_L_6710			840		/* Minimum operating voltage (mV) [BOARD] */
#define DEFAULT_VLIMIT_H_6710			1035		/* Maximum operating voltage (mV) [BOARD] */

#define DEFAULT_VMARGIN_L_6715			70		/* Margin given in FF corner (mV) [BOARD] */
#define DEFAULT_VMARGIN_H_6715			30		/* Margin given in SS corner (mV) [BOARD] */
#define DEFAULT_VLIMIT_L_6715			668		/* Minimum operating voltage (mV) [BOARD] */
#define DEFAULT_VLIMIT_H_6715			835		/* Maximum operating voltage (mV) [BOARD] */

#define DEFAULT_THRESHOLD_MARGIN		5		/* Margin between low and high rosc threshold (mV) */
...
#define LOCAL_FIT_DAC_OFFSET			30		/* DAC offset for local fit (mV) */
#define GLOBAL_FIT_SAFETY_MARGIN		20		/* Safety margin for global fit (mV) */
#define NOMINAL_VOLTAGE_28NM			1030		/* Nominal voltage (mV) */
#define NOMINAL_VOLTAGE_16NM			830		/* Nominal voltage (mV) */
#define NOMINAL_VOLTAGE(sih)			(BCM6715_CHIP(sih->chip) ? NOMINAL_VOLTAGE_16NM : \
						NOMINAL_VOLTAGE_28NM)
#define NOMINAL_VOLTAGE_ERR			5		/* Max distance from nominal voltage (mV) */
...
#define ROSC_FREQ_6715				50		/* ROSC frequency (MHz) */
#define ROSC_FREQ(sih)				(BCM6715_CHIP(sih->chip) ? ROSC_FREQ_6715 : \
						(BCM6710_CHIP(sih->chip) ? ROSC_FREQ_6710 : \
						ROSC_FREQ_43684))
...
#define CEN_ROSC_MASK_0_28NM			0xff000000
#define CEN_ROSC_MASK_1_28NM			0xffffffff
#define CEN_ROSC_MASK_0_16NM			0x00000000
#define CEN_ROSC_MASK_1_16NM			0xfffffffc
...
#define NUMBER_OF_OSCILLATORS(sih)		(BCM6715_CHIP(sih->chip) ? (NUMBER_OF_CENTRALS(sih) + NUMBER_OF_SENSORS_16NM*NUMBER_OF_REMOTES(sih)) : \
						(NUMBER_OF_CENTRALS(sih) + NUMBER_OF_SENSORS_28NM*NUMBER_OF_REMOTES(sih)))
#define NUMBER_OF_OSCILLATORS_MAX		(NUMBER_OF_CENTRALS_MAX + NUMBER_OF_SENSORS_MAX*NUMBER_OF_REMOTES_MAX)
#define IGNORE_OSCILLATOR			0
#define INVALID_OSCILLATOR_COUNT		0

#define AVS_FLAGS_DEFAULT			0xffff
#define RDAC_RTOP_RATIO_MASK			0x000f
#define RDAC_RTOP_RATIO_SHIFT			0
...
static const osc_freq_t oscillator_thresholds_43684[] = {
	...
	/* 22 */	IGNORE_OSCILLATOR,	/* Excluded because there is poor correlation with process corner, */
	/* 23 */	IGNORE_OSCILLATOR	/* resulting in large errors in the convergence algorithm. */
};
...
static const osc_freq_t oscillator_thresholds_6710[] = {
 ...
 /* 6715 central oscillator frequency thresholds at 0.72V, SS, 125C condition, units are MHz */
static const osc_freq_t oscillator_thresholds_6715[] = {
	/*  0 */	FLOAT_TO_INT(1.0898),
	/*  1 */	FLOAT_TO_INT(5.3886),
	/*  2 */	IGNORE_OSCILLATOR,
	/*  3 */	FLOAT_TO_INT(7.2792),
	/*  4 */	FLOAT_TO_INT(5.2102),
	/*  5 */	FLOAT_TO_INT(4.0866),
	/*  6 */	FLOAT_TO_INT(3.5226),
	/*  7 */	FLOAT_TO_INT(4.2240),
	/*  8 */	FLOAT_TO_INT(3.7257),
	/*  9 */	FLOAT_TO_INT(2.9673),
	/* 10 */	FLOAT_TO_INT(2.5943),
	/* 11 */	FLOAT_TO_INT(2.0202),
	/* 12 */	FLOAT_TO_INT(1.7365),
	/* 13 */	FLOAT_TO_INT(6.6900),
	/* 14 */	FLOAT_TO_INT(5.9673),
	/* 15 */	FLOAT_TO_INT(4.7543),
	/* 16 */	FLOAT_TO_INT(4.1702),
	/* 17 */	FLOAT_TO_INT(3.2687),
	/* 18 */	FLOAT_TO_INT(2.8055),
	/* 19 */	IGNORE_OSCILLATOR,	/* Excluded because there is poor correlation with process corner, */
	/* 20 */	IGNORE_OSCILLATOR,	/* resulting in large errors in the convergence algorithm. */
	/* 21 */	IGNORE_OSCILLATOR,
	/* 22 */	IGNORE_OSCILLATOR,
	/* 23 */	IGNORE_OSCILLATOR,
	/* 24 */	IGNORE_OSCILLATOR,
	/* 25 */	IGNORE_OSCILLATOR,
	/* 26 */	IGNORE_OSCILLATOR,
	/* 27 */	IGNORE_OSCILLATOR,
	/* 28 */	IGNORE_OSCILLATOR,
	/* 29 */	IGNORE_OSCILLATOR,
	/* 30 */	IGNORE_OSCILLATOR,
	/* 31 */	IGNORE_OSCILLATOR,
	/* 32 */	IGNORE_OSCILLATOR,
	/* 33 */	IGNORE_OSCILLATOR
};
...
#define LVM_SUPPORT(sih)			(BCM43684_CHIP((sih)->chip) || BCM6715_CHIP(sih->chip))
#else
...
struct avs_context {
...
#endif
	bool		initialized;
	bool		tracking;		/* AVS tracking switch, FALSE for read voltage and temp only */
	bool		trace;			/* Print out tracing procedure */
	uint		iterations;
	uint		dac_adjustments[2];	/* Down/up DAC adjustment count */
	bool		dac_enabled;
	uint		function_mode;		/* Current AVS function mode */
	uint		flags;			/* AVS flags in srom/nvram text file */
	...
	dac_code_t	dac_code_setting;	/* external DAC setting */

	uint		voltage_nominal;	/* Nominal voltage */
	...
	uint		local_fit_v_offset;	/* voltage offset for local fit */
...
get_rosc_threshold(avs_context_t *avs, uint osc_index)
{
	...
	switch (CHIPID(avs->sih->chip)) {
		CASE_BCM6715_CHIP:
			ASSERT(NUMBER_OF_CENTRALS(avs->sih) <=
				(sizeof(oscillator_thresholds_6715) / sizeof(oscillator_thresholds_6715[0])));
			if (osc_index < NUMBER_OF_CENTRALS(avs->sih)) {
				/* Central oscillators have hardcoded thresholds */
				freq = oscillator_thresholds_6715[osc_index];
			} else {
				/* Remote oscillators share a threshold based on type */
				osc_index -= NUMBER_OF_CENTRALS(avs->sih);
				switch (osc_index % NUMBER_OF_SENSORS_16NM) {
					case 0:
						freq = oscillator_thresholds_6715[7];	/* Remote UVT S type ROSC is a copy of Central ROSC 7 */
						break;
					case 1:
						freq = oscillator_thresholds_6715[8];	/* Remote UVT H type ROSC is a copy of Central ROSC 8 */
						break;
					case 2:
						freq = oscillator_thresholds_6715[9];	/* Remote LVT S type ROSC is a copy of Central ROSC 9 */
						break;
					case 3:
						freq = oscillator_thresholds_6715[10];	/* Remote LVT H type ROSC is a copy of Central ROSC 10 */
						break;
					case 4:
						freq = oscillator_thresholds_6715[11];	/* Remote SVT S type ROSC is a copy of Central ROSC 11 */
						break;
					case 5:
						freq = oscillator_thresholds_6715[12];	/* Remote SVT H type ROSC is a copy of Central ROSC 12 */
						break;
					default:
						ASSERT(0);
						break;
		...
		CASE_BCM6710_CHIP:
			ASSERT(NUMBER_OF_CENTRALS(avs->sih) <=
				(sizeof(oscillator_thresholds_6710) / sizeof(oscillator_thresholds_6710[0])));
			if (osc_index < NUMBER_OF_CENTRALS(avs->sih)) {
				/* Central oscillators have hardcoded thresholds */
				...
				/* Remote oscillators share a threshold based on type */
				osc_index -= NUMBER_OF_CENTRALS(avs->sih);
				freq = (osc_index % NUMBER_OF_SENSORS_28NM) ?
		...
		default:
			ASSERT(NUMBER_OF_CENTRALS(avs->sih) <=
				(sizeof(oscillator_thresholds_43684) / sizeof(oscillator_thresholds_43684[0])));
			if (osc_index < NUMBER_OF_CENTRALS(avs->sih)) {
				/* Central oscillators have hardcoded thresholds */
 ...
 * @return			Voltage (V * SCALING_FACTOR).
 */

static uint
dac_code_to_voltage(linear_eq_t voltage_daccode, dac_code_t dac_code)
{
	ASSERT(!(voltage_daccode.slope == 0 && voltage_daccode.intercept == 0));

	if (dac_code != INVALID_DAC_CODE && voltage_daccode.slope != 0) {
		return ((dac_code - voltage_daccode.intercept) * SCALING_FACTOR) / voltage_daccode.slope;
	}
...
voltage_to_margin(linear_eq_t voltage_margin, uint voltage)
{
	...
	return (int)(((voltage_margin.slope * (int)voltage) / SCALING_FACTOR) + voltage_margin.intercept);
}
...
read_central_rosc_count(avs_context_t *avs, uint osc_num)
{
	...
	ASSERT(osc_num < NUMBER_OF_CENTRALS(avs->sih));

	if (osc_num >= NUMBER_OF_CENTRALS(avs->sih)) {
		return 0;
	}
...
read_remote_rosc_count(avs_context_t *avs, uint osc_num, osc_count_t *svt_count, osc_count_t *hvt_count)
{
	...
	*svt_count = *hvt_count = INVALID_OSCILLATOR_COUNT;
	...
	while (i--) {
		...
		reg = read_register(avs, BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
		if ((reg & REMOTE_ROSC_CONTROL_VALID) == REMOTE_ROSC_CONTROL_VALID) {
			/* Read counters */
			sleep(PMB_REMOTE_ROSC_DELAY_US);
			reg = read_register(avs, BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
			sleep(PMB_REMOTE_ROSC_DELAY_US);
...
/**
 * Read the most recent REMOTE oscillator for 6 sensors.
 *
 * @param avs		Pointer to AVS context.
 * @param osc_num	Oscillator index.
 * @param count		oscillator count array, or INVALID_OSCILLATOR_COUNT on invalid index or read error.
 */

static void
read_remote_rosc_count_16nm(avs_context_t *avs, uint osc_num, osc_count_t *count)
{
	int i;
	uint32 reg;
	...
	for (i=0; i<NUMBER_OF_SENSORS_16NM; i++)
		count[i] = INVALID_OSCILLATOR_COUNT;

	if (osc_num >= NUMBER_OF_REMOTES(avs->sih)) {
		return;
	}

	i = PVT_REGISTER_REREAD_COUNT;
	while (i--) {
		/* Read control register */
		reg = read_register(avs, BCHP_AVS_PMB_S_000U_AVS_ROSC_CONTROL + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
		if ((reg & REMOTE_ROSC_CONTROL_VALID) == REMOTE_ROSC_CONTROL_VALID) {
			/* Read counters */
			sleep(PMB_REMOTE_ROSC_DELAY_US);
			reg = read_register(avs, BCHP_AVS_PMB_S_000U_AVS_ROSC_COUNT + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
			sleep(PMB_REMOTE_ROSC_DELAY_US);

			count[0] = (reg & BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_S_MASK) >> BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_S_SHIFT;
			count[1] = (reg & BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_H_MASK) >> BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_H_SHIFT;

			/* Ignore any remote that returns an oscillator value below the threshold */
			if (count[0] < ROSC_DETECT_THRESHOLD) count[0] = INVALID_OSCILLATOR_COUNT;
			if (count[1] < ROSC_DETECT_THRESHOLD) count[1] = INVALID_OSCILLATOR_COUNT;
			break;
		}
	...
	while (i--) {
		/* Read control register */
		reg = read_register(avs, BCHP_AVS_PMB_S_000L_AVS_ROSC_CONTROL + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
		if ((reg & REMOTE_ROSC_CONTROL_VALID) == REMOTE_ROSC_CONTROL_VALID) {
			/* Read counters */
			sleep(PMB_REMOTE_ROSC_DELAY_US);
			reg = read_register(avs, BCHP_AVS_PMB_S_000L_AVS_ROSC_COUNT + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
			sleep(PMB_REMOTE_ROSC_DELAY_US);

			count[2] = (reg & BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_S_MASK) >> BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_S_SHIFT;
			count[3] = (reg & BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_H_MASK) >> BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_H_SHIFT;

			/* Ignore any remote that returns an oscillator value below the threshold */
			if (count[2] < ROSC_DETECT_THRESHOLD) count[2] = INVALID_OSCILLATOR_COUNT;
			if (count[3] < ROSC_DETECT_THRESHOLD) count[3] = INVALID_OSCILLATOR_COUNT;
			break;
		}
	...
	while (i--) {
		/* Read control register */
		reg = read_register(avs, BCHP_AVS_PMB_S_000S_AVS_ROSC_CONTROL + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
		if ((reg & REMOTE_ROSC_CONTROL_VALID) == REMOTE_ROSC_CONTROL_VALID) {
			/* Read counters */
			sleep(PMB_REMOTE_ROSC_DELAY_US);
			reg = read_register(avs, BCHP_AVS_PMB_S_000S_AVS_ROSC_COUNT + (osc_num * BCHP_AVS_PMB_OFFSET(avs->sih)));
			sleep(PMB_REMOTE_ROSC_DELAY_US);

			count[4] = (reg & BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_S_MASK) >> BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_S_SHIFT;
			count[5] = (reg & BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_H_MASK) >> BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT_COUNT_H_SHIFT;

			/* Ignore any remote that returns an oscillator value below the threshold */
			if (count[4] < ROSC_DETECT_THRESHOLD) count[4] = INVALID_OSCILLATOR_COUNT;
			if (count[5] < ROSC_DETECT_THRESHOLD) count[5] = INVALID_OSCILLATOR_COUNT;
			break;
		}
...
initialize_oscillators(avs_context_t *avs)
{
	...
	if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
		write_register(avs, BCHP_AVS_HW_MNTR_SEQUENCER_MASK_CEN_ROSC_0, CEN_ROSC_MASK_0_28NM);
	} else if (BCM6715_CHIP(avs->sih->chip)) {
		write_register(avs, BCHP_AVS_HW_MNTR_SEQUENCER_MASK_CEN_ROSC_0, CEN_ROSC_MASK_0_16NM);
		write_register(avs, BCHP_AVS_HW_MNTR_SEQUENCER_MASK_CEN_ROSC_1, CEN_ROSC_MASK_1_16NM);
	}
	...
	for (i=0; i<NUMBER_OF_REMOTES(avs->sih); i++) {
		...
		if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
			write_register(avs, BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL + (i * BCHP_AVS_PMB_OFFSET(avs->sih)),
			...
			write_register(avs, BCHP_AVS_PMB_S_000U_AVS_ROSC_CONTROL + (i * BCHP_AVS_PMB_OFFSET(avs->sih)),
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_RO_EN_S |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_RO_EN_H |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_ECTR_EN_S |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_ECTR_EN_H |
				...
				(ROSC_MEASUREMENT_TIME << BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_TEST_INTERVAL_SHIFT));
			sleep(PMB_REMOTE_ROSC_DELAY_US);
			write_register(avs, BCHP_AVS_PMB_S_000L_AVS_ROSC_CONTROL + (i * BCHP_AVS_PMB_OFFSET(avs->sih)),
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_RO_EN_S |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_RO_EN_H |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_ECTR_EN_S |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_ECTR_EN_H |
				...
				(ROSC_MEASUREMENT_TIME << BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_TEST_INTERVAL_SHIFT));
			sleep(PMB_REMOTE_ROSC_DELAY_US);
			write_register(avs, BCHP_AVS_PMB_S_000S_AVS_ROSC_CONTROL + (i * BCHP_AVS_PMB_OFFSET(avs->sih)),
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_RO_EN_S |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_RO_EN_H |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_ECTR_EN_S |
				BCHP_AVS_PMB_S_000_AVS_ROSC_CONTROL_ECTR_EN_H |
...
static int
pvt_register_to_value(avs_context_t *avs, pvt_t pvt, uint32 reg)
{
	...
	switch(pvt) {
		case PVT_TEMPERATURE:
			if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
				 ...
				 *	t = 450 - (adc * 0.54956)
				 */
				return FLOAT_TO_INT(450) - ((int)reg * FLOAT_TO_INT(0.54956));
			}
		...
		case PVT_1V_0:
			if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
				 ...
				 */
				return (reg * MV_TO_INT(ADC_FULL_SCALE_VOLTAGE(avs->sih)) * 10) / 7168;
			} else if (BCM6715_CHIP(avs->sih->chip)) {
				/* Convert PVT measurement to voltage, unit is V * SCALING_FACTOR
				 *
				 *	v = adc / 1024 * ADC_FULL_SCALE_VOLTAGE / 0.8
				 *	v = (ADC_FULL_SCALE_VOLTAGE * adc) / 819.2
				 *	v = (ADC_FULL_SCALE_VOLTAGE * adc * 10) / 8192
				 */
				return (reg * MV_TO_INT(ADC_FULL_SCALE_VOLTAGE(avs->sih)) * 10) / 8192;
			}
...
static int
raw_adc_register_to_voltage(avs_context_t *avs, uint32 reg)
{
	 ...
	 */
	return (reg * MV_TO_INT(ADC_FULL_SCALE_VOLTAGE(avs->sih))) / 1024;
}

#endif

static dac_code_t
raw_adc_register_to_dac_code(avs_context_t *avs, uint32 reg)
{
	AVS_ERROR_IF(reg > MAX_ADC_CODE, ("%s: invalid value %u(0x%x)\n", __FUNCTION__, reg, reg));
	...
	if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
		...
		/* Convert raw ADC measurement to the corresponding DAC code
		 *
		 *	reg = round(1536 - 1.3026 * dac)
		 *	dac = round((1536 - reg) / 1.3026)
		 */
		return (FLOAT_TO_INT(1536) - (reg * SCALING_FACTOR) + SCALING_FACTOR) / FLOAT_TO_INT(1.3026);
	} else {
...
reset_measurements(avs_context_t *avs, reset_t reset)
{
	...
	if (reset & RESET_CENTRAL) {
		...
		if (NUMBER_OF_CENTRALS(avs->sih) > 31)
	...
	if (reset & RESET_REMOTE) {
		...
		if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
			for (i=0; i<NUMBER_OF_REMOTES(avs->sih); i++) {
				...
				read_register(avs, BCHP_AVS_PMB_S_000_AVS_ROSC_COUNT + (i * BCHP_AVS_PMB_OFFSET(avs->sih)));
				sleep(PMB_REMOTE_ROSC_DELAY_US);
			}
		} else if (BCM6715_CHIP(avs->sih->chip)) {
			for (i=0; i<NUMBER_OF_REMOTES(avs->sih); i++) {
				/* Read counters to clear valid bits in counter control registers */
				read_register(avs, BCHP_AVS_PMB_S_000U_AVS_ROSC_COUNT + (i * BCHP_AVS_PMB_OFFSET(avs->sih)));
				sleep(PMB_REMOTE_ROSC_DELAY_US);
				read_register(avs, BCHP_AVS_PMB_S_000L_AVS_ROSC_COUNT + (i * BCHP_AVS_PMB_OFFSET(avs->sih)));
				sleep(PMB_REMOTE_ROSC_DELAY_US);
				read_register(avs, BCHP_AVS_PMB_S_000S_AVS_ROSC_COUNT + (i * BCHP_AVS_PMB_OFFSET(avs->sih)));
				sleep(PMB_REMOTE_ROSC_DELAY_US);
			}
		}
	}
...
{
	return pvt_register_to_value(avs, pvt, read_pvt_register(avs, pvt, averaging));
}
...
read_tmon_value(avs_context_t *avs)
{
	...
	if (BCM43684_CHIP(avs->sih->chip)) {
		...
		if (reg & BCHP_AVS_TMON_TEMPERATURE_MEASUREMENT_VALID) {
			...
			return pvt_register_to_value(avs, PVT_TEMPERATURE, (reg & BCHP_AVS_TMON_TEMPERATURE_MEASUREMENT_DATA_MASK) >> 1);
		}
	}
...
{
	ASSERT(BCM43684_CHIP(avs->sih->chip) || BCM6715_CHIP(avs->sih->chip));
...
check_LVM(avs_context_t *avs, uint voltage)
{
	if (LVM_SUPPORT(avs->sih)) {
		...
		if (avs->lvm_state == FALSE && voltage <= MV_TO_INT(LVM_ENABLE_THRESHOLD(avs->sih))) {
			enable = TRUE;
		} else if (avs->lvm_state == TRUE && voltage > MV_TO_INT(LVM_ENABLE_THRESHOLD(avs->sih) + LVM_ENABLE_HYSTERESIS)) {
			enable = FALSE;
		} else {
			return;
		}
...
get_board_dac_code(avs_context_t *avs)
{
	...
	if (reg != INVALID_PVT_VALUE) {
		...
		dac_code = raw_adc_register_to_dac_code(avs, reg);
	...
	AVS_TRACE(("%s: adc_code=%u Vfb=%umV dac_code=%u\n", __FUNCTION__, reg, TO_MV(raw_adc_register_to_voltage(avs, reg)), dac_code));

#if defined(BCMDBG) || defined(AVS_ENABLE_STATUS)
	avs->board_dac_code   = dac_code;
	avs->voltage_feedback = raw_adc_register_to_voltage(avs, reg);
#endif
...
write_dac_code(avs_context_t *avs, dac_code_t dac_code)
{
	...
	/* If switching to DAC drive mode, charge the Cdac capacitor first to avoid a voltage spike */
	if (avs->function_mode != BCHP_AVS_PVT_MNTR_CONFIG_PVT_MNTR_CTRL_MODE_DAC_DRIVE) {
		AVS_TRACE(("%s: charging Cdac (%u)\n", __FUNCTION__, avs->function_mode));
		set_function_mode(avs, BCHP_AVS_PVT_MNTR_CONFIG_PVT_MNTR_CTRL_MODE_VTMON_DAC, TRUE);
		sleep(DAC_CAP_CHARGE_TIME_US);
	}
 ...
 * For 6715, remote oscillators are 6 * NUMBER_OF_REMOTES
 *
...
check_thresholds(avs_context_t *avs, uint osc_index, osc_count_t count, bool *increase, bool *decrease)
{
	...
	AVS_TRACE(("%c OSC%02d ",
		osc_index < NUMBER_OF_CENTRALS(avs->sih) ? 'C' : 'R',
		osc_index < NUMBER_OF_CENTRALS(avs->sih) ? osc_index : osc_index - NUMBER_OF_CENTRALS(avs->sih)));
...
track(avs_context_t *avs)
{
	...
	osc_count_t count[NUMBER_OF_SENSORS_MAX];
	...
	if (avs->tracking == FALSE) {
		if (avs->dac_code_setting) {
			set_dac_voltage(avs, avs->dac_code_setting);
			avs->dac_code_setting = 0;
		}
		return BCME_OK;
	}
	...
	AVS_TRACE(("%s: v=%umV dac_code=%u AVStemp=%dmdC ARMtemp=%dmdC LVM=%u\n",
	...
	if (voltage < avs->vlimit_low) {
...
#ifdef BCMDBG
		for(i=0; i<NUMBER_OF_CENTRALS(avs->sih); i++)
#else
		for(i=0; i<NUMBER_OF_CENTRALS(avs->sih) && increase == FALSE; i++)
#endif /* BCMDBG */
		{
...
#endif /* BCMDBG */
		{
			if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
				...
				read_remote_rosc_count_16nm(avs, i, count);
				check_thresholds(avs, osc_index++, count[0], &increase, &decrease);
				check_thresholds(avs, osc_index++, count[1], &increase, &decrease);
				check_thresholds(avs, osc_index++, count[2], &increase, &decrease);
				check_thresholds(avs, osc_index++, count[3], &increase, &decrease);
				check_thresholds(avs, osc_index++, count[4], &increase, &decrease);
				check_thresholds(avs, osc_index++, count[5], &increase, &decrease);
			}
		}

		if (increase == TRUE) {
			AVS_TRACE(("%s: voltage %umV LOW\n", __FUNCTION__, TO_MV(voltage)));
		} else if (decrease == TRUE) {
			AVS_TRACE(("%s: voltage %umV HIGH\n", __FUNCTION__, TO_MV(voltage)));
		} else {
...
find_nominal_voltage(avs_context_t *avs, dac_code_t initial_dac_code)
{
	...
	AVS_TRACE(("%s: v=%umV initial_dac=%u\n", __FUNCTION__, avs->voltage_nominal, dac_code));
	...
	while(iterations--) {
		...
		voltage_delta = (int)(voltage - MV_TO_INT(avs->voltage_nominal));

		AVS_TRACE(("dac_code=%u v=%umV delta=%dmV\n", dac_code, TO_MV(voltage), TO_MV(voltage_delta)));

		/* Exit if the measured voltage is within the required range */
		if (ABS(voltage_delta) <= MV_TO_INT(NOMINAL_VOLTAGE_ERR)) {
			return dac_code;
		}
...
initialize_thresholds(avs_context_t *avs, predict_context_t *context)
{
	...
	AVS_TRACE(("%s: vconv=%umV margin=%dmV/%dmV\n", __FUNCTION__,
...
predict(avs_context_t *avs, predict_context_t* context)
{
	...
	osc_count_t threshold, count[NUMBER_OF_SENSORS_MAX];
	...
	if (context->pass == 0) {
		...
		AVS_TRACE(("upper measurement: dac=%u v=%umV\n", context->dac_high, TO_MV(context->vhigh)));
		...
		for(i=0; i<NUMBER_OF_REMOTES(avs->sih); i++) {
			if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
				...
				predict_store_initial_count(avs, osc_index++, context, count[0]);
				predict_store_initial_count(avs, osc_index++, context, count[1]);
				predict_store_initial_count(avs, osc_index++, context, count[2]);
				predict_store_initial_count(avs, osc_index++, context, count[3]);
				predict_store_initial_count(avs, osc_index++, context, count[4]);
				predict_store_initial_count(avs, osc_index++, context, count[5]);
			}
		}
	}
	...
	for(i=0; i<NUMBER_OF_REMOTES(avs->sih); i++) {
		if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
			...
			predict_find_vconv(avs, osc_index++, context, delta_voltage, count[1], threshold);
			threshold = get_rosc_threshold(avs, osc_index);
			predict_find_vconv(avs, osc_index++, context, delta_voltage, count[2], threshold);
			threshold = get_rosc_threshold(avs, osc_index);
			predict_find_vconv(avs, osc_index++, context, delta_voltage, count[3], threshold);
			threshold = get_rosc_threshold(avs, osc_index);
			predict_find_vconv(avs, osc_index++, context, delta_voltage, count[4], threshold);
			threshold = get_rosc_threshold(avs, osc_index);
			predict_find_vconv(avs, osc_index++, context, delta_voltage, count[5], threshold);
		}
	}
...
/**
 * Parse avsflags in SROM/NVRAM text file
 *
 * @param avs		Pointer to AVS context.
 */

static void
parse_avs_flags(avs_context_t *avs)
{
	if (BCM6715_CHIP(avs->sih->chip)) {
		/* Get Rdac/Rtop ratio */
		uint ratio = (avs->flags & RDAC_RTOP_RATIO_MASK) >> RDAC_RTOP_RATIO_SHIFT;
		switch (ratio) {
		case 4:
			/* if Rdac/Rtop == 4, overwrite Vstep to 300uV */
			avs->dac_min_vstep = FLOAT_TO_INT(DEFAULT_DAC_VOLTAGE_STEP_RATIO_4);
			break;
		case 2:
		default:
			/* if Rdac/Rtop == 2 or avsflags is not set, use the default value (600uV) */
			avs->dac_min_vstep = FLOAT_TO_INT(DEFAULT_DAC_VOLTAGE_STEP_RATIO_2);
			break;
		}
	}
}
...
validate_config(avs_context_t *avs)
{
	...
	if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
		if (avs->vlimit_low < MV_TO_INT(MIN_VLIMIT_28NM))	return BCME_ERROR;
		if (avs->vlimit_high > MV_TO_INT(MAX_VLIMIT_28NM))	return BCME_ERROR;
	} else if (BCM6715_CHIP(avs->sih->chip)) {
		if (avs->vlimit_low < MV_TO_INT(MIN_VLIMIT_16NM))	return BCME_ERROR;
		if (avs->vlimit_high > MV_TO_INT(MAX_VLIMIT_16NM))	return BCME_ERROR;
	}
	if (avs->vlimit_low > avs->vlimit_high)				return BCME_ERROR;

	/* Margin checks */
	if (avs->vmargin_low > MV_TO_INT(MAX_VMARGIN))			return BCME_ERROR;
	if (avs->vmargin_high > MV_TO_INT(MAX_VMARGIN))			return BCME_ERROR;

	/* Voltage step checks */
	if (avs->dac_min_vstep < FLOAT_TO_INT(MIN_DAC_VOLTAGE_STEP_MV(avs->sih)))
									return BCME_ERROR;
	...
	/* Threshold margin */
	if (avs->threshold_margin < MV_TO_INT(MIN_THRESHOLD_MARGIN))	return BCME_ERROR;
	if (avs->threshold_margin > MV_TO_INT(MAX_THRESHOLD_MARGIN))	return BCME_ERROR;

	/* Nominal voltage */
	if (avs->voltage_nominal > TO_MV(avs->vlimit_high))		return BCME_ERROR;
	if (avs->voltage_nominal < TO_MV(avs->vlimit_low))		return BCME_ERROR;
...
{
	int result = BCME_ERROR, pass, delta, vmargin = 0, vdelta;
	uint vsum, vsum_safe, duration;
	uint64 init_start = OSL_SYSUPTIME_US();
	dac_code_t dac_code;
	predict_context_t *predict_context = NULL;

	STATIC_ASSERT(ROSC_MEASUREMENT_TIME_CONTROL <= 0x7f);
	ASSERT(NUMBER_OF_REMOTES(avs->sih) <= NUMBER_OF_REMOTES_MAX);

	AVS_TRACE(("initializing AVS\n"));
	avs->initialized = FALSE;
	...
	AVS_TRACE(("margins=%umV/%umV voltage=%umV/%umV\n",
		TO_MV(avs->vmargin_low), TO_MV(avs->vmargin_high),
		TO_MV(avs->vlimit_low), TO_MV(avs->vlimit_high)));

	vdelta = (avs->vlimit_high - avs->vlimit_low) + (int)(avs->vmargin_low - avs->vmargin_high);
	if (vdelta == 0) {
	...
	avs->voltage_margin.slope     = ((int)(avs->vmargin_high - avs->vmargin_low) * SCALING_FACTOR) / vdelta;
	avs->voltage_margin.intercept = avs->vmargin_low - ((avs->voltage_margin.slope * (int)(avs->vlimit_low - avs->vmargin_low)) / SCALING_FACTOR);

	AVS_TRACE(("margins: slope=%dmV/V intercept=%dmV\n", TO_MV(avs->voltage_margin.slope), TO_MV(avs->voltage_margin.intercept)));
	...
	avs->local_fit_dac_offset = (dac_code_t)((MV_TO_INT(avs->local_fit_v_offset) * 1000 /* mV */) / avs->dac_min_vstep);
	ASSERT(avs->local_fit_dac_offset > 0);

	AVS_TRACE(("local fit: dac_offset=%u (%umV)\n", avs->local_fit_dac_offset, avs->local_fit_v_offset));
	...
	for(pass=0; pass<PREDICT_PASSES; pass++) {
		...
		AVS_TRACE(("vconv=%umV margin=%dmV vsum=%umV\n",
		...
		AVS_TRACE(("dac: slope=%dsteps/V intercept=%dsteps exp=%uuV/step real=%uuV/step\n",
		...
		if (pass < PREDICT_PASSES-1) {
			...
			vsum_safe = vsum + MV_TO_INT(GLOBAL_FIT_SAFETY_MARGIN);
			vsum_safe = MIN(vsum_safe, avs->vlimit_high);
			AVS_TRACE(("safety_margin=%umV vsum_safe=%umV\n",
				GLOBAL_FIT_SAFETY_MARGIN, TO_MV(vsum_safe)));
...
BCMATTACHFN(avs_attach)(osl_t *osh, si_t *sih, void *context, char *vars)
{
	...
	switch(CHIPID(avs->sih->chip)) {
		CASE_BCM6715_CHIP:
			avs->vmargin_low	= MV_TO_INT(DEFAULT_VMARGIN_L_6715);
			avs->vmargin_high	= MV_TO_INT(DEFAULT_VMARGIN_H_6715);
			avs->vlimit_low		= MV_TO_INT(DEFAULT_VLIMIT_L_6715);
			avs->vlimit_high	= MV_TO_INT(DEFAULT_VLIMIT_H_6715);
			break;
		CASE_BCM6710_CHIP:
			avs->vmargin_low	= MV_TO_INT(DEFAULT_VMARGIN_L_6710);
			avs->vmargin_high	= MV_TO_INT(DEFAULT_VMARGIN_H_6710);
			avs->vlimit_low		= MV_TO_INT(DEFAULT_VLIMIT_L_6710);
			avs->vlimit_high	= MV_TO_INT(DEFAULT_VLIMIT_H_6710);
			break;
		case BCM43684_CHIP_ID:
		default:
			avs->vmargin_low	= MV_TO_INT(DEFAULT_VMARGIN_L_43684);
			avs->vmargin_high	= MV_TO_INT(DEFAULT_VMARGIN_H_43684);
			avs->vlimit_low		= MV_TO_INT(DEFAULT_VLIMIT_L_43684);
			avs->vlimit_high	= MV_TO_INT(DEFAULT_VLIMIT_H_43684);
			break;
	}
	...
	avs->dac_min_vstep	= (uint)get_config(vars, rstr_avsvstep,   0, FLOAT_TO_INT(DEFAULT_DAC_VOLTAGE_STEP_RATIO_2));
	...
	avs->threshold_margin	= (uint)get_config(vars, rstr_avsthrm,    0, MV_TO_INT(DEFAULT_THRESHOLD_MARGIN));
	avs->voltage_nominal	= NOMINAL_VOLTAGE(avs->sih);
	avs->local_fit_v_offset	= LOCAL_FIT_DAC_OFFSET;
	avs->flags              = (uint)get_config(vars, rstr_avsflags,   0, AVS_FLAGS_DEFAULT);
	parse_avs_flags(avs);
 ...
 * AVS tracking switch.
 *
 * Turn on/off AVS tracking.
 *
 * @param avs		AVS context.
 * @param tracking	tracking on/off
 * @return		BCME_OK on success.
 */

int
avs_tracking_switch(avs_context_t *avs, bool tracking)
{
	if ((tracking != avs->tracking) && (tracking == FALSE))
		avs->initialized = FALSE;

	avs->tracking = tracking;
	return BCME_OK;
}
...
avs_get_value(avs_context_t *avs, avs_value_index_t index, uint *value)
{
	...
	*value = 0;
	...
	if (index == AVS_INDEX_TRACKING) {
		*value = avs->tracking;
		return BCME_OK;
	}

	if (index == AVS_INDEX_FLAGS) {
		*value = avs->flags;
		return BCME_OK;
	}
...
avs_set_vlimit_low(avs_context_t *avs, uint value)
{
	...
	avs->vlimit_low = MV_TO_INT(value);
	AVS_TRACE(("vlimit_low=%umV\n", TO_MV(avs->vlimit_low)));
	return BCME_OK;
}
...
avs_set_vlimit_high(avs_context_t *avs, uint value)
{
	...
	avs->vlimit_high = MV_TO_INT(value);
	AVS_TRACE(("vlimit_high=%umV\n", TO_MV(avs->vlimit_high)));
	return BCME_OK;
}
...
avs_set_vmargin_low(avs_context_t *avs, uint value)
{
	...
	avs->vmargin_low = MV_TO_INT(value);
	AVS_TRACE(("vmargin_low=%umV\n", TO_MV(avs->vmargin_low)));
	return BCME_OK;
}
...
avs_set_vmargin_high(avs_context_t *avs, uint value)
{
	...
	avs->vmargin_high = MV_TO_INT(value);
	AVS_TRACE(("vmargin_high=%umV\n", TO_MV(avs->vmargin_high)));
	return BCME_OK;
}
...
avs_set_threshold_margin(avs_context_t *avs, uint value)
{
	...
	avs->threshold_margin = MV_TO_INT(value);
	AVS_TRACE(("threshold_margin=%umV\n", TO_MV(avs->threshold_margin)));
	return BCME_OK;
}

int
avs_set_flags(avs_context_t *avs, uint value)
{
	if (avs->initialized)
		return BCME_BUSY;
...
int
avs_set_dac(avs_context_t *avs, uint value)
{
	avs->dac_code_setting = value;
	AVS_TRACE(("dac=%u\n", value));
	return BCME_OK;
}

int
avs_set_vnominal(avs_context_t *avs, uint value)
{
	if (avs->initialized)
		return BCME_BUSY;

	avs->voltage_nominal = value;
	AVS_TRACE(("vnominal=%umV\n", TO_MV(avs->voltage_nominal)));
	return BCME_OK;
}

int
avs_set_vlocalfit(avs_context_t *avs, uint value)
{
	if (avs->initialized)
		return BCME_BUSY;

	avs->local_fit_v_offset = value;
	AVS_TRACE(("vlocalfit=%umV\n", TO_MV(avs->local_fit_v_offset)));
	return BCME_OK;
}

int
avs_dump_osccnt(avs_context_t *avs)
{
	int i, j, osc_index = 0;
	osc_count_t count[NUMBER_OF_SENSORS_MAX];

	if (avs->initialized == FALSE) {
		AVS_ERROR(("AVS not initialized\n"));
		return BCME_NOTREADY;
	}

	for (i=0; i<NUMBER_OF_CENTRALS(avs->sih); i++) {
		count[0] = read_central_rosc_count(avs, i);
		AVS_PRINT(("C OSC%02u count=%05u(%05ukHz)%s\n",
			osc_index, count[0],
			TO_KHZ(rosc_count_to_freq(avs, count[0])),
			count == INVALID_OSCILLATOR_COUNT ? " INVALID" : ""));
		osc_index++;
	}
	...
	for (i=0; i<NUMBER_OF_REMOTES(avs->sih); i++) {
		if (BCM43684_CHIP(avs->sih->chip) || BCM6710_CHIP(avs->sih->chip)) {
			read_remote_rosc_count(avs, i, &count[0], &count[1]);
			for (j=0; j<NUMBER_OF_SENSORS_28NM; j++) {
				AVS_PRINT(("R OSC%02u count=%05u(%05ukHz)%s\n",
					osc_index - NUMBER_OF_CENTRALS(avs->sih), count[j],
					TO_KHZ(rosc_count_to_freq(avs, count[j])),
					count == INVALID_OSCILLATOR_COUNT ? " INVALID" : ""));
				osc_index++;
			}
		} else if (BCM6715_CHIP(avs->sih->chip)) {
			read_remote_rosc_count_16nm(avs, i, count);
			for (j=0; j<NUMBER_OF_SENSORS_16NM; j++) {
				AVS_PRINT(("R OSC%02u count=%05u(%05ukHz)%s\n",
					osc_index - NUMBER_OF_CENTRALS(avs->sih), count[j],
					TO_KHZ(rosc_count_to_freq(avs, count[j])),
					count == INVALID_OSCILLATOR_COUNT ? " INVALID" : ""));
...
int
avs_set_trace(avs_context_t *avs, uint value)
{
	avs->trace = value;
	AVS_TRACE(("trace %s\n", value? "on" : "off"));
	return BCME_OK;
...
avs_status(avs_context_t *avs, struct bcmstrbuf *buf)
{
	...
	if (avs->initialized) {
		...
		bcm_bprintf(buf, fmt, "ARMtemp");
		if (BCM43684_CHIP(avs->sih->chip))
			bcm_bprintf(buf, "%dmdC\n", TO_MDC(avs->last_arm_temp));
		else
			bcm_bprintf(buf, "N/A\n");
		...
		bcm_bprintf(buf, fmt, "dac_code");		bcm_bprintf(buf, "%u (pred:%umV)\n", avs->dac_code, TO_MV(dac_code_to_voltage(avs->voltage_daccode, avs->dac_code)));
		...
		bcm_bprintf(buf, fmt, "voltage_daccode_eq");	bcm_bprintf(buf, "slope=%dmsteps/V int=%dsteps\n", avs->voltage_daccode.slope, avs->voltage_daccode.intercept);
		...
		bcm_bprintf(buf, fmt, "Vmargin");		bcm_bprintf(buf, "%dmV\n", TO_MV(avs->vmargin));
