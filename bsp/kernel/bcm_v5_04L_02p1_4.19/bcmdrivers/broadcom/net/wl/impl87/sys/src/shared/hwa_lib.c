...
BCMATTACHFN(hwa_dma_attach)(hwa_dev_t *dev)
{
	...
	HWA_PRINT("%s HWA[%d] channels[%u:%u] burstlen[%u:%u:%u:%u:%u] outst_rds[%u:%u]\n",
...
hwa_dma_rx_burstlen_adjust(hwa_dma_t *dma)
{
	...
	*/
	...
	HWA_PRINT("%s adjust channels[1] rx burstlen[%u]\n", HWAde, rx_bl);
	...
	if (HWAREV_GE(dev->corerev, 131)) {
		...
		HWA_PRINT("%s adjust channels[0] rx burstlen[%u]\n", HWAde, rx_bl);
	}
	...
	HWA_PKTPGR_EXPR({
		...
		fastdma = &dev->fastdma;
		// HWA_FASTDMA_CHANNEL0/1
		rx_bl = fastdma->burst_length[HWA_FASTDMA_CHANNEL0].rx;
		rx_bl = MIN(rx_bl, DMA_BL_128);
		fastdma->burst_length[HWA_FASTDMA_CHANNEL0].rx = rx_bl;
		HWA_UPD_REG_ADDR(HWA00, &regs->common.dmarx0ctrl, rx_bl,
			HWA_COMMON_DMARX0CTRL_DMARX0BL);
		fastdma->burst_length[HWA_FASTDMA_CHANNEL1].rx = rx_bl;
		HWA_UPD_REG_ADDR(HWA00, &regs->common.dmarx1ctrl, rx_bl,
			HWA_COMMON_DMARX1CTRL_DMARX1BL);
		HWA_PRINT("%s adjust fastdma channels[0/1] rx burstlen[%u]\n", HWAde, rx_bl);

		// HWA_FASTDMA_CHANNEL2 is for HWA internal to sysmem,
		// keep original 256 bytes setting.
	});
...
/* Although HWA can use fastdma in PktPgr off mode but it still have 8 bytes
 * alignment limitation.  We never verify it in PktPgr off mode yet.
 * So restrict fastdma only for PktPgr mode.
*/
void // HWA Internal Fast DMA Engines
BCMATTACHFN(hwa_fastdma_attach)(hwa_dev_t *dev)
{
	uint32 i, v32, txbl, rxbl;
	hwa_regs_t *regs;
	hwa_fastdma_t *fastdma;
	...
	// Default setting CH-0/1
	fastdma->channels_max = HWA_FASTDMA_CHANNEL2;
	fastdma->burst_length_max = DMA_BL_512;
	for (i = 0; i < HWA_FASTDMA_CHANNELS_MAX; i++) {
		fastdma->burst_length[i].tx = DMA_BL_256;
		fastdma->burst_length[i].rx = DMA_BL_256;
	}
	if (HWAREV_GE(dev->corerev, 133)) {
		// Has three channels
		fastdma->channels_max = HWA_FASTDMA_CHANNELS_MAX;

		// Has arbitration optino: 0:priority; 1:round ronbin.
		v32 = HWA_RD_REG_NAME(HWAde, regs, common, dmaarbcfg);
		fastdma->arbitration.tx = BCM_GBF(v32, HWA_COMMON_DMAARBCFG_TXDMAARBCFG);
		fastdma->arbitration.rx = BCM_GBF(v32, HWA_COMMON_DMAARBCFG_RXDMAARBCFG);
#if defined(HWA_QT_TEST) && FALSE
		// RR
		fastdma->arbitration.tx = 1;
		fastdma->arbitration.rx = 1;
#endif
		HWA_PRINT("%s HWA[%d] FASTDMA arbitration<T:%s / R:%s>\n",
			HWA00, dev->unit, (fastdma->arbitration.tx == 1) ? "RR" : "FXD",
			(fastdma->arbitration.rx == 1) ? "RR" : "FXD");
	}

	// Read PCIe configuration
	txbl = pcie_h2ddma_tx_get_burstlen(dev->pciedev);
	rxbl = pcie_h2ddma_rx_get_burstlen(dev->pciedev);
	txbl = MIN(txbl, fastdma->burst_length_max);
	rxbl = MIN(rxbl, fastdma->burst_length_max);
	for (i = 0; i < HWA_FASTDMA_CHANNELS_MAX; i++) {
		fastdma->burst_length[i].tx = txbl;
		fastdma->burst_length[i].rx = rxbl;
	}
	if (HWAREV_GE(dev->corerev, 133)) {
		// RX-CH2: Only for HWA internal to sysmem, fixed length 256 bytes.
		fastdma->burst_length[HWA_FASTDMA_CHANNEL2].rx = DMA_BL_256;
	}

	// Print it out
	HWA_PRINT("%s HWA[%d] fastdma channels[%u] burstlen[%u:%u:%u:%u",
		HWA00, dev->unit, fastdma->channels_max,
	...
	if (HWAREV_GE(dev->corerev, 133)) {
		HWA_PRINT(":%u:%u", fastdma->burst_length[HWA_FASTDMA_CHANNEL2].tx,
	...
	// CH-0
	v32 = BCM_SBF(fastdma->burst_length[HWA_FASTDMA_CHANNEL0].tx,
		HWA_COMMON_DMATX0CTRL_DMATX0BL);
	HWA_UPD_REG_ADDR(HWA00, &regs->common.dmatx0ctrl, v32,
		HWA_COMMON_DMATX0CTRL_DMATX0BL);
	v32 = BCM_SBF(fastdma->burst_length[HWA_FASTDMA_CHANNEL0].rx,
		HWA_COMMON_DMARX0CTRL_DMARX0BL);
	HWA_UPD_REG_ADDR(HWA00, &regs->common.dmarx0ctrl, v32,
		HWA_COMMON_DMARX0CTRL_DMARX0BL);
	// CH-1
	v32 = BCM_SBF(fastdma->burst_length[HWA_FASTDMA_CHANNEL1].tx,
		HWA_COMMON_DMATX1CTRL_DMATX1BL);
	HWA_UPD_REG_ADDR(HWA00, &regs->common.dmatx1ctrl, v32,
		HWA_COMMON_DMATX1CTRL_DMATX1BL);
	v32 = BCM_SBF(fastdma->burst_length[HWA_FASTDMA_CHANNEL1].rx,
		HWA_COMMON_DMARX1CTRL_DMARX1BL);
	HWA_UPD_REG_ADDR(HWA00, &regs->common.dmarx1ctrl, v32,
		HWA_COMMON_DMARX1CTRL_DMARX1BL);
	if (HWAREV_GE(dev->corerev, 133)) {
		// CH-2
		v32 = BCM_SBF(fastdma->burst_length[HWA_FASTDMA_CHANNEL2].tx,
			HWA_COMMON_DMATX2CTRL_DMATX2BL);
		HWA_UPD_REG_ADDR(HWA00, &regs->common.dmatx2ctrl, v32,
			HWA_COMMON_DMATX2CTRL_DMATX2BL);
		v32 = BCM_SBF(fastdma->burst_length[HWA_FASTDMA_CHANNEL2].rx,
			HWA_COMMON_DMARX2CTRL_DMARX2BL);
		HWA_UPD_REG_ADDR(HWA00, &regs->common.dmarx2ctrl, v32,
			HWA_COMMON_DMARX2CTRL_DMARX2BL);

		// Config arbitration options.
		v32 = 0U
			| BCM_SBF(fastdma->arbitration.tx, HWA_COMMON_DMAARBCFG_TXDMAARBCFG)
			| BCM_SBF(fastdma->arbitration.rx, HWA_COMMON_DMAARBCFG_RXDMAARBCFG)
			| 0U;
		HWA_WR_REG_NAME(HWA00, regs, common, dmaarbcfg, v32);
	}
	...
	if (dev->fastdma_en) {
		...
		if (HWAREV_LE(dev->corerev, 132)) {
			// XXX, 6715Ax-PP uses partial fastdma to WAR BCAFWUCODE-6451
			// bit0 and bit1.
			v32 = 0U
			...
			// bit3 to bit9
			// XXX, remove bit3, 5, 7, 9 for freepkt ring stuck debugging.
			// XXX, To imporve stability on high rssi environment
			// Remove bit3 to bit9
		} else {
			v32 = 0U
			| BCM_SBIT(HWA_COMMON_DMATXSEL_FETCH_SWPKTC_WDMATX)
			...
			| BCM_SBIT(HWA_COMMON_DMATXSEL_FETCH_PAGEINRXPROCESS_WDMATX)
			| 0U;
		}
		...
		if (HWAREV_LE(dev->corerev, 132)) {
			...
			// XXX, remove bit2, 6, 9, 12, 13 for freepkt ring stuck debugging.
			// XXX, To imporve stability on high rssi environment
			// bit2, 14
			v32 |= 0U
			// bit2
			...
			| BCM_SBIT(HWA_COMMON_DMARXSEL_MOVE_D11B_WDMARX)
			| BCM_SBIT(HWA_COMMON_DMARXSEL_MOVE_PAGEINRXPROCESS_WDMARX)
			| 0U;
		}
	...
	// FASTDMA channels enabled by default.
	fastdma->enabled = TRUE;
	...
	volatile void *regs, si_t *sih, uint unit, uint bustype)
{
	...
	hwa_dev_local->fastdma_en = 0;
	// XXX, fastdma has 4K boundary issue in A0 [rev 131]. Don't use all.
	// A0-TX: needs partial fastdma to WAR BCAFWUCODE-6451
	// A0-RX: needs partial fastdma for Rx throughput turning.
	HWA_PKTPGR_EXPR(hwa_dev_local->fastdma_en = 1);  // force enable for PP
	...
	HWA_PKTPGR_EXPR({
		if (HWAREV_GE(hwa_dev_local->corerev, 133)) {
			// B0: recycle dirty part by HW.
			hwa_dev_local->d11b_recycle_pagein = TRUE;
			// rxpsrc_ring_hwa2cfg::pp_alloc_freerph is for SW issued alloc_rph
			// request and may be fulfilled by taking the rph from free_rph request.
			// It doesn't help d11b recycle, so B0 still needs d11b_recycle_war.
			hwa_dev_local->d11b_recycle_war = TRUE;
		} else {
			// d11b_recycle_war is enabled by default for Ax
			hwa_dev_local->d11b_recycle_war = TRUE;
		}
	});
	...
	HWA_PKTPGR_EXPR(hwa_fastdma_attach(hwa_dev_local)); //HWA fast dma
...
hwa_config(struct hwa_dev *dev)
{
	...
	if (HWA_HME_MACIFS_EN(dev)) {
		...
		if (freesz <= 0) {
			HWA_ERROR(("%s Invalid HME MACIFS free size %d\n", HWA00, freesz));
			HWA_ASSERT(0);
			return;
		}
...
#ifndef PCIE_HBQD /* 16bit dmaindex firmware target build */
...
hwa_rx_enable(hwa_dev_t *dev)
{
	HWA_RXPATH_EXPR({
		...
		// Skip the rx enable if dev is not up
		if (dev->up == FALSE) {
			HWA_TRACE(("%s: dev is not up, skip rx enable\n", __FUNCTION__));
			return;
		}
		...
		HWA_PKTPGR_EXPR({
			...
			// will failed with 0 packet count resp when fastdma is enabled.
	...
	bool verbose, bool dump_regs, bool dump_txfifo_shadow, uint8 *fifo_bitmap)
{
	...
	HWA_BPRINT(b, "+ driver_mode<%s> macif<%s, %s> host<%s, %s, 0x%08x> hwaconf<0x%08x>\n",
		(dev->driver_mode == HWA_FD_MODE) ? "FD" : "NIC",
		(dev->macif_placement == HWA_MACIF_IN_DNGLMEM) ? "DNGL" : "HOST",
		(dev->macif_coherency == HWA_HW_COHERENCY) ? "HW" : "SW",
		(dev->host_coherency == HWA_HW_COHERENCY) ? "HW" : "SW",
		(dev->host_addressing == HWA_64BIT_ADDRESSING) ? "64b" : "32b",
		dev->host_physaddrhi, HWACONF);
...
hwa_regs_dump(hwa_dev_t *dev, struct bcmstrbuf *b, uint32 block_bitmap)
{
	...
	if (block_bitmap & HWA_DUMP_CMN) {
		...
		if (HWAREV_GE(dev->corerev, 133)) {
			HWA_BPR_REG(b, common, dma_dbg_msg_addr);
			HWA_BPR_REG(b, common, dma_dbg_msg_data);
		}
		...
		if (HWAREV_GE(dev->corerev, 133)) {
			HWA_BPR_REG(b, common, dmaarbcfg);
		}
		...
		if (HWAREV_GE(dev->corerev, 133)) {
			HWA_BPR_REG(b, common, dmatx2ctrl);
			HWA_BPR_REG(b, common, dmarx2ctrl);
		}
	}
	...
	if (intstatus & HWA_COMMON_INTSTATUS_PACKET_PAGER_INTR_MASK) {
		...
		if (pageintstatus) {
			...
			if (pageintstatus & HWA_COMMON_PAGEIN_INT_MASK) {
				...
				hwa_pktpgr_response(dev, hwa_pktpgr_pagein_rsp_ring,
			...
			//  HWA_PKTPGR_PAGEMGR_PUSH_MPDU
			//  HWA_PKTPGR_PAGEMGR_PULL_MPDU
			if (pageintstatus & HWA_COMMON_PAGEMGR_INT_MASK) {
				// XXX, PageMgr ErrInts are not handled yet!
				hwa_pktpgr_response(dev, hwa_pktpgr_pagemgr_rsp_ring,
					HWA_PKTPGR_PAGEMGR_CALLBACK);
#ifdef HNDPQP
				// Postpone pqp_pgi_wake() after all responses
				// in hwa_pktpgr_pagemgr_rsp_ring are processed.
				if (dev->pktpgr.pqp_pgi_wake) {
					dev->pktpgr.pqp_pgi_wake = FALSE;
					pqp_pgi_wake();
				}
#endif /* HNDPQP */
			}
			...
			if (pageintstatus & HWA_COMMON_PAGERBM_INT_MASK) {
				HWA_PRINT("%s%s\n", (pageintstatus &
					...
					"HDBM-TH1 " : "", (pageintstatus &
					HWA_COMMON_PAGEINTSTATUS_PPDDBMTH1INT_MASK) ?
					"DDBM-TH1 " : "");
			}

			if (pageintstatus & HWA_COMMON_PAGEERR_INT_MASK) {
				HWA_PRINT("pageintstatus errors 0x%x\n",
					(pageintstatus & HWA_COMMON_PAGEERR_INT_MASK));
			}
		}
