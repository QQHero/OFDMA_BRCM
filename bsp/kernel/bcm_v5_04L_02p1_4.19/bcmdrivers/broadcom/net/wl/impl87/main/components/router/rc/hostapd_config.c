 ...
 *
 * $Id: hostapd_config.c 810873 2022-04-18 13:36:09Z $
 */
...
#define HAPD_R0KH_R1KH_KEY_LEN		16	/* R0KH or R1KH Key length */
...
#define HAPD_CFG_PERRADIO		0x10	// Flag to indicate that nvram is per radio
#define HAPD_CFG_USEDEFAULT		0x20	// Flag to write the default value in config file
#define HAPD_CFG_COMMON			(HAPD_CFG_BSS | HAPD_CFG_IFR)
#define HAPD_CFG_LRG_BUF		0x40	// Flag to allocate large size buffer to write to
						//  conf file
#define WPA_SUP_CFG_GBL			0x80	// Flag to indicate global settings for supplicant
...
#define HAPD_AKM_PSK2_SHA256		0x100

/* Start of Enterprise akm */
#define HAPD_AKM_WPA			0x200
#define HAPD_AKM_WPA2			0x400
#define HAPD_AKM_WPA2_OSEN		0x800
#define HAPD_AKM_WPA3_SUITE_B           0x1000
#define HAPD_AKM_WPA3			0x2000
...
#define HAPD_DISABLE_BCN		"1"	/* start BSS with beaconing disabled by default */
#define HAPD_ENABLE_BCN			"0"
...
#define SPP_AMSDU_DISABLED		0	// SPP AMSDU disabled
#define SPP_AMSDU_CAPABLE		1	// SPP AMSDU capable not required
#define SPP_AMSDU_REQUIRED		2	// SPP AMSDU required, but not capable
#define SPP_AMSDU_CAPABLE_AND_REQUIRED	3	// SPP AMSDU capable and required
...
#define WPAX_FIELDS_NUM			8
#define SAE_FIELDS_NUM			5
#define FBT_FIELDS_NUM			9
#define RADIUS_ACC_FIELDS_NUM		7
#define RADIUS_FIELDS_NUM		(18 + RADIUS_ACC_FIELDS_NUM)
#define PREAUTH_FIELDS_NUM		1
#define WPS_FIELDS_NUM			19
#define MAP_FIELDS_NUM			5
#define WPASUPP_WPS_FIELDS_NUM		10
#define HAPD_DPP_FIELDS_NUM		6
#define WPASUPP_DPP_FIELDS_NUM		5
#define OWE_FIELDS_NUM			2
#ifdef CMWIFI_RDKB
#define BRCM_HOTSPOT_FIELDS_NUM		2
#endif /* CMWIFI_RDKB */
...
static bool hapd_is_cap_set(char *nv_ifname, char *cap_needle);
...
static int hapd_nas_id_conv_fn(char *nvif, char *carr_nv, char **ph, char *out, int sz);
static int hapd_radius_client_addr_conv_fn(char *nvif, char *carr_nv, char **ph, char *out, int sz);
...
static int hapd_acct_server_conv_fn(char *nv_if, char *cfg_arr_nv, char **ph, char *out, int sz);
static int hapd_acct_interim_interval_conv_fn(char *nv_if,
		char *cfg_arr_nv, char **ph, char *out, int sz);
static int hapd_secondary_acct_server_conv_fn(char *nv_if,
		char *cfg_arr_nv, char **ph, char *out, int sz);
static int hapd_radius_das_port_conv_fn(char *nv_if,
		char *cfg_arr_nv, char **ph, char *out, int sz);
static int hapd_radius_das_client_conv_fn(char *nv_if,
		char *cfg_arr_nv, char **ph, char *out, int sz);
...
static int hapd_wps_upnp_conv_fn(char *nvif, char *carr_nv, char **ph, char *out, int sz);
...
static int start_disabled_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph, char *out_val,
		int out_sz);
...
static int hapd_dpp_connector_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph,
		char *out_val, int out_sz);
static int hapd_dpp_controller_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph,
		char *out_val, int out_sz);
...
static int hapd_owe_trans_ifname_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph,
		char *out_val, int out_sz);
static int hapd_spp_amsdu_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph,
		char *out_val, int out_sz);
#ifdef CMWIFI_RDKB
static int hapd_rdk_radius_greylist_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out, int sz);
static int hapd_brcm_hs20_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out, int sz);
static int hapd_hs20_release_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out, int sz);
#endif /* CMWIFI_RDKB */
...
int start_hostapd(char *fltrd_iflist, char *pri_iflist, char *vr_iflist);
...
static hapd_nvram_config_t cfg_arr[] =
{
	...
	{"wpa_pairwise_retries", "wpa_pairwise_update_count", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"4", NULL},
	{"wpa_pmk_lifetime", "dot11RSNAConfigPMKLifetime", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "", NULL},
	...
	{"r0kh_id", "nas_identifier", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), NULL, hapd_nas_id_conv_fn},
	{"", "radius_client_addr", (HAPD_CFG_COMMON), "127.0.0.1", hapd_radius_client_addr_conv_fn},
	...
	{"radius2_key", "auth_server_shared_secret", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"12345678", NULL},
	{"radius_acct_ipaddr", "acct_server_addr", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0.0.0.0", hapd_acct_server_conv_fn},
	{"radius_acct_port", "acct_server_port", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"1813", NULL},
	{"radius_acct_key", "acct_server_shared_secret", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"12345678", NULL},
	{"radius_acct_interim_interval", "radius_acct_interim_interval", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"600", hapd_acct_interim_interval_conv_fn},
	{"radius_acct2_ipaddr", "acct_server_addr", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0.0.0.0", hapd_secondary_acct_server_conv_fn},
	{"radius_acct2_port", "acct_server_port", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"1813", NULL},
	{"radius_acct2_key", "acct_server_shared_secret", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"12345678", NULL},
	{"radius_das_port", "radius_das_port", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "",
		hapd_radius_das_port_conv_fn},
	{"", "radius_das_client", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "",
		hapd_radius_das_client_conv_fn},
	{"radius_reauth_interval", "eap_reauth_period", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"", NULL},
	{"radius_operator_name", "radius_auth_req_attr", (HAPD_CFG_COMMON | HAPD_CFG_PERRADIO), "", NULL},
	{"radius_location_data", "radius_auth_req_attr", (HAPD_CFG_COMMON | HAPD_CFG_PERRADIO), "", NULL},
	...
	{"wps_upnp_support", "upnp_iface", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "",
		hapd_wps_upnp_conv_fn},
	...
	{"bss_bcn_disabled", "start_disabled", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "1",
		start_disabled_conv_fn},
	...
	{"dpp_pfs", "dpp_pfs", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "0", NULL},
	{"dpp_mud_url", "dpp_mud_url", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"https://example.com/mud", NULL},
	{"dpp_configurator_connectivity", "dpp_configurator_connectivity",
		(HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "0", NULL},
	{"", "dpp_controller", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "",
		hapd_dpp_controller_conv_fn},
	...
	{"beacon_prot", "beacon_prot", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0", NULL},
	{"spp_amsdu", "spp_amsdu", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0", hapd_spp_amsdu_conv_fn},
	{"transition_disable", "transition_disable", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0", NULL},
	{"ft_rrb_lo_sock", "ft_rrb_lo_sock", (HAPD_CFG_IFR | HAPD_CFG_PERBSS), "1", NULL},
#ifdef CMWIFI_RDKB
	{"rdk_radius_greylist", "rdk_greylist", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0", hapd_rdk_radius_greylist_conv_fn},
	{"vlan_id", "ap_vlan", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0", NULL},
	/**
	 * Settings for hs20
	 *
	 * We don't set hostapd's hs20 flag, because our proprietary hotspot app
	 * is used, but still we need to tell hostapd hotspot is enabled for some
	 * hotspot features, e.g., add some hotspot AVPs in RADIUS Access-Request.
	 * So we introduce a new hostapd configuration item 'brcm_hs20', it is set
	 * when nvram variable hspot is set and our proprietary hotspot app is used.
	 *
	 * TODO: brcm_hs20 is to be removed when we set hostapd's hs20 flag to 1,
	 * i.e., we start to fully use hostapd's hs20 feature.
	 */
	{"", "brcm_hs20", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0", hapd_brcm_hs20_conv_fn},
	{"hs2cap", "hs20_release", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS),
		"0", hapd_hs20_release_conv_fn},
#endif /* CMWIFI_RDKB */
	{0, 0, 0, 0, 0}
};
...
static hapd_nvram_config_t wpa_supp_nw_cfg_arr[] =
{
	...
	{"dpp_csign", "dpp_csign", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "", NULL},
	{"dpp_netaccess_key", "dpp_netaccesskey", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "", NULL},
	{"dpp_pp_key", "dpp_pp_key", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "", NULL},
	{"dpp_pfs", "dpp_pfs", (HAPD_CFG_COMMON | HAPD_CFG_PERBSS), "0", NULL},
	{0, 0, 0, 0, 0}
};
#ifdef HAPD_WDS_DRAFT
...
/* helps check if the string returned by 'wl cap' contains the token cap_needle */
static bool
hapd_is_cap_set(char *nv_ifname, char *cap_needle)
{
	char os_ifname[IFNAMSIZ] = {0};
	char caps[WLC_IOCTL_MEDLEN], cap[80], *next;
	...
	if (wl_iovar_get(os_ifname, "cap", (void *)caps, sizeof(caps))) {
		return FALSE;
	}

	foreach(cap, caps, next) {
		if (!strcmp(cap, cap_needle)) {
			return TRUE;
		}
	}
...
{
	wlc_rev_info_t rev;
	char *ifname, tmp[HAPD_MIN_BUF] = {0};

	ifname = nvram_safe_get(strcat_r(nv_ifname, "_ifname", tmp));
...
hapd_wpasupp_get_security_details(char *nv_ifname, int *out_akm)
{
	...
	if (!strcmp(nvram_safe_get(nv_name), "enabled")) {
		...
		foreach(tmp, akm_val, next) {
			...
			if (!strcmp(tmp, "saeft") && hapd_wpasupp_is_ftsae_supported(nv_ifname)) {
	...
	if (akm & HAPD_AKM_WPA3_SAE) {
		if (akm & HAPD_AKM_PSK2) {
			if ((akm & HAPD_AKM_PSK2_FT) && fbt_enabled) {
				if (mfp_required) {
					snprintf(key_mgmt, HAPD_MAX_BUF, "%s",
						HAPD_KEY_MGMT_WPA_FT_SHA256_SAE);
				} else {
					snprintf(key_mgmt, HAPD_MAX_BUF, "%s",
						HAPD_KEY_MGMT_WPA_FT_PSK_SAE);
				}
			} else if (akm & HAPD_AKM_WPA3_DPP) {
				if (mfp_required) {
					snprintf(key_mgmt, HAPD_MAX_BUF, "%s",
						HAPD_KEY_MGMT_WPA_SHA256_SAE_DPP);
				} else {
					snprintf(key_mgmt, HAPD_MAX_BUF, "%s",
						HAPD_KEY_MGMT_WPA_PSK_SAE_DPP);
				}
			} else {
				if (mfp_required) {
					snprintf(key_mgmt, HAPD_MAX_BUF, "%s",
						HAPD_KEY_MGMT_WPA_SHA256_SAE);
				} else {
					snprintf(key_mgmt, HAPD_MAX_BUF, "%s",
						HAPD_KEY_MGMT_WPA_PSK_SAE);
				}
			}
		} else {
			if (akm & HAPD_AKM_WPA3_DPP) {
				snprintf(key_mgmt, HAPD_MAX_BUF, "%s",
					HAPD_KEY_MGMT_SAE_DPP);
			} else {
				snprintf(key_mgmt, HAPD_MAX_BUF, "%s",
					HAPD_KEY_MGMT_SAE);
			}
...
hapd_key_mgmt_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph, char *out_val, int out_sz)
{
	...
	int fbt_enabled = 0;
	int wps_add_sae = 0;
	int flags = WPA_PSK_FLAG_IS_HAPD;
	wlc_rev_info_t rev;
	...
	psk_required = ((akm & HAPD_AKM_PSK2) || (akm & HAPD_AKM_PSK) ||
		(akm & HAPD_AKM_PSK2_SHA256) || (akm & HAPD_AKM_WPA3_SAE) ||
		(akm & HAPD_AKM_WPA3_SAE_FT)) ? TRUE : FALSE;
	...
	switch (akm) {
...
#ifdef CMWIFI_RDKB
					/* Per radius greylist access control feature request,
					 * hostapd has to send access-request with customer
					 * specific AVPs to the radius server, so skip RADIUS
					 * fields only when rdk_greylist is not enabled.
					 *
					 * There are changes in hostapd as well. */
					snprintf(nv_name, sizeof(nv_name), "%s_%s",
						nv_ifname, "rdk_radius_greylist");
					if (!nvram_match(nv_name, "1"))
#endif /* CMWIFI_RDKB */
						ret += RADIUS_FIELDS_NUM;
				}
		...
		case HAPD_AKM_PSK:
			if (!strcmp(cfg_arr_nvname, "wpa")) {
				...
				hapd_wpasupp_psk_key_hndlr(nv_name, ph, out_val, out_sz, flags);
				// skip SAE fields
				ret = SAE_FIELDS_NUM;
			}
			break;
			...
			/* WPA2PSK | WPA2PSK_FT */
		case HAPD_AKM_PSK2:
		case HAPD_AKM_PSK2_SHA256:
		case (HAPD_AKM_PSK2 | HAPD_AKM_PSK2_SHA256):
		case (HAPD_AKM_PSK2 | HAPD_AKM_PSK2_FT):
		case (HAPD_AKM_PSK2 | HAPD_AKM_PSK2_FT | HAPD_AKM_PSK2_SHA256):
			if (!strcmp(cfg_arr_nvname, "wpa")) {
#ifdef CMWIFI_RDKB
		...
		case HAPD_AKM_WPA2:
		case HAPD_AKM_WPA3:
		case (HAPD_AKM_WPA2 | HAPD_AKM_WPA3):
			if (!strcmp(cfg_arr_nvname, "wpa")) {
				snprintf(out_val, out_sz, "%d", HAPD_SEC_WPA2);
			} else if (!strcmp(cfg_arr_nvname, "crypto")) {
				// Set wpa_pairwise based on crypto
		...
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_WPA3_DPP):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_WPA3_SAE_FT):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2 | HAPD_AKM_WPA3_DPP):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2 | HAPD_AKM_PSK2_SHA256):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2 | HAPD_AKM_PSK2_SHA256 | HAPD_AKM_WPA3_DPP):
		...
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2 | HAPD_AKM_PSK2_FT |
			HAPD_AKM_PSK2_SHA256):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2 | HAPD_AKM_PSK2_FT |
			HAPD_AKM_WPA3_SAE_FT):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2 | HAPD_AKM_PSK2_FT |
			HAPD_AKM_WPA3_SAE_FT  | HAPD_AKM_PSK2_SHA256):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2_SHA256):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_WPA3_SAE_FT | HAPD_AKM_PSK2_SHA256):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2_FT | HAPD_AKM_PSK2_SHA256):
		case (HAPD_AKM_WPA3_SAE | HAPD_AKM_PSK2_FT |
			HAPD_AKM_WPA3_SAE_FT  | HAPD_AKM_PSK2_SHA256):
			if (!strcmp(cfg_arr_nvname, "wpa")) {
				snprintf(out_val, out_sz, "%d", HAPD_SEC_WPA2);
			} else if (!strcmp(cfg_arr_nvname, "crypto")) {
...
static int
hapd_nas_id_conv_fn(char *nvif, char *carr_nv,
		char **ph, char *out, int sz)
{
#ifdef CMWIFI_RDKB
	FILE *fp;
	const char *cmd = "/usr/sbin/deviceinfo.sh -cmac";
	char nv_name[HAPD_MAX_BUF] = {0}, *ptr;
	int ret = 0;
	const int txt_mac_addr_len = 18;

	if (sz < txt_mac_addr_len) {
		cprintf("%s: output buf sz is too small\n", __func__);
		return -1;
	}

	snprintf(nv_name, sizeof(nv_name), "%s_%s", nvif, carr_nv);
	ptr = nvram_safe_get(nv_name);
	/*
	 * If nvram 'r0kh_id' is defined for fbt, don't overwrite
	 * nas_identifier specified by 'r0kh_id'
	 */
	if (*ptr)
		return 0;
	...
	if (fp == NULL) {
		cprintf("%s: popen failed\n", __func__);
		return -1;
	}

	if (!fgets(out, txt_mac_addr_len, fp)) {
		ret = -1;
		cprintf("%s: fgets failed\n", __func__);
	}
...
static int
hapd_radius_client_addr_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph, char *out_val,
		int out_sz)
{
	char *radius_client_ifname;
	int fd;
	struct ifreq ifr;

	radius_client_ifname = nvram_get("radius_client_ifname");
	if (!radius_client_ifname || *radius_client_ifname == '\0') {
		return 0;
	} else {
		fd = socket(AF_INET, SOCK_DGRAM, 0);
		if (fd == -1) {
			cprintf("%s:%d fail to get ip addr of %s\n",
				__FUNCTION__, __LINE__, radius_client_ifname);
			return -1;
		}
		ifr.ifr_addr.sa_family = AF_INET;
		snprintf(ifr.ifr_name, sizeof(ifr.ifr_name), "%s", radius_client_ifname);
		if (ioctl(fd, SIOCGIFADDR, &ifr) == -1) {
			close(fd);
			cprintf("%s:%d fail to get ip addr of %s\n",
				__FUNCTION__, __LINE__, radius_client_ifname);
		...
		char **ph, char *out_val, int out_sz)
{
	...
	if (ptr != NULL && (atoi(ptr) == 1)) {
		get_all_lanifname(out_val, out_sz);
	} else {
		snprintf(out_val, out_sz, "%s", "");
	}
		...
		char **ph, char *out_val, int out_sz)
{
	...
	if (nvram_match(nv_name, "wpa2") ||
		...
		nvram_match(nv_name, "wpa3")||		/* trailing space */
		nvram_match(nv_name, "wpa3 ")||		/* trailing space */
		nvram_match(nv_name, "wpa2 wpa3") ||
		nvram_match(nv_name, "wpa3 wpa2") ||
...
static int
hapd_acct_server_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};
	char *ptr = NULL;

	snprintf(nv_name, sizeof(nv_name), "%s_radius_acct_enabled", nv_ifname);
	if (!nvram_match(nv_name, "1")) {
		return RADIUS_ACC_FIELDS_NUM - 1;
	}
...
static int
hapd_acct_interim_interval_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};
	char *ptr = NULL;
...
static int
hapd_secondary_acct_server_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};
	char *ptr = NULL;
	...
	ptr = nvram_get(nv_name);
	if (!ptr) {
		/* skip rest of secondary accounting server settings */
		return 2;
	} else {
		snprintf(out_val, out_sz, "%s", ptr);
	}
...
static int
hapd_radius_das_port_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};
	char *ptr = NULL;
...
static int
hapd_radius_das_client_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};
	char *ptr = NULL;
	...
	ptr = nvram_safe_get(nv_name);
	if (ptr[0] != '\0') {
		size = snprintf(out_val, out_sz, "%s", ptr);
	} else {
		return -1;
	}
	...
	/* RDKB configures DAS on its GREYLIST enabled SSID,
	 * key can be null for open SSID,
	 * DAS client may disconnect the WiFi client via DISCONNECT DAS message. */
	if (ptr[0] != '\0') {
		snprintf(out_val + size, out_sz - size, " %s", ptr);
	}
#ifndef CMWIFI_RDKB
	else {
		return -1;
	}
#endif /* #ifndef CMWIFI_RDKB */
		...
		char **ph, char *out_val, int out_sz)
{
	...
	ifnames_listsz = get_all_lanifnames_listsz();
	if (ifnames_listsz == 0) {
	...
	get_all_lanifnames_list(ifnames_list, ifnames_listsz);
	...
	case (WLC_BAND_2G | WLC_BAND_5G):	// Dual band case
...
/* Callback fn to fill upnp_iface field required for ER(external registrar) functionality */
static int
hapd_wps_upnp_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph, char *out_val, int out_sz)
{
	char *ptr = nvram_get(cfg_arr_nvname);

	if (ptr && !atoi(ptr)) {
		/* Skip friendly_name attribute which is required for only ER functionality */
		return 1;
	}

	return hapd_br_conv_fn(nv_ifname, cfg_arr_nvname, ph, out_val, out_sz);
}
...
wpa_supp_dpp_connector_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **ph, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0}, *akms;
	...
	if (!strstr(akms, "dpp")) {
		return WPASUPP_DPP_FIELDS_NUM - 1;
	}
...
hapd_mfp_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph, char *out_val, int out_sz)
{
	...
	if (akm & (HAPD_AKM_WPA3_SAE | HAPD_AKM_WPA3_SAE_FT | HAPD_AKM_WPA3_DPP |
		...
		if ((akm & (HAPD_AKM_PSK2 | HAPD_AKM_WPA2)) && !nvram_match(nv_name, "2")) {
...
static int
start_disabled_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};
	...
	if (nvram_match(nv_name, "1")) {
		/* beaconing disabled */
		snprintf(out_val, out_sz, "%s", HAPD_DISABLE_BCN);

	} else {
		snprintf(out_val, out_sz, "%s", HAPD_ENABLE_BCN);
	}
	return 0;
}
...
/* Callback fn to fill dpp specific parameters */
static int
hapd_dpp_controller_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph,
		char *out_val, int out_sz)
{
	char nv_name_ip_addr[HAPD_MAX_BUF] = {0};
	char nv_name_pkhash[HAPD_MAX_BUF] = {0};
	char nv_name_dpp_role[HAPD_MAX_BUF] = {0};
	char *ptr1 = NULL, *ptr2 = NULL;

	snprintf(nv_name_dpp_role, sizeof(nv_name_dpp_role), "%s_dpp_role", nv_ifname);

	/* If role is not dpp_relay, skip dpp_controller field. */
	if (!nvram_match(nv_name_dpp_role, "dpp_relay")) {
		return 0;
	}

	/* build interface/bss specific nvram - wlX_nvram or wlX.Y_nvram */
	snprintf(nv_name_ip_addr, sizeof(nv_name_ip_addr), "%s_ctrl_ipaddr", nv_ifname);
	snprintf(nv_name_pkhash, sizeof(nv_name_pkhash), "%s_ctrl_pkhash", nv_ifname);

	ptr1 = nvram_safe_get(nv_name_ip_addr);
	ptr2 = nvram_safe_get(nv_name_pkhash);

	if (ptr1[0] != 0 && ptr2[0] != 0) {
		snprintf(out_val, out_sz, "ipaddr=%s pkhash=%s ", ptr1, ptr2);
	}
...
static int
hapd_spp_amsdu_conv_fn(char *nv_ifname, char *cfg_arr_nvname, char **ph,
	char *out_val, int out_sz)
{
	wlc_rev_info_t rev;
	char nv_name[HAPD_MAX_BUF] = {0};

	if (!hapd_is_cap_set(nv_ifname, "spp")) {
		cprintf("Err: rc: %d: SPP AMSDU not supported in cap \n", __LINE__);
		return -1;
	}
	...
	if (rev.corerev < 129) {
		cprintf("Err: rc: %d: SPP AMSDU not supported by corerev %d"
	...
	if (nvram_match(nv_name, "0")) {
		snprintf(out_val, out_sz, "%d", SPP_AMSDU_DISABLED);
	} else if (nvram_match(nv_name, "1")) {
		snprintf(out_val, out_sz, "%d", SPP_AMSDU_CAPABLE);
	} else if (nvram_match(nv_name, "2")) {
		snprintf(out_val, out_sz, "%d", SPP_AMSDU_REQUIRED);
	} else if (nvram_match(nv_name, "3")) {
		snprintf(out_val, out_sz, "%d", SPP_AMSDU_CAPABLE_AND_REQUIRED);
	} else {
		/* Default disabled */
		snprintf(out_val, out_sz, "%d", SPP_AMSDU_DISABLED);
	}
...
#ifdef CMWIFI_RDKB
/* Callback fn to fill rdk_greylist parameters */
static int
hapd_rdk_radius_greylist_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **placeholder, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};
...
/* Callback fn to fill brcm_hs20 parameters */
static int
hapd_brcm_hs20_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **placeholder, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};

	snprintf(nv_name, sizeof(nv_name), "%s_hspot", nv_ifname);
	if (!nvram_match(nv_name, "1"))
		return BRCM_HOTSPOT_FIELDS_NUM - 1; //skip hs20_release

	snprintf(nv_name, sizeof(nv_name), "%s_%s", nv_ifname, "hsflag");
	if (!nvram_match_bitflag(nv_name, 0, "1")) //#define HSFLG_HS_EN 0
		return BRCM_HOTSPOT_FIELDS_NUM - 1; //skip hs20_release
...
/* Callback fn to fill hs20_release parameters */
static int
hapd_hs20_release_conv_fn(char *nv_ifname, char *cfg_arr_nvname,
		char **placeholder, char *out_val, int out_sz)
{
	char nv_name[HAPD_MAX_BUF] = {0};
	char *nv_value = NULL;
	...
	snprintf(out_val, out_sz, "%d", atoi(nv_value) + 1);
	return 0;
}
#endif /* CMWIFI_RDKB */
	...
	for (idx = 0; cfg_arr[idx].nv_name != NULL; idx++) {
		...
		if (cfg_arr[idx].flags & HAPD_CFG_USEDEFAULT) {
			...
			if (cfg_arr[idx].cb_fn != NULL) {
				...
				} else if (cfg_arr[idx].flags & HAPD_CFG_PERRADIO) {
					char *pCh = NULL, *primary_nv_ifname;

					primary_nv_ifname = strdup(nv_ifname);
					if (!primary_nv_ifname) {
						cprintf("Err: rc: %d: conversion fn returned err"
							" for %s in iface %s\n", __LINE__,
							cfg_arr[idx].nv_name, nv_ifname);
						goto free_mem;
					}
					pCh = strchr(primary_nv_ifname, '.');
					if (pCh)
						primary_nv_ifname[pCh - primary_nv_ifname] = '\0';
					snprintf(nvram_name, sizeof(nvram_name), "%s_%s",
						primary_nv_ifname, cfg_arr[idx].nv_name);
					free(primary_nv_ifname);
				} else {
		...
		/* Default value in cfg_arr can be NULL */
		if (!val || val[0] == '\0') {
			goto free_mem;
		}
...
start_hostapd(char *filtered_iflist, char *primary_iflist, char *virtual_iflist)
{
	...
	if (!filtered_iflist  || *filtered_iflist == '\0') {
	...
	foreach(ifname, primary_iflist, next) {
		...
		if (hapd_wpasupp_is_ifce_ap(ifname) == FALSE) { /* non AP mode */
			if (!virtual_iflist  || *virtual_iflist == '\0') {
	...
	foreach(filename, filelist, next) {
		...
		snprintf(cmd, sizeof(cmd), "hostapd %s %s &",
				(nvram_match("hapd_dbg", "1") ? "-ddt" : "-t"), filename);
#else
		snprintf(cmd, sizeof(cmd), "hostapd %s %s &",
	...
	foreach(ifname, ifnames_list, next) {
		...
		dpp_role = nvram_safe_get(nv_name);
		if (!strcmp(dpp_role, "configurator") || !strcmp(dpp_role, "enrollee")) {
			...
			snprintf(nv_name, sizeof(nv_name), "%s_dpp_role", nv_ifname);
			if (!strcmp(dpp_role, "configurator")) {
				memset(cmd, 0x0, sizeof(cmd));
...
start_wpa_supplicant(int idx, char *supplicant_iflist, hapd_wpasupp_wds_info_t *wdsi_ptr)
{
	...
	char filename[HAPD_MAX_BUF] = {0}, cmd[HAPD_MAX_BUF * 4] = {0};
	char *next;
	char *lan_ifname = NULL, *lan_ifnames, tmp[32];
	int ret = -1;
	char nv_name[HAPD_MIN_BUF] = {0};
	int i;
	...
	foreach(ifname, supplicant_iflist, next) {
		...
		for (i = 0; i < HAPD_MAX_BRIDGE_NUM; i++) {
			...
			if (lan_ifnames[0] != '\0' && find_in_list(lan_ifnames, ifname)) {
				if (!i)
					snprintf(tmp, sizeof(tmp), "lan_ifname");
				else
	...
	foreach(ifname, supplicant_iflist, next) {
		...
		snprintf(nv_name, sizeof(nv_name), "%s_dpp_role", nv_ifname);
		if (nvram_match(nv_name, "enrollee")) {
			char dpp_priv_key[HAPD_MAX_BUF] = {0};
			char mac[HAPD_MAX_BUF] = {0};
			char *cptr = NULL;
			FILE *fp;
...
void akms_to_dpp_conf(char *akms, char *dpp_conf)
{
	memset(dpp_conf, 0, HAPD_MIN_BUF);
	if (strstr(akms, "psk2"))
		strcat(dpp_conf, "-psk");
	if (strstr(akms, "sae"))
		strcat(dpp_conf, "-sae");
	...
	for (idx = 0; idx < MAX_RADIO_NUM; idx++) {
		...
		foreach(ifname, filtered_iflist, next) {
			...
			akms = nvram_safe_get(nv_name);
			{
				...
				char ssid_hex[(2*MAX_SSID_LEN) + 1] = {0};
				char pass[WLIF_PSK_MAX_SZ + 1] = {0};
				char pass_hex[2*WLIF_PSK_MAX_SZ + 1] = {0};
				char *cptr = NULL;

				memset(nv_name, 0x0, sizeof(nv_name));
				snprintf(nv_name, sizeof(nv_name), "%s_ssid", nv_ifname);
				if ((cptr = nvram_safe_get(nv_name))) {
					memcpy(ssid, cptr, MIN(MAX_SSID_LEN, strlen(cptr)));
				}

				wl_ascii_str_to_hex_str(ssid, sizeof(ssid), ssid_hex,
						sizeof(ssid_hex));

				memset(nv_name, 0x0, sizeof(nv_name));
				snprintf(nv_name, sizeof(nv_name), "%s_wpa_psk", nv_ifname);
				if ((cptr = nvram_safe_get(nv_name))) {
					memcpy(pass, cptr, MIN(WLIF_PSK_MAX_SZ, strlen(cptr)));
				}

				wl_ascii_str_to_hex_str(pass, sizeof(pass), pass_hex,
						sizeof(pass_hex));
				...
				if (nvram_match(nv_name, "configurator")) {
					char dpp_conf[HAPD_MIN_BUF];
					akms_to_dpp_conf(akms, dpp_conf);

					/* If AKM is DPP, no need to set passphrase */
					if (!strcmp(akms, "dpp")) {
						snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s "
							"dpp_configurator_sign conf=ap%s ssid=%s "
							"configurator=1",
							nv_ifname, dpp_conf, ssid_hex);
					} else {
						snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s "
							"dpp_configurator_sign conf=ap%s ssid=%s "
							"pass=%s configurator=1",
							nv_ifname, dpp_conf, ssid_hex, pass_hex);
					}
					system(cmd);
				}
			}
