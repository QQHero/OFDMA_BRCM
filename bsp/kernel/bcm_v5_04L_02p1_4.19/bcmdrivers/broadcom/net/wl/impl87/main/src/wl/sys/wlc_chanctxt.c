 ...
 *
 * $Id: wlc_chanctxt.c 809378 2022-03-15 04:52:49Z $
 */
...
wlc_txqueue_start(wlc_info_t *wlc, wlc_bsscfg_t *cfg, chanspec_t chanspec)
{
	...
	WL_CHANCTXT_TRACE(("[ENTRY]wl%d.%d: %s: chanspec %s, call by %p\n",
...
wlc_chanctxt_free(wlc_chanctxt_info_t *chanctxt_info, wlc_chanctxt_t *chanctxt)
{
	...
	ASSERT(chanctxt != NULL);
#ifdef PHYCAL_CACHING
	chanspec = chanctxt->chanspec; /* Need to know which chanspec to delete */
#endif /* PHYCAL_CACHING */
	...
	uint oldqi_stopped_flag)
{
	...
	wlc_wds_scb_switch_queue(wlc, cfg, chanctxt->qi);
#endif /* WDS */
