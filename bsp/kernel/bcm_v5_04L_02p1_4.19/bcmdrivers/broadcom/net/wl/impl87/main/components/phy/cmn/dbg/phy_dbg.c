 ...
 *
 * $Id: phy_dbg.c 782932 2020-01-09 10:39:50Z $
 */
#include <phy_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
void wlc_phy_print_phydbg_regs(phy_info_t *pi)
{
...
int
phy_dbg_dump_getlistandsize(phy_info_t *pi, uint8 type)
{
#ifdef PHY_DUMP_BINARY
...
phy_dbg_regval_get(phy_dbg_info_t *dbgi, void *data, int length)
{
	...
	ret = phy_dbg_dump_tbls(pi, dbgi->type, buffer, &buffer_length_bytes, FALSE);
...
int
phy_dbg_dump_tbls(phy_info_t *pi, uint8 type, uchar *p, int * buf_len, bool fatal_dump)
{
#ifdef PHY_DUMP_BINARY
	phy_dbg_info_t *di = pi->dbgi;
	phy_type_dbg_fns_t *fns = di->fns;
	...
	if (phy_dbg_dump_getlistandsize(pi, type) < 0)
...
phy_dbg_page_parser(phy_info_t *pi, uint8 type, int buf_len)
{
	...
	phy_dbg_dump_getlistandsize(pi, type);
