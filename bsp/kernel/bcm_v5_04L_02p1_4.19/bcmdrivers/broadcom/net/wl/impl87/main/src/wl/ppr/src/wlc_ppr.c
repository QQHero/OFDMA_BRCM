...
struct ppr_ru_offset {
	uint32 hdr;
	/* ru26 tx power limits */
	ppr_he_mcs_rateset_t ru26_bl;
	ppr_he_ru_rl_rateset_t ru26_rl;
	/* ru52 tx power limits */
	ppr_he_mcs_rateset_t ru52_bl;
	ppr_he_ru_rl_rateset_t ru52_rl;
	/* ru106 tx power limits */
	ppr_he_mcs_rateset_t ru106_bl;
	ppr_he_ru_rl_rateset_t ru106_rl;
	/* ru242 tx power limits */
	ppr_he_mcs_rateset_t ru242_bl;
	ppr_he_ru_rl_rateset_t ru242_rl;
	/* ru484 tx power limits */
	ppr_he_mcs_rateset_t ru484_bl;
	ppr_he_ru_rl_rateset_t ru484_rl;
	/* ru996 tx power limits */
	ppr_he_mcs_rateset_t ru996_bl;
	ppr_he_ru_rl_rateset_t ru996_rl;
};
...
/* Set the HE RU PPR RL values for the group specified by Nss, rusize and tx chains */
	...
	wl_tx_mode_t mode, wl_he_rate_type_t rusize, const int8 power)
{
	...
	if (rusize == WL_HE_RT_RU996) {
		ppr_ru_offset_ptr->ru996_rl.pwr[Nss - 1 + txbf_en*WL_TX_CHAINS_MAX] = power;
	} else if (rusize == WL_HE_RT_RU484) {
		ppr_ru_offset_ptr->ru484_rl.pwr[Nss - 1 + txbf_en*WL_TX_CHAINS_MAX] = power;
	} else if (rusize == WL_HE_RT_RU242) {
		ppr_ru_offset_ptr->ru242_rl.pwr[Nss - 1 + txbf_en*WL_TX_CHAINS_MAX] = power;
	} else if (rusize == WL_HE_RT_RU106) {
		ppr_ru_offset_ptr->ru106_rl.pwr[Nss - 1 + txbf_en*WL_TX_CHAINS_MAX] = power;
	} else if (rusize == WL_HE_RT_RU52) {
		ppr_ru_offset_ptr->ru52_rl.pwr[Nss - 1 + txbf_en*WL_TX_CHAINS_MAX] = power;
	} else if (rusize == WL_HE_RT_RU26) {
		ppr_ru_offset_ptr->ru26_rl.pwr[Nss - 1 + txbf_en*WL_TX_CHAINS_MAX] = power;
	}
}

/* Set the HE RU PPR BL values for the group specified by MCS, rusize */
void ppr_set_he_ruoffset_bl(ppr_ru_offset_t* ppr_ru_offset_ptr, wl_he_rate_type_t rusize,
	uint8 mcsidx, const int8 power)
{
	if (rusize == WL_HE_RT_RU996) {
		ppr_ru_offset_ptr->ru996_bl.pwr[mcsidx] = power;
	} else if (rusize == WL_HE_RT_RU484) {
		ppr_ru_offset_ptr->ru484_bl.pwr[mcsidx] = power;
	} else if (rusize == WL_HE_RT_RU242) {
		ppr_ru_offset_ptr->ru242_bl.pwr[mcsidx] = power;
	} else if (rusize == WL_HE_RT_RU106) {
		ppr_ru_offset_ptr->ru106_bl.pwr[mcsidx] = power;
	} else if (rusize == WL_HE_RT_RU52) {
		ppr_ru_offset_ptr->ru52_bl.pwr[mcsidx] = power;
	} else if (rusize == WL_HE_RT_RU26) {
		ppr_ru_offset_ptr->ru26_bl.pwr[mcsidx] = power;
	}
}
	...
	uint8 mcsidx, wl_tx_nss_t Nss, bool txbf, bool RL)
{
	if (RL == FALSE) {
		if (rusize == WL_HE_RT_RU996) {
			return ppr_ru_offset_ptr->ru996_bl.pwr[mcsidx];
		} else if (rusize == WL_HE_RT_RU484) {
			return ppr_ru_offset_ptr->ru484_bl.pwr[mcsidx];
		} else if (rusize == WL_HE_RT_RU242) {
			return ppr_ru_offset_ptr->ru242_bl.pwr[mcsidx];
		} else if (rusize == WL_HE_RT_RU106) {
			return ppr_ru_offset_ptr->ru106_bl.pwr[mcsidx];
		} else if (rusize == WL_HE_RT_RU52) {
			return ppr_ru_offset_ptr->ru52_bl.pwr[mcsidx];
		} else {
			return ppr_ru_offset_ptr->ru26_bl.pwr[mcsidx];
		}
	} else {
		if (rusize == WL_HE_RT_RU996) {
			return ppr_ru_offset_ptr->ru996_rl.pwr[(Nss-1) + 4*(uint8)(txbf)];
		} else if (rusize == WL_HE_RT_RU484) {
			return ppr_ru_offset_ptr->ru484_rl.pwr[(Nss-1) + 4*(uint8)(txbf)];
		} else if (rusize == WL_HE_RT_RU242) {
			return ppr_ru_offset_ptr->ru242_rl.pwr[(Nss-1) + 4*(uint8)(txbf)];
		} else if (rusize == WL_HE_RT_RU106) {
			return ppr_ru_offset_ptr->ru106_rl.pwr[(Nss-1) + 4*(uint8)(txbf)];
		} else if (rusize == WL_HE_RT_RU52) {
			return ppr_ru_offset_ptr->ru52_rl.pwr[(Nss-1) + 4*(uint8)(txbf)];
		} else {
			return ppr_ru_offset_ptr->ru26_rl.pwr[(Nss-1) + 4*(uint8)(txbf)];
		}
	}
}
...
/* Constructor routine for opaque PPR offset RU struct */
ppr_ru_offset_t* ppr_ru_offset_create(osl_t* osh)
{
	ppr_ru_offset_t* ppr_ru_offset_ptr;

#ifndef BCMDRIVER
	BCM_REFERENCE(osh);
	if ((ppr_ru_offset_ptr = (ppr_ru_offset_t*)malloc(sizeof(ppr_ru_offset_t))) != NULL) {
#else
	if ((ppr_ru_offset_ptr = (ppr_ru_offset_t*)MALLOC_NOPERSIST(osh,
		sizeof(ppr_ru_offset_t))) != NULL) {
#endif
		ppr_ru_offset_clear(ppr_ru_offset_ptr);
	} else {
		PPR_ERROR(("ppr_ru_offset_create: MALLOC(%d) failed\n",
			(uint32)sizeof(ppr_ru_offset_t)));
	}
	return ppr_ru_offset_ptr;
}

/* Destructor routine for opaque PPR RU offset struct */
void ppr_ru_offset_delete(osl_t* osh, ppr_ru_offset_t* ppr_ru_offset_ptr)
{
	if (ppr_ru_offset_ptr->hdr & PPR_HDR_FLAG_PREALLOC)
		return;

#ifndef BCMDRIVER
	BCM_REFERENCE(osh);
	free(ppr_ru_offset_ptr);
#else
	MFREE(osh, ppr_ru_offset_ptr, sizeof(*ppr_ru_offset_ptr));
#endif
}
...
{
	memset(ppr_ru_offset_ptr, 0, sizeof(ppr_ru_offset_t));
	ppr_ru_offset_ptr->hdr = 0;
}
...
/**
 * Get the minimum power for a 11ag rate with the given bw and tx chains.
 * Disabled rates are ignored
 */
int ppr_get_ofdm_min(ppr_t* pprptr, wl_tx_bw_t bw, wl_tx_mode_t mode,
 wl_tx_chains_t tx_chains, int8* mcs_min)
{
	...
	*mcs_min = TXPPR_TXPWR_MAX;
	...
	if (bw_pwrs != NULL) {
		powers = ppr_get_ofdm_group(bw_pwrs, mode, tx_chains);
		if (powers != NULL) {
			for (i = 0; i < sizeof(ppr_ofdm_rateset_t); i++) {
...
ppr_ru_map_vec_all(ppr_mapfn_t fn, void *context, ppr_ru_t *pprptr1, ppr_ru_t *pprptr2)
{
	...
	int8 *rptr1;
	int8 *rptr2;
	uint size = SIZE_RU_PPR;	/* ppr size of struct ppr_ru */
	bool ret = TRUE;

	if ((pprptr1 != NULL) && (pprptr2 != NULL)) {
		rptr1 = (int8 *)&pprptr1->ru26;
		rptr2 = (int8 *)&pprptr2->ru26;
		/* Convert ppr_ru 1 to ppr_ru 2 */
