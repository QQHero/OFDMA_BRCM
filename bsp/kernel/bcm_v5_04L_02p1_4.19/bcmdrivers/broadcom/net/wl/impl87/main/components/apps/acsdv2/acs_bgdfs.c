 ...
 *
 *	$Id: acs_bgdfs.c 805468 2021-11-28 01:43:00Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
reduce_bw:
	...
	for (i = 0; i < count; i++) {
		...
		cand_is_weather = ((cand_chinfo & WL_CHAN_RADAR_EU_WEATHER) != 0);
	...
	/* Condition: c_info->bgdfs160 is true only for chips that are capable of running BGDFS
	 * on a bandwidth that is independent of current channel bandwidth.
	 */
	if (considered == 0 && c_info->bgdfs160) {
		if (bw > ACS_BW_20 && (c_info->country_is_edcrs_eu || bw > chbw)) {
...
acs_bgdfs_ahead_trigger_scan(acs_chaninfo_t * c_info)
{
	...
	/* BGDFS on 5G monitor mode, move channel to pref channel */
	if (c_info->acs_bgdfs->ch_move_on_5g_monitor) {
		acs_bgdfs->next_scan_chan = c_info->rs_info.pref_chspec;
		ACSD_INFO("%s: BGDFS channel move on 5G monitor pref_chspec 0x%x \n",
			c_info->name, c_info->rs_info.pref_chspec);
	}

	/* For FCC country codes, calculate candidate scores for all valid channels and
	 * best chanspec is used to initiate bgdfs/zdfs_2g CAC.
	 */
	if (!is_etsi) {
		acs_dfsr_set_reentry_type(ACS_DFSR_CTX(c_info), DFS_REENTRY_IMMEDIATE);
		c_info->switch_reason = ACS_DFSREENTRY;
		c_info->acs_bgdfs_move = TRUE;
		if (!acs_select_chspec(c_info)) {
			c_info->acs_bgdfs_move = FALSE;
			return BCME_OK;
		}
		c_info->acs_bgdfs_move = FALSE;
		acs_bgdfs->next_scan_chan = c_info->selected_chspec;
		chosen_chspec = acs_bgdfs->next_scan_chan;
		if (acs_is_dfs_chanspec(c_info, c_info->selected_chspec)) {
			goto bgdfs_move;
		} else {
			ACSD_INFO("%s: Not allowing bgdfs as chosen chspec is not dfs channel"
...
bgdfs_move:
	...
	if (!is_etsi && !(FIXCHSPEC(c_info) || (MONITORCHECK(c_info) &&
			!c_info->acs_bgdfs->ch_move_on_5g_monitor)) &&
	...
	if (is_etsi  && acs_bgdfs->preclear_etsi) {
		/*
		* Move channel if ch_move_on_5g_monitor is TRUE
		* Or pre-clearing/stunt the selected channel for future use
		*/
		ACSD_INFO("%s: ETSI %s DFS channel 0x%0x\n", c_info->name,
			c_info->acs_bgdfs->ch_move_on_5g_monitor ? "moving to" : "pre-clearing",
			chosen_chspec);
		if ((ret = acs_bgdfs_attempt(c_info, chosen_chspec,
			c_info->acs_bgdfs->ch_move_on_5g_monitor ? FALSE : TRUE)) != BCME_OK) {
			return ret;
		}
	}
	return BCME_OK;
