 ...
 *
 * $Id: phy_noise.c 810501 2022-04-08 03:34:50Z $
 */
...
static void phy_noise_gain_bias_reset(phy_noise_info_t *noisei);
...
wlc_phy_noise_calc(phy_info_t *pi, uint32 *cmplx_pwr, int8 *pwr_ant, uint8 extra_gain_1dB)
{
...
#ifdef WL_EAP_NOISE_MEASUREMENTS
	PHY_INFORM(("wl%d:--> RXGAIN: %d\n", pi->sh->unit, wlapi_bmac_read_shm(pi->sh->physhim,
	...
	FOREACH_CORE(pi, i) {
		...
		PHY_INFORM(("wl%d:wlc_phy_noise_calc_phy: pwr_ant[%d] = %d\n",
			pi->sh->unit, i, pwr_ant[i]));
	}

	PHY_INFORM(("wl%d:%s: samples %d ant %d\n", pi->sh->unit, __FUNCTION__,
		pi->phy_rxiq_samps, pi->phy_rxiq_antsel));
}
...
WLBANDINITFN(phy_noise_init)(phy_init_ctx_t *ctx)
{
	...
	if (pi->sh->interference_mode_override == TRUE) {
		...
		if (CHSPEC_IS2G(pi->radio_chanspec)) {
			...
			/* for NPHY in 5G, only values 0 and 1 are valid options */
...
#ifdef WL_EAP_NOISE_MEASUREMENTS
	/* Load biases for current band */
	phy_noise_gain_bias_reset(ii);
#endif /* WL_EAP_NOISE_MEASUREMENTS */
...
void
wlc_phy_interf_rssi_update(phy_info_t *pi, chanspec_t chanspec, int8 leastRSSI)
{
...
void
wlc_phy_hwaci_mitigate_intr(phy_info_t *pi)
{
	phy_noise_info_t *noisei = pi->noisei;
	phy_type_noise_fns_t *fns = noisei->fns;
...
int
phy_noise_sample_request_external(phy_info_t *pi)
{
	uint8  channel;
...
int
phy_noise_sample_request_crsmincal(phy_info_t *pi)
{
	uint8  channel;
	phy_noise_info_t *noise_info = pi->noisei;
	phy_type_noise_fns_t *fns = noise_info->fns;
...
int
phy_noise_sample_intr(phy_info_t *pi)
{
	phy_noise_info_t *noisei = pi->noisei;
	phy_type_noise_fns_t *fns = noisei->fns;
	phy_type_noise_ctx_t *ctx = (phy_type_noise_ctx_t *) fns->ctx;
...
int8
phy_noise_lte_avg(phy_info_t *pi)
{
	phy_noise_info_t *noisei = pi->noisei;
	phy_type_noise_fns_t *fns = noisei->fns;
	phy_type_noise_ctx_t *ctx = fns->ctx;
...
/* phy_noise_hwknoise_isenabled - exports status of HW Knoise "in use"
 * Returns
 *  OK: HW Knoise is enabled/running
 *  UNSUPPORTED: HW Knoise is disabled or not an option
 */
int
phy_noise_hwknoise_isenabled(phy_info_t *pi)
{
	phy_noise_info_t *noise_info = pi->noisei;
	phy_type_noise_fns_t *fns = noise_info->fns;
	phy_type_noise_ctx_t *ctx = (phy_type_noise_ctx_t *)fns->ctx;
	/* HW Knoise (PHY-based noise floor measurements) is supported on phyrevs GE130 */
	if (ISACPHY(pi) && (fns->hwknoise_ena)) {
		return fns->hwknoise_ena(ctx);
	}
	return BCME_UNSUPPORTED;
}
...
int
phy_noise_interf_chan_stats_upd(phy_info_t *pi, chanspec_t chanspec, uint32 crsglitch,
...
int
phy_noise_sched_set(phy_info_t *pi, phy_bgnoise_schd_mode_t reason, bool upd)
{
...
bool
phy_noise_sched_get(phy_info_t *pi)
{
	return (pi->phynoise_disable);
}

int
phy_noise_pmstate_set(phy_info_t *pi, bool pmstate)
{
...
int8
phy_noise_avg_per_antenna(phy_info_t *pi, int coreidx)
{
	uint8 i, idx;
	int32 tot = 0;
	int8 result = 0;
 ...
 *
 * gaintype: WL_PHY_NB_GAIN_LO or WL_PHY_NB_GAIN_HI
 * band: WL_PHY_NB_2G or WL_PHY_NB_5G
...
int phy_noise_get_gain_bias_range(phy_info_t *pi, int gaintype, int band, wl_phynoisebias_t *dst)
{
	int rc = BCME_OK;
	static wl_phynoisebias_t nb_no_bias = {WL_NUM_NOISE_BIASES, {0, 0, 0, 0}};
	wl_phynoisebias_t *src = &nb_no_bias;
	bool sub_bands = TRUE;

	switch (band) {
		case WL_PHY_NB_2G:
			/* 2G has no subbands, but uses "ranged" utilities */
			sub_bands = FALSE;
			/* Fall through */
		case WL_PHY_NB_5G:
		case WL_PHY_NB_5G_RADAR:
		case WL_PHY_NB_6G:
			switch (gaintype) {
			   case WL_PHY_NB_GAIN_LO:
					/* Fall through */
			   case WL_PHY_NB_GAIN_HI:
					src = &pi->phynoise_bias_ranges[band][gaintype];
					break;
				default:
					rc = BCME_UNSUPPORTED;
					break;
	...
	/* Copy from phy_info to IOVAR buffer */
	if (rc == BCME_OK) {
		ASSERT(dst);
		bcopy(src, dst, sizeof(wl_phynoisebias_t));
	}
	if (!sub_bands) {
		dst->count = 1;
	}
...
int phy_noise_set_gain_bias_range(phy_info_t *pi, int gaintype, int band, wl_phynoisebias_t *src)
{
	int rc = 0;
	static wl_phynoisebias_t nb_no_bias = {WL_NUM_NOISE_BIASES, {0, 0, 0, 0}};
	wl_phynoisebias_t *dst;
	bool sub_bands = TRUE;
	...
	switch (band) {
		...
		case WL_PHY_NB_6G:
			switch (gaintype) {
			   case WL_PHY_NB_GAIN_LO:
			   case WL_PHY_NB_GAIN_HI:
					dst = &pi->phynoise_bias_ranges[band][gaintype];
					break;
				default:
					return BCME_UNSUPPORTED;
			}
	...
	/* Error check input - reset on error */
	if (src->count == 0) {
		src = &nb_no_bias;
	} else if (src->count > WL_NUM_NOISE_BIASES) {
		src = &nb_no_bias;
	}

	/* Save from IOVAR buffer to phy_info */
	bcopy(src, dst, sizeof(wl_phynoisebias_t));

	/* Support both old version (single value) and
	 * new version (four values per UNII bands).
	 */
	if ((!sub_bands) || (dst->count < WL_NUM_NOISE_BIASES)) {
		/* For now, only support 1 or 4 entries */
		dst->count = 1;
	}

	/* Promotion of non-set values, from last supplied, e.g.:
	 * wl phynoisebiasXX 1 -> wl phynoisebiasXX 1 1 1 1
	 */
	if (sub_bands && dst->count < WL_NUM_NOISE_BIASES) {
		int i, j;
		i = dst->count - 1; // index of last valid entry
		j = i+1;			// promoted index
		for (i = dst->count-1; i < WL_NUM_NOISE_BIASES; i++, j++) {
			dst->dBs[j] = dst->dBs[i];
		}
	}

	/* Reset to immediately apply changes */
	phy_noise_gain_bias_reset(pi->noisei);

	return rc;
}

void phy_noise_get_gain_biases(phy_info_t *pi, int8 *hi, int8 *lo, int8 *radarhi, int8 *radarlo)
{
	ASSERT(pi); ASSERT(hi); ASSERT(lo);
	/* Pass back the current/active values */
	*hi = pi->phynoise_bias_hi;
	*lo = pi->phynoise_bias_lo;

	/* Radar may not be necessary */
	if (radarhi != NULL) {
		*radarhi = pi->phynoise_bias_rdrhi;
	}
	if (radarlo != NULL) {
		*radarlo = pi->phynoise_bias_rdrlo;
	}
}
...
static void phy_noise_gain_bias_reset(phy_noise_info_t *noisei)
{
	...
	range = phy_ac_chanmgr_get_chan_freq_range(pi, chanspec, PRIMARY_FREQ_SEGMENT);
	/* Convert chan-freq-range to noise-bias-freq-range (chan-range starts at 1) */
	switch (band) {
		case WL_PHY_NB_2G:
			/* 2G has no subbands, and no range */
			ASSERT(range == 0);
			range = 0;
			break;
		case WL_PHY_NB_5G:
			/* fall through */
		case WL_PHY_NB_6G:
			/* UNII ranges start at 1 */
			ASSERT(range > 0);
			range -= 1;
			break;
		default:
			PHY_ERROR(("%s: bad band %d! not set\n", __FUNCTION__, band));
			ASSERT(0);
			/* Exit early - bad index into array */
			return;
	}

	/* Asign current noise bias based on freq range */
	pi->phynoise_bias_lo =
		pi->phynoise_bias_ranges[band][WL_PHY_NB_GAIN_LO].dBs[range];
	pi->phynoise_bias_hi =
		pi->phynoise_bias_ranges[band][WL_PHY_NB_GAIN_HI].dBs[range];
	pi->phynoise_bias_rdrlo =
		pi->phynoise_bias_ranges[WL_PHY_NB_5G_RADAR]
			[WL_PHY_NB_GAIN_LO].dBs[range];
	pi->phynoise_bias_rdrhi =
		pi->phynoise_bias_ranges[WL_PHY_NB_5G_RADAR]
			[WL_PHY_NB_GAIN_HI].dBs[range];
	PHY_NONE(("%s: Biases for band %s: range %d hi %d lo %d radarhi %d radarlo %d\n",
		__FUNCTION__, (band == WL_PHY_NB_5G) ? "5G" :
		(band == WL_PHY_NB_6G) ? "6G" : "2G",
		range, pi->phynoise_bias_hi, pi->phynoise_bias_lo,
		pi->phynoise_bias_rdrhi, pi->phynoise_bias_rdrlo));
}
#endif /* WL_EAP_NOISE_MEASUREMENTS */
