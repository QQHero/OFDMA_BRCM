...
static int bsd_process_bss_trans_resp(void *data, int response, bool brute_force);
...
static const bsd_reason_name_str_t bsd_reason_names[] = {
	...
	{BSD_STEERING_RESULT_NOT_HAPPENED, "steer not happened"},
		...
		for (bssidx = 0; bssidx < WL_MAXBSSCFG; bssidx++) {
			...
			if ((bssinfo->valid) && BSD_BSS_BSD_OR_WBD_ENABLED(bssinfo)) {
				...
				while (assoclist) {
					...
					/*  band */
					band = bsd_get_sta_band(info, &assoclist->addr);
...
static int
bsd_process_bss_trans_resp(void *data, int response, bool brute_force)
{
	...
	/* Each steering that actually took place  to be added in bounce table
	 * irrespective of steering result, otherwise we shall retry opportunistically
	*/
	if ((response == BSD_BSS_RESPONSE_ACCEPT) || brute_force) {
	...
	if (response == BSD_BSS_RESPONSE_UNKNOWN) {
		if (bssinfo->intf_info->band != callback_data->steer_bssinfo->intf_info->band) {
			...
			if (!(callback_data->steer_bssinfo->intf_info->band & band)) {
				/* Unblock mac on current interface */
				wl_wlif_unblock_mac(bssinfo->wlif_hdl, sta->addr, 0);
				BSD_STEER("Skip STA "MACF" no response for %s band sta\n",
					ETHER_TO_MACF(sta->addr),
					band & BSD_BAND_6G ? "6G" :
					band & BSD_BAND_5G ? "5G" : "2G");
#ifdef CMWIFI_RDKB
				snprintf(log_buf, sizeof(log_buf), "Remove STA "MACF" from blacklist on %s for band[%d] sta when BTM Unknown\n",
					ETHER_TO_MACF(sta->addr), bssinfo->ifnames, band);
				WLAN_APPS_LOGS("BSD", log_buf);
#endif
				goto end;
			}
		...
		if (intfidx != callback_data->steer_bssinfo->intf_info->idx) {
			...
			for (bssidx = 0; bssidx < WL_MAXBSSCFG; bssidx++) {
				...
				if (!strcmp(tmp_bssinfo->ssid,
					...
					int timeout = (intf_info->band != BSD_BAND_2G) ?
						 info->block_sta_timeout : info->block_sta_2g_timeout;
					if ((response == BSD_BSS_RESPONSE_ACCEPT) || brute_force) {
						...
						bsd_update_steering_record(info, &(sta->addr), bssinfo, callback_data->steer_bssinfo,
							BSD_STEERING_RESULT_NOT_HAPPENED);
					}
				}
			}
		}
