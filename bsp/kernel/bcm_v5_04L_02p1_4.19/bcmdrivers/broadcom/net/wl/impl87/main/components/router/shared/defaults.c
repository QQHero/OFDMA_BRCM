 ...
 *
 * $Id: defaults.c 803637 2021-10-04 11:23:30Z $
 */
...
unset_generic_nvrams()
{
	...
	char* nvrams_to_unset[] = {
		...
		"wl_bss_bcn_disabled",	/* beaconing on BSS */
		"oce_cert_test",
		"rnr_tbtt_len"
	};
...
void
fbt_restore_defaults(char *in_prefix, bool uap_deinit_fbt)
{
	char prefix[] = "wlXXXXXXXXXXXXXXXXXXXXXXXXXXXX_mssid_";
	int unit, subunit;

	/* Erase FBT NVRAMs with each "map_bss_names" on WBD UAP */
	if (uap_deinit_fbt) {
		char *list_names = NULL, iter_name[15], *next = NULL;

		list_names = nvram_safe_get("map_bss_names");
		foreach(iter_name, list_names, next) {
			/* Unset self fbt nvrams for this map_bss_name */
			sprintf(prefix, "%s_", iter_name);
#ifdef MULTIAP
			wbd_unset_self_fbt_nvrams(prefix);
			wbd_unset_fbt(prefix);
#endif /* MULTIAP */
		}
	}
	...
	if (in_prefix != NULL) {
#ifdef MULTIAP
		wbd_dap_deinit_fbt_nvram_config(in_prefix);
		wbd_unset_fbt(in_prefix);
#endif /* MULTIAP */
		return;
	}

	/* Traverse through all Primary Prefix to restore fbt_aps NVRAMs */
	for (unit = 0; unit < MAX_RADIOS; unit++) {
		...
		/* Traverse through all Virtual Prefix */
		for (subunit = 0; subunit < MAX_BSS_PER_RADIO; subunit++) {
