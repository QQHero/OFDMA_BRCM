 ...
 *
 * $Id: wlc_rrm.c 810857 2022-04-18 06:12:56Z $
 */
...
static void wlc_rrm_process_gas_request(wlc_rrm_info_t *rrm_info, bcm_gas_t *gas,
	wlc_bsscfg_t *bsscfg, struct ether_addr *peer_addr);
...
wlc_rrm_get_sta_cap(wlc_info_t *wlc, wlc_bsscfg_t *cfg, struct scb *scb, uint8 *rrm_cap)
{
	...
	if (WL11K_ENAB(wlc->pub) && BSSCFG_AP(cfg)) {
		memcpy(rrm_cap, rrm_cfg->rrm_cap, DOT11_RRM_CAP_LEN);
	}
}
#endif /* WL11K_AP */
...
wlc_rrm_gas_event_cb(void *context, bcm_gas_t *gas, bcm_gas_event_t *event)
{
	...
	if (event->type != BCM_GAS_EVENT_QUERY_REQUEST) {
		WL_ERROR(("Ignore GAS request with event [%d] \n", event->type));
		goto end;
	}
	 ...
	 */
	wlc_rrm_gas_parse_query_list(event->queryReq.data, event->queryReq.len, &flag);
	...
	if (flag & MBO_GAS_RQST_ANQP_QUERY) {
		...
		wlc_bss_mac_event(rrm_info->wlc, bsscfg, WLC_E_GAS_RQST_ANQP_QUERY,
...
/* handle incoming GAS query request frame */
static void
wlc_rrm_process_gas_request(wlc_rrm_info_t *rrm_info, bcm_gas_t *gas, wlc_bsscfg_t *bsscfg,
	struct ether_addr *peer_addr)
{
	...
	wlc_rrm_get_nbr_report(rrm_info->wlc, bsscfg, list_cnt, buf);
	...
	bcm_gas_set_bsscfg_index(gas, bsscfg->_idx);
...
wlc_rrm_calc_anqp_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	*/
	...
	if (!isset(data->cfg->ext_cap, DOT11_EXT_CAP_IW)) {
		...
		if (wlc_is_mbo_enabled(mbo, data->cfg)) {
