 ...
 *
 * $Id: km_hw_algo_hwktab.c 795548 2021-02-09 10:20:51Z $
 */
	...
	int ins, bool tx, const wlc_key_info_t *key_info, const uint8 *data, size_t data_len)
{
	...
	case WLC_KEY_DATA_TYPE_TKHASH_P1:
		if (!KM_HW_TKIP_TSC_TTAK_SUPPORTED(hw, hw_idx) &&
			(KM_HW_COREREV_GE80(hw) && !WLC_KEY_IS_DEFAULT_BSS(key_info))) {
			err = BCME_UNSUPPORTED;
			break;
		}
		...
		/* Update Rx Phase1 Keys & TSC in shm after Tx P1 keys+TSC */
		if (!tx && KM_HW_COREREV_GE80(hw))
				shm_addr += KM_HW_TKIP_TSC_TTAK_SIZE;

		KM_DBG_ASSERT(data_len == (TKHASH_P1_KEY_SIZE + sizeof(uint32)));

		KM_HW_COPYTO_SHM(KM_HW_WLC(hw), shm_addr, data,
			(int)data_len - sizeof(uint32));
		shm_addr += TKHASH_P1_KEY_SIZE;
		KM_HW_WRITE_SHM(KM_HW_WLC(hw), shm_addr,
			ltoh16_ua(data + TKHASH_P1_KEY_SIZE));
