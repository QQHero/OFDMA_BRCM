 ...
 *
 * $Id: wlc_akm_ie.c 808950 2022-03-03 10:52:16Z $
 */
...
static const bcm_iovar_t akm_iovars[] = {
	...
	{"dh_ie", IOV_DH_IE, IOVF_OPEN_ALLOW, 0, IOVT_BUFFER, 0},
	{NULL, 0, 0, 0, 0, 0}
};
...
#endif /* AP */
#ifdef WL_SPP_AMSDU
static void wlc_bsscfg_update_rsn_cap(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi, void *pmcast);
#endif
...
#ifdef WL_SPP_AMSDU
static uint8
wlc_set_rsn_spp_cap(wlc_bsscfg_t *cfg)
{
	uint8 cap = 0;
	if (BSSCFG_IS_RSN_CAP_SPPC(cfg)) {
		cap |= RSN_CAP_SPPC >> 8;
	}

	if (BSSCFG_IS_RSN_CAP_SPPR(cfg)) {
		cap |= RSN_CAP_SPPR >> 8;
	}
	return cap;
}
...
static bool
wlc_enable_spp_amsdu(wlc_bsscfg_t *bsscfg, struct scb *scb, uint16 rsn_cap)
{
	bool ap_cap, ap_req, sta_cap, sta_req;
	bool ret = TRUE;

	/* If SPP AMSDU is supported */
	ap_cap = BSSCFG_IS_RSN_CAP_SPPC(bsscfg);
	ap_req = BSSCFG_IS_RSN_CAP_SPPR(bsscfg);
	sta_cap = ((rsn_cap & RSN_CAP_SPPC) != 0);
	sta_req = ((rsn_cap & RSN_CAP_SPPR) != 0);

	if (ap_cap && sta_cap) {
		/* Enable SPP for this scb */
		scb->flags |= SCB_AMSDU_SPP;
	}
	else if (((ap_req) && (!sta_cap)) || ((!ap_cap) && (sta_req)) ||
		((ap_cap || ap_req) && ((!sta_cap) && (sta_req))) ||
		((ap_cap || ap_req) && ((sta_cap) && (!sta_req)))) {
		/* Disable AMSDU */
...
wlc_wpa_cap(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *cap, int len)
{
	...
	if (BSSCFG_AP(cfg) &&
	    (cfg->WPA_auth & (WPA2_AUTH_UNSPECIFIED | WPA2_AUTH_1X_SHA256 |
			WPA3_AUTH_SUITEB)) &&
...
#endif
#ifdef WL_SPP_AMSDU
	/* Set AMSDU SPP capabilities */
	if (D11REV_GE(wlc->pub->corerev, 129)) {
		cap[1] |= wlc_set_rsn_spp_cap(cfg);
	}
#endif /* WL_SPP_AMSDU */
...
wlc_write_rsn_ie_safe(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *buf, int buflen)
{
	...
	if (WPA_auth & WPA2_AUTH_1X_SHA256) {
		...
		/* If pure SAH256, set MFP to required */
		if (!(WPA_auth & WPA2_AUTH_UNSPECIFIED)) {
			BSSCFG_SET_AKM_REQUIRES_MFP(cfg);
			BSSCFG_SET_MFP_CAPABLE(cfg);
		}
	}
	if (WPA_auth & WPA3_AUTH_SUITEB) {
	...
	if (WPA_auth & (WPA2_AUTH_PSK | WPA2_AUTH_PSK_SHA256)) {
		...
		if (BSSCFG_STA(cfg) && (WPA_auth & WPA2_AUTH_PSK_SHA256))
			...
			/* Only WPA-PSK AKM should go here.
			 * PSK- SHA256 AKM should Go after AKM FT
			 */
			if (wlc_write_oui_and_type(&buflen, &WPA_len, auth->list,
				WPA2_OUI, akm_type, &count)) {
				return orig_buf;
		...
		/* To update the SHA-256 AKM After the FT AKM */
		akm_wpa_psk = akm_type;
	}
#ifdef WLFBT
...
#endif /* WLFBT */
#ifdef MFP
	/* SHA-256 AKM should be after the FT AKM.
	 * For AP, akm should be WPA-PSK and SHA256 if
	 * mfp is Capable
	 */
	if ((akm_wpa_psk == RSN_AKM_SHA256_PSK) ||
		(BSSCFG_AP(cfg) && (WPA_auth & WPA2_AUTH_PSK_SHA256))) {
		akm_type = RSN_AKM_SHA256_PSK;
		if (wlc_write_oui_and_type(&buflen, &WPA_len, auth->list,
			WPA2_OUI, akm_type, &count)) {
			return orig_buf;
...
#ifdef WL_SPP_AMSDU
static void
wlc_bsscfg_update_rsn_cap(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi, void *pmcast)
{
	wpa_suite_mcast_t *mcast;
	wpa_suite_ucast_t *ucast;
	wpa_suite_auth_key_mgmt_t *mgmt;
	...
	cfg->flags2 &= ~(WLC_BSSCFG_FL2_RSN_CAP_SPPC |
			WLC_BSSCFG_FL2_RSN_CAP_SPPR);

	/* Maintain SPP AMSDU flags in bsscfg */
	if ((cap[1] & (RSN_CAP_SPPC  >> 8)) != 0) {
		cfg->flags2 |= WLC_BSSCFG_FL2_RSN_CAP_SPPC;
	}
	if ((cap[1] & (RSN_CAP_SPPR  >> 8)) != 0) {
		cfg->flags2 |= WLC_BSSCFG_FL2_RSN_CAP_SPPR;
	}
}
#endif /* WL_SPP_AMSDU */
...
wlc_check_wpa2ie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, bcm_tlv_t *wpa2ie, struct scb *scb)
{
	...
	uint16 count;
	uint16 pmkid_count;
#if defined(MFP) || defined(WLWNM_AP) || defined(WL_SPP_AMSDU)
	uint16 rsn_cap;
#endif /* MFP || WLWNM_AP || WL_SPP_AMSDU */
	...
	scb->flags &= ~SCB_AMSDU_SPP;
	if (D11REV_GE(wlc->pub->corerev, 129)) {
		if (!wlc_enable_spp_amsdu(bsscfg, scb, rsn_cap)) {
			WL_ERROR(("wl%d: %s: RSN IE cap mismatch from %s\n",
...
wlc_parse_wpa2_ie(wlc_info_t *wlc, bcm_tlv_t *wpa2ie, wlc_bss_info_t *bi)
{
	...
	for (i = 0, j = 0;
		...
		if (bcmwpa_akm2WPAauth((uint8 *)&mgmt->list[i], &WPA_auth)) {
			bi->WPA_auth_support |= WPA_auth;
			/* To support roaming to mfp required AP with indriver supplicant */
			if (WPA_auth == WPA2_AUTH_PSK_SHA256) {
				bi->WPA_auth_support |= WPA2_AUTH_PSK;
			}
		}
	}
	...
	/* Copy RSN capabilities to local bss info */
	bi->wpa2.cap[0] = cap[0];
	bi->wpa2.cap[1] = cap[1];
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
...
#ifdef WL_SPP_AMSDU
		wpa2_ie = (bcm_tlv_t *)params;
		wlc_bsscfg_update_rsn_cap(bsscfg, NULL, &wpa2_ie->data[WPA2_VERSION_LEN]);
#endif
		as->ie_len = p_len;
	...
	case IOV_SVAL(IOV_DH_IE): {
		wlc_assoc_t *as = bsscfg->assoc;

		/* Mostly used by external supplicants to set the DH IE
		 * on the STA, so set the assoc ie for the STA
		 */
		if (BSSCFG_AP(bsscfg)) {
			err = BCME_NOTSTA;
		...
		/* Do not allow more than max IE len */
		if (p_len > sizeof(dh_params_ie_t) ||
			(p_len != (((uint8*)params)[TLV_LEN_OFF] + TLV_HDR_LEN)))
			break;

		if (as->dh_params) {
			MFREE(wlc->osh, as->dh_params, sizeof(dh_params_ie_t));
			as->dh_params = NULL;
		}
		...
		if (!(as->dh_params = MALLOC(wlc->osh, sizeof(dh_params_ie_t)))) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
				wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			err = BCME_NOMEM;
			break;
		...
		bcopy((uint8*)params, as->dh_params, p_len);
...
void
wlc_host_rsnxe_deinit(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	wlc_akm_bsscfg_deinit(wlc->akmi, cfg);
}
...
wlc_akm_write_dh_params_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	/* Include Diffie-Hellman Parameter element, if included
	 * in assoc response sent by hostapd.
	 */
...
#ifdef STA
static int
wlc_akm_arq_write_dh_ie(void *ctx, wlc_iem_build_data_t *data)
{
	wlc_bsscfg_t *bsscfg = data->cfg;
	wlc_assoc_t *as = bsscfg->assoc;
	bcm_tlv_t *dh_params_ie = (bcm_tlv_t *)data->buf;

	/* Include Diffie-Hellman Parameter element */
	if ((bsscfg->WPA_auth == WPA3_AUTH_DPP) && as && as->dh_params) {
		bcopy(as->dh_params, dh_params_ie, as->dh_params->len + TLV_HDR_LEN);
	}
...
static uint
wlc_akm_arq_calc_dh_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	uint16 ie_len = 0;
	wlc_bsscfg_t *bsscfg = data->cfg;
	wlc_assoc_t *as = bsscfg->assoc;
	...
	/* Include Diffie-Hellman Parameter element */
	if ((bsscfg->WPA_auth == WPA3_AUTH_DPP) && as && as->dh_params) {
		ie_len = as->dh_params->len + TLV_HDR_LEN;
	}
...
static uint
wlc_akm_arsp_calc_rsnxe_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (!rsnxe_info || (rsnxe_info->rsnxe_len == 0) || !rsnxe_info->rsnxe) {
		goto done;
	}
...
static int
wlc_akm_arsp_write_rsnxe_ie(void *ctx, wlc_iem_build_data_t *data)
{
	rsnxe_ie_info_t *rsnxe_info = NULL;
	...
	/* Advertise local Extended RSN capabilities as part of RSNXE in (re)assoc response. */
	/* Local rsnxe_info */
	rsnxe_info = RSNXE_INFO_GET(data->cfg);
	...
	bcopy((uint8*)rsnxe_info->rsnxe, rsnxe_ie, rsnxe_info->rsnxe_len);
...
wlc_akm_is_sae_h2e_enabled(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	if ((bai->ext_sup_rates[bai->ext_sup_rates_len - 1] & 0x7f) ==
			DOT11_BSS_MEMBERSHIP_SELECTOR_SAE_H2E_ONLY) {
...
int wlc_akm_save_scb_rsnxe(wlc_info_t *wlc, struct scb *scb, dot11_rsnxe_t *rsnxe_ie_tlv)
{
	int err = BCME_OK;
	if (scb && rsnxe_ie_tlv && rsnxe_ie_tlv->len >= 1) {
		rsnxe_ie_info_t *rsnxe_info = NULL;
		/* save copy of RSNXE */
		...
		if (rsnxe_info == NULL) {
			rsnxe_info = (rsnxe_ie_info_t *)
				MALLOCZ(wlc->osh, sizeof(rsnxe_ie_info_t));
			if (rsnxe_info == NULL) {
				WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
					wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
				return BCME_NOMEM;
...
wlc_akm_arq_parse_rsnxe_ie(void *ctx, wlc_iem_parse_data_t *data)
{
	...
	/* Do not process RSNXE sent by STA, if AP does not suppport H2E */
	if (rsnxe_info && rsnxe_info->rsnxe_len && rsnxe_info->rsnxe) {
		if (data->ie != NULL) {
			dot11_rsnxe_t *rsnxe_ie_tlv = (dot11_rsnxe_t *)data->ie;
			err = wlc_akm_save_scb_rsnxe(wlc, scb, rsnxe_ie_tlv);
		}
	}
...
BCMATTACHFN(wlc_akm_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_iem_add_build_fn_mft(wlc->iemi, arqfstbmp, DOT11_MNG_DH_PARAM_ID,
	      wlc_akm_arq_calc_dh_ie_len, wlc_akm_arq_write_dh_ie, akm) != BCME_OK) {
	...
	if (wlc_iem_add_build_fn_mft(wlc->iemi, arspfstbmp, DOT11_MNG_RSNXE_ID,
		wlc_akm_arsp_calc_rsnxe_ie_len, wlc_akm_arsp_write_rsnxe_ie, akm) != BCME_OK) {
		WL_ERROR(("wl%d: wlc_akm_attach: wlc_iem_add_build_fn_mft failed, "
					" rsnxe ie\n", wlc->pub->unit));
		goto fail;
	}
