...
hndgci_send_break(si_t *sih)
{
	...
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secilcr),
...
_gci_write_uart_data(si_t *sih, gci_t *gu)
{
	...
	sii = SI_INFO(sih);
	if (!sii->regs[SI_CC_IDX]) {
		GCI_ERROR(("core not initialized \n"));
		return;
	}

	r = (uint8 *)((uchar *)sii->regs[SI_CC_IDX] +
		OFFSETOF(chipcregs_t, gci_seciuartdata));
	...
	if (!gu->uart.tx_count) {
		...
		si_gci_indirect(sih, 0,
			OFFSETOF(chipcregs_t, gci_intmask), GCI_INTSTATUS_STFAE, 0);
...
static void hndgci_uart_tx_timer_handler(hnd_timer_t *t)
{
	...
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_intmask), GCI_INTSTATUS_STFAE, GCI_INTSTATUS_STFAE);
...
_gci_read_uart_data(si_t *sih)
{
	...
	udata = si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_seciuartdata), 0, 0);
...
hndgci_uart_intr_handler(si_t *sih, uint32 intstatus)
{
	...
	if (intstatus & GCI_INTSTATUS_SPE) {
		si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_intstat),
	...
	if (intstatus & GCI_INTSTATUS_STFF) {
		...
		/* enable tx fifo almost empty intr */
		si_gci_indirect(sih, 0,
			OFFSETOF(chipcregs_t, gci_intmask), GCI_INTSTATUS_STFAE,
			GCI_INTSTATUS_STFAE);
	}
...
hndgci_config_pinmux(si_t *sih)
{
	switch (CHIPID(sih->chip)) {
		case BCM4335_CHIP_ID:
			...
			si_gci_set_functionsel(sih, CC4335_PIN_GPIO_04,
				CC4335_FNSEL_GCI0);
			si_gci_set_functionsel(sih, CC4335_PIN_GPIO_05,
				CC4335_FNSEL_GCI0);
		break;
		default:
 ...
 */
static int
hndgci_uart_init(si_t *sih, uint8 baudrate_idx)
{
	...
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secifcr), 0xFFFFFFFF, regval);
	OSL_DELAY(10);
	...
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secimcr), 0xFFFFFFFF, regval);

	/* enable rx */
	regval = (SECI_UART_LCR_RX_EN | SECI_UART_LCR_TXO_EN);
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secilcr), 0xFFFFFFFF, regval);

	/* rx fifo level to indicate almost full condition */
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_rxfifoctrl), 0xFFFFFFFF, 0x80E);

	/* tx fifo level to indicate almost empty condition */
	si_gci_direct(sih, OFFSETOF(chipcregs_t, gci_secififolevel), 0xFFFFFFFF,
		(txf_ae_level << 8));
	...
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_intmask), ~0, intmask);
 ...
 */
int
hndgci_init(si_t *sih, osl_t *osh, uint8 mode, uint8 br_idx)
{
...
hndgci_deinit(si_t *sih)
{
	...
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_intmask), ~0, 0);

	/* enable wake events same as gci interrupts */
	si_gci_indirect(sih, 0,
		OFFSETOF(chipcregs_t, gci_wakemask), ~0, 0);
