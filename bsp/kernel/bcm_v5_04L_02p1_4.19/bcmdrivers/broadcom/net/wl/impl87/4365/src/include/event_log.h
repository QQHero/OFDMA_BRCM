...
struct si_pub; /* forward declaration to prevent inclusion of <siutils.h> */
...
#define EVENT_LOG_WL_BLOCK_SIZE		0x100
#define EVENT_LOG_PSM_BLOCK_SIZE	0x100
#define EVENT_LOG_BUS_BLOCK_SIZE	0x200
#define EVENT_LOG_ERROR_BLOCK_SIZE	0x200
#define EVENT_LOG_MEM_API_BLOCK_SIZE	0x80
...
extern int event_log_init(const struct si_pub *sih);
extern bool event_log_is_ready(void);
extern int event_log_set_init(const struct si_pub *sih, int set_num, int size);
extern int event_log_set_expand(const struct si_pub *sih, int set_num, int size);
extern int event_log_set_shrink(const struct si_pub *sih, int set_num, int size);
...
extern bool event_log_rom_address(void *addr);
