 ...
 *
 * $Id: wlc_dfs.c 810805 2022-04-14 10:18:05Z $
 */
...
wlc_dfs_csa_each_up_ap(wlc_info_t *wlc, wl_chan_switch_t *csa, bool docs)
{
	...
	if (WLC_IS_CSA_TIMER_IN_PROGRESS(wlc) ||
		(APSTA_ENAB(wlc->pub) && BSSCFG_STA(wlc->cfg) &&
			BSSCFG_IS_CSA_IN_PROGRESS(wlc->cfg))) {
	...
	void *params, uint p_len, void *arg, int len, int val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_DFS_AP_MOVE):
	{
		...
		if (APSTA_ENAB(wlc->pub) && BSSCFG_AS_STA(wlc->cfg)) {
			if (wlc->is_edcrs_eu) {
				preclear_only = TRUE;
			} else {
				WL_ERROR(("wl%d: Rejecting BGDFS for Repeaters in Non-ETSI\n",
					wlc->pub->unit));
				err = BCME_UNSUPPORTED;
				break;
			}
		...
		if ((int16)chspec == -2) {
			...
			wlc_channel_send_chan_event(wlc, bsscfg,
				WL_CHAN_REASON_DFS_AP_MOVE_STUNT, WLC_BAND_PI_RADIO_CHANSPEC);
...
#ifdef WL_AP_CHAN_CHANGE_EVENT
		wlc_channel_send_chan_event(wlc, bsscfg, WL_CHAN_REASON_DFS_AP_MOVE_START,
			chspec);
#endif /* WL_AP_CHAN_CHANGE_EVENT */
		break;
	}
...
wlc_dfs_scan_complete_sc(wlc_dfs_info_t *dfs, int reason, int return_home_chan)
{
	...
	if (send_event) {
		wlc_channel_send_chan_event(wlc, wlc->cfg, rc, target_chspec);
	}
#endif /* WL_AP_CHAN_CHANGE_EVENT */
	...
	if (wlc_radar_detected(dfs, FALSE) == TRUE) {
		...
		if (WL11H_AP_ENAB(wlc)) {
			...
			if (WLC_IS_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc)) {
				...
				wlc_do_chanswitch(wlc->cfg, dfs->old_chanspec);
	...
	if (!dfs->dfs_cac.duration) {
		...
		if (WLC_IS_SWITCH_TO_OLD_CHANSPEC_AFTER_CAC(wlc)) {
			...
			if (!BSSCFG_AP(wlc->cfg) &&
				(wlc_assoc_get_as_state(wlc->cfg) == AS_DFS_CAC_START)) {
...
wlc_dfs_cacstate_init(wlc_dfs_info_t *dfs)
{
	...
	if (wlc_radar_chanspec(wlc->cmi, chspec) == TRUE) {
		...
		WL_REGULATORY(("wl%d: CAC or ISM state for cactime[%d] skip_pre_ism[%d]"
			...
			AP_ENAB(wlc->pub), wlc_cac_is_clr_chanspec(dfs, chspec), chspec));
