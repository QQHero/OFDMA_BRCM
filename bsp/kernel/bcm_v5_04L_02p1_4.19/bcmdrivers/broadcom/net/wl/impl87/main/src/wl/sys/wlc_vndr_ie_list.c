 ...
 *
 * $Id: wlc_vndr_ie_list.c 800247 2021-06-22 05:48:20Z $
 */
 ...
 */
vndr_ie_listel_t *
wlc_vndr_ie_list_add_info(osl_t *osh, vndr_ie_listel_t **vndr_ie_listp,
	vndr_ie_info_t *ie_info, int info_len)
{
	...
	while (curr_list_el != NULL) {
		...
		if (ie_info->vndr_ie_data.len == curr_list_el->vndr_ie_infoel.vndr_ie_data.len) {
	...
	if ((new_list_el = (vndr_ie_listel_t *)
	     MALLOCZ(osh, info_len + VNDR_IE_EL_HDR_LEN)) == NULL) {
		WL_ERROR(("wl: %s(): out of memory\n", __FUNCTION__));
		return NULL;
	}

	bcopy((char *)ie_info, (char *)&new_list_el->vndr_ie_infoel, info_len);
...
/*
 * Create a vendor IE information element object only with pkt flag and vndr ie
 * Return value: address of the new object.
 */
vndr_ie_listel_t *
	...
	uint32 pktflag, vndr_ie_t *vndr_iep)
{
	...
	if ((ie_info = (vndr_ie_info_t *)MALLOCZ(osh, info_len)) == NULL) {
		WL_ERROR(("wl: %s(): out of memory\n", __FUNCTION__));
		return NULL;
	}

	ie_info->pktflag = pktflag;
	bcopy((char *)vndr_iep, (char *)&ie_info->vndr_ie_data, ie_len);

	pviel =  wlc_vndr_ie_list_add_info(osh, vndr_ie_listp, ie_info, info_len);
	MFREE(osh, ie_info, info_len);
	return pviel;
}
	...
	const vndr_ie_buf_t *ie_buf, int len)
{
	...
	for (ieindex = 0; ieindex < totie; ieindex++, bufaddr += info_len) {
		...
		if (wlc_vndr_ie_list_add_info(osh, vndr_ie_listp, ie_info, info_len) == NULL) {
	...
	const vndr_ie_buf_t *ie_buf, int len)
{
	...
	for (ieindex = 0; ieindex < totie; ieindex++, bufaddr += info_len) {
		...
		ie_len = (int) (vndr_iep->len + VNDR_IE_HDR_LEN);
		info_len = VNDR_IE_INFO_HDR_LEN + ie_len;
	...
	bool *bcn_upd, bool *prbresp_upd)
{
	...
	*/
	...
	if (!(vndr_ie_bufp = MALLOCZ(osh, vndr_ie_setbuf_len))) {
	...
	vndr_ie_buf_t *ie_buf, int len, uint32 pktflag)
{
	...
	if (totie != 0) {
		...
		copylen += VNDR_IE_INFO_HDR_LEN * totie;
	} else {
...
#define VIE_EA_CMP(ea1, ea2) \
	((((uint16 *)(ea1))[0] ^ ((uint16 *)(ea2))[0]) | \
	 (((uint16 *)(ea1))[1] ^ ((uint16 *)(ea2))[1]) | \
	 (((uint16 *)(ea1))[2] ^ ((uint16 *)(ea2))[2]))

uint8 *
wlc_vndr_ie_list_find_by_sta(vndr_ie_listel_t *vndr_ie_listp, struct ether_addr *da, uint32 pktflag)
{
	vndr_ie_listel_t *curr_list_el;
	...
	while (curr_list_el != NULL) {
		if (!VIE_EA_CMP(&curr_list_el->vndr_ie_infoel.sta_addr, da))  {
