 ...
 *
 *
 * $Id: ceventd.c 809956 2022-03-28 14:37:00Z $
 */
...
ca_config_pktlog_init_out_file(ca_wksp_t *cwksp, const char *nvram_val)
{
	...
	if (nv_out_file_path && (nv_out_file_path[0] != '\0')) {
		CA_DBG("pktlog nvram found \n");
		const int pathLen = strnlen(nv_out_file_path, CA_FILE_PATH_LEN);
		/* -9 to accommodate _bak and .pcap extension */
		if (pathLen > 0 && pathLen < (CA_FILE_PATH_LEN - 9)) {
			ca_snprintf(cwksp->pktlog_out_path, sizeof(cwksp->pktlog_out_path),
				"%s.pcap", nv_out_file_path);
			ca_snprintf(cwksp->pktlog_out_bak_path, sizeof(cwksp->out_bak_path),
				"%s_bak.pcap", nv_out_file_path);
			CA_ERR("path:%s, bak:%s\n", cwksp->pktlog_out_path,
				cwksp->pktlog_out_bak_path);
			cwksp->pktlog_out = fopen(cwksp->pktlog_out_path, "r");
			if (cwksp->pktlog_out != NULL) {
		...
		CA_DBG("Not found pktlog nvram parameters\n");
	}
}
...
{
	int ret = BCME_OK;
	uint32 limit_kb = 0, pktlog_limit_kb = 0;
	...
	limit_kb = ca_get_config_val_uint("ceventd_out_limit_kb", CA_OUT_LIMIT_DEFAULT_KB);
	cwksp->out_limit = (limit_kb >= CA_OUT_LIMIT_MIN_KB && limit_kb <= CA_OUT_LIMIT_MAX_KB) ?
		(limit_kb << 10) : (CA_OUT_LIMIT_DEFAULT_KB << 10);
	CA_DBG("cwksp out limit %ub\n", cwksp->out_limit);

	ca_config_init_out_file(cwksp, "ceventd_out");

	/* pktlog initialization */
	ca_pktlog_init(cwksp);
	pktlog_limit_kb = ca_get_config_val_uint("ceventd_pktlog_out_limit_kb",
		CA_PKTLOG_OUT_LIMIT_DEFAULT_KB);
	cwksp->pktlog_out_limit = (pktlog_limit_kb << 10); /* kB to B */
	cwksp->pktlog_dump_period = ca_get_config_val_uint("ceventd_pktlog_dump_period",
		CA_PKTLOG_DUMP_PERIOD);
...
ca_term_hdlr(int sig)
{
	if (gCwksp) {
		...
		gCwksp->pktlog_dump_triggered = 1;
	}
...
void
ca_pktlog_dump(ca_wksp_t *cwksp, uint64 cur_time)
{
	uint32 t_diff;
	t_diff = (uint32)(cur_time - cwksp->pktlog_dump_ts);
	if (cwksp->pktlog_out == NULL) {
		CA_DBG("Pktlog dump file is not set\n");
		return;
	}

	if ((cwksp->pktlog_out_size >= cwksp->pktlog_out_limit) ||
		(t_diff >= cwksp->pktlog_dump_period) || (cwksp->pktlog_dump_triggered)) {
		/* flush pktlog_h into file */
		ca_write_pktlog(cwksp);

		if (cwksp->pktlog_dump_triggered)
