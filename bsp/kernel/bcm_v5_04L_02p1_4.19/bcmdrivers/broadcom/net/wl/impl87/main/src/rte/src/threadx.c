 ...
 *
 * $Id: threadx.c 798655 2021-05-10 09:17:17Z $
 */
...
#if defined(THREADX_STACK_CHECKING) && !defined(__ARM_ARCH_7A__) && \
	!defined(__ARM_ARCH_7R__)
#error "SVC/IRQ stack checking not supported on this platform"
#endif /* THREADX_STACK_CHECKING && !__ARM_ARCH_7A__ && !__ARM_ARCH_7R__ */

#if defined(THREADX_STACK_CHECKING) && (SVC_STACK_GUARD_SIZE == 0)
#error "SVC/IRQ stack checking needs a guard area"
#endif /* THREADX_STACK_CHECKING && !SVC_STACK_GUARD_SIZE */
...
#ifdef WATCHDOG_DISABLE_FOR_MFGTEST
static bool watchdog_enabled = FALSE;
#else
static bool watchdog_enabled = TRUE;
#endif /* WATCHDOG_DISABLE_FOR_MFGTEST */

#endif /* THREADX_WATCHDOG */

extern uint32 _stackbottom, _stacktop, _stackguard;

/**
 * Check integrity of the SVC mode stack, which used for ThreadX and IRQs.
 *
 * At startup, the stack guard area (_stackguard to _stacktop), as well as the stack area
 * (_stacktop to _stackbottom) is filled with a known pattern. The guard area is checked
 * for this pattern to detect it being overwritten.
 */

void
hnd_thread_check_os_stack(void)
{
#ifdef THREADX_STACK_CHECKING
	uint32* guard = (uint32*)_stackguard;

	ASSERT(_stackguard != _stacktop);	/* No guard area (@see SVC_STACK_GUARD_SIZE) */

	while (guard < (uint32*)_stacktop) {
		if (*guard != TX_STACK_FILL) {
			printf("ThreadX stack guard was overwritten at %p with value %08x\n",
				guard, *guard);
			HND_DIE();
		}
		guard++;
	}
#endif /* THREADX_STACK_CHECKING */
}
...
threads_cmd(void *arg, int argc, char *argv[])
{
	...
	stack_ptr = (uint32*)_stacktop;
	while (stack_ptr < (uint32*)_stackbottom && *stack_ptr == TX_STACK_FILL)
		++stack_ptr;

	printf("   %2u %4u %6u %6u %6u %08x %08x %p %s\n",
		0, 0, _stackbottom - _stacktop, 0, ((uint32*)_stackbottom - stack_ptr) * 4,
		_stacktop, _stackbottom, (void*)0, "ThreadX/IRQ");

	do {
		stack_ptr = thread->tx_thread_stack_start;
		end_ptr   = thread->tx_thread_stack_ptr;
		...
		printf("   %2u %4u %6u %6u %6u %08x %08x %p %s\n",
			...
			thread->tx_thread_priority, (uint)thread->tx_thread_stack_size,
			stack_used, stack_max,
			(uint)thread->tx_thread_stack_start,
			(uint)thread->tx_thread_stack_end + 1,
...
watchdog_timer_cb(hnd_timer_t *t)
{
	...
	/* Run additional periodic checks */
	hnd_thread_check_os_stack();
}
...
{
#if !defined(THREADX_ARM_GT_TIMER)
	threadx_advance_time();
#endif /* !THREADX_ARM_GT_TIMER */
}
...
hnd_thread_sleep(osl_ext_time_us_t us)
{
	...
	if (is_initialized == TRUE) {
		us += threadx_low_power_time_since_update();
