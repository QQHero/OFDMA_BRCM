 ...
 *
 * $Id: bcmstdlib.c 708017 2017-06-29 14:11:45Z $
 */
...
static post_printf_hook bcmstdlib_post_printf_hook = NULL;
#endif
...
vsnprintf(char *buf, size_t bufsize, const char *fmt, va_list ap)
{
	...
	/* XXX Microsoft _vsnprintf() will not null terminate on overflow,
...
#ifdef EVENT_LOG_ROM_PRINTF_MAP
void register_post_printf_hook(post_printf_hook fn)
{
	bcmstdlib_post_printf_hook = fn;
}
void unregister_post_printf_hook(void)
{
	bcmstdlib_post_printf_hook = NULL;
}
post_printf_hook get_current_post_printf_hook(void)
{
	return bcmstdlib_post_printf_hook;
}
#endif
...
#ifdef EVENT_LOG_ROM_PRINTF_MAP
	if (bcmstdlib_post_printf_hook)
	{
		hook_return_value = bcmstdlib_post_printf_hook(buffer, count,
			(void*)__builtin_return_address(0));
	}
#endif

#ifdef EVENT_LOG_ROM_PRINTF_MAP
	/* The hook did not handle the buffer or it did but wants to
	 * print it to the console as well.
	 */
	if (hook_return_value <= 0)
#endif
