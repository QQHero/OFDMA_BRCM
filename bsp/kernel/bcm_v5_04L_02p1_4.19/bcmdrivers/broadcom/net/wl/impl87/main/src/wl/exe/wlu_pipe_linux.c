 ...
 *
 * $Id: wlu_pipe_linux.c 803673 2021-10-05 05:41:32Z $
 */
...
rwl_open_transport(int remote_type, char *port, int ReadTotalTimeout, int debug)
{
	...
	case REMOTE_DONGLE:
		g_irh = rwl_open_serial(remote_type, g_rwl_device_name_serial, g_rwl_device_baud);
		if (g_irh == FAIL) {
			/* Initial port opening settings failed in reboot.
			 * So retry opening the serial port
			 */
			g_irh = rwl_open_serial(remote_type, g_rwl_device_name_serial,
					g_rwl_device_baud);
			if (g_irh == FAIL) {
...
#ifdef RWL_SOCKET
	case REMOTE_SOCKET:
		g_irh = rwl_opensocket(AF_INET, SOCK_STREAM, IPPROTO_TCP);
		if (g_irh == FAIL) {
			DPRINT_ERR(ERR, "\nCan't open socket \n");
			return NULL;
		}
