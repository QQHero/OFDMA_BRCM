...
#define SW_PAGING_LRU
//#define SW_PAGING_LRU_WATCHDOGS
...
#define LRU_TAB_COUNT	2		/** current and last */
#define LRU_TAB_IDX0	0
#define LRU_TAB_IDX1	1
#define LRU_PAGE_IDX_INVALID	((uint32)-1)
...
typedef struct sw_paging
{
	 ...
	 */
#ifdef SW_PAGING_LRU
	uint32 *free_pages[LRU_TAB_COUNT];
	uint8 *lru_epochs[LRU_TAB_COUNT];	/** LRU lookup table: current and last */
	uint8 lru_tab_idx;			/** Current LRU table index: 0 or 1 */
	uint8 lru_page_idx;			/** Index into the current free_pages table */
	uint8 lru_epoch;			/** Current epoch for the LRU algorithm: 0-255 */
	uint8 lru_wd_pages;			/** Number of watchdog pages */
#else
	uint32 *free_pages;
#endif /* SW_PAGING_LRU */
	...
	uint32 page_fault_log_dma_cnt;	/** Number of DMA transfers since logged */
	...
	uint32 dma_xfer_cnt;		/** Total DMA transfer count */
...
/**
 * PCIE IPC handshake
 * - BIND: Setup user SBTOPCIE_USR_SWP and HME_USER_HMOSWP
 * - LINK: Release user SBTOPCIE_USR_SWP and switch to HME/BME based dma
 */
int  sw_paging_bind_pcie(void); /* PCIE IPC Bind phase */
int  sw_paging_link_pcie(void); /* PCIE IPC Link phase */

/** swp debug tool functions */
void hnd_cons_swp_dump(void *arg, int argc, char *argv[]);
void sw_paging_log_start(void);
void sw_paging_log_stop(void);
void sw_paging_log_clear(void);
void sw_paging_log_dump(void);
void sw_paging_log_usage(void);
