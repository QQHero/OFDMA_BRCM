 ...
 *
 * $Id: wbd_master.c 808908 2022-03-02 13:00:24Z $
 */
...
wbd_master_retrieve_blanket_nvram_config(wbd_blanket_master_t *wbd_master)
{
	...
	if (flags & WBD_NVRAM_FLAGS_MASTER_DONT_UPDATE_MEDIA_INFO) {
		wbd_master->flags |= WBD_FLAGS_BKT_DONT_UPDATE_MEDIA_INFO;
	} else {
		wbd_master->flags &= ~WBD_FLAGS_BKT_DONT_UPDATE_MEDIA_INFO;
	}
...
static int
wbd_init_master(wbd_info_t *info, bool reload)
{
	int ret = WBDE_OK;
	WBD_ENTER();
...
/* Initialize the FBT NVRAMs for a map BSS name */
static uint32
wbd_uap_init_fbt_nvram_config(char *prefix, ieee1905_fbt_info_type *fbt_info, int *in_fbt)
{
	char *nvval = NULL;
	char strnvval[WBD_MAX_BUF_256] = {0};
	char new_value[WBD_MAX_BUF_256] = {0};
	char data[WBD_MAX_BUF_256];
	int fbt = 0, fbt_overds;
	uint32 rc_flags = 0, uinval;
	...
	if (!wbd_is_fbt_possible(prefix)) {
		WBD_INFO("Prefix[%s] FBT is not possible\n", prefix);
		goto end;
	}
	...
	if (fbt <= 0) {
		WBD_INFO("Prefix[%s] WBD_FBT[%d] is disabled\n", prefix, fbt);
		wbd_unset_self_fbt_nvrams(prefix);
		goto end;
	}

	/* [6] Get MDID from NVRAM, if not present generate the mdid */
	fbt_info->mdid = wbd_get_mdid(prefix);
	memset(new_value, 0, sizeof(new_value));
	snprintf(new_value, sizeof(new_value), "%d", fbt_info->mdid);
	rc_flags |= blanket_nvram_prefix_match_set(prefix, NVRAM_FBT_MDID, new_value, FALSE);
	...
	if (!nvval) {
		...
		blanket_nvram_prefix_set(prefix, NVRAM_FBT_OVERDS, strnvval);
		rc_flags |= WBD_FLG_NV_COMMIT;
		fbt_overds = WBD_FBT_DEF_OVERDS;
	} else {
		fbt_overds = (int)strtol(nvval, NULL, 0);
	}

	if (fbt_overds) {
		fbt_info->ft_cap_policy |= FBT_MDID_CAP_OVERDS;
	} else {
		fbt_info->ft_cap_policy &= ~FBT_MDID_CAP_OVERDS;
	}

	/* [8] Read fbt reassoc time NVRAM and set default */
	uinval = blanket_get_config_val_uint(prefix, NVRAM_FBT_REASSOC_TIME, 0);
	if (uinval <= 0) {
		memset(strnvval, 0, sizeof(strnvval));
		snprintf(strnvval, sizeof(strnvval), "%d", WBD_FBT_DEF_REASSOC_TIME);
		blanket_nvram_prefix_set(prefix, NVRAM_FBT_REASSOC_TIME, strnvval);
		rc_flags |= WBD_FLG_NV_COMMIT;
		fbt_info->tie_reassoc_interval = WBD_FBT_DEF_REASSOC_TIME;
	} else {
		fbt_info->tie_reassoc_interval = uinval;
	}
...
static void
wbd_master_add_bssinfo_to_controller(bool reload)
{
	...
	/* If reload, remove the previously added BSS info details */
	if (reload) {
	...
	foreach(bss_name, bss_info_names, next_name) {
		...
		nvval = blanket_nvram_prefix_safe_get(prefix, NVRAM_BSS_ENABLED);
		if (strcmp(nvval, "0") == 0) {
			bss.disabled = 1;
		}
		...
		if (I5_IS_BSS_FRONTHAUL(bss.map_flag)) {
			rc_flags |= wbd_uap_init_fbt_nvram_config(prefix, &bss.fbt_info, &fbt);
		}
#endif /* WLHOSTFBT */

		WBD_INFO("prefix: [%s] band: [0x%x] SSID: [%s] AuthType [%d] EncryptType: [%d] "
			"key [%s] map_flag [0x%x] closed [%d] MDID [%d] ft_cap_policy [%d] "
			"tie_reassoc_interval[%d] disabled[%d]\n",
			prefix, bss.band_flag, bss.ssid.SSID,
			bss.AuthType, bss.EncryptType, bss.NetworkKey.key,
			bss.map_flag, bss.Closed, bss.fbt_info.mdid,
			bss.fbt_info.ft_cap_policy, bss.fbt_info.tie_reassoc_interval,
			bss.disabled);

		ieee1905_add_bssto_controller_table(&bss);

		/* If it supports fronthaul BSS, then create a blanket out of that */
		if (!reload && I5_IS_BSS_FRONTHAUL(bss.map_flag)) {
	...
	if (rc_flags & WBD_FLG_NV_COMMIT) {
		nvram_commit();
		WBD_INFO("NVRAMs Modified. nvram_commit\n");
	}

	/* Load the configurations for all the master_info */
	wbd_master_load_all_master_info_configurations(g_wbdinfo);
}
...
wbd_ieee1905_device_init_cb(i5_dm_device_type *i5_device)
{
	...
	wbd_master_create_onboot_channel_scan_req_timer(g_wbdinfo, i5_device->DeviceId);
#endif /* MULTIAPR2 */
...
wbd_ieee1905_device_deinit_cb(i5_dm_device_type *i5_device)
{
	...
	/* Do below activity only when Agent Devices of Repeaters LEAVE */
	if (!(I5_IS_MULTIAP_CONTROLLER(i5_device->flags) ||
		WBD_ONBOOT_NOT_FORCED_ON_CTRLAGENT(i5_device->flags))) {

		/* Average Stats of All Existing Repeaters for same 20 MHz Channel,
		 * excluding this Device, and Send it to ACSD
		 */
		wbd_master_average_and_send_pry_info_to_acsd(i5_device->DeviceId,
			i5_device, &i5_device->stored_chscan_results);
	}
	...
	int if_band)
{
	...
	ieee1905_send_backhaul_sta_capability_query(al_mac);
}

/* Callback from IEEE 1905 module when the controller gets Channel Selection Response */
static void
wbd_ieee1905_recv_chan_selection_response_cb(unsigned char *al_mac, unsigned char *interface_mac,
	t_I5_CHAN_SEL_RESP_CODE chan_sel_resp_code)
{
	WBD_DEBUG("Received callback for Channel Selection Response from Device ["MACDBG"] \n",
		MAC2STRDBG(al_mac));
	wbd_master_process_chan_selection_response_cb(al_mac, interface_mac, chan_sel_resp_code);
}
	...
	for (i = 0; i < cac_list->count; i++) {
		...
		if (cac_compl->status == MAP_CAC_STATUS_RADAR_DETECTED) {
			...
			if (cac_compl->opclass > REGCLASS_5G_40MHZ_LAST) {
				blanket_get_bw_from_rc(cac_compl->opclass, &bw);
				zwdfs_msg.cntrl_chan = wf_chspec_ctlchan(CHBW_CHSPEC2(
					bw, cac_compl->chan, WL_CHANSPEC_BAND_5G));
			} else {
...
wbd_ieee1905_process_cac_status_cb(i5_dm_device_type *device)
{
	...
	wbd_master_set_dfs_channel_inactive(device);
}
#endif /* MULTIAPR2 */
...
wbd_master_register_ieee1905_callbacks(ieee1905_call_bks_t *cbs)
{
	...
	cbs->recv_chan_selection_resp = wbd_ieee1905_recv_chan_selection_response_cb;
...
static int wbd_master_fill_vlan_policy(ieee1905_config *config, uint16 sec_vlan_id)
{
	...
	foreach(name, nvval, next) {
		...
		map = (unsigned char)blanket_get_config_val_uint(prefix,
			NVRAM_MAP, 0);
		/* No trafic seration, if the bss is not Fronthaul */
		if (!(map & IEEE1905_MAP_FLAG_FRONTHAUL)) {
			continue;
		}
...
/* Read the Update Media Info configuration */
static void
wbd_master_read_flag_config(ieee1905_config *config)
{
	uint16 flags = 0;
	WBD_ENTER();

	/* Get NVRAM : wbd_master_flags */
	flags = blanket_get_config_val_uint(NULL, WBD_NVRAM_MASTER_FLAGS, 0);
	if (flags & WBD_NVRAM_FLAGS_MASTER_DONT_UPDATE_MEDIA_INFO) {
		config->flags |= I5_INIT_FLAG_DONT_UPDATE_MEDIA_INFO;
	} else {
		config->flags &= ~I5_INIT_FLAG_DONT_UPDATE_MEDIA_INFO;
	}
	WBD_EXIT();
...
/* Load the master. Allocate, Initialize and read all the configurations. This function will
 * get called for both load(initial bootup) as well for reload(CLI option). In case of reload
 * no need to reallocate any memory. So, make sure whether to call your changes in reload or not
 * whenever modifying this part or the functions called from here
 */
static int
wbd_master_load(bool reload)
{
	int ret = WBDE_OK;
	...
	g_wbdinfo = wbd_info_init(&ret, wbd_get_ginfo(), reload);
	WBD_ASSERT_MSG("wbd_info_init failed: %d (%s)\n", ret, wbderrorstr(ret));

	wbd_master_read_flag_config(&config);
	...
	wbd_master_add_bssinfo_to_controller(reload);

	/* Initialize Master */
	ret = wbd_init_master(g_wbdinfo, reload);
...
wbd_master_reload()
{
	wbd_master_load(TRUE);

	/* send the renew so, that all the agents gets reconfigured */
	ieee1905_send_ap_autoconfig_renew(NULL);
}

/* If vendors want to send/receive additional vendor specific TLVs in any controller messages,
 * they can register their functions here
 */
void
wbd_master_register_vendors()
{
...
main(int argc, char *argv[])
{
	...
	/* Set Process Name */
	WBDSTRNCPY(g_wbd_process_name, "wbd_master", sizeof(g_wbd_process_name));
	...
	ret = wbd_master_load(FALSE);
	WBD_ASSERT_MSG("Master Load Failed failed: %d (%s)\n", ret, wbderrorstr(ret));
