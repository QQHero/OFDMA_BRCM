 ...
 *
 * $Id: wbd_appeventd.c 783904 2020-02-12 08:54:21Z $
 */
...
/* Copy device_id into buf and send as event data. */
void
wbd_appevetd_controller_detected(int evt, struct ether_addr *device_id)
{
	unsigned char app_data[WBD_APPEVENT_BUFSIZE] = {0};
	app_event_wbd_controller_data_t *controller_data = NULL;

	controller_data = (app_event_wbd_controller_data_t*)app_data;
	eacopy(device_id, &(controller_data->device_id));

	app_event_sendup(evt, APP_E_WBD_STATUS_SUCCESS, app_data, sizeof(*controller_data));
}
