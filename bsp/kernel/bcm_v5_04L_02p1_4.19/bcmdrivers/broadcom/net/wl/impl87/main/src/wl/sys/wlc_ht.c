 ...
 *
 * $Id: wlc_ht.c 809472 2022-03-16 09:41:01Z $
 */
    ...
    ht_cap_ie_t *cap_ie, ht_add_ie_t *add_ie, obss_params_t *obss_params)
{
    ...
    ASSERT(scb != NULL);
    scbwas40 = (scb->flags & SCB_IS40) != 0;
    ht_cap = SCB_HT_CAP(scb);
    scb_old_sgi = scb->flags2 & (SCB2_SGI20_CAP | SCB2_SGI40_CAP);
...
wlc_ht_build_cap_ie(wlc_bsscfg_t *cfg, ht_cap_ie_t *cap_ie, uint8 *supp_mcs, bool is2G)
{
    ...
    /* Indicate MIMO PS when RXCHAIN PWRSAVE is enabled */
    if (BSSCFG_AP(cfg) && RXCHAIN_PWRSAVE_ENAB(wlc->ap)) {
        hti->cap_ie.cap &= ~HT_CAP_MIMO_PS_MASK;
        hti->cap_ie.cap |= (wlc->stf->mimops_mode << HT_CAP_MIMO_PS_SHIFT);
    }
