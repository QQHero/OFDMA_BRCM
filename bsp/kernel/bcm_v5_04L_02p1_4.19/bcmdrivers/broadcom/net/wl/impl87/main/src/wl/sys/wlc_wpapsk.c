 ...
 *
 * $Id: wlc_wpapsk.c 781112 2019-11-12 02:08:37Z $
 */
...
#include <wlc_wpa.h>
#endif /* BCMSUP_PSK || WLFBT || BCMAUTH_PSK */
#include <wlc_sup.h>
	...
	uint8 *auth_ies, uint auth_ies_len)
{
	...
	/* get AP's WPA IE */
	if (bcmwpa_is_rsn_auth(wpa->WPA_auth)) {
		auth_wpaie = (uchar *)bcm_find_tlv(auth_ies, auth_ies_len, DOT11_MNG_RSN_ID);
		if (auth_wpaie == NULL) {
...
wlc_wpa_set_ucipher(wpapsk_t *wpa, ushort ucipher, bool wep_ok)
{
	...
	case CRYPTO_ALGO_AES_GCM:
		wpa->ptk_len = AES_GCM_PTK_LEN;
		wpa->tk_len = AES_GCM_TK_LEN;
		wpa->desc = WPA_KEY_DESC_V2;
		break;
	case CRYPTO_ALGO_AES_GCM256:
		wpa->ptk_len = AES_GCM256_PTK_LEN;
		wpa->tk_len = AES_GCM256_TK_LEN;
		wpa->desc = WPA_KEY_DESC_V2;
		break;
