 ...
 *
 * $Id: wlc_twt.c 807220 2022-01-14 12:49:18Z $
 */
...
#define WLC_TWT_MAX_BID0		1		/* Max BID0 for scheduled AP/STA BCAST */
#define WLC_TWT_MAX_TWT_LINKS		(WLC_TWT_MAX_ITWT_SCB + WLC_TWT_MAX_BID0)
#define WLC_TWT_MAX_ITWT_TOTAL		16		/* Max total individual TWTs supported */

#define WLC_TWT_FIRST_SP_AHEAD_NOPM	20000		/* Safety margin for TWT in future NO PM */
#define WLC_TWT_FIRST_SP_AHEAD_PM	30000		/* Safety margin for TWT in future in PM */
#define WLC_TWT_AF_LIFETIME		500000		/* usec */
...
#define WLC_TWT_AF_PEND_UPDATE_ITWT_RSP	8	/* Waiting for iTWT Setup Response AF Ack */
...
#define TSB_USR_TRIG_SHIFT	9
#define TSB_USR_ANCD_SHIFT	10
#define TSB_USR_SPDUR_MASK	0xf000
#define TSB_USR_SPDUR_SHIFT	12
#define TSB_USR_IDX_MASK	0x01ff
#define TSB_USR_IDX_SHIFT	0
...
/* A wake duration unit was added allowing the max wake duration to be higher then 64 msec. At the
 * moment we dont support higher values then 127 * 1024 usec (when using 8msec schedid), so put a
 * cap on it. Ucode support is needed to be able to use higher wake durations. When using the 4msec
 * schedid the max duration is limited to 63 * 1024. The TSB only has 4bits to program duration.
 */
#ifdef WLC_TWT_USE_4MS_SCHEDID
#define WLC_TWT_MAX_WAKE_DURATION	(255 << 8)
#else
#define WLC_TWT_MAX_WAKE_DURATION	(511 << 8)
#endif /* WLC_TWT_USE_4MS_SCHEDID */

/* There is an error in HRT. It is roughly timeout / 170, measured using auto correct algo */
#define WLC_TWT_HRT_ERROR_CORRECTION	170
/* The auto correction updater of HRT has a minumum and maximum */
#define WLC_TWT_MIN_HRT_CORRECTION	5	/* 20% correction on timeout is max */
#define WLC_TWT_MAX_HRT_CORRECTION	400	/* 0.25% correction on timeout is min */
#define WLC_TWT_HRT_DELAY		500	/* Delay HRT always little bit, to let algo work
						 * on sytems that dont have error. (usec)
						 */

#define WLC_TWT_TX_RELEASE_PRETWT	(1 << 0)
#define WLC_TWT_TX_RELEASE_RX		(1 << 1)
#define WLC_TWT_TX_RELEASE		(WLC_TWT_TX_RELEASE_PRETWT | WLC_TWT_TX_RELEASE_RX)

#define WLC_TWT_TXD_SPSTART_DEBUG
#define WLC_TWT_SP_START_DBG_CNT	32
#define WLC_TWT_SCHEDID_TXD_CNT		128
...
typedef struct twt_bcast_desc {
	...
	wl_twt_sdesc_v2_t desc;			/* Bcast TWT parameters */
...
typedef struct twt_indv_desc {
	...
	wl_twt_sdesc_v2_t desc;			/* Individual TWT parameters */
	...
	uint8		tx_release;		/* Bitmap to track tx release reasons */
	bool		tx_closed;		/* Used for all traffic */
	wlc_hrt_to_t	*tx_release_timer;	/* Used for announced traffic */
	wlc_hrt_to_t	*tx_close_timer;	/* Used for all traffic */
...
#if defined(BCMDBG_DUMP_TWT)
	uint32		cnt_pretwt;
	uint32		cnt_rx_outside_sp;
...
typedef struct twt_scb {
	...
	uint		txstatus;	/* Last received txstatus for AF Tx */
	wlc_hrt_to_t	*af_txcomplete;	/* Used for asynch processing AF Tx status processing */
	twt_indv_desc_t	indv[WLC_TWT_MAX_TWT_LINKS];
	twt_indv_desc_t	*bid0;
	twt_indv_desc_t	update;
	...
	bool		first_pretwt;	/* Has there been a pretwt for this SCB */
} twt_scb_t;
...
struct wlc_twt_info {
	...
	bool		expect_pretwt;		/* Waiting for pretwt int (at least one) */
	int		rlm_prog_idx;		/* Next index of RLM to program next schedule */
	int		rlm_read_idx;		/* Next index of RLM to process interrupt for */
	uint16		last_schedid;		/* schedid of last RLM block programmed */
	wlc_hrt_to_t	*rlm_scheduler_timer;	/* Delayed scheduler timer */
	twt_scb_t	*scb_twt_list;
#ifdef WLC_TWT_TXD_SPSTART_DEBUG
	wlc_twt_rlm_t	twtschedblk[AMT_IDX_TWT_RSVD_SIZE];
	uint32		sp_start_dbg[WLC_TWT_SP_START_DBG_CNT];	/* Track last used sp_start */
	uint32		txd_tsf[WLC_TWT_SCHEDID_TXD_CNT];	/* Track last tsf of txd */
	uint32		txd_data[WLC_TWT_SCHEDID_TXD_CNT];	/* Track last used twt data txd */
	uint32		txd_dbg_idx;
	uint32		sp_start_dbg_idx;
#endif /* WLC_TWT_TXD_SPSTART_DEBUG */
	uint32		hrt_error_correction;
	bool		skip_hrt_error_correction;
#ifdef DEBUG_TWT_PS
	...
	uint32		last_pretwt_tsf_h;	/* Debug: PreTWT TSF high */
	uint32		last_pretwt_tsf_l;	/* Debug: PreTWT TSF low */
	uint8		last_tsbidx;		/* Debug: PreTWT TSB Index */
};
...
static void wlc_twt_ie_parse_single_bcast_ie(twt_ie_bcast_t *bcast_ie, wl_twt_sdesc_v2_t *desc);
static int wlc_twt_ap_btwt_config(wlc_twt_info_t *twti, wlc_bsscfg_t *cfg, wl_twt_sdesc_v2_t *desc);
...
static int wlc_twt_sta_itwt_req_send(wlc_twt_info_t *twti, scb_t *scb, wl_twt_sdesc_v2_t *setup);
static int wlc_twt_sta_btwt_req_send(wlc_twt_info_t *twti, scb_t *scb, wl_twt_sdesc_v2_t *setup);
...
static int wlc_twt_itwt_stop(wlc_twt_info_t *twti, scb_t *scb, twt_indv_desc_t *indv,
	bool enter_pm);
static int wlc_twt_itwt_update(wlc_twt_info_t *twti, scb_t *scb, twt_indv_desc_t *indv);
...
static void wlc_twt_itwt_clear_release(void *arg);
static void wlc_twt_join_bid0(wlc_twt_info_t *twti, scb_t *scb);
...
BCMATTACHFN(wlc_twt_attach)(wlc_info_t *wlc)
{
	...
	twti->hrt_error_correction = WLC_TWT_HRT_ERROR_CORRECTION;
	wlc->pub->_twt = TRUE;
	...
	if (twti->itwt_count) {
		...
		while (scb_twt) {
			for (i = 0; i < ARRAYSIZE(scb_twt->indv); i++) {
				...
				indv = &scb_twt->indv[i];
				bcm_bprintf(b, "------------------------\n");
				bcm_bprintf(b, "%s TWT for SCB %p, AID %d, MAC " MACF
					" RLM Id %d\n", i < WLC_TWT_MAX_BID0 ?
					((indv->desc.flow_flags & WL_TWT_FLOW_FLAG_BROADCAST) ?
					"Broadcast" : "Individual") : "BID 0",
				...
				bcm_bprintf(b, "ID=%d Interval=%d/%d (m/e) =%d (usec) Duration=%d "
					...
					indv->desc.wake_duration_unit ?
					1024 * indv->desc.wake_duration :
				...
				bcm_bprintf(b, "Count SP=%d Rx trig=%d (outside SP %d) Paused=%d"
					" Tx pkt=%d\n", indv->cnt_sp, indv->cnt_rx_trig,
					indv->cnt_rx_outside_sp, indv->cnt_paused,
					indv->cnt_tx_pkt);
...
wlc_twt_dump_schedblk(wlc_info_t *wlc)
{
	...
	WL_PRINT(("UCODE engine active = %d, (i)TWT usr count = %d\n",
		twti->rlm_scheduler_active, twti->itwt_count));
	WL_PRINT(("Last pre TWT interrupt: %08x:%08x TSB idx = %d\n", twti->last_pretwt_tsf_h,
		twti->last_pretwt_tsf_l, twti->last_tsbidx));
	for (j = 0; j < ARRAYSIZE(twti->twtschedblk); j++) {
		WL_PRINT(("%d) usr_count = %d, schedid = 0x%04x  %s%s\n", j,
			twti->twtschedblk[j].usr_count, twti->twtschedblk[j].schedid,
			(j == twti->rlm_read_idx) ? "R" : "",
			(j == twti->rlm_prog_idx) ? "W" : ""));
...
#ifdef WLC_TWT_TXD_SPSTART_DEBUG
	for (j = 0; j < WLC_TWT_SP_START_DBG_CNT; j++) {
		i = (j + twti->sp_start_dbg_idx + 1);
		if (i >= WLC_TWT_SP_START_DBG_CNT) {
			i -= WLC_TWT_SP_START_DBG_CNT;
		}
		sp_start_dbg = twti->sp_start_dbg[i];
		if (sp_start_dbg) {
			WL_PRINT(("%2d:0x%04x (%d)\n", ((sp_start_dbg >> 16) & 0xff),
				sp_start_dbg & 0xffff, (sp_start_dbg >> 24)));
		}
	}
	for (j = 0; j < WLC_TWT_SCHEDID_TXD_CNT; j++) {
		i = (j + twti->txd_dbg_idx);
		if (i >= WLC_TWT_SCHEDID_TXD_CNT) {
			i -= WLC_TWT_SCHEDID_TXD_CNT;
		}
		if (twti->txd_tsf[i]) {
			WL_PRINT(("%08x:%2d:0x%04x\n", twti->txd_tsf[i], (twti->txd_data[i] >> 16),
				twti->txd_data[i] & 0xffff));
		}
	}
#endif /* WLC_TWT_TXD_SPSTART_DEBUG */
}
...
wlc_twt_scb_init(void *ctx, scb_t *scb)
{
	...
	*pscb_twt = wlc_scb_sec_cubby_alloc(wlc, scb, wlc_twt_scb_secsz(ctx, scb));
	...
	scb_twt = *pscb_twt;
	if (scb_twt != NULL) {
		scb_twt->bid0 = &scb_twt->indv[WLC_TWT_MAX_ITWT_SCB];

		scb_twt->af_txcomplete = wlc_hrt_alloc_timeout(wlc->hrti);
		if (!scb_twt->af_txcomplete) {
...
wlc_twt_scb_deinit(void *ctx, scb_t *scb)
{
	...
	if (scb_twt->af_txcomplete) {
		wlc_hrt_free_timeout(scb_twt->af_txcomplete);
	}
...
static void wlc_twt_scb_state_upd_cb(void *ctx, scb_state_upd_data_t *notif_data)
{
	...
	if (BSSCFG_AP(scb->bsscfg) && (oldstate & ASSOCIATED) && !SCB_ASSOCIATED(scb)) {
		WL_TWT(("%s: Check for TWT update need as STA is disassociated\n", __FUNCTION__));
		WL_TWT(("%s: SCB %p, AID %d, MAC " MACF "\n", __FUNCTION__,
			scb, SCB_AID(scb), ETHERP_TO_MACF(&scb->ea)));
	...
	if (BSSCFG_AP(scb->bsscfg) && !(oldstate & ASSOCIATED) && SCB_ASSOCIATED(scb)) {
		WL_TWT(("%s: Check for BID0 join as STA is associated\n", __FUNCTION__));
		/* SCB is associated. Check if it is broadcast capable sta and if bcast TWTs
		 * have been configured. Need bid0 and another schedule
		 */
		wlc_twt_join_bid0(twti, scb);
	}
}
	...
	uint16 *rlen, bool set, wlc_if_t *wlcif)
{
	wlc_twt_info_t *twti = ctx;
	wlc_info_t *wlc = twti->wlc;
	wl_twt_setup_v2_t *setup = (wl_twt_setup_v2_t *)params;
	wlc_bsscfg_t *cfg;
	scb_t *scb;
	int err = BCME_OK;
	...
	bool set, wlc_if_t *wlcif)
{
	wlc_twt_info_t *twti = ctx;
	wlc_info_t *wlc = twti->wlc;
	wl_twt_list_v2_t *list_input = (wl_twt_list_v2_t *)params;
	...
	if (list_input->version != WL_TWT_LIST_VER) {
		err = BCME_VERSION;
		goto done;
	}
	...
	for (i = 0; i < ARRAYSIZE(scb_twt->indv); i++) {
		if (scb_twt->indv[i].state != WLC_TWT_STATE_INACTIVE) {
			...
			*desc = scb_twt->indv[i].desc;
			if (WLC_TWT_FID_IS_BID(desc->id)) {
				desc->id = WLC_TWT_FID2BID(desc->id);
			}
			count++;
			size += sizeof(*desc);
		}
	}
	list->indv_count = count - list->bcast_count;
done:
	list->version = WL_TWT_LIST_VER;
...
wlc_twt_bcast_adjust_twt_params(wlc_twt_info_t *twti, twt_bcast_desc_t *bcast)
{
	...
	/* WFA R2 testing is with configuration e=10. Since it doesnt matter much in which way
	 * wake_interval is presented, we switch to e=10. This has to come after doing the
	 * schedid shift, since that is the minimum as determined by ucode capabilities
	 */
	bcast->desc.wake_interval_exponent = WLC_WFA_TSF_SHIFT;
	bcast->desc.wake_interval_mantissa <<= (WLC_SCHEDID_TSF_SHIFT - WLC_WFA_TSF_SHIFT);
 ...
 * If an id is given (not 0) then try to find it. If existing then update... (not supported yet)
 */
static int
wlc_twt_ap_btwt_config(wlc_twt_info_t *twti, wlc_bsscfg_t *cfg, wl_twt_sdesc_v2_t *desc)
{
	wlc_info_t *wlc = twti->wlc;
	int err = BCME_OK;
	twt_bss_t *bss_twt;
	 ...
	 */
	bid = desc->id;
	if ((bid > WL_TWT_ID_BCAST_MAX) && (bid != WL_TWT_ID_BCAST_AUTO)) {
		err = BCME_BADARG;
		goto fail;
	}
	...
	bcast->twt_l = next_beacon & WLC_TSFL_SCHEDID_MASK;
...
wlc_twt_ap_btwt_teardown(wlc_twt_info_t *twti, wlc_bsscfg_t *cfg, uint8 bid)
{
	...
	/* Set teardown count, so it is known it is invalid. */
	if (bcast->state != WLC_TWT_STATE_INACTIVE) {
		bcast->teardown_count = WLC_TWT_TEARDOWN_COUNT;
	} else {
		err = BCME_BADARG;
	}
	/* Send the necessary teardown frames */
	FOREACH_BSS_SCB(wlc->scbstate, &scbiter, cfg, scb) {
		...
		if (bid == 0) {
			/* Does the STA have bid0 or bidx session then use teardown for that bid */
			for (i = 0; i < ARRAYSIZE(scb_twt->indv); i++) {
				indv = &scb_twt->indv[i];
				if ((indv->state == WLC_TWT_STATE_ACTIVE) &&
					(indv->desc.flow_flags & WL_TWT_FLOW_FLAG_BROADCAST)) {
					indv->teardown_retry_cnt = WLC_TWT_TEARDOWN_TRY;
					wlc_twt_teardown_send(twti, cfg, scb, TRUE,
						WLC_TWT_FID2BID(indv->desc.id));
				}
			}
		} else {
			/* Check if there is a session for this BID */
			indv = wlc_twt_find_fid(scb_twt, WLC_TWT_BID2FID(bid));
			if (indv) {
				indv->teardown_retry_cnt = WLC_TWT_TEARDOWN_TRY;
				wlc_twt_teardown_send(twti, cfg, scb, TRUE, bid);
			}
		}
		/* Check teardown handler !! */
	}
...
wlc_twt_bid_first_free(twt_bss_t *bss_twt, uint8 *bid)
{
	...
	/* skip first BID, as BID 0 has special function */
	for (i = WL_TWT_ID_BCAST_MIN + 1; i < ARRAYSIZE(bss_twt->bcast); i++) {
		if (bss_twt->bcast[i].state == WLC_TWT_STATE_INACTIVE) {
			*bid = i;
			return BCME_OK;
		}
...
wlc_twt_calc_bcast_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	/* bid_count identifies how many descriptors are taken. We need at least 2, where bid 0
	 * has to have an active schedule
	 */
	if ((bss_twt->bid_count < 2) || (bss_twt->bcast[0].state == WLC_TWT_STATE_INACTIVE)) {
		return 0;
	}
...
wlc_twt_write_bcast_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	/* bid_count identifies how many descriptors are taken. We need at least 2, where bid 0
	 * has to have an active schedule (reserved)
	 */
	if ((bss_twt->bid_count < 2) || (bss_twt->bcast[0].state == WLC_TWT_STATE_INACTIVE)) {
		return BCME_OK;
	}
 ...
 */
static void
wlc_twt_ie_parse_single_bcast_ie(twt_ie_bcast_t *bcast_ie, wl_twt_sdesc_v2_t *desc)
{
	uint8 *ie_param;
...
/**
 * Support function. Copy bcast descriptor info in indv descriptor and prep TWT. No checking
 * on parameters is done.
 */
static void
wlc_twt_prep_indv_for_bcast(wlc_twt_info_t *twti, scb_t *scb, twt_indv_desc_t *indv, uint8 bid)
{
	wlc_info_t *wlc = twti->wlc;
	twt_bss_t *bss_twt;
	uint32 twt_l, twt_h;
	...
	bss_twt = TWT_BSS(twti, SCB_BSSCFG(scb));
...
/**
 * To be called when BCAST schedules are configure and sta is BCAST capable and is required to
 * follow schedule BID 0
 */
static void
wlc_twt_join_bid0(wlc_twt_info_t *twti, scb_t *scb)
{
	twt_scb_t *scb_twt;
	twt_bss_t *bss_twt;
	twt_indv_desc_t *indv;
	uint8 total_teardown_count;
	twt_bcast_desc_t *bcast;
	uint i;
	...
	/* Count the number of bcast schedules which are being torn down. They cant be joined
	 * anymore, and as such cannot be counted for bid_count.
	 */
	total_teardown_count = 0;
	for (i = WL_TWT_ID_BCAST_MIN; i < ARRAYSIZE(bss_twt->bcast); i++) {
		bcast = &bss_twt->bcast[i];
		if (bcast->teardown_count) {
			total_teardown_count++;
		}
	}

	/* Does STA support BTWT, are there BTWT schedules and is there no active schedule ? */
	if (isclr((uint8 *)&scb_twt->cap, WLC_TWT_CAP_BCAST_TWT_SUPPORT) ||
		((bss_twt->bid_count - total_teardown_count) < 2) ||
		(bss_twt->bcast[0].state == WLC_TWT_STATE_INACTIVE) ||
		(scb_twt->indv_active_cnt)) {
		/* No need to join BID0 */
		return;
	}

	WL_TWT(("%s: SCB %p, AID %d, MAC " MACF " is joining BID 0\n", __FUNCTION__,
		scb, SCB_AID(scb), ETHERP_TO_MACF(&scb->ea)));

	indv = scb_twt->bid0;

	wlc_twt_prep_indv_for_bcast(twti, scb, indv, 0);

	wlc_twt_setup_itwt_complete(twti, scb, indv);
}

/**
 * To be called when possible BID 0 membership is to be terminated: Either when STA is about become
 * member of (non 0) BTWT schedule, or when individual TWT link has been established.
 */
static void
wlc_twt_leave_bid0_membership(wlc_twt_info_t *twti, scb_t *scb)
{
	twt_scb_t *scb_twt;
	twt_indv_desc_t *indv;
	...
	/* Only one bid0 is supported at the moment */
	indv = scb_twt->bid0;
	if (indv->state == WLC_TWT_STATE_ACTIVE) {
		wlc_twt_teardown_itwt(twti, scb, indv, TRUE);
	}
}
 ...
 * Ugliness has come to the specification by putting wake duration bit in the Control field,
 * even though this is irrelevant bit to some TWT AFs, so it is passed on as parameter to this
 * function. Callee needs to know ahead what unit to choose. Default is 0 (256usec). Use
 * use 0 for TWT_CTRL_WAKE_DURATION_256usec or 1 for TWT_CTRL_WAKE_DURATION_1msec
 */
static void*
	...
	uint8 wake_duration_unit)
{
	...
	*body += S1G_AF_TWT_SETUP_TWT_IE_OFF;
	...
	if (wake_duration_unit)
		twt_top->ctrl |= TWT_CTRL_WAKE_DURATION_1msec;
...
wlc_twt_sta_itwt_reject_send(wlc_twt_info_t *twti, scb_t *scb, twt_ie_indv_t *twt_ie,
	uint8 dialog_token, uint8 wake_duration_unit)
{
	int err = BCME_OK;
	void *pkt = NULL;
	uint8 *body;

	pkt = wlc_twt_build_af(scb, dialog_token, &body, TRUE, wake_duration_unit);
	if (!pkt) {
		err = BCME_NOMEM;
		goto fail;
	}
...
wlc_twt_indv_adjust_twt_params(wlc_twt_info_t *twti, scb_t *scb, twt_indv_desc_t *indv)
{
	...
	/* With D5.0 a wake_duration unit got introduced allowing for 1msec uint so 256usec, also
	 * allowing for wake_duration of 255 msec iso 64 msec. Currently we dont support larger
	 * windows, so we recalculate duration here and limit the duration to 64 msec for now.
	 */
	if (indv->desc.wake_duration_unit) {
		wake_duration <<= 2;
		if (wake_duration > WLC_TWT_MAX_WAKE_DURATION) {
			/* Keep wake_duration 1msec unit but use max and multiple of schedid */
			wake_duration = WLC_TWT_MAX_WAKE_DURATION & WLC_TSFL_SCHEDID_MASK;
			indv->desc.wake_duration = wake_duration >> 10;	/* shift over 1msec unit */
		}
	}
	...
	if ((indv->desc.wake_duration != 255) && ((indv->desc.wake_duration != 40) ||
		...
		if (indv->desc.wake_duration_unit) {
			indv->desc.wake_duration = wake_duration >> 10;
		} else {
...
wlc_twt_indv_af_setup_req_recv(wlc_twt_info_t *twti, scb_t *scb, twt_ie_indv_t *twt_ie,
	uint8 setup_command, uint8 dialog_token, uint8 wake_duration_unit, bool in_pm)
{
	wlc_info_t *wlc = twti->wlc;
	twt_scb_t *scb_twt;
	int err = BCME_OK;
	twt_indv_desc_t *indv = NULL;
	twt_indv_desc_t *to_update = NULL;
	uint8 flow_id;
	uint8 i;
	void *pkt;
	uint8 *body;
	...
	flow_id = getbits((uint8 *)&twt_ie->request_type, sizeof(twt_ie->request_type),
		TWT_REQ_TYPE_FLOW_ID_IDX, TWT_REQ_TYPE_FLOW_ID_FSZ);
	for (i = 0; i < WLC_TWT_MAX_ITWT_SCB; i++) {
		if (scb_twt->indv[i].state != WLC_TWT_STATE_INACTIVE) {
			if (scb_twt->indv[i].desc.id == flow_id) {
				/* In D4.0 it was added that on using the same flow_id the reply
				 * should always be accept, where the new parameters are to be
				 * accepted, or where the old parameters are to be returned. It is
				 * tried to Accept the new parameters.
				 */
				WL_TWT(("wl%d: %s: try updating on already in use flow ID\n",
					wlc->pub->unit, __FUNCTION__));
				/* Use the update struct as temporarily storage. */
				indv = &scb_twt->update;

				/* track the indv which is to be updated. */
				to_update = &scb_twt->indv[i];
			}
		} else {
			if (indv == NULL) {
				indv = &scb_twt->indv[i];
	...
	if ((to_update == NULL) && (twti->itwt_count >= WLC_TWT_MAX_ITWT_TOTAL)) {
		err = BCME_NORESOURCE;
		goto fail;
	}
	/* Decode the twt_ie into wl twt setup descriptor */
	wlc_twt_parse_twt_setup_ie(twt_ie, indv);
	/* Store wake duration unit in indv now */
	indv->desc.wake_duration_unit = wake_duration_unit;
	...
	WL_TWT(("%s twt = %08x:%08x (hex), Duration = %d (%s), Interval (m/e) = %d:%d\n",
		__FUNCTION__, indv->twt_h, indv->twt_l, indv->desc.wake_duration,
		indv->desc.wake_duration_unit ? "1024usec" : "256usec",
	...
	if ((setup_command == TWT_SETUP_CMD_REQUEST_TWT) ||
		...
		if ((wake_interval & WLC_TSFL_SCHEDID_MASK_INV) ||
		    ...
		    (wake_duration > WLC_TWT_MAX_WAKE_DURATION) ||
		    (indv->twt_l & WLC_TSFL_SCHEDID_MASK_INV)) {
			if (to_update) {
				/* When a schedule is to be updated, then reply should always be
				 * accept. Handled in default error handler routine.
				 */
				err = BCME_UNSUPPORTED;
				goto fail;
			}
	...
	pkt = wlc_twt_build_af(scb, dialog_token, &body, TRUE, indv->desc.wake_duration_unit);
	if (!pkt) {
		err = BCME_NOMEM;
		goto fail;
	}

	/* Determine the TWT field. This the moment of the first SP where schedule will start.
	 * When this is an update then we use the current next TWT of the SP being scheduled
	 */
	if (to_update) {
		indv->twt_h = to_update->twt_h;
		indv->twt_l = to_update->twt_l;

		goto skip_twt_calculation;
	}
	 ...
	 * matching the TWT and on timeout of frames use retry with larger value. After
	 * discussing this with some of the vendors at Plugfest it was decided that a short time
	 * is fine and a STA will counter for a time in the past. Quick setup times were found more
	 * important so the defines were lowered to get a quick TWT setup.
	 */
	wlc_twt_get_tsf(wlc, SCB_BSSCFG(scb), &tsf_l, &tsf_h);
	if (in_pm) {
...
skip_twt_calculation:

	if (to_update) {
		/* set frametimeout to default value * 4, frame will got out on TWT link */
		wlc_lifetime_set(wlc, pkt, 4 * WLC_TWT_AF_LIFETIME);
	} else {
		/* set frametimeout to default value */
		wlc_lifetime_set(wlc, pkt, WLC_TWT_AF_LIFETIME);
	}
	...
	if (response == TWT_SETUP_CMD_ACCEPT_TWT) {
		...
		if (to_update) {
			scb_twt->af_state = WLC_TWT_AF_PEND_UPDATE_ITWT_RSP;
			scb_twt->af_retry_count = 0;	/* No retries, longer lifetime  */
		} else {
			scb_twt->af_state = WLC_TWT_AF_PEND_SETUP_ITWT_RSP;
			scb_twt->af_retry_count = 2;	/* Two retries */
		}
...
fail:
	if (err != BCME_OK) {
		...
		if (to_update) {
			/* The intention was to update. This failed, the specification tells that
			 * it is required to respond with ACCEPT with the old schedule
			 */

			/* Allocate AF and fill header */
			pkt = wlc_twt_build_af(scb, dialog_token, &body, TRUE,
				to_update->desc.wake_duration_unit);
			if (pkt) {
				/* Fill up AF indvidual part of IE using old schedule */
				wlc_twt_itwt_build_setup_ie((twt_ie_indv_t *)body, to_update,
					TWT_SETUP_CMD_ACCEPT_TWT);
				/* commit for transmission */
				if (WL_TWT_ON()) {
					prhex("iTWT re-Accept", body - S1G_AF_TWT_SETUP_TWT_IE_OFF -
						WLC_TWT_TOP_IE_SZ, S1G_AF_TWT_SETUP_TWT_IE_OFF +
						WLC_TWT_TOP_IE_SZ + WLC_TWT_INDV_IE_SZ);
				}
				(void)wlc_sendmgmt(wlc, pkt, wlc->active_queue, scb);
			}
		} else {
...
wlc_twt_indv_retry_setup_resp(wlc_twt_info_t *twti, scb_t *scb, twt_indv_desc_t *indv)
{
	...
	while ((delta_h == WLC_TWT_TSF_MAX) || (delta_l < WLC_TWT_FIRST_SP_AHEAD_PM)) {
		wlc_uint64_add(&indv->twt_h, &indv->twt_l, 0, wake_interval);
 ...
 * is ongoing then finalize it now. This is STA functionality; not well maintained !!
 */
static int
...
wlc_twt_indv_af_setup_recv(wlc_twt_info_t *twti, scb_t *scb, uint8 *body, uint body_len,
	uint8 dialog_token, uint8 wake_duration_unit, uint16 fc)
{
	uint8 setup_command;
	...
	case TWT_SETUP_CMD_DEMAND_TWT:
		err = wlc_twt_indv_af_setup_req_recv(twti, scb, twt_ie, setup_command,
			dialog_token, wake_duration_unit, fc & FC_PM ? TRUE : FALSE);
		break;
	...
	uint8 dialog_token, uint8 wake_duration_unit)
{
	...
	pkt = wlc_twt_build_af(scb, dialog_token, &body, FALSE, wake_duration_unit);
	if (!pkt) {
		err = BCME_NOMEM;
		goto fail;
	}
...
wlc_twt_bcast_af_setup_req_recv(wlc_twt_info_t *twti, scb_t *scb, twt_ie_bcast_t *twt_ie,
	uint8 setup_command, uint8 dialog_token, uint8 wake_duration_unit)
{
	wlc_info_t *wlc = twti->wlc;
	twt_scb_t *scb_twt;
	twt_bss_t *bss_twt;
	...
	bid = getbits((uint8 *)&twt_ie->bcast_info, sizeof(twt_ie->bcast_info),
		TWT_BCAST_INFO_ID_IDX, TWT_BCAST_INFO_ID_FSZ);
	/* STA cannot become member of bid 0 */
	if (bid == 0) {
		WL_TWT(("%s BTWT CMD_REQUEST bid %d not allowed\n", __FUNCTION__, bid));
		err = BCME_NORESOURCE;
		goto fail;
	}
	...
	wlc_twt_prep_indv_for_bcast(twti, scb, indv, bid);

	/* Allocate AF and fill header */
	pkt = wlc_twt_build_af(scb, dialog_token, &body, FALSE, indv->desc.wake_duration_unit);
	if (!pkt) {
		err = BCME_NOMEM;
		goto fail;
	}

	twt = (indv->twt_l >> WLC_TWT_BTWT_SHIFT) & WLC_TWT_BTWT_MASK;
...
fail:
	if (err != BCME_OK) {
		/* On failure we reply with reject. For this the original twt_ie is used */
		wlc_twt_sta_bcast_reject_send(twti, scb, twt_ie, dialog_token, wake_duration_unit);
 ...
 * (reject), which is to be treated as teardwon request. This is STA functionality; not well
 * maintained !!
 */
static int
	...
	uint8 dialog_token, uint8 wake_duration_unit, uint16 fc)
{
	...
	case TWT_SETUP_CMD_DEMAND_TWT:
		err = wlc_twt_bcast_af_setup_req_recv(twti, scb, twt_ie, setup_command,
			dialog_token, wake_duration_unit);
		break;
	case TWT_SETUP_CMD_REJECT_TWT:
...
wlc_twt_af_setup_recv(wlc_twt_info_t *twti, scb_t *scb, uint8 *body, uint body_len, uint16 fc)
{
	...
	wake_duration_unit = (twt_top->ctrl & TWT_CTRL_WAKE_DURATION_1msec) ? 1 : 0;
 ...
 * callback function WLF2_PCB1_TWT_AF and this function will be called. This is the HRT timer
 * callback version. During processing of tx completes certains actions in tx path (fifo free) can
 * get called. These should not be called on packet callback functions. So the processing of an
 * AF complete has been made asynchronus using a 0-timer.
 */
static void
wlc_twt_af_txcomplete_async(void *arg)
{
	scb_t *scb = (scb_t *)arg;
	...
	case WLC_TWT_AF_PEND_UPDATE_ITWT_RSP: /* iTWT update uses same as setup */
	case WLC_TWT_AF_PEND_SETUP_ITWT_RSP:
		...
		if (!indv) {
			...
			if ((!no_ack) && (af_state == WLC_TWT_AF_PEND_UPDATE_ITWT_RSP)) {
				if (scb_twt->update.desc.id == scb_twt->af_fid) {
					/* Update the schedule, no update in database needed */
					wlc_twt_itwt_update(twti, scb, indv);
					break;
				}
				/* Shouldnt happen, but treat as normal setup */
				ASSERT(0);
			}
			if (no_ack) {
				...
				/* iTWT (can be bast as well !!) setup complete, if the STA is
				 * 'member' of BID 0 then it is time to terminate bid 0
				 * membership now.
				 */
				wlc_twt_leave_bid0_membership(twti, scb);
...
/**
 * Action Frame tx complete callback - used to complete some of the actions. Set the packet
 * callback function WLF2_PCB1_TWT_AF and this function will be called. Use timer callback to
 * do the actual procssing.
 */
static void
wlc_twt_af_txcomplete(wlc_info_t *wlc, void *pkt, uint txstatus)
{
	...
	scb_twt->txstatus = txstatus;

	wlc_hrt_add_timeout(scb_twt->af_txcomplete, 1, wlc_twt_af_txcomplete_async, scb);
}
...
wlc_twt_teardown_retry(wlc_twt_info_t *twti, scb_t *scb, uint8 af_teardown)
{
	...
	if (negotiation == TWT_CTRL_NEGO_INDV_REQ_RSP) {
		...
		if (!indv) {
			...
			WL_TWT(("%s Retrying indv teardown, count=%d\n", __FUNCTION__,
				indv->teardown_retry_cnt));
	...
	if (negotiation == TWT_CTRL_NEGO_BCAST_MGMT) {
		...
		id = (af_teardown & TWT_TEARDOWN_ID_BCAST_MASK) >> TWT_TEARDOWN_ID_SHIFT;
		if (id) {
			indv = wlc_twt_find_fid(scb_twt, WLC_TWT_BID2FID(id));
		} else {
			if (scb_twt->bid0->state == WLC_TWT_STATE_ACTIVE) {
				indv = scb_twt->bid0;
			} else {
				indv = NULL;
			}
		...
		if (!indv) {
			...
			indv->teardown_retry_cnt--;
			WL_TWT(("%s Retrying bcast teardown, count=%d\n", __FUNCTION__,
				indv->teardown_retry_cnt));
			err = wlc_twt_teardown_send(twti, SCB_BSSCFG(scb), scb, TRUE, id);
		}
	}
	...
	uint8 id)
{
	...
	if (bcast) {
		...
		if (id == 0) {
			body[S1G_AF_TWT_TEARDOWN_FLOW_OFF] |= TWT_TEARDOWN_ALL_TWT;
		}
	} else {
		body[S1G_AF_TWT_TEARDOWN_FLOW_OFF] =
	...
	if (WL_TWT_ON()) {
		prhex("TWT Teardown", body, S1G_AF_TWT_TEARDOWN_FLOW_OFF + 1);
	}
	/* commit for transmission */
	if (!wlc_sendmgmt(wlc, pkt, wlc->active_queue, scb)) {
		err = BCME_BUSY;
 ...
 * TWT_INFO_ALL_FLOW_ID then all links will be put in suspend (or resumed).
 */
static int
...
wlc_twti_fid_alloc(wlc_twt_info_t *twti, scb_t *scb, uint8 *flow_id, twt_indv_desc_t **indv)
{
	...
	*indv = NULL;
	...
	if (count == WLC_TWT_MAX_ITWT_SCB) {
		return BCME_NORESOURCE;
	}

#if WLC_TWT_MAX_ITWT_SCB == 1
	*flow_id = 0;
#else
	for (i = 0; i < WLC_TWT_MAX_ITWT_SCB; i++) {
		if ((fid_bmp & (1 << i)) == 0) {
 ...
 */
static int
wlc_twt_sta_itwt_req_send(wlc_twt_info_t *twti, scb_t *scb, wl_twt_sdesc_v2_t *setup)
{
	wlc_info_t *wlc = twti->wlc;
	wlc_bsscfg_t *cfg;
	twt_scb_t *scb_twt;
 ...
 */
static int
wlc_twt_sta_btwt_req_send(wlc_twt_info_t *twti, scb_t *scb, wl_twt_sdesc_v2_t *setup)
{
	wlc_info_t *wlc = twti->wlc;
	wlc_bsscfg_t *cfg;
	twt_scb_t *scb_twt;
...
wlc_twt_itwt_rlm_scheduler_update_trigger(void *arg)
{
	...
	/* Make sure timer was created */
	ASSERT(twti->rlm_scheduler_timer);
		...
		for (i = 0; i < ARRAYSIZE(scb_twt->indv); i++) {
			...
			if (delta_h != 0) {
				if (delta_h != WLC_TWT_TSF_MAX) {
					...
					WL_ERROR(("%s TSF (%08x:%08x) too far in future for TWT"
						" (%08x:%08x)\n", __FUNCTION__, delta_h, delta_l,
						indv->twt_h, indv->twt_l));
					continue;
				}
				/* There is a problem with HRT timer. It is not always accurate.
				 * and differs per chip. The larger the interval the more likely
				 * the timer is too late. We correct that lowering the timer with
				 * correction. This correction is updated here on failure and
				 * the value is lowered to get more correction:
				 * new_timeout -= (new_timeout / correction);
				 * On each failure lower the correction with 25%.
				 */
				if ((twti->hrt_error_correction > WLC_TWT_MIN_HRT_CORRECTION) &&
					(!twti->skip_hrt_error_correction)) {
					WL_TWT(("%s TSF (%08x:%08x) too far in future for TWT"
						" (%08x:%08x), interval=%d (%d => %d)\n",
						__FUNCTION__, delta_h, delta_l, indv->twt_h,
						indv->twt_l, indv->wake_interval,
						twti->hrt_error_correction,
						twti->hrt_error_correction -
						((twti->hrt_error_correction + 3) / 4)));
					twti->hrt_error_correction -=
						((twti->hrt_error_correction + 3) / 4);
					twti->skip_hrt_error_correction = TRUE;
				}
			...
			if (WLC_TSFL_TO_SCHEDID(delta_l - 1) == 0) {
				...
				if ((twti->hrt_error_correction > WLC_TWT_MIN_HRT_CORRECTION) &&
					(!twti->skip_hrt_error_correction)) {
					WL_TWT(("%s TSF (%08x:%08x) to near for TWT"
						" (%08x:%08x), interval=%d, (%d => %d)\n",
						__FUNCTION__, delta_h, delta_l, indv->twt_h,
						indv->twt_l, indv->wake_interval,
						 twti->hrt_error_correction,
...
skip_rlm_programming:
			...
			/* Update the individual twt SP start time */
	...
	for (i = 0; i < ARRAYSIZE(twt_rlm_arr); i++) {
		...
		/* There is a possibility that schedule is using a schedid which already got
		 * programmed. This can happen due to update or suspend/resume, and since we
		 * pre-program up to 2 schedules. We get a new schedule with the same schedid as
		 * the last programmed schedule, just a different user. For obvious reasons ucode
		 * can not handle this so this should be avoided. Ignore the schedule if the
		 * schedid is the same as the last programmed schedid.
		 */
		if (twt_rlm->schedid == twti->last_schedid) {
			WL_TWT(("%s Skipping already used schedid %d (user=%d, i=%d)\n",
				__FUNCTION__, twti->last_schedid, twt_rlm->usr_info[0], i));
			twti->skip_hrt_error_correction = TRUE;
			continue;
		}
		if (i == 0) {
			...
			if ((twti->hrt_error_correction > WLC_TWT_MIN_HRT_CORRECTION) &&
				(!twti->skip_hrt_error_correction)) {
				WL_TWT(("%s Schedule almost missed (%d => %d)\n",
					__FUNCTION__, twti->hrt_error_correction,
					twti->hrt_error_correction -
					((twti->hrt_error_correction + 7) / 8)));
				twti->hrt_error_correction -=
					((twti->hrt_error_correction + 7) / 8);
			}
			twti->skip_hrt_error_correction = TRUE;
		...
		if ((usrnum != 0) || ((twti->rlm_read_idx == twti->rlm_prog_idx) &&
			(twti->expect_pretwt))) {
			...
			WL_TWT(("%s: Lacking RLM space TSF(l)=0x%08x schedid=0x%04x"
				"R/W=%d/%d, expect pretwt=%d\n",
				__FUNCTION__, tsf_l, twt_rlm->schedid, twti->rlm_read_idx,
				twti->rlm_prog_idx, twti->expect_pretwt));
		...
		memcpy(&twti->twtschedblk[twti->rlm_prog_idx], twt_rlm, sizeof(*twt_rlm));
		wlc_getrand(wlc, &random_idx, 1);
		random_idx = random_idx % twt_rlm->usr_count;
		if (random_idx) {
			memcpy(&twti->twtschedblk[twti->rlm_prog_idx].usr_info[0],
				&twt_rlm->usr_info[random_idx],
				sizeof(twt_rlm->usr_info[0]) *
				(twt_rlm->usr_count - random_idx));
			memcpy(&twti->twtschedblk[twti->rlm_prog_idx].usr_info[random_idx],
				&twt_rlm->usr_info[0],
				sizeof(twt_rlm->usr_info[0]) * random_idx);
		}
		wlc_ratelinkmem_update_link_twtschedblk(wlc, twti->rlm_prog_idx,
			(uint8 *)&twti->twtschedblk[twti->rlm_prog_idx],
			sizeof(twti->twtschedblk[twti->rlm_prog_idx]));

		WL_TRACE(("%s: idx [%d %d] tsf 0x%x schedid 0x%x usrcnt %d\n", __FUNCTION__,
			twti->rlm_prog_idx, twti->rlm_read_idx, tsf_l,
			twt_rlm->schedid, twt_rlm->usr_count));

		twti->rlm_prog_idx++;
		if (twti->rlm_prog_idx == AMT_IDX_TWT_RSVD_SIZE) {
			twti->rlm_prog_idx = 0;
		}
		twti->expect_pretwt = TRUE;
		twti->last_schedid = twt_rlm->schedid;
		programmed = TRUE;
	}

	if ((!programmed) && (!twti->skip_hrt_error_correction) &&
		(twti->hrt_error_correction < WLC_TWT_MAX_HRT_CORRECTION)) {
		if (new_timeout > 10000) {
			WL_TWT(("%s Too early, no schedules programmed, new_timeout=%d (%d=>%d)\n",
				__FUNCTION__, new_timeout, twti->hrt_error_correction,
			twti->hrt_error_correction + ((twti->hrt_error_correction + 15) / 16)));
		}
		twti->hrt_error_correction += ((twti->hrt_error_correction + 15) / 16);
		twti->skip_hrt_error_correction = TRUE;
	}
failed_rlm_programming:
	...
	/* On NIC logging can take quite a bit of time. As a result the new timeout is wrong.
	 * When there is any logging (can be anything, like apps, fifo, twt, etc.) then timing
	 * will be very bad. So we need to re-read TSF one more time to correct. On FD this is
	 * not relevant, it will all be in usec timing error.
	 */
	wlc_twt_get_tsf(wlc, wlc->primary_bsscfg, &delta_l, &delta_h);
	wlc_uint64_sub(&delta_h, &delta_l, tsf_h, tsf_l);
	if (new_timeout > delta_l) {
		new_timeout -= delta_l;
	} else {
		new_timeout = 0;
	}
#endif
	/* Make sure timer expires inside, on some systems correction will put it back to much */
	new_timeout += WLC_TWT_HRT_DELAY;
	new_timeout -= (new_timeout / twti->hrt_error_correction);
	twti->skip_hrt_error_correction = FALSE;
...
static void
wlc_twt_itwt_clear_release(void *arg)
{
	twt_indv_desc_t *indv = (twt_indv_desc_t *)arg;
...
wlc_twt_apps_ready_for_twt(wlc_twt_info_t *twti, scb_t *scb)
{
...
#ifdef WLCFP
	/* Trigger CFP path update check. TWT is now set to block CFP */
	...
	/* Evict STA from DL-HEMMU */
	wlc_mutx_twtsta_evict_from_hemmu(wlc->mutx, scb);
	ASSERT(!SCB_HEMMU(scb));
...
wlc_twt_sp_pause(wlc_twt_info_t *twti, scb_t *scb, twt_indv_desc_t *indv, uint16 fc)
{
	...
	WL_TWT(("%s Enter AID %d (%d/%d)\n", __FUNCTION__, SCB_AID(scb), scb_twt->indv_active_cnt,
		scb_twt->indv_paused_cnt));
	...
	if (scb_twt->indv_active_cnt == scb_twt->indv_paused_cnt) {
		...
		wlc_hrt_del_timeout(indv->tx_release_timer);
		indv->tx_release = 0;
	}
}
...
wlc_twt_itwt_start(wlc_twt_info_t *twti, scb_t *scb, twt_indv_desc_t *indv)
{
	...
	wake_interval = indv->desc.wake_interval_mantissa *
		(1 << indv->desc.wake_interval_exponent);
	/* conversion from 256us unit: */
	wake_duration = ((uint32)indv->desc.wake_duration << 8);
	/* if wake_duration_unit is 1 then unit is 1024us, shift 2 more: */
	if (indv->desc.wake_duration_unit) {
		wake_duration <<= 2;
	}
	...
	ASSERT(indv->tx_release_timer == NULL);
	...
	indv->tx_release_timer = wlc_hrt_alloc_timeout(wlc->hrti);
	if (!indv->tx_release_timer) {
		WL_ERROR(("%s Failed to allocate tx_release_timer traffic timer\n", __FUNCTION__));
	...
	if (!twti->rlm_scheduler_timer) {
		...
		if (!twti->rlm_scheduler_timer) {
			...
			wlc_hrt_free_timeout(indv->tx_close_timer);
			wlc_hrt_free_timeout(indv->tx_release_timer);
			indv->tx_close_timer = NULL;
			indv->tx_release_timer = NULL;
			return BCME_NORESOURCE;
		}
...
static int
wlc_twt_itwt_update(wlc_twt_info_t *twti, scb_t *scb, twt_indv_desc_t *indv)
{
	twt_scb_t *scb_twt;
	twt_indv_desc_t update;
	uint32 delta_h, delta_l;
	...
	/* First copy the current configuration in update */
	update = *indv;
	/* Now update the part which is renegotiated */
	update.desc = scb_twt->update.desc;
	update.twt_h = scb_twt->update.twt_h;
	update.twt_l = scb_twt->update.twt_l;

	/* An update on the indv given as parameter was desired. The trickiest part is to update
	 * the existing schedule. There is a small problem though, the current schedule can
	 * still be active and the parameters of that schedule might be needed to for example
	 * calculate release data (wlc_twt_check_for_release_sp). However, the update is complete
	 * and if the wlc_twt_check_for_release_sp is calculating a wrong release (it will possibly
	 * not release while it could) is not worth the extra storage and complexity needed to
	 * do delayed update. Also because a next update might already be started by STA at this
	 * point, it is better to update and set it to be programmed next. TWT has to be at or
	 * later then current TWT, then scheduler will pick it up.
	 */
	update.trigger = (update.desc.flow_flags & WL_TWT_FLOW_FLAG_TRIGGER) ? 1 : 0;
	if (indv->trigger != update.trigger) {
		twti->itwt_trig_count -= indv->trigger;
		twti->itwt_trig_count += update.trigger;

		if (update.trigger && twti->itwt_trig_count == 1) {
			/* first trigger mode itwt should set twt ul params */
			wlc_ulmu_twt_params(twti->wlc->ulmu, TRUE);
		}
		if (indv->trigger && twti->itwt_trig_count == 0) {
			/* last trigger-enabled itwt should set original ul params */
			wlc_ulmu_twt_params(twti->wlc->ulmu, FALSE);
	...
	wake_interval = update.desc.wake_interval_mantissa *
		(1 << update.desc.wake_interval_exponent);
	update.wake_interval = WLC_SCHEDID_TO_TSFL(WLC_TSFL_TO_SCHEDID(wake_interval));

	wake_duration = ((uint32)update.desc.wake_duration << 8);
	/* Adjust interval and duration in case of WFA test, see wlc_twt_indv_adjust_twt_params */
	if (wake_duration & WLC_TSFL_SCHEDID_MASK_INV) {
		wake_duration &= WLC_TSFL_SCHEDID_MASK;
		if (indv->desc.wake_duration == 255) {
	...
	update.wake_duration = WLC_TSFL_TO_SCHEDID(wake_duration);

	delta_h = update.twt_h;
	delta_l = update.twt_l;
	wlc_uint64_sub(&delta_h, &delta_l, indv->twt_h, indv->twt_l);
	if (delta_h != 0) {
		WL_ERROR(("%s Delta TSF (%08x:%08x) negative, adjusting\n", __FUNCTION__,
			delta_h, delta_l));
		if (delta_h != WLC_TWT_TSF_MAX) {
			WL_ERROR(("%s TSF not adjustable, aborting\n", __FUNCTION__));
			return BCME_ERROR;
		} else {
			/* Keep adding interval till delta_h is 0 */
			while (delta_h) {
				wlc_uint64_add(&delta_h, &delta_l, 0, wake_interval);
			}
		}
	}
	update.twt_h = indv->twt_h;
	update.twt_l = indv->twt_l;
	wlc_uint64_add(&update.twt_h, &update.twt_l, delta_h, delta_l);

	WL_TWT(("%s SP interval is %d (usec), wake_duration is %d (usec),"
		" trigger=%d trig count=%d\n", __FUNCTION__,
		wake_interval, wake_duration, update.trigger, twti->itwt_trig_count));
	WL_TWT(("%s TWT update (%08x:%08x) => (%08x:%08x)\n", __FUNCTION__, indv->twt_h,
		indv->twt_l, update.twt_h, update.twt_l));

	/* Finally copy the updated schedule into current schedule */
	*indv = update;
	twti->skip_hrt_error_correction = TRUE;
 ...
 * returns TRUE if SP was released for data. When it is announced traffic then both pretwt and
 * rx packet have to come to start releasing traffic.
 */
static bool
	...
	uint32 tsf_h, uint32 tsf_l, bool pretwt_int)
{
	 ...
	 */
	i = 0;
	while (i < ARRAYSIZE(twti->twtschedblk)) {
		/* when it is pre TWT interrupt then only process the ucode tsb idx (read_idx) */
		if (pretwt_int) {
			twt_rlm = &twti->twtschedblk[twti->rlm_read_idx];
			i = ARRAYSIZE(twti->twtschedblk);
		} else {
		...
		for (usr_idx = 0; usr_idx < twt_rlm->usr_count; usr_idx++) {
			...
			if (sp_offset < sp_dur) {
				 ...
				 * duration. If unnannounced then done, if announced then make sure
				 * both pretwt and rx packet have come.
				 */
				/* Determine time till end of window to be set for close timer(s) */
				...
				if (indv->desc.flow_flags & WL_TWT_FLOW_FLAG_UNANNOUNCED) {
					indv->tx_release = WLC_TWT_TX_RELEASE;
				} else {
					if (pretwt_int) {
						indv->tx_release |= WLC_TWT_TX_RELEASE_PRETWT;
						/* Make sure this flag gets cleared at end of SP
						 * otherwise it remains set and looses its purpose
						 */
						wlc_hrt_add_timeout(indv->tx_release_timer,
							(uint)timeout, wlc_twt_itwt_clear_release,
							indv);
					} else {
						indv->tx_release |= WLC_TWT_TX_RELEASE_RX;
					}
				}
				if (indv->tx_release == WLC_TWT_TX_RELEASE) {
...
#if defined(BCMDBG_DUMP_TWT)
	if (!pretwt_int) {
		/* received Rx outside SP */
		indv->cnt_rx_outside_sp++;
	}
#endif /* BCMDBG_DUMP_TWT */
	return FALSE;
found:
...
wlc_twt_intr(wlc_twt_info_t *twti)
{
	...
	wlc_twt_get_tsf(wlc, wlc->primary_bsscfg, &tsf_l, &tsf_h);
	twti->last_pretwt_tsf_h = tsf_h;
	twti->last_pretwt_tsf_l = tsf_l;
	twti->last_tsbidx = tsbidx;
	...
	/* Go to the next expected for tscb_idx (simplify loop) */
	tsbidx++;
	if (tsbidx == AMT_IDX_TWT_RSVD_SIZE) {
		tsbidx = 0;
	}
	while (twti->rlm_read_idx != tsbidx) {
		...
		/* Go to the next expected read_idx */
		twti->rlm_read_idx++;
		if (twti->rlm_read_idx == AMT_IDX_TWT_RSVD_SIZE) {
			twti->rlm_read_idx = 0;
		}
		/* if read_idx is not tsb_idx at this point, then we are dealing with a late
		 * pretwt interrupt handling multiple tsb_idx updates in one loop. Track that in
		 * counter.
		 */
...
wlc_twt_scb_get_schedid(wlc_twt_info_t *twti, scb_t *scb, uint16 *schedid)
{
	...
	*schedid = scb_twt->cur_sp_start;
...
#ifdef WLC_TWT_TXD_SPSTART_DEBUG
	{
		...
		sp_start_dbg = twti->sp_start_dbg[twti->sp_start_dbg_idx];
		if (((sp_start_dbg & 0xffffff) == ((scb_twt->rlm_id << 16) | *schedid)) &&
			(((sp_start_dbg & 0xff000000) >> 24) < 255)) {
			twti->sp_start_dbg[twti->sp_start_dbg_idx] =
				((((sp_start_dbg & 0xff000000) >> 24) + 1) << 24) |
				(sp_start_dbg & 0xffffff);
		} else {
			twti->sp_start_dbg_idx++;
			if (twti->sp_start_dbg_idx == WLC_TWT_SP_START_DBG_CNT) {
				twti->sp_start_dbg_idx = 0;
			}
			twti->sp_start_dbg[twti->sp_start_dbg_idx] = (1 << 24) |
				(scb_twt->rlm_id << 16) | *schedid;
		}

		wlc_twt_get_tsf(wlc, wlc->primary_bsscfg, &twti->txd_tsf[twti->txd_dbg_idx], NULL);
		twti->txd_data[twti->txd_dbg_idx] = (scb_twt->rlm_id << 16) | *schedid;
		twti->txd_dbg_idx++;
		if (twti->txd_dbg_idx >= WLC_TWT_SCHEDID_TXD_CNT) {
			twti->txd_dbg_idx = 0;
		}
	}
#endif /* WLC_TWT_TXD_SPSTART_DEBUG */
