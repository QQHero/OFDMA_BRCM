 ...
 *
 * $Id: phy_hc.c 789652 2020-08-04 09:39:04Z $
 */
...
int
phy_hc_debugcrash_forcefail(phy_info_t *pi, phy_crash_reason_t dctype)
{
...
int
phy_hc_debugcrash_health_check(phy_info_t *pi, phy_healthcheck_type_t hc)
{
	phy_hc_info_t *hci = pi->hci;
	if (HEALTH_CHECK_ENAB(hci)) {
...
phy_hc_healthcheck(phy_hc_info_t *hci, phy_healthcheck_type_t hc)
{
	...
	if ((hc == PHY_HC_ALL) || (hc == PHY_HC_TEMPSENSE)) {
		...
		if (phy_temp_current_temperature_check(pi->tempi) == BCME_RANGE) {
			temperature = phy_temp_sense_read(pi);
