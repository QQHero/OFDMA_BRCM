 ...
 *
 * $Id: wlc_11u.c 801241 2021-07-19 14:59:53Z $
 */
...
static int wlc_11u_add_default_iw_ie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg);

static uint8 wlc_11u_get_ant_if_set(wlc_info_t *wlc);
static void wlc_11u_update_all_ap_ant(wlc_11u_info_t *m11u,
	wlc_11u_bsscfg_cubby_t *from_cubby);
...
BCMATTACHFN(wlc_11u_attach)(wlc_info_t *wlc)
{
	...
	wlc->pub->_11u = TRUE;
	if (wlc_probresp_register(wlc->mprobresp, m11u, wlc_11u_check_probe_req_iw, FALSE) !=
			BCME_OK) {
		WL_ERROR(("wl%d: %s: wlc_probresp_register() failed\n",
		          wlc->pub->unit, __FUNCTION__));
		goto fail;
	}
	return m11u;
	...
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_SVAL(IOV_IE): {
		...
		ie_setbuf_t *ie_bufp = (ie_setbuf_t *)arg;
		uint8 *ie_data;
		...
		ie_data = (uint8 *)&ie_bufp->ie_buffer.ie_list[0].ie_data;
		bcopy((uint8 *)&ie_bufp->ie_buffer.iecount, &iecount, sizeof(iecount));
...
wlc_11u_upd_ie_ptr(wlc_11u_info_t *m11u, wlc_bsscfg_t *cfg, uint8 ie_type)
{
	...
	case DOT11_MNG_INTERWORKING_ID:
		...
		wlc_11u_update_all_ap_ant(m11u, cubby_11u);
		break;
	case DOT11_MNG_QOS_MAP_ID:
		cubby_11u->qos_ie =
...
wlc_11u_calc_iw_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
	...
	if (cubby_11u->iw_ie != NULL) {
		goto get_iw_ie;
	} else if (isset(cfg->ext_cap, DOT11_EXT_CAP_IW)) {
		wlc_11u_add_default_iw_ie(wlc, cfg);
		goto get_iw_ie;
	} else {
		return 0;
	}
...
get_iw_ie:
	iw_ie = wlc_vndr_ie_find_by_type(wlc->vieli, cfg, DOT11_MNG_INTERWORKING_ID);
...
static int
wlc_11u_add_default_iw_ie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
	wlc_11u_bsscfg_cubby_t *cubby_11u;
	int err = BCME_OK;
	uint8 ie_data[3];
	...
	/* Configure Interworking IE with only ACCESS NETWORK OPTION field, by default
	 * Private network[0] as Access network type in ACCESS NETWORK OPTION field is
	 * configured.
	 *
	 * Interworking IE:
	 * ----------------------------------------------------------------------------
	 *| ID | LEN | ACCESS NETWORK OPTION | VENUE INFO(optional) | HESSID(optional)|
	 * ----------------------------------------------------------------------------
	 *
	 * ACCESS NETWORK OPTIONS:
	 * ----------------------------------------------------------------------------
	 *| Access network type[b0-b3] | Internet[b4] | ASRA[b5] | ESR[b6] | UESA[b7] |
	 * ----------------------------------------------------------------------------
	 */
	memset(ie_data, 0, sizeof(ie_data));
	ie_data[0] = DOT11_MNG_INTERWORKING_ID; /* id */
	ie_data[1] = 1; /* len */
	/* if any ap cfg configured with other ANT from default PRIVATE, use */
	ie_data[2] = wlc_11u_get_ant_if_set(wlc);

	err = wlc_11u_set_ie(wlc->m11u, bsscfg, ie_data, NULL, NULL);
	if (err != BCME_OK) {
		WL_ERROR(("wl%d:%d failed to set default interworking IE\n",
			wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg)));
		return err;
	}

	cubby_11u = WLC_11U_BSSCFG_CUBBY(wlc->m11u, bsscfg);
	cubby_11u->iw_ie = wlc_vndr_ie_find_by_type(wlc->vieli, bsscfg,
		DOT11_MNG_INTERWORKING_ID);

	/* ISSUE: during reboot, operations in IOVAR: Interworking being
	 * reset with bsscfg_init and deinit call.
	 * Enable interworking info explictly as both Interworking capability set
	 * in EXT CAP IE and Interworking IE is already configured.
	 */
	cubby_11u->interworking = TRUE;
	return BCME_OK;
}

static uint8
wlc_11u_get_ant_if_set(wlc_info_t *wlc)
{
	wlc_11u_info_t *m11u = wlc->m11u;
	wlc_11u_bsscfg_cubby_t *cubby_11u = NULL;
	wlc_bsscfg_t *cfg = NULL;
	uint8 i = 0;
	uint8 default_ap_ant = IW_ANT_PRIVATE_NETWORK;
	uint8 apcfg_ant = IW_ANT_PRIVATE_NETWORK;
	uint8 ap_iw_len = 0;
	...
	FOREACH_AP(wlc, i, cfg) {
		...
		apcfg_ant = (cubby_11u->iw_ie[TLV_HDR_LEN+IW_ANT_OFFSET] & IW_ANT_MASK);
		if (apcfg_ant != default_ap_ant) {
			break;
		}
	}
	return apcfg_ant;
...
static void
wlc_11u_update_all_ap_ant(wlc_11u_info_t *m11u, wlc_11u_bsscfg_cubby_t *from_cubby)
{
	wlc_bsscfg_t *cfg = NULL;
	wlc_11u_bsscfg_cubby_t *cubby_11u;
	uint8 i = 0;
	uint8 ap_iw_len = 0;
	uint8 from_cubby_ant = 0;

	if (!from_cubby || !from_cubby->iw_ie) {
		return; /* return silently if either the cubby or iw_ie is null */
	}

	from_cubby_ant = (from_cubby->iw_ie[TLV_HDR_LEN+IW_ANT_OFFSET] & IW_ANT_MASK);

	if (from_cubby_ant == IW_ANT_PRIVATE_NETWORK) {
		/* already set at init time */
		return;
	}

	FOREACH_AP(m11u->wlc, i, cfg) {
		cubby_11u = WLC_11U_BSSCFG_CUBBY(m11u, cfg);
		...
		cubby_11u->iw_ie[TLV_HDR_LEN+IW_ANT_OFFSET] &= ~IW_ANT_MASK;
		cubby_11u->iw_ie[TLV_HDR_LEN+IW_ANT_OFFSET] |= from_cubby_ant;
	}
}
#endif /* WL11U */
