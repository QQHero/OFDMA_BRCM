 ...
 *
 * $Id: hnd_pkt.c 793845 2020-12-11 15:14:45Z $
 */
 ...
 */
static void *
hnd_pkt_alloc_callsite(osl_t *osh, uint len, enum lbuf_type type, void *callsite)
{
	void *pkt;
...
#endif
	pkt = (void *)lb_alloc(len, type, callsite);
...
{
	return hnd_pkt_alloc_callsite(osh, len, type, CALL_SITE);
}
#else
/**
 * Allocates an empty packet in dongle memory.
...
#if defined(BCMDBG_MEMFAIL)
	pkt = hnd_pkt_alloc_callsite(osh, len, type, CALL_SITE);
#else
