 ...
 *
 * $Id: phy.c 809672 2022-03-22 04:44:16Z $
 */
#include <typedefs.h>
#include <bcmdefs.h>
#include <bcmdevs.h>
...
void
wlc_phy_set_shmdefs(phy_info_t *pi, const shmdefs_t *shmdefs)
{
	pi->shmdefs = shmdefs;
}
...
BCMATTACHFN(phy_module_attach)(shared_phy_t *sh, void *regs, int bandtype, char *vars)
{
	...
	uint phyversion;
	osl_t *osh = sh->osh;
	void *pwrdet_ac_5gext = pi ? pi->pwrdet_ac_5gext : NULL;
	int ret = 0;
	...
	if (sflags & (SISF_5G_PHY | SISF_DB_PHY)) {
		...
		if ((pwrdet_ac_5gext == NULL) &&
			((pwrdet_ac_5gext = MALLOCZ(sh->osh, sizeof(srom18_5gext_pwrdet_t)))
				== NULL)) {
			PHY_ERROR(("wl%d: %s: out of memory, malloced %d bytes\n", sh->unit,
		...
		if (pi) {
			pi->pwrdet_ac_5gext = pwrdet_ac_5gext;
		}
	}
#endif /* BAND6G */
	...
	/* default 2G or 5G channel and 20MHz bandwidth */
	pi->bw = WL_CHANSPEC_BW_20;
	pi->radio_chanspec = BAND_2G(bandtype) ? CH20MHZ_CHSPEC(1, WL_CHANSPEC_BAND_2G) :
			CH20MHZ_CHSPEC(36, WL_CHANSPEC_BAND_5G);
	...
	if ((pi->tpci = phy_tpc_attach(pi, bandtype)) == NULL) {
...
#ifdef WLWARENG
	/* Attach WAR-engine check module */
	if ((pi->warengi = phy_wareng_attach(pi)) == NULL) {
		PHY_ERROR(("%s: phy_wareng_attach failed\n", __FUNCTION__));
		goto err;
	}
#endif /* WLWARENG */
...
#ifdef WLC_TXSHAPER
	//if (!IS_SLICE_SCAN(pi)) {
		/* Attach transmitter spectral shapin module */
		if ((pi->txssi = phy_txss_attach(pi)) == NULL) {
			PHY_ERROR(("phy_module_attach: phy_txss_attach failed\n"));
			goto err;
		}
	//}
#endif /* WLC_TXSHAPER */
...
#ifdef WLWARENG
	/* Detach WAR-engine module */
	if (pi->warengi != NULL) {
		phy_wareng_detach(pi->warengi);
	}
#endif /* WLWARENG */

#ifdef WLC_TXSHAPER
	/* Detach transmitter spectral shapin module */
	if (pi->txssi != NULL) {
		MODULE_DETACH(pi->txssi, phy_txss_detach);
	}
#endif /* WLC_TXSHAPER */
...
#if BAND6G
	if (pi->pwrdet_ac_5gext != NULL) {
	    phy_mfree(pi, pi->pwrdet_ac_5gext, sizeof(srom18_5gext_pwrdet_t));
	    pi->pwrdet_ac_5gext = NULL;
	}
#endif /* BAND6G */
...
void
BCMATTACHFN(phy_machwcap_set)(phy_info_t *pi, uint32 machwcap)
{
	pi->sh->machwcap = machwcap;
}

int
phy_bss_init(phy_info_t *pi, bool bonlyap, int noise)
{
	if (bonlyap) {
...
int8
phy_preamble_override_get(phy_info_t *pi)
{
	return pi->n_preamble_override;
}

int
phy_hw_clk_state_upd(phy_info_t *pi, bool newstate)
{
	if (!pi || !pi->sh) {
		return BCME_ERROR;
	}
...
int
phy_hw_state_upd(phy_info_t *pi, bool newstate)
{
	if (!pi || !pi->sh) {
		return BCME_ERROR;
	}
