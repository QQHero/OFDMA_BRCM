 ...
 *
 * $Id: wlc_nar.c 806082 2021-12-13 09:35:18Z $
 *
 */
...
wlc_nar_watchdog(void *handle)
{
	 ...
	 */
	if (nit->nar_enabled && nit->packets_in_queue) {
		struct scb *scb;
		struct scb_iter scbiter;
		...
		FOREACHSCB(nit->wlc->scbstate, &scbiter, scb) {
			...
			if ((cubby) && (cubby->packets_in_queue)) {
				int prec;
...
wlc_nar_transmit_packet(void *handle, struct scb *scb, void *pkt, uint prec)
{
	...
	FOREACH_CHAINED_PKT(pkt, pkt1) {
		...
		/* If the queue is already full, dequeue some packets before queueing this one */
