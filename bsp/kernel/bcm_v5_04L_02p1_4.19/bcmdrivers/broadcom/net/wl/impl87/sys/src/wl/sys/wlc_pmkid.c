 ...
 *
 * $Id: wlc_pmkid.c 783348 2020-01-24 10:25:17Z $
 */
...
static int wlc_pmkid_doiovar(void *handle, uint32 actionid,
...
wlc_pmkid_bss_init(void *ctx, wlc_bsscfg_t *cfg)
{
	...
	bss_pmkid_info_t *bss_pmkid = NULL;
	int err = BCME_OK;

	if (BSSCFG_INFRA_STA(cfg)) {
		bss_pmkid = (bss_pmkid_info_t *)MALLOCZ(pmkid_info->osh, sizeof(bss_pmkid_info_t));
		if (bss_pmkid == NULL) {
...
BCMATTACHFN(wlc_pmkid_attach)(wlc_info_t *wlc)
{
	...
	bsscfg_cubby_params.context = pmkid_info;
	bsscfg_cubby_params.fn_deinit = wlc_pmkid_bss_deinit;
	bsscfg_cubby_params.fn_init = wlc_pmkid_bss_init;

	/* reserve cubby in the bsscfg container for per-bsscfg private data */
	if ((pmkid_info->cfgh = wlc_bsscfg_cubby_reserve_ext(wlc, sizeof(bss_pmkid_info_t *),
...
static int
wlc_pmkid_doiovar(void *handle, uint32 actionid,
	void *params, uint plen, void *arg, uint alen, uint vsize, struct wlc_if *wlcif)
{
	...
	/* Don't process the IOVAR if pmkid cubby is NULL */
...
wlc_pmkid_event(wlc_pmkid_info_t *pmkid_info, wlc_bsscfg_t *cfg)
{
	...
	e = wlc_event_alloc(wlc->eventq, WLC_E_PMKID_CACHE);
	if (e == NULL) {
	...
	e->data = wlc_event_data_alloc(wlc->eventq,
	                               e->event.datalen, WLC_E_PMKID_CACHE);
	if (e->data == NULL) {
	...
	struct ether_addr *bssid, bcm_tlv_t *wpa2_ie, uint8 *fbt_pmkid, uint32 WPA_auth)
{
	...
	if (fbt_pmkid) {
		...
		if (SUP_ENAB(cfg->wlc->pub) && BSS_SUP_ENAB_WPA(cfg->wlc->idsup, cfg) &&
			(npmkid > 0)) {
			/* add just one for now */
			wlc_sup_add_pmkid(cfg->wlc->idsup, cfg, bssid, pmkid[0]);
		}
...
wpa_calc_pmkid_for_okc(wlc_pmkid_info_t *pmkid_info, wlc_bsscfg_t *cfg, struct ether_addr *auth_ea,
	struct ether_addr *sta_ea, uint8 *pmk, uint pmk_len, int *index)
{
	...
	(cfg->current_bss->wpa2.flags & RSN_FLAGS_SHA256)) {
		kdf_calc_pmkid(auth_ea, sta_ea, pmk, pmk_len, pmkid->PMKID);
	}
	else
#endif /* MFP */
	{
		wpa_calc_pmkid(auth_ea, sta_ea, pmk, pmk_len, pmkid->PMKID);
	}
#if defined(BCMINTSUP) && defined(BCMSUP_PSK)
	}
#endif
