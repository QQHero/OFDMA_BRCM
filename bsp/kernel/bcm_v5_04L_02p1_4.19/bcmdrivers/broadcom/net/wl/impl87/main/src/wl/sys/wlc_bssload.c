 ...
 *
 * $Id: wlc_bssload.c 806833 2022-01-05 10:56:00Z $
 */
...
enum {
	...
	IOV_BSSLOAD_FAKE_CHAN_UTIL,	/* set ongoing band steering victim */
	IOV_LAST
};

static const bcm_iovar_t bssload_iovars[] = {
#ifdef WLBSSLOAD
	{"bssload", IOV_BSSLOAD, (0), 0, IOVT_BOOL, 0},
	{"bssload_static", IOV_BSSLOAD_STATIC,
	0, 0, IOVT_BUFFER, OFFSETOF(wl_bssload_static_t, is_static)},
#endif /* WLBSSLOAD */
#if defined(STA) && defined(WLBSSLOAD_REPORT)
	{"bssload_report", IOV_BSSLOAD_REPORT,
	0, 0, IOVT_BUFFER, sizeof(wl_bssload_t),
	},
	{"bssload_report_event", IOV_BSSLOAD_REPORT_EVENT,
	0, 0, IOVT_BUFFER, sizeof(wl_bssload_t),
	},
#endif /* defined(STA) && defined(WLBSSLOAD_REPORT) */
	{"bssload_fake_chan_util", IOV_BSSLOAD_FAKE_CHAN_UTIL,
	(0), 0, IOVT_BUFFER, sizeof(IOVT_UINT8)
	},
	{NULL, 0, 0, 0, 0, 0},
};
...
struct wlc_bssload_info {
	...
	uint8 fake_chan_util;			/* fake channel utilization value */
#endif /* WLBSSLOAD */
};

typedef struct bssload_bss {
...
/* local functions */
/* module */
static int wlc_bssload_doiovar(void *ctx, uint32 actionid,
...
static int wlc_bssload_bss_init(void *ctx, wlc_bsscfg_t *cfg);
static void wlc_bssload_bss_deinit(void *ctx, wlc_bsscfg_t *cfg);
...
BCMATTACHFN(wlc_bssload_attach)(wlc_info_t *wlc)
{
	...
	if (wlc_bsscfg_cubby_reserve(wlc, 0,
		wlc_bssload_bss_init, wlc_bssload_bss_deinit, NULL, mbssload) < 0) {
...
#if defined(STA) && defined(WLBSSLOAD_REPORT)
#if !defined(WLBSSLOAD_REPORT_DISABLED)
	...
	if (BSSLOAD_REPORT_ENAB(wlc->pub)) {
		int err;
		if ((err = wlc_iem_add_parse_fn(wlc->iemi, FC_PROBE_RESP, DOT11_MNG_QBSS_LOAD_ID,
			wlc_bssload_process_bl_ie, wlc)) != BCME_OK) {
			WL_ERROR(("wl%d: %s: add parse_fn failed, err %d\n",
				wlc->pub->unit, __FUNCTION__, err));
			goto fail;
...
static int
wlc_bssload_doiovar(void *ctx, uint32 actionid,
	void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
	...
	case IOV_GVAL(IOV_BSSLOAD_FAKE_CHAN_UTIL):
		if (BSSCFG_AP(bsscfg)) {
			*ret_int_ptr = mbssload->fake_chan_util;
		}
		break;

	case IOV_SVAL(IOV_BSSLOAD_FAKE_CHAN_UTIL):
		if (BSSCFG_AP(bsscfg)) {
			mbssload->fake_chan_util = (uint8)int_val;
		}
		break;

	default:
		WL_ERROR(("wl%d.%d: %s: Unsupported BSSload event: %d\n", WLCWLUNIT(wlc),
			WLC_BSSCFG_IDX(bsscfg), __FUNCTION__, actionid));
		err = BCME_UNSUPPORTED;
		break;
	}
...
static int
wlc_bssload_bss_init(void *ctx, wlc_bsscfg_t *cfg)
{
	wlc_bssload_info_t *mbssload = (wlc_bssload_info_t *)ctx;
	wlc_info_t *wlc = mbssload->wlc;

	/* bss load reporting */
	if (BSSLOAD_REPORT_ENAB(wlc->pub) && !BSSCFG_AP(cfg)) {
		if ((cfg->bssload = (wlc_bssload_t *)
		     MALLOCZ(wlc->osh, sizeof(wlc_bssload_t))) == NULL) {
			WL_ERROR(("wl%d: %s: out of mem, malloced %d bytes\n",
			          wlc->pub->unit, __FUNCTION__, MALLOCED(wlc->osh)));
			return BCME_NOMEM;
		}
...
static void
wlc_bssload_bss_deinit(void *ctx, wlc_bsscfg_t *cfg)
{
	wlc_bssload_info_t *mbssload = (wlc_bssload_info_t *)ctx;
	wlc_info_t *wlc = mbssload->wlc;
	...
	if (BSSLOAD_REPORT_ENAB(wlc->pub)) {
		if (cfg->bssload != NULL) {
			MFREE(wlc->osh, cfg->bssload, sizeof(wlc_bssload_t));
			cfg->bssload = NULL;
		}
	}
}
	...
	cca_ucode_counts_t *cca_stats)
{
	...
	delta.wifi = tmp.wifi - cca_stats->wifi;

	if (delta.usecs == 0)
		return;

	cu = delta.ibss + delta.txdur + delta.obss + delta.noctg + delta.nopkt + delta.wifi;
...
static void wlc_bssload_watchdog(void *ctx)
{
	...
	if (WLBSSLOAD_ENAB(wlc->pub)) {
		...
		if ((chan_util != mbssload->chan_util) && wlc->pub->up) {
			...
			/* update bsscfg with latest bss load */
			FOREACH_BSS(wlc, idx, bsscfg) {
				if (bsscfg->up && (BSSCFG_AP(bsscfg) || BSSCFG_IBSS(bsscfg)))
					bsscfg->current_bss->qbss_load_chan_free =
						WLC_QBSS_LOAD_CHAN_FREE_MAX - chan_util;
			}
		}
	}
}
...
wlc_bssload_write_qbss_load_ie(void *ctx, wlc_iem_build_data_t *data)
{
	...
	if (mbssload->fake_chan_util > chan_util) {
		chan_util = mbssload->fake_chan_util;
	}
