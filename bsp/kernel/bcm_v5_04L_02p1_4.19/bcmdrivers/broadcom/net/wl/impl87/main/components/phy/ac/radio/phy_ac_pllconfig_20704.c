...
{
	/* 20704_procs.tcl r853242: 20704_pll_config */
	uint32 icp_fx;
	uint32 kvco_fx;
	...
	if (lo_freq < 3000) {
		...
		int i;
		const uint16 vco_pulling_channels_63178_47622sliceB[] = {
			6395, 6405, 6595, 6605, 6795, 6805, 6995, 7005, 7195, 7205
		};
		const uint16 vco_pulling_channels_47622sliceA[] = {
		...
		/* 47622 slice A */
		if (pi->sh->sih->enum_base == BCM47622_DEV_A_PHYS_ADDR) {
			vco_pulling_channels = &vco_pulling_channels_47622sliceA[0];
			vco_pulling_sz = ARRAYSIZE(vco_pulling_channels_47622sliceA);
		} else {
			vco_pulling_channels = &vco_pulling_channels_63178_47622sliceB[0];
			vco_pulling_sz = ARRAYSIZE(vco_pulling_channels_63178_47622sliceB);
		}
	...
	MOD_RADIO_PLLREG_20704(pi, PLL_REFDOUBLER1, 0, RefDoubler_pu,
			doubler == DOUBLER_1);
	MOD_RADIO_PLLREG_20704(pi, PLL_CFG2, 0, rfpll_pfd_dbl_en,
			doubler == DOUBLER_2);
	MOD_RADIO_PLLREG_20704(pi, LOGEN_REG0, 0, logen_div3en,
			div3_enable);

	/* ------------------------------------------------------------------- */
	/* PFD Reference Frequency */
	/* <0.8.24> * <0.1.0> --> <0.8.24> */
	pfd_ref_freq_fx = pll->xtal_fx * (doubler == DOUBLER_0 ? 1 : 2);
	/* ------------------------------------------------------------------- */
		...
		uint32 kvco_fx, uint32 icp_fx)
{
	...
	printf("%s = %u\n", "RFP0_pll_refdoubler1.RefDoubler_pu",
			READ_RADIO_PLLREGFLD_20704(pi, PLL_REFDOUBLER1, 0, RefDoubler_pu));
	printf("%s = %u\n", "RFP0_pll_cfg2.rfpll_pfd_dbl_en",
			READ_RADIO_PLLREGFLD_20704(pi, PLL_CFG2, 0, rfpll_pfd_dbl_en));
	printf("%s = %u\n", "RFP0_logen_reg0.logen_div3en",
			READ_RADIO_PLLREGFLD_20704(pi, LOGEN_REG0, 0, logen_div3en));
}
#endif /* DBG_PLL */
