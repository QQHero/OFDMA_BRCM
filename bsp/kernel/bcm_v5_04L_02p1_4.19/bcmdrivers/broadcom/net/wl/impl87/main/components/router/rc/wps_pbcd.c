 ...
 *
 * $Id: wps_pbcd.c 808944 2022-03-03 09:49:12Z $
 */
#include <stdio.h>
#include <unistd.h>
#include <pthread.h>
...
#define PBCD_DPP_DBG_MSG			0x0020
...
#define PBCD_DPP_DBG(fmt, arg...) \
	if (wps_pbcd_msglevel & PBCD_DPP_DBG_MSG) \
		PBCD_PRINT("Debug: ", fmt, ##arg)
...
#define DPP_PSK_PREFIX				"<3>DPP-CONFOBJ-PSK "
#define DPP_PASS_PREFIX				"<3>DPP-CONFOBJ-PASS "
#define DPP_PP_KEY_PREFIX			"<3>DPP-PP-KEY "
...
#if defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK)
static bool wps_pbc_active[MAX_RADIO_NUM] = { FALSE };
#endif /* defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK) */
...
typedef void (*wps_pbcd_reset_wps_ifnames_data)(wps_pbcd_info_t *info);
...
static void wps_pbcd_reset_wps_ifnames_data_handler(wps_pbcd_info_t *info);
...
static void dpp_update_ui_status_update_handler(wps_pbcd_info_t *info, char *nvifname);
static bool wps_pbcd_is_dpp_enabled(char *nvifname);
...
#define PBCD_INFO_WPS_ACTIVE			0x1
#define PBCD_INFO_MAP_BACKHAUL_STA		0x2

#define IS_IFR_MULTIAP_BACKHAUL_STA(f)		((f) & PBCD_INFO_MAP_BACKHAUL_STA)
#define IS_WPS_ACTIVE(f)			((f) & PBCD_INFO_WPS_ACTIVE)
struct wps_pbcd_info {
	...
	void *arg;
	bool skip_ui_update;
	uint32 flags;	/* flags of type PBCD_INFO_XXXX */
};

typedef struct bss_list {
	struct bss_list	*next;
	char		ifname[16];
	uint8		up;
	nbr_rpt_elem_t	self_nbr_rpt;
} bss_list_t;
/*
 * per radio rnr info
 */
typedef struct {
	...
	char oob_nbr_radio_list[WPS_PBCD_MIN_BUF_SIZE];
	struct escan_bss *escan_bss_head;
	bool escan_timer;
	bool is_ap;
	uint8 unit;
	bss_list_t *bss_list; /* each bss ifname + nbr_rpt_elem with self info */
	bool oob_nbr_discovery;
...
static int wps_pbcd_remove_rrm_nbr(char *ifname, const struct ether_addr *bssid);
static int wps_pbcd_init_co_host_nbr_info();
static int wps_pbcd_set_co_host_nbr_info();
static void wps_pbcd_copy_bss_nbr_list(bss_list_t *dst_bss_list, bss_list_t *scr_bss_list);

static void wps_pbcd_refresh_co_host_nbr_info();
static bool wps_pbcd_is_interface_enabled(char *ifname);
// Utility functions
...
static int
wps_pbcd_init_co_host_nbr_info()
{
	...
	if (ifnames[0] == '\0') {
		PBCD_PRINT("Info: ", "did not find any interface to add to ifnames"
			" from lan_ifnames=[%s]\n", lan_ifnames);
		return 0;
	}

	/* prepare nbr info for all ap bss presents */
	for (idx = 0; idx < MAX_RADIO_NUM; idx++) {
		...
		foreach(ifname, ifnames, next) {
			...
			snprintf(ioctl_buf, sizeof(ioctl_buf),  "%s_mode", nv_ifname);
			if (!nvram_match(ioctl_buf, "ap")) {
				PBCD_EVT_DBG("interface[%s] not in ap mode, ignore\n",
					nv_ifname);
				continue;
			}
			...
			memset(ioctl_buf, 0, sizeof(ioctl_buf));
			wps_pbcd_remove_rrm_nbr(ifname, &ether_bcast);
			bss = (bss_list_t*)malloc(sizeof(bss_list_t));
			if (!bss) {
				PBCD_ERR("bss_list_t malloc failed for size[%zu] \n",
					sizeof(bss_list_t));
				goto finish;
			}
			memcpy(bss->ifname, ifname, sizeof(bss->ifname));

			*(uint32*)ioctl_buf = htod32(WLC_IOCTL_MAXLEN);
			err = wl_ioctl(ifname, WLC_GET_BSS_INFO, ioctl_buf, WLC_IOCTL_MAXLEN);
			if (err) {
			...
			memset(&bss->self_nbr_rpt, 0, sizeof(bss->self_nbr_rpt));

			err = wps_pbcd_fill_rrm_nbr_info(ifname, bi, &bss->self_nbr_rpt);
			if (err) {
				PBCD_EVT_DBG("%s failed to fill bss info into rrm_nbr\n", ifname);
				goto finish;
			}

			bss->up = wps_pbcd_is_interface_enabled(bss->ifname);

			bss->next = ri->bss_list;
			ri->bss_list = bss;
		}
	}
...
{
	wps_pbcd_radio_rnr_info_t *ri = NULL, *rj = NULL;
	bss_list_t *ri_bss_list = NULL, *rj_bss_list = NULL;
	int i, j;

	/* populate nbr entries across bands presents */
	for (i = 0; i < MAX_RADIO_NUM; i++) {
		bool no_nbr_radio_list = FALSE;

		ri = &(g_wps_pbcd_info_list->radio_rnr_info[i]);
		if (!ri->enabled) {
			continue;
		}
		if (ri->oob_nbr_radio_list[0] == '\0') {
			no_nbr_radio_list = TRUE;
		}
		ri_bss_list = ri->bss_list;

		for (j = 0; j < MAX_RADIO_NUM; j++) {
			rj = &(g_wps_pbcd_info_list->radio_rnr_info[j]);
			if (!rj->enabled) {
				continue;
			}

			ri_bss_list = ri->bss_list;
			rj_bss_list = rj->bss_list;

			if (no_nbr_radio_list) {
				/* only add 6G bss nbrs to 2G and 5G */
				if ((ri->band_type != WLC_BAND_6G) &&
					(rj->band_type == WLC_BAND_6G)) {
					wps_pbcd_copy_bss_nbr_list(ri_bss_list, rj_bss_list);
				}
			} else {
				if (!find_in_list(ri->oob_nbr_radio_list, rj->ifname)) {
...
#define WLC_RRM_NBR_FLAG_BAND_AUTO	0X00
#define WLC_RRM_NBR_FLAG_BAND_2G	0X01
#define WLC_RRM_NBR_FLAG_BAND_5G	0X02
#define WLC_RRM_NBR_FLAG_BAND_6G	0X04
#define WLC_RRM_NBR_FLAG_BAND_ALL	0X08
#define WLC_RRM_NBR_FLAG_HIDDEN_SSID	0X10
#define WLC_RRM_NBR_FLAG_SAME_SSID	0X20

#define WLC_NBR_DISCOVERY_CAP_6G_UPR			32
...
wps_pbcd_fill_rrm_nbr_info(char *ifname, wl_bss_info_t *bi, nbr_rpt_elem_t *nre)
{
	...
	int closed = 0;
	int band_type = 0;
	int nbr_discovery_cap = 0;
	int rnr_tbtt_len = 0;
	char outbuf[WPS_PBCD_RNR_BUF_SIZE] = {0};
	int mbssid_trans = 0;
	wl_mbssid_t *ptr = NULL;
	wl_mbssid_t mbssid;
	...
	if (dtoh32(bi->version) == WL_BSS_INFO_VERSION) {
		...
		wl_ioctl(ifname, WLC_GET_CLOSED, &closed, sizeof(closed));

		wl_iovar_getint(ifname, "rnr_tbtt_len", &rnr_tbtt_len);
		...
		if (closed) {
			nre->flags |= WLC_RRM_NBR_FLAG_HIDDEN_SSID;
		}
		band_type = wps_pbcd_get_band_type(ifname);

		/* BSS PARAM represents bss parameters subfield of TBTT information field
		 * -------------------------------------------------------------------------------
		 * | reserve |colocated | UPR   | member of ess with|XMIT | MBSSID | Same  | OCT  |
		 * | bit7    |AP        | active| 2/5g colocated AP |bssid|        | ssid  | bit 0|
		 * -------------------------------------------------------------------------------
		 */
		memset(&mbssid, 0, sizeof(mbssid));
		memset(outbuf, 0, sizeof(outbuf));
		wl_iovar_getbuf(ifname, "mbssid", &mbssid, sizeof(mbssid), outbuf, sizeof(outbuf));
		ptr = (wl_mbssid_t*)outbuf;

		if (ptr->version == WL_MBSSID_VER) {
			if (ptr->status == 1) {
				nre->bss_params |= TBTT_INFO_BSS_PARAMS_MBSSID;
				wl_iovar_getint(ifname, "transmit_bss", &mbssid_trans);
				if (mbssid_trans == 1) {
					nre->bss_params |= TBTT_INFO_BSS_PARAMS_TRANSMITTED_BSSID;
				}
			}
		}

		if (band_type == WLC_BAND_6G) {
			nre->flags |= WLC_RRM_NBR_FLAG_BAND_6G;
			wl_iovar_getint(ifname, "nbr_discovery_cap", &nbr_discovery_cap);
			if (nbr_discovery_cap == WLC_NBR_DISCOVERY_CAP_6G_UPR && mbssid_trans) {
				nre->bss_params |= TBTT_INFO_BSS_PARAMS_UPR_ACTIVE; /* UPR ACTIVE */
			}
		} else if (band_type == WLC_BAND_5G) {
			nre->flags |= WLC_RRM_NBR_FLAG_BAND_5G;
		} else if (band_type == WLC_BAND_2G) {
			nre->flags |= WLC_RRM_NBR_FLAG_BAND_2G;
		}

		nre->bss_params |= (TBTT_INFO_BSS_PARAMS_COLOCATED_AP |
				TBTT_INFO_BSS_PARAMS_ESS_MEMBER_WITH_2G_5G);
		nre->tbtt_len = rnr_tbtt_len;
...
static int
wps_pbcd_remove_rrm_nbr(char *ifname, const struct ether_addr *bssid)
{
...
wps_pbcd_escanresults_evt_hndlr(bcm_event_t *be, char *ifname)
{
	...
	if (status == WLC_E_STATUS_PARTIAL) {
		...
		for (idx = 0; idx < MAX_RADIO_NUM; idx++) {
			ri = &g_wps_pbcd_info_list->radio_rnr_info[idx];
			if (!ri->enabled ||(ri->band_type == WLC_BAND_6G) ||
				(ri->bss_info_count >= MAX_OCE_RRM_NBR_ELMNTS)) {
...
{
	bss_list_t *ri_bss_list = NULL;
	int idx = -1;
	struct ether_addr bssid;
	wps_pbcd_radio_rnr_info_t *ri = NULL;
	nbr_rpt_elem_t nbr_elt;
	char ioctl_buf[WLC_IOCTL_MAXLEN] = {0};
	char *primary_ifname_for_event = NULL;
	char nv_ifname[IFNAMSIZ] = {0};
	wl_bss_info_t *bi;
	uint8 band_type = 0;

	band_type = wps_pbcd_get_band_type(evt_ifname);
	...
	for (idx = 0; idx < MAX_RADIO_NUM; idx++) {
		...
		if (ri->band_type == band_type) {
			primary_ifname_for_event = ri->ifname;
			break;
		}
	}

	if (!primary_ifname_for_event) {
		/* no radio holding this bss, return */
		return;
	}
	...
	if (ntoh16(event->flags) & WLC_EVENT_MSG_LINK) {
		memset(ioctl_buf, 0, sizeof(ioctl_buf));
		if (osifname_to_nvifname(evt_ifname, nv_ifname, IFNAMSIZ) < 0) {
			PBCD_EVT_DBG("%s: error converting osifname to nvifname()\n",
				evt_ifname);
			return;
		}
	...
	for (idx = 0; idx < MAX_RADIO_NUM; idx++) {
		...
		if (ri->oob_nbr_radio_list[0] == '\0') {
			/* no nbr list present for radio, only update 6G's info if present
			 * to 2G and 5G
			 */
			if (ri->band_type == WLC_BAND_6G) {
				continue;
			...
			if (band_type != WLC_BAND_6G) {
				/* this is not required, as for no_nbr_radio_list
				 * radio contains only 6G as nbrs
				 */
				continue;
			}
			ri_bss_list = ri->bss_list;
			while (ri_bss_list) {
				if (!strncmp(evt_ifname, ri_bss_list->ifname, IFNAMSIZ)) {
					ri_bss_list = ri_bss_list->next;
					continue;
				}

				if (ntoh16(event->flags) & WLC_EVENT_MSG_LINK) {
					wps_pbcd_add_rrm_nbr(ri_bss_list->ifname, &nbr_elt);
				} else {
					wps_pbcd_remove_rrm_nbr(ri_bss_list->ifname, &bssid);
				}
				ri_bss_list = ri_bss_list->next;
			}
		} else {
			if (!find_in_list(ri->oob_nbr_radio_list, primary_ifname_for_event)) {
				PBCD_EVT_DBG("%s not in oob_nbr_radio_list[%s], skip\n",
					primary_ifname_for_event, ri->oob_nbr_radio_list);
				continue;
			}
			ri_bss_list = ri->bss_list;
			while (ri_bss_list) {
	...
	/* link up/down event on 6G link changes the TxBSSID ownership, again update
	 * bss_list->self_nbr_report and propagate to lower bands as nbr list
	 */
	wps_pbcd_refresh_co_host_nbr_info();
	wps_pbcd_set_co_host_nbr_info();
}

#ifdef MULTIAP
/* Finds ie matching type, oui from the tlv buffer */
bcm_tlv_t *
wps_pbcd_wl_find_ie(uint8 id, void *tlvs, int tlvs_len, const char *voui, uint8 *type, int type_len)
{
	bcm_tlv_t *ie;
	uint8 ie_len;
...
static void
wps_pbcd_assoc_evt_hndlr(bcm_event_t *pvt_data, char *ifname)
{
	uint8 *data, *ies;
	uint32 datalen, ies_len;
	uint32 status;
	multiap_ie_t *map_ie;
	uint8 ie_type = WFA_OUI_TYPE_MULTIAP;
	bool map_fh_bss = FALSE;

	wps_pbcd_info_t *pbcd_info = wps_pbcd_get_info_obj(ifname);
	if (!pbcd_info) {
		PBCD_EVT_DBG("%s did not find pbcd info obj \n", ifname);
		return;
	}

	status = ntoh32(pvt_data->event.status);
	/*
	 * Skip further (re)assoc responce frame processing if status is not success,
	 * or wps is not started or interface is not backhaul sta.
	 */
	if (status != WLC_E_STATUS_SUCCESS || !IS_WPS_ACTIVE(pbcd_info->flags) ||
		!IS_IFR_MULTIAP_BACKHAUL_STA(pbcd_info->flags)) {
		PBCD_EVT_DBG("%s skipped  (re)assoc response processing status %d flags %x\n",
			ifname, status, pbcd_info->flags);
		return;
	}
	...
	map_ie = (multiap_ie_t*)wps_pbcd_wl_find_ie(DOT11_MNG_VS_ID, ies, ies_len,
		WFA_OUI, &ie_type, 1);
	if (map_ie && map_ie->len > MAP_IE_FIXED_LEN) {
		int tlv_len = 0;

		bcm_tlv_t *tlv = NULL;
		tlv_len = map_ie->len;
		for (tlv = (bcm_tlv_t *)map_ie->attr; tlv != NULL; tlv = (bcm_tlv_t *)bcm_next_tlv
			((bcm_tlv_t *)tlv, &tlv_len)) {
			if (tlv->id  == MAP_EXT_ATTR) {
				multiap_ext_attr_t *attr = (multiap_ext_attr_t*)tlv;
				if (attr->attr_val & MAP_EXT_ATTR_FRNTHAUL_BSS) {
					map_fh_bss = TRUE;
					break;
				}
	...
	if (!map_fh_bss) {
		PBCD_EVT_DBG("%s in the peer device (re)assoc response did not find multiap ie "
			"with fronthaul bit being set skipping the multiap onboarding\n", ifname);
...
wps_pbcd_drv_events_cb(bcm_usched_handle *hdl, void *arg, bcm_usched_fds_entry_t *entry)
{
	...
	case WLC_E_ASSOC:
	case WLC_E_REASSOC:
		wps_pbcd_assoc_evt_hndlr(dpkt, ifname);
		break;
#endif	/* MULTIAP */
...
wps_pbcd_led_and_ui_status_update_handler(wps_pbcd_info_t *pbcd_info)
{
	...
	get_ifname_unit(pbcd_info->bss.ifname, &unit, &subunit);
#endif /* defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK) */
...
#if defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK)
		status = "START";
		wps_pbc_active[unit] = TRUE;
#endif /* defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK) */
		pbcd_info->flags |= PBCD_INFO_WPS_ACTIVE;
	} else if (strstr(data, WPS_EVENT_SUCCESS)) {	// Wps is successful
		idx = WLIF_WPS_UI_OK;
		blink_type = WLIF_WPS_BLINKTYPE_STOP;
#if defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK)
		if (wps_pbc_active[unit]) {
			status = "SUCCESS";
			wps_pbc_active[unit] = FALSE;
		}
#endif /* defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK) */
...
#if defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK)
	if (status) {
		FILE *fp = fopen("/rdklogs/logs/wifihealth.txt", "a+");
		if (fp) {
			char date[30];
			time_t raw_time;

			time(&raw_time);
			strftime(date, 30, "%Y-%m-%d %H:%M:%S", localtime(&raw_time));
			fprintf(fp, "%s RDKB_WPS_PBC_%d %s\n", date, unit + 1, status);
			fclose(fp);
		}
	}
#endif /* defined(CMWIFI_RDKB) || defined(BCA_CPEROUTER_RDK) */
...
static void
dpp_update_ui_status_update_handler(wps_pbcd_info_t *info, char *nvifname)
{
	char *data = info->reply_buf;
	DPP_UI_SCSTATE idx = DPP_UI_INIT;

	if (strstr(data, DPP_EVENT_AUTH_INIT_FAILED) || strstr(data, DPP_EVENT_FAIL)) {
		idx = DPP_UI_STATUS_AUTH_FAILURE;
	} else if (strstr(data, DPP_EVENT_CONF_FAILED)) {
		idx = DPP_UI_STATUS_CONFIGURE_FAILURE;
	} else if (strstr(data, DPP_EVENT_CONF_SENT)) {
		idx = DPP_UI_STATUS_CONFIG_SUCCESS;
	/* Update idx for failure cases of EVENT_RX also */
	} else if (strstr(data, DPP_EVENT_NOT_COMPATIBLE) || (strstr(data, DPP_EVENT_RX) &&
			strstr(data, "ignore="))) {
		idx = DPP_UI_STATUS_NOT_COMPATIBLE;
	} else if (strstr(data, AP_STA_CONNECTED)) {
		idx = DPP_UI_ASSOCIATED;
	} else if (strstr(data, AP_STA_DISCONNECTED)) {
		idx = DPP_UI_DISASSOCIATED;
	} else if (strstr(data, DPP_EVENT_RESPONSE_PENDING) ||
			strstr(data, DPP_EVENT_SCAN_PEER_QR_CODE)) {
		idx = DPP_UI_STATUS_RESPONSE_PENDING;
	}

	wl_wlif_update_dpp_ui(idx, nvifname);
}

// Resets the wps_data ifname when wps session either fails or succeeds
static void
wps_pbcd_reset_wps_ifnames_data_handler(wps_pbcd_info_t *info)
{
	char *data = info->reply_buf;
...
non_wps_process_hostapd_wpa_supplicant_ctrl_events(wps_pbcd_info_t *pbcd_info)
{
	...
	tmp = strstr(data, AP_STA_POSSIBLE_PSK_MISMATCH);
	if (tmp) {
		char *end = NULL;
		int i = 0;
		scb_val_t scb_val;
		...
		end = data + pbcd_info->reply_len;

		/*
		* sample of event:
		* <3>AP-STA-POSSIBLE-PSK-MISMATCH 40:9c:28:02:0f:9b
		*/
		tmp += strlen(AP_STA_POSSIBLE_PSK_MISMATCH);
		while (tmp < end) {
			char strbuf[3] = {0};
			...
			WLIF_STRNCPY(strbuf, tmp, sizeof(strbuf));
			scb_val.ea.octet[i++] = (uint8)strtoul(strbuf, NULL, 16);
			tmp += 2;
		}
		scb_val.val = DOT11_RC_MIC_FAILURE;
		err = wl_iovar_set(pbcd_info->bss.ifname, "nas_notify_event",
				&scb_val, sizeof(scb_val));
		if (err) {
			PBCD_ERR("set nas_notify_event iovar error=%d, iface=%s"
				" mac=%02x:%02x:%02x:%02x:%02x:%02x\n",
				err, pbcd_info->bss.ifname,
...
static int hostapd_wpa_supplicant_dpp_ctrl_events(wps_pbcd_info_t *pbcd_info)
{
	...
	char dpp_pp_key[WPS_PBCD_MAX_BUF] = {0};
	char dpp_pass[WPS_PBCD_MAX_BUF] = {0};
	char tmp[WPS_PBCD_NVNAME_BUF_SIZE] = {0}, tmp_boot_strap_key[WPS_PBCD_MAX_BUF_SIZE] = {0};
	bool ap = 0;

	PBCD_DPP_DBG("\nInfo: wps_pbcd %s %d received on %s len [%zu] and data [%s] \n", __func__,
		__LINE__, pbcd_info->wpa_ctrl_dir, pbcd_info->reply_len, pbcd_info->reply_buf);

	if (strstr(pbcd_info->reply_buf, DPP_EVENT_RESPONSE_PENDING)) {
		tmp_ptr = strstr(pbcd_info->reply_buf, "DPP:");
		snprintf(tmp_boot_strap_key, sizeof(tmp_boot_strap_key), "%s_tmp_boot_strap_key",
				pbcd_info->bss.nvifname);
		nvram_set(tmp_boot_strap_key, tmp_ptr);
	}

	/* Don't update dpp_status for TX,RX and TX_STATUS events.
	 * Consider updating dpp_status for failure cases of EVENT_RX also
	 */
	if (!(strstr(pbcd_info->reply_buf, DPP_EVENT_TX) || (strstr(pbcd_info->reply_buf,
			DPP_EVENT_RX) && !strstr(pbcd_info->reply_buf, "ignore=")) ||
			strstr(pbcd_info->reply_buf, DPP_EVENT_TX_STATUS))) {
		dpp_update_ui_status_update_handler(pbcd_info, pbcd_info->bss.nvifname);
	}
	...
	if ((cptr = strstr(pbcd_info->reply_buf, DPP_EVENT_CONNECTOR))) {
	...
	} else if (!ap && (cptr = strstr(pbcd_info->reply_buf, DPP_EVENT_PP_KEY))) {
		strncpy(dpp_pp_key, cptr + strlen(DPP_PP_KEY_PREFIX) - 3 /* '<3>' */,
				pbcd_info->reply_len - strlen(DPP_PP_KEY_PREFIX));
		dpp_pp_key[pbcd_info->reply_len - strlen(DPP_PP_KEY_PREFIX)] = '\0';
		strncpy(pbcd_info->dpp_creds.dpp_pp_key, dpp_pp_key,
				pbcd_info->reply_len - strlen(DPP_PP_KEY_PREFIX));
	} else if ((cptr = strstr(pbcd_info->reply_buf, DPP_EVENT_CONFOBJ_PASS))) {
		strncpy(dpp_pass, cptr + strlen(DPP_PASS_PREFIX) - 3 /* '<3>' */,
				pbcd_info->reply_len - strlen(DPP_PASS_PREFIX));
		dpp_pass[pbcd_info->reply_len - strlen(DPP_PASS_PREFIX)] = '\0';
		strncpy(pbcd_info->dpp_creds.dpp_pass, dpp_pass,
				pbcd_info->reply_len - strlen(DPP_PASS_PREFIX));
		wl_wlif_hexstr2ascii((const char *)dpp_pass, pbcd_info->dpp_creds.dpp_psk,
				WLIF_PSK_MAX_SZ);
		if (ap) {
			memset(cmd, 0, sizeof(cmd));
			snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set wpa_passphrase %s",
					pbcd_info->bss.ifname, pbcd_info->dpp_creds.dpp_psk);
			system(cmd);
		}
		...
		memset(pbcd_info->dpp_creds.akm, 0, sizeof(pbcd_info->dpp_creds.akm));
		strncpy(pbcd_info->dpp_creds.akm, dpp_akm,
				pbcd_info->reply_len - strlen(DPP_AKM_PREFIX));

		/* Set security parameters based on akm. */
		if (ap && ((strcmp(dpp_akm, DPP_AKM) == 0)||
			(strcmp(dpp_akm, PSK_AKM) == 0) ||
			(strcmp(dpp_akm, SAE_AKM) == 0) ||
			(strcmp(dpp_akm, PSK_SAE_AKM) == 0) ||
			(strcmp(dpp_akm, DPP_SAE_AKM) == 0) ||
			(strcmp(dpp_akm, DPP_PSK_SAE_AKM) == 0))) {
			memset(cmd, 0, sizeof(cmd));
			snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set wpa 2",
					pbcd_info->bss.ifname);
			system(cmd);
			PBCD_DPP_DBG("\n %s: %d, cmd = %s\n", __func__, __LINE__, cmd);
			if (strcmp(dpp_akm, DPP_PSK_SAE_AKM) == 0) {
				memset(cmd, 0, sizeof(cmd));
				snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set "
					"wpa_key_mgmt \"DPP WPA-PSK WPA-PSK-SHA256 SAE\"",
					pbcd_info->bss.ifname);
				system(cmd);
				PBCD_DPP_DBG("\n %s: %d, cmd = %s\n", __func__, __LINE__, cmd);
				memset(cmd, 0, sizeof(cmd));
				snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set ieee80211w 1",
					pbcd_info->bss.ifname);
				system(cmd);
				PBCD_DPP_DBG("\n %s: %d, cmd = %s\n", __func__, __LINE__, cmd);
			} else if (strcmp(dpp_akm, DPP_AKM) == 0) {
				memset(cmd, 0, sizeof(cmd));
				...
				system(cmd);
				PBCD_DPP_DBG("\n %s: %d, cmd = %s\n", __func__, __LINE__, cmd);
			} else if (strcmp(dpp_akm, DPP_SAE_AKM) == 0) {
				memset(cmd, 0, sizeof(cmd));
				snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set "
				...
				system(cmd);
				PBCD_DPP_DBG("\n %s: %d, cmd = %s\n", __func__, __LINE__, cmd);
			} else if (strcmp(dpp_akm, PSK_SAE_AKM) == 0) {
				memset(cmd, 0, sizeof(cmd));
				snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set "
				...
				system(cmd);
				PBCD_DPP_DBG("\n %s: %d, cmd = %s\n", __func__, __LINE__, cmd);
			} else if (strcmp(dpp_akm, PSK_AKM) == 0) {
				memset(cmd, 0, sizeof(cmd));
				snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set "
				...
				system(cmd);
				PBCD_DPP_DBG("\n %s: %d, cmd = %s\n", __func__, __LINE__, cmd);
			} else if (strcmp(dpp_akm, SAE_AKM) == 0) {
				memset(cmd, 0, sizeof(cmd));
				snprintf(cmd, sizeof(cmd), "hostapd_cli -i %s set wpa_key_mgmt SAE",
					pbcd_info->bss.ifname);
				system(cmd);
				PBCD_DPP_DBG("\n %s: %d, cmd = %s\n", __func__, __LINE__, cmd);
				memset(cmd, 0, sizeof(cmd));
...
wps_pbcd_process_hostapd_wpasupplicant_ctrl_events(wps_pbcd_info_t *pbcd_info)
{
	...
	pbcd_info->reset_wps_ifnames_data_handler(pbcd_info);
...
wps_pbcd_get_wl_radio_iflist(char *wl_radio_iflist, int rlistsz)
{
	...
	get_all_lanifnames_list(ifnames_list, rlistsz);
...
wps_pbcd_init_radio_rnr_info(wps_pbcd_info_list_t  *g_wpi_list)
{
	...
	rlistsz = get_all_lanifnames_listsz();
	...
	foreach(ifname, wl_radio_iflist, next) {
		...
		ri->oob_nbr_discovery = (oob_nbr_discovery & 0x01) ? TRUE : FALSE;
		...
		/* update radio list to populate rnr entries */
		memset(nv_name, 0, sizeof(nv_name));
		snprintf(nv_name, sizeof(nv_name), "%s_oob_nbr_radio_list", nv_ifname);

		ret = nvram_safe_get(nv_name);
		if (ret[0] == '\0') {
			PBCD_EVT_DBG("%s: empty oob_nbr_radio iflist, skip\n", ri->ifname);
		} else {
			foreach(tmp_ifname, ret, tmp_next) {
				if (add_to_list(tmp_ifname, ri->oob_nbr_radio_list,
					sizeof(ri->oob_nbr_radio_list)) != 0) {
					PBCD_ERR("error adding %s to rnr_src_list of %s\n",
						tmp_ifname, ri->ifname);
				}
			}
		}
		PBCD_EVT_DBG("%s: oob_nbr_radio_list: \"%s\"\n", ri->ifname,
			ri->oob_nbr_radio_list);
	}
...
wps_pbcd_fill_info_objects_list(wps_pbcd_info_list_t *list)
{
	...
	int unit = -1;
	bool is_dpp_enabled = FALSE;

	ifnames_size = get_all_lanifnames_listsz();
	...
	get_all_lanifnames_list(ifnames, ifnames_size);
	...
	foreach(ifname, ifnames, next) {
		...
		is_dpp_enabled = wps_pbcd_is_dpp_enabled(nvifname);
		// Check if wps is possible on the interface or DPP is enable
		if ((wps_pbcd_is_wps_possible_on_interface(ifname, nvifname, unit) == FALSE) &&
			(is_dpp_enabled == FALSE)) {
			continue;
		}
		...
		if (nvram_match(tmp, "ap")) {
			...
			if (wl_wlif_is_map_onboarding(nvifname)) {
				...
				pbcd_info->flags |= PBCD_INFO_MAP_BACKHAUL_STA;
			} else
#endif	/* MULTIAP */
			{
				pbcd_info->apply_network_settings = wl_wlif_apply_creds;
		...
		if (is_dpp_enabled) {
			pbcd_info->dpp_wpa_ctrl_events_handler =
				hostapd_wpa_supplicant_dpp_ctrl_events;
		}
...
wps_pbcd_free_info_objects_list(wps_pbcd_info_list_t *list)
{
	...
	if (list) {
		...
		for (idx = 0; idx < MAX_RADIO_NUM; idx++) {
			...
			/* release bss list holding nbr info */
			bss = rnr_info->bss_list;
			while (bss) {
				iter = bss->next;
				free(bss);
				bss = iter;
...
// Returns TRUE if DPP is enabled on the input interface
static bool
wps_pbcd_is_dpp_enabled(char *nvifname)
{
	char nv_name[WPS_PBCD_NVNAME_BUF_SIZE] = {0};
	snprintf(nv_name, sizeof(nv_name), "%s_dpp_role", nvifname);
	if (nvram_match(nv_name, "configurator") ||
		nvram_match(nv_name, "enrollee")) {
		return TRUE;
	}
	return FALSE;
}
...
static void
wps_pbcd_copy_bss_nbr_list(bss_list_t *dst_bss_list, bss_list_t *src_bss_list)
{
	bss_list_t *from_bss_list;
	bss_list_t *to_bss_list;

	to_bss_list = dst_bss_list;

	while (to_bss_list) {
		from_bss_list = src_bss_list;
		while (from_bss_list) {
			if (!strncmp(to_bss_list->ifname, from_bss_list->ifname,
				sizeof(to_bss_list->ifname))) {

				from_bss_list = from_bss_list->next;
				continue;
			}

			if (from_bss_list->up) {
				(void)wps_pbcd_add_rrm_nbr(to_bss_list->ifname,
				&from_bss_list->self_nbr_rpt);
			}

			from_bss_list = from_bss_list->next;
		}
		to_bss_list = to_bss_list->next;
	}
}

static bool
wps_pbcd_is_interface_enabled(char *ifname)
{
	int unit, subunit, isup = 0;
	char nv_ifname[IFNAMSIZ] = {0};
	char buf[16] = {0};

	if (osifname_to_nvifname(ifname, nv_ifname, IFNAMSIZ) < 0) {
		PBCD_DBG("%s: error converting osifname to nvifname()\n",
				ifname);
		return FALSE;
	}
	if (get_ifname_unit(nv_ifname, &unit, &subunit) < 0) {
		PBCD_DBG("%s: get_ifname_unit() failed \n", nv_ifname);
		return FALSE;
	}
	subunit = htod32(subunit);

	wl_iovar_getbuf(ifname, "bss", &subunit, sizeof(subunit), buf, sizeof(buf));

	isup = *(int*)buf;
	isup = dtoh32(isup);

	PBCD_DBG("%s: ifname[%s] isup[%d]\n", __FUNCTION__, ifname, isup);
	return isup;
}
...
wps_pbcd_refresh_co_host_nbr_info()
{
	...
	/* populate nbr entries across bands present */
	for (i = 0; i < MAX_RADIO_NUM; i++) {
		ri = &(g_wps_pbcd_info_list->radio_rnr_info[i]);
		if (!ri->enabled) {
			continue;
		...
		while (bss) {
			...
			*(uint32*)ioctl_buf = htod32(WLC_IOCTL_MAXLEN);
			err = wl_ioctl(bss->ifname, WLC_GET_BSS_INFO, ioctl_buf,
				WLC_IOCTL_MAXLEN);
			if (err) {
				PBCD_ERR("%s failed to get bss info.\n", bss->ifname);
				break;
			...
			err = wps_pbcd_fill_rrm_nbr_info(bss->ifname, bi, &bss->self_nbr_rpt);
			if (err) {
				PBCD_ERR("%s failed to fill bss info into rrm_nbr\n", bss->ifname);
				break;
			}
