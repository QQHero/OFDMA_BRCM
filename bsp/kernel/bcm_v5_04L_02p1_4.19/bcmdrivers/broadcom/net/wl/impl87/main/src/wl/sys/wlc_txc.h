 ...
 *
 * $Id: wlc_txc.h 802554 2021-08-31 15:41:10Z $
 */
...
extern void wlc_txc_detach(wlc_txc_info_t *txc_info);

/* update feature's runtime enab state */
extern void wlc_txc_upd(wlc_txc_info_t *txc_info);

/* invalidate the entry */
extern void wlc_txc_inv(wlc_txc_info_t *txc_info, struct scb *scb);

/* prepare sdu for tx - i.e. iv update */
extern void wlc_txc_prep_sdu(wlc_txc_info_t *txc_info, struct scb *scb, wlc_key_t *key, void *p);

/* check if the cache has the entry */
extern bool wlc_txc_hit(wlc_txc_info_t *txc_info, struct scb *scb,
	void *pkt, uint pktlen, uint fifo, uint8 prio);
/* copy the header to the packet and return the address of the copy */
extern d11txhdr_t *wlc_txc_cp(wlc_tx_rtc_t *rtc, void *pkt, uint *flags);
/* install an entry into the cache */
extern void wlc_txc_add(wlc_txc_info_t *txc_info, struct scb *scb,
	void *pkt, uint txhlen, uint fifo, uint8 prio, uint16 txh_off, uint d11hdr_len);

/* invalidate all entries */
extern void wlc_txc_inv_all(wlc_txc_info_t *txc_info);

extern void* wlc_scb_txc_info(wlc_info_t *wlc, struct scb *scb);
#endif /* _wlc_txc_h_ */
