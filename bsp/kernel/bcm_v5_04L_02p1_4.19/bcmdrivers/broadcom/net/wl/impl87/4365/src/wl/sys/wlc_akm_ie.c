 ...
 *
 * $Id: wlc_akm_ie.c 811008 2022-04-21 16:04:38Z $
 */
...
static const bcm_iovar_t akm_iovars[] = {
	...
	{"rsnxe", IOV_RSNXE, (IOVF_OPEN_ALLOW), IOVT_BUFFER, 0},
	{"dh_ie", IOV_DH_IE, IOVF_OPEN_ALLOW, IOVT_BUFFER, 0},
	{NULL, 0, 0, 0, 0}
};
...
wlc_write_rsn_ie_safe(wlc_info_t *wlc, wlc_bsscfg_t *cfg, uint8 *buf, int buflen)
{
...
#if defined(WL_SAE) || defined(WL_DPP)
	...
	if (WPA_auth & WPA3_AUTH_DPP) {
		...
		BUFLEN_CHECK_AND_RETURN(WPA_SUITE_LEN, buflen, orig_buf);
		bcopy(DPP_OUI, auth->list[count].oui, DOT11_OUI_LEN);
		auth->list[count++].type = akm_type;
...
wlc_check_wpa2ie(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, bcm_tlv_t *wpa2ie, struct scb *scb)
{
	...
	if ((count != 1) ||
		...
		(WPA_auth & WPA2_AUTH_PSK)))) ||
#ifdef WL_DPP
		((bcmp(mgmt->list[0].oui, DPP_OUI, DPP_OUI_LEN) == 0) &&
...
#ifdef WL_DPP
		if (!bcmp(mgmt->list[i].oui, DPP_OUI, DPP_OUI_LEN)) {
			switch (mgmt->list[i].type) {
				case RSN_AKM_DPP:
					bi->wpa2.auth[j++] = mgmt->list[i].type;
					break;
...
#endif /* defined(AP) && defined(WL_DPP) */

#if defined(STA) && defined(WL_DPP)
static int
...
#endif /* defined(STA) && defined(WL_DPP) */
