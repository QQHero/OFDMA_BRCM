...
BCMATTACHFN(wlc_mbo_ap_attach)(wlc_info_t *wlc)
{
	...
	/* bcn/probresp */
	if (wlc_iem_add_build_fn_mft(wlc->iemi, anqp_ie_build_fstbmp,
		DOT11_MNG_ADVERTISEMENT_ID, wlc_mbo_calc_anqp_ie_len, wlc_mbo_build_anqp_ie,
		mbo) != BCME_OK) {
...
wlc_mbo_gas_event_cb(void *context, bcm_gas_t *gas, bcm_gas_event_t *event)
{
	...
	if (event->type != BCM_GAS_EVENT_QUERY_REQUEST) {
		WL_INFORM(("wl%d: %s Ignore GAS request with event [%d] \n", wlc->pub->unit,
	...
	if (flag & MBO_GAS_RQST_ANQP_QUERY) {
		...
		WL_INFORM(("wl%d:%d ANQP query rcvd with payload [%d]bytes\n", wlc->pub->unit,
			WLC_BSSCFG_IDX(bsscfg), event->queryReq.len));
		wlc_bss_mac_event(mbo->wlc, bsscfg, WLC_E_GAS_RQST_ANQP_QUERY,
			peer_addr, 0, 0, 0, event->body, event->body_len);
	}
