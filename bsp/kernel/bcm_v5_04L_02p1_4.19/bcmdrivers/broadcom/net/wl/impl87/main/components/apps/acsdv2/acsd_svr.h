 ...
 *
 * $Id: acsd_svr.h 809799 2022-03-23 21:05:18Z $
 */
...
#define ACS_CHANIM_POLL_DEFAULT	60 /* Query chanim_stats in cur ch only after 60sec */
...
#define CHANIM_DFLT_INTF_FORCE_CS_SCAN	0
...
#define ACS_MAX_LIST_LEN			256 /* Max Possible
...
#define ACS_ALLOW_UNCLEARED_DFS_CH_DEFAULT	0
...
#define ACS_5G_MONITOR_NO_BGDFS			0	/* BGDFS on 5G monitor mode */
#define ACS_BGDFS_PRECLEAR_ETSI			1	/* if etsi, enable preclearing channels */
...
#ifdef BAND6G
#define ACS_CHANIM_BUF_LEN			(8*1024)
#else /* BAND6G */
...
#define WL_CHANSPEC_REG_TXPWR_FIXED_LEN		(sizeof(wl_chanspec_reg_txpwr_t) \
...
#define ACS_MAX_RECORD		64 /* retain 64 recent channel change details */

#define ACS_REPEATER_VERSION	1

#define ACS_CHAN_UTIL_SIGN_CHANGE	10
#define ACS_BSS_SIGN_LIMIT	5
#define ACS_REP_NOISE		5
#define ACS_REEVAL_TIME_LIMIT		300
#define ACS_EDCRS_HI_MIN_REACT_INTERVAL	10

/* define for apcs reason code */
#define ACS_INIT		0
#define ACS_IOCTL		1
#define ACS_CHANIM		2
#define ACS_CSTIMER		3
#define ACS_SIGN_REP_CHANGE	4
#define ACS_TXDLY		5
#define ACS_NONACSD		6
#define ACS_DFSREENTRY		7
#define ACS_TXFAIL		8
#define ACS_ZDFS		9
#define ACS_EDCRS_HI_CHANGE	10
#define ACS_MAX			11

#define ACS_INITIAL_AUTOCHANNEL	0
#define ACS_ENABLE_CI_SCAN	1
#define ACS_IMMEDIATE_DFSR_DISABLE 0
#define ACS_SKIP_SCAN_ON_RESTART 0
#define ACS_ESCAN_RETRY_TIME	60
...
typedef struct {
	...
	acs_record_t record[ACS_MAX_RECORD];
} chanim_info_t;
...
typedef struct {
	chanspec_t chanspec;
	uint8 chan_avail;
	int16 rep_nbr_nctrl;  /* No of Bss in control channel */
	int16 rep_nbr_next20; /* No of neighbors in ext20 */
	int16 rep_nbr_next40; /* No of neighbors in ext40 */
	int16 rep_nbr_next80; /* No of neighbors in ext80 */
...
struct acs_bgdfs_info {
	...
	bool bgdfs_on_5g_monitor;	/* BGDFS on 5G monitor mode */
	bool ch_move_on_5g_monitor;
	uint8 preclear_etsi;		/* Enable/disable preclearing channels in ETSI */
};
...
typedef struct acs_channel_weights_s {
	uint16 count;
	uint8 ch_list[ACS_MAX_LIST_LEN];
	int16 wt_list[ACS_MAX_LIST_LEN];
} acs_channel_weights_t;
...
#define ACSD_CHAN_CHANGE_THROUGH_REP_STATS	1
#define ACS_BSS_BEACON_PROBE_LEN	1200
...
struct acs_chaninfo {
	...
	wl_chanspec_reg_txpwr_t *reg_txpwr_params;
	bool fallback_to_txpwr_max;
	uint8 bss_sign_change;
	uint8 chan_util_sign_change;
	uint16 reeval_time_limit;
	int8 acs_rep_noise;
	uint8 acs_chan_change_through_rep_stats;
	bool end_repeater_stats;
	bool scanrpt_change;
	bool rep_first_exception;
	uint8 acs_initial_autochannel; /* Custom knob: treat autochannel CLI as initial selection */
	acs_channel_weights_t channel_weights;
	uint8 acs_enable_ci_scan;
	uint8 acs_allow_immediate_dfsr;
	bool acs_is_in_ap_mode;
	bool acs_restart;
	uint8 acs_skip_scan_on_restart;
	uint8 acs_chanim_poll;
	bool acs_chanim_intf_force_cs_scan;
	bool acs_bgdfs_move;
	uint16 acs_edcrs_hi_min_react_interval;
	uint32 acs_edcrs_hi_prev_chan_timestamp;
	bool acs_chan_change_on_edcrs_hi;
	bool acs_edcrs_hi_first_changed;
	acs_bgdfs_info_t *acs_bgdfs_saved;	/* saved structure when bgdfs is enabled */
	uint8 acs_is_scan_success;
	bool acs_allow_uncleared_dfs_ch;
};
...
extern void acs_retrieve_config_bgdfs(acs_bgdfs_info_t *acs_bgdfs, char * prefix);
extern void acs_runtime_retrieve_config(acs_chaninfo_t *c_info, char *prefix);
...
extern int acs_get_reg_txpwr_info(acs_chaninfo_t *c_info, wl_chanspec_reg_txpwr_t *param,
		int param_len, char *buf, int buf_len);
extern int acs_set_chan_weight_table(acs_chaninfo_t *c_info, char *ch_wt_list,
		unsigned int vector_size, uint8 band);
extern void acs_get_time_since_boot(struct timeval *tv);
extern void acs_set_initial_chanspec(acs_chaninfo_t *c_info);
extern void acs_get_reg_txpwr_for_allchans(acs_chaninfo_t* c_info);
extern void acs_get_txpwr_for_allchans(acs_chaninfo_t* c_info);
