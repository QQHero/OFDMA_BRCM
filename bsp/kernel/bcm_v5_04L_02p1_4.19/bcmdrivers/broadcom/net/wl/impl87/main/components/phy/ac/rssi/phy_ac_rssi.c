 ...
 *
 * $Id: phy_ac_rssi.c 810767 2022-04-13 15:43:04Z $
 */
...
BCMATTACHFN(phy_ac_rssi_register_impl)(phy_info_t *pi, phy_ac_info_t *aci, phy_rssi_info_t *ri)
{
	...
	/* RSSI accuracy improvement in low SNR region */
	pi->sromi->sr18_rssi_low_snr =
		(bool) ((pi->sh->boardflags4 & BFL4_SROM18_RSSI_LOW_SNR) >> 27);
	...
	FOREACH_CORE(pi, core) {
		...
		if (PHY_BAND5G_ENAB(pi)) {
			...
			if (ACMAJORREV_128(pi->pubpi->phy_rev)) {
				/* WAR for BCAWLAN-233886, because NVRAM is fixed at this stage */
				for (i = 0; i < ACPHY_RSSIOFFSET_NVRAM_PARAMS; i++) {
					for (j = 0; j < ACPHY_NUM_BW; j++) {
						pi_ac->sromi->rssioffset.
							rssi_corr_normal_5g[ant][i][j] -= 7;
					}
				}
			}
		}
...
phy_ac_rssi_compute(phy_type_rssi_ctx_t *ctx, wlc_d11rxhdr_t *wrxh)
{
	...
	/* Applying dB_qdb to REV40 and later */
...
phy_ulofdma_per_user_rxstats(phy_type_rssi_ctx_t *ctx, wlc_d11rxhdr_t *wrxh)
{
	...
	*/
	...
	if (D11REV_GE(pi->sh->corerev, 128) && (frame_type == 4) && (he_format == 3)) {
		...
		bzero(user_snr_delta, sizeof(int8)*MAX_NUM_ULOFDMA_USERS);
		bzero(user_preempted, sizeof(int8)*MAX_NUM_ULOFDMA_USERS);
		...
		for (user = 0; user < num_users; user++) {
			...
			if (D11REV_GE(pi->sh->corerev, 132)) {
				user_snr_delta[user] =
				    (uint8)PRXS_USER_SNRDELTA_REV_GE129(rxh, user);
				user_preempted[user] =
				    (uint8)PRXS_USER_PREEMPTED_REV_GE129(rxh, user);
			}
		}
		...
		for (user = 0; user < MAX_NUM_ULOFDMA_USERS; user++) {
			...
			per_user_stat->user_snr_delta[user] = user_snr_delta[user];
			per_user_stat->user_preempted[user] = user_preempted[user];
...
phy_tracked_freq_offset(phy_type_rssi_ctx_t *ctx, wlc_d11rxhdr_t *wrxh)
{
	...
	if (D11REV_GE(pi->sh->corerev, 128)) {
		...
		/* Scaling factor selection
		   ...
		   For 6715:
		   A band: 4688 ~= 1/(3.2e-6*2^19)*3/2/200*2^20 (overflow with 2^20)
		   G band: 5208 ~= 1/(3.2e-6*2^19)*10/3/200*2^19 (overflow with 2^21)
		*/

		tracked_freq_stat->tracked_freq_offsetHz = total * 4883;
		if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
			if (CHSPEC_IS2G(pi->radio_chanspec)) {
				tracked_freq_stat->pll_wild_base_offset =
				    (uint32)(total * 5208) / 250000;
			} else {
				tracked_freq_stat->pll_wild_base_offset =
				    (uint32)(total * 4688) / 500000;
			}
		} else {
			if (CHSPEC_IS2G(pi->radio_chanspec)) {
				tracked_freq_stat->pll_wild_base_offset =
				    (uint32)(total * 4340) / 500000;
			} else {
				tracked_freq_stat->pll_wild_base_offset =
				    (uint32)(total * 7716) / 1000000;
			}
		}
	}
}
...
static void phy_ac_align_sniffer(phy_type_rssi_ctx_t *ctx)
{
	...
	if (pi->u.pi_acphy->sniffer_aligner.enabled &&
		...
		if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
			...
			wild_base_high = READ_RADIO_PLLREGFLD_20708(pi, PLL_FRCT2,
					0, rfpll_frct_wild_base_high);
			wild_base_low  = READ_RADIO_PLLREGFLD_20708(pi, PLL_FRCT3,
					0, rfpll_frct_wild_base_low);
		}
		...
		if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
			...
			MOD_RADIO_PLLREG_20708(pi, PLL_FRCT2, 0,
					rfpll_frct_wild_base_high, (uint16) wild_base_high);
			MOD_RADIO_PLLREG_20708(pi, PLL_FRCT3, 0,
					rfpll_frct_wild_base_low,  (uint16) wild_base_low);
		}
...
phy_ac_rssi_compute_compensation(phy_type_rssi_ctx_t *ctx, int16 *rxpwr_core, bool db_qdb)
{
	...
	if ((SROMREV(pi->sh->sromrev) >= 18) && pi->sromi->sr18_rssi_low_snr) {
		/* In order to improve the accuracy of RSSI in low power state */
		/* remove background noise power */
		/* The Mapping Function as Following  */
		/* rssi_adj = 10*log10(10.^(rxpwr_qdBm/10) - 10.^(noise/10)); */
		/* rxpwr_adj_n89[10] = { -95, -91, -89, -87, -86, -84, -83, -82, -81, -79}; */
		/* rxpwr_adj_n90[10] = { -96, -92, -90, -88, -87, -85, -84, -83, -82, -80}; */
		/* rxpwr_adj_n91[10] = { -97, -93, -91, -89, -88, -86, -85, -84, -83, -81}; */
		/* rxpwr_adj_n92[10] = { -98, -94, -92, -90, -89, -87, -86, -85, -84, -82}; */
		/* rxpwr_adj_n93[10] = { -99, -95, -93, -91, -90, -88, -87, -86, -85, -83}; */
		/* rxpwr_adj_n94[10] = { -100, -96, -94, -92, -91, -89, -88, -87, -86, -84}; */
		/* rxpwr_adj_n95[10] = { -101, -97, -95, -93, -92, -90, -89, -88, -87, -85}; */
		/* rxpwr_adj_n96[10] = { -102, -98, -96, -94, -93, -91, -90, -89, -88, -86}; */
		/* rxpwr_adj_n97[10] = { -103, -99, -97, -95, -94, -92, -91, -90, -89, -87}; */
		/* rxpwr_adj_n98[10] = { -104, -100, -98, -96, -95, -93, -92, -91, -90, -88}; */
		/* rxpwr_adj_n99[10] = { -105, -101, -99, -97, -96, -94, -93, -92, -91, -89}; */

		const uint8 rxpwr_adj_tb[40] =
			{0, 12, 20, 26, 30, 34, 37, 40, 42, 45, 47, 49,
			51, 53, 54, 56, 58, 59, 61, 62, 64, 65, 67, 68,
			69, 71, 72, 73, 74, 76, 77, 78, 79, 80, 82, 83,
			84, 85, 86, 87};
		int16 noise_pwr_qdBm, tmp, idx;
		...
		FOREACH_CORE(pi, core) {
			idx = rxpwr_core_qdBm[core]-noise_pwr_qdBm-1;
			idx = (idx < 0) ? 0 : idx;
			if (idx < 37) {
				tmp = noise_pwr_qdBm-49+rxpwr_adj_tb[idx];
				rxpwr_core_qdBm[core] = tmp;
				tmp = ((tmp >= 0) ? ((tmp + 2) >> 2) : -1 *
					((-1 * tmp + 2) >> 2));
				rxpwr_core[core] = tmp;
...
_phy_ac_rssi_init_gain_err(phy_type_rssi_ctx_t *ctx)
{
	...
	int8 init_gain_to_program = (ACMAJORREV_128(pi->pubpi->phy_rev)) ?
...
void
phy_ac_hwrssi_set_offset(phy_info_t *pi)
{
	uint8  core;
	if (!ACMAJORREV_GE130(pi->pubpi->phy_rev)) {
		return;
	...
	FOREACH_CORE(pi, core) {
		MOD_PHYREGCE(pi, HwRssiTROffset, core, TRinROffset,
			pi->hwrssioffset_cmn[core] << 2);
		MOD_PHYREGCE(pi, HwRssiTROffset, core, TRinTOffset,
			pi->hwrssioffset_trt[core] << 2);
	}
}

void
phy_ac_rssi_mode_select(phy_info_t *pi, bool hwrssi_mode)
{
	phy_ac_rssi_data_t *data = pi->u.pi_acphy->rssii->data;

	PHY_TRACE(("%s %d\n", __FUNCTION__, hwrssi_mode));

	if (!ACMAJORREV_GE130(pi->pubpi->phy_rev)) {
		return;
	}
	if (hwrssi_mode == 0) {
		MOD_PHYREG(pi, IqestCmd, hw_rssi_mode, 0);
		if (CHSPEC_IS160(pi->radio_chanspec)) {
			MOD_PHYREG(pi, RxStatPwrOffset0, rx_status_pwr_offset0, 45);
			MOD_PHYREG(pi, RxStatPwrOffset1, rx_status_pwr_offset1, 44);
			MOD_PHYREG(pi, RxStatPwrOffset2, rx_status_pwr_offset2, 44);
			MOD_PHYREG(pi, RxStatPwrOffset3, rx_status_pwr_offset3, 44);
		} else {
			// for non-160mhz, use the default value.
			MOD_PHYREG(pi, RxStatPwrOffset0, rx_status_pwr_offset0, 46);
			MOD_PHYREG(pi, RxStatPwrOffset1, rx_status_pwr_offset1, 46);
			MOD_PHYREG(pi, RxStatPwrOffset2, rx_status_pwr_offset2, 46);
			MOD_PHYREG(pi, RxStatPwrOffset3, rx_status_pwr_offset3, 46);
		}
	} else {
		...
		//MOD_PHYREG(pi, IqestCmd, hw_rssi_latch_n, 1);
		//MOD_PHYREG(pi, RxStatusLatchCtrl, rxstatus_report_leg_rssi, 1);
	}
	data->hwrssi_en = hwrssi_mode;
}

bool
phy_ac_noise_get_hwrssi_en(phy_info_t *pi)
{
	phy_ac_rssi_data_t *data = pi->u.pi_acphy->rssii->data;
