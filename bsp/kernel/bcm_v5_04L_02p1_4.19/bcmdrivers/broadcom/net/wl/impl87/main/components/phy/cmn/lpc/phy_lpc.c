 ...
 *
 * $Id: phy_lpc.c 783949 2020-02-13 09:58:24Z $
 */
...
/* ********************************************* */
uint8
wlc_phy_lpc_getminidx(phy_info_t *pi)
{
...
uint8
wlc_phy_lpc_getoffset(phy_info_t *pi, uint8 index)
{
	phy_type_lpc_fns_t *fns = pi->lpci->fns;
	if (fns->getpwros)
...
uint8
wlc_phy_lpc_get_txcpwrval(phy_info_t *pi, uint16 phytxctrlword)
{
	phy_type_lpc_fns_t *fns = pi->lpci->fns;
	if (fns->gettxcpwrval)
...
void
wlc_phy_lpc_set_txcpwrval(phy_info_t *pi, uint16 *phytxctrlword, uint8 txcpwrval)
{
	phy_type_lpc_fns_t *fns = pi->lpci->fns;
	if (fns->settxcpwrval)
...
void
wlc_phy_lpc_algo_set(phy_info_t *pi, bool enable)
{
	phy_type_lpc_fns_t *fns = pi->lpci->fns;
	pi->lpc_algo = enable;
	if (fns->setmode)
...
#ifdef WL_LPC_DEBUG
uint8 *
wlc_phy_lpc_get_pwrlevelptr(phy_info_t *pi)
{
	phy_type_lpc_fns_t *fns = pi->lpci->fns;
