 ...
 *
 * $Id: wluc_wnm.c 807402 2022-01-19 13:01:36Z $
 */
...
wl_wnm_bss_select_table(void *wl, cmd_t *cmd, char **argv)
{
	...
	if ((argc > 2) && ((argc - 1) % 3)) {
		if ((((argc - 1) % 3) == 2) || (stricmp(argv[argc-1], "a") &&
			stricmp(argv[argc-1], "b") && stricmp(argv[argc-1], "6g") &&
			stricmp(argv[argc-1], "all"))) {
			return BCME_USAGE_ERROR;
		}
	}
	...
	for (i = 1; i < argc; i = i + 3) {
		if (str2bandtype(argv[i], &bandtype, BND_IS_STRING_INC_BAND_ALL)) {
			btcfg->band = bandtype;
		} else {
...
wl_wnm_bss_select_weight(void *wl, cmd_t *cmd, char **argv)
{
	...
	/* Second arg: value or band */
	if (argc >= 2) {
		if (str2bandtype(argv[1], &bandtype, BND_IS_STRING_INC_BAND_ALL)) {
			bwcfg.band = bandtype;
		} else {
	...
	/* third arg: band */
	if (argc >= 3) {
		if (str2bandtype(argv[2], &bandtype, BND_IS_STRING_INC_BAND_ALL)) {
			bwcfg.band = bandtype;
		} else {
			return BCME_USAGE_ERROR;
		}
	...
	if ((argc == 2) && (bwcfg.band != WLC_BAND_AUTO)) {
		...
		printf("%s %s weight = %d\n",
			...
			(bwcfg.band == WLC_BAND_2G) ? "2G" :
				(bwcfg.band == WLC_BAND_5G) ? "5G" : "6G", bwcfg.weight);
	} else {
