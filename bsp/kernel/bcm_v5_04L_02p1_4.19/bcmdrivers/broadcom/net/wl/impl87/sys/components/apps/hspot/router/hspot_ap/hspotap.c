...
#define HSERR_EN_ACT_CMPLT_EVT		"Failed to enable action frame complete event\n"
#define HSERR_EN_ACT_OFFCH_CMPLT_EVT	"Failed to enable action frame offchan complete event\n"
#define HSERR_EN_ACT_RX_EVT		"Failed to enable action frame rx event\n"
...
#define HSERR_OSEN_WTOUT_AKM		"OSEN IE can't be enabled if osen not present in wl_akm,"\
					" Disabling OSEN.\n"
/* ----------------------------------- ERROR Strings ------------------------------------ */
...
decode_hspot_flags(char *prefix, hspotApT *hspotap, char* err_nvram)
{
	...
	/* OSEN can't be enabled if "osen" not in wl_akm, Disabling OSEN */
	ptr = nvram_get(strcat_r(prefix, NVNM_AKM, varname));
	if (ptr) {
	if (hspotap->osen_ie_enabled && !find_in_list(ptr, NVVAL_OSEN)) {
		hspotap->osen_ie_enabled = 0;
		set_hspot_flag(prefix, HSFLG_OSEN, 0);
		HS20_WARNING(HSERR_OSEN_WTOUT_AKM);
	}
	}
...
update_rc_ie(hspotApT *hspotap)
{
	...
	char varname[NVRAM_MAX_PARAM_LEN] = {0};
	int oui_extra = 0; /* addtional OUI for Number of ANQP OIs */
	...
	if (hspotap->ouilist.numOi) {
		...
		/* addtional OUI for Number of ANQP OIs can be configured via nvram */
		oui_extra = atoi(nvram_safe_get(strcat_r(hspotap->prefix, NVNM_OUI_EXTRA, varname)));
		HS20_INFO(PRNT_FLG, varname, oui_extra);

		/* if not configured (oui_extra == 0), keep using the original algorithm */
		oui_extra = oui_extra ? oui_extra : (hspotap->ouilist.numOi - iter_isbeacon);
...
update_ap_ie(hspotApT *hspotap)
{
	...
	if (ap_ANQPenabled || ap_MIHenabled) {
		...
		bcm_encode_t ad;
		int qresponselimit = atoi(nvram_safe_get(NVNM_ADVP_QRL));

		qresponselimit = qresponselimit ? qresponselimit : 0x7f;

		/* encode advertisement protocol IE */
		bcm_encode_init(&ie, sizeof(buffer), buffer);
		bcm_encode_init(&ad, sizeof(adBuffer), adBuffer);
		bcm_encode_ie_advertisement_protocol_tuple(&ad, FALSE, qresponselimit,
...
gas_event_handler(hspotApT *hspotap, bcm_gas_event_t *event, uint16 *status)
{
	...
	if (event->type == BCM_GAS_EVENT_QUERY_REQUEST) {
		if (nvram_match(NVNM_HS2_ANQP_HAL, "1" )) {
			HS20_INFO(PRNT_STR, "gas_event_handler", "handle by wifi_hal");
		}
		else {
			if (hspotap->iw_ie_enabled || hspotap->hs_ie_enabled) {
...
/* Enable GAS Action Fame Rx Events, only if IW or HS is Enabled */
static void
gas_init_wlan_handler(hspotApT *hspotap)
{
	if (!hspotap->iw_ie_enabled && !hspotap->hs_ie_enabled) {
		return;
	}

	if (wl_enable_event_msg(hspotap->ifr, WLC_E_ACTION_FRAME_COMPLETE) < 0) {
		HS20_ERROR(HSERR_EN_ACT_CMPLT_EVT);
	}
	if (wl_enable_event_msg(hspotap->ifr, WLC_E_ACTION_FRAME_OFF_CHAN_COMPLETE) < 0) {
		HS20_ERROR(HSERR_EN_ACT_OFFCH_CMPLT_EVT);
	}
	if (wl_enable_event_msg(hspotap->ifr, WLC_E_ACTION_FRAME_RX) < 0) {
		HS20_ERROR(HSERR_EN_ACT_RX_EVT);
	}
}
/* ========================== GAS ENGINE SUPPORT ========================== */
...
main(int argc, char **argv)
{
	...
	while ((ifr = wlif(total_ifr)) != NULL) {
		...
		if (get_hspot_flag(hspotap->prefix, HSFLG_ANQP)) {
			if (nvram_match(NVNM_HS2_ANQP_HAL, "1" )) {
				HS20_INFO("anqp handled by wifi_hal");
			} else {
