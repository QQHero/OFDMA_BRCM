 ...
 *
 * $Id: wlc_pwrstats.c 808142 2022-02-10 01:30:35Z $
 */
...
static const bcm_iovar_t pwrstats_iovars[] = {
	...
	0, 0, IOVT_BUFFER, WL_PWR_STATS_HDRLEN,
	},
...
pwrstats_bsscfg_up_down(void *ctx, bsscfg_up_down_event_data_t *evt_data)
{
	...
	if (!evt_data->up) {
		...
		if (cfg->BSS && BSSCFG_STA(cfg) && BSSCFG_IS_PRIMARY(cfg)) {
...
int wlc_mimo_siso_metrics_report(wlc_info_t *wlc)
{
...
int wlc_get_mimo_siso_meas_metrics(wlc_info_t *wlc, uint8 *destptr, int destlen)
{
	...
	if (wlc->pub->hw_up && wlc->clk) {
		rc = wlc_mimo_siso_metrics_report(wlc);
		if (rc != BCME_OK) {
			return rc;
		}
	}
