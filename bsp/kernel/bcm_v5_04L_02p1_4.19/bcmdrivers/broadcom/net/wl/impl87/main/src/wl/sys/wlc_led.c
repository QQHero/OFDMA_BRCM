 ...
 *
 * $Id: wlc_led.c 804615 2021-11-04 17:51:52Z $
 */
...
#if defined(WLLED_CLED)
#define WLLED_CLED_CORE(chipid) ((EMBEDDED_2x2AX_CORE(chipid)) || \
					(CHIPID(chipid) == BCM6878_CHIP_ID))
#endif /* WLLED_CLED */
...
struct led_info {
	...
	uint32		blink_pins;
#if defined(WLLED_CLED)
	unsigned int	cled;			/* 0 or CLED number if used */
#endif /* WLLED_CLED */
};
...
static const bcm_iovar_t led_iovars[] = {
	...
	(0), 0, IOVT_BUFFER, sizeof(wl_led_info_t)
	},
...
static void wlc_led_wi9(led_info_t *li, struct led *led);
...
#if defined(WLLED_CLED)
/*
 * Use Broadcom Serial led controller (cled) interface instead
 * of wlan GPIO's for embedded WiFi SoC's
 *
 * Enabled only for embedded WiFi SoC'w with LED board parameters set
 * Only 1 led to control all the WiFi activity
 *
 * Default LED: Behavor (wi9)
 *  - wl up, LED ON
 *  - wl down, LED OFF
 *  - wl activity, LED blinking medium
 *  - Logical OR of (LED_ACTIVITY|LED_BRADIO)
 * Any changes in the board (gpio/led) should be done by changing board parameters
 * Default behavor can be changed using nvram (ledbh0)
 * wlgpio's can not be changed using nvram (wl0gpio%d)
 *
 */
static void
BCMATTACHFN(wlc_led_cled_attach)(wlc_info_t *wlc, led_info_t *li)
{
	struct led *led = NULL;
	...
	/* CLED is ued for internal WiFi interfaces */
	/* Check if the Chip is internal WiFi SoC */
	if (!WLLED_CLED_CORE(wlc->pub->sih->chip))
	    return;

	/* Check if board supports WiFi LED activity through cled */
#if defined(BCM_VPCIE_WIFI_CORE_INFO)
	core = osl_pci_slot(wlc->pub->osh) - 1;
#else /* !BCM_VPCIE_WIFI_CORE_INFO */
#if defined(WLAN1_PHYS_BASE)
	if (wlc->pub->sih->enum_base == WLAN1_PHYS_BASE) {
	    core = 1;
	}
#endif /* WLAN1_PHYS_BASE */
#endif /* !BCM_VPCIE_WIFI_CORE_INFO */

	li->cled = kerSysGetWifiLed(core);
	if (li->cled == 0)
	    return;

	/* Use only activity + Radio (WI9) */
	/* Disable b/a radio */
	led = &li->led[0];
	led->pin = 0;
	led->behavior = WL_LED_WI9;
	led->activehi = TRUE;
...
static void
BCMATTACHFN(wlc_led_cled_detach)(wlc_info_t *wlc, led_info_t *li)
{
	/* Nothing to do */
	return;
}
...
int
wlc_led_cled(led_info_t *li, uint32 mask, uint32 val, bool activehi)
{
	if (li->cled == 0) {
	    /* wl_gpio, not processed */
	    return 1;
	}

	kerSysWifiLed(li->cled, (mask&val));
...
#if defined(WLLED_CLED)
	/* override for cled based internal WiFi SoC's */
	wlc_led_cled_attach(wlc, li);
#endif /* WLLED_CLED */
...
BCMATTACHFN(wlc_led_detach)(led_info_t *li)
{
	...
	WL_TRACE(("wl: %s: li = %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(li)));
...
wlc_led_a_radio(led_info_t *li, struct led *led)
{
	...
	a_radio = FALSE;
	if (IS_SINGLEBAND(wlc) || (wlc->bandlocked == TRUE)) {
...
wlc_led_assoc(led_info_t *li, struct led *led)
{
	...
	if (!li->up) {
		if (WOWL_ACTIVE(li->pub) ||
			...
			(li->wlc->primary_bsscfg->flags & WLC_BSSCFG_PRESERVE))) {
			return;
		}
		else {
...
/* 1. Led off when not UP */
/* 2. Solid after interface UP */
/* 3. Medium blinking when data transfer */
static void
wlc_led_wi9(led_info_t *li, struct led *led)
{
	if (!li->up) {
		/* make sure li->activity is clear since wlc_led_timer may be stopped in wlc_down */
		li->activity = FALSE;
...
static int
wlc_led_doiovar(void *hdl, uint32 actionid,
