 ...
 *
 * $Id: wlc_bmac.h 802692 2021-09-03 10:19:13Z $
 */
...
typedef struct wlc_stf_rs_shm_offset {
	...
	uint8 txpwr[WLC_NUMRATES][BW160_IDX+1];
...
#define TXS_PACKAGE_SIZE	sizeof(wlc_txs_pkg16_t)

/* HW generated Tx Status for d11 rev >= 40 */
#define WLC_TXS40_STATUS_BYTES		TX_STATUS_MACTXS_BYTES
#define WLC_TXS40_STATUS_PKGS		(TX_STATUS_MACTXS_BYTES / TXS_PACKAGE_SIZE)

typedef struct wlc_txs40_status {
	union {
		uint8			u8[WLC_TXS40_STATUS_BYTES];
		wlc_txs_pkg16_t		pkg16[WLC_TXS40_STATUS_PKGS];
...
#else /* WL_PSMR1 */
#define wlc_bmac_enable_macr1(a) do {} while (0)
#define wlc_bmac_suspend_macr1_and_wait(a) do {} while (0)
#define wlc_bmac_read_shm1(a, b) do {} while (0)
#define wlc_bmac_write_shm1(a, b, c) do {} while (0)
#endif /* WL_PSMR1 */
...
extern bool wlc_bmac_detect_and_handle_fifoerrors(wlc_hw_info_t *wlc_hw);
...
extern bool wlc_bmac_is_pcielink_slowspeed(wlc_hw_info_t *wlc_hw);
...
extern void wlc_bmac_service_txstatus(wlc_hw_info_t *wlc_hw);
extern void wlc_bmac_upd_ifs(wlc_hw_info_t *wlc_hw);

/**
 * WLC_DEEP_RING_NFIFO: this is a fix for peak UDP throughput dropping by 300Mbps for a HE 160 MHz
 * client after introducing a new form of admission control. To fix this throughput drop issue, a
 * group of 4 FIFOs are reserved and attached with larger DMA ring. While all other MU FIFO are
 * attached to ring size of 512, those 4 FIFOs have ring size of 1024. FIFO allocator will allocate
 * those only to BE traffic of OFDMA clients with bandwidth of 160MHz.
 */
#ifdef DONGLEBUILD
