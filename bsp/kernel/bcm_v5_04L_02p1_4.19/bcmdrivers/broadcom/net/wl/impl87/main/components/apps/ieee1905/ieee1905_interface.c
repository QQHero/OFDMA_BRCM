...
void i5InterfaceAddDefaultBrouteEntries(char *ifname)
{
  ...
  snprintf(cmd, 256,
               "ebtables %s -t broute -D BROUTING -d " I5_MAC_DELIM_FMT " -p 0x%x -j DROP;",
               use_concurrent ? "--concurrent" : "", I5_MAC_PRM(I5_MULTICAST_MAC), I5_PROTO);
  i5Trace("%s\n", cmd);
  system(cmd);
  snprintf(cmd, 256,
               "ebtables %s -t broute -I BROUTING 1 -d " I5_MAC_DELIM_FMT " -p 0x%x -j DROP;",
               use_concurrent ? "--concurrent" : "", I5_MAC_PRM(I5_MULTICAST_MAC), I5_PROTO);
  i5Trace("%s\n", cmd);
  system(cmd);
  ...
  snprintf(cmd, 256,
               "ebtables %s -t broute -D BROUTING -d " I5_MAC_DELIM_FMT " -p 0x%x -j DROP;",
               use_concurrent ? "--concurrent" : "", I5_MAC_PRM(i5_config.i5_mac_address), I5_PROTO);
  i5Trace("%s\n", cmd);
  system(cmd);
  snprintf(cmd, 256,
               "ebtables %s -t broute -I BROUTING 1 -d " I5_MAC_DELIM_FMT " -p 0x%x -j DROP;",
  ...
  snprintf(cmd, 256,
               "ebtables %s -t broute -D BROUTING -d " I5_MAC_DELIM_FMT " -p 0x%x -j DROP;",
               use_concurrent ? "--concurrent" : "", I5_MAC_PRM(macAddr), I5_PROTO);
  i5Trace("%s\n", cmd);
  system(cmd);
  snprintf(cmd, 256,
               "ebtables %s -t broute -I BROUTING 1 -d " I5_MAC_DELIM_FMT " -p 0x%x -j DROP;",
...
void i5InterfaceAdd(char *ifname, unsigned short matchMediaType)
{
  ...
  if ( I5_MEDIA_TYPE_BRIDGE == mediaType) {
    ...
    if (I5_IS_CTRLAGENT(self_device->flags)) {
      int brprio = 0x7000;
      snprintf(cmd, sizeof(cmd), "brctl setbridgeprio %s %d", ifname, brprio);
      system(cmd);
    }
    pifsock->u.sll.options = USE_AL_MAC_AS_SRC;
