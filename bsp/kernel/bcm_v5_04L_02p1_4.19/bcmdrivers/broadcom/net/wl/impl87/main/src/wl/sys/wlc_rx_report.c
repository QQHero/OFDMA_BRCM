 ...
 *
 * $Id: wlc_rx_report.c 803989 2021-10-14 19:10:00Z $
 */
	...
	case IOV_GVAL(IOV_SCB_RX_REPORT_DATA):
		{
			...
			uint8 * ndrec;
			uint8 tid_count;
			const int SIZEOF_CNTS = sizeof(iov_rx_report_counters_t);
			...
			FOREACH_BSS_SCB(wlc->scbstate, &scbiter, bsscfg, scb) {
				if (SCB_ASSOCIATED(scb)) {
					...
					rem_len -= (sizeof(iov_rx_report_record_t) - SIZEOF_CNTS);

					drec = (iov_rx_report_record_t *)ndrec;
					...
					drec->tid_monitor_mask = 0;
					drec->tid_report_mask = 0;
					tid_count = 0;
					...
					for (tid = 0; tid < NUMPRIO; tid++) {
						...
						if (cs) {
							...
							drec->tid_monitor_mask |= (1<<tid);

							/* Only report tid with activity */
							if (cs->rxbyte != 0) {
								if (rem_len < SIZEOF_CNTS) {
									return BCME_BUFTOOSHORT;
								}
								...
								/* Flag tid as valid */
								drec->tid_report_mask |= (1<<tid);

								/* Copy values to output record */
								cd->rxbyte = cs->rxbyte;
					...
					++num_records;
					ndrec += (sizeof(iov_rx_report_record_t) +
						(tid_count-1) * SIZEOF_CNTS);
				}
			}
			...
			output->length_struct_record = sizeof(iov_rx_report_record_t);
			output->length_struct_counters = SIZEOF_CNTS;
