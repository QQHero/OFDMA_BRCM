 ...
 *
 * $Id: wlc_airiq_scan.c 768810 2018-10-25 19:49:27Z $
 *
 */
...
/* function: wlc_airiq_3p1_chanspec_overlap_ne
 * return value: true if LO of scan chanspec overlaps home chanspec and
 * is not the same as the home chanspec
 */
static bool
wlc_airiq_3p1_chanspec_overlap_ne(chanspec_t home_chanspec,
		chanspec_t scan_chanspec)
{
	int upper, lower, bw, chan_sc, chan;
	/* If scan_chanspec center frequency overlaps with main chanspec,
	 * return true.
	 */
	switch (CHSPEC_BW(home_chanspec)) {
		...
		case WL_CHANSPEC_BW_80:
			...
			break;
		default:
			WL_ERROR(("%s: Invalid chanspec 0x%x\n",
			    __FUNCTION__, home_chanspec));
			ASSERT(0);
			return TRUE;
	}

	chan = CHSPEC_CHANNEL(home_chanspec);

	upper = chan + bw;
	lower = chan - bw;

	chan_sc = CHSPEC_CHANNEL(scan_chanspec);

	if(chan == 10 && chan_sc == 14) {
	    return FALSE;
	}
	if (chan_sc >= lower && chan_sc <= upper && chan != chan_sc) {
		return TRUE;
	}
...
airiq_update_scan_config(airiq_info_t *airiqh, airiq_config_t *sc)
{
	...
	/* prune the list of invalid chanspecs for 3+1 mode */
	for (k = 0; k < cnt; k++) {
		if (sc->phy_mode == PHYMODE_3x3_1x1 &&
			wlc_airiq_3p1_chanspec_overlap_ne(airiqh->wlc->chanspec,
				sc->chanspec_list[k])) {
			/* channel overlaps - skip */
			WL_AIRIQ(("%s: skipping channel %d (0x%x)\n", __FUNCTION__,
				CHSPEC_CHANNEL(sc->chanspec_list[k]), sc->chanspec_list[k]));
		} else {
			airiqh->scan.chanspec_list[real_cnt] = sc->chanspec_list[k];
			airiqh->scan.dwell_interval_ms[real_cnt] = sc->dwell_interval_ms[k];
			airiqh->scan.capture_interval_us[real_cnt] = sc->capture_interval_us[k];
			airiqh->scan.capture_count[real_cnt] = sc->capture_count[k];
			airiqh->scan.core_config[real_cnt] = sc->core_config[k];
			real_cnt++;
		}
	}

	if (real_cnt <= 0) {
		WL_ERROR(("wl%d: %s No valid chanspecs in scan config.\n",
	...
	for (k = 0; k < cnt; k++) {
		...
		/* check core mask */
		if (airiqh->wlc->stf->core3_p1c) {
			if (sc->core_config[k] != 3 || phy_mode != PHYMODE_3x3_1x1) {
				WL_ERROR(("wl%d: %s core %d invalid; (must be core 3)\n",
				    WLCWLUNIT(airiqh->wlc), __FUNCTION__, sc->core_config[k]));
				return BCME_UNSUPPORTED;
			}
			if (CHSPEC_BW(pi->radio_chanspec) !=
					CHSPEC_BW(sc->chanspec_list[k])) {
				WL_ERROR(("wl%d: %s chanspec 0x%x bw must match chanspec 0x%x\n",
				    WLCWLUNIT(airiqh->wlc), __FUNCTION__, sc->chanspec_list[k],
				    pi->radio_chanspec));
				return BCME_BADCHAN;
			}
		} else if ((1 << sc->core_config[k]) & ~airiqh->wlc->stf->valid_rxchain_mask) {
			WL_ERROR(("wl%d: %s Requested core %d not valid: core mask 0x%x\n",
				WLCWLUNIT(airiqh->wlc), __FUNCTION__, sc->core_config[k],
				airiqh->wlc->stf->valid_rxchain_mask));
			return BCME_UNSUPPORTED;
		}
...
{
	wlc_phy_set_val_sc_chspec(pi, scan_chanspec);
	wlc_phy_set_val_phymode(pi, PHYMODE_3x3_1x1);
...
void wlc_airiq_scantimer(void *arg)
{
	...
	airiqh->scan.channel_idx = airiqh->next_chan_index % chancnt;
	...
	if (airiqh->phy_mode == PHYMODE_3x3_1x1) {
		if (CHSPEC_BW(pi->radio_chanspec) !=
			CHSPEC_BW(airiqh->scan.chanspec_list[airiqh->scan.channel_idx])) {
			/* Bandwidth changed */
			WL_AIRIQ(("%s: wl%d BW change chspec = 0x%x scan chspec = 0x%x\n",
				__FUNCTION__, airiqh->wlc->pub->unit,
	...
	if (airiqh->scan.scan_start) {
		...
		/* AirIQ scanning causes throughput hit when IBSS usage is high. So we query
		 the ccastats inbss value and use it to throttle the number of channels scanned in
		 each AirIQ sweep. Two threshold values are used throttle_thr1 and throttle_thr2. */
		airiqh->fft_count = 0;
		airiqh->capture_limit = MAX(10,
			airiqh->scan.capture_count[airiqh->scan.channel_idx]);
		airiqh->next_chan_index = airiqh->scan.channel_idx;
		cur_stats = wlc_lq_chanim_chanspec_to_stats(airiqh->wlc->chanim_info,
						airiqh->wlc->chanspec,FALSE);
		if (cur_stats) {
			chanutil = cur_stats->chanim_stats.ccastats[CCASTATS_INBSS];
		}

		if (chanutil < airiqh->throttle_thr1) {
			sweep_end = (airiqh->next_chan_index == 0);
		} else if (chanutil < airiqh->throttle_thr2) {
			sweep_end = (airiqh->num_chan_scanned >  airiqh->throttle_maxscan1);
		} else { /*chanutil >= airiqh->throttle_thr2*/
			sweep_end = (airiqh->num_chan_scanned >  airiqh->throttle_maxscan2);
		}
...
int wlc_airiq_start_scan_phase2(airiq_info_t *airiqh)
{
	...
	if (airiqh->phy_mode != PHYMODE_3x3_1x1) {
		...
		wlc_block_datafifo(airiqh->wlc, DATA_BLOCK_SCAN, DATA_BLOCK_SCAN);
		/* mute AP */
...
int wlc_airiq_scan_abort(airiq_info_t *airiqh, bool upgrade)
{
	...
	/* set channel */
	wlc_set_chanspec(airiqh->wlc, airiqh->wlc->home_chanspec);
	...
	wlc_phy_clear_tssi(WLC_PI(airiqh->wlc));
	/* Remove fifo suspend for tx data */
	wlc_tx_resume(airiqh->wlc);
	/* Remove block for tx data */
	wlc_block_datafifo(airiqh->wlc, DATA_BLOCK_SCAN, 0);
...
{
	if (wlc->airiq) {
		if (PHYMODE(wlc) == PHYMODE_3x3_1x1)
			return wlc->airiq->scan_enable;
		else
			return FALSE;
	}
