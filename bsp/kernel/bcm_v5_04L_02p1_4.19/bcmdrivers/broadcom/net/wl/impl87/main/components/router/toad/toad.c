 ...
 *
 * A failure to retrieve the information assumes that the scheduler is not supported.
 */
static int
toad_scheduler_enabled(toad_context *tc)
{
	char iob[ sizeof("taf") + sizeof(wl_taf_define_t) + sizeof("enable") + 1 ];
	/*
	 * NB text param block in taf define iovar requires terminating '\0' to signify
	 * no param is given; setting buffer size + 1 above allocates space for that;
	 * the memset 0 later will ensure the trailing '\0'
	 */
	wl_taf_define_t *td;
	int rc;
	...
	TOAD_TRACE(tc, ("wl get taf enable failed, status %d, assuming not supported.\n", rc));
	return rc;
}
...
toad_station_list_arrange(toad_context *tc)
{
	...
	if (tc->ordering && toad_station_count(tc)) {
		...
		TOAD_TRACE(tc, ("sorting mbase %p[%lu] sz %lu\n", mbase,
				(unsigned long)mcount, (unsigned long)msize));
		...
		TOAD_TRACE(tc, ("re-prioritizing %lu stations\n", (unsigned long)mcount));
...
/*
 * toad_retrieve_sta_ssid() - retrieve the station SSID and bsscfg index from the driver.
 *
 * FIXME: We need a way (new iovar?) to determine the SSID name and index for this station.
 * (index being the relative index of this ssid as defined in the router config). For the
 * time being, we are using the SSID from either the nvram or the command line, hence this
...
toad_station_list_retrieve(toad_context *tc)
{
	...
	rc = toad_scheduler_enabled(tc);
	if (rc < 0) {
		TOAD_ERROR(tc, ("TAF is not supported on interface %s\n", toad_main_ifname(tc)));
		return -1;
	} else if (rc == 0) {
...
toad_main_loop(toad_context *tc)
{
	...
	while (rc == 0) {
		/* TOAD to reload settings from nvram in every iteration */
		if (tc->settings) {
		...
		if (!tc->settings) {
			toa_log_error("Failed to load settings!\n");
