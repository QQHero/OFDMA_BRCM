 ...
 *
 * $Id: phy_utils_radio.c 681646 2017-01-27 17:03:36Z $
 */
...
phy_utils_valid_radio(phy_info_t *pi)
{
	if (VALID_RADIO(pi, RADIOID(pi->pubpi->radioid))) {
		...
		} else if (PI_INSTANCE(pi) == 1 && pi->pubpi->radiorev != BCMRADIOREV_AUX) {
			PHY_ERROR(("%s: Chip's radiorev=%d, BCMRADIOREV_AUX=%d\n",
			           __FUNCTION__, pi->pubpi->radiorev, BCMRADIOREV_AUX));
			ASSERT(pi->pubpi->radiorev == BCMRADIOREV_AUX);
		}
#else
