...
ifeq ($(CONFIG_UGW_BCM_DONGLE),1)
	WIFI_KERNEL_SRC = $(TOP_PATH)/cbb/wifi/tendawifi/kernel
	EXTRA_CFLAGS += -I$(WIFI_KERNEL_SRC)/dongle_drv/td_wl_core
	EXTRA_CFLAGS += -I$(WIFI_KERNEL_SRC)/dongle_drv/include
	EXTRA_CFLAGS += -I$(WIFI_KERNEL_SRC)/dongle_drv/td_wldebug
	EXTRA_CFLAGS += -I$(WIFI_KERNEL_SRC)/dongle_drv/td_multiap_steer
	EXTRA_CFLAGS += -I$(WIFI_KERNEL_SRC)/dongle_drv/td_private_ie/inc
ifeq ($(TD_EASYMESH_SUPPORT), 1)
	EXTRA_CFLAGS += -I$(WIFI_KERNEL_SRC)/dongle_drv/td_easymesh
	EXTRA_CFLAGS += -DBCM -DTD_EASYMESH_SUPPORT
endif

else
    EXTRA_CFLAGS += -I$(WLSRC_BASE)/wl/td_proc/

	WLFILES_SRC += src/wl/td_proc/td_wlan_proc.c
	WLFILES_SRC += src/wl/td_proc/td_debug.c
	WLFILES_SRC += src/wl/td_proc/td_skb.c

ifeq ($(TD_EASYMESH_SUPPORT), 1)
	EXTRA_CFLAGS += -DTD_EASYMESH_SUPPORT -DTD_EM_BROADCOM -I$(WLSRC_BASE)/wl/td_easymesh/td_easymesh_common/include
endif
ifeq ($(AI_MESH_SUPPORT), 1)
	EXTRA_CFLAGS += -DAI_MESH_SUPPORT -I$(WLSRC_BASE)/wl/ai_mesh
endif

ifeq ($(CONFIG_WIFIBASE), 1)
#ifeq ($(CONFIG_WB_EASYMESH), 1)
	#EXTRA_CFLAGS += -DCONFIG_WB_EASYMESH
#endif
	EXTRA_CFLAGS += -DCONFIG_WIFIBASE -I$(WLSRC_BASE)/wl/td_wifibase/ -I$(WIRELESS_ROOT_DIR)/wifibase/kernel/include/ 
endif
endif
...
ifndef CONFIG_UGW_BCM_DONGLE
EXTRA_CFLAGS += -I$(WLSRC_BASE)/wl/td_proc/
endif
#此处新增的宏BCM需要和private_ie模块中的宏保持一致，否则会导致编译问题
ifeq ($(TD_PRIVATE_IE_SUPPORT), 1)
	EXTRA_CFLAGS += -DTD_PRIVATE_IE -DBCM -I$(WLSRC_BASE)/wl/td_privite_ie/include
endif
ifeq ($(TD_STEER), 1)
	EXTRA_CFLAGS += -DTD_STEER -DBROADCOM -I$(WLSRC_BASE)/wl/td_multiap_steer -DBRCM_CHIP=$(BRCM_CHIP)
endif
ifeq ($(TD_BCM6710_WIFI_SUPPORT), 1)
	EXTRA_CFLAGS += -DTD_6710_WIFI6G
endif
#endif
...
else ifneq (,$(filter "y","$(BUILD_43602NIC)"))
	WLTUNEFILE := wltunable_lx_43602nic.h
else
