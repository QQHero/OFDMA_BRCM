 ...
 *
 * $Id: wlc_iocv_high.c 782710 2020-01-02 10:18:53Z $
 */
	...
	void *high_ctx, void *low_ctx)
{
	...
	if ((high = MALLOCZ(osh, sz)) == NULL) {
		WL_IOCV_ERR(("%s: high MALLOC failed\n", __FUNCTION__));
		return NULL;
	}
	high->osh = osh;
	...
	/* allocate storage */
	if ((cmn = MALLOCZ(osh, sz)) == NULL) {
		WL_IOCV_ERR(("%s: cmn MALLOC failed\n", __FUNCTION__));
		return NULL;
	}
	high->cmn = cmn;
	if (cmn->iovt == NULL) {
