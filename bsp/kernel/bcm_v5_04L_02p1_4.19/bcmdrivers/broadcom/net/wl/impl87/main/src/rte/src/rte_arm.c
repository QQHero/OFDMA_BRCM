 ...
 *
 * $Id: rte_arm.c 804053 2021-10-18 16:20:23Z $
 */
...
hnd_print_stack(uint32 sp)
{
	...
	for (i = 0, j = 0; j < 16 && i < stack_headroom; i++) {
		...
		if ((stack[i] <= (uint32)info._text_end) ||
		    ...
		    (stack[i] >= (uint32)&pageable_start && stack[i] <= (uint32)&pageable_end) ||
#endif /* SW_PAGING */
		    FALSE) {
			printf("sp+%x %08x\n", (i*4), stack[i]);
			j++;
 ...
 */
static uint8 __attribute__ ((__section__ (".pagetable.")))
	loader_pagetable_array[PAGETABLE_SIZE];
static uint8 __attribute__ ((__section__ (".pagetable2.")))
	l2_pagetable_array[L2_PAGETABLE_ARRAY_SIZE];
...
uint32
hnd_get_pagetable_size(void)
{
	return sizeof(loader_pagetable_array) +
		sizeof(l2_pagetable_array);
}
...
extern char pagetable_start[];
extern char pagetable_end[];

/* Update the existing MMU table.
 * Protect text area by making it readonly and executable.
 * Adjacent to the text area, the area upto _ram_mpu_region_end (includes rodata) is protected with
 * "Do Not eXecute" and is readonly.
 * The rest is considered data area and is protected with "Do Not eXecute" and is writable.
 * This is to protect against buffer overflows which are being misused for hacking purpose.
 * This function should be called after all initialisation has finished and all memory has been
 * reclaimed. Once reclaim is done all heap and stack can be marked as non-executable.
 * Testing:
...
ca7_execute_protect_on(si_t *sih)
{
	...
	/* Round up text_split on page boundary. This is needed because the memory
	 * following this text will be made NOEXEC_NOWR and all text needs to be executable.
	 */
	text_split = ((uint32)text_end + PAGE_SIZE) & ~(PAGE_SIZE - 1);
	addr = 0;
	size = text_split;
	...
	/* Round down nowr_split on page boundary. This is needed because _ram_mpu_region_end
	 * may not be aligned on a page boundary. The memory following that will need to be
	 * writable.
	 */
	nowr_split = (uint32)_ram_mpu_region_end & ~(PAGE_SIZE - 1);
	addr = text_split;
	size = nowr_split - text_split;
	/* Verify that the NOEXEC_NOWR code section is at least 1 PAGE in size */
	ASSERT(size >= PAGE_SIZE);
	/* Now make the code section in RAM RO (with XN) */
	MMU_MSG(("%s: setting 0x%08x (size 0x%08x) as NOEXEC_NOWR\n", __FUNCTION__,
		addr, size));
	HND_MMU_SET_L2_PAGE_ATTRIBUTES(addr, addr, size, L2S_PAGE_ATTR_NOEXEC_NOWR, FALSE);
	...
	/* Mark the pagetables as RO */
	addr = (uint32)pagetable_start;
	size = ((uint32)pagetable_end & ~(PAGE_SIZE - 1)) - addr;
	MMU_MSG(("%s: setting 0x%08x (size 0x%08x) as NOEXEC_NOWR\n", __FUNCTION__,
		addr, size));
	HND_MMU_SET_L2_PAGE_ATTRIBUTES(addr, addr, size, L2S_PAGE_ATTR_NOEXEC_NOWR, FALSE);
...
/* Update the existing MMU table.
 * During trap handling rodata is used to store the "fatal buffer", see hnd_get_fatal_logbuf.
 * To avoid a trap, protection needs to be disabled on the rodata region.
 */
void
ca7_disable_rodata_protection(si_t *sih)
{
	uint32 nowr_split, text_split;
	uint32 addr, size;
	...
	MMU_PRT(("Turn rodata memory protection OFF\n"));

	/* Round down text_split on page boundary. This is needed because the entire rodata
	 * region needs to be writable
	 */
	text_split = ((uint32)text_end) & ~(PAGE_SIZE - 1);

	/* Round down nowr_split on page boundary, the same slit as used in ca7_execute_protect_on.
	 */
	nowr_split = (uint32)_ram_mpu_region_end & ~(PAGE_SIZE - 1);

	addr = text_split;
	size = nowr_split - text_split;
	/* Make the rodata section in RAM normal memory */
