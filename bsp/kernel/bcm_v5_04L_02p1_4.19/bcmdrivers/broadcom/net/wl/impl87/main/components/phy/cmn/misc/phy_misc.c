 ...
 *
 * $Id: phy_misc.c 798316 2021-04-29 09:36:22Z $
 */
...
wlc_phy_test_init(phy_info_t *pi, int channel, bool txpkt)
{
	...
	wlc_phy_chanspec_set(pi, CH20MHZ_CHSPEC(channel, WL_CHANNEL_2G5G_BAND(channel)));
...
void
wlc_phy_set_deaf(phy_info_t *pi, bool user_flag)
{
	phy_type_misc_fns_t *fns = pi->misci->fns;
...
void
wlc_phy_clear_deaf(phy_info_t *pi, bool user_flag)
{
	phy_type_misc_fns_t *fns = pi->misci->fns;
...
/**
 * @param int_val       0:             stop transmitting tone
 *                     -1:             transmit 0KHz tone
 *                    <-50 or >50:     transmit [int_val * 2] KHz tone
...
wlc_phy_iovar_tx_tone(phy_info_t *pi, int32 int_val)
{
	...
	if (fns->phy_type_misc_iovar_tx_tone != NULL) {
		ret = fns->phy_type_misc_iovar_tx_tone(fns->ctx, int_val);
	}
	...
	struct wlc_if *wlcif, int err)
{
	...
	/* store chanspec */
	chspec = phy_utils_get_chanspec(pi);

	/* prepare IOV buffer for IOV SET CHANSPEC */
	bzero(iovbuf, sizeof(iovbuf));
	strncpy(iovbuf, "chanspec", strlen("chanspec"));

	for (result->nvalues = 0; result->nvalues < list->count; ++result->nvalues) {
		chanspec_t sweep_chspec;

		/* set sweep chanspec arg for IOV SET CHANSPEC */
		sweep_chspec = WL_CHANSPEC_BW_20 |
				WL_CHANNEL_2G5G_BAND(list->element[result->nvalues]) |
				list->element[result->nvalues];
		*(chanspec_t*)(iovbuf + strlen("chanspec") + 1) = sweep_chspec;
		IFERR(wlapi_wlc_ioctl(pi->sh->physhim, WLC_SET_VAR, &iovbuf,
				(sizeof(uint32) + strlen("chanspec") + 1), wlcif));
	...
	bzero(iovbuf, sizeof(iovbuf));
	strncpy(iovbuf, "chanspec", strlen("chanspec"));
	*(chanspec_t*)(iovbuf + strlen("chanspec") + 1) = chspec;
	IFERR(wlapi_wlc_ioctl(pi->sh->physhim, WLC_SET_VAR, &iovbuf,
			(sizeof(uint32) + strlen("chanspec") + 1), wlcif));
...
#ifdef ATE_BUILD
void
wlc_phy_gpaio(phy_info_t *pi, wl_gpaio_option_t option, int core)
{
	ASSERT(pi != NULL);
...
void
wlc_phy_hold_upd(phy_info_t *pi, mbool id, bool set)
{
	ASSERT(id);
	...
	if (id & PHY_HOLD_FOR_SCAN) {
		...
		if (set && !(mboolisset(pi->measure_hold, PHY_HOLD_FOR_SCAN))) {
			phy_watchdog_suspend(pi);
		} else if (!set && (mboolisset(pi->measure_hold, PHY_HOLD_FOR_SCAN))) {
...
void
wlc_phy_mute_upd(phy_info_t *pi, bool mute, mbool flags)
{
...
#ifdef ATE_BUILD
void
wlc_ate_gpiosel_disable(phy_info_t *pi)
{
 ...
 */
uint8
wlc_phy_get_bfe_ndp_recvstreams(phy_info_t *pi)
{
	phy_misc_info_t *misci = pi->misci;
	phy_type_misc_fns_t *fns = misci->fns;
	phy_type_misc_ctx_t *ctx = fns->ctx;
...
void
phy_misc_set_ldpc_override(phy_info_t *pi, bool ldpc)
{
	phy_misc_info_t *misci = pi->misci;
	phy_type_misc_fns_t *fns = misci->fns;
	phy_type_misc_ctx_t *ctx = fns->ctx;
...
void
wlc_phy_preamble_override_set(phy_info_t *pi, int8 val)
{
	phy_misc_info_t *misci = pi->misci;
	phy_type_misc_fns_t *fns = misci->fns;
	phy_type_misc_ctx_t *ctx = fns->ctx;
...
#ifdef WFD_PHY_LL
void
wlc_phy_wfdll_chan_active(phy_info_t *pi, bool chan_active)
{
	phy_misc_info_t *misci = pi->misci;
	phy_type_misc_fns_t *fns = misci->fns;
	phy_type_misc_ctx_t *ctx = fns->ctx;
...
int
phy_misc_set_lo_gain_nbcal(phy_info_t *pi, bool lo_gain)
{
	phy_type_misc_fns_t *fns = pi->misci->fns;
...
/* WARs */
int
phy_misc_set_filt_war(phy_info_t *pi, bool filt_war)
{
	phy_type_misc_fns_t *fns = pi->misci->fns;
...
bool
phy_misc_get_filt_war(phy_info_t *pi)
{
	phy_type_misc_fns_t *fns = pi->misci->fns;
...
int
phy_misc_tkip_rifs_war(phy_info_t *pi, uint8 rifs)
{
	phy_type_misc_fns_t *fns = pi->misci->fns;
...
uint16
wlc_phy_classifier(phy_info_t *pi, uint16 mask, uint16 val)
{
	phy_type_misc_fns_t *fns = pi->misci->fns;
