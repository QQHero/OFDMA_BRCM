...
{"Eth", "Eth",
 "Wi-Fi 11b", "Wi-Fi 11g", "Wi-Fi 11a", "Wi-Fi 11n24", "Wi-Fi 11n5", "Wi-Fi 11ac", "Wi-Fi 11ad", "Wi-Fi 11af", "Wi-Fi 11ax",
 "PLC", "PLC",
 "MoCA",
 "Unknown"};
...
static unsigned char const* i5UtilsGetNameForMediaType(unsigned short mediaType)
{
   switch (mediaType) {
      ...
      default:
         return i5UtilsInterfaceNamesForMediaTypes[14];
   }
}
...
#define I5_BRCM_VNDR_OUI  "\x00\x10\x18"
...
/* Convert string format of OUI(colon separated) to binary */
static inline int i5String2OUI(const char *stroui, unsigned char *oui, unsigned char oui_len)
{
  int i = 0;
  char *c = NULL;
  ...
  if (stroui && oui) {
    ...
    for (;;) {
      oui[i++] = (uint8)strtoul(stroui, &c, 16);
      if (!*c++ || i == oui_len) {
        break;
      }
      stroui = c;
...
static inline unsigned char getYANGTimeStampToString(time_t in_time, char *buf, int buf_len)
{
  char tm_buf[IEEE1905_TS_MAX_LEN];
  unsigned char len = 0;
  struct tm *ts;

  ts = localtime(&in_time);
  strftime(tm_buf, sizeof(tm_buf), "%Y-%m-%dT%H:%M:%S.0Z%z", ts);
  memcpy(&buf[len], tm_buf, strlen(tm_buf)-2);
  len += strlen(tm_buf)-2;
  buf[len++] = ':';
  memcpy(&buf[len], &tm_buf[strlen(tm_buf)-2], 2);
  len += 2;
