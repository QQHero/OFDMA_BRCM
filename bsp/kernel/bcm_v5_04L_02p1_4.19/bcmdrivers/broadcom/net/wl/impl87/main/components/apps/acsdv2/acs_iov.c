 ...
 *
 *	$Id: acs_iov.c 793825 2020-12-11 05:26:14Z $
 */
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
acs_get_chanspec_info(acs_chaninfo_t *c_info, chanspec_t chspec)
{
	...
	FOREACH_20_SB(chspec, sub_channel) {
		sub_chspec = CH20MHZ_CHSPEC(sub_channel, WL_CHANNEL_2G5G_BAND(sub_channel));
...
#endif /* ZDFS_2G */
int acs_get_reg_txpwr_info(acs_chaninfo_t *c_info, wl_chanspec_reg_txpwr_t *param, int param_len,
	char *buf, int buf_len)
{
	return wl_iovar_getbuf(c_info->name, "chanspec_reg_txpwr", param, param_len, buf, buf_len);
}
