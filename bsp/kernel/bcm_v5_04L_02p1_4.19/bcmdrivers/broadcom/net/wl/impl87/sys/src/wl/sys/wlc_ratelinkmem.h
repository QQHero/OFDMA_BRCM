 ...
 *
 *
 * $Id: wlc_ratelinkmem.h 803989 2021-10-14 19:10:00Z $
 */
...
typedef struct wlc_rlm_rate_store {
...
#else /* WL_RATELINKMEM */
#define wlc_ratelinkmem_hw_init(wlc)
#define wlc_ratelinkmem_vbss_link_index(wlc, bsscfg) \
	((0 && (bsscfg))? D11_RATE_LINK_MEM_IDX_INVALID : D11_RATE_LINK_MEM_IDX_INVALID)
#define wlc_ratelinkmem_get_scb_rate_index(wlc, scb) \
	((0 && (scb))? D11_RATE_LINK_MEM_IDX_INVALID : D11_RATE_LINK_MEM_IDX_INVALID)
#define wlc_ratelinkmem_get_scb_link_index(wlc, scb) \
	((0 && (scb))? D11_RATE_LINK_MEM_IDX_INVALID : D11_RATE_LINK_MEM_IDX_INVALID)
#define wlc_ratelinkmem_retrieve_cur_scb(wlc, index) ((0 & (index)) ? NULL : (void*)0)
#define wlc_ratelinkmem_retrieve_cur_rate_store(wlc, index, newrate) \
	((0 & (index) & (newrate)) ? NULL : (void*)0)
#define wlc_ratelinkmem_retrieve_cur_link_entry(wlc, index) ((0 & (index)) ? NULL : (void*)0)
#define wlc_ratelinkmem_update_rate_entry(wlc, scb, rateset, flags) \
	({BCM_REFERENCE(rateset); (0 && (scb) && (flags)) ? BCME_ERROR : BCME_ERROR;})
#define wlc_ratelinkmem_upd_txstatus(wlc, index, flags) \
	({BCM_REFERENCE(index); BCM_REFERENCE(flags);})
#define wlc_ratelinkmem_update_link_entry(wlc, scb) (BCM_REFERENCE(scb))
#define wlc_ratelinkmem_upd_lmem_int(wlc, scb, clr_txbf_stats)
#define wlc_ratelinkmem_update_link_entry_all(wlc, bsscfg, ampdu_only, clr_txbf_stats)
#define wlc_ratelinkmem_rate_store_dump(wlc, rate_store, b)
#define wlc_ratelinkmem_link_entry_dump(wlc, link_entry, b)
#define wlc_ratelinkmem_update_band(wlc)
#define wlc_ratelinkmem_lmem_isvalid(wlc, scb) (FALSE)
#define wlc_ratelinkmem_scb_amt_release(wlc, scb, index)
