...
#endif /* !(defined(BCMHWA) && defined(HWA_RXPOST_BUILD)) */
...
static bool pciedev_process_tx_post(struct dngl_bus *bus, void *p, uint16 msglen,
...
static void pciedev_sync_dmaindex_d2h_wr(struct dngl_bus * pciedev, msgbuf_ring_t* ring);
...
static int pciedev_update_txstatus(struct dngl_bus *pciedev, uint32 status,
	uint16 rindex, uint16 flowid, uint16 seq, bool hold, uint8 *key_seq);
...
static int pciedev_update_txstatus(struct dngl_bus *pciedev, uint32 status,
	uint16 flowid, bool hold);

#if defined(HWA_PKTPGR_BUILD)
static int pciedev_update_multiple_txstatus(struct dngl_bus *pciedev, uint32 status,
	uint16 flowid, bool hold, void *p);

#define PCIEDEV_UPDATE_MULTIPLE_TXSTATUS(pciedev, status, flowid, hold, p) \
	pciedev_update_multiple_txstatus(pciedev, status, flowid, hold, p)
#else /* ! HWA_PKTPGR_BUILD */

#define PCIEDEV_UPDATE_MULTIPLE_TXSTATUS(pciedev, status, flowid, hold, p) \
	pciedev_update_txstatus(pciedev, status, flowid, hold)
#endif /* ! HWA_PKTPGR_BUILD */

#endif /* ! PROP_TXSTATUS */

#ifndef HWA_TXPOST_BUILD
static int pciedev_v2r_fetch_packets(struct dngl_bus *pciedev,
	msgbuf_ring_t *flow_ring, uint16 v2r_request, bool sqs_force);
...
{
	uint8 tid_ac = MSGBUF_TID(msgbuf);

	if (SCB_FLOWID_VALID(msgbuf->scb_flowid)) {
		wlc_sqs_vpkts_rewind(msgbuf->scb_flowid, tid_ac, count);
	}
}
...
sqs_vpkts_forward(struct dngl_bus *pciedev, msgbuf_ring_t *msgbuf, uint16 count)
{
	...
	if (SCB_FLOWID_VALID(msgbuf->scb_flowid)) {
		wlc_sqs_vpkts_forward(msgbuf->scb_flowid, tid_ac, count);
	}
}
...
sqs_flow_ring_active(void* arg, uint16 ringid)
{
	 ...
	 */
	if ((FLOW_RING_SUP_PEND_ISSET(msgbuf)) ||
#ifdef PCIEDEV_FAST_DELETE_RING
...
static uint16 /* Handle a V2R request */
sqs_v2r_request_hwa(void* arg, uint16 ringid, uint16 v2r_request, bool sqs_force)
{
	struct dngl_bus *pciedev = (struct dngl_bus *)arg;
	msgbuf_ring_t *msgbuf;
	uint16 wi_index;
...
#ifdef HNDPQP
	ret_len = MIN((int)ret_len, hbm_avail);
#endif /* HNDPQP */
...
#endif /* PROP_TXSTATUS */
		msgbuf->fetch_pending += process_len;
		pciedev->fetch_pending += process_len;
...
/* Use M2M DMA to fetch packets from flow ring */
static uint16 /* Handle a V2R request */
sqs_v2r_request_legacy(void* arg, uint16 ringid, uint16 v2r_request, bool sqs_force)
{
	struct dngl_bus *pciedev = (struct dngl_bus *)arg;
	msgbuf_ring_t *msgbuf;
	uint16 maxpktcnt, remain_len, transfer_count;
	uint16 ret = 0;
	uint16 process_len = 0;

	msgbuf = pciedev->h2d_submitring_ptr[ringid];
	ASSERT(msgbuf);

	remain_len = v2r_request;
	maxpktcnt = msgbuf->flow_info.maxpktcnt;

	/* Break up the fetch request into chunks of maxpktcnt */
	while (remain_len) {
		transfer_count = MIN(remain_len, maxpktcnt);

		/* Submit the fetch request */
		ret = pciedev_v2r_fetch_packets(pciedev, msgbuf, transfer_count, sqs_force);

		if (ret == 0) {
			/* V2R fetch failed due to resource crunch */
			break;
		}
		process_len += ret;
		remain_len -= ret;
...
/* Convert virtual packets in Host to real pkts in dongle
 * @param ringid  Flow irngid
 * @param v2r_request  No of packets to be fetched.
 * @param sqs_force : Request coming form a TAF scheduler or not
 *
...
sqs_v2r_request(void* arg, uint16 ringid, uint16 v2r_request, bool sqs_force)
{
	uint16 ret = 0;
	struct dngl_bus *pciedev = (struct dngl_bus *)arg;
	msgbuf_ring_t *msgbuf;
	...
	PCI_TRACE(("%s: ringid %d v2r_request %d force %d \n",
		__FUNCTION__, ringid,
		v2r_request, sqs_force));

	/* Don't block v2r request if FLOW_RING_FLUSH_PENDING is set */
	if (FLOW_RING_SUP_PEND_ISSET(msgbuf) && !FLOW_RING_FLUSH_PEND_ISSET(msgbuf)) {
		return 0;
	}

#ifdef PROP_TXSTATUS
	/* If requesting from a PS POLL, make sure one whole mpdu is pulled */
	if (sqs_force) {
		v2r_request = MAX(v2r_request, pciedev_reuse_seq_amsdu_cnt(pciedev, msgbuf));
	}
#endif /* PROP_TXSTATUS */
	tid_ac = MSGBUF_TID(msgbuf);

	ASSERT(v2r_request <= MSGBUF_VPKTS(msgbuf));

#if defined(BCMHWA) && defined(HWA_TXPOST_BUILD)
	ret = sqs_v2r_request_hwa(arg, ringid, v2r_request, sqs_force);
#else
	ret = sqs_v2r_request_legacy(arg, ringid, v2r_request, sqs_force);
#endif /* BCMHWA && HWA_TXPOST_BUILD */

	if (SCB_FLOWID_VALID(msgbuf->scb_flowid) && (ret != 0)) {
		wlc_sqs_v2r_enqueue(msgbuf->scb_flowid, tid_ac, ret);
	}
 ...
 */
int
sqs_fetch_packets_force(struct dngl_bus *pciedev, msgbuf_ring_t *msgbuf)
{
	int v2r_request;
	int ret = 0;

	if (SCB_FLOWID_VALID(msgbuf->scb_flowid)) {
		/* Update virtual packets */
		MSGBUF_VPKTS_ENQ(msgbuf);
	}

	/* Estimate the v2r fetch count */
	v2r_request = MSGBUF_VPKTS(msgbuf);
	...
	if (v2r_request > 0) {
		 ...
		 * completion of DMA.
		 */
		ret = sqs_v2r_request(pciedev, msgbuf->ringid, v2r_request, TRUE);
	}
...
pciedev_schedule_flow_ring_read_buffer(struct dngl_bus *pciedev)
{
	...
	while (!dll_end(prio, &pciedev->active_prioring_list)) {
		...
		while (1) {
			...
			if ((flow_ring->scb_flowid == SCB_FLOWID_INVALID) &&
				(flow_ring->status & FLOW_RING_ROAM_CHECK)) {
				int response_cfp;
				response_cfp = dngl_flowring_update(pciedev->dngl,
					flow_ring->flow_info.ifindex,
					flow_ring->ringid, tid_ac,
					flow_ring->flow_info.da, FLOW_RING_CREATE,
				...
				if (response_cfp == BCME_OK) {
					PCI_PRINT(("CFP relink succeeded for ringid<%d>"
						...
						flow_ring->fetch_ptr,
						MSGBUF_WR_IDX_PEEKED(flow_ring)));
					/* Sync the wr_peeked pointer */
					MSGBUF_WR_IDX_PEEKED(flow_ring) = flow_ring->fetch_ptr;
				}
				flow_ring->status &= ~FLOW_RING_ROAM_CHECK;
			}

			if (!SCB_FLOWID_VALID(flow_ring->scb_flowid)) {
			...
			if (!flow_ring->flow_info.pktinflight && !flow_ring->fetch_pending) {
				FLOW_RING_SUP_PEND_RESET(flow_ring);
			}

			/* Update virtual packets */
			MSGBUF_VPKTS_ENQ(flow_ring);

			if (FLOW_RING_SUP_PEND_ISSET(flow_ring)) {
				goto nextnode;
			}

#ifdef WLTAF
			/* Trigger TAF scheduler if atleast one flow ring has virtual packets */
			if (pciedev->taf_scheduler && flow_ring->taf_capable &&
				MSGBUF_VPKTS(flow_ring)) {
				taf_schedule = TRUE;
				v2r_request = 0;
			} else
#endif /* WLTAF */
			{
				v2r_request = MSGBUF_VPKTS(flow_ring);
			}
			...
			if (sqs_fetch_allowed && (v2r_request > 0)) {
				...
				ret = sqs_v2r_request(pciedev, flow_ring->ringid,
					v2r_request, TRUE);
				if (ret) {
...
static int
pciedev_v2r_fetch_packets(struct dngl_bus *pciedev, msgbuf_ring_t *flow_ring,
	uint16 v2r_request, bool sqs_force)
{
	uint8 *dest_addr;
	uint8 *host_addr;		/**< lower 32 bits of an address on the host */
	...
	ret_len = MIN(ret_len, v2r_request);
	...
	if (dest_addr != NULL) {
		...
		if (sqs_force) {
			PCIEDEV_FLOW_FETCH_FLAG_SQS_FORCE_SET(flowfetchp);
		}
#ifdef BCMPCIEDEV
		if (BCMPCIEDEV_ENAB()) {
		...
		pciedev->last_flowfetchp = flowfetchp;
...
cleanup:
	DBG_BUS_INC(pciedev, pciedev_v2r_fetch_packets);
#ifdef PROP_TXSTATUS
	pciedev_update_rdp_ptr_unacked(flow_ring);
#endif /* PROP_TXSTATUS */
...
pciedev_flowring_fetch_cb(struct fetch_rqst *fr, bool cancelled)
{
	...
	pciedev_sqs_v2r_dequeue(pciedev, flow_fetch->msg_ring->ringid,
		MSGBUF_TID(flow_fetch->msg_ring),
		(fr->size - offset)/MSGBUF_LEN(flow_fetch->msg_ring),	/* Packet count */
		PCIEDEV_FLOW_FETCH_FLAG_SQS_FORCE_ISSET(flow_fetch)); 	/* Force request */
...
cleanup:
...
#ifdef WLTAF
			/* Once a fetch request is completly handled, inform TAF */
			if (PCIEDEV_FLOW_FETCH_FLAG_EOPS_RQST_ISSET(flow_fetch)) {
	...
	if (!flow_fetch->msg_ring->flow_info.pktinflight &&
		...
		FLOW_RING_SUP_PEND_RESET(flow_fetch->msg_ring);
	}
	...
	uint16 flowid, uint16 seq, bool hold, uint8 *key_seq)
{
	...
	ASSERT(flow_ring->flow_info.pktinflight);
	...
	if (((status == WLFC_CTL_PKTFLAG_D11SUPPRESS) ||
		...
		if (flow_ring->flow_info.pktinflight > 0 || flow_ring->fetch_pending > 0) {
			FLOW_RING_SUP_PEND_SET(flow_ring);
...
/**
 * Called when eg the wl layer reported status back on the transmission of a packet.
 *
 * Update the flow ring (in device memory) designated by (flowid) with the caller supplied
 * parameters, so the host can react to the new status of the flow ring. Does not generate an
 ...
 */
static int
pciedev_update_txstatus(struct dngl_bus *pciedev, uint32 status, uint16 flowid, bool hold)
{
	msgbuf_ring_t *flow_ring;
...
#if defined(HWA_PKTPGR_BUILD)
static int
pciedev_update_multiple_txstatus(struct dngl_bus *pciedev, uint32 status,
	uint16 flowid, bool hold, void *p)
{
	uint8 frag_num, idx;
	...
	for (; p; p = PKTNEXT(pciedev->osh, p)) {
		/* frag_num could be 0 (pkt fetch), always update first one */
		frag_num = PKTFRAGTOTNUM(pciedev->osh, p);
		ASSERT(frag_num <= HWA_PP_PKT_FRAGINFO_MAX);
...
#if defined(PROP_TXSTATUS)
	pciedev_update_txstatus(pciedev, WLFC_CTL_PKTFLAG_WLSUPPRESS,
		PKTFRAGRINGINDEX(pciedev->osh, lfrag), flowid, 0, FALSE, NULL);
#else /* ! PROP_TXSTATUS */
	pciedev_update_txstatus(pciedev, WLFC_CTL_PKTFLAG_WLSUPPRESS, flowid, FALSE);
#endif /* ! PROP_TXSTATUS */
...
#else /* !(defined(BCMHWA) && defined(HWA_RXPOST_BUILD)) */
static int
pciedev_process_rxpost_msg(struct dngl_bus *pciedev, uint32 *bufid, uint16 *len,
	dma64addr_t *haddr)
{
...
pciedev_ring_update_readptr(struct dngl_bus *pciedev, msgbuf_ring_t *ring, uint16 bytes_read)
{
	...
	if (ring->dmaindex_d2h_supported && (pciedev->dmaindex_h2d_rd.inited)) {
		pciedev_sync_dmaindex_h2d_rd(pciedev, ring);
	}
#endif
}
...
pciedev_ring_update_writeptr(msgbuf_ring_t *ring, uint16 bytes_written)
{
	...
	if (ring->dmaindex_d2h_supported && pciedev->dmaindex_d2h_wr.inited) {
			...
			pciedev_sync_dmaindex_d2h_wr(pciedev, ring);
	}
#else
...
pciedev_resource_avail_check(struct dngl_bus *pciedev, msgbuf_ring_t *msgbuf)
{
	...
	if (MSGBUF_CHECK_NOWRITE_SPACE(msgbuf) && pciedev->dmaindex_d2h_rd.inited) {
		...
		pciedev_sync_dmaindex_d2h_rd(pciedev, msgbuf);
	}
#endif /* PCIE_DMA_INDEX */
...
#if defined(PROP_TXSTATUS) && defined(PCIEDEV_FAST_DELETE_RING)
static bool
...
#endif /* HWA_PKTPGR_BUILD */
#endif /* PROP_TXSTATUS && PCIEDEV_FAST_DELETE_RING */
...
#if !defined(PROP_TXSTATUS)
		/* With HNDPQP, delete_idx always points to MSGBUF_RD */
		ASSERT(flow_ring->status & FLOW_RING_FAST_DELETE_ACTIVE);
#else /* PROP_TXSTATUS */
		/* Check if the rd_idx of the packet is "after" ring delete_idx */
		if (PCIEDEV_FASTDELETE_NOTXSTATUS(pciedev, flow_ring,
			PKTFRAGRINGINDEX(pciedev->osh, p), txstatus, &ts, p))
#endif /* PROP_TXSTATUS */
		{
...
pciedev_queue_rxcomplete_msgring(struct dngl_bus *pciedev, msgbuf_ring_t *ring)
{
	...
	defined(HWA_RXPOST_BUILD)
		if (!avail_host_ring_entry && pciedev->dmaindex_d2h_rd.inited)
			pciedev_sync_dmaindex_d2h_rd(pciedev, ring);
#endif
		return;
	}
...
pciedev_lbuf_callback(void *arg, void *p)
{
	...
	if (PKTISTXFRAG(pciedev->osh, p)) {
		...
		if (PCIEDEV_UPDATE_MULTIPLE_TXSTATUS(pciedev, status, ringid,
			PKTISTXSHOLD(pciedev->osh, p), p))
		{
			BCMPCIE_IPC_HPA_TEST(pciedev, PKTFRAGPKTID(pciedev->osh, p),
				BCMPCIE_IPC_PATH_TRANSMIT, BCMPCIE_IPC_TRANS_RESPONSE);
			BUZZZ_KPI_PKT1(KPI_PKT_BUS_TXCMPL, 2,
		...
		if (PKTNEEDRXCPL(pciedev->osh, p) || BCM_RXCPL_FRST_IN_FLUSH(p_rxcpl_info)) {
			 ...
			 * in PKTFREE path.
			 */
#if defined(HWA_PKTPGR_BUILD) || HWACONF_LE(130)
			if (PKTISRXFRAG(pciedev->osh, p)) {
...
#endif /* HWA_PKTPGR_BUILD || HWACONF_LE(130) */
...
#if defined(HWA_PKTPGR_BUILD)
void
pciedev_hwa_d2h_req_q_send(struct dngl_bus *pciedev)
{
	pciedev_queue_d2h_req_send(pciedev);
}
#endif /* HWA_PKTPGR_BUILD */
...
static inline void cfp_flows_sendup(uint16 scb_flowid, cfp_pktlist_t *pktlist);

/** Integrate CFP Flow classification within PCIe bus layer */
static inline void pciedev_classify(struct dngl_bus *pciedev, uint16 scb_flowid,
...
cfp_flows_sendup(uint16 scb_flowid, cfp_pktlist_t *pktlist)
{
	...
	wlc_cfp_tx_sendup(FD_CFP_UNIT, scb_flowid, pktlist->prio,
 ...
 */
static inline void
pciedev_classify(struct dngl_bus *pciedev, uint16 scb_flowid,
	void *pkt, uint8 pkt_prio, bool cfp_not_capable)
{
	...
	if (cfp_not_capable) {
		...
		ASSERT_SCB_FLOWID(scb_flowid);

		/* Save CFP flowid into pkt */
		PKTSETCFPFLOWID(pkt, scb_flowid); /* CFP packets have LLCSNAP */
		wlc_cfp_pkt_prepare(FD_CFP_UNIT, scb_flowid, pkt_prio,
...
pciedev_dispatch_pktlists(struct dngl_bus *pciedev, msgbuf_ring_t *msgbuf)
{
	...
	if (cfp_flows->pend == 1) { /* check if only one pktlist is pending */
		...
		CFP_FLOWS_TRACE(("CFP1 TX ringid<%u> scb_flowid<%u> pend<%u> "
			" prio<%u> pkts<%u> list<%p,%p>\n",
			msgbuf->ringid, scb_flowid, cfp_flows->pend,
			pktlist->prio, pktlist->pkts, pktlist->head, pktlist->tail));

		cfp_flows_sendup(scb_flowid, pktlist); /* CFP fastpath sendup */
		...
		while (pktlist < def_pktlist) { /* traverse CFP capable packet lists */
			...
			if (pktlist->head != NULL) { /* non-empty pktlist */
				...
				CFP_FLOWS_TRACE(("CFP# TX ringid<%u> scb_flowid<%u> pend<%u> "
					"prio<%u> pkts<%u> list<%p,%p>\n",
					msgbuf->ringid, scb_flowid, cfp_flows->pend,
		...
		if (def_pktlist->head != NULL) {
			CFP_FLOWS_TRACE(("DNGL TX ringid<%u> scb_flowid<%u> "
				"pkts<%u> list<%p,%p>\n",
				msgbuf->ringid, scb_flowid,
	...
	uint16 pktlen, msgbuf_ring_t *ring, uint16 fetch_ptr, uint32 flags)
{
	 ...
	 */
	cfp_enabled = wlc_cfp_tx_enabled(FD_CFP_UNIT, ring->scb_flowid, &pciedev->cfp_exptime);

#ifdef PROP_TXSTATUS
	cfp_enabled = cfp_enabled && (!FLOWRING_HAS_REUSED_SEQ_PKT(ring));
#endif /* PROP_TXSTATUS */
#endif /* WLCFP */
	...
	while (pktlen) {
		...
		if (ring->status & FLOW_RING_FLUSH_PENDING) {
			...
			if (IS_IOCTLREQ_PENDING(pciedev) ||
				...
				if (!pciedev_update_txstatus(pciedev, BCMPCIE_PKT_FLUSH, fetch_ptr,
					...
					if ((ring->status & FLOW_RING_DELETE_RESP_PENDING) &&
						(pciedev->fastdeletering) &&
						(PCIEDEV_FASTDELETE_NOTXSTATUS(pciedev, ring,
							fetch_ptr, BCMPCIE_PKT_FLUSH, NULL, NULL)))
					{
						goto skip_update;
					}
#endif /* PCIEDEV_FAST_DELETE_RING */
...
#else /* ! PROP_TXSTATUS */
				if (!pciedev_update_txstatus(pciedev, BCMPCIE_PKT_FLUSH,
					ring->ringid, FALSE)) {
					PCIEDEV_QUEUE_TXSTATUS(pciedev, pktid, if_id,
						ring->ringid, BCMPCIE_PKT_FLUSH, NULL);
				}
...
#ifdef PROP_TXSTATUS
		else if (!(FLOW_RING_SUP_PEND_ISSET(ring))) {
			if (amsdu_sup_part1->pkts) {
				void *lfrag, *n;
			...
			if (pciedev_process_tx_post(pciedev, p, msglen, ring->ringid, fetch_ptr,
				cfp_enabled, (pktlen == msglen)) == FALSE) {
				break;
			}
		} else {
			...
			if (!FLOW_RING_SUP_PEND_ISSET(ring)) {
...
skip_update:
	...
	if (!FLOW_RING_SUP_PEND_ISSET(ring)) {
		/* Advance the rd_ptr if not in suppression pending state */
		MSGBUF_RD(ring) = fetch_ptr;
	}
#endif /* HNDPQP */
...
#ifdef WLCFP
	pciedev_classify(pciedev, flow_ring->scb_flowid, lfrag, PKTPRIO(lfrag), cfp_not_capable);
#else /* ! WLCFP */
	...
	void **lfrag, uint16 fetch_ptr, bool last_packet)
{
	...
	ASSERT(!(FLOW_RING_SUP_PEND_ISSET(flow_ring)));
	...
	uint16 ringid, uint16 fetch_ptr, uint32 cfp_enabled, bool last_packet)
{
	...
	if (lfrag == NULL) {
		/* Refetch later from host */
#if defined(PROP_TXSTATUS)
		/* Mark this pkt as suppressed */
		flow_ring->flow_info.pktinflight++;
		pciedev->pend_user_tx_pkts++;
		pciedev_update_txstatus(pciedev, WLFC_CTL_PKTFLAG_WLSUPPRESS,
			fetch_ptr, ringid, 0, FALSE, NULL);
#else /* ! PROP_TXSTATUS */
		pciedev_rewind_flow_fetch_ptr(flow_ring, fetch_ptr);
#endif /* ! PROP_TXSTATUS */
...
#if defined(BCMFRAGPOOL) && defined(BCM_DHDHDR)
	headroom = PKTLEN(pciedev->osh, lfrag) - ETHER_HDR_LEN;
#endif /* BCMFRAGPOOL &&BCM_DHDHDR */
...
void
pciedev_sqs_stride_resume(struct dngl_bus *pciedev)
{
	pciedev_schedule_sqs_timer(pciedev);
}
       ...
       msgbuf_ring_t *flow_ring, uint16 index)
{
...
#ifdef BCM_DHDHDR
	{
		...
		if (isset(flow_ring->reuse_sup_seq, bit_idx)) {
			...
			if (isset(flow_ring->reuse_sup_key_seq, bit_idx)) {
				...
				key_seq = &flow_ring->reuse_key_seq_list[
					bit_idx * WL_KEY_SEQ_INFO_SIZE];
...
#else /* ! BCM_DHDHDR */
	{
		...
		memcpy(&buf[TLV_HDR_LEN + WLFC_CTL_VALUE_LEN_PKTTAG], &seq, sizeof(uint16));
	}
#endif /* ! BCM_DHDHDR */
...
#if defined(PCIE_DMA_INDEX) && defined(SBTOPCIE_INDICES)
void /* Sync to host, dmaindex_h2d_rd of H2D rings[common rings + flow rings] */
pciedev_sync_dmaindex_h2d_rd(struct dngl_bus * pciedev, msgbuf_ring_t * ring)
{
	uint32 array_idx;
	uint32 haddr_lo32;
	uint32 haddr_hi32;
	uint64 haddr_u64;
	uint32 index_val;
	const size_t index_sz = PCIE_RW_INDEX_SZ;

	BUZZZ_KPI_QUE1(KPI_QUE_BUS_RD_UPD, 2, MSGBUF_RD(ring), ring->ringid);

	array_idx  = ring->ringid; // In H2D rings, ringid is array index

	// Compute host address where the h2d_rd will be transferred
	haddr_hi32 = HADDR64_HI(pciedev->dmaindex_h2d_rd.haddr64);
	haddr_lo32 = HADDR64_LO(pciedev->dmaindex_h2d_rd.haddr64);
	haddr_lo32 = (uint32)BCMPCIE_RW_INDEX_ADDR(haddr_lo32, index_sz, array_idx);
	haddr_u64  = LOHI32_TO_U64(haddr_lo32, haddr_hi32);

	// Transfer h2d_rd to host h2d_rd indices array by pio in D2H direction
	index_val  = MSGBUF_RD(ring); // value to transfer to host from MSGBUF
	sbtopcie_pio((uintptr)&index_val, haddr_u64, index_sz, SBTOPCIE_DIR_D2H);
}

static void /* Sync to host, dmaindex_d2h_wr of D2H completion ring */
pciedev_sync_dmaindex_d2h_wr(struct dngl_bus * pciedev, msgbuf_ring_t* ring)
{
	uint32 array_idx;
	uint32 haddr_lo32;
	uint32 haddr_hi32;
	...
	// D2H ring, need to compute array index from ringid
	array_idx  = BCMPCIE_D2H_RING_IDX(ring->ringid);

	// Compute host address where the d2h_wr will be transferred
	haddr_hi32 = HADDR64_HI(pciedev->dmaindex_d2h_wr.haddr64);
	haddr_lo32 = HADDR64_LO(pciedev->dmaindex_d2h_wr.haddr64);
	haddr_lo32 = (uint32)BCMPCIE_RW_INDEX_ADDR(haddr_lo32, index_sz, array_idx);
	haddr_u64  = LOHI32_TO_U64(haddr_lo32, haddr_hi32);

	// Transfer d2h_wr to host d2h_wr indices array by pio in D2H direction
	index_val  = MSGBUF_WR(ring); // value to transfer to host from MSGBUF
	sbtopcie_pio((uintptr)&index_val, haddr_u64, index_sz, SBTOPCIE_DIR_D2H);
}

void /* Sync from host, dmaindex_d2h_rd for D2H completion ring */
pciedev_sync_dmaindex_d2h_rd(struct dngl_bus * pciedev, msgbuf_ring_t* ring)
{
	uint32 array_idx;
	uint32 haddr_lo32;
	uint32 haddr_hi32;
	...
	// Compute host address from where the d2h_rd will be retrieved
	haddr_hi32 = HADDR64_HI(pciedev->dmaindex_d2h_rd.haddr64);
	haddr_lo32 = HADDR64_LO(pciedev->dmaindex_d2h_rd.haddr64);
	haddr_lo32 = (uint32)BCMPCIE_RW_INDEX_ADDR(haddr_lo32, index_sz, array_idx);
	haddr_u64  = LOHI32_TO_U64(haddr_lo32, haddr_hi32);

	// Fetch d2h_rd from host d2h_rd indices array by pio in H2D direction
	sbtopcie_pio((uintptr)&index_val, haddr_u64, index_sz, SBTOPCIE_DIR_H2D);
	MSGBUF_RD(ring) = index_val; // value fetched from host saved into MSGBUF
}
...
pciedev_upd_last_queued_flowring(struct dngl_bus * pciedev, uint16 ringid)
{
	...
	if (flow_ring != pciedev->txcpl_last_queued_ring) {
		...
		if (pciedev->txcpl_last_queued_ring) {
			pciedev_sync_dmaindex_h2d_rd(pciedev, pciedev->txcpl_last_queued_ring);
		}
...
pciedev_spktq_callback(void *arg, struct spktq *spq)
{
...
#ifdef PROP_TXSTATUS
	pciedev->bulk_txs_pending = 0;
	expect_rindex = PKTFRAGRINGINDEX(pciedev->osh, p);

	if ((MSGBUF_RD(flow_ring) != expect_rindex) ||
#if defined(HWA_PKTPGR_BUILD)
	...
	while ((p = spktq_deq(spq))) {
		...
		while (p) { /* while (p) */
			 ...
			 */
			if (status || (flowid != expect_flowid) ||
				(PKTFRAGRINGINDEX(pciedev->osh, p) != expect_rindex) ||
#if defined(HWA_PKTPGR_BUILD)
				(pciedev_is_contiguous_rindex(pciedev, p,
					MSGBUF_MAX(flow_ring)) == FALSE) ||
#endif
				FALSE) {
				spktq_enq_head(spq, p);
				goto bailout;
			...
			PCIEDEV_FLOW_COMPL_TS(flow_ring, PKTFRAGRINGINDEX(pciedev->osh, p), 0);
#else /* ! PROP_TXSTATUS */
			if (flowid != expect_flowid) {
				PCI_PRINT(("%s: expect_flowid<%d> pkt flowid<%d>\n",
					__FUNCTION__, expect_flowid, flowid));
				spktq_enq_head(spq, p);
				goto bailout;
			}

			PCIEDEV_FLOW_COMPL_TS(flow_ring, 0);
#endif /* ! PROP_TXSTATUS */
...
bailout:
	...
	ASSERT(flow_ring->flow_info.pktinflight >= nframes);
	...
	uint16 eth_type, uint8 prio, hwa_cfp_tx_info_t *hwa_cfp_tx_info)
{
	...
	cfp.flags.disabled = !wlc_cfp_tx_enabled(FD_CFP_UNIT, flow_ring->scb_flowid,
	...
	hwa_cfp_tx_info->pktlist_prio = MSGBUF_TID(flow_ring);
	...
	/* Return CFP Flowid */
	hwa_cfp_tx_info->scb_flowid = flow_ring->scb_flowid;
...
pciedev_flowring_state_get(struct dngl_bus *pciedev, uint16 flowid)
{
	...
	else if (FLOW_RING_SUP_PEND_ISSET(flow_ring)) {
		return FLOW_RING_STATE_SUPPRESS_PEND;
	} else if (!SCB_FLOWID_VALID(flow_ring->scb_flowid) ||
		(flow_ring->status & FLOW_RING_FLUSH_PENDING)) {
		return FLOW_RING_STATE_FLUSH_PEND;
	} else {
	...
	void **pktcs, uint32 *pktc_cnts)
{
	...
	void **txlbuf_prev, *txlbuf_uprev, *txlbuf_ulprev, *txlbuf_mprev;
	void *txlbuf_fmprev;
	void *curr, *next;
	...
	/* Update fetch pending and pkt inflight count */
	ASSERT(flow_ring->fetch_pending >= pkt_count);
	ASSERT(pciedev->fetch_pending >= pkt_count);

	flow_ring->fetch_pending -= pkt_count;
	pciedev->fetch_pending -= pkt_count;

	flow_ring->flow_info.pktinflight += pkt_count;
	pciedev->pend_user_tx_pkts += pkt_count;
	...
	/* Save chain info */
	if (amsdu_sup_part1->pkts) {
		ASSERT((FLOW_RING_SUP_PEND_ISSET(flow_ring)) == 0);
	...
	while (curr) {
		...
		if (hwa_cfp_tx_info->cfp_capable && unicast && tid_ac_matched) {
			...
			PKTSETCFPFLOWID(curr, hwa_cfp_tx_info->scb_flowid);

			/* Prepare the packet for CFP Transmit */
			wlc_cfp_pkt_prepare(FD_CFP_UNIT, hwa_cfp_tx_info->scb_flowid,
		...
		if ((pktc_type != HWA_PKTC_TYPE_FLOW_MISMATCH) &&
			(!(FLOW_RING_SUP_PEND_ISSET(flow_ring))) &&
			(!(flow_ring->status & FLOW_RING_FLUSH_PENDING)) &&
	...
	if (!FLOW_RING_SUP_PEND_ISSET(flow_ring)) {
		/* Advance the rd_ptr if not in suppression pending state */
		MSGBUF_RD(flow_ring) = (MSGBUF_RD(flow_ring) + pkt_index) % MSGBUF_MAX(flow_ring);
	}
#endif /* !PROP_TXSTATUS */
	...
	void **pktcs, uint32 *pktc_cnts)
{
	...
	while (curr) {
		...
		if (hwa_cfp_tx_info->cfp_capable && unicast && tid_ac_matched) {
			...
			/* Save CFP Flowid into the packet */
			PKTSETCFPFLOWID(lfrag, hwa_cfp_tx_info->scb_flowid);
...
pciedev_hwa_process_pending_flring_resp(struct dngl_bus *pciedev, uint16 ringid)
{
	...
	if (ring &&
		(ring->status & (FLOW_RING_FLUSH_PENDING | FLOW_RING_DELETE_RESP_PENDING))) {
	...
	uint16 pkt_count, bool sqs_force)
{
	...
	if (SCB_FLOWID_VALID(msgbuf->scb_flowid)) {
		if (wlc_sqs_v2r_pkts(msgbuf->scb_flowid, prio) > 0) {
			wlc_sqs_v2r_dequeue(msgbuf->scb_flowid, prio, pkt_count, sqs_force);
		}
	}
}
...
#else
	struct dngl_bus * pciedev = (struct dngl_bus *)arg;
	if (pciedev->fetch_pending) {
		PCIEDEV_FLOW_FETCH_FLAG_EOPS_RQST_SET(pciedev->last_flowfetchp);
		return BCME_OK;
	} else {
		return BCME_ERROR;
	}
...
/* Rollback the flowring fetch ptr back to last read ptr
 * 	Resets all pending fetch requests.
 *	Maintain a rollback pending state if fetch pending
 *
 *	Issued during PS mode ON transition
 */
void
pciedev_flow_ring_ps_update(struct dngl_bus * pciedev, uint16 ringid, bool ps_on)
{
	msgbuf_ring_t *msgbuf;
	...
	PCI_TRACE(("%s : ringid %d ps_on %d Write %d read %d Fetch %d "
		"fetch pending %d inflight %d  \n",
		__FUNCTION__, ringid, ps_on, MSGBUF_WR(msgbuf), MSGBUF_RD(msgbuf),
		msgbuf->fetch_ptr, msgbuf->fetch_pending,
		msgbuf->flow_info.pktinflight));

	BCM_REFERENCE(msgbuf);
#ifdef PROP_TXSTATUS
	if (ps_on) {
		if ((msgbuf->flow_info.pktinflight > 0) ||
			(msgbuf->fetch_pending > 0)) {
			/* Mark as suppression pending */
			FLOW_RING_SUP_PEND_SET(msgbuf);

			/* If there was a partial fetched AMSDU, clean that up. */
			pciedev_flush_cached_amsdu_frag(pciedev, msgbuf);
		}
	}
#else
	/* If flow ring was already marked as supppression pending,
	 * Don't need to reset fetch_pending and rewind sqs_vpkts.
	 * This may be happened when there are not enough resources
	 * for HWA to handle txpost response.
	 */
	if (msgbuf->fetch_pending && ps_on && !FLOW_RING_SUP_PEND_ISSET(msgbuf)) {
		uint16 count = 0;
		if (msgbuf->fetch_ptr >= MSGBUF_RD(msgbuf)) {
			count = msgbuf->fetch_ptr - MSGBUF_RD(msgbuf);
		} else {
			count = msgbuf->fetch_ptr + MSGBUF_MAX(msgbuf) - MSGBUF_RD(msgbuf);
		}

		ASSERT(count == msgbuf->fetch_pending);

		/* Rewind the fetch pointer */
		msgbuf->fetch_ptr = MSGBUF_RD(msgbuf);

		/* Mark the flow ring as suppression pending.
		 * Reusing the alreading exisitng FLOW_RING_SUP_PENDING flag.
		 * Reset on fetch_pending and inflight ==0
		 */
		FLOW_RING_SUP_PEND_SET(msgbuf);

		if (count) {
			/* Increment Virtual packets */
			sqs_vpkts_rewind(pciedev, msgbuf, count);
		}
	}
#endif /* PROP_TXSTATUS */
}
