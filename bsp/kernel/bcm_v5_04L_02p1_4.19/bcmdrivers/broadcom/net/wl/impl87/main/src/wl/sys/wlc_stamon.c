 ...
 *
 * $Id: wlc_stamon.c 806611 2021-12-24 06:22:29Z $
 */
...
struct wlc_stamon_sta_cfg_entry {
	...
	int  rcmta_idx; /* index of RCMTA/AMT tables */
...
static const bcm_iovar_t stamon_iovars[] = {
	...
	{"sta_monitor_cnt", IOV_STA_MONITOR_CNT, (0), 0, IOVT_BUFFER, sizeof(stamon_cnts_t)},
...
static int wlc_stamon_send_dump_info(wlc_stamon_info_t *ctxt, void* param,
	int paramlen, stamon_dump_t *dump_info, int len);
...
static void wlc_stamon_activate(wlc_stamon_info_t *ctxt);
static void wlc_stamon_deactivate(wlc_stamon_info_t *ctxt);
	...
	struct ether_addr *ea, bool enab)
{
	...
	if (ETHER_ISNULLADDR(ea) || ETHER_ISMULTI(ea)) {
		...
		WL_ERROR(("wl%d: %s: Invalid MAC address "MACF".\n",
			WLCUNIT(ctxt), __FUNCTION__,
			ETHERP_TO_MACF(ea)));
...
#if defined(BCMDBG) || defined(WLMSG_ERROR)
		WL_ERROR(("wl%d: %s: STA "MACF" not found.\n",
			WLCUNIT(ctxt), __FUNCTION__,
			ETHERP_TO_MACF(ea)));
...
wlc_stamon_sta_config(wlc_stamon_info_t *ctxt, wlc_stamon_sta_config_t* cfg)
{
	...
	if (cfg->cmd == STAMON_CFG_CMD_DSB) {
		...
		chanspec_t phy_chspec = phy_utils_get_chanspec(WLC_PI(wlc));
		...
		if (cfg_idx < 0) {
			...
			if (cfg_idx < 0) {
				WL_ERROR(("wl%d:%s: No free slots available to add ["MACF"] "
					" to sta monitor \n", WLCUNIT(ctxt), __FUNCTION__,
		...
		if (STAMON_ENAB(WLCPUB(ctxt))) {
			scb = wlc_scbapfind(wlc, &ctxt->stacfg[cfg_idx].ea);
	...
	int8 idx, bool enab)
{
	...
	if (D11REV_GE(WLCPUB(ctxt)->corerev, 40)) {
		...
		bitmap = enab ? NBITVAL(C_ADDR_MONITOR_NBIT): 0;
		...
		wlc_write_shm(WLC(ctxt), (M_SECKINDXALGO_BLK(WLC(ctxt)) +
...
wlc_stamon_rcmta_slots_reserve(wlc_stamon_info_t *ctxt)
{
	...
	for (cfg_idx = 0; cfg_idx < cfg_max_idx; cfg_idx++) {
		ctxt->stacfg[cfg_idx].rcmta_idx =
			wlc_keymgmt_alloc_amt(WLC(ctxt)->keymgmt);
	...
	if (err == BCME_OK) {
		ctxt->amt_start_idx = ctxt->stacfg[0].rcmta_idx;
		ctxt->amt_max_idx = ctxt->stacfg[cfg_max_idx - 1].rcmta_idx;
	}
#ifdef ACKSUPR_MAC_FILTER
...
wlc_stamon_rcmta_slots_free(wlc_stamon_info_t *ctxt)
{
	...
	ctxt->amt_start_idx = ctxt->amt_max_idx = 0;
	wlc_stamon_rxcounters_update(WLC(ctxt), NULL, TRUE);
	return BCME_OK;
}
...
wlc_stamon_rxcounters_update(wlc_info_t *wlc, void *p, bool reset)
{
	...
	if (!reset) {
		mac_header = (struct dot11_header *)(PKTDATA(wlc->pub->osh, p) +
...
wlc_stamon_stats_update(wlc_info_t *wlc, const struct ether_addr* ea, int value)
{
	...
	if (ETHER_ISNULLADDR(ea)) {
		WL_INFORM(("NULL MAC address %s\n", __FUNCTION__));
		return BCME_ERROR;
	}

	if ((idx = wlc_stamon_sta_find(ctxt, ea)) != -1) {
		uint8 phy_chan = wf_chspec_ctlchan(phy_utils_get_chanspec(WLC_PI(wlc)));
...
/*
 * Send monitor sta's dump info to caller
 */
static int
wlc_stamon_send_dump_info(wlc_stamon_info_t *ctxt, void* param, int paramlen,
		stamon_dump_t *dump_info, int len)
{
	uint i, j;
	int status = BCME_ERROR;
	int amt_idx = -1;
	...
	dump_info->version = STAMON_MODULE_VER;

	for (i = 0, j = 0; i < STACFG_MAX_ENTRY(ctxt); i++) {
		if (!ETHER_ISNULLADDR(&ctxt->stacfg[i].ea)) {
#if defined(BCMDBG)
			uint k;
			uint16 attr;
			struct ether_addr ea;
			for (k = 0; k < (int)WLC(ctxt)->pub->max_addrma_idx; k++) {
				wlc_get_addrmatch(WLC(ctxt), k, &ea, &attr);
				if (ETHER_ISNULLADDR(&ea) && !(attr & AMT_ATTR_VALID))
					continue;
				if (!ether_cmp(&ea, &ctxt->stacfg[i].ea)) {
					amt_idx = k;
					break;
				}
			}
#endif /* BCMDBG */
			bcopy(&ctxt->stacfg[i].ea, &dump_info->sta_data[j].ea, ETHER_ADDR_LEN);
			dump_info->sta_data[j].rssi = ctxt->stacfg[i].rssi;
			dump_info->sta_data[j].chanspec = ctxt->stacfg[i].chanspec;
			dump_info->sta_data[j].offchan_time = ctxt->stacfg[i].offchan_time;
			dump_info->sta_data[j].rcmta_idx = amt_idx;
			status = BCME_OK;
			j++;
		}
	}
	dump_info->count = j;
	if (dump_info->count == 0) {
		/* DHD copy the response if error is BCME_OK, in this
		 * case entry is not found, but that doesn't mean
		 * IOCTL operation failed
		 */
	...
	int paramlen, void* bptr, int len)
{
	...
	} else if (stamon_cfg->cmd == STAMON_CFG_CMD_GET_STATUS) {
		stamon_status_t* status_info = NULL;
		if (ctxt == NULL)
			return BCME_UNSUPPORTED;
		status_info = (stamon_status_t *)(bptr);
		ASSERT(status_info != NULL);
		status_info->version = STAMON_MODULE_VER;
		status_info->status = WLCPUB(ctxt)->_stamon;
		err = BCME_OK;
	} else if (stamon_cfg->cmd == STAMON_CFG_CMD_DUMP) {
		stamon_dump_t* dump_info = NULL;
		if (!bptr || (len < (sizeof(stamon_dump_t) + ctxt->stacfg_num
						* sizeof(stamon_data_t)))) {
			return BCME_BUFTOOSHORT;
		}
		dump_info = (stamon_dump_t *)(bptr);
		dump_info->status = WLCPUB(ctxt)->_stamon;
		ctxt->stamon_cnts.version = STAMON_CNTR_VER;
		dump_info->rxstamondata = ctxt->stamon_cnts.rxstamondata;
		dump_info->rxstamonmngt = ctxt->stamon_cnts.rxstamonmngt;
		dump_info->rxstamoncntl = ctxt->stamon_cnts.rxstamoncntl;
		err = wlc_stamon_send_dump_info(ctxt, param, paramlen, dump_info, len);
		dump_info->length = sizeof(stamon_dump_t) +
			(dump_info->count * sizeof(stamon_data_t));
	} else {
...
wlc_offchan_timer_delete(wlc_stamon_info_t *ctxt)
{
	...
	if (ctxt->chspec_return) {
		...
		/* dump_flag_qqdx */
		printk("change wlc->chanspec:wlc_stamon:wlc_offchan_timer_delete:(0x%04x:0x%04x)",wlc->chanspec, ctxt->chspec_return);
		/* dump_flag_qqdx */
...
wlc_stamon_offchan_timer(void *arg)
{
	...
	if (ctxt->chspec_return) {
		wlc_bmac_suspend_mac_and_wait(wlc->hw);
		/* dump_flag_qqdx */
		printk("change wlc->chanspec:wlc_stamon:wlc_stamon_offchan_timer1:(0x%04x:0x%04x)",wlc->chanspec, ctxt->chspec_return);
		/* dump_flag_qqdx */
		wlc_channel_set_chanspec(wlc->cmi, ctxt->chspec_return);
		wlc_bmac_enable_mac(wlc->hw);
	...
	do {
		...
		if (ctxt->stacfg[i].chanspec != phy_utils_get_chanspec(WLC_PI(wlc)) &&
			ctxt->stacfg[i].offchan_time != 0 &&
			wlc_mac_request_entry(wlc, NULL, WLC_ACTION_RM, 0) == BCME_OK) {
			/* Going off channel. Record home chanspec */
			ctxt->chspec_return = phy_utils_get_chanspec(WLC_PI(wlc));

			wlc_bmac_suspend_mac_and_wait(wlc->hw);
		/* dump_flag_qqdx */
		printk("change wlc->chanspec:wlc_stamon:wlc_stamon_offchan_timer2:(0x%04x:0x%04x)",wlc->chanspec,ctxt->stacfg[i].chanspec);
		/* dump_flag_qqdx */
...
static void
wlc_stamon_activate(wlc_stamon_info_t *ctxt)
{
	int8 cfg_idx;
	wlc_info_t *wlc;
	struct scb *scb;
...
static void
wlc_stamon_deactivate(wlc_stamon_info_t *ctxt)
{
	int8 cfg_idx;
