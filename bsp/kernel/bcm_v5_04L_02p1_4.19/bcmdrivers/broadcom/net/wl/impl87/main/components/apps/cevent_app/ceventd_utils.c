 ...
 *
 *
 * $Id: ceventd_utils.c 810596 2022-04-11 12:14:18Z $
 */
...
static void
ca_pktlog_add_pcap_rec(pktlog_node_t *pktlog_data, int datalen, uint64 ca_ts)
{
	pktlog_data->pcap_rec_h.ts_sec = ca_ts/1000;
	pktlog_data->pcap_rec_h.ts_usec = (ca_ts % 1000) * 1000;
	pktlog_data->pcap_rec_h.incl_len = datalen;
	pktlog_data->pcap_rec_h.orig_len = datalen;
}
static int
ca_process_pktlog(ca_wksp_t *cwksp, void *data, int datalen, uint64 ca_ts)
{
	pktlog_node_t *pktlog_data = NULL;
	pktlog_data =
		(pktlog_node_t *)malloc(sizeof(pktlog_node_t));
	if (pktlog_data == NULL) {
		CA_ERR("Can't alloc pktlog_data \n");
		return BCME_ERROR;
	}

	pktlog_data->data = malloc(datalen);
	if (pktlog_data->data == NULL) {
		free(pktlog_data);
		CA_ERR("Can't alloc pktlog_data->data \n");
		return BCME_ERROR;
	}

	ca_pktlog_add_pcap_rec(pktlog_data, datalen, ca_ts);
	cwksp->pktlog_out_size += sizeof(ca_pktlog_pcap_rec_hdr_t);

	memcpy(pktlog_data->data, data, datalen);
	if (ca_d_verbose == CA_VERBOSE_DBG) {
		ca_hexdump_ascii("pktlog_data", (uint8 *)pktlog_data->data, 32);
	}

	dll_append(&cwksp->pktlog_h, &pktlog_data->pktlog_list_p);
	cwksp->pktlog_out_size += datalen;
	CA_DBG("cwksp->pktlog_out_size : %d\n",
		cwksp->pktlog_out_size);
	return BCME_OK;
}
...
{
	/* adding wire shark pcap header */
	ca_pktlog_pcap_hdr_t pcap_h;
	...
	fwrite(&pcap_h, sizeof(pcap_h), 1, cwksp->pktlog_out);
	fflush(cwksp->pktlog_out);
	cwksp->pktlog_out_size += sizeof(ca_pktlog_pcap_hdr_t);
	cwksp->pktlog_dump_s += sizeof(ca_pktlog_pcap_hdr_t);
}
...
{
	/* init pktlog list */
	dll_init(&cwksp->pktlog_h);
	cwksp->pktlog_out_size = 0;
	cwksp->pktlog_dump_s = 0;
	cwksp->pktlog_dump_triggered = 0;
	cwksp->pktlog_dump_ts = ca_get_curr_time();

	/* init output file */
	ca_config_pktlog_init_out_file(cwksp, "ceventd_pktlog_out");
	/* add pcap header */
	if (cwksp->pktlog_out) {
		ca_pktlog_add_pcap_header(cwksp);
	}
}
...
{
	dll_t *elem_p, *next_p;
	pktlog_node_t *pktdata;

	/* remove all pktlog list */
	for (elem_p = dll_head_p(&cwksp->pktlog_h);
		!dll_end(&cwksp->pktlog_h, elem_p); elem_p = next_p) {
		next_p = dll_next_p(elem_p);

		pktdata = (pktlog_node_t *)elem_p;
		free(pktdata->data);
		dll_delete(elem_p);
		free(pktdata);
	}
	...
	/* close file */
	if (cwksp->pktlog_out) {
		fclose(cwksp->pktlog_out);
		cwksp->pktlog_out = NULL;
	}

	CA_DBG("change current file into back up file\n");
	rename(cwksp->pktlog_out_path, cwksp->pktlog_out_bak_path);

	/* reinit output file */
	cwksp->pktlog_out = fopen(cwksp->pktlog_out_path, "wb");
...
ca_write_pktlog(ca_wksp_t *cwksp)
{
	...
	if (cwksp->pktlog_out_size == cwksp->pktlog_dump_s) {
		return;
	}
	...
	for (elem_p = dll_head_p(&cwksp->pktlog_h);
		...
		pktdata = (pktlog_node_t *)elem_p;
		datalen = pktdata->pcap_rec_h.incl_len;

		/* write pcap record header */
		fwrite(&pktdata->pcap_rec_h, sizeof(ca_pktlog_pcap_rec_hdr_t),
				1, cwksp->pktlog_out);
		cur_size += sizeof(ca_pktlog_pcap_rec_hdr_t);
		/* write data */
		fwrite(pktdata->data, datalen, 1, cwksp->pktlog_out);
...
ca_process_eapd_pkt(ca_wksp_t *cwksp, int bytes, uint64 ca_ts)
{
	...
	/*
	 * Packet logging is working only when CEVENT has data
	 * acsdv2 also sends chanspec_t with CEVENT, so, discard it to avoid these data in pcap file
	 * Ignore CEVENT_TYPE_A2D and CEVENT_TYPE_D2A not to dump invalid packet
	 */
	if (CEVENT_DATA_LEN(ce) && (CEVENT_DATA_LEN(ce) > sizeof(chanspec_t)) &&
		(ce->type != CEVENT_TYPE_D2A) && (ce->type != CEVENT_TYPE_A2D)) {
		ca_process_pktlog(cwksp, ce->data, CEVENT_DATA_LEN(ce), ca_ts);
	}
