 ...
 *
 * $Id: ieee1905.c 808909 2022-03-02 13:11:01Z $
 */
...
static void ieee1905_startup(void)
{
  ...
  ieee1905_glist_init(&i5_config.policyConfig.no_err_code_list);
...
void i5RemoveAllGetBSSIDTimer(i5_dm_interface_type *pdmif)
{
  ...
  foreach_i5glist_item(pIterIf, i5_dm_interface_type, pdmdev->interface_list) {
     ...
     */
    if (!i5DmIsInterfaceWireless(pIterIf->MediaType)) {
      continue;
    }
    ...
    if (i5DmIsInterfaceWireless(pdmif->MediaType)) {
      ...
      if (i5_config.cbs.get_interface_info) {
        ...
        if (I5_IS_MULTIAP_AGENT(i5_config.flags)) {
          ...
          i5WlCfgUpdateMediaInfo(pdmif->InterfaceId, info.chanspec, info.bssid, info.mapFlags);
	  pdmif->band = (unsigned char)ieee1905_get_band_from_radiocaps(&pdmif->ApCaps.RadioCaps);
	} else if ((!I5_IS_MAP_CERT(i5_config.flags)) && (!I5_DONT_ADD_MEDIA_INFO(i5_config.flags))) {
          i5WlCfgUpdateMediaInfo(pdmif->InterfaceId, info.chanspec, info.bssid, info.mapFlags);
	}
...
int ieee1905_add_bssto_controller_table(ieee1905_client_bssinfo_type *bss)
{
  ...
  i5TraceInfo("ALID["I5_MAC_DELIM_FMT"] band_flag[0x%x] ssid_len[%d] ssid[%s] auth[0x%x] "
        ...
        "FBT_MDID[%d] fbt_caps[%d] fbt_reassoc_interval[%d] disabled[%d]\n",
        ...
        clientbss->fbt_info.tie_reassoc_interval, clientbss->disabled);
...
{
  i5MessageApAutoconfigurationRenewSend(neighbor_al_mac_address, 0);
}
  ...
  unsigned short prim_vlan_id, int is_8021q_present)
{
   ...
   * AP possible.
   */
  /* XXX: This is just a workaround to fix one certification issue where we remove VLAN
   * interface created for backhaul STA as we didn't get any traffic separation policy in
   * association response
   *
   * Here is the text from spec "If a Multi-AP Agent has not received a Multi-AP Default
   * 802.1Q Setting subelement in the (Re-)Association Response frame, or a Traffic Separation
   * Policy TLV, or the most recently received Traffic Separation Policy TLV has the Number of
   * SSIDs field set to zero, the Multi-AP Agent shall consider the Primary VLAN ID not configured"
   *
   * What we do here is to remove the VLAN interface if VLAN ID is not received in the assoc
   * response or VLAN ID is changed in the controller.
   *
   * In the Test case after we get traffic separation policy, we do rc restart and there is an
   * association of backhaul STA to Marvell agent. But the Marvell agent will not do any restart
   * after getting traffic separation policy which the UCC will send it after it is sent to
   * MAUT(BRCM), so we are not getting traffic separation policy in association response
   * So, we remove VLAN interface here.
   *
   * To fix the certification issue we are moving this part of code under map_cert NVRAM. So, for
   * certification will not remove it. We will also be reporting this issue to WFA
   */
...
int ieee1905_sta_assoc_failed_connection(unsigned char *mac, uint16 status, uint16 reason)
{
  ...
  i5MessageFailedConnectionMessageSend(pDeviceController, mac, status, reason);
...
int
ieee1905_send_backhaul_sta_capability_query(unsigned char *al_mac)
{
  i5_dm_device_type *pDevice = i5DmDeviceFind(al_mac);
  ...
  i5MessageBackhaulSTACapabilityQuerySend(pDevice->psock, al_mac);
