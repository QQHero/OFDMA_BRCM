 ...
 *
 * $Id: bcmcdc.c 793581 2020-12-02 14:38:26Z $
 */
...
static void *cdc_proto_pkt_header_push(dngl_proto_info_t *proto, void *p);
static int cdc_proto_pkt_header_pull(dngl_proto_info_t *proto, void *p);
static void cdc_proto_dev_event(dngl_proto_info_t *proto, void *data);
static dngl_proto_info_t *cdc_proto_attach(osl_t *osh,
	struct dngl *dngl, struct dngl_bus *bus, char *name, bool link_init);
static void cdc_proto_detach(dngl_proto_info_t *proto);
static void cdc_proto_ctrldispatch(dngl_proto_info_t *proto, void *p, uchar *ext_buf);
...
static struct dngl_proto_ops cdc_proto_ops = {
...
BCMRAMFN(get_cdc_proto_ops)(void)
{
	return &cdc_proto_ops;
}

static void cdc_indicate(dngl_proto_info_t *cdc, uint32 cmd, uint32 flags,
                         uint32 status, uchar *buf, ulong len, void *p);

static void
cdc_indicate(dngl_proto_info_t *cdc, uint32 cmd, uint32 flags, uint32 status,
...
{
	dngl_proto_info_t *cdc;

	trace("%s", name);

	if ((cdc = MALLOC(osh, sizeof(dngl_proto_info_t))) == NULL)
		return NULL;

	memset(cdc, 0, sizeof(dngl_proto_info_t));
	cdc->name = name;
	cdc->dngl = dngl;
	cdc->bus = bus;
	cdc->osh = osh;
...
static void
cdc_proto_detach(dngl_proto_info_t *cdc)
{
	trace("%s", cdc->name);
	MFREE(cdc->osh, cdc, sizeof(dngl_proto_info_t));
}

/* dispatcher for all control messages received from the host */
static void
cdc_proto_ctrldispatch(dngl_proto_info_t *cdc, void *p, uchar *ext_buf)
{
...
static void *
cdc_proto_pkt_header_push(dngl_proto_info_t *cdc, void *p)
{
	struct bdc_header *h;
	osl_t *osh = cdc->osh;
...
static int
cdc_proto_pkt_header_pull(dngl_proto_info_t *cdc, void *p)
{
	struct bdc_header *h;
	osl_t *osh = cdc->osh;
	int ver;
...
static void
cdc_proto_dev_event(dngl_proto_info_t *cdc, void *data)
{
#ifdef NOTYET
...
void
proto_pr46794WAR(dngl_proto_info_t *cdc)
{
	cdc->pr46794WAR = TRUE;
	err("proto_pr46794WAR ON");
	/* punt for now */
}
#endif
