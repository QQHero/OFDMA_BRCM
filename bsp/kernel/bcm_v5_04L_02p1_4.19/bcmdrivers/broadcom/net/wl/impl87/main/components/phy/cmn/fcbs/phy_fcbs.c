 ...
 *
 * $Id: phy_fcbs.c 783949 2020-02-13 09:58:24Z $
 */
...
/* Local functions */
#ifdef ENABLE_FCBS
static bool wlc_phy_hw_fcbs_init(phy_info_t *pi, int chanidx)
{
	...
	pi->phy_fcbs->initialized[chanidx] =
		wlc_phy_hw_fcbs_init_chanidx(pi, chanidx);

	return TRUE;
}

static bool wlc_phy_hw_fcbs_init_chanidx(phy_info_t *pi, int chanidx)
{
...
static int wlc_phy_hw_fcbs(phy_info_t *pi, int chanidx, bool set)
{
	uint16 ptr;
	int i;
...
/* Might call mac_suspend() */
bool wlc_phy_fcbs_arm(phy_info_t *pi, chanspec_t chanspec, int chanidx)
{
	ASSERT((chanidx >= 0) && (chanidx < MAX_FCBS_CHANS));
	...
	if ((chanspec == pi->radio_chanspec) && IS_FCBS(pi) &&
		...
		return wlc_phy_fcbs_init(pi, chanidx);
	} else {
...
int wlc_phy_fcbs(phy_info_t *pi, int chanidx, bool set)
{
	...
	if (IS_FCBS(pi)) {
		return wlc_phy_hw_fcbs(pi, chanidx, set);
	}
	ASSERT((chanidx >= 0) && (chanidx < MAX_FCBS_CHANS));
...
bool wlc_phy_fcbs_uninit(phy_info_t *pi, chanspec_t chanspec)
{
	int chanidx;
