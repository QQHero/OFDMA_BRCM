...
#ifndef CONFIG_TENDA_WLAN_REPEATER
...
enum wlc_ap_iov {
    ...
    IOV_PREV_RXFRAME = 75,
#ifdef CONFIG_TENDA_WLAN_AUTOHIDE_SSID
    IOV_AUTO_HIDE = 76,        /*< The SSID is automatically hidden */
...
#endif
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    {"prev_rxframe", IOV_PREV_RXFRAME, 0, 0, IOVT_UINT32, 0},
#ifdef CONFIG_TENDA_WLAN_AUTOHIDE_SSID
    {"auto_hide", IOV_AUTO_HIDE, 0, 0, IOVT_INT32, 0},
#endif /* CONFIG_TENDA_WLAN_AUTOHIDE_SSID */
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
    {NULL, 0, 0, 0, 0, 0 }
};
...
#ifdef CONFIG_TENDA_PRIVATE_KM
            /* over 3s report  password_derr */
            if(scb->wsec_auth_timeout > 3) {
#ifdef CONFIG_UGW_BCM_DONGLE
                wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_STA_PASSERR, 0, 0, NULL, 0);              
#else
                if (km_eventscenter_wifiev_sta_passerr_handler) {
                    km_eventscenter_wifiev_sta_passerr_handler(scb->ea.octet);
                }
#endif
            }
#endif
    ...
    void *p, bool short_preamble, d11rxhdr_t *rxh)
{
    ...
    if (WSEC_TKIP_ENABLED(bsscfg->wsec) && wlc_keymgmt_tkip_cm_enabled(wlc->keymgmt, bsscfg)) {
...
#ifdef CONFIG_TENDA_PRIVATE_KM
        {
            int index = 0;
            int ap_count = 0;
            wlc_bsscfg_t *tmp_bsscfg = NULL;
            wlc_info_t *tmp_wlc = NULL;
            char ea_buf[ETHER_ADDR_STR_LEN] = {0};
            char *sa_buf = NULL;
            int current_ap_assoc_cnt = 0, max_ap_assoc_cnt = 0;
            int all_ap_max_assoc_cnt = 0;

            tmp_wlc = ((wlc_ap_info_pvt_t *)ap)->wlc;
            FOREACH_UP_AP(tmp_wlc, index, tmp_bsscfg) {
                all_ap_max_assoc_cnt += tmp_bsscfg->maxassoc;
                ap_count++;
            }

            max_ap_assoc_cnt = all_ap_max_assoc_cnt / ap_count;
            current_ap_assoc_cnt = wlc_ap_stas_associated(ap);
            if (current_ap_assoc_cnt >= max_ap_assoc_cnt) {
                sa_buf = bcm_ether_ntoa(&hdr->sa, ea_buf);
#ifdef CONFIG_UGW_BCM_DONGLE
                wlc_mac_event(wlc, WLC_E_TENDA_KM, NULL, KM_WIFIEV_STA_MAXCNTREJ, 0, 0, sa_buf, ETHER_ADDR_STR_LEN);
#else
                if (km_eventscenter_wifiev_sta_maxcntrej_handler) {
                    km_eventscenter_wifiev_sta_maxcntrej_handler(sa_buf);
                }
#endif
            }
        }
...
#ifdef CONFIG_TENDA_PRIVATE_KM
                char ea_buf[ETHER_ADDR_STR_LEN], *sa_buf;
                sa_buf = bcm_ether_ntoa(&hdr->sa, ea_buf);
#ifdef CONFIG_UGW_BCM_DONGLE
                wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_STA_BLACKREJ, 0, 0, sa_buf, ETHER_ADDR_STR_LEN);
#else
                if (km_eventscenter_wifiev_sta_blackrej_handler) {
                    km_eventscenter_wifiev_sta_blackrej_handler(sa_buf);
                }
#endif
#endif
                break;
    ...
    if ((status == DOT11_SC_SUCCESS) && (auth_alg == DOT11_FAST_BSS) &&
        ...
        if (DOT11_SC_SUCCESS != status) {
            td_em_client_event(__FUNCTION__, __LINE__, (void *)bsscfg, bsscfg->cur_etheraddr.octet, 
                            scb->ea.octet, TD_EASYMESH_CLIENT_CONNECT_FAIL, 0, status);
        }
#else
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
        if (DOT11_SC_SUCCESS != status) {
            if(!TDCORE_FUNC_IS_NULL(td_em_client_event)) {
                TDCORE_FUNC(td_em_client_event)((void *)bsscfg, (char *)bsscfg->cur_etheraddr.octet, 
                            (char *)scb->ea.octet, TD_EASYMESH_CLIENT_CONNECT_FAIL, 0, status);
            }
		}
#endif
#endif
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN)
    TDP_INFO("[TX] ASSOC wl%d.%d %pM (sc=%d), rssi %d",wlc->pub->unit, 
        WLC_BSSCFG_IDX(bsscfg), &scb->ea, resp->status, scb->rssi);
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
...
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
    td_em_client_event(__FUNCTION__, __LINE__, (void *)bsscfg, bsscfg->cur_etheraddr.octet, 
                        scb->ea.octet, TD_EASYMESH_CLIENT_JOIN, 0, 0);
    td_em_clientcapability_notify(scb->ea.octet, bsscfg->cur_etheraddr.octet, param->body_len, param->body);
#else
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    if(!TDCORE_FUNC_IS_NULL(td_em_client_event)) {
        TDCORE_FUNC(td_em_client_event)((void *)bsscfg, (char *)bsscfg->cur_etheraddr.octet, 
                        (char *)scb->ea.octet, TD_EASYMESH_CLIENT_JOIN, 0, 0);
    }
    if(!TDCORE_FUNC_IS_NULL(td_em_clientcapability_notify)) {
        TDCORE_FUNC(td_em_clientcapability_notify)(scb->ea.octet, bsscfg->cur_etheraddr.octet, param->body_len, param->body, bsscfg);
    }
#endif
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
        WL_ASSOC(("wl%d: wlc_apup_allowed: defer AP UP, STA associating: "
...
wlc_ap_sta_probe_complete(wlc_info_t *wlc, uint txstatus, struct scb *scb, void *pkt)
{
    ...
    if (SCB_AUTHENTICATED(scb)) {
         ...
         */
        TDP_INFO("[TX] DEAUTH wl%d.%d %pM, reason %d", wlc->pub->unit, 
            WLC_BSSCFG_IDX(cfg), &scb->ea, DOT11_RC_INACTIVITY);
    }
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
    ...
    void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
    ...
    case IOV_SVAL(IOV_SCB_PROBE): {
        ...
        /*解决终端老化时间概率性被覆盖为5的问题，不使用驱动scb_activity_time默认的5，用wserver配置的值*/
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    case IOV_SVAL(IOV_PREV_RXFRAME):
        if (int_val >= 0) {
            bsscfg->prev_rxframe = (uint32)int_val;
        }
        break;
    case IOV_GVAL(IOV_PREV_RXFRAME):
        *ret_int_ptr = (int32)bsscfg->prev_rxframe;
        break;
#ifdef CONFIG_TENDA_WLAN_AUTOHIDE_SSID
    case IOV_SVAL(IOV_AUTO_HIDE):
        if (int_val >= 0) {
            bsscfg->td_auto_hide = (uint32)int_val;
        }
        break;
    case IOV_GVAL(IOV_AUTO_HIDE):
        *ret_int_ptr = (int32)bsscfg->td_auto_hide;
        break;
#endif /* CONFIG_TENDA_WLAN_AUTOHIDE_SSID */
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
...
wlc_ap_acs_update(wlc_info_t *wlc)
{
    ...
    WL_INFORM(("wl%d: %s: changing chanspec to %d\n",
        wlc->pub->unit, __FUNCTION__, appvt->chanspec_selected));
		/* dump_flag_qqdx */
		printk("change wlc->home_chanspec:wlc_ap:wlc_set_home_chanspec:(0x%04x:0x%04x)",wlc->home_chanspec, appvt->chanspec_selected);
		/* dump_flag_qqdx */
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN)
    TDP_INFO("[TX] AUTH wl%d.%d %pM (sc=%d)", WLCWLUNIT(wlc), 
        WLC_BSSCFG_IDX(bsscfg), &scb->ea, status);
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
