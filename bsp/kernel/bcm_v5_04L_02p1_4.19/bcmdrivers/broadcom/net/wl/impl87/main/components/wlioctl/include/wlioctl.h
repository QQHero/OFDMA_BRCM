 ...
 *
 * $Id: wlioctl.h 810646 2022-04-12 05:14:38Z $
 */
...
#define WL_SCAN_PARAMS_FIXED_SIZE_V2    \
        (80 + WL_SCAN_PARAMS_EXTRA_SIZE_1 + WL_SCAN_PARAMS_EXTRA_SIZE_2)
...
struct wb_wmm_info_s
{
    unsigned char   priority;   //0~7  802.1d
    unsigned int    stream_id;     //哪种标志
    unsigned int    action;
    union {
        wb_ip_info_t ip_info;
    } stream;
};

typedef struct wb_wmm_info_s wme_param_ac_ip_t;
#endif
...
typedef struct {
    ...
    uint16      states;     /* replace PAD above to check APClient status, PJL */
#else
 ...
 */
#define SCB_RX_REPORT_DATA_STRUCT_VERSION    5
 ...
 */
#define SCB_TXRX_SUMMARY_STRUCT_VERSION        1

typedef struct {
    uint64 phyrate;        /* running total of phyrate */
    uint64 bytes;        /* running total of bytes transffered successfully */
    uint32 bw;        /* running total of bw of successful packets */
    uint32 nss[4];        /* running total of nss of sucessful packets */
...
typedef struct chanim_stats_v3 {
    uint32 glitchcnt;               /**< normalized as per second count */
    uint32 badplcp;                 /**< normalized as per second count */
...
#ifdef GAME_SPEEDUP
typedef struct special_sta {
    struct ether_addr   ea;
    uint8           action; //add 1 delete 2
    uint8           idlerate_thresh[2];
    int8            rssi_thresh[2];
    uint32          utils_thresh[2]; //Queue utilization threshold
    uint8           timer_windows;
}special_sta_t;
#endif
#endif /* _wlioctl_h_ */
