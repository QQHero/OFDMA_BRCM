 ...
 *
 * $Id: phy_rxgcrs.c 808547 2022-02-21 18:58:59Z $
 */
...
int wlc_phy_iovar_force_tr2t(phy_info_t *pi, bool force_tr2t)
{
	phy_rxgcrs_info_t *info = pi->rxgcrsi;
	phy_type_rxgcrs_fns_t *fns = info->fns;

	if (fns->force_tr2t != NULL) {
		return (fns->force_tr2t)(fns->ctx, force_tr2t);
	} else {
		PHY_INFORM(("%s: No support for this PHY or build\n", __FUNCTION__));
		return BCME_UNSUPPORTED;
	}
}
