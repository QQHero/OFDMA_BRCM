 ...
 *
 * $Id: wlc_assoc.c 807087 2022-01-12 11:16:36Z $
 */
	...
	uint8 *body, uint body_len, bool short_preamble)
{
	...
	if ((auth_status != DOT11_SC_SUCCESS) && !(BSSCFG_EXT_AUTH(cfg) &&
...
#endif /* WLFBT */
		if (BSSCFG_EXT_AUTH(cfg) && WPA3_VALID_AUTH_STATUS(auth_status))
			return;
...
wlc_roam_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr, uint bss_type)
{
	...
	if (status != WLC_E_STATUS_SUCCESS && status != WLC_E_STATUS_ABORT) {
		if (CCX_ENAB(wlc->pub) && wlc->ccx->fast_roam && WLC_ROAM_ENABLED(roam)) {
...
wlc_assoc_do_dfs_reentry_cac_if_required(wlc_bsscfg_t *cfg)
{
	...
	if ((wlc_radar_chanspec(wlc->cmi, bi->chanspec)) &&
		...
		wl_add_timer(wlc->wl, as->timer,
			(wlc_dfs_get_cactime_ms(wlc->dfs) + DELAY_10MS), FALSE);
		wlc_assoc_change_state(cfg, AS_DFS_CAC_START);
		ret = wlc_set_dfs_cacstate(wlc->dfs, ON);
		if (ret == BCME_OK) {
...
wlc_assoc_disable_apcfg_on_repeater(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
	...
	primary_cfg = wlc->cfg;
	if (!(DWDS_ENAB(primary_cfg) || MAP_ENAB(primary_cfg) ||
#if defined(WET) || defined(WET_DONGLE)
		(WET_ENAB(wlc)) ||
#endif /* WET || WET_DONGLE */
		primary_cfg->_psta)) {
	...
	if (wlc->keep_ap_up == WLC_KEEP_AP_UP_ALWAYS) {
		WL_INFORM(("wl%d: keep_ap_up[%d] Not disabling downstream AP\n", wlc->pub->unit,
			wlc->keep_ap_up));
		/* if upstream connection lost and home chanspec is Radar channel,
		 * sta state machine could have marked channel as passive again.
		 * lets clear passive bit for this keep_ap_up mode as earlier
		 * we were in ISM mode.
		 */
		if ((WL11H_ENAB(wlc) && wlc_radar_chanspec(wlc->cmi, cfg->target_bss->chanspec)) ||
			wlc_restricted_chanspec(wlc->cmi, cfg->target_bss->chanspec)) {
			if (wlc_quiet_chanspec(wlc->cmi, cfg->target_bss->chanspec)) {
				WL_INFORM(("wl%d: clear quiet bit for [%x]\n", wlc->pub->unit,
					cfg->target_bss->chanspec));
				wlc_clr_quiet_chanspec(wlc->cmi, cfg->target_bss->chanspec);
				wlc_mute(wlc, OFF, 0);
			}
		}
		return;
