 ...
 *
 * $Id: bcm_notif.c 780917 2019-11-06 12:00:48Z $
 */
...
#endif /* defined(BCMDBG_ERR) || defined (BCMDBG) */
                              ...
                              bcm_notif_client_data passthru)
{
	...
	if (nodep && nodep == nodep->next) {
		...
		while (nodep) {
			...
			/* We should not loop all the way around! But it can happen */
			if (nodep == hdl->tail)
			{
				NOTIF_ERROR(("%s: not found\n", __FUNCTION__));
				break;
			}
		}
	}
...
/*
 * bcm_notif_exist_interest()
 *
 * Check an interested clients's exist
 *
 * Parameters
 *    hdl        Opaque list handle
 * Returns:
 *    BCME_OK     Client interest is existed
 *    BCME_ERROR  Not existed
 */
int bcm_notif_exist_interest(bcm_notif_h hdl)
{
	if (!hdl)
		return BCME_ERROR;

	return hdl->tail ? BCME_OK : BCME_ERROR;
}
