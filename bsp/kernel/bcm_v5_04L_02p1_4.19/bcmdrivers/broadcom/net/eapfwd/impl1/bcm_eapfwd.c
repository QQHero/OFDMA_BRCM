...
#define EAPFWD_FLCTL_EXHAUSTION_LO_PCNT 25 /* Favored Pkt threshold */
#define EAPFWD_FLCTL_EXHAUSTION_HI_PCNT 10
...
struct eap_dev {                /** eap_dev */
    ...
    unsigned int        exhaustion_lo[EAP_WLAN_RADIOS_MAX]; /* avail < lo: admit ONLY favored */
    unsigned int        exhaustion_hi[EAP_WLAN_RADIOS_MAX]; /* avail < hi: admit none */
...
static int eapfwd_pktsetprio(struct sk_buff *skb, bool update_vtag);
...
static inline bool ucast_should_drop_skb(pktlist_context_t *pktlist_context,
        uint32_t avail, uint16_t pkt_prio, uint16_t pkt_dest, uint32_t radio_idx)
{
    if (eap_dev_g.flctl_enable[radio_idx] == 0) return false;

    if (avail <= eap_dev_g.exhaustion_hi[radio_idx]) return true; // drop ALL
    if ((avail <= eap_dev_g.exhaustion_lo[radio_idx]) &&
...
eap_xmit_enqueue(struct sk_buff * skb)
{
    ...
    uint32_t avail = gbpm_get_avail_bufs(); /* BPM free buffer availability */
#endif
    ...
    /**
     * Customers with their own forwarding plane may replace eapfwd_pktsetprio to 
     * just PKTSETPRIO call. This is because the priority may already be known in 
     * advance and set in skb->priority. If so, then do the following:
     * PKTSETPRIO(skb, skb->priority);
     * prio = PKTPRIO(skb);
     */
    prio = eapfwd_pktsetprio(skb, 0);
    ...
    if (ucast_should_drop_skb(pktlist_context, avail, prio, dest, radio))
    {
        drop_credits++;
            ...
            HOOKP wl_pktfwd_xfer_hook)
{
    ...
    {
        uint32_t bpm_total = gbpm_get_total_bufs();
        eap_dev_g.exhaustion_lo[radio_idx] =
            ((bpm_total * EAPFWD_FLCTL_EXHAUSTION_LO_PCNT) / 100);
        eap_dev_g.exhaustion_hi[radio_idx] =
            ((bpm_total * EAPFWD_FLCTL_EXHAUSTION_HI_PCNT) / 100);
...
static int eapfwd_pktsetprio(struct sk_buff *skb, bool update_vtag) 
{
	...
	PKTFWD_ASSERT(ISALIGNED((uintptr)pktdata, sizeof(uint16)));
	...
	PKTFWD_ASSERT(priority >= 0 && priority <= MAXPRIO);
...
static ssize_t eapfwd_flctl_wr_proc(struct file *file, const char *buff, size_t len, loff_t *offset)
{
    ...
    uint32_t radio_idx, exhaustion_lo, exhaustion_hi, pkt_prio_favor, flctl_enable;
    if (copy_from_user(input, buff, len) != 0)
        return -EFAULT;
    ret = sscanf(input, "%u %u %u %u %u",
        &radio_idx, &exhaustion_lo, &exhaustion_hi, &pkt_prio_favor, &flctl_enable);
    if (ret < 5)
        goto Usage;
    ...
    eap_dev_g.exhaustion_lo[radio_idx] = exhaustion_lo;
    eap_dev_g.exhaustion_hi[radio_idx] = exhaustion_hi;
    ...
    goto Exit;
Usage:
    printk("\nUsage: <radio_id> <exhaustion_lo> <exhaustion_hi> <pkt_prio_favor> <enable(0/1)>\n");
Exit:
    return len;
}
#endif /* EAPFWD_FLCTL */
