...
static int port_xport_drv_init(mac_drv_t *mac_drv)
{
    ...
    pmc_xport_power_on(1);

    rc |= _port_xport_drv_init(XPORT_PORT_ID_0_0);
    rc |= _port_xport_drv_init(XPORT_PORT_ID_0_2);
    rc |= _port_xport_drv_init(XPORT_PORT_ID_1_0);
    rc |= _port_xport_drv_init(XPORT_PORT_ID_1_1);
    rc |= _port_xport_drv_init(XPORT_PORT_ID_1_2);
    rc |= _port_xport_drv_init(XPORT_PORT_ID_1_3);
#else
    rc |= _port_xport_drv_init(XPORT_PORT_ID_AE);
    rc |= _port_xport_drv_init(XPORT_PORT_ID_CB);
#endif
...
static int port_xport_dt_priv(const dt_handle_t handle, int mac_id, void **priv)
{
    unsigned long xport_intf = 0;

    xport_intf = dt_property_read_u32(handle, "xport-intf");
    if (xport_intf >= 0)
        *priv = (void *)(unsigned long)xport_intf;
