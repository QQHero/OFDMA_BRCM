...
int enetxapi_rx_pkt(int hw_q_id, FkBuff_t **fkb, enetx_rx_info_t *rx_info)
{
    ...
    *fkb = fkb_init((uint8_t *)info.data , BCM_PKT_HEADROOM, (uint8_t *)(info.data + info.data_offset), info.size);
    ...
    rx_info->is_mcast_fwd_exception = (info.reason == rdpa_cpu_rx_reason_mcast_exception) ? 1 : 0;
...
#ifdef GPON
#if !defined(CONFIG_BCM963158) && !defined(RDP_UFC)
...
#if (defined(CONFIG_BCM_PON_XRDP) && defined(CONFIG_NET_SWITCHDEV))
    .switchdev_ops =
    {
