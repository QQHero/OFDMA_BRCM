...
int bcm_find_mac_portid(char *mac)
{
    int vid = -1, ret;
    unsigned int val = 0;

    if (!mac) {
        printk("[%s] [%d] find mac portid fail! mac == NULL!!!\n", __func__, __LINE__);
        return -EINVAL;
    }

    ret = _enet_arl_read(mac, &vid, &val);
    if (ret == FALSE) {
        ret = _enet_arl_read_ext(1, (uint8_t *)mac, &vid, &val);
    }
