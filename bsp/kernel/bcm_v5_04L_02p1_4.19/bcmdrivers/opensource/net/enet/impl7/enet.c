...
extern int port_link_speedDulplex_set(unsigned int port_id, uint32_t speed, uint32_t duplex);
extern int port_link_speedDulplex_get(int port_id, struct lan_port_status *port_status);
extern char *br_bdb_by_mac_dev(unsigned char *mac);
extern int bcm_find_mac_portid(char *mac);
#endif

#ifndef WCFG_MESH_LINK_LOOP_DISABLE
extern int(*td_link_loop_eth_rx_func_hook)(struct sk_buff *skb, struct net_device *dev);
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_KM    
    if (NF_DROP == km_driver_rx_packet_hook(skb, dev, port->p.port_id)) {
        return 0;
    }
#endif
#ifndef WCFG_MESH_LINK_LOOP_DISABLE
    if (td_link_loop_eth_rx_func_hook && td_link_loop_eth_rx_func_hook(skb, skb->dev)) {
        kfree_skb(skb);
        return 0;
    }
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_KM                
                km_driver_tx_packet_hook(skb, dev);
#endif
...
static int enet_ioctl(struct net_device *dev, struct ifreq *ifr, int cmd)
{
    ...
    unsigned char buf[8];
    unsigned char *dev_name = NULL;
    struct mac_port cliport;
#endif
...
#endif            
#if defined(CONFIG_TENDA_PRIVATE_BSP) && defined(CONFIG_SWITCH_BCM)
        case BCM6756_IOCTL_READ_DEV:
            memset(buf, 0, sizeof(buf));
            data_32 = (unsigned long *)ifr->ifr_data;
            if (copy_from_user(&buf, data_32, sizeof(buf))) {
                return -EFAULT;
            }
            if (buf[0] == ((DEV_READ_STAT & 0xff00) >> 8) 
                && (buf[1] == (DEV_READ_STAT & 0x00ff))) {
                dev_name = br_bdb_by_mac_dev(&buf[2]);
                if (!dev_name) {
                    return -EEXIST;
                }

                memset(buf, 0, sizeof(buf));
                memcpy(buf, dev_name, sizeof(buf));
                if (copy_to_user((void *)ifr->ifr_data, (void *)&buf, sizeof(buf))) {
                    return -EFAULT;
                }
                return 0;
            }
            break;
        case BCM6756_IOCTL_READ_PORT_STATUS:
            memset(&port_status, 0, sizeof(struct lan_port_status));
            data_32 = (unsigned long *)ifr->ifr_data;
            if (copy_from_user(&port_id, data_32, 1*sizeof(unsigned long))) {
                return -EFAULT;
            }

            if (port_id & (1 << 8)) {
                /*我们传进来的是一个端口的映射值（即bit位代表port几），这里转换出来*/
                port_id = port_id & (~(1 << 8)); 
                if (0 != (rc = port_link_speedDulplex_get(port_id, &port_status))) {
                    return -EFAULT;
                }
                if (copy_to_user((void *)ifr->ifr_data, (void *)&port_status, sizeof(struct lan_port_status))) {
                    return -EFAULT;
                }
                return 0;
            }
            break;
        case BCM6756_IOCTL_SET_PORT_STATUS:
            data = (unsigned long *)ifr->ifr_data;

            if (copy_from_user(args, data, 4*sizeof(unsigned long))) {
                return -EFAULT;
            }
        /*
            args[1] port_id/phy_id
            args[2] speed
            args[3] duplex
        */
            switch(args[0]) {
                case BCM_IOCTL_SET_PORT_MODE:
                case BCM_IOCTL_SET_PORT_DOWN_UP:
                    if (0 != (rc= port_link_speedDulplex_set(args[1], args[2],args[3]))) {
                        return -EFAULT;
                    }
                    return 0;
            }
        ...
        case BCM6756_IOCTL_FIND_MAC_PORTID:
            memset(&cliport, 0, sizeof(cliport));
            data_32 = (unsigned long *)ifr->ifr_data;
            if (copy_from_user(&cliport, data_32, sizeof(cliport))) {
                return -EFAULT;
            }
    
            if (cliport.opmode != BCM6756_MAC_FIND_MODE) {
                return -EFAULT;
            }
    
            rc = bcm_find_mac_portid(cliport.mac);
            ...
            cliport.port_id = rc;
            if (copy_to_user(data_32, (void *)&cliport, sizeof(cliport))) {
                return -EFAULT;
            }
