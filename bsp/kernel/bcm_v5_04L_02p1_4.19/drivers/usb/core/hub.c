...
#if defined(CONFIG_SUPPORT_CUSTOM_USB_LED)
#include <linux/usb/led.h>
usb_connet_checkout usb_check = NULL;
EXPORT_SYMBOL_GPL(usb_check);
#endif
...
#if defined(CONFIG_SUPPORT_CUSTOM_USB_LED)
    if(usb_check) {
        if(portstatus & (USB_PORT_STAT_CONNECTION | USB_PORT_STAT_ENABLE) == 1){
            usb_check(USB_LINK);
        }
        else if((portstatus & (USB_PORT_STAT_CONNECTION | USB_PORT_STAT_ENABLE)) == 0){
            usb_check(USB_UNLINK);
        }
    }
#endif
