...
#if defined(CONFIG_TENDA_PRIVATE_BSP) && defined(CONFIG_FLASH_LOCK_SUPPORT)
        return 0;
#else
        return -EBADMSG;
#endif
...
static int tenda_spinand_unlock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
    struct spinand_device *spinand = mtd_to_spinand(mtd);
    int ret = 0;
...
static int tenda_spinand_lock(struct mtd_info *mtd, loff_t ofs, uint64_t len)
{
    struct spinand_device *spinand = mtd_to_spinand(mtd);
    int ret = 0;
    ...
    if (1 >= g_flash_lock_count) {
        ret = spinand_lock_block(spinand, BL_ALL_LOCKED);
    }
    g_flash_lock_count--;
    mutex_unlock(&spinand->lock);
...
#if !defined(CONFIG_TENDA_PRIVATE_BSP)
...
#if defined(CONFIG_TENDA_PRIVATE_BSP) && defined(CONFIG_FLASH_LOCK_SUPPORT)
    mtd->_lock = tenda_spinand_lock;
    mtd->_unlock = tenda_spinand_unlock;
#endif
