...
#if defined(CONFIG_MTD_PARTITION_LOCK)
#define  BRCM_MIN_BEB_PER_MTDPART 10
#else
...
#if defined(CONFIG_MTD_PARTITION_LOCK)
static ssize_t ubi_ro_mode_store(struct device *dev, struct device_attribute *attr,
...
static struct device_attribute dev_ro_mode =
    __ATTR(ro_mode, (S_IWUSR|S_IRUGO), dev_attribute_show, ubi_ro_mode_store);
#else
static struct device_attribute dev_ro_mode =
             ...
             const char *buf, size_t count)
{
    ...
    ubi->ro_mode = simple_strtoul(buf, NULL, 10);
