...
#define PORT_EXTERN1        5       //phyid0~4 is switch's port, phyid 5 is cpu's GPHY port
#define PHY_SPEED_10M       1
#define PHY_SPEED_100M      2
#define PHY_SPEED_1000M     3
...
int phy_priv_fun(phy_dev_t *phy_dev, int op, ...)
{
    ...
    va_start(ap, op);
    return phy_dev->phy_drv->priv_fun(phy_dev, op, ap);
    va_end(ap);
}
...
#if defined(CONFIG_TENDA_PRIVATE_BSP) && defined(CONFIG_SWITCH_BCM)
int port_link_speedDulplex_get(int port_id, lan_port_status_t *port_status)
{
    phy_dev_t *phy_dev;
    ...
    if (!port_status) {
        printk("%s invalid status\n", __func__);
        return -2;
    }

    phy_dev = &phy_devices[port_id];

    if (!phy_dev->phy_drv) {
        return -3;
    }

    port_status->link = phy_dev->link;
    switch (phy_dev->speed) {
        case PHY_SPEED_1000:
            port_status->speed = PHY_SPEED_1000M;
            break;
        case PHY_SPEED_100:
            port_status->speed = PHY_SPEED_100M;
            break;
        case PHY_SPEED_10:
            port_status->speed = PHY_SPEED_10M;
            break;
        default:
            port_status->speed = PHY_SPEED_UNKNOWN;
            break;
    }
    port_status->duplex = phy_dev->duplex;

    return 0;
}
EXPORT_SYMBOL(port_link_speedDulplex_get);

int port_link_speedDulplex_set(unsigned int port_id, uint32_t speed, uint32_t duplex)
{
    int ret;
    phy_dev_t *phy_dev;
    uint32_t caps = 0;

    if (port_id > BP_MAX_SWITCH_PORTS || port_id < 0) {
        printk("%s invalid port id %d\n", __func__, port_id);
        return -1;
    }
    ...
    switch (speed) {
        case PHY_SPEED_1000M:
            caps |=  (duplex == PHY_HALF_DUPLEX ? PHY_CAP_1000_HALF : (PHY_CAP_1000_FULL | PHY_CAP_1000_HALF));
            caps |= PHY_CAP_100_HALF | PHY_CAP_100_FULL | PHY_CAP_10_HALF | PHY_CAP_10_FULL;
            break;
        case PHY_SPEED_100M:
            caps |= (duplex == PHY_HALF_DUPLEX ? PHY_CAP_100_HALF : (PHY_CAP_100_FULL | PHY_CAP_100_HALF));
            caps |= PHY_CAP_10_HALF | PHY_CAP_10_FULL;
            break;
        case PHY_SPEED_10M:
            caps |= (duplex == PHY_HALF_DUPLEX ? PHY_CAP_10_HALF : (PHY_CAP_10_FULL | PHY_CAP_10_HALF));
            break;
        default:
            speed = PHY_SPEED_UNKNOWN;
            if((ret = phy_dev_speed_set(phy_dev, speed, duplex) )) {
                printk("port status set err\n");
                return -2;
            }
            return 0;
    }
...
EXPORT_SYMBOL(port_link_speedDulplex_set);
#endif
