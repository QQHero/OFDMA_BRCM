...
#if defined(CONFIG_TENDA_PRIVATE_BSP) && defined(CONFIG_SWITCH_BCM)
#define MAX_PORT_NUM 8
static  int port_e_status[MAX_PORT_NUM] = {};
static  int port_i_status[MAX_PORT_NUM] = {};
#endif
...
int unit_port_oam_idx_array[COMPAT_MAX_SWITCHES][COMPAT_MAX_SWITCH_PORTS] = {[0 ... COMPAT_MAX_SWITCHES-1][0 ... COMPAT_MAX_SWITCH_PORTS-1] = -1};
...
static int tr_port_by_net_port(enetx_port_t *port, void *_ctx)
{
    ...
    switch (net_port)
    {
        ...
        case NET_PORT_LAN_0 ... NET_PORT_LAN_7:
            if (!port->port_info.is_gpon && !port->port_info.is_epon && !port->port_info.is_epon_ae && port->p.mac && port->p.mac->mac_id == net_port)
            {
                *(enetx_port_t **)_ctx = port;
                return 1;
            }
...
static int _handle_add_port_ae(enetx_port_t **_p)
{
    ...
    unit_port_array[0][MAX_SWITCH_PORTS-1] = p;
    p->has_interface = 1;
...
static int _handle_add_port(struct net_port_t *net_port)
{
    ...
    if (port == NET_PORT_GPON)
        p->dev->bcm_nd_ext.path.hw_subport_mcast_idx = NETDEV_PATH_HW_SUBPORTS_MAX;
...
extern void handle_phy_move(enetx_port_t *port, phy_dev_t *phy_dev, int external_endpoint);
#else
...
static char *print_net_dev_info(enetx_port_t *port)
{
    ...
    if (dev)
    {
        ...
        snprintf(buf, sizeof(buf), "%-10s: %4s %8s %02X:%02X:%02X:%02X:%02X:%02X",
                dev->name, netdev_link?"Up":"Down",
                netdev_link? mac_dev_speed_to_str(mac_cfg.speed): "",
    ...
    {   /* Internal device */
        snprintf(buf, sizeof(buf), "%-10s: %4s %8s %02X:%02X:%02X:%02X:%02X:%02X",
                "<IntnlDev>", "Up", mac_dev?mac_dev_speed_to_str(mac_cfg.speed):"", 0, 0, 0, 0, 0, 0);
    }

    return buf;
}

static char *print_phy_link_down(phy_dev_t *phy_dev)
{
#if defined(DSL_DEVICES)
    ...
    switch (phy_serdes->sfp_module_type)
    {
        case SFP_FIXED_PHY:
            return "Down";
        case SFP_NO_MODULE:
            return "No SFP Module";
        default:
            return "SFP Module In";
    }
#else
    return "Down";
#endif
...
static char *inter_phy_type_string(phy_dev_t *phy_dev, int inter_phy_type)
{
    ...
    strcpy(buf, inter_phy_string[inter_phy_type]);
    if (phy_dev->an_enabled && inter_phy_type != INTER_PHY_TYPE_UNKNOWN)
        strcat(buf, "_A");
    return buf;
}
...
char *get_inter_phy_types(phy_dev_t *phy_dev)
{
    ...
    phy_dev_config_speed_get(phy_dev, &config_speed, &config_duplex);
        ...
        n += snprintf(buf+n, sz-n, "%20s:%16s", 
            inter_phy_types_string(config_types),
            inter_phy_type_string(phy_dev, current_type));
...
static char *print_phy_link(phy_dev_t *phy_dev)
{
    ...
    n += snprintf(buf+n, sz-n, "Cfg: %s%8s;", an?"ANG.":"FIX.", phy_dev_speed_to_str(max_spd));
    n += snprintf(buf+n, sz-n, " %16s %16s", phy_dev->link?"Up": print_phy_link_down(phy_dev), 
        phy_dev->link?phy_dev_speed_to_str(phy_dev->speed):"");
...
static char *_print_phy_info(phy_dev_t *phy_dev, char *bf, int indent)
{
    ...
    n += snprintf(bf+n, sz-n, "PHY_ID:0x%02x:%6s %5s %5s %8s %12s %12s %s",
            phy_dev->addr,
            (PhyIsExtPhyId(phy_dev)||phy_dev->phy_drv->phy_type==PHY_TYPE_I2C_PHY)? "Extrnl":"OnChip",
            (phy_dev->mii_type==PHY_MII_TYPE_RGMII)? "RGMII": phy_dev->cascade_prev? "Cscde": "",
            apd_status[apd_enable],
            power_up? "PwrUp": "PwrDn",
            phy_dev_speed_to_str(max_spd),
            PhyIsPortConnectedToExternalSwitch(phy_dev)? "2_EXTSW":
            PhyIsFixedConnection(phy_dev)? "PHY_FIX":
            phy_dev_get_phy_name(phy_dev),
...
static char *print_port_info(enetx_port_t *p)
{
    ...
    snprintf(buf, sizeof(buf), "%sSw:P%d Lgcl:%2d %3s",
...
#if defined(CONFIG_TENDA_PRIVATE_BSP) && defined(CONFIG_SWITCH_BCM)
int switch_phy_status_get(int port_id, uint32_t *link)
{
    ...
    // 这里适配规则：0为内部phy，即wan口；1/2/3/4分别为switch的四个端口
    // phyid是从0开始计算，eth1对应的是0，eth2是1，eth3是2，eth4是3，加上3为portid
    // 5为内部phy，即总线,减去4为portid
    // CONFIG_BCM_EXT_SWITCH代表有外置switch，portid与驱动的port直接对应，不做转换
#if !defined(CONFIG_BCM_EXT_SWITCH)
    *link = port_i_status[port_id];
#else
    if (0 == port_id) {
        *link = port_i_status[port_id];
    } else if (port_id > 2) {
        *link = port_e_status[port_id-3];
    } else if (1 == port_id) {
        *link = port_i_status[port_id+4];
    }
#endif

    return 0;
}
EXPORT_SYMBOL(switch_phy_status_get);
#endif
...
#if defined(CONFIG_TENDA_PRIVATE_BSP) && defined(CONFIG_SWITCH_BCM)
     if (p->p.mac->mac_id < MAX_PORT_NUM) {
        if (PORT_ON_ROOT_SW(p)) {
            port_i_status[p->p.mac->mac_id] = phy->link;
       } else {
           port_e_status[p->p.mac->mac_id] = phy->link;
       }
    }
#endif
}
...
#if defined(DSL_DEVICES)
        {
            ...
            if (ethctl->flags == ETHCTL_FLAG_ACCESS_SILENT_START)
            {
                ...
                if (ethctl->op == ETHSETMIIREG)
                    ret = phy_dev_silent_start_set(phy_dev, ethctl->phy_reg); 
                else
            ...
            else if (ethctl->flags == ETHCTL_FLAG_ACCESS_SERDES_POWER_MODE)
            {
                /* The single PHY is specified, so should not operate on cascade */
                if (ethctl->op == ETHSETMIIREG)
                    ret = phy_dev_apd_set(phy_dev, ethctl->phy_reg); 
                else
    ...
    case ETHSETPHYWOLSLEEP:
        {
            ...
            if (ethctl->flags & ETHCTL_FLAG_MPMAC_SET)
                memcpy(pwi.mac_addr, ethctl->mpmac, ETH_ALEN);
            else
                memcpy(pwi.mac_addr, dev->dev_addr, ETH_ALEN);
            return phy_dev_set_wol_sleep(phy_dev, &pwi) ? -EFAULT : 0;
        }
...
#if defined(DSL_DEVICES)
static void auto_caps_to_speed(int caps, phy_speed_t *speed)
{
    int speeds = 0;
    ...
    if (caps & (PHY_CAP_1000_FULL|PHY_CAP_1000_HALF))
    {
        *speed = PHY_SPEED_1000;
        speeds++;
    }

    if (caps & (PHY_CAP_100_FULL|PHY_CAP_100_HALF))
    {
        *speed = PHY_SPEED_100;
        speeds++;
    }

    if (caps & (PHY_CAP_10_FULL|PHY_CAP_10_HALF))
    {
        *speed = PHY_SPEED_10;
        speeds++;
    }
...
int enet_ioctl_compat_ethswctl(struct net_device *dev, struct ifreq *rq, int cmd)
{
    ...
    enetx_port_t *port = NULL;
    int i;
#if defined(DSL_DEVICES) || defined(CONFIG_BCM96855)
    int ret;
#endif
...
#define PEAK_RATE 10000000000UL
        ...
        case ETHSWPORTTXRATE:
            ...
            return -(EOPNOTSUPP);
#endif // SF2_DEVICE
#elif defined(RUNNER) /* !defined(DSL_DEVICES) */
            {
    ...
    case ETHSWPHYMODE:
        {
            ...
            else if (ethswctl->type == TYPE_GET)
            {
                ...
                else if (phy_dev->phy_drv->caps_get)
                {
                    ...
                    auto_caps_to_speed(caps, &speed);
                    duplex = (caps & (PHY_CAP_AUTONEG|PHY_CAP_10000|PHY_CAP_5000|PHY_CAP_2500|
    ...
    case ETHSWOAMIDXMAPPING:
        ...
        else
        {
            ...
            for (u = 1; u < COMPAT_MAX_SWITCHES; u++)
#else
            for (u = 0; u < COMPAT_MAX_SWITCHES; u++)
#endif
            {
