 ...
 *
 * $Id: wlc_dfs.c 810245 2022-04-04 05:11:16Z $
 */
...
enum wlc_dfs_iov {
    ...
    IOV_DFS_HANDLE_RADAR_ONSTA = 15,    /* Disabled by default. Radar detection logic for
                         ...
                         * WET/DWDS/MAP.
                         */
    IOV_ALLOW_BGDFS_ON_APSTA = 16,
    IOV_DFS_RESET = 17,    /* clear radar status, clear cac time */
    IOV_LAST
};
...
static const bcm_iovar_t wlc_dfs_iovars[] = {
    ...
    {"dfs_reset", IOV_DFS_RESET, (0), 0, IOVT_BUFFER, 0},
    ...
    {"allow_bgdfs_on_apsta", IOV_ALLOW_BGDFS_ON_APSTA, 0, 0, IOVT_BOOL, 0},
    {NULL, 0, 0, 0, 0, 0}
};
...
#define BGDFS_DISCARD_RESULT_TRAFFIC_PERCENT_ADJACENT(wlc)    (5)    /* adjacent channel */
...
struct wlc_dfs_info {
    ...
    struct wl_timer *radar_report_timer;    /* timer for radar report handler */
    struct wl_timer *retry_timer;        /* timer for retrying radar report */
    int radar_report_cfg_id;
...
static void wlc_dfs_cacstate_abort(wlc_dfs_info_t *dfs);
...
static void wlc_radar_report_retry_handler(void *arg);
static int wlc_radar_report_timer_init(wlc_dfs_info_t *dfs);
static void wlc_dfs_send_action_frame_complete(wlc_info_t *wlc, uint txstatus, void *arg);
static bool wlc_dfs_process_action_frame_status(wlc_dfs_info_t *dfs, wlc_bsscfg_t *bsscfg,
    uint txstatus);
static void wlc_dfs_start_radar_report_timer(wlc_info_t *wlc);
...
wlc_dfs_cac_state_change(wlc_dfs_info_t *dfs, uint newstate)
{
    ...
    if (!scan_in_progress && ((dfs->dfs_cac.status.state == WL_DFS_CACSTATE_IDLE) ||
            ...
            (cfg->cac_started == FALSE)) {
        /* This change handles the pre-cleared channels in EU country code.
         * When moving into a pre-cleared channel due to zdfs (bgdfds) the
         * driver only sends a cac_end event. Also when doing a wl down and
         * up, the channel cac_state is persistant and driver only sends a
         * cac_end event. Hostapd expects both cac_start and cac_end in a
         * sequence to be in syncronization with the driver and hence
         * requires both cac_start and cac_end events. This change sends
         * dummy cac_start event before issuing a regular cac_end event for
         * pre-cleared channels and keeps hostapd in sync.
         */
#if defined(BCMDBG) || defined(WLMSG_DFS)
    ...
    if (!cac_event->scan_core) {
        if (newstate == WL_DFS_CACSTATE_PREISM_CAC) {
            ...
            cfg->cac_started = FALSE;
                ...
                cfg->cac_started == TRUE) {
            WL_DFS(("wl%d.%d: Init cac_started to FALSE due %s\n",
...
BCMATTACHFN(wlc_dfs_detach)(wlc_dfs_info_t *dfs)
{
    ...
    if (dfs->retry_timer != NULL) {
        wl_free_timer(wlc->wl, dfs->retry_timer);
    }
#endif /* SLAVE_RADAR */
    ...
    void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
    ...
    case IOV_SVAL(IOV_DFS_RESET):
        wlc_dfs_reset_all(dfs);
        WL_DFS(("wl%d: Clear radar status, clear cac time\n", wlc->pub->unit));
        break;
    ...
    case IOV_SVAL(IOV_DFS_AP_MOVE):
    {
        ...
        /* Reject BGDFS request when operating as Repeater on non-ETSI and
         * When primary bss is a STA and is assocaited
         */
        if (APSTA_ENAB(wlc->pub) && BSSCFG_AS_STA(wlc->primary_bsscfg) &&
                !wlc->pub->_allow_bgdfs_on_apsta) {
            if (wlc->is_edcrs_eu) {
                preclear_only = TRUE;
            } else {
        ...
        /* Honour BW_CAP check for moving to a chanspec */
        if (!CHSPEC_IN_BWCAP(wlc->band->bw_cap, chspec) &&
                wlc->band->bandtype != WLC_BAND_2G) {
            WL_DFS(("wl%d: No BGDFS as BWCap 0x%02x doesnt cover chspec 0x%04x BW\n",
                WLCWLUNIT(wlc), wlc->band->bw_cap, chspec));
            err = BCME_BADCHAN;
            break;
        }
        ...
        csa.chspec = chspec;
        csa.count = MAX((WLC_DFS_CSA_MSEC/bsscfg->current_bss->beacon_period),
            WLC_DFS_CSA_BEACONS);
        ...
        if (wlc->is_edcrs_eu && !wlc_quiet_chanspec(wlc->cmi, chspec) &&
            ...
            WL_DFS(("wl%d: Skip CAC - channel 0x%x is already "
                    ...
                    wlc->pub->unit, chspec));
            wlc_csa_each_up_ap(wlc, &csa, FALSE);
        } else {
    ...
    case IOV_SVAL(IOV_DFS_HANDLE_RADAR_ONSTA):
        ...
        break;
#endif /* SLAVE_RADAR */
    case IOV_GVAL(IOV_ALLOW_BGDFS_ON_APSTA): {
        *ret_int_ptr = (int32)wlc->pub->_allow_bgdfs_on_apsta;
        break;
        }
    case IOV_SVAL(IOV_ALLOW_BGDFS_ON_APSTA): {
        wlc->pub->_allow_bgdfs_on_apsta = bool_val;
        break;
        }
...
wlc_dfs_bg_downgrade_wlc(wlc_info_t *wlc)
{
    ...
    FOREACH_UP_AP(wlc, idx, bsscfg) {
        ...
        WL_MODE_SWITCH(("wl%d.cfg:%d %s modesw_entry\n", wlc->pub->unit,
            bsscfg->_idx, __FUNCTION__));
        err = wlc_modesw_handle_oper_mode_notif_request(wlc->modesw, bsscfg,
                new_oper_mode, TRUE, MODESW_CTRL_OPMODE_IE_REQD_OVERRIDE);
        if (err != BCME_OK) {
    ...
    if (BGDFS_ENAB(wlc->pub) && all->num_sub_status < max_num_sub) {
        ...
        if (PHYMODE_BGDFS_ACTIVE(dfs->phymode)) {
            ...
            if (dfs->scan_both) {
                ...
                dfs->cfg->cac_started = FALSE;
            } else {
                sub0->state = WL_DFS_CACSTATE_IDLE;
                if (dfs->cfg->cac_started == TRUE) {
                    WL_DFS(("wl%d.%d: cac_started to FALSE due to %s\n",
...
wlc_dfs_dump(void *ctx, struct bcmstrbuf *b)
{
    ...
    if (dfs->dfs_cac.chanspec_forced_list) {
        for (i = 0; i < dfs->dfs_cac.chanspec_forced_list->num; i++)
            bcm_bprintf(b, "chanspec_forced %x state %d cactime %u\n",
                dfs->dfs_cac.chanspec_forced_list->list[i],
                dfs->dfs_cac.status.state,
                dfs->dfs_cac.cactime);
    }
...
wlc_radar_report_handler(void *arg)
{
    ...
    dfs->radar_report_cfg_id = WLC_MAXBSSCFG;
    wl_del_timer(wlc->wl, dfs->retry_timer);
...
static void
wlc_radar_report_retry_handler(void *arg)
{
    wlc_dfs_info_t *dfs;
    wlc_bsscfg_t *bsscfg;
    ...
    bsscfg = wlc_bsscfg_find(dfs->wlc, dfs->radar_report_cfg_id, NULL);

    if (bsscfg) {
        wlc_11h_send_basic_report_radar(dfs->wlc, bsscfg,
...
BCMATTACHFN(wlc_radar_report_timer_init)(wlc_dfs_info_t *dfs)
{
    ...
    if (!(dfs->retry_timer = wl_init_timer(wlc->wl,
        wlc_radar_report_retry_handler, dfs, "retry_timer"))) {
        goto timer_init_err;
    }
    return BCME_OK;

timer_init_err:
    WL_ERROR(("wl%d: wlc_retry_timer_init failed\n", wlc->pub->unit));
    return BCME_ERROR;
}
#endif /* STA && SLAVE_RADAR */
static void
...
wlc_dfs_channel_oos(wlc_dfs_info_t *dfs, uint8 channel, uint32 tdwr_block_time)
{
    ...
    chspec = CH20MHZ_CHSPEC(channel, WL_CHANSPEC_BAND_5G);
    if (!wlc_radar_chanspec(wlc->cmi, chspec)) {
        return;
    }

    /* channel will be blocked only if 'dfs_auto_channel_change_disabled' is false */
    if (!dfs->dfs_auto_channel_change_disabled) {
...
wlc_dfs_chanspec_oos(wlc_dfs_info_t *dfs, chanspec_t chanspec)
{
    ...
    bool has_subband_info;
    bool radar_on_tdwr_left = FALSE, radar_on_tdwr_right = FALSE;
    uint8 channel, radar_sb, pos_20 = LOWER_20_POS_20MHZ;
    uint last_channel;
    ...
    has_subband_info = DFS_HAS_SUBBAND_INFO(wlc);
    radar_sb = dfs->radar_subbands;
...
wlc_dfs_rearrange_channel_list(wlc_dfs_info_t *dfs, chanspec_list_t *ch_list)
{
    ...
    for (i = 0; i < ch_list->num; i++) {
        ...
        if (((~WL_CHANSPEC_CTL_SB_MASK) & cur_ch) ==
                ...
                wlc_is_european_weather_radar_channel(wlc, chspec) || /* TDWR */
...
wlc_dfs_chanspec(wlc_dfs_info_t *dfs, bool radar_detected)
{
    ...
    if (!chspec) {
        ...
        chspec = wlc_channel_rand_chanspec(dfs->wlc, FALSE, radar_detected);
    }
...
wlc_radar_detected(wlc_dfs_info_t *dfs, bool scan_core)
{
    ...
    radar_type = phy_radar_detect(WLC_PI(wlc), &radar_detected, scan_core, FALSE);
    ...
    if ((wlc_11h_get_spect_state(wlc->m11h, cfg) & radar_sim_mask) ||
        ...
        WL_DFS(("wl%d: ###### DETECTED A RADAR on Chanspec 0x%x subband 0x%x type %d "
            "on core %d\n", wlc->pub->unit, chspec,
            dfs->radar_subbands, radar_type, scan_core));
...
#ifdef CONFIG_UGW_BCM_DONGLE
    {
        struct km_wifiev_dfs_chanspes_detected_param param;
        param.cur_chan = radar_data.current_chanspec;
        param.target_chan = radar_data.target_chanspec;
        param.radar_type = radar_data.radar_info[0].radar_type;

        wlc_mac_event(wlc, WLC_E_TENDA_KM, NULL, KM_WIFIEV_DFS_CHANSPES_DETECTED, 0, 0, &param, sizeof(param));
    }
#else
    if (km_eventscenter_wifiev_dfs_chanspes_detected_handler) {
        km_eventscenter_wifiev_dfs_chanspes_detected_handler(radar_data.current_chanspec, radar_data.target_chanspec, radar_data.radar_info[0].radar_type);
    }
#endif
#endif
...
wlc_dfs_cacstate_cac(wlc_dfs_info_t *dfs)
{
    ...
    if (!dfs->dfs_cac.duration) {
        ...
        if (WL11H_STA_ENAB(wlc) && wlc_dfs_get_radar(wlc->dfs)) {
            ...
            /* STA to move to ISM if CAC triggered
             * due to Radar detected DFS Re-entry
             */
            if (wlc_assoc_get_as_state(wlc->primary_bsscfg) == AS_DFS_CAC_START) {
                wlc_dfs_cacstate_ism_set(dfs);
            }
        }
#endif /* STA && SLAVE_RADAR */
...
wlc_dfs_cacstate_ism(wlc_dfs_info_t *dfs)
{
     ...
     * the DATA fifo in wlc_csa_each_up_ap
     */
...
#if defined(SLAVE_RADAR)
/* Send 11h Basic Measurement Report */
void
wlc_dfs_send_action_frame(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
    wlc_dfs_info_t *dfs = wlc->dfs;

    if (dfs->radar_report_cfg_id != WLC_MAXBSSCFG) {
        return;
    }
    dfs->radar_report_cfg_id = WLC_BSSCFG_IDX(bsscfg);
...
/*
 * This function will check the txstatus value for the Basic Measurement Report sent by the slave.
 * If ACK received, will send disassoc immediately.If ACK not received,
 * will retry again until certain time.
 * If retry limit exceeded or time expired, will send disassoc request anyway.
 */
static bool
wlc_dfs_process_action_frame_status(wlc_dfs_info_t *dfs, wlc_bsscfg_t *bsscfg, uint txstatus)
{
    ...
    if ((txstatus & TX_STATUS_ACK_RCV) || !bsscfg ||
        ...
        if (dfs->actionfr_retry_counter < WLC_RADAR_REPORT_RETRY_LIMIT) {
            ...
            wl_add_timer(wlc->wl, wlc->dfs->retry_timer, 1, FALSE);
            return FALSE;
        } else {
 ...
 * the basic report slave sends. Will retry until certain time and later send
 * disassoc.
 */
static void
...
/*
 * DFS CAC is Aborted. This is an intermediate state change to IDLE.
 */
static void
wlc_dfs_cacstate_abort(wlc_dfs_info_t *dfs)
{
    /* This is an intermediate state nothing to be done */
    WL_DFS(("wl%d: dfs : In state %s chanspec 0x%04x\n", WLCWLUNIT(dfs->wlc),
        wlc_dfs_cacstate_str(dfs->dfs_cac.status.state),
        phy_utils_get_chanspec(WLC_PI(dfs->wlc))));
...
BCMRAMFN(wlc_dfs_cacstate_handler)(void *arg)
{
    ...
    ASSERT(wlc_dfs_cacstate_fn_ary[dfs->dfs_cac.status.state] != NULL);
...
void wlc_deinit_dfs_cfg(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg)
{
        // derefrence:
    if (wlc->dfs && wlc->dfs->cfg == bsscfg) {
        wlc->dfs->cfg = NULL;
    }
}
#endif
...
{
    /*
     * In case this is called for a non DFS channel, 'dfs' may be NULL,
     * it is not necessary to assert it.  Simply return false, it is
     * much more generic and more widely usable this way.
     */
    if (!dfs) return FALSE;
...
/* WAR for HostAPD to function seamless across channel switches/across DFS states.
 * This is a intermediate state when CAC is aborted due to channel switch.
 */
void
wlc_dfs_abort_cac(wlc_dfs_info_t *dfs)
{
    wlc_info_t* wlc = dfs->wlc;
    wlc_bsscfg_t *apcfg;
    int idx;
    ...
    FOREACH_UP_AP(wlc, idx, apcfg) {
        dfs->cfg = apcfg;
        /* set state to Abort CAC */
        wlc_dfs_cac_state_change(dfs, WL_DFS_CACSTATE_ABORT_CAC);
    }
}
