 ...
 *
 * $Id: wlc_rrm.c 807713 2022-01-28 10:38:02Z $
 */
...
/* RRM_NBR_SCAN macros */
#define RRM_NBR_SCAN_WILDCARD_SSID    1
#define RRM_NBR_SCAN_SAME_SSID        2
...
typedef struct rrm_nbr_rep {
    ...
    dot11_meas_rep_t *opt_lci;
    dot11_meas_rep_t *opt_civic;            /**< Location Civic Measurement */
    dot11_meas_rep_t *opt_locid;
} rrm_nbr_rep_t;
...
static int wlc_rrm_add_loc(wlc_rrm_info_t *rrm_info, dot11_meas_rep_t **loc,
...
static int wlc_rrm_add_tbtt_info_for_nbr(uint8 *pbuf, wlc_bsscfg_t *bsscfg,
    nbr_rpt_elem_t *nbr_elt, bool directed_prb_req);
static int wlc_rrm_count_nbrs_same_rclass_chan(wlc_rnr_info_t *pnbr_info, uint8 chan, uint8 rclass,
    uint8 size);
...
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
        TDCORE_FUNC_IF(maps_beacon_rep_handler) {
            TDCORE_FUNC(maps_beacon_rep_handler)(scb, cfg);
        }
#endif

#if defined(CONFIG_TENDA_PRIVATE_KM) && defined(CONFIG_EVENTS_CENTER)
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
#ifdef CONFIG_UGW_BCM_DONGLE
    wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_BEACON_REP, 0, 0, &scb->beacon_rep, sizeof(scb->beacon_rep));
#else
    if (km_eventscenter_wifiev_beacon_rep_handler) {
        km_eventscenter_wifiev_beacon_rep_handler(scb->ea.octet, (void *)&scb->beacon_rep);
    }
#endif
#endif
#endif
...
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
        beacon_buf = (uint8 *)kmalloc(body_len + TD_EM_MACADDRLEN + 3, GFP_ATOMIC);
        if (beacon_buf) {
            //beacon_buf[0]和beacon_buf[1]用于保存body_len字段
            //这个body_len表示的长度是802.11 wireless management部分的长度
            beacon_buf[0] = body_len >> 8 & 0xff;
            beacon_buf[1] = body_len & 0xff;
            //偏移掉保存长度两字节
            memcpy(beacon_buf + 2, &scb->ea, TD_EM_MACADDRLEN);
            //偏移掉保存长度两字节和mac地址6字节
            memcpy(beacon_buf + 2 + TD_EM_MACADDRLEN, body, body_len);
            WL_RRM(("[%s] STA MAC -%pM \n", __FUNCTION__,&scb->ea));
            WL_RRM(("[%s] Body len -%d \n", __FUNCTION__,body_len));

            td_easymesh_beacon_metrics_rsp_notifiy_tlv((void *)beacon_buf);
            kfree(beacon_buf);
        }
#else
#ifdef TD_EASYMESH_SUPPORT
        beacon_buf = (uint8 *)MALLOCZ(wlc->osh, body_len + TD_EM_MACADDRLEN + 3);
        if (beacon_buf) {
            //beacon_buf[0]和beacon_buf[1]用于保存body_len字段
            //这个body_len表示的长度是802.11 wireless management部分的长度
            beacon_buf[0] = body_len >> 8 & 0xff;
            ...
            if(!TDCORE_FUNC_IS_NULL(td_easymesh_beacon_metrics_rsp_notifiy_tlv)) {
                TDCORE_FUNC(td_easymesh_beacon_metrics_rsp_notifiy_tlv)((void *)beacon_buf, cfg);
            }
            MFREE(wlc->osh, beacon_buf, body_len + TD_EM_MACADDRLEN + 3);
        }
#endif
#endif
    }
...
#if defined(WL11K_BCN_MEAS) || defined(WL11K_ALL_MEAS) || (defined(WL_FTM_11K) && \
    defined(WL_PROXDETECT) && defined(WL_FTM))
    ...
    for (; ie != NULL && idx < count;
        ...
        case DOT11_MEASURE_TYPE_NOISE:
            ...
            req[idx].chanspec = CH20MHZ_CHSPEC(rmreq_noise->channel,
                    WL_CHANNEL_2G5G_BAND(rmreq_noise->channel));
        ...
        case DOT11_MEASURE_TYPE_CHLOAD:
            ...
            req[idx].chanspec = CH20MHZ_CHSPEC(rmreq_chanload->channel,
                    WL_CHANNEL_2G5G_BAND(rmreq_chanload->channel));
        ...
        case DOT11_MEASURE_TYPE_FRAME:
            ...
            req[idx].chanspec = CH20MHZ_CHSPEC(rmreq_frame->channel,
                    WL_CHANNEL_2G5G_BAND(rmreq_frame->channel));
...
#define wlc_rrm_parse_requests(rrm_info, cfg, ie, len, req, count)

#endif /* WL11K_BCN_MEAS || WL11K_ALL_MEAS || (WL_FTM_11K && WL_PROXDETECT && WL_FTM) */
...
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    bzero(&scb->beacon_rep, sizeof(td_11k_rrm_info_t));
#endif
    ...
    if ((bcmwifi_rclass_get_chanlist(wlc_channel_country_abbrev(wlc->cmi),
        ...
        for (i = 0; i < chanlist->count; i++)
            bcn_req->chanspec_list[i] = CH20MHZ_CHSPEC(chanlist->element[i],
                    WL_CHANNEL_2G5G_BAND(chanlist->element[i]));
    }
    ...
    for (i = 0; i < MAXCHANNEL; i++) {
        if (isset(chanlist_bitmask, i)) {
            bcn_req->chanspec_list[channel_count++] =
                CH20MHZ_CHSPEC(i + 1, WL_CHANNEL_2G5G_BAND(i + 1));
        }
    }
    ...
    wlc_rrm_req_t *req)
{
    ...
    if (WLC_RRM_REPORT_ALL_CHANNELS(rmreq_bcn)) {
        ...
        bcn_req->chanspec_list[0] = CH20MHZ_CHSPEC(rmreq_bcn->channel,
                WL_CHANNEL_2G5G_BAND(rmreq_bcn->channel));
    }
...
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    //steerd的beacon report会存在多个不同bssid的情况，所以去掉对bssid的判断
    {
        if (TD_MAX_BEACON_REPORT == scb->beacon_rep.beacon_report_num) {
            scb->beacon_rep.beacon_report_num = 0;
        }
        scb->beacon_rep.beacon_report[scb->beacon_rep.beacon_report_num].op_class = rmrep_bcn->reg;
        scb->beacon_rep.beacon_report[scb->beacon_rep.beacon_report_num].channel = rmrep_bcn->channel;
        scb->beacon_rep.beacon_report[scb->beacon_rep.beacon_report_num].RCPI = rmrep_bcn->rcpi;
        scb->beacon_rep.beacon_report[scb->beacon_rep.beacon_report_num].RSNI = rmrep_bcn->rsni;
        bcopy(&rmrep_bcn->bssid, &(scb->beacon_rep.beacon_report[scb->beacon_rep.beacon_report_num].bssid), ETHER_ADDR_LEN);
        scb->beacon_rep.beacon_report_num++;
    }
#endif
...
wlc_rrm_begin_scan(wlc_rrm_info_t *rrm_info)
{
    ...
    if (bcn_req->scan_type == DOT11_RMREQ_BCN_TABLE) {
        ...
        ret = wlc_scan_request(wlc, DOT11_BSSTYPE_ANY, &bcn_req->bssid,
            1, &bcn_req->ssid, 0, NULL, bcn_req->scan_type, 1, bcn_req->duration,
...
wlc_rrm_send_lcirep(wlc_rrm_info_t *rrm_info, uint8 token)
{
    ...
    if (rrm_cfg->nbr_rep_self &&
        ...
        lci_rep = rrm_cfg->nbr_rep_self->opt_lci;
...
wlc_rrm_send_civiclocrep(wlc_rrm_info_t *rrm_info, uint8 token)
{
    ...
    if (rrm_cfg->nbr_rep_self &&
        ...
        civic_rep = rrm_cfg->nbr_rep_self->opt_civic;
...
static void wlc_rrm_send_locidrep(wlc_rrm_info_t *rrm_info, uint8 token)
{
    ...
    if (rrm_cfg->nbr_rep_self &&
        ...
        locid_rep = rrm_cfg->nbr_rep_self->opt_locid;
...
wlc_rrm_nbr_scancb(void *arg, int status, wlc_bsscfg_t *cfg)
{
    ...
    if (ap_bi->flags2 & WLC_BSS2_VHT) {
            ap_bssid_info |= DOT11_NGBR_BI_VHT;
    }

    if (ap_bi->flags3 & WLC_BSS3_HE) {
            ap_bssid_info |= DOT11_NGBR_BI_HE;
    }
    ...
    for (i = 0; i < bsslist->count; i++) {
        ...
        nbr_bssid_info = DOT11_NGBR_BI_REACHABILTY | DOT11_NGBR_BI_SEC |
            ...
            ((bi->flags & WLC_BSS_HT) ? DOT11_NGBR_BI_HT: 0) |
            ((bi->flags2 & WLC_BSS2_VHT) ? DOT11_NGBR_BI_VHT: 0) |
            ((bi->flags3 & WLC_BSS3_HE) ? DOT11_NGBR_BI_HE: 0);

        /* Phytype of neighbor */
#ifdef WL11AX
        if (bi->flags3 & WLC_BSS3_HE) {
            nbr_elt.phytype = DOT11_PHYTYPE_HE;
        } else
#endif /* WL11AX */
#ifdef WL11AC
        if (bi->flags2 & WLC_BSS2_VHT) {
            nbr_elt.phytype = DOT11_PHYTYPE_VHT;
        } else
#endif /* WL11AC */
        if (bi->flags & WLC_BSS_HT) {
            nbr_elt.phytype = DOT11_PHYTYPE_HT;
        }
    ...
    int body_len)
{
    ...
    if (rrm_cfg == NULL) {
        WL_ERROR(("wl%d: %s: rrm_cfg is NULL\n", wlc->pub->unit, __FUNCTION__));
        ASSERT(rrm_cfg != NULL);
        return;
    }
...
nrreq_next_tlv:
    ...
    if (rrm_cfg->nbr_scan) {
        if (rrm_cfg->nbr_scan == RRM_NBR_SCAN_SAME_SSID) {
            ssid.SSID_len = MIN(DOT11_MAX_SSID_LEN, (uint8)cfg->SSID_len);
            bcopy(cfg->SSID, ssid.SSID, ssid.SSID_len);
        }
        bcmerr = wlc_scan_request(wlc, DOT11_BSSTYPE_ANY, &ether_bcast, 1, &ssid,
                0, NULL, DOT11_SCANTYPE_ACTIVE, -1, 0, -1, -1, NULL, 0, 0, TRUE,
    ...
    wlc_ssid_t *ssid, uint8 flags, int addtype)
{
    ...
    if ((flags & RRM_NREQ_FLAGS_OWN_REPORT) && ssid_self_match) {
        ...
        if (BSS_N_ENAB(wlc, cfg)) {
            ...
            nbr_rep_tmp.nbr_elt.phytype = DOT11_PHYTYPE_HT;
        }
        if (BSS_VHT_ENAB(wlc, cfg)) {
            nbr_rep_tmp.nbr_elt.bssid_info |= DOT11_NGBR_BI_VHT;
            nbr_rep_tmp.nbr_elt.phytype = DOT11_PHYTYPE_VHT;
        }
        if (isset(rrm_cfg->rrm_cap, DOT11_RRM_CAP_FTM_RANGE))
            nbr_rep_tmp.nbr_elt.bssid_info |= DOT11_NGBR_BI_FTM;
        if (BSS_HE_ENAB(wlc, cfg)) {
            nbr_rep_tmp.nbr_elt.bssid_info |= DOT11_NGBR_BI_HE;
            nbr_rep_tmp.nbr_elt.phytype = DOT11_PHYTYPE_HE;
        }
    ...
    for (i = 0; i < list_cnt; i++) {
        ...
        if (nbr_elt->flags & WL_RRM_NBR_RPT_LCI_FLAG) {
            if (nbr_rep->opt_lci != NULL) {
                dot11_meas_rep_t *lci_rep = nbr_rep->opt_lci;
                memcpy(ptr, lci_rep, lci_rep->len + TLV_HDR_LEN);
        ...
        if (nbr_elt->flags & WL_RRM_NBR_RPT_CIVIC_FLAG) {
            if (nbr_rep->opt_civic != NULL) {
                dot11_meas_rep_t *civic_rep = nbr_rep->opt_civic;
                memcpy(ptr, civic_rep, civic_rep->len + TLV_HDR_LEN);
...
/** AP specific functionality */
static int
wlc_rrm_count_nbrs_same_rclass_chan(wlc_bsscfg_t *bsscfg, uint8 chan, uint8 rclass)
{
    uint8 count = 0;
    rrm_bsscfg_cubby_t *rrm_cfg;
    rrm_nbr_rep_t *nbr_rep;

    rrm_cfg = RRM_BSSCFG_CUBBY(bsscfg->wlc->rrm_info, bsscfg);
    nbr_rep = rrm_cfg->nbr_rep_head;

    while (nbr_rep) {
        if ((nbr_rep->nbr_elt.channel == chan) &&
            (nbr_rep->nbr_elt.reg == rclass)) {
            count++;
        }
        nbr_rep = nbr_rep->next;
    }
    ...
    bool directed_prb_req)
{
    ...
    if (!memcmp(bsscfg->SSID, nbr_elt->ssid.SSID,
        MAX(bsscfg->SSID_len, nbr_elt->ssid.SSID_len))) {
        if ((nbr_elt->flags & WLC_RRM_NBR_FLAG_HIDDEN_SSID) &&
            !directed_prb_req) {
            nbr_elt->bss_params &= ~TBTT_INFO_BSS_PARAMS_SAME_SSID;
        } else {
            nbr_elt->bss_params |= TBTT_INFO_BSS_PARAMS_SAME_SSID;
        }
    } else {
        if (nbr_elt->bss_params & TBTT_INFO_BSS_PARAMS_SAME_SSID)
            nbr_elt->bss_params &= ~TBTT_INFO_BSS_PARAMS_SAME_SSID;
    }
...
bool
wlc_rrm_any_nbr_hidden(wlc_bsscfg_t *cfg)
{
    wlc_info_t *wlc = NULL;
    rrm_bsscfg_cubby_t *rrm_cfg = NULL;
    rrm_nbr_rep_t *nbr_rep = NULL;
    ...
    while (nbr_rep) {
        if ((nbr_rep->nbr_elt.flags & WLC_RRM_NBR_FLAG_HIDDEN_SSID)) {
            return TRUE;
        }
        nbr_rep = nbr_rep->next;
    }
...
/* fill static rnr info to be populated in beacon and probe response frames from static
 * list of rrm neighbors.
 * directed_prb_req : Set/reset TBTT's info "same ssid" to handle directed
 * probe request for closed network
 */
int
wlc_rrm_fill_static_rnr_info(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 *pibuf,
    uint8 max_pibuf_len, uint8 *len_copied, bool directed_prb_req)
{
    ...
    int8 index;
    uint8 nbrs_with_same_rclass_chan = 0;
    ...
    while (nbr_rep) {
        ...
        if (index == -1) {
            ...
            if (!rnr_info[index].pbuf) {
                nbrs_with_same_rclass_chan =
                    wlc_rrm_count_nbrs_same_rclass_chan(bsscfg, chan, rclass);

                ASSERT(nbrs_with_same_rclass_chan);
                /* add 4 bytes for tbtt header[2] + opclass[1] + chan[1] */
                required_buf_len +=
                    ((nbrs_with_same_rclass_chan *
                    TBTT_INFO_FIELD_HDR_LEN) + 4);

                if (required_buf_len >= max_pibuf_len) {
                    WL_ERROR(("wl%d:%d, required buf len[%d] exceeding[%d]"
                        " passed buffer length, prepare whatever"
                        " is available\n", wlc->pub->unit,
                        WLC_BSSCFG_IDX(bsscfg), required_buf_len,
                        max_pibuf_len));
                    /* crossing threshold, prepare whatever is available
                     * and return
                     */
                    goto write_nbr_ap_info;
                }
                rnr_info[index].pbuf = (uint8*)MALLOCZ(wlc->osh,
                (nbrs_with_same_rclass_chan * TBTT_INFO_FIELD_HDR_LEN));

                rnr_info[index].max_count = nbrs_with_same_rclass_chan;

            }
            wlc_rrm_add_tbtt_info_for_nbr(rnr_info[index].pbuf, bsscfg,
                &nbr_rep->nbr_elt, directed_prb_req);
    ...
    /* Debug code only */
    WL_INFORM(("wl%d: rnr buffer after populating static RRM nbr list, nbr_count[%d]\n",
        wlc->pub->unit, nbr_count));
...
write_nbr_ap_info:
     ...
     */
    pnbr_ap_info = pibuf;
    ibuf_len = BCM_TLV_MAX_DATA_SIZE;
    for (index = 0; index < nbr_count; index++) {
        nbytes_rd = 0;
        ...
        wlc_write_neighbor_ap_info_field(pnbr_ap_info, ibuf_len, &rnr_info[index],
            &nbytes_rd);
        if (nbytes_rd == 0) {
            WL_ERROR(("wl%d:%d Avoid  Overflow.. for copy[%d] bytes from rnr_info"
                " index[%d] tbtt_len[%d] count[%d] max_count[%d] pbuf[%p]"
                " to output buffer\n", wlc->pub->unit,
                WLC_BSSCFG_IDX(bsscfg), nbytes_rd, index, rnr_info[index].tbtt_len,
                rnr_info[index].count, rnr_info[index].max_count,
                rnr_info[index].pbuf));
            ret = BCME_NORESOURCE;
            /* maxdata copied, goto finish */
            goto finish;
        }
    ...
    for (index = 0; index < nbr_count; index++) {
        if (rnr_info[index].pbuf) {
            MFREE(wlc->osh, rnr_info[index].pbuf,
                (rnr_info[index].max_count * TBTT_INFO_FIELD_HDR_LEN));
...
/** AP specific functionality */
int
wlc_write_neighbor_ap_info_field(uint8 *pibuf, uint16 ibuf_len, wlc_rnr_info_t *nbr_rnr_info,
    uint8 *len_copied)
{
    neighbor_ap_info_field_t info;
    uint16 tbtt_info_hdr = 0;
    uint16 n_bytes_to_copy = 0;

    ASSERT(nbr_rnr_info);

    n_bytes_to_copy = NEIGHBOR_AP_INFO_FIELD_HDR_LEN +
        (nbr_rnr_info->count * (nbr_rnr_info->tbtt_len));

    if (n_bytes_to_copy > ibuf_len) {
        /* Overflow */
        return BCME_BUFTOOSHORT;
    }
    ...
    memcpy(pibuf, (uint8*)&info, NEIGHBOR_AP_INFO_FIELD_HDR_LEN);
    pibuf += NEIGHBOR_AP_INFO_FIELD_HDR_LEN;
    memcpy(pibuf, nbr_rnr_info->pbuf, nbr_rnr_info->count * (nbr_rnr_info->tbtt_len));

    pibuf += nbr_rnr_info->count * (nbr_rnr_info->tbtt_len);
    *len_copied = (uint8)n_bytes_to_copy;
    ...
    rrm_nbr_rep_t *nbr_rep, int *buflen, uint8 flags)
{
    ...
    *buflen -= DOT11_RM_IE_LEN;
    ...
    if (nbr_rep->opt_lci != NULL) {
        dot11_meas_rep_t *opt_lci_rep = nbr_rep->opt_lci;
        ...
        memcpy(ptr, (((char *)opt_lci_rep) + DOT11_RM_IE_LEN), copy_len);
    ...
    rrm_nbr_rep_t *nbr_rep, int *buflen, uint8 flags)
{
    ...
    *buflen -= DOT11_RM_IE_LEN;
    ...
    if (nbr_rep->opt_civic != NULL) {
        dot11_meas_rep_t *opt_civic_rep = nbr_rep->opt_civic;
        ...
        memcpy(ptr, (((char *)nbr_rep->opt_civic) + DOT11_RM_IE_LEN), copy_len);
    ...
    void *p, uint plen, void *a, int alen)
{
    ...
    if (rrm_cfg_cmd->id <= WL_RRM_FRNG_NONE ||
        rrm_cfg_cmd->id >= ARRAYSIZE(rrmfrng_cmd_fn))
        return BCME_BADARG;
    ...
    void *p, uint plen, void *a, int alen)
{
    ...
    if (param->id <= WL_RRM_CONFIG_NONE ||
        param->id >= ARRAYSIZE(rrmconfig_cmd_fn))
        return BCME_BADARG;
    ...
    void *p, uint plen, void *a, int alen)
{
    ...
    if (rrm_cfg_cmd->len != alen - WL_RRM_CONFIG_MIN_LENGTH) {
        return BCME_BADARG;
    }

    if (rrm_cfg_cmd->id <= WL_RRM_CONFIG_NONE ||
        rrm_cfg_cmd->id >= ARRAYSIZE(rrmconfig_cmd_fn))
        return BCME_BADARG;
...
*/
static int
wlc_rrm_add_loc(wlc_rrm_info_t *rrm_info, dot11_meas_rep_t **loc,
    uint8 *new_loc, int len, uint8 type)
{
...
wlc_rrm_free_self_report(wlc_rrm_info_t *rrm_info, wlc_bsscfg_t *bsscfg)
{
    ...
    if (nbr_rep) {
        ...
        if (nbr_rep->opt_civic) {
            rep = nbr_rep->opt_civic;
            MFREE(rrm_info->wlc->osh, rep,
                rep->len + TLV_HDR_LEN);
        }
        if (nbr_rep->opt_locid) {
            rep = nbr_rep->opt_locid;
            MFREE(rrm_info->wlc->osh, rep,
                rep->len + TLV_HDR_LEN);
        }
        MFREE(rrm_info->wlc->osh, nbr_rep, sizeof(rrm_nbr_rep_t));
        ...
        rrm_nbr_rep_t *nbr_rep, wlc_bsscfg_t *cfg)
{
    while (mr_tlvs) {
        if (mr_tlvs->id == DOT11_MNG_MEASURE_REPORT_ID) {
            ...
            if (meas_rep->type == DOT11_MEASURE_TYPE_LCI) {
                nbr_rep->opt_lci = meas_elt;
            } else if (meas_rep->type == DOT11_MEASURE_TYPE_CIVICLOC) {
                nbr_rep->opt_civic = meas_elt;
            }
        }
    ...
    while (nbr_rep) {
        ...
        if (!wf_chspec_valid(frng_req->targets[i].chanspec)) {
            ...
            frng_req->targets[i].chanspec = CH20MHZ_CHSPEC(nbr_rep->nbr_elt.channel,
                    WL_CHANNEL_2G5G_BAND(nbr_rep->nbr_elt.channel));
        }
        ...
        void *p, uint plen, void *a, uint alen, uint vsize, struct wlc_if *wlcif)
{
    ...
    case IOV_SVAL(IOV_RRM_NBR_ADD_NBR):
    {
        ...
        if (nbr_elt.version == WL_RRM_NBR_RPT_VER) {
            if (nbr_elt.ssid.SSID_len > DOT11_MAX_SSID_LEN) {
                WL_ERROR(("wl%d:%d nbr's ssid len[%d] is more than max"
                    " supported[%d]\n", wlc->pub->unit, WLC_BSSCFG_IDX(cfg),
                    nbr_elt.ssid.SSID_len, DOT11_MAX_SSID_LEN));
                return BCME_BADARG;
            }
#if BAND6G || defined(WL_OCE_AP)
            /* valid supported tbtt_len 0, 11 or 13 */
            if ((nbr_elt.tbtt_len != 0) &&
                (nbr_elt.tbtt_len != NBR_AP_TBTT_BSSID_SHORT_SSID_LEN) &&
                (nbr_elt.tbtt_len != TBTT_INFO_FIELD_HDR_LEN)) {
                WL_ERROR(("wl%d:%d Invalid nbr's tbtt len[%d] to prepare RNR IE\n",
                    wlc->pub->unit, WLC_BSSCFG_IDX(cfg), nbr_elt.tbtt_len));
                return BCME_BADARG;
            }
#endif /* BAND6G || WL_OCE_AP */
            if (!VALID_NBR_CHAN(nbr_elt.channel)) {
                WL_ERROR(("wl%d:%d Invalid nbr's channel[%d] to prepare RNR IE\n",
                    wlc->pub->unit, WLC_BSSCFG_IDX(cfg), nbr_elt.channel));
                return BCME_BADARG;
            }

            if ((nbr_elt.reg > MAXREGCLASS) || !nbr_elt.reg) {
                WL_ERROR(("wl%d:%d Invalid nbr's regclass[%d] to prepare RNR IE\n",
                    wlc->pub->unit, WLC_BSSCFG_IDX(cfg), nbr_elt.reg));
                return BCME_BADARG;
            }
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN)
    TDP_INFO("wl%d.%d : set rrm_val default value(0x%x)\n", wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg),
        DOT11_RRM_CAP_NEIGHBOR_REPORT_ENAB | DOT11_RRM_CAP_BCN_PASSIVE_ENAB | DOT11_RRM_CAP_BCN_ACTIVE_ENAB);
#endif
#else
 ...
 */
bool
wlc_rrm_ssid_is_in_nbr_list(wlc_bsscfg_t *cfg, wlc_ssid_t *ssid, uint32 *s_ssid,
    uint8 short_ssid_len)
{
    rrm_bsscfg_cubby_t *rrm_cfg;
    rrm_nbr_rep_t *nbr_rep;
    int i;
    ...
    while (nbr_rep != NULL) {
        ...
        for (i = 0; i < short_ssid_len / sizeof(s_ssid[0]); i++) {
            if (s_ssid[i] == nbr_short_ssid) {
