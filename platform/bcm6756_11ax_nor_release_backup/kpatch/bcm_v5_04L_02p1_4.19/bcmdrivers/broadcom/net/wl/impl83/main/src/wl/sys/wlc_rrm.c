 ...
 *
 * $Id: wlc_rrm.c 798928 2021-05-17 05:25:13Z $
 */
...
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
        maps_beacon_rep_handler(scb, cfg);
#endif
    ...
    bool directed_prb_req)
{
    ...
    if (!memcmp(bsscfg->SSID, nbr_elt->ssid.SSID, bsscfg->SSID_len)) {
...
wlc_rrm_fill_static_rnr_info(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, uint8 *pibuf,
    uint8 *len_copied, bool directed_prb_req)
{
