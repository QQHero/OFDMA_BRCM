...
*
*
* $Id: wlc_msch.c 789974 2020-08-12 18:41:09Z $
*/
...
static bool msch_check_bf_timeslot(wlc_msch_info_t *msch_info, msch_timeslot_t *ts, bool cur_ts);
...
static bool msch_chn_seq_update_entity(wlc_msch_info_t *msch_info, wlc_msch_req_handle_t *req_hdl,
	msch_req_entity_t *entity);
static bool _msch_enity_next_ts_overlap(wlc_msch_info_t *msch_info, msch_req_entity_t *entity);
static void msch_chan_cont_req_update(wlc_msch_info_t * msch_info, wlc_msch_req_handle_t *req_hdl);
...
static uint32 wlc_msch_switch_time(wlc_msch_info_t *msch_info, chanspec_t from, chanspec_t to);
...
#define msch_display_time(cur_time)		msch_time_h(cur_time), msch_time_l(cur_time)
#else
#define msch_display_time(cur_time)		(uint32)((cur_time) >> 32), (uint32)(cur_time)
#endif /* MSCH_DEBUG_FANCY_TIME */
...
#ifdef MSCH_DEBUG_PRINT
static uint msch_debug_level = (MSCH_ERROR_ON | MSCH_DEBUG_ON | MSCH_INFORM_ON | MSCH_TRACE_ON);
#define MSCH_MESSAGE_ERROR(args)	do {if (msch_debug_level & MSCH_ERROR_ON) { \
		WL_PRINT(args);}} while (0)
#define MSCH_MESSAGE_DEBUG(args)	do {if (msch_debug_level & MSCH_DEBUG_ON) { \
		WL_PRINT(args);}} while (0)
#define MSCH_MESSAGE_INFORM(args)	do {if (msch_debug_level & MSCH_INFORM_ON) { \
		WL_PRINT(args);}} while (0)
#define MSCH_MESSAGE_TRACE(args)	do {if (msch_debug_level & MSCH_TRACE_ON) { \
		WL_PRINT(args);}} while (0)
#else
#define MSCH_MESSAGE_ERROR(args)
#define MSCH_MESSAGE_DEBUG(args)
#define MSCH_MESSAGE_INFORM(args)
#define MSCH_MESSAGE_TRACE(args)
#endif /* MSCH_DEBUG_PRINT */

static msch_timeslot_t*
msch_timeslot_alloc(wlc_msch_info_t *msch_info)
{
	...
	MSCH_MESSAGE_INFORM(("%s: timeslot ALLOC %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(timeslot)));
...
_msch_slot_skip(wlc_msch_info_t *msch_info,  msch_req_entity_t *entity)
{
	...
	MSCH_MESSAGE_DEBUG(("%s: idx %d chan 0x%x skip start time %u\n",
		__FUNCTION__, entity->cur_chn_idx, entity->chanspec,
	...
	if (req_param->interval) {
		...
		if (cur_ts) {
			cur_chnsw_time = wlc_msch_switch_time(msch_info,
				cur_ts->chan_ctxt->chanspec, entity->chan_ctxt->chanspec);
		}
		if (n_ts) {
			next_chnsw_time = wlc_msch_switch_time(msch_info,
				n_ts->chan_ctxt->chanspec, entity->chan_ctxt->chanspec);
		}
			...
			if (!cur_slot && entity->pend_slot.timeslot == ts) {
				...
				*/
				if (entity->req_hdl->req_param->flags &
					...
					MSCH_MESSAGE_DEBUG(("%s: ts_d idx %d chan 0x%x "
						"New start time %u\n", __FUNCTION__,
	...
	bool free2list)
{
	...
	MSCH_MESSAGE_INFORM(("%s: timeslot FREE %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(timeslot)));
...
msch_req_entity_alloc(wlc_msch_info_t *msch_info)
{
	...
	MSCH_MESSAGE_INFORM(("%s: req_entity ALLOC %p\n", __FUNCTION__,
...
static void
msch_req_entity_free(wlc_msch_info_t *msch_info, msch_req_entity_t *req_entity,
	bool free2list)
{
	uint32 freelist_size;
	MSCH_MESSAGE_INFORM(("%s: req_entity FREE %p\n", __FUNCTION__,
		OSL_OBFUSCATE_BUF(req_entity)));
...
_msch_chan_ctxt_alloc(wlc_msch_info_t *msch_info, chanspec_t chanspec)
{
	...
	MSCH_MESSAGE_INFORM(("%s: chan_ctxt ALLOC %p\n", __FUNCTION__,
...
{
	MSCH_MESSAGE_TRACE(("%s: Entry\n", __FUNCTION__));
	MSCH_MESSAGE_INFORM(("%s: chan_ctxt FREE %p\n", __FUNCTION__,
		OSL_OBFUSCATE_BUF(chan_ctxt)));
...
_msch_req_handle_alloc(wlc_msch_info_t *msch_info)
{
	...
	MSCH_MESSAGE_INFORM(("%s: req_hdl ALLOC %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(req_hdl)));
...
{
	uint32 freelist_size;
	MSCH_MESSAGE_INFORM(("%s: req_hdl FREE %p\n", __FUNCTION__, OSL_OBFUSCATE_BUF(req_hdl)));
...
/* channel change function, majorly invokes wlc_set_chanspec() */
static int
msch_chan_adopt(wlc_msch_info_t *msch_info, msch_chan_ctxt_t *chan_ctxt)
{
	...
	MSCH_MESSAGE_DEBUG(("%s Change Chanspec 0x%x, reason 0x%08x\n", __FUNCTION__, chanspec,
		chansw_reason));
	...
	wlc_enable_mac(wlc);
	ASSERT((phy_utils_get_chanspec(wlc->pi) == chanspec));
	return BCME_OK;
}
	...
	msch_chan_ctxt_t *chan_ctxt, wlc_msch_req_param_t *req_param, msch_req_timing_t *slot)
{
	...
	if (msch_info->cur_msch_timeslot &&
		...
		MSCH_MESSAGE_ERROR(("%s: req prestart time %u is less than current "
	...
	timeslot = msch_timeslot_alloc(msch_info);
	if (!timeslot) {
		MSCH_MESSAGE_ERROR(("%s: msch_timeslot_alloc failed!!\n", __FUNCTION__));
		ASSERT(FALSE);
		return NULL;
	}
	...
	MSCH_MESSAGE_DEBUG(("%s: TS REQ chan 0x%x type 0x%x pre_start_time %d.%06d duration %u\n",
		__FUNCTION__, chan_ctxt->chanspec, req_param->req_type,
		msch_display_time(timeslot->pre_start_time), (uint32)timeslot->sch_dur));
	...
	if (ts == msch_info->cur_msch_timeslot) {
		...
		if (new_end_time) {
			if (new_end_time != ts->end_time) {
				...
				MSCH_MESSAGE_DEBUG(("%s: Updated cur ts end_time to %d.%06d!!\n",
					__FUNCTION__, msch_display_time(ts->end_time)));
...
static int
msch_req_entity_create(wlc_msch_info_t *msch_info, chanspec_t from_chanspec,
	chanspec_t to_chanspec, wlc_msch_req_handle_t *req_hdl, uint16 chan_idx)
{
	msch_list_elem_t *elem;
	...
	chan_ctxt = _msch_chan_ctxt_alloc(msch_info, to_chanspec);
	if (!chan_ctxt) {
		MSCH_MESSAGE_ERROR(("%s: msch_chan_ctxt_alloc failed!!\n", __FUNCTION__));
		ret = BCME_NOMEM;
		goto fail;
	}
	/* If there is a change in channel bandwidth, then update the context bandwidth */
	if (chan_ctxt->chanspec) {
		/* pick widest band */
		if (CHSPEC_BW(to_chanspec) > CHSPEC_BW(chan_ctxt->chanspec)) {
			chan_ctxt->chanspec = to_chanspec;
		}
	} else {
		chan_ctxt->chanspec = to_chanspec;
	}
	...
	req_entity = msch_req_entity_alloc(msch_info);
	if (!req_entity) {
		MSCH_MESSAGE_ERROR(("%s: msch_req_entity_alloc failed!!\n", __FUNCTION__));
		ret = BCME_NOMEM;
		goto fail;
	}
	...
	req_entity->chanspec = to_chanspec;
	...
	if (!MSCH_START_FLEX(req_param->req_type)) {
		...
		chansw_time = wlc_msch_switch_time(msch_info, from_chanspec, to_chanspec);
	...
	if (MSCH_START_FIXED(req_param->req_type)) {
		...
		MSCH_MESSAGE_ERROR(("%s: No MSCH reqest type found\n", __FUNCTION__));
		ret = BCME_BADARG;
		goto fail;
	}
...
_msch_req_entity_destroy(wlc_msch_info_t *msch_info, msch_req_entity_t *req_entity)
{
	...
	msch_req_entity_free(msch_info, req_entity, TRUE);
}
	...
	uint64 free_slot, msch_req_entity_t *entity, uint32 prep_dur)
{
	...
	if (msch_info->flex_list_cnt > 1) {
		if (entity->both_flex_list.next) {
			...
			next_bf_pre_start -= wlc_msch_switch_time(msch_info, chan_ctxt->chanspec,
				next_entity->chan_ctxt->chanspec);
			...
			if (next_entity != entity && next_entity->req_hdl->req_param->interval) {
				...
				next_bf_pre_start -= wlc_msch_switch_time(msch_info,
					chan_ctxt->chanspec,
					next_entity->chan_ctxt->chanspec);
...
_msch_slot_start_end_notif(wlc_msch_info_t *msch_info, msch_timeslot_t *ts)
{
	...
	if (next_fire > cur_time) {
		MSCH_MESSAGE_DEBUG(("%s: next fire time is greater than cur_time\n", __FUNCTION__));
		return next_fire;
	}
	...
	while ((elem = msch_list_remove_head(&msch_info->msch_req_timing_list))) {
		...
		if (slot->flags & MSCH_RC_FLAGS_START_FIRE_DONE) {
			...
			if (ts->fire_time != slot->end_time) {
				MSCH_MESSAGE_DEBUG(("%s: fire %d.%06d end_time "
					"%d.%06d cur_time %d.%06d\n", __FUNCTION__,
					msch_display_time(ts->fire_time),
					msch_display_time(slot->end_time),
					msch_display_time(cur_time)));
			}
			...
			if (ts->fire_time != slot->start_time) {
				MSCH_MESSAGE_DEBUG(("%s: fire %d.%06d start_time "
					"%d.%06d cur_time %d.%06d\n", __FUNCTION__,
					msch_display_time(ts->fire_time),
					msch_display_time(slot->start_time),
					msch_display_time(cur_time)));
			}
...
_msch_chsw_tmr_handle_cur_ts(wlc_msch_info_t *msch_info, uint64 cur_time)
{
	...
	if (cur_ts->state == MSCH_TIMER_STATE_ONCHAN_FIRE) {
		...
		if (!msch_list_empty(&msch_info->msch_req_timing_list)) {
			...
			if (cur_ts->fire_time != MSCH_INFINITE_TIME) {
				MSCH_MESSAGE_INFORM(("%s: cur_ts->fire_time is not infinite\n",
					__FUNCTION__));
		...
		if (cur_ts->end_time != MSCH_INFINITE_TIME) {
			if (cur_ts->end_time > cur_time) {
				MSCH_MESSAGE_INFORM(("%s: end_time is greater than cur_time\n",
					__FUNCTION__));
				return CHSW_TIMER_DONE;
			}
			if (msch_check_bf_timeslot(msch_info, cur_ts, TRUE)) {
	...
	if (cur_ts->state == MSCH_TIMER_STATE_TS_COMPLETE) {
		if (!msch_info->next_timeslot ||
				...
				msch_check_bf_timeslot(msch_info, msch_info->next_timeslot,
				FALSE)) {
...
_msch_chsw_timer(void *arg)
{
	...
	MSCH_MESSAGE_DEBUG(("%s: current_time: %d.%06d\n", __FUNCTION__,
		msch_display_time(cur_time)));
	...
	if (!msch_info->next_timeslot->fire_time ||
		...
		MSCH_MESSAGE_DEBUG(("%s: fire_time not set: cur_fire %d.%06d\n", __FUNCTION__,
			msch_display_time(msch_info->next_timeslot->fire_time)));
	...
	if (cur_time < msch_info->next_timeslot->fire_time) {
		if (msch_info->next_timeslot->ts_flags & MSCH_TS_FLAG_DONT_DESTROY) {
			WL_ERROR(("%s : no cur ts, next dont destroy set\n", __FUNCTION__));
	...
	/* Switch to channel */
	if (cur_ts->chan_ctxt->chanspec != phy_utils_get_chanspec(wlc->pi)) {
		msch_chan_adopt(msch_info, cur_ts->chan_ctxt);
	}
	...
	if (cur_ts) {
		if (cur_ts->fire_time != MSCH_INFINITE_TIME) {
			...
			MSCH_MESSAGE_DEBUG(("%s: fire_time: %d.%06d, diff %d\n", __FUNCTION__,
				msch_display_time(cur_ts->fire_time),
...
_msch_next_chn_seq_pend_slot(wlc_msch_info_t *msch_info, msch_req_entity_t *req_entity)
{
	...
	MSCH_MESSAGE_DEBUG(("%s: req_start: %u \n", __FUNCTION__, req_param->start_time_l));
	 ...
	 */
	req_entity->actual_start_time = req_entity->pend_slot.start_time;
	MSCH_MESSAGE_DEBUG(("channel_index: %d \n", req_entity->cur_chn_idx));
	MSCH_MESSAGE_DEBUG(("%s: start %u end %u\n", __FUNCTION__,
...
_msch_next_pend_slot(wlc_msch_info_t *msch_info, msch_req_entity_t *req_entity)
{
	...
	if (MSCH_START_FIXED(req_param->req_type)) {
		...
		if (req_param->interval) {
			...
			chansw_time = wlc_msch_switch_time(msch_info, INVCHANSPEC,
				req_entity->chanspec);
			req_entity->pend_slot.pre_start_time =
				req_entity->pend_slot.start_time - chansw_time;
			req_entity->pend_slot.end_time = req_entity->pend_slot.start_time +
...
_msch_timeslot_chn_clbk(wlc_msch_info_t *msch_info, msch_timeslot_t *ts, uint32 clbk_type)
{
	...
	MSCH_MESSAGE_TRACE(("%s: Entry clbk_type 0x%x\n", __FUNCTION__, clbk_type));
	ASSERT(ts);
	...
	while ((elem = msch_list_iterate_pend_callback(req_entity_list))) {
		...
		if (clbk_type == MSCH_CT_PRE_ONCHAN) {
			 ...
			 */
			cb_info.chanspec = phy_utils_get_chanspec(wlc->pi);
		}
...
_msch_arm_chsw_timer(wlc_msch_info_t *msch_info)
{
	...
	if (msch_info->cur_msch_timeslot &&
		...
		if (msch_info->next_timeslot &&
			...
			msch_info->next_timeslot->pre_start_time))) {
			MSCH_MESSAGE_ERROR(("%s:new_ts overlap cur_end %u next_start %u\n",
				__FUNCTION__,
				(uint32)msch_info->cur_msch_timeslot->end_time,
	...
	if (msch_info->next_timeslot == NULL) {
		MSCH_MESSAGE_INFORM(("%s: no next timeslot to schedule\n", __FUNCTION__));
		return;
	}
	...
	MSCH_MESSAGE_INFORM(("%s: current time: %x-%x, switch_time: %x-%x, diff: %u\n",
		__FUNCTION__, (uint32)(cur_time >> 32), (uint32)cur_time,
		(uint32)(switch_time >> 32), (uint32)switch_time,
		(uint32)(switch_time - cur_time)));

	/* arm timer */
	ASSERT(msch_info->chsw_timer != NULL);
	MSCH_MESSAGE_INFORM(("%s: wlc_msch_add_timeout: %d\n", __FUNCTION__, (int)(switch_time)));
	...
	if (msch_info->cur_msch_timeslot &&
		...
		if (msch_check_bf_timeslot(msch_info, msch_info->cur_msch_timeslot, TRUE)) {
	...
	MSCH_MESSAGE_DEBUG(("%s : switch_time: %d.%06d, diff %d\n", __FUNCTION__,
		msch_display_time(timeslot->fire_time), (int)(switch_time)));
	...
	if (interval_change) {
		if (msch_info->max_lo_prio_interval == 0 ||
			msch_info->flex_list_cnt == 0) {
				MSCH_MESSAGE_DEBUG(("%s: interval: %d flex_list_cnt %d\n",
					__FUNCTION__,
					msch_info->max_lo_prio_interval,
					msch_info->flex_list_cnt));
			return;
...
_msch_schd_new_piggyback_curts(wlc_msch_info_t *msch_info, msch_req_entity_t *new_entity)
{
	...
	if (MSCH_START_FIXED(req_param->req_type)) {
		...
		min_dur = MSCH_MIN_ONCHAN_TIME;
		min_dur += wlc_msch_switch_time(msch_info, cur_ts->chan_ctxt->chanspec,
			new_entity->chan_ctxt->chanspec);
...
_msch_schd_new_req(wlc_msch_info_t *msch_info, wlc_msch_req_handle_t *req_hdl)
{
	...
	if (!elem) {
		MSCH_MESSAGE_ERROR(("wl%d:%s: error entity list empty \n",
	...
	if (chan_ctxt == new_entity->chan_ctxt) {
		...
		if (chan_ctxt->chanspec != phy_utils_get_chanspec(wlc->pi)) {
			msch_chan_adopt(msch_info, chan_ctxt);
		}
		if (_msch_schd_new_piggyback_curts(msch_info, new_entity) == MSCH_SCHD_NEXT_SLOT) {
			goto schd_next;
		}
		return MSCH_ALREADY_SCHD;
	} else if (msch_info->next_timeslot) {
		chansw_time = wlc_msch_switch_time(msch_info,
			msch_info->next_timeslot->chan_ctxt->chanspec, new_entity->chanspec);
		if (MSCH_START_FIXED(req_param->req_type) &&
			(new_entity->pend_slot.start_time >
		...
		chansw_time = wlc_msch_switch_time(msch_info, chan_ctxt->chanspec,
			new_entity->chanspec);
		if (cur_ts->end_time != MSCH_INFINITE_TIME &&
			cur_ts->end_time >
	...
	uint64 slot_start_time)
{
	...
	if (entity->pend_slot.timeslot) {
		MSCH_MESSAGE_ERROR(("wl%d: %s: timeslot already scheduled\n",
			msch_info->wlc->pub->unit, __FUNCTION__));
		ASSERT(0);
		return;
	}
	...
	if (req_hdl->req_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
		if (entity->pend_slot.flags & MSCH_RC_FLAGS_SPLIT_SLOT_START) {
			...
			if ((slot_start_time < (entity->pend_slot.start_time + prep_dur)) &&
				...
				MSCH_MESSAGE_INFORM(("%s: update remaing slot start_time %d.%06d\n",
					__FUNCTION__,
					msch_display_time(entity->pend_slot.start_time)));
			}
		}
 ...
 */
static uint64
msch_curts_slot_endtime(wlc_msch_info_t *msch_info)
{
	uint64 end_time = 0;
...
_msch_get_next_slot_fire(wlc_msch_info_t *msch_info)
{
	...
	MSCH_MESSAGE_INFORM(("%s: no slot fire pending\n", __FUNCTION__));
...
_msch_get_next_sfix(wlc_msch_info_t *msch_info, uint64 slot_start, uint64 prep_dur)
{
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		if (sfix_entity->chan_ctxt != cur_chan_ctxt) {
			chansw_time = wlc_msch_switch_time(msch_info,
				phy_utils_get_chanspec(wlc->pi), sfix_entity->chan_ctxt->chanspec);
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		if (sfix_entity->chan_ctxt != entity->chan_ctxt) {
			chansw_time = wlc_msch_switch_time(msch_info,
				sfix_entity->chan_ctxt->chanspec, entity->chan_ctxt->chanspec);
		} else {
			chansw_time = wlc_msch_switch_time(msch_info, INVCHANSPEC,
				entity->chan_ctxt->chanspec);
		}
...
_msch_slotskip_timer(void *arg)
{
	...
	while ((req_elem = msch_list_iterate(req_elem))) {
		...
		if (msch_info->next_timeslot) {
			 ...
			 */
			MSCH_MESSAGE_INFORM(("Current and next time slots are NULL, check "
				"for cur time\n"));
			slot_end_time = cur_time;
			last_chanspec = phy_utils_get_chanspec(wlc->pi);
		}
		...
		MSCH_MESSAGE_DEBUG(("%s: entity %p start time : %d.%06d, channel 0x%x\n",
			__FUNCTION__, entity,
			msch_display_time(entity->pend_slot.start_time), entity->chanspec));

		slot_due = entity->pend_slot.start_time;
		slot_due -= wlc_msch_switch_time(msch_info, last_chanspec, entity->chanspec);

		MSCH_MESSAGE_DEBUG(("cur time : %d.%06d slot_due %u\n",
			msch_display_time(cur_time), (uint32)slot_due));
...
_msch_schd(wlc_msch_info_t *msch_info)
{
	...
	chanspec_t cur_chspec = phy_utils_get_chanspec(wlc->pi);
	uint32 max_prep_time = wlc_msch_switch_time(msch_info, INVCHANSPEC, INVCHANSPEC);
	...
	if (msch_info->next_timeslot) {
		MSCH_MESSAGE_ERROR(("wl%d: %s: redundant call\n",
			msch_info->wlc->pub->unit, __FUNCTION__));
		return;
	}
	...
	if (sfix_entity) {
		...
		chansw_time = wlc_msch_switch_time(msch_info, cur_chspec,
			...
			if (sfix_slot->start_time < slot_avail) {
				if (!(sfix_entity->req_hdl->req_param->flags &
					...
					if (_msch_check_sfix_slot_skip(msch_info, cur_chspec,
						slot_start_time)) {
						MSCH_MESSAGE_DEBUG(("%s: skip slot, start skip "
							"timer\n", __FUNCTION__));
						return;
					}
				}
	...
	if (dfix_entity) {
		chansw_time = wlc_msch_switch_time(msch_info, cur_chspec,
			dfix_entity->chan_ctxt->chanspec);
	...
	if (bflex_entity) {
		...
		/* - We check if user requested min_dur (e.g. 45ms) can be serviced
	...
	if (bflex_entity && (slot_dur >= bf_min_dur)) {
		...
		if (msch_info->flex_list_cnt > 1) {
			...
			if (cur_ts && (slot_start_time > bf_start_time)) {
				...
				if (slot_dur >= (prep_dur + chansw_time +
					MSCH_MIN_ONCHAN_TIME + chansw_time +
		...
		if (slot_dur && slot_dur != MSCH_INFINITE_TIME) {
			...
			if (msch_info->flex_list_cnt > 1) {
				...
				slot_dur -= wlc_msch_switch_time(msch_info,
					bflex_entity->chan_ctxt->chanspec,
					sfix_entity->chan_ctxt->chanspec);
			}
...
_msch_check_sfix_slot_skip(wlc_msch_info_t *msch_info, chanspec_t chanspec, uint64 end_time)
{
	...
	msch_req_entity_t *entity;
	uint32 chsw_time;
	uint32 max_chsw_time = wlc_msch_switch_time(msch_info, INVCHANSPEC, INVCHANSPEC);
	...
	while ((elem = msch_list_iterate(elem))) {
		 ...
		 */
		chsw_time = wlc_msch_switch_time(msch_info, chanspec, entity->chan_ctxt->chanspec);
...
static bool
msch_check_bf_timeslot(wlc_msch_info_t *msch_info, msch_timeslot_t *ts, bool cur_ts)
{
	msch_list_elem_t *elem;
	msch_req_entity_t *entity;
	...
	msch_req_entity_t *entity)
{
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		prep_time = wlc_msch_switch_time(msch_info,
			ts->chan_ctxt->chanspec, sfix_entity->chan_ctxt->chanspec);
...
_msch_update_next_bf_endtime(wlc_msch_info_t *msch_info, msch_timeslot_t *ts)
{
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		if (MSCH_BOTH_FLEX(entity->req_hdl->req_param->req_type) &&
			...
			MSCH_MESSAGE_DEBUG(("update bf end: %u\n", (uint32)bf_end_time));
			entity->pend_slot.end_time = bf_end_time;
		}
	}
}
	...
	uint64 slot_start, uint64 dur)
{
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		req_dur = dur;
		req_dur -= wlc_msch_switch_time(msch_info,
			entity->chan_ctxt->chanspec, sfix_entity->chan_ctxt->chanspec);
...
_msch_update_both_flex_list(wlc_msch_info_t *msch_info, uint64 slot_start, uint64 slot_dur)
{
	...
	uint64 cur_time = msch_current_time(msch_info);
	uint64 next_bf_start = slot_start + slot_dur;
		...
		if (req_entity->pend_slot.start_time < cur_time) {
			...
			if (elem->next) {
				next_entity = LIST_ENTRY(elem->next, msch_req_entity_t,
					both_flex_list);
				next_bf_start = next_entity->pend_slot.start_time;
			}
			...
			if ((req_entity->pend_slot.end_time < slot_start) &&
				(slot_start > next_bf_start) &&
				((slot_start - next_bf_start) > MSCH_MIN_ONCHAN_TIME)) {
				break;
			}
		...
		if (msch_info->flex_list_cnt > 1) {
			...
			while ((elem2 = msch_list_iterate(elem2))) {
				...
				if (next_entity->pend_slot.start_time > cur_time) {
					...
					min_dur -= wlc_msch_switch_time(msch_info,
						req_entity->chanspec, next_entity->chanspec);
				}
				min_dur = MIN(min_dur,
 ...
 */
static uint32
msch_check_chn_seq_extra_prep_dur(wlc_msch_info_t *msch_info, msch_req_entity_t *entity,
	chanspec_t chspec)
{
	...
	if ((next_chan != (chanspec_t)-1) &&
		...
		chansw_time = wlc_msch_switch_time(msch_info, chspec, next_chan);
	}
	return chansw_time;
}
	...
	chanspec_t chanspec, uint64 *curts_end_time, uint32 flags, bool *split_curts)
{
	...
	if (!curts_end_time) {
		MSCH_MESSAGE_ERROR(("%s: Cur ts end time is not valid\n", __FUNCTION__));
		return -1;
	}

	if (!split_curts) {
		MSCH_MESSAGE_ERROR(("%s: split_curts %p Null parameters\n", __FUNCTION__,
			split_curts));
		ASSERT(0);
		return -1;
	}
	...
	*curts_end_time = -1;
	...
	if (cur_ts->end_time != MSCH_INFINITE_TIME) {
		...
		ts_end_time = msch_curts_slot_endtime(msch_info);
	}

	*split_curts = ((flags & MSCH_REQ_FLAGS_PREMT_IMMEDIATE) != 0);
	MSCH_MESSAGE_DEBUG(("%s: Cur ts end_time %d.%06d\n", __FUNCTION__,
		msch_display_time(ts_end_time)));

	/* msch_curts_slot_endtime returns 0 when no slots are pending
	...
	if (!ts_end_time || ts_end_time == MSCH_INFINITE_TIME) {
		...
		preempt_slot_prep += wlc_msch_switch_time(msch_info,
			cur_ts->chan_ctxt->chanspec, chanspec);
	...
	chansw_time = wlc_msch_switch_time(msch_info, cur_ts->chan_ctxt->chanspec, chanspec);
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		if (entity->cur_slot.timeslot &&
			...
			if (!(entity->req_hdl->req_param->flags &
				...
				MSCH_MESSAGE_DEBUG(("%s: preempt req fail, cur ts "
					"does not support\n", __FUNCTION__));
				return -1;
			}
			if (!extra_prep_time &&
				...
				chansw_time = msch_check_chn_seq_extra_prep_dur(msch_info, entity,
					chanspec);
				if (chansw_time) {
	...
	if (*split_curts) {
		 ...
		 * for curt_ts
		 */
		chansw_time = wlc_msch_switch_time(msch_info, cur_ts->chan_ctxt->chanspec,
			chanspec);
	...
	MSCH_MESSAGE_DEBUG(("%s: preempt_start_time %d.%06d curts_end_time %d.%06d\n",
		__FUNCTION__, msch_display_time(ts_end_time), msch_display_time(*curts_end_time)));
...
_msch_reduce_curts(wlc_msch_info_t *msch_info, uint64 end_time)
{
	...
	MSCH_MESSAGE_DEBUG(("%s: Cutting ts end time %d.%06d\n", __FUNCTION__,
		msch_display_time(msch_info->cur_msch_timeslot->end_time)));

	MSCH_MESSAGE_DEBUG(("%s: Updating current ts end time to %d.%06d\n", __FUNCTION__,
		msch_display_time(end_time)));
...
_msch_enity_next_ts_overlap(wlc_msch_info_t *msch_info, msch_req_entity_t *entity)
{
	...
	if (ts) {
		...
		ts_end_time += wlc_msch_switch_time(msch_info, ts->chan_ctxt->chanspec,
			entity->chan_ctxt->chanspec);
		start_time -= wlc_msch_switch_time(msch_info, phy_utils_get_chanspec(wlc->pi),
			entity->chan_ctxt->chanspec);
...
static int
msch_check_param(wlc_msch_info_t *msch_info, wlc_msch_req_param_t *param,
	chanspec_t from, chanspec_t to)
{
	uint64 time_check;
	uint64 cur_time = msch_current_time(msch_info);
	...
	if (param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
		if (!MSCH_BOTH_FIXED(param->req_type)) {
			MSCH_MESSAGE_ERROR(("wl%d: %s: wrong request type for chan contigous"
				", req failed\n", msch_info->wlc->pub->unit, __FUNCTION__));
			return BCME_BADARG;
		}
		return BCME_OK;
	...
	if (MSCH_START_FIXED(param->req_type)) {
		...
		if (!param->interval && (time_check < (cur_time +
			wlc_msch_switch_time(msch_info, from, to)))) {
			MSCH_MESSAGE_ERROR(("wl%d: %s: start time not in future\n",
				msch_info->wlc->pub->unit, __FUNCTION__));
			return BCME_BADARG;
		}
	}
...
BCMATTACHFN(wlc_msch_attach)(wlc_info_t *wlc)
{
	...
	/* Initialize switch times with defaults depending on host/target */
#ifdef DONGLEBUILD
	msch_info->bandswitch_time = MSCH_FD_DEFAULT_BANDSWITCH_TIME;
	msch_info->channelswitch_time = MSCH_FD_DEFAULT_CHANNEL_SWITCH_TIME;
#else
	msch_info->bandswitch_time = MSCH_NIC_DEFAULT_BANDSWITCH_TIME;
	msch_info->channelswitch_time = MSCH_NIC_DEFAULT_CHANNEL_SWITCH_TIME;
#endif /* DONGLEBUILD */
...
BCMATTACHFN(wlc_msch_detach)(wlc_msch_info_t *msch_info)
{
	...
	while ((elem = msch_list_remove_head(&msch_info->free_req_entity_list))) {
		...
		msch_req_entity_free(msch_info, req_entity, FALSE);
	}
	...
	uint32 free_heapsize)
{
	...
	MSCH_MESSAGE_INFORM(("%s: size_reserved=%d, free_heapsize=%d, allocable_chans=%d\n",
		__FUNCTION__, size_reserved, free_heapsize, allocable_chans));
	...
	wlc_msch_req_handle_t **p_req_hdl)
{
	...
	wlc_info_t *wlc = msch_info->wlc;
	chanspec_t from_chanspec = phy_utils_get_chanspec(wlc->pi);
	...
	if ((ret = msch_check_param(msch_info, req_param, from_chanspec,
		chanspec_list[0])) != BCME_OK) {
		return ret;
	}
	...
	if (MSCH_PREEMPT_REQ(req_param->flags)) {
		if (!MSCH_START_FLEX(req_param->req_type) ||
			...
			MSCH_MESSAGE_ERROR(("%s: Request not supported, type %d "
				"dur %d flags 0x%x\n", __FUNCTION__,
		...
		if (msch_info->next_timeslot &&
			(msch_info->next_timeslot->ts_flags & MSCH_TS_FLAG_DONT_DESTROY)) {
			MSCH_MESSAGE_ERROR(("%s: next ts is fixed due and cannot support "
				"pre-empt\n", __FUNCTION__));
		...
		if (!msch_info->cur_msch_timeslot) {
			...
			prep_time += wlc_msch_switch_time(msch_info,
				phy_utils_get_chanspec(wlc->pi), chanspec_list[0]);
			...
			chanspec_list[0], &new_end_time, req_param->flags, &split_curts))
				== MSCH_INFINITE_TIME) {
			MSCH_MESSAGE_ERROR(("%s: current timeslot not pre-emptable.\n",
				__FUNCTION__));
			return BCME_UNSUPPORTED;
		}
	...
	if ((req_param->flags & MSCH_REQ_FLAGS_MERGE_CONT_SLOTS) &&
		...
		MSCH_MESSAGE_ERROR(("%s: Merge slots set but duration (%d) and "
	...
	if (!req_hdl) {
		MSCH_MESSAGE_ERROR(("%s: msch_req_handle_alloc failed!!\n", __FUNCTION__));
		return BCME_NOMEM;
	}
	...
	if (req_hdl->req_param == NULL) {
		...
		MSCH_MESSAGE_ERROR(("%s: req_param alloc failed!!\n", __FUNCTION__));
		return BCME_NOMEM;
	}
	memcpy(req_hdl->req_param, req_param, sizeof(*req_hdl->req_param));

	MSCH_MESSAGE_INFORM(("chanspec_list:\n"));
	for (chanspec_num = 0; chanspec_num < chanspec_cnt; chanspec_num++) {
		MSCH_MESSAGE_INFORM(("[%02d] 0x%04x(%d)\n", chanspec_num,
			chanspec_list[chanspec_num], (chanspec_list[chanspec_num] & 0xFF)));
	}

	MSCH_MESSAGE_INFORM(("flags = 0x%04x\treq_type = %d\tpriority = %d\n",
		req_hdl->req_param->flags, req_hdl->req_param->req_type,
		req_hdl->req_param->priority));
	MSCH_MESSAGE_INFORM(("start_time = %x-%x\tduration = %d\tinterval = %d\n",
	...
	if (req_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
		ASSERT(MSCH_START_FIXED(req_param->req_type));
		ASSERT(req_param->duration >
			wlc_msch_switch_time(msch_info, INVCHANSPEC, chanspec_list[0]));
		req_hdl->chn_list = MALLOCZ(msch_info->wlc->osh,
			(sizeof(chanspec_t) * chanspec_cnt));

		if (!req_hdl->chn_list) {
			MSCH_MESSAGE_ERROR(("%s: chanspec list memory alloc "
				"(%d bytes) failed!!\n", __FUNCTION__,
				(int)((sizeof(chanspec_t) * chanspec_cnt))));
			goto fail;
		}
	...
	MSCH_MESSAGE_DEBUG(("%s: req_entity_create, chanspec_cnt %d\n", __FUNCTION__,
		chanspec_cnt));
	for (chanspec_num = 0; chanspec_num < chanspec_cnt; chanspec_num++) {
		to_chanspec = chanspec_list[chanspec_num];
		if (to_chanspec == ((chanspec_t)-1)) {
		...
		if (MSCH_START_FIXED(req_hdl->req_param->req_type) &&
			(entity = _msch_req_entity_available(msch_info, req_hdl, to_chanspec))) {
			entity->flags |= MSCH_ENTITY_FLAG_MULTI_INSTANCE;
			continue;
		}
		ret = msch_req_entity_create(msch_info, from_chanspec, to_chanspec,
			req_hdl, chanspec_num);
		if (ret != BCME_OK) {
			MSCH_MESSAGE_ERROR(("%s: msch_req_entity_create failed\n", __FUNCTION__));
			goto fail;
		}
	 ...
	 */
	if (req_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
		msch_chan_cont_req_update(msch_info, req_hdl);
	}
	...
	*/
	...
	if (ret != MSCH_FAIL) {
		*p_req_hdl = req_hdl;
		MSCH_MESSAGE_DEBUG(("%s: create req_hdl 0x%08x\n", __FUNCTION__,
	...
	wlc_msch_req_param_t *preempt_param, chanspec_t preempt_chanspec)
{
	...
	if (!WLC_CHAN_COEXIST(preempt_chanspec, ts->chan_ctxt->chanspec)) {
		split_slot_start += wlc_msch_switch_time(msch_info,
			ts->chan_ctxt->chanspec, preempt_chanspec);
	}
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		MSCH_MESSAGE_DEBUG(("Schd split slot, start %u end %u duration %u\n",
...
wlc_msch_timeslot_unregister(wlc_msch_info_t *msch_info, wlc_msch_req_handle_t **p_req_hdl)
{
	...
	*p_req_hdl = NULL;
	...
	if (!_msch_req_hdl_valid(msch_info, req_hdl)) {
		MSCH_MESSAGE_ERROR(("wl%d: %s: Invalid req_hdl\n",
			msch_info->wlc->pub->unit, __FUNCTION__));
		*p_req_hdl = NULL;
		return BCME_OK;
	}
	...
	if (msch_info->next_timeslot) {
		if ((!msch_info->next_timeslot->chan_ctxt) ||
			((msch_info->next_timeslot->ts_flags & MSCH_TS_FLAG_DONT_DESTROY) &&
		 !msch_check_bf_timeslot(msch_info, msch_info->next_timeslot, FALSE))) {
			_msch_timeslot_destroy(msch_info, msch_info->next_timeslot, FALSE);
			msch_info->next_timeslot = NULL;
		}
	}
	...
	uint32 timeslot_id)
{
	...
	if ((req_hdl = *p_req_hdl) == NULL) {
		MSCH_MESSAGE_ERROR(("wl%d: %s: req_hdl NULL\n",
			msch_info->wlc->pub->unit, __FUNCTION__));
		return BCME_BADARG;
	}
	...
	*/
	...
	if (drift < 0) {
		...
		if (_msch_check_curts_preemption(msch_info, ABS(drift), entity->chanspec,
			...
			MSCH_MESSAGE_DEBUG(("%s: Cutting current ts end time "
				"from %d.%06d to %d.%06d\n", __FUNCTION__,
				msch_display_time(msch_info->cur_msch_timeslot->end_time),
				msch_display_time(cur_ts_endtime)));
...
msch_timeslot_update_slot_endtime(wlc_msch_info_t *msch_info, wlc_msch_req_handle_t *p_req_hdl,
	...
	if (msch_info->next_timeslot &&
		(msch_info->next_timeslot->ts_flags & MSCH_TS_FLAG_DONT_DESTROY)) {
		MSCH_MESSAGE_ERROR(("%s: next timeslot is fixed, reject request\n", __FUNCTION__));
		return BCME_ERROR;
	}
	...
	wlc_msch_req_param_t *param, uint32 update_mask)
{
	...
	MSCH_MESSAGE_DEBUG(("%s: start time %d.%06d\n", __FUNCTION__,
		msch_display_time(((uint64)param->start_time_h << 32) + param->start_time_l)));
	...
	if (update_mask & MSCH_UPDATE_START_TIME) {
		...
		if (param_req_type == MSCH_RT_START_FLEX ||
			...
			MSCH_MESSAGE_ERROR(("wl%d: %s: error update start time"
		...
		*/
		...
		if (cur_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
			if ((uint32)ABS(drift) > (cur_param->interval + cur_param->duration)) {
				MSCH_MESSAGE_ERROR(("wl%d: %s: drift is "
						"more than interval [drift %d]\n",
						msch_info->wlc->pub->unit, __FUNCTION__, drift));
			...
			MSCH_MESSAGE_ERROR(("%s: start time %u is less than current time %u\n",
		...
		while ((elem = msch_list_iterate(elem))) {
			...
			if (cur_param->flags & MSCH_REQ_FLAGS_CHAN_CONTIGUOUS) {
				if (msch_chn_seq_update_entity(msch_info, p_req_hdl, entity)) {
			...
			MSCH_MESSAGE_DEBUG(("%s: idx %d chan 0x%x drift %d New start time %u\n",
				__FUNCTION__, entity->cur_chn_idx, entity->chanspec,
 ...
 */
static void
msch_chan_cont_req_update(wlc_msch_info_t * msch_info, wlc_msch_req_handle_t *req_hdl)
{
	msch_list_elem_t *elem;
	msch_req_entity_t *entity;
	...
	while ((elem = msch_list_iterate(elem))) {
		...
		msch_list_remove(&entity->start_fixed_link);
		if (msch_chn_seq_update_entity(msch_info, req_hdl, entity)) {
			seq_start = entity->pend_slot.end_time -
				((entity->cur_chn_idx + 1) *
				req_hdl->req_param->duration);
		}
		MSCH_MESSAGE_DEBUG(("%s: idx %d chan 0x%x New start time %u\n", __FUNCTION__,
			entity->cur_chn_idx, entity->chanspec,
			(uint32)entity->pend_slot.start_time));
...
*/
static bool
msch_chn_seq_update_entity(wlc_msch_info_t *msch_info, wlc_msch_req_handle_t *req_hdl,
	msch_req_entity_t *entity)
{
	uint64 cur_time = msch_current_time(msch_info);
	...
	prep_time = wlc_msch_switch_time(msch_info, phy_utils_get_chanspec(wlc->pi),
		entity->chan_ctxt->chanspec);
	...
	while (idx <= req_hdl->last_chan_idx) {
		if (entity->chanspec == req_hdl->chn_list[idx]) {
			 ...
			 */
			if (entity->pend_slot.end_time > (cur_ts_end + prep_time)) {
				break;
			}
		}
		idx++;
...
_msch_get_next_start_time(wlc_msch_info_t *msch_info)
{
	...
	if (cur_ts) {
		if (cur_ts->end_time != MSCH_INFINITE_TIME) {
			...
			cur_ts_endtime = msch_curts_slot_endtime(msch_info);
			if (cur_ts_endtime) {
...
wlc_msch_query_timeslot(wlc_msch_info_t *msch_info, uint64 start_time, uint32 duration)
{
	...
	if (!start_time && cur_ts) {
		if (cur_ts->end_time != MSCH_INFINITE_TIME) {
			...
			next_start_time = msch_curts_slot_endtime(msch_info);
		}
		...
		next_start_time += wlc_msch_switch_time(msch_info,
			phy_utils_get_chanspec(msch_info->wlc->pi), msch_info->next_timeslot ?
			msch_info->next_timeslot->chan_ctxt->chanspec : INVCHANSPEC);
		return next_start_time;
	}

	/* TODO not yet implemented */
	return (msch_current_time(msch_info) + wlc_msch_switch_time(msch_info, INVCHANSPEC,
		phy_utils_get_chanspec(msch_info->wlc->pi)));
}

/*
 * Queries the slot availability - returns the end time of current timeslot
 * This function is replacing wlc_msch_query_timeslot, which was incomplete.
 */
uint64
wlc_msch_endtime_timeslot(wlc_msch_info_t *msch_info, chanspec_t to)
{
	msch_timeslot_t *cur_ts = msch_info->cur_msch_timeslot;
	uint64 next_start_time = 0;
	...
	if (cur_ts) {
		if (cur_ts->end_time != MSCH_INFINITE_TIME) {
			next_start_time = cur_ts->end_time;
		} else {
			/* Get cur_ts slot end time */
			next_start_time = msch_curts_slot_endtime(msch_info);
		}
		...
		/* Add time for switching to next channe, if we know next channel */
		next_start_time += wlc_msch_switch_time(msch_info,
			phy_utils_get_chanspec(msch_info->wlc->pi), msch_info->next_timeslot ?
			msch_info->next_timeslot->chan_ctxt->chanspec : to);

		return next_start_time;
	}

	/* Return the time + default channel switch. this is not serving much purpose */
	return (msch_current_time(msch_info) + wlc_msch_switch_time(msch_info,
		phy_utils_get_chanspec(msch_info->wlc->pi), to));
}
...
int32
msch_calc_slot_duration(wlc_msch_info_t *msch_info, wlc_msch_cb_info_t *cb_info)
{
...
static uint32
wlc_msch_switch_time(wlc_msch_info_t *msch_info, chanspec_t from, chanspec_t to)
{
	uint32 switch_time;
	wlc_info_t *wlc = msch_info->wlc;
	uint32 bandswitch_time = msch_info->bandswitch_time;

	if (wlc->bandlocked || IS_SINGLEBAND(wlc)) {
		bandswitch_time = 0;
	}

	if ((from == INVCHANSPEC) || (to == INVCHANSPEC)) {
		return msch_info->channelswitch_time + bandswitch_time;
	}

	/* Same primary channel for chanspecs ? */
	if (WLC_CHAN_COEXIST(from, to)) {
		return 0;
	}

	switch_time = msch_info->channelswitch_time;
	if (CHSPEC_BAND(from) != CHSPEC_BAND(to)) {
		switch_time += bandswitch_time;
	}
		...
		if (req_entity->req_hdl == req_hdl) {
			...
			if (req_entity->cur_slot.timeslot &&
				phy_utils_get_chanspec(wlc->pi) !=
					req_entity->chan_ctxt->chanspec) {

				msch_chan_adopt(msch_info, req_entity->chan_ctxt);
			}
		}
	}
}
