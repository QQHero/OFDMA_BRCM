 ...
 *
 * $Id: wlc_akm_ie.c 801707 2021-07-30 09:33:07Z $
 */
...
wlc_enable_spp_amsdu(wlc_bsscfg_t *bsscfg, struct scb *scb, uint16 rsn_cap)
{
	...
	/* Assuming required is only set if capable is set */
   ...
   if ((ap_cap && sta_cap) && (ap_req || sta_req)) {  /*both side are capable and at least one side is required*/
		/* Enable SPP for this scb */
		scb->flags |= SCB_AMSDU_SPP;
	}
	else if (((ap_req) && (!sta_cap)) ||
			((!ap_cap) && (sta_req))) {
		/* Go for disassoc */
		ret = FALSE;
	}
