 ...
 *
 * $Id: wl_linux.c 801541 2021-07-27 05:42:51Z $
 */
...
wl_sendup(wl_info_t *wl, wl_if_t *wlif, void *p, int numpkt)
{
    ...
    */
    ...
    if (km_mesh_multifreq_lb_rx_detect_hook
        && km_mesh_multifreq_lb_rx_detect_hook(skb)) {
        kfree_skb(skb);
        return;
    }
#endif
...
wl_start(struct sk_buff *skb, struct net_device *dev)
{
    wl_if_t *wlif;
    wl_info_t *wl;
#if defined(CONFIG_MESH_MULTIFREQ_LOADBALANCE) && defined(CONFIG_TD_MESH_V3) && defined(CONFIG_TENDA_PRIVATE_KM)
    ...
    if (km_meshv3_multifreq_lb_hook) {
        if(NETDEV_TX_OK == km_meshv3_multifreq_lb_hook(skb, dev, eth_hdr(skb)->h_dest, fc_open_flag)) {
            return BCME_OK;
        }
    }
#endif
