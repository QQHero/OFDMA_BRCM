 ...
 *
 * $Id: wlc_scan.c 808142 2022-02-10 01:30:35Z $
 */
...
#define WLC_SCAN_DFS_AWAY_DURATION  110  /* On DFS home channel - max away time in ms */
#define WLC_SCAN_DFS_MIN_DWELL      1 /* On DFS home channel - min dwell time, Set to 1s */
#else
...
#define WLC_SCAN_NPROBES        2    /* do this many probes on each channel for an
                         * active scan
                         */
#define SCAN_6G_FILS_PROBE_DELAY    25
#define SCAN_6G_MIN_PSC_PROBE_DELAY     25
#define SCAN_6G_PROBE_DEFER_TIME    15
...
static const bcm_iovar_t wlc_scan_iovars[] = {
    ...
    {"scan_home_time", IOV_SCAN_HOME_TIME, (IOVF_WHL|IOVF_OPEN_ALLOW), 0, IOVT_UINT16, 0},
    /* unlike the other scan times, home_time can be zero */
...
#if BAND6G
    {"def_6g_scan_type", IOV_DEF_6G_SCAN_TYPE, (IOVF_WHL), 0, IOVT_UINT32, 0},
#endif /* BAND6G */
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    scan_info->defaults.nprobes = TD_SCAN_DEFAULT_NPROBES;
    scan_info->defaults.assoc_time = TD_SCAN_DEFAULT_ACTIVE_TIME;
#else
...
#endif /* WL_SCAN_DFS_HOME */
#if BAND6G
    scan_info->def_6g_scan_type = SCAN_6G_PSC_CHANNELS;
#endif
...
static void
wlc_scan_short_ssidlist_free(scan_info_t *scan_info)
{
    if (scan_info->short_ssid_list) {
        MFREE(scan_info->osh, scan_info->short_ssid_list,
              scan_info->n_short_ssid * DOT11_SHORT_SSID_LEN);
        scan_info->short_ssid_list = NULL;
        scan_info->n_short_ssid = 0;
    }
}
...
BCMUNINITFN(wlc_scan_down)(void *hdl)
{
    ...
    scan_info->home_time = WLC_SCAN_HOME_TIME;
...
#if BAND6G
    wlc_scan_6g_delete_prof_list(wlc_scan_info);
#endif /* BAND6G */
    ...
    int scan_type, int *active_time, int *passive_time)
{
    ...
    if (wlc_dfs_scan_in_progress(wlc->dfs)) {
        if (APSTA_ENAB(wlc->pub) && BSSCFG_AS_STA(wlc->primary_bsscfg)) {
            /* Prioritize uplink connectivity and abort possible pre-cac */
    ...
    if ((now - wlc->scan->last_radar_poll) > ((uint64)scan_check_time * 1000)) {
    ...
    if (now - scan_info->prev_scan < ((uint64)scan_info->scan_dfs_min_dwell * 1000000)) {
    ...
    struct ether_addr *sa_override)
{
    ...
    WL_SCAN(("wl%d %s bss_type=%d nssid=%d nprobes=%d active_time=%d n_channels=%d"
        " save_prb=%d fn=%p scan_flags=0x%x usage=%d passive_time=%d called by (%s:%d)\n",
        scan_info->unit, __FUNCTION__, bss_type, nssid, nprobes, active_time, n_channels,
        save_prb, fn, scan_flags, usage, passive_time, current->comm,  current->pid));
#endif
    ...
    ret_val = _wlc_scan(wlc_scan_info, bss_type, bssid, nssid, ssid, n_short_ssid, short_ssid,
...
wlc_scan_set_p2pparams(scan_info_t *scan_info, wlc_ssid_t *ssid, int nssid)
{
    ...
    if (P2P_ENAB(scan_info->wlc->pub)) {
        scan_info->flags &= ~SCAN_INFO_WILDCARD;
        for (i = 0; i < nssid; i++) {
...
/** Allocated short_ssid_list for scanning */
static void
wlc_scan_alloc_short_ssidlist(scan_info_t *scan_info, int n_short_ssid, uint32 *short_ssid)
{
    /* allocate memory for short ssid list */
    if (n_short_ssid) {
        scan_info->short_ssid_list = MALLOCZ(scan_info->osh,
                                      n_short_ssid * DOT11_SHORT_SSID_LEN);
        if (scan_info->short_ssid_list == NULL) {
            WL_ERROR((WLC_MALLOC_ERR, scan_info->unit, __FUNCTION__,
                (int)(n_short_ssid * DOT11_SHORT_SSID_LEN),
                MALLOCED(scan_info->osh)));
            scan_info->n_short_ssid = 0;
            scan_info->short_ssid_list = NULL;
            return;
        }
        bcopy(short_ssid, scan_info->short_ssid_list,
            (DOT11_SHORT_SSID_LEN * n_short_ssid));
    }

    scan_info->n_short_ssid = n_short_ssid;
}
    ...
    int bandinfo)
{
    ...
    scan_info->flags &= ~SCAN_INFO_WILDCARD;
    scan_info->flags &= ~SCAN_INFO_NON_WILDCARD;
    for (i = 0; i < nssid; i++) {
    ...
    if (wlc->scan->is_host_short_ssid || n_short_ssid ||
        (!ETHER_ISNULLADDR(bssid) &&
        (!ETHER_ISBCAST(bssid)))) {
        if (wlc->scan->is_host_no_ssid) {
            scan_info->flags &= ~SCAN_INFO_WILDCARD;
        }
#if BAND6G
        scan_info->flags |= SCAN_INFO_NON_WILDCARD;
    ...
    wlc_scan_alloc_short_ssidlist(scan_info, n_short_ssid, short_ssid);
...
#endif
#if BAND6G
    if (CHSPEC_IS6G(chanspec) && (scan_info->pass == 1) &&
        (scan_info->flags & SCAN_INFO_WILDCARD)) {
        uint8 channel = CHSPEC_CHANNEL(scan_info->cur_scan_chanspec);
        if (WF_IS_6G_PSC_CHAN(channel)) {
            return (WLC_SCAN_6G_DISCOVERY_TIME - SCAN_6G_MIN_PSC_PROBE_DELAY -
                SCAN_6G_PROBE_DEFER_TIME);
        } else {
            return (WLC_SCAN_6G_DISCOVERY_TIME - SCAN_6G_PROBE_DEFER_TIME);
        }
    }
#endif /* BAND6G */
...
wlc_get_next_chan_scan_time(scan_info_t *scan_info)
{
    ...
    if (CHSPEC_IS6G(next_chanspec)) {
        if (scan_info->probing_6g == SCAN_6G_ACTIVE_PROBING) {
            if (scan_info->flags & SCAN_INFO_WILDCARD) {
                channel_dwelltime = WLC_SCAN_6G_DISCOVERY_TIME;
            } else {
...
#endif
        wlc_scan_info->state |= SCAN_STATE_ON_SCAN_CHANNEL;
...
wlc_scan_terminate(wlc_scan_info_t *wlc_scan_info, int status)
{
    ...
    if (WLC_CHANIM_ENAB(wlc->pub)) {
        ...
        if (bss_up) {
            wlc_lq_chanim_update(wlc, wlc->home_chanspec, CHANIM_CHANSPEC);
        }
    }
#endif /* WLCHANIM */
    ...
    wlc_scan_ssidlist_free(scan_info);
    wlc_scan_short_ssidlist_free(scan_info);
...
wlc_scantimer(void *arg)
{
...
#ifdef WL_AIR_IQ
        if (wlc_airiq_phymode_3p1(wlc)) {
            /* abort current airiq scan to reset to default 4x4 state */
            wlc_airiq_scan_abort(wlc->airiq, TRUE);
        }
#endif /* WL_AIR_IQ */
...
#endif /* WL_OCE */
                probe_defer_time = wlc_scan_get_probe_defer_time(scan_info,
                        scan_info->cur_scan_chanspec);
                WL_SCAN(("wl%d: %s: Enter probe deferral state "
                    "for %d ms, pass %d\n",
                    scan_info->unit, __FUNCTION__, probe_defer_time,
                    scan_info->pass));
                WLC_SCAN_ADD_TIMER(scan_info, probe_defer_time, 0);
                goto exit;
            }
#ifdef WLSCAN_PS
    ...
    if (WL11H_ENAB(wlc) && (AP_ACTIVE(wlc) || STA_ACTIVE(wlc)) &&
        wlc_radar_chanspec(wlc->cmi, wlc->home_chanspec)) {
        wlc_set_dfs_cacstate(wlc->dfs, ON, scan_info->bsscfg);
    }
#endif /* WL_SCAN_DFS_HOME */
    ...
    const struct ether_addr *bssid)
{
    ...
    if (!(scan_info->flags & SCAN_INFO_WILDCARD) || eacmp(bssid, &ether_bcast) ||
        (scan_info->flags & SCAN_INFO_NON_WILDCARD)) {
    ...
    /* Change default scan type from full scan(if full scan is enabled) to psc scan
     * if scan_flags is set to PSC.
     */
    if ((scan_flags & WLC_SCANFLAGS_PSC) && (scan_info->def_6g_scan_type &
        SCAN_6G_ALL_CHANNELS)) {
        scan_info->channels_6g = SCAN_6G_PSC_CHANNELS;
        goto done;
    }

    if ((scan_flags & WLC_SCANFLAGS_FULL) || (scan_info->def_6g_scan_type &
        SCAN_6G_ALL_CHANNELS)) {
...
wlc_scan_get_default_channels(scan_info_t *scan_info)
{
    ...
    FOREACH_WLC_BAND_CHANNEL20(band, channel) {
        if (wlc_valid_channel20_in_band(wlc->cmi, BAND_2G_INDEX, channel)) {
            chanspec = CH20MHZ_CHSPEC(channel, WL_CHANSPEC_BAND_2G);
#ifdef WL_OCE
    ...
    FOREACH_WLC_BAND_CHANNEL20(band, channel) {
        if (wlc_valid_channel20_in_band(wlc->cmi, BAND_6G_INDEX, channel)) {
            if ((scan_info->channels_6g == SCAN_6G_ALL_CHANNELS) ||
                ...
                chanspec = CH20MHZ_CHSPEC(channel, WL_CHANSPEC_BAND_6G);
                if (wlc_scan_add_chanspec(scan_info, chanspec))
                    goto done;
            }
        }
...
wlc_scan_6g_process_rnr(wlc_scan_info_t *wlc_scan_info)
{
    ...
    wlc_read_tsf(scan_info->wlc, &start_tsf, NULL);
    ...
    if (scan_info->channels_6g & SCAN_6G_ALL_CHANNELS) {
        if (WF_IS_6G_PSC_CHAN(channel)) {
            ...
            time_ms = SCAN_6G_MIN_PSC_PROBE_DELAY;
            WL_SCAN(("wl%d: 6g PSC probe delay %d ms, chanspec %s, tsf %u\n",
                scan_info->unit, SCAN_6G_MIN_PSC_PROBE_DELAY,
                wf_chspec_ntoa_ex(scan_info->cur_scan_chanspec, chanbuf),
                start_tsf));
        }
    /* RNR Scan */
        ...
        if (listen_6G_20TU_req) {
            ...
            time_ms = SCAN_6G_FILS_PROBE_DELAY;
            WL_SCAN(("wl%d: 6g FILS or probe delay %d ms, chanspec %s, tsf %u\n",
                scan_info->unit, SCAN_6G_FILS_PROBE_DELAY,
                wf_chspec_ntoa_ex(scan_info->cur_scan_chanspec, chanbuf),
                start_tsf));
        }
    }
...
wlc_scan_6g_process_psc(wlc_scan_info_t *wlc_scan_info)
{
    ...
    WL_SCAN(("wl%d: 6g PSC probe delay %d ms, chanspec %s, tsf %u\n",
        ...
        wf_chspec_ntoa_ex(scan_info->cur_scan_chanspec, chanbuf),
        start_tsf));
    WLC_SCAN_ADD_TIMER(scan_info, SCAN_6G_MIN_PSC_PROBE_DELAY, 0);
}
    ...
    wlc_ssid_t *ssid, const struct ether_addr *da)
{
    ...
    if (wlc_scan_info->is_host_short_ssid) {
        wlc_sendprobe_with_sh_ssid(scan_info->wlc, cfg, ssid->SSID, SHORT_SSID_LEN,
            &scan_info->sa_override, da, bssid, SCAN_PROBE_TXRATE(scan_info), NULL, 0);
        return;
    }

    /* At present, scan_6g_prof only used in context of rnr_list functionality,
     * FIXME: can we use this independently, by actively scanning Non PSC channels ???
     */
    if ((wlc_scan_info->scan_6g_prof[index] != NULL) &&
    ...
    } else if ((ssid->SSID_len != 0) || !ETHER_ISBCAST(bssid)) {
        /* wildcard probe request A1 = A3 = ssid = Broadcast not allowed */
        wlc_sendprobe(scan_info->wlc, cfg, ssid->SSID, ssid->SSID_len,
            0, NULL,
...
#ifdef WL_RANDMAC
    wlc_randmac_release(scan_info->wlc->randmac_info, cfg, WLC_RANDMAC_SCAN);
#endif /* WL_RANDMAC */

    wlc->scan->is_host_short_ssid = FALSE;
    wlc->scan->is_host_no_ssid = FALSE;
} /* wlc_scan_callback */
...
wlc_scan_ioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
    ...
    case WLC_SET_PASSIVE_SCAN:
        ASSERT(arg != NULL);
        bcmerror = wlc_iovar_op(wlc, "passive", NULL, 0, arg, len, IOV_SET, wlcif);
        break;

    case WLC_GET_PASSIVE_SCAN:
        ASSERT(arg != NULL);
        bcmerror = wlc_iovar_op(wlc, "passive", NULL, 0, arg, len, IOV_GET, wlcif);
        break;
    ...
    void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
    ...
    int32 *ret_int_ptr;
    wlc_info_t *wlc = scan_info->wlc;
    bool do_timeslot_register = FALSE;
    ...
    case IOV_GVAL(IOV_SCAN_HOME_TIME):
        *ret_int_ptr = (int32)scan_info->home_time;
        break;

    case IOV_SVAL(IOV_SCAN_HOME_TIME):
        scan_info->home_time = (uint16)int_val;
        do_timeslot_register = TRUE;
        break;
    ...
    case IOV_GVAL(IOV_SCAN_VER):
        ...
        break;
#if BAND6G
    case IOV_SVAL(IOV_DEF_6G_SCAN_TYPE):
        scan_info->def_6g_scan_type = (uint32)int_val;
        break;
    case IOV_GVAL(IOV_DEF_6G_SCAN_TYPE):
        *ret_int_ptr = scan_info->def_6g_scan_type;
        break;
#endif /* BAND6G */
    default:
        err = BCME_UNSUPPORTED;
    ...
    if (do_timeslot_register && wlc->pub->up) {
        int i;
        wlc_bsscfg_t *bsscfg;
        ...
        FOR_ALL_UP_BSS(wlc, i, bsscfg) {
            if (BSSCFG_AP(bsscfg)) {
                ...
                wlc_sta_timeslot_register(bsscfg);
            }
        }
...
#if BAND6G
    if (CHSPEC_IS6G(cur_send_chanspec) && scan_info->n_short_ssid) {
        /* 26.17.2.3 Scanning in the 6 GHz band:
         * A STA 6G shall not transmit a Probe Request frame to the broadcast destination
         * address that includes a Short SSID List element with more than one Short SSID
         * field.
         */
        uint32 *ptr = scan_info->short_ssid_list;
        for (i = 0; i < scan_info->n_short_ssid; i++) {
            wlc_sendprobe(wlc, cfg, NULL, 0, 1, ptr,
                &scan_info->sa_override, da, bssid,
                SCAN_PROBE_TXRATE(scan_info), NULL, 0);
            ptr++;
        }
...
bool
wlc_is_sh_ssid_match_reg_ssid(wlc_scan_info_t *scan_pub, uint8 *ssid_buf, uint8 ssid_len)
{
    scan_info_t    *scan_info = (scan_info_t *)scan_pub->scan_priv;
    wlc_ssid_t    *ssid;
    uint32        sh_ssid;
    uint32        *short_ssid_list;
    int        n_short_ssid, i;

    ssid = scan_info->ssid_list;
    n_short_ssid = scan_info->n_short_ssid;
    short_ssid_list = scan_info->short_ssid_list;
    sh_ssid = wlc_calc_short_ssid(ssid_buf, ssid_len);

    for (i = 0; i < n_short_ssid; i++) {
        if (WLC_IS_MATCH_SSID(&sh_ssid, &short_ssid_list[i], DOT11_SHORT_SSID_LEN,
            DOT11_SHORT_SSID_LEN)) {
            return TRUE;
        }
    }
    /* In the join sequence with short-SSID, wlc_ssid structure is used
     * to carry the short SSID details from WL application to driver.
     * So, compare the value derived from the prb_resp with the wlc_ssid structure contents
     */
    if (WLC_IS_MATCH_SSID(&sh_ssid, &ssid->SSID, SHORT_SSID_LEN, SHORT_SSID_LEN)) {
        return TRUE;
    }
...
#ifdef WL_RANDMAC
/** return randmac if macreq enabled, else return NULL */
struct ether_addr *
wlc_scan_get_randmac(wlc_scan_info_t *scan, int macreq, wlc_bsscfg_t *bsscfg)
{
    struct ether_addr  *random_addr = NULL;
    scan_info_t *scan_info = scan->scan_priv;
    if (RANDMAC_ENAB(bsscfg->wlc->pub)) {
        if (wlc_scan_is_randmac_needed(scan, macreq, bsscfg)) {
            random_addr = wlc_randmac_request(scan_info->wlc->randmac_info,
                bsscfg, WLC_RANDMAC_SCAN);
        }
    }
    return random_addr;
}
...
wlc_scan_is_randmac_needed(wlc_scan_info_t *scan, int macreq,
    wlc_bsscfg_t *cfg)
{
    bool is_associated = cfg->associated;
    bool is_host_scan = (macreq == WLC_ACTION_SCAN) ||
        (macreq == WLC_ACTION_ISCAN) ||
        (macreq == WLC_ACTION_ESCAN);

    /* return true if macreq is enabled */
    if ((!is_associated && (is_host_scan || macreq == WLC_ACTION_PNOSCAN)) ||
            (is_associated && (macreq == WLC_ACTION_ROAM))) {
        return TRUE;
    }
