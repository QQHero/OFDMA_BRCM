 ...
 *
 * $Id: wlc_lq.c 797487 2021-04-06 06:33:36Z $
 */
...
static void wlc_lq_chanim_copy_to_acc_us(wlc_info_t *wlc,
        chanim_acc_us_t *acc_us, wlc_chanim_stats_us_t *stats_us);
#endif /* WLCHANIM_US */
...
static void
wlc_lq_chanim_copy_to_acc_us(wlc_info_t *wlc, chanim_acc_us_t *acc_us,
    wlc_chanim_stats_us_t *stats_us)
{
    int i = 0;
    ...
    for (i = 0; i < CCASTATS_MAX; i++) {
        if (i == CCASTATS_TXOP) {
            uint32 ccastats_us_h, ccastats_us_l;
            ccastats_us_h = (uint32)((uint64)stats_us->chanim_stats_us.ccastats_us[i]
                    >> 32) & MASK_32_BITS;
            ccastats_us_l = (uint32)(stats_us->chanim_stats_us.ccastats_us[i]
                    & MASK_32_BITS);
            acc_us->ccastats_us[i] = math_div_64(ccastats_us_h, ccastats_us_l,
                    slottime);
        } else {
...
wlc_lq_chanim_if_info_setup(chanim_info_t *c_info, chanspec_t chanspec)
{
    ...
    if (if_info == NULL) {
        ...
        wlc_lq_chanim_copy_to_acc_us(wlc, &if_info->acc_us, if_info->stats_us);
