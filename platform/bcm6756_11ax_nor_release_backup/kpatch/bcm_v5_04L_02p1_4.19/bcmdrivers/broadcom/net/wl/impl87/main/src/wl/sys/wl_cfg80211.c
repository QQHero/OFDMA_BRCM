 ...
 *
 * $Id: wl_cfg80211.c 810850 2022-04-17 13:20:28Z $
 */
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    TDP_INFO ("In chanspec 0x%x DFS state %d\n", dfs_sub_status->chanspec, dfs_sub_status->state);
#else
    ...
    const wl_event_msg_t *e, void *data)
{
    ...
    if (wl_get_mode_by_netdev(cfg, ndev) == WL_MODE_AP) {
        ...
        if (wl_is_linkup(cfg, e, ndev)) {
            ...
            if ((WLC_E_LINK == event)) {
                if (g_em_mesh_cfg & EM_XMESH_BIT) {
                    if(!TDCORE_FUNC_IS_NULL(td_em_extend_down_notify_linux))
                        TDCORE_FUNC(td_em_extend_down_notify_linux)(EM_XMESH_BIT|EM_LINKUP);
                } else {
                    if(!TDCORE_FUNC_IS_NULL(td_em_extend_down_notify_linux))
                        TDCORE_FUNC(td_em_extend_down_notify_linux)(EM_LINKUP);
                }
            }
#endif
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    TDP_INFO("wl_bss_roaming_done succeeded to " MACDBG " (ch:%d)\n",
