...
int wps_build_xmesh_only_m1(struct wps_device_data *dev, struct wpabuf *msg)
{
	if (dev->xmesh_only == 1) {
		wpa_printf(MSG_ERROR, "WPS:  * Xmesh Only M1");
		wpabuf_put_be16(msg, ATTR_XMESH_ONLY_FLAG);
		wpabuf_put_be16(msg, strlen("tenda_xmesh_only"));
		wpabuf_put_data(msg, "tenda_xmesh_only", strlen("tenda_xmesh_only"));
	}
	return 0;
}
#endif
