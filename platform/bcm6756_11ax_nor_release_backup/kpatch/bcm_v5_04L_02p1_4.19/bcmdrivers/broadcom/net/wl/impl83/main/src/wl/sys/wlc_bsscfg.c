 ...
 *
 * $Id: wlc_bsscfg.c 798835 2021-05-13 08:30:53Z $
 */
...
wlc_bsscfg_up(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
    ...
    if (BSSCFG_AP(cfg)) {
         ...
         * required or not.
         */
#ifndef CONFIG_TENDA_PRIVATE_WLAN
        /* 在sta模式时，不需要做此判断，我们需要在dfs信道保持ap up */
...
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
        maps_vap_available_handler(cfg);
#endif
#ifdef CONFIG_TENDA_PRIVATE_KM
    int bandtype, state = -1; //cfg->up在bsscfg_up函数中状态值不对,采用自定义变量
    if (km_eventscenter_wifiev_intf_status_changed_handler) {
        state = 1;
        if (CHSPEC_IS2G(wlc->chanspec)) {
            bandtype = 0;//2.4G
        } else if (CHSPEC_IS5G(wlc->chanspec)) {
            bandtype = 1;//5G
        } else if (CHSPEC_IS6G(wlc->chanspec)) {
            bandtype = 2;//6G
        }
        km_eventscenter_wifiev_intf_status_changed_handler(bandtype, state, cfg->cur_etheraddr.octet);
    }
#endif
end:
...
wlc_bsscfg_down(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
     ...
     */
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    maps_vap_unavailable_handler(cfg);
#endif
#ifdef CONFIG_TENDA_PRIVATE_KM
    int bandtype, state = -1; //cfg->up在bsscfg_up函数中状态值不对,采用自定义变量
    if (km_eventscenter_wifiev_intf_status_changed_handler) {
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
extern void wlc_deinit_dfs_cfg(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg);
#endif
void
