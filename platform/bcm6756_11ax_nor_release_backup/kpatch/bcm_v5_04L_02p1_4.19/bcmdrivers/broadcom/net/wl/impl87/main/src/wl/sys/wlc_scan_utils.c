 ...
 *
 * $Id: wlc_scan_utils.c 810510 2022-04-08 05:47:36Z $
 */
...
#endif
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    {"scan_state", IOV_SCAN_STATE, (0), 0, IOVT_INT32, 0},
#endif
    {NULL, 0, 0, 0, 0, 0}
};
    ...
    actcb_fn_t act_cb, void *act_cb_arg)
{
    ...
    if ((err = wlc_mac_request_entry(wlc, NULL, macreq, scan_flags)) == BCME_OK) {
        ...
        if (!BSS_P2P_DISC_ENAB(wlc, cfg)) {
            /* override source MAC */
            sa_override = wlc_scan_get_randmac(wlc->scan, macreq, cfg);
        }
#endif /* WL_RANDMAC */

        err = wlc_scan(wlc->scan, bss_type, bssid, nssid, ssids,
            n_short_ssid, short_ssids, scan_type, nprobes, active_time, passive_time,
...
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN) && !defined(CONFIG_UGW_BCM_DONGLE)
        td_em_ap_updatechannelscanresults(cfg);
#else
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
        if(!TDCORE_FUNC_IS_NULL(td_em_ap_updatechannelscanresults)) {
            TDCORE_FUNC(td_em_ap_updatechannelscanresults)(cfg);
        }
#endif
#endif
        for (i = 0; i < wlc->scan_results->count; i++) {
...
#ifdef CONFIG_TENDA_SCAN_PRIORITY
        /* 触发扫描获取扫描信息后，将AS_SCAN_ABORT状态恢复成正常AS_IDLE状态 */
        if (AS_SCAN_ABORT == wlc_assoc_get_as_state(wlc->primary_bsscfg)) {
            wlc_assoc_change_state(wlc->primary_bsscfg, AS_IDLE);
        }
#endif/* CONFIG_TENDA_SCAN_PRIORITY */
    ...
    chanspec_t chanspec_start, int macreq, wlc_bsscfg_t *cfg)
{
    ...
    /* For APs, avoid scan when there is a CSA in progress */
    if (BSSCFG_AP(cfg) && BSSCFG_IS_CSA_IN_PROGRESS(cfg)) {
    ...
    if (arg_len >= WL_SCAN_PARAMS_FIXED_SIZE) {
        ...
        if (params.ssid_type & WL_SCAN_SSIDFLAGS_SHORT_SSID) {
        ...
        if (params.scan_type_2 & (WLC_SCANFLAGS_FULL >> 17)) {
            scan_flags |= WLC_SCANFLAGS_FULL;
        }
#endif /* BAND6G */
        ...
        n_short_ssid = (uint)(params.n_short_ssid);
        ...
        /* locate and appended short ssids */
        if (n_short_ssid) {
            uint offset = chanspec_num * sizeof(uint16);
            uint32 *ptr = NULL;
            offset = ROUNDUP(offset, sizeof(uint32));
            offset += nssid * sizeof(wlc_ssid_t);
            if ((uint)arg_len < (offset + n_short_ssid * DOT11_SHORT_SSID_LEN)) {
                bcmerror = BCME_BUFTOOSHORT;
                goto done;
            }
            short_ssids = (uint32 *)((char*)chanspec_list + offset);
            ASSERT(ISALIGNED(short_ssids, sizeof(uint32)));
            ptr = short_ssids;
...
wlc_scan_utils_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
    ...
    /*
        执行默认的AP扫描命令时，nprobes和active_time都为-1，SDK将其
        配置为默认值nprobes=2,active_time=20. 在此参数下，2.4G AP扫描
        有较大的几率扫不到AP。因此将2.4G主动扫描的nprobes设置为3,
        active_time设置为90，即扫描每个信道时，执行3次probe，每个信道
        扫描持续时间为90ms，即每次probe持续时间为90/3=30ms.一次probe
        就是发送probe request, 然后等待probe response.
    */
    if (len >= WL_SCAN_PARAMS_FIXED_SIZE) {
        wl_scan_params_t *params = (wl_scan_params_t *)arg;

        if ((WLC_BAND_2G == wlc->band->bandtype) &&
                (DOT11_SCANTYPE_ACTIVE == params->scan_type) &&
                (DOT11_BSSTYPE_ANY == params->bss_type) &&
                (CHANNELS_ALL_AVAILABLE == params->channel_num) &&
                (WLC_SCAN_PARAMS_NPROBES_DEFAULT == params->nprobes) &&
                (WLC_SCAN_PARAMS_ACTIVE_TIME_DEFAULT == params->active_time)) {
            /* 将2.4G AP扫描的probe次数、扫描时间增大，提高扫到AP的几率 */
            params->nprobes = TD_SCAN_DEFAULT_NPROBES;
            params->active_time = TD_SCAN_DEFAULT_ACTIVE_TIME;
        }

        if ((WLC_BAND_5G == wlc->band->bandtype) &&
                (DOT11_SCANTYPE_ACTIVE == params->scan_type) &&
                (DOT11_BSSTYPE_ANY == params->bss_type) &&
                (WLC_SCAN_PARAMS_NPROBES_DEFAULT == params->nprobes) &&
                (WLC_SCAN_PARAMS_ACTIVE_TIME_DEFAULT == params->active_time)) {
...
#ifdef CONFIG_TENDA_SCAN_PRIORITY
        chanspec_num = (uint)(params->channel_num & WL_SCAN_PARAMS_COUNT_MASK);
        no_ssid = (uint)((params->channel_num >> WL_SCAN_PARAMS_NSSID_SHIFT) &
                           WL_SCAN_PARAMS_COUNT_MASK);
        if (CHANNELS_ALL_AVAILABLE == chanspec_num && SCAN_NO_SSID == no_ssid) {
            /* 取消正在进行的其他扫描动作，本次页面扫描优先 */
            if (SCAN_IN_PROGRESS(wlc->scan)) {
                wlc_scan_abort(wlc->scan, WLC_E_STATUS_ABORT);
            }
            /* 驱动默认优先级为(ASSOC > SCAN > ROAM > RM)，上级断电后，驱动会不断进行ASSOC相
            关的SCAN，导致无法进行进行页面扫描。页面扫描动作与驱动关联扫描动作目的相同，所以
            页面扫描优先不会影响正常关联过程 */
            if (AS_IDLE != wlc_assoc_get_as_state(wlc->primary_bsscfg)) {
                wlc_assoc_abort(wlc->primary_bsscfg);
            }
            /* 将AS_IDLE状态改为AS_SCAN_ABORT状态，触发页面扫描时将禁止其它关联相关的扫描 */
            wlc_assoc_change_state(wlc->primary_bsscfg, AS_SCAN_ABORT);
        }
#endif /* CONFIG_TENDA_SCAN_PRIORITY */
...
#ifdef CONFIG_TENDA_SCAN_PRIORITY
                if (AS_SCAN_ABORT == wlc_assoc_get_as_state(wlc->primary_bsscfg)) {
                    WL_ERROR(("ESCAN request while in assoc progress, abort current scan\n"));
                    err = BCME_BUSY;
                    break;
                }
#endif/* CONFIG_TENDA_SCAN_PRIORITY */
...
#ifdef CONFIG_TENDA_SCAN_PRIORITY
                if (AS_SCAN_ABORT == wlc_assoc_get_as_state(wlc->primary_bsscfg)) {
                    WL_ERROR(("ISCAN request while in assoc progress, abort current scan\n"));
                    err = BCME_BUSY;
                    break;
                }
#endif/* CONFIG_TENDA_SCAN_PRIORITY */
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    case IOV_GVAL(IOV_SCAN_STATE): {
        int *state = (int *)arg;

        *state = sui->wlc->scan->in_progress;
        break;
    }
#endif
...
/** Called back by the IE parse module when a frame with a TIM IE is received  */
static int
wlc_scan_parse_tim_ie(void *ctx, wlc_iem_parse_data_t *data)
{
    wlc_iem_ft_pparm_t *ftpparm = data->pparm->ft;
    wlc_bss_info_t *bi = ftpparm->scan.result;

    bi->dtim_period = 1;    // default
    if (data->ie != NULL && data->ie[TLV_LEN_OFF] >= DOT11_MNG_TIM_FIXED_LEN) {
        bcm_tlv_t *tim_ie = (bcm_tlv_t *)data->ie;
        bi->dtim_period = tim_ie->data[DOT11_MNG_TIM_DTIM_PERIOD];
    } else {
        WL_INFORM(("Missing TIM IE\n"));
    }
...
BCMATTACHFN(wlc_scan_register_iem_fns)(wlc_info_t *wlc)
{
    ...
    if ((err = wlc_iem_add_parse_fn_mft(iemi, scanfstbmp, DOT11_MNG_TIM_ID,
            wlc_scan_parse_tim_ie, wlc)) != BCME_OK) {
        WL_ERROR(("wl%d: %s wlc_iem_add_parse_fn failed, err %d, tim in scan\n",
            wlc->pub->unit, __FUNCTION__, err));
        goto fail;
    }
...
wlc_recv_scan_parse_handle_prb_resp_escan(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_bss_info_t *bi,
    struct dot11_management_header *hdr_original, uint8 *body, uint body_len)
{
    wl_escan_result_t *escan_result;
    ...
    /* During escan, while parsing the Non-Transmit profile in the beacon,
     * the source mac address is getting changed by wlc_multibssid_update_bi_to_nontransmitted
     * in the original header before sending the WLC_E_ESCAN_RESULT event and not reverting
     * to the original source address before this function return.
     * Since the caller function uses the source mac address in header to pick the scb info,
     * Use a header copy for local parsing to avoid ASSERTs in other functions
     * due to mis matched scb and source mac address
     */
    bcopy((char*)hdr_original, (char*)&hdr, sizeof(struct dot11_management_header));
    ...
    do { /* go through all multiple BSSID IEs */
        ...
        do {
            /* if there is only partial profile in This MBSSID IE and
             * No other full profile. Skip and move to
             * the next MBSSID IE
             */
            if (prof_total_cnt <= (current_cnt - 1)) {
                break;
            }
#endif /* WL_MBSSID */
        ...
        if (MULTIBSSID_ENAB(wlc->pub) &&
            ...
            if (wlc_multibssid_valid_mbssidie(mbssid_ie)) {
                dot11_mbssid_cap_t *next_mbssid_cap_ie = NULL;
                uint8 *next_profile_add = (uint8 *)mbssid_ie +
                        MULTIBSSID_IDX_IE_LEN3 + TLV_HDR_LEN;
                uint8 next_profile_len = *((uint8 *)mbssid_ie +
                        MULTIBSSID_IDX_IE_LEN3 + 1u);
                next_mbssid_cap_ie = (dot11_mbssid_cap_t *)
                            bcm_parse_tlvs(next_profile_add,
                            next_profile_len,
                            DOT11_MNG_NONTRANS_BSSID_CAP_ID);
                current_cnt = 1;
                if (next_mbssid_cap_ie == NULL) {
                    /* Non-Transmit BSS profile is splitted
                     * so pick the next profile since the split
                     * profile is handled in last
                     * pass of the loop
                     */
                    current_cnt = 2;
                }
                /* If there is only partial profile in MBSSID
                 * (no other complete profile) and move to next mbssid
                 */
                if (wlc_multibssid_caltotal(cfg->wlc, cfg, mbssid_ie) >
                        current_cnt -1) {
                    wlc_multibssid_update_bi_to_nontransmitted(wlc, cfg, bi,
                        &bi_original, &hdr, current_cnt - 1, mbssid_ie);
                }
            } else {
        ...
        if (BSS) {
...
#endif
            if (bcn_prb && ((scan->state & SCAN_STATE_SAVE_PRB) ||
#if defined(STA) && defined(GSCAN)
            send_to_host ||
#endif
#ifdef NLO
            ...
            if (wlc_recv_scan_parse_update_existing_bss(wlc, cfg,
                ...
                if (MULTIBSSID_ENAB(wlc->pub) &&
                    bi.max_mbssid_indicator && prof_total_cnt) {
                    int retval;
                    /* update bcn frame body according to this
                     * nontransmitted BSSID profile
                     */
                    if (BSS->bcn_prb) {
                        retval = wlc_multibssid_recv_preprocess_bcn(wlc,
                            FALSE, hdr, (uint8 *)BSS->bcn_prb,
                            BSS->bcn_prb_len, mbssid_ie, current_cnt,
                            BSS->BSSID.octet[ETHER_ADDR_LEN - 1]);
                        if (retval < 0) {
            ...
            if (MULTIBSSID_ENAB(wlc->pub) &&
                ...
                if (mbssid_ie) {
                    if (wlc_multibssid_valid_mbssidie(mbssid_ie)) {
                        ...
                        total_prof_cnt = wlc_multibssid_caltotal(cfg->wlc,
                                cfg, mbssid_ie);
                        /* If there is only partial profile in MBSSID
                         * no other complete profile ,Then
                         * move to next mbssid
                         */
                        if (total_prof_cnt > (current_cnt -1)) {
                            (void)memcpy(&bi, &bi_original,
                                    sizeof(wlc_bss_info_t));
    ...
    struct dot11_management_header *hdr, uint8 *body, int body_len)
{
    ...
    if (!(scan->state & SCAN_STATE_ON_SCAN_CHANNEL)) {
        WL_SCAN(("wl%d: SCAN: recv_scan ignore pkts until STATE_ON_SCAN_CHANNEL(not on home channel)\n",
                wlc->pub->unit));
        return BCME_OK;
    }
    ...
    do { /* go through all multiple BSSID IEs */
        ...
        do {
            ...
            if (((scan->bss_type == DOT11_BSSTYPE_ANY ||
                ...
                (ssid && wlc_is_sh_ssid_match_reg_ssid(scan, ssid, ssid_len)) ||
#ifdef NLO
                  (cfg && cfg->nlo) ||
#endif /* NLO */
                  FALSE) &&
...
#endif /* WL_MBSSID */
                if (cfg) {
                    return wlc_recv_scan_parse_handle_prb_resp(wlc, cfg, wrxh,
            ...
            if (MULTIBSSID_ENAB(wlc->pub) && mbssid_ie) {
                if (prof_total_cnt > current_cnt) {
                    ...
                    if (!ssid_ie) {
                        WL_SCAN(("wl%d: did not find ssid ie in nontransmit"
                            "bssid profile maybe due to the "
                            "possibility of profile split\n",
                            wlc->pub->unit));
                    }
                    current_cnt++;
                } else {
        ...
        if (MULTIBSSID_ENAB(wlc->pub) && mbssid_ie) {
            ...
            if (wlc_multibssid_valid_mbssidie(mbssid_ie)) {
                ...
                /* In case of nontransmit bssid profile fragmentation across
                 * multiple mbssid ie's, there can be a nontrasmit bssid profile
                 * without having ssid ie within the mbssid ie.
                 */
                if (!ssid_ie) {
                    WL_SCAN(("wl%d: Could not find ssid ie in nontrans "
                        "bssid profile \n", wlc->pub->unit));
                    *ssid = 0;
                    ssid_len = 0;
                }
                    ...
                    uint ssid_len, bool add)
{
    ...
    for (indx = 0; indx < sui->iscan_ignore_count; indx++) {
        if (!bcmp(sui->iscan_ignore_list[indx].bssid, bssid, ETHER_ADDR_LEN) &&
            (sui->iscan_ignore_list[indx].ssid_sum == ssid_sum) &&
            (sui->iscan_ignore_list[indx].ssid_len == ssid_len) &&
            (CHSPEC_BAND(sui->iscan_ignore_list[indx].chanspec) == band)) {
            return indx;
        }
    }

    if (add) {
        wlc_BSSIgnoreAdd(wlc, bssid, chanspec, ssid, ssid_len);
                    ...
                    uint ssid_len)
{
    ...
    bcopy(&new_entry, &sui->iscan_ignore_list[sui->iscan_ignore_count], sizeof(iscan_ignore_t));
    sui->iscan_ignore_count++;
    ...
    uint8 *body, uint body_len, wlc_bss_info_t *bi)
{
    ...
    tim = bcm_find_tlv(body, body_len, DOT11_MNG_TIM_ID);
    if (tim != NULL && tim->len >= DOT11_MNG_TIM_FIXED_LEN) {
        bi->dtim_period = tim->data[DOT11_MNG_TIM_DTIM_PERIOD];
    }
...
wlc_scan_utils_shubscan_request(wlc_info_t *wlc)
{
    ...
    ret = wlc_scan_request(wlc, DOT11_BSSTYPE_ANY, &ether_bcast,
            1, &ssid, 0, NULL, DOT11_SCANTYPE_ACTIVE, -1, -1, -1, -1, NULL,
