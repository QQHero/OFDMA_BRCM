 ...
 *
 * $Id: wlc_probresp.c 800993 2021-07-12 18:25:17Z $
 */
    ...
    uint8 *body, int body_len)
{
    ...
    /* Determines whether the signal is below the threshold during the scan */
    ...
    //±éÀú221×Ö¶Î£¬²¢½âÎötdË½ÓÐie×Ö¶Î
    for(ie = bcm_parse_tlvs(body, body_len, DOT11_MNG_VS_ID);ie != NULL; (ie = bcm_next_tlv(ie, &ie_body_len))) {
        if((DOT11_MNG_VS_ID == ie->id)) {
            td_recv_ie((td_element_t *)ie, cfg, mprobresp->wlc, hdr->sa.octet, TD_PE_FRAME_TYPE_PROBE_REQ, wrxh->rssi);
        }
    }
