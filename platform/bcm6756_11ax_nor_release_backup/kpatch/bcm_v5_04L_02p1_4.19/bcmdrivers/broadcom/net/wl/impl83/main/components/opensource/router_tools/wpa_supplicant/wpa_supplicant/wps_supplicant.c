                    ...
                    struct wpa_ssid *new_ssid)
{
    ...
    for (ssid = wpa_s->conf->ssid, next = ssid ? ssid->next : NULL; ssid;
        ...
        wpa_config_remove_network(wpa_s->conf, ssid->id);
                    ...
                    /* use for wps client cfg save to td flash hongguiyang 202008 start*/
#define WL_INF_LEN 2
#define CMD_LEN 64
#define CMD_CFM_WAIT_USEC  10000

void td_wps_write_cfg_to_flash(const struct wps_credential *cred, char *name)
{
    char cmd[CMD_LEN] = {0};
    unsigned char index = 0;
    
    if (!cred || !name) {
        printf("wps write flash fail param null\n");
        return;
    }

    /* bcm  sta mode wl1~16 can use, don't consider wl1.1 ...*/
    if (strlen(name) > WL_INF_LEN) {
        index = atoi(name+WL_INF_LEN);
    }
    
    /* ssid */
    snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_ssid %s", index, cred->ssid);
    system(cmd);
    usleep(CMD_CFM_WAIT_USEC);

    /* sercury ("none", "wep", "wpapsk", "wpa", "8021x", "wpa3sae"),wps support none and wpapsk */
    memset(cmd, 0, sizeof(cmd));
    if (WPS_AUTH_OPEN == cred->auth_type) {
        snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_security none", index);    
    } else {
        snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_security wpapsk", index);      
    }
    system(cmd);
    usleep(CMD_CFM_WAIT_USEC);
    ...
    /* key */
    memset(cmd, 0, sizeof(cmd));
    snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_wpapsk_key %s", index, cred->key);
    system(cmd);
    usleep(CMD_CFM_WAIT_USEC);

    /* crypto ("aes", "tkip", "tkip+aes")*/
    memset(cmd, 0, sizeof(cmd));
    if ((WPS_ENCR_AES | WPS_ENCR_TKIP) == cred->encr_type) {
        snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_wpapsk_crypto tkip+aes", index);
    } else if (WPS_ENCR_TKIP == cred->encr_type) {
        snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_wpapsk_crypto tkip", index);
    } else {
        snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_wpapsk_crypto aes", index);        
    }
    system(cmd);
    usleep(CMD_CFM_WAIT_USEC);

    /* wpapsk_type ("psk", "psk2", "psk+psk2") */
    memset(cmd, 0, sizeof(cmd));
    if (WPS_AUTH_WPA2PSK == cred->auth_type) {
        snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_wpapsk_type psk2", index);
    } else if (WPS_AUTH_WPAPSK == cred->auth_type) {
        snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_wpapsk_type psk", index);
    } else {
        snprintf(cmd, sizeof(cmd)-1, "cfm set wlan%d.x_extend_wpapsk_type psk+psk2", index);
    }
    system(cmd);
    usleep(CMD_CFM_WAIT_USEC);
    return;
                   ...
                   const struct wps_credential *cred)
{
    ...
    /* m8 need to write cfg to flash  hongguiyang202008 */
    (void)td_wps_write_cfg_to_flash(cred, wpa_s->ifname);
...
static void wpa_supplicant_wps_event_overlap(struct wpa_supplicant *wpa_s)
{
    wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_OVERLAP;
}
static void wpa_supplicant_wps_event_timeout(struct wpa_supplicant *wpa_s)
{
    wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_TIMEOUT;
}
static void wpa_supplicant_wps_event_active(struct wpa_supplicant *wpa_s)
{
    wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_ACTIVE;
}
static void wpa_supplicant_wps_event_disable(struct wpa_supplicant *wpa_s)
{
                      ...
                      struct wps_event_fail *fail)
{
    ...
    if (WPS_PBC_STATUS_ACTIVE == wpa_s->wps_stats.pbc_status) {
        wpa_s->wps_stats.pbc_status = WPS_PBC_STATUS_DISABLE;
    }
                     ...
                     union wps_event_data *data)
{
    ...
    case WPS_EV_PBC_OVERLAP:
        wpa_supplicant_wps_event_overlap(wpa_s);
        break;
    case WPS_EV_PBC_TIMEOUT:
        wpa_supplicant_wps_event_timeout(wpa_s);
        break;
    case WPS_EV_PBC_ACTIVE:
        wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_ACTIVE);
        wpa_supplicant_wps_event_active(wpa_s);
        break;
    case WPS_EV_PBC_DISABLE:
        wpa_msg(wpa_s, MSG_INFO, WPS_EVENT_DISABLE);
        wpa_supplicant_wps_event_disable(wpa_s);
        break;
...
int wpas_wps_init(struct wpa_supplicant *wpa_s)
{
    ...
    wps->dev.xmesh_only = wpa_s->conf->map_hbss_sta;
#endif
