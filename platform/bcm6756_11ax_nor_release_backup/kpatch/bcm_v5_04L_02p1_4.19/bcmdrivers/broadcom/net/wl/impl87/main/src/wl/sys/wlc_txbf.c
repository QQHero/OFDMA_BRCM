 ...
 *
 * $Id: wlc_txbf.c 808146 2022-02-10 04:14:23Z $
 */
    ...
    */
    ...
    if (HE_ENAB_BAND(pub, wlc->band->bandtype)) {
        ...
        if (my_bfe_capable & TXBF_HE_MU_BFE_CAP) {
            hecap |= (1 << C_BFECAPHE_MUEN_NBIT);
        }

        if (my_bfe_capable & TXBF_HE_SU_BFE_CAP) {
            hecap |= (1 << C_BFECAPHE_SUEN_NBIT);
        }
    }
...
BCMATTACHFN(wlc_txbf_init)(wlc_txbf_info_t *txbf)
{
    ...
    if (D11REV_GE(corerev, 40)) {
        if (WLC_BITSCNT(wlc->stf->hw_txchain) > 1) {
            ...
            if (D11REV_GE(corerev, 129) && D11REV_LT(corerev, 133)) {
...
#ifdef WL_PSMX
/* Function to check if a particular HE client is qualified for mu sounding */
static bool
wlc_txbf_hemu_sounding_link_qualify(wlc_txbf_info_t *txbf, scb_t *scb)
{
    if ((!HE_DLOMU_ENAB(txbf->pub) && !HE_DLMMU_ENAB(txbf->pub)) ||
...
bool
wlc_disable_bfe_for_smth(wlc_txbf_info_t *txbf, wlc_bsscfg_t *cfg, bool ishe)
{
    bool disable_bfe;
    wlc_info_t *wlc = txbf->wlc;
     ...
     */
    if (BSSCFG_AP(cfg) && wlc_phy_is_smth_en(WLC_PI(wlc), ishe) &&
    ...
    // no seperated config of smth_per_nss_SUTxbf for vht and he
    if (disable_bfe == FALSE &&
        ((cfg->bfe_capable & TXBF_SU_BFE_CAP) > 0 ||
        (cfg->bfe_capable & TXBF_HE_SU_BFE_CAP) > 0)) {
...
wlc_txbf_bfe_init(wlc_txbf_info_t *txbf, txbf_scb_info_t *bfi)
{
    ...
    wlc_write_shm(wlc, shm_addr(bfi_blk, C_BFI_BFE_MYAID_POS), SCB_BSSCFG(scb)->AID);
    ...
    */
    if (SCB_ASSOCIATED(scb)) {
        ...
        bssid = (ETHER_ISNULLADDR(&SCB_BSSCFG(scb)->BSSID) ?
            &SCB_BSSCFG(scb)->target_bss->BSSID : &SCB_BSSCFG(scb)->BSSID);
...
wlc_txbf_bfr_init_ge128(wlc_txbf_info_t *txbf, txbf_scb_info_t *bfi)
{
    ...
    if (PSMX_ENAB(pub)) {
        ...
        if ((bfi->flags & MU_BFE) ||
            ((bfi->flags & HE_MU_BFE) && HE_DLMMU_ENAB(wlc->pub))) {
            /* update rate capabilities for this mu user */
            wlc_svmp_update_ratecap(wlc, scb, bfi->amt_index, bfe_nrx);
...
wlc_txbf_bfr_init(wlc_txbf_info_t *txbf, txbf_scb_info_t *bfi)
{
    ...
    if (PSMX_ENAB(wlc->pub) && (bfi->flags & MU_BFE)) {
        int bss_mac5 = (SCB_BSSCFG(scb)->cur_etheraddr.octet[5]) & 0x3f;
        wlc_bmac_suspend_macx_and_wait(wlc->hw);
        bfi_blk = txbf->mx_bfiblk_base +
            bfi->mx_bfiblk_idx * BFI_BLK_SIZE(wlc->pub->corerev);
        wlc_txbf_invalidate_bfridx(txbf, bfi, bfi->mx_bfiblk_idx);
    ...
    uint8 nss, uint8 rate, uint8 bw, bool is_ldpc, bool is_imp)
{
    ...
    2, 6, 8, 10, 10, 10, 10, 10, 10, 10, 10, 10,
    2, 6, 7, 7, 7, 7, 7, 7, 5, 5, 5, 5};
    /* add 1qdb for mcs3~11 gain_4x2 to favor txbf to
     * consider the worse power penalty under 2ss_TXBF2
     */
    uint8 gain_4x2[24] = {0, 6, 16, 19, 19, 19, 19, 19, 19, 19, 19, 19,
    2, 6, 8, 12, 12, 12, 12, 12, 12, 12, 12, 12};
    ...
    txpwr204080_t* txpwrs)
{
    ...
    if (txbf->mode == TXBF_ON) {
        ...
        if (nsts > MIN(WLC_BITSCNT(wlc->stf->txchain), (bfe_sts_cap + 1))) {
            return FALSE;
        }
        if (D11REV_GE(wlc->pub->corerev, 128)) {
            *shm_index = 0;
...
wlc_txbf_imp_txstatus(wlc_txbf_info_t *txbf, scb_t *scb, tx_status_t *txs)
{
    ...
    if (TX_STATUS_MACTXS_S5(txs) & TX_STATUS40_IMPBF_LOW_MASK)
        return;

    bfi->imp_used ++;
    if (TX_STATUS_MACTXS_S5(txs) & TX_STATUS40_IMPBF_BAD_MASK) {
        bfi->imp_bad ++;
    }
    ...
    for (i = 0; i < txbf->mu_max_links; i++) {
        ...
        if (bfi->vht_cap & VHT_CAP_INFO_MU_BEAMFMR) {
            bcm_bprintf(b, "\tmu bfr cap, bfe link %s\n",
                (SCB_BSSCFG(scb)->bfe_capable ? "ON" : "OFF"));
        }
...
wlc_txbf_info_dump(wlc_txbf_info_t *txbf, bcmstrbuf_t *b)
{
    ...
    for (i = 0; i < TXBF_MUTXV_MAX; i++) {
        ...
        if (bfi->bfr_capable) {
            ...
            bcm_bprintf(b, "    bfr cap 0x%x, myaddr as bfe %s\n",
                bfi->bfr_capable,
                (SCB_BSSCFG(scb)->bfe_capable & bfi->bfr_capable) ? "ON" : "OFF");
        }
    ...
    for (i = 0; i < entries; i++) {
        ...
        if ((flags & TXBF_DUMP_FLAG_MU) && (i < TXBF_MUTXV_MAX)) {
    ...
    for (i = 0; i <= txbf->max_link; i++) {
        ...
        if (bfi->vht_cap & VHT_CAP_INFO_SU_BEAMFMR)
            bcm_bprintf(b, "\tsu bfr cap, bfe link %s\n",
                (SCB_BSSCFG(scb)->bfe_capable && (wlc_vht_get_cap_info(wlc->vhti) &
...
uint32 td_get_wlc_txbf(wlc_txbf_info_t *txbf)
{
    ...
    void *p, uint plen, void *a, uint alen, uint vsize, wlc_if_t *wlcif)
{
    ...
    case IOV_SVAL(IOV_TXBF_MUTIMER):
        ...
        if (!PSMX_ENAB(pub)) {
            WL_ERROR(("wl%d: PSMx not supported. Can't enable txbf_mutimer\n",
                pub->unit));
            return BCME_UNSUPPORTED;
        }
        if (D11REV_GE(wlc->pub->corerev, 128)) {
...
wlc_txbf_vht_upd_bfr_bfe_cap(wlc_txbf_info_t *txbf, wlc_bsscfg_t *cfg, uint32 *cap)
{
    ...
    if ((wlc->txbf->virtif_disable && !BSSCFG_IS_PRIMARY(cfg) && !BSSCFG_PSTA(cfg)) ||
        ...
        //wlc_disable_bfe_for_smth(..., FALSE) meanse checking for VHT
        bfe = wlc_disable_bfe_for_smth(txbf, cfg, FALSE) ? 0: cfg->bfe_capable;
    }
...
wlc_txbf_tbcap_update(wlc_txbf_info_t *txbf, scb_t *scb)
{
    ...
    bfi = (txbf_scb_info_t *)TXBF_SCB_INFO(txbf, scb);
    if (!bfi || !(HE_DLOMU_ENAB(txbf->pub) || HE_DLMMU_ENAB(txbf->pub)) ||
        !SCB_HE_CAP(scb)) {
        return;
    }
