 ...
 *
 * $Id: wlc_rspec.c 807958 2022-02-05 00:58:55Z $
 */
...
typedef struct {
	...
	uint32	vht[WL_RATESET_SZ_VHT_MCS_P][WL_TX_CHAINS_MAX];	/**< VHT counts */
...
#ifdef GAME_SPEEDUP
int
wlc_set_special_ratespec_override(wlc_info_t *wlc, ratespec_t wl_rspec)
{
   ratespec_t rspec;
   int err;
   err = wlc_wlrspec2ratespec(wlc->primary_bsscfg, wl_rspec, &rspec);
   if (!err) {
       wlc->special_fixrate = rspec;
       if (WLC_TXC_ENAB(wlc))
