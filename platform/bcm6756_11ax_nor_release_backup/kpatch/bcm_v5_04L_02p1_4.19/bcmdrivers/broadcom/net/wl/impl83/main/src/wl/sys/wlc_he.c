 ...
 *
 * $Id: wlc_he.c 801047 2021-07-13 10:30:30Z $
 */
...
#ifdef WLAMSDU_TX
#ifdef WL_EAP_PER_VAP_CONFIG
    if (SCB_BSSCFG(scb) && SCB_BSSCFG(scb)->flag_vap_amsdu_override) {
        amsdu_enabled = AMSDU_BSSCFG_ENAB(wlc->pub);
    }
#endif /* WL_EAP_PER_VAP_CONFIG */
    if (amsdu_enabled) {
...
wlc_he_configure_bsscolor_phy(wlc_he_info_t *hei, wlc_bsscfg_t *cfg)
{
	...
	if (cfg == NULL) {
		...
		if (BSSCFG_AP(cfg)) {
			...
			bsscolor.color = hei->bsscolor;
			bsscolor.index = HE_BSSCOLOR_APIDX;
			wlc_write_shm(wlc, M_MY_BSSCOLOR(wlc->hw), bsscolor.color);
		} else {
			bhei = BSS_HE_INFO(hei, cfg);
			/* Disable BSSColor filtering ? */
