...
static easymesh_timer_t g_hb_timer;
#endif
...
static void td_em_hbtimer_handler(unsigned long data);
#else
...
td_em_device_type_e td_em_get_media_type(em_osif osif)
{
    ...
    if (osif->current_bss->flags3 & WLC_BSS3_HE) {
        if (WL_CHANSPEC_BAND_2G == band) {
            return TD_EM_DEVICE_TYPE_2G_11AX;
        } else if (WL_CHANSPEC_BAND_5G == band) {
            return TD_EM_DEVICE_TYPE_5G_11AX;
        } else if (WL_CHANSPEC_BAND_6G == band) {
            return TD_EM_DEVICE_TYPE_6G_11AX;
        } else {
    ...
    FOREACH_STA(wlc, i, osif_iter) {
        if ((osif_iter->map_attr & MAP_EXT_ATTR_BACKHAUL_STA) 
            ...
            if (TD_EM_CTROL_DISCONNECT == status) {
                ...
                TD_EM_DBG_MSG("TD_EM_CTROL_DISCONNECT \n");
                if (g_em_mesh_cfg & EM_XMESH_BIT) {
                    TD_EM_DBG_MSG("em_xmesh_bit \n");
                    /* link down=0x2 */
...
int td_em_set_xmesh_hb_drv(em_osif osif, unsigned char *send_buf)
{
    ...
    wlc = osif->wlc;
    TD_EM_DBG_MSG("in status = %d!\n", status);

    td_em_set_bbss_status(wlc, status);
    td_em_action_bsta_status(wlc, status);
  
    /*This prevents the upper-layer 
    layer from sending heartbeat packets without delivering the role configuration */
    if (g_hb_timer.hb_timer) {
        wl_del_timer(g_hb_timer.wl, g_hb_timer.hb_timer);
...
static void td_em_init_hbtimer(em_osif osif)
{
    ...
    g_hb_timer.hb_timer = wl_init_timer(osif->wlc->wl, td_em_hbtimer_handler, osif->wlc, "hbtimer");
    g_hb_timer.wl = osif->wlc->wl;
    wl_add_timer(osif->wlc->wl, g_hb_timer.hb_timer, TD_EM_HB_TIMEOUT * 1000, FALSE);
