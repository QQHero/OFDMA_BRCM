 ...
 *
 * $Id: dhd_linux.c 809751 2022-03-23 03:27:54Z $
 */
...
#ifdef CONFIG_TENDA_PRIVATE_KM
    if (IS_SKBUFF_PTR(skb) && km_driver_wireless_tx_packet_hook(skb, net)) {
        return BCME_OK;
    }
#endif
...
socram:
    ...
    /* check if wserver is running */
    monitor_task = _get_task_info("wserver");
#else
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    /* send SIGWINCH so that wserver */
    send_sig_info(SIGWINCH, (void *)1L, monitor_task);
#else
