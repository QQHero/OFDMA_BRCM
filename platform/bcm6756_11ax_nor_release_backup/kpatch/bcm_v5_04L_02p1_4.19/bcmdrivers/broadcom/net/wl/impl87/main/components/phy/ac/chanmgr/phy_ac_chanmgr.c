 ...
 *
 * $Id: phy_ac_chanmgr.c 810649 2022-04-12 05:26:24Z $
 */
...
struct phy_ac_chanmgr_info {
    ...
    */
    ...
    int origEdCrsEn;
    int origEdCrsEn80u;
    int orig_Core3LpfGain;
    int orig_Core3OvrGain;
...
static uint8 phy_ac_get_subband_indx(phy_info_t *pi);
...
BCMATTACHFN(phy_ac_chanmgr_std_params_attach)(phy_ac_chanmgr_info_t *chanmgri)
{
    ...
    BCM_REFERENCE(pi);
    chanmgri->data->curr_band = CHSPEC_BANDTYPE(chanmgri->pi->radio_chanspec);
    chanmgri->data->curr_subband_idx = 255;
...
static const uint32 clip2_tbl_maj131[] =
    {0xa984322, 0xa986444, 0xa987555, 0xaa98777, 0xaa99888, 0xaaa9988, 0xaaaa999, 0xaaaa999};
...
static const uint8 avvmid_set130[18][5][8] = {
    {
    ...
    {
        {5, 5, 5, 5, 80, 80, 80, 80},
        {2, 2, 2, 2, 155, 161, 153, 155},
        {2, 2, 2, 2, 156, 163, 152, 157},
        {2, 2, 2, 2, 154, 162, 157, 158},   /* pdet_id = 10, 2G = MCM2SD */
        {2, 2, 2, 2, 158, 164, 159, 158}},  /*               6G = MCM6 Qorvo QPF4656 ES3 */
    {
        {3, 3, 3, 3, 120, 120, 120, 120},
        {1, 1, 1, 1, 155, 155, 155, 155},
        {1, 1, 1, 1, 155, 155, 155, 155},
        {1, 1, 1, 1, 155, 155, 155, 155},   /* pdet_id = 11, 2G = MCM2_R Richwave FEM */
        {1, 1, 1, 1, 155, 155, 155, 155}},  /*               5G = MCM5DTW  */
    {
        {3, 3, 3, 3, 155, 155, 155, 155},
        {1, 1, 1, 1, 189, 185, 200, 196},
        {1, 1, 1, 1, 190, 184, 200, 195},
        {1, 1, 1, 1, 198, 192, 201, 200},   /* pdet_id = 12, 2G = X2MCH2_QB Qorvo QPF7219 */
        {1, 1, 1, 1, 195, 191, 199, 193}},  /*               5G = MCH5DTW SKY DPD FEM ES5 */
    {
        {3, 3, 3, 3, 120, 120, 120, 120},
        {4, 4, 4, 4,  85,  85,  82,  86},
        {4, 4, 4, 4,  84,  87,  84,  86},
        {4, 4, 4, 4,  87,  86,  85,  87},   /* pdet_id = 13, 2G = empty */
        {4, 4, 4, 4,  87,  87,  87,  91}},  /*               5G = MCM5_S, SKY85755 */
    {
        {3, 3, 3, 3, 120, 120, 120, 120},
        {2, 2, 2, 2, 141, 143, 140, 141},
        {2, 2, 2, 2, 143, 144, 141, 143},
        {2, 2, 2, 2, 144, 146, 143, 146},   /* pdet_id = 14, 2G = empty */
        {2, 2, 2, 2, 146, 147, 145, 145}},  /*               5G = MCM5_Q, QPF4526 */
    {
        {3, 3, 3, 3, 120, 120, 120, 120},
        {4, 4, 4, 4,  50,  44,  49,  50},
        {4, 4, 4, 4,  48,  44,  54,  53},
        {4, 4, 4, 4,  52,  51,  52,  52},   /* pdet_id = 15, 2G = empty */
        {4, 4, 4, 4,  56,  50,  58,  56}},  /*               5G = MCM5_R, RTC7676E */
    {
        {3, 3, 3, 3, 120, 120, 120, 120},
        {3, 3, 3, 3, 136, 142, 135, 139},
        {3, 3, 3, 3, 138, 144, 137, 141},
        {3, 3, 3, 3, 142, 146, 143, 143},   /* pdet_id = 16, 2G = empty */
        {3, 3, 3, 3, 131, 146, 142, 142}},  /*               5G = XMCH5_Q, QPF4516B */
    {
        {3, 3, 3, 3, 120, 120, 120, 120},
        {3, 3, 3, 3, 115, 115, 115, 115},
        {3, 3, 3, 3, 115, 115, 115, 115},
        {3, 3, 3, 3, 115, 115, 115, 115},   /* pdet_id = 17, 2G = empty */
        {3, 3, 3, 3, 115, 115, 115, 115}},  /*               5G = MCH6D, SKY85788-11 ES4 */
...
static const wl_radar_thr2_t wlc_phy_radar_thresh2_axphy_atlas_2Gp5G = {
...
static const wl_radar_thr2_t wlc_phy_radar_thresh2_axphy_6715B0_2Gp5G = {
    WL_RADAR_THR_VERSION,
    0x6c4, 0x20, 0x6c4, 0x20, 0x6b8, 0x20, 0x6c0, 0x20, 0x6c0, 0x20, 0x6c4, 0x20,
    0x6b4, 0x20, 0x6b8, 0x20, 0xa, 0xa, 0xa, 0x258, 0x258, 0x258, 0x258, 0x5124, 0xc,
    0xe, 0x1941, 0x19, 0x0, 0x14, 0x64, 0xf0, 0xfffb, 0x5
};
    ...
    if (ACMAJORREV_47_129_130(pi->pubpi->phy_rev)) {
        if (pi->u.pi_acphy->chanmgri->papr_gamma_iovar != -1) {
            ...
            if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
                ...
                /* 6715: Used for 160M only. */
                // 11ag 6~24mbps
                lowMcsGamma = 900;
                // Disabled for other 11ag rates
                highMcsGamma_c8_c11 = 4095;
                // Enabled for non-11ag rates due to spurs on non-linear FEM
                highMcsGamma = PHY_PAPDEN(pi) ? 2048 : 4095;
            }
        }
    } else {
    ...
    if (enable) {
        ...
        if (ACMAJORREV_33(pi->pubpi->phy_rev) ||
            ...
            for (i = 4; i <= 31; i++) {
                /* Program enable/gainidx */
                if (i <= 11) {
                    /* Enable for 11ag 6~24mbps */
                    gamma = (i <= 8) ? 0x80 : 0x0;
                } else {
                    /* For other rates, enable only for non-linear FEM */
                    gamma = PHY_PAPDEN(pi) ? 0x80 : 0x0;
                }
                wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_PAPR, 1, i,
                        32, &gamma);
            }
            ...
            /* program GammaOffset fields */
            // gammaOffsetComp is to avoid gamma overflow for subband
            gammaOffsetComp = 4095 - (gamma & 0xfff);
            for (i = 0; i <= 3; i++) {
                // Essentially to disable subband PAPRR. Enble it for 160mhz
                uint16 gammaOffset = ((i < 3) ? gammaOffsetComp : 0);
                gamma = (gammaOffset << 13) | gammaOffset;
                j = 32 + i;
                wlc_phy_table_write_acphy(pi, ACPHY_TBL_ID_PAPR, 1, (0x40+j),
                        32, &gamma);
            }
...
wlc_phy_set_reg_on_reset_majorrev32_33_37_47_51_129_130_131(phy_info_t *pi)
{
    ...
    */
    ...
    if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
        ...
        MOD_PHYREG(pi, tx_shaper_common12, txshaper_he_tb_bypass, 0x1);
#ifdef WLC_TXSHAPER
        phy_ac_txss_txshaper_bypass_sync(pi->u.pi_acphy->txssi);
#endif /* WLC_TXSHAPER */
    ...
    */
    ...
    if (ACMAJORREV_47(pi->pubpi->phy_rev)) {
        ...
        */
        ...
        MOD_PHYREG(pi, bfdsConfig, config_enableWiderNDP, 1);
    } else if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
        /* Enable Wider NDP feature */
        MOD_PHYREG(pi, bfdsConfig, enableWiderNDP, 1);
    ...
    if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
        ...
        */
        ...
        if (ACMAJORREV_129(pi->pubpi->phy_rev)) {
            ...
            FOREACH_CORE(pi, core) {
                uint32 tbl = ACPHY_TBL_ID_MCLPAGCCLIP2TBL0 + 32 * core;
                sz = (uint8) ARRAYSIZE(clip2_tbl_maj131);
                wlc_phy_table_write_acphy(pi, tbl, sz, 0x0, 32, clip2_tbl_maj131);
            }
        } else if (ACMAJORREV_51(pi->pubpi->phy_rev)) {
            FOREACH_CORE(pi, core) {
                uint32 tbl = ACPHY_TBL_ID_MCLPAGCCLIP2TBL0 + 32 * core;
...
wlc_phy_set_regtbl_on_bw_change_acphy(phy_info_t *pi)
{
    ...
    uint32 NvAdjAXVal;
    uint8 lpf_txbq1[] = {0, 1, 2, 3}, lpf_txbq2[] = {4, 5, 6, 7};
...
#ifndef BCMQT
#ifdef WLVASIP
    if (!wlc_phy_is_scan_chan_acphy(pi) && ACMAJORREV_130(pi->pubpi->phy_rev)) {
    ...
    } else if (wlapi_get_vasip_fw_loaded(pi->sh->physhim)) {
        ruAllocParams = (pi->radio_chanspec & WL_CHANSPEC_BW_MASK) >>
            WL_CHANSPEC_BW_SHIFT;
        ruAllocParams -= (WL_CHANSPEC_BW_20 >> WL_CHANSPEC_BW_SHIFT);
    ...
    if (ACMAJORREV_129(pi->pubpi->phy_rev) && PHY_IPA(pi) &&
            CHSPEC_IS20(pi->radio_chanspec) &&
            ((BOARDFLAGS(GENERIC_PHY_INFO(pi)->boardflags)) & BFL_BTCOEX)) {
        WRITE_PHYREG(pi, TxRealFrameDelay, 0xd0);
...
wlc_phy_farrow_setup_nontiny(phy_info_t * pi)
{
    ...
    if (ACMAJORREV_128(pi->pubpi->phy_rev)) {
        ...
        wlc_phy_farrow_setup_28nm(pi, 1 /* DAC rate mode */ );
        /* Configure AFE div */
        wlc_phy_radio20710_afe_div_ratio(pi, use_ovr, 0, FALSE);
    } else {
...
chanspec_setup_regtbl_on_chan_change(phy_info_t *pi)
{
    ...
    if (ACMAJORREV_GE47(pi->pubpi->phy_rev)) {
        /* Need to call rfseqext on chan change since 6G/5G rfseqext is channel dependent */
        wlc_phy_set_rfseqext_tbl(pi, pi->u.pi_acphy->sromi->srom_low_adc_rate_en);
    }
...
wlc_phy_set_reg_on_bw_change_acphy_majorrev130(phy_info_t *pi)
{
    ...
    if (chspec_is160) {
        FOREACH_CORE(pi, core) {
            MOD_PHYREGCE(pi, STRPwrThreshL, core, strPwrThresh, 13);
            MOD_PHYREGCE(pi, STRPwrThreshU, core, strPwrThresh, 13);
            MOD_PHYREGCE(pi, STRPwrThreshLSub1, core, strPwrThresh, 13);
            MOD_PHYREGCE(pi, STRPwrThreshUSub1, core, strPwrThresh, 13);
            MOD_PHYREGCE(pi, STRPwrThresh_4_, core, strPwrThresh, 13);
            MOD_PHYREGCE(pi, STRPwrThresh_5_, core, strPwrThresh, 13);
            MOD_PHYREGCE(pi, STRPwrThresh_6_, core, strPwrThresh, 13);
            MOD_PHYREGCE(pi, STRPwrThresh_7_, core, strPwrThresh, 13);
        }
    } else if (chspec_is80) {
        FOREACH_CORE(pi, core) {
            MOD_PHYREGCE(pi, STRPwrThreshL, core, strPwrThresh, 26);
            MOD_PHYREGCE(pi, STRPwrThreshU, core, strPwrThresh, 26);
            MOD_PHYREGCE(pi, STRPwrThreshLSub1, core, strPwrThresh, 26);
            MOD_PHYREGCE(pi, STRPwrThreshUSub1, core, strPwrThresh, 26);
            MOD_PHYREGCE(pi, STRPwrThresh_4_, core, strPwrThresh, 26);
            MOD_PHYREGCE(pi, STRPwrThresh_5_, core, strPwrThresh, 26);
            MOD_PHYREGCE(pi, STRPwrThresh_6_, core, strPwrThresh, 26);
            MOD_PHYREGCE(pi, STRPwrThresh_7_, core, strPwrThresh, 26);
        }
    } else {
        FOREACH_CORE(pi, core) {
            MOD_PHYREGCE(pi, STRPwrThreshL, core, strPwrThresh, 52);
            MOD_PHYREGCE(pi, STRPwrThreshU, core, strPwrThresh, 52);
            MOD_PHYREGCE(pi, STRPwrThreshLSub1, core, strPwrThresh, 52);
            MOD_PHYREGCE(pi, STRPwrThreshUSub1, core, strPwrThresh, 52);
            MOD_PHYREGCE(pi, STRPwrThresh_4_, core, strPwrThresh, 52);
            MOD_PHYREGCE(pi, STRPwrThresh_5_, core, strPwrThresh, 52);
            MOD_PHYREGCE(pi, STRPwrThresh_6_, core, strPwrThresh, 52);
            MOD_PHYREGCE(pi, STRPwrThresh_7_, core, strPwrThresh, 52);
        }
    }

    // FFT input shift
    MOD_PHYREG(pi, fft_backoff_cont_0, fft_enable_input_shift_threshold_vht,
        chspec_is80 ? 0x2 : chspec_is160 ? 0x4 : 0x0);
...
BCMATTACHFN(phy_ac_chanmgr_nvram_attach)(phy_ac_chanmgr_info_t *ac_info)
{
    ...
    if ((PHY_GETVAR_SLICE(pi, ed_thresh2g)) != NULL) {
        pi_ac->sromi->ed_thresh2g = (int8)PHY_GETINTVAR_SLICE(pi, ed_thresh2g);
    } else {
        pi_ac->sromi->ed_thresh2g = 0;
    }

    if ((PHY_GETVAR_SLICE(pi, ed_thresh5g)) != NULL) {
        pi_ac->sromi->ed_thresh5g = (int8)PHY_GETINTVAR_SLICE(pi, ed_thresh5g);
    } else {
    ...
    if (ACMAJORREV_GE47(pi->pubpi->phy_rev) && !ACMAJORREV_128(pi->pubpi->phy_rev)) {
        ...
        pi_ac->sromi->ed_thresh_default = -45;     /*Avoid sta connection failure when the interference is large*/
    } else {
...
wlc_phy_tx_farrow_setup_28nm(phy_info_t *pi, uint16 dac_rate_mode, bool wbcal)
{
    ...
    uint8 p_afediv_lut_6GL_rev130[] = {240, 120, 72, 40};
    uint8 p_afediv_lut_6GU_rev130[] = {240, 120, 72, 48};
    uint16 p_afediv_lut_mode2_rev130[] = {128, 64, 32, 16, 288, 144, 64, 32, 320, 160, 80, 32};
#endif // REGULATORY_DEBUG
    ...
    if (dac_rate_mode == 2) {
        if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
            if (wbcal) { // Used by wbcal 20M and 40M
                if (ri->wbcal_afediv_ratio != -1) {
                    uint16 nda = (ri->wbcal_afediv_ratio >> 8) & 0x3F;
                    p_afediv_lut_mode2_rev130[0] = 16*nda; // 2G 20M
                    p_afediv_lut_mode2_rev130[1] = 8*nda;  // 2G 40M
                    p_afediv_lut_mode2_rev130[4] = 32*nda; // 5G 20M
                    p_afediv_lut_mode2_rev130[5] = 16*nda; // 5G 40M
                    p_afediv_lut_mode2_rev130[8] = 32*nda; // 6G 20M
                    p_afediv_lut_mode2_rev130[9] = 16*nda; // 6G 40M
                } else {
                    if (fc >= 6665) { // 6G high
                        p_afediv_lut_mode2_rev130[8] = 384; // 6G 20M
                        p_afediv_lut_mode2_rev130[9] = 192; // 6G 40M
                    } else if (fc >= 6115) { // 6G mid
                        p_afediv_lut_mode2_rev130[8] = 352; // 6G 20M
                        p_afediv_lut_mode2_rev130[9] = 176; // 6G 40M
                    } else if (fc > 5320) { // 5G mid/high, 6G low
                        p_afediv_lut_mode2_rev130[4] = 320; // 5G 20M
                        p_afediv_lut_mode2_rev130[5] = 160; // 5G 40M
                        p_afediv_lut_mode2_rev130[8] = 320; // 6G 20M
                        p_afediv_lut_mode2_rev130[9] = 160; // 6G 40M
                    }
                }
            }
            dac_div_idx = CHSPEC_IS2G(pi->radio_chanspec) ? bw_idx :
                (CHSPEC_IS5G(pi->radio_chanspec) ? (bw_idx + 4) : (bw_idx + 8));
            dac_div_ratio = p_afediv_lut_mode2_rev130[dac_div_idx];
        } else if (ACMAJORREV_131(pi->pubpi->phy_rev) && wbcal) {
            if (fc >= 5905) {
                dac_div_ratio = 96;
            } else {
                dac_div_ratio = CHSPEC_IS2G(pi->radio_chanspec) ? 128 : 72;
            }
        ...
        if (ACMAJORREV_47_51_129_131(pi->pubpi->phy_rev)) {
            if (ACMAJORREV_131(pi->pubpi->phy_rev) && wbcal && (dac_rate_mode == 1)) {
                if (CHSPEC_IS160(pi->radio_chanspec)) {
                    dac_div_ratio = 32;
                } else {
                    if (fc >= 5905) {
            ...
            if (wbcal) { // Used by wbcal 80M and 160M
                if (ri->wbcal_afediv_ratio != -1) {
                    uint16 nda = (ri->wbcal_afediv_ratio >> 8) & 0x3F;
                    p_afediv_lut_rev130[6] = 8*nda;  // 5G & 6G 80M
                    p_afediv_lut_rev130[7] = 8*nda;  // 5G & 6G 160M
                } else {
                    if (fc >= 6665) { // 6G high
                        p_afediv_lut_rev130[6] = 96; // 80M
                        p_afediv_lut_rev130[7] = 48; // 160M
                    } else if (fc >= 6115) { // 6G mid
                        p_afediv_lut_rev130[6] = 88; // 80M
                        p_afediv_lut_rev130[7] = 40; // 160M
                    } else if (fc > 5320) { // 5G mid/high, 6G low
                        p_afediv_lut_rev130[6] = 80; // 80M
                        p_afediv_lut_rev130[7] = 40; // 160M
                    } else { // 5G low
                        p_afediv_lut_rev130[6] = 72; // 80M
                        p_afediv_lut_rev130[7] = 40; // 160M
                    }
                }
                dac_div_idx = CHSPEC_IS2G(pi->radio_chanspec) ? bw_idx : bw_idx + 4;
...
wlc_phy_rx_farrow_setup_28nm(phy_info_t *pi, chanspec_t chanspec_sc, uint8 sc_mode)
{
    ...
    if ((ACMAJORREV_130(pi->pubpi->phy_rev)) && (PHY_COREMASK_SISO(stf_shdata->phyrxchain))) {
        ...
        p_afediv_lut_6GL[0] = 184;
        p_afediv_lut_6GU[0] = 216;
        p_afediv_lut_6GUU[0] = 216;
    }
...
static uint8
phy_ac_get_subband_indx(phy_info_t *pi)
{
    uint8 sb = 0;
    uint16 channel;
...
{
    phy_info_acphy_t *pi_ac = pi->u.pi_acphy;
    uint8 sb = phy_ac_get_subband_indx(pi);
    ...
    /* SUB-BAND CHANGED ? */
    if (ACMAJORREV_47_130(pi->pubpi->phy_rev)) {
        if (sb != pi_ac->chanmgri->data->curr_subband_idx) {
            pi_ac->chanmgri->data->curr_subband_idx = sb;
            mboolset(pi_ac->CCTrace, CALLED_ON_SUBBAND_CHG);
        }
    }

    /* BAND CHANGED ? */
    if (CCT_INIT(pi_ac) ||
        (pi_ac->chanmgri->data->curr_band != CHSPEC_BANDTYPE(pi->radio_chanspec))) {
    ...
    if (CHSPEC_IS2G(pi->radio_chanspec))
        ...
        if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
            if (CHSPEC_IS5G(pi->radio_chanspec)) {
                MOD_PHYREG(pi, ChannelControl, currentChannel, 0);
            } else {
                MOD_PHYREG(pi, ChannelControl, currentChannel, 6);
            }
        }
    }
...
chanspec_tune_txpath(phy_info_t *pi)
{
    ...
    bzero((uint8 *)tx_pwr_idx_store, sizeof(tx_pwr_idx_store));
...
phy_ac_chanmgr_chanspec_set(phy_type_chanmgr_ctx_t *ctx, chanspec_t chanspec)
{
    ...
    /* HW Knoise: If home channel measurements are pending, cancel them so results are not
     *  reported for the wrong channel. Exclude scan measurements (STATE_EXTERNAL) as
     *  timing is short, and results may have a callback in progress.
     *  SW Knoise behavior, below, is only to clear the noise-state, which is compatible
     *  with HW Knoise.
     */
    if ((pi->phynoise_state & (PHY_NOISE_STATE_MON | PHY_NOISE_STATE_CRSMINCAL)) &&
        !wlc_phy_is_scan_chan_acphy(pi)) {
        phy_info_acphy_t *pi_ac = (phy_info_acphy_t *)pi->u.pi_acphy;
        PHY_NONE(("wl%d:%s cs:0x%04x/chn:%u noise:0x%x%s%s%s scanning %d reset HW Knoise\n",
            pi->sh->unit, __FUNCTION__, (int)chanspec, CHSPEC_CHANNEL(chanspec),
            pi->phynoise_state,
            ...
            SCAN_INPROG_PHY(pi)));
        /* Reset KW Knoise. Safe to call if HW Knoise is not supported or disabled */
        phy_ac_hwknoise_engine(pi_ac->noisei, KNA_RESET);
    }
    ...
    case  PHYMODE_BGDFS:
        if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
            ...
            if (CHSPEC_ISPHY5G6G(chanspec) && CHSPEC_ISPHY5G6G(chanspec_sc) &&
                ...
                /* update radar threshold */
                if (!st->rparams.thrs2_iov_update)
                    st->rparams.radar_thrs2 =
                        wlc_phy_radar_thresh2_axphy_atlas_2Gp5G;
            ...
            if (CHSPEC_IS2G(chanspec) && CHSPEC_ISPHY5G6G(chanspec_sc)) {
                if (BCM6715X2_PKG(pi->sh->sih->otpflag)) { /* 6715X2 */
                    /* gpio13=1 to enable HW input blanking when phymode 31 */
                    si_gpioout(pi->sh->sih, gpio13_mask, (1 << 13),
                        GPIO_DRV_PRIORITY);
                    si_gpioouten(pi->sh->sih, gpio13_mask, gpio13_mask,
                        GPIO_DRV_PRIORITY);
                    si_gpiocontrol(pi->sh->sih, gpio13_mask, 0,
                        GPIO_DRV_PRIORITY);
                } else { /* 6715MCH2 */
                    /* enable gpio15 HW input blanking via chip control */
            ...
            if (is6715mch2) {
                ...
                if (!st->rparams.thrs2_iov_update)
                    st->rparams.radar_thrs2 =
                        wlc_phy_radar_thresh2_axphy_6715B0_2Gp5G;
                ...
                st->rparams.radar_args.max_span_lp = CHSPEC_IS20(chanspec_sc) ?
                    ((chanmgri->orig_max_span_lp & 0xf0ff) | 0x0300) :
                    chanmgri->orig_max_span_lp;
                st->rparams.radar_args.st_level_time = CHSPEC_IS20(chanspec_sc) ?
                    ((chanmgri->orig_st_level_time & 0xfff) | 0x7000) :
                    chanmgri->orig_st_level_time;
                /* override rxgain on core3 to improve FCC-5 radar fm estimation */
                chanmgri->orig_Core3LpfGain =
                    READ_PHYREGCE(pi, RfctrlCoreLpfGain, 3);
                chanmgri->orig_Core3OvrGain =
                    READ_PHYREGFLD(pi, RfctrlOverrideGains3, lpf_bq1_gain);
                WRITE_PHYREGCE(pi, RfctrlCoreLpfGain, 3,
                    ((chanmgri->orig_Core3LpfGain & 0xf8) | 0x3));
                MOD_PHYREGCE(pi, RfctrlOverrideGains, 3, lpf_bq1_gain, 1);
            }
            ...
            WRITE_PHYREG(pi, ed_crsEn, chanmgri->origEdCrsEn & 0xfff);
            WRITE_PHYREG(pi, ed_crsEn_80u, chanmgri->origEdCrsEn80u & 0xfff);
            /* enable bundleDacDiodePwrupEn to avoid FW trap when TX test in STA mode */
            MOD_PHYREG(pi, Logen_AfeDiv_reset_select, bundleDacDiodePwrupEn, 1);
        } else {
    ...
    case 0:
        if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
            ...
            if (CHSPEC_IS2G(chanspec) && CHSPEC_ISPHY5G6G(chanspec_sc)) {
                if (BCM6715X2_PKG(pi->sh->sih->otpflag)) { /* 6715X2 */
                    /* gpio13=0 to disable HW input blanking when phymode 0 */
                    si_gpioout(pi->sh->sih, gpio13_mask, (0 << 13),
                        GPIO_DRV_PRIORITY);
                    si_gpioouten(pi->sh->sih, gpio13_mask, gpio13_mask,
                        GPIO_DRV_PRIORITY);
                    si_gpiocontrol(pi->sh->sih, gpio13_mask, 0,
                    ...
                    /* disable gpio15 HW input blanking via chip control */
                    pmu_chipctrl1 = si_pmu_chipcontrol(pi->sh->sih, 1, 0, 0);
                    si_pmu_chipcontrol(pi->sh->sih, 1, 0xFFFFFFFF,
            ...
            if (is6715mch2) {
                ...
                if (chanmgri->orig_st_level_time != 0 && CHSPEC_IS20(chanspec_sc))
                    st->rparams.radar_args.st_level_time =
                        chanmgri->orig_st_level_time;
                /* restore rxgain on core3 */
                WRITE_PHYREGCE(pi, RfctrlCoreLpfGain, 3,
                    chanmgri->orig_Core3LpfGain);
                MOD_PHYREGCE(pi, RfctrlOverrideGains, 3, lpf_bq1_gain,
                    chanmgri->orig_Core3OvrGain);
            }
            ...
            if (chanmgri->origEdCrsEn80u != 0)
                WRITE_PHYREG(pi, ed_crsEn_80u, chanmgri->origEdCrsEn80u);
            /* disable bundleDacDiodePwrupEn to restore the setting when phymode 0 */
            MOD_PHYREG(pi, Logen_AfeDiv_reset_select, bundleDacDiodePwrupEn, 0);
        } else {
            PHY_ERROR(("wl%d: %s: Unsupported PHY revision for BGDFS\n",
                     pi->sh->unit, __FUNCTION__));
        ...
        case  PHYMODE_BGDFS:
            if (ACMAJORREV_32(pi->pubpi->phy_rev) ||
                ...
                if (CHSPEC_IS2G(chanspec) && CHSPEC_ISPHY5G6G(chanspec_sc)) {
                    phy_utils_write_phyreg_p1c(pi,
                        ACPHY_RadarBlankCtrl2(pi->pubpi->phy_rev), 0x8000);
                    /* No ExternalBlank on 6715X2 */
                    if (!BCM6715X2_PKG(pi->sh->sih->otpflag)) {
                        MOD_PHYREG_p1c(pi, RadarSearchCtrl,
                            ExternalBlankEn, 1);
                    }
                } else {
                    phy_utils_write_phyreg_p1c(pi,
