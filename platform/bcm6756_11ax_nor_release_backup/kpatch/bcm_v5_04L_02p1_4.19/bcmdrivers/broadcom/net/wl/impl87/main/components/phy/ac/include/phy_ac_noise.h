 ...
 *
 * $Id: phy_ac_noise.h 810501 2022-04-08 03:34:50Z $
 */
...
#define ACPHY_ENABLE_FCBS_HWACI(pi) \
    (ACMAJORREV_4((pi)->pubpi->phy_rev) || ACMAJORREV_GE40(pi->pubpi->phy_rev))
...
#define ACPHY_MCLIP_ACI_MITIGATION(pi) (ACMAJORREV_51_131((pi)->pubpi->phy_rev) || \
...
typedef struct phy_ac_noise_data {
    ...
    bool    knoise_en; /* 0 = knoise measurement is disabled */
    uint16 initgaincodeA_bk[PHY_CORE_MAX];
    uint16 rfseq_bk[PHY_CORE_MAX];
} phy_ac_noise_data_t;
...
int32 phy_ac_noise_get_dynamic_ed_thresh(phy_info_t *pi, bool mode, uint8 *txop);
#else
...
/* knoise enable API */
extern int phy_ac_noise_knoise_enable(phy_info_t *pi, bool set_val, uint8 mode);
extern bool phy_ac_noise_get_knoise_en(phy_info_t *pi);
extern uint8 phy_ac_noise_get_knoise_mode(phy_info_t *pi);
extern void phy_ac_knoise_iov(phy_info_t *pi, int32 set_val);
/* hw-knoise "actions" for hw-knoise engine API */
typedef enum {
    KNA_ATTACH,
    KNA_UNATTACH,
    KNA_SUSPEND,
...
/* hw-knoise "retun codes" from hw-knoise engine API */
typedef enum {
    KNE_OK,
    KNE_ALREADY_RUNNING,
    KNE_IN_PROGRESS,
...
/* single entry point hw-knoise engine to take and report measurements */
eknoise_eng_code phy_ac_hwknoise_engine(phy_ac_noise_info_t *noisei, eknoise_eng_action action);

#if defined(BCMDBG) || defined(WLTEST)
/* hw-knoise IOVAR handlers overriding default filtering parameters */
