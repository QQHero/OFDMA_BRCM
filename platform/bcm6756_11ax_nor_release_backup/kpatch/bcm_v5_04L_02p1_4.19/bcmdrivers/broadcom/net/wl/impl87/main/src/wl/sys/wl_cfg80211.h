...
#define WL_DPP_AF_ARRAY_SIZE    8
#define WL_DPP_AF_OFF_CHAN_DWELL_TIME    10000 /* ms */
#define WL_DPP_AF_RETRY_DWELL_TIME    50
...
#define MIN_FREQ_5G 5180 /* Min 5G frequency value */
#define MAX_FREQ_5G 5885 /* Max 5G frequency value */
#define MIN_FREQ_6G 5935 /* Min 6G frequency value */
#define MAX_FREQ_6G 7125 /* Max 6G frequency value */
#define MAX_FREQ_2G 2484 /* Max 2G frequency value */
#define WL_INVBAND 0xFFFFu /* Invalid band */
...
#if defined(CONFIG_BCM_KF_NL80211_6G_BAND_SUPPORT) || LINUX_VERSION_CODE >= \
    KERNEL_VERSION(5, 4, 0)
#define KERNEL_SUPPORTS_6GHZ
#endif
...
struct parsed_ies {
    ...
    u32 osen_ie_len;
    const bcm_tlv_t *rsnxe_ie;
    u32 rsnxe_ie_len;
    const bcm_tlv_t *ext_sup_rates_ie;
    u32 ext_sup_rates_ie_len;
};
...
struct virtual_if_info {
    ...
    struct mutex reg_lock;        /* for netdev registration synchronization */
    bool registered;
};
#endif
...
#else
#define BCM_CFG80211_MAGIC    (0x14e4eb1a)
#endif
...
struct bcm_cfg80211 {
    ...
    bool registered;        /* indicates associated wiphy instance is registered */
    ...
    chanspec_t af_sent_chspec;    /**< 20 mhz chanspec on which action frame was sent */
...
extern s32 wl_cfg80211_post_ifdel(struct net_device *ndev);
...
#define wl_cfg_is_oce_ie(ie, tlvs, len)    wl_cfg_has_ie(ie, tlvs, len, \
...
int wl_cfg80211_register_wiphy(struct net_device *primary_ndev);
...
int wl_net_os_enable_packet_filter(struct net_device *dev, int val);
int wl_net_os_rxfilter_add_remove(struct net_device *dev, int val, int num);
#endif /* !BCMDONGLEHOST */
