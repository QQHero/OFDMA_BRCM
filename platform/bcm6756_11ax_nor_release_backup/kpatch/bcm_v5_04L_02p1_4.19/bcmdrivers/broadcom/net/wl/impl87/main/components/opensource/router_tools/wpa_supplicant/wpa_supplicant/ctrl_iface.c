...
static int wpa_ctrl_iface_wps_get_status(struct wpa_supplicant *wpa_s,
                         char *buf, size_t buflen)
{
    int ret;
    ...
    ret = os_snprintf(pos, end - pos, "Last WPS result: %s\n",
              (WPS_STATUS_SUCCESS == wpa_s->wps_stats.status ?
               "Success":
               (WPS_STATUS_FAILURE == wpa_s->wps_stats.status ?
                "Failed" : "None")));
    if (os_snprintf_error(end - pos, ret)) {
        return pos - buf;
    }
    pos += ret;
    if ((WPS_STATUS_FAILURE == wpa_s->wps_stats.status) &&
    ...
    ret = os_snprintf(pos, end - pos, "wpa_s WPS: %s\n",
              wpa_s->wps_success ? "Success" : "Failed");
    if (os_snprintf_error(end - pos, ret)) {
        return pos - buf;
    }
    pos += ret;
                     ...
                     char *buf, size_t *resp_len)
{
    ...
    if (os_strcmp(buf, "PING") == 0) {
        ...
        if (res == -2) {
            ...
            reply_len = -1;
    } else if (0 == os_strncmp(buf, "WPS_GET_STATUS", 14)) {
        /* add command to get wps status */
        reply_len = wpa_ctrl_iface_wps_get_status(wpa_s, reply, reply_size);
