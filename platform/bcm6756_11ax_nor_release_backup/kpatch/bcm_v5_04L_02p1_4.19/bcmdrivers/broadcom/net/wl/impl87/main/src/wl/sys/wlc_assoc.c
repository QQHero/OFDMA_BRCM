 ...
 *
 * $Id: wlc_assoc.c 810066 2022-03-30 09:48:41Z $
 */
...
#define BSSCFG_AUTH_TIMEOUT(cfg) \
    (BSSCFG_EXT_AUTH(cfg)) ? WLC_EXT_AUTH_TIMEOUT : WECA_AUTH_TIMEOUT + DELAY_10MS
...
#define WLC_ASSOC_CAC_BUFFER_TIME_MS (2000)
/* DFS CAC Time + Buffer time */
#define WLC_DFS_GET_ASSOC_CAC_TIME(dfs) \
    (wlc_dfs_get_cactime_ms(dfs) + WLC_ASSOC_CAC_BUFFER_TIME_MS)
...
static bool
wlc_assoc_req_known_entry(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
    int i;

    /* check if 'cfg' is in assoc_req array */
    for (i = 0; i < WLC_MAXBSSCFG; i ++) {
    ...
    const struct ether_addr *bssid, const chanspec_t *chanspec_list, int n_channels)
{
    ...
    if (ASSOC_CACHE_ASSIST_ENAB(wlc) && wlc->scan_results->count > 0) {
        ...
        */
        ...
        if (as->type == AS_ROAM) {
            /* override source MAC */
            sa_override = wlc_scan_get_randmac(wlc->scan, WLC_ACTION_ROAM, cfg);
        }
#endif /* WL_RANDMAC */
        ...
        err = wlc_scan(wlc->scan, bss_type, bssid, 1, &ssid, 0, NULL, scan_type, nprobes,
...
wlc_join_noscb(wlc_info_t *wlc, wlc_bsscfg_t *cfg, wlc_assoc_t *as)
{
    ...
    wl_del_timer(wlc->wl, as->timer);
    wl_add_timer(wlc->wl, as->timer, WECA_AUTH_TIMEOUT + DELAY_10MS, 0);
}
...
#ifdef MFP
        else if (WLC_MFP_ENAB(wlc->pub) && SCB_MFP(scb) && !SCB_AUTHORIZED(scb)) {
            wlc_scb_ampdu_disable(wlc, scb);
        }
#endif /* MFP */
    ...
    uint8 *challenge_text, bool short_preamble, uint state)
{
     ...
     * Also need to reset the state if the Auth frame got stuck in
     * Tx buffer by any chance
     */
    wl_add_timer(wlc->wl, as->timer, WECA_AUTH_TIMEOUT + DELAY_10MS, 0);
...
extern bool wl_is_client_mode(wlc_info_t *wlc);
static void td_set_he_rate(wlc_bsscfg_t *bsscfg, wlc_bss_info_t *bi)
{
    bsscfg->current_bss->rateset.he_bw80_rx_mcs_nss    = bi->he_sup_bw80_rx_mcs;
    bsscfg->current_bss->rateset.he_bw80_tx_mcs_nss    = bi->he_sup_bw80_tx_mcs;
    bsscfg->current_bss->rateset.he_bw160_tx_mcs_nss   = bi->he_sup_bw160_tx_mcs;
    bsscfg->current_bss->rateset.he_bw160_rx_mcs_nss   = bi->he_sup_bw160_rx_mcs;
    bsscfg->current_bss->rateset.he_bw80p80_tx_mcs_nss = bi->he_sup_bw80p80_tx_mcs;
    bsscfg->current_bss->rateset.he_bw80p80_rx_mcs_nss = bi->he_sup_bw80p80_rx_mcs;
}
#endif
...
wlc_join_bss(wlc_bsscfg_t *cfg)
{
    ...
    cfg->ap_orig_chanspec = bi->chanspec;
#ifdef WL_PWRSTATS
...
#ifdef CONFIG_TENDA_WLAN_REPEATER
    if (wl_is_client_mode(wlc)) {
        if (BAND_5G(band->bandtype)) {
            if (bi->flags & WLC_BSS_HT) {
                wlc->pub->_n_enab |= SUPPORT_11N;
            } else {
                wlc->pub->_n_enab &= ~(uint8)SUPPORT_11N;
            }

            if (bi->flags2 & WLC_BSS2_VHT) {
                wlc->pub->vht_features = WL_VHT_FEATURES_DEFAULT | WL_VHT_FEATURES_RATES_ALL | WL_VHT_FEATURES_1024QAM;
                WLC_VHT_FEATURES_PROP_MCS_SET(wlc->pub, VHT_PROP_MCS_MAP_10_11);
                wlc->pub->_vht_enab = ENABLE;
            } else {
                wlc->pub->vht_features = 0;
                wlc->pub->_vht_enab = DISABLE;
            }
...
#define WLC_GET_BW_FROM_CHSPEC(cs) \
                ((CHSPEC_IS8080(cs) || CHSPEC_IS160(cs)) ? 15 : \
                CHSPEC_IS80(cs) ? 7 : \
                CHSPEC_IS40(cs) ? 3 : 1)

            band->bw_cap = WLC_GET_BW_FROM_CHSPEC(chanspec);
            FOREACH_BSS(wlc, idx, bsscfg) {
                bsscfg->current_bss->rateset.vht_mcsmap = bi->vht_rxmcsmap;
                bsscfg->current_bss->rateset.vht_mcsmap_prop = bi->vht_txmcsmap_prop;
                td_set_he_rate(bsscfg, bi);
            }
        } else if (BAND_2G(band->bandtype)) {
            if (bi->flags & WLC_BSS_HT) {
                wlc->pub->_n_enab |= SUPPORT_11N;
            } else {
                wlc->pub->_n_enab &= ~(uint8)SUPPORT_11N;
            ...
            if (bi->flags2 & WLC_BSS2_VHT) {
                wlc->pub->vht_features = WL_VHT_FEATURES_DEFAULT | (WL_VHT_FEATURES_RATES_ALL & ~WL_VHT_FEATURES_1024QAM) | WL_VHT_FEATURES_2G;
                wlc->pub->_vht_enab = ENABLE;
            } else {
                wlc->pub->vht_features = 0;
                wlc->pub->_vht_enab = DISABLE;
            ...
            chanspec = wlc_max_valid_bw_chanspec(wlc, band, cfg, bi->chanspec);
        }
    } else {
        chanspec = wlc_max_valid_bw_chanspec(wlc, band, cfg, bi->chanspec);
    }
...
#if defined(WL_SAE)
    } else if ((as->state == AS_WAIT_AUTH_CONFIRM) || (as->state == AS_START_EXT_AUTH)) {
...
#endif /* ROBUST_DISASSOC_TX */
    else if (as->state == AS_DFS_CAC_START) {
        WL_ASSOC(("wl:%d.%d DFS CAC timedout\n",
...
#ifdef SLAVE_RADAR
    /* Don't call the assoc timer handler, if the DFS CAC Done successfully */
    if ((oldstate == AS_DFS_CAC_START) && (newstate == AS_DFS_ISM_INIT))
        wlc_assoc_timer_del(wlc, cfg);
    ...
    uint8 *body, uint body_len, bool short_preamble)
{
    ...
    if ((auth_status != DOT11_SC_SUCCESS) &&
        !((cfg->auth == WL_AUTH_SAE_KEY) && (WPA3_VALID_AUTH_STATUS(auth_status)))) {
    ...
    if (BSSCFG_EXT_AUTH(cfg) && (auth_alg == DOT11_SAE)) {
        ...
        WL_ASSOC(("wl%d: Sae External Auth: auth_seq = %d\n", WLCWLUNIT(wlc),
                auth_seq));
        wlc_assoc_change_state(cfg, AS_WAIT_AUTH_CONFIRM);
        wlc_assoc_timer_del(wlc, cfg);
...
#else
    timeout = WLC_ASSOC_RETRY_TIMEOUT;
#endif /* MFP */
...
wlc_disassoc_tx(wlc_bsscfg_t *cfg, bool send_disassociate)
{
    ...
    if (send_disassociate) {
        if (ETHER_ISNULLADDR(cfg->BSSID.octet)) {
            ...
            wlc_is_quiet_chanspec(wlc->cmi, current_bss->chanspec)) ||
            wlc_restricted_chanspec(wlc->cmi, current_bss->chanspec)) {
            WL_ASSOC(("wl%d: JOIN: avoid sending DISASSOC to %s on "
...
wlc_assoc_success(wlc_bsscfg_t *cfg, struct scb *scb)
{
    ...
    struct scb *prev_scb;
#ifdef WL_SPP_AMSDU
    bool sta_cap, sta_req, ap_cap, ap_req;
#endif
    ASSERT(scb != NULL);
...
#endif /* MFP */
#ifdef WL_SPP_AMSDU
    /* Reset SPP AMSDU flag */
    scb->flags &= ~SCB_AMSDU_SPP;

    /* Stations SPP AMSDU capability is indicated in assoc request.
     * Since assoc is success AP supports it, enable SPP AMSDU if supported
     */
    /* If SPP AMSDU is supported */
    sta_cap = BSSCFG_IS_RSN_CAP_SPPC(cfg);
    sta_req = BSSCFG_IS_RSN_CAP_SPPR(cfg);
    ap_cap = IS_RSN_CAP_SPPC(cfg->target_bss);
    ap_req = IS_RSN_CAP_SPPR(cfg->target_bss);

    if (sta_cap && ap_cap) {
        /* Enable SPP for this scb */
        scb->flags |= SCB_AMSDU_SPP;
    }
    else if (((sta_req) && (!ap_cap)) || ((!sta_cap) && (ap_req)) ||
        ((sta_cap || sta_req) && ((!ap_cap) && (ap_req))) ||
        ((sta_cap || sta_req) && ((ap_cap) && (!ap_req)))) {
        /* Disable AMSDU */
        scb->flags &= ~SCB_AMSDUCAP;
        scb->flags &= ~SCB_AMSDU_SPP;
        scb->flags2 &= ~SCB2_AMSDU_IN_AMPDU_CAP;
        scb->flags2 |= SCB2_AMSDU_DISABLE;
    }
#endif /* WL_SPP_AMSDU */
    /* clear PM state */
    wlc_set_pmoverride(cfg, FALSE);
    ...
    uint auth_status, uint auth_type)
{
    ...
    if ((as->state != AS_WAIT_AUTH_CONFIRM) && (as->state != AS_START_EXT_AUTH)) {
    ...
    if (status == WLC_E_STATUS_SUCCESS) {
        ...
        if (BSSCFG_EXT_AUTH(cfg) &&
            WPA3_VALID_AUTH_STATUS(auth_status))
            return;
    ...
    uint assoc_status, bool reassoc, uint bss_type)
{
    ...
    if (status == WLC_E_STATUS_TIMEOUT) {
        ...
        if (assoc_status == DOT11_SC_INVALID_PMKID)
            goto do_event;
    } else if (status == WLC_E_STATUS_ABORT) {
...
wlc_roam_complete(wlc_bsscfg_t *cfg, uint status, struct ether_addr *addr, uint bss_type)
{
    ...
    if (status == WLC_E_STATUS_SUCCESS) {
        ...
        WL_ASSOC(("wl%d: JOIN: roam found no networks, disable apcfg "
            "if radio in repeater mode and upstream STA has lost "
            "contact to AP\n", WLCWLUNIT(wlc)));
        if (!wlc_bss_connected(cfg)) {
            wlc_assoc_disable_apcfg_on_repeater(wlc, cfg);
        }
...
wlc_join_attempt_info_end(wlc_bsscfg_t *cfg, int status)
{
    ...
    wlc_congest_channel_req_t cca_results = {0};
    int cca_status;
#endif /* CCA_STATS */
...
wlc_mac_request_scan(wlc_info_t *wlc, wlc_bsscfg_t *cfg, int req, uint scan_flags)
{
    ...
    if (req == WLC_ACTION_SCAN || req == WLC_ACTION_ISCAN || req == WLC_ACTION_ESCAN) {
        ...
        if (wlc_radar_chanspec(wlc->cmi, wlc->home_chanspec) && (
                ...
                /* 被动扫描时不检查当前信道是否处在静默期，否则会导致刚切换到dfs信道时被动扫描失败 */
                (WL_SCANFLAGS_PASSIVE == scan_flags ? 0 : wlc_quiet_chanspec(wlc->cmi, wlc->home_chanspec)) ||
//                wlc_quiet_chanspec(wlc->cmi, wlc->home_chanspec)) &&
#endif
                 FALSE ) && (!WLC_APSTA_ON_RADAR_CHANNEL(wlc) && AP_ACTIVE(wlc) &&
    ...
    /* is any assoc/roam in progress? */
        ...
        /* AS_SCAN_ABORT状态可以正常触发扫描 */
        if (AS_SCAN_ABORT == as->state) {
            err = BCME_OK;
        }
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
    }
    ...
    uint *roam_delta)
{
    ...
    if (band->roam_prof) {
        ...
        band->roam_prof[0].roam_flags = (int8)WL_ROAM_PROF_DEFAULT;
    ...
    struct ether_addr *sa, uint16 reason_code)
{
    ...
    if (!hwrs_scb) {
        ...
        if (BSSCFG_AP(cfg) &&
             ...
             * Don't call wlc_scb_clearstatebit as
             * notify callback can lead to wlc_tx_fifo_scb_flush().
             * If scb is still authorized wlc_scb_clearstatebit() will be
             * called from wlc_ap_scb_unusable_stas_timeout().
             * Link will be closed for this scb, so no new packets will
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN)
    TDP_INFO("wl%d.%d: %s: send deauth for "MACF" with reason %d", wlc->pub->unit,
        WLC_BSSCFG_IDX(cfg), __FUNCTION__, ETHERP_TO_MACF(da), reason_code);
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
...
wlc_deauth_sendcomplete(wlc_info_t *wlc, uint txstatus, void *arg)
{
    ...
    if ((scb->sent_deauth == NULL) || (scb->sent_deauth != cbarg)) {
        WL_ASSOC(("wl%d.%d: %s: deauth complete for scb "MACF", but scb does not match"
            " (deauth sent: %p)\n",
            wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
            ETHERP_TO_MACF(&scb->ea), scb->sent_deauth));
    ...
    wlc_bss_mac_event(wlc, SCB_BSSCFG(scb), WLC_E_DEAUTH, (struct ether_addr *)arg,
        WLC_E_STATUS_SUCCESS, cbarg->reason_code, 0, 0, 0);
    ...
    if (BSSCFG_STA(bsscfg)) {
        ...
        /* SCB will be freed from wlc_watchdog */
        SCB_MARK_DEL(scb);
    } else {
#ifdef WL_HAPD_WDS
...
wlc_handle_ap_lost(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
    ...
    if (BSSCFG_STA(cfg) && APSTA_ENAB(wlc->pub)) {
        ...
        if (wlc->ul_ofdma_repeater_ac_param_mod) {
            edcf_acparam_t *acp = &cfg->wme->wme_param_ie.acparam[AC_BK];
            ...
            */
            wlc_edcf_acp_set_one(wlc, cfg, acp, TRUE);
        }
#endif /* DWDS */
        wlc_sta_assoc_upd(cfg, FALSE);
...
wlc_assoc_do_dfs_reentry_cac_if_required(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi)
{
    ...
    if ((wlc_radar_chanspec(wlc->cmi, bi->chanspec)) &&
        ...
        if (ret == BCME_OK) {
            ...
            wlc_assoc_timer_del(wlc, cfg);
            /* Start a assoc timer to handle the DFS CAC fail case */
            wl_add_timer(wlc->wl, as->timer,
                (WLC_DFS_GET_ASSOC_CAC_TIME(wlc->dfs)), FALSE);
...
void
wlc_assoc_dh_params_free(wlc_info_t *wlc, wlc_bsscfg_t *cfg)
{
    if (cfg->assoc && cfg->assoc->dh_params) {
        MFREE(wlc->osh, cfg->assoc->dh_params, sizeof(dh_params_ie_t));
        cfg->assoc->dh_params = NULL;
    }
}
