 ...
 *
 * $Id: wlc_tx.c 801541 2021-07-27 05:42:51Z $
 *
 */
...
wlc_pull_q(wlc_info_t *wlc, enum wme_ac ac, struct spktq *output_q, uint *fifo_idx);
...
static void wlc_common_q_scb_free(wlc_info_t *wlc, scb_t *from_scb);
...
static void wlc_hwa_tx_fifo_sync_hpktq(wlc_info_t *wlc, txq_t *txq, uint fifo, uint8 flag,
    bool is_active_q);
static void wlc_hwa_tx_fifo_hpktq_flush(wlc_info_t *wlc, txq_t *txq, uint fifo, uint16 *pktcnt);
...
/**
 * Free all SCB specific pkts from common Q.
 */
static void
wlc_common_q_scb_free(wlc_info_t *wlc, scb_t *from_scb)
{
    ...
    if (!PIO_ENAB(pub)) {
        ...
        while ((p = spktq_deq(spktq))) {
            ...
            if (D11REV_GE(corerev, 128)) {
                ...
                txh->rev128.OrderSeq = txqi->order_seq[WME_PRIO2AC(PKTPRIO(p))];
            }
...
wlc_low_txq_pkt_cb(void *ctx, void *pkt)
{
    ...
    if (ret & MAP_PKTS_CB_FREE) {
        ...
        if ((scb = WLPKTTAGSCBGET(pkt))) {
            SCB_PKTS_INFLT_FIFOCNT_DECR(low_txq_map->wlc, scb, PKTPRIO(pkt));
        }
    }
    return ret;
}
    ...
    for (item_p = dll_head_p(&txq->active); !dll_end(&txq->active, item_p);
        ...
        HWA_PKTPGR_EXPR({
            if (hpktq_n_pkts(txq_swq) > 0) {
                wlc_hwa_tx_fifo_hpktq_flush(wlc, txq, fifo_idx, &pktcnt);
            }
        });
#endif /* BCMHWA */
 ...
 *
 * @param[in]    ac        Caller specified WME access category (priority) in the range [0..3]
 * @param[inout] output_q  Single priority packet queue
 * @param[out]   fifo_idx
 */
static uint BCMFASTPATH
wlc_pull_q(wlc_info_t *wlc, enum wme_ac ac, struct spktq *output_q, uint *fifo_idx)
{
    ...
    /* Use a prec_map that matches the wme access category parameter */
    ASSERT(ac < AC_COUNT);
    prec_map = wlc->fifo2prec_map[ac]; /* prec_map now maps to wme access category 0..3 */
    pkt_cnt = 0;
    ...
    while (TRUE) {
...
#endif /* BCMPCIEDEV */
        if ((blocked_q.q.n_pkts) && (prec != prev_prec)) {
        ...
        *fifo_idx = fifo;
        if ((scb->ac_suspend & (1 << WME_PRIO2AC(PKTPRIO(pkt[0])))) &&
            !SCB_MARKED_DEL(scb)) {
            pkt[0] = cpktq_mdeq(&qi->cpktq, prec_map, &prec);
            spktq_enq_head(&blocked_q, pkt[0]);
        } else {
            break;
        }
    }
    ...
    while ((pkt_cnt < max_pkt_cnt) && prec_map &&
        ...
        if (BCMPCIEDEV_ENAB() && BCMLFRAG_ENAB() &&
             ...
             * memory access is allowed in PCIE D3 suspend mode.
             */
            PKTFREE(osh, pkt[0], TRUE);
            continue;
        }
        ...
        if (*fifo_idx != fifo) {
             ...
             * changes.
             */
            cpktq_penq(wlc, &qi->cpktq, prec, pkt[0], TRUE);
            break;
        }
...
#ifdef WL_BSSCFG_TX_SUPR
        if (wlc_bsscfg_tx_supr(wlc, scb, pkt[0], prec, &suppress_to_host)) {
            continue;
        }
#endif /* WL_BSSCFG_TX_SUPR */
...
wlc_send_q(wlc_info_t *wlc, wlc_txq_info_t *qi)
{
    ...
    for (ac = AC_COUNT - 1; ac >= 0; ac--) {
        ...
        wlc_low_txq_enq_list(wlc, qi->low_txq, fifo_idx, &temp_q, pkt_cnt);
    } /* for */
#ifdef WLTAF
...
wlc_tx_fill_rate_info_block_internal(wlc_info_t *wlc, scb_t *scb, wlc_bsscfg_t *bsscfg,
    d11ratemem_rev128_rate_t *rate_info_block, ratespec_t rspec, enum wme_ac ac,
    ...
    if (N_ENAB(wlc->pub)) {
        ...
        if (!RSPEC_ACTIVE(wlc->band->rspec_override)) {
            ...
            if (wlc->stf->ldpc_tx == ON ||
                ...
                wlc->stf->ldpc_tx == AUTO) ||
#ifdef WL_PROXDETECT
                (PROXD_ENAB(wlc->pub) && wlc_proxd_frame(wlc, parms->pkttag) &&
                    RSPEC_ISHE(rspec)) ||
#endif /* WL_PROXDETECT */
                FALSE) {
                if (!rspec_legacy) {
    ...
    d11ratemem_rev128_entry_t *rate_entry, void *rate_store, void *rs)
{
    ...
    if (scb == WLC_RLM_SPECIAL_RATE_SCB(wlc)) {
        ...
        for (k = 0; k < D11_REV128_RATEENTRY_NUM_RATES; k++) {
            bool use_mimops_rts = FALSE, fixed_rate = FALSE;
            bool use_rts = FALSE;
            bool use_cts = FALSE;
            dummy_mcl = 0;
            dummy_mch = 0;
...
#ifdef BCMDBG
    {
    /* in case of FLUSHFIFO_FLUSHSCB, packet in flight for this fifo should be 0.
     * If partial_reclaim is TRUE. Defer the check after partial reclaim is finish.
     */
    scb_t *scb = (scb_t*)wlc->fifoflush_scb;
    if ((flag == FLUSHFIFO_FLUSHSCB) && !SCB_INTERNAL(scb) && !partial_reclaim) {
        enum wme_ac ac;
            ...
            HWA_PKTPGR_EXPR({
                ...
                while (hwa_pktpgr_txfifo_shadow_reclaim(WL_HWA_DEVP(wlc),
                    ...
                    if (fifo != ULMU_TRIG_FIFO) {
                        wlc_hwa_tx_fifo_shadow_account(wlc, txq,
                            fifo, flag);
                        partial_reclaim = TRUE;
                    }
                }
...
wlc_tx_fifo_scb_flush(wlc_info_t *wlc, struct scb *remove)
{
    ...
    wlc_common_q_scb_free(wlc, remove);
...
wlc_hwa_tx_fifo_sync_hpktq(wlc_info_t *wlc, txq_t *txq, uint fifo, uint8 flag, bool is_active_q)
{
    ...
    do {
        ...
        empty = hwa_pktpgr_txfifo_shadow_pull_from_host(WL_HWA_DEVP(wlc),
            WLC_HW_MAP_TXFIFO(wlc, fifo), &pkt_list, &temp_hpq);
        if (spktq_n_pkts(&pkt_list) > 0) {
            spktq_enq_list(&txq_swq->spktq, &pkt_list);
...
static void
wlc_hwa_tx_fifo_hpktq_flush(wlc_info_t *wlc, txq_t *txq, uint fifo, uint16 *pktcnt)
{
    struct spktq pkt_list;    /**< single priority packet queue */
    txq_swq_t *txq_swq;    /**< a non-priority queue backing one d11 FIFO */
    bool empty;
...
/** Callback. Match packet for a specific scb */
static uint32
wlc_scb_map_pkts(void *ctx, void *pkt)
{
    struct scb *map_scb = (struct scb *)ctx;
    struct scb *scb = WLPKTTAGSCBGET(pkt);
    uint32 ret = MAP_PKTS_CB_NOOP;

    if ((scb == NULL) || (scb == map_scb))
        ret =  MAP_PKTS_CB_FREE;
...
{
    wlc_txq_info_t *qi = SCB_WLCIFP(scb)->qi;
    wlc_txq_map_pkts(wlc, qi, wlc_scb_map_pkts, (void *)scb);
}
