...
#ifndef _CRT_SECURE_NO_DEPRECATE_
#define _CRT_SECURE_NO_DEPRECATE_ 1
#endif
...
#define AES_CBC_128_KEY "Tenda0123456789M"
 ...
 * AES key schedule
 */
static void aes_set_key( aes_context *ctx, uint8 *key, int keysize )
{
    int i;
    uint32 *RK, *SK;
    ...
    */
    ...
    RK = ctx->erk;
    //防止key长度不是8倍数导致GET_UINT32_BE操作越界
    strncpy((char*)key_buf, (char*)key, (sizeof(key_buf) -1));

    for( i = 0; i < (keysize >> 5); i++ )
    {
        GET_UINT32_BE( RK[i], key_buf, i << 2 );
    }
 ...
 */
static unsigned int aes_cbc_encrypt( aes_context *ctx,
                      unsigned char iv[16],
                      unsigned char *input,
...
void IE_AesCbcDecrypt(char *input, char *output, int len,char *key , unsigned int *Decrypt_len)
{
    ...
    else if (24 >= strlen(key))
    {
        aes_set_key(&ctx, (unsigned char *)key, 192);
    }
    else
    {
        aes_set_key(&ctx, (unsigned char *)key, 256);
    }

    aes_cbc_decrypt(&ctx, iv, (unsigned char *)input, (unsigned char *)output, len, Decrypt_len);
}
...
unsigned int IE_AesCbcEncrypt(char *input, char *output, int len, char *key)
{
    ...
    return aes_cbc_encrypt(&ctx, iv, (unsigned char *)input, (unsigned char *)output, len);
}
EXPORT_SYMBOL(IE_AesCbcEncrypt);

/*
* rtl1500项目中该文件在5g中编译，需要导出，其他方案中不需要导出。
*/
#ifdef TD_8832BR_WIFI6
EXPORT_SYMBOL(IE_AesCbcDecrypt);
#endif
