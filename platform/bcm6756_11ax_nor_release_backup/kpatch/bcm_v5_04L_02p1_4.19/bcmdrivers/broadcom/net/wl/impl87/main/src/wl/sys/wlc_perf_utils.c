 ...
 *
 * $Id: wlc_perf_utils.c 806537 2021-12-23 06:43:29Z $
 */
...
static const bcm_iovar_t perf_utils_iovars[] = {
    ...
    {"txrx_summary", IOV_SCB_TXRX_SUMMARY, (IOVF_GET_UP), 0, IOVT_BUFFER, 0 },
#endif /* TXRX_SUMMARY */
    {NULL, 0, 0, 0, 0, 0}
};
...
#if defined(TXRX_SUMMARY)
static int wlc_txrx_summary_cubby_init(void *handle, struct scb *scb);
static void wlc_txrx_summary_cubby_exit(void *handle, struct scb *scb);
#endif /* TXRX_SUMMARY */
...
static wlc_txrx_summary_scb_cubby_t **
wlc_txrx_summary_scb_cubby_ptr(struct wlc_perf_utils *pui, struct scb *scb)
{
    return (wlc_txrx_summary_scb_cubby_t **)SCB_CUBBY(scb, pui->scb_handle);
}

static wlc_txrx_summary_scb_cubby_t *
wlc_txrx_summary_scb_cubby(struct wlc_perf_utils *pui, struct scb *scb)
{
    return *wlc_txrx_summary_scb_cubby_ptr(pui, scb);
}
 ...
 */
wlc_txrx_summary_scb_counters_t *
wlc_txrx_summary_scb_counters(struct wlc_info *wlc, struct scb *scb)
{
    return (scb) ? wlc_txrx_summary_scb_cubby(wlc->pui, scb) : NULL;
}
static int
wlc_txrx_summary_cubby_init(void *handle, struct scb *scb)
{
    wlc_perf_utils_t *pui = handle;

    *wlc_txrx_summary_scb_cubby_ptr(pui, scb) = NULL;

    return BCME_OK;
}

static void
wlc_txrx_summary_cubby_exit(void *handle, struct scb *scb)
{
    wlc_perf_utils_t *pui = handle;
    wlc_txrx_summary_scb_cubby_t *cubby = wlc_txrx_summary_scb_cubby(pui, scb);

    if (cubby) {
        MFREE(pui->wlc->osh, cubby, sizeof(*cubby));
        *wlc_txrx_summary_scb_cubby_ptr(pui, scb) = NULL;
    }
}
#endif /* TXRX_SUMMARY */
...
#if defined(TXRX_SUMMARY)
    if ((pui->scb_handle = wlc_scb_cubby_reserve(wlc, sizeof(wlc_txrx_summary_scb_cubby_t *),
        wlc_txrx_summary_cubby_init, wlc_txrx_summary_cubby_exit, NULL, pui)) < 0) {
        goto fail;
    }
#endif /* TXRX_SUMMARY */
#if defined(WLPKTDLYSTAT)
            ...
            case 'M':
            {
                ...
                if (addr_OK) {
                    ...
                    bcm_bprintf(b, "%s%s%s%s",
                        SCB_VHTMU(scb) ? " vhtmu" : "",
                        SCB_HEMMU(scb) ? " hemmu" : "",
                        SCB_DLOFDMA_ADM(scb) ? " dlofdma" : "",
                        SCB_ULOFDMA_ADM(scb) ? " ulofdma" : "");
                } else {
                    iov_pktq->pktq_log.v06.num_prec[index] = 0;
                }
                break;
...
#if defined(TXRX_SUMMARY)
    case IOV_GVAL(IOV_SCB_TXRX_SUMMARY): {

        struct scb_iter scbiter;
        struct scb *scb = NULL;
        uint32 tsf_time = (R_REG(wlc->osh, D11_TSFTimerLow(wlc)));
        int num_records = 0;
        iov_txrx_summary_struct_t *output = (iov_txrx_summary_struct_t *)arg;
        int rem_len;
        int32           int_val = 0;

        if (p_len >= sizeof(int_val)) {
            memcpy(&int_val, params, sizeof(int_val));
        }
        if (len < sizeof(iov_txrx_summary_struct_t)) {
            return BCME_BADLEN; /* way too short */
        }
        rem_len = len - sizeof(iov_txrx_summary_struct_t);

        bsscfg = wlc_bsscfg_find_by_wlcif(wlc, wlcif);
        ASSERT(bsscfg != NULL);
        FOREACH_BSS_SCB(wlc->scbstate, &scbiter, bsscfg, scb) {
            if (SCB_ASSOCIATED(scb)) {
                iov_txrx_summary_record_t *drec;
                iov_txrx_summary_scb_counters_t *cd; /* Counter destination */
                wlc_txrx_summary_scb_counters_t *cs; /* Counter source */
                wlc_txrx_summary_scb_cubby_t *cubby;

                cubby = wlc_txrx_summary_scb_cubby(pui, scb);
                ...
                if (!cubby) {
                    ...
                    *wlc_txrx_summary_scb_cubby_ptr(pui, scb) = cubby;
                }

                if (rem_len < sizeof(iov_txrx_summary_record_t)) {
                    return BCME_BUFTOOSHORT;
                }
                rem_len -= (sizeof(iov_txrx_summary_record_t));
                ...
                /* Copy rx counter values to output record */
                cd->rx.phyrate = cs->rx.phyrate;
                cd->rx.bytes = cs->rx.bytes;
                cd->rx.attempted = cs->rx.attempted;
                cd->rx.mpdu = cs->rx.mpdu;
                cd->rx.tones = cs->rx.tones;
                cd->rx.mpdu_su = cs->rx.mpdu_su;
                cd->rx.mpdu_mumimo = cs->rx.mpdu_mumimo;
                cd->rx.mpdu_ofdma = cs->rx.mpdu_ofdma;
                cd->rx.retried = cs->rx.retried;
                cd->rx.bw = cs->rx.bw;
                cd->rx.nss[0] = cs->rx.nss[0];
                cd->rx.nss[1] = cs->rx.nss[1];
                cd->rx.nss[2] = cs->rx.nss[2];
                cd->rx.nss[3] = cs->rx.nss[3];

                /* Copy rx counter values to output record */
                cd->tx.phyrate = cs->tx.phyrate * PERF_LOG_RATE_FACTOR_100;
                cd->tx.bytes = cs->tx.bytes;
                cd->tx.attempted = cs->tx.attempted;
                cd->tx.mpdu = cs->tx.mpdu;
                cd->tx.tones = cs->tx.tones;
                cd->tx.mpdu_su = cs->tx.mpdu_su;
                cd->tx.mpdu_mumimo = cs->tx.mpdu_mumimo;
                cd->tx.mpdu_ofdma = cs->tx.mpdu_ofdma;
                cd->tx.retried = cs->tx.retried;
                cd->tx.bw = cs->tx.bw;
                cd->tx.nss[0] = cs->tx.nss[0];
                cd->tx.nss[1] = cs->tx.nss[1];
                cd->tx.nss[2] = cs->tx.nss[2];
                cd->tx.nss[3] = cs->tx.nss[3];

                drec->time_delta = tsf_time - cs->time_delta;
                cs->time_delta = tsf_time;

                memset(&cs->rx, 0, sizeof(cs->rx));
                memset(&cs->tx, 0, sizeof(cs->tx));
                ++num_records;
            }
        }
        output->structure_version = SCB_TXRX_SUMMARY_STRUCT_VERSION;
        output->structure_count = num_records;

        /* Indicate length of the structures used */
        output->sizeof_iov_txrx_summary_record_t = sizeof(iov_txrx_summary_record_t);
        output->sizeof_iov_txrx_summary_scb_counters_t =
        ...
        if (SCB_ASSOCIATED(scb)) {
            ...
            if (total_1s != 0) {
                ...
                if (pui->txdelay_params->tune) {
                    WL_ERROR(("TXDLY:["MACF"] rssi=%d avg_1s(%5d)-avg(%d)=%d "
                        "(avg*ratio)/100=%d\n", ETHER_TO_MACF(scb->ea),
                        rssi, avg_1s, avg, avg_1s - avg, delta));
                }
...
#ifdef GAME_SPEEDUP
void
wlc_txq_special(wlc_info_t *wlc, special_scb_t *special_scb, uint8 txop_percent)
{
    struct pktq *q;
    uint i;
    uint32  timelo, timehi;
    uint8 j = wlc->current_event_cnt % SPECIAL_EVENT_CNT;

    timelo = (R_REG(wlc->osh, D11_TSFTimerLow(wlc)));
    timehi = (R_REG(wlc->osh, D11_TSFTimerHigh(wlc)));

    q = scb_ampdu_prec_pktq(wlc, special_scb->scb);
    ...
    for (i = 0; i < q->num_prec; i++) {
        if (q->pktqlog->_prec_log & (1 << i)) {
            pktq_counters_t*        pktq_count = q->pktqlog->_prec_cnt[i];
            special_pktq_counters_t*    special_pktq_count = &(wlc->special_prec_cnt[i]);
            uint64 time_delta = 0;
            uint64 air = 0;
            if (special_pktq_count->_logtimelo != 0 || special_pktq_count->_logtimehi != 0) {
                time_delta = (uint64)timehi - (uint64)special_pktq_count->_logtimehi;
                time_delta <<= 32;
                time_delta += (uint64)timelo - (uint64)special_pktq_count->_logtimelo;
            }

            special_scb->queue_utils[j] += pktq_count->max_used;

            if (pktq_count->airtime >= special_pktq_count->airtime) {
                air = (pktq_count->airtime - special_pktq_count->airtime)* 100;
                do_div(air, time_delta);
                special_scb->idlerate_percent[j] += (uint8)air;
            } else {
                /*maybe impact by wl pktq_stats A: read and clear, avoid raise event */
                special_scb->idlerate_percent[j] = 100;
            }
            //store last value
            special_pktq_count->airtime = pktq_count->airtime;
            pktq_count->max_used = pktqprec_n_pkts(q, i);
            special_pktq_count->_logtimelo = timelo;
            special_pktq_count->_logtimehi = timehi;
        } else {
            pktq_counters_t* prec_cnt =
                wlc_txq_prec_log_enable(wlc, q, i, (q->num_prec > NUMPRIO));
            if (prec_cnt) {
    ...
    special_scb->idlerate_percent[j] += txop_percent;
    if (special_scb->idlerate_percent[j] > 100) {
        special_scb->idlerate_percent[j] = 100;
    }


    if (special_scb->idlerate_level == 0) {
        if(special_scb->idlerate_percent[j] < special_scb->idlerate_thresh[1] ) {
            special_scb->idlerate_level = 2;
            special_scb->need_event |= IDLERATE_EVNET_FLAG;
        } else if (special_scb->idlerate_percent[j] < special_scb->idlerate_thresh[0] ) {
            special_scb->idlerate_level = 1;
            special_scb->need_event |= IDLERATE_EVNET_FLAG;
        }
    } else if (special_scb->idlerate_level == 1) {
        if(special_scb->idlerate_percent[j] < special_scb->idlerate_thresh[1]){
            special_scb->idlerate_level = 2;
            special_scb->need_event |= IDLERATE_EVNET_FLAG;
        }
    ...
    if (special_scb->utils_level == 0) {
        if(special_scb->queue_utils[j] > special_scb->utils_thresh[1] ) {
            special_scb->utils_level = 2;
            special_scb->need_event |= UTILS_EVENT_FLAG;
        } else if (special_scb->queue_utils[j] > special_scb->utils_thresh[0] ) {
            special_scb->utils_level = 1;
            special_scb->need_event |= UTILS_EVENT_FLAG;
        }
    } else if (special_scb->utils_level == 1) {
        if(special_scb->queue_utils[j] > special_scb->utils_thresh[1] ) {
            special_scb->utils_level = 2;
            special_scb->need_event |= UTILS_EVENT_FLAG;
        }
