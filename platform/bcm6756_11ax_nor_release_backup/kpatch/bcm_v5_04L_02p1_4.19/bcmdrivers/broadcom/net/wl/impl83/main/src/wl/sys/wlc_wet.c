 ...
 *
 * $Id: wlc_wet.c 792817 2020-11-05 01:14:50Z $
 */
...
wet_sta_add_ip(wlc_wet_info_t *weth, wet_sta_t *sta, uint8 ipver,
        uint8 *iaddr) {
    if (wet_sta_ip_inlink(sta, ipver, iaddr) == 0)
        return WET_STA_HASH_OK;
    else {
        /* no ip assigned to this sta, assign a new one */
        wet_sta_iplink_t *p_sta_head = sta->ip_link[ipver];
        wet_sta_iplink_t **p_sta = &(sta->ip_link[ipver]);
        *p_sta = MALLOCZ(weth->pub->osh, sizeof(wet_sta_iplink_t));
        if (*p_sta == NULL) {
            WL_ERROR(("allocate ip space error!\n"));
            return WET_STA_HASH_OK;
        }
        bcopy(iaddr, (*p_sta)->ip, IP_ADDR_LEN[ipver]);
        (*p_sta)->next = p_sta_head;
    ...
    uint8 *iaddr, wet_sta_t **saddr)
{
    ...
    for (sta = weth->stahash_ip[ipver][i]; sta; sta = sta->next_ip[ipver]) {
        if (bcmp(sta->ip[ipver], iaddr, IP_ADDR_LEN[ipver]) &&
                wet_sta_ip_inlink(sta, ipver, iaddr))
            continue;
        *saddr = sta;
        return 0;
    }
