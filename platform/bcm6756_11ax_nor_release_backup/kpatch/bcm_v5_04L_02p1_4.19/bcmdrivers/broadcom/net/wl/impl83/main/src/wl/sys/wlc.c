 ...
 *
 * $Id: wlc.c 801256 2021-07-19 18:27:17Z $
 */
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
int wlc_td_sta_info(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, const struct ether_addr *ea,
...
wlc_watchdog(void *arg)
{
    ...
    */
    ...
    if (wlc->pub->rxdma_stuck) {
        wlc->pub->rxdma_stuck = FALSE;
        WL_ERROR(("wl%d: %s: rx dma stuck. HAMMERING!\n",
            wlc->pub->unit, __FUNCTION__));
        wlc_fatal_error(wlc);
    }
...
wlc_doioctl(void *ctx, uint cmd, void *arg, uint len, struct wlc_if *wlcif)
{
    ...
    case WLC_SET_SSID: {
        ...
        if (km_eventscenter_wifiev_set_ssid_handler) {
            km_eventscenter_wifiev_set_ssid_handler(bsscfg->wlc->pub->unit, bsscfg->wlcif->wlif->dev->name, ssid->SSID, ssid->SSID_len);
        }
#endif
        ...
        if (BSSCFG_STA(bsscfg)) {
            ...
            if (len > sizeof(wlc_ssid_t)) {
                if (((wl_join_params_t *)arg)->ssid_type &
                    WL_SCAN_SSIDFLAGS_SHORT_SSID) {
    ...
    case WLC_SCB_DEAUTHORIZE: {
        ...
        /* (de)authorize/authenticate a single station */
            ...
            if (enable && ((DWDS_ENAB(bsscfg) && SCB_DWDS_CAP(scb)) ||
                ...
                bcmerror = wlc_wds_create(wlc, scb, WDS_DYNAMIC);
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
                if (BCME_OK == bcmerror && NULL != scb->wds->wlif) {
    ...
    case WLC_TD_STEER_MAPS:
        bcmerror = maps_ioctl_handler(arg, len, bsscfg);
        break;
#endif
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    ...
    void *params, uint p_len, void *arg, uint len, uint val_size, struct wlc_if *wlcif)
{
    ...
    */
    ...
    case IOV_SVAL(IOV_CHANSPEC): {
        ...
        if (MCHAN_ENAB(wlc->pub) && BSS_P2P_ENAB(wlc, bsscfg))
...
wlc_statsupd(wlc_info_t *wlc)
{
    ...
    rxgiant_delta = cnt->rxgiant - wlc->pub->last_rxgiant;
    wlc->pub->last_rxgiant = cnt->rxgiant;
    if (rxgiant_delta >= 50) {
        WL_ERROR(("wl%d: rxgiant over %d in %d sec!\n", pub->unit,
            rxgiant_delta, SW_TIMER_MAC_STAT_UPD));
        wlc->pub->rxdma_stuck = TRUE;
    }
    ...
    int (*pre_send_fn)(wlc_info_t*, wlc_bsscfg_t*, void*, void *), void *data)
{
    ...
    WL_PS_EX(scb, ("wl%d.%d: sending null frame to "MACF"\n", wlc->pub->unit,
        WLC_BSSCFG_IDX(bsscfg), ETHERP_TO_MACF(da)));
                        ...
                        td_sta_info_t *td_sta)
{
    ...
    strncpy(td_sta->ifname, scb->bsscfg->wlcif->wlif->dev->name, sizeof(td_sta->ifname) - 1);
