 ...
 *
 * $Id: wlc.h 810739 2022-04-13 06:48:38Z $
 */
...
#define BCNMISC_COLOR_COLLISION        (1<<3)    /* bcns promisc mode override for color
                         * collision detect
                         */
#define BCNMISC_MONITOR            (1<<4)    /* bcns promisc mode override for monitor */
#define BCNMISC_AXSTA_OBSS_NBWRU    (1<<5)    /* bcns promisc mode override for OBSS Narrow
                         * Bandwidth RU tolerance check
                         */
...
/* AC to prio bitmaps */
#define DPI_PRIO_BK_BMP        (0x1)
#define DPI_PRIO_BE_BMP        (0x1F << 1)
#define DPI_PRIO_VI_BMP        (0x1 << 6)
#define DPI_PRIO_VO_BMP        (0x1 << 7)
...
/* Limit number of PRS retransmissions */
#define MAX_PRS_RTX_MIN            2
#define MAX_PRS_RTX_DEF            4
#define MAX_PRS_RTX            10
...
#define CHSPEC2WLC_BW(chanspec) \
    ((CHSPEC_BW(chanspec) - WL_CHANSPEC_BW_20) >> WL_CHANSPEC_BW_SHIFT)

/* Returns TRUE if chanspec is under allowed BW Cap. Else returns FALSE */
#define CHSPEC_IN_BWCAP(bw_cap, chspec) \
    (((WLC_BW_CAP_20MHZ << (CHSPEC2WLC_BW(chspec))) & (bw_cap)) != 0)
...
typedef uint16 (*wlc_d11hdrs_fn_t)(wlc_info_t *wlc, void *pkt, struct scb *scb,
    uint txparams_flags, uint frag, uint nfrags, uint fifo, uint next_frag_len,
...
typedef struct wlc_prb_rqst_param {
    wlc_ssid_t ssid;
    uint8 short_ssid_len;
    uint32 *short_ssid;
} wlc_prb_rqst_param_t;


#ifdef GAME_SPEEDUP
#define SPECIAL_EVENT_CNT   10      //单个special sta 记录单个元素(idlerate_percent\rssi\queue_utils)的最大计数
#define MAX_SPECIAL_SCB     16      //最多可以监控的special(game speedup) sta的个数

#define IDLERATE_EVNET_FLAG 1<<0
#define RSSI_EVENT_FLAG     1<<1
#define UTILS_EVENT_FLAG    1<<2
extern bool wlc_special_scb_exist(wlc_info_t *wlc);
typedef struct special_scb {
    bool        used;
    struct      scb *scb;
    uint8       idlerate_thresh[2];
    int8        rssi_thresh[2];
    uint32      utils_thresh[2];
    uint8       idlerate_percent[SPECIAL_EVENT_CNT];
    int8        rssi[SPECIAL_EVENT_CNT];
    uint32      queue_utils[SPECIAL_EVENT_CNT];
...
typedef struct special_chanim 
{
    uint64      last_uptime;
    uint32      last_txop;
}special_chanim_t;

typedef struct special_pktq_counters
{
    uint32  _logtimelo;
    uint32  _logtimehi;
    uint64  airtime;
}special_pktq_counters_t;

typedef struct special_sta_event
{
...
typedef struct  special_event_info
{
    struct      ether_addr ea;
    uint16      pad;
    uint8       idlerate_percent[SPECIAL_EVENT_CNT];
    int8        rssi[SPECIAL_EVENT_CNT];
    uint32      queue_utils[SPECIAL_EVENT_CNT];
}special_event_info_t;

typedef struct  special_all_event
{
    uint32      sta_num;
    uint32      datalen;
    struct      special_event_info special_event_info[MAX_SPECIAL_SCB];
}special_all_event_t;
...
struct wlc_info {
    ...
    /* SCB txrx_summary */
    wlc_txrx_summary_info_t *txrx_summary_handle;
    /* AMPDU */
    ...
    wme_param_ac_ip_t wme_ac_ip[NUM_ACIP];
#endif
    ...
    uint32        lifetime_txfifo;    /**< TX FIFO pkt lifetime in us units */
    ...
    uint32        apsd_usp_reset;        /**< number of apsd usp resets */
    ...
    */
    ...
    /* WL_PM_MUTE_TX */
    wlc_pm_mute_tx_t *pm_mute_tx;        /**< power-save mode with muted transmit path */
    ...
    bool        tcpack_fast_tx;        /**< Faster AMPDU release and Tx ucode for TCPACK */
    ...
    uint32        last_lpas_phy_wd_ms;    /**< Last time phy_wd was invoked in LPAS mode */
    ...
    bool        _dma_ct;        /**< true if BCM_DMA_CT (Cut Through DMA) feature is
                         * enabled.In future, plan is to have this cleared
                         * or set depending on MAC capability.
                         */
    ...
    wlc_tvpm_info_t    *tvpm;            /**< Thermal, Voltage, and Power Mitigation */
...
/* data_block_ps cunters */
    ...
    bool ul_ofdma_repeater_ac_param_mod;    /* Flag for AC_BK param workaround for repeater
                         * station's UL-OFDMA support. JIRA: BCAWLAN-223309
                         */
    ...
    uint8        rnr_update_method;    /* Method to create Reduced Neighbor report
                         ...
                         * scan = 1
                         */
    uint8        rnr_tbtt_len;        /* tbtt info field bytes to be present in RNR */
    ...
    wlc_td_extend_info_t *td_extend;
#endif
    bool        mclx_mode;        /* identifies MCLX cards for 2x2^2x2 mode */
    bool        mclx_mode_band_changed;    /* band changed, chains need updating */
    int        lpi_mode;
    uint8        check_short_ssid;
    /* Holds phy_noise of the current channel, read on channel switch */
    int8            phy_noise;

#ifdef GAME_SPEEDUP
    struct          special_scb *special_scb;
    struct          special_chanim  special_chanim;
    struct          wl_timer *special_timer;
    uint8           timer_windows;
    uint8           current_event_cnt;
    special_pktq_counters_t special_prec_cnt[PKTQ_MAX_PREC];
    bool            special_run;
    uint8           special_traffic;
    ratespec_t      special_fixrate;
#endif
#if defined(BCMDBG)
...
#define CHSPECBW_TO_RSPECBW(chspec_bw)    \
...
#define GETNEXTTXP(wlc, fifo)   wlc_bmac_dma_getnexttxp((wlc), (fifo), HNDDMA_RANGE_TRANSMITTED)
...
extern int wlc_validate_set_chanspec(wlc_info_t *wlc, chanspec_t chspec);
...
extern uint16 wlc_backhaul_bss_assocscb_getcnt(wlc_info_t *wlc, wlc_bsscfg_t *cfg);
#endif
...
extern void wlc_sendprobe_with_sh_ssid(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg,
...
extern void wlc_write_shm(wlc_info_t *wlc, uint byte_offset, uint16 v);
extern void wlc_update_shm(wlc_info_t *wlc, uint byte_offset, uint16 v, uint16 mask);
extern uint16 wlc_read_shm(wlc_info_t *wlc, uint byte_offset);
extern void wlc_set_shm(wlc_info_t *wlc, uint byte_offset, uint16 v, int len);
extern void wlc_copyto_shm(wlc_info_t *wlc, uint byte_offset, const void* buf, int len);
extern void wlc_copyfrom_shm(wlc_info_t *wlc, uint byte_offset, void* buf, int len);

#if defined(WL_PSMX)
extern uint16 wlc_read_shmx(wlc_info_t *wlc, uint byte_offset);
extern void wlc_write_shmx(wlc_info_t *wlc, uint byte_offset, uint16 v);
extern uint16 wlc_read_macregx(wlc_info_t *wlc, uint byte_offset);
extern void wlc_write_macregx(wlc_info_t *wlc, uint byte_offset, uint16 v);
#else
#define wlc_read_shmx(a, b) (((a) && (b)) ? 0 : 0)
#define wlc_write_shmx(a, b, c) ({BCM_REFERENCE(a); BCM_REFERENCE(b); BCM_REFERENCE(c);})
#define wlc_read_macregx(a, b) (((a) && (b)) ? 0 : 0)
#define wlc_write_macregx(a, b, c) ({BCM_REFERENCE(a); BCM_REFERENCE(b); BCM_REFERENCE(c);})
#endif /* WL_PSMX */

#if defined(WL_PSMR1)
extern uint16 wlc_read_shm1(wlc_info_t *wlc, uint byte_offset);
extern void wlc_write_shm1(wlc_info_t *wlc, uint byte_offset, uint16 v);
...
#else
#define wlc_read_shm1(a, b) (((a) && (b)) ? 0 : 0)
#define wlc_write_shm1(a, b, c) ({BCM_REFERENCE(a); BCM_REFERENCE(b); BCM_REFERENCE(c);})
#define wlc_read_macreg1(a, b) (((a) && (b)) ? 0 : 0)
#define wlc_write_macreg1(a, b, c) ({BCM_REFERENCE(a); BCM_REFERENCE(b); BCM_REFERENCE(c);})
#endif /* WL_PSMR1 */
...
extern void wlc_bss_update_beacon(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, bool check_mbssid);

extern uint8 wlc_get_apcfg_idx_used_for_upr_fd_frame(wlc_info_t *wlc);
...
extern bool wlc_prec_enq_head(wlc_info_t *wlc, struct pktq *q, void *pkt, int prec,
    bool head, int *evit_prec);
...
enum wlc_bandunit wlc_bandtype2bandunit_noassert(int bandtype);
...
enum wlc_bandunit wlc_chspecband2bandunit_noassert(uint16 chspec_band);
enum wlc_bandunit wlc_chspecband2bandunit(uint16 chspec_band);
...
#if defined(BCMDBG)
extern bool msg_mac_filter_ex(int msglvl, scb_t *scb);
extern bool msg_mac_filter_ex_pfx(int msglvl, scb_t *scb,
    msg_macfilter_pfx_t *msg_mac_filter_prefix, const char* function_name);
#endif /* BCMDBG */

#ifdef BCMPCIEDEV
extern int wlc_bus_scb_link_update(wlc_info_t *wlc, struct wlc_if *wlcif,
