 ...
 *
 * $Id: wlc_ampdu.c 801256 2021-07-19 18:27:17Z $
 */
...
wlc_ampdu_agg_pktc(void *ctx, struct scb *scb, void *p, uint prec)
{
    ...
    if (!(ini = scb_ampdu->ini[tid])) {
        ...
        if (!ini) {
            WL_AMPDU_TX(("%s: ini NULL -- returning\n", __FUNCTION__));
            goto txmod_ampdu;
        }
    }
    ...
    while (1) { /* loop over each packet in the caller supplied chain */
        ...
        { /* ! CFP_ENAB(wlc->pub) */
            ...
            if ((n == NULL) ||
                 ...
                 */
                ((n = wlc_amsdu_pktc_agg(ami, scb, p, n, tid)) == NULL) &&
#endif  /* WLAMSDU_TX */
                TRUE))) {
#if defined(BCM_PKTFWD_FLCTL)
...
#endif /* PROP_TXSTATUS */
            {
                uint8 extra_n = wlc_amsdu_agg_attempt(ami, scb, txq, p, tid);
#if defined(BCM_PKTFWD_FLCTL)
                if (extra_n) {
    ...
    uint16 seq, bool suppress_pkt)
{
    ...
    if (ini->tx_exp_seq != seq) {
        ...
        WL_AMPDU_ERR(("wl%d: r0hole: "MACF": tx_exp_seq 0x%x, seq 0x%x\n",
            ampdu_tx->wlc->pub->unit, ETHER_TO_MACF(ini->scb->ea),
            ini->tx_exp_seq, seq));
    ...
    scb_ampdu_tid_ini_t *ini, uint16 seq, bool suppress_pkt)
{
    ...
    if (txretry == 0) {
        if (ini->tx_exp_seq != seq) {
            ...
            /* pkts bypassed (or lagging) on way down */
            if (offset > SEQNUM_MAX / 2) {
                WL_AMPDU_ERR(("wl%d: rlag: "MACF": tx_exp_seq 0x%x, seq 0x%x\n",
                    ampdu_tx->wlc->pub->unit, ETHER_TO_MACF(ini->scb->ea),
                    ini->tx_exp_seq, seq));
                WLCNTINCR(ampdu_tx->cnt->txrlag);
                AMPDUSCBCNTINCR(scb_ampdu->cnt.txrlag);
...
#if defined(BCMDBG) || defined(BCMDBG_AMPDU)
        WLCNTCONDINCR(ampdu_tx->amdbg, ampdu_tx->amdbg->
            schan_depth_histo[MIN(AMPDU_MAX_MPDU, aggfifo_space)]);
#endif
        if (aggfifo_space == 0) {
            return BCME_BUSY;
        }
...
free_and_next:
    ...
    if (ampdu_dpdbg) {
        for (k = 0; k < rnum; k++) {
            WLCNTADD(ampdu_dpdbg->txmpdu_succ_per_ft[AMPDU_PPDU_FT(txs_mu, txs_mutype)],
                rs_txs.txsucc_cnt[k]);
        }
    }
#endif /* PKTQ_LOG */
    ...
    if (!was_acked) {
        ...
        case FT_VHT:
            ...
            bw = ((rs_txs.txrspec[k] & WL_RSPEC_BW_MASK) >> WL_RSPEC_BW_SHIFT) - 1;
            rateidx = (nss - 1) * MAX_VHT_RATES + mcs;
...
ampdu_cleanup_tid_ini(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 tid, bool force)
{
    ...
    __wlc_ampdu_update_link_credits(wlc, scb, ini->tid, n_pkts, TRUE);
#else
...
wlc_ampdu_recv_addba_resp(ampdu_tx_info_t *ampdu_tx, struct scb *scb, uint8 *body, int body_len)
{
    ...
    if ((ini == NULL) || !token_valid || (ini->ba_state == AMPDU_TID_STATE_BA_ON)) {
        WL_AMPDU_CTL(("wl%d: %s: "MACF": unsolicited packet, token 0x%x\n",
            wlc->pub->unit, __FUNCTION__, ETHER_TO_MACF(scb->ea), addba_resp->token));
        WLCNTINCR(ampdu_tx->cnt->rxunexp);
        return;
    }
    ...
    if ((status != DOT11_SC_SUCCESS) ||
        ...
        WL_AMPDU_ERR(("wl%d: %s: "MACF": Failed. status %d wsize %d policy %d tid %d\n",
            wlc->pub->unit, __FUNCTION__, ETHER_TO_MACF(scb->ea),
    ...
    WL_AMPDU_CTL(("wl%d: wlc_ampdu_recv_addba_resp: Turning BA ON: tid %d wsize %d "
        "token 0x%x\n",    wlc->pub->unit, tid, wsize, addba_resp->token));
...
wlc_cfp_ampdu_ps_send(wlc_info_t *wlc, struct scb *scb, uint32 tid_bmp, uint32 extra_flags)
{
    ...
    WL_PS_EX(scb, ("wl%d.%d: %s AID %d prio %d extra_flags 0x%x\n",
...
wlc_ampdu_taf_release(void* ampduh, void* scbh, void* tidh, bool force, taf_scheduler_public_t* taf)
{
...
#endif /* WLCFP */
    {
        if (taf->ias.is_ps_mode && qlen > 0) {
             ...
             * max packets in psq should be apsd_cnt to satisfy
             * the apsd trigger process.
             */
            if (apsd_cnt > 0) {
