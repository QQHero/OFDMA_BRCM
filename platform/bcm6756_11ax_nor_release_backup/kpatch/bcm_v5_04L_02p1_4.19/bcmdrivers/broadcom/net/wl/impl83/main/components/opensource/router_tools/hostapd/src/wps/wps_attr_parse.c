			...
			const u8 *pos, u16 len)
{
	...
	case ATTR_XMESH_ONLY_FLAG:
		if (!strcmp(pos, "tenda_xmesh_only")) {
			attr->xmesh_only = 1;
			wpa_printf(MSG_ERROR, "WPS: peer device is xmesh only");
		} else {
			wpa_printf(MSG_ERROR, "WPS: Invalid Xmesh Only Flag, length %u", len);
			return -1;
		}
		break;
#endif
