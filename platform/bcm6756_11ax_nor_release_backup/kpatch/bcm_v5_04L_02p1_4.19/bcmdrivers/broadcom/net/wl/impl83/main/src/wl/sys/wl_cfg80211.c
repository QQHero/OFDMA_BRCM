 ...
 *
 * $Id: wl_cfg80211.c 801707 2021-07-30 09:33:07Z $
 */
...
wl_validate_set_spp_amsdu_cap(struct net_device *dev, s32 bssidx, u8 cap)
{
    ...
    if (cap & ((RSN_CAP_SPPC | RSN_CAP_SPPR)  >> 8)) {
        ...
        spp_amsdu = cap & (RSN_CAP_SPPR  >> 8) ? 2 : 1;
    }
    ...
    s32 bssidx, bool update_ssid)
{
    ...
    } else if (bss->wps_ie == NULL && ies->wps_ie_len > 0) {
#else
    ...
    struct cfg80211_beacon_data *info)
{
    ...
    if ((ssid_ie = bcm_find_tlv(&info->head[ie_offset],
        ...
        if (dev_role == NL80211_IFTYPE_AP) {
            ...
            if (!hidden_ssid &&
#ifdef CONFIG_TENDA_PRIVATE_WLAN
            /* 隐藏网络开启时，使能WPS，走到这里时ssid_ie->len为0，
               将进入此条件分支，清空cfg->hostapd_ssid.SSID，且
               update_ssid = true, 下面执行wl_cfg80211_hostapd_sec
               会down、up bss, 导致客户端掉线。况且ssid_ie->len为0时
               走此分支明显也是不正常的，因此加上(ssid_ie->len > 0)条件.
            */
                (ssid_ie->len > 0) &&
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
    kfree(wdev->wiphy->bands[IEEE80211_BAND_2GHZ]->iftype_data);
    wdev->wiphy->bands[IEEE80211_BAND_2GHZ]->iftype_data = NULL;
#endif /* KERNEL >= 4.19 */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
    kfree(wdev->wiphy->bands[IEEE80211_BAND_5GHZ]->iftype_data);
    wdev->wiphy->bands[IEEE80211_BAND_5GHZ]->iftype_data = NULL;
#endif /* KERNEL >= 4.19 */
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4, 19, 0)
    kfree(wdev->wiphy->bands[IEEE80211_BAND_6GHZ]->iftype_data);
    wdev->wiphy->bands[IEEE80211_BAND_6GHZ]->iftype_data = NULL;
#endif /* KERNEL >= 4.19 */
