 ...
 *
 * $Id: wlc_ap.c 801520 2021-07-26 10:47:38Z $
 */
...
enum wlc_ap_iov {
    ...
    IOV_PREV_RXFRAME = 69,
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && defined(CONFIG_TENDA_WLAN_AUTOHIDE_SSID)
    IOV_AUTO_HIDE = 70,        /**< The SSID is automatically hidden */
...
#ifdef CONFIG_TENDA_WLAN_RSSI_LIMIT
static void wlc_ap_rssi_limit(wlc_ap_info_t *ap);
#endif
...
#ifdef CONFIG_TENDA_WLAN_RSSI_LIMIT
static void 
wlc_ap_rssi_limit(wlc_ap_info_t *ap)
{
    wlc_ap_info_pvt_t *appvt = (wlc_ap_info_pvt_t *) ap;
    wlc_info_t *wlc = appvt->wlc;
    struct scb *scb;
    ...
    FOREACHSCB(wlc->scbstate, &scbiter, scb) {
        ...
        if (!SCB_ASSOCIATED(scb) || !SCB_AUTHENTICATED(scb)) {
            continue;
        }

        scb_rssi = wlc_lq_rssi_ma(wlc, cfg, scb);
        if (scb_rssi && scb_rssi < cfg->far_sta_rssi) {
            wlc_scb_set_auth(wlc, cfg, scb, FALSE, AUTHENTICATED,
                DOT11_RC_UNSPECIFIED);
        }
    }
...
wlc_ap_process_assocreq_decision(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, assoc_decision_t *dc)
{
    ...
    if (scb->state & PENDING_ASSOC) {
        ...
        if ((param->status == DOT11_SC_SUCCESS) &&
            ...
            ((scb->WPA_auth == WPA3_AUTH_SAE_FBT) && (scb->pmkid_included)))) {
...
wlc_ap_update_static_rnr_nbr_info(wlc_info_t* wlc, wlc_bsscfg_t *bsscfg)
{
    ...
    wlc_rrm_fill_static_rnr_info(wlc, bsscfg, bsscfg->rnr_nbr_ap_info,
        &bsscfg->rnr_nbr_ap_info_size, FALSE);
...
wlc_write_rnr_element(wlc_bsscfg_t *bsscfg, uint8 *cp, wlc_iem_build_data_t *data)
{
    ...
    if (data->cbparm && data->cbparm->ft && data->cbparm->ft->bcn.ssid) {
        ...
        if ((ssid->SSID_len == cfg->SSID_len) &&
            ...
            wlc_rrm_fill_static_rnr_info(bsscfg->wlc, bsscfg, nbr_ap_info, &bytes_rd,
                TRUE);
