...
#if CONFIG_TENDA_GAME_SPEEDUP
    setbit(app->bitvec, WLC_E_SPECIAL_STA_EVENT);
    setbit(app->bitvec, WLC_E_SPECIAL_STA_ALL_EVENT);
#endif

	if (str = nvram_get("mevent_evts"))
	{
		result = strtok(str, ",");
		while (result != NULL)
		{
			setbit(app->bitvec, strtol(result, NULL, 0));
			result = strtok(NULL, ",");
		}
	}
