 ...
 *
 * $Id: wlc_assoc.c 801229 2021-07-19 11:49:27Z $
 */
...
void wlc_join_BSS_post_ch_switch(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi,
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
        WL_ASSOC(("DWDS_ENAB %d assoc->type %d\n", DWDS_ENAB(cfg), cfg->assoc->type));
        if (DWDS_ENAB(cfg) && AS_ROAM == cfg->assoc->type) {
...
#endif /* MFP */
#ifdef WL_SPP_AMSDU
    /* check for SPP_AMSDU capabilities mismatch */
    if ((!BSSCFG_IS_RSN_CAP_SPPC(cfg) && IS_RSN_CAP_SPPR(bi)) ||
        (BSSCFG_IS_RSN_CAP_SPPR(cfg) && (!IS_RSN_CAP_SPPC(bi)))) {

        WL_ASSOC(("wl%d: %s: BSSID %s pruned, SPP AMSDU capability mismatch\n",
            WLCWLUNIT(wlc), __FUNCTION__, bcm_ether_ntoa(&bi->BSSID, eabuf)));
        return prune;
    }
#endif /* WL_SPP_AMSDU */
 ...
 */
void wlc_join_BSS_post_ch_switch(wlc_bsscfg_t *cfg, wlc_bss_info_t *bi, bool ch_changed,
           chanspec_t chanspec)
{
    wlc_info_t *wlc = cfg->wlc;
    wlc_assoc_t *as = cfg->assoc;
...
wlc_assoc_success(wlc_bsscfg_t *cfg, struct scb *scb)
{
    ...
    if (BSSCFG_IS_RSN_CAP_SPPC(cfg) &&
        IS_RSN_CAP_SPPC(cfg->target_bss)) {
        scb->flags |= SCB_AMSDU_SPP;
    }
#endif /* WL_SPP_AMSDU */
    /* clear PM state */
