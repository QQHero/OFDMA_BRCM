 ...
 *
 * $Id: wlc_rx.c 801491 2021-07-23 23:50:18Z $
 *
 */
#include <wlc_cfg.h>
#include <typedefs.h>
...
#endif /* WLTDLS */
            {
                WL_PS_EX(scb, ("wl%d.%d:%s(): call wlc_apps_apsd_trigger().\n",
                    pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
                WL_TRACE(("wl%d.%d:%s(): call wlc_apps_apsd_trigger().\n",
                    pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
                wlc_apps_apsd_trigger(wlc, scb, f.ac);
...
pktfetch:
    ...
    if (sendup_80211) {
        ...
        if (ether_type == ETHER_TYPE_802_1X) {
            WL_ASSOC(("wl%d: %s RX 8021X frame\n", pub->unit, __FUNCTION__));
        }
#endif /* STA */
...
#endif /* BCM_CEVENT */
    if (ether_type == ETHER_TYPE_802_1X) {
        WL_ASSOC_LT(("rx:802.1x from "MACF"\n", ETHER_TO_MACF(scb->ea)));
    }
...
wlc_recvdata_sendup_msdus(wlc_info_t *wlc, struct scb *scb, struct wlc_frminfo *f)
{
    ...
    if (WET_ENAB(wlc) && BSSCFG_STA(bsscfg) && !SCB_DWDS(scb) &&
        (eacmp((struct ether_addr *)eh->ether_shost, &bsscfg->cur_etheraddr) == 0)) {
        PKTFREE(osh, p, TRUE);
        return;
    }
#endif /* WET */
...
wlc_recv_mgmt_ctl(wlc_info_t *wlc, osl_t *osh, wlc_d11rxhdr_t *wrxh, void *p)
{
    ...
    case FC_AUTH:
        ...
        /* Determine whether the signal is below the threshold during association */
        if (cfg) {
            if (wrxh->rssi < cfg->far_sta_rssi) {
                break;
            }
        }
#endif
        ...
        /* TODO: 增加开关，防止steer未配置时auth失败 */
            if(maps_auth_handler((unsigned char *)&hdr->sa.octet, wrxh->rssi, bsscfg)){
    ...
    case FC_PROBE_REQ:
...
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
       {
            ...
            if(maps_probe_req_handler((unsigned char *)&hdr->sa.octet, wrxh->rssi, wlc->primary_bsscfg, 
...
#if defined(TD_STEER) && defined(CONFIG_TENDA_PRIVATE_WLAN)
        maps_disassoc_handler(scb, bsscfg);
#endif
#if defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
...
#ifdef CONFIG_TENDA_PRIVATE_WIFI2
    if (BAND_6G(wlc->band->bandtype)) {
        snprintf(ifname, IFNAMSIZ, "wlan3");
    }
#endif
...
#if defined(AI_MESH_SUPPORT) && defined(CONFIG_TENDA_PRIVATE_WLAN)
    wlc_bss_info_t scan_info;
    wlc_recv_scan_parse_bcn_prb(wlc, wrxh, &hdr->bssid, TRUE, body,
                    body_len, &scan_info);
    if (body) {
        scan_info.bcn_prb = (struct dot11_bcn_prb *) body;
        scan_info.bcn_prb_len = body_len;
        if (ai_mesh_rx_beacon((unsigned char *)&hdr->sa, &scan_info, sizeof(wlc_bss_info_t))) {
            //break;
        }
    }
#endif
    ...
    while (tags_len >= TLV_HDR_LEN &&
        ...
        case DOT11_MNG_HE_OP_ID:
            ...
            if (BAND_6G(bcn_bandtype)) {
                wlc_he_op_ie_read_optional_fields(wlc, he_op_ie_p, &bi);
                if (bi.chanspec != INVCHANSPEC) {
                    bcn_channel = wf_chspec_ctlchan(bi.chanspec);
                }
            }
#endif /* STA */
            break;
