 ...
 *
 * $Id: wlc_scb_ratesel.c 807766 2022-01-31 11:03:52Z $
 */
...
wlc_scb_ratesel_init_rcb_itxs(wlc_ratesel_info_t *wrsi, struct scb *scb, uint8 ac)
{
    ...
    /* Skip ratesel init for scbs going through delete */
    if (!scb || SCB_DEL_IN_PROGRESS(scb)) {
        return;
    }

    rcb_itxs = SCB_RATESEL_ITXS_CUBBY(wrsi, scb, ac);
    ASSERT(rcb_itxs);
    ...
    ratesel_txs_t *rs_txs, tx_status_t *txs, uint16 flags)
{
    ...
    /* Skip ratesel update for scbs going through delete */
    if (!scb || !rcb_itxs || SCB_DEL_IN_PROGRESS(scb)) {
        return;
    }
...
/* Get the ulrt txr */
uint16
wlc_scb_ratesel_get_ulrt_txr(wlc_ratesel_info_t *wrsi, struct scb *scb, uint8 rtidx)
{
    rcb_t *state;

    state = wlc_scb_ratesel_get_cubby(wrsi, scb, 0);
    ASSERT(state);
    return wlc_ratesel_get_ulrt_txr(state, rtidx);
}
#if defined(BCMDBG)
void
wlc_scb_ratesel_ulrt_dump_clr(wlc_ratesel_info_t *wrsi, struct scb *scb)
{
    rcb_t *state;

    state = wlc_scb_ratesel_get_cubby(wrsi, scb, 0);
    ASSERT(state);
    wlc_ratesel_ulrt_dump_clr(state);
}
#endif
#endif /* WL_ULRT_DRVR */
...
wlc_scb_ratesel_link_bw_upd(wlc_info_t *wlc, struct scb *scb)
{
    ...
    uint8 bw = BW_20MHZ;
#ifdef WL11AC
    uint8 scb_oper_mode_bw = 0; /* init to invalid nr; BW_20MHZ == 1 */
#endif
    chanspec_t chanspec = wlc->chanspec;

#ifdef CONFIG_TENDA_PRIVATE_KM
    /* 这里取rcb的时候可能已经被deinit给置为NULL了，需要先判空 */
    if (!rcb) {
        return (bw);
    }
#endif
     ...
     */
    if (SCB_HE_CAP(scb) && wlc_he_received_valid_omi(wlc->hei, scb)) {
        scb_oper_mode_bw = wlc_he_get_omi_bw(wlc->hei, scb);
    } else if ((SCB_HE_CAP(scb) || SCB_HT_CAP(scb) || SCB_VHT_CAP(scb)) &&
    ...
    bw = (scb_oper_mode_bw > 0 && scb_oper_mode_bw < bw) ? scb_oper_mode_bw : bw;
#endif /* WL11AC */

    wlc_ratesel_set_link_bw(rcb, bw);

#ifdef WL_ULMU
    /* invoke bw change callback to ulmu */
    wlc_ulmu_scb_bw_upd(wlc, scb, bw);
#endif
...
wlc_scb_ratesel_init(wlc_info_t *wlc, struct scb *scb)
{
    ...
    rs_init.bw_auto_dl = TRUE;
    rs_init.bw_auto_ul = FALSE;
#ifdef WL11AX
    /* bw_auto is forced off if DLOFDMA is on for the given STA */
    rs_init.bw_auto_ul = SCB_ULOFDMA_ADM(scb) ? FALSE : rs_init.bw_auto_ul;
    WL_RATE(("%s bw_auto_dl %d bw_auto_ul %d dl_ofdma policy 0x%x "
        "ul_ofdma policy 0x%x\n", __FUNCTION__, rs_init.bw_auto_dl,
         rs_init.bw_auto_ul, SCB_DLOFDMA_ADM(scb), SCB_ULOFDMA_ADM(scb)));
#endif /* WL11AX */
    ...
    for (ac = 0; ac < WME_MAX_AC(wlc, scb); ac++) {
        ...
        wlc_ratesel_init_params_sync(rs_init.state, &rs_init);
        ...
        if (BSS_N_ENAB(wlc, scb->bsscfg)) {
            if (((WLC_STF_GET_SCB_MIMOPS_ENAB(wlc->stf, scb) &&
                ...
                new_rateset.mcs[2] = 0;
                new_rateset.mcs[3] = 0;
            } else if (wlc->stf->op_txstreams == 3) {
                new_rateset.mcs[3] = 0;
            }
        }
...
/* API to get max operating BW for UL MU Tx
 * It returns link_bw except for following case: For a client that is operating
 * on 160MHz, but does not support UL 2x996 RU, treat it as 80MHz
 */
uint8
wlc_scb_ratesel_get_ulmu_link_bw(wlc_info_t *wlc, struct scb *scb)
{
    uint8 link_bw;

    link_bw = wlc_scb_ratesel_get_link_bw(wlc, scb);

    if (link_bw == BW_160MHZ && !SCB_HE_UL2x996_CAP(wlc->hei, scb)) {
        link_bw = BW_80MHZ;
    }
