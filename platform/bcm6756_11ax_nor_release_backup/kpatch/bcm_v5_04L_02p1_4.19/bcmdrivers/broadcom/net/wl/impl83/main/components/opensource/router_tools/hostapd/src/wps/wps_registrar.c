...
static int wps_get_ssid_passphrase_from_conf(const char *key, char *value, int max_len)
{
    FILE *f;
    char buf[4096], *pos;
    u8 found = 0;
    #define XMESH_HIDEN_AP_CONFIG_FILE "/var/run/hostapd-wl1.3.conf"

    if (strcmp(key, "ssid") && strcmp(key, "wpa_passphrase")) {
        wpa_printf(MSG_ERROR,"WPS: read from conf, %s not support !\n", key);
        return -1;
    }

    f = fopen(XMESH_HIDEN_AP_CONFIG_FILE, "r");
    if(NULL == f) {
        wpa_printf(MSG_ERROR,"WPS: read from conf, %s fopen failed !\n", XMESH_HIDEN_AP_CONFIG_FILE);
        return -1;
    }
    ...
    while (fgets(buf, sizeof(buf), f)) {
        ...
        pos = os_strchr(buf, '=');
        if (pos == NULL) {
            wpa_printf(MSG_ERROR, "WPS: invalid line '%s'", buf);
            continue;
        }
        *pos = '\0';
        pos++;

        if (!os_strcmp(buf, key)) {
            if (strlen(pos) > max_len) {
                wpa_printf(MSG_ERROR, "WPS: key value too long error\n");
                break;
            }
            strcpy(value, pos);
            wpa_printf(MSG_ERROR, "WPS: found key=%s:value=%s\n", key, value);
            found = 1;
            break;
        }
    }
...
#ifdef CONFIG_TD_MESH_V3
    if (wps->peer_dev.xmesh_only) {
        if (wps_get_ssid_passphrase_from_conf("ssid", ssid, SSID_MAX_LEN) || wps_get_ssid_passphrase_from_conf("wpa_passphrase", key, KEY_MAX_LEN)) {
            wpa_printf(MSG_ERROR, "WPS :  get ssid or passphase from conf fail !\n");
            return -1;
        }
        strcpy(wps->cred.ssid, ssid);
        wps->cred.ssid_len = strlen(ssid);
        strcpy(wps->cred.key, key);
        wps->cred.key_len = strlen(key);
    }
#endif
...
#ifdef CONFIG_TD_MESH_V3
    wps_process_xmesh_only_m1(&wps->peer_dev, attr->xmesh_only);
#endif
    wps->state = SEND_M2;
    return WPS_CONTINUE;
}
