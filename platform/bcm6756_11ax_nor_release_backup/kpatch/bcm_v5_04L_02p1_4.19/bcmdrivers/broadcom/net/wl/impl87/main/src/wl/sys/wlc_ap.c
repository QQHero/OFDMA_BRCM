...
#ifdef CONFIG_UGW_BCM_DONGLE
                wlc_mac_event(wlc, WLC_E_TENDA_KM, &hdr->sa, KM_WIFIEV_STA_MAXCNTREJ, 0, 0, NULL, 0);
#else
                if (km_eventscenter_wifiev_sta_maxcntrej_handler) {
                    km_eventscenter_wifiev_sta_maxcntrej_handler(hdr->sa.octet);
                }
#endif
            }
        }
...
#ifdef CONFIG_TENDA_PRIVATE_KM
#ifdef CONFIG_UGW_BCM_DONGLE
                wlc_mac_event(wlc, WLC_E_TENDA_KM, &hdr->sa, KM_WIFIEV_STA_BLACKREJ, 0, 0, NULL, 0);
#else
                if (km_eventscenter_wifiev_sta_blackrej_handler) {
                    km_eventscenter_wifiev_sta_blackrej_handler(hdr->sa.octet);
                }
#endif
#endif
                break;
