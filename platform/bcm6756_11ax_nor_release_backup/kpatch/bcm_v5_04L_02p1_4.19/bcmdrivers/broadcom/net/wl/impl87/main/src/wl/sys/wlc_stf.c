 ...
 *
 * $Id: wlc_stf.c 810597 2022-04-11 12:54:02Z $
 */
...
wlc_stf_txcore_set(wlc_info_t *wlc, uint8 idx, uint8 core_mask)
{
    ...
    if ((BAND_5G6G(wlc->band->bandtype) && idx == OFDM_IDX) ||
        ...
        /* In 2G, check for 11b or 11a (11g-only mode) */
        ((idx == CCK_IDX && RSPEC_ISCCK(wlc->bcn_rspec)) ||
        (idx == OFDM_IDX && RSPEC_ISOFDM(wlc->bcn_rspec))))) {
...
wlc_stf_txchain_get(wlc_info_t *wlc, ratespec_t rspec)
{
    ...
    if (WLC_PHY_AS_80P80(wlc, wlc->chanspec)) {
        if (chains & 1) {
                    WL_ERROR(("wl%d: INVALID rspec for BW80p80\n", wlc->pub->unit));
            ASSERT(0);
        } else {
            chains = chains >> 1;
        }
...
wlc_stf_txcore_shmem_write(wlc_info_t *wlc, bool forcewr)
{
    ...
    if (wlc->stf->shmem_base != wlc->pub->m_coremask_blk &&
        ...
        WL_ERROR(("BAD shmem base address"));
        ASSERT(0);
        return;
    }
...
typedef struct wlc_bw_translation {
    uint rspec_bw;        /* WL_RSPEC_BW_20MHZ */
    uint tx_bw;        /* WL_TX_BW_20 */
} wlc_bw_translation_t;
    ...
    wl_tx_chains_t txbf_chains)
{
    ...
    wl_tx_mode_t mode;
    wl_tx_nss_t nss = wlc_ratespec_nss(rspec);
    wl_tx_nss_t rspec_nss = nss;
    wl_tx_chains_t chains = wlc_stf_txchain_get(wlc, rspec);
    uint i, j, bw_idx, rspec_bw;
    wl_tx_bw_t tx_bw;
    const wlc_bw_translation_t bw_translation[BW160_IDX + 1] = {
        {WL_RSPEC_BW_20MHZ, WL_TX_BW_20},
        {WL_RSPEC_BW_40MHZ, WL_TX_BW_40},
        {WL_RSPEC_BW_80MHZ, WL_TX_BW_80},
        {WL_RSPEC_BW_160MHZ, WL_TX_BW_160},
    };
    ...
    rspec_bw = RSPEC_BW(rspec);
    tx_bw = ppr_get_ch_bw(txpwr);

    for (i = BW20_IDX; i <= BW160_IDX; i++) {
        if (bw_translation[i].rspec_bw == rspec_bw)
            break;
    }
    ASSERT(i <= BW160_IDX);

    for (j = BW20_IDX; j <= BW160_IDX; j++) {
        if (bw_translation[j].tx_bw == tx_bw)
            break;
    }
    ASSERT(j <= BW160_IDX);

    /* Choose the BW supported by both rspec and stf->txpwr_ctl */
    bw_idx = MIN(i, j);

    if (RSPEC_ISHE(rspec)) {
        for (i = 0; i <= bw_idx; i++) {
            if (txbf_chains > 1 && txbf_chains >= (int)rspec_nss) {
                /* Fill up txpwr for txbf ON */
                ppr_get_he_mcs(txpwr, mcs_pwr_bw_table[i], rspec_nss,
                    WL_TX_MODE_TXBF, txbf_chains, &he_mcs_pwrs, WL_HE_RT_SU);
        ...
        for (i = 0; i <= bw_idx; i++) {
            if (txbf_chains > 1 && txbf_chains >= (int)rspec_nss) {
                /* Fill up txpwr for txbf ON */
                ppr_get_vht_mcs(txpwr, mcs_pwr_bw_table[i], rspec_nss,
                    WL_TX_MODE_TXBF, txbf_chains, &mcs_pwrs);
...
static void
wlc_stf_pproff_shmem_write_ge128(wlc_info_t *wlc) /* C_CHECK */
{
    uint8 idx, rate, txchain, bw_idx;
    ...
    chanspec_bw = (wlc->chanspec & WL_CHANSPEC_BW_MASK) >> WL_CHANSPEC_BW_SHIFT;
    chanspec_bw -= (WL_CHANSPEC_BW_20 >> WL_CHANSPEC_BW_SHIFT);
    /* CCK rates */

    memset(&stf_rs, 0, sizeof(stf_rs));
    for (bw_idx = BW20_IDX; bw_idx <= chanspec_bw; bw_idx++) {
        memset(&dsss_pwrs, 0, sizeof(dsss_pwrs));
        memset(&ofdm_pwrs, 0, sizeof(ofdm_pwrs));

        cck_ptr = (uint8*)dsss_pwrs.pwr;
        txchain = (uint8)WLC_BITSCNT(wlc->stf->txcore[CCK_IDX][1]);
        if (is_8080)
            bw = WL_TX_BW_20IN8080;
        else if (bw_idx == BW160_IDX)
            bw = WL_TX_BW_20IN160;
        else if (bw_idx == BW80_IDX)
            bw = WL_TX_BW_20IN80;
        else if (bw_idx == BW40_IDX)
            bw = WL_TX_BW_20IN40;
        else
            bw = WL_TX_BW_20;

        if (txchain >= WL_TX_CHAINS_1 && txchain <= WL_TX_CHAINS_4) {
            ppr_get_dsss(wlc->stf->txpwr_ctl, bw, txchain, &dsss_pwrs);
        }

        ofdm_ptr = (uint8*)ofdm_pwrs.pwr;
        txchain = (uint8)WLC_BITSCNT(wlc->stf->txcore[OFDM_IDX][1]);
        ...
        if (is_8080)
            ...
            bw = WL_TX_BW_20;
        if (txchain == WL_TX_CHAINS_1) {
            ppr_get_ofdm(wlc->stf->txpwr_ctl, bw, WL_TX_MODE_NONE, txchain, &ofdm_pwrs);
        } else if (txchain >= WL_TX_CHAINS_2 && txchain <= WL_TX_CHAINS_4) {
            ppr_get_ofdm(wlc->stf->txpwr_ctl, bw, WL_TX_MODE_CDD, txchain, &ofdm_pwrs);
        }
...
wlc_stf_pproff_shmem_write(wlc_info_t *wlc) /* C_CHECK */
{
    ...
    if (D11REV_GE(wlc->pub->corerev, 128)) {
        return wlc_stf_pproff_shmem_write_ge128(wlc);
    }
...
wlc_stf_compute_ack_pprs(wlc_info_t *wlc)
{
    ...
    bzero(dsss_pwrs1, sizeof(dsss_pwrs1));
    bzero(ofdm_pwrs1, sizeof(ofdm_pwrs1));
...
wlc_stf_set_optxrxstreams(wlc_info_t *wlc, uint8 new_streams)
{
     ...
     */
    if (current_streams > new_streams && BSSCFG_AP(wlc->primary_bsscfg)) {
        if (TXPKTPENDTOT(wlc)) {
            wlc_block_datafifo(wlc, DATA_BLOCK_TXCHAIN, DATA_BLOCK_TXCHAIN);
