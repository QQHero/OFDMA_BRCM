 ...
 *
 * $Id: wlu.c 801227 2021-07-19 11:40:32Z $
 */
#include <typedefs.h>
#include <wlioctl.h>
...
wl_join(void *wl, cmd_t *cmd, char **argv)
{
    ...
    if (!stricmp(*argv, "-x")) {
        ...
        join_params->ssid.SSID_len = sizeof(short_ssid);
        join_params->ssid_type = WL_SCAN_SSIDFLAGS_SHORT_SSID;

        memcpy(join_params->ssid.SSID, &short_ssid, join_params->ssid.SSID_len);

        /* Include ssid_type into the ioctl buffer to parse in driver */
        join_params_size = sizeof(wlc_ssid_t) + sizeof(join_params->ssid_type);
...
wl_vndr_ie(void *wl, const char *command, uint32 pktflag_ok, char **argv)
{
    ...
    err = wl_mk_ie_setbuf(wl, command, pktflag_ok, argv, &ie_setbuf, &buflen);
    if (err)
        return err;
