...
{
    char *wan_ifnames[] = {"eth0","ppp1","eth0.1","eth4.1","eth2.1"};
    int i = 0;
    if(!ifname)
        return 0;
    for(i = 0; i < ARRAY_SIZE(wan_ifnames); i++)
    {
...
int inet6_addr_del_global(struct net_device *dev)
{
    struct inet6_ifaddr *ifp;
    struct inet6_ifaddr *ifn;
    struct inet6_dev *idev;
...
void addrconf_prefix_rcv(struct net_device *dev, u8 *opt, int len, bool sllao)
{
    ...
    if (pinfo->autoconf && in6_dev->cnf.autoconf) {
        ...
        if (pinfo->prefix_len >= 17 && pinfo->prefix_len <= 64) {
