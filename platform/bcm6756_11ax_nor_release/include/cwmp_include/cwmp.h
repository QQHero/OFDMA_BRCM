...
#include "event.h"
#include "queue.h"
#include "ugw_platform.h"


#ifndef CWMP_GDDX_GENERIC_VERSION
#define CWMP_GDDX_GENERIC_VERSION
#endif
...
#ifdef CWMP_SOAP_LOWERCASE
#define SOAP_ENV_DEFAULT        "soapenv"
#define SOAP_ENC_DEFAULT        "soapenc"
#else
...
#define CWMP_INFORM_EVENT_CODE_18  "X CT-COM BIND1"
...
#define NOTIFICATIONOFF_STR                 "0"
#define PASSIVE_NOTIFICATION_STR            "1"
#define ACTIVE_NOTIFICATION_STR             "2"

#define DEFAULT_NOTIFY_NUM                  32

typedef struct CWMP_NOTIFY_T
{
    char                name[UGW_TMP_STR_LEN];
    char                value[UGW_TMP_STR_LEN];
    int                 notify_en;
}CWMP_NOTIFY_T_STRU;

extern CWMP_NOTIFY_T_STRU DEFAULT_NOTIFY_LIST[];

typedef struct CWMP_NOTIFY_PARM_T
{
    CWMP_NOTIFY_T_STRU parameter;
    CWMP_NOTIFY_T_STRU *next;
}CWMP_NOTIFY_PARM_T_STRU;

extern CWMP_NOTIFY_PARM_T_STRU *g_notify_parm_list;
...
struct parameter_node_st
{
    ...
    parameter_notifyset_handler_pt     notify_set;
    parameter_notifyget_handler_pt     notify_get;
...
struct loadst
{
    char interface[UGW_TMP_STR_LEN+1];
    char fromurl[UGW_URL_LEN+1];
    unsigned int dscp ;
...
extern int g_connecttype_ppp_flag;
extern int g_ctcom_event_bootstrap_flag;
...
parameter_t* cwmp_create_parameternotify(env_t * env , const char * name, const void *  value, int type);
...
xmldoc_t* cwmp_create_inform_message(env_t * env ,  header_t * header,
        ...
        parameter_list_t * pl);
xmldoc_t * cwmp_create_getparameterattribute_response_message(env_t * env ,
        header_t * header,
        parameter_list_t * pl);
...
xmldoc_t *  cwmp_create_getparametervalues_response_message(env_t * env ,
    header_t * header,
    parameter_list_t * pl);
xmldoc_t* cwmp_create_getparametervalues_response_message_tr098(env_t * env ,  header_t * header, parameter_list_t * pl ,parameter_node_t * param_node,char *path_name);
...
int     cwmp_parse_getparametervalues_message_tr098(env_t * env , xmldoc_t * doc, parameter_node_t * root, parameter_list_t ** ppl, fault_code_t *fault,parameter_node_t ** node,char ** path);
