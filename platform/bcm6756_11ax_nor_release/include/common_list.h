...
static __inline void __list_add(struct list_head *node,
 ...
 */
static __inline void list_add_tail(struct list_head *node, struct list_head *head)
{
...
#define list_for_each_entry_safe(pos, n, head, member) \
    for (pos = list_first_entry(head, typeof(*pos), member); \
         &pos->member != (head) && (n = list_next_entry(pos, member), 1); \
         pos = n)
...
#define list_for_each_entry_safe_reverse(pos, n, head, member) \
    for (pos = list_last_entry(head, typeof(*pos), member); \
         &pos->member != (head) && (n = list_prev_entry(pos, member), 1); \
         pos = n)
...
static __inline void hlist_insert_behind(struct hlist_node *n,
...
{
    hlist_insert_behind(n, &prev->next);
}
