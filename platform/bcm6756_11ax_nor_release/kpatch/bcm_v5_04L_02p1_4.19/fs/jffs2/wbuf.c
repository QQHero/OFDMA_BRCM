...
static int __jffs2_flush_wbuf(struct jffs2_sb_info *c, int pad)
{
    ...
    */
...

    {
            ...
            mtd_unlock(c->mtd, c->wbuf_ofs, c->wbuf_pagesize);
#endif
            ret = mtd_write(c->mtd, c->wbuf_ofs, c->wbuf_pagesize,
                    &retlen, c->wbuf);
#if defined(CONFIG_FLASH_LOCK_SUPPORT)
            mtd_lock(c->mtd, c->wbuf_ofs, c->wbuf_pagesize);
#endif
    }
...
#if defined(CONFIG_FLASH_LOCK_SUPPORT)
    mtd_unlock(c->mtd, jeb->offset, ops.ooblen);
#endif
        ret = mtd_write_oob(c->mtd, jeb->offset, &ops);
#if defined(CONFIG_FLASH_LOCK_SUPPORT)
    mtd_lock(c->mtd, jeb->offset, ops.ooblen);
#endif
