 ...
 *
 * $Id: wlc_qoscfg.c 803989 2021-10-14 19:10:00Z $
 */
...
#endif
#ifdef CONFIG_TENDA_GAME_SPEEDUP
    {"wme_ac_ip", IOV_WME_AC_IP, 0, 0, IOVT_BUFFER, sizeof(wme_param_ac_ip_t)},
#endif
    {NULL, 0, 0, 0, 0, 0}
};
...
wlc_edcf_acp_set_hw(wlc_info_t *wlc, wlc_bsscfg_t *cfg, edcf_acparam_t *acp)
{
    ...
    if (aci == AC_BK && wlc->ul_ofdma_repeater_ac_param_mod == TRUE) {
        ...
        * of AC_BE.
        */
        acp = &cfg->wme->wme_param_ie.acparam[AC_BE];
    }
...
#ifdef CONFIG_TENDA_GAME_SPEEDUP
    case IOV_SVAL(IOV_WME_AC_IP):
        if (WME_ENAB(wlc->pub)) {
            wme_param_ac_ip_t *prms = (wme_param_ac_ip_t *)arg;
            wme_param_ac_ip_t *wme = wlc->wme_ac_ip;
            int i = 0,itemid = 0;
            bool del = FALSE;
            ...
            if(prms->action == 2) {
                memset(wme,0,sizeof(wme_param_ac_ip_t)*NUM_ACIP);
                break;
            }
           
            WL_WNM(("%s(): set IOV_WME_AC_IP action:%d taf_id: %d pri %d \n",
            __FUNCTION__,
            prms->action,
            prms->stream_id,
            prms->priority));
            
            switch (prms->stream_id) {
                case IP_ID:{
                    WL_WNM(("%s(): set IOV_WME_AC_IP 0x%8x:%u protocol %d pri %d \n",
                        __FUNCTION__,
                        prms->stream.ip_info.srcip,
                        prms->stream.ip_info.srcport,
                        prms->stream.ip_info.protocol,
                        prms->priority));

                    for(i = 0;i < NUM_ACIP; i++) {
                        if(wme[i].action == FALSE) {
                            itemid =i;
                            break;
                        }
                    }
                    ...
                    for(i = 0;i < NUM_ACIP; i++) {
                        if(wme[i].action == TRUE) {
                            if(wme[i].stream.ip_info.dstip == prms->stream.ip_info.dstip) {
                                    if(prms->action == 0) {
                                        wlc->wme_ac_ip[i].stream.ip_info.srcip = 0;
                                        wlc->wme_ac_ip[i].stream.ip_info.srcport = 0;
                                        wlc->wme_ac_ip[i].priority = 0;
                                        wlc->wme_ac_ip[i].stream.ip_info.dstip = 0;
                                        wlc->wme_ac_ip[i].stream.ip_info.dstport = 0;
                                        wlc->wme_ac_ip[i].stream.ip_info.protocol = 0;
                                        wlc->wme_ac_ip[i].action = FALSE;
                                        del = TRUE;
                                    }
                                    if(wme[i].stream.ip_info.srcip == prms->stream.ip_info.srcip) {
                                        itemid = i;
                                    }
                                }
                        }
                    ...
                    WL_WNM(("%s %d itemid=%d del %d\n",__func__,__LINE__,itemid,del));
                    
                    if(!del)
                    {
                        wlc->wme_ac_ip[itemid].stream.ip_info.srcip = prms->stream.ip_info.srcip;
                        wlc->wme_ac_ip[itemid].stream.ip_info.srcport = prms->stream.ip_info.srcport;
                        wlc->wme_ac_ip[itemid].priority = prms->priority;
                        wlc->wme_ac_ip[itemid].stream.ip_info.dstip = prms->stream.ip_info.dstip;
                        wlc->wme_ac_ip[itemid].stream.ip_info.dstport = prms->stream.ip_info.dstport;
                        wlc->wme_ac_ip[itemid].stream.ip_info.protocol = prms->stream.ip_info.protocol;
                        if(wlc->wme_ac_ip[itemid].priority > 0
                            && wlc->wme_ac_ip[itemid].stream.ip_info.srcip
                            && wlc->wme_ac_ip[itemid].stream.ip_info.dstip)
                                wlc->wme_ac_ip[itemid].action = TRUE;
                    }
                }
                break;
    ...
    case IOV_GVAL(IOV_WME_AC_IP):{
        ...
        for (i = 0; i < NUM_ACIP; i++) {
            WL_WNM(("%d\t\t%08x:%d\t\t%08x:%d\t\t%s\t\t%d\n",
                i,
                wlc->wme_ac_ip[i].stream.ip_info.srcip,
                wlc->wme_ac_ip[i].stream.ip_info.srcport,
                wlc->wme_ac_ip[i].stream.ip_info.dstip,
                wlc->wme_ac_ip[i].stream.ip_info.dstport,
                wlc->wme_ac_ip[i].stream.ip_info.protocol == 6 ? "TCP": wlc->wme_ac_ip[i].protocol == 17 ? "UDP" : "NULL",
                wlc->wme_ac_ip[i].priority));

                prms[i].stream.ip_info.srcip = wlc->wme_ac_ip[i].stream.ip_info.srcip;
                prms[i].stream.ip_info.srcport = wlc->wme_ac_ip[i].stream.ip_info.srcport;
                prms[i].stream.ip_info.dstip = wlc->wme_ac_ip[i].stream.ip_info.dstip;
                prms[i].stream.ip_info.dstport = wlc->wme_ac_ip[i].stream.ip_info.dstport;
                prms[i].stream.ip_info.protocol = wlc->wme_ac_ip[i].stream.ip_info.protocol;
...
#endif /*CONFIG_TENDA_GAME_SPEEDUP*/
        default:
...
{
    int ac;
    edcf_acparam_t *acp_be_ie, *acp_curr_ie;
    BCM_REFERENCE(wlc);
    bcm_bprintf(b, "\nEDCF params for %s\n", desc);

    acp_be_ie = acp_ie;
    acp_curr_ie = acp_ie;
    for (ac = 0; ac < AC_COUNT; ac++, acp_curr_ie++) {
        if (ac == AC_BK && wlc->ul_ofdma_repeater_ac_param_mod == TRUE &&
            (desc[0] != 'B')) {
            /* AC_BK was overwritten by AC_BE */
            acp_ie = acp_be_ie;
        } else {
            acp_ie = acp_curr_ie;
        }
