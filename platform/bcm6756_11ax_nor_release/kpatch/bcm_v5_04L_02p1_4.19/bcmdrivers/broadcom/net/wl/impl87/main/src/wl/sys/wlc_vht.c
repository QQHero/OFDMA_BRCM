 ...
 *
 * $Id: wlc_vht.c 810393 2022-04-06 16:37:11Z $
 */
...
BCMATTACHFN(wlc_vht_init_defaults)(wlc_vht_info_t *vhti)
{
    ...
    if (D11REV_GE(wlc->pub->corerev, 128) && D11REV_LT(wlc->pub->corerev, 132)) {
...
wlc_vht_tdls_write_brcm_ie(void *ctx, wlc_iem_build_data_t *data)
{
    ...
    if (encap & VHT_IE_ENCAP_VHTIE) {
        ...
        vht_op_ie_t vht_op = {0, 0, 0, VHT_CAP_MCS_MAP_NONE_ALL};
        ...
        chanspec_t ht_chanspec, uint8 ht_ccfs2)
{
    ...
    switch (vht_op_ie->chan_width) {
        case VHT_OP_CHAN_WIDTH_80:
            ...
            else if ((ht_band == WL_CHANSPEC_BAND_5G) &&
...
wlc_vht_calc_oper_mode_ie_len(void *ctx, wlc_iem_calc_data_t *data)
{
    ...
    if (data->cbparm->vht &&
        (narrow_bw || (cfg->oper_mode_enabled) ||
#ifdef WL_MODESW
        (WLC_MODESW_ENAB(wlc->pub) && wlc_modesw_opmode_ie_reqd(wlc->modesw, cfg)) ||
#endif
        FALSE))
    ...
    vht_cap_ie_t *vht_cap_ie, vht_op_ie_t *vht_op_ie, uint8 vht_ratemask)
{
    ...
    ASSERT(scb != NULL);
    cubby_info = (wlc_vht_scb_info_t *)SCB_VHT_INFO(vhti, scb);
    vht_cap = SCB_VHT_CAP(scb);
    ...
    */
    ...
    if (cubby_info->rxmcsmap != new_rateset.vht_mcsmap) {
        ...
        if (BSSCFG_STA(bsscfg) || SCB_LEGACY_WDS(scb)) {
...
wlc_vht_update_scb_oper_mode(wlc_vht_info_t *vhti, struct scb *scb, uint8 oper_mode)
{
    ...
    WL_MODE_SWITCH(("wl%d: updating oper mode: old=%x, new=%x, for scb "MACF"\n",
...
#ifdef WL11AX
    /* Inform HE module about oper_mode change before rate init */
    if (SCB_HE_CAP(scb)) {
    ...
    ratespec_t rspec, struct wl_rxsts *sts)
{
    ...
    if (D11REV_GE(wlc->pub->corerev, 40)) {
        /* For corerev >= 128, check if RX status is valid */
...
wlc_vht_bsscfg_updn(void *ctx, bsscfg_up_down_event_data_t *evt)
{
    ...
    if (evt->up) {
        ...
        /* XXX, let's configure maximum number of MSDUs in an A-MSDU only
        ...
        */
        wlc_bsscfg_set_ext_cap(cfg, DOT11_EXT_CAP_NUM_MSDU,
                isbitset(wlc->pub->_amsdu_max_msdu_num, 0));
        wlc_bsscfg_set_ext_cap(cfg, DOT11_EXT_CAP_NUM_MSDU+1,
                isbitset(wlc->pub->_amsdu_max_msdu_num, 1));
    }
}
...
wlc_send_action_vht_oper_mode(wlc_vht_info_t *vhti, wlc_bsscfg_t *bsscfg,
    ...
    if ((p = wlc_frame_get_action(wlc, ea,
        &bsscfg->cur_etheraddr, &bsscfg->BSSID,
        body_len, &pbody, DOT11_VHT_ACTION_OPER_MODE_NOTIF)) == NULL) {
        WL_ERROR(("wl%d: %s: wlc_frame_get_action failed\n",
                 wlc->pub->unit, __FUNCTION__));
        return;
    }
