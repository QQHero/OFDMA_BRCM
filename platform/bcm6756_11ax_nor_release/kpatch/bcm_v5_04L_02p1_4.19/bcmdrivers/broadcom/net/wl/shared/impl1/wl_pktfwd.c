...
_wl_pktwd_free_wds_d3lut_elem(d3fwd_wlif_t * d3fwd_wlif)
{
    ...
    if(d3fwd_wlif->wds_d3lut_elem)
    {
        ...
        d3lut_del(wl_pktfwd->d3lut, d3lut_elem->sym.v8, d3lut_elem->key.domain);
        ret = 1;
    ...
    {   /* If d3fwd_wlif is in run queue, remove it by freeing packets */
        ...
        if (runq->d3fwd_wlif == d3fwd_wlif)
        {
            for (prio = 0; prio < D3FWD_PRIO_MAX; ++prio)
            {
                ...
                if (!dll_empty(worklist) && worklist->next_p)
#else
                  ...
                  struct net_device * net_device, bool is_wlan)
{
    ...
    if (is_wlan)
    {
        ...
        if (!d3fwd_wlif->net_device) {
            WARN_ON_ONCE(!d3fwd_wlif->net_device);
        ...
        if(is_netdev_wlan_dwds_ap(d3fwd_wlif) || is_netdev_wlan_dwds_client(d3fwd_wlif)) {
            ...
            if (exist == false) {
                ...
                //当sta_free_list链表上没有空间时,会将sta_free_list也当作一个sta,挂在sta_list上;导致sta_list和sta_free_list由两个链表合成一个链表
                if (sta != NULL
#ifdef CONFIG_TENDA_PRIVATE_WLAN
                    && (sta != (d3lut_sta_t *)&d3fwd_wlif->sta_free_list)
#endif
                ) {
