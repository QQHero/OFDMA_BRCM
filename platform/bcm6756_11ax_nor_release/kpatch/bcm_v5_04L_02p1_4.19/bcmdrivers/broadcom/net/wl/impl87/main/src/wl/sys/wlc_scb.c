 ...
 *
 * $Id: wlc_scb.c 810129 2022-03-31 14:43:13Z $
 */
...
#endif /* BCM_PKTFWD */
#include <wlc_fifo.h>
#if defined(CONFIG_TENDA_PRIVATE_KM) && defined(CONFIG_UGW_BCM_DONGLE)
#include "td_wl_vendor.h"
#endif
...
static bool is_valid_bandunit(wlc_info_t *wlc, enum wlc_bandunit bandunit);
#endif
#if defined(BCMDBG)
static void wlc_scb_txinfo_dump(struct scb *scb, struct bcmstrbuf *b);
...
#ifndef DONGLEBUILD //dongle编译不需要 kill_proc_info 函数
...
#else
#define SIG_LINKUP 1 //dongle编译时 SIGUSR1 没有定义 这里用1表示up
#define SIG_LINKDOWN 0 //dongle编译时 SIGUSR2 没有定义 这里用0表示down
#endif
...
static int kill_proc_to_wserver(wlc_info_t *wlc, int sig)
{
    ...
    if (wserver_pid > 0) {
        ...
        data[0] = wserver_pid;
        data[1] = sig;
        wlc_bss_mac_event(wlc, NULL, WLC_E_TENDA_WLAN, NULL, WLC_E_TENDA_WLAN_WSER_SIG, 0, 0, (void*)data, sizeof(data));
#else
        kill_proc_info(sig, SEND_SIG_PRIV, wserver_pid);
#endif
    } else {
         WL_ERROR(("wserver pid error: %d\n", wserver_pid));
        return BCME_ERROR;
    }

    return BCME_OK;
    }
#endif //CONFIG_TENDA_WLAN_REPEATER
...
static void td_sta_online(wlc_info_t *wlc, struct scb *scb, uint8 add_state)
{
    ...
    if (sta_online) {
...
#ifdef CONFIG_UGW_BCM_DONGLE
        {
            ...
            memcpy(param.ssid, bsscfg->SSID, DOT11_MAX_SSID_LEN);
            param.ssid_len = bsscfg->SSID_len;

            wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_CLIENT_ONLINE, 0, 0, &param, sizeof(param));
        }
#else
...
#ifdef CONFIG_UGW_BCM_DONGLE
        {
            struct km_wifiev_sta_join_param param;
            memcpy(param.ifname, wl_ifname(bsscfg->wlc->wl, bsscfg->wlcif->wlif), IFNAMSIZ);
            param.rssi = wlc_lq_rssi_get(wlc, scb->bsscfg, scb);
            param.unit = WLCWLUNIT(wlc);
            param.sup11k = SCB_RRM(scb);
            param.sup11v = wlc_wnm_get_scbcap(wlc, scb);

            wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_STA_JOIN, 0, 0, &param, sizeof(param));
        }
#else
...
#endif
#ifdef CONFIG_TENDA_WLAN_AUTOHIDE_SSID        /* Hidden SSID */
         /* When a client goes online, the number of original clients is increased by one and then compared */
        if ((wlc_bss_assocscb_getcnt(wlc, bsscfg) + 1 >= bsscfg->maxassoc)
                && (SCB_AUTO_HIDE_ON == bsscfg->td_auto_hide)) {
...
#endif /* CONFIG_TENDA_WLAN_AUTOHIDE__SSID */
    }
}
...
void td_sta_offline(wlc_info_t *wlc, struct scb *scb, uint8 rm_state)
{
    ...
    if (sta_offline) {
#ifdef CONFIG_TENDA_PRIVATE_KM
#ifdef CONFIG_UGW_BCM_DONGLE
        wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_CLIENT_OFFLINE, 0, 0, wl_ifname(bsscfg->wlc->wl, bsscfg->wlcif->wlif), IFNAMSIZ);
#else
...
#ifdef CONFIG_UGW_BCM_DONGLE
        {
            ...
            param.sta_status = KM_WIRELESS_STA_OFFLIN;

            wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_STA_STATUS, 0, 0, &param, sizeof(param));
        }
#else
...
#ifdef CONFIG_TENDA_PRIVATE_KM
#ifdef CONFIG_UGW_BCM_DONGLE
        wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_STA_LEAVE, 0, 0, NULL, 0);
#else
...
#endif
#ifdef CONFIG_TENDA_WLAN_AUTOHIDE_SSID        /* Display SSID */
...
#endif /* CONFIG_TENDA_WLAN_AUTOHIDE_SSID */
    }
}
#endif
...
#ifdef BCMDBG
    scb->ac_suspend_start = INVALID_AC_SUSP_START;
#endif /* BCMDBG */
...
{
    if (scb->histo) {
        MFREE(wlc->osh, scb->histo, sizeof(wl_rate_histo_maps2_t));
    }
}
#endif /* WLSCB_HISTO */
...
victim_found:
    WL_ASSOC(("wl%d.%d: %s: reclaim scb "MACF", idle %d sec\n",  wlc->pub->unit,
        WLC_BSSCFG_IDX(oldscb->bsscfg), __FUNCTION__, ETHER_TO_MACF(oldscb->ea), oldest));
...
#ifdef GAME_SPEEDUP
   if((state & AUTHENTICATED) && (scb->special_scb)) {
       special_scb = scb->special_scb;
       special_scb->used = FALSE;
       scb->special_scb = NULL;
   }
#endif
...
#if defined(BCMDBG_ASSERT)
static bool
is_valid_bandunit(wlc_info_t *wlc, enum wlc_bandunit bandunit)
{
    wlcband_t *band = wlc_get_band(wlc, bandunit);
    return (band != NULL && band->bandunit == bandunit);
}
#endif
...
#if defined(BCMDBG)
#define SCB_DUMP_LEN (1 * 1024)
static void
wlc_scb_txinfo_dump(struct scb *scb, struct bcmstrbuf *b)
{
    uint16  i;
...
void
wlc_scb_print_txinfo(wlc_info_t *wlc, struct scb *scb)
{
    char    *buf = NULL;
    struct  bcmstrbuf bstr;
    uint16  c = 0;

    buf = MALLOCZ(wlc->osh, SCB_DUMP_LEN);

    if (!buf) {
        return;
    }

    bcm_binit(&bstr, buf, SCB_DUMP_LEN);
    wlc_scb_txinfo_dump(scb, &bstr);
    while (buf[c] && (c < SCB_DUMP_LEN)) {
        printf("%c", buf[c++]);
    }
    MFREE(wlc->osh, buf, SCB_DUMP_LEN);
}
...
#ifdef BCMDBG
    FOREACHSCB(wlc->scbstate, &scbiter, scb) {
        if (scb->ac_suspend_start != INVALID_AC_SUSP_START) {
            uint32 suspend_diff = wlc->pub->now - scb->ac_suspend_start;
            if (suspend_diff > 1) {
                WL_ERROR(("wl%d.%d %s: "MACF" traffic has been paused for"
                    "at least %d seconds!\n", wlc->pub->unit,
                    WLC_BSSCFG_IDX(SCB_BSSCFG(scb)), __FUNCTION__,
                    ETHER_TO_MACF(scb->ea), suspend_diff-1));
            }
        }
    }
#endif /* BCMDBG */
...
#ifdef CONFIG_TENDA_PRIVATE_KM
#ifdef CONFIG_UGW_BCM_DONGLE
            int8 rssi = wlc_lq_rssi_get(wlc, scb->bsscfg, scb);
            wlc_mac_event(wlc, WLC_E_TENDA_KM, &scb->ea, KM_WIFIEV_STA_UPDATE_RSSI, 0, 0, &rssi, sizeof(rssi));
#else
...
wlc_scbfind_delete(wlc_info_t *wlc, wlc_bsscfg_t *bsscfg, struct ether_addr *ea)
{
    ...
    FOREACH_WLC_BAND(wlc, i) {
        ...
        if (scb) {
            WL_ASSOC(("wl%d: %s: scb for the STA-"MACF" already exists\n",
...
#ifdef BCMDBG
    if (scb->ac_suspend_start == INVALID_AC_SUSP_START) {
        scb->ac_suspend_start = wlc->pub->now;
    }
#endif /* BCMDBG */
...
wlc_scb_resume_traffic(wlc_info_t* wlc, struct scb *scb, uint8 ac)
{
    ...
    if (wlc_fifo_isMU(wlc->fifo, scb, ac) &&
        !wlc_fifo_is_ulofdma(wlc->fifo, scb, ac) &&
        !wlc_fifo_scb_inflt_ac(scb, ac)) {
        WL_MUTX(("%s SCB "MACF" free fifo for ac=%d\n",
            __FUNCTION__, ETHER_TO_MACF(scb->ea), ac));
        wlc_fifo_free_ac(wlc->fifo, scb, ac);
    }
...
BCMATTACHFN(wlc_scb_flowid_attach)(wlc_info_t* wlc, scb_module_t *scbstate)
{
    ...
    scbstate->amt_lookup = MALLOCZ(wlc->osh, sizeof(uint16) * amt_size);
