...
#else
    ASSERT(!ETHER_ISNULLADDR(&wlc->pub->cur_etheraddr));

    bcopy(&wlc->pub->cur_etheraddr, &newmac, ETHER_ADDR_LEN);
    newmac.octet[5] =
        (newmac.octet[5] & ~(WLC_MBSS_UCIDX_MASK(wlc->pub->corerev))) |
        ((newmac.octet[5] + cfg_idx) & WLC_MBSS_UCIDX_MASK(wlc->pub->corerev));
#endif
