 ...
 *
 * $Id: wlc_obss_dynbw.c 804115 2021-10-20 13:36:09Z $
 */
 ...
 * @file
 * @brief
 * XXX Twiki: [DynamicBandwidthImplementationInterferenceBased]
 */
#include <wlc_cfg.h>
#include <wlc_types.h>
#include <typedefs.h>
...
struct wlc_obss_dynbw {
    ...
    bool psd_meas_pending; /* TRUE indicates a pseudo measurement request is pending
...
static void wlc_obss_dynbw_bsscfg_state_upd_cb(void *ctx, bsscfg_state_upd_data_t *evt);
...
static int wlc_obss_dynbw_doiovar(void *hdl,
...
#ifdef WLCSA
static void wlc_obss_dynbw_csa_cb(void *ctx, wlc_csa_cb_data_t *notif_data);
#endif
     ...
     */
    {
        ...
        if (result) {
            WL_MODE_SWITCH(("OBSS Detected TXOP: sample_dur:%d ,"
                "txop:%d,txdur:%d,ibss%d\n", sample_dur, txop, txdur, ibss));
        }
    }
    return result;
}
...
static int
wlc_obss_dynbw_doiovar(void *hdl, uint32 actionid,
        void *p, uint plen, void *a, uint alen, uint val_size, struct wlc_if *wlcif)
{
    ...
    switch (actionid) {
        ...
        case IOV_SVAL(IOV_OBSS_DYN_BWSW_ENAB):
            if (prot->dyn_bwsw_enabled && !int_val) {
                /* obss_dbs moving from enable to disable.
                 * Need to restore channel and state machine.
                 */
                wlc_info_t *wlc = prot->wlc;
                wlc_bsscfg_t *cfg;
                int idx;
                FOREACH_UP_AP(wlc, idx, cfg) {
                    bool enabled = TRUE;
                    uint8 oper_mode = wlc_modesw_derive_opermode(wlc->modesw,
                            prot->main_chanspec, cfg,
                            wlc->stf->op_rxstreams);

                    if (WLC_MODESW_ENAB(wlc->pub)) {
                        if (wlc_modesw_is_req_valid(wlc->modesw, cfg) !=
                                TRUE) {
                            err = BCME_BUSY;
                            break;
                        }
                        err = wlc_modesw_handle_oper_mode_notif_request(
                                wlc->modesw, cfg, oper_mode,
                                enabled, 0);
                        if (err == BCME_OK) {
                            wlc_obss_dynbw_restore_defaults(prot, cfg,
                                prot->main_chanspec);
                        } else {
                            break;
                        }
...
BCMATTACHFN(wlc_obss_dynbw_attach)(wlc_info_t *wlc)
{
    ...
    if ((wlc_bsscfg_state_upd_register(wlc,
            wlc_obss_dynbw_bsscfg_state_upd_cb, prot)) != BCME_OK) {
...
#ifdef WLCSA
    if (wlc->csa) {
        if (wlc_csa_cb_register(wlc->csa, wlc_obss_dynbw_csa_cb, prot) != BCME_OK) {
...
#ifdef WLCSA
    if (wlc->csa) {
        if (wlc_csa_cb_unregister(wlc->csa, wlc_obss_dynbw_csa_cb, prot) != BCME_OK) {
...
wlc_obss_dynbw_manage_bw_switch(wlc_obss_dynbw_t *prot)
{
    ...
    /* When BGDFS is in progress, OBSS DBS is NOT allowed */
    ...
    if (wlc_obss_dynbw_interference_detected_for_bwsw(prot, NULL)) {
        ...
        FOREACH_AS_BSS(wlc, idx, cfg) {
            ...
            WL_MODE_SWITCH(("cfg:%p, bwswpsd:%d\n", OSL_OBFUSCATE_BUF(cfg),
                po_bss->bwswpsd_state));
            if ((CHSPEC_IS8080(cfg->current_bss->chanspec) ||
                    CHSPEC_IS160(cfg->current_bss->chanspec)) &&
                    !WLC_PHY_160_FULL_NSS(wlc)) {
                continue;
            }
            if (WL11H_ENAB(wlc) &&
            ...
            /* pseudo ug active/pending, don't do anything, let the
            * timer decide
            */
            if (po_bss->bwswpsd_state != BWSW_PSEUDOUPGD_NOTACTIVE) {
                continue;
            ...
            WL_MODE_SWITCH(("cfg = %d, Orig Chanspec = %x\n", idx,  original_chspec));
            if (WLC_MODESW_ENAB(wlc->pub)) {
        ...
        FOREACH_BSS(wlc, idx, cfg) {
            ...
            po_bss->obss_bwsw_no_activity_cfm_count++;
            po_bss->obss_bwsw_activity_cfm_count = 0;
            WL_MODE_SWITCH(("%s:cfg-%d IF-:%d.of.%d;oacnt:%d\n", __FUNCTION__, idx,
                po_bss->obss_bwsw_no_activity_cfm_count,
                po_bss->obss_bwsw_no_activity_cfm_count_max,
                prot->obss_active_cnt));
...
bool
wlc_obss_dynbw_active(wlc_obss_dynbw_t *prot)
{
    if (prot->obss_active_cnt) {
        return TRUE;
    } else {
        return FALSE;
    }
        ...
        wlc_bsscfg_t *bsscfg, uint32 *rspec_bw)
{
    ...
    if (po_bss->bwswpsd_cspec_for_tx != INVCHANSPEC) {
        *rspec_bw = CHSPECBW_TO_RSPECBW(CHSPEC_BW(po_bss->bwswpsd_cspec_for_tx));
        ...
        uint32 elapsed_time, void *vstats)
{
    ...
    if (wlc_obss_dynbw_interference_detected_for_bwsw(prot, stats)) {
        ...
        FOREACH_AS_BSS(wlc, idx, cfg) {
            dynbw_bsscfg_cubby_t *tpo_bss = DYNBW_BSSCFG_CUBBY(prot, cfg);
        ...
        wlc_bsscfg_t * bsscfg, dynbw_bsscfg_cubby_t *po_bss)
{
    if (po_bss->bwswpsd_state == BWSW_PSEUDOUPGD_NOTACTIVE) {
        ...
        if (bsscfg->wlc->chanspec == po_bss->orig_chanspec) {
...
wlc_obss_dynbw_modesw_cb(void *ctx, wlc_modesw_notif_cb_data_t *notif_data)
{
    ...
    case (MODESW_PHY_UP_COMPLETE):
        ...
        if (po_bss->bwswpsd_state == BWSW_PSEUDOUPGD_PENDING) {
            ...
            */
            if (!(prot->psd_meas_pending)) {
                /* first cfg registering the psdstat */
                if (BCME_OK == wlc_obss_dynbw_init_psdstat_cb(prot,
                        notif_data->cfg)) {
                    prot->psd_meas_pending = TRUE;
                } else {
        ...
        wlc_bsscfg_t *bsscfg, chanspec_t beacon_chanspec)
{
    ...
    if (WLC_MODESW_ENAB(prot->wlc->pub)) {
        if (po_bss->obss_activity == OBSS_ACTIVE_HT) {
            bw = wlc_modesw_get_bw_from_opermode(bsscfg->oper_mode, beacon_chanspec);
            bw = MIN(bw, bw_bcn);
            ...
            if (bw == WL_CHANSPEC_BW_20) {
                ctl_chan = wf_chspec_primary20_chan(beacon_chanspec);
                return CH20MHZ_CHSPEC(ctl_chan, CHSPEC_BAND(beacon_chanspec));
            }
        }
    }
        ...
        chanspec_t chspec)
{
    ...
    /* 调用到此函数时该内存可能已经被 wlc_obss_dynbw_bssinfo_deinit 释放，需要先检查 */
    if (!po_bss) {
        return;
    }
#endif
    ...
    if (po_bss->obss_activity != OBSS_INACTIVE && prot->obss_active_cnt != 0) {
        prot->obss_active_cnt--;
        po_bss->obss_activity = OBSS_INACTIVE;
    }
    po_bss->bwswpsd_state = BWSW_PSEUDOUPGD_NOTACTIVE;
 ...
 * chanspec change is caused by oper_mode BW down/upgrade and OBSS_DBS
 * but without any change in above bsscfg states. On wl up, the function
 * restores prot_obss variables and states to default values.
 */
static void
wlc_obss_dynbw_bsscfg_state_upd_cb(void *ctx, bsscfg_state_upd_data_t *evt)
{
    wlc_obss_dynbw_t *prot = (wlc_obss_dynbw_t *)ctx;
    ...
    old_up = evt->old_up;
    new_en = bsscfg->enable;
    new_up = bsscfg->up;

    WL_MODE_SWITCH(("wl%d.%d: %s: old_enable %d, old_up %d, new_enable %d, new_up %d\n",
            WLCWLUNIT(prot->wlc), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__,
    ...
    /* clbk from wl down. do nothing */
    if (old_up == TRUE && new_up == FALSE) {
        WL_MODE_SWITCH(("wl%d.%d: %s: clbk from wl down.return\n",
                WLCWLUNIT(prot->wlc), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
        return;
    }

    WL_MODE_SWITCH(("wl%d.%d: %s:got callback from wl up. intf up resetting prot_obss\n",
            WLCWLUNIT(prot->wlc), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
...
#ifdef WLCSA
static void
wlc_obss_dynbw_csa_cb(void *ctx, wlc_csa_cb_data_t *notif_data)
{
    wlc_obss_dynbw_t *prot = (wlc_obss_dynbw_t *)ctx;
    wlc_bsscfg_t *bsscfg = notif_data->cfg;
    dynbw_bsscfg_cubby_t *po_bss = DYNBW_BSSCFG_CUBBY(prot, bsscfg);
