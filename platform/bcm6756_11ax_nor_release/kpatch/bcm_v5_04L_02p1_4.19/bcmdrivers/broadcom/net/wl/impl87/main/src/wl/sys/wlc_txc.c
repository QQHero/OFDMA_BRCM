 ...
 *
 * $Id: wlc_txc.c 809891 2022-03-25 15:51:33Z $
 */
...
#define WLPKTF_TOGGLE_MASK	(WLF_AMPDU_MPDU)
...
void wlc_txc_prep_sdu(wlc_txc_info_t *txc_info, struct scb *scb, wlc_key_t *key, void *p)
{
	wlc_txc_t *txc = (wlc_txc_t *)txc_info;
	wlc_txc_info_priv_t *priv;
	scb_txc_info_t *sti;
...
#ifdef GAME_SPEEDUP
    if (wlc->special_fixrate) {
        miss |= (prio == wlc->special_traffic);
    }
...
#ifdef GAME_SPEEDUP
   if(wlc->special_fixrate && (prio == wlc->special_traffic)) {
       return;
   }
#endif
