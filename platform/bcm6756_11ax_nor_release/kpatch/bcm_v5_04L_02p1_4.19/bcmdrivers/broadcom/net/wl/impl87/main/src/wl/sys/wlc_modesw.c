 ...
 *
 * $Id: wlc_modesw.c 809609 2022-03-19 00:02:39Z $
 */
...
wlc_modesw_send_omnm_event(wlc_modesw_info_t *mswi)
{
    ...
    wlc_bss_mac_event(wlc, wlc->primary_bsscfg, WLC_E_OMN_MASTER, NULL, 0,
            0, 0, &ev, ev.len);
    WL_INFORM(("wl%d: %s: sending event WLC_E_OMN_MASTER/%d with mod=%d\n",
...
wlc_modesw_omn_master_set(wlc_modesw_info_t *mswi, wl_omnm_mod_t mod, uint16 flags)
{
    ...
    WL_INFORM(("wl%d: %s: called with mod=%d, flags=0x%04x\n",
    ...
    WL_INFORM(("wl%d: %s: set mod=%d as master with expiry after %d secs at "
...
static void
wlc_modesw_apsta_send_omn_action_frame(wlc_modesw_info_t *modesw_info, wlc_bsscfg_t *bsscfg,
    ...
    scb = wlc_scbfindband(wlc, bsscfg, ea,
        CHSPEC_BANDUNIT(bsscfg->current_bss->chanspec));
    if (scb == NULL) {
        WL_ERROR(("wl%d.%d: Inside %s scb not found \n",
    ...
    if (wlc_modesw_is_connection_vht(wlc, bsscfg) &&
        ...
        if (p == NULL) {
            WL_ERROR(("wl%d.%d: %s Unable to allocate the mgmt frame \n",
                wlc->pub->unit, WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
            return;
        }

        WL_MODE_SWITCH(("wl%d.%d: %s Sending Oper Mode Action Frame oper_mode 0x%x..\n",
            WLCWLUNIT(wlc), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__, oper_mode_new));
    ...
    wlc_bsscfg_t * bsscfg)
{
    ...
    oper_mode_new = pmodesw_bsscfg->oper_mode_new;
    ...
    if (APSTA_ENAB(wlc->pub) && BSSCFG_STA(wlc->primary_bsscfg)) {
        /* Send OMN Action Frame to RootAP */
        WL_MODE_SWITCH(("wl%d.%d: %s APSTA MODESW: SendingAction\n",
            WLCWLUNIT(wlc), WLC_BSSCFG_IDX(bsscfg), __FUNCTION__));
        /* send action frame */
        wlc_modesw_apsta_send_omn_action_frame(wlc->modesw, wlc->primary_bsscfg,
            &(wlc->primary_bsscfg->BSSID), oper_mode_new);
    }
    ...
    wlc_bsscfg_t *bsscfg, uint32 *rspec_bw)
{
    ...
    override_chspec = wlc_modesw_find_downgrade_chanspec(modesw_info->wlc, bsscfg,
        pmodesw_bsscfg->oper_mode_new, pmodesw_bsscfg->oper_mode_old);
    *rspec_bw = CHSPECBW_TO_RSPECBW(CHSPEC_BW(override_chspec));
    ...
    uint8 oper_mode, uint8 enabled)
{
    ...
    /* Upgrade */
    if (!wlc_modesw_is_downgrade(wlc, bsscfg, bsscfg->oper_mode, oper_mode)) {
        WL_MODE_SWITCH(("wl%d.%d: AP upgrade \n", WLCWLUNIT(wlc), WLC_BSSCFG_IDX(bsscfg)));
        pmodesw_bsscfg->new_chanspec = wlc_modesw_find_upgrade_chanspec(modesw_info,
            bsscfg, oper_mode, bsscfg->oper_mode);

        WL_MODE_SWITCH(("wl%d.%d: Got the new chanspec as %x bw = %x\n",
            WLCWLUNIT(wlc), WLC_BSSCFG_IDX(bsscfg),
            pmodesw_bsscfg->new_chanspec,
    ...
    /* Downgrade */
        ...
        WL_MODE_SWITCH(("wl%d.%d: AP Downgrade Case \n", WLCWLUNIT(wlc),
    ...
    wlc_bsscfg_t *bsscfg, uint8 oper_mode, uint8 enabled, uint32 ctrl_flags)
{
    ...
    WL_MODE_SWITCH(("wl%d.%d: Processing new bw: %d, old bw: %d; new nss: %d, old nss: %d "
            "old160: %d new160: %d\n", WLCWLUNIT(wlc), WLC_BSSCFG_IDX(bsscfg),
            new_bw, old_bw, new_nss, old_nss, old160, new160));

    pmodesw_bsscfg->ctrl_flags = ctrl_flags;

    if (BSSCFG_AP(bsscfg) || BSSCFG_IBSS(bsscfg)) {
        /* Initialize the AP operating mode if UP */
        WL_MODE_SWITCH(("wl%d.%d AP CASE \n", WLCWLUNIT(wlc), WLC_BSSCFG_IDX(bsscfg)));
    ...
    if (BSSCFG_INFRA_STA(bsscfg) || BSSCFG_PSTA(bsscfg)) {
        if (WLC_BSS_ASSOC_NOT_ROAM(bsscfg)) {
            ...
            if (scb) {
                ...
                * and continue with other means of mode switch
                * (MIMOPS, CH WIDTH etc)
                */
                if (wlc_modesw_is_connection_vht(wlc, bsscfg) &&
                    !(SCB_OPER_MODE_NOTIF_CAP(scb))) {
                    WL_ERROR(("wl%d: No capability for opermode switch in"
                        "VHT AP....Unexpected..\n", WLCWLUNIT(wlc)));
                }
            }
    ...
    wlc_bsscfg_t *bsscfg, uint8 rxstreams)
{
    ...
    if (wlc_modesw_is_connection_vht(wlc, bsscfg)) {
        ...
        oper_mode = DOT11_D8_OPER_MODE(rxnss_type, rxnss, FALSE, FALSE, bw);
    }
