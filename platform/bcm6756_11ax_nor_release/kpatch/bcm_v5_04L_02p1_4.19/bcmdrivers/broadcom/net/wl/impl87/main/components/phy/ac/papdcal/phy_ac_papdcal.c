 ...
 *
 * $Id: phy_ac_papdcal.c 810275 2022-04-04 18:23:59Z $
 */
#include <phy_cfg.h>
#include <typedefs.h>
#include <bcmdefs.h>
...
uint16 papd_gainctrl_calidx_6g[PHY_CORE_MAX];
...
uint16 wbcal_sum_RX_dump[PHY_CORE_MAX], wbcal_sum_TX_dump[PHY_CORE_MAX];
uint16 wbcal_scale_int_in_dump[PHY_CORE_MAX], wbcal_scale_int_out_dump[PHY_CORE_MAX];

/* MACROS */
#define WBPAPD_REFDB_BASE    6770
...
typedef struct {
    ...
    bool    highres_en;
    int16    target_sum_RX_2g;
    int16    target_sum_RX_5g;
    int16    target_sum_RX_6g;
    ...
    uint8    papd_calidx_2gBW[2];
    uint8    papd_calidx_5gBW[4];
    uint16    peak_psd_limit_6G2G;
    uint16    peak_psd_limit_5G0;
    uint16    peak_psd_limit_5G1;
    uint16    peak_psd_limit_5G2;
    bool    ext_dyn_range;
    uint16    wbcal_const_pow_scale_ext;
    uint8    wbcal_dc_accum_wait_ext;
    uint8    wbcal_window_len_pow_ext;
    uint32    wbcal_scale_end_ext;
    uint32    wbcal_start_ext;
...
struct phy_ac_papdcal_info {
    ...
    int32    wbpapd_sampcapt_iovar;
    int32    peak_psd_limit_iovar;
    int8    stateCts2selfTssiPwr[PHY_CORE_MAX]; // 2 fractional bits.
    uint8    stateTssigcCalIdx[PHY_CORE_MAX]; // cal_idx chosen by TSSI gain control.
    uint16    stateTssigcWfmTruncIdx; // Truncation index chosen by TSSI gain control.
...
static const char BCMATTACHDATA(rstr_wb_papd_calidx2g)[] = "wb_papdcalidx2g";
static const char BCMATTACHDATA(rstr_wb_papd_calidx5g)[] = "wb_papdcalidx5g";
...
/* Regulatory limits */
static const char BCMATTACHDATA(rstr_peak_psd_limit_6G2G)[] = "peak_psd_limit_6G2G";
static const char BCMATTACHDATA(rstr_peak_psd_limit_5G0)[] = "peak_psd_limit_5G0";
static const char BCMATTACHDATA(rstr_peak_psd_limit_5G1)[] = "peak_psd_limit_5G1";
static const char BCMATTACHDATA(rstr_peak_psd_limit_5G2)[] = "peak_psd_limit_5G2";
...
static int phy_ac_papdcal_get_wbpapd_sampcapt(phy_type_papdcal_ctx_t *ctx, int32* wbpapd_sampcapt);
static int phy_ac_papdcal_set_wbpapd_sampcapt(phy_type_papdcal_ctx_t *ctx, int32 wbpapd_sampcapt);
static int phy_ac_papdcal_get_peak_psd_limit(phy_type_papdcal_ctx_t *ctx, int32* peak_psd_limit);
static int phy_ac_papdcal_set_peak_psd_limit(phy_type_papdcal_ctx_t *ctx, int32 peak_psd_limit);
#endif
...
static void wbpapd_tssi_gctrl(phy_info_t *pi);
static void wlc_phy_force_rfseq_rx2tx_wbcal_war(phy_info_t *pi);
...
BCMATTACHFN(phy_ac_papdcal_nvram_attach)(phy_info_t *pi, phy_ac_papdcal_info_t *ac_info)
{
    ...
    if (PHY_BAND5G_ENAB(pi)) {
        if (ACMAJORREV_131(pi->pubpi->phy_rev) && WBPAPD_ENAB(pi)) {
            ...
            /* Older chips do not have BW160 dedicated pacalshift5g */
            ac_info->pacalshift5g[3] = ac_info->pacalshift5g[2];
        }
    ...
    phy_papdcal_info_t *cmn_info)
{
    ...
    fns.get_wbpapd_sampcapt = phy_ac_papdcal_get_wbpapd_sampcapt;
    fns.set_wbpapd_sampcapt = phy_ac_papdcal_set_wbpapd_sampcapt;
    fns.get_peak_psd_limit = phy_ac_papdcal_get_peak_psd_limit;
    fns.set_peak_psd_limit = phy_ac_papdcal_set_peak_psd_limit;
#endif
#if defined(WLTEST) || defined(DBG_PHY_IOV) || defined(WFD_PHY_LL_DEBUG)
#ifndef ATE_BUILD
...
phy_ac_populate_papd_params(phy_ac_papdcal_info_t *papd_info)
{
    ...
    papd_params->cal_refdb_6g = 20;
    papd_params->num_eps_tables = 1;
    papd_params->ext_dyn_range = FALSE;
    ...
    if (ACMAJORREV_4(pi->pubpi->phy_rev)) {
        ...
        if (PHY_EPAPD(pi)) {
            ...
            papd_params->papd_calidx_6g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_papd_calidx, 48 << PAPD_CAL_IDX_5G_SHIFT)) &
                PAPD_CAL_IDX_5G) >> PAPD_CAL_IDX_5G_SHIFT;
        } else {
            papd_params->papd_calidx_2g = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
        ...
        FOREACH_CORE(pi, core) {
            ...
            papd_gainctrl_calidx_6g[core] = papd_params->papd_calidx_6g;
        }
        ...
        if (WBPAPD_ENAB(pi)) {
            ...
            // In 2G, target_sum_RX = 160, log10(160) = 2257/2^10.
            papd_params->target_sum_RX_2g = 2257;
            papd_params->target_sum_RX_5g = 2257;
            // In 6G, target_sum_RX = 134.6, log10(134.6) = 2180/2^10.
            papd_params->target_sum_RX_6g = 2180;
            papd_params->epsilon_offset_5g = (((uint32)PHY_GETINTVAR_DEFAULT(pi,
            ...
            papd_params->papd_calidx_5g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                ...
                PAPD_CAL_IDX_5G) >> PAPD_CAL_IDX_5G_SHIFT;
            papd_params->papd_calidx_6g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_papd_calidx, 41 << PAPD_CAL_IDX_5G_SHIFT)) &
                PAPD_CAL_IDX_5G) >> PAPD_CAL_IDX_5G_SHIFT;
            FOREACH_CORE(pi, core) {
                papd_params->bbmult_2g[core] = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
            ...
            papd_params->cal_refdb_2g = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_calref_db, 12)) & PAPD_CALREF_DB_2G;
            papd_params->cal_refdb_5g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_calref_db, 12 << PAPD_CALREF_DB_5G_SHIFT)) &
                PAPD_CALREF_DB_5G) >> PAPD_CALREF_DB_5G_SHIFT;
            papd_params->cal_refdb_6g = 16;
            papd_params->tia_mode_2g = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_tia_gain_mode, 4)) & TIA_GAIN_MODE_2G;
            papd_params->tia_mode_5g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_tia_gain_mode, 6 << TIA_GAIN_MODE_5G_SHIFT)) &
                TIA_GAIN_MODE_5G) >> TIA_GAIN_MODE_5G_SHIFT;
            papd_params->peak_psd_limit_6G2G = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_peak_psd_limit_6G2G, 0xFFFF));
            papd_params->peak_psd_limit_5G0 = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_peak_psd_limit_5G0, 0xFFFF));
            papd_params->peak_psd_limit_5G1 = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_peak_psd_limit_5G1, 0xFFFF));
            papd_params->peak_psd_limit_5G2 = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_peak_psd_limit_5G2, 0xFFFF));
        ...
        if (WBPAPD_ENAB(pi)) {
            ...
            papd_params->dac_rate_mode_80M_160M = 1; // 160M
            papd_params->papd_calidx_2g = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
            ...
            papd_params->wbcal_lut_len = 64;
            papd_params->wbcal_start = 13500;
            papd_params->wbcal_start_ext = 11700;
            papd_params->wbcal_end = 208920;
            papd_params->wbcal_scale_start = 600;
            papd_params->wbcal_scale_end = 2648;
            papd_params->wbcal_scale_end_ext = 1624;
            papd_params->wbcal_dcc_en = 1;
            papd_params->wbcal_dc_accum_wait = 6;
            papd_params->wbcal_dc_accum_wait_ext = 12;
            papd_params->wbcal_dly_filt_en = 0;
            papd_params->wbcal_dccorr_ovr_en = 0;
            papd_params->wbcal_dcoffset_real = 0;
            papd_params->wbcal_dcoffset_imag = 0;
            ...
            papd_params->eps_stop_idx = 38;
            papd_params->wbcal_const_pow_scale = 128;
            papd_params->wbcal_const_pow_scale_ext = 40;
            papd_params->wbcal_window_len_pow = 11;
            papd_params->wbcal_window_len_pow_ext = 10;
            papd_params->wbcal_macbuf_offset = 65536;
            papd_params->wbcal_waveform_sz = 4000;
            papd_params->rx_atten_2g =  ((uint32)PHY_GETINTVAR_DEFAULT(pi,
            ...
            papd_params->buf_offset_5g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                ...
                PAPD_BUF_OFFSET_5G) >> PAPD_BUF_OFFSET_5G_SHIFT;
            papd_params->buf_offset_5g_160M = 0x23;
            papd_params->delayfilt_gamma_2g = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_frac_del, 26)) & PAPD_FRACDELAY_OFFSET_2G;
            /* By default frac bits is 11 */
            papd_params->wbcal_gfrac_bits = ((uint8)PHY_GETINTVAR_DEFAULT(pi,
            ...
            papd_params->target_sum_RX_2g = 2257;
            papd_params->target_sum_RX_5g = 2257;
            papd_params->target_sum_RX_6g = 2257;
            papd_params->epsilon_offset_2g = ((uint32)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_eps_offset, 0)) & PAPD_EPS_OFFSET_2G;
            papd_params->epsilon_offset_5g = (((uint32)PHY_GETINTVAR_DEFAULT(pi,
            ...
            papd_params->papd_calidx_6g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                ...
                PAPD_CAL_IDX_5G) >> PAPD_CAL_IDX_5G_SHIFT;
            if (PHY_EPAPD(pi)) {
                /* Extend WBPAPD waveform dynamic range */
                papd_params->ext_dyn_range = TRUE;

                for (i = 0; i < 4; i++) {
                    papd_params->papd_calidx_5gBW[i] =
                        (uint8)(PHY_GETINTVAR_ARRAY_DEFAULT
                        (pi, rstr_wb_papd_calidx5g, i,
                        papd_params->papd_calidx_5g));
                    if (i < 2) {
                        papd_params->papd_calidx_2gBW[i] =
                            (uint8)(PHY_GETINTVAR_ARRAY_DEFAULT
                            (pi, rstr_wb_papd_calidx2g, i,
                            papd_params->papd_calidx_2g));
                    }
                }
            }
            ...
            papd_params->cal_refdb_5g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                ...
                PAPD_CALREF_DB_5G) >> PAPD_CALREF_DB_5G_SHIFT;
            papd_params->cal_refdb_6g = (((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_calref_db, 11 << PAPD_CALREF_DB_5G_SHIFT)) &
                PAPD_CALREF_DB_5G) >> PAPD_CALREF_DB_5G_SHIFT;
            papd_params->tia_mode_2g = ((uint16)PHY_GETINTVAR_DEFAULT(pi,
                rstr_wb_tia_gain_mode, 4)) & TIA_GAIN_MODE_2G;
...
{
    phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
    if (wbpapd_gctrl < 0 || wbpapd_gctrl > 3) {
        return BCME_ERROR;
    } else {
...
static int
phy_ac_papdcal_get_wbpapd_sampcapt(phy_type_papdcal_ctx_t *ctx, int32* wbpapd_sampcapt)
{
    phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
    *wbpapd_sampcapt = (int32)info->wbpapd_sampcapt_iovar;
    return BCME_OK;
}

static int
phy_ac_papdcal_set_wbpapd_sampcapt(phy_type_papdcal_ctx_t *ctx, int32 wbpapd_sampcapt)
{
    phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
    info->wbpapd_sampcapt_iovar = wbpapd_sampcapt;
    return BCME_OK;
}

static int
phy_ac_papdcal_get_peak_psd_limit(phy_type_papdcal_ctx_t *ctx, int32* peak_psd_limit)
{
    phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
    *peak_psd_limit = (int32)info->peak_psd_limit_iovar;
    return BCME_OK;
}

static int
phy_ac_papdcal_set_peak_psd_limit(phy_type_papdcal_ctx_t *ctx, int32 peak_psd_limit)
{
    phy_ac_papdcal_info_t *info = (phy_ac_papdcal_info_t *)ctx;
    info->peak_psd_limit_iovar = peak_psd_limit;
    return BCME_OK;
}
...
static int8 phy_ac_wbcal_eps_stopidx(phy_info_t *pi, uint8 core, uint32 *end_eps_entry);
...
static void notch_set_by_override_20708(phy_info_t *pi, uint8 core);
static void calculate_calibrated_epsOffset(phy_info_t *pi, uint8 core, uint8 cal_idx,
...
static uint8 wbpapd_tia_gain_control(phy_info_t *pi, uint16 wbcal_scale_int_in,
    uint16 wbcal_scale_int_out, uint32 end_eps_entry, uint8 gfrac_bits, uint8 tia1,
    int8 diff_cal_idx);
static int16 scale_log10(uint32 input);
...
phy_ac_papd_radio_lpbk_setup_20710(phy_info_t *pi, bool epapd_flag)
{
    ...
    uint8 core, papd_loopback_mode, papd_bypass_mode;
    uint8 tia_gain = CHSPEC_IS2G(pi->radio_chanspec) ? params->tia_mode_2g :
        params->tia_mode_5g;
    uint8 rx_atten = CHSPEC_IS2G(pi->radio_chanspec) ? params->rx_atten_2g :
        params->rx_atten_5g;
    uint8 tx_atten = CHSPEC_IS2G(pi->radio_chanspec) ? params->tx_atten_2g :
        params->tx_atten_5g;
    ...
    FOREACH_CORE(pi, core) {
        ...
        if (CHSPEC_IS2G(pi->radio_chanspec)) {
            ...
            if (papd_loopback_mode == MAINGM_PATH) {
                ...
                if (papd_bypass_mode == 0) {
                    RADIO_REG_LIST_START
                        MOD_RADIO_REG_20710_ENTRY(pi, RX5G_REG3, core,
                            rxdb_auxgm_en, 1)
                        MOD_RADIO_REG_20710_ENTRY(pi, RX5G_REG3, core,
                            rxdb_auxgm_bypass_en, 0)
                    RADIO_REG_LIST_EXECUTE(pi, core);
                } else {
                    RADIO_REG_LIST_START
                        MOD_RADIO_REG_20710_ENTRY(pi, RX5G_REG3, core,
            ...
            if (!epapd_flag) {
                ...
                RADIO_REG_LIST_START
                    ...
                    MOD_RADIO_REG_20710_ENTRY(pi, RX5G_REG4, core,
                        rxdb_gm_ibias_ptat_slope, 4)
                RADIO_REG_LIST_EXECUTE(pi, core);
                MOD_RADIO_REG_20710(pi, RX5G_REG5, core, rxdb_lna_epapd_attn,
                        rx_atten);
            }
        } else {
            /* ePAPD CAL 5G */
            ...
            MOD_RADIO_REG_20710(pi, RX5G_REG5, core,
                    rxdb_lna_epapd_attn, rx_atten);
            ...
            MOD_RADIO_REG_20710(pi, TX2G_PA_REG0, core, tx2g_pa_cal_atten,
                tx_atten);
            MOD_RADIO_REG_20710(pi, RX5G_REG4, core, rxdb_coup_loopback_attn, 3);
        }
...
phy_ac_wbcal_widen_filter(phy_info_t *pi, uint8 tia_mode, uint8 core)
{
    ...
    if (!ACMAJORREV_130(pi->pubpi->phy_rev)) {
        ...
        notch_set_by_override_20708(pi, core);
    }
}
...
lpf_calc_20708(phy_info_t *pi, uint8 core)
{
    ...
    /* LPF Spare: In RX always 1, in TX 1 for 20/40 and 0 for 80/160 */
    if (bw_idx == 2 || bw_idx == 3) {
        // BQ2BW is 6/7, disable lpf_spare ECO
        MOD_RADIO_REG_20708(pi, LPF_REG12, core, lpf_spare, 0);
    } else if (bw_idx == 0 || bw_idx == 1) {
        // BQ2BW is 4/5, disable lpf_spare ECO
        MOD_RADIO_REG_20708(pi, LPF_REG12, core, lpf_spare, 1);
    } else {
        // BQ2 in Rx mode BW, disable lpf_spare ECO
        MOD_RADIO_REG_20708(pi, LPF_REG12, core, lpf_spare, 1);
    }
    // HW6715-145 1: ECO Disabled, 0: Enabled
    MOD_RADIO_REG_20708(pi, SPARE_CFG0, core, spare_0, 0x1);
...
{
    uint8 bw_idx;
    uint8 opamp3_bias_table[] = {20, 20, 40, 40};
    uint8 opamp4_bias_table[] = {20, 20, 40, 40};
...
static void
notch_set_by_override_20708(phy_info_t *pi, uint8 core)
{
    phy_ac_radio_data_t *data = phy_ac_radio_get_data(pi->u.pi_acphy->radioi);
    uint16  rccal_gmult_rc = data->rccal_gmult_rc;
    uint16 temp, g1_tuned_nogain;
    uint16 g1_tuned, g1_cm_tuned, g2_tuned, g3_tuned, c0_tuned, c1_tuned, c2_tuned;
    uint8 bw_idx;

    uint16 g1_tbl[] = {621, 689, 811, 811};
    uint16 g2_tbl[] = {621, 689, 811, 811};
    uint16 g3_tbl[] = {153, 170, 510, 620};
    uint8 c0_tbl[] = {44, 22, 27, 16};
    uint8 c1_tbl[] = {11, 6, 6, 3};
    uint8 c2_tbl[] = {5, 2, 0, 0};
    uint16 gain = 2896; // rc_gain = 3
    ...
    temp = (g1_tbl[bw_idx] * rccal_gmult_rc) >> 12;
    g1_tuned_nogain = (temp > 1023) ? 1023 : temp;
    g1_tuned = (g1_tuned_nogain * gain) >> 12;
    g1_cm_tuned = (g1_tuned_nogain * (4096 - gain)) >> 12;
    temp = (g2_tbl[bw_idx] * rccal_gmult_rc) >> 12;
    g2_tuned = (temp > 1023) ? 1023 : temp;
    temp = (g3_tbl[bw_idx] * rccal_gmult_rc) >> 12;
    g3_tuned = (temp > 1023) ? 1023 : temp;
    c0_tuned = (c0_tbl[bw_idx] * gain) >> 12;
    c1_tuned = c1_tbl[bw_idx];
    c2_tuned = c2_tbl[bw_idx];

    RADIO_REG_LIST_START
        MOD_RADIO_REG_20708_ENTRY(pi, LPF_NOTCH_OVR1, core, ovr_lpf_notch_g1, 1)
        MOD_RADIO_REG_20708_ENTRY(pi, LPF_NOTCH_OVR1, core, ovr_lpf_notch_g1_cm, 1)
        MOD_RADIO_REG_20708_ENTRY(pi, LPF_NOTCH_OVR1, core, ovr_lpf_notch_g2, 1)
        MOD_RADIO_REG_20708_ENTRY(pi, LPF_NOTCH_OVR1, core, ovr_lpf_notch_g3, 1)
        MOD_RADIO_REG_20708_ENTRY(pi, LPF_NOTCH_OVR1, core, ovr_lpf_notch_c0, 1)
        MOD_RADIO_REG_20708_ENTRY(pi, LPF_NOTCH_OVR1, core, ovr_lpf_notch_c1, 1)
        MOD_RADIO_REG_20708_ENTRY(pi, LPF_NOTCH_OVR1, core, ovr_lpf_notch_c2, 1)
    RADIO_REG_LIST_EXECUTE(pi, core);
    MOD_RADIO_REG_20708(pi, LPF_NOTCH_REG2, core, lpf_notch_g1, g1_tuned);
    MOD_RADIO_REG_20708(pi, LPF_NOTCH_REG5, core, lpf_notch_g1_cm, g1_cm_tuned);
    MOD_RADIO_REG_20708(pi, LPF_NOTCH_REG3, core, lpf_notch_g2, g2_tuned);
    MOD_RADIO_REG_20708(pi, LPF_NOTCH_REG4, core, lpf_notch_g3, g3_tuned);
    MOD_RADIO_REG_20708(pi, LPF_NOTCH_REG1, core, lpf_notch_c0, c0_tuned);
    MOD_RADIO_REG_20708(pi, LPF_NOTCH_REG1, core, lpf_notch_c1, c1_tuned);
    MOD_RADIO_REG_20708(pi, LPF_NOTCH_REG1, core, lpf_notch_c2, c2_tuned);
}
    ...
    uint8 gfrac_bits, uint32 *original_eps_table, int16 *epsilonOffset)
{
    ...
    curr_max_location = phy_ac_wbcal_eps_stopidx(pi, core, &eps_complex);

    if (curr_max_location == 0) {
        PHY_PAPD(("%s epsOffset cal failed with all zero eps entries.\n",
            __FUNCTION__));
        *epsilonOffset = -70;
    } else {
        uint32 scaling = 1, thresh = 2147483647; // 2^31-1
        uint8 scaling_offset = 0, i = 0;
        eps_complex = original_eps_table[tbl_step * curr_max_location];
        phy_papdcal_decode_epsilon(eps_complex, &eps_re, &eps_im);
        tempclip = (one + eps_re) * (one + eps_re) + (eps_im * eps_im);

        /* Determine scaling factor and offset to improve qm_log10 accuracy.
         * Normalize input of qm_log10 to the range of [2^30, 2^31-1].
         */
        while (!(thresh >= scaling*tempclip && thresh/2 <= scaling*tempclip) && i <= 31) {
            i++;
            scaling = scaling << 1; // multiplied by 2
            scaling_offset += 12; // 40log10(2)
        }
        if (i == 32) {
            // Normalization failed. Reset scaling and scaling offset.
            scaling = 1;
            scaling_offset = 0;
        }
         ...
         */
        // Multiplied by scaling factor to improve qm_log10 accuracy
        qm_log10((int32)(scaling*tempclip), 0, &log10mantissa, &log10exp);
        ...
        // Add scaling_offset = 40log10(scaling) back due to scaling up previously
        fortylog10 = ((173174 + 24660 * gfrac_bits - 40 * log10mantissa + 512) >> 10)
                 + scaling_offset;
        *epsilonOffset = curr_max_location - (fortylog10 + dacplusrfgain);
        ...
        if (CHSPEC_IS2G(pi->radio_chanspec)) {
            ...
            /* 5G UNII-4 band is not optimized for 6715 */
            if (CHSPEC_IS20(pi->radio_chanspec) || CHSPEC_IS40(pi->radio_chanspec)) {
                *epsilonOffset -= 2;
            } else if (CHSPEC_IS80(pi->radio_chanspec) ||
...
wlc_phy_papd_dump_eps_trace_acphy(phy_info_t *pi, struct bcmstrbuf *b)
{
    ...
    FOREACH_ACTV_CORE(pi, stf_shdata->phytxchain, core) {
        ...
        bcm_bprintf(b, "papdcalidx core%d %d\n", core, CHSPEC_IS2G(pi->radio_chanspec) ?
            papd_gainctrl_calidx_2g[core] : (CHSPEC_IS5G(pi->radio_chanspec) ?
            papd_gainctrl_calidx_5g[core] : papd_gainctrl_calidx_6g[core]));
        ...
        bcm_bprintf(b, "wbcal_sum_RX core%d %d\n", core, wbcal_sum_RX_dump[core]);
        bcm_bprintf(b, "wbcal_sum_TX core%d %d\n", core, wbcal_sum_TX_dump[core]);
        bcm_bprintf(b, "wbcal_scale_int_in core%d %d\n", core,
            wbcal_scale_int_in_dump[core]);
        bcm_bprintf(b, "wbcal_scale_int_out core%d %d\n", core,
            wbcal_scale_int_out_dump[core]);
        bcm_bprintf(b, "stateCts2selfTssiPwr core%d %d\n", core,
            pi->u.pi_acphy->papdcali->stateCts2selfTssiPwr[core]);
        PHY_CAL(("\n"));
    }
    bcm_bprintf(b, "stateTssigcWfmTruncIdx %d\n",
        pi->u.pi_acphy->papdcali->stateTssigcWfmTruncIdx);
    PHY_CAL(("\n"));
}
...
BCMATTACHFN(phy_ac_papdcal_nvram_attach_old)(phy_ac_papdcal_info_t *papdcal_info)
{
    ...
    papdcal_info->wbpapd_sampcapt_iovar = -1;
    papdcal_info->peak_psd_limit_iovar = -1;
    papdcal_info->stateTssigcWfmTruncIdx = 0xFFFF;
#if defined(BCMDBG)
    papdcal_dumpinfo = 1;
#else
    papdcal_dumpinfo = 0;
...
phy_ac_papd_phy_cleanup_majorrev51_128_129_130_131(phy_info_t *pi)
{
    ...
    if (ACMAJORREV_131(pi->pubpi->phy_rev) && WBPAPD_ENAB(pi)) {
        FOREACH_CORE(pi, core) {
            /* This bit affects normal TX operation, so it needs to be restored */
    ...
    if (APAPD_ENAB(papdcali)) {
        ...
        if (papdcali->papd_abort == 0 || first_cal == 1 || papdcali->fullcal) {
            ...
            tx_idx = (CHSPEC_IS2G(pi->radio_chanspec)) ?
                ...
                papdcali->papd_params->papd_calidx_6g);
    ...
    if ((ACMAJORREV_51_129_130_131(pi->pubpi->phy_rev) || ACMAJORREV_128(pi->pubpi->phy_rev)) &&
        ...
        FOREACH_CORE(pi, core) {
            ...
            if (ACMAJORREV_131(pi->pubpi->phy_rev) &&
                CHSPEC_IS160(pi->radio_chanspec)) {
                /* Cal mode 6 */
                MOD_PHYREGCE(pi, wbcal_ctl_21, core, wbcal_bypass_fir, 1);
...
wlc_phy_txpwr_papd_cal_run_tiny(phy_info_t *pi,    uint8 tx_pre_cal_pwr_ctrl_state, int8 cal_core)
{
    ...
    phy_info_acphy_t *aci = (phy_info_acphy_t *)pi->u.pi_acphy;
    phy_ac_papdcal_params_t *params    = aci->papdcali->papd_params;
    phy_ac_papdcal_info_t *papdcali = aci->papdcali;
    ...
    if (ACMAJORREV_51(pi->pubpi->phy_rev)) {
        ...
        numiter = params->papd_iter;
        phy_ac_papd_radio_lpbk_setup_20710(pi, PHY_EPAPD(pi));
    } else {
    ...
    if (pi->u.pi_acphy->sromi->srom_low_adc_rate_en) {
        if (ACMAJORREV_51(pi->pubpi->phy_rev)) {
            ...
            wlc_phy_low_rate_adc_enable_acphy(pi, FALSE);
            wlc_phy_radio20710_afe_div_ratio(pi, 1, dac_rate_mode, WBPAPD_ENAB(pi));
        }
    }
    ...
    if (ACMAJORREV_130(pi->pubpi->phy_rev) && (papdcali->wbpapd_gctrl_iovar == 3)) {
        wbpapd_tssi_gctrl(pi);
    }
...
phy_ac_papd_cal_mode0_1(phy_info_t *pi, acphy_papdCalParams_t *calParams,  uint8 *calmode)
{
    ...
    if (PHY_EPAPD(pi) && swctrl->valid) {
        MOD_PHYREGCE(pi, FemOutputOvrCtrl, core, femCtrlOutput,
            swctrl->band[band_idx].femctrl[core]);
        MOD_PHYREGCE(pi, FemOutputOvrCtrl, core, femCtrlOutputOvr, 1);
    }
...
phy_ac_wbcal_run(phy_info_t *pi, uint8 for_core)
{
    ...
    int16 log10mantissa1 = CHSPEC_IS2G(pi->radio_chanspec) ? params->target_sum_RX_2g :
        (CHSPEC_IS5G(pi->radio_chanspec) ? params->target_sum_RX_5g :
        params->target_sum_RX_6g);
    int16 log10mantissa2, log10exp2;
    ...
    uint16 ref_dB = CHSPEC_IS2G(pi->radio_chanspec) ?
        params->cal_refdb_2g : (CHSPEC_IS5G(pi->radio_chanspec) ? params->cal_refdb_5g :
        params->cal_refdb_6g);
    ...
    int8 wbpapd_bbm_gctrl_iter = (ACMAJORREV_131(pi->pubpi->phy_rev) && !PHY_EPAPD(pi)) ? 3 :
        0;
    int16 bbm_corr_dB;
    int32 bbm_corr_multipl;
    uint16 bbmult_dacg = bbmult;
    int8 sampleCaptureCore = -1;
    int32 vasip_sc_ctrl;
    uint8 bw_idx;
    uint32 end_eps_entry;

    if (ACMAJORREV_130(pi->pubpi->phy_rev) && CHSPEC_IS6G(pi->radio_chanspec) &&
        CHSPEC_IS80(pi->radio_chanspec)) {
        // Performance optimization for 6G 80M is WIP.
        // For now, 6G 20/40/160M performs better with params->cal_refdb_6g,
        // but 6G 80M performs better with params->cal_refdb_5g.
        ref_dB = params->cal_refdb_5g;
    }

    if (PHY_EPAPD(pi) && swctrl->valid && swctrl->band[band_idx].mode != 0) {
        xcore_wbpapd_cal_en = TRUE;
        core = swctrl->band[band_idx].lp_core[core];
        // core means on_core starting from here.
        if ((papdcali->wbpapd_sampcapt_iovar != -1) &&
            (for_core == papdcali->wbpapd_sampcapt_iovar)) {
            sampleCaptureCore = swctrl->band[band_idx].lp_core[for_core];
        }
    } else {
    ...
    if (params->ext_dyn_range) {
        start_lo = (uint16)(params->wbcal_start_ext & 0xFFFF);
        start_hi  = (uint8)((params->wbcal_start_ext >> 16) & 0x3F);
        scale_stop_lo = (uint16)(params->wbcal_scale_end_ext & 0xFFFF);
        scale_stop_hi  = (uint8)((params->wbcal_scale_end_ext >> 16) & 0x3F);
        wbcal_const_pow_scale = params->wbcal_const_pow_scale_ext;
    }
    ...
    if (params->ext_dyn_range) {
        MOD_PHYREGCEE(pi, dpdeq_loopback_config1, core, loopback_eq_cal_start_ctr, 1650);
    } else {
    ...
    if (ACMAJORREV_130_131(pi->pubpi->phy_rev)) {
        if (params->ext_dyn_range) {
            MOD_PHYREGCE(pi, wbcal_ctl_1A, core, wbpapd_scale_window_len_pow,
                params->wbcal_window_len_pow_ext);
            MOD_PHYREGCE(pi, wbcal_ctl_22, core, wbcal_dc_accum_wait,
                params->wbcal_dc_accum_wait_ext);
        } else {
            MOD_PHYREGCE(pi, wbcal_ctl_1A, core, wbpapd_scale_window_len_pow,
    ...
    if (papdcali->pacalidx_iovar != -1) {
    ...
    } else if (ACMAJORREV_131(pi->pubpi->phy_rev) && PHY_EPAPD(pi)) {
        if (CHSPEC_IS160(pi->radio_chanspec)) {
            bw_idx = 3;
        } else if (CHSPEC_IS80(pi->radio_chanspec)) {
            bw_idx = 2;
        ...
        papd_calidx = CHSPEC_IS2G(pi->radio_chanspec) ?
            params->papd_calidx_2gBW[bw_idx >= 2 ? 0 : bw_idx] :
            params->papd_calidx_5gBW[bw_idx >= 4 ? 0 : bw_idx];
    } else {
        papd_calidx = (CHSPEC_IS2G(pi->radio_chanspec)) ? params->papd_calidx_2g :
            ((CHSPEC_IS5G(pi->radio_chanspec)) ? params->papd_calidx_5g :
            params->papd_calidx_6g);
        if (ACMAJORREV_130(pi->pubpi->phy_rev) && (wbpapd_gctrl == 3) &&
            (papdcali->stateTssigcWfmTruncIdx != 0xFFFF)) {
            papd_calidx = papdcali->stateTssigcCalIdx[for_core];
        }
    }
    ...
    while (papd_lut_select < num_eps_tables) {
        if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
            /* Different DAC rate modes result in different Tx powers at DAC.
             * Compensate here so cal power is the same for BW20 and BW40,
             * leading to the same PA amount of compression.
             * Warning: for iPA, scaling is used only for cal, not for comp
             * (final epsilon offset calculation).
             * Nowdays with iPA bbmult gctrl no need to do so,
             * but in iPA case we want to maintain old NVRAM compatibility.
             * That is why original bbmult variable is not modified for iPA.
             * But for ePA we want the scaled value also to be used for comp
             * (final epsilon offset calculation).
             */
            if (PHY_IPA(pi)) {
                if (CHSPEC_IS20(pi->radio_chanspec)) {
                ...
                if (CHSPEC_IS20(pi->radio_chanspec)) {
                    ...
                    /* Roughly divide by sqrt(2) */
                    bbmult = bbmult * 7 / 10;
                }
                /* For ePA/DPD use the same BBMULT value for both cal and comp */
                bbmult_dacg = bbmult;
            }
        }
        /* Write Tx gains */
        ...
        if (CHSPEC_IS2G(pi->radio_chanspec)) {
            ...
            papd_gainctrl_calidx_6g[for_core] = papd_calidx;
        }
        papd_gainctrl_bbmult[for_core] = bbmult;

        /* Write TIA gain */
        WRITE_PHYREGCE(pi, RfctrlCoreRXGAIN1, core, 0);
        MOD_PHYREGCE(pi, RfctrlCoreRXGAIN1, core, rxrf_tia_gain, tia_mode);
        if (ACMAJORREV_130(pi->pubpi->phy_rev) && CHSPEC_ISPHY5G6G(pi->radio_chanspec)) {
            /* Special IDAC cal for TIA gain in PAPD loopback path */
            phy_ac_dccal_papd_special(pi, tia_mode, core);
        }
        if (ACMAJORREV_131(pi->pubpi->phy_rev) && PHY_EPAPD(pi)) {
            /* Run external DC cal */
            MOD_PHYREGCE(pi, wbcal_ctl_22, core, wbpapd_cal_dcc_en, 0);
            MOD_PHYREGCE(pi, wbcal_ctl_23, core,
                wbpapd_cal_dc_corr_override_en, 1);
            WRITE_PHYREGCE(pi, wbcal_ctl_24, core, 0);
            /* Will not do RX2TX here since it causes WBCAL SPINWAIT issues.
             * LPF is already configured in TX path so DC estimate shouldn't be
             * affected.
             */
            phy_ac_dccal_idac4papd(pi, core, 1);
            MOD_PHYREGCE(pi, wbcal_ctl_22, core, wbpapd_cal_dcc_en, 1);
            MOD_PHYREGCE(pi, wbcal_ctl_23, core,
                wbpapd_cal_dc_corr_override_en, 0);
        }
        /* Widen Rx filter */
        phy_ac_wbcal_widen_filter(pi, tia_mode, core);
        // Let the logged tia_gain follow the core for which the cal is done.
        papd_tiagain[for_core] = tia_mode;
        ...
        if (wbstate == CAL_EPSOFFSET) {
            ...
            MOD_PHYREGCE(pi, wbcal_ctl_10, core, wbcal_stop_lo, stop_lo);
            if ((papdcali->wbpapd_sampcapt_iovar != -1) &&
                (core == sampleCaptureCore) && (wbstate == FIRST)) {
                MOD_PHYREGCE(pi, wbcal_ctl_11, core, wbcal_stop_hi, 15);
            } else {
                MOD_PHYREGCE(pi, wbcal_ctl_11, core, wbcal_stop_hi, stop_hi);
            }
        ...
        if (wbstate == CAL_EPSOFFSET) {
            ...
            if ((papdcali->wbpapd_sampcapt_iovar != -1) &&
                (core == sampleCaptureCore) && (wbstate == FIRST)) {
                PHY_PAPD(("Calling 5_28nm sample capture on core %d.\n", core));
                // mainMUX = 8, Pack = 2, Trig = Force, secMUX = 5
                vasip_sc_ctrl = (0x8 << 20) | ((core & 0x3) << 8) | 0x85;
                wlc_vasip_sample_capture_set(pi, vasip_sc_ctrl);
            }
        ...
        if (ACMAJORREV_130_131(pi->pubpi->phy_rev)) {
            // Let the logged coeffs follow the core for which the cal is done.
            papd_prefilter_coeff_i[for_core][0] = READ_PHYREG(pi,
                loopback_lmseq_debug_group0);
            papd_prefilter_coeff_q[for_core][0] = READ_PHYREG(pi,
                loopback_lmseq_debug_group1);
            papd_prefilter_coeff_i[for_core][1] = READ_PHYREG(pi,
                loopback_lmseq_debug_group2);
            papd_prefilter_coeff_q[for_core][1] = READ_PHYREG(pi,
                loopback_lmseq_debug_group3);
            papd_prefilter_coeff_i[for_core][2] = READ_PHYREG(pi,
                loopback_lmseq_debug_group4);
            papd_prefilter_coeff_q[for_core][2] = READ_PHYREG(pi,
                loopback_lmseq_debug_group5);
            papd_prefilter_coeff_i[for_core][3] = READ_PHYREG(pi,
                loopback_lmseq_debug_group6);
            papd_prefilter_coeff_q[for_core][3] = READ_PHYREG(pi,
                loopback_lmseq_debug_group7);
            papd_prefilter_coeff_i[for_core][4] = READ_PHYREG(pi,
                loopback_lmseq_debug_group8);
            papd_prefilter_coeff_q[for_core][4] = READ_PHYREG(pi,
                loopback_lmseq_debug_group9);
            papd_prefilter_coeff_i[for_core][5] = READ_PHYREG(pi,
                loopback_lmseq_debug_group10);
            papd_prefilter_coeff_q[for_core][5] = READ_PHYREG(pi,
                loopback_lmseq_debug_group11);
            papd_prefilter_coeff_i[for_core][6] = READ_PHYREG(pi,
                loopback_lmseq_debug_group12);
            papd_prefilter_coeff_q[for_core][6] = READ_PHYREG(pi,
        ...
        PHY_PAPD(("%s core%d sum_TX %d, sum_RX %d, scale_out 0x%x, scale_in 0x%x\n",
            ...
            wbcal_scale_int_in));
        if (wbstate != CAL_EPSOFFSET) {
            wbcal_sum_RX_dump[for_core] = wbcal_sum_RX;
            wbcal_sum_TX_dump[for_core] = wbcal_sum_TX;
            wbcal_scale_int_in_dump[for_core] = wbcal_scale_int_in;
            wbcal_scale_int_out_dump[for_core] = wbcal_scale_int_out;
        }
        ...
        if (wbstate != CAL_EPSOFFSET) {
            err = phy_ac_wbcal_eps_stopidx(pi, core, &end_eps_entry);
            if (err == 0) {
        ...
        if ((wbstate == FIRST) && ((wbpapd_gctrl == 2) || (wbpapd_gctrl == 3))) {
            /* Use initial cal results for Tx and Rx gain control */

            // Step 1. Tx gain control.
            if (papdcali->pacalidx_iovar == -1) {
                /* Do this only when no calidx overriding with iovar */
                if ((wbpapd_gctrl == 2) || ((wbpapd_gctrl == 3) &&
                    (papdcali->stateTssigcWfmTruncIdx == 0xFFFF))) {
            ...
            if (papdcali->papdtiagain_iovar == -1) {
                ...
                if (!ACMAJORREV_130(pi->pubpi->phy_rev)) {
                    qm_log10((int32)(wbcal_sum_RX), 0, &log10mantissa2,
                        &log10exp2);
                    // Align the exponent to 10.
                    if (log10exp2 > 10) {
                    ...
                    // 10 fractional bits.
                    tia_gain_adj_dB_part1 = (log10mantissa1 - log10mantissa2)
                        * 20;
                    // 10 fractional bits.
                    tia_gain_adj_dB_part2 = error_stop_index << 8;
                    tia_gain_adj_dB = tia_gain_adj_dB_part1 -
                        tia_gain_adj_dB_part2;
                    tia_gain_mode_delta = (tia_gain_adj_dB / 3) >> 10;
                    tia_mode = tia_mode + tia_gain_mode_delta;
                } else {
                    tia_mode = wbpapd_tia_gain_control(pi, wbcal_scale_int_in,
                        wbcal_scale_int_out, end_eps_entry,
                        wbcal_gfrac_bits,
                        tia_mode, diff_cal_idx);
                }
                if (tia_mode > 15) {
                    /* TIA hard limit high */
                    tia_mode = 15;
            ...
            // wbstate == CAL_EPSOFFSET
            calculate_calibrated_epsOffset(pi, core, papd_calidx, papd_lut_select,
                wbcal_gfrac_bits, eps_table, &calc_epsoffset);
            wbstate = FIRST;
            cals_on_the_core = 0;
            // Go to cal next table.
            papd_lut_select++;
    ...
    if (ACMAJORREV_131(pi->pubpi->phy_rev)) {
        ...
        phy_ac_papd_find_maxamam_endfill_epstbl(pi, for_core, FALSE);
    }
...
static int8
phy_ac_wbcal_eps_stopidx(phy_info_t *pi, uint8 core, uint32 *end_eps_entry)
{
    uint16 eps_table_size = phy_ac_papdcal_eps_table_size(pi, core);
    uint8 tbl_step = 1;
    uint8 stop_idx;
    uint32 eps_table_entry = 0;
    uint8 papd_lut_select = 0;
    int32 multi_tbl_en = pi->u.pi_acphy->papdcali->wbpapd_multitbl_iovar;
    uint8 epsilon_table_ids[] = { ACPHY_TBL_ID_EPSILON0, ACPHY_TBL_ID_EPSILON1,
        ACPHY_TBL_ID_EPSILON2, ACPHY_TBL_ID_EPSILON3};
    ...
    for (stop_idx = eps_table_size; stop_idx > 0; stop_idx--) {
        wlc_phy_table_read_acphy_dac_war(pi, epsilon_table_ids[core], 1,
            tbl_step * stop_idx + eps_table_size * papd_lut_select, 32,
            &eps_table_entry, core);
        if (eps_table_entry > 0) {
            break;
        }
    }

    *end_eps_entry = eps_table_entry;
    return stop_idx;
}
    ...
    bool load)
{
    ...
    if (ACMAJORREV_130(pi->pubpi->phy_rev)) {
        if (CHSPEC_IS80(pi->radio_chanspec) || CHSPEC_IS160(pi->radio_chanspec))
            dac_rate_mode = params->dac_rate_mode_80M_160M;
    } else if (ACMAJORREV_131(pi->pubpi->phy_rev) && CHSPEC_IS160(pi->radio_chanspec)) {
        dac_rate_mode = params->dac_rate_mode_80M_160M;
    }
    ...
    if (start) {
        ...
        // Modify start_ptr and len based on stateTssigcWfmTruncIdx.
        if (papdcali->stateTssigcWfmTruncIdx != 0xFFFF &&
            papdcali->stateTssigcWfmTruncIdx > 0) {
            PHY_PAPD(("Original start_ptr = %d, len = %d. ", start_ptr, len));
            if (start_ptr > 0) {
                // epsilonOffset cal.
                start_ptr = 2 * papdcali->stateTssigcWfmTruncIdx;
            } else {
                // main cal.
                len = 2 * papdcali->stateTssigcWfmTruncIdx;
            }
            PHY_PAPD(("Modified start_ptr = %d, len = %d.\n", start_ptr, len));
        }
        ...
        if (ACMAJORREV_131(pi->pubpi->phy_rev) && PHY_EPAPD(pi)) {
            wlc_phy_force_rfseq_rx2tx_wbcal_war(pi);
        } else {
...
/* Forcing gated clocks on during RX2TX causes WBPAPD spinwait timeout issues on REV131.
 * WAR is not to force gated clocks on during RX2TX.
 * BCAWLAN-229433
 */
static void
wlc_phy_force_rfseq_rx2tx_wbcal_war(phy_info_t *pi)
{
    uint16 trigger_mask = ACPHY_RfseqTrigger_rx2tx_MASK(pi->pubpi->phy_rev);
    uint16 status_mask = ACPHY_RfseqStatus0_rx2tx_MASK(pi->pubpi->phy_rev);
    ...
    /* Skip forcing gated clocks on - WAR */
    /* wlapi_bmac_phyclk_fgc(pi->sh->physhim, ON);
     * W_REG(pi->sh->osh, D11_PSM_PHY_CTL(pi), (uint16)0x6);
     */
    /* Disable Stalls */
    if (stall_val == 0) {
        ACPHY_DISABLE_STALL(pi);
    ...
    ACPHY_ENABLE_STALL(pi, stall_val);
    OSL_DELAY(1);
    /* Skip forcing gated clocks off - WAR */
    /* W_REG(pi->sh->osh, D11_PSM_PHY_CTL(pi), (uint16)phyctl_save);
...
phy_ac_papd_cal_eps_calc_tiny(phy_info_t *pi, uint8 core, uint16 *bbmult)
{
    ...
    papd_params = papdcali->papd_params;
            ...
            if (is5g) {
                if (ACMAJORREV_128(pi->pubpi->phy_rev)) {
                    ...
                    if (IS20MHZ(pi)) {
                        ...
                        lut_shift += papdcali->pacalshift5g[3];
                    }
                }
            } else {
                if (IS20MHZ(pi)) {
    ...
    if (ACMAJORREV_129_130(pi->pubpi->phy_rev) || ACMAJORREV_128(pi->pubpi->phy_rev)) {
        ...
        epsilonoffsettemp = -24 - 2*dig_gain_dB + lut_shift -
            ...
            (CHSPEC_IS5G(pi->radio_chanspec) ? papd_params->cal_refdb_5g :
            papd_params->cal_refdb_6g));
    } else {
...
phy_ac_papd_cal_set_tx_gain(phy_info_t *pi, uint8 core, uint16 *bbmult, uint8 *calmode)
{
    ...
    if (ACMAJORREV_51_129_130_131(pi->pubpi->phy_rev) ||
        ...
        if (is2g) {
            ...
            papd_gainctrl_calidx_6g[core] = tx_idx;
        }
...
{
    phy_ac_papdcal_info_t *papdcali = pi->u.pi_acphy->papdcali;
    phy_ac_papdcal_params_t *params = papdcali->papd_params;
    ...
    const uint16 scalem9dB = 23253; // 1/(2*sqrt(2)) approximated as 23253/2^16.
    const uint16 scale1p898 = 3887; // 1.898 * 2^11.
    const uint16 scalem0dB = 32768; // divide samples by 1 scaled up x2^15
    const uint16 scalem5dB = 58271; // divide samples by 1.77828 scaled up x2^15
    const uint32 scalem10dB = 103622; // divide samples by 3.16228 scaled up x2^15
    ...
    int32 scalediv14 = 1;
    int32 scalediv23 = 1;
    const int16 scalestep14 = (int16)((((uint32)(scalem10dB - scalem5dB)) * 64) / (len ? len :
        1));
    const int16 scalestep23 = (int16)((((uint32)(scalem5dB - scalem0dB)) * 64) / (len ? len :
        1));
    ...
    } else if (papdcali->stateTssigcWfmTruncIdx < len*8) {
        wPtr1 = 0; // Won't be used in this case
        wPtr2 = 0; // Won't be used in this case
        wPtr3 = startidx + papdcali->stateTssigcWfmTruncIdx*4*2 - 4;
        wPtr4 = startidx + papdcali->stateTssigcWfmTruncIdx*4*2;
        // Reduce len because of truncation.
        len = papdcali->stateTssigcWfmTruncIdx / 8;
    } else { // len*8 < stateTssigcWfmTruncIdx < len*8*2
        wPtr1 = startidx + len*8*4;
        wPtr2 = startidx + (papdcali->stateTssigcWfmTruncIdx*2-len*8)*4 - 4;
        wPtr3 = startidx + papdcali->stateTssigcWfmTruncIdx*4*2 - 4;
        wPtr4 = startidx + papdcali->stateTssigcWfmTruncIdx*4*2;
    }
    PHY_PAPD(("%s: len = %d, wPtr0 = %d, wPtr1 = %d, wPtr2 = %d, wPtr3 = %d, wPtr4 = %d.\n",
        __FUNCTION__, len, wPtr0, wPtr1, wPtr2, wPtr3, wPtr4));
    ...
    for (k0 = 0; k0 < len; k0++) {
        if (params->ext_dyn_range) {
            /* Scale the waveform samples to extend dynamic range of the waveform
             * by 10dB towards the lower amplitudes.
             * Lower (1, 4) and upper (2, 3) segments should meet exactly in the middle
             * (at sample len for lower and sample 0 for higher segments),
             * and both scaled -5dB at that point.
             */
            scalediv14 = scalem5dB + (((len - k0 - 1) * scalestep14) / 64);
            scalediv23 = scalem5dB - ((k0 * scalestep23) / 64);
        }
        ...
        for (k1 = 0; k1 < 2; k1++) {
            ...
            for (k2 = 0; k2 < 2; k2++) {
                ...
                for (k3 = 0; k3 < 2; k3++) {
                    /* Segments 1 & 4. */
                    /* Scale samples to increase waveform dynamic range */
                    if (params->ext_dyn_range) {
                        fOuti = (int16)((((int32)hbf2Outi[k3]) * (1 << 15))
                                / scalediv14);
                        fOutq = (int16)((((int32)hbf2Outq[k3]) * (1 << 15))
                                / scalediv14);
                    } else {
                        fOuti = hbf2Outi[k3];
                        fOutq = hbf2Outq[k3];
                    ...
                    if (papdcali->stateTssigcWfmTruncIdx == 0xFFFF ||
                        ...
                        tmpi = samplei * scalem9dB;
                        tmpq = sampleq * scalem9dB;
                        fOuti = tmpi >> 16;
                        fOutq = tmpq >> 16;
                        fOut = (((uint16)fOuti)<<16) | (uint16)fOutq;
                        W_REG(pi->sh->osh, D11_XMT_TEMPLATE_RW_PTR(pi),
                    ...
                    /* Segments 2 & 3.
                     * Segments 2 & 3 should not be scaled down extra 5dB.
                     */
                    if ((papdcali->stateTssigcWfmTruncIdx == 0xFFFF) ||
                        (papdcali->stateTssigcWfmTruncIdx == 0) || (k4 +
                        len * 8 < papdcali->stateTssigcWfmTruncIdx)) {
                        tmpi = samplei * scale9dB + (samplei<<16);
                        tmpq = sampleq * scale9dB + (sampleq<<16);
                        /* Scale samples to increase waveform dynamic
                         * range.
                         */
                        if (params->ext_dyn_range) {
                            fOuti = (int16)((((int32)(tmpi >> 15)) *
                                (1 << 15)) / scalediv23);
                            fOutq = (int16)((((int32)(tmpq >> 15)) *
                                (1 << 15)) / scalediv23);
                        } else {
                            fOuti = tmpi >> 15;
                            fOutq = tmpq >> 15;
                        ...
                        if (papdcali->stateTssigcWfmTruncIdx != 0xFFFF &&
                            papdcali->stateTssigcWfmTruncIdx != 0) {
                            // Scale 9 dB then -9 dB means no scaling.
                            fOut = (((uint16)samplei)<<16) |
                                (uint16)sampleq;
                        }
                        W_REG(pi->sh->osh, D11_XMT_TEMPLATE_RW_PTR(pi),
                            wPtr2);
        ...
        bool only_report_maxamam_idx)
{
    ...
    uint8 backstep = PHY_EPAPD(pi) ? 3 : 7;
    ...
    /* Max AMAM happens to be one of last backstep eps elements */
    eps_stop_idx = eps_stop_idx > backstep ? eps_stop_idx - backstep : 0;
...
void
wlc_phy_tssi_to_power_acphy(phy_info_t *pi)
{
    phy_ac_papdcal_info_t *papdcali;
    int16 tssi, idle_tssi, normalized_tssi, tbl_offset;
    uint8 estpwr;
    uint8 core;

    papdcali = (phy_ac_papdcal_info_t *)pi->u.pi_acphy->papdcali;

    for (core = 0; core < PHYCORENUM((pi)->pubpi->phy_corenum); core ++) {
        papdcali->stateCts2selfTssiPwr[core] = 127;
    }

    FOREACH_CORE(pi, core) {
        tssi = READ_PHYREGFLDCE(pi, TssiVal_path, core, TSSIVal);
        idle_tssi = READ_PHYREGCE(pi, TxPwrCtrlIdleTssi_path, core);

        tssi -= (tssi >= 512) ? 1024 : 0;
        idle_tssi -= (idle_tssi >= 512) ? 1024 : 0;

        normalized_tssi = 1024 - (tssi - idle_tssi);

        tbl_offset = normalized_tssi >> 3;
        // Saturate to [0, 127]
        tbl_offset = (tbl_offset < 0) ? 0 : ((tbl_offset > 127) ? 127: tbl_offset);
        wlc_phy_table_read_acphy(pi, ACPHY_TBL_ID_ESTPWRLUTS(core), 1,
            tbl_offset, 8, &estpwr);

        papdcali->stateCts2selfTssiPwr[core] = (int8)estpwr;
        PHY_PAPD(("%s: After CTS2SELF, TSSI on core %d = %d, idle_tssi = %d, ",
            __FUNCTION__, core, tssi, idle_tssi));
        PHY_PAPD(("normalized_tssi = %d, tbl_offset = %d, pwr = %d/4 dBm.\n",
            normalized_tssi, tbl_offset, (int8)estpwr));
    }
}
...
{
    phy_ac_papdcal_info_t *papdcali;
    int8 peak_psd_limit = 0, tmp;
    uint8 delta_cts2self_wbwfm = 5;
    uint8 base_cal_idx = 41;
    int16 peak_psd_dbm_MHz, backoff, truncate_3dB_steps;
    int8 backoff_residual;
    int8 min_tssi_pwr;
    uint8 core;
    uint16 wfm_truncate_LUT[] = {59896, 46864, 29440, 21496, 8464}; //Each is a multiple of 8.
    phy_ac_papdcal_params_t *params;

    papdcali = (phy_ac_papdcal_info_t *)pi->u.pi_acphy->papdcali;
    params    = papdcali->papd_params;

    min_tssi_pwr = papdcali->stateCts2selfTssiPwr[0];

    // Obtain peak PSD limit from SROM/NVRAM.
    if (CHSPEC_IS2G(pi->radio_chanspec)) {
        peak_psd_limit = (params->peak_psd_limit_6G2G) & 0xFF;
    } else if (CHSPEC_IS5G(pi->radio_chanspec)) {
        if (CHSPEC_CHANNEL(pi->radio_chanspec) <= 48) {
            peak_psd_limit = (params->peak_psd_limit_5G0) & 0xFF;
        } else if (CHSPEC_CHANNEL(pi->radio_chanspec) <= 64) {
            peak_psd_limit = (params->peak_psd_limit_5G0) >> 8;
            if (CHSPEC_IS160(pi->radio_chanspec)) {
                // Channel 50o
                tmp = (params->peak_psd_limit_5G0) & 0xFF;
                peak_psd_limit = (peak_psd_limit > tmp) ? tmp : peak_psd_limit;
            }
        } else if (CHSPEC_CHANNEL(pi->radio_chanspec) <= 144) {
            peak_psd_limit = (params->peak_psd_limit_5G1) & 0xFF;
        } else if (CHSPEC_CHANNEL(pi->radio_chanspec) <= 165) {
            peak_psd_limit = (params->peak_psd_limit_5G1) >> 8;
            if (CHSPEC_IS160(pi->radio_chanspec)) {
                // Channel 163o
                tmp = (params->peak_psd_limit_5G2) & 0xFF;
                peak_psd_limit = (peak_psd_limit > tmp) ? tmp : peak_psd_limit;
            }
        } else {
            peak_psd_limit = (params->peak_psd_limit_5G2) & 0xFF;
            if (CHSPEC_IS80(pi->radio_chanspec)) {
                // Channel 171q
                tmp = (params->peak_psd_limit_5G1) >> 8;
                peak_psd_limit = (peak_psd_limit > tmp) ? tmp : peak_psd_limit;
            }
        }
    } else if (CHSPEC_IS6G(pi->radio_chanspec)) {
        peak_psd_limit = (params->peak_psd_limit_6G2G) >> 8;
    }
    PHY_PAPD(("%s: peak_psd_limit = %d. CHANNEL = %d.\n", __FUNCTION__, peak_psd_limit,
        CHSPEC_CHANNEL(pi->radio_chanspec)));

    // Override with iovar, if available.
    if (papdcali->peak_psd_limit_iovar > 0) {
        peak_psd_limit = papdcali->peak_psd_limit_iovar;
    }

    if (peak_psd_limit > 20) {
        // Do not pursue peak PSD limit higher than 20 dBm/MHz.
        peak_psd_limit = 0;
    }

    if (peak_psd_limit > 0) {
        // Find the minimum in TSSI power.
        for (core = 1; core < PHYCORENUM((pi)->pubpi->phy_corenum); core ++) {
            if (min_tssi_pwr > papdcali->stateCts2selfTssiPwr[core]) {
                min_tssi_pwr = papdcali->stateCts2selfTssiPwr[core];
            }
        }
        PHY_PAPD(("%s: min_tssi_pwr = %d\n", __FUNCTION__, min_tssi_pwr));

        peak_psd_dbm_MHz = min_tssi_pwr + (delta_cts2self_wbwfm << 2);
        backoff = peak_psd_dbm_MHz - (peak_psd_limit << 2);
        truncate_3dB_steps = (backoff * 341 + 2048) >> 12; // round(backoff/3.0)

        if (truncate_3dB_steps > 0) {
            papdcali->stateTssigcWfmTruncIdx =
                wfm_truncate_LUT[truncate_3dB_steps - 1];
        } else {
            papdcali->stateTssigcWfmTruncIdx = 0;
        }
        PHY_PAPD(("%s: peak_psd_dbm_MHz = %d, peak_psd_limit = %d, backoff = %d\n",
            __FUNCTION__, peak_psd_dbm_MHz, peak_psd_limit, backoff));
        PHY_PAPD(("%s: truncate_3dB_steps = %d, stateTssigcWfmTruncIdx=%d\n",
            __FUNCTION__, truncate_3dB_steps, papdcali->stateTssigcWfmTruncIdx));

        for (core = 0; core < PHYCORENUM((pi)->pubpi->phy_corenum); core ++) {
            peak_psd_dbm_MHz = papdcali->stateCts2selfTssiPwr[core] +
                (delta_cts2self_wbwfm << 2);
            backoff = peak_psd_dbm_MHz - (peak_psd_limit << 2);
            backoff_residual = backoff - truncate_3dB_steps * 3 * 4;
            papdcali->stateTssigcCalIdx[core] = base_cal_idx +
                ((backoff_residual + 1) >> 1);
            PHY_PAPD(("core = %d, peak_psd_dbm_MHz = %d, backoff = %d\n",
                core, peak_psd_dbm_MHz, backoff));
            PHY_PAPD(("backoff_residual = %d, stateTssigcCalIdx[core]=%d\n",
                backoff_residual, papdcali->stateTssigcCalIdx[core]));
        }
    }
}
    ...
    int8 diff_cal_idx)
{
    ...
    //uint32 target = 3500;
    //uint32 peakTx;
    //uint32 peakTx_tbl[6][3] = {{4031, 3554, 3266}, {2294, 2128, 2027}, {1517, 1429, 1354},
    //    {1026, 958, 902}, {811, 753, 716}, {537, 529, 527}};

    int16 log10mantissa_eps;
    int16 log10mantissa_txscale;
    int16 log10mantissa_rxscale;
    uint16 one = (1 << gfrac_bits);
    uint8 tia2 = 0;
    int16 offset_eps = gfrac_bits * 616; // gfrac_bits * 6.02/10*2^10
    uint16 wfm_truncate_LUT[] = {59896, 46864, 29440, 21496, 8464}; //Each is a multiple of 8.
    uint8 wfm_trunc_idx, bw_idx;
    bool eps_gt_1;

    // Pre-calculated constant.
    int16 log10mantissa_target = 7258;
    // Pre-calculated constant from peakTx_tbl.
    int16 log10mantissa_peakTx_tbl[6][3] = {{7385, 7273, 7198}, {6884, 6818, 6774},
        {6517, 6464, 6416}, {6169, 6108, 6055}, {5960, 5894, 5850}, {5594, 5580, 5577}};
    int16 log10mantissa_peakTx;

    if (papdcali->stateTssigcWfmTruncIdx == 0xFFFF || papdcali->stateTssigcWfmTruncIdx == 0) {
        wfm_trunc_idx = 0;
    } else if (papdcali->stateTssigcWfmTruncIdx == wfm_truncate_LUT[0]) {
        wfm_trunc_idx = 1;
    } else if (papdcali->stateTssigcWfmTruncIdx == wfm_truncate_LUT[1]) {
        wfm_trunc_idx = 2;
    } else if (papdcali->stateTssigcWfmTruncIdx == wfm_truncate_LUT[2]) {
        wfm_trunc_idx = 3;
    } else if (papdcali->stateTssigcWfmTruncIdx == wfm_truncate_LUT[3]) {
        wfm_trunc_idx = 4;
    } else {
        wfm_trunc_idx = 5;
    }
    ...
    log10mantissa_peakTx = log10mantissa_peakTx_tbl[wfm_trunc_idx][bw_idx];

    phy_papdcal_decode_epsilon(end_eps_entry, &eps_re, &eps_im);
    tempclip = (one + eps_re) * (one + eps_re) + (eps_im * eps_im);
    eps_gt_1 = (tempclip > (1<<(2*gfrac_bits))); // eps > 1
    PHY_PAPD(("%s eps_re %d, eps_im %d, tempclip %d, eps_gt_1 %d\n",
        __FUNCTION__, eps_re, eps_im, tempclip, eps_gt_1));

    // Calculate 20*log10(peakEps).
    log10mantissa_eps = scale_log10(tempclip);

    //// Calculate 20*log10(target).
    //tempclip = target * target;
    //log10mantissa_target = scale_log10(tempclip);

    //// Calculate 20*log10(peakTx).
    //for (wfm_trunc_idx = 0; wfm_trunc_idx < 6; wfm_trunc_idx ++) {
    //    for (bw_idx = 0; bw_idx < 3; bw_idx ++) {
    //        peakTx = peakTx_tbl[wfm_trunc_idx][bw_idx];
    //        tempclip = peakTx * peakTx;
    //        log10mantissa_peakTx = scale_log10(tempclip);
    //    }
    //}

    // Calculate 20*log10(wbcal_scale_int_in).
    tempclip = wbcal_scale_int_in * wbcal_scale_int_in;
    log10mantissa_rxscale = scale_log10(tempclip);

    // Calculate 20*log10(wbcal_scale_int_out).
    tempclip = wbcal_scale_int_out * wbcal_scale_int_out;
    log10mantissa_txscale = scale_log10(tempclip);

    // Calculate new Tia gain.
    //tia2 = (log10mantissa_target - (log10mantissa_peakTx + log10mantissa_txscale -
    //    log10mantissa_eps + offset_eps - log10mantissa_rxscale) +
    //     diff_cal_idx / 2 * 1024 / 10) * 10 / 3 / 1024 + tia1;
    if (((ACMAJORREV_130(pi->pubpi->phy_rev) && CHSPEC_IS6G(pi->radio_chanspec)) ||
        !ACMAJORREV_130(pi->pubpi->phy_rev)) && ((diff_cal_idx >= 0) && eps_gt_1)) {
        // tx_idx2 > tx_idx1, power of main cal < power of the 1st cal.
        tia2 = (log10mantissa_target - (log10mantissa_peakTx + log10mantissa_txscale -
            log10mantissa_rxscale) +
            diff_cal_idx * 51) * 10 / 3072 + tia1;
    } else {
        // tx_idx2 < tx_idx1, power of main cal > power of the 1st cal.
        tia2 = (log10mantissa_target - (log10mantissa_peakTx + log10mantissa_txscale -
            log10mantissa_eps + offset_eps - log10mantissa_rxscale) +
...
static int16 scale_log10(uint32 input)
{
    /* This function shifts the input so that its 2nd MSB is 1,
     * then call qm_log10 function to calculate the math function log10().
     * The purpose of doing this is to improve numerical accuracy of qm_log10.
     * In other words, normalize input of qm_log10 to the range of [2^30, 2^31-1].
     * Keep 10 fractional bits in the result.
     */
    uint32 scaling = 1, thresh = 2147483647; // 2^31-1
    uint8 i = 0;
    uint16 scaling_offset = 0;
    int16 log10mantissa, log10exp;

    while (!(thresh >= scaling*input && thresh/2 <= scaling*input) && i <= 31) {
        i++;
        scaling = scaling << 1; // multiplied by 2
        scaling_offset += 308; // log10(2) with 10 fractional bits.
    }
    if (i == 32) {
        // Normalization failed. Reset scaling and scaling offset.
        scaling = 1;
    ...
    // Multiplied by scaling factor to improve qm_log10 accuracy
    qm_log10((int32)(scaling*input), 0, &log10mantissa, &log10exp);
    PHY_PAPD(("%s input %d, scaling %d, scaling_offset %d, log10mantissa %d, log10exp %d, ",
        __FUNCTION__, input, scaling, scaling_offset, log10mantissa, log10exp));

    // Keep 10 fractional bits in mantissa.
    if (log10exp > 10) {
        log10mantissa = (log10mantissa + (1 << (log10exp - 11))) >> (log10exp - 10);
    } else if (log10exp < 10) {
        log10mantissa = log10mantissa << (10 - log10exp);
    }
    PHY_PAPD(("keeping 10 frac bits log10mantissa %d, ", log10mantissa));
    log10mantissa -= scaling_offset;
    PHY_PAPD(("minus scaling_offset log10mantissa %d\n", log10mantissa));

    return log10mantissa;
}
