 ...
 *
 * $Id: wlc_wet.c 809031 2022-03-04 20:27:49Z $
 */
...
void wlc_wet_dump_iplink(wlc_wet_info_t *weth, struct bcmstrbuf *b, wet_sta_t *sta, int ipver);
#endif
static void wlc_wet_free_iplink(wlc_wet_info_t *weth);
...
static uint8 llc_hdr_stp[SNAP_HDR_LEN] = {0x42, 0x42, 0x03, 0x00, 0x00, 0x00};
...
static inline void
remove_sta_from_iphash(wlc_wet_info_t *weth, wet_sta_t *sta, uint8 *iaddr, uint ipver)
{
    wet_sta_t *sta2, **next;
...
static int
wet_sta_add_ip(wlc_wet_info_t *weth, wet_sta_t *sta, uint8 ipver,
        uint8 *iaddr) {
    if (wet_sta_ip_inlink(sta, ipver, iaddr))
        ...
        if (p_sta == NULL) {
            WL_ERROR(("allocate ip space error!\n"));
            return WET_STA_HASH_OK;
        }
...
static int
wet_sta_remove_ip(wlc_wet_info_t *weth, wet_sta_t *sta, uint8 ipver,
        uint8 *iaddr) {
    wet_sta_iplink_t *p_sta = sta->ip_link[ipver];
    ...
    if (send) {
        ...
        */
        if (IP_ISMULTI(iaddr_dest) && !iaddr_src) {
            WL_INFORM(("wl%d:recv multicast frame from "MACF
                ". Don't update hash table\n",
                WLCUNIT(weth), ETHERP_TO_MACF(eh + ETHER_SRC_OFFSET)));
            wet_table_upd = FALSE;
        }

        if (wet_table_upd && wet_sta_update_all(weth, cfg, IPVER_4, iaddr,
        (struct ether_addr*)(eh + ETHER_SRC_OFFSET), &sta) < 0) {
            WL_ERROR(("wl%d: wet_ip_proc: unable to update STA %u.%u.%u.%u "MACF"\n",
                WLCUNIT(weth), iaddr[0], iaddr[1], iaddr[2],
                iaddr[3], ETHERP_TO_MACF(eh + ETHER_SRC_OFFSET)));
            return -1;
        }
        ea = (struct ether_addr *)WLCBSSCFGADDR(cfg);
    ...
    /* no action for received bcast/mcast ethernet frame */
        ...
        if (IP_ISMULTI(iaddr_dest)) {
            ...
            if (wet_sta_find_ip(weth, IPVER_4, iaddr, &sta) < 0) {
                WL_ERROR(("wl%d: %s: unable to find STA %u.%u.%u.%u\n",
        ...
        /* Receive mcast/bcast frame: convert dest MAC w/ PKTC defined */
        ea_off = ETHER_DEST_OFFSET;
        eacopy(eh + ea_off, &mcast_ea);
        ea = &mcast_ea;
    }
    ...
    uint8 *eh, uint8 *arph, int length, int send)
{
    ...
    if (send) {
        ...
        (struct ether_addr*)(eh + ETHER_SRC_OFFSET), &sta) < 0) {
            WL_ERROR(("wl%d: wet_arp_proc: unable to update STA %u.%u.%u.%u "MACF"\n",
                WLCUNIT(weth), iaddr[0], iaddr[1], iaddr[2], iaddr[3],
                ETHERP_TO_MACF(eh + ETHER_SRC_OFFSET)));
            return -1;
        }
    ...
    uint8 *eh, uint8 *iph, uint8 *udph, uint8 *dhcp, int length, int send)
{
    ...
    if (send) {
        ...
        (struct ether_addr*)(dhcp + DHCP_CHADDR_OFFSET), &sta) < 0) {
            WL_ERROR(("wl%d: wet_dhcpc_proc: unable to update STA "MACF"\n",
                WLCUNIT(weth), ETHERP_TO_MACF(dhcp + DHCP_CHADDR_OFFSET)));
            return -1;
        }
    ...
    /* relay recv'd reply to its destiny */
        ...
        (struct ether_addr*)(dhcp + DHCP_CHADDR_OFFSET), &sta) < 0) {
            WL_ERROR(("wl%d: wet_dhcpc_proc: unable to find STA "MACF"\n",
                WLCUNIT(weth), ETHERP_TO_MACF(dhcp + DHCP_CHADDR_OFFSET)));
            return -1;
        }
        /*
    ...
    /* it should not recv non-relay reply at all, but just in case */
        ...
        WL_ERROR(("wl%d: wet_dhcpc_proc: ignore recv'd frame from "MACF"\n",
            WLCUNIT(weth), ETHERP_TO_MACF(dhcp + DHCP_CHADDR_OFFSET)));
        return -1;
    }
    ...
    uint8 *eh, uint8 *iph, uint8 *udph, uint8 *dhcp, int length, int send)
{
    ...
    if (wet_sta_find_mac(weth, (struct ether_addr*)(dhcp + DHCP_CHADDR_OFFSET), &sta) < 0) {
        WL_ERROR(("wl%d: wet_dhcps_proc: unable to find STA "MACF"\n",
            WLCUNIT(weth), ETHERP_TO_MACF(dhcp + DHCP_CHADDR_OFFSET)));
        return -1;
    }
...
#define IP6_HDRLEN (40)
    ...
    if ((IP_VER(iph) != IP_VER_6) ||
        (length < IP6_HDRLEN)) {
        WL_INFORM(("wl%d: wet_ip_proc: %s non IPv6 frame, ignored\n",
            WLCUNIT(weth), send ? "send" : "recv"));
        return -1;
    }
    ...
    /* no action for received bcast/mcast ethernet frame */
        ...
        if (IPV6_ISMULTI(iaddr)) {
            ...
            if (wet_sta_find_ip(weth, IPVER_6, iaddr, &sta) < 0) {
                WL_ERROR(("wl%d: %s: unable to find STA "
    ...
    uint8 *eh, uint8 *iph, uint8 *icmp6h, int icmp6len, int send)
{
    ...
    if (icmp6len < sizeof(icmpv6_hdlr_t)) {
        WL_INFORM(("wl%d: %s: length too small. ignore icmp6len %d !!\n",
            WLCUNIT(weth), __FUNCTION__, icmp6len));
        return -1;
    }
    /*
     * Replace source MAC in Ethernet header as well as source MAC in
    ...
    if (send) {
        ...
        for (icmp6hdlr = &icmpv6_hash[WET_ICMPV6_HASH(type)];
            ...
            bcopy(WLCBSSCFGADDR(cfg), eh + ETHER_SRC_OFFSET, ETHER_ADDR_LEN);
            return (icmp6hdlr->icmpv6_proc)(weth, cfg, eh, iph, icmp6h, icmp6len, send);
        }
    }
    ...
    uint8 *eh, uint8 *iph, uint8 *udph, uint8 *dhcp, int length, int send)
{
    ...
    if (wet_sta_update_mac(weth, cfg,
        ...
        WL_ERROR(("wl%d: wet_dhcpc_proc: unable to update STA "MACF"\n",
            WLCUNIT(weth), ETHERP_TO_MACF(dhcp + duid_offset)));
        return -1;
    }
    ...
    uint8 *iaddr, struct ether_addr *eaddr, wet_sta_t **saddr)
{
    ...
    /* bail out if we can't find nor create any */
        ...
        WL_ERROR(("wl%d: wet_sta_update_all: unable to alloc STA ipv%d, %u.%u.%u.%u "
            MACF"\n",
            WLCUNIT(weth), ((ipver == IPVER_4) ? 4: 6),
            iaddr[0], iaddr[1], iaddr[2], iaddr[3],
            ETHERP_TO_MACF(eaddr)));
        return -1;
    }
    ...
    struct ether_addr *eaddr, wet_sta_t **saddr)
{
    ...
    /* bail out if we can't find nor create any */
        ...
        WL_ERROR(("wl%d: wet_sta_update_mac: unable to alloc STA "MACF"\n",
            WLCUNIT(weth), ETHERP_TO_MACF(eaddr)));
        return -1;
    }
...
wet_sta_remove_mac_entry(wlc_wet_info_t *weth, struct ether_addr *eaddr)
{
    ...
    if (!found) {
        WL_ERROR(("wl%d: wet_sta_remove_mac_entry: unable to find STA "MACF" entry\n",
            WLCUNIT(weth), ETHERP_TO_MACF(eaddr)));
        return -1;
    }
...
wet_sta_find_mac(wlc_wet_info_t *weth, struct ether_addr *eaddr, wet_sta_t **saddr)
{
    ...
    WL_ERROR(("wl%d: wet_sta_find_mac: unable to find STA "MACF"\n",
        WLCUNIT(weth), ETHERP_TO_MACF(eaddr)));
    return -1;
}
...
#if defined(BCM_NBUFF_PKT) && defined(SKB_BLOG_TOHANDLE_PKT)
        if (PKTISBLOG_TOHANDLE(sdu))
            PKTSETBLOG_TOHANDLE(pkt);
#endif
#if defined(BCM_BLOG)
        if (((struct sk_buff*)sdu)->blog_p) {
...
static void
wlc_wet_free_iplink(wlc_wet_info_t *weth)
{
    int i, ipver;
    wet_sta_t *sta;
...
void
wlc_wet_dump_iplink(wlc_wet_info_t *weth, struct bcmstrbuf *b, wet_sta_t *sta, int ipver)
{
    wet_sta_iplink_t *p_sta = (sta->ip_link[ipver]);
    while (p_sta) {
        if (ipver == IPVER_4)
...
wlc_wet_dump(wlc_wet_info_t *weth, struct bcmstrbuf *b)
{
    ...
    for (i = 0; i < WLC_MAX_URE_STA; i ++) {
        if (bcmp(&weth->mac[i], zeromacaddr, ETHER_ADDR_LEN) || (uint32)(weth->ip[i][0])) {
            bcm_bprintf(b, "Host MAC %u:"MACF"\n", i,
                ETHER_TO_MACF(weth->mac[i]));
    ...
    for (i = 0; i < WET_NUMSTAS; i ++) {
        ...
        if (!IPV6_ADDR_NULL(sta->ip[IPVER_6])) {
            bcm_bprintf(b, "%u\t%u\t"MACF"\t%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x:%x%x\n",
                ...
                sta->ip[IPVER_6][15]);
            wlc_wet_dump_iplink(weth, b, sta, IPVER_6);
