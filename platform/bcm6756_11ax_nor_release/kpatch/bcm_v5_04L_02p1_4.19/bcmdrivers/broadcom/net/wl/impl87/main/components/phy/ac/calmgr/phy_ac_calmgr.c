 ...
 *
 * $Id: phy_ac_calmgr.c 810937 2022-04-19 23:23:35Z $
 */
...
phy_ac_calmgr_wd(phy_type_calmgr_ctx_t *ctx)
{
    ...
    if (ACMAJORREV_5(pi->pubpi->phy_rev)) {
        wlc_phy_radio2069_check_vco_cal_acphy(pi);
    }
...
wlc_phy_cals_acphy(phy_type_calmgr_ctx_t *ctx, uint8 legacy_caltype, uint8 searchmode)
{
...
#ifdef WL_PROXD_PHYTS
    /* skip cal if ranging loopback is setup */
    if (phy_ac_tof_phyts_is_loopback_setup(pi)) {
        return;
    }
#endif /* WL_PROXD_PHYTS */
    ...
    /* Let CTS2SELF being transmitted to all hwtxchains
     * for proper wbcal all hwtxchains
     */
    if (PHY_PAPDEN(pi) && WBPAPD_ENAB(pi) && ACMAJORREV_130(pi->pubpi->phy_rev)) {
        orig_rxchains = stf_shdata->phyrxchain;
        wlapi_bmac_write_shm(pi->sh->physhim, M_COREMASK_BLK(pi) + (0x1 * 2),
    ...
    if (phase_id == PHY_CAL_PHASE_IDLE) {
        ...
        if (ACMAJORREV_32(pi->pubpi->phy_rev) || ACMAJORREV_33(pi->pubpi->phy_rev) ||
            ...
            if (pi->sh->up) {
                ...
                if (PHY_PAPDEN(pi) && WBPAPD_ENAB(pi) &&
                    ACMAJORREV_GE130(pi->pubpi->phy_rev)) {
                    wlc_phy_tssi_to_power_acphy(pi);
                }
            }
        }
