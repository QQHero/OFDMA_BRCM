 ...
 *
 * $Id: wlc_pub.h 806909 2022-01-07 10:25:53Z $
 */
...
#if defined(BCMDBG) || defined(WLTEST) || defined(DL_RU_STATS_DUMP)
#define BCMDBG_DUMP_MSCHED /* enables (downlink scheduler) 'wl dump msched' functionality */
#define BCMDBG_DUMP_SCB_ULMU /* enables uplink scheduler related entries in 'wl dump scb' output */
#endif

#if defined(BCMDBG) || defined(WLTEST) || defined(UL_RU_STATS_DUMP)
#define BCMDBG_DUMP_UMSCHED /* enables (uplink scheduler) 'wl dump umsched' functionality */
#endif

#if !defined(DONGLEBUILD) && defined(WLTEST) /* For MFGTEST NIC driver builds */
#define BCMDBG_DUMP_MUTX
#endif /* !DONGLEBUILD) && WLTEST */

#if !defined(BCMDBG_DUMP_MURX) && defined(WLCNT)
#if (defined(BCMDBG) || defined(WLDUMP) || defined(BCMDBG_MU))
#define BCMDBG_DUMP_MURX
#endif
#endif /* !BCMDBG_DUMP_MURX && WLCNT */

#if defined(BCMDBG)

#if !defined(BCMDBG_DUMP_D11CNTS)
#define BCMDBG_DUMP_D11CNTS
#endif /* !BCMDBG_DUMP_D11CNTS */

#if !defined(BCMDBG_DUMP_RATELINKMEM)
#define BCMDBG_DUMP_RATELINKMEM
#endif /* !BCMDBG_DUMP_RATELINKMEM */

#if !defined(BCMDBG_DUMP_TXBF)
#define BCMDBG_DUMP_TXBF
#endif /* !BCMDBG_DUMP_TXBF */

#if !defined(BCMDBG_DUMP_TWT)
#define BCMDBG_DUMP_TWT
...
typedef struct wlc_pub {
    ...
    uint8           _pref_transmit_bss;     /**< pref trasnmit bss index */
...
#ifdef BCM_CSIMON_AP
    bool            _csimon_ap;             /* Channel State Info Monitor feature for AP */
#endif
    ...
    bool        _iapp;            /* 802.11f/IAPP support */
    bool            _allow_bgdfs_on_apsta;     /* allow zdfs when sta is associated */
    ...
    uint8        _amsdu_max_msdu_num;        /* maximum number of MSDUs in an A-MSDU */
#ifdef MFP
    bool            _uc_mgmt_sw_encr;        /* Enable SW encryption for UC mgmt frames */
#endif /* MFP */
    uint32         _evt_diag_capture;
    bool            _airiq;            /* Airiq feature */
    int             newkey_pktdrop_timeout;    /* Max pkt drops duration in new key installation */
#if defined(WL_PRQ_RAND_SEQ)
    bool            _prq_rand_seq;      /* randomize probe seq is enabled */
#endif
#if defined(CONFIG_TENDA_WLAN_REPEATER) || (defined(TD_EASYMESH_SUPPORT) && defined(CONFIG_UGW_BCM_DONGLE))
    int32          wserver_pid;
#endif
#ifdef CONFIG_TENDA_PRIVATE_WLAN
    uint32          mcast_limit_thresh;
    uint32          mcast_pkts_len;
    uint32          mcast_drop_cnt;
    unsigned long   mcast_stats_timeout;
    bool            mcast_limit_enable;
#endif
} wlc_pub_t;

#ifdef WL_DIAG_CAPTURE_TXRX_FRAMES
extern void wlc_diag_mgmt_ctl(wlc_info_t *wlc, int evt, uint16 fc,
    wlc_bsscfg_t *bsscfg, void *hdr, int len);
#define CAPTURE_MGMT_FRAMES    1
#define CAPTURE_MGMT_CTL_FRAMES    2
#define DIAG_CAPTURE_NONE(wlc)    (!(wlc)->pub->_evt_diag_capture)
#define DIAG_CAPTURE_ALL(wlc)    (CAPTURE_MGMT_CTL_FRAMES == (wlc)->pub->_evt_diag_capture)
#define DIAG_CAPTURE_MGMT(wlc)    (CAPTURE_MGMT_FRAMES == (wlc)->pub->_evt_diag_capture)
#else /* WL_DIAG_CAPTURE_TXRX_FRAMES */
#define wlc_diag_mgmt_ctl(a, b, c, d, e, f)   do {} while (0)
...
typedef struct {
    ...
    union {
        ...
        } taf;                /**< TAF private use when active */
#endif /* WLTAF */
        uint8   not_used_yet;        /**< Unused if TAF is not active */
    };
    uint8        flags4;        /**< Unused */
...
typedef struct {
    ...
    uint32        req_trig_us; /* requsted trigger us */
...
#define WLF_CSI_NULL_PKT    0x00010000    /**< CSI monitor  */
#define WLF_WME_NOACK        0x00040000    /**< pkt use WME No ACK policy */
#define WLF_UTXD        0x00080000    /**< UTXD pkt id for rev129 onwards */
...
#endif /* WLOLPC */
#define WLF2_PCB1_MFP_SA    12    /**< wlc_mfp_sa_query_cb */
...
#define WLF2_PCB2_CSA        3    /**<  wlc_csa_nulldata_complete */
...
#if defined(MFP)
/* flag for .11w Protected Management Frames(MFP) */
#define WLPKTFLAG_MFP(pkttag)    ((pkttag)->flags & WLF_MFP)
#else
#define WLPKTFLAG_MFP(pkttag)    FALSE
#endif /* MFP */
 ...
 */
#if defined(BCMDBG)
#define WLPKTTAGSCBGET(p)    (wlc_scb_flowid_global_lookup(WLPKTTAGSCBFLOWIDGET(p), CALL_SITE))
#else
...
#define AIRIQ_ENAB(pub) ((pub)->_airiq)
#else
#define AIRIQ_ENAB(pub) (0)
#endif

#ifdef BCM_CSIMON_AP
#define CSIMON_ENAB_AP(pub) ((pub)->_csimon_ap)
#else
#define CSIMON_ENAB_AP(pub) (0)
#endif /* BCM_CSIMON */
...
#define HE_DLOMU_ENAB(pub)        (HE_ENAB(pub) && WLC_HE_FEATURES_DLOMU(pub))
#define HE_ULOMU_ENAB(pub)        (HE_ENAB(pub) && WLC_HE_FEATURES_ULOMU(pub))
...
#define HE_DLOMU_ENAB(pub)        (FALSE)
#define HE_ULOMU_ENAB(pub)        (FALSE)
...
#define AMPDU_MAC_ENAB(pub) (AMPDU_AQM_ENAB(pub))
#define AMPDU_HOST_ENAB(pub) (!AMPDU_AQM_ENAB(pub))
...
#define WLC_INVALID_BSS_IDX       0xFF
...
/* Software UC MFP support */
#ifdef MFP
#define SW_MFP_ENAB(pub)    ((pub)->_uc_mgmt_sw_encr)
#else
#define SW_MFP_ENAB(pub)    (FALSE)
#endif /* MFP */
...
extern void wlc_intr_process_rxfifo_interrupts(wlc_info_t *wlc, bool m2m_intr);
...
#ifdef CONFIG_TENDA_PRIVATE_WLAN
extern int wlc_get_cnts_td(wlc_info_t *wlc, void *buf, int buflen);
#endif /* CONFIG_TENDA_PRIVATE_WLAN */
#endif /* WLCNT */
...
#if defined(CONFIG_WL) || defined(CONFIG_WL_MODULE) || defined(WL_USE_STB_THREAD)
#define WL_STB_THREAD()    TRUE
#else
#define WL_STB_THREAD()    FALSE
#endif /* defined(CONFIG_WL) || defined(CONFIG_WL_MODULE) */
...
#ifdef TXRX_SUMMARY
/* Macros to update the named counter, provided the structure pointer if non-NULL */
#define SCB_TXRX_SUMMARY_COND_FIND(counters, wlc, scb) \
    do { (counters) = wlc_txrx_summary_scb_counters(wlc, scb); } while (0)
#define SCB_TXRX_SUMMARY_COND_INCR(counters, counter_name) \
    do { if (counters) { (counters)->counter_name += 1; } } while (0)
#define SCB_TXRX_SUMMARY_COND_ADD(counters, counter_name, delta) \
    do { if (counters) { (counters)->counter_name += (delta); } } while (0)

#define IS_FT_MUMIMO(ampdu_ft) ((ampdu_ft == AMPDU_PPDU_FT_VHTMU) || \
                (ampdu_ft == AMPDU_PPDU_FT_HEMMU) || \
                (ampdu_ft == AMPDU_PPDU_FT_HEMOM) || \
                (ampdu_ft == AMPDU_PPDU_FT_HEMOM))
#define SCB_TXRX_SUMMARY_MUTX_COND_ADD(counters, ampdu_ft, delta) \
    if (ampdu_ft != AMPDU_PPDU_FT_SU) { \
        if ((counters) && (IS_FT_MUMIMO((ampdu_ft)))) { \
            SCB_TXRX_SUMMARY_COND_ADD(counters, tx.mpdu_mumimo, delta); \
        } else { \
            SCB_TXRX_SUMMARY_COND_ADD(counters, tx.mpdu_ofdma, delta); \
        } \
    } else { \
        SCB_TXRX_SUMMARY_COND_ADD(counters, tx.mpdu_su, delta); \
    }
#define RUTYPE_TO_RUCNT(index)    (index == MAC_LOG_MU_RU_26) ? 26: \
                (index == MAC_LOG_MU_RU_52) ? 52: \
                (index == MAC_LOG_MU_RU_106) ? 242: \
                (index == MAC_LOG_MU_RU_242) ? 242: \
                (index == MAC_LOG_MU_RU_484) ? 484: \
                (index == MAC_LOG_MU_RU_996) ? 996: \
                (index == MAC_LOG_MU_RU_2x996) ? (2 * 996): 0

#else /* TXRX_SUMMARY */
#define SCB_TXRX_SUMMARY_COND_FIND(c, w, s)    /* no SCB_TXRX_SUMMARY support */
#define SCB_TXRX_SUMMARY_COND_INCR(c, n)    /* no SCB_TXRX_SUMMARY support */
#define SCB_TXRX_SUMMARY_COND_ADD(c, n, a)    /* no SCB_TXRX_SUMMARY support */
#define SCB_TXRX_SUMMARY_MUTX_COND_ADD(c, ft, a) /* no SCB_TXRX_SUMMARY support */
#define RUTYPE_TO_RUCNT(index)            /* no SCB_TXRX_SUMMARY support */
#endif /* TXRX_SUMMARY */
...
#elif defined(STS_XFER_TXS_DISABLED)
    #define STS_XFER_TXS_ENAB(pub)        (0)
#else
    #define STS_XFER_TXS_ENAB(pub)        (1)
#endif
#else /* ! STS_XFER_TXS */
    #define STS_XFER_TXS_ENAB(pub)        (0)
#endif /* ! STS_XFER_TXS */

#define PHYRXS_MODE_OFF            (0)
#define PHYRXS_MODE_FIFO        (1)
#define PHYRXS_MODE_M2M            (2)

#if defined(STS_XFER_PHYRXS)
#if defined(ROM_ENAB_RUNTIME_CHECK) || !defined(DONGLEBUILD)
    #define STS_XFER_PHYRXS_ENAB(pub)    ((pub)->_sts_xfer_phyrxs)
#elif defined(STS_XFER_PHYRXS_DISABLED)
    #define STS_XFER_PHYRXS_ENAB(pub)    (0)
#else
    #define STS_XFER_PHYRXS_ENAB(pub)    (1)
#endif
...
#if defined(ROM_ENAB_RUNTIME_CHECK) || !defined(DONGLEBUILD)
    #define STS_XFER_PHYRXS_FIFO_ENAB(pub)    ((pub)->_sts_xfer_phyrxs == PHYRXS_MODE_FIFO)
#elif defined(STS_XFER_PHYRXS_DISABLED)
    #define STS_XFER_PHYRXS_FIFO_ENAB(pub)    (0)
#else
    #define STS_XFER_PHYRXS_FIFO_ENAB(pub)    (1)
#endif
#else /* ! STS_XFER_PHYRXS_FIFO */
    #define STS_XFER_PHYRXS_FIFO_ENAB(pub)    (0)
#endif /* ! STS_XFER_PHYRXS_FIFO */

#if defined(STS_XFER_PHYRXS_M2M)
#if defined(ROM_ENAB_RUNTIME_CHECK) || !defined(DONGLEBUILD)
    #define STS_XFER_PHYRXS_M2M_ENAB(pub)    ((pub)->_sts_xfer_phyrxs == PHYRXS_MODE_M2M)
#elif defined(STS_XFER_PHYRXS_DISABLED)
    #define STS_XFER_PHYRXS_M2M_ENAB(pub)    (0)
#else
    #define STS_XFER_PHYRXS_M2M_ENAB(pub)    (1)
#endif
...
#ifdef WL_IAPP
#define IAPP_ENAB(pub)            ((pub)->_iapp)
#else
#define IAPP_ENAB(pub)            (0)
#endif /* WL_IAPP */

#ifdef WL_MBSSID

#define MBSSID_ALLOWED(pub)    \
    (D11REV_GT(pub->corerev, 129) || \
    (D11REV_IS(pub->corerev, 129) && D11MINORREV_GT(pub->corerev_minor, 1)))
#define MBSSID_ENAB(pub, band) ((pub->_mbssid_enab == WLC_MBSSID_ENAB_AUTO)? \
        (BAND_6G(band)? MBSSID_ALLOWED(pub): 0) : pub->_mbssid_enab)
#else
#define MBSSID_ENAB(pub, band) 0
#endif /* WL_MBSSID */
/* Max key exchange duration, 3 seconds */

#define WL_MAX_KEY_DUR    3
#endif /* _wlc_pub_h_ */
