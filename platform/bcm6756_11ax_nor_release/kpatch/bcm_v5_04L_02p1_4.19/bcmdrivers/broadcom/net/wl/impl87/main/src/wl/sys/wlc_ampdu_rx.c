 ...
 *
 * $Id: wlc_ampdu_rx.c 811011 2022-04-21 16:32:25Z $
 */
...
static bool wlc_ampdu_block_acked_frame(wlc_info_t *wlc, scb_t *scb, d11rxhdr_t *rxh, uint8 *plcp);
    ...
    if (AMPDU_HOST_REORDER_ENAB(wlc->pub) && resp->queued) {
        if (BCMPCIEDEV_ENAB()) {
            ...
            if (p != NULL) {
                ...
                WL_AMPDU_ERR_EX(scb, ("unable to allocate packet for"
                    " reordering\n"));
            }
        }
    }
        ...
        if (AMPDU_IS_PKT_PENDING(wlc, resp, indx)) {
            ...
            } else
            {
                ...
                if (resp->exp_seq != pkt_h_seqnum(wlc, p)) {
                    WL_AMPDU_ERR_EX(scb, ("sequence number mismatched\n"));
                ...
                WL_AMPDU_RX_EX(scb, ("released seq 0x%x\n", resp->exp_seq));
...
BCMATTACHFN(wlc_ampdu_rxcfg_init)(wlc_info_t *wlc, ampdu_rx_config_t *ampdu_rx_cfg)
{
    ...
    if (D11REV_IS(wlc->pub->corerev, 61)) {
        ampdu_rx_cfg->mpdu_density = AMPDU_DENSITY_16_US;
    } else {
        ...
        for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
            ...
            if (resp->alive) {
                ...
                if (resp->dead_cnt >= lim) {
                    WL_AMPDU_ERR_EX(scb, ("tid %d cleaning up "
        ...
        for (tid = 0; tid < AMPDU_MAX_SCB_TID; tid++) {
            ...
            if (resp_off->ampdu_recd) {
                ...
                if (resp_off->ampdu_cnt >= AMPDU_RESP_NO_BAPOLICY_TIMEOUT) {
                    WL_AMPDU_ERR_EX(scb, ("ampdus recd for"
    ...
    bool is_first_mpdu)
{
    ...
    if (is_ampdu && !wlc_ampdu_block_acked_frame(wlc, scb, rxh, plcp)) {
        is_ampdu = FALSE;
    ...
    SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, rx.bytes, agglen);
    ...
    if (txrx_summary_cnt) {
        uint rxnss;
        SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, rx.bw, wlc_ratespec_bw(rspec));
        SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, rx.phyrate,    wf_rspec_to_rate(rspec));

        rxnss = wlc_ratespec_nss(rspec);
        if (rxnss && (rxnss <= MAX_NSS)) {
        ...
        if ((D11_FT(ft_fmt) == FT_HE) && (D11_HEFMT(ft_fmt) == HE_FMT_HETB)) {
            uint16 ruidx = ((plcp[6] & 0x0f) << 3) | ((plcp[7] & 0x1C) >> 2);
            uint32 ru_type = wf_he_ruidx_to_ru_type(ruidx);
            SCB_TXRX_SUMMARY_COND_INCR(txrx_summary_cnt, rx.mpdu_ofdma);
            SCB_TXRX_SUMMARY_COND_ADD(txrx_summary_cnt, rx.tones,
                wf_he_ru_type_to_tones(ru_type));
        } else if ((D11_FT(ft_fmt) == FT_HE) && (D11_HEFMT(ft_fmt) == HE_FMT_HEMU)) {
            SCB_TXRX_SUMMARY_COND_INCR(txrx_summary_cnt, rx.mpdu_mumimo);
        } else
#endif /* WL11AX */
        {
            SCB_TXRX_SUMMARY_COND_INCR(txrx_summary_cnt, rx.mpdu_su);
        }
    }
#endif /* TXRX_SUMMARY */
#endif /* PKTQ_LOG */
    ...
    if (HE_ULOMU_ENAB(wlc->pub) && !SCB_ULOFDMA_ADM(scb)) {
    ...
    if (D11_FT(ft_fmt) == FT_HE) {
        if (D11_HEFMT(ft_fmt) == HE_FMT_HETB) {
            ...
            uint16 ruidx = ((plcp[6] & 0x0f) << 3) | ((plcp[7] & 0x1C) >> 2);
            uint32 ru_type = wf_he_ruidx_to_ru_type(ruidx);
            if (ru_type < ARRAYSIZE(ampdu_rx->amdbg->rxrutype)) {
                ampdu_rx->amdbg->rxrutype[ru_type]++;
            }
            if (SCB_ULMMU_ADM(scb)) {
...
wlc_ampdu_recvdata(ampdu_rx_info_t *ampdu_rx, struct scb *scb, struct wlc_frminfo *f)
{
    ...
    if (!(f->fc & FC_WEP) && (f->WPA_auth != WPA_AUTH_DISABLED) &&
        ...
        if (key_info.algo != CRYPTO_ALGO_OFF) {
            WL_AMPDU_ERR_EX(scb, ("tid %d fc=0x%x FC_WEP bit is not "
    ...
    resp = scb_ampdu->resp[tid];
    block_acked_frame = wlc_ampdu_block_acked_frame(wlc, scb, f->rxh, plcp);
    ...
    if ((resp == NULL) ||
        ...
        if (block_acked_frame) {
            ...
            WL_AMPDU_RX_EX(scb, ("tid %d non-blockacked frame"
                " received: seq  0x%x, exp seq 0x%x :: P %p FC %02x\n",
                tid, seq, resp? resp->exp_seq : 0, f->p, f->fc));
        }
        wlc_recvdata_ordered(wlc, scb, f);
        return;
    }
    ...
    if ((f->seq & FRAGNUM_MASK) || (f->fc & FC_MOREFRAG)) {
        WL_AMPDU_RX_EX(scb, ("tid %d unexp frag in AMPDU SCB :"
            "seq  0x%x, exp seq 0x%x :: P %p FC %02x\n",
            tid, f->seq, resp->exp_seq, f->p, f->fc));
    ...
    WL_AMPDU_RX_EX(scb, ("tid %d receiving seq 0x%x exp seq %x indx %d\n",
        tid, seq, resp->exp_seq, indx));
    ...
    if (((offset < resp->ba_wsize) && AMPDU_IS_PKT_PENDING(wlc, resp, indx)) ||
        ...
        WL_AMPDU_RX_EX(scb, ("tid %d duplicate seq 0x%x(dropped),"
    ...
    if (offset >= resp->ba_wsize) {
        ...
        WL_AMPDU_RX_EX(scb, ("tid %d out of window pkt with"
            " seq 0x%x delta %d (exp seq 0x%x): moving window fwd\n",
            tid, seq, delta, resp->exp_seq));
        ...
        if (resp != NULL) {
            ...
            WL_AMPDU_RX_EX(scb, ("tid %d goto rxoow receiving seq 0x%x"
                ", exp seq 0x%x indx %d\n",
                tid, seq, resp->exp_seq, indx));
        } else {
            PKTFREE(wlc->osh, f->p, FALSE);
    ...
    if (seq == resp->exp_seq) {
        ...
        WL_AMPDU_RX_EX(scb, ("tid %d Releasing pending %d packets\n",
            tid, resp->queued));
    ...
    WL_AMPDU_RX_EX(scb, ("tid %d q out of order seq 0x%x(exp 0x%x)\n",
        tid, seq, resp->exp_seq));
...
#endif
    WL_AMPDU_CTL_EX(scb, ("tid %d BA ON: seq 0x%x"
        " wsize %d\n", tid, start_seq, wsize));
    ...
    uint16 initiator, uint16 reason)
{
    ...
    if (category & DOT11_ACTION_CAT_ERR_MASK) {
        WL_AMPDU_ERR_EX(scb, ("unexp error action frame\n"));
    ...
    WL_AMPDU_EX(scb, ("AMPDU OFF: tid %d initiator %d reason %d\n",
...
wlc_ampdu_recv_bar(ampdu_rx_info_t *ampdu_rx, struct scb *scb, uint8 *body, int body_len)
{
    ...
    if (tmp & DOT11_BA_CTL_MTID) {
        WL_AMPDU_CTL_EX(scb, ("multi tid not supported\n"));
        WLCNTINCR(ampdu_rx->cnt->rxunexp);
        return;
    }
    ...
    if (resp == NULL) {
        WL_AMPDU_CTL_EX(scb, ("uninitialized tid %d\n", tid));
        WLCNTINCR(ampdu_rx->cnt->rxunexp);
        return;
    }
    ...
    WL_AMPDU_CTL_EX(scb, ("length %d tid %d seq 0x%x\n",
        body_len, tid, seq));
    ...
    if (offset > (SEQNUM_MAX >> 1)) {
        WL_AMPDU_CTL_EX(scb, ("ignore bar with offset 0x%x\n", offset));
        return;
    }
    ...
    uint8 token, uint16 timeout, uint16 param_set)
{
    ...
    WL_AMPDU_CTL_EX(scb, ("status %d param_set 0x%x\n",
        status, param_set));
...
wlc_ampdu_free_flow_id(ampdu_rx_info_t *ampdu_rx, scb_ampdu_tid_resp_t *resp, struct scb *scb)
{
    ...
    if (!AMPDU_HOST_REORDER_ENAB(wlc->pub)) {
        WL_AMPDU_ERR_EX(scb, ("ERROR: AMPDU Host reordering not"
                "enabled, so shouldn't be here\n"));
        ASSERT(0);
        return -1;
    }
 ...
 */
static bool
wlc_ampdu_block_acked_frame(wlc_info_t *wlc, scb_t *scb, d11rxhdr_t *rxh, uint8 *plcp)
{
    ...
    WL_AMPDU_RX_EX(scb, ("UCRxstatus 2 %x Frame Type %x  AMPDU PLCP(HT) %x "
        "smpdu %d  ampdu %x \n",
        RxStatus2, phy_ftfmt,
...
wlc_ampdu_ulmu_reqbytes_upd(ampdu_rx_info_t *ampdu_rx, scb_t *scb)
{
    ...
    WL_AMPDU_RX_EX(scb, ("recv_bytes %d\n", scb_ampdu->ema.recv_bytes));
    wlc_ulmu_evaluate_admission(ampdu_rx->wlc->ulmu, scb);
    if (SCB_ULOFDMA_ADM(scb)) {
        WL_AMPDU_RX_EX(scb, ("admitting\n"));
    }
}
