...
static DEFINE_SPINLOCK(td_sf2_stats);
#endif
...
static int port_sf2mac_cfg_set(mac_dev_t *mac_dev, mac_cfg_t *mac_cfg)
{
    ...
    v8 |= ((mac_dev->mac_id == IMP_PORT_ID) ? REG_CONTROL_MPSO_MII_SW_OVERRIDE : REG_PORT_STATE_OVERRIDE) | 
...
#ifdef CONFIG_TENDA_PRIVATE_KM
    memcpy(&mac_dev->stats, mac_stats, sizeof(*mac_stats));    
    spin_unlock_bh(&td_sf2_stats);
#endif
    return 0;
}
static int port_sf2mac_stats_clear(mac_dev_t *mac_dev)
