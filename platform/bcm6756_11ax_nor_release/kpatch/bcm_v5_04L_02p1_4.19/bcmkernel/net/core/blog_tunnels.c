...
/*为保证开gre加速vpn正常穿透，需要传入参数update_flag来判断是否更新序列号、确认号，bcm_pptp_xmit_update函数加速代码会调用，
加速里面已经编译成二进制了，若修改了该函数实现，加速那边同步不了，故重新定义一个函数*/
static int tenda_pptp_xmit_update(uint16_t call_id, uint32_t* seqNum, uint32_t* ackNum, uint32_t daddr, uint32_t* update_flag)
{
    struct pppox_sock *sock;
    struct pptp_opt *opt;
    int i, ack_flag = PPTP_NOT_ACK;
...
void blog_pptp_xmit(struct sk_buff *skb_p, uint32_t h_proto) 
{
    if (skb_p && blog_gre_tunnel_accelerated())
    {
            ...
            && ip_p->proto == BLOG_IPPROTO_GRE )
        {
            ...
            /* the pkt is PPTP with seq number */
            if (
#ifndef CONFIG_TENDA_PRIVATE_KM
                (blog_pptp_xmit_update_fn != NULL) &&
#endif
                gre_flags.seqIe && gre_flags.keyIe && gre_flags.ver) 
            {    
                call_id = ntohs(*(uint16_t*) (grehdr_p + 3));
...
#else
                tenda_pptp_xmit_update(call_id, &seqNum, &ackNum, daddr, &update_flag);

                if(1 == update_flag)   //若是拨往本地pptp的包则要更新序列号、确认号，穿透包不用
                {
                    *(uint32_t*) (grehdr_p + 4) = htonl(seqNum);
                }

                if(gre_flags.ackIe && (1 == update_flag)) /* the pkt is PPTP with ack number */
                {    
                    *(uint32_t*) (grehdr_p + 6) = htonl(ackNum);
                }
#endif
