...
static int blog_skip_fastpath(struct sk_buff *skb)
{
    /* unknown for user define. */
    blog_skip(skb, blog_skip_reason_unknown);
...
#ifdef CONFIG_TENDA_PRIVATE_KM
static int blog_need_slow_learning(struct sk_buff *skb)
{
    /* blog_p非空才能学习，且该方案需要数据包走协议栈才能学习到完整转发信息。 */
    return !!skb->blog_p;
}
#endif
...
#ifdef CONFIG_SKIP_FASTPATH
    km_skip_fastpath_hook = blog_skip_fastpath;
#endif
#ifdef CONFIG_TENDA_PRIVATE_KM
    km_fastpath_need_slow_learning = blog_need_slow_learning;
#endif
