...
static void sysrq_handle_uart_monitor(int key)
{
    g_uart_monitor ^= 1;
}
static struct sysrq_key_op sysrq_uart_monitor_op = {
    .handler    = sysrq_handle_uart_monitor,
    .help_msg    = "uart-monitor(g)",
    .action_msg    = "Uart Monitor On/Off",
};
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_KM
/* 发送消息给sysrq，杀掉一个用户进程，并打印出那个进程的名字 */
static void send_sig_one(int sig)
{
    struct task_struct *p;
...
static void sysrq_handle_kill_one(int key)
{
    send_sig_one(SIGKILL);
    console_loglevel = CONSOLE_LOGLEVEL_DEBUG;
}
static struct sysrq_key_op sysrq_kill_one_op = {
    .handler    = sysrq_handle_kill_one,
    .help_msg    = "kill-one-task(v)",
    .action_msg    = "Kill one Task",
    .enable_mask    = SYSRQ_ENABLE_SIGNAL,
};
#endif
...
#ifdef CONFIG_TENDA_PRIVATE_KM
    &sysrq_kill_one_op,                /* v 增加杀掉单个进程的版本，将进程信息打印出来 */
#else
    NULL,                /* v */
#endif
