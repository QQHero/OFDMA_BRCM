                ...
                int pnum)
{
    ...
    ubi_warn(ubi, "PEB %d contains corrupted VID header, and the data does not contain all 0xFF",
        pnum);
    ubi_warn(ubi, "this may be a non-UBI PEB or a severe VID header corruption which requires manual inspection");
#else
...
#if defined(CONFIG_TENDA_PRIVATE_BSP)
        ubi_warn(ubi, "%d PEBs are corrupted and preserved",
            ai->corr_peb_count);
#else
