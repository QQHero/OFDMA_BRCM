...
#if defined(CONFIG_SA_FASTPATH_NAT)
...
#ifdef CONFIG_TENDA_PRIVATE_KM
        km___copy_skb_header_hook(new_skb, skb);
        nf_conntrack_get(skb_nfct(skb));
        new_skb->_nfct = skb->_nfct;
#endif
        consume_skb(skb);
        skb = new_skb;
        skb_put(skb, len);
    ...
    if (npi < 0) {
        ...
        if ((ppp->dev->flags & IFF_UP) == 0 ||
            ...
            skb_scrub_packet(skb, !net_eq(ppp->ppp_net,
                              dev_net(ppp->dev)));
#if defined(CONFIG_SA_FASTPATH_NAT)
            if (ctf_forward(skb, skb->dev) != CTFE_ERROR) {
                return ;
            }
#endif
            netif_rx(skb);
...
#ifdef CONFIG_TENDA_PRIVATE_KM
        km___copy_skb_header_hook(ns, skb);
#endif
...
#if defined(CONFIG_SA_FASTPATH_NAT)
void ppp_rxstats_upd(void *pppif, struct sk_buff *skb)
{
    struct ppp *ppp;
...
void ppp_txstats_upd(void *pppif, struct sk_buff *skb)
{
    struct ppp *ppp;
    if(pppif == NULL || skb == NULL)
        return ;
...
int ppp_get_conn_pkt_info(void *pppif, struct ctf_ppp *ctfppp){
    ...
    if (sk) {
        ...
        case PX_PROTO_OE:
            if (ppp->vj && (ppp->flags & SC_COMP_TCP)) {
                ctfppp->psk.pppox_protocol = 0x11;
                break;
            }
            if ((ppp->xstate & SC_COMP_RUN) && ppp->xc_state) {
                ctfppp->psk.pppox_protocol = 0x11;
        ...
        case PX_PROTO_PPTP:
            ...
            if (ppp->vj && (ppp->flags & SC_COMP_TCP)) {
                ctfppp->psk.pppox_protocol = 0x11;  /* ?ó?üê±ó?óú′ó???-?ó?ú·￠3?￡???í? */
                break;
            }
            if ((ppp->xstate & SC_COMP_RUN) && ppp->xc_state) {
                ctfppp->psk.pppox_protocol = 0x11;
