              ...
              int length)
{
    ...
    int offset;
    u32 ns, nr;
    /* 如果是经过ipsec加密过的数据包 则将session会话中authentication_type置为L-
       2TP_OVER_IPSEC fastnat将不进行转发该数据包 */
#ifdef CONFIG_SA_FASTPATH
    if (CTF_TEST_BIT(skb, CTF_IPSEC)) {
        session->authentication_type = CTF_L2TP_OVER_IPSEC;
    }
#endif
...
#ifdef CONFIG_SA_FASTPATH_NAT_L2TP
    ctf_session_delete_ipc(session);
#endif
