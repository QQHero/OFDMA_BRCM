...
/* 暂时使用wifi2宏来控制一些与主线存在差异的地方，后续删除 */
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && defined(CONFIG_TENDA_PRIVATE_WIFI2)
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && defined(CONFIG_TENDA_PRIVATE_WIFI2)
    if (!err) {
        struct wm_station_info wm_sta = {(char *)mac_addr, 0, 1};
        wm_add_station_hook_func(dev->ifindex, &wm_sta);
    }
#endif
    wdev_unlock(dev->ieee80211_ptr);
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && defined(CONFIG_TENDA_PRIVATE_WIFI2)
    {
        struct wm_if_info wm_if = {wdev, wdev->preset_chandef.chan ? wdev->preset_chandef.chan->band : -1, params.crypto.n_ciphers_pairwise,
                                    (char *)params.ssid, params.ssid_len};
        wm_update_wifi_if_info_func(dev->ifindex, &wm_if);
    }
#endif
    wdev_unlock(wdev);
...
static int nl80211_del_station(struct sk_buff *skb, struct genl_info *info)
{
    ...
    if (!ret) {
        wm_del_station_hook_func(dev->ifindex, (char *)params.mac);
    }
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && defined(CONFIG_TENDA_PRIVATE_WIFI2)
    if (!err) {
        int band = connect.channel ? connect.channel->band : -1;
        struct wm_if_info wm_if = {dev->ieee80211_ptr, band, connect.crypto.n_ciphers_pairwise, (char *)connect.ssid, connect.ssid_len};
        wm_update_wifi_if_info_func(dev->ifindex, &wm_if);
    }
#endif
    wdev_unlock(dev->ieee80211_ptr);
                 ...
                 gfp_t gfp)
{
    ...
    if (cr->status == WLAN_STATUS_SUCCESS) {
        struct wm_station_info wm_sta = {(char *)cr->bssid, 0, 0};
        wm_add_station_hook_func(netdev->ifindex, &wm_sta);
    }
#endif
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && defined(CONFIG_TENDA_PRIVATE_WIFI2)
    wm_del_station_hook_func(netdev->ifindex, NULL);
#endif
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && defined(CONFIG_TENDA_PRIVATE_WIFI2)
    {
        struct wm_station_info wm_sta = {(char *)mac_addr, 0, 0};
        wm_add_station_hook_func(dev->ifindex, &wm_sta);
    }
#endif
...
#if defined(CONFIG_TENDA_PRIVATE_WLAN) && defined(CONFIG_TENDA_PRIVATE_WIFI2)
    wm_del_station_hook_func(dev->ifindex, (char *)mac_addr);
#endif
