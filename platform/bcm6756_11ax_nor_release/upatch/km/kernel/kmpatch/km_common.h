...
#define ctf_ipc_v6_add(skb, hooknum, manip) do {    \
    if (CTF_ENAB_V6(kcih)) {        \
        (kcih)->ipc_v6_add(kcih, skb, hooknum, manip); \
    } } while (0)
#else
...
#ifdef CONFIG_SA_FASTPATH_IPV6
#ifdef CONFIG_TD_IPV6_V2
typedef void (*ctf_ipc_v6_add_t)(ctf_t *ci, struct sk_buff *skb, u_int32_t hooknum, struct nf_conntrack_tuple *manip);
#else
...
struct ctf_ipc_v6 {
    ...
    u_int8_t dir;
#ifdef CONFIG_TD_IPV6_V2
    struct ctf_nat_v6 nat;        /* Manip data for SNAT, DNAT */
#endif
    void *ppp_ifp;    /* PPP interface handle */
    u_int16_t pppoe_sid[0];    /* PPPOE session to use, sid[0]表示单侧pppoe下的ssid??                                      ssid[1]表示双侧pppoe时的接收报文ssid */
};
...
extern int32_t (*km_audit_show_user_info_hook)(struct seq_file *s, void *v);
extern int32_t (*km_audit_clear_audit_info)(void *online_ip);
#endif
...
#ifdef CONFIG_KM_AUDIT
/* 上网审计上报事件 */
extern void (*km_eventscenter_kmev_audit_handler)(void *obj);
#endif
